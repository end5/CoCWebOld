...
import { campFollowers, returnToCampUseOneHour } from "../Camp";
...
export const ValeriaFlags = {
    VELARIA_FUTA: 0,
    TIMES_VALERIA_GOO_THREESOMED: 0,
};
...
export function valeriaFollower(player: Character): NextScreenChoices {
    ...
    return {
        choices: [
            ["Appearance", valeriaAppearance],
            ["Spar", valeriaSpar],
            ["Sex", sex],
            ["Talk", talkWithValeria],
            ["Take", takeValeria],
            ["", undefined],
            ["", undefined],
            ["", undefined],
...
// Valeria] -- [Spar]
function valeriaSpar(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.Valeria); // 79;
    CView.text("You ask Valeria if she would be up for a bit of battle practice.");
    ...
    const monster = new GooArmor();
    monster.effects.add(StatusEffectType.Spar, 0, 0, 0, 0);
    monster.inventory.gems = 0;
    return CombatManager.beginBattle(player, monster);
}

// [Valeria] -- [Spar] -- PC Victorious
function pcWinsValeriaSpar(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.Valeria); // 79;
    CView.text("\"<i>Oof!</i>\" Valeria grunts, nearly losing her human shape as she tumbles to the hard-packed dirt.  \"<i>All right, all right, you win!  Take it easy, partner,</i>\" she laughs, letting her greatsword dissipate back into her body.");
...
export function pcWinsValeriaSparDefeat(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.Valeria); // 79;
    CView.text("You collapse, ");
    if (player.stats.HP < 1) CView.text("in too much pain");
    else CView.text("too turned on");
    CView.text(" to fight any longer.  Valeria's on you in a heartbeat, her gooey greatsword pressed to your throat.");
    ...
    if (player.stats.HP < 1 || player.gender === 0) {
        CView.text(", offering you a hand up.");
        CView.text("\n\n\"<i>C'mon, let's get back to </i>\"");
        // (Return to main Camp menu)
        return { next: returnToCampUseOneHour };
    }
    // [else; If PC lost via lust & has a gender:
    else {
        CView.text(".  \"<i>Well, since you're so... eager... I might as well get my daily fluids while we're here.</i>\"");
        // (Go to Valeria's gender-appropriate FemDom sex scenes)
        return { next: valeriaSexDominated };
    }
}
...
function followersValeriaSex(player: Character, display: boolean): NextScreenChoices {
    ...
    if (display) {
        CView.clear();
        CView.text("With a lusty grin, you ask your gooey friend if ");
        if (player.gender > 0) CView.text("she's interested in a little fluid exchange");
        else CView.text("she wouldn't mind helping you get some release, despite your genderlessness");
        CView.text(".");
    ...
    if (ValeriaFlags.VELARIA_FUTA === 1) {
        dickText = "Lose Dick";
    }
    return {
        choices: [
            ...
            ["", undefined],
            ["", undefined],
            ["Back", valeriaFollower],
        ]
    };
}

// Valeria -- [Sex] -- [Dick/No Dick]
function valeriaDickToggle(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.Valeria); // 79;
    CView.clear();
    // [If Valeria has a dick:]
    if (ValeriaFlags.VELARIA_FUTA === 1) {
        CView.text("Before you do anything, you ask Valeria if she wouldn't mind hiding her facsimile of a dick, at least when you aren't having sex.");
        ...
        ValeriaFlags.VELARIA_FUTA = 0;
    }
    // [If Valeria doesn't have a dick:]
    else {
        ValeriaFlags.VELARIA_FUTA = 1;
        CView.text("Struggling to find a less-than-awkward way of phrasing this, you ask Valeria if she wouldn't mind growing a certain extra appendage for you.");
...
function valeriaGetFucked(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.Valeria); // 79;
    CView.clear();
    CView.text("You disrobe and, ");
    if (player.body.vaginas.length > 0) CView.text("stroking the slick lips of your [vagina], ask Valeria to fuck you.");
    else CView.text("posing seductively, ask Valeria to fuck you.");
    CView.text("\n\nShe grins.  \"<i>You just lie back and let me take care of everything, partner.</i>\"  You do as she asks, flopping onto your back ");
    if (player.body.legs.isBiped()) CView.text("and spreading your legs ");
    CView.text("as Valeria ");
    if (ValeriaFlags.VELARIA_FUTA === 0) CView.text("forms a thick, gooey cock");
    else CView.text("grabs her thick, gooey cock");
    CView.text(" and looms over you.");
    ...
    if (player.body.chest.sort(BreastRow.Largest)[0].rating > 3) CView.text("bumping over your [chest] and stopping for a brief moment to tickle your sensitive tits with her goopy bottom before ");
    CView.text("coming to a stop over your crotch.  She pours herself into your lap, ");
    // [if PC has cock: \"<i>
    if (player.body.cocks.length > 0) CView.text("her gooey ass devouring your " + describeCock(player, player.body.cocks.get(0)) + " as she slides down your belly");
    if (player.body.cocks.length > 0 && player.body.vaginas.length > 0) CView.text(" and ");
    if (player.body.vaginas.length > 0) CView.text(" letting bits of her goo-body slither into your wet, waiting [vagina], ever so slightly peeling your walls apart");
    // If PC has no gender
    if (player.gender === 0) CView.text("poking the shaft of her cock gingerly, teasingly against your [asshole]");
    CView.text(".");
    ...
    if (player.body.cocks.length > 0) CView.text(", as she rhythmically flexes her ass cheeks, shifting her fluidity around your shaft within her with a feeling like lotion-covered hands gently stroking you");
    CView.text(".");
    ...
    if (player.body.cocks.length <= 0 && player.body.vaginas.length > 0) {
        ...
        if (player.body.chest.find(BreastRow.FuckableNipples)) CView.text("s");
        CView.text(" deeper inside you than ever before.  You feel streaks of goo pouring into you as she cums, pouring her essence into your womb ");
        if (player.body.chest.find(BreastRow.FuckableNipples)) CView.text("and [chest] ");
        CView.text("until slime overflows from inside you, pooling under her hips.  Filled beyond your capacity, you scream your pleasure and cum, clamping down on Valeria's cock");
        if (player.body.chest.find(BreastRow.FuckableNipples)) CView.text("s");
        CView.text(" until they literally pop inside you, flowing back out in goopy streaks.");
    }
    // (If Male/Herm)
        ...
        if (player.body.vaginas.length > 0) CView.text("s");
        CView.text(" literally popping as she climaxes, filling your stretched ass ");
        if (player.body.vaginas.length > 0) CView.text("and vag ");
        CView.text("with warm, sticky goop.");
    }
    // (If PC is Genderless OR [Gooflation])
    else {
        return gooFlation(player, false);
    }
    CView.text("\n\nYou collapse, goop flowing freely from your abused body.  Laughing, Valeria pours out of your lap, ");
    if (player.gender > 0) CView.text("patting her belly full of your juices, ");
    CView.text("and looms over you.  \"<i>That was fun, partner,</i>\" she says, leaning down to give you a wet peck on the cheek. \"<i>Let's do that again soon, all right?</i>\"");
...
function gooFlation(player: Character, clearText: boolean): NextScreenChoices {
    ...
    if (clearText) {
        CView.clear();
        CView.text("\"<i>Just relax, partner,</i>\" she grins, reaching up to run a hand through your [hair].  \"<i>Just let ol' Valeria take care of everything...</i>\"");
    ...
    if (player.body.vaginas.length <= 0) CView.text("one lonely ");
    CView.text("hole.  You gulp as the tentacle-fuck begins, her long tendrils pouring one by one into your [asshole], each so small and soft it easily slips inside you.  One piles onto another, tickling and teasing your anal walls as more and more of her slithers into you.");
    ...
    if (clearText) {
        CView.text("\n\nYou collapse, goop flowing freely from your abused body.  Laughing, Valeria pours out of your lap, ");
        if (player.gender > 0) CView.text("patting her belly full of your juices, ");
        CView.text("and looms over you.  \"<i>That was fun, partner,</i>\" she says, leaning down to give you a wet peck on the cheek. \"<i>Let's do that again soon, alright?</i>\"");
        player.orgasm();
        player.stats.sens += 1;
...
function penetrateValeria(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.Valeria); // 79;
    CView.clear();
    CView.text("\"<i>Mmm, that's a tasty-looking cock,</i>\" Valeria says as you disrobe, letting your " + describeCock(player, player.body.cocks.get(0)) + " flop free.  \"<i>I just might have to get a sample,</i>\" she chuckles, squatting down before you and taking your shaft in hand.  Rather than giving you a simple handy, however, the goo-girl places her palm against the head of your cock and presses forward.");
    CView.text("\n\nYou gasp as her gooey skin parts, letting your " + describeCock(player, player.body.cocks.get(0)) + " slip inside her.  Her palm and forearms act like a snug, wet cocksleeve, shifting and molding to perfectly fit around your prick as you slide into her up to the hilt.  She grins up at you as she gives you a handy the likes of which only a goo-girl could manage, using her penetrable palm like a cunt.  You groan as she moves her arm, slowly stroking your cock inside of her, occasionally varying her motions, making wide circles around you or moving side to side.");
    CView.text("\n\nBefore you can get too comfortable, however, Valeria sidles forward and moves your " + describeCock(player, player.body.cocks.get(0)) + " from her arm to her breast with a wet POP.  She squeezes her palmable C-cups together and leans in, letting your cock slip in between them and into her gooey flesh.  You moan as the pleasurable wetness and warmth of her interior returns when the goo-girl begins to titty-fuck you, slipping her wet breasts along the length of your shaft as the head bobs in and out of her cunt-like interior.");
    CView.text("\n\nYou sink your fingers into the goo's shoulders and start to guide her speed, moving her back and forth along your cock at your own pace like a particularly lush onahole.  The normally-dominant Valeria makes a surprised, happy squeal as you seize control.  She redoubles her efforts of tit-fucking your " + describeCock(player, player.body.cocks.get(0)) + " as you use her as your own personal sex-toy, roughly ramming her back and forth on your shaft until you feel an orgasm bubbling up inside you.");
...
function valeriaSexDominated(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.Valeria); // 79;
    CView.clear();
    CView.text("Making a show of playing hesitant and nervous, you tell Valeria you'd like her to take charge.  A wide grin quickly spreads across her girlish features.  \"<i>Mmm, feeling subby today, partner?  Good, good... You just let Valeria take good care of you, " + mf(player, "handsome", "cutie") + ".</i>\"");
    ...
    // (PC has Vagina)
    if (player.body.vaginas.length > 0 && (player.body.cocks.length <= 0 || randInt(2) === 0)) {
        ...
        if (player.body.chest.sort(BreastRow.Largest)[0].rating > 1) {
            CView.text("  As she hammers your cunny, bits of her goo swirl around your [chest], squeezing and massaging your tits. You squirm as she roughly teases your boobs, pinching at your nipples and squeezing your tender flesh roughly.");
            if (player.lactationQ() > 0) CView.text("  To her delight, a spray of warm milk jets out of your sore nipples, milky white mixing into blue goo like oil in water.  \"<i>Mmm, tasty!</i>\" she teases, massaging more and more from you.");
        }
        ...
        else CView.text("little ");
        CView.text(mf(player, "boy", "girl") + " ready to cum?</i>\" she withdraws herself from you, leaving you feeling suddenly very, very empty.  \"<i>I didn't say you could cum yet, slut! I'm not done with you yet.</i>\"");
        ...
        if (player.vaginalCapacity() > 80 || player.analCapacity() > 80) CView.text(" and your own receptibility");
        CView.text(".  Hugging your back, the goo-girl grabs at your [chest], squeezing your tits and pinching at your nipples.  \"<i>Moan for me, bitch,</i>\" Valeria growls into your ear, giving your [nipples] a particularly violent tug.  You let out a lewd, whorish moan.  The goo-girl grins, her tongue lashing out to lick your ear as she rewards you with a single long thrust into your depths, filling and stretching your [vagina] and [asshole] completely before withdrawing.");
        ...
        if (player.lactationQ() > 0) CView.text("and milk ");
        CView.text("swirling around inside her.  \"<i>We'll do this again sometime,</i>\" she adds, walking off to another part of camp with a wink.");
    }
    // (PC has Dick)
    else {
        CView.text("\n\nValeria begins to use her goo to peel back your clothes, soon revealing your defenseless, half-erect  package.  She makes a show of licking her lips as tendrils of goo wrap tightly around [eachCock] like a warm, wet onahole. You submit to your gooey bonds, and seeing your lack of resistance, Valeria smiles down at you and squeezes your " + describeCocksLight(player) + " tighter.");
        CView.text("\n\nYou gasp with pleasure as she starts to stroke [eachCock], jerking you off as she looms over you, grinning wickedly.  \"<i>Oh, you like that do you?</i>\" the armor-goo asks.  \"<i>Well then, I might just let you get off, too... If you're a good " + mf(player, "boy", "girl") + "</i>\"  She starts to increase her tempo, making you squirm and writhe as she wanks your " + describeCocksLight(player) + ", licking her lips as little bubbles of pre start to form.");
        ...
        CView.text(" " + mf(player, "boy", "girl") + " ready to cum? Well, go on then - we've got time for more.</i>\"");
...
function talkWithValeria(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.Valeria); // 79;
    CView.clear();
    CView.text("You ask Valeria if she wouldn't mind just talking for a little bit.");
    ...
    if (describeRace(player) !== "human") CView.text(" were");
    CView.text(".  I just, I dunno, changed, I guess.</i>\"");
    CView.text("\n\nChanged?");
    ...
    if (player.gender > 0) return {
        choices: [
            ["Flirt", flirtWithValeria],
            ["Accept", acceptValeriasNeeds],
            ["Gross", declineValeriasNeeds],
        ]
    };
    else return {
        choices: [
...
// [Flirt]
function flirtWithValeria(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.Valeria); // 79;
    CView.clear();
    CView.text("You slip an arm around Valeria's slender shoulders and pull her close.  Patting your crotch, you tell her you're practically a gourmet chef for a girl like her.  She giggles and slips a hand easily into your [armor], ");
    if (player.body.cocks.length > 0) CView.text("brushing along the length of your rapidly-hardening cock");
    else CView.text("brushing a wet, warm knuckle across your [clit]");
    CView.text(".");
    ...
    // (Display Sex Options)
    return followersValeriaSex(player, false);
}

// [Accept]
function acceptValeriasNeeds(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.Valeria); // 79;
    CView.clear();
    CView.text("You smile and tell Valeria that you're perfectly all right with her special needs");
    if (player.effects.has(StatusEffectType.SlimeCraving)) CView.text(" after all, you have the same ones");
    CView.text(", as long as they don't interfere with your mission as Champion.");
...
// [Gross]
function declineValeriasNeeds(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.Valeria); // 79;
    CView.clear();
    CView.text("You grimace and push the goo-girl away.  You've got no interest in her corrupted 'needs,' especially with a look like that on her face.  She gasps as you push her, nearly falling over; she catches herself and glowers angrily.");
...
export function valeriaAndGooThreeStuff(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You cautiously approach with Valeria's voice egging you on, \"<i>We gonna fuck her? We're gonna fuck her, ain't we, [name]?</i>\" She affectionately fondles your ");
    const list = ["[hips]"];
    if (player.body.balls.count > 0) list.push("[balls]");
    if (player.body.cocks.length > 0) list.push("[multiCockDescriptLight]");
    if (player.body.vaginas.length > 0) list.push("[vagina]");
    list.push("[asshole]");
    list.push("[nipples]");
    ...
    CView.text("\n\nThe bluish");
    if (monster.body.skin.tone !== "blue") CView.text(" " + (monster as GooGirl).gooColor6());
    CView.text(" juices slowly meld together until you cannot tell one from the other, and they firm up into a semi-translucent, flat-bottomed sphere, trapping you there. Your debauched and hopelessly aroused state is visible to any who would wander by, stroked by dark-colored currents that wrap around your ");
    if (player.body.cocks.length > 0 || player.body.vaginas.length > 0) CView.text("genitals");
    else CView.text("[asshole]");
    CView.text(" with sinful tightness. An involuntary shiver of excitement traverses up your spine, though your body is held almost completely stock-still in its sloshing vice.");
    ...
    CView.text("\n\nYou ");
    if (ValeriaFlags.TIMES_VALERIA_GOO_THREESOMED === 0) CView.text("demand that she release you");
    else CView.text("playfully comment that she really ought to release you");
    CView.text(", but the troublesome, liquid armor shakes her head negatively.");
    ...
    return { next: valeriaGooRapeII };
}

function valeriaGooRapeII(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("The lump that bumped you earlier smacks off ");
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 3) CView.text("the valley of your cleavage");
    else CView.text("your chest");
    CView.text(" once more, and this time you catch a glimpse of it out of the corner of your eye. It's the goo-girl's core, clasped in a fist of deep-blue. It pulses faintly as it's held there, just before you, while subtle vibrations and temperature fluctuations alert you to some kind of secret communications between the two women. Just what are they planning?");
    CView.text("\n\nYou get your answer in the form of a sudden");
    if (ValeriaFlags.TIMES_VALERIA_GOO_THREESOMED === 0) CView.text(", unexpected");
    else CView.text(" but expected");
    CView.text(" pressure at your [asshole]. It's so... so slick that you have to wonder how they can manage to push with such authority at your rearmost orifice, but they manage it all the same. You clench to deny them entrance, squeezing your sphincter shut tighter than locked door. Valeria's giggling voice coos from somewhere below, \"<i>Oh, don't be a spoilsport. Come on, it'll be fun.</i>\"");
    ...
    if (!player.body.wombs.find(Womb.Pregnant) && player.body.vaginas.length > 0) {
        ...
        // {cuntChange: MAXIMUM}
        displayStretchVagina(player, player.vaginalCapacity() * .75, true, true, false);
    ...
    // Fuckable nipples
    if (player.body.chest.find(BreastRow.FuckableNipples) && player.body.chest.sort(BreastRow.Largest)[0].rating >= 5) {
        ...
        if (player.body.vaginas.length > 0) CView.text(" as your snatch");
        else CView.text(" as the randiest slut's snatch");
        CView.text(". Valeria takes this as an invitation, and " + numToCardinalText(player.body.chest.reduce(BreastRow.TotalNipples, 0)) + " phallic objects greedily push at your cunt-nipples, eagerly sliding themselves inside your welcome chest-pussies. Looking down, you can see the navy-blue outlines of the phallic tendrils waving and writhing as they push their way deep, embedding themselves tit-deep in blissful warmth.");
    }
    // Dick
    if (player.body.cocks.length > 0) {
        CView.text("\n\nValeria's voice calls out once more, \"<i>You didn't think I'd forget ");
        if (player.body.cocks.length === 1) CView.text("this little guy");
        else CView.text("these little guys");
        CView.text(", did you? ");
        if (player.body.cocks.sort(Cock.Largest)[0].area >= 50) CView.text("...Maybe big guy is a bit more appropriate. Whatever. ");
        CView.text("I was just saving the best for last. Hold on tight, tiger.</i>\"");
        CView.text("\n\nThe sleeve of squishy ecstasy surrounding [eachCock] tightens up a little to hold your twitching, ecstatic boner");
        if (player.body.cocks.length > 1) CView.text("s");
        CView.text(" in place, and something even warmer than the slippery container");
        if (player.body.cocks.length > 1) CView.text("s");
        CView.text(" pushes at your head");
        if (player.body.cocks.length > 1) CView.text("s");
        CView.text(", circling your urethra");
        if (player.body.cocks.length > 1) CView.text("s");
        CView.text(". You inadvertantly clench your muscles in a way that makes your " + describeCocksLight(player) + " so very swollen, in turn making it that much easier for the gelatinous fiend to have her way with your cumslit");
        if (player.body.cocks.length > 1) CView.text("s");
        CView.text(". You feel that warm, wet fluid slowly peel it open and push inside. It doesn't hurt at all, but it is a little weird having your " + describeCocksLight(player) + " slowly stuffed in such a way. The warmth travels the whole way down your dick");
        if (player.body.cocks.length > 1) CView.text("s");
        CView.text(" to your middle, where you can feel it travel a bit further before it finally stops at your [balls].");
    }
    // Merge
    CView.text("\n\nGiggling, the sapphire slut does something that makes the body-wrapping sphere jiggle and contract slightly. ");
    if (ValeriaFlags.TIMES_VALERIA_GOO_THREESOMED > 0) CView.text("You grin in eager anticipation, waiting to feel the flood of fluid that you know is coming. It won't take long.... You feel it in your butt first.");
    else CView.text("At first, you don't realize what it is. You're floating in a bit of a sexual daze by now and barely cogent of what's going on, but it does feel quite nice, whatever it is. The spreading heat in your posterior is what finally clues you in to what's happening.");
    CView.text(" Pumping deep into your bowels, the hardened protrusion is no longer simply stuffing you. It's pumping something slick and wet and oh so gooey straight into your [asshole], filling you up so strangely that you aren't sure what to make of beyond how... well, good it feels to be stuffed like this.");
    ...
    if (player.body.vaginas.length > 0) {
        ...
        if (player.body.vaginas.get(0).wetness <= 2) CView.text("leaking");
        else if (player.body.vaginas.get(0).wetness <= 3) CView.text("dripping");
        else CView.text("squirting");
        CView.text(" your girlcum in feverish releases of ecstasy, repeatedly getting off on this perverse, doubled slime-flation.");
    }
    // {titties}
    if (player.body.chest.find(BreastRow.FuckableNipples) && player.body.chest.sort(BreastRow.Largest)[0].rating >= 5) {
        ...
        if (player.body.chest.length > 1) CView.text("top row of breasts");
        else CView.text("breasts");
        CView.text(" pulling away from your body, gaining cup sizes in a span of seconds, your skin stretching taut under the moist deluge. Your tit-twats quiver with delight, feeling nothing but obscene, ever-growing pleasure until they're squeezing down on the sloppy tentacle-cocks, milking them to cum harder and faster, which in turn bloats your ecstatic tits into blissful, bloated orbs.");
    }
    // {cocks!}
    if (player.body.cocks.length > 0) {
        CView.text("\n\nThe sealed, cock-plugging tube");
        if (player.body.cocks.length > 1) CView.text("s");
        CView.text(" in [eachCock] ");
        if (player.body.cocks.length > 1) CView.text("do not");
        else CView.text("does not");
        CView.text(" differ greatly from Valeria's other liquid attentions. ");
        if (player.body.cocks.length > 1) CView.text("They start");
        else CView.text("It starts");
        CView.text(" by thickening, stretching your cumvein");
        if (player.body.cocks.length > 1) CView.text("s");
        CView.text(" as wide as ");
        if (player.body.cocks.length > 1) CView.text("they");
        else CView.text("it");
        CView.text("'ll go without pain and then rolling thick pulses of gelatinous matter into your urethra");
        if (player.body.cocks.length > 1) CView.text("s");
        CView.text("s");
        CView.text(". The sleeve");
        if (player.body.cocks.length > 1) CView.text("s");
        CView.text(" squeeze");
        if (player.body.cocks.length === 1) CView.text("s");
        CView.text(" down on your length");
        if (player.body.cocks.length > 1) CView.text("s");
        CView.text(" harder, starting to slide around in ways that would be impossible for a vagina, and you find yourself helplessly blowing your load straight into the dick-plug");
        if (player.body.cocks.length > 1) CView.text("s");
        CView.text(", spurting bliss that can't even hope to escape from your loins. You feel ");
        if (player.body.balls.count === 0) CView.text("something expanding as you cum, filling up with spunk or slime or some mixture of the two");
        else CView.text("your [balls] expanding as you cum, filling up with spunk or slime or some mixture of the two, weighing down your sack with ever-increasing heaviness. Tender licks of tongue roll across the surface as you fill beyond completely");
        CView.text(".");
    }
    // Butt fillinggasm
    ...
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 5 && player.body.chest.find(BreastRow.FuckableNipples)) CView.text(", pausing to handle your hefty, swelling tits");
    CView.text(", moaning and groaning as Valeria's shape gradually coalesces back around you. The goo-girl's core vanishes from view and reappears down south, pressing against your [asshole] as the \"pipe\" that filled you so full melts into slime. You scream in anal ecstasy as it slips past your ring, lodging firmly in your butt while the jellied mass inside convulses, rubbing against your walls in all the right ways, almost like it's experiencing an orgasm all its own.");
    ...
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 5 && player.body.chest.find(BreastRow.FuckableNipples)) {
    ...
    if (player.body.cocks.length > 0 && player.body.balls.count > 0) {
        ...
        CView.text(" Your balls look positively swollen with libidinous juices.");
    }
    CView.text(" Your belly is stuffed into a parody of a pregnant woman's");
    if (player.body.vaginas.length > 0) CView.text(", so gravid from both holes that a real mother would be shamed by your obscene \"pregnancy.\"");
    else CView.text(".");
    ...
    ValeriaFlags.TIMES_VALERIA_GOO_THREESOMED++;
    player.orgasm();
    // v1 = time till birth.
    ...
    if (player.body.cocks.length > 0) {
        ...
        else player.effects.get(StatusEffectType.GooStuffed).value2 = 1;
    }
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 5 && player.body.chest.find(BreastRow.FuckableNipples)) player.effects.get(StatusEffectType.GooStuffed).value4 = 1;
    return { next: returnToCampUseOneHour };
}
...
export function birthOutDatGooSlut(player: Character) {
    ...
    if (player.body.skin.type === SkinType.FUR) CView.text("furry ");
    CView.text("bitch in heat. Gods, there's so much!");
    if (player.effects.get(StatusEffectType.GooStuffed).value4 > 0) {
        CView.text("\n\n");
        if (!player.body.chest.find(BreastRow.FuckableNipples)) CView.text("Stiffening up");
        else CView.text("Opening up");
        CView.text(", your [nipples], suddenly disgorge rivers of gelatinous slime, adding to growing puddle below with, odd-colored obscene lactation. You grab one with one hand and squeeze, shooting out huge streamers of milky-goo until your tit is sore and aching. You repeat on the other side");
        if (player.body.chest.length > 1) CView.text(" and the next row down");
        if (player.body.chest.length > 2) CView.text(", and so on");
        CView.text(", working out every drop of thick, body-filling slime while your ass is still busy draining. Even after they empty, they don't seem to lose a single bit of the size they've gained....");
    }
    if (player.body.cocks.length > 0 && player.effects.get(StatusEffectType.GooStuffed).value2 > 0) {
...
As you make your way back home, you hear an annoyed \"<i>Mooooo!</i>\" from Isabella's section of the  Cocking an eyebrow, you wander over to the busy cow-girl. Isabella's arms are currently crossed over her prodigious chest, her shield planted in the ground in front of her. Standing a few feet in front of her is Valeria, snuggly fit into her steel plates, her greatsword held firmly in hand.
