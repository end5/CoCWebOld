...
export class LivingStatueScenes {
	...
	public encounter() {
		...
		if (User.settings.silly()) DisplayText(" On the plus side, it seems to lack knees. That should make dodging its attacks a bit easier.");
		DisplayText("Moss clings to its chin an age-gifted beard. Bits of it shake loose as it speaks. <i>\"Tresspasser!\"</i>");
	...
	public beatUpDaStatue(hpVictory: boolean) {
		DisplayText().clear();
	...
	private takeTheStone() {
		DisplayText().clear();

		DisplayText("You carefully step through the cratered rubble to claim your prize. It's a chunk of lethicite,");

		// 9999 dis shit.
		if (player.hasKeyItem("Marae's Lethicite")) DisplayText(" easily as big as Marae's.");
		else DisplayText(" like the purple crystals in the factory, only this one is three times as big.");
	...
	public fuckinMarbleOP(hpVictory: boolean, pcCameWorms: boolean) {
		DisplayText().clear();
		DisplayText("You slump to your knees, overwhelmed and unable to see the shadow of the falling hammer. Your last thoughts are of regret.");
