...
export interface GnollSpearThrowerFlags {
    HAVE_ENCOUNTERED_GNOLL_PLAINS: number;
}

const gnollSpearThrowerFlags: GnollSpearThrowerFlags = {
    HAVE_ENCOUNTERED_GNOLL_PLAINS: 0
};

User.flags.set(FlagType.GnollSpearThrower, gnollSpearThrowerFlags);

// Female Gnoll.  First Page.
export function gnoll2Encounter(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Gnoll_Spear);
    DisplayText().clear();
    // <First Encounter>
    if (gnollSpearThrowerFlags.HAVE_ENCOUNTERED_GNOLL_PLAINS === 0) {
        gnollSpearThrowerFlags.HAVE_ENCOUNTERED_GNOLL_PLAINS++;
    ...
    return CombatManager.beginBattle(player, [], [new GnollSpearThrower()]);
}

// <Hyena Victorious – Anal>
export function hyenaSpearLossAnal(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Gnoll_Spear);
    // Oh shit get anal raped.
    if (player.inventory.items.has(ConsumableName.SuccubisDream)) {
        return Scenes.plains.gnollScene.yoDawgIHeardULiekGNollBallzzzdahdakjldh();
    }
    DisplayText().clear();
    ...
    if (player.inventory.equipment.weapon.displayname !== "fists") DisplayText("Before you can recover, she tosses your " + player.inventory.equipment.weapon.displayname + " into the long grass.  She roughly pulls the " + player.inventory.equipment.armor.displayName + " from your body and it, too, is tossed into the high grass, leaving you naked before this amazon.");
    ...
    // <Paragraph for Non-Taurs>
        ...
        else DisplayText("plunging into your " + Desc.Butt.describeButthole(player.torso.butt) + " and pinching until ");
    ...
    if (player.torso.tails.count > 0) DisplayText("One furred paw wraps around your tail before pulling up sharply, exposing your " + Desc.Butt.describeButthole(player.torso.butt) + " to the hot, savanna air.  Sharp fangs clamp onto the tail, holding it up to rob you of any remaining modesty.");
    // <For characters without tails>
    else DisplayText("One furred paw disappears before a single claw presses roughly against your " + Desc.Butt.describeButthole(player.torso.butt) + " until she hears your breath catch in your throat.");
    ...
    DisplayText("Clawed hands slide around your sides to roughly squeeze your " + Desc.Breast.describeChest(player) + ", using the painful hold to brace her body.  The hardened shaft vanishes as her hips pull back suddenly.  The barest touches against your entrance is all the warning you have before she shoves the two inch thick hyena meat roughly forward, stretching your poor hole wide around the sudden intruder.");
    ...
    DisplayText("  Muscles deep within you ripple, spasming around the strange intruder buried deep within you.  The hyena roars her pleasure, slamming deep into your " + Desc.Butt.describeButthole(player.torso.butt) + " as your body milks her hard cunt for the juices flowing from her.\n\n");
    ...
    // <For Slimes>
    if (player.torso.hips.legs.type === LegType.GOO) DisplayText("  Even as your eyes slide closed, you see the hyena kicking open the circle of dust.");
...
export function hyenaVictory(player: Character, monster: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Gnoll_Spear);
    DisplayText().clear();
    ...
    if (player.stats.lust >= 33 && player.gender > 0) {
        ...
        return { choices: [["Get BJ", penor], ["Get Licked", vagoo]], persistantChoices: [["Leave", Scenes.camp.returnToCampUseOneHour]] };
    }
    else return { next: Scenes.camp.returnToCampUseOneHour };
}

// <Hyena Defeat - Fellatio>
function hyenaVictoryRapeFellatio(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Gnoll_Spear);
    const cocksThatFit = player.torso.cocks.reduce(Cock.CocksThatFitOrSort(40, 2), []);
    const firstCock = cocksThatFit[0];
    const secondCock = cocksThatFit[1];
    DisplayText().clear();
    ...
    DisplayText("Dark brown eyes watch your hands as you slowly slip out of your " + player.inventory.equipment.armor.displayName + ".  The potent smell of hyena musk starts to fill the air as the dark shaft of the gnoll's pseudopenis slides from her sheath.  Eyes of brown fire fix on your now exposed " + Desc.Cock.describeMultiCockShort(player) + ".  Her pink tongue slides along her powerful teeth and dark lips, giving you pause, but her gaze is filled with nothing but lust and need.  Soft whines escape from her lips as she all but strains towards your " + Desc.Cock.describeCock(player, firstCock) + ".  Her spotted paws slide up to her heavy breasts, squeezing the orbs as the now submissive amazon tries to tempt you.\n\n");

    DisplayText("With a start, you realize that she is actually begging your permission.  Several moments pass as you let the warrior wait, watching her shift and squirm in anticipation.  A slight movement rests your " + Desc.Cock.describeCock(player, firstCock) + " on her warm nose, forcing the gnoll to breathe in your distinctive scent.  You can't hide your smile as you finally give her permission.  Instantly, the hyena's dark muzzle darts forward, burying her nose against the very base of your shaft, breathing deeply.  As she pulls back, that pink tongue darts out, feeling wet and velvety along your shaft until her dark lips press against your very tip.\n\n");
    ...
    if (firstCock.length > 7) DisplayText("The tip of your member presses hard against the back of her mouth for a long moment until she swallows, letting your " + Desc.Cock.describeCock(player, firstCock) + " slide into her tight throat.  ");
    ...
    if (firstCock.length > 7) DisplayText("in the back of her mouth ");
    ...
    // <Two Cocks>
        ...
        DisplayText("Even as the most incredible sensations flow through you from the feel of her muzzle, a spotted hand leaves one pendulous breast to wrap around your unattended " + Desc.Cock.describeCock(player, secondCock) + ".  Her powerful grip feels like a vice as she begins to slide the paw along the length.  Moving in time to the rippling of her throat, the hand tries hard to match the flood of sensations radiating from her muzzle.");
    ...
    if (player.torso.cocks.count > 2) {
        DisplayText("Even as the most incredible sensations flow through you from the feel of her muzzle, a spotted hand leaves one pendulous breast to wrap around your unattended " + Desc.Cock.describeCock(player, secondCock) + ".  Soon, her second hand slides along ");
    ...
    // <Cock 0 is 7\" or greater>
    if (firstCock.length >= 7) DisplayText("The muzzle starts to pull away from your " + Desc.Cock.describeCock(player, firstCock) + " until the tip finally slips from her throat, letting her gasp for breath.  ");
    ...
    // <Cock 0 is 7\" or greater>
    if (firstCock.length >= 7) {
        DisplayText("Orgasm crashes through your body and mind as time loses its meaning.  The gnoll's muzzle plunges forward once more, enveloping the pulsing shaft deep into her maw.  The powerful muscles of her throat ripple as she swallows, milking your " + Desc.Cock.describeCock(player, firstCock) + " for each spurt of your potent seed.");
        // <Two Cocks>
        if (player.torso.cocks.count === 2) DisplayText("  Your second " + Desc.Cock.describeCock(player, secondCock) + " throbs in her hand, spraying her face and throat with thick ropes of cum.");
        // <Three or More Cocks>
    ...
    // <Cock 0 is less than 7\">
        ...
        DisplayText("Orgasm crashes through your body and mind as time loses its meaning.  The gnolls's cheeks sink in as she starts to suck as hard as she can, increasing the glorious pressure on your trapped member.  The powerful muscles of her muzzle ripple as she swallows, milking your " + Desc.Cock.describeCock(player, firstCock) + " as each spurt of your potent seed splashes across her tongue.");
        // <Two Cocks>
        if (player.torso.cocks.count === 2) DisplayText("  Your " + Desc.Cock.describeCock(player, secondCock) + " throbs in her hand, spraying her face and throat with thick ropes of cum.");
        // <Three or More Cocks>
        if (player.torso.cocks.count >= 3) DisplayText("  Your " + Desc.Cock.describeMultiCockShort(player) + " throb in her paws, coating her muzzle, neck, and breasts with thick ropes of cum.");
...
function victoryRapeHyenaCunnilingus(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Gnoll_Spear);
    DisplayText().clear();
