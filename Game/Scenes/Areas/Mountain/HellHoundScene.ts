...
import { playerRapesCultist } from "../Lake/FetishCultistScene";
...
export interface HellhoundFlags {
    UNKNOWN_FLAG_NUMBER_00141: number;
}

const hellhoundFlags: HellhoundFlags = {
    UNKNOWN_FLAG_NUMBER_00141: 0
};

User.flags.set(FlagType.Hellhound, hellhoundFlags);

export function hellhoundEncounter(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplayText("You hear a fiery howl as a demonic, two-headed beast-man leaps out in front of you!");
    DisplaySprite(SpriteName.Hellhound);
    return CombatManager.beginBattle(player, [], [new HellHound()]);
}

export function hellhoundRapesPlayer(player: Character, monster: Character): NextScreenChoices {
    ...
    if (player.torso.hips.legs.type === LegType.CENTAUR) DisplayText("hocks and knees");
    ...
    if (player.torso.vaginas.count > 0 && player.torso.hips.legs.type !== LegType.NAGA) DisplayText("slamming his twin dicks into your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " and your " + Desc.Butt.describeButthole(player.torso.butt) + ".  ");
    else DisplayText("slamming his lower dick into your " + Desc.Butt.describeButthole(player.torso.butt) + ", while rubbing the other between the cheeks of your " + Desc.Butt.describeButt(player) + ".  ");
    if (player.torso.vaginas.count > 0 && player.torso.hips.legs.type !== LegType.NAGA) {
        if (player.vaginalCapacity() < monster.torso.cocks.get(0).area) DisplayText("You feel an intense mixture of sensations in your lower body as your " + Desc.Butt.describeButthole(player.torso.butt) + " feels like it is being torn apart, while your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " is filled with an intense pleasure at being filled with the creature's large member.  ");
        else DisplayText("Your lower body explodes with pain as the hellhound forces himself in too quickly for either your " + Desc.Butt.describeButthole(player.torso.butt) + " or your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " to handle.  ");
...
export function hellHoundGetsRaped(player: Character): NextScreenChoices {
    DisplayText().clear();
    // (Raped by player)
    ...
    if (player.torso.cocks.count > 1) DisplayText("One of the heads eagerly starts licking your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " while the other starts to work on your " + Desc.Cock.describeCock(player, player.torso.cocks.get(1)) + ". The large, flat tongues give you a rather unusual feeling, but it is definitely effective.   As you approach your peak, the heads quickly take your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " and " + Desc.Cock.describeCock(player, player.torso.cocks.get(1)) + " into their mouths. The pleasure from this new arrangement quickly overwhelms you, leaving you dazed.  When your senses return to you, you look up to see the hellhound's tongues running all over their mouths, clearly very happy.  With a pair of happy barks, the hellhound turns and runs away.");
...
export function HellHoundMasterEncounter(): NextScreenChoices {
    DisplayText().clear();
    // Initial encounter (A)
    ...
    if (hellhoundFlags.UNKNOWN_FLAG_NUMBER_00141 === 0) {
        ...
        return { yes: acceptCrazyPresences, no: declineCrazyPresences };
    }
    else if (hellhoundFlags.UNKNOWN_FLAG_NUMBER_00141 === 1) {
        // Second Encounter (D)
        ...
        return { yes: giveUpLethicite, no: declineGivingAwayLethicite };
        // Player choose yes or no, no -> E, yes -> F
    }
}

// choose no (B)
function declineCrazyPresences(): NextScreenChoices {
    DisplayText().clear();
...
// choose yes (C)
function acceptCrazyPresences(): NextScreenChoices {
    DisplayText().clear();
...
function acceptCrazyPresencesII(player: Character): NextScreenChoices {
    DisplayText().clear();
    ...
    if (player.inventory.keyItems.has("Marae's Lethicite") && player.inventory.keyItems.get("Marae's Lethicite").value2 < 3) DisplayText(" You extract Marae's lethicite from your pack, and wonder if you really want to trade it for the hellfire he offered.");
    // advance to repeat version
    hellhoundFlags.UNKNOWN_FLAG_NUMBER_00141 = 1;
    return { next: Scenes.camp.returnToCampUseOneHour };
}

// Choose no (E)
function declineGivingAwayLethicite(): NextScreenChoices {
    DisplayText().clear();
...
// Choose yes (F)
function giveUpLethicite(player: Character): NextScreenChoices {
    DisplayText().clear();
    ...
    if (player.inventory.keyItems.has("Marae's Lethicite") && player.inventory.keyItems.get("Marae's Lethicite").value2 === 0) return { choices: [["Give All", giveALLTHELETHICITES], ["Give Part", giveLethicitePiece]] };
    else return { next: giveLethicitePiece };
}

// Regular Lethicite
function giveLethicitePiece(player: Character): NextScreenChoices {
    DisplayText().clear();
    ...
    if (player.inventory.keyItems.has("Marae's Lethicite") && player.inventory.keyItems.get("Marae's Lethicite").value2 === 1) {
    ...
    player.perks.add(PerkType.Hellfire);
    // Hellhounds no longer encounterable.
    hellhoundFlags.UNKNOWN_FLAG_NUMBER_00141++;
    return { next: Scenes.camp.returnToCampUseOneHour };
}

// Merae's Lethicite
function giveALLTHELETHICITES(player: Character): NextScreenChoices {
    DisplayText().clear();
    ...
    if (player.torso.cocks.filter(Cock.FilterType(CockType.DOG)).length >= 2) {
        // the male bad end
        ...
        if (player.torso.cocks.count > 2 || player.torso.cocks.find(Cock.CockThatFits(60))) {
            ...
            while (player.torso.cocks.count > 2) {
                player.torso.cocks.remove(player.torso.cocks.count - 1);
            }
        }
...
export function hellHoundPropahRape(player: Character, monster: Character): NextScreenChoices {
    DisplayText().clear();
    ...
    // --- ELSE (CORRUPTION >= 20) ---
        ...
        if (player.stats.cor < 40 && !player.perks.has(PerkType.Masochist)) {
            ...
            player.combat.stats.loseHP(20, monster);
            player.stats.lust += -99;
