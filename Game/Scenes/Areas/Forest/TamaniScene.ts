...
class TamaniTimeAware implements ITimeAware {
    ...
    */
    ...
    public womb = new FlagWomb();

    public constructor() {
        flagWomb = new PregnancyStore(TamaniFlags.TAMANI_PREGNANCY_TYPE, TamaniFlags.TAMANI_PREGNANCY_INCUBATION, 0, 0);
        flagWomb.addPregnancyEventSet(PregnancyType.PLAYER, 96, 48);
    ...
    public timeChange(): boolean {
        flagWomb.pregnancyAdvance();
        trace("\nTamani time change: Time is " + Time.hour + ", incubation: " + flagWomb.incubation + ", event: " + flagWomb.event);
        if (flagWomb.isPregnant && flagWomb.incubation === 0) tamaniGivesBirth(); // Silently clear Tamani's pregnancy if the player doesn't encounter her in time
        return false;
    }
...
const tamaniTimeAware = new TamaniTimeAware();
const flagWomb = tamaniTimeAware.womb;

function tamaniGivesBirth() {
    if (flagWomb.type === PregnancyType.PLAYER) { // Don't want drider eggs to add to her daughers
    ...
    flagWomb.knockUpForce(); // Clear Pregnancy
}
...
function tamaniFirstTimeConsentual(player: Character): NextScreenChoices {
    ...
    CView.text("Her hips sway seductively as she approaches with her right hand dipping into the moist honeypot of her sex.  You disrobe, throwing your " + player.inventory.equipment.armor.displayName + " to the side before you reach down and lift her up, pressing her curvy body against your ");
    ...
    if (player.body.cocks.get(0).area <= 90) {
        ...
        CView.text("It takes a moment to put your " + player.inventory.equipment.armor.displayName + " back on and make ready to leave, but somehow you know this isn't the last you've seen of this goblin.");
        // [CORRUPT]
...
function tamaniMaleRepeatEncounter(player: Character): NextScreenChoices {
    ...
    if (flagWomb.isPregnant || TamaniFlags.TAMANI_NUMBER_OF_DAUGHTERS > 0) CView.text("While exploring, you're startled by the feeling of tiny hands stroking the insides of your thighs.  You look down and find Tamani there, grinning wolfishly,  \"<i>Ready for another fuck, big " + boyGirl(player.gender) + "?\"\n\n");
    // (ELSE)
    ...
    // [Let Her – Get dommed]
    // [No – starts fight]
    return { choices: [["Take Her", tamaniSexWon], ["Let Her", partial(tamaniSexLetHer, player, false)], ["No", tamaniStartFight]] };
}
...
// [let her]
function tamaniSexLetHer(player: Character, inCombat: boolean): NextScreenChoices {
    CView.sprite(SpriteName.Tamani); // 56;
    tamaniKnockUp(player);
    ...
    if (inCombat) {
        ...
        if (player.body.legs.type === LegType.CENTAUR) {
            ...
            CView.text("Tamani sidles up to you and pats your flank, cooing, \"<i>There there horsey, everything's going to be okay.  Momma Tamani's going to take really good care of you.</i>\"  Before you know it your " + player.inventory.equipment.armor.displayName + " is on the ground and the goblin is underneath you, climbing up to your crotch.\n\n");
        }
        // Not taur
        else {
    ...
    // Not-taurs
        ...
        if (player.body.cocks.get(0).area <= 90) {
            ...
            if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 2) CView.text("Your " + describeAllBreasts(player) + " bounce and jiggle obscenely as the goblin twists and pulls, abusing your nipples.");
...
function tamaniSexLost(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.Tamani); // 56;
    tamaniKnockUp(player);
    ...
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 6) CView.text("but can't manage to see your bloated prick past the wall of titflesh.\n\n");
...
function tamaniSexWon(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.Tamani); // 56;
    tamaniKnockUp(player);
    let cockThatFits = player.body.cocks.find(Cock.CockThatFits(90));
    if (!cockThatFits) cockThatFits = player.body.cocks.sort(Cock.Largest)[0];
    ...
    if (cockThatFits.area <= 90) {
        ...
        CView.text("It's too tempting of a target to resist.  You open your " + player.inventory.equipment.armor.displayName + " and allow your " + describeCocksLight(player) + " to flop free.  You're already hard from the enticing display, and in a moment you're pressing against her lust-slicked pussy");
...
function tamaniPregnantFuck(player: Character): NextScreenChoices {
    ...
    if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 2) CView.text(describeAllBreasts(player));
    else CView.text("chest");
    CView.text(".\n\n");
...
function tamaniKnockUp(player: Character) {
    if (flagWomb.isPregnant) return; // Already preggers
    flagWomb.knockUp(new Pregnancy(PregnancyType.PLAYER, 216)); // Nine day long pregnancy
...
export function encounterTamani(player: Character): NextScreenChoices {
    ...
    // Dudezillaz:
        ...
        switch (flagWomb.event) {
            case 2: return tamaniPregnantEncounter(player); break;	// She's moderately pregnant
            case 3: return tamaniPoopsOutBabies(player); break;		// She's close to giving birth so do it now
...
function getRapedByTamaniYouHypnoSlut(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.Tamani); // 56;
    ...
    CView.text("Tamani crooks her finger and you come running, already feeling yourself straining against your " + player.inventory.equipment.armor.displayName + " for a chance to fuck your wife and mistress.  She giggles, \"<i>Ohhh, how sweet, you're so ready to do your duty.</i>\"\n\n");
    ...
    if (primary) {
        ...
        if (cockCount > 0) CView.text("main ");
        ...
        if (cockCount > 1) {
            if (primary) secondary = player.body.cocks.get(0);
        ...
        if (cockCount > 0) CView.text(" around your " + describeCock(player, secondary));
        ...
        if (cockCount === 1) CView.text("her tongue hanging out as she pants nonsensical pleasure-noises.");
        ...
        CView.text("A surge of pride spreads through you at her praise, and you lean down to give your wife a long french kiss before heading back to   On the way back your head slowly clears, and you wonder what came over you back there?!");
        tamaniKnockUp(player);
        player.orgasm();
        player.stats.int += -.5;
        player.stats.sens += -1;
    ...
    // [DOESNT FIT]
        ...
        CView.text("Tamani is completely right.  You grab her ass, feeling your fingers sink in to her supple flesh as you pull her back towards your ");
        if (cockCount > 1) CView.text("main ");
        ...
        if (player.body.chest.sort(BreastRow.Largest)[0].rating < 1) CView.text("on your chest ");
        ...
        if (cockCount > 1) CView.text("primary ");
        ...
        if (cockCount > 1) {
            if (cockCount === 2) CView.text("  Your other" + describeCock(player, player.body.cocks.get(1)));
        ...
        CView.text("A surge of pride spreads through you at her praise, and you get up to give your wife a long french kiss before heading back to   On the way back your head slowly clears, and you wonder what came over you back there?!");
        tamaniKnockUp(player);
        player.orgasm();
        player.stats.int += -.5;
        player.stats.sens += -1;
...
function tamaniBeaten(player: Character): NextScreenChoices {
    ...
    if (player.pregnancy.ovipositor.fertilizedEggs === 0) flagWomb.knockUp(new Pregnancy(PregnancyType.DRIDER_EGGS, 72));
