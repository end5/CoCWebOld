...
export function playerHuntScore(player: Character): number {
    ...
    // trace("Base = " + String((player.stats.int + player.stats.spe) - (player.stats.fatigue * 2)));
...
export function firstWildHuntEncounter(player: Character): NextScreenChoices {
    ...
    return { choices: [["Wait", partial(firstWildHuntChase, player, true)], ["Run", partial(repeatWildHuntWait, player, false)]] };
}
...
function firstWildHuntChase(player: Character, waited: boolean = false): NextScreenChoices {
    ...
    if (waited)
        return player.inventory.items.createAdd(player, ItemType.Consumable, ConsumableName.CaninePepper, returnToCampUseOneHour);
    else return player.inventory.items.createAdd(player, ItemType.Consumable, ConsumableName.FoxBerry, returnToCampUseOneHour);
}
...
function whatsMyPrize(player: Character): NextScreenChoices {
    ...
    const selector: number = randInt(3);

    if (selector === 0) return player.inventory.items.createAdd(player, ItemType.Consumable, ConsumableName.CaninePepper, returnToCampUseOneHour);
    if (selector === 1) return player.inventory.items.createAdd(player, ItemType.Consumable, ConsumableName.FoxBerry, returnToCampUseOneHour);
    if (selector === 2) return player.inventory.items.createAdd(player, ItemType.Consumable, ConsumableName.NeonPinkEgg, returnToCampUseOneHour);
}
...
function predatoryPrey(player: Character): NextScreenChoices {
    ...
    if (!player.body.legs.isTaur()) {
        ...
        if (player.body.vaginas.length > 0 && player.body.cocks.length <= 0) {
            ...
            if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier > 0) CView.text("  The friction begins milking you, making you ooze milk down the trunk.");
...
function gwynnGibsGifts(player: Character): NextScreenChoices {
    ...
    return player.inventory.items.createAdd(player, ItemType.Consumable, ConsumableName.PrincessPucker, returnToCampUseOneHour);
}
