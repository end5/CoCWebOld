...
export function essrayleMeetingI(player: Character): NextScreenChoices {
    ...
    if (EssrayleFlags.MET_ESSY === 0) {
        ...
        if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 10) CView.text("own");
...
function plantsForMe(player: Character): NextScreenChoices {
    ...
    // (If breasts
    if (player.body.chest.sort(BreastRow.Largest)[0].rating > 1) CView.text("Your nipples stiffen, breasts puffing out with a heated breath");
    ...
    if (player.body.chest.get(0).rating >= 1) {
        ...
        // [Yes] [No]
        return { yes: partial(plantsForMe2, player, 1), no: partial(plantsForMe2, player, 2) };
    }
    // Else
    else {
        // [Next]
        return { next: partial(plantsForMe2, player, 0) };
    }
}
...
function plantsForMe2(player: Character, BE: number = 0): NextScreenChoices {
    ...
    if (BE === 1) {
        ...
        growTopBreastRowDownwards(player, 7, player.body.chest.length, false);
...
function feedTrappedEssy(player: Character): NextScreenChoices {
    ...
    if (player.gender === 3) {
        ...
        return {
            choices: [
                ["Cock", partial(hasCockFeedEssy, player, true)],
                ["Pussy", partial(hasPussyFeedEssy, player, true)],
            ]
        };
    }
}
...
function hasCockFeedEssy(player: Character, newPage: boolean = true): NextScreenChoices {
    ...
    if (player.body.cocks.sort(Cock.Largest)[0].length < 20) CView.text("her lower pair of wobbling udders");
...
function acceptEssyPrizes(player: Character): NextScreenChoices {
    ...
    if (player.body.chest.sort(BreastRow.Largest)[0].rating <= 1) {
        ...
        if (player.body.chest.length === 1) player.body.chest.add(new BreastRow());
    ...
    // if four+ breasts:
        ...
        growTopBreastRowDownwards(player, 6, player.body.chest.length, false);
    }
    boostLactation(player, player.body.chest.length);
...
function yesGimmeGiantNipplesEssy(player: Character): NextScreenChoices {
    ...
    return player.inventory.items.createAdd(player, ItemType.Consumable, ConsumableName.PurpleFruit, returnToCampUseOneHour);
}
...
// [Either choice]
function essyRewardEpilogueOUTTIES() {
