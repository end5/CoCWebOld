...
export const TamainsDaughtersFlags = {
...
User.flags.set(FlagType.TamainsDaughters, TamainsDaughtersFlags);

export class TamanisDaughtersTimeAware implements ITimeAware {
    ...
    // TamainsDaughtersFlags.TAMANI_TIMES_HYPNOTISED
    ...
    public constructor() {
        pregnancy = new PregnancyStore(TamainsDaughtersFlags.TAMANI_DAUGHTERS_PREGNANCY_TYPE, TamainsDaughtersFlags.TAMANI_DAUGHTER_PREGGO_COUNTDOWN, 0, 0);
    ...
    public timeChange(): boolean {
        ...
        trace("\nTamani's Daughters time change: Time is " + Time.hour + ", incubation: " + pregnancy.incubation + ", event: " + pregnancy.event);
        if (pregnancy.isPregnant && pregnancy.incubation === 0) {
            TamainsDaughtersFlags.TAMANI_NUMBER_OF_DAUGHTERS += TamainsDaughtersFlags.TAMANI_DAUGHTERS_PREGNANCY_COUNT;
            TamainsDaughtersFlags.TAMANI_DAUGHTERS_PREGNANCY_COUNT = 0;
            pregnancy.knockUpForce(); // Clear Pregnancy
        }
        // Put a cap on daughters if they havent been met yet.
        if (TamainsDaughtersFlags.TIMES_ENCOUNTED_TAMANIS_DAUGHTERS === 0 && TamainsDaughtersFlags.TAMANI_NUMBER_OF_DAUGHTERS > 30) {
            TamainsDaughtersFlags.TAMANI_NUMBER_OF_DAUGHTERS = 30;
        }
        // Lower daughter population by 1 every fourth day once population gets high
        if (TamainsDaughtersFlags.TAMANI_NUMBER_OF_DAUGHTERS > 40 && Time.hour > 23 && Time.days % 4 === 0) {
            TamainsDaughtersFlags.TAMANI_NUMBER_OF_DAUGHTERS--;
        }
        return false;
    }
...
const tamaniTimeAware = new TamanisDaughtersTimeAware();
const tamanisDaughtersWomb = tamaniTimeAware.womb;

let tamaniPresent: boolean = false; // Used to communicate between this class and TamanisDaughters.as
...
function tdCup(): string {
    if (TamainsDaughtersFlags.TAMANI_NUMBER_OF_DAUGHTERS < 20) return "C";
    else if (TamainsDaughtersFlags.TAMANI_NUMBER_OF_DAUGHTERS < 30) return "D";
    else if (TamainsDaughtersFlags.TAMANI_NUMBER_OF_DAUGHTERS < 40) return "DD";
    else if (TamainsDaughtersFlags.TAMANI_NUMBER_OF_DAUGHTERS < 50) return "E";
    else if (TamainsDaughtersFlags.TAMANI_NUMBER_OF_DAUGHTERS < 60) return "EE";
    return "F";
}
...
export function encounterTamanisDaughters(player: Character): NextScreenChoices {
    ...
    if (TamainsDaughtersFlags.TIMES_ENCOUNTED_TAMANIS_DAUGHTERS > 0 && randInt(10) === 0) {
        tamaniPresent = true;
        CView.text("While roaming along, you find your path ahead blocked by " + numToCardinalText(int(TamainsDaughtersFlags.TAMANI_NUMBER_OF_DAUGHTERS / 4)) + " goblins.  At the forefront of the mob is Tamani");
        if (TamainsDaughtersFlags.TAMANI_TIMES_HYPNOTISED >= 10) CView.text(", your wife");
        ...
        return {
            choices: [
                ["Fight", fightTamanisDaughters],
                ["Fuck Them", fuckYoDaughtersHomie],
                ["Let Them", legTamanisDaughtersRAEPYou]
            ]
        };
    }
    tamaniPresent = false;
    CView.text("While roaming along, you find your path ahead blocked by ");
    CView.text(numToCardinalText(Math.floor(TamainsDaughtersFlags.TAMANI_NUMBER_OF_DAUGHTERS / 4)) + " goblins.  You ");
    if (player.inventory.equipment.weapon.displayName === "fists") CView.text("ready your fists ");
    ...
    // first time
    if (TamainsDaughtersFlags.TIMES_FUCKED_TAMANIS_DAUGHTERS === 0) {
        ...
        return {
            choices: [
                ["Fight", fightTamanisDaughters],
                ["Fuck Them", fuckYoDaughtersHomie],
                ["Play Dumb", playDumbToTamanisDaughters],
                ["Let Them", legTamanisDaughtersRAEPYou]
            ]
        };
    }
...
function playDumbToTamanisDaughters(player: Character): NextScreenChoices {
    ...
    if (player.stats.int / 2 + 25 > randInt(75)) {
        ...
        if (TamainsDaughtersFlags.TIMES_ENCOUNTED_TAMANIS_DAUGHTERS > 1) CView.text("They really must not be paying much attention to what you look like.");
    ...
    if (TamainsDaughtersFlags.TIMES_ENCOUNTED_TAMANIS_DAUGHTERS === 1) CView.text("first ");
...
// [Fight Them]
function fightTamanisDaughters(player: Character, monster: Character): NextScreenChoices {

    CView.text("You whirl around threateningly, intent on putting Tamani's wayward brood back in their place.\n\n");
    return CombatManager.beginBattle(player, new TamanisDaughters());
    CView.sprite(SpriteName.TamainsDaughters); // 57;
    if (tamaniPresent) {
        // (+5 mob strength)
        monster.stats.str += 5;
...
// [Fuck them]
function fuckYoDaughtersHomie(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.TamainsDaughters); // 57;
    TamainsDaughtersFlags.TIMES_FUCKED_TAMANIS_DAUGHTERS++;
    const cocks: number = player.body.cocks.length;
    const daughters: number = Math.floor(TamainsDaughtersFlags.TAMANI_NUMBER_OF_DAUGHTERS / 2);

    CView.text("You shrug out of your " + player.inventory.equipment.armor.displayName + " and grab hold of ");
    ...
    if (primary) {
        ...
        if (daughters > 10) {
            ...
            if (daughters > 20) {
                ...
                if (tamaniPresent) CView.text(" while Tamani walks away");
        ...
        if (daughters <= 20) {
            ...
            if (tamaniPresent) CView.text(" and Tamani passionately frenches you.");
            else CView.text(".");
    ...
    // [DOESNT FIT]
        ...
        // (No Tamani:
        if (!tamaniPresent) CView.text("She presses something against your backdoor and before you have a chance to react, something hot and wet is filling your " + describeButthole(player.body.butt) + ".  You look down as she pulls out a tube of pink residue, and sways with burning desire.\n\n");
        ...
        if (!tamaniPresent) CView.text("You're handed another bottle as t");
        else CView.text("T");
        ...
        if (!tamaniPresent) CView.text("You shrug and drink it, ");
...
// [Let them]
function legTamanisDaughtersRAEPYou(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.TamainsDaughters); // 57;
    TamainsDaughtersFlags.TIMES_FUCKED_TAMANIS_DAUGHTERS++;
    const cocks: number = player.body.cocks.length;
    const daughters: number = Math.floor(TamainsDaughtersFlags.TAMANI_NUMBER_OF_DAUGHTERS / 2);
    ...
    if (primary) {
        ...
        else CView.text("the sultry gaze of Tamani's oldest daughter as she rides your " + describeCock(player, primary));
        ...
        if (tamaniPresent) {
            ...
            if (TamainsDaughtersFlags.TAMANI_TIMES_HYPNOTISED < 10) CView.text("'Wait... wife... mistress?' your mind wonders, rejecting the foreign thoughts.  You look up at Tamani, confused for a moment");
        ...
        // NO TAMANI:
            ...
            if (daughters < 20) {
                ...
                CView.text("Amazingly, you orgasm again.  An orgiastic squeal erupts from your " + describeCock(player, primary) + "'s current owner, breaking off into pants and gasps ");
                ...
                CView.text("  Disappointed moans rise up from the others as they realize they've lost the cum-lottery.   The winner, patting her belly happily, stands up, filling the air with a loud 'slurrrrrp' as her twat noisily releases your " + describeCock(player, primary) + ".");
    ...
    // [NO FIT]
        ...
        // [Tamani is There]
            ...
            if (TamainsDaughtersFlags.TAMANI_TIMES_HYPNOTISED >= 10) CView.text("wife");
            ...
            if (cocks > 2) {
                ...
                if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier >= 1) {
                    ...
                    if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier < 2) CView.text("leak");
                    else if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier < 3) CView.text("drip");
                    else if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier < 5) CView.text("spew");
            ...
            if (TamainsDaughtersFlags.TAMANI_TIMES_HYPNOTISED > 10) CView.text("She's the best wife ever.  You nuzzle deep into her cleavage, sighing happily.");
            else if (TamainsDaughtersFlags.TAMANI_TIMES_HYPNOTISED > 5) CView.text("She really is a great wife... wait, wife? You shrug away the thought and enjoy slowly motorboating her breasts.");
...
function tamaniDaughtersCombatLossDrain(player: Character): NextScreenChoices {
    ...
    // Vars
    const cockCount: number = player.body.cocks.length;
    ...
    if (cockCount > 1) CView.text("one of ");
    ...
    if (player.body.legs.type !== LegType.CENTAUR) {
        ...
        if (cockCount > 1) CView.text(" things not restrained are your " + describeCocksLight(player) + ", standing at attention despite, or perhaps because of, your predicament.\n\n");
    ...
    if (TamainsDaughtersFlags.UNKNOWN_FLAG_NUMBER_00058 === 0) CView.text("\"<i>Stop worrying.  I invented this while I was waiting on mom to bring me back more of your cum, just in case we ever got our hands on you.  I promise, after you've had a taste of my chair you'll never want to leave.</i>\"\n\n");
    ...
    if (cockCount > 1) CView.text("s are");
    else CView.text(" is");
    CView.text(" totally encapsulated within ");
    if (cockCount === 1) CView.text("a ");
    CView.text("wet orifice");
    if (cockCount > 1) CView.text("s");
    ...
    if (cockCount > 1) CView.text("each of ");
    ...
    if (TamainsDaughtersFlags.UNKNOWN_FLAG_NUMBER_00058 === 0) CView.text("Time for your medicine!  We need you to cum enough for each of us, and maybe have a little left over to play with, so suck up the medicine, okay?  Just relax and let it fill you so that you give us all that yummy cummy!</i>\"\n\n");
    ...
    // EXHIBITIONISTZ
    if (TamainsDaughtersFlags.PC_FETISH > 0) CView.text("unimaginably turned on by being used and abused by such machinery in front of an audience of your own horny children.");
    ...
    if (cockCount === 1) CView.text("that");
    ...
    TamainsDaughtersFlags.UNKNOWN_FLAG_NUMBER_00058++;
    ...
    if (TamainsDaughtersFlags.UNKNOWN_FLAG_NUMBER_00058 < 4) {
        ...
        if (TamainsDaughtersFlags.UNKNOWN_FLAG_NUMBER_00058 === 0) {
...
function tamaniDaughtersBadEndChoice(player: Character): NextScreenChoices {
    ...
    return {
        choices: [
            ["Yes", tamaniDaughtersYesBadEndMePlease],
            ["No", tamaniDaughtersDeclineBadEnd],
            ["Individual", tamanisDaughtersFillIndividuallyBADEND]
        ]
    };
}
...
function loseToDaughtersWithTamaniThere(player: Character): NextScreenChoices {
    ...
    if (TamainsDaughtersFlags.TAMANI_TIMES_HYPNOTISED < 2) CView.text("It tastes syrupy-sweet");
    ...
    CView.text("She reaches into your " + player.inventory.equipment.armor.displayName + " to rub ");
    if (player.body.cocks.length > 1) CView.text("one of ");
    ...
    if (TamainsDaughtersFlags.TAMANI_TIMES_HYPNOTISED > 10) CView.text("Your body must remember how much it loves being my pussy-hungry husband.");
    ...
    if (TamainsDaughtersFlags.TAMANI_TIMES_HYPNOTISED < 10) CView.text("filling your mind with truths");
    ...
    // (FITS)
        ...
        if (player.body.cocks.sort(Cock.Largest)[0].area < 30) CView.text(" in spite of the large sizes they usually handle.");
...
function knockUpDaughters(player: Character) {
    if (tamanisDaughtersWomb.isPregnant) return;
    tamanisDaughtersWomb.knockUp(PregnancyType.PLAYER, 216); // Nine day long pregnancy, just like mom
    // Determine how many kids...
    TamainsDaughtersFlags.TAMANI_DAUGHTERS_PREGNANCY_COUNT = 2;
    const cum: number = player.cumQ();
    // Breeder perk is awesome
    if (player.perks.has(PerkType.MaraesGiftStud)) TamainsDaughtersFlags.TAMANI_DAUGHTERS_PREGNANCY_COUNT += 3;
    if (cum >= 50 && randInt(2) === 0) TamainsDaughtersFlags.TAMANI_DAUGHTERS_PREGNANCY_COUNT++;
    if (cum >= 100 && randInt(2) === 0) TamainsDaughtersFlags.TAMANI_DAUGHTERS_PREGNANCY_COUNT++;
    if (cum >= 200 && randInt(2) === 0) TamainsDaughtersFlags.TAMANI_DAUGHTERS_PREGNANCY_COUNT++;
    if (cum >= 300 && randInt(2) === 0) TamainsDaughtersFlags.TAMANI_DAUGHTERS_PREGNANCY_COUNT++;
    if (cum >= 400 && randInt(2) === 0) TamainsDaughtersFlags.TAMANI_DAUGHTERS_PREGNANCY_COUNT++;
    if (cum >= 500 && randInt(2) === 0) TamainsDaughtersFlags.TAMANI_DAUGHTERS_PREGNANCY_COUNT++;
    if (cum >= 600 && randInt(2) === 0) TamainsDaughtersFlags.TAMANI_DAUGHTERS_PREGNANCY_COUNT++;
}

function combatWinAgainstDaughters(player: Character, monster: Character): NextScreenChoices {
    CView.sprite(SpriteName.TamainsDaughters); // 57;
    if (monster.stats.HP < 1) {
        CView.clear().text("You smile in satisfaction as " + monster.desc.a + monster.desc.name + " collapses, unable to continue fighting.");
        if (player.stats.lust >= 33 && player.body.cocks.length > 0) {
            ...
            return { yes: fuckYoDaughtersHomie, no: returnToCampUseOneHour };
        }
