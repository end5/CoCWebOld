...
import { StatusEffectType } from "../../Game/Effects/StatusEffectType";
import { guyGirl } from "../../Game/Descriptors/GenderDescriptor";
import { returnToCampUseOneHour } from "../../Game/Scenes/Camp";
import { describeVagina, describeClit } from "../../Game/Descriptors/VaginaDescriptor";
import { VaginaWetness } from "../../Game/Body/Vagina";
...
export function encounterFaerie(player: Character): NextScreenChoices {
    ...
    if (player.body.cocks.length > 0 && (player.body.vaginas.length <= 0 || randInt(2) === 0)) {
        ...
        if (randInt(player.stats.spe / 2) + player.effects.get(StatusEffectType.FaerieFucked).value1 > 15) {
            if (player.effects.get(StatusEffectType.FaerieFucked).value1 < 5) {
            ...
            else if (player.effects.get(StatusEffectType.FaerieFucked).value1 < 10) {
            ...
            else if (player.effects.get(StatusEffectType.FaerieFucked).value1 < 15) {
    ...
    if (player.body.vaginas.length > 0) return {
        choices: [
            ["Shoo Away", faerieShooAway],
            ["Nothing", faerieDoNothing],
            ["Rape", faerieRAEP],
        ]
    };
    else return {
...
function faerieRAEP(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.Faerie); // 17;
    // Count secksins
    if (!player.effects.has(StatusEffectType.FaerieFemFuck)) player.effects.add(StatusEffectType.FaerieFemFuck, 1, 0, 0, 0);
    else player.effects.get(StatusEffectType.FaerieFemFuck).value1 += 1;
    ...
    if (player.effects.get(StatusEffectType.FaerieFemFuck).value1 === 1) CView.text("She juices herself and screams, \"<i>Let me goooooooo,</i>\" trying to sound outraged instead of turned on, but the tiny girl's body gives away the lie.");
    else if (player.effects.get(StatusEffectType.FaerieFemFuck).value1 <= 5) CView.text("She juices herself and moans, \"<i>Stop teasing meeeeee,</i>\" doing her best to wriggle back against you, as if she could somehow impale herself on your digit.");
    ...
    // Non-Taurs
        ...
        if (player.effects.get(StatusEffectType.FaerieFemFuck).value1 < 4) CView.text("Her mood immediately shifts from panic to desire, and she licks her lips hungrily, locking her eyes onto your feminine folds.");
    ...
    // [FIRST TIME]
    if (player.effects.get(StatusEffectType.FaerieFemFuck).value1 === 1) {
    ...
    // [REPEAT LOW]
    else if (player.effects.get(StatusEffectType.FaerieFemFuck).value1 <= 5) {
...
function faerieDoNothing(player: Character): NextScreenChoices {
    ...
    if (player.body.chest.reduce(BreastRow.AverageNippleLength, 0) >= 1) {
        ...
        if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier > 1) CView.text("A squirt of milk shoots inside her, making the faerie moan. She looks up at you with lusty, slitted eyes, squeezing her legs together to draw more from you.\n\n");
        ...
        if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier > 1.5) CView.text("\n\nA copious gout of your milk escapes her rosy folds.");
        player.orgasm();
        player.stats.lib += -2;
    ...
    if (player.body.chest.reduce(BreastRow.AverageNippleLength, 0) > 3 || player.body.chest.sort(BreastRow.Largest)[0].rating <= 2) {
...
function faerieCaptureHJ(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.Faerie); // 17;
    if (player.effects.has(StatusEffectType.FaerieFucked)) player.effects.get(StatusEffectType.FaerieFucked).value1 += 2;
    else player.effects.add(StatusEffectType.FaerieFucked, 2, 0, 0, 0);

    if (player.effects.get(StatusEffectType.FaerieFucked).value1 < 15) {
    ...
    if (player.body.legs.isTaur()) {
        ...
        // Epilogue!
        if (player.effects.get(StatusEffectType.FaerieFucked).value1 < 10) CView.text("The faerie burps and giggles again before glaring up at you, accusing you with a mildly unfocused glare and asking, \"<i>Did you know we get drunk on cum?  Caushe I TRY SO HARRD not to get meshed up like this.</i>\"\n\n");
        else if (player.effects.get(StatusEffectType.FaerieFucked).value1 < 15) CView.text("The faerie burps and laughs drunkenly, patting the side of your " + describeLeg(player) + " and slurring, \"<i>Oh by Marae's ripe titsh!  I needed that.  Do you thhink you could catsch me again?  I love feeling your cum coating my body.</i>\"\n\n");
        ...
        if (player.effects.get(StatusEffectType.FaerieFucked).value1 < 15) CView.text("She licks her fingers and rolls around laughing, \"<i>Hehe, who caresh!  I'm happy! WHEEEEE!</i>\"\n\n");
    ...
    // Non-taurs
        ...
        if (player.cumQ() > 250) {
            CView.text("You cum uncontrollably, regretting your fertility as your body paints the inside of your " + player.inventory.equipment.armor.displayName + " with goopy whiteness.  ");
        ...
        if (!player.effects.has(StatusEffectType.Jizzpants)) player.effects.add(StatusEffectType.Jizzpants, 1, 0, 0, 0);
    }
    return { next: returnToCampUseOneHour };
}
