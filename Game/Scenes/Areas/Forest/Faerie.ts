...
export function encounter(player: Character): NextScreenChoices {
    ...
    if (player.torso.cocks.count > 0 && (player.torso.vaginas.count <= 0 || randInt(2) === 0)) {
        ...
        if (player.statusAffects.has(StatusAffectType.FaerieFucked) && randInt(player.stats.spe / 2) + player.statusAffects.get(StatusAffectType.FaerieFucked).value1 > 15) {
            if (player.statusAffects.get(StatusAffectType.FaerieFucked).value1 < 5) {
                ...
                DisplayText("It would be cute if she wasn't dressed up like such a slut.  You bet you could get her to help pleasure you, but she might not like it.  Or you could be a nice " + Desc.Gender.guyGirl(player.gender) + " and let her go...\n\nDo you force her to pleasure you?");
            }
            else if (player.statusAffects.get(StatusAffectType.FaerieFucked).value1 < 10) {
            ...
            else if (player.statusAffects.get(StatusAffectType.FaerieFucked).value1 < 15) {
            ...
            else DisplayText("\n\nYou lazily make a grab for her and easily snatch her out of the air.  Her body is sticky with a mix of desire and your last encounter.  You can feel her humping against your pinky while she begs, \"<i>Come on, let me crawl into your " + player.inventory.equipment.armor.displayName + " and wrap myself around your shaft.  I promise I'll only drink a little pre-cum this time, just enough to let me get off.  I'll be a good faerie slut, just let me get you off!</i>\"\n\nDo you let the faerie get you off?");
        ...
        if (player.stats.lust >= 90) {
            ...
            if (player.torso.cocks.count === 1) DisplayText("your dick");
    ...
    if (player.torso.vaginas.count > 0) return { choices: [["Shoo Away", faerieShooAway], ["Nothing", faerieDoNothing], ["Rape", faerieRAEP]] };
...
function faerieRAEP(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Faerie);
    // Count secksins
    if (!player.statusAffects.has(StatusAffectType.FaerieFemFuck)) player.statusAffects.add(StatusAffectType.FaerieFemFuck, 1, 0, 0, 0);
    else player.statusAffects.get(StatusAffectType.FaerieFemFuck).value1 += 1;
    ...
    if (player.statusAffects.get(StatusAffectType.FaerieFemFuck).value1 === 1) DisplayText("She juices herself and screams, \"<i>Let me goooooooo,</i>\" trying to sound outraged instead of turned on, but the tiny girl's body gives away the lie.");
    else if (player.statusAffects.get(StatusAffectType.FaerieFemFuck).value1 <= 5) DisplayText("She juices herself and moans, \"<i>Stop teasing meeeeee,</i>\" doing her best to wriggle back against you, as if she could somehow impale herself on your digit.");
    ...
    if (player.torso.hips.legs.isTaur()) {
        DisplayText("\n\nYou bop the tiny Faerie on the head to daze her briefly, then place her on a branch. You back yourself up against the tiny creature, lifting your tail so she can see your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ". The scent washes toward her and you hear a high pitched giggle; evidently that was more than enough to give her quite the contact high.  You feel a strange sensation in your slit as she slides her legs inside you and wraps her arms around your " + Desc.Vagina.describeClit(player) + ".\n\n");

        // [If cock-like clit:
        if (player.torso.clit.length >= 3) {
            DisplayText("The tiny fae begins jerking your clit like a cock, squeezing her arms tightly around you and sliding in and out of your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ". Her motions are frenetic and unpredictable, but incredibly pleasurable.  She starts licking at your " + Desc.Vagina.describeClit(player) + " as your femcum runs down it, which only serves to make her more excited. She gets so excited that her legs start kicking wildly as she screams \"<i>Swim! Swim! Swim! Swim!</i>\" over and over again.  ");
            // [Small amount of cum:
            if (player.torso.vaginas.get(0).wetness <= VaginaWetness.WET) DisplayText("The fae giggles more and more as the fluid seeps about her and your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " ripples. She hugs your " + Desc.Vagina.describeClit(player) + " tighter and starts gently gnawing at it, such a peculiar sensation that you cum suddenly, and wetly.  Her giggles quickly become all-out laughter, and she loses her grip on your clit, sprawling to the ground into a small puddle of femcum.\n\n");
            // [Normal amount of cum:
            else if (player.torso.vaginas.get(0).wetness <= VaginaWetness.DROOLING) DisplayText("The fae giggles more and more as the fluid squirts about her and your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " ripples. She hugs your " + Desc.Vagina.describeClit(player) + " tighter and starts gently gnawing at it, such a peculiar sensation that you cum suddenly, and wetly.  Her giggles quickly become all-out laughter, and she loses her grip on your clit, sprawling to the ground into a puddle of femcum.\n\n");
            // [Huge amount of cum:
        ...
        // [All other clits:
            ...
            DisplayText("The tiny fae rubs her hands around your " + Desc.Vagina.describeClit(player) + " as if entranced by it. Your body responds by pumping out more femcum, which she laps up happily.  She starts laughing maniacally and banging on your clit like a drum, periodically yelling out \"<i>CONGA!</i>\" for some reason. The strange ministrations feel incredible though, and you feel your love canal squeezing down on the faerie's tiny body.  ");
            // [Small amount of cum:
            if (player.torso.vaginas.get(0).wetness <= VaginaWetness.WET) DisplayText("You cum suddenly, and wetly. The fae giggles more and more as the fluid seeps about her and your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " ripples. Her giggles quickly become all-out laughter, and she loses her grip on your innards, sprawling to the ground into a small puddle of femcum.\n\n");
            // [Normal amount of cum:
            else if (player.torso.vaginas.get(0).wetness <= VaginaWetness.DROOLING) DisplayText("You cum suddenly, and wetly. The fae giggles more and more as the fluid squirts around her and your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " ripples. Her giggles quickly become all-out laughter, and she loses her grip on your innards, sprawling to the ground into a puddle of femcum.\n\n");
            // [Huge amount of cum:
            else DisplayText("You cum suddenly, and wetly. The fae tries desperately to hold on to your " + Desc.Vagina.describeClit(player) + " but the amount of fluid overwhelms her and she's sent spiralling to the ground into a huge puddle of your fluid, her giggling frame floating on the surface as her legs kick about erratically.\n\n");
        }
    }
    // Non-Taurs
    else {
        DisplayText("\n\nYou release the lower portion of your " + player.inventory.equipment.armor.displayName + ", revealing your aroused slit to the faerie.  ");
        if (player.statusAffects.get(StatusAffectType.FaerieFemFuck).value1 < 4) DisplayText("Her mood immediately shifts from panic to desire, and she licks her lips hungrily, locking her eyes onto your feminine folds.");
        ...
        if (player.torso.vaginas.get(0).wetness >= VaginaWetness.SLAVERING) DisplayText("releasing a tiny squirt");
        else if (player.torso.vaginas.get(0).wetness >= VaginaWetness.DROOLING) DisplayText("dribbling juices");
        else if (player.torso.vaginas.get(0).wetness >= VaginaWetness.WET) DisplayText("growing so slippery the faerie nearly loses her footing");
        else DisplayText("feeling yourself moistening with need");
        DisplayText(" from the tiny touches.\n\n");

        // (small) <= .50\"
        if (player.torso.clit.length <= .5) {
            DisplayText("She pulls apart your lips, revealing your tiny bud and repositioning herself to plant her feet inside you.  The flawless skin of her thighs pulls another gasp of pleasure from your lips.  They squeeze tightly around your " + Desc.Vagina.describeClit(player) + ", scissoring her gash across its sensitive surface.   You squirm, too engrossed in the rough grinding your button is receiving to worry about the faerie.   She clings to you, hanging on for dear life as your crotch nearly throws her free.  During the gyrations, she's slammed back into the " + Desc.Vagina.describeClit(player) + ", instantly penetrated by the nub with a wet 'schlick'.\n\n");
            DisplayText("Squealing and bouncing as she hangs on tightly, the faerie noisily orgasms around your clit, squirting her own fluids into your aching " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ".  The fluid tingles, and you shove your fingers in, smearing the sticky-sweet faerie-cum through your passage.   Before you can get far with it, your own orgasm goes off, squeezing your fingers and rippling around them, trying to milk your hand as if it was a dick.  Your legs go weak and wobbly, forcing you down on your " + Desc.Butt.describeButt(player) + " as the waves of pleasure flow through you, soaking the faerie in girlcum.\n\n");
        }
        // (medium) <= .1.25\"
        else if (player.torso.clit.length <= 1.25) {
            DisplayText("She watches, entranced as your " + Desc.Vagina.describeClit(player) + " hardens, poking between your lips, flushed with blood like a tiny cock.   The faerie swivels around, planting her dainty butt squarely on your snatch, sinking down a bit into the folds as she wraps her legs around the pulsating 'shaft'.   She hugs it, pressing it between her tiny breasts and licking it up and down, making you moan and squirm from unexpected stimulation of your most sensitive area.\n\n");
            DisplayText("You spread your " + Desc.Leg.describeLegs(player) + ", careful not to dislodge the faerie as she releases the " + Desc.Vagina.describeClit(player) + " and stands up, placing her dripping gash against the tip.   A quick plunge later and she's bottomed out, pressing her hips into the opening of your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " her feet slipping over the outer folds as she tries to maintain her balance.   You start rocking back and forth happily, bouncing the faerie up and down.  She moans, cute and barely audible, but sexy in a way that makes your sopping fuckhole even wetter.\n\n");
            DisplayText("She orgasms on you, squirting copiously, drenching your " + Desc.Vagina.describeClit(player) + " and " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " in clear faerie-fluid.  It tingles, wicking into your button and soaking into your snatch, enhancing every sensation.  You can feel the cool forest air as it flows over your vulva, seeming to stroke you, and without any chance of holding yourself back, you plunge your fingers into your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", immediately orgasming from the penetration, not even noticing the exhausted faerie sliding off the large clit and slipping partway into your cunt.\n\n");
        }
        // (streeeetch – large) <= 4.5\"
        else if (player.torso.clit.length <= 4.5) {
            DisplayText("Entranced by the growing " + Desc.Vagina.describeClit(player) + ", the faerie caresses her body, watching your love-button swell up, not stopping until it looks too huge for her tiny frame.  She climbs in a circle around it, awestruck by the size and majesty of your cock-like button.    She looks up at you, aroused but worried, saying, \"<i>You're so... BIG.  Oh goddess, I want to feel it inside me!</i>\"\n\n");
            DisplayText("She grabs hold of its slippery surface with both hands and jumps, lifting her lower body up before gravity yanks it back down onto the tip of your " + Desc.Vagina.describeClit(player) + ".  The tip barely slips in, despite the slippery wetness of the faerie.   She screams, though in pleasure or pain you cannot be sure.  You reason that it must be pleasure, because the faerie is wiggling her hips and grabbing hold of the rest of your " + Desc.Vagina.describeClit(player) + ", straining to pull herself further down the fem-cock.  Her belly starts to distort, displaying the cylindrical bulge on her tummy, expanding and contracting slightly as each of your heart-beats works through your clit.\n\n");
            DisplayText("In time, she manages to fully impale herself, quivering in orgasm as she gets off from the vibrations your pounding heart sends through your " + Desc.Vagina.describeClit(player) + ".  Her tongue lolls out and her eyes roll back, shut down by the extreme penetration, pain, and pleasure of the act.  You feel her cum soaking into you, sliding down into your slit and making your sensitive slit tingle.  Watching her get off is all it takes to bring you to orgasm with her, and the walls of your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " clamp down hungrily, contracting and gushing fluids over the faerie as she lies there, impaled on your crotch like a perverted ornament.\n\n");
        }
        // (too big) (else – hump dat shit)
        else {
            DisplayText("Entranced by your swollen " + Desc.Vagina.describeClit(player) + ", the faerie watches it slowly erect, filling with blood like a smooth over-sensitive cock.  She tentatively touches it, gasping and pulling back when it twitches in response.   With a look of awe, she turns to you and says, \"<i>There's no way I could take this beautiful monster, but I know I can make it feel good!</i>\"\n\n");
            DisplayText("She jumps onto it, making it bounce in the air as it takes her relatively insubstantial weight.  Embracing it in a full-body hug, she starts grinding on it, smearing her thick faerie juices into the clit and giggling every time you twitch from the feeling.  You squirm, sinking down from the raw sensation, your " + Desc.Leg.describeLegs(player) + " giving out underneath you.   Grabbing hold of a stump, you try to steady yourself, but the faerie humping your " + Desc.Vagina.describeClit(player) + " is interfering with your motor ability, and you slump into the forest loam, happily twitching as orgasm washes over you.\n\n");
            DisplayText("Your " + Desc.Vagina.describeClit(player) + " jumps, throwing the tiny woman off.  She slips and scrabbles across the surface of your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", sliding into your soaking gash.  She's squeezed tightly, sloshed around in the wetness of your orgasm.   The faerie's eyes cross, as she grows dizzy and battered in the sizzling whirlpool that is your groin.\n\n");
        }
    }
    // [OH SHIT ITS OVER, POOR BITCH CRAWLS OUT ALL STONE ON GIRLCUM]
    // [FIRST TIME]
    if (player.statusAffects.get(StatusAffectType.FaerieFemFuck).value1 === 1) {
    ...
    // [REPEAT LOW]
    else if (player.statusAffects.get(StatusAffectType.FaerieFemFuck).value1 <= 5) {
        DisplayText("The faerie slowly drags herself out of your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", smiling broadly with her eyes dilated wide.  She slips off you, dropping to the ground and giggling, \"<i>Everything feels so soft.  Mmmm that was fun!</i>\"\n\n");
...
function faerieDoNothing(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Faerie);
    DisplayText().clear();
    if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].nipples.length >= 1) {
        ...
        if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 1) DisplayText("A squirt of milk shoots inside her, making the faerie moan. She looks up at you with lusty, slitted eyes, squeezing her legs together to draw more from you.\n\n");
        DisplayText("Eventually you both find a rhythm and soon she's moaning loudly.  ");
        if (player.torso.vaginas.count > 0) DisplayText("With your other hand you start diddling your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", adding your own soft moans to hers.  ");
        DisplayText("A few blissful moments later, she shudders and you feel her uncontrolled spasms around your nipple.  ");
        if (player.torso.vaginas.count > 0) DisplayText("You join her shortly after.  ");
        ...
        if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 1.5) DisplayText("\n\nA copious gout of your milk escapes her rosy folds.");
        player.orgasm();
        player.stats.lib += -2;
        return { next: Scenes.camp.returnToCampUseOneHour };
    }
    if (player.torso.clit.length >= 1.0 && player.torso.clit.length <= 4.5 && player.torso.vaginas.count > 0 && randInt(2) === 0) {
        ...
        if (player.torso.vaginas.get(0).wetness >= VaginaWetness.DROOLING) DisplayText("You drench the poor faerie completely in your female juices, soaking her hair and body. Overwhelmed and spent, you drop her to the ground and catch your breath. She licks up what's around her face, but is too weak to do anything else but lie in the dirt.\n\n");
    ...
    if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].nipples.length > 3 || player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating <= 2) {
...
function faerieCaptureHJ(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Faerie);
    if (player.statusAffects.has(StatusAffectType.FaerieFucked)) player.statusAffects.get(StatusAffectType.FaerieFucked).value1 += 2;
    else player.statusAffects.add(StatusAffectType.FaerieFucked, 2, 0, 0, 0);
    DisplayText().clear();
    if (player.statusAffects.get(StatusAffectType.FaerieFucked).value1 < 15) {
    ...
    DisplayText("You let her loose and she hovers in place, as if pondering her one last chance to escape.  She sighs and looks back up, blushing fiercely as she lands on your hip and gazes down at the bulge of your groin.  You can't help but laugh as she slips under your " + player.inventory.equipment.armor.displayName + ", crawling across your sensitive thigh towards your " + Desc.Cock.describeMultiCockShort(player) + ".\n\n");
    // Taurs get a special scene!
    if (player.torso.hips.legs.isTaur()) {
        DisplayText("The tiny Faerie climbs on top of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)));
        if (player.torso.cocks.count > 0) DisplayText("largest " + Desc.Cock.nounCock(CockType.HUMAN));
        ...
        DisplayText("She smashes onto the end of your " + Desc.Cock.describeMultiCockShort(player) + " and grasps at it. Her face crushes into your urethra as her tiny legs wrap themselves around the tip. Your wildly flailing cock starts to grow larger as your orgasm approaches, but the faerie doesn't notice as she happily drinks up your pre.\n\n");
        // [No testicles:
        if (player.torso.balls.quantity === 0) DisplayText("Your tiny globules of semen go straight into her open mouth and she sucks them down gleefully before falling with a splat onto the pre soaked ground.\n\n");
        else {
            // [Small amount of cum:
            if (player.cumQ() < 50) DisplayText("Your semen splashes straight into her face and she's quick to suck it up. She falls with a splat onto the pre soaked ground while your member drips periodic droplets of cum onto her head.\n\n");
            // [Normal amount of cum:
            else if (player.cumQ() < 200) DisplayText("Your semen washes into her face and she loses her grip on your " + Desc.Cock.describeMultiCockShort(player) + ". She falls with a splat onto the pre soaked ground and you spray her with periodic spurts of fresh cum.\n\n");
            // [Huge amount of cum:
            else DisplayText("Your semen collides with her face and she is propelled off of your cock onto the pre soaked ground. Your " + Desc.Balls.describeBalls(true, true, player) + " continue pumping out cum like a hose until she's almost swimming in it.\n\n");
        }
        player.orgasm();
        player.stats.lib += -.5;
        // Epilogue!
        if (player.statusAffects.get(StatusAffectType.FaerieFucked).value1 < 10) DisplayText("The faerie burps and giggles again before glaring up at you, accusing you with a mildly unfocused glare and asking, \"<i>Did you know we get drunk on cum?  Caushe I TRY SO HARRD not to get meshed up like </i>\"\n\n");
        else if (player.statusAffects.get(StatusAffectType.FaerieFucked).value1 < 15) DisplayText("The faerie burps and laughs drunkenly, patting the side of your " + Desc.Leg.describeLeg(player) + " and slurring, \"<i>Oh by Marae's ripe titsh!  I needed that.  Do you thhink you could catsch me again?  I love feeling your cum coating my body.</i>\"\n\n");
        ...
        if (player.statusAffects.get(StatusAffectType.FaerieFucked).value1 < 15) DisplayText("She licks her fingers and rolls around laughing, \"<i>Hehe, who caresh!  I'm happy! WHEEEEE!</i>\"\n\n");
    ...
    // Non-taurs
        ...
        if (player.torso.cocks.find(Cock.HasKnot)) DisplayText("climbs atop your knot, wrapping her legs around the narrower shaft to hold on.  You can feel her cheeks resting atop the 'bulb' of your canine anatomy, teasing you with feminine features you're far too large to penetrate.  ");
        else if (player.torso.cocks.get(0).type === CockType.HORSE) DisplayText("climbs atop your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", hanging onto your ring of prepuce and wrapping her legs as far around your horse-like maleness as she can.  ");
        else if (player.torso.cocks.get(0).type === CockType.DEMON) DisplayText("climbs atop your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", hanging on to the corrupted nubs and nodules as she threads her legs between them, squeezing you tightly as she hangs on.  You can feel her wet gash sitting atop a particularly sensitive bump, teasing you with a tiny cunt you'll never be able to penetrate.  ");
        else if (player.torso.cocks.get(0).type === CockType.TENTACLE) DisplayText("climbs onto your squirming " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", wrapping her legs tightly around it as it wiggles and writhes with excitement.  Unbidden, it curls around and rubs its reddish-purple head against her face like an animal.  She gives it a gentle squeeze and licks it.  ");
        ...
        DisplayText("Your internal muscles clench unconsciously, squeezing out a dollop of pre that rolls down into the faerie's hair, soaking her head and face.  You can't see her reaction, but you can feel it oozing between her body and you, lubricating her as she humps and rubs against you.  Tiny muffled moans escape your " + player.inventory.equipment.armor.displayName + ", indicating that some part of her is enjoying the task.\n\n");
        DisplayText("Though she can only stimulate a few inches of you at a time, it feels really good – better than it should, and a budding warmth on the edge of release builds inside you.  Too late you realize you should have gotten at least partially undressed.  You cum before you can do anything about it, splattering your " + player.inventory.equipment.armor.displayName + " with seed and leaving a wet patch on the crotch.  You can feel it dripping back onto you and the faerie as more spunk squirts out, soaking the tiny girl in spooge as the wet spot grows.  ");
        if (player.cumQ() > 250) {
            DisplayText("You cum uncontrollably, regretting your fertility as your body paints the inside of your " + player.inventory.equipment.armor.displayName + " with goopy whiteness.  ");
            if (player.cumQ() > 500) DisplayText("The proof of your release forms a puddle around you as your legs give out and y");
        ...
        if (player.cumQ() > 500) DisplayText(" in the cum");
        ...
        player.orgasm();
        player.stats.lib += -.5;
        if (!player.statusAffects.has(StatusAffectType.Jizzpants)) player.statusAffects.add(StatusAffectType.Jizzpants, 1, 0, 0, 0);
    }
    return { next: Scenes.camp.returnToCampUseOneHour };
}
