...
function tentacleFun(player: Character): NextScreenChoices {
    ...
    if (player.body.vaginas.length > 0 && randInt(2) === 0) { // Vaginal Variant 50% of the time
        CView.text("You saunter over to a dangling group of perverted looking vines, discarding your " + player.inventory.equipment.armor.displayName + " along the way. Running your fingertips along the bulbous-tipped tentacle-like vines, you find one that looks ");
        ...
        if (randInt(4) === 0 && player.body.hips.rating <= 10) { // +hip up to 10
            CView.text("A strange shifting occurs below your waist, making your " + player.inventory.equipment.armor.displayName + " feel tight.  <b>Your hips have grown larger</b>, becoming " + describeHips(player) + ".  ");
        ...
        if (randInt(4) === 0 && player.body.butt.rating <= 10) {
            CView.text("A strange shifting occurs on your backside, making your " + player.inventory.equipment.armor.displayName + " feel tight.  <b>Your butt has grown larger</b>, becoming a " + describeButt(player) + ".  ");
...
function treeBoobFun(player: Character): NextScreenChoices {
    ...
    if (!player.effects.has(StatusEffectType.LustyTongue)) {
        if (randInt(4) === 0) { // 25% Chance of sensitive mouth status – increased lust gain/hour due to licking your lips :3
            ...
            player.effects.add(StatusEffectType.LustyTongue, 24, 0, 0, 0);
        }
    ...
    if (randInt(2) === 0) { // 50% Chance of breast-growth
        if (player.body.chest.sort(BreastRow.Largest)[0].rating > 5) {
            ...
            growSmallestBreastRow(player, 1, 1 + randInt(3), false);
        }
        else {
            if (player.body.chest.sort(BreastRow.Largest)[0].rating === 0) { // No tits yet
                CView.text("  As you redress, you realize you have grown a pair of luscious breasts!  Perhaps this was a good idea after all...");
                growSmallestBreastRow(player, 1, 2 + randInt(3), false);
            }
