...
function functions, only. Dxasmodeus grants exclusive rights to Fenoxo to add upon events to meet with
...
For further information and license requests, dxasmodeus may be contacted through function
...
export function encounter(player: Character): NextScreenChoices {
    ...
    if (player.gender === Gender.NONE && randInt(3) === 0 && !player.torso.hips.legs.isNaga() && !player.torso.hips.legs.isTaur() && !player.torso.hips.legs.isGoo()) {
    ...
    if (player.stats.cor > 50) {
        ...
        return { choices: [["Fight", startTentacleBeastCombat], ["Submit", tentacleLossRape]] };
    }
...
// ---------- Never used
// function tentacleEntice(player: Character, tentacleBeast: Character): NextScreenChoices {
...
//             tentacleBeast.stats.lust += 10 + randInt(5);
//         }
//         // Failure:
//         else
...
//         if (randInt(2) === 0) {
//             // GENDERLEZZ
//             if (player.gender === Gender.NONE) DisplayText("You brazenly turn your back on the creature and, glancing over your shoulder, begin bending over and presenting your " + Desc.Butt.describeButt(player) + " to the beast. It pauses and observes while you bend over further, presenting a full view of both your back door and your " + Desc.Butt.describeButthole(player.torso.butt) + ". You shift from side to side and observe the beast match your movements. You have obtained its attention to say the least.\n\n");
//             // CHICKS
//             else DisplayText("You brazenly turn your back on the creature and, glancing over your shoulder, begin bending over and presenting your " + Desc.Butt.describeButt(player) + " to the beast. It pauses and observes while you bend over further, presenting a full view of both your back door and your honey hole. You shift from side to side and observe the beast match your movements. You have obtained its attention to say the least.\n\n");
//             tentacleBeast.stats.lust += 10 + randInt(5);
//         }
//         // Failure
//         else {
//             DisplayText("You begin shaking your hips and grabbing your " + Desc.Breast.describeAllBreasts(player) + " to distract the creature. However, the near-miss from the tentacle it attempted to swat you with convinces you of its desire to beat your ass, rather than fuck it.\n\n");
//         }
//     }
//     kGAMECLASS.combatRoundOver();
// }

export function tentacleVictoryRape(player: Character) {
    DisplayText().clear();
    DisplaySprite(SpriteName.Tentacle_Monster);
    // Male/Herm
    if (player.gender === Gender.MALE || player.gender === Gender.HERM) {
...
function futaTentacleEpilogue(player: Character): NextScreenChoices {
    ...
    if (User.flags.get<PlayerFlags>(FlagType.Player).giacomo > 0) {
...
export function tentacleLossRape(player: Character, monster: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Tentacle_Monster);
    // Genderless madness
    if (player.gender === Gender.NONE) {
        ...
        if (player.torso.hips.legs.isTaur()) {
            centaurGenderlessRetardation(player);
            return;
        }
        else if (player.torso.hips.legs.isNaga()) {
            genderlessHilarityForNagaKenDolls(player);
            return;
        }
        else if (player.torso.hips.legs.isGoo()) {
            tentacularGenderGooTimes(player);
            return;
        }
        else {
            if (player.stats.cor < 75) DisplayText("While you attempt to resist the abomination, its raw muscle mass is too much. ");
    ...
    // Horsecock surprise!
    if (player.torso.cocks.filter(Cock.FilterType(CockType.HORSE)).length > 0 && player.torso.cocks.get(0).length > 15 && player.torso.cocks.get(0).thickness >= 3) {
        if (player.stats.cor < 75 && player.stats.lust < 100) DisplayText("It grabs you before you can get away!\n\nWhile you attempt to resist the abomination, its raw muscle mass is too much. ");
        ...
        if (player.torso.balls.quantity > 0) DisplayText(", and you feel your " + Desc.Balls.describeBalls(false, true, player, true) + " rise and drop in unison to the muscular contractions pumping freshly made cum into position for release");
        DisplayText(".\n\n");
        ...
        if (player.torso.balls.quantity > 0) DisplayText("Your balls have overfilled the creature!\n\n");
    ...
    if (player.stats.lust > 99) {
        if (!player.statusAffects.has(StatusAffectType.TentacleBadEndCounter)) {
            player.statusAffects.add(StatusAffectType.TentacleBadEndCounter, 0, 0, 0, 0);
        }
        else {
            // count up
            player.statusAffects.get(StatusAffectType.TentacleBadEndCounter).value1++;
            // Bad end
            if (player.statusAffects.get(StatusAffectType.TentacleBadEndCounter).value1 >= 3 && player.stats.cor > 50 && player.gender === Gender.HERM) {
    ...
    // Centaur madness!
        ...
        player.slimeFeed();
        DisplayText("They test your body, slipping about over your form.  A small tentacle finds its way into your mouth, coiling about your tongue and down your throat.  It's careful not to make you choke, seemingly as curious about your innards as it is about your shell.  You're given little time to think though, as a surge of fluid is deposited into your stomach, making your desire to cum grow even more.  The sharp spines coiled about you act similarly, spreading warmth about them wherever they touch your " + Desc.Skin.skin(player) + ".\n\n");
        // has at least 1 cock, engulfable:
        if (player.torso.cocks.count > 0) {
            if (player.torso.cocks.sort(Cock.SmallestCockArea)[0].area <= 50) {
                DisplayText("More aphrodisiac-toxin pours into you, causing " + Desc.Cock.describeMultiCockSimpleOne(player) + " to expand.  ");
                if (player.torso.cocks.count > 1) DisplayText("  The creature seems surprised at first to discover such a large brace of cocks, testing their texture and wrapping around each individually.  Your " + Desc.Cock.describeMultiCockShort(player) + " responds by wriggling about and tempting the beast to continue its exploration, but the gesture is futile and they're abandoned, though not for long.");
                DisplayText("\n\n");

                DisplayText("A peculiar sensation rolls over it as an unseen tentacle engulfs you, rippling and milking your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ".  Your body naturally tries to drive into it but the tentacle isn't strong enough to provide resistance.  Your wild humping causes it to bump up and down against your underbelly, a surprisingly pleasurable feeling.  The tentacle pays no heed, continuing to ripple and constrict around you;  a suckling noise accompanies the sensation of your pre-cum being suctioned out.\n\n");

            }
            // has cock, not engulfable:
            else {
                DisplayText("More aphrodisiac-toxin pours into you, causing " + Desc.Cock.describeMultiCockSimpleOne(player) + " to expand. Something bumps up against the tip but can't seem to fit around your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ".  It continues trying for a while, sending pangs of pleasure down the length.  The tentacle eventually gives up and latches onto the tip, positioned right at the opening to your urethra.  It sucks up your pre-cum as it drips from you, accompanied by a loud suckling noise.");
                // [With testicles:
                if (player.torso.balls.quantity > 0) DisplayText("The sucking reaches all the way to your " + Desc.Balls.describeBalls(true, true, player) + ", a spectacularly strange sensation that nevertheless feels wonderful.");
                DisplayText("\n\n");
            }
        }
        // has vagina:
        if (player.torso.vaginas.count > 0) {
            DisplayText("A squirming tentacle forces its way inside your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", undulating and squirming as it works its way deeper and deeper.  Your body responds by pumping out more fluid, making the passage of the monstrous thing easier.");
            Mod.Vagina.displayStretchVagina(player, 32, true, true, false);
            if (player.torso.cocks.count > 0) {
                if (player.torso.cocks.sort(Cock.SmallestCockArea)[0].area <= 50) DisplayText("  Your humping appears to not affect the creatures continuing efforts, despite the force of your body.");
            }
        ...
        if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 1 && player.lactationQ() <= 0) {
            DisplayText("Roving tentacles latch onto your " + Desc.Breast.describeAllBreasts(player) + "; tiny spikes jabbing into each " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + " and injecting some sort of hot fluid.");
            if (player.torso.chest.count * 2 === 2) DisplayText("  The anus-like tips affix to them.");
        ...
        // Anus === gaping:
        if (player.torso.butt.looseness >= 4) {
            DisplayText("Your " + Desc.Butt.describeButthole(player.torso.butt) + " makes an inviting target for the squirming mass and it's quick to capitalize.  A particularly bulbous appendage slides deep inside, roiling about in a way that not even your well-trained hole has been treated to.");
            if (player.torso.cocks.count > 0) DisplayText("  A series of undulating lumps pass over your prostate, pushing out a splash of pre-cum.");
        ...
        if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 1 && player.lactationQ() > 0 && player.lactationQ() < 1000) {
            DisplayText("Roving tentacles latch onto your " + Desc.Breast.describeAllBreasts(player) + ", tiny spikes jabbing into your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s and injecting some sort of hot fluid.  The pressure inside grows nearly unbearable as you feel your milk production increase.  To your relief, an anus-like tip attaches to each nipple.  They suckle at you incessantly and before long your nipples ache from overuse and your breasts have run completely dry.\n\n");
            Mod.Breast.boostLactation(player, 1);
        }
        // Breasts > Manly, lactating, enough to overfill:
        else if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 1 && player.lactationQ() >= 1000) {
            DisplayText("Roving tentacles latch onto your " + Desc.Breast.describeAllBreasts(player) + ", tiny spikes jabbing into your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + " and injecting some sort of hot fluid.  The pressure inside grows nearly unbearable as you feel your milk production increase.  To your relief, an anus-like tip attaches to each nipple.  They suckle at you incessantly and before long your nipples ache from overuse, but your breasts are still prepared to provide more milk!  The suction decreases as the beast before you becomes overfilled and eventually is forced to give up.\n\n");
            if (player.torso.cocks.count > 0) {
                DisplayText("Your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " explodes inside the creature, ");
                if (player.cumQ() <= 500) DisplayText("pushing the creature to the edge of its fluid-containing abilities.");
        ...
        if (player.torso.cocks.count > 0) {
            ...
            DisplayText("The creature's desires are soon fulfilled as your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " starts to swell.  ");
            // [has testicles:
            if (player.torso.balls.quantity > 0) DisplayText("Your " + Desc.Balls.describeBalls(true, true, player) + " tighten up against you in preparation for their inevitable release, ready to spray their boiling load into the beast.  ");
        ...
        // has vagina:
        if (player.torso.vaginas.count > 0) {
            DisplayText("Your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " ripples about the coiled intruder as you climax; fem-cum drips down the tentacle and fills the area with your musky scent.  You rear up as a surge of euphoria races through you, managing to overpower the tentacles holding your forelegs down for the briefest of moments.  But even with your forelegs free, the tentacle in your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " remains, rippling with waves of seed that spray inside you in massive, hot globules.  The sticky substance flooding your love canal pushes you over the edge and you orgasm again, spraying more as you cry out in pleasure.\n\n");
        }
        // has cock, normal cum amount, anus < gaping:
        if (player.torso.cocks.count > 0 && player.cumQ() < 1500 && player.torso.butt.looseness < 4) {
            DisplayText("Just as you think it's over, another tentacle rams into your " + Desc.Butt.describeButthole(player.torso.butt) + " and begins roughly massaging your prostate as it swells massively, causing another surge of cum to leave you, and another, and another.");
        ...
        if (player.torso.vaginas.count > 0) {
            DisplayText("Just as you think it's over, a tentacle rams into your " + Desc.Butt.describeButthole(player.torso.butt) + " and begins to swell massively, causing another surge of girlcum to leave you, and another, and another.");
            Mod.Butt.displayStretchButt(player, 40, true, true, false);
    ...
    // Milk surprise!
    if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 3.5 && player.gender > 0) {
        player.slimeFeed();
        ...
        if (player.gender === Gender.MALE) {
            ...
            if (player.torso.cocks.count > 1) DisplayText(" begin ");
        ...
        // Pussy
            ...
            // AND dick(s)
            if (player.torso.cocks.count > 0) {
                DisplayText(Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " and " + Desc.Cock.describeMultiCockShort(player));
                DisplayText(" begin ");
            }
            // Nope just pussy
            else {
        ...
        DisplayText("You shudder as your " + Desc.Breast.describeAllBreasts(player) + " are quickly encircled and molested by the smaller tentacles.  Your swollen mammaries ache as the tentacles attach their orifices to your oozing nipples.  The tentacles begin a distinct milking pattern, alternating which nipple is milked first; you moan in delight and watch as your milk travels through the tentacle shaft and down to the shambling beast's body.\n\n");
        // (Optional Paragraphs)
        if (player.gender === Gender.FEMALE) {
            // [Female/Virgin-Tight Cunt]
            if (player.vaginalCapacity() < 30) DisplayText("The beast senses your excitement and with beguiling speed swiftly impales your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " with one of its massive, knobbly tentacles.  You squeal in pain and pleasure as you feel every bumpy inch pound into you, your cunt being stretched to unbelievable proportions.  The tentacle quickly bottoms out in your shallow hole, pressing urgently against your cervix as it begins to rhythmically pound your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ".\n");
            // [Female/Loose-Moist Cunt]
            else DisplayText("The beast senses your excitement and with beguiling speed swiftly impales your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " with one of its massive, knobbly tentacles.  You moan like a whore as the beast's knobbly cock slides into with ease, every bump sending shivers through your spine as it finally bottoms out deep in your cunt, pressing into your cervix urgently.  The monster begins to pound heartily at your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", filling the air with lewd squishing sounds.\n");
            if (Mod.Vagina.displayStretchVagina(player, 20, true)) DisplayText("\n");
            DisplayText("\n");
        }// HERMS
        if (player.gender === Gender.HERM) {
            // [Herm/Virgin-Tight Cunt]
            if (player.vaginalCapacity() < 30) DisplayText("The beast senses your excitement and with beguiling speed swiftly impales your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " with one of its massive, knobbly tentacles.  You wail in excitement and pain, but before you can even digest the invasion, another tentacle impales itself on your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ".  The anus-like opening gapes to envelope you, slowly devouring your member.  The double assault drives your body wild, and you begin pumping back against the invader and thrusting your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " deeper into its tight fuck hole.\n\n");
            // [Herm/Loose-Wet Cunt]
            else DisplayText("The beast senses your excitement and with beguiling speed swiftly impales your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " with one of its massive, knobbly tentacles but before you can even digest the invasion another tentacle impales itself on your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", the anus like opening gaping to envelope you.  The double assault drives your body wild, and you begin pumping back against the invader and thrusting your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " deeper into its tight fuck hole.\n\n");
            if (Mod.Vagina.displayStretchVagina(player, 20, true)) DisplayText("\n");
            DisplayText("\n");
        }
        DisplayText("You slowly become aware that the beast has slowed its assault on your genitals and soon stops altogether, withdrawing entirely.  The beast lets out an audible gurgle and you smile as you feel the tentacles re-double their assault on your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s.  The beast slowly lifts you off the ground with its strong tentacles, suspending you about three feet off the ground before flipping you over.  You hang suspended in the air, your " + Desc.Breast.describeAllBreasts(player) + " dangling lewdly under you.  Suddenly you feel the desire to \"<i>moo</i>\" as the attack on your aching " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s continues.  The tentacles continue their assault for what seems like hours, but then you gradually sense the tentacles beginning to slow.  Another gurgling sound confirms your suspicions - the beast wants your milk, but it's obvious you have far too much to offer!  You grin wickedly when the beast's tentacles begin to sag, quickly reaching up to fondle and massage your " + Desc.Breast.describeBreastRow(player.torso.chest.get(0)) + ".  The stimulation causes even more milk to gush down the tentacles length.  After a few moments of the increased assault the beast groans and releases you, the tentacles popping off your nipples audibly, spraying your milk about as they release you.\n\n");
        // [Female/Herm]
        if (player.gender >= 2) DisplayText("Your " + Desc.Breast.describeAllBreasts(player) + " ache, but you can tell immediately they are not depleted.  More milk dribbles as the tentacles try to retreat, and you grin, hardly satisfied with the beast's attack.  You reach between your thighs, seizing the nearest knobbly tentacle.  The beast is so sated it offers no resistance as you begin to pound your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " with the living dildo.  The idea of turning the tables on the raping beast spurs you on to new heights and you cum quickly around the knobbly shaft, your cunt spasming and milking the bumpy tentacle hard.  As you finish with the tentacle the beast gives a final gurgle and retreats into the forest.");
        // [Male]
        else DisplayText("You feel your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s dribbling milk as the tentacles attempt their retreat.  You realize the beast has nowhere near drained you and you grin eagerly as your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " throbs mightily.  You reach back and seize the nearest knobby tentacle, the beast offering no resistance as you shove your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " into the tight, puckered orifice.  You moan in delight, grunting happily as you fuck the tight hole wildly.  The thought of turning the tables on the raping beast drives you closer to the edge; soon you bury all of your cock into the tight fuck tool and unload your massive torrent of cum into the tentacle.  Your hot cum gushes into the beast and you can feel the tentacle throb and squirm in protest as you fill the beast even more.  After your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " slips free the beast lets out a final gurgle of defeat and slithers away into the forest.");
        player.orgasm();
        player.stats.tou += 0.5;
        player.stats.spe -= 0.5;
        player.stats.int -= 0.5;
    ...
    if (player.gender === Gender.MALE) {
        ...
        if (player.stats.cor < 75) DisplayText("It grabs you before you can get away!\n\nWhile you attempt to resist the abomination, its raw muscle mass is too much. ");
        DisplayText("It pins you to the ground easily. You immediately feel a sharp, horrible pain ");
        if (player.torso.cocks.count > 1) DisplayText("at the base of your " + Desc.Cock.describeMultiCockShort(player) + ".");
        ...
        if (player.torso.cocks.count === 1) DisplayText("your member immediately becomes fully erect and pre-cum leaks liberally from your tip.");
        ...
        if (player.torso.cocks.count === 1) DisplayText("In your daze you see a monstrous, hollow tentacle poised over your furious cock. You scream in shock and protest, but your cries fall upon deaf ears. The tentacle descends upon your penis, now begging for release, and clamps down upon your pubic mound, fully encapsulating your member.");
        else DisplayText("In your daze you see " + player.torso.cocks.count + " monstrous, hollow tentacles poised over your furious cocks.  You scream in shock and protest, but your cries fall upon deaf ears.  The tentacles descend upon your " + Desc.Cock.describeMultiCockShort(player) + ", all begging for release, and clamps down upon your pubic mound, fully encapsulating your dicks.");
    }
    if (player.gender === Gender.FEMALE) {
        player.slimeFeed();
        player.stats.spe -= 1;
        ...
        if (player.stats.cor < 75) DisplayText("You struggle to break free, but the creature only constricts you further, ensuring your immobility. ");
        DisplayText("A quick flex of the tentacles securing your legs leaves you spreadeagled before the maw of the horror.  ");
        if (player.stats.cor < 75) DisplayText("Fearing for your life, you scream and struggle for help, but only the apathetic sounds of nature respond.");
        DisplayText("\n\n" + (player.torso.chest.count * 2 + 1) + " thorny tendrils appear and pierce your breasts and groin. A sharp pain and a burning sensation tear through you, overriding the previous wave of pleasure. You feel fluids being injected into you and a distinctive, agonizing misery flows into your veins.  Your breasts and ");
        if (player.torso.vaginas.count === 1) DisplayText("clit ");
    ...
    if (player.gender === Gender.HERM) {
        ...
        if (player.torso.cocks.count > 1) DisplayText(Desc.Cock.describeMultiCockShort(player) + " immediately become fully erect and leak pre-cum liberally from their tips.  ");
        else DisplayText("member immediately becomes fully erect and pre-cum leaks liberally from your tip.  ");
        DisplayText("  " + numToCardinalCapText((player.torso.chest.countNipples())) + " thorny tentacles pierce your nipples, and you feel as if someone shot acid into your tits, which immediately begin to swell.");
        Mod.Breast.growTopBreastRowDownwards(player, 1, player.torso.chest.count, false);
        ...
        if (player.torso.cocks.count > 1) DisplayText("furious cocks.  ");
        ...
        if (player.torso.cocks.count > 1) DisplayText(Desc.Cock.describeMultiCockShort(player) + ", now begging for release, and clamps down around your pubic mound, fully encapsulating your members.  ");
        else DisplayText(Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", now begging for release, and clamps down upon your pubic mound, fully encapsulating your member.");
    }
    // Call page 2!
    return { next: tentacleRapeContinuation };
}

function tentacleRapeContinuation(player: Character) {
    player.orgasm();
    player.stats.tou += 1;
    player.stats.int += -.5;
    player.stats.lib += 2;
    ...
    if (player.gender === Gender.MALE) {
        ...
        if (player.torso.cocks.count === 1)
            DisplayText("using a sucking-stroking motion on your helpless " + Desc.Cock.describeMultiCockShort(player) + ". The swelling of the ass tentacle pressures your prostate in a paradoxically pleasurable and painful manner. You realize, much to your terror, that this beast is MILKING you of your semen!");
        else
            DisplayText("using a sucking-stroking motion on your " + Desc.Cock.describeMultiCockShort(player) + ".  The swelling of the ass tentacle pressures your prostate in a paradoxical pleasurable and painful manner.  You realize, much to your terror, that this beast is MILKING you of your semen!");
        Mod.Butt.displayStretchButt(player, 50, true);
        ...
        if (player.torso.cocks.count === 1)
            DisplayText(Desc.Cock.describeMultiCockShort(player) + " and ass and in a very short time, your phallus explodes, launching stream upon stream of hot, thick cum into the horror. Your hips and pelvis buck violently with each thrust as the creature masterfully strokes your " + Desc.Cock.describeMultiCockShort(player) + "  and milks your prostate of your fluids. You cry with each orgasm, prompting the thing to milk you harder. After an eternity of successive ejaculations, the creature withdraws its unholy arms and leaves you in a bruised, lacerated, overfucked heap on the ground, discarded like a person throws away a corn cob after a meal.");
        else
            DisplayText(Desc.Cock.describeMultiCockShort(player) + " and ass and in a very short time, your dicks explode, launching stream upon stream upon stream of hot, thick cum into the horror.  Your hips and pelvis buck violently with each thrust as the creature masterfully strokes your " + Desc.Cock.describeMultiCockShort(player) + " and milks your prostate of your fluids.  You cry with each orgasm, prompting the thing to milk you harder. After an eternity of successive ejaculations, the creature withdraws its unholy arms and leaves you in a bruised, lacerated, overfucked heap on the ground, discarded like a person throws away a corn cob after a meal.");
    }
        ...
        DisplayText("\n\nIn a coordination that can only signify higher intelligence, the monster fucks your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)));
        if (player.torso.vaginas.count > 1)
            DisplayText("s");
        ...
        if (player.torso.vaginas.count > 1)
            ...
            DisplayText("clit. ");
        Mod.Vagina.displayStretchVagina(player, player.vaginalCapacity() * 0.76, true);
        ...
        Mod.Breast.boostLactation(player, .3);
        return { next: tentacleRapeContinuationForFemales };
    }
    else if (player.gender === Gender.HERM) {
        if (player.torso.cocks.count === 1) {
            DisplayText("A sharp tug tells you that the creature has sealed itself upon your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ". You see " + player.torso.chest.count * 2 + " smaller tentacles latch onto your erect nipples. You feel milk begin to leak out as the creature makes a perfect seal around your areola. A thick, phallic tentacle probes underneath your trapped " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " until it finds your vaginal opening. You cry out as the member punches past your opening and bottoms out in your womb. The tentacle swells up until it completely fills your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ".  ");
            Mod.Vagina.displayStretchVagina(player, player.vaginalCapacity() * 0.76, true, false, true);
            DisplayText("With freakish coordination, the beast sucks your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " and tits while hammering away at your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ". The overwhelming pleasure courses through your body and triggers an immediate orgasm, sending gouts of cum into the tentacle sealed around your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ". The sensation of your fluids entering the creature prompts it to suck your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " harder as well as hammer your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " faster, leading to a chain of orgasms.\n\n");
            ...
            DisplayText("A sharp tug tells you that the creature has sealed itself upon your " + Desc.Cock.describeMultiCockShort(player) + ". You see " + player.torso.chest.count * 2 + " smaller tentacles latch onto your erect nipples. You feel milk begin to leak out as the creature makes a perfect seal around your areola. A thick, phallic tentacle probes underneath your trapped cocks until it finds your vaginal opening. You cry out as the member punches past your opening and bottoms out in your womb. The tentacle swells up until it completely fills your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ".");
            Mod.Vagina.displayStretchVagina(player, player.vaginalCapacity() * 0.76, true, true, false);
            DisplayText("  With freakish coordination, the beast sucks your " + Desc.Cock.describeMultiCockShort(player) + " and tits while hammering away at your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ". The overwhelming pleasure courses through your body and triggers an immediate orgasm, sending gouts of cum into the tentacles sealed around your pricks. The sensation of your fluids entering the creature prompts it to suck your throbbing cocks harder as well as hammer your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " faster, leading to a chain of orgasms.\n\n");
            DisplayText("Drunk with pleasure, you revel in the sensation of cumming into the creature while it breast feeds from you. All you can do is drown in the experience of being milked from top to bottom. The creature begins piledriving your box faster and you feel like the creature is going to impale you with its phallic tentacle.\n\n");
            DisplayText("The creature's milking tentacles stop moving and you feel the dick-tentacle press sharply against your womb. You feel the thunderous force of hot fluid lance into your body as the creature cums repeatedly inside you, triggering yet another orgasm. The creature cums in surges and shoots repeatedly inside you. Within moments, excess cum spews out of your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " as it cannot hold anymore, but the creature keeps cumming.\n\n");
            DisplayText("After a while the creature withdraws its tentacles from you. It poises the tentacle-cock over your face and lets out one last load, covering your face in hot, thick sperm. You reflexively open your mouth and allow loads of the salty juice down your throat. Once spent, the creature shambles off, leaving you well milked and cum-soaked.");
        }
...
function tentacleRapeContinuationForFemales(player: Character) {
    DisplayText().clear();
    DisplaySprite(SpriteName.Tentacle_Monster);

    if (player.torso.vaginas.count === 1) { // single coochie
        ...
        if ((player.perks.has(PerkType.BigClit) && player.torso.clit.length > 2) || player.torso.clit.length > 3)
            DisplayText("almost " + numToCardinalText(Math.floor(player.torso.clit.length * 1.75)) + " inches in length. ");
        // normal girls get big clits
        else
            DisplayText("almost four inches in length.  Bruised and sore, you pass into unconsciousness ");
    }
    else {
        DisplayText("Satisfied, the creature drops you smartly and withdraws its limbs from you and lumbers away.  Covered completely in cum, you see that your " + player.torso.vaginas.count + " clits have swollen up to almost four inches in length.  Bruised and sore, you pass into unconsciousness, ");
    }
    // Not too corrupt
    if (player.stats.cor < 75)
    ...
    if (player.torso.clit.length < 7 && player.torso.clit.length >= 3.5 && player.perks.has(PerkType.BigClit)) {
        player.torso.clit.length += .1 + player.stats.cor / 100;
        DisplayText("Your massive clitty eventually diminishes, retaining a fair portion of its former glory.  It is now " + Math.floor(player.torso.clit.length * 10) / 10 + " inches long when aroused, ");
        if (player.torso.clit.length < 5)
            DisplayText("like a tiny cock.");
        if (player.torso.clit.length >= 5 && player.torso.clit.length < 7)
            DisplayText("like a slick throbbing cock.");
        if (player.torso.clit.length >= 7)
            DisplayText("like a big thick cock.");
    }
    // Grow clit if smaller than 3.5"
    else if (player.torso.clit.length < 3.5) {
    ...
    if (player.torso.vaginas.get(0).looseness === VaginaLooseness.TIGHT) player.torso.vaginas.get(0).looseness = VaginaLooseness.NORMAL;
    player.slimeFeed();
    return { next: Scenes.camp.returnToCampUseOneHour };
}

// Centaur v. Tentacle Monster: (display if pc is unsexed centaur)
function centaurGenderlessRetardation(player: Character) {
    DisplayText().clear();
    DisplaySprite(SpriteName.Tentacle_Monster);
    ...
    if (player.torso.balls.quantity === 0) {
        ...
        if (player.stats.cor < 50 && player.stats.lust < 70) DisplayText("dread");
        else DisplayText("anticipation");
        DisplayText(", but stops when it reaches your haunches.  Another starts testing the same area, briefly touching your " + Desc.Butt.describeButthole(player.torso.butt) + " but clearly not finding what it's looking for.\n\n");
        ...
        DisplayText("A tentacle smacks your " + Desc.Butt.describeButt(player) + " hard, and the voice returns to normal.\n\n");
    ...
    // (Followup scene, if pc has seen above at least once, is unsexed centaur and has balls: -Z)
        ...
        DisplayText(", slipping forward to probe between your haunches.  It arrives at and discovers your " + Desc.Balls.describeSack(player) + " with some little ceremony, stroking and fondling it.\n\n");

        DisplayText("\"<i>Now THIS is what I'm talking about!</i>\" the creature's eerie voice sings out.  \"<i>Daddy needs his medicine!</i>\"\n\n");

        DisplayText("The tentacle, now joined by a second, hunts around your " + Desc.Balls.describeBalls(true, true, player) + ", seeking any organs that might serve as a release valve for their contents.  You stare at it as it searches, quite certain you know what's coming next.\n\n");
...
function tentacularGenderGooTimes(player: Character) {
    DisplayText().clear();
    DisplaySprite(SpriteName.Tentacle_Monster);
