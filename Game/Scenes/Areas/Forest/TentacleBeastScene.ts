...
User.flags.set(FlagType.TentacleBeast, TentacleBeastFlags);
...
function tentacleEntice(player: Character, monster: Character): NextScreenChoices {
    // Spoiler for Entice Attack Male/Herm:
    if (player.gender === 1 || player.gender === 3) {
        if (randInt(2) === 0) {
            CView.text("In an effort to distract the creature, you begin gyrating your hips and swinging your penis in a shameless imitation of MeatSpin. The Tentacled Horror briefly pauses to observe your actions and rears similar to a posturing spider, considering your next actions.\n\n");
            monster.stats.lust += 10 + randInt(5);
        }
        // Failure:
        else
            CView.text("You grab your penis and shake it feverishly at the creature in order to distract it.  It swats a tentacle at you, forcing you to adroitly dodge the reprisal.  Apparently, the beast found you unimpressive.\n\n");
    }
    // Spoiler for Entice Attack-Female:
        ...
        if (randInt(2) === 0) {
            // GENDERLEZZ
            if (player.gender === 0) CView.text("You brazenly turn your back on the creature and, glancing over your shoulder, begin bending over and presenting your " + describeButt(player) + " to the beast. It pauses and observes while you bend over further, presenting a full view of both your back door and your " + describeButthole(player.body.butt) + ". You shift from side to side and observe the beast match your movements. You have obtained its attention to say the least.\n\n");
            // CHICKS
            else CView.text("You brazenly turn your back on the creature and, glancing over your shoulder, begin bending over and presenting your " + describeButt(player) + " to the beast. It pauses and observes while you bend over further, presenting a full view of both your back door and your honey hole. You shift from side to side and observe the beast match your movements. You have obtained its attention to say the least.\n\n");
            monster.stats.lust += 10 + randInt(5);
        }
        // Failure
        else {
            CView.text("You begin shaking your hips and grabbing your " + describeAllBreasts(player) + " to distract the creature. However, the near-miss from the tentacle it attempted to swat you with convinces you of its desire to beat your ass, rather than fuck it.\n\n");
        }
    }
    combatRoundOver();
}

function tentacleVictoryRape(player: Character): NextScreenChoices {
...
function tentacleLossRape(player: Character, monster: Character, noCombat: boolean): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.TentacleBeast); // 100;
    // Genderless madness
    if (player.gender === 0) {
    ...
    if (player.stats.lust > 99) {
        if (!player.effects.has(StatusEffectType.TentacleBadEndCounter)) {
            player.effects.add(StatusEffectType.TentacleBadEndCounter, 0, 0, 0, 0);
        }
        else {
            // count up
            player.effects.get(StatusEffectType.TentacleBadEndCounter).value1++;
            // Bad end
            if (player.effects.get(StatusEffectType.TentacleBadEndCounter).value1 >= 3 && player.stats.cor > 50 && player.gender === 3) {
    ...
    // Centaur madness!
        ...
        if (player.body.cocks.length > 0) {
            if (player.body.cocks.sort(Cock.Smallest)[0].area <= 50) {
        ...
        if (player.body.vaginas.length > 0) {
            ...
            if (player.body.cocks.length > 0) {
                if (player.body.cocks.sort(Cock.Smallest)[0].area <= 50) CView.text("  Your humping appears to not affect the creatures continuing efforts, despite the force of your body.");
            }
        ...
        if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 1 && player.lactationQ() <= 0) {
        ...
        if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 1 && player.lactationQ() > 0 && player.lactationQ() < 1000) {
        ...
        else if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 1 && player.lactationQ() >= 1000) {
        ...
        if (player.body.cocks.length > 0 && player.cumQ() < 1500 && player.body.butt.looseness < 4) {
            ...
            else return { next: returnToCampUseTwoHours };
        }
        // has cock, normal cum amount, anus == gaping:
    ...
    // Milk surprise!
    if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier >= 3.5 && player.gender > 0) {
        player.slimeFeed();
    ...
    if (player.gender === 3) {
        ...
        growTopBreastRowDownwards(player, 1, player.body.chest.length, false);
...
function tentacleRapeContinuationForFemales(player: Character): NextScreenChoices {
    ...
    if (player.body.vaginas.get(0).looseness === VaginaLooseness.TIGHT) player.body.vaginas.get(0).looseness = VaginaLooseness.NORMAL;
    player.slimeFeed();
    return { next: returnToCampUseOneHour };
}
