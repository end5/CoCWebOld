...
export interface FrogGirlFlags {
...
// Intro
export function findTheFrogGirl(): NextScreenChoices {
    DisplayText().clear();
    ...
    return { choices: [["Follow", followDatFrog], ["Resist", resistDatFrog]] };
}
...
function followDatFrog(player: Character): NextScreenChoices {
    ...
    else DisplayText("your bare but sensitive groin");
...
// Resist:
function resistDatFrog(): NextScreenChoices {
    DisplayText().clear();
    ...
    return { choices: [["Question", questDatFrogGirl], ["TeachLesson", teachDatFrogALesson]], persistantChoices: [["Leave", leaveFrogBe]] };
}
// Leave her be:
function leaveFrogBe(): NextScreenChoices {
    DisplayText().clear();
...
function questDatFrogGirl(player: Character): NextScreenChoices {
    ...
    return {
        choices: [
            ["Vaginally", player.torso.vaginas.count > 0 ? superBonusFrogEggsInYerCooch : undefined],
            ["Carry", carryBeeGirlsEggsVoluntarilyYouButtSlut]
        ],
        persistantChoices: [["Leave", continueOnYourWay]]
    };
}

// Continue on your way:
function continueOnYourWay(): NextScreenChoices {
    DisplayText().clear();
...
// Teach Her a Lesson
function teachDatFrogALesson(): NextScreenChoices {
    DisplayText().clear();
...
export function birthFrogEggsAnal(player: Character) {
    ...
    player.torso.butt.wetness += 1;
    if (player.torso.butt.wetness > 5) player.torso.butt.wetness = 5;
    player.orgasm();
    player.stats.sens += 1;
}
...
function superBonusFrogEggsInYerCooch(player: Character): NextScreenChoices {
    ...
    if (player.torso.tails.count > 0) DisplayText(" hooked over your tail,");
...
export function layFrogEggs(player: Character): NextScreenChoices {
    ...
    DisplayText("You shudder as you feel the familiar tingle of stimulant slime running down your thighs.");
    ...
    if (player.torso.vaginas.get(0).wetness < VaginaWetness.SLAVERING) {
        ...
        DisplayText("  It rapidly becomes clear that it has.  <b>Your pussy is wetter than ever.</b>");
    }
    if (player.torso.hips.rating < 25) {
        player.torso.hips.rating++;
        DisplayText("  There's an extra thickness to your [hips] as well.");
    }
    player.orgasm();
    player.stats.sens += 1;
    return { next: Scenes.camp.returnToCampUseOneHour };
