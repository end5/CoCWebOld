...
export const PhoukaFlags = {
    ...
    FREED_VALA: 0,
    TIMES_PC_DEFEATED_VALA: 0,
    TIMES_FUCKED_VALA_IN_DUNGEON: 0,
    INVESTIGATED_VALA_AFTER_ZETAZ_DEFEATED: 0,
    FORM: 0,
};
...
export class PhoukaTimeAware implements ITimeAware {
    // Implementation of ITimeAware
    public timeChange(player: Character): boolean {
        if (player.effects.get(StatusEffectType.PhoukaWhiskeyAffect).value1 > 0) {
            player.effects.get(StatusEffectType.PhoukaWhiskeyAffect).value1 += -1; // Count down hours until player is not drunk
            if (player.effects.get(StatusEffectType.PhoukaWhiskeyAffect).value1 <= 0) {
...
export function phoukaEncounter(player: Character): NextScreenChoices { // General entry point for everything except halloween special encounter
    ...
    if (choiceChance < 2)
        return phoukaFaerieFire(player);
    else if (choiceChance < 5)
        return phoukaStuck(player);
    else return phoukaMasturbating(player);
}

function phoukaFaerieFire(player: Character): NextScreenChoices { // In old Irish legends the Phouka are responsible for Will-o’-the-wisps. In the deep woods it’s Kitsune, but in the bog it’s a Phouka
    CView.clear();
    CView.text("As you’re wading through the bog a flickering light off in the distance catches your eye.  ");
    if (PhoukaFlags.PHOUKA_ENCOUNTER_STATUS !== 0) {
        CView.text("You recall your last encounter with one of these 'lanterns' out here in the bog.\n\nPerhaps it would be best to leave this faerie fire alone.");
    }
    else { // Special text for the first encounter
        CView.text("It seems to be moving around in one of the dense parts of the bog, where the canopy of dead trees and creeping vines create shadows as deep as twilight even at noon.\n\nYou can't make out a figure holding the light, but you decide to follow.  Spurred on by the thought that it may be someone else like yourself, or perhaps some agent of the demons, you keep trying to catch up with the light up ahead.  This trip into the bog is about to pay off.");
    ...
    const choices: ScreenChoice[] = [["Follow", phoukaStuck]];
    if (PhoukaFlags.PHOUKA_ENCOUNTER_STATUS !== 0 || player.inventory.keyItems.has("Traveler's Guide")) choices[4] = ["Leave", phoukaFaerieFireLeave];
    return { choices };
}

function phoukaFaerieFireLeave(player: Character): NextScreenChoices {
    if (PhoukaFlags.PHOUKA_ENCOUNTER_STATUS === 0)
        CView.text("\n\nYou decide it's best not to mess with weird floating lights out in the bog and return to your "); else CView.text("\n\nAt the moment you don't feel the need to meet one of those " + phoukaNameText("phouka", "creatures") + " again.");
    return { next: returnToCampUseOneHour };  // Return to camp, 1 hour used
}

function phoukaStuck(player: Character): NextScreenChoices {
    CView.clear();
    const hasAlcohol: boolean = player.inventory.items.has(ConsumableName.PhoukaWhiskey);
    CView.text("You follow some firmer ground into one of the many copses of dead trees.  The bog is quiet and still here.  Up ahead you see something glint in the hollow of a dead tree, but before you can investigate the firm ground beneath your feet liquefies and you begin to sink.\n\nYou try to leap to another muddy lump of exposed ground, but even before you land it has gone soft as well.  You are soon encased in mud almost to your waist.\n\nThat's when you feel something slick and warm dart past you in the muck.  Wherever it goes it drags a thick net behind it.  Your ");
    if (player.body.legs.isNaga()) CView.text("tail is");
    else CView.text("legs are");
    CView.text(" soon tangled in a solid web of intertwined roots.  Once you are bound in place, a large black eel launches itself free of the muck and lands in a nearby puddle.\n\nYour attacker seems to melt, then reforms, becoming a five inch tall " + phoukaName() + ".  His skin and wings are coal black, and his eyes are green and shiny like those of a cat.  He isn't wearing any clothes, and a fully erect cock almost an inch long juts out of his groin.  Apart from that and a lack of breasts, his body looks feminine, much like the faeries of the woods.\n\nThe " + phoukaName() + " leers at you, clearly deciding what he wants to do next.");
    if (hasAlcohol) CView.text("\n\nYou suspect that if you offer up that whiskey in your pack the " + phoukaName() + " probably won’t try to rape you.");
    if (PhoukaFlags.PHOUKA_ENCOUNTER_STATUS === 0) PhoukaFlags.PHOUKA_ENCOUNTER_STATUS = 1;

    const choices = [];
    if (hasAlcohol) choices[0] = ["Offer Drink", phoukaStuckOfferWhiskey];
    choices[1] = ["Talk", phoukaTalk];
    choices[2] = ["Fight", phoukaStartFight];
    return { choices };
}

function phoukaStuckOfferWhiskey(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You pull out the bottle and ask the " + phoukaName() + " if he might be interested in a drink.  He looks at the bottle and says <i>“Well we both know where ya picked that up, hmmm?”</i>  Then he smiles and adds <i>“Course that’s some other phouka’s problem if he let ye beat him.”</i>");
...
function phoukaStartFight(player: Character): NextScreenChoices {
    ...
    return CombatManager.beginBattle(player, new Phouka(phoukaName()));
}

function phoukaStartFightSilenced(player: Character): NextScreenChoices {
    player.effects.add(StatusEffectType.WebSilence, 0, 0, 0, 0);
    return CombatManager.beginBattle(player, new Phouka(phoukaName()));
}

function phoukaTalk(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You ask the " + phoukaName());
    if (player.stats.level < 10)
        CView.text(" if perhaps there's some way he could release you."); else CView.text(" if he really wants a taste of what you're going to give him.");
    ...
    if (player.stats.cor + randInt(125) <= 110) { // Players with 0% corruption have a 12% chance that the Phouka is willing to talk, players with 100% corruption have a 92% chance
        ...
        CView.text("  You open your mouth to say something more to the ");
        if (player.stats.cor < 34)
            CView.text("vile little monster"); else if (player.stats.cor < 67)
            CView.text("perverted " + phoukaName()); else CView.text("fuckable little " + phoukaName());
        CView.text(" but he must not feel like listening.  He launches a ball of black mud at your face, sealing your mouth with sticky and [if (corruption <= 50)ewww -][if (corruption > 50)mmmm -] salty muck.");
        player.effects.add(StatusEffectType.WebSilence, 0, 0, 0, 0);
        return CombatManager.beginBattle(player, new Phouka(phoukaName()));
    }
        ...
        switch (player.stats.level < 10 ? randInt(8) : randInt(12)) { // Low level characters don't get the chance to be offered whiskey
            ...
            case 6: // Offers you a chance to submit, which leads to the various defeat sex scenes
                CView.text(" considers for a moment and then says, <i>“You can't get away from me right now, so why don't you enjoy it?  Otherwise, maybe I bring in some of my friends an we all have at ya.”</i> The confidence in the " + phoukaName() + "'s gaze suggests he isn't bluffing.  He slowly circles you, hovering a few feet out of reach.  His eyes seem to be drinking in every detail of your body in a way that [if (corruption < 50)makes shivers run down your spine][if (corruption >= 50)makes your heart beat faster].\n\nThe " + phoukaName() + " continues, ");
                if (player.body.vaginas.length > 0)
                    CView.text("<i>“Maybe yer inta bunnies, hrm?  Big bunny with a nice hard cock and strong legs, I can do that fer ya.  Fill that tight little muff o' yers up with a big load o' spunk!”\n\n“Or maybe ");
                else
                    CView.text("<i>“");
                CView.text("I could change inta a goat for ya.  That suit yer fancy?  Is it a fond dream o' yers to let a goat blow its load in yer cornhole?");
                if (player.body.vaginas.length > 0)
                    ...
                    CView.text("  If it ain't then tough shite, cause that's what I want.”</i>");

                const choices = phoukaSexAddStandardMenuChoices(player);
                choices[4] = ["Fight", phoukaStartFight];
                return { choices };
                break;
            case 7:
            case 8: // Decides he doesn't want you
                CView.text(" takes a long look at you.  Finally he grins and tells you <i>“You know what?  You're not quite what I was lookin for.  Us phooka got varied tastes, ya know.”</i>  Then the phouka lazily flies off, whistling some strange tune to himself.\n\nYou wriggle against your bonds and after a few minutes you are free once more.  You decide that's enough excitement for now and head back to ");
                if (PhoukaFlags.PHOUKA_LORE === 0) PhoukaFlags.PHOUKA_LORE = 1; // Now you know what to call them
                return { next: returnToCampUseOneHour };  // Return to camp, 1 hour used
                break;
            default: // (9, 10 or 11) Offers to share some whiskey
                ...
                else return phoukaDrinkWhilePregnant(player, false);
        } // End of Switch
    }
}
...
function phoukaDrinkWhilePregnant(player: Character, playerOfferedTheBooze: boolean): NextScreenChoices {
    ...
    if (acceptable < 0) {
        ...
        if (acceptable === -4) CView.text("  One of them seems to crave the whiskey, but the other is a more normal child.");
        CView.text("  Sure, some of the elders around the village said a beer or some light cider was all right for a woman heavy with child, but you doubt that whiskey would be acceptable.  You politely refuse the drink, hoping the phouka won't be offended.");
    }
    else {
        CView.text("\n\nYou put one hand on your belly, about to refuse the drink.  Then you realize that yes, this is exactly what your ");
        if (acceptable <= 2)
            CView.text("baby needs.  Some light kicks inside your " + (acceptable === 2 ? "distended colon" : "womb") + " suggest that your child knows");
        else
            CView.text("babies need.  Some light kicks inside your womb and your distended colon suggest that your children know");
        CView.text(" what's up for grabs.  <i>“Go on and have some already. I try not to drink alone.  Always end up fucked by an imp or a drider or something that way.”</i>");
    }

    const choices = [];
    choices[0] = ["Refuse", (playerOfferedTheBooze ? phoukaDrinkRefuseAfterOffer : phoukaDrinkRefuse)];
    if (acceptable) choices[1] = ["Drink", phoukaDrinkAccept];
    return { choices };
}

function phoukaDrinkAccept(player: Character): NextScreenChoices { // In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
    CView.clear();
    CView.text("You accept the cup and give it a careful sniff.  Who knows what the phouka put in this stuff?  The smell reminds you of some scents from the old general store back home.  You try some and find it remarkably smooth for something supposedly cooked up from bog water and peat.  The black bunny gives you a crooked smile and settles down next to you, pouring out some more of the powerful spirits.\n\nBy the time you and the phouka have drunk half the bottle, both of you are feeling a lot better.");
    player.slimeFeed();
    switch (phoukaWhiskeyDrink(player)) {
        case 0: // Player isn't pregnant
            player.stats.lust += randInt(5) + 1;
            player.stats.cor += randInt(2) + 1;
            // Minor lust and corruption gains (since you aren’t drinking it all yourself). These gains are permanent
            break;
        case 1: // Child is a phouka or satyr, loves alcohol
            CView.text("A warm feeling in your belly lets you know your passenger enjoyed the meal as well.");
            break;
    ...
    if (player.stats.tou < 30) {
        ...
        return { choices: phoukaSexAddStandardMenuChoices(player) };
    }
    if (player.stats.tou < 70) {
        CView.text("  It looks like the phouka is holding his liquor about as well as you are.");
    }
    else {
        CView.text("  It seems you can handle your liquor better than the phouka.");
    }
    CView.text("  Soon both of you start to share stories about your adventures in Mareth.");
    return phoukaDrinkTalk(player, false);
}
...
function phoukaDrinkRefuse(player: Character): NextScreenChoices { // In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
    ...
    if (player.pregnancy.womb.pregnancy.incubation <= 100) { // Pregnancy is obvious to the phouka
        ...
        const choices = phoukaSexAddStandardMenuChoices(player);
        choices[0] = ["Leave", phoukaDrinkRefusePregnantLeave];
        return { choices };
    }
    else { // Less obvious pregnancy or player not pregnant
        CView.text("<i>“You damn prude.  That does it! I don't have to sit around listening to this.  I try to be friendly like some people say ya should instead of just raping ya.  What's it get me, huh?”</i>");
        if ((player.stats.lust > 66) || ((player.stats.cor > 50) && (player.stats.lust > 33))) {
            ...
            return { choices: [["Leave", phoukaDrinkRefuseLeave]] };
    }
}

function phoukaDrinkRefuseLeave(player: Character): NextScreenChoices { // In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
    CView.clear();
    CView.text("You extract yourself from the mud and get out of the bog before the phouka changes its mind.");
    return { next: returnToCampUseOneHour };  // Return to camp, 1 hour used
}

function phoukaDrinkRefusePregnantLeave(player: Character): NextScreenChoices { // In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
    CView.clear();
    CView.text("<i>“Ah fuck.  You're a killjoy.  Don't you know the best time to get porked is when you've already got a bun in the oven?”</i>\n\nThe black bunny waves you away dismissively. <i>“Fine, yeah, go home.  I hope the little bastard hurts when it pops out.  I would have stretched you out but good.”</i>");
    return { next: returnToCampUseOneHour };  // Return to camp, 1 hour used
}

function phoukaDrinkRefuseAfterOffer(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("The phouka looks disappointed but downs one of the cups and says <i>“How come chicks that’re knocked up are always such stick in the muds?”</i> He starts to sip the second cup, looks over at you and chuckles. <i>“I guess in yer case you’ve got a good excuse - ye are stuck in the mud!”</i> The phouka keeps drinking and it isn’t long before he starts to reminisce.");
    return phoukaDrinkTalk(player, true);
}

function phoukaDrinkTalk(player: Character, playerNotDrinking: boolean): NextScreenChoices {
    ...
    if (playerIsInebriated) {
        ...
        return { choices: [["Leave", phoukaDrinkLeave]] };
    }
    else {
...
export function phoukaHalloween(player: Character): NextScreenChoices {
    ...
    if (player.stats.str < 40) {
        CView.text("You find it very difficult to operate and have to squeeze it with your thighs, but you manage to pump a respectable amount before passing it on.");
    }
    else if (player.stats.str < 80) {
        CView.text("You find that treacle pumping is as hard as the phouka said, but you keep at it.  With what you pumped you must be as strong as they are.");
    }
    else {
        CView.text("You don't know why they said this was difficult.  You have no problem pumping the treacle; in fact they have to stop you partway through to change barrels.");
    }
...
export function phoukaPlayerWins(player: Character, hpVictory: boolean): NextScreenChoices {
    ...
    if (hpVictory) { // You win by physical damage, the phouka cheats and runs
        ...
        if (player.stats.lust < 33) {
            ...
            return phoukaLeaveOnLustWin(player, false);
        }
        else {
            CView.text("  While he's busy, you extract yourself from the muck, and once you're free, you look over at the lusty " + phoukaName());
            if (player.stats.cor < 34)
                CView.text(".  You could sate yourself just to be sure you're not distracted on the way home.");
            else if (player.stats.cor < 67)
                CView.text(".  You could take advantage of the situation to slake your lust.");
            else CView.text(".  It would be so easy to rape this perverted faerie, how can you resist?");

            const choices = [];
            choices[0] = ["Leave", phoukaLeaveOnLustWin];
            choices[1] = ["Anal Ride", phoukaSexFaeriePostCombat]; // Works for all
            if (player.body.cocks.length > 0) choices[2] = ["Fuck Faerie", phoukaSexFaerieFemalePostCombat]; // Male or Herm Only
            return { choices };
        }
    }
}
...
export function phoukaPregUpdate(player: Character): boolean { // Belly size doesn't change, instead you get updates on what's going on
    ...
    if (player.pregnancy.womb.pregnancy.incubation === 36) { // Stage 5:
        ...
        if (PhoukaFlags.PREGNANCY_CORRUPTION > 6)
            ...
            CView.text("Despite the relatively small size of your belly, you spend most of your time feeling mildly ill and you can't wait to force this thing out.\n");
        else CView.text("Despite being pregnant you still don't feel restricted or slowed in any way.  It's as if the tiny life inside is living in harmony with you.\n");
        return true;
    }
    return false;
}

function phoukaSexAddStandardMenuChoices(player: Character): ScreenChoice[] { // This happens several times so it's broken out here in case additional options get added later
    const choices = [];
    if (player.body.vaginas.length > 0) {
        choices[1] = ["Bunny", phoukaSexBunnyChoice];
        choices[2] = ["Horse", phoukaSexHorseChoice];
    }
    choices[3] = ["Goat", phoukaSexGoatChoice];
    return choices;
}

export function phoukaSexBunny(player: Character, postCombat: boolean, lustLoss: boolean): NextScreenChoices { // Player must have a vagina for bunny-morph sex
    CView.clear();
    if (postCombat) {
        ...
        CView.text(".  You watch as the bunny's erection stiffens, your ");
        if (lustLoss)
            CView.text("mind so overcome with lust you can't");
        else CView.text("body too bruised to");
        CView.text(" resist any further.");
    }
    else {
    ...
    if ((player.stats.lust > 80) || (player.stats.cor > 50))
        ...
        CView.text(" You are powerless to stop the bunny-morph as he undoes the last few straps holding your [armor] in place.");
    ...
    if (player.body.vaginas.find(Vagina.Virgin))
        CView.text("You scream as the " + phoukaName() + "'s cock tears your hymen apart.  He stops thrusting and says <i>“A virgin? Here? Good goddess, missy, ya should have told me.”</i> In a lower tone he adds <i>“so I could have enjoyed it more.”</i> Your whole vagina burns as every fold is stretched and widened by the passage of the rabbit's cock.");
    else if (postCombat && (player.body.vaginas.get(0).looseness < 2))
        CView.text("You nearly pass out from the pain as the " + phoukaName() + "'s cock lengthens and thickens inside you.");
    else if (!postCombat || (player.stats.cor > 50))
        CView.text("Even as the invader lengthens and thickens you push back against it. <i>“Good girl. Good little fuck slut,”</i> whispers the " + phoukaName());
    displayStretchVagina(player, 50, true);
    ...
    return phoukaSexPregnate(player, postCombat);
}

function phoukaSexBunnyChoice(player: Character): NextScreenChoices {
    return phoukaSexBunny(player, false, false);
}

export function phoukaSexGoat(player: Character, postCombat: boolean, lustLoss: boolean): NextScreenChoices { // Player does not have a vagina or has chosen anal
    CView.clear();
    if (postCombat) {
        if (lustLoss)
    ...
    if (postCombat)
        CView.text("He mounts you immediately, giving you no chance to adjust to his invading cock.  The goat-morph's first powerful thrust drives your face into the muck as his tries to force his narrow cockhead into your ass.");
    else
        CView.text("The goat rubs his cockhead against your pucker, spreading a thick layer of pre over your hole before slowly pressing home his cock.  His first few thrusts are gentle enough that you have the chance to spread your ass and get used to this invader.");
    CView.text("  Disturbingly you feel his front 'hooves' split into hard little fingers that he wraps around your [if (isTaur = true)belly][if (isTaur = false)[chest]] to hold onto you even tighter.\n\n");
    if (postCombat)
        CView.text("The " + phoukaName() + " shows no interest in you except as a receptacle for his cum.  After stabbing his hips at you several times you feel the full length of his cock penetrate your pucker.");
    else
        CView.text("Once your [butt] has adjusted, the goat morph begins to thrust more quickly.  You are surprised when you feel his balls slapping against your [if (isNaga = true)tail][if (isNaga = false)inner thighs] - he can't already be in that deep, can he?");
    CView.text("  Once the goat's cock is inside, he stops moving and tenses up.  You feel his cock growing larger and larger until you fear your abused anus is going to split apart.");
    displayStretchButt(player, 45, true);
    ...
    if (postCombat)
        CView.text("The goat-morph waits only a few seconds before unceremoniously yanking his deflating cock from your ass.");
    else
        CView.text("Realizing that you haven't yet cum, the goat morph continues to rock in and out of your ass.  The air fills with the sucking and slurping noises of the goat's seed alternately being sucked deeper or forced out of your ass.  Finally you reach your limit and cum[if (hasCock = true), your own cock turning the mud white beneath you].");
    player.stats.cor += randInt(1) + (postCombat ? 1 : 3);
    // Extra two corruption for being enough of a pervert to want to fuck the phouka
    player.orgasm();
    if (postCombat) CView.text("  While you're recovering the goat-morph reaches into your gem pouch and takes a handful.");
...
function phoukaSexGoatChoice(player: Character): NextScreenChoices {
    return phoukaSexGoat(player, false, false);
}

export function phoukaSexHorse(player: Character, postCombat: boolean, lustLoss: boolean): NextScreenChoices { // Player has a vagina
    CView.clear();
    if (postCombat) {
        CView.text("As you collapse the " + phoukaName());
    ...
    if (!postCombat || lustLoss)
        ...
        CView.text("You're hardly in any condition to refuse.  This stallion is going to pound you with that meatstick no matter what you do[if (corruption <= 50), so you steel yourself for what is to come][if (corruption > 50).  The thought of being wrecked by such an enormous staff has your cunt dripping girl-cum already.]");
    ...
    if (player.body.legs.isTaur())
        CView.text("He walks up behind you and mounts you.  You feel his front legs rubbing against your flanks and have to admit to yourself that this feels right.  The " + phoukaName() + "'s equine cock was built to force its way inside a mare[if (vagCapacity < 40), but it has difficulty with your tight cuntlips.  The stallion has to try twice more before his cock slides into place between your labia][if (vagCapacity >= 40) and it does just that].");
    else {
        if (player.body.legs.isNaga())
            CView.text("He walks in front of you, turns your way and then simply walks over you, forcing your upper body to bend backward over your tail.  Thankfully your snake-like spine means bending into a 'U' shape isn't that uncomfortable.");
        else
            CView.text("He walks up behind you and then simply walks right over top of you, his chest knocking you to the ground.  You find yourself on all fours with a massive cockhead grinding against your ass.");
        CView.text("  You hear the " + phoukaName() + " say <i>“Now line me up with yer cunt lass, or I'll lay down on top of ya and have a nap.”</i> Not wanting to be crushed under that kind of bulk, you reach [if (isNaga = true)up][if (isNaga = false)behind you] and grab his member.  You place his cock at the entrance to your [vagina] and the stallion steps forward.  [if (vagCapacity < 50)His cock bends and then snaps free of your cunt, slapping against his belly. <i>“Again! And do it right this time!”</i> he shouts.  You line it up once more and press against it with your hands so it has nowhere to go but in.]");
    }
    CView.text("  You're convinced you're going to be split in half by this monster horse cock.\n\n");
    if (player.body.vaginas.find(Vagina.Virgin))
        CView.text("You scream as the " + phoukaName() + "'s cock tears your hymen apart.  He stops mid-thrust and asks <i>“A virgin? Here? Good goddess missy ya should have told me.”</i> In a lower tone he adds <i>“so I could have enjoyed it more.”</i> Your whole vagina burns as every fold is stretched and widened by the passage of the " + phoukaName() + "'s huge horse cock.  You think he's delighted to have taken a woman from virgin to gaping in just one thrust.  ");
    else if ((postCombat) && (player.body.vaginas.get(0).looseness < 3))
        CView.text("You nearly pass out from the pain as the " + phoukaName() + "'s cock drives deeper and deeper inside you.  With such a wide head forcing its way in, you're sure the stretching will be permanent.  ");
    else if (!postCombat || (player.stats.cor > 50))
        CView.text("Even as the invader thrusts deeper you push back against it. <i>“Good girl, ya love horse cock, don't ya?”</i> whispers the " + phoukaName() + ".  ");
    displayStretchVagina(player, 80, true);
...
function phoukaSexFaerie(player: Character, postCombat: boolean): NextScreenChoices {
    ...
    if (player.body.vaginas.length > 0) {
        ...
        return phoukaSexPregnate(player, true);
    }
    else {
        CView.text("[butt].");
        displayStretchButt(player, 20, true);
        ...
        if (player.body.legs.isTaur())
            CView.text("You sigh in frustration as the " + phoukaName() + " pulls free from your ass, obviously about to leave.  He notices your condition and thinks better of it. <i>“Just gimme a sec.  I’ll get ye off,”</i> he says. <i>“After all, I do want ye to come back fer more.”</i>\n\nHe starts jacking himself off, whispering something to himself under his breath.  When his cock looks nice and hard he grabs your flanks and drives the full length back inside you.  As he pistons into your rectum, you feel that denied orgasm building and at last you cum.");
        else
            CView.text("Not yet satisfied, you wait for the flood to stop, then begin to really ride your man-sized faerie.\n\nYou don't give him the chance to go soft.  Your [if (isNaga = true)tail][if (isNaga = false)legs] feel rubbery and your muscles are burning when you finally cum.");
        if (player.body.cocks.length > 0) CView.text("  [EachCock] fires several blasts of cum into the bog [if (isTaur = true)beneath you][if (isTaur = false)and all over your faerie partner].");
        ...
        player.stats.cor += randInt(1) + 3;
        // Extra two corruption for being enough of a pervert to want to fuck the phouka
        if (postCombat) {
            // clearStatuses(false);
            // awardPlayer(); // This will provide loot and return to camp, 1 hour used
        }
        else return { next: returnToCampUseOneHour }; // Return to camp, 1 hour used
    }
}

function phoukaSexFaerieNonCombat(player: Character): NextScreenChoices {
    return phoukaSexFaerie(player, false);
}

function phoukaSexFaeriePostCombat(player: Character): NextScreenChoices {
    return phoukaSexFaerie(player, true);
}

function phoukaSexFaerieFemale(player: Character, postCombat: boolean): NextScreenChoices {
    ...
    if ((PhoukaFlags.FREED_VALA === 1) || (PhoukaFlags.TIMES_PC_DEFEATED_VALA > 0) || (PhoukaFlags.TIMES_FUCKED_VALA_IN_DUNGEON > 0) || (PhoukaFlags.INVESTIGATED_VALA_AFTER_ZETAZ_DEFEATED > 0)) // If any of these is true you've had some contact with Vala
        CView.text(" and she reminds you quite a bit of Vala, though Vala's chest is far larger.");
    else
        CView.text(", but she still looks more human than most of the people you've met in Mareth.");
    ...
    if (player.body.cocks.sort(Cock.Largest)[0].length < 8)
        CView.text("You push the last inch of your " + Math.floor(player.body.cocks.sort(Cock.Largest)[0].length) + " prick into the moaning slut.");
    else if (player.body.cocks.sort(Cock.Largest)[0].length < 15)
        CView.text("You feed the last inch of your " + Math.floor(player.body.cocks.sort(Cock.Largest)[0].length) + " cock into her.  Your cockhead bumps against her cervix and the slut beneath you begs for more.");
    else {
        CView.text("You drive as much of your cock as you can into the little faerie slut, but there's just too much meat for her to take in.");
        if (player.stats.cor <= 50)
            CView.text("  It still feels amazing and you give her even more pleasure by rubbing your cockhead against her cervix.");
        else
    ...
    if (player.stats.cor <= 33)
        CView.text("You certainly don't want a phouka to learn where your campsite is.  You help her into the bushes and tell her she just needs to hide out for a few days.");
    else if (player.stats.cor <= 66)
        CView.text("You laugh and remind her that's what she's wanted to do to you.");
    else
        CView.text("Why shouldn't the other phouka get the chance to enjoy her?  Before she can fully recover you grab some tree limbs and vines.  You pin her to the ground and use them to tie her up tight.  With her arms and legs lashed together and her ass in the air she's an inviting target.");
    CView.text("  Satisfied, you march back to ");
    player.stats.cor += randInt(1) + 3;
    // Extra two corruption for being enough of a pervert to want to fuck the phouka
    if (postCombat) {
        // clearStatuses(false);
...
function phoukaSexFaerieFemaleNonCombat(player: Character): NextScreenChoices {
    return phoukaSexFaerieFemale(player, false);
}

function phoukaSexFaerieFemalePostCombat(player: Character): NextScreenChoices {
    return phoukaSexFaerieFemale(player, true);
}
...
function phoukaSexPregnate(player: Character, postCombat: boolean): NextScreenChoices { // Whether by horse, bunny or (male) faerie sex it all ends up here if the PC has a vagina
    ...
    if ((!postCombat) || (player.stats.cor > 50)) {
        if (PhoukaFlags.FORM === PHOUKA_FORM_HORSE)
            ...
            if (postCombat)
                CView.text("The bastard " + phoukaName() + " laughs as you cum. <i>“Yeah, get nice and comfy while my seed takes root.  You lasses always love bein’ bred.”</i>");
            else CView.text("Perhaps he's being a considerate lover or perhaps he just wants to better his chances of knocking you up.");
        }
    }
    else {
        if (PhoukaFlags.FORM === PHOUKA_FORM_HORSE)
    ...
    if (player.pregnancy.womb.isPregnant()) {
        ...
        if (PhoukaFlags.PHOUKA_LORE === 0) PhoukaFlags.PHOUKA_LORE = 1; // Now you know what to call them
        if (!postCombat || (player.stats.cor > 50)) CView.text(", yer a real prize slut");
        CView.text(".”</i> The phouka gains more altitude and flies off deeper into the bog, whistling happily to himself.  ");
        if (isFirstTime)
            CView.text("You have to wonder how long this pregnancy will last. If this were a human child people would judge you to be at least four months along.");
        else CView.text("At least you know what to expect from this form of pregnancy now.");
    }
    else CView.text("When you pull yourself up your belly begins to deflate and leak its gooey grey payload into the mud.  The " + phoukaName() + " says <i>“Ah well, there's always next time,”</i> and buzzes away to some other part of the bog.");
    return phoukaSexPregnateEnd(player, postCombat);
}
