...
import { describeCock, describeOneOfYourCocks } from "../../../Descriptors/CockDescriptor";
...
export const ChameleonGirlFlags = {
    TIMES_MET_CHAMELEON: 0,
};
User.flags.set(FlagType.ChameleonGirl, ChameleonGirlFlags);
/**
...
export function encounterChameleon(player: Character): NextScreenChoices {
    ...
    if (ChameleonGirlFlags.TIMES_MET_CHAMELEON === 0) {
        ...
        // ENCOUNTER INTRO; REPEAT (Z edited)
        CView.text("You work your way through the dense foliage of the bog, pushing aside branches and slogging through the thick mud in search of something new.  Feeling exhausted, you slow down and look for a place to rest; a small clearing with shallow water and firmer ground seems to fit the bill, and you sit back against a tree to catch your breath.  You're so soaked by now that you hardly notice the murky water beneath you and the slick mud on the trunk seeping into your " + player.inventory.equipment.armor.displayName + ".");
        CView.text("\n\nA sudden scrape sounds behind you!  You spin around in time to see the familiar shape of a chameleon girl peeling off the trees.  She approaches you with a menacing glare in her eyes, growling \"<i>This is my bog!  Get out now or... or face the consequences!</i>\"  You raise your " + player.inventory.equipment.weapon.displayName + " and prepare to defend yourself.");
    }
    ChameleonGirlFlags.TIMES_MET_CHAMELEON++;
    return CombatManager.beginBattle(player, monster);
}
// LOSS SCENES (Intro) (Z edited)
export function loseToChameleonGirl(player: Character, monster: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ChameleonGirl); // 89
    // -Lose by lust
    if (player.stats.lust > 99) {
        CView.text("Losing control to your own growing arousal, you fall to your knees and desperately start working to get at your needy body beneath your " + player.inventory.equipment.armor.displayName + ".");
    }
    // Lose by HP
    else CView.text("Too weak to keep fighting, you stagger back and fall to your knees.");
    ...
    if (player.gender === 3 && randInt(2) === 0) return loseToChameleonWithCockAnBallsAnCunt(player);
    else if (player.gender === 1 || (player.gender === 3 && randInt(2) === 0)) return dudesLoseToChamChams(player, monster);
    else if (player.body.vaginas.length > 0) return loseToChamChamWithPCCunt(player, monster);
    else {
...
// Herm Loss (Z edited)
function loseToChameleonWithCockAnBallsAnCunt(player: Character): NextScreenChoices {
    ...
    CView.text("\n\n\"<i>Um, strip for me!</i>\" she commands, as forcefully as she can manage.  Still, it almost sounds like a question.  Too weak to do anything but play along, you comply, shedding your " + player.inventory.equipment.armor.displayName + ".");

    CView.text("\n\nAs you set the last bit of your " + player.inventory.equipment.armor.displayName + " aside in the water, you're suddenly struck in your chest and pushed to the ground.  The chameleon girl's wide foot keeps you pinned down, and she looks at you almost apologetically.  She shakes the worry from her face, and tries to appear stern once again.  \"<i>I'll... I'll show you who's the boss around here!</i>\"  Her tail darts towards you and uncoils at its end.  The tip starts probing around your " + describeVagina(player, player.body.vaginas.get(0)) + ", brushing across your " + describeClit(player) + " as it feels around for your entrance.");
    ...
    // [if(cockarea <40)
    if (largestCock.area < 40) CView.text(" and occasionally lifting it to take the shaft between two dexterous toes and work it more delicately");
    CView.text(".  She blushes as she watches your breaths turn to pants from the pleasure.");

    CView.text("\n\nThe feelings of being filled and having your " + describeCock(player, largestCock) + " serviced by her foot are just too good, and you quickly reach orgasm, spattering her foot and shin with your hot cum.  Her face is red as she tries to maintain her composure, and her entire body is starting to grow flush with barely contained excitement.  She softly rubs your semen into your softening man-meat as you recover from your orgasm, looking at it almost blankly and smiling.  After a moment she snaps herself out of her reverie, remembering that she was supposed to be punishing you but obviously too aroused to do much.  \"<i>Um, yeah! And you, uhh, better not come back!</i>\"  She rushes off back into the trees, presumably to masturbate somewhere where you won't notice.  You shake your head at the girl's antics, dress yourself, and head back to ");
...
// -Male Loss (Z edited)
function dudesLoseToChamChams(player: Character, monster: Character): NextScreenChoices {
    const randCock = player.body.cocks.random();
    CView.sprite(SpriteName.ChameleonGirl); // 89
    CView.text("\n\nRecognizing that you're no danger, her skin shifts back into its natural " + monster.body.skin.tone + " coloration as she walks towards you.  She swings her hips seductively as she splashes slowly through the water and licks her lips with a loud smack of her tongue, then reaches out and forcefully pulls your " + player.inventory.equipment.armor.displayName + " above your waist before you can even try to resist her advances, and pushes you back against a tree.  With a few quick pulls of fabric she's stripped her own clothes, and she tosses her thong over a low-hanging branch.  The other band of silk she uses to quickly bind your wrists.  Were you in any shape to fight back you could pull free, but you're too ");
    if (player.stats.lust > 99) CView.text("overwhelmed by lust");
    else CView.text("battered");
    CView.text(" to even rip the makeshift binding.");

    CView.text("\n\nShe makes short work of your " + player.inventory.equipment.armor.displayName + " in her haste to see her prize, then stands up and, still looking down, lets her tongue fall out of her mouth and find its way to [oneCock].  In an amazing display of control, she manages to wrap the warm pink appendage around your man-meat, sliding the slick length of her warm tongue up and down your shaft almost as if she were massaging you.  You shudder softly as she tickles the underside of your cock with just the tip of her tongue, trailing sticky saliva around its head.  Your member is soon erect and throbbing with need; satisfied, she slurps the slender length of muscle back into her mouth.");
    ...
    CView.text("\n\nShe catches herself, and looks back down at you and sneers.  \"<i>Y-yeah, and you better not come back around my bog again,</i>\" she says between tired breaths as she takes the silk from your wrists and dresses herself again.  She splashes off through the water, leaving you in silence.  You take a while to recover from the experience before managing to work up the energy to get out of the mud and back to your ");
    player.orgasm();
    return { next: returnToCampUseOneHour };
    // send player back to camp, reset hours since cum, remove gems and add time
}
...
// -Female Loss (Z edited)
function loseToChamChamWithPCCunt(player: Character, monster: Character): NextScreenChoices {
    CView.sprite(SpriteName.ChameleonGirl); // 89
...
// VICTORY SCENES INTRO(Z edited)
export function defeatChameleonGirl(player: Character, monster: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ChameleonGirl); // 89
    // -Win by lust
    if (monster.stats.lust > 99) {
        CView.text("Unable to control her arousal, the chameleon girl collapses to her knees and begins masturbating underneath her thong, having lost all capacity to fight you; she moans and throws her head back as her hand splashes in and out of the water she's kneeling in.  Her skin returns to its usual " + monster.body.skin.tone + " and then keeps going, shifting closer and closer to pink as her moans increase in both volume and volubility.");
        if (player.stats.lust >= 33) CView.text("  Do you help the horny girl to get you both off?  Or maybe you could punish her bad attitude with something from your bag...");
    }
    // -Win by HP
    else {
        CView.text("Too weak to continue fighting, the chameleon girl drops to her knees, exhausted.  Her skin returns to its usual " + monster.body.skin.tone + ", unable to maintain the camouflage.");
        if (player.stats.lust >= 33) CView.text("  Do you use the girl in her weakened state to get yourself off?  Or maybe you could punish her bad attitude with an item from your bags...");
    }
    ...
    let pussy: ClickFunction;
    let herm: ClickFunction;
    let item: ClickFunction;
    if (player.body.cocks.length > 0) dick = partial(manFucksChameleonWithBiggishWang, player, monster);
    if (player.body.vaginas.length > 0) pussy = partial(femaleHasWinSexWithChamCham, player, monster);
    if (player.gender === 3) herm = partial(fuckDatChameleonAsACoolGuyGirlHerm, player, monster);
    // let PC use item
    if ((player.inventory.items.has(ConsumableName.SuccubiMilk) || player.inventory.items.has(ConsumableName.SuccubiMilkPure)) && player.body.cocks.length > 0) item = partial(useAnItemOnTheChamcham, player, monster);
    else if (player.inventory.items.has(ConsumableName.SensitivityDraft) && (player.inventory.items.has(ConsumableName.LustDraft) || player.inventory.items.has(ConsumableName.LustDraftEnhanced))) item = partial(useAnItemOnTheChamcham, player, monster);
    return {
        choices: [
            ["Use Dick", dick],
            ["Use Pussy", pussy],
...
// -Herm Victory (Z edited)
function fuckDatChameleonAsACoolGuyGirlHerm(player: Character, monster: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ChameleonGirl); // 89
    ...
    if (player.stats.cor < 50) CView.text("and guiltily ");
    CView.text("roll her onto her back and get up to collect your things, rinsing yourself with the water and shaking the mud out before putting your " + player.inventory.equipment.armor.displayName + " back on and heading back to ");
    player.orgasm();
    return { next: returnToCampUseOneHour };
}
...
function manFucksChameleonWithBiggishWang(player: Character, monster: Character): NextScreenChoices {
    ...
    if (player.body.balls.count > 0) CView.text("your " + describeBalls(true, true, player) + " clench and ");
    CView.text("the semen erupts from the end of your " + describeCock(player, largestCock) + " spilling out hot spunk over her folded tongue.  She gasps audibly and arches her back as the thick ropes of your jizz coat her tongue, shoving her long fingers as far as she possibly can into her dripping pussy, and flashes so brightly pink she nearly glows.");
    if (player.cumQ() > 750) CView.text("  The torrent of cum bursting from your cock just doesn't stop, and her eyes roll back into her head as your semen coats her face and runs off her tongue, drenching her breasts and belly in white.  Her head falls back and her tongue goes limp as she's sent into ecstasy by the flood.");
    ...
    if (player.cumQ() > 750) CView.text("  Even the murky water around her is turned white as the pools of your cum on her body run off into it.");
    CView.text("  Satisfied, you put your " + player.inventory.equipment.armor.displayName + " back on and head back to ");
...
// -Female (for herms without dicks) (Z edited)
function femaleHasWinSexWithChamCham(player: Character, monster: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ChameleonGirl); // 89
    CView.text("You shed your " + player.inventory.equipment.armor.displayName + " and approach the fallen chameleon girl");
    if (monster.stats.lust > 99) CView.text(", her moans of pleasure growing more desperate as you approach");
    CView.text(".  You look her up and down, following the smooth curves of her thin body with your eyes.  Squatting down beside her, you take her weak body by the shoulders in one arm and trail your fingertips lightly up her torso with the other, feeling the soft skin of her belly.");
    if (monster.stats.lust > 99) CView.text("  She coos lustily at the sensation.");
    else CView.text("  Her eyes widen, and she looks from your hand to your face and back again.");
    CView.text("  When your hand reaches the silk cloth covering her chest, you pull it off with a light tug and cast it off to the side.  You cup her small breast in your palm, and as you squeeze it her skin tone changes perceptibly, flashing to a much brighter shade of the colors it was.");
    ...
    if (player.stats.cor < 50) CView.text("and somewhat guiltily ");
    CView.text("roll her onto her back and get up to collect your things, shaking out the mud and water before putting your " + player.inventory.equipment.armor.displayName + " back on and heading to ");
    // send player back to camp, reset hours since cum, add gems and time
    player.orgasm();
    return { next: returnToCampUseOneHour };
}
...
// Item Use Scenes Intro (Victory) (Z edited)
function useAnItemOnTheChamcham(player: Character, monster: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ChameleonGirl); // 89
    CView.text("Looking at the poor girl kneeling pathetically before you, you almost feel sorry for her.  But, she did try to rape you and claim that you were on 'her' territory.  You rummage through your bags, trying to find a fun way to punish her.  Fun for you, at least.");
    ...
    let drafts: ClickFunction;
    if ((player.inventory.items.has(ConsumableName.SuccubiMilk) || player.inventory.items.has(ConsumableName.SuccubiMilkPure)) && player.body.cocks.length > 0) milk = giveTheChameleonASuccubiMilk;
    if (player.inventory.items.has(ConsumableName.SensitivityDraft) && (player.inventory.items.has(ConsumableName.LustDraft) || player.inventory.items.has(ConsumableName.LustDraftEnhanced))) drafts = partial(doseDatChameleonWithLustAndSensitivityDrafts, player, monster);
    ...
    return {
        choices: [
            ...
            ["", undefined],
            ["", undefined],
            ["Back", partial(defeatChameleonGirl, player, monster)],
        ]
    };
}
...
function giveTheChameleonASuccubiMilk(player: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ChameleonGirl); // 89
    CView.text("You find a bottle of Succubi Milk tucked away in a pouch.  A wicked smile comes to your face as you imagine filling her so full of the stuff that her tits grow to the point where she can't even move, but you ultimately decide that just one dose will suffice for now.");
    ...
    if (player.hasLongTail()) CView.text(" with your tail,");
    else CView.text(" by straddling her,");
    CView.text(" your cock aimed for her pillowy tits.  She pushes and squirms beneath you, but cannot get enough leverage to move you.  You take her breasts in your hands and shimmy up her torso towards them, squeezing them around your " + describeCock(player, largestCock) + ".");
    ...
    player.orgasm();
    if (player.inventory.items.has(ConsumableName.SuccubiMilkPure)) player.inventory.items.consumeItem(ConsumableName.SuccubiMilkPure);
    else {
...
// -Lust and Sens Draft (you you you you you need to find more ways to start sentences without using pronouns, she?) (Z edited)
function doseDatChameleonWithLustAndSensitivityDrafts(player: Character, monster: Character): NextScreenChoices {
    CView.clear();
    CView.sprite(SpriteName.ChameleonGirl); // 89
    CView.text("You dig through your bags and come across a particularly tortuous combination of fluids.  With a Lust Draft in one hand and a Sensitivity Draft in the other, you return to the chameleon girl, swirling them and smiling at her menacingly.  She starts to protest, but you cut her off by shoving the opening of the Lust Draft vial into her open mouth.");
    ...
    CView.text("\n\nToo turned on to just sit there, she falls forward, catching herself on you, then looks up, pleading with her eyes.  You laugh mirthlessly at her grovelling form, and tell her to beg if she wants you.  \"<i>Please!</i>\" she squeals, her eyes watering, \"<i>I need this!</i>\"  She tugs insistently at your " + player.inventory.equipment.armor.displayName + ", begging for you to fill her and crying out that she can't stand to feel so empty.  You grin, and wonder if the sensitivity draft has started to take effect.  Two fingers run lightly up her forearm produce shivers at the obviously amplified sensation, and a tweak of her nipple elicits a shrill shriek.  \"<i>Why does it feel so good?!</i>\"  She puts her face into your lap and begs, \"<i>I don't even know what's going on, please, just fuck me, pleasepleaseplease...</i>\" trailing off as she grinds her pussy into you.");
    ...
    if (player.body.cocks.length <= 0 || player.body.cocks.sort(Cock.Smallest)[0].area > monster.vaginalCapacity()) {
