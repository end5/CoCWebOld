...
export interface KaijuFlags {
    KAIJU_MEETINGS: number;
    KAIJU_DISABLED: number;
    KAIJU_TALK_CYCLE: number;
    KAIJU_COCK: number;
    KAIJU_BAD_END_COUNTER: number;
}

const kaijuFlags: KaijuFlags = {
    KAIJU_MEETINGS: 0,
    KAIJU_DISABLED: 0,
    KAIJU_TALK_CYCLE: 0,
    KAIJU_COCK: 0,
    KAIJU_BAD_END_COUNTER: 0,
};

User.flags.set(FlagType.Kaiju, kaijuFlags);

// First encounter
// Boat
export function kaijuMeeting(player: Character): NextScreenChoices {
    DisplayText().clear();
    if (kaijuFlags.KAIJU_MEETINGS === 0) {
        ...
        // [Yes/No]
        return { yes: meetDatKaijuYo, no: noMeetingKaijus };
    }
    else {
        if (player.statusAffects.has(StatusAffectType.FactoryOverload) && !kaijuCock()) kaijuGrowsWangus();
        else if (kaijuFlags.KAIJU_BAD_END_COUNTER >= 5) return kaijuBadEndToyBOOSH(player);
        else return repeatKaijuEncounter(player);
    }
}

function kaijuCock(): boolean {
    return (kaijuFlags.KAIJU_COCK === 1);
}

// [If no]
function noMeetingKaijus(): NextScreenChoices {
    DisplayText().clear();
...
// [If yes]
function meetDatKaijuYo(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
    kaijuFlags.KAIJU_MEETINGS = 1;
    if (player.statusAffects.has(StatusAffectType.FactoryOverload)) kaijuFlags.KAIJU_COCK = 1;
    ...
    return {
        choices: [
            ["Compliment", complimentKaiju],
            player.gender > 0 ? ["Flirt", flirtWithKaiju] : ["", undefined],
            ["Insult", insultTheKaijuFirstMeeting]
        ]
    };
}

// [If insult]
function insultTheKaijuFirstMeeting(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
    if (kaijuCock()) {
        ...
        if (player.statusAffects.has(StatusAffectType.KnowsWhitefire) && (!player.perks.has(PerkType.BloodMage) && player.stats.fatigue + kGAMECLASS.spellCost(30) > 100)) {
            ...
            // [Yes][No]
            return { yes: yesBurnDatClit, no: corruptKaijuInsertion };
        }
        else {
            return { next: corruptKaijuInsertion };
        }
    }
    else {
        ...
        kaijuFlags.KAIJU_DISABLED = 1;
        return { next: Scenes.camp.returnToCampUseOneHour };
    }
}

// [If yes]
function yesBurnDatClit(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
    // fatigue(30, 1);
    player.stats.fatigueMagic(30);
...
function corruptKaijuInsertion(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
...
// [If compliment]
function complimentKaiju(): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
    ...
    return {
        choices: [
            ["Let Her", letKaijuHaveWayWithYou],
            ["Stop It", stopItPlease],
        ]
    };
}

// [If stop it]
function stopItPlease(): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
...
// [If let her]
function letKaijuHaveWayWithYou(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
...
// [If flirt]
function flirtWithKaiju(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
    ...
    if (kaijuCock()) {
        ...
        if (player.gender > 0) {
            DisplayText("\n\nYou grind your functions against her member, determined to get yourself off as well.");
...
function repeatKaijuEncounter(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
    ...
    let giveCock: ClickFunction;
    // [If PC has 15 incubus drafts (regular or pure) and Venus is not already a herm]
    if (
        (
            player.inventory.items
                .filter(Inventory.FilterName(ConsumableName.IncubusDraft))
                .reduce(Inventory.TotalQuantity, 0) +
            player.inventory.items
                .filter(Inventory.FilterName(ConsumableName.IncubusDraftPure))
                .reduce(Inventory.TotalQuantity, 0)
        ) >= 10 && !kaijuCock()
    ) {
        ...
        giveCock = yesTurnKaijuFuta;
    }
    // [Peek][Hug Boobs][Fuck][Urethra Fuck (only if Venus is a herm)][Talk][Leave]
    return {
        choices: [
            ["Peek", peekAtSomePhatAssKaijuButt],
            ["Hug Boobs", kaijuRepeatBoobHug],
            ["Fuck", fuckThisGiantYouDumbCunt],
            kaijuCock() ? ["UrethraFuck", urethraFuckDatGiantCock] : ["", undefined],
            ["Talk", talkToKaiju],
            ["Give Cock", giveCock],
        ],
        persistantChoices: [["Leave", leaveRepeatKaiju]]
    };
}
// [If Hug Boobs]
function kaijuRepeatBoobHug(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
    ...
    DisplayText("\n\n\"<i>My, that was a mighty big hug.  I think I'll need another nap. Come visit me again some time,</i>\" she says, slowly putting you down in your boat.  You slowly roll away as the jolly green giantess giggles and slowly wades off.");
    // [Giant turtle girl now encounter-able at Boat and Swamp, corruption increases slightly]
    kaijuFlags.KAIJU_BAD_END_COUNTER++;
    return { next: Scenes.camp.returnToCampUseOneHour };
}

// [If Fuck]
function fuckThisGiantYouDumbCunt(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
    ...
    if (player.stats.sens >= 50) {
        ...
        if (kaijuCock()) {
            ...
            // [If vagina and/or cock is present]
            if (player.gender > 0) {
                DisplayText("You grind your functions against her member, determined to get yourself off as well.  ");
            }
            // [If PC has boobs]
...
// [If Urethra Fuck]
function urethraFuckDatGiantCock(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
...
// [If Leave]
function leaveRepeatKaiju(): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
...
// [If Talk]
function talkToKaiju(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
    kaijuFlags.KAIJU_TALK_CYCLE++;
    kaijuFlags.KAIJU_BAD_END_COUNTER--;
    ...
    if (kaijuFlags.KAIJU_TALK_CYCLE === 1) {
        // Dialog one:
    ...
    // Dialog two:
    else if (kaijuFlags.KAIJU_TALK_CYCLE === 2) {
    ...
    else if (kaijuFlags.KAIJU_TALK_CYCLE === 3) {
        // Dialog three:
...
// [If Peek]
function peekAtSomePhatAssKaijuButt(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
    kaijuFlags.KAIJU_BAD_END_COUNTER++;
    ...
    // [Lust is increased]
    player.stats.lust += 10 + player.stats.lib / 3;
    // [If Venus is corrupt/herm]
    if (kaijuCock()) {
        DisplayText("\n\nThough she appears to be ending the show, you know you can encourage her to go even further.  Do you egg her on?");
        // [Yes][No]
        return { yes: yesKaijuGimmePeepShowsMoar, no: noKaijuPeepShows };
    }
    else {
        return { next: noKaijuPeepShows };
    }
}

// [If yes]
function yesKaijuGimmePeepShowsMoar(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
    kaijuFlags.KAIJU_BAD_END_COUNTER++;
...
function noKaijuPeepShows(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
...
function kaijuGrowsWangus(): NextScreenChoices {
    kaijuFlags.KAIJU_COCK = 1;
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
    kaijuFlags.KAIJU_BAD_END_COUNTER++;
    ...
    return {
        choices: [
            ["Yes", helpNewFutaKaijuGetOff],
            ["No", dontGetFutaTurtlesOffToday],
            ["Mock", mockDatTurtleGirl],
        ]
    };
}

// [If no]
function dontGetFutaTurtlesOffToday(): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
...
// [If Mock]
function mockDatTurtleGirl(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
...
// [If yes]
function helpNewFutaKaijuGetOff(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
    kaijuFlags.KAIJU_BAD_END_COUNTER++;
...
// [If Incubi Drafts]
function incubiDraftsDatKaiju(): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
    ...
    return { yes: yesTurnKaijuFuta, no: repeatKaijuEncounter };
}

// [If Yes]
function yesTurnKaijuFuta(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
    kaijuFlags.KAIJU_BAD_END_COUNTER += 2;
    let counter: number = 10;
    while (counter > 0) {
        counter--;
        if (player.inventory.items.has(ConsumableName.IncubusDraftPure)) player.inventory.items.consumeItem(ConsumableName.IncubusDraftPure);
        else if (player.inventory.items.has(ConsumableName.IncubusDraft)) player.inventory.items.consumeItem(ConsumableName.IncubusDraft);
    }
...
function kaijuBadEndToyBOOSH(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
    ...
    if (player.canFly()) {
        ...
        return { yes: flyAwayFromBadEnd, no: badEndPartTwo };
    }
    else {
        return { next: badEndPartTwo };
    }
}

function flyAwayFromBadEnd(): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
...
// [End wings]
function badEndPartTwo(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Venus);
    ...
    if (player.torso.neck.head.face.tongue.type === TongueType.SNAKE) DisplayText("  Your snaky forked tongue begins lashing about, flicking at the nipple tip in your mouth while rubbing at the underside of it, urging the tender spot to give up a drop of the heavenly white liquid.");
    // [If PC has demonic/dragon tongue]
    else if (player.torso.neck.head.face.tongue.type > TongueType.HUMAN) {
