...
export interface MinotaurMobFlags {
    TIMES_MET: number;
    MINOTAUR_TRIBE_SIZE: number;
    NUMBER_OF_SONS_PENDING: number;
    GROWNUP_COUNTDOWN: number;
}

const minotaurMobFlags = {
    TIMES_MET: 0,
    MINOTAUR_TRIBE_SIZE: 0,
    NUMBER_OF_SONS_PENDING: 0,
    GROWNUP_COUNTDOWN: 0,
};

User.flags.set(FlagType.MinotaurMob, minotaurMobFlags);

export class MinotaurMobScene implements ITimeAware {
    ...
    public timeChange(): boolean {
        ...
        if (minotaurMobFlags.GROWNUP_COUNTDOWN > 30) minotaurMobFlags.GROWNUP_COUNTDOWN = 30;
        if (minotaurMobFlags.GROWNUP_COUNTDOWN < 0) minotaurMobFlags.GROWNUP_COUNTDOWN = 0;
        // Countdown for son growing up
        if (minotaurMobFlags.GROWNUP_COUNTDOWN > 0) {
            minotaurMobFlags.GROWNUP_COUNTDOWN--;
            // Hit zero, move kid to grown up pile!
            if (minotaurMobFlags.GROWNUP_COUNTDOWN <= 0 && minotaurMobFlags.NUMBER_OF_SONS_PENDING > 0) {
                minotaurMobFlags.NUMBER_OF_SONS_PENDING--;
                minotaurMobFlags.MINOTAUR_TRIBE_SIZE++;
            }
        }
        // NEXT KID!
        if (minotaurMobFlags.NUMBER_OF_SONS_PENDING > 0 && minotaurMobFlags.GROWNUP_COUNTDOWN === 0) {
            minotaurMobFlags.GROWNUP_COUNTDOWN = 30;
        }
        return false;
    }
...
export function minoCumAddiction(raw: number = 10): void {
    const playerFlags = User.flags.get<PlayerFlags>(FlagType.Player);
    // Increment minotaur cum intake count
    playerFlags.UNKNOWN_FLAG_NUMBER_00340++;
    // Fix if variables go out of range.
    if (playerFlags.MINOTAUR_CUM_ADDICTION_TRACKER < 0) playerFlags.MINOTAUR_CUM_ADDICTION_TRACKER = 0;
    if (playerFlags.MINOTAUR_CUM_ADDICTION_STATE < 0) playerFlags.MINOTAUR_CUM_ADDICTION_STATE = 0;
    if (playerFlags.MINOTAUR_CUM_ADDICTION_TRACKER > 120) playerFlags.MINOTAUR_CUM_ADDICTION_TRACKER = 120;

    // Turn off withdrawal
    // if(playerFlags.MINOTAUR_CUM_ADDICTION_STATE > 1) playerFlags.MINOTAUR_CUM_ADDICTION_STATE = 1;
    // Reset counter
    playerFlags.TIME_SINCE_LAST_CONSUMED_MINOTAUR_CUM = 0;
    // If highly addicted, rises slower
    if (playerFlags.MINOTAUR_CUM_ADDICTION_TRACKER >= 60) raw /= 2;
    if (playerFlags.MINOTAUR_CUM_ADDICTION_TRACKER >= 80) raw /= 2;
    if (playerFlags.MINOTAUR_CUM_ADDICTION_TRACKER >= 90) raw /= 2;
    // If in withdrawl, readdiction is potent!
    if (playerFlags.MINOTAUR_CUM_ADDICTION_STATE === 3) raw += 10;
    if (playerFlags.MINOTAUR_CUM_ADDICTION_STATE === 2) raw += 5;
    raw = Math.round(raw * 100) / 100;
    // PUT SOME CAPS ON DAT' SHIT
    if (raw > 50) raw = 50;
    if (raw < -50) raw = -50;
    playerFlags.MINOTAUR_CUM_ADDICTION_TRACKER += raw;
    // Recheck to make sure shit didn't break
    if (playerFlags.MINOTAUR_CUM_ADDICTION_TRACKER > 120) playerFlags.MINOTAUR_CUM_ADDICTION_TRACKER = 120;
    if (playerFlags.MINOTAUR_CUM_ADDICTION_TRACKER < 0) playerFlags.MINOTAUR_CUM_ADDICTION_TRACKER = 0;
}

export function minotaurAddicted(player: Character): boolean {
    return player.perks.has(PerkType.MinotaurCumAddict) || User.flags.get<PlayerFlags>(FlagType.Player).MINOTAUR_CUM_ADDICTION_STATE >= 1;
}

export function minotaurNeed(): boolean {
    return User.flags.get<PlayerFlags>(FlagType.Player).MINOTAUR_CUM_ADDICTION_STATE > 1;
}

export function meetMinotaurSons(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minotaur_Sons);
    player.stats.lust += 10;
    minotaurMobFlags.TIMES_MET++;
    // First Meeting
    if (minotaurMobFlags.TIMES_MET === 1) {
        // (Non-Addicted)
        if (!minotaurAddicted(player)) {
            DisplayText("You explore the hilly, almost cliff-like high mountains, searching for the next fight, item, or discovery that will put you closer to your goal.  The hot sun beats down on your back like a baleful flame, seeming to cook you in your " + player.inventory.equipment.armor.displayName + " with every step you take.  You raise an arm to mop away the sweat that streams down your brow, but as you lower it, you're confronted with a most unwelcome sight.\n\n");
            ...
            DisplayText("You ready your " + player.inventory.equipment.weapon.displayname + " and adopt a fighting pose, muscles already quivering from the intensity of the lust-inducing, mind-numbing musk of so many minotaurs packed so closely together.  They aren't the first aphrodisiac-exuding beast-men you've dealt with, and though you've ");
            if (player.stats.cor < 33) DisplayText("enjoyed their affections more than you ought to");
            else if (player.stats.cor < 66) DisplayText("indulged yourself in their glorious cum more than a few times");
            else DisplayText("never felt like turning down a fuck from one of these glorious cum-dispensing fuck-beasts");
            DisplayText(", these brutes will find you no easy prey.  Trembling slightly from the adrenaline now coursing through your veins, you hold fast, but they merely stare at you, not even brandishing weapons.  Irritated, you bark out a challenge, hoping to goad them into making a foolish mistake.\n\n");

            DisplayText("\"<i>Looks like Mom doesn't recognize us,</i>\" says the biggest of the three, between deep, rumbling chuckles.\n\n");

            DisplayText("The next-largest sibling smirks and remarks, \"<i>You would think she'd at least know her children when she met them.  Maybe our dads smacked her around a little too hard?</i>\"  After a moment to let his comment sink in, your jaw drops.\n\n");

            DisplayText("Now that you think about it, the fur patterns of ");
            if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE > 3) DisplayText("some of ");
            DisplayText("the minotaurs you've birthed match these minotaurs' coats perfectly – these bull-men must be your progeny, all grown up.  The small one haltingly asks, \"<i>S-since she looks brain dead... will you guys let me use her cunt this time?  I never get to have any pussy!</i>\"\n\n");

            DisplayText("You can't help but laugh with a mixture of maternal pride and rational worry.  What do your offspring have planned for you?  Judging by the three stiffening loincloths, they won't hold back much longer.  What will you do?");
            // [Fight] [Submit] [Negotiate] [Run]
            return {
                choices: [
                    ["Fight", fightOTaurs],
                    ["Submit", submitToMinotaurMob],
                    ["Negotiate", negotiate],
                ],
                persistantChoices: [["Leave", runFromMinotaurs]]
            };
        }
        // (Addicted)
        else {
            ...
            DisplayText("You start to ready your " + player.inventory.equipment.weapon.displayname + " and adopt a fighting pose, wobbling dangerously close to one of the virile, drug-producing specimens before you.  They aren't the first aphrodisiac-exuding beast men you've dealt with, not by a long shot, but part of you just can't wait to see how these ones will taste. Trembling slightly from the overwhelming, addiction-induced need coursing through your veins, you try to restrain yourself and fight the good fight. They simply stare at you, not even bothering to brandish weapons.  Gods, they smell so good!  Why haven't they tried to rape you yet!?  You absentmindedly tease one of your nipples while you wait for the naturally dominant bull-men to make their move.\n\n");

            DisplayText("\"<i>Looks like Mom doesn't recognize us,</i>\" says the biggest of the three in between deep, rumbling chuckles.\n\n");

            DisplayText("The next largest sibling smirks and remarks, \"<i>You would think she'd at least know her children when she met them.  Maybe she's craving cum too much to remember how to talk?  Is that what it is?  Is Mommy a cum-craving whore?</i>\"\n\n");

            DisplayText("You gape, the desire to satiate your cravings replaced momentarily by the raw shock of the realization.\n\n");

            DisplayText("Now that you look at think about it, however, the fur patterns of these minotaurs <b>do</b> match ");
            if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE > 3) DisplayText("some of ");
            DisplayText("those of your offspring perfectly... these minotaurs must be your progeny, all grown up.  The small one asks, \"<i>S-since she's already so cum-crazy, can I use her cunt this time?  She's probably all worn out from birthing us, so I bet it's her worst hole.  You guys don't mind, right?</i>\"\n\n");

            DisplayText("Judging by the three stiffening loincloths and subsequent moistening of bulges, you'll have a very hard time turning down their offer.  ");
            if (player.stats.cor < 33) DisplayText("You wouldn't fuck your own offspring just for another hit of cum... would you?");
            else if (player.stats.cor < 66) DisplayText("You're not sure how to feel about this morally, but you can't keep from licking your lips, fantasizing about the fix these wonderful kids of yours are so willing to give you.");
            else DisplayText("Who cares if they're your offspring?  They're all grown up and they smell so... appetizing.");
            // [Fight] [Submit] [Run]
            return {
                choices: [
                    ["Fight", fightOTaurs],
    ...
    // Repeat meetings
        ...
        // Non tribe size
        if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE < 20) {
            // [Non Addicted Repeat]
            if (!minotaurAddicted(player)) {
                DisplayText("It doesn't take long for your exploration into the mountains to bring you face-to-face with your minotaur offspring once again.  There're " + numToCardinalText(minotaurMobFlags.MINOTAUR_TRIBE_SIZE) + " of them here, arranged around you in a loose semicircle.  You can smell their familiar and arousing odor in the air; while it brings a tingle to your nethers, you're determined to resist it.  ");
                if (player.torso.vaginas.get(0).wetness <= 2) DisplayText("A trickle of wetness runs down your " + Desc.Leg.describeLeg(player) + " almost immediately.");
                ...
                if (player.pregnancy.womb.isPregnant() && player.pregnancy.womb.pregnancy.type === PregnancyType.MINOTAUR) DisplayText("Pregnant with another brother for us, and already you come back for more?");
                else if (player.pregnancy.womb.isPregnant()) DisplayText("Huh... Looks like Mommy couldn't keep her legs closed, and she got knocked up by some vermin.  You're such a disappointment to your beloved family.  Don't worry, though, mom, we'll still fuck you.");
                else DisplayText("Hey mom, come back so we could all have another whack at you?  We could use another brother.");
                DisplayText("</i>\"\n\n");

                DisplayText("\"<i>Nah, she's just back to finish getting addicted, aren't you mother-dear?  You came back to visit your sons and guzzle our cum until you're scooping the dregs of it from your ");
                if (!player.pregnancy.womb.isPregnant()) DisplayText("soon-to-be ");
                DisplayText("knocked up puss,</i>\" taunts another minotaur.\n\n");

                DisplayText("The runt of the litter lurks in the back, stroking himself while whining and plaintively muttering for his turn.  It would be pathetic if you couldn't smell the pre-cum he's dripping from all the way over here.\n\n");

                DisplayText("What do you do?");
            }
            // [Addicted Repeat]
            else {
                DisplayText("It doesn't take too long for your exploration into the mountains to bring you face-to-face with your minotaur offspring once again.  There're " + numToCardinalText(minotaurMobFlags.MINOTAUR_TRIBE_SIZE) + " of them here, arranged around you in a loose semicircle.  You can smell their familiar, arousing odor drifting around you, and though you want to be strong, there's nothing you want more than to bury your face beneath those loincloths and start slurping your reward for having such wonderful children.  ");
                if (player.torso.vaginas.get(0).wetness <= 2) DisplayText("A trickle of wetness runs down your " + Desc.Leg.describeLeg(player) + " almost immediately.");
                else if (player.torso.vaginas.get(0).wetness <= 4) DisplayText("A steady flow of your juices soaks your " + Desc.Leg.describeLegs(player) + " almost immediately, just from the scent invading your nostrils.");
                else DisplayText("A flood of wetness soaks your " + player.inventory.equipment.armor.displayName + " and " + Desc.Leg.describeLegs(player) + " with feminine drool almost immediately as your body reflects the obscene lust coursing through your veins.");
                ...
                if (player.pregnancy.womb.isPregnant() && player.pregnancy.womb.pregnancy.type === PregnancyType.MINOTAUR) DisplayText("Pregnant with another brother for us, and you're already back for another fix?  Why even leave in the first place?");
                else if (player.pregnancy.womb.isPregnant()) DisplayText("Huh... you couldn't keep your legs closed, eh, mom?  Looks like you're all knocked up with vermin, too!  You're a disappointment to our family... don't worry, though, we'll still give you a fix.");
                else DisplayText("Hey, mom!  You come back so we could have another go at knocking you up?  Heh.  Knowing you, you're probably just here for another cum fix.");
                DisplayText("</i>\"\n\n");
                ...
                DisplayText("The runt of the litter, the 'minitaur', lurks in the back of the group, stroking himself and protesting timidly about how it was 'his turn'.  He leans around his two giant brothers and whines, \"<i>Mom, can I use your pussy?  I'm sooo pent-up and my ass is still sore from big bro!</i>\"\n\n");

                if (minotaurNeed()) {
                    DisplayText("Your mind is already fogging under the intense need of your addiction, and you do the only thing a junkie like you can do in this situation – drop down on all fours and get ready to be used.");
                    // Same as 'submit'
                    return { next: submitToMinotaurMob };
                }
                DisplayText("Your mind is fogging from the scent in the air, but thankfully, you aren't in withdrawal right now.  You can try to resist and maybe even turn the tables on your brood, or you can run.");
            }
            // [Fight] [Submit] [Run]
            return {
                choices: [
                ...
                persistantChoices: [["Run", runFromMinotaurs]]
            };
        }
        // Tribe sized
        else {
            ...
            if (!minotaurAddicted(player)) {
                ...
                DisplayText("A bemused snort rouses you from the day-dream, and you're now face-to-face with the largest of your sons once again.  He sneers and grunts, \"<i>Hey Mom... ");
                if (!player.pregnancy.womb.isPregnant()) DisplayText("Come back to add to the tribe?");
                else if (player.pregnancy.womb.pregnancy.type === PregnancyType.MINOTAUR) DisplayText("Come back to show off the lil' bro yer growin'?");
                ...
                else if (player.torso.vaginas.get(0).wetness >= 3) DisplayText("You can feel your juices dribbling out from under your " + player.inventory.equipment.armor.displayName + ", fed by your body's primal response to the pheromones in the air.");
                else DisplayText("You can feel your " + player.inventory.equipment.armor.displayName + " clinging to your sopping nethers, the liquid need only growing from the pheromones in the air.");
                DisplayText("  Many of the loincloths are wet, practically dripping with the minotaurs' copious pre-cum.  You can see the curves and swells of their huge, bloated balls - it looks like some of these poor guys have saved up quite a big load for you.  A fantasy of losing yourself in a sea of minotaur spooge crosses your mind...\n\n");

                DisplayText("No! ");
                if (player.stats.cor < 33) DisplayText("You can't think that way! Sure, you've birthed minotaurs enough to create this huge tribe of horny monsters, but you're a champion!");
                else if (player.stats.cor < 66) DisplayText("You shouldn't think that way! Sure, you've had enough minotaurs inside you to spawn this tribe of muscular beast-men, but you've got a mission to focus on.");
                else DisplayText("You shouldn't think that way! There's a whole bevy of other creatures out there waiting to be fucked over, and if your boys want a piece of you, they're the ones who should be kneeling down and getting ready to be fucked!");
                DisplayText("  What do you do?");
            }
            // [Repeat Tribe-Size Addict]
                ...
                DisplayText("A bemused snort rouses you from the daydream, and you come face to face with the largest of your sons once again.  He smirks and grunts, \"<i>Hey Mom.  ");
                if (!player.pregnancy.womb.isPregnant()) DisplayText("Come for a baby? Or do you just want to be spunk-stuffed?");
                else if (player.pregnancy.womb.pregnancy.type === PregnancyType.MINOTAUR) DisplayText("Come back for some more spunk? I bet your next son comes out as a spooge-slurping minitaur.  We could use another cock-sleeve around here.");
                ...
                if (minotaurNeed()) {
                    DisplayText("This is perfect... you need it so bad!  You drop down on all fours and wiggle your child-bearing hips, hoping they'll hurry and cum inside you.");
                    // SUBMIT!
                    return { next: submitToMinotaurMob };
                    return;
                }
                DisplayText("This is glorious – so many horny, willing boys, all here for you to squeeze dry.  Even if you wanted to leave, you'd have to fight them to make an opening.  It'd be better to just beat them into submission and take your time savoring their wondrous spunk.");
            }
            // [Fight] [Submit]
            return {
                choices: [
...
function negotiate(): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minotaur_Sons);
    ...
    DisplayText("The second largest smirks and retorts, \"<i>Chill out, Mom... it'll be fine.  I know what your problem is.  You just haven't had enough cum yet... trust me, you'll love it!  Once we've each blown a few loads in your holes, you won't be able to get enough!  Promise!!</i>\"\n\n");

    DisplayText("\"<i>Heh.  You ready to fuck and suck until you give me another bitch-boy brother to fuck when you aren't around?</i>\" asks the burliest brother.\n\n");

    DisplayText("The meek, quiet 'minitaur' speaks last. \"<i>P-Please mom?  I've never cum in a real pussy before...</i>\"\n\n");

    DisplayText("Looks like they're only interested in one thing.");
    // [Fight] [Submit] [Run]
    return {
        choices: [
            ["Fight", fightOTaurs],
...
// [Run]
function runFromMinotaurs(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minotaur_Sons);
    DisplayText("You turn tail and try to run, ");
    if (player.canFly()) DisplayText("leaping from the small cliffs to take flight");
    else DisplayText("fleeing down the mountainside");
    DisplayText(".  ");
    // ESCAPE!
    if ((player.canFly() && player.stats.spe > randInt(40)) || (!player.canFly() && player.stats.spe > randInt(60))) {
        DisplayText("A furry arm nearly catches your " + Desc.Leg.describeLeg(player) + ", but you slip free and quickly escape your lusty brood.");
        return { next: Scenes.camp.returnToCampUseOneHour };
    }
    // FAIL:
    else {
        DisplayText("A furry arm catches hold of your " + Desc.Leg.describeLeg(player) + " and yanks on it hard enough to slam you into the cliff face.  Disoriented and bleeding, you wobble free, dazed.  It's a fight!");
        // go to combat
        return fightOTaurs(player);
    }
}
// [Fight]
function fightOTaurs(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Minotaur_Sons);
    return CombatManager.beginBattle(player, [], [new MinotaurMob()]);
}

// [Submit]
function submitToMinotaurMob(player: Character): NextScreenChoices {
    player.stats.lust = 100;
    player.stats.lust += 1;
    DisplaySprite(SpriteName.Minotaur_Sons);
    return minotaurDeFeet(player);
}

function minotaurDeFeet(player: Character): NextScreenChoices {
    DisplayText().clear();
    ...
    if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE >= 25 && minotaurNeed()) return minotaurGangBadEnd(player);
    else if (player.pregnancy.womb.isPregnant()) {
        if (player.pregnancy.womb.pregnancy.incubation > 120) return analSpearSemiPregMinotaurGangbang(player);
        else return loseToMinoMobVeryPregnant(player);
    }
    else return nonAddictMinotaurGangBang(player);
}

// [Loss – Non-Addict Gangbang]
function nonAddictMinotaurGangBang(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Minotaur_Sons);
    ...
    if (player.torso.hips.legs.isTaur()) {
        DisplayText("A sudden, forceful push rolls your equine body onto its flank, and a larger member is pressed against your clutching ");
        if (player.torso.tails.count === 0) DisplayText("asshole");
        ...
        DisplayText("A sudden, powerful yank lifts one of your legs high into the air, and you feel a larger member pressing against your clutching ");
        if (player.torso.tails.count === 0) DisplayText("asshole");
        else DisplayText("tailhole");
        DisplayText(".  ");
    }
    ...
    else if (player.torso.vaginas.get(0).wetness >= 3) DisplayText("drools a thick stream of lubricants to puddle on the ground with each thrust.");
    else DisplayText("drips lubricant with every thrust, leaving tiny blotches of wetness on the ground.");
    ...
    if (!player.pregnancy.womb.isPregnant()) DisplayText("I hope you don't mind me making another brother.");
    else if (player.pregnancy.womb.pregnancy.type === PregnancyType.MINOTAUR) DisplayText("I hope you don't mind me saying hi to my little brother.");
    ...
    if (player.torso.cocks.count > 0) {
        ...
        else DisplayText("a constant flow of cum that utterly soaks the minotaur's fur and balls with your wasted, inferior seed.");
    }
    DisplayText("\n\n");
    ...
    DisplayText("The minitaur stays inside you, reaching to rub your puffy, red nether-lips as he goes soft in your depths.  The hesitation gives his cum time to absorb through the lining of your intestine.  Once he pulls out, the world is spinning around you, but you try to crawl to ");
    if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE <= 3) DisplayText(" the minotaur that gave you your first dose.  Maybe he's ready to give you more? He smirks and slaps you across the face with his soft shaft.  You giggle drunkenly and start to lick his balls - they HAVE to have more for you!  ");
    else {
        DisplayText(" a minotaur that hasn't had his chance to take you yet.  The beast laughs and smiles, pulling you to your knees.  ");
        // (1-2 more end paragraph)
        if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE <= 5) DisplayText("You open your mouth wide, licking your lips until they're shiny and inviting enough for the stud to fuck.  He does not disappoint.  The fat head pushes past your puckered cock-suckers and slides into your throat, the passage eased by the leavings of the one before him.  You sigh happily and begin to suck his cock like a lollipop, though all you want is his creamy center.  The others crowd around, touching themselves and waiting for another turn.  ");
        else DisplayText("You open your mouth wide, licking your lips until they're shiny and inviting enough for the stud to fuck.  He does not disappoint.  The fat head pushes past your puckered cock-suckers and slides into your throat, the passage eased by the leavings of the one before him.  You sigh happily and begin to suck his cock like a lollipop, though all you want is his creamy center.  Two more swollen fuck-sticks find their way to your waiting holes, and you giggle in dizzy bliss when you're packed full of your sons spunk once again.  ");
    }
    DisplayText("You black out at that point, but when you wake up soaked in cum with a bottle of it next to you, you know it was a good night.");
    // Preggers chance!
    player.pregnancy.womb.knockUp(new Pregnancy(PregnancyType.MINOTAUR, IncubationTime.MINOTAUR), 75);
    player.orgasm();
    player.stats.spe += -.5;
    player.stats.int += -.5;
    player.stats.lib += .5;
    ...
    minoCumAddiction(20);
    DisplayText("\n\n");
    return player.inventory.items.createAdd(player, ItemType.Consumable, ConsumableName.MinotaurCum, Scenes.camp.returnToCampUseEightHours);
}

// *[Loss - Very Pregnant, Lots of BJs]
function loseToMinoMobVeryPregnant(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Minotaur_Sons);
    DisplayText("The largest stud grunts, \"<i>Heh.  Mom's body looks like it's about to pop.</i>\"  His brother pushes him aside and rubs his furred hand across the taut " + Desc.Skin.skin(player) + " of your belly as he says, \"<i>");
    if (player.pregnancy.womb.pregnancy.type !== PregnancyType.MINOTAUR) DisplayText("She doesn't know how to keep her hungry twat closed.  You can see the vermin squirming in her belly.  Disgusting.");
    ...
    DisplayText("You frown in ");
    if (player.stats.cor < 33) DisplayText("irritation at being addressed so insultingly");
    else if (player.stats.cor < 66) DisplayText("annoyance at their familiarity");
    else DisplayText("irritation at losing control of the situation");
    ...
    if (player.pregnancy.womb.pregnancy.type !== PregnancyType.MINOTAUR) DisplayText("isn't");
    ...
    DisplayText("You look up and oogle the " + numToCardinalText((minotaurMobFlags.MINOTAUR_TRIBE_SIZE - 1)) + " dicks hovering just inches away from you.  Even the minitaur is there, his smaller dick dripping the sweet cream of his need at the sight of his slutty, utterly debased mother.  It turns you on so much, even more than the cream in your belly.  You reach out and grab two of the dicks to stroke, lubricating fingers with hot, wet pre-cum.");
    if (
        player.torso.hips.legs.isNaga() ||
        player.torso.tails.find(Tail.FilterType(TailType.DEMONIC)) ||
        player.torso.tails.find(Tail.FilterType(TailType.COW)) ||
        player.torso.tails.find(Tail.FilterType(TailType.SHARK)) ||
        player.torso.tails.find(Tail.FilterType(TailType.LIZARD)) ||
        player.torso.tails.find(Tail.FilterType(TailType.KANGAROO))
    ) DisplayText("  Even your tail reaches up to curl around a third, stroking the thick meat.");
    DisplayText("\n\n");
    ...
    DisplayText("\"<i>Good bitch,</i>\" he proclaims as the last few spurts soak you.  Once done, he turns and saunters away, calling out, \"<i>Next!</i>\"\n\n");
    ...
    if (player.pregnancy.womb.pregnancy.type === PregnancyType.MINOTAUR) DisplayText("That baby is probably mine too.  You let your son father his brother.");
    else DisplayText("That baby isn't even the right species.  You probably get knocked up by every male you meet, but you always come back to suck off your sons.");
    DisplayText("  How does that make you feel, whore?</i>\" asks the mouth-filling dick's owner.\n\n");

    DisplayText("You hum, \"<i>good,</i>\" though it comes out as more of a, \"<i>mpppphhhhhh.</i>\"  Your drug-addled vision is all bubbles and synaesthetic rainbows at this point.  He chuckles malevolently when another minotaur resumes slapping your ass.  The minotaur in the driver's seat grabs your ");
    if (player.torso.neck.head.horns.amount > 0) DisplayText("horns");
    ...
    DisplayText("The frantic face-humping comes to a final, explosive end with an eruption of stringy seed falling into your already-full belly.  This beast-man withdraws coolly.  He pulls out quickly enough that the bulk of his cum is added to your frosted face and " + Desc.Head.describeHair(player) + ".  Much of it falls down to soak your " + Desc.Breast.describeChest(player) + " and huge, pregnant belly.  You look more like a creme-covered doughnut than a person at this point, and you nearly climax from the mental picture of your huge, pregnant form lying in a puddle of minotaur goo while the beast-men use your face as a cum-receptacle.\n\n");
    ...
    // (short)
    if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE < 8) {
        ...
        DisplayText("The minitaur moans, \"<i>T-Thanks... ohhh... is it okay if I umm - ooooh - cum now?</i>\"\n\n");
        ...
        if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE > 3) {
            if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE === 4) DisplayText("Sorry bro, it looks like Mom's done.  You'll have to snag a shot at her later - she's pretty fucking pregnant, and I don't want her broken.");
            else DisplayText("Sorry bros, it looks like Mom's done.  You'll have to snag a shot at her later - she's pretty fucking pregnant, and I don't want her broken.");
        }
        else DisplayText("Damn, that was nice.  Who would've guessed a slut as pregnant as Mom would be such a great cock-guzzler?");
        DisplayText("</i>\"");
    }
    // (Long)
    else {
        ...
        if (player.pregnancy.womb.pregnancy.type === PregnancyType.MINOTAUR) DisplayText("we wouldn't want the next member of our tribe injured.");
        else DisplayText("we wouldn't want to ruin our favorite toy would we?");
        DisplayText("  Look at her, she's smiling with a dick in her mouth while she passes out.  Mom's the best.</i>\"");
    }
    player.orgasm();
    player.stats.spe += -.5;
    player.stats.int += -.5;
...
// SEMI PREG
// > 120 && <= 216
function analSpearSemiPregMinotaurGangbang(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Minotaur_Sons);
    ...
    DisplayText("The biggest brute slams the lesser beast away and gropes at your tits, grunting, \"<i>Who cares?  She's still a hot piece of ass.</i>\"  None-too-gentle in his rough handling");
    if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 1.5) DisplayText(", you're soon expressing drops of milk into his squeezing mitts.");
    else DisplayText(", you're soon moaning and squirming from his crude caresses.");
    DisplayText("  Soon, the turgid length of his horse-like cock is prodding your pregnant tummy, lubing it with narcotic spooge that you find yourself wishing was in your mouth or pussy.\n\n");

    DisplayText("A nervous voice interrupts the brute's groping.  \"<i>Umm, g-g-guys... we should be careful with her.  You guys want ");
    if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE >= 20) DisplayText("to make the tribe even bigger");
    else DisplayText("more brothers");
    ...
    if (player.pregnancy.womb.pregnancy.type === PregnancyType.MINOTAUR) DisplayText("our little brother");
    else DisplayText("her baby-maker");
    DisplayText(".</i>\"\n\n");

    DisplayText("One of the minotaurs grabs your jaw and opens it, swiftly inserting his drooling phallus into your maw.  \"<i>Well we can still plug her gob and her ass.  Dibs on her lips.</i>\"\n\n");
    ...
    if (!minotaurAddicted(player)) DisplayText("We've got to get mom so high on us that she'll float back to her camp and come crawling back to us tomorrow.");
    ...
    else if (player.torso.vaginas.get(0).wetness >= 2) DisplayText("trail dribbles of");
    else DisplayText("drip");
    DisplayText(" girl-juice everywhere.\n\n");

    // (nipcunts)
    if (player.torso.chest.find(BreastRow.FuckableNipples)) {
        DisplayText("Probing hands grasp your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s, exploring their surface.  Thin, feminine fingers slip inside your nippleholes, and a whispered exclaimation reaches your ears from somewhere below your brutally fucked form.\n\n");
        DisplayText("\"<i>Oh, wow...</i>\" says the voice of the minitaur.  \"<i>I w-wonder if I could use this...</i>\"\n\n");
        DisplayText("Sparing a glance downward, you see your smallest minotaur son looking furtively around for his brothers, a sheepishly guilty grin on his face as he touches his throbbing member with the hand not shoved halfway into your nipple.  He kneels down and points his cock upward, trying to guide it into your nipplecunt despite the violent heaving of your body as the two monstrous bull-men above rock you back and forth.  In one brief lull, he seizes his chance and jams his little flare into your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + ", almost immediately falling back onto his hands as the double-team gathers speed again and his cockhead is swirled and tweaked in your pendulating chest.\n\n");

        DisplayText("\"<i>O-ohh,</i>\" he moans, louder than before.  \"<i>Amazing, mom... you're... just a-mazing.</i>\"\n\n");

        DisplayText("Unfortunately, you're not the only one who can hear his rapt praise now.  The beast railing your mouth looks down and meets the eyes of the minitaur just as his head lolls back in pleasure.  Your eyes roll when you feel his cock jam further into your nipple as he starts in panic, but his big brother just ");
        if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE < 7) DisplayText("shakes his head.  \"<i>You're only saving up trouble for later, little cunt.  Your hole after Mom's, remember.</i>\"  The minitaur wistfully withdraws his cock and crawls a few paces off, where he sits in a sulk.");
        else DisplayText("nods his head toward one of the idle members of the tribe, who pulls the runt out of your nipplehole with a 'pop' and carries him off, the line of horny minotaurs parting and then forming up again after he passes.  From behind them you can hear squeals of agony and then ecstasy as your smallest son joins you in your state of overfucked, drugged euphoria.");
        DisplayText("\n\n");
    }
    // (Lactation, no nipcunts)
    else if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 1) {
        DisplayText("Wet lips clasp around a " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + " and gently suck at it, drawing a small trickle of milk from your leaky tit.  ");
        if (player.torso.chest.count * 2 === 2) DisplayText("Your ");
        else DisplayText("An");
...
function victoryMinotaurGang(player: Character, monster: Character): NextScreenChoices {
    ...
    // 	[win options]
    let getSuck;
    if (player.torso.cocks.count > 0) getSuck = { func: forceMinitaurToGiveOral, args: [1] };
    let nipFuck;
    ...
    return {
        choices: [
            ["Gangbang", { func: victoryAllThePenetrationsMinotaurGangBang, args: [monster] }],
            ["Tit-Fuck", titFuck],
            ["Nipple-Fuck", nipFuck],
            ["Get Licked", { func: forceMinitaurToGiveOral, args: [0] }],
            ["Get Sucked", getSuck],
            ["Discipline", disciplineEldestMinotaurSon],
        ],
        persistantChoices: [["Leave", Scenes.camp.returnToCampUseOneHour]]
    };
}
// *[Victory Tit-Fuck] (for only the fattest of fat bitch titties)
function victoryMinotaurGangTitFuck(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minotaur_Sons);
    ...
    DisplayText("\"<i>Y-Yes Mom,</i>\" rumbles the defeated beast.  You release him, watching as he slumps back and exhales a tired moo of relief.  The poor boy is so pent up, so needy, that you don't mind indulging him and his brothers a little bit.  ");
    if (minotaurAddicted(player)) DisplayText("Of course it helps that pleasing them will give you exactly what you want - more of that delicious, wondrous cock-cream.  ");
    DisplayText("\"<i>Fantastic.  You're good boys, I know you are.  Now come over here.  Yes, you too - all of you come to mommy.  I'm going to punish the lot of you.</i>\"\n\n");

    DisplayText("You lower yourself down ");
    if (player.torso.hips.legs.isBiped()) DisplayText("to your knees");
    else if (player.torso.hips.legs.isTaur()) DisplayText("on your forelegs");
    ...
    DisplayText("The first two 'boys' to get within arm's reach of you find themselves on the receiving ends of twin handjobs.  As they stamp their hooves in pleasure, they slowly get closer and closer, guided by your skilled, loving fingertips.  You coo in pleasure at sight of their leaky cocks, pulling them closer until they touch your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s and smear your chest with their needy pre-spunk.\n\n");
    ...
    DisplayText("You press your hands into the sides of your " + Desc.Breast.describeBreastSize(player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + " to mash the flesh tightly about the two lucky penises.  The minotaurs moo in delight, immediately slicking your " + Desc.Skin.skinFurScales(player) + " with another huge wave of pre-cum, and before they can adjust to the tightness of your chest, you begin to drag your weighty love-cushions up... and finally down.  You stroke their completely-enveloped cocks languidly, enjoying the complete control and dominance it's giving you over your herd.  Trembling mightily, they fight their bodies to be still for you.  Such good boys.\n\n");
    ...
    if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE >= 10) {
        ...
        DisplayText("The runt of your litter, the minitaur, is whining, \"<i>I-I'm sorry Mom but I just couldn't hold b-back anymore.  Ung... your mouth is so hot.  I can't control it!</i>\"  He grabs your ");
        if (player.torso.neck.head.horns.amount > 0) DisplayText("horns");
        else DisplayText("hair");
        DisplayText(" and pulls you down, ramming the fat flare of his dick into your throat a split second before he cums.  The salty deluge fills your throat to the brim before it rolls down into your stomach, adding the already full load inside you.  You hum in happiness as you're stuffed with spunk.  The added throat vibration keeps your femmy son spurting nice and long for you.\n\n");
        ...
        if (minotaurAddicted(player)) DisplayText("I'd hate to see you go mad with need around a lesser beast.");
        else DisplayText("This will be so much better once you learn to need it.");
        DisplayText("</i>\"\n\n");
    ...
    // (SMALL)
        ...
        if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE === 3) DisplayText("one minotaur");
        else DisplayText(numToCardinalText(minotaurMobFlags.MINOTAUR_TRIBE_SIZE - 2) + " minotaurs");
        ...
        if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE === 3) DisplayText("You grab the minitaur's cock and drag the shy beast-man over your cum-slicked tits, wasting no time in burying the shaft in mommy's mammaries, where it belongs.");
        else {
            DisplayText("You grab the minitaur and ");
            if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE === 4) DisplayText("the remaining brother");
            else DisplayText("one of the remaining brothers");
            DisplayText(", pulling them hard towards your cum-slicked tits.  You waste no time burying their twin members in the cum-slicked crevasse that is their mommy's mammaries, right where they belong.");
        }
        DisplayText("  \"<i>Good boys.  Now give Mommy more cum... she's ");
        if (player.perks.has(PerkType.BimboBrains) || player.perks.has(PerkType.FutaFaculties)) DisplayText("like, ");
        DisplayText("getting sooo wasted on it,</i>\" you command.\n\n");
        ...
        if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE > 3) DisplayText("His glaze spurts up along the opening created by his larger brother's shaft, and soon he too is cumming, obediently orgasming for you.  ");
        DisplayText("One of the previously sated brothers produces a bottle and places it in front of you, letting it fill with cum before he corks it off.  He grunts, \"<i>");
        if (!minotaurAddicted(player)) DisplayText("Maybe we can get Mom completely hooked.  Then we won't have to fight her?");
        else DisplayText("That'll be a nice snack for her later.  We wouldn't want her to lose control around lesser species.");
        DisplayText("</i>\"\n\n");

        DisplayText("You gather up a few more mouthfuls to swallow before stumbling to your feet.  There's a soup of semen swimming around your brain making it difficult to think, let alone walk!  Absentmindedly, you pick up the cum-filled jar.  You wouldn't want to forget that.  ");
        if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE < 5) DisplayText("The minotaurs let you get dressed while they sigh happily, clearly pleased with the outcome.");
        else {
            DisplayText("Before you get far, a strong arm grabs you.  Oh yeah... another son that needs punished.  You sink to your knees and tend to him");
            if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE > 5) {
                DisplayText(" and his brother");
                if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE >= 6) DisplayText("s");
            }
            DisplayText(".  The act passes in a blur, but you");
            if (!player.torso.hips.legs.isGoo()) DisplayText(" look like a white goo-girl");
            else DisplayText("r gelatinous body is filled with long slashes of white");
            DisplayText(" by the time you manage to leave.  You nearly forget your equipment, but grab it at the last second.");
        }
    }
    player.orgasm();
    player.stats.spe += -.5;
    ...
    minoCumAddiction(15);
    DisplayText("\n\n");
    return player.inventory.items.createAdd(player, ItemType.Consumable, ConsumableName.MinotaurCum, Scenes.camp.returnToCampUseOneHour);
}

// *[Victory - Double/Triple penetration]
function victoryAllThePenetrationsMinotaurGangBang(player: Character, monster: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minotaur_Sons);
    ...
    if (monster.stats.lust > 99) DisplayText(" nearly instantly, too aroused to turn down the idea of their mother doing ANYTHING with their hard shafts.");
    else DisplayText(" somewhat sluggishly as they work to get nude for you, despite their wounds.  As they finish, they start to move a bit faster, recovering from the battle quicker than you would have thought.");
    ...
    DisplayText("Within seconds a crowd has formed behind you.  Numerous hands squeeze and caress your " + Desc.Butt.describeButt(player) + ", a few even daring to press at the tight ring of your " + Desc.Butt.describeButthole(player.torso.butt) + ".  They jostle around a bit as they fight over the honor of penetrating your back door, but a winner is selected before you bore of the spectacle.  He grabs your " + Desc.Butt.describeButt(player) + " in both hands to aid in his efforts to align his cock with your pucker, squeezing softly.\n\n");

    DisplayText("The beast-man finally hits your anus with his tip and pushes forward.  ");
    if (player.analCapacity() < 40) DisplayText("Your " + Desc.Butt.describeButthole(player.torso.butt) + " is so tight that he's forced to push and release over and over.  The painful penetration drags on and on as he widens your hole to accept his monstrous cock.");
    else if (player.analCapacity() < 60) DisplayText("Your " + Desc.Butt.describeButthole(player.torso.butt) + " slowly widens to accept the cock that's being deposited into its depths.  You find yourself somewhat grateful that you've been stretched enough to prevent this from being painful.");
    else DisplayText("Your " + Desc.Butt.describeButthole(player.torso.butt) + " devours the cock easily.  The only thing even close to a tight fit is the feeling of your pucker being stretched at his very base.");
    ...
    if (player.torso.chest.find(BreastRow.FuckableNipples) && minotaurMobFlags.MINOTAUR_TRIBE_SIZE >= 4) DisplayText("  There's still more you could be doing.");
    DisplayText("\n\n");

    // (Nipplefuck PG:)
    if (player.torso.chest.find(BreastRow.FuckableNipples) && minotaurMobFlags.MINOTAUR_TRIBE_SIZE > 4) {
        ...
        DisplayText(" drips from your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s in sympathy with your packed pussy.  A wicked idea coalesces inside your lusty psyche as you ask, \"<i>Did you boys know about my OTHER pussies?  Why don't you try my breasts?</i>\"  ");
        if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE === 4) DisplayText("One stud assembles next to ");
        else if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE >= 5 && player.torso.chest.count * 2 === 2) DisplayText("Two studs assemble around ");
        else {
            let dicks: number = player.torso.chest.count * 2;
            if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE - 3 < dicks) dicks = minotaurMobFlags.MINOTAUR_TRIBE_SIZE - 3;
        ...
        if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE === 4) DisplayText("He excitedly fingers");
        else DisplayText("They excitedly finger");
        DisplayText(" your tits, slipping a few digits in to explore the depths of your chest-pussies.  ");
        if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE === 4) DisplayText("He licks his fingers clean, and a moment later, a cock is spearing one of your nipples, disappearing deep into the tingling passages concealed within your chest.\n\n");
    ...
    else DisplayText(" that you can barely hang on to your sanity, but it feels like all the minotaur-drug leaking into your bloodstream might wash away even that.");
    DisplayText("\n\n");
    ...
    if (player.torso.chest.find(BreastRow.FuckableNipples) && minotaurMobFlags.MINOTAUR_TRIBE_SIZE > 4) DisplayText("your nipples squirt, ");
    ...
    if (player.torso.chest.find(BreastRow.FuckableNipples) && minotaurMobFlags.MINOTAUR_TRIBE_SIZE > 4) {
        if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE > 4) DisplayText("Meanwhile, your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s get a similar feeling from the studs arranged around them");
        else DisplayText("Meanwhile, your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + " gets a similar feeling from the stud pumping away at it");
    ...
    if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE < 20) DisplayText("gang");
    else DisplayText("tribe");
    DisplayText(" smirks at you as you struggle to right yourself.\n\n");
    ...
    DisplayText("After getting dressed, you idly scoop a handful of cum from your sodden box and devour it - the perfect snack for the long walk home.");
...
function forceMinitaurToGiveOral(player: Character, choice: number = 0): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minotaur_Sons);
    ...
    DisplayText("The pathetic minitaur is hopelessly addicted to his brothers' cum.  You shrug, tired of his bitching and wanting the satisfaction you so desperately crave.  \"<i>Whining about your problems won't make them go away.  Now put that mouth to use and ");
    // (FORK BETWEEN CUNT/DICK VARIATIONS)
    // (CUNTPUSSIEFUCKER)
    if (choice === 0) {
        DisplayText("start licking Momma's pussy.</i>\"  As if to emphasize your point, you ");
        if (player.torso.hips.legs.isTaur()) {
            DisplayText("turn ");
            if (player.torso.tails.count > 0) DisplayText("and lift your tail ");
            DisplayText("to expose the glistening pinkness of your womanhood.");
        }
        // ELSE
        else {
        ...
        else DisplayText(", revealing its cute button self.");
        DisplayText("\n\n");
        ...
        if (player.torso.vaginas.get(0).wetness >= 5) DisplayText("  The wave of squirting fluids drips from his shaggy chin and runs down his chest.");
        else if (player.torso.vaginas.get(0).wetness >= 4) DisplayText("  The stream of dribbling fluids drips from his shaggy chin to run down his chest.");
        else if (player.torso.vaginas.get(0).wetness >= 3) DisplayText("  The dribbling fluid runs down his shaggy chin and drips towards the ground.");
    ...
    // (PENISCOCKSUCKERFUCK)
        ...
        DisplayText("The girly 'monster' crawls forward, ");
        if (player.torso.hips.legs.isTaur()) DisplayText("disappearing under your hind-legs as he attends to his newly commanded duty.");
        else DisplayText("looking up at you longingly as he leans in towards " + Desc.Cock.describeMultiCockSimpleEach(player) + ".");
        ...
        if (player.torso.cocks.count > 1) {
            ...
            if (player.torso.cocks.count > 2) DisplayText("one of your ignored penises");
            else DisplayText("on your ignored penis");
        }
        DisplayText(".");
        if (player.torso.vaginas.count > 0) DisplayText("  You begin to cum into your son's mouth, your pussy hugging his finger tightly inside its depths.");
        DisplayText("\n\n");

        DisplayText("Your jism ");
        if (player.cumQ() < 100) DisplayText("squirts into his mouth repeatedly, puddling on his tongue before he noisily swallows it.");
        else if (player.cumQ() <= 250) DisplayText("squirts into his mouth over and over, puddling on his tongue and forcing his cheeks to bulge slightly before he swallows.");
        else if (player.cumQ() <= 1000) DisplayText("spurts into his mouth, filling it with the first burst alone.  He swallows it just in time for the next, and he gulps noisily to keep up with your orgasm.");
        else {
            DisplayText("blasts into his mouth and dribbles from the corners of his lips as his throat noisily attempts to gulp down the huge semen-flow.");
            if (player.cumQ() >= 2000) DisplayText("  Spunk runs from his nostrils as he utterly fails.");
        }
        DisplayText("  He pulls back before you finish and lets the last few waves of semen of take him in the face.");
...
function victoryBJNippleFuckMinotaurGang(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minotaur_Sons);
    DisplayText("You pull down the top portion of your " + player.inventory.equipment.armor.displayName + " and shake your " + Desc.Breast.describeChest(player) + " back and forth.  \"<i>Would two of you strapping young lads like to play with your Mom's nipples?</i>\" you tease.  The minotaurs stir from their defeated poses with lust burning in their eyes as they rise and advance.  Giggling, you circle your areolae with your fingertip and moan lewdly, teasing the poor beasts until you tire of the stimulation and work a fingertip into ");
    if (player.torso.chest.countNipples() === 2) DisplayText("each");
    else DisplayText("two");
    DisplayText(" of the leaky nipple-cunts.\n\n");

    DisplayText("The second largest of your strapping sons dares to ask, \"<i>If you're going to let us fuck your nipples, why did you have to fight us first?</i>\"\n\n");

    DisplayText("\"<i>Because, dear boy, I needed to show you rebellious fools who's in charge around here.  Now that you've been put in your place, it's time you gave Mom a little sugar,</i>\" you explain as you wiggle seductively.\n\n");
    ...
    if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE === 3) DisplayText("the minitaur, much to the smaller monster's chagrin.");
    else DisplayText("the rest of his kin, much to their chagrin.");
    DisplayText("  The pair look at you with ravenous eyes as you continue to toy with the nipple openings, slowly stretching them as they get wetter and wetter.\n\n");

    DisplayText("You smile and ");
    if (player.tallness >= 72) DisplayText("drop down");
    else DisplayText("shift position");
    DisplayText(" to get your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s at a better angle, popping your fingers free of their slimy depths.  \"<i>Well, go on then,</i>\" you say with an expectant tone.  \"<i>Fuck them.</i>\"  The minotaurs do not disappoint.  Each of them takes one of your " + Desc.Breast.describeBreastSize(player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + " in their hands and presses their fat, flat tips against the slightly-gaped openings.  With ever-increasing pressure, they push their flares harder against the unyielding nipple-pussies, until at last they stretch the poor tit-cunts wide and dilate the inside of your mammaries.\n\n");
    ...
    else if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating <= 4) DisplayText("inches");
    else if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating <= 8) DisplayText("most of a foot");
    else if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating <= 14) DisplayText("over a foot");
    else if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating <= 30) DisplayText("nearly two feet");
    else DisplayText("the whole length");
    ...
    if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE >= 7) DisplayText(", chased by jeers from the remainder of your brood");
    ...
    if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 3) DisplayText(" while you hose sprays of milk around the twin rods");
    else if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 1.5) DisplayText(" while you dribble milk around the twin rods");
    DisplayText(".\n\n");

    DisplayText("The two cumming studs pull out of the tight holes as your convulsing fingertips milk their balls, squeezing the heavy sacks uncontrollably.  Ropes of potent, drug-laced spooge spatter across your " + Desc.Breast.describeChest(player) + ", shoulders, and face, just in time for the cock plugging your mouth to explode in orgasm with the force of a bursting dam.  You're utterly soaked in seed, both inside and out - an incestuous whore through and through.  The two largest minotaurs finally exhaust their reserves and step back, high-fiving as they go.  Your smaller, girlish boy takes longer to finish, but by the time he does you feel positively stuffed.  It will be quite some time before you're hungry for a");
    if (!minotaurAddicted(player)) DisplayText(" 'meal'");
    else DisplayText("nother fix");
    DisplayText(".\n\n");

    DisplayText("You stand up straight and stretch, cum pouring from your nipples.  The assembled minotaurs ");
    if (minotaurMobFlags.MINOTAUR_TRIBE_SIZE < 5) DisplayText("relax and fan themselves while giving you lusty looks");
...
function minotaurGangBadEnd(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minotaur_Sons);
    ...
    DisplayText("You feel like you're floating, cushioned in a bed of clouds with every nerve firing off nothing but pleasure and happiness.  There's a wet, slap-slap-slap nagging at you, but you close your eyes and forget it, letting your fingers play across your " + Desc.Breast.describeChest(player) + " to ");
    ...
    return { next: minotaurGangBadEnd2 };
}

// [Next]
function minotaurGangBadEnd2(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minotaur_Sons);
...
// [Discipline Eldest]
function disciplineEldestMinotaurSon(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minotaur_Sons);
    ...
    DisplayText("\"<i>Are you the oldest?</i>\" you ask him with crossed arms and a stern gaze.\n\n");

    DisplayText("\"<i>Yeah,</i>\" he snorted, trying to put up a facade of bravery.  He turns over, now lying on his back as if lounging on the ground.\n\n");

    DisplayText("\"<i>Explain yourself,</i>\" you demand.  \"As the oldest, you're supposed to set an example for your younger brothers.\"\n\n");

    DisplayText("\"<i>A mother is supposed to set an example, and the only example you've set is being a dirty whore,</i>\" he laughs.  \"<i>Is it so wrong to want to give Mom her cum fix?</i>\"\n\n");
