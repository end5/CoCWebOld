...
export interface IzumiFlags {
    IZUMI_MET: number;
    IZUMI_LAST_ENCOUNTER: number;
    IZUMI_TIMES_SUBMITTED: number;
    IZUMI_SEEN_PC_GENDER: Gender;
    IZUMI_TIMES_GRABBED_THE_HORN: number;
}

const izumiFlags = {
    IZUMI_MET: 0,
    IZUMI_LAST_ENCOUNTER: 0,
    IZUMI_TIMES_SUBMITTED: 0,
    IZUMI_SEEN_PC_GENDER: Gender.NONE,
    IZUMI_TIMES_GRABBED_THE_HORN: 0,
};

User.flags.set(FlagType.Izumi, izumiFlags);

// Return a height-based nickname for the player
export function heightDesc(player: Character, doName: boolean = true): string {
    if (randInt(4) !== 0) return "kid";
    else {
        ...
        if (player.tallness < 96) return "kid";
            ...
            if (doName) return player.desc.short;
            else return "kid";
        }
    }
}
...
export function actsLikeACow(player: Character): boolean {
    if (player.statusAffects.has(StatusAffectType.Heat)) return true;
    if (player.statusAffects.has(StatusAffectType.Rut)) return true;
    if (player.statusAffects.has(StatusAffectType.Dysfunction)) return true;
    return false;
}
...
export function encounter() {
    ...
    if (izumiFlags.IZUMI_MET === 0) {
        DisplayText("As you wander the wasted landscape searching for anything of interest, you come across a large rock blocking what appears to be the entrance to a cave.  Wondering about how to best get around it, you suddenly become aware of a strange sound on the edge of your hearing.\n\n");
        ...
        return { choices: [["Enter", enterFirstTime], ["Leave", abortabortleavethefukkencave]] };
    }
    else {
        ...
        return { choices: [["Enter", enterAfterMet], ["Leave", nopeLeavePlz]] };
    }
}

// Repeat encounters with Izumi after having met in the past
export function enterAfterMet(player: Character): NextScreenChoices {
    DisplayText().clear();

    if (izumiFlags.IZUMI_LAST_ENCOUNTER === 0) izumiFlags.IZUMI_LAST_ENCOUNTER = 2;

    if (izumiFlags.IZUMI_LAST_ENCOUNTER === 1) {
        DisplayText("Remembering the outcome of your last visit, you peer cautiously around the mouth of the cave.  Izumi, as before, is seated by the fire, happily puffing away at whatever bizarre narcotic she keeps in her pipe.  She sees you almost immediately, raising a hand cheerfully and beckoning for you to enter.\n\n");

        DisplayText("“<i>Hey, it’s you! How’s it going, " + heightDesc(player) + "? Killed any demon kings yet?</i>” Izumi ribs. “<i>Let me guess.. You’re here to take revenge on the big bad Oni, grr argh, right? Or...</i>”  She pauses for a second to shift around into a more comfortable position that - completely inadvertently, you’re sure - ends up with her showing off much more of her thighs and impressive bust.  “<i>Could it be you just came back to ask for another turn, hmm?</i>”\n\n");
    }
    else if (izumiFlags.IZUMI_LAST_ENCOUNTER === 2) {
        DisplayText("Remembering your last visit, you peer cautiously around the mouth of the cave.  Izumi, as before, is seated by the fire, happily puffing away at whatever bizarre narcotic she keeps in her pipe.  She sees you almost immediately, raising a hand cheerfully and beckoning for you to enter.\n\n");

        DisplayText("“<i>Oh hey, you came back!</i>”  She announces.  “<i>That’s good, ‘cause for a while there I thought you were a wuss.  Crazy idea, right?  So, did you decide to try your luck, or are you just gonna give up and let me have some fun?</i>”\n\n");
    }
    else if (izumiFlags.IZUMI_LAST_ENCOUNTER === 3) {
        ...
        DisplayText("“<i>So, uh...</i>” she begins.  Then she scowls, blushing slightly.  “<i>So, you came back to apologize, right?  You damn well better have!  In fact, I’m not forgiving you until I’ve made you feel as humiliated as I did, so just give in already, okay?!</i>”\n\n");
    }
    return { choices: [["Fight", fightTheFuckhugeOni], ["Surrender", fuckhugeOniGetsWhatSheWants], ["Flee", fuckinChegIt]] };
}

// Leave the cave, no meeting
function abortabortleavethefukkencave(): NextScreenChoices {
    DisplayText().clear();
    ...
    DisplayText("You head back to camp, having found nothing else of interest.");
    return { next: Scenes.camp.returnToCampUseOneHour };
}

// Already met, dun wanna get oniraepd again plz
function nopeLeavePlz(): NextScreenChoices {
    DisplayText().clear();
...
// Long: Untested
// Short: Tested
function enterFirstTime(player: Character): NextScreenChoices {
    DisplayText().clear();
    izumiFlags.IZUMI_MET = 1;
    ...
    if (player.tallness >= 108) {
        ...
        if (player.stats.str >= 75) {
            DisplayText("Putting your shoulder to the stone, you push with all your might, trying to roll it out of the way.  It resists for a moment, then gives, slowly but surely rattling and scraping across the uneven ground.\n\n");
        }
        // Low strength variant
        else {
            DisplayText("Putting your shoulder to the stone, you push with all your might, trying to roll it out of the way.  It resists, but you are determined, your [feet] scrabbling against loose rocks and gravel, trying to find the purchase you need to move the boulder.\n\n");
        }
        ...
        if (player.stats.str < 75) {
            DisplayText("  She gives the boulder a shove - with just one hand - and it starts to give, scraping across the uneven ground.\n\n");
        }
        else {
            DisplayText("\n\n");
        }
        ...
        DisplayText("When you finally manage to drag your eyes back to her face, her expression is quite different. Instead of suspicion, she treats you to a sly smile.\n\n");

        DisplayText("“<i>Well now,</i>” she drawls in a syrupy, seductive tone, twirling an errant lock of her hair thoughtfully in her fingers.  “<i>You’re a </i>big<i> one, aren’t you?  And here you are,");
        if (player.stats.str >= 75) DisplayText(" forcing");
        else DisplayText(" trying to force");
        DisplayText(" your way through little Izumi’s door uninvited.  I’d tell you off, but I’ve got to admit, I don’t get many visitors of your... </i>stature.<i>");

        if (player.torso.neck.head.horns.type !== HornType.COW_MINOTAUR) {
            DisplayText("  Or at least, ones without a pair of bulls horns");
            if (actsLikeACow(player) === true) {
                DisplayText(" - and the temperament to match - ");
            }
            else {
                DisplayText(", ");
            }
            ...
            if (actsLikeACow(player) === true) {
                DisplayText("  Or at least, ones without the temperament to match the... physique, ");
            }
        }
        DisplayText("if you follow?</i>”\n\n");

        DisplayText("She turns and heads back into the cave, quickly disappearing into the gloom, the strange woman motioning you to follower her inside with a wave.  You creep slowly into the darkness after her, your sight gradually adapting to the darkness.\n\n");
    }
    // "short" variant
    else {
        DisplayText("You squeeze through the gap, entering the cave.  Almost immediately, a palpable wave of heat rushes over you, sweat already starting to bead on your brow; it’s <i>stuffy</i> in here!\n\n");
    }
    ...
    if (player.stats.tou <= 75) DisplayText(" and of course, that obnoxiously endurance-sapping heat");
    DisplayText(".\n\n");
    ...
    if (player.tallness < 108) {
        ...
        if (RaceScore.minotaurScore(player) > 3) DisplayText("plus it looks like you're thinking with the head on top of your neck, rather than the head between your legs, unlike the stupid brutes who usually wander around these parts.  ");
        else if (RaceScore.dogScore(player) > 3) DisplayText("plus it looks like you'd actually be able to resist chasing down a stick, if I threw one outta' the cave.  ");
        else if (RaceScore.goblinScore(player) > 3) DisplayText("plus you don't seem to be as insufferable as the rest of those little green whores who trollop around these parts, hunting their next lay.  ");
        else if (RaceScore.demonScore(player) > 3) DisplayText("plus you're not forever going on about corruption this, enslavement that, demonic taint the other, unlike the insufferable clods who usually wander around these parts.  ");
        else if (RaceScore.harpyScore(player) > 3) DisplayText("plus you're not screeching incessantly, like the feathery sluts who usually flap their way around these parts.  They're lucky they </i>can<i> fly, 'cause I'd give them something to really squawk about if I could get my hands on them.  ");
        else if (RaceScore.lizardScore(player) > 3) DisplayText("plus, you don't have the same creepy eyes that the scaley idiots from around these parts keep trying to use on me.  ");
        else DisplayText("plus you don’t look like the troglodytes who usually wander around these parts.  ");
        DisplayText("Sit down, sit down!</i>”\n\n");
    }

    DisplayText("Cautiously, you edge towards the lip of the fire, and");
    if (player.tallness >= 108) {
        DisplayText(" a");
    }
    else {
    ...
    DisplayText(" shadow leans forward, finally becoming visible.");
    if (player.tallness < 108) {
    ...
    DisplayText("  The second factor, of course, would be the enormous curved horn of red, rock-like bone that juts proudly from her forehead.  Still, they’re no less inhuman than some of the other creatures you’ve encountered since arriving....\n\n");

    DisplayText("“<i>Hmm?</i>”  She looks blankly at you.  The sudden realisation that you’ve been staring at her horn for an uncomfortably long time hits you.  “<i>What?  Is there something on my face?</i>”  Blushing, you open your mouth to answer, but as you breathe in, a rush of coarse smoke down your throat");
    if (player.stats.tou < 60) DisplayText(" causes you to cough and splutter.\n\n");
    else DisplayText(" momentarily steals your voice, having been ill-prepared for the sudden burn.\n\n");

    DisplayText("The strange woman blinks at you in confusion for a moment, then her face cracks into a grin.  “<i>Oh, right, I forgot people like you tend to be pretty fragile.  Hold on");
    if (player.stats.tou < 60) DisplayText(", kid. Let");
    ...
    return { next: enterFirstTimePartII };
}

function enterFirstTimePartII(player: Character): NextScreenChoices {
    DisplayText().clear();

    DisplayText("Still");
    if (player.stats.tou < 60) DisplayText(" struggling with");
    else DisplayText(" distracted by");
    DisplayText(" the smoke, you turn to see her, only barely through the gloom, at the boulder blocking entry to the cave.");
    if (player.stats.str <= 75) DisplayText("  To your amazement, the boulder begins to move, and as more light - along with a much-needed rush of fresh air - floods into the smoky cavern, you see she’s actually rolling it aside with her <i> bare hands </i>.  Even given her great size, this woman’s strength must be immense!\n\n");
    else DisplayText("  The boulder begins to move, and as more light - along with a welcome rush of fresh air - floods into the smoky cavern, you see she’s rolling it aside with nothing more than her bare hands with graceful, if unnerving, ease.\n\n");
    ...
    if (player.tallness < 108) DisplayText("She wears a silken kimono, embroidered with stars, dragons and flowers, which you can’t help but notice is of a truly scandalous cut – your eye is inevitably drawn to her enormous breasts, bulging dangerously against the fabric as though straining to escape.  If she were human-sized and remained in proportion, they would probably be around a DD, but at her size, you honestly have no idea.\n\n");

    DisplayText("She sits, right knee raised so she can rest her arm on it, her left foot tucked behind the right.  As she does so, her kimono slides aside,");
    if (player.stats.cor <= 33) DisplayText(" exposing an uncomfortable amount of creamy-white thigh flesh.");
    else if (player.stats.cor > 33 && player.stats.cor < 66) DisplayText(" exposing a tantalising view of creamy-white thigh flesh.");
    else DisplayText(" exposing a delicious view of her thigh, the creamy-white flesh almost screaming out for your caresses.");
    DisplayText("  If she notices, then she doesn’t seem to care.\n\n");

    DisplayText("“<i>Better, right?</i>” she asks, and you nod.  “<i>Sorry, I didn’t realize the smoke was getting to you at first.  It’s been a while since I’ve seen anything but a minotaur or a demon, at least ones that seem more interested in polite conversation than reaming everything in sight.</i>”  She grimaces, briefly.  “<i>So, what’s your story, " + heightDesc(player, false) + "?</i>”  Whoever she is, she seems friendly enough");
    if (player.tallness <= 96) DisplayText(" - if a little... imposing -");
    else DisplayText(",");
    DisplayText(" so you give her your name, and, hesitant to discuss your true quest in any depth, detail some of your past encounters in this strange world.  Nodding encouragingly, she raises a long, elaborately carved pipe to her lips, puffing away as you tell your story....\n\n");
    ...
    DisplayText("“<i>Ah, but where to begin?  My clan calls me ‘Izumi, <b>The Font Of All Strength’</b>.</i>”  She snorts in amusement at your reaction to her name.  “<i>I know, it’s a little pompous, right?  Just use Izumi, I always do.  Either way, I’m a traveller from another world.");

    if (RaceScore.humanScore(player) > 3) {
        ...
        DisplayText("She takes a long, powerful drag on her pipe, her lips locked tight around the mouthpiece.  “<i>It was fun for a while, but bothering goatherds for the next thousand years wasn’t my scene, frankly, so I left.  Bailed.  Disappeared.");
    }
    ...
    if (player.stats.str <= 75 || player.tallness < 96) DisplayText(" You can’t help but feel like a sheep yourself - one who’s just noticed the wolf is staring it down.\n\n");
    ...
    return { choices: [["Smoke", smokeThePipe], ["NoSmoke", dontSmokeThePipe]] };
}
...
function smokeEffect(player: Character, smokedPipe: boolean) {
    ...
    player.statusAffects.add(StatusAffectType.IzumisPipeSmoke, SMOKE_DURATION, deltaSpd, deltaSns, deltaLib);
    ...
    player.stats.lust += lustMod;
}

// Update the duration of the pipe smoke effect
export function updateSmokeDuration(player: Character, hours: number) {
    if (player.statusAffects.has(StatusAffectType.IzumisPipeSmoke)) {
        player.statusAffects.get(StatusAffectType.IzumisPipeSmoke).value1 -= hours;

        if (player.statusAffects.get(StatusAffectType.IzumisPipeSmoke).value1 <= 0) {
            smokeEffectWearsOff(player);
        }
    }
}

// Method to contain removal mechanics + scene text to spit out
function smokeEffectWearsOff(player: Character) {

    if (player.statusAffects.has(StatusAffectType.IzumisPipeSmoke)) {
        player.stats.spe += Math.abs(player.statusAffects.get(StatusAffectType.IzumisPipeSmoke).value2);
        player.stats.sens -= player.statusAffects.get(StatusAffectType.IzumisPipeSmoke).value3;
        player.stats.lib -= player.statusAffects.get(StatusAffectType.IzumisPipeSmoke).value4;
        ...
        player.statusAffects.remove(StatusAffectType.IzumisPipeSmoke);
    }
}

// Actual introduction scene content for pipesmokin
function smokeThePipe(player: Character): NextScreenChoices {
    DisplayText().clear();

    DisplayText("Whatever it is she has in there, the smoke is thick and cloying and even the smallest puff makes");
    if (player.stats.tou <= 60) DisplayText(" you cough and splutter");
    else if (player.stats.tou < 75) DisplayText(" you flinch, holding back a cough");
    else DisplayText(" your eyes water");
    DisplayText(" - much to your mutual amusement.  You talk and joke for a while longer, the smoke making you feel incredibly relaxed, as though all the tension were simply melting from your body.");
    if (player.stats.tou <= 80) DisplayText("  After a while, you find it hard to even raise your arms to reach for the pipe again.\n\n");
    else DisplayText("\n\n");

    smokeEffect(player, true);

    return smokeJoins(player);
}

// Turn down the pipe -- still recieve SOME effect from the smoke in the cave
function dontSmokeThePipe(player: Character): NextScreenChoices {
    DisplayText().clear();
    ...
    smokeEffect(player, false);

    return smokeJoins(player);
}

// Continue after the pipe smoke split
function smokeJoins(player: Character): NextScreenChoices {
    ...
    if (player.stats.str <= 75 || player.stats.tou <= 75 || player.tallness <= 96) DisplayText("  You swallow, hard, suddenly feeling oddly nervous.");
    DisplayText("  She’s even larger up close, and you can clearly see the solid, capable looking muscle that makes up her frame....\n\n");
    ...
    if (player.tallness < 108) {
        ...
        DisplayText("“<i>I think you’d better make it up to me, yeah?</i>”  She continues, slurring her words slightly as she reaches out with one hammer-like hand, then seizes hold of your [armor].  “<i>Otherwise I might get nasty.  And you don’t want me to get <i>nasty</i> with ya’, do ya’?</i>”\n\n");
    }
    else {
...
function fuckinChegIt(): NextScreenChoices {
    izumiFlags.IZUMI_LAST_ENCOUNTER = 2;

    DisplayText().clear();

    DisplayText("Spotting an opening, you decide to beat a hasty retreat, as far away from the immense woman as possible.\n\n");
 ...
 * FIGHT SHIT
 */
function fightTheFuckhugeOni(player: Character): NextScreenChoices {
    DisplayText().clear();

    // First time
    if (izumiFlags.IZUMI_LAST_ENCOUNTER === 0) {
        ...
        DisplayText("“<i>Oh, you think you can take me?  That’s cute, " + heightDesc(player) + ".  That’s real cute,</i>” Izumi says, breaking off the conversation.  She picks herself up from the floor, abruptly breaking off the conversation to wander around the cave, flexing her muscles and rotating her joints as though preparing to exercise - all the while, her grin growing increasingly confident, more... ravenous.\n\n");

        if (player.tone <= 75) DisplayText("“<i>Alright!  I’ll tell you what I’ll do... I’ll give myself a handicap.</i>”  Izumi turns to face you, cracks her neck, then extends a hand, folding the other arm behind her back.  “<i>I’ll only use one arm.  I wouldn’t want to put you in your place </i>too<i> quickly.  Where would be the fun in that?</i>”  She leers.\n\n");

        DisplayText("“<i>If you can beat me, I’ll let you go.  If not, well... to the victor go the spoils.  Know what I mean?</i>”  She says, her smirk somehow growing perceptibly wider.\n\n");

        if (player.tone <= 75) DisplayText("The idea that she thinks she can beat you with one hand behind her back is insulting to say the least, but on the other hand, her overconfidence could be her downfall.\n\n");

        DisplayText("You nod, rising to your [feet] and dropping into a fighting stance. Izumi grins.  “<i>Let me just say this to start... I am </i>quite<i> strong.</i>”\n\n");
    }
    // PC won last fight
    else if (izumiFlags.IZUMI_LAST_ENCOUNTER === 3) {
        DisplayText("“<i>So, you’re not gonna apologize?!</i>” Izumi scowls at you, blushing slightly. “<i>You’re a real jerk, you know that?  Fine then, I’ll just have to spank some manners into you!  Don’t cry!</i>”\n\n");

        DisplayText("She drops into a fighting stance once again, but this time, instead of bending one arm behind her back, she hesitantly holds it up in front of her horn.  Her blush deepens as you stare at her.  “<i>Y-You cheat! I’m not falling for that kind of thing again!</i>” she explains hotly.\n\n");
    }
    // PC lost last fight
    else if (izumiFlags.IZUMI_LAST_ENCOUNTER === 1) {
        DisplayText("There’s no way you’re just going to lie back and let Izumi walk all over you again; at least, not without resistance.\n\n");
        ...
        DisplayText("You nod, rising to your feet and dropping into a fighting stance.\n\n");
    }
    // PC ran away
    else if (izumiFlags.IZUMI_LAST_ENCOUNTER === 2) {
        DisplayText("Sooner or later, you’re going to have to face up to the large Oni.  No time like the present, right?\n\n");
    ...
    return CombatManager.beginBattle(player, [], [new Izumi()]);
}

/**
 * SURRENDER SCENES
 */
function fuckhugeOniGetsWhatSheWants(player: Character) {
    DisplayText().clear();

    izumiFlags.IZUMI_TIMES_SUBMITTED++;
    izumiFlags.IZUMI_LAST_ENCOUNTER = 1;
    izumiFlags.IZUMI_SEEN_PC_GENDER = player.gender;
    // Male/Herms
    if (player.torso.cocks.count > 0) {
        DisplayText("You swallow your pride and shake your head, signalling Izumi that you don’t intend to resist.  Her face splits into a truly lascivious grin and she licks her lips, hungrily.\n\n");

        DisplayText("“<i>Yeah, that’s what I thought.</i>”  She smirks.  “<i>You just do exactly what I say, when I say, and nothing bad will happen to you.  Well... Nothing <i>too</i> bad, anyway.</i>”\n\n");

        DisplayText("Suddenly, she lunges forwards, one enormous hand planting itself onto your");
        if (player.torso.hips.legs.isTaur()) DisplayText(" flank and forcing you sideways onto the ground.");
        ...
        if (player.torso.cocks.sort(Cock.LargestCockArea)[0].length <= 6) return surrenderSmallCock(player);
        else if (player.torso.cocks.sort(Cock.LargestCockArea)[0].length <= 14) return surrenderMediumCock(player);
        else return surrenderLargeCock(player);
    }
    // Female
    else if (player.torso.vaginas.count > 0) {
        return surrenderOhGodTheFemaleSurrenderContentIsFuckingHugeSendHelp(player);
    }
    // Genderless
    else {
        return noDickNoVagNoService(player);
    }
}

// Male/Herm scene for cocks <= 4"
function surrenderSmallCock(player: Character): NextScreenChoices {
    ...
    DisplayText("You try to look away, blushing crimson, but the weight on your");
    if (player.torso.hips.legs.isTaur()) DisplayText(" flank");
    ...
    if (player.torso.hips.legs.isTaur()) DisplayText(" perches her muscular ass on your flank, your body making for a suitable seat.");
    else DisplayText(" drops to the floor, laying next to you on her side.");
    DisplayText("  She releases your chin, but her hand snakes around behind your head to your other cheek, pulling you in towards her.  You can’t help but grunt in surprise as the side of your head is forcibly pushed up against one massive, pillowy-soft breast");

    if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 1 && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating < 3) DisplayText(", your [chest] dwarfed by the larger womans own pair.");
    else if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 3 && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating < 5) DisplayText(", your [chest] moulding themselves against the underside of the Oni’s larger pair.");
    else if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 5 && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating <= 8) DisplayText(", your [chest] compressing against the Oni’s larger pair, forming a delicious valley of flesh between your bodies.");
    else DisplayText(", your [chest] dwarfing the larger womans own pair.");

    DisplayText("  “<i>There.</i>”  She announces happily, softly stroking your cheek as though to reassure you.  “<i>All better now, right?  Good.</i>”  She doesn’t even wait for a response before turning her attention back to your [cock biggest]....\n\n");
    ...
    if (player.stats.str <= 75) DisplayText(" and you would be powerless to resist...");
    else DisplayText(" and you would struggle to deny her will...");
    DisplayText(" a feeling that is only compounded by the burning sense of shame you feel as she jacks you off with a measly two fingers.\n\n");
    ...
    if (player.torso.vaginas.count > 0) DisplayText(", sticky moisture dribbling across your thigh, your [vagina] clenching in lust - fueled harmony with your cock");
    DisplayText("... “<i>Hey, don’t get upset.  After all, </i>” She pauses for a moment, her hand lingering in place.  “<i>Some women </i>like<i> ‘em small...</i>”\n\n");

    DisplayText("“<i>So, let me guess.  In a place like this, it’s gotta be weird having something this small, right?  I mean, it’s a pretty unique trait, given the circumstances.</i>”  She continues.  “<i>Hey, tell me something.  Do the harpies leave you alone?  Are you so small that they think you’re ");
    if (player.gender === Gender.HERM) DisplayText(" just a regular");
    else DisplayText(" a");
    DisplayText(" girl?</i>”  Suddenly, her hand bottoms out around your [cock biggest]");

    if (player.torso.balls.quantity > 0) DisplayText(" and her free fingers reach out to gently stroke against your [balls].");
    else if (player.torso.vaginas.count > 0) DisplayText(" and her free fingers reach out to gently stroke against your [vag].");
    else DisplayText(".");

    DisplayText("  At the same time, she leans in close to your ear.  “<i>Do the </i>Minotaurs?”  She hisses suggestively, eyes flashing.\n\n");

    if (User.flags.get<PlayerFlags>(FlagType.Player).MINOTAUR_CUM_ADDICTION_STATE >= 1) {
        ...
        DisplayText("“<i>Ahaha, well now, look at that!</i>”  She cries, beaming.  “<i>I guess I called it right, hmm?  Geez, did the minotaurs do something to you, or were you always this perverted?</i>”  Izumi snickers.  You can’t even muster up the energy to respond, you’re so drained from your sudden orgasm.\n\n");
    }
    else {
        if (!player.torso.hips.legs.isTaur()) DisplayText("As you squirm in her arms, she releases your head from her grip, instead pushing you up into a seated position.");

        DisplayText("“<i>Hold on, I want to watch </i>”  She says, shifting position until her head is placed on one side of your hip, staring at your [cock biggest]");
        if (!player.torso.hips.legs.isTaur()) DisplayText(" as one arm reaches around to continue the handjob from the other side");
        DisplayText(".  She stares intently at your cock, an expectant smirk plastered over her face, her free hand working");
        if (player.torso.hips.legs.isTaur()) DisplayText(" over your flank");
        else DisplayText(" you over with increased vigour");
        ...
        if (player.cumQ() <= 100) DisplayText(" dribbles a hot, sticky mess onto the cold cavern floor.");
        else if (player.cumQ() > 100 && player.cumQ() <= 250) DisplayText(" spurts and splutters a hot, sticky mess onto the cold cavern floor.");
        else if (player.cumQ() > 250) {
            DisplayText(" shoots out rope after rope of hot, sticky cum across the cold cavern floor");
            if (player.cumQ() > 500) {
                DisplayText(", a small puddle of creamy-white forming between you and the cavern wall");
                if (player.cumQ() > 1000) DisplayText(", slowly expanding into a small lake....\n\n");
            }
            else {
                DisplayText(".\n\n");
            }
        ...
        DisplayText("“<i>Hah!  Oh yeah, thar she blows!</i>”  Izumi crows, staring fixatedly at your spasming dick like a woman possessed.  “<i>Now ain’t that a pretty sight... Yeesh, and you just keep cumming and cumming.  Did being teased feel that good, " + heightDesc(player) + "?</i>”  She grins. You can’t seem to find the energy to respond, panting for breath as you stare in confusion at your already-deflated cock, your balls aching from the strain of such a forceful ejaculation.\n\n");
    }
...
function surrenderMediumCock(player: Character): NextScreenChoices {
    DisplayText("“<i>Huh.  Not bad, " + heightDesc(player) + "; I gotta say, I thought you people’d be");
    ...
    DisplayText("In mere moments, you’re fully erect, as hard as you’ve ever been");
    if (player.torso.vaginas.count > 0) {
        DisplayText(", your [vagina]");
        if (player.torso.vaginas.get(0).wetness === 0) DisplayText(" moistening");
        else if (player.torso.vaginas.get(0).wetness === 1) DisplayText(" dribbling runnels of sticky fem-juice");
        else if (player.torso.vaginas.get(0).wetness === 2) DisplayText(" constantly oozing feminine arousal");
        else DisplayText(" a soppy mess, gushing a veritable lake of fem-slime");
        DisplayText(" in lust-fueled harmony with your cock.");
    }
    else DisplayText(".");
    DisplayText("  Izumi looks down at you and flashes you an indulgent smile.");

    DisplayText("“<i>So, I guess we can start now.</i>”  Says the Oni as she drops into position,");
    if (player.torso.hips.legs.isTaur()) DisplayText(" perching her muscular ass on your flank, your lower body offering a comfortable seat for Izumi.");
    else DisplayText(" laying on her side next to you.");
    DisplayText("  She releases your");
    if (player.torso.hips.legs.isTaur()) DisplayText(" flank");
    else DisplayText(" chest");
    DisplayText(" and instead leans on her elbow, leaving you uncomfortably free and completely unsure of what to do with yourself as she wraps her hand around your [cock biggest] and slowly begins to stroke you.  Her palms are warm and pleasingly soft, something you didn’t expect from the well-muscled Oni woman.\n\n");
    ...
    DisplayText("Without warning, your hips spasm and buck");
    if (player.torso.hips.legs.isTaur()) DisplayText(" upwards");
    ...
    return { choices: [["Obey", surrenderMediumCockObey], ["Refuse", surrenderMediumCockRefuse]] };
}

// Male/Herm play along scene split
function surrenderMediumCockObey(player: Character): NextScreenChoices {
    ...
    DisplayText("ou manage to relax and simply wait passively for Izumi to resume her handjob.\n\n");
    ...
    DisplayText("Almost immediately, you feel your orgasm starting to rise within you.  You try to stammer out a warning, but Izumi’s pace continues unabated.\n\n");
    ...
    DisplayText("cock");
    DisplayText(" to the very last drop.\n\n");

    DisplayText("Finally, she relents, and your extended orgasm mercifully subsides.  Izumi stares indulgently down at your shivering form for a moment, before muttering a quiet “<i>Heh,</i>” and wandering back over to her tent.  You can only manage to rest there, laid prone in a");
    if (player.torso.vaginas.count > 0) DisplayText(" mixed puddle of your own fem-slime and");
    else DisplayText(" puddle of your own");
    DisplayText(" cum, staring at the cavern roof and panting madly for a good few minutes afterwards.  Once you regain the use of your legs, you retrieve your clothes and wander back to camp in a daze.\n\n");
...
function surrenderMediumCockRefuse(player: Character): NextScreenChoices {
    ...
    DisplayText("Ignoring her, you continue thrusting, twisting and turning, but always Izumi follows you effortlessly, thwarting every twist and turn you make in a desperate attempt to fuck her hand.  Finally, after an eternity of painful stalemate, you give in, collapsing to the ground, momentarily exhausted.\n\n");
    ...
    DisplayText("The insane feeling is just too much, and you feel your orgasm boiling up within you.  Just as you feel yourself rising to crest the orgamsic wave and hit sweet release... Izumi lets go.\n\n");

    DisplayText("You cry out in anger and pain as you find your impending orgasm so cruelly denied, your [cock biggest] twitching and throbbing painfully");
    if (player.torso.vaginas.count > 0) DisplayText(", [vagina] spasming in sympathy");
    DisplayText(", desperate for release.  Somehow, she could sense you were on the verge of orgasm, and she chose to deny you that pleasure, you just <b>know</b> it.  You shoot Izumi a vicious, hateful glare, only to see her laying there quite calmly, her expression blank.\n\n");

    DisplayText("“<i>Hm?</i>”  She grunts, looking surprised.  “<i>Oh, do you want some help?  Sure; here, let me handle this for you.</i>”  She says, her tone even.  She reaches out and, very deliberately, softly flicks your [cockhead] with a single finger.\n\n");

    DisplayText("The sensation is almost nothing, but as close as you are, it’s just enough.  You groan in a mix of pleasure and heartbroken anguish as the tiny amount of contact forces you over the edge, your load forcing its way up through your cock with agonizing slowness until finally");
    if (player.cumQ() <= 250) DisplayText(" spurting free, spattering down randomly over you as Izumi watches, happily observing the effects of your ruined orgasm.");
    else if (player.cumQ() <= 500) DisplayText(" sputtering freely from your [cock], glob after glob of thick jizz showering down randomly over you as Izumi watches, mildly amused with the results of your ruined orgasm.");
    else if (player.cumQ() <= 1000) DisplayText(" a torrent of fertile spunk rushes forth from your [cock], glob after glob of thick jizz showering down randomly over you, covering you from head to toe.  Izumi simply watches, amused with the results of your ruined orgasm.");
    else DisplayText("a torrent of fertile spunk rushes forth from your [cock], rope after rope of the thick cum launching into the air and splattering down over you.  Your orgasm continues unabated as you slowly paint yourself in a creamy white layer, Izumi giggling quietly, evidently quite proud of herself.");
    DisplayText("  Finally, you finish cumming, and as you lay panting in the results of her handiwork, Izumi gently pats you on the head.\n\n");

    DisplayText("“<i>So, you learned something today, right?  Next time, just do what I say, and the results are much more... fun.</i>” She smiles, then turns away.  Retrieving your clothes, you drag yourself back to camp, feeling decidedly shaky.\n\n");
...
// Urtadicks itt
function surrenderLargeCock(player: Character): NextScreenChoices {
    DisplayText("Izumi grunts in surprise as your [cock biggest] flops free of its confinement.  “<i>Damn.  This is pretty impressive, you know?</i>”  She frowns, running an exploratory hand across the length of your flesh.  “<i>Let me guess, you got a bit too frisky with one of the natives?  Maybe didn’t think to boil the water before you drank it?  Look, no offence, but there is no way this thing is legit, " + heightDesc(player) + ".</i>”\n\n");
    ...
    DisplayText("Izumi releases you from the floor, allowing you to sit");
    if (player.torso.hips.legs.isTaur()) DisplayText(" back on your haunches, front end raised");
    ...
    DisplayText("“<i>Five!</i>”  She yells, and her left hand joins the first in gripping your cock, this time about halfway down your shaft.  Again, her wrist rotates gently as she massages the area, but her hand doesn’t actually go anywhere.  Her grin widens into a truly ominous smile as she leans forwards conspiratorially.\n\n");

    DisplayText("“<i>Four.</i>”  She whispers, and without warning her legs unfold, both feet flying out from beneath her and latching onto the [sheath] of your [cock biggest]!  Izumi leers at you with undisguised amusement as she watches your reaction to the sensation of her bare soles rolling over the [sheath] of your shaft");
    if (player.torso.balls.quantity > 0 && player.torso.vaginas.count <= 0) DisplayText(", occasionally sliding down to massage your [balls] before returning to your cock.");
    else if (player.torso.balls.quantity === 0 && player.torso.vaginas.count > 0) DisplayText(", occasionally sliding down to tease at the lips of your [vagina].");
    else if (player.torso.balls.quantity > 0 && player.torso.vaginas.count > 0) DisplayText(", occasionally sliding down, alternating between a gentle massage of your [balls] and teasing caresses of your [vagina].");
    DisplayText("\n\n");
    ...
    DisplayText("“<i>It’s good, right?</i>”  Izumi’s voice penetrates your thoughts, and you open your eyes to see her gazing up at you, amber irises glittering in the half-light of the fire.  “<i>You know, there’s one more level to .. but looking at the state you’re in, there’s no way you could endure it for long.  In terms of pleasure, it outclasses what I’ve been doing up to now completely. Do you want to see it?</i>”  You don’t even have to think about it.  You nod your head, unsure if you can manage the difficult task of speaking, and Izumi giggles.  “<i>Alright, " + heightDesc(player) + ", but don’t say I didn’t warn you.  Here we go...</i>”\n\n");
    ...
    DisplayText("You can only grunt and shiver under the onslaught as Izumi works you over with this bizarre new approach.  There’s no gentleness here, you realize; no playful teasing or long, drawn out pauses.  This move is designed for one thing, and one thing only - to force you to cum.");
    if (player.torso.balls.quantity > 0) DisplayText("  You groan, your aching balls coming to the same conclusion as you have, twitching under a passing stroke from one of Izumi’s soles and tensing up in readiness.");
    DisplayText("  You understand now why Izumi warned you about this technique; you are no longer being teased, gently coaxed towards the edge and then pulled back at the last minute.  As of now, you are being milked.\n\n");
    ...
    if (player.cumQ() > 500) DisplayText("You remain sat there for a crippling eternity, unable to do anything but moan as you inexplicably continue to cum, your aching erection firing out shot after shot of hot, dripping goo as Izumi watches you from the safety of the shadows, grinning happily like the cat that got the cream.  ");

    DisplayText("Eventually, your orgasm subsides, and you collapse");
    if (player.torso.hips.legs.isTaur()) DisplayText(" sideways");
    else DisplayText(" backwards");
    DisplayText(" onto the ground, staring up at the roof of the cave in a daze.  After a few seconds, Izumi’s face appears in your field of vision, albeit upside down.\n\n");
...
function surrenderOhGodTheFemaleSurrenderContentIsFuckingHugeSendHelp(player: Character): NextScreenChoices {
    ...
    if (izumiFlags.IZUMI_TIMES_SUBMITTED === 0) {
        DisplayText("  Before you can remove your clothes, however, Izumi grabs a hold of your wrist.\n\n");

        DisplayText("“<i>Not so fast, I’ve never really had a chance to play with a girl before.  I wanna take my time on this one...</i>” She says.  Smiling suggestively, she reaches out and slowly divests you of your [armorname].");
        if (player.stats.cor <= 33) DisplayText("  You can’t help but feel a crippling sense of embarrassment as the strange woman undresses you, and you look away, embarrassed, while Izumi’s hands explore your body.");
        else if (player.stats.cor >= 66) DisplayText("  As Izumi slowly reveals your naked body to the world, you begin to feel an oh-so-familiar heat building up inside of you.  You look down at Izumi with pride as she inspects your body, running a skillful hand over your chest with a thoughtful look on her face.");
        DisplayText("\n\n");

        // Big knockers!
        if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 15) {
            DisplayText("Izumi’s eyes widen in surprise as she pulls aside your clothing to reveal your impressive bust, your [chest] finally free.\n\n");

            DisplayText("“<i>Wha- Wait, these are real?!</i>”  she exclaims, loudly.");
            ...
            // High(er) corruption
                ...
                DisplayText("“<i>Let me guess, you intentionally made them grow this big?</i>”  She scowls.  She reaches out and pinches one of your nipples between her thumb and forefinger, causing you to let out another excited little groan.\n\n");

                DisplayText("You respond with a shrug, teasingly, not entirely wanting Izumi’s abuse to stop.  Izumi’s frown deepens and she tweaks your nipple, twisting it painfully to the side, eliciting another gasp from you.\n\n");
            }
            DisplayText("“<i>How shameless.</i>”  She grumbles, returning her gaze to your bust.  “<i>How utterly shameless....</i>”\n\n");
        }
    }
    else {
        DisplayText("Before you can finish removing your clothes, Izumi grabs a hold of your wrist.  She takes over, placing herself firmly in charge of your state of dress.\n\n");
    }

    // PC is an exhibitionist!
    if (User.flags.get<PlayerFlags>(FlagType.Player).PC_FETISH >= 1) {
        ...
        DisplayText("Izumi grunts in surprise as she surveys your dripping folds");
        if (player.torso.cocks.count > 0) DisplayText(", your [cock] slowly thickening under her scrutiny");
        DisplayText(".  You shift in place,");
        if (player.torso.hips.legs.isNaga()) DisplayText(" wriggling atop your [legs]");
        else DisplayText(" subtly parting your [legs] and even ");
        if (player.torso.hips.legs.isTaur()) DisplayText(" rolling your butt upward");
        else DisplayText(" tilting your hips forwards");
        ...
        if (!player.torso.hips.legs.isNaga()) DisplayText("You rub your thighs together in delight");
        else DisplayText("You wriggle and writhe in delight");
        DisplayText(" as Izumi stares at your proudly displayed chest, enjoying the sensation of being watched and idly wishing you had something to fill the aching void");
        if (player.torso.hips.legs.isNaga()) DisplayText(" deep within your [vagina].  You slither around in place, your [leg] shifting and weaving around, moving and undulating... b");
        else DisplayText(" between your [legs] as well.  As you move your weight repeatedly from one [foot] to the other, you shiver in enjoyment.  B");
        DisplayText("eing stared at feels so good.... You blink, suddenly called back to reality by Izumi’s voice.\n\n");

        DisplayText("“<i>Look, " + heightDesc(player) + ", I know I’m kind of an intimidating specimen, but you don’t really need to be scared.</i>” She smirks.  “<i>You keep shivering in fear like that and I’m liable to take offence, yeah?</i>”  You blink again in confusion before realization finally dawns.  Izumi has absolutely no idea how much this is turning you on.  The idea of your secret arousal going completely unnoticed is surprisingly erotic, and that thought turns out to be just enough to push you over the edge.  You wince and bite your lip to stifle your moans, quivering in the throes of a miniature orgasm, even though you’ve really yet to be even touched... and on a whim, you decide to keep eye contact with Izumi for the entire thing.\n\n");

        DisplayText("Staring into those steady amber eyes as you quietly cum adds an entire new level of deliciously wrong-feeling pleasure to the affair, something you resolve to enjoy for as long as possible. Finally, your orgasm subsides, and you visibly relax.\n\n");

        DisplayText("“<i>There.  See?  I ain’t so bad.</i>”  Izumi smiles again, still apparently oblivious to your fun at her expense.  “<i>Now, we get to move on to the good part.  You ready? </i>” She asks.  You smile and nod, still basking in the post-coital warmth of your inner glow.\n\n");
    }
    ...
    if (!player.torso.hips.legs.isTaur()) {
        DisplayText("Without warning, Izumi violently lunges forward, grabbing at you.  You are only able to let out a surprised yelp before she grips onto your arms, pulling you towards her and spinning you around.");
        if (!player.torso.hips.legs.isNaga()) DisplayText("  Before you know it, she has you down on your knees, the cold stone of the floor digging into your [skinfurscales] a little as she circles behind you, smirking cruelly.");
        else DisplayText("  She takes hold of your shoulders and forces you lower to the ground, forced to slither more and more of your tail out from under yourself, until Izumi towers over you.  She circles behind you, a cruel smirk the only hint of her intentions.");
        DisplayText("\n\n");
        ...
        if (player.torso.hips.legs.isNaga()) DisplayText(" [leg]");
        ...
        DisplayText("As you squirm in Izumi’s grip, it begins to dawn on you that you’re not getting away from   Your resistance begins to lessen until eventually you begin to relax, finally admitting to yourself that this really doesn’t feel half bad...\n\n");
    }
    // Taur variant
    else {
        ...
        DisplayText("As you squirm under Izumi’s body, it begins to dawn on you that you’re not getting away from   Your resistance begins to lessen until eventually you begin to relax, finally admitting to yourself that this really doesn’t feel half bad....\n\n");
    }
    ...
    if (User.flags.get<PlayerFlags>(FlagType.Player).PC_FETISH >= 1) {
        return { choices: [["ShowMeOff", surrenderFemaleExhibitionVariant], ["HideMe", surrenderFemaleNonExhibitionVariant]] };
    }
    else {
        return { next: surrenderFemaleNonExhibitionVariant };
    }
}

// Female surrender, "regular" path
function surrenderFemaleNonExhibitionVariant(player: Character): NextScreenChoices {
    DisplayText().clear();
    ...
    DisplayText("“<i>Get lost, ya scummy vultures!</i>” She yells,");
    if (player.torso.hips.legs.isTaur()) DisplayText(" sitting upright on your back and");
    ...
    DisplayText("Despite your uncomfortable situation, you can’t help but feel kind of... reassured.  The smile soon becomes something more than friendly though, and suddenly Izumi’s head darts forwards and she forces her tongue into your surprised");
    if (player.torso.neck.head.face.hasMuzzle()) DisplayText(" maw");
    ...
    DisplayText("“<i>Now, where were we?</i>” Izumi murmurs.  You look down as she gently rolls a hand around to cup one of your breasts. “<i>Oh yeah,</i>” She adds, playfully. “I<i> remember...</i>” Izumi gropes at your chest experimentally, stroking, fondling, squeezing, molesting your bust as she gauges it for firmness.\n\n");
    ...
    if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating <= 2) {
        ...
        DisplayText("“<i>It’s fine.  Relax, kitten; I don’t care what kind of crazy kinks you think you need to get yourself off,</i>” She murmurs, reassuringly. Then you feel her hand take gentle hold of your wrist. “<i>But I </i>do<i> want to see it happen...</i>” She adds");
        if (!player.torso.hips.legs.isTaur()) DisplayText(", guiding your hand down to your own nethers, which you’re shocked - in a foggy, distant kind of way - to discover are sopping wet");
        DisplayText(".\n\n");
        ...
        DisplayText("“<i>Good girl,</i>” Izumi says, kindly, giving you a gentle kiss on the forehead as you rest your fatigued body against her bust. She reaches down between your legs and gives your still-throbbing pussy a couple of wet, slapping pats, as though congratulating it on a job well done... The impacts unfortunately causing you to yelp and tense up again as another mini-orgasm rolls through you. “<i>Hah!  Sorry.  Didn’t think you would still be that sensitive. But, you did good, " + heightDesc(player) + ".</i>” She releases you, finally, and you slide to the floor, struggling to stand as Izumi hands you your clothes.\n\n");

        DisplayText("“<i>Come back any time, sweet-stuff,</i>” She says, blowing you a kiss as she wanders back to her pipe. “<i>Always got time for cute little morsels like you.</i>” You nod, absently, as you stagger from the cave and make your way back to camp.\n\n");

        DisplayText("A schoolgirl outfit would be a pretty kinky thing to own, you have to admit...\n\n");
    }
    // C-Cup+
    else {
        ...
        DisplayText("Satisfied with her inspection, one giant hand slides");
        if (player.torso.hips.legs.isNaga()) DisplayText(" over your [hip].");
        else if (player.torso.hips.legs.isDrider()) DisplayText(" between your spindly legs, slapping them lightly to force you to spread them wider.");
        else DisplayText(" between your thighs, slapping them lightly to force you to open them wider.");
        ...
        if (player.torso.hips.legs.isNaga()) DisplayText(" along");
        else DisplayText(" between");
        DisplayText(" your [legs] to trail ever-so-softly over your nethers, a feather-light touch clearly designed to leave you wanting more.\n\n");
        ...
        DisplayText("Izumi’s finger is naturally much larger than your own, so thick that it feels almost like an actual cock, rather than just a probing digit... but the way it flexes and bends within you, slowly worming its way in deeper to gently probe at your most sensitive spots, is distinctly different.");

        if (player.torso.vaginas.get(0).looseness <= VaginaLooseness.NORMAL) DisplayText("  She takes it slowly, pushing into you with agonizing care so as not to cause you any distress.  Inch by inch, centimeter by centimeter, she slides her finger into your pussy, finally bottoming out down to the knuckle.  You let out a little gasp of surprise and alarm as she gives her finger a sudden quick, tickling twitch, as though tickling a pet under its chin, then grins savagely at your reaction.\n\n");
        else if (player.torso.vaginas.get(0).looseness >= VaginaLooseness.GAPING) DisplayText("  Izumi’s finger slips between your cavernous walls with ease, quickly bottoming out inside you.  She lets out an amused grunt of surprise at the ease with which she’s able to fit her finger inside you.\n\n");

        DisplayText("“<i>I guess someone’s been busy, huh? And here was me thinking you were a nice girl.</i>”  Izumi teases.  “<i>Well, I guess we can skip the foreplay then....</i>”\n\n");

        DisplayText("One hand now actively engaged in exploring your nethers, Izumi begins to grope your [chest] in earnest, nibbling on your neck as she grips a nipple between thumb and forefinger and squeezes, almost painfully tweaking and pulling at the sensitive flesh.  She pulls gently but insistently at your [nipple] in a rhythmic motion.  “Haha, say... you ever been milked, " + heightDesc(player) + "?”  She murmurs, ominously. \n\n");
        ...
        if ((player.statusAffects.has(StatusAffectType.BreastsMilked)) && (player.lactationQ() > 750)) {
        ...
        DisplayText("Enraptured by the concept, you submerge yourself in fantasy as Izumi drains your fat, eager udders.  Your eyes roll back into your head as you daydream of rough hands, methodical inspections of your swollen breast-udders, and finally the chilling thrill of being guided towards the cold embrace of those steely, sucking tubes for another day’s ‘work’.  You’re not entirely sure through your lust-addled haze, but you think you might even have heard yourself murmur out a little ‘moo’ once or twice...\n\n");

        DisplayText("Finally, you realize - with a slight note of disappointment - that Izumi has finished, her hand snaking back down");
        if (player.torso.hips.legs.isNaga()) DisplayText(" to");
        else DisplayText(" between");
        DisplayText(" your [legs] as she returns her attention to your groin.\n\n");
...
function surrenderFemaleExhibitionVariant(player: Character): NextScreenChoices {
    DisplayText().clear();
    ...
    if (player.torso.hips.legs.isBiped()) DisplayText(", you spread your legs a little wider,");
    DisplayText(" you");
    DisplayText(" arch your back a little further, muscles straining as you struggle to present a better view to your audience.  You moan with greater enthusiasm, hoping to draw in a larger crowd.  You feel Izumi’s breasts shake, pressed up against your back, as she chuckles to herself.\n\n");
    ...
    return { yes: surrenderFemaleLookitYouTheCenterOfAttentionYouSlut, no: surrenderFemaleNonExhibitionVariant };
}

// Female surrender, Exhibitionist variant scene body
function surrenderFemaleLookitYouTheCenterOfAttentionYouSlut(player: Character): NextScreenChoices {
    ...
    DisplayText("“<i>Lucky number one!</i>”  Izumi extends a hand towards you in an expansive gesture.  “<i>She’s all yours.  Pick any hole, she’s too cockhungry to care.</i>”  The imp eyes you up for a moment, then steps onto your");
    if (player.torso.hips.legs.isBiped()) DisplayText(" thighs");
    else DisplayText(" [legs]");
    DisplayText(", pulling aside his loincloth to reveal his raging erection.  It bobs up and down, the bulbous head mere inches from your face.  “<i>Well?</i>”  Izumi asks.  “<i>What are you waiting for?  Get started, slut!</i>”\n\n");
    ...
    if (player.torso.hips.legs.isBiped()) DisplayText(" and on your knees");
    DisplayText(", sucking on an imp’s cock like it was candy.  Suddenly, it doesn’t seem nearly so bad.\n\n");
    ...
    DisplayText("“<i>This crowd came here to see a whore getting fucked, not to see some amateur try her best.  If you aren’t gonna try, I’ll do it for you.</i>”  Izumi’s expression is almost feral with lust as she forces you into a brutal");
    if (player.torso.neck.head.face.hasMuzzle()) DisplayText(" muzzle");
    else DisplayText(" face");
    DisplayText("-fuck, tears of exertion streaming down your face.  “<i>Tongue his balls, then you get to come up for air,</i>” she commands, and you comply.  A few seconds later, you’re rewarded with the distinct hot, gloopy sensation of the imp blowing his load into your throat - and, much to your lust-addled surprise, a quick shoulder-squeeze from Izumi.  “<i>Sorry if I’m being a little rough, " + heightDesc(player) + ". Just playing the part, you know?</i>”  She whispers, reassuringly.\n\n");

    DisplayText("The imp pulls himself free of your mouth, staggering away to the safety of the rocks.  More importantly however, you realize that the ring of spectators has begun to close in, apparently satisfied that they’re no longer confined to just spectating.\n\n");

    DisplayText("“<i>Hope you’re ready for round two.</i>” Izumi mutters.  You can’t help but nod, smiling hungrily as you eye the approaching crowd.\n\n");
    ...
    DisplayText("You’re in no position to resist as Izumi casually marches off into the cave, dragging you along.  The best you can do is stumble along next to her, even a journey as short as heading back into the gloomy cave difficult after such an extensive export function performance.  She bundles you inside of her tent tenderly, and you quickly surrender to exhaustion...\n\n");
 ...
 * LOSS SCENES
 */
export function fuckedUpByAFuckhugeOni(player: Character, titLoss: boolean = false): NextScreenChoices {
    izumiFlags.IZUMI_LAST_ENCOUNTER = 1;
    izumiFlags.IZUMI_SEEN_PC_GENDER = player.gender;
    ...
    if (player.stats.HP <= 0) {
        DisplayText("You fall to the ground, reeling from Izumi’s iron-hard muscles - and other parts besides.  Grinning widely, Izumi visibly relaxes, satisfied that you’re in no fit state to continue fighting.\n\n");
    }
    else if (player.stats.lust >= 100) {
        DisplayText("You fall to the ground, too horny to resist further against the Oni’s iron-hard muscles - and other parts beside.  Grinning widely, Izumi visibly relaxes, satisfied that you’re in no fit state to continue fighting.\n\n");
    }
    ...
    DisplayText("Reflexively, you");
    if (player.torso.hips.legs.isNaga()) DisplayText(" slither");
    else if (player.torso.hips.legs.isDrider()) DisplayText(" scuttle");
    else DisplayText(" scurry");
    DisplayText(" backwards, self-preservation instincts kicking in, intent on escaping the deranged Oni.  To your surprise, rather than snatching or leaping at you, she just blinks and looks at you in confusion.\n\n");

    DisplayText("“<i>Huh?  What’s the matter?</i>”  She asks, looking genuinely nonplussed.  She reaches out towards you and you shuffle away again, putting a little more distance between the two of you.");
    if (player.stats.lust >= 100) DisplayText("  Despite how much the fight with the enormous Oni has turned you on, you’re still hesitant to capitulate to her will entirely.");
    ...
    DisplayText("“<i>Oh, I get it.  It’s me, right?  You don’t find a girl like me attractive.</i>”  Her face falls as she stares at the floor, looking positively morose.  “<i>It’s okay, I don’t blame you.  This whole situation probably seems pretty weird, right?  One minute I’m smacking you around, the next I’m trying to jump you... I forget sometimes that not everyone acts the way Oni do when it comes to stuff like </i>”  She sniffs, rubbing one massive palm into her eye and scowling.\n\n");
    ...
    DisplayText("You hesitate.  It seems like she’s being honest about letting you go, which is unexpected to say the least.  If you wanted to, you’re pretty sure you could just get up and");
    if (player.torso.hips.legs.isNaga()) DisplayText(" slither");
    else DisplayText(" walk");
    DisplayText(" out of the cave without any trouble.  On the other hand...");
    if (player.stats.lust >= 75) DisplayText(" the idea of sex with Izumi <i>is</i> quite a tantalizing prospect.  F");
    else DisplayText(" f");
    DisplayText("rom your position, you can clearly make out how tightly her kimono clings to her substantial assets, how only a thin scrap of material dangling between those powerful thighs - not even reaching low enough to touch the floor - obscures your vision and prevents you from seeing <i>everything</i>.");
    ...
    if (player.torso.cocks.count > 0) {
        choices[0] = ["Anal", cowedIntoPuttingItInIntoHerGiantOniButt];
        choices[1] = ["Vaginal", fuckhugeOniWantsYourBabiesOrSomeShit];
    }

    if (player.torso.vaginas.count > 0 && player.torso.cocks.count <= 0) {
        choices[2] = ["69", littleChampLittleChampFuckhugeOniIsCominTaEatcha];
    }

    if (player.torso.cocks.count <= 0 && player.torso.vaginas.count <= 0) {
        choices[0] = ["Next", noDickNoVagNoService];
    }

    if (player.torso.vaginas.count > 0 || player.torso.cocks.count > 0) {
        persistantChoices.push(["Leave", lossSceneLeave]);
    }
...
function lossSceneLeave(): NextScreenChoices {
    DisplayText().clear();
...
function lossSceneStay(player: Character) {
    DisplayText().clear();
    ...
    if (player.torso.cocks.count > 0) DisplayText("“<i>Or maybe,</i>”  She whispers seductively into your ear, “<i>You wanna bury that little Oni of yours in </i>me<i>.  Hmmmm?</i>”  You shudder as you feel her hand snake into your underwear and run sinuously across your crotch, readying you for the ordeal ahead...\n\n");
}

// Special loss scene if the player loses whilst being titsmothered by izumi
export function deathBySnuSnuIMeanGiantOniTits(player: Character): NextScreenChoices {
    izumiFlags.IZUMI_LAST_ENCOUNTER = 1;

    if (player.torso.hips.legs.isTaur() || player.torso.hips.legs.isDrider() || player.torso.cocks.count <= 0) {
        return fuckedUpByAFuckhugeOni(player, true);
    }
    ...
    if (player.tallness <= 60) {
        ...
        if (player.torso.cocks.sort(Cock.LargestCockArea)[0].length <= 5) {
            ...
            DisplayText(" trembling, undersized prick");
            if (player.torso.cocks.count > 1) DisplayText("s");
            DisplayText(", from tip to base, as your hips roll into your backstroke.  You shiver in embarrassment, causing Izumi to snort with amusement.\n\n");
        }
    }
    ...
    DisplayText("Finally, you start to feel an ominous pressure building up in your groin, your breathing growing laboured and desperate.  Izumi clearly notices your animalistic intent, as she looks down at you and speaks, massaging the back of your neck vigorously as you suck forcefully at her breast.\n\n");
    ...
    DisplayText("You positively explode, throwing open your mouth and howling in ecstasy as your long-awaited orgasm bursts out of you.  You feel a jet of hot, warm goo pulse free from");
    if (player.torso.cocks.count > 1) DisplayText(" each of");
    DisplayText(" your iron-hard cock");
    if (player.torso.cocks.count > 1) DisplayText("s");
    DisplayText("... only to splatter instantly against the insides of your [armor].  Your eyes go wide as you realize in a rush of embarrassment that you forgot to even so much as slip your cock");
    if (player.torso.cocks.count > 1) DisplayText("s");
    DisplayText(" free before humping yourself to a climax against Izumi’s stomach.  The realisation does nothing to hinder your orgasm, however - and may even have heightened it - as surge after surge of baby batter explodes free from your erection");
    if (player.torso.cocks.count > 1) DisplayText("s");
    DisplayText(", coating the insides of your underwear with thick, sticky goo.\n\n");

    DisplayText("Your underwear thoroughly decorated, your cock");
    if (player.torso.cocks.count > 1) DisplayText("s relent");
    else DisplayText(" relents");
    DisplayText(", deflating and ceasing");
    if (player.torso.cocks.count > 1) DisplayText(" their");
    else DisplayText(" it’s");
    DisplayText(" relentless demands.  Puffing and panting, you manage to raise your head enough to look up at Izumi, still wearing that everpresent smirk.\n\n");
...
function cowedIntoPuttingItInIntoHerGiantOniButt(player: Character): NextScreenChoices {
    lossSceneStay(player);

    DisplayText("“<i>Hey,</i>”  Izumi whispers, fingers rolling tantalizingly over [eachCock].  “<i>Let’s try something dirty...</i>”\n\n");

    DisplayText("She pushes you gently onto your back, pulling your clothes aside and freeing your erection");
    if (player.torso.cocks.count > 1) DisplayText("s");
    DisplayText(" to the air.  Her grin widens as she stands, removing her clothing - such as it is - and hurling it across the cave.  She stands there for a moment, hands on her hips, eyeing up your");
    if (player.torso.cocks.count > 1) DisplayText(" [multicock]");
    ...
    if (player.torso.cocks.count > 1) DisplayText("s");
    DisplayText(" twitch in response to the sight.");
    if (player.torso.cocks.count > 1) DisplayText("  They");
    else DisplayText("  It");
    DisplayText(" must have been visible to Izumi, too, since she laughs and speaks up a moment later.\n\n");
    ...
    DisplayText("The heat from your enthusiastic lovemaking continues to rise, and before long, Izumi is openly panting from exertion, her tongue lolling out of her mouth in arousal. Her face is flushed, and a few errant beads of sweat are beginning to trail down her back by the time she speaks again.\n\n");
    ...
    DisplayText("With a primal roar, you bury yourself to the hilt in the Oni girl’s backside as your orgasm explodes out of you.  Izumi lets out a triumphant laugh, arching her back in pleasure at the sensation of your cum flooding her insides");
    if (player.torso.cocks.count > 1) {
        DisplayText(", the cock");
        if (player.torso.cocks.count > 2) DisplayText("s");
        DisplayText(" not buried to the root in Izumi’s butt erupting in concert, splattering the joining between you with musky jizz.");
    }
    else {
        DisplayText(".");
    }
...
function fuckhugeOniWantsYourBabiesOrSomeShit(player: Character): NextScreenChoices {
    lossSceneStay(player);
    ...
    if (player.torso.balls.quantity > 0) DisplayText(" balls");
    else DisplayText(" load");
    DisplayText(" into her twitching pussy.\n\n");

    // TODO: Add cumq variants

    DisplayText("When you finally release her, the two of you can only gasp for breath as you sag back onto the ground.  Izumi avoids your gaze, blushing furiously as she absently wipes her mouth with the back of her hand.\n\n");
...
function noDickNoVagNoService(player: Character): NextScreenChoices {
    lossSceneStay(player);
    izumiFlags.IZUMI_SEEN_PC_GENDER = player.gender;

    DisplayText("Izumi’s fingers stroke questingly over your groin for a moment, then she frowns, confused.\n\n");
    ...
    DisplayText("Before you can protest, you find yourself being spun effortlessly upside down by the oversized Oni, suspended by your");
    if (player.torso.hips.legs.isNaga()) DisplayText(" tail");
    else DisplayText(" ankle");
    DisplayText(") in an extremely uncomfortable position.  Izumi’s grin becomes positively feral as she tears your clothing free");
    if (!player.torso.hips.legs.isNaga()) DisplayText("before grabbing onto your other ankle, forcibly spreading your legs apart and leaving you creating a very undignified Y-shape");
    DisplayText(".\n\n");

    DisplayText("“<i>Well, " + heightDesc(player) + ", you really are weird... no dick </i>or<i> pussy.  I, err...</i>”  She says, the confusion caused your evident lack of equipment spreading across her face.  “<i>I gotta say, I’m fresh out of ideas over here.</i>”\n\n");
...
function littleChampLittleChampFuckhugeOniIsCominTaEatcha(player: Character): NextScreenChoices {
    lossSceneStay(player);

    if (izumiFlags.IZUMI_SEEN_PC_GENDER !== 2 && player.torso.cocks.count <= 0) {
        DisplayText("Izumi’s fingers stroke questingly over your slit for a moment, then she frowns, confused.\n\n");

        DisplayText("“<i>Huh. You, uh.. You actually </i>don’t<i> have a dick.</i>”  She says, sounding a little taken aback.  “<i>You know, I gotta say, I think you’re the first person I’ve met on this rock without a wang? Seriously. I just kind of assumed you had one. Kinda lost track of my gameplan here, ‘cause I was really expecting something to grab onto down here.</i>”  She taps her lip with her free hand while you stare at her, bemused.  Then an ominous, toothy grin spreads slowly across her face.  “<i>Okay, new plan.</i>” She announces, then her hand snaps out to snatch hold of your");
        if (player.torso.hips.legs.isNaga()) DisplayText(" tail");
        else DisplayText(" ankle");
        DisplayText(" with an iron grip.  “<i>I guess I’ll just have to eat ya.</i>”\n\n");
    }
    ...
    DisplayText(" in an extremely uncomfortable position.  Izumi’s grin becomes positively feral as she tears your clothing free before");
    if (!player.torso.hips.legs.isNaga()) DisplayText(" grabbing onto your other ankle, forcibly spreading your legs apart and leaving you creating a very undignified Y-shape");
    DisplayText(".\n\n");
    ...
    DisplayText("Detaching her mouth from your crotch with a wet gasp, Izumi chuckles to herself as she lowers you gently to the floor.\n\n");
...
export function touchThatFluffyHorn(player: Character, izumi: Character): NextScreenChoices {
    izumiFlags.IZUMI_LAST_ENCOUNTER = 3;
    izumiFlags.IZUMI_TIMES_GRABBED_THE_HORN++;

    DisplayText().clear();

    DisplayText("Izumi slips backwards onto one knee and holds up a hand for pause,");
    if ((player.stats.HP / player.stats.maxHP()) <= 0.1) DisplayText(" the brief reprieve giving both of you a moment to recover.\n\n");
    else if (izumi.stats.lust >= 100) DisplayText(" the Oni’s cheeks tinged a rosy peach in contrast to her alabaster skin.\n\n");
    else DisplayText(" momentarily staggered by your furious onslaught.\n\n");
    ...
    if (User.settings.silly()) {
        ...
        DisplayText("And then you feel it.  Weakly at first, but growing in strength with every passing moment; waves of energy start rolling off of Izumi, each stronger than the last, making your hair stand on end.  Somehow, the oni’s roar is causing her to positively <i>radiate</i> power – and that power is growing stronger every second.  You begin to worry that you might actually be in real danger here.  Izumi was pretty strong when she claimed to be just playing around and using only a single hand to fight you, but there’s nothing playful about her appearance now.  In fact, in her strange, crouched stance, muscles straining against some invisible force, snarling and roaring at the empty air as she stares into nothing, you can’t help but feel she looks like nothing more than a crazed monster...  a demon, straight out of a story made to scare wayward children.  You have to do something, you realize.  You have to interrupt her, before she finishes whatever it is she’s doing!\n\n");
        ...
        DisplayText("Specifically, it gets a whole lot redder.\n\n");

        DisplayText("Izumi blinks, once, twice, as the hair-raising aura dissipates almost immediately.\n\n");

        DisplayText("“Um,” she says, surprisingly quietly given her formerly berserk appearance.  “C-Could you please let go of that?” she asks, slowly sinking to her knees.  Your feet touch solid ground again, Izumi’s hand still wrapped limply around your neck, apparently forgotten.\n\n");
        ...
        DisplayText("Experimentally, you roll your thumb over the rocky surface of the horn.  Instantly, Izumi <i>visibly</i> flinches, her eyes widening in surprise and shock.  “<i>Hey,</i>” she says, simply, her voice wavering.  “<i>Hey,</i>” she repeats. “<i>Come on now.  Enough is enough, right?</i>”\n\n");
    }
    // Super awesome Gedan edit mode!
        ...
        // Tease victory
        if (izumi.stats.lust >= 100) // I assume LustMax is always presumed to be 100?
        {
            ...
            DisplayText("[teaseText]");
            DisplayText("Izumi pays your attempts to distract her no mind, focused entirely on her strange ritual. You slink forwards, hoping that something a little more... hands on might do the trick. But ");
        }
        else {
            DisplayText("You dart forwards, hoping to catch Izumi off-guard while she’s distracted, but ");
        }

        DisplayText("as soon as you get close, those crazed, staring eyes lock onto yours - ");

        if (player.stats.spe >= 90) {
            DisplayText("her arm lunges out in an uppercutting grab, but your reflexes are faster, managing to dodge out of the way at the last moment.  Sensing an opportunity with the Oni suddenly off-balance, you make for a grab of your own - reaching out for the bright red curve of Izumi’s horn.\n\n");
        }
        else if (player.stats.str >= 90) {
            DisplayText("and before you can react, her arm lunges out in an uppercutting grab, her fist latching around your throat and wrenching you from the floor.  You lash out, swinging for Izumi’s arm, managing to pound against the iron-hard limb with enough force to dislodge the choking collar of fingers from your neck.  Sensing an opportunity with the Oni momentarily recovering, you make for a grab of your own - reaching out for the bright red curve of Izumi’s horn.\n\n");
        }
        else if (player.stats.int >= 75) {
            DisplayText("her arm lunges out in an uppercutting grab, her fist latching around your throat and wrenching you from the floor.  Izumi is far too strong to best directly; leverage, you realize, is what you need to free yourself.  You reach out for the bright red curve of Izumi’s horn. \n\n");
            ...
            else DisplayText("thinking quickly, you slip open your [armor] with one hand, bearing your [cock] to the world, your other hand reaching for the bright red curve of Izumi’s horn, ");

            DisplayText("trying to force her gaze toward your ‘assets.’\n\n");
        }
        else {
            DisplayText("You lash out, swinging at Izumi’s face, but you can’t concentrate while being held off the floor by your neck!  In desperation, you make a grab for the one thing you can think of to try steadying yourself to brace for a strike - the bright red curve of Izumi’s horn.\n\n");
        }

        DisplayText("As soon as your fingers wrap around it, however, Izumi’s brutal, victorious expression changes.  Specifically, it gets a whole lot redder.\n\n");

        DisplayText("Izumi blinks once, twice, as the hair-raising visage she was maintaining drops almost immediately.\n\n");

        DisplayText("“<i>Um,</i>”  She says, surprisingly quietly given her formerly berserk appearance.");
        if (player.stats.spe < 90 || player.stats.str < 90) {
            DisplayText("“<i>C-Could you please let go of that?</i>” She asks, slowly sinking to her knees.  Your feet touch solid ground again, Izumi’s hand still wrapped limply around your neck, apparently forgotten.\n\n");
            DisplayText("“<i>Really. Um. Please let go.</i>”");
        }
        else DisplayText("“<i>C-Could you please let go of that?</i>”");
    ...
    const choices = [];
    choices.push(["Let Go", letGoAndLeaveYouSillyFuck]);

    izumiFlags.IZUMI_SEEN_PC_GENDER = player.gender;

    if (player.torso.vaginas.count > 0) {
        choices.push(["TribHorn", tribThatRockyHornGirl]);
    }

    if (player.torso.cocks.count > 0) {
        choices.push(["LetsFuck", letGoAndFuck]);
        choices.push(["RevengeAnal", gonnaGetMeSomeRevengeButtsexin]);
        choices.push(["RevengeOral", sayThatsAPrettyFaceBeAShameIfSomebodyJizzedAllOverIt]);

        if (player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE)).length >= 8) choices.push(["Tentacruel", tentacruelRevengeFux]);
    }
    return { choices };
}

// Pfft, nofux
function letGoAndLeaveYouSillyFuck(): NextScreenChoices {
    DisplayText().clear();
...
// Kinda friendly fux
function letGoAndFuck(player: Character): NextScreenChoices {
    DisplayText().clear();
    ...
    if (player.tallness < 80) DisplayText(" down");
    else if (player.tallness > 108) DisplayText(" up");
    DisplayText(" at you, planting her hands on her hips.  “<i>So, what are ya gonna do to me now you’ve won, hotshot?</i>”\n\n");
    ...
    if (player.tallness < 80) DisplayText(" from above");
    else if (player.tallness > 108) DisplayText(" from below");
    DisplayText(".\n\n");

    DisplayText("Your roving hands locate one already hardening nipple and bring it up to your mouth. Izumi lets out a hiss as you latch onto her breast, sucking and licking away at the stiffening flesh with abandon.\n\n");
    ...
    DisplayText("“<i>Well?  Come on already,</i>” she says, smirking lasciviously.  “<i>You wanted to do me, right? Well then, you’d better hurry up and get started before I get embarrassed...</i>”\n\n");

    DisplayText("You advance on Izumi, noting the hungry light in her eyes as you remove your [armor].  Her half-discarded kimono acts as a sort of blanket, softening the ground as you crawl in between the enormous woman’s thighs, positioning yourself to thrust into your new conquest.  “<i>Yeah, let’s do it, " + heightDesc(player) + "... I’m ready, so show me what you’ve got.</i>”  She murmurs, half to herself, and you’re only too happy to oblige.\n\n");
    ...
    if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 1 && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating < 3) DisplayText(" smothering your [chest] with her own, much larger, pair.\n\n");
    else if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 3 && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating < 5) DisplayText(" your [chest] moulding themselves against the Oni’s larger pair.\n\n");
    else if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 5 && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating <= 8) DisplayText(" your [chest] compressing against the Oni’s larger pair, forming a delicious valley of flesh.\n\n");
    else if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 8) DisplayText(" your [chest] dominating the Oni’s own in the battle for space between the two of you.\n\n");
    else DisplayText(" pressing you into her chest.\n\n");

    DisplayText("“<i>You don’t need to be so restrained.  It’s not like you’re gonna hurt me if you pick up the pace a little.</i>”  She looks away, her blush visibly deepening.  “<i>N-Not that this isn’t nice by itself, you understand, I just... you know, if ya want to, you can go faster, I don’t mind...</i>”\n\n");
    ...
    if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 0) DisplayText(" against your own");
    DisplayText(" as she gasps for air.\n\n");
    ...
    DisplayText("“<i>Haha, hahh,</i>” Izumi mutters, sounding almost regretful, “<i>Ah, geez. I really came, huh... and from you playing with my horn, too...</i>” she laments.  She covers her eyes with one hand and grins ruefully, her face still reddened from her recent orgasm.  “<i>That’s so embarrassing...</i>”\n\n");
    ...
    DisplayText("“<i>U-Um. Thanks, but p-please don’t do things like that again,</i>” she stammers. “<i>Not that I hate you for it, or anything!  It was nice, it’s just, I-I don’t, uh... I don’t really know what to, uh, you know...</i>”  She sees your expression and sighs, apologetically.  “<i>Sorry, I just never know how to react to stuff like  Don’t worry about it, okay?</i>”  She smiles, stroking your hair lovingly.\n\n");

    DisplayText("You decide not to make an issue of it, since Izumi is clearly embarrassed.  The two of you redress, and as you make to leave, Izumi whistles for your attention.  You turn, and she throws you a small leather bag, which you open to discover a handful of gems inside.\n\n");
...
function gonnaGetMeSomeRevengeButtsexin(player: Character): NextScreenChoices {
    DisplayText().clear();
    ...
    DisplayText("“<i>Okay, fine,</i>” She says hotly, slipping out of her clothing. “<i>You’re just gonna take a quick look and then you’ll let go, right?  And by the way, you’re pretty perverted for making a girl strip in front of you, you know?!</i>”  She has no idea...\n\n");
    ...
    if (player.torso.cocks.sort(Cock.LargestCockArea)[0].length <= 4) {
        ...
        DisplayText("<i>Smack!</i>  Izumi’s head jerks around, her expression thunderous, as the open palm slap hammers home onto her rump.\n\n");

        DisplayText("“<i>What the </i>hell<i> do you think you’re doing, kid?!</i>”  She bellows.  You grin happily at her, glad you’ve finally gotten her attention.\n\n");

        DisplayText("“<i>Tell me how big it is.</i>”  You command.\n\n");
        ...
        DisplayText("“<i>It’s big,</i>” she spits.\n\n");

        DisplayText("“<i>How big?</i>” you ask, feeling pleased with yourself.\n\n");

        DisplayText("“<i>Fuck, I don’t-</i>”  You raise an eyebrow, your hand still raised threateningly above her reddening rump.  She glares at it for a moment.  “<i>Fine. I get it, already. It’s.. Really big, okay?  Huge.  Massive.</i>”  She growls.\n\n");

        DisplayText("“<i>Keep going.</i>”  You say, lowering your hand to caress her cheeks as you begin to thrust once again.\n\n");
        ...
        DisplayText("“<i>Ahhn.  Nooo.  If you keep this up, I’ll become your slave.</i>”  She sighs, her voice monotone.");
        if (player.stats.cor >= 50) DisplayText("  You hate to admit it, but in a messed up way, her apathy is beginning to turn you on a bit by itself.  You knew you weren’t the most well-endowed adventurer in the first place, and to someone physically huge like Izumi the problem must be magnified.  For some reason, you find that idea hot as hell....\n\n");
    ...
    // Regular dicks
        ...
        DisplayText("“<i>Oh, just going all the way");
        if (player.torso.balls.quantity > 0) DisplayText(" balls");
        else DisplayText(" hilt");
        ...
        DisplayText("“<i>Will you cut that out?!  Stop trying to get all romantic or whatever, you....  Just stop it!  Okay?!  I thought you were meant to be </i>raping<i> me!?</i>”\n\n");

        DisplayText("Well, if she insists... reaching up, you grab a handful of her golden mane, wrap it around your fists and start to pound forwards with your hips, ramming yourself repeatedly home against Izumi’s plowable cheeks while jerking backwards on your newfound ‘reins’ for support.\n\n");
        ...
        if (player.cumQ() < 1500) {
            ...
            DisplayText("But you refuse, holding yourself hilted inside her, still wrapped up in the throes of your extended orgasm to care a jot for Izumi’s panic.  She’s forced to simply wait, clutching her aching stomach and whimpering in confusion, until you decide that the sex is over.\n\n");
    ...
    // Urtadicks plz
        ...
        DisplayText("Her expression soon changes as you unveil your [cock biggest], however.  One look at your thickening meat is all it takes to evaporate the last of her bad attitude, replacing it with fear.\n\n");
        ...
        DisplayText("Finally, you can cum no more, and Izumi slumps down with fatigue as her orgasm subsides too.  You withdraw, with agonizing slowness, from Izumi’s slick and exhausted hole; it attempts to close up behind you, but it looks like it might take a while before she’s fully back to normal.\n\n");
        ...
        DisplayText("You turn to say something to Izumi, but to your surprise you discover she’s actually fallen asleep.  Shaking your head in amusement at the Oni girl’s inferior stamina, you make your way out of the cave and back to camp, counting the gems as you go. \n\n");
    }
...
// Huehue, Tentacruel!
function tentacruelRevengeFux(player: Character): NextScreenChoices {
    DisplayText().clear();
    ...
    DisplayText("With the danger averted, you’re free to enjoy yourself and waste no time in enthusiastically beginning to fuck Izumi’s throat.  She gags and splutters around your invading member, the rumbling of her muffled, mumbling protests adding a fun extra degree of sensation to the facefuck.\n\n");
...
// Facefuck
function sayThatsAPrettyFaceBeAShameIfSomebodyJizzedAllOverIt(player: Character): NextScreenChoices {
    DisplayText().clear();
    ...
    DisplayText("Finally, you’re fully erect, Izumi’s occasionally-wincing gaze locked on the [cockhead biggest] of your [cock biggest], bobbing in the air in front of her.  Your smile widens into a leer as you put phase two of your ingenious plan into action.\n\n");

    DisplayText("“<i>Kiss it,</i>” you order. Izumi’s eyes flicker briefly to look up at your face, but by this point she knows better than to attempt disobedience.  Leaning hesitantly forwards, Izumi purses her lips as she moves in for the kiss.");

    const largestCock = player.torso.cocks.sort(Cock.LargestCockArea)[0];

    if (largestCock.type === CockType.HORSE) DisplayText("  Visibly unsure of where to put herself, Izumi finally settles for brushing her lips against the exact center of the flared mass that makes up the head of your stallion-prick.");
    else if (largestCock.type === CockType.DOG) DisplayText("  Her eyes darting nervously from side to side, Izumi leans forwards and takes the very tip of your dogcock into her mouth, pressing her lips against the main bulk of what passes for your head.");
    else DisplayText("  You feel the soft, warm sensation of Izumi’s lips brushing against the sensitive head of your cock for a moment, then the process repeats itself as Izumi actually darts in again, unbidden, for a second smooch.");
    DisplayText("\n\n");
    ...
    DisplayText("Her mouth latches onto you like a vacuum, sucking on your [cockHead biggest] so strongly it makes your knees weak, her tongue performing bizarre oral gymnastics around your cock.  You can only grunt in surprised appreciation; whatever else you might think about her, Izumi is <i>good</i> at   Or maybe she’s just desperate, trying to force you to cum as quickly as possible so that you’ll release her precious horn?  That seems distinctly possible, so you decide to take steps to counter it.\n\n");

    DisplayText("Yanking forwards on Izumi’s horn again, you pull her bodily down your length until you’re buried to the [sheath], swallowed up completely into Izumi’s tingling-hot mouth, her nose pressed up against your stomach.  She lets out another strangled, slightly frustrated cry, staring up at you in confusion.\n\n");

    DisplayText("You order her to stop, and you can see the confusion bloom in her eyes.\n\n");
    ...
    DisplayText("From this angle, you have a much better line of approach to Izumi’s throat, giving you a far deeper maximum thrust - something you happily take advantage of almost immediately in order to fuck Izumi’s mouth in earnest.");
    ...
    if (player.cumQ() > 500) DisplayText("  Her surprise only deepens when you simply keep cumming, your prolonged orgasm flooding her mouth faster than she can swallow, forcing her cheeks to bulge out and tiny rivulets of cum to occasionally slip free of the tight seal her lips have formed around your cock.\n\n");
    ...
    DisplayText("Finally, it becomes apparent that you’re now completely done...  as is Izumi.  The two of you remain where you are, panting, for a few moments.  You’re the first to regain your feet, while Izumi burbles to herself, unable to even muster up the energy to even wipe her face.\n\n");
...
function tribThatRockyHornGirl(player: Character): NextScreenChoices {
    DisplayText().clear();
    ...
    DisplayText("“<i>Ah, shit.</i>” She mutters, just before you push her to the ground, forcing her onto her back and swinging your leg over to straddle her chest.  She lets out a little ‘Oof!’ as you land.  You make a mental note to punish her for that, too; just what is she trying to imply, here?\n\n");
    ...
    DisplayText("You let out an amused laugh.  You have to admit, it’s pretty funny that the big bad muscle-girl couldn’t manage to control herself even while she’s technically being raped.\n\n");

    DisplayText("“<i>Having fun?</i>”  You ask, smiling to yourself.  Izumi’s eyes snap open, her face turning an even deeper shade of crimson than before as her eyes flicker down to her crotch.  Poor girl, you think; she might not even have fully realized what she was doing.  Well, she does now...\n\n");

    DisplayText("“<i>I-It’s... it’s not... s-shut up!</i>” She stammers, adding “<i>It’s not my f-fault if you keep t-touching me in weird places and I get... distracted!  </i>You’re<i> the perverted one here, not me!</i>”\n\n");

    if (player.stats.cor > 80) DisplayText("“<i>Whatever.  Just try not to cum too hard while I’m making you my bitch, you little slut.</i>” You shoot back, returning");
    else DisplayText("Ignoring her protests entirely, you return");
    DisplayText(" your attention to her horn.  Still though, there’s something missing; as fun as it is molesting and toying with the oversized lout currently protesting her innocence while buried up to the knuckle in her own cunt, you’re not quite feeling it enough to get off yourself.  Clearly, that has to change.\n\n");
    ...
    DisplayText("“<i>You came, didn’t you?</i>” You ask, sweetly.  Izumi trembles a little, her eyes rolling over to you.  She seems to be having trouble focusing.\n\n");

    DisplayText("“<i>..’es,</i>”  She croaks, almost inaudibly.  She sounds as though she’s on the verge of tears, presumably from embarrassment - you’re pretty certain you haven’t actually done anything that could hurt her.  You shake your head, chuckling to yourself as you redress and rummage through Izumi’s belongings for a trophy.\n\n");

    DisplayText("“<i>I’m taking this,</i>” You announce cheerfully, hefting a small bag of gems in one hand.  “<i>‘Kay.</i>” Izumi’s approval doesn’t matter to you either way, but your smile widens at her submissive, exhausted tone.\n\n");
