...
export class Swamp {
	...
	public exploreSwamp() {
		...
		if (!kGAMECLASS.kihaFollower.followerKiha() && player.stats.cor < 60 && Flags.list[FlagEnum.KIHA_AFFECTION_LEVEL] >= 1 && Flags.list[FlagEnum.HEL_FUCKBUDDY] > 0 && player.torso.cocks.count > 0 && Flags.list[FlagEnum.KIHA_AND_HEL_WHOOPIE] === 0) {
		...
		switch (choice) {
			...
			case 4:
				...
				break;
			default:
				DisplayText("New explore code fucked up.  YOU BONED (TELL FEN)");
