...
export class SandWitchTimeAware implements ITimeAware {
    public womb: FlagWomb;

    public constructor() {
        this.womb = new FlagWomb();
        // sandWitchWomb = new PregnancyStore(SandWitchFlags.EGG_WITCH_TYPE, SandWitchFlags.EGG_WITCH_COUNTER, 0, 0);
        // sandWitchWomb.addPregnancyEventSet(PregnancyType.BEE_EGGS, 96);
        // Event: 0 (= not pregnant), 1, 2 (< 96)
        // sandWitchWomb.addPregnancyEventSet(PregnancyType.DRIDER_EGGS, 96);
        // Event: 0 (= not pregnant), 1, 2 (< 96)
    }
    ...
    public deserialize(saveObject: SandWitchTimeAware) {
        this.womb.deserialize(saveObject.womb);
    }
}

const sandWitchTimeAware = new SandWitchTimeAware();
const sandWitchWomb = sandWitchTimeAware.womb;

export function encounter(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.SandWitch); // 50
    CView.clear();
    CView.text("A strange woman seems to appear from the dunes themselves.  She identifies herself as a sand witch, and politely asks if she can cast a spell on you.");
    if (player.effects.get(StatusEffectType.Exgartuan).value1 === 1 && player.body.cocks.get(0).area > 100 && player.effects.get(StatusEffectType.Exgartuan).value2 === 0) {
        CView.text("\n\nThe " + player.inventory.equipment.armor.displayName + " covering your lower half hits the ground, as if yanked down by magic.  Your " + describeCock(player, player.body.cocks.get(0)) + " pulsates darkly, growing rigid in seconds as the demon within you takes over.  It barks, \"<i>Fuck, how about I cast my spell on you baby?</i>\"\n\n");
        CView.text("The sandwitch ");
        if (player.stats.cor < 50)
            CView.text("and you both turn crimson");
        else CView.text("turns crimson");
        CView.text(" as you yank your " + player.inventory.equipment.armor.displayName + " back into place.  You're in charge here, not some possessed appendage!   Exgartuan yells something snide, but it's muffled too much to understand.  You look up in time to sidestep an attack from the Sand Witch.  It looks like you'll have to fight her!");
        return CombatManager.beginBattle(player, new SandWitch());
    }
    else return { yes: allowSandWitchMagic, no: refuseSandWitchMagic };
}

function allowSandWitchMagic(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.SandWitch); // 50
    CView.clear();
    if (player.body.hair.color === "sandy blonde") {
        CView.text("She smiles wickedly and intones, \"<i>Tresed eht retaw llahs klim ruoy.</i>\"\n\n");
        if (player.body.chest.length === 0 || player.body.chest.sort(BreastRow.Largest)[0].rating === 0) {
            ...
            player.body.chest.get(0).count = 2;
            player.body.chest.get(0).rating = 3;
            if (player.body.chest.get(0).nipples.count < 1) player.body.chest.get(0).nipples.count = 1;
            player.stats.sens += 2;
            player.stats.lust += 1;

        }
        if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 1 && player.body.chest.sort(BreastRow.Largest)[0].rating <= 2) {
        ...
        if (player.body.chest.get(0).nipples.count < 1) {
            CView.text("Two dark spots appear on your chest, rapidly forming into sensitive nipples.  ");
            player.body.chest.get(0).nipples.count = 1;
            player.stats.sens += 2;
            player.stats.lust += 1;

        }
        if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier > 0) {
            CView.text("A strong pressure builds in your chest, painful in its intensity.  You yank down your top as ");
            if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier < 2)
                CView.text("powerful jets of milk spray from your nipples, spraying thick streams over the desert sands.  You moan at the sensation and squeeze your tits, hosing down the tainted earth with an offering of your milk.  You blush as the milk ends, quite embarassed with your increased milk production.  ");
            if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier >= 2 && player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier <= 2.6)
            ...
            if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier > 2.6 && player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier < 3)
                CView.text("thick hoses of milk erupt from  your aching nipples, forming puddles in the sand.  You smile at how well you're feeding the desert, your milk coating the sand faster than it can be absorbed.  The constant lactation is pleasurable... in a highly erotic way, and you find yourself moaning and pulling on your nipples, totally outside of your control.  In time you realize the milk has stopped, and even had time to soak into the sands.  You wonder at your strange thoughts and pull your hands from your sensitive nipples.  ");
            if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier >= 3)
        ...
        if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier === 0) {
            CView.text("A pleasurable release suddenly erupts from your nipples!  Streams of milk are spraying from your breasts, soaking into the sand immediately.  It stops all too soon, though the witch assures you that you can lactate quite often now.  ");
        ...
        if (PlayerFlags.sand === 1) {
            if (player.stats.cor <= 33)
                ...
                CView.text("You wonder if you should've resisted and tried for some sex as she departs.");
            else CView.text("You wish you had said no, so you could fuck with her and her magnificent quartet of breasts some more.");
        }
    }
    else {
        CView.text("She smiles wickedly and intones, \"<i>nuf erutuf rof riah ydnas, nus tresed eht sa ydnas.</i>\"\n\nYou feel a tingling in your scalp, and realize your hair has become a sandy blonde!");
        player.body.hair.color = "sandy blonde";
    }
    return { next: returnToCampUseOneHour };
}

function refuseSandWitchMagic(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.SandWitch); // 50
    CView.clear();
    CView.text("With an inarticulate scream of rage, the Sand Witch attacks!");
    return CombatManager.beginBattle(player, new SandWitch());
}

function sandwitchRape(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.SandWitch); // 50

    // LUST DEFEAT
    if (player.stats.lust > 99) {
        // BAD END START
        if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 9 && player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier >= 3 && player.stats.cor >= 25) {
            CView.clear().text("Overwhelmed by the intense pleasure caused by the vibrating sphere inside your body, you fall to your knees.\n\nYou whimper pathetically, desperate to cum, as the Sand Witch steps nearer to you and gently strokes your face. She smiles cruelly and lets her hands drop down to your chest, then tears your top away, letting your huge breasts bounce free. The mysterious woman firmly rubs and kneads them, making you gasp and writhe, until she starts lightly flicking your painfully hard nipples. You edge closer and closer to orgasm, panting like a whore while the witch teases you with her hands and magic.\n\n");
            CView.text("Just before you cum, she leans down to whisper to you, tickling your ear with her warm breath: \"<i>Evals klim ym emoceb llahs uoy.</i>\"\n\nA familiar pressure builds in your chest, but it's far stronger than ever before. You feel your breasts swell rapidly, and milk sprays from your nipples, splattering you and the witch with white droplets. Your eyes roll back and your tongue hangs out as you cum, mind nearly breaking from the world-shattering orgasm that thunders through you.\n\nYou black out.");
            return { next: sandWitchBadEnd };
            return;
        }
        player.orgasm();
        CView.clear().text("Your wobbly legs give out underneath you as your body's will to fight evaporates.\n\n");
        if (player.body.hair.color.indexOf("sandy blonde") !== -1) {
            CView.text("The Sand Witch smiles wickedly and intones, \"<i>Tresed eht retaw llahs klim ruoy.</i>\"\n\n");
            if (player.body.chest.length === 0 || player.body.chest.sort(BreastRow.Largest)[0].rating === 0) {
                CView.text("You grow a perfectly rounded pair of C-cup breasts!  ");
                if (player.body.chest.length === 0) player.body.chest.add(new BreastRow());
                player.body.chest.get(0).count = 2;
            ...
            if (player.body.chest.sort(BreastRow.Largest)[0].rating > 0 && player.body.chest.sort(BreastRow.Largest)[0].rating < 3) {
                CView.text("Your breasts suddenly balloon outwards, stopping as they reach a perfectly rounded C-cup.  ");
                player.body.chest.get(0).rating = 3;
                player.stats.sens += 1;
                player.stats.lust += 1;
            ...
            if (player.body.chest.reduce(BreastRow.AverageNipplesPerBreast, 0) < 1) {
                ...
                let temp = player.body.chest.length;
                while (temp > 0) {
                    temp--;
                    // If that breast didnt have nipples reset length
                    if (player.body.chest.get(0).nipples.count < 1) player.body.chest[0].nippleLength = .2;
                    player.body.chest.get(0).nipples.count = 1;
                }
                player.stats.sens += 2;
                player.stats.lust += 1;
            ...
            if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier > 0) {
                CView.text("A strong pressure builds in your chest, painful in its intensity.  You yank down your top as ");
                if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier < 2) CView.text("powerful jets of milk spray from your nipples, spraying thick streams over the desert sands.  You moan at the sensation and squeeze your tits, hosing down the tainted earth with an offering of your milk.  You blush as the milk ends, quite embarassed with your increased milk production.  ");
                if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier >= 2 && player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier <= 2.6) CView.text("eruptions of milk squirt from your nipples, hosing thick streams everywhere.  The feeling of the constant gush of fluids is very erotic, and you feel yourself getting more and more turned on.  You start squeezing your breasts as the flow diminishes, anxious to continue the pleasure, but eventually all good things come to an end.  ");
                if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier > 2.6 && player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier < 3) CView.text("thick hoses of milk erupt from your aching nipples, forming puddles in the sand.  You smile at how well you're feeding the desert, your milk coating the sand faster than it can be absorbed.  The constant lactation is pleasurable... in a highly erotic way, and you find yourself moaning and pulling on your nipples, totally outside of your control.  In time you realize the milk has stopped, and even had time to soak into the sands.  You wonder at your strange thoughts and pull your hands from your sensitive nipples.  ");

                if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier >= 3) CView.text("you drop to your knees and grab your nipples.  With a very sexual moan you begin milking yourself, hosing out huge quantities of milk.  You pant and grunt, offering as much of your milk as you can.  It cascades down the dune in a small stream, and you can't help but blush with pride... and lust.  The erotic pleasures build as you do your best to feed the desert all of your milk.  You ride the edge of orgasm for an eternity, milk everywhere.  When you come to, you realize you're kneeling there, tugging your dry nipples.  Embarrassed, you stop, but your arousal remains.  ");
                if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier < 3) {
            ...
            if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier === 0) {
                CView.text("A pleasurable release suddenly erupts from your nipples!  Twin streams of milk are spraying from your breasts, soaking into the sand immediately.  It stops all too soon, though the witch assures you that you can lactate quite often now.  ");
                boostLactation(player, 1);
                player.stats.lib += .5;
                player.stats.sens += 1;
                player.stats.lust += 10;
...
function sandWitchBadEnd(player: Character): NextScreenChoices {
    CView.clear();
...
function sandWitchBadEndPartThree(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Something breaks within your soul, accompanied by a strangled sob. Tears of humiliation trickle down your face as you part your quivering lips. \"<i>Please milk me, Mistress. Your slave needs to be milked.</i>\"\n\n");
    if (player.perks.has(PerkType.MarblesMilk)) {
        CView.text("No sooner have you spoken before a torrent of milk spurts from each of your nipples, but the effect isn't as pleasurable as it used to be; in fact you're starting to feel very weak.  The Sand Witch looks at you in confusion as the stream of milk from your massive breasts quickly slows and your breasts start to shrivel up.\n\n");
        CView.text("Trying to figure out why you feel so incredibly weak, you think back on what might have caused this.  When was the last time you had some of Marble's milk?  Feeling your body start to wither away, the realization hits you.  You chuckle softly at your captor as the world starts to go dark; looks like you'll be the one to have the last laugh...\n\n");
        CView.text("The last thing you hear is the Sand Witch screaming \"<i>What the hell is going on?  I ALWAYS get what I want!</i>\"");
    }
    else {
...
function sandwitchRaped(player: Character, monster: Character): NextScreenChoices {
    ...
    if (player.gender === 1 || player.gender === 3) {
        ...
        // Really corrupt
            ...
            if (player.body.cocks.length === 1) {
                // DOGGIES
                if (player.body.cocks.filter(Cock.FilterType(CockType.DOG)).length > 0 && player.body.cocks.get(0).knotMultiplier > 1.4) knotSandwitch(player);
                else {
                    CView.text("You place your " + describeFoot(player) + " in the wet sand, pulling her hair and making her look you in the eyes. The sand witch looks shocked at your display of power, only turning you on more. You undo your " + player.inventory.equipment.armor.displayName + " and grope her nethers, only to find a surprise. She has " + numToCardinalText(player.body.cocks.length + 1) + " pussies and each seems to be made to fit you exactly. Pushing her to the ground, you bring the seeping head of your " + describeCock(player, player.body.cocks.get(0)) + " to one of her nethers, slowly easing into her depths.  ");
                    CView.text("Thrusting deep, you hit bottom, and marvel at the sensations as you feel the head of your " + describeCock(player, player.body.cocks.get(0)) + " rub against the end of her slick cunts, only making you lust after her more. As though responding to your will, your " + describeCock(player, player.body.cocks.get(0)) + " grows just a tiny bit more in length and thickness.  ");
                    CView.text("With a satisfied growl of lust from yourself, and a screech of pain from the sand witch, you break past her cervix and into her womb. Pulling out, you rub yourself against her wet outer lips, bringing yourself closer and closer to your peak as the sand witch struggles, her eyes turning wide with fright as she starts to understand you hold more power than her.  ");
                    CView.text("With a bestial cry of lust, you shove your shaft deep within one of her vaginas, past her cervix and into her womb where you blast your potent, corrupted, demon-tainted seed.  ");
                    CView.text("\n\nThe sand witch screams in savage denial as you plant your seed deep within her body, your corruption no doubt making it extra potent. You gaze into her eyes, reveling in her expression of horror. Without uttering a word, you continue to fuck her, making sure your seed is well-entrenched within her womb, feeling it splatter and bubble around your " + describeCock(player, player.body.cocks.get(0)) + ".  You wonder if she will remember the pleasure her body felt today, and if she will be more likely to drop to her knees before you than to fight... ");
                    CView.text("Finally finished, you pull your " + describeCock(player, player.body.cocks.get(0)) + " from one of her well-used slits, your " + describeCock(player, player.body.cocks.get(0)) + " drooling cum even now. With a grin, you walk away, happy with your little encounter.  ");
                    player.orgasm();
                }
            }
            // Multiprick
            if (player.body.cocks.length > 1) {
                ...
                CView.text("Using the corruption in your blood, you place your " + describeFoot(player) + " upon the budding shoots, and wipe them from existence. The sand witch looks shocked at your display of power, only turning you on more. You undo your loin cloth and grope her nethers, only to find a surprise. She has " + numToCardinalText(player.body.cocks.length + 1) + " loose wet cunts and each seems to be made to fit you exactly. Pushing her to the ground, you bring the seeping heads of your " + describeCocksLight(player) + " to her nethers, slowly easing into her depths.  ");
                CView.text("Thrusting deep, you hit bottom, and marvel at the sensations as you feel the heads of your " + describeCocksLight(player) + " rub against the end of her wet pussies, only making your desire to penetrate increase. As though responding to your will, your " + describeCocksLight(player) + " grow just a tiny bit more in length and thickness.  ");
                CView.text("With a satisfied growl of lust from yourself, and a screech of pain from the sand witch, you break past her cervixes and into her wombs. Pulling the heads of your " + describeCocksLight(player) + " from her wombs, you rub and rut at the outside of her cunts, bringing yourself closer and closer to your peak as the sand witch struggles, her eyes turning wide with fright as she starts to understand you hold more power than her.  ");
                CView.text("With a bestial cry of lust, you shove your shafts deep within her vaginas, past her cervices and into her womb where you blast your potent, corrupted, demon-tainted seed.  ");
                // A few dicks...
                CView.text("\n\nThe sand witch screams in savage denial as you plant your seed deep within her body, your corruption no doubt making it extra potent. You gaze into her eyes, reveling in her expression of horror. Without uttering a word, you continue to fuck her, making sure your seed is well-entrenched within her wombs, feeling it splatter and bubble around your " + describeCocksLight(player) + ".  You wonder if she will remember the pleasure her body felt today, and if she will be more likely to drop to her knees before you than to fight... ");
                CView.text("Finally finished, you pull your " + describeCocksLight(player) + " from her well used slits, your " + describeCocksLight(player) + " drooling cum even now. With a grin, you walk away, happy with your little encounter.  ");
            }
            return { next: returnToCampUseOneHour };
        }
    }
    // Femfem
        ...
        CView.text("You smile at the sand witch, trying to seem enticing. A tiny smile plays across her face even though she supports a worried frown. She reaches up to you, and you allow her to gently pull you down to her. You marvel at her multiple breasts, each perfectly rounded. Each has a nipple approximately two inches long, enough to suck if you wanted to.  ");
        CView.text("Your attention wanders down to the damp sand underneath her legs, and her musky scent hits your nose.\n\n");
        return sandwitchBewbs(player);
    }
}

function sandwitchBewbs(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.SandWitch); // 50
    CView.clear();
    CView.text("You move yourself atop the defeated sand witch, and bring your lips to her breasts.  The moment the first drop of milk touches your tongue, you drink, hungrily, needfully, demandingly. You grope and massage her breasts as she leans back and cries out, her eyes wide in surprise. You guzzle and drink her sweet cream, the small seedlings underneath you appearing to rustle and move as you drink the alabaster flow once intended for the desert sands.  ");
    CView.text("Warmth fills your gluttonous belly, and the sand witch places a hand to your stomach, rubbing it lightly as you drink from her breasts, moving on when the one you're drinking from is drained.  ");
    CView.text("You burp, and flush scarlet, but the sand witch lays a hand against your face, leans close, and kisses your lips sweetly. Her tongue snakes out and dives into your mouth. She pulls your face against hers, grinding her breasts against your chest. You can feel fresh milk dribbling against your " + player.body.skin.desc + " and the scents are driving you wild with lust.  ");
    CView.text("She breaks the kiss and smiles at you, whispering. Your eyes slowly close as you grow drowsy; you realize too late that she's casting a spell. You fall asleep, the sand witch's milk in your stomach churning, gurgling.  ");
    CView.text("You wake up in the desert, immediately grabbing at your body to see if anything's changed...");
    player.slimeFeed();
...
function sandwitchSex(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.SandWitch); // 50
    CView.clear();
    CView.text("With a charming, disarming smile, you reach between her legs and touch against the slick lips of her pussies. She has ");
    if (player.body.cocks.length > 1) CView.text(numToCardinalText(player.body.cocks.length));
    else CView.text("two");
    CView.text(" of them!  The sand witch appears frightened, but as you gently tease her lips with your fingers, she slowly starts to breathe faster, starting to moan as her legs spread themselves against her better judgment. Grinning in satisfaction, you bring your rigid " + describeCock(player, player.body.cocks.get(0)) + " against her lips, and push slowly, gently.  ");
    CView.text("The sand witch cries out, her hands grabbing your arms, pressing hard against your " + player.body.skin.desc + " as you slowly and teasingly ease your " + describeCock(player, player.body.cocks.get(0)) + " into her tight depths. You almost feel like a virgin, your " + describeCock(player, player.body.cocks.get(0)) + " throbbing, the need to cum almost beyond your ability to control.  ");
    CView.text("You rest against her chest, her large breasts squirting milk against your own chest as your weight appears to excite her enough to make her lactate, if what you're doing already wasn't enough.  ");
    CView.text("You kiss the sand witch, getting accustomed to her unbelievable tightness. You hump backwards and forwards, grinning like a fool with his first girlfriend. Not even a succubus would be able to rival her lush depths.  ");
    CView.text("You finally get serious and ease yourself into fucking the sand witch, slowly gaining speed and depth as your groins slap together with such ferocity that a normal person would be bruised upon their next waking. You pound the sand witch, making her scream in ecstasy, her already tight depths turning vice-like as you grunt your lust, cumming and filling her depths. Your ");
    if (player.body.balls.count >= 2) CView.text("balls ache and your ");
    CView.text(describeCock(player, player.body.cocks.get(0)) + " throbs with need, even as you empty your lust into the defeated sand witch.");
    CView.text("\n\nWith a satisfied smirk, you lay atop your prize, your " + describeCock(player, player.body.cocks.get(0)) + " still twitching within her depths. One of the sand witch's hands rubs at your head, teasing your " + player.body.skin.desc + ". Like a fresh youth, you fall asleep, " + describeCock(player, player.body.cocks.get(0)) + " buried deep in the sand witch.  ");
    CView.text("With a start you wake up, only to find the sand witch still passed out beside you.  Almost thinking it was a dream, you touch your groin tenderly and rub softly. There's a faint trace of soreness, and you remember the ferocity with which you mashed your pelvises together.  You get up and get ready.  ");
    player.orgasm();
    return { next: returnToCampUseOneHour };
}
/*
...
function sandwitchCentaurBoning(player: Character, monster: Character) {
    CView.sprite(SpriteName.SandWitch); // 50
    player.orgasm();
    CView.text("The Sand Witch is panting before you, her clothes in disarray ");
    // [win via HP]
    if (monster.stats.HP < 1) CView.text("and torn in many places. ");
    // [win via lust]
    else CView.text("and her fingers squeezing her nipples as lust overwhelms her. ");
    // [has breasts]
    if (player.body.chest.sort(BreastRow.Largest)[0].rating > 3) {
        CView.text("As you approach, her eyes drift up to your " + describeAllBreasts(player) + " and she ");
        // [win via HP]
        if (monster.stats.HP < 1) CView.text("is unable to stop herself from slowly licking her lips. ");
        // [win via lust]
        else CView.text("sensually licks her parted lips. ");
    }
    // [lactating]
    if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier > 1) {
        CView.text("Her eyes widen when she sees the beads of milk already dripping from your " + describeNipple(player, player.body.chest.get(0)) + "s as her hand drops between her legs and she begins to slowly masturbate.");
    }
    CView.text("\n\n");
    // [Corruption < 70]
    if (player.stats.cor < 70) {
        CView.text("You smile as you approach");
        // [has breasts]
        if (player.body.chest.sort(BreastRow.Largest)[0].rating > 0) CView.text("; your " + describeNipple(player, player.body.chest.get(0)) + "s  become stiff");
        // [cock only]
        if (player.body.cocks.length > 0 && player.body.vaginas.length <= 0) CView.text(" and your " + describeCock(player, player.body.cocks.get(0)) + " hardens.");
        // [cunt only]
        if (player.body.vaginas.length > 0 && player.body.cocks.length === 0) CView.text(" and your " + describeVagina(player, player.body.vaginas.get(0)) + " begins to drip with moisture.");
        // [cock and cunt]
        if (player.body.vaginas.length > 0 && player.body.cocks.length > 0) CView.text(" and your " + describeCock(player, player.body.cocks.get(0)) + " hardens as your " + describeVagina(player, player.body.vaginas.get(0)) + " begins to drip with moisture.");
        if (player.gender === 0) CView.text(" her.");
        CView.text(" She seems hesitant at first, but soon approaches and begins to run her hands along your stomach and your " + describeHips(player));
        // [has breasts]
        if (player.body.chest.sort(BreastRow.Largest)[0].rating > 0) CView.text(", eyes fixed on your " + describeAllBreasts(player));
        CView.text(".\n\n");
        ...
        // [has breasts]
        if (player.body.chest.sort(BreastRow.Largest)[0].rating > 0) CView.text("fondle your " + describeAllBreasts(player) + " and ");
        CView.text("tease your " + describeNipple(player, player.body.chest.get(0)) + "s, and you can feel her grinding herself against your strong shoulders. ");
        // [orb penetrated player during combat]
        CView.text("With a start you realize that the witch's orb is still inside you as it suddenly begins to pulse within your " + assholeOrPussy(player) + ", causing you to cry out and hasten your pace across the dunes. ");
        CView.text("You lose track of time and location as you ride, feeling her orgasm over and over against you, her milk pouring down your back and spilling onto the sands. ");
        // [has breasts]
        if (player.body.chest.sort(BreastRow.Largest)[0].rating > 0) {
            // [lactating]
            if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier >= 1) {
                CView.text("Your own " + describeNipple(player, player.body.chest.get(0)) + "s  are leaking as well, ");
                // [light-med lactation]
                if (player.body.chest.sort(BreastRow.LactationMost)[0].lactationMultiplier < 3) CView.text("small streams running out ");
                // [heavy lactation]
                else CView.text("torrents gushing out ");
                CView.text("under her expert hands. ");
            }
        }
...
// Knot sand witch
function knotSandwitch(player: Character) {
    CView.sprite(SpriteName.SandWitch); // 50
    CView.clear();
    CView.text("You press the tip of your " + describeCock(player, player.body.cocks.get(0)) + " at the entrance of her fragent flower.  It slides in, thanks to the pre already dribbling from your cock, your eyes rolling back in your head from the touch of her slippery folds.  You can't help but suddenly stab the length of your " + describeCock(player, player.body.cocks.get(0)) + " to the hilt within her, moaning as the tip pushes against her cervix.\n\n");
    CView.text("As your " + describeCock(player, player.body.cocks.get(0)) + " stirs within her, she begins to push back, her velvet walls gripping you tightly as you get a extra bit inside.  She pulls free and pounds in again, causing her to grunt from the force of your knot stretching her.  Your balls swing back and forth, slapping and bouncing against her ass in a delicious way.  You can feel the beginnings of the orgasm building within your loins, pulsing up through the tip of your " + describeCock(player, player.body.cocks.get(0)) + ". As the pleasure increases, your body readies itself for the spill, and your knot begins to swell.  ");
    CView.text("You begin to buck into her at a frenzied pace, your body's need to release your motivation.  At first, your knot swelling is an extra sensation, a tightness that squeezes deliciously as more and more blood is forced inside by sheer stimulation.\n\n");
    CView.text("Then it begins to get in the way, preventing your length from going all the way in, causing the pointed tip of your " + describeCock(player, player.body.cocks.get(0)) + " to lust for the unattainable depths of her cunt.  You press in, and get the sensation of the knot being squeezed in, crushing you with warm wet pleasure within her spasming cunt, only to release as you pull free.  As it continues to swell, you can no longer get your dick all the way inside her, your sperm-swollen knot bloated beyond her capacity.  Your desire is almost at its peak; within your grasp, but without that vice-like tightness around your knot it will be hard to cum.  ");
    CView.text("You grunt and push hard, desperate to force the bulbous cock-flesh inside, and you're rewarded as it pops in with a wet squelch. The tightening pressure around the base of your " + describeCock(player, player.body.cocks.get(0)) + " pushes you over the edge, and the cum begins to shoot in her tight hole, with your " + describeCock(player, player.body.cocks.get(0)) + " trapped inside!\n\n");
    // ORGAZMO
    if (player.cumQ() < 25) CView.text("The cum slicks your dick as you continue trying to hump, easing the over-sensation of your dick while you're glued together by the bulbous knot locked in her snatch.");
    if (player.cumQ() >= 25 && player.cumQ() < 100) CView.text("Your cum expands her pussy, stretching her out and leaving your " + describeCock(player, player.body.cocks.get(0)) + " swimming in your spunk, trapped within her tight-stretched walls by your massive knot.");
    if (player.cumQ() >= 100 && player.cumQ() < 500) CView.text("Your cum begins to pump into her corked body, filling her hole to the brim.  Surely the pressure is piling the jizz deep into her womb, and after what feels like forever, the overfull spunk shoves back on your knot, a little dribbling out around the edges of her taut feminine folds as her pussy reaches its limit.");
    if (player.cumQ() >= 500) CView.text("Your cum spurts into her in rapid jets, coating the inside of her walls briefly white as more continues to pour into her. Her cunt begins to fill, and as she strugles to relieve the pressure that continues to pump into her, she only succeeds in spurring your orgasm to a new height.  You continue to blast within her, your knot holding her fast as she squirms, each shot of cum from your prick managing to squirt a tiny bit of jizz from your locked genitals.");
    player.orgasm();
}

// Sandwitch multi-cock corrupt rape
function rapeSandwitchMultis(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.SandWitch); // 50
    CView.clear().text("With the corruption of this realm comfortably throbbing in your shameless veins, you feel your mind opening to new possibilities.  Despite the slickly perfect fit of the sand witch's multiple pussies as you pound into her, and the way she moans with each thrust of your titanic members, you feel there are still some further alleys of pleasure within her left to explore.  Long inured to carnal pleasure that would have rendered your earlier, purer self unconscious,  your mind wanders back to the witch's weapon of lust, the vibrating stone of pleasure.  You feel it's time to give her a dose of her own medicine.\n\n");
    CView.text("You grin wickedly at the humiliated and whimpering witch as you rape her.  Between involuntary gasps of pleasure as you steadily pound into her sopping snatches, she looks afraid, not knowing the source of your mirth.  It soon becomes clear enough. She struggles a little once she hears the low buzzing of her own corrupted pleasure stone and bucks and thrashes as you push the persistent little charm into her tight, but well-cum-lubed ass.  The feel of the stone vibrating within her spreads through to your cocks, enhancing the pleasure as she clenches and spasmes while being violated multiple ways.  All too soon, it brings you to a gushing orgasm.  As you finally withdraw from the troublesome sand witch, you spit on her milk-oozing form and take your leave.");
    player.orgasm();
    return { next: returnToCampUseOneHour };
}

function beatSandwitch(player: Character, monster: Character): NextScreenChoices {
    CView.sprite(SpriteName.SandWitch); // 50
    if (monster.stats.lust > 99) CView.clear().text("You smile in satisfaction as the " + monster.desc.name + " drops down on all fours and begins masturbating feverishly.  Sadly you realize your own needs have not been met.  Of course you could always fuck the horny witch...\n\nDo you rape her?");
    else CView.clear().text("You smile in satisfaction as the " + monster.desc.name + " drops down on all fours and struggles to rise.  Sadly you realize your own needs have not been met.  Of course, you could always fuck the witch...");
    CView.text("  Of course, just taunting, teasing, and humiliating her for her arrogance would be equally amusing, <b>but it would give her plenty of time to turn the tables...</b>");
    let temp2: ClickFunction;
    let temp3: ClickFunction;
    if (User.settings.silly()) temp3 = missingoSex;
    if (player.inventory.keyItems.has("Deluxe Dildo")) temp2 = sandwitchGetsDildoed;
    let shouldra: ClickFunction;
    ...
    return {
        choices: [
            ...
            ["Lay Eggs", ovi],
            ["Taunt Her", partial(sandwitchSpanking, player, monster)],
            ["", undefined],
            ["", undefined],
            ["", undefined],
...
function missingoSex(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.SandWitch); // 50
    CView.clear();
    CView.text("You stride proudly towards your latest conquest, the prone witch starting at you with a look of pure and delightful helplessness. She backs away on all fours, but a simple \"Stop.\" uttered from your lips is enough to freeze her in place. You look do/n at her, your \" + multiCockDescript(0) + \" throbbing within your clothes. What clothes? Oh, those clothes th$t you happen to be tearing from your body, much to her dismay.\n\n");

    CView.text("Clearly, this bitch needs the dick. And you're just the ^&a*pion to give it to her. You lick your lips and drop to your @nees, taking hold of her le-s and forcibly spreading them apart. Looking down, you see that her body is quite ready for you, her exposed fuckhole pouted and moist. You cannot help but laugh a- the situation, the motions making your \" + multiCockDescript(0) + \" bounce in the open air as you co-vulse with laugh-er. Her res$$tance only serves to arouse you, making you savor that choice ass before you claim it for your own.\n\n");

    CView.text("You want to embarrass her further, complete her humiliation upon the sands she once thought to be her domain. You tear her robes into little shreds, exposing her four generously sized breasts. Your eyes follow her dri-^ling nipples, the pupils bouncing as they do. \"Wh-what are you going to do? Rape me?!\" cries the witch, her tone equal parts terrified and wishful. Her thighs attempt to close, though you're sure it's more t$ grind them together than any atte-pt at preserving her modesty. Leaning down, y-u open up to let your tongue lap at her swollen clit. She arches her back and moans, thrusting her hips towa-ds you as bes- she can.\n\n");

    CView.text("0)w is t$e time. But no, she won't get the dick. She hasn't EAR*ED the dick. She isn't even wor$h your \" + describeVagina(player, player.body.vaginas.get(0)) + \" yet. No n- no, you need -o warm that ho_ cunt up. You whistle to g-t her attention, and as you do, you present your -i%-s ^a_=e* up to*--h-r. Her eyes wi_en, the wo$an's jaw going slack. \"N-no.. not t-e whole thing.\" you nod _n respo-se to her plea. \"The WHO-E THING?!\"\n\n");

    CView.text("Now she begins to panic. She shake- her head rapidly as you draw bac- your %s99=, and thrust them towards her >pen sl1t...");
...
// {Next Page}
function missingoSex3(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.SandWitch); // 50
    CView.clear();
    CView.text("_at_r@_ly, you %i-+. Ho@-@@ly, her h_le c_n't take @_ch _ po_ndi_g. Yo_ can t_ll t-is b_ loo_ing _t her pr_ne,',, v@^@erable b@dy, #ipe _or y-ur a_use. Y_ur *i- isn't ev|n en_ug_ to do this! Y_ur-i*))s fall sho_t, la@d@ng i( the sa_d, sp_as_i_g he- bo^y wit@ a @ave @f the -tuff. She gasps in su!2!i!e, ba_el_ a?le to k_ep it fr_m g_tti_g into h_r m_uth. @er @it@ har_ly fa_e as well, bei@g c@_t_d ge_ero_sl_, ^er milk mi$ing w--h it. T_e re_ult_ng co@cocti@n sti-ks to her c_est, l@a_in@ it q_ite t_e m_s_.\n\n");

    CView.text("\"@ @a@'t b@lie^e y_u j@st _rie_ th_t.\" s@e sa_s @?th a )_=f. \"Yo_'re @oi(g to g_t a_d in all s))ts o& u_c_mfo_table pla_es ^& yo_ ke-_p t@at u@.\"\n\n");

    CView.text("\"@_i@_...\" s_e says, ##o@%7g @t yo_ s_@_y. \"W_@ d_n't @o_ (ull b=c( an_ @r_ _h_@ a_@in....\"\n\n");
...
// {Next Page}
function missingoSex4(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.SandWitch); // 50
    CView.clear();
    CView.text("_oo_o@_oo, ooo %o-+. oo@-@@oo, ooo o_oo o_o'o oooo @_oo _ oo_ooo_o. oo_ oo o_oo o-oo o_ ooo_ooo _o ooo oo_o,',, o@^@oooooo o@oo, #ooo _oo o-oo o_ooo. o_oo oo- ooo'o oo|o oo_oo_ oo oo oooo! o_oo-o*))o oooo ooo_o, oo@o@oo o( ooo oo_o, oo_oo_o_o oo- oo^o ooo@ o @ ooo@o ooo -oooo. ooo ooooo oo oo!2!o!o, oo_oo_ o?oo oo o_oo oo oo_o o_ooo_o oooo o_o o_ooo. @oo @oo@ ooo_oo oo_o oo oooo, ooo@o o@_o_o oo_oo_oo_, ^oo oooo oo$ooo o--o oo. o_o oo_ooo_oo oo@ooooo@o ooo-oo oo ooo o_ooo, l@o_oo@ oo o_ooo o_o o_o_.\n\n");

    CView.text("\"@ @o@'o o@ooo^o o_o o@oo _ooo_ oo_o.\" o@o oo_o @?oo o )_=o. \"oo_'oo @oo(o oo o_o o_o oo ooo o))oo o& o_o_ooo_ooooo ooo_oo ^& oo_ oo-_o o@oo o@.\"\n\n");

    CView.text("\"@_o@_...\" o_o oooo, ##o@%7o @o oo_ o_@_o. \"o_@ o_o'o @o_ (ooo o=o( oo_ @o_ _o_@ o_@oo....\"");
    // [Fuck this!]
    return {
        choices: [
            ["Fuck This!", missingoSex5],
        ]
    };
}

// {Next Page}
function missingoSex5(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.SandWitch); // 50
    CView.clear();
    CView.text("Yeah no, fuck this shit. Seriously. You grip your controller with a barely restrained fury, fighting the urge to throw it across the room in a fit. Taking a moment, you collect yourself, and test the  Yep, totally unresponsive to your buttons now. Damn thing froze AGAIN.\n\n");

    CView.text("You sigh a defeated sigh and cut the power to your Super Fentendo. After removing your Corruption of Champions cartridge and inspecting it for damage, you flip it over and blow into the open end of it a few times. Crisp, clean blowing, from one end to the other. Thorough. Professional. These games should be proud to have you as their owner.\n\n");

    CView.text("You flip up the little plastic fedora on your Imp-head-shaped Fentendo, exposing the cartridge slot and giving it a looksee. No problems there, it seems. Just to be sure, you huff and puff and blow for dust. It's bad enough that this system only supports one game, AND you have to go out to the store and pick up a new copy of it every time the developer makes one or two tiny little changes to the  But those crashes and terrible dialogue in the last scene? You'd think that some of the writers just weren't trying anymore.\n\n");

    CView.text("The system powers up with the usual blips and boops you'd expect from this 8-bit powerhouse. So far so good. The Corruption of Champions title screen pops up, heralded by a little pixelated imp eagerly waving at the player with both hands and something else that protrudes from his body. The CoC theme kicks up, and you smile to yourself. Goddamn if that theme isn't catchy as all getout. You're going to have it stuck in your head for the rest of the day, you wager, but at least you'll have something to hum while you masterbate.\n\n");

    CView.text("Oh goddammit. You remember that your progress from before was lost, and now you have to start from the beginning. You can't help but roll your eyes and groan, though you're hardly angry enough to put a controller-shaped hole through your TV screen over this. But hey! At least you were smart enough to jot down the Champion Password. Perhaps one day video games will be sophisticated enough to have some internal means of storing progress, but at least with this, you should be able to get all of your old stuff!");

    // Restore]
    return {
        choices: [
            ["Restore", missingoSex6],
        ]
    };
}
// {Next Page}
function missingoSex6(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.SandWitch); // 50
    player.orgasm();
    CView.clear();
    CView.text("You successfully restore your  Everything's intact. At least, you thought it was. You must have messed up the password somewhere along the line, but oddly enough, the only difference is the clock being set an hour ahead and a few extra gems and EXP in your pocket.\n\n");

    CView.text("It is like fucking christmas up in here.");
    // {Player gains about as many gems experience points as they would from a Sand Witch encounter, and the Champion is returned to }
    return { next: returnToCampUseOneHour };
}
...
function ovipositSandWitches(player: Character, monster: Character): NextScreenChoices {
    CView.clear();
    CView.text("As you glance down at the ");
    if (monster.stats.HP < 1) CView.text("bruised and battered");
    else CView.text("lusty");
    CView.text(" sand witch, you feel the familiar tug of arousal in your loins.  Yet your current... situation offers quite the opportunity to relieve some tension through the use of your 'other' sexual appendage.  Licking your lips in anticipation, you lower yourself down to the obstinate spell caster, who immediately reacts with ");
    if (monster.stats.HP < 1) CView.text("fright as she clamors to get away.");
    else CView.text("an uneasy curiosity to see what exactly you have planned.");

    // PC won through HP victory:
    if (monster.stats.HP < 1) CView.text("\n\nRolling your eyes, you offer a sympathetic hand to the defeated witch, showing her that you don't mean to hurt her any further, that you have something more... pleasurable in mind.  ");
    CView.text("The sand witch slows to a dead stop as she assesses your intentions, which are made all the more clear as you disrobe and toss your [armor] aside, exposing your ");
    if (player.body.cocks.length > 0) CView.text("hardened  " + describeCocksLight(player));
    if (player.gender === 3) CView.text(" and ");
    if (player.body.vaginas.length > 0) CView.text(describeClit(player));
    CView.text(" for her viewing consumption.  Making quite the persuasive argument of non-hostility as you do, the sand witch finally relents, removing her cloak and exposing her four-breasted, ");
    if (player.body.cocks.length > 1) CView.text(numToCardinalText(player.body.cocks.length + 1));
    else CView.text("two");
    CView.text("-sexed form, allowing the sun to glisten off her bare body.");
    ...
    if (player.canOvipositBee()) CView.text("bee-morphs");
    else CView.text("driders");
    CView.text(" can do to their partners and unsure if this is really what she wants.  The poor girl seems to need a little 'encouragement'... of course, you could just skip all that regardless if you really wanted.");

    // [Foreplay 1st]    [Get Fucking]
    return {
        choices: [
            ["Foreplay 1st", eggwitchForeplay],
            ["Get Fucking", getToFuckingWithZeEggsInWitch],
        ]
    };
}

// [Foreplay 1st]
function eggwitchForeplay(player: Character): NextScreenChoices {
    CView.clear();
...
function laySomeEggsInThatWitchFinally(player: Character): NextScreenChoices {
    CView.clear();
    const cunt: boolean = (randInt(2) === 0);
    CView.text("Before long the intense need to culminate the act grows too lucid to ignore, and with a hum of satisfaction you finally bore your ovipositor through the tight ");
    if (cunt) CView.text("folds of her vulva");
    else CView.text("ring of her anus");
    CView.text(".  Like something out of a fever dream, the bizarre appendage you wield burrows further and further into her ");
    if (cunt) CView.text("sex");
    else CView.text("ass");
    CView.text(", the ");
    if (cunt) CView.text("ribbed contours of her pussy");
    else CView.text("tight fit of her innards");
    CView.text(" enrapturing your egg-tube with every passing inch.  You begin the process of lubricating the antsy witch's ");
    if (cunt) CView.text("love-");
    else CView.text("anal ");
    CView.text("canal for the delivery of your young; the witch barely contains her moans and utter glee at being filled with such a hot, humid substance.  In perverted delight the sand witch coos between kisses, her moans betraying a curiosity to explore just how pleasant mating this way can truly be.");
    ...
    if (cunt) CView.text("opening of your lover's cervix");
    else CView.text("deepest recesses of her sphincter");
    CView.text(".  The joining of oviduct to ");
    if (cunt) CView.text("womb");
    else CView.text("anus");
    CView.text(" now complete, you prepare yourself for the inevitable convulsion of orgasm along your bizarre appendage, grinning down at the sand witch with a smirk that can only convey that the fun's about to begin.  With a sudden, violent quake your ovipositor rumbles with the first of many eggs, squeezing your unborn brood down your shaft and pushing ");
    if (cunt) CView.text("at the waiting lips of the sand witch's vagina");
    else CView.text(" through her rectal walls");
    CView.text(".  With a meticulous push, the forerunner of your young manages to infiltrate her passage, working its way valiantly into her depths with every convulsion.");
    ...
    if (cunt) CView.text("into her waiting womb");
    else CView.text("as far as you can safely tuck it");
    CView.text(", nestled and sound until the scramble for life can begin.");

    CView.text("\n\n\"<i>More... give me more...</i>\" the sand witch whimpers, caressing your ");
    // (Bee, non-horse:
    if (player.canOvipositBee() && !player.body.legs.isTaur()) CView.text("torso");
    else CView.text("pelvis");
    CView.text(" to spur you on.  Of course you don't have a problem with fulfilling her wishes; the neglect of your throbbing sex");
    if (player.body.cocks.length > 1 || player.gender > 2) CView.text("es");
    CView.text(" is the only point of dissatisfaction, a point you make clear when you ");
    if (player.body.cocks.length > 0) CView.text("take hold of your " + describeCocksLight(player) + " and bring it");
    else CView.text("bring your " + describeVagina(player, player.body.vaginas.get(0)));
    CView.text(" to her face.  The cute stare of her half-glazed eyes turns to the lunge of lips toward your sex, the sand witch driven like a woman possessed in her efforts to get the next batch going...");

    CView.text("\n\nFor what feels like an eternity you sit atop the pliant sorceress, having her tend to your sex");
    if (player.body.cocks.length > 1 || player.gender === 3) CView.text("es");
    CView.text(" while you slowly pump her full of eggs, her girlish little moans resonating through your body as she orally stimulates your ");
    ...
    CView.text(".  At long last her enthusiastic efforts bring you to a more 'traditional' release, lathering ");
    const facial: number = randInt(2);
    if (facial === 0) CView.text("her facial features with proof of your climax");
    else if (facial === 1) CView.text("the interior of her mouth with sweltering streams of your cum");
    CView.text(".  Having finally achieved release, and also having run out of eggs, you pull yourself free of the desert woman with a loud pop, the ");
    if (player.canOvipositBee()) CView.text("honey-like");
...
export function sammitchBirthsDriders(player: Character): NextScreenChoices {
    CView.clear();
    // Occurs 8 days (or moar appropriate time) after fertile oviposition, triggered when exploring the Desert
    CView.text("As you struggle with the loose footing of the hot desert sand, a weak howl reaches your ears.  Curiously, you stop and listen, trying to locate the potential source of the sound.  Yet all you can really hear is the wind against the sand-blasted, desolate terrain, giving you cause to shrug it off and continue on your merry way.");
    ...
    if (player.stats.cor < 50) CView.text("Intending to help");
    else CView.text("Curious");
    CView.text(", you slide down the dune that you are on and come to a stop next to the distressed woman.  However, upon seeing her face you realize that this is the very sand witch that you had a hot and steamy tumble with!");
    ...
    if (player.stats.cor < 60) return helpZeWithBirfBabies(player);
    else return reluctantlyHelpZeWitch(player);
}

// [Help her!] (corr < 60)
// no new PG
function helpZeWithBirfBabies(player: Character): NextScreenChoices {
    ...
    if (player.body.legs.isTaur() || player.body.legs.isDrider()) {
        ...
        if (player.body.legs.isDrider()) CView.text("skitter");
        else CView.text("gallop");
        CView.text(" across the dry desert sea...");
    }
    // PC is everything else:
    else CView.text("\n\nEasing a hand under her armpit, you give her the support she needs to hobble along the hot desert sand.  Which direction to hobble in gives you cause for concern though; you don't really know where you are at this point.  Making the only choice you can, you concentrate on the   With the rough picture in mind you tell the sand witch to hold on, before the two of you slowly make your way across the dry desert sea...");
    return { next: sandwitchBirthsYourMonstrosities };
}

// [Must I really?] (corr >=60)
function reluctantlyHelpZeWitch(player: Character): NextScreenChoices {
    ...
    // PC is a drider/centaur/whatever:
    if (player.body.legs.isDrider() || player.body.legs.isTaur()) {
...
function sandwitchBirthsYourMonstrosities(player: Character): NextScreenChoices {
    ...
    if (player.stats.cor < 60) CView.text(", with a quick kiss on the lips");
    CView.text(", whisper in her ear that you'll see her again...");
...
export function witchBirfsSomeBees(player: Character): NextScreenChoices {
    CView.clear();
    // Occurs 8 days (or moar appropriate time) after fertile oviposition; triggered when exploring the Desert
    ...
    if (User.settings.silly()) CView.text(", echoing tiny little cries of \"<i>@</i>\"");
...
function sandwitchSpanking(player: Character, monster: Character): NextScreenChoices {
    CView.clear();
    CView.text("Feeling euphoric after your clear victory, you lazily circle the panting witch and laugh at her.  \"<i>Look at you,</i>\" you taunt, \"<i>A mighty witch of the sands, laid low by a helpless traveller.");
    if (monster.stats.HP < 1) CView.text("  You can't even stand up!");
    else CView.text("  You can't even stop masturbating, can you?");
    CView.text("</i>\"  You kick her hand away, and she tumbles down into a distraught heap, crying softly");
    if (monster.stats.lust > 99) CView.text(" but still masturbating");
    CView.text(".  It's time for some fun!");
    ...
    if (player.stats.cor < 33) CView.text("and though you feel kind of bad about using her this way, she likely would have done worse to you.");
    else if (player.stats.cor < 66) CView.text("and though you aren't sure you ought to be using her this way, she likely would have done worse to you.");
    else CView.text("and though you used to find things like this distasteful, you don't really give a shit anymore.");
    CView.text("  You tweak and tease with a goofy smile, laughing as you aim one of the tits up into the witch's face, splattering her with her own alabaster juices.  You're so involved in it that you fail to notice the increasingly angry gaze that settles over your captive's face.  Her tits are just so goddamn fun to tease that you don't really waste any time paying attention to her from the neck up.  That's a mistake.");
    ...
    if (player.body.cocks.length > 0) CView.text("Worse still, [eachCock] is now unprotected and pressed against the rough stone by your body-weight.  It's less than pleasant.  ");
    else if (player.body.vaginas.length > 0) CView.text("Worse still, your [vagina] is pressed up against the rough stone.  It's less than pleasant.  ");
    CView.text("You bristle at your helplessness");
    if (player.perks.has(PerkType.FireLord) || player.perks.has(PerkType.Hellfire) || player.perks.has(PerkType.Dragonfire)) CView.text(", breathing out a gout of flame in rage, but you can't even direct the blast properly like this.  All it does is melt some of the sand below into glass");
    else CView.text(", but there's nothing you can do");
    CView.text(".  Fingers tickle the " + describeSkin(player) + " of your bottom, and then, without warning, your [butt] is slapped!  The sound of the hit rings out across the desert, hard enough to make moisture bead at the corners of your eyes.");
    ...
    if (player.stats.cor < 50) CView.text("She sounds almost fanatical in her dedication to her order.");
    else CView.text("What the hell is she on about?  Does she actually think her little torture session is going change your attitude?");
