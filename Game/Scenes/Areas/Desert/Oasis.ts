...
User.flags.set(FlagType.Oasis_Demons, OasisFlags);
...
function oasisRunAway(player: Character): NextScreenChoices {
    ...
    if (player.stats.spe > 15 && player.stats.spe / 2 > randInt(40)) {
        CView.clear().text("You bolt out from under your bush and scramble away over the sand. Before long the swishing sounds of pursuit fade away and looking back you see the few demons with the gusto to follow you tramping back to the oasis.");
        return { next: returnToCampUseOneHour };
    }
    else {
        CView.clear().text("You scramble away from the demons, but are too late. A swift demon with canine features tackles you to the ground.  Luckily he loses his grip as you tumble onto the sand and you slither free, stand up and wheel to face the host of leering demons which begin to advance with malicious intent.");
        return CombatManager.beginBattle(player, new DemonPack());
    }
}
...
function oasisTalkAccept(player: Character): NextScreenChoices {
    ...
    if (!player.effects.has(StatusEffectType.VoluntaryDemonpack))
        player.effects.add(StatusEffectType.VoluntaryDemonpack, 0, 0, 0, 0);
    else
...
function oasisSexing(player: Character): NextScreenChoices {
    ...
    if (player.effects.get(StatusEffectType.VoluntaryDemonpack).value1 >= 6 && player.body.vaginas.length > 0) {
...
function oasisBadEndEpilogue(player: Character): NextScreenChoices {
    CView.sprite(SpriteName.Oasis_Demons); // 46
    CView.clear().text("After one year");
