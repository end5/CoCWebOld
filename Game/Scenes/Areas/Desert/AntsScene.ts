...
import { returnToCampUseOneHour, loversCount, campLoversMenu } from "../../Camp";
...
import { describeNipple, describeBreastRow } from "../../../Descriptors/BreastDescriptor";
...
import { izmaFollower } from "../../NPCs/IzmaScene";
import { FlagWomb } from "../../../Body/Pregnancy/FlagWomb";

export const AntsFlags = {
    PHYLLA_EGG_LAYING: 0,
    ANT_KIDS: 0,
...
User.flags.set(FlagType.Ants, AntsFlags);
/**
 * Created by aimozg on 05.01.14.
 */

export class AntsTimeAware implements ITimeAware {
    public womb: FlagWomb = new FlagWomb();

    // Implementation of ITimeAware
    public timeChange(): boolean {
        if (AntsFlags.PHYLLA_EGG_LAYING > 0 && randInt(5) === 0 && AntsFlags.ANT_KIDS < 5000) AntsFlags.ANT_KIDS++;
        if (Time.hour > 23) {
            ...
            if (AntsFlags.PHYLLA_EGG_LAYING > 0) AntsFlags.DAYS_PHYLLA_HAS_SPENT_BIRTHING++;
            if (AntsFlags.PHYLLA_GEMS_HUNTED_TODAY > 0) AntsFlags.PHYLLA_GEMS_HUNTED_TODAY = 0;
            if (phyllaWaifu()) AntsFlags.DAYS_PHYLLA_IN_CAMP++;
        }
        return false;
    }
    ...
    public serialize() {
        return {
            womb: this.womb.serialize()
        };
    }

    public deserialize(saveObject: AntsTimeAware) {
        this.womb.deserialize(saveObject.womb);
    }
}
const antsTimeAware = new AntsTimeAware();
const womb = antsTimeAware.womb;

export function phyllaWaifu(): boolean {
    return AntsFlags.ANT_WAIFU > 0;
}

export function antColonyEncounter(player: Character): NextScreenChoices {
    // WAIFU GET!
    // trace("ANT WINS: " + AntsFlags.ANT_ARENA_WINS + " ANT LOSSES: " + AntsFlags.ANT_ARENA_LOSSES);
    if (AntsFlags.ANT_ARENA_WINS - AntsFlags.ANT_ARENA_LOSSES >= 2 && AntsFlags.ANT_ARENA_WINS >= 4 && player.gender > 0) {
        if (AntsFlags.PHYLLA_STAY_HOME > 0) return bumpIntoTheAntColonyAfterStayHomePhylla(player);
        else return antGirlGoodEnd(player);
    }
    else if (AntsFlags.PC_READY_FOR_ANT_COLONY_CHALLENGE === 1)
        return antColonyChallenge(player);
    else if (AntsFlags.PHYLLA_SAVED === 1)
        return enterTheColony(player);
    else return firstAntColonyEncounter(player);
}

function phyllaCapacity(): number {
    return AntsFlags.PHYLLA_CAPACITY;
}
...
function keepHidden(player: Character): NextScreenChoices {
    AntsFlags.ANT_COLONY_KEPT_HIDDEN = 1;
    ...
    if ((player.stats.lib < 41 && player.stats.lust < 50) || player.stats.cor < 66 || player.gender === 0) {
        ...
        CView.text("\n\nAssuring yourself that she'll be fine, you take the opportunity to flee while the demons are distracted, heading back to   Leaving the ant-girl to her fate.");
        // [End of Event]
    ...
    // ►(If Over 41 - Male)
        ...
        CView.text("You know exactly what's going to happen once the demons 'subdue' their prey.  The growing throbbing in your loins begs for release, and you quickly and quietly take off your " + player.inventory.equipment.armor.displayName + ", making sure the demons don't notice you.  You might as well watch the show as you're not gonna be able to leave unnoticed until the demons are done anyways.");
        ...
        CView.text("\n\nYour body satisfied, you drift off.  When you awaken much later, the demons and the ant seem to have moved on, thankfully without noticing you.  You put your armor back on and return to ");
        // [End of Event]
...
function playHero(player: Character): NextScreenChoices {
    ...
    monster.effects.add(StatusEffectType.PhyllaFight, 0, 0, 0, 0);
...
function enterTheColony(player: Character): NextScreenChoices {
    ...
    AntsFlags.PC_READY_FOR_ANT_COLONY_CHALLENGE = 1;
    return { next: returnToCampUseOneHour };
}
...
function antColonyChallenge(player: Character): NextScreenChoices {
    ...
    return {
        choices: [
            ["Fight", antColiseumFight],
            ["", undefined],
            ["", undefined],
            ["", undefined],
            ["Leave", leaveAntColony],
        ]
    };
}
...
function leaveAntColony(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("Deciding to better prepare yourself first, you inform the thin fight manager that you will return later.  You leave the colony, heading back to ");
    return { next: returnToCampUseOneHour };
}
...
function antColiseumFight(player: Character): NextScreenChoices {
    ...
    // ►[Fight #1]
    if (AntsFlags.ANT_ARENA_WINS + AntsFlags.ANT_ARENA_LOSSES === 0) {
        ...
        monster.effects.add(StatusEffectType.NoLoot, 0, 0, 0, 0);
    }
    // ►[Fight #2]
    else if (AntsFlags.ANT_ARENA_WINS + AntsFlags.ANT_ARENA_LOSSES === 1) {
        // ►(Minotaur - Intro)
    ...
    // ►[Fight #3]
        ...
        // (Gnoll - Intro)
        if (AntsFlags.MET_ANT_ARENA_GNOLL === 0) CView.text("With due ceremony, the manager leads you down to one of the staging areas for the arena.  You watch through the bars as the stadium fills far past capacity.  There must be a thousand ants here... maybe even the whole colony.  Gazing out into the crowd, you pick out the royal seating and in it, Princess Phylla.  The princess is in much finer attire than you're used to seeing - her chest is actually covered by a fine red and blue dress and her hair is combed, framing her face nicely.  Her mother is nowhere to be seen, and you can tell that Phylla is taking full advantage.  She waves and cheers for you just like the crowd is and, perhaps by her orchestration, you hear the crowd pick up a chant in your name.  Seeing Phylla chant with them makes you feel invigorated.  As the gates raise, you charge out into the center of the arena, roaring your finest battle cry; your opponent is taken somewhat aback but finds resolve and braces for your charge.");
...
// (Tentacle Beast - Win) Standard Tentacle Beast Win Scene. (Again we're going to need to adapt the ending so the PC does not go back to )
...
export function phyllaTentaclePCLoss(player: Character): NextScreenChoices {
    ...
    // (+1 Loss Score)
    AntsFlags.ANT_ARENA_LOSSES++;
    if (AntsFlags.ANT_ARENA_LOSSES > 3) {
...
export function phyllaPCBeatsGnoll(player: Character): NextScreenChoices {
    ...
    if (AntsFlags.ANTS_PC_BEAT_GNOLL === 0) {
        ...
        AntsFlags.ANTS_PC_BEAT_GNOLL++;
    }
    else if (AntsFlags.ANT_ARENA_WINS - AntsFlags.ANT_ARENA_LOSSES >= 2 && AntsFlags.ANT_ARENA_WINS >= 3 && player.gender > 0) {
        AntsFlags.ANT_ARENA_WINS++;
...
export function phyllaGnollBeatsPC(player: Character): NextScreenChoices {
    ...
    if (AntsFlags.ANTS_PC_LOST_TO_GNOLL === 0) {
        AntsFlags.ANTS_PC_LOST_TO_GNOLL++;
...
function malePhyllaContinuation(player: Character, cockThatFits: Cock): NextScreenChoices {
    ...
    if (AntsFlags.PHYLLA_EGG_LAYING > 0) CView.text("nipples and ");
...
function phyllaFirstTimePureBabiesFuckEnding(player: Character): NextScreenChoices {
    ...
    if (player.canOvipositSpider()) {
        ...
        if (player.stats.int / 20 + randInt(20) + 1 < 16) {
            ...
            if (womb.isPregnant()) CView.text("  I just can't hold anything else inside me. I'm sorry! Please don't be mad... I mean I will!  Just a-after... this batch...");
...
function phyllaCorruptMascEnding(player: Character): NextScreenChoices {
    ...
    const secondCockThatFits = player.body.cocks.filter(Cock.CocksThatFit(phyllaCapacity()))[1];
    if (secondCockThatFits) {
        ...
        if (player.body.cocks.sort(Cock.Largest)[0].area > phyllaCapacity()) {
...
function cuntmuffinLingusPhyllaDickBig(player: Character): NextScreenChoices {
    ...
    if (player.body.cocks.sort(Cock.Longest)[0].length < 69) {
    ...
    if (player.body.cocks.sort(Cock.Longest)[0].length < 69) CView.text("ceiling");
    ...
    AntsFlags.DIDNT_FUCK_PHYLLA_ON_RECRUITMENT = 1;
...
function femalePhyllaFirstTimePlusCock(player: Character): NextScreenChoices {
    ...
    if (player.body.cocks.sort(Cock.Smallest)[0].area > phyllaCapacity()) {
        // Dicks too Big:
    ...
    // If PC has an appropriately sized dick(s):
        ...
        if (player.body.cocks.filter(Cock.FilterType(CockType.TENTACLE)).length > 0) {
            ...
            const smallestCocks = player.body.cocks.sort(Cock.Smallest);
            if (player.body.cocks.length === 2 && smallestCocks[1].area + smallestCocks[0].area <= phyllaCapacity())
            ...
            // TODO: "player.body.cocks[smallestCocks[0]] < 4" changed to cockLength. Not sure if it is the appropriate attribute.
            else if (smallestCocks[0].length < 4) CView.text("your " + describeCock(player, smallestCocks[0]) + ", stroking it apologetically as she scissors you.  You feel her almost insatiable appetite for all parts of your body.  You feel Phylla's mind slowly becoming lost in pleasure.");
            // PC only has a dick(s) that('s) (are) more than 4 inches in width:
            else CView.text("your " + describeCock(player, smallestCocks[0]) + ", but she simply can't fit any number of her hands around the entirety of your glans; instead, she takes to caressing the head of your " + describeCock(player, player.body.cocks.sort(Cock.Largest)[0]) + ".");
...
function getAntWaifuYoShit(player: Character): NextScreenChoices {
    CView.clear();
    CView.text("You smile at her and tell her you would love for her to join you at your   Her face brightens like the sun and she quickly gathers the very few possessions she owns - mostly clothing, the pillows, and some jewelry.  Together you promptly leave the colony and head back to ");
    CView.text("\n\n(<b>Phylla has moved in!  She can be found in the lovers tab!</b>)");
    AntsFlags.ANT_WAIFU = 1;
    return { next: returnToCampUseOneHour };
}
...
function tellPhyllaToStayTheFuckAtHomeThatCunt(player: Character): NextScreenChoices {
    ...
    CView.text("\n\nYou nod and without another word, head back to ");
    AntsFlags.PHYLLA_STAY_HOME = 1;
    return { next: returnToCampUseOneHour };
}
...
export function introductionToPhyllaFollower(player: Character): NextScreenChoices {
    if (AntsFlags.PHYLLA_CAPACITY < 50) AntsFlags.PHYLLA_CAPACITY = 50;
    CView.clear();
    if (womb.isPregnant() && womb.pregnancy.incubation === 0) {
        return phyllaLaysSomeDriderEggs(player);
    }
    // [Follower > Phylla Intro (First Time)]
    if (AntsFlags.PHYLLA_CAMP_VISITS === 0) {
        ...
        if (AntsFlags.DIDNT_FUCK_PHYLLA_ON_RECRUITMENT === 1 && player.body.cocks.length > 0) {
    ...
    if (AntsFlags.PHYLLA_EGG_LAYING === 0) choices[2] = ["Lay Eggs", phyllaLaysEggsToggle];
    else choices[2] = ["No Eggs", phyllaLaysEggsToggle];
    if (AntsFlags.ANT_KIDS > 0) choices[3] = ["Children", phyllasKidsChildren];
    choices[4] = ["Appearance", phyllaPearance];
    choices[5] = ["Find Gems", phyllaDigsForGems];
    choices[9] = ["Back", campLoversMenu];

    AntsFlags.PHYLLA_CAMP_VISITS++;
    return { choices };
}
...
function phyllaSexMenu(player: Character): NextScreenChoices {
    ...
    if (player.body.cocks.length > 0) {
        ...
        if (AntsFlags.PHYLLA_EGG_LAYING > 0 && AntsFlags.ANT_KIDS >= 10) choices[1] = ["Fuck Her", dudesFuckEggLayingBitches];
        if (AntsFlags.ANT_KIDS > 10 && player.stats.cor >= 75) choices[3] = ["Orgy (Male)", orgyWithDatColonyCorruptDudes];
    }
    // Straight Sex (Lesbian/Fisting) - Written
    if (player.body.vaginas.length > 0) {
        choices[2] = ["Lesbian Sex", lesbianFisting];
        // While Giving Birth (Female) - Written
        if (AntsFlags.PHYLLA_EGG_LAYING > 0 && AntsFlags.ANT_KIDS >= 10) choices[2] = ["Lesbian Sex", birfingSexWithAntsForDasLadies];
        // Orgy w/ Colony (Female)
        // You tell Phylla you're interested in 'inspecting' your children.
        if (AntsFlags.ANT_KIDS > 10 && player.stats.cor >= 75) choices[4] = ["Orgy (Female)", antColonyOrgy4Ladies];
    }
...
function talkAboutAntHistory(player: Character): NextScreenChoices {
    CView.clear();
    AntsFlags.TALKED_WITH_PHYLLA_ABOUT_HISTORY = 1;
    ...
    // [If corruption more than 80]
        ...
        if (!player.effects.has(StatusEffectType.DungeonShutDown)) {
...
function talkAboutAntMatingAndRituals(player: Character): NextScreenChoices {
    ...
    // If corruption more than 60:
        ...
        if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 4) CView.text("\n\n\"<i>At least not like yours... I mean, yours are nicer!</i>\"");
        ...
        if (player.stats.cor >= 80) {
            ...
            if (AntsFlags.PHYLLA_EGG_LAYING > 0) CView.text("milk dripping ");
            ...
            if (AntsFlags.PHYLLA_EGG_LAYING > 0) CView.text(", sending jets of milk shooting out onto the floor");
            ...
            CView.text("\n\nYou've had your fun, and Phylla is none the wiser.  You wink at her and thank her for the show. The comment clearly goes over her head leaving her wondering about your meaning as you set off back to ");
            player.stats.lust += 15;

        }
    }
    AntsFlags.PHYLLA_INHERITED_KNOWLEDGE = 1;
    return { next: returnToCampUseOneHour };
}
...
function phyllasLifePastAndFuture(player: Character): NextScreenChoices {
    CView.clear();
    // (If Izma at Camp)
    if (izmaFollower() && AntsFlags.PHYLLA_IZMA_TALK === 0) {
        AntsFlags.PHYLLA_IZMA_TALK = 1;
        ...
        if (player.stats.cor < 40) {
            ...
            if (AntsFlags.PHYLLA_INHERITED_KNOWLEDGE === 1) {
            ...
            CView.text("\n\nThough still off-putting this seems to calm Phylla a bit.  The last memory you recall is the one where Izma submitted to you. At the end of one of your 'fights' Izma confesses that you're her alpha and, if allowed, she would be happy to spend her life at your ");
            ...
            CView.text("\n\nYou head back to your   When you get there you see Izma drying off after a swim in the stream. You give her a devious look and a very wide smile.  She looks around to make sure you're actually smiling at her.  \"<i>What's that about?</i>\" She inquires.  You tell her it's nothing she needs to worry about for now.");
        }
        // If Corruption more than 40: (Continued From: I didn't mean to watch but...</i>\")
            ...
            if (player.stats.cor >= 80) {
                ...
                CView.text("\n\nYou laugh in her face before leaving to return to your  You should introduce your new play thing to Izma.  Maybe, if you allow it, Izma can have her very own beta.");
            }
            else {
        ...
        CView.text("\n\nYou mull this over in your head, and you must have been thinking pretty hard.  When you get up to leave you find Phylla has completely passed out on your lap.  You take this opportunity to gently unwrap yourself from her, and head back to ");
...
function phyllaBeeeJays(player: Character): NextScreenChoices {
    CView.clear();
    AntsFlags.PHYLLA_BLOWJOBS++;
    if (AntsFlags.PHYLLA_BLOWJOBS === 1) {
    ...
    if (smallestCock.area < 60) {
        ...
        // Dick size more than 60 inches (subsequent blowjobs):
            ...
            if (AntsFlags.PHYLLA_BLOWJOBS === 2) CView.text("last time");
...
function nopeNotOnMouthOrWhateverFuckThisNoise(player: Character): NextScreenChoices {
    ...
    if (!player.body.cocks.filter(Cock.CanAutoFellate)) CView.text(" or anything like it would feel like.");
...
function pureBJEnding(player: Character, linked: boolean = true): NextScreenChoices {
    ...
    else if (AntsFlags.TIMES_LINKED_BJ_SUCK === 0) {
        CView.text(", ");
        // If PC linked, first time occurring:
        CView.text("leading to you jesting through the link and calling her a damn dirty cheater.");
    }
    if (AntsFlags.TIMES_LINKED_BJ_SUCK > 0) CView.text("  You can feel her relying less and less on the link with every subsequent time; there's no doubt in your mind that she will no longer \"<i>cheat</i>\" her way into making you cum before you know it.");
    const smallestCock = player.body.cocks.sort(Cock.Smallest)[0];
...
function corruptPhyllaEndings(player: Character): NextScreenChoices {
    ...
    // If first time:
    if (AntsFlags.PHYLLA_BLOWJOBS === 1) CView.text("achieved the desired effect.");
    // Every other time:
...
function dickPhylla(player: Character): NextScreenChoices {
    ...
    // (NO BJ experience)
    if (AntsFlags.PHYLLA_BLOWJOBS === 0) {
...
function birfingSexWithAntsForDasLadies(player: Character): NextScreenChoices {
    ...
    if (player.body.cocks.length > 0 && player.body.cocks.find(Cock.CockThatFits(42))) {
        ...
        if (player.body.cocks.filter(Cock.CocksThatFit(42))[1]) CView.text("two of ");
        CView.text("your " + describeCocksLight(player) + " into her mouth and mashes ");
        if (player.body.cocks.filter(Cock.CocksThatFit(42))[1]) CView.text("them");
        else CView.text("it");
    ...
    if (player.body.cocks.sort(Cock.Largest)[0].area >= 500) {
...
function orgyWithDatColonyCorruptDudes(player: Character): NextScreenChoices {
    ...
    if (AntsFlags.PHYLLA_EGG_LAYING > 0) CView.text("large, lactating ");
    ...
    CView.text("\n\nYou might want to stay and watch that, but you've spent too long down here already.  You collect your things, trying your best not to step on the twenty or so passed out ants on the floor as you head back to ");
    player.orgasm();
    player.stats.sens += -2;
...
function antColonyOrgy4Ladies(player: Character): NextScreenChoices {
    CView.clear();
    AntsFlags.TIMES_CORRUPT_FEMALE_ANT_ORGY++;
    // (First Time)
    if (AntsFlags.TIMES_CORRUPT_FEMALE_ANT_ORGY === 1) {
...
function phyllaLaysEggsToggle(player: Character): NextScreenChoices {
    CView.clear();
    // (Lay Eggs)
    if (AntsFlags.PHYLLA_EGG_LAYING === 0) {
        AntsFlags.PHYLLA_EGG_LAYING = 1;
        ...
        if (AntsFlags.TALKED_WITH_PHYLLA_ABOUT_HISTORY > 0) CView.text("  You tell her you want a colony like those she spoke of in the times before the corruption.");
    ...
    // (Stop Laying Eggs)
        ...
        CView.text("\n\nSmiling, you thank her for complying with all your demands.  You leave the colony, heading back to ");
        return { next: returnToCampUseOneHour };
    }
}
...
function lickThatAntButt(player: Character): NextScreenChoices {
    ...
    // First Time:
    if (AntsFlags.ANTS_BIRTHED_FROM_LICKING === 0) {
    ...
    AntsFlags.PHYLLA_COOLDOWN = 12;
    AntsFlags.ANTS_BIRTHED_FROM_LICKING++;
    if (AntsFlags.ANT_KIDS < 5000) AntsFlags.ANT_KIDS += 5;
    return { next: returnToCampUseOneHour };
}
...
function dontLickAntButt(player: Character): NextScreenChoices {
    ...
    if (AntsFlags.ANT_KIDS > 1) CView.text("; the children can tend to the eggs and I can really use the rest");
...
function phyllaLaysSomeDriderEggs(player: Character): NextScreenChoices {
    ...
    AntsFlags.PHYLLA_TIMES_DRIDER_EGG_LAYED++;
    AntsFlags.PHYLLA_DRIDER_BABIES_COUNT += 5 + randInt(4);
    ...
    if (player.stats.cor < 75) {
        ...
        if (AntsFlags.PHYLLA_EGG_LAYING > 0) CView.text("both eggs are just fine on their own");
...
function phyllaDigsForGems(player: Character): NextScreenChoices {
    ...
    if (AntsFlags.ANT_KIDS > 10) kidsMod++;
    if (AntsFlags.ANT_KIDS > 50) kidsMod++;
    if (AntsFlags.ANT_KIDS > 150) kidsMod++;
    if (AntsFlags.ANT_KIDS > 300) kidsMod++;
    if (AntsFlags.ANT_KIDS > 600) kidsMod++;
    if (AntsFlags.ANT_KIDS > 1000) kidsMod++;
    if (AntsFlags.ANT_KIDS > 2000) kidsMod++;
    // Success
    if (AntsFlags.PHYLLA_GEMS_HUNTED_TODAY === 0 && randInt(20) + kidsMod > 10) {
        let gems: number = 0;
    ...
    AntsFlags.PHYLLA_GEMS_HUNTED_TODAY = 1;
    return { next: returnToCampUseOneHour };
}
...
function eggDatBitch(player: Character): NextScreenChoices {
    ...
    // First Time:
    if (AntsFlags.TIMES_EGG_IMPREGNATING_PHYLLA === 0) {
