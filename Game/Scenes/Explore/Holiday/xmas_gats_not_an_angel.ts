...
export class XmasAngel {
    ...
    public encounterQuoteUnquoteAngel() {
        ...
        //ii. Yes
            ...
            DisplayText("You haven't found anything as of yet, so you figure you might as well keep going.  Following the quiet cries from above, you trace the spiralling side of the snow-capped behemoth, eventually reaching a small cliff hanging beneath the mountain's peak.  The sight you're met with manages to take your breath away - and considering the inhabitants of the land around you, that in itself is quite a difficult feat.");
    ...
    public findTheWindUpKey() {
        ...
        //[if (tallness > 60) 
        if (player.tallness > 60) DisplayText("  You can't even stand straight up below the low-hanging ceiling, forced to carefully tread along on your knees over the slick, hard rock.");
        DisplayText("  Squinting around the small, deep green grotto, a pedestal in the center calls out to you.  As you approach it, it becomes evident to you why things are so chilly in here; the outcrop is made up of clear ice - and within, the bodies of the many who had found this place before you.  At least that mystery is cleared up.  Atop the frozen prop lies a small, crystal... crank? Upon closer inspection, it actually resembles a - kitty titties, it's a windup key!  Despite the fact that you should've seen this one coming, you surmise that this is the item you've come searching for.");
    ...
    public giveThatBitchAKeyYo() {
        ...
        if (Flags.list[FlagEnum.GATS_ANGEL_TIME_TO_FIND_KEY] < 150) {
            ...
            DisplayText("All throughout your perilous climb, you cannot help but ogle at the brilliant little crystal key.  You have a suspicion as to how this innocent little key will help the crying colossus on the peak, though you aren't entirely sure this is the artifact she required.  But you have a hunch that it is.");
    ...
    public theWorstEndingEverDotJPG() {
        ...
        //[if (corruption > 49)
        if (player.stats.cor > 49) DisplayText("  It seems like the best response to you for now.  Based on the giantess's sobbing soliloquy, you figure that standing in for the love of the soldier's life will get you straight to the action.");
        else DisplayText("  Wait, what?  It appears that in your desperation to contain the situation, your lingering doubt was enough to get you blurt out a bold faced lie.  You figure that you may as well roll with it for now.");
        ...
        DisplayText(" to try and convince the confused soldier.  You look North over, figuring the best way to work him under your spell.  It isn't a long search, as you decide that his ");
        //[if (hasCock = true) 
        if (player.torso.cocks.count > 0) DisplayText("keyhole");
        //[if (hasCock = false) 
        else DisplayText("windup key");
        DisplayText(" would make a worthy candidate for your first alluring act of debauchery.");
        ...
        //[if (hasCock = true) 
        if (player.torso.cocks.count > 0) DisplayText("You slide the key playfully down your wrist, rubbing your [cock] with your free hand.  You aren't entirely sure if he'll even feel a sensation from your penetration, but perhaps it'll at least get him in the mood.  With your manhood at full mast, you gently skirt along the edges of the keyhole.");
        else DisplayText("A mischievous tongue traces along the smooth, perfect surface of the windup key.  You promptly lower the makeshift toy precariously in front of your [cunt]; if there was ever a time to make a wind up pun or remark as some sort of goofy foreplay, now would be the time.");
        ...
        //[if (corruption > 49) 
        if (player.stats.cor > 49) DisplayText("  ...Well, that was a waste of time.");
        ...
        //[if (corruption > 49) 
        if (player.stats.cor > 49) DisplayText("  More importantly, you have some unsatisfied, pent up lust that you'd like to expend.");
    ...
    public fuckTheAngelBadEnd() {
        ...
        DisplayText(" on a person's tongue to fuck it thoroughly.  You wonder if she's already long forgotten the pile of bolts she was originally trying to save.  A man frozen in time, never to wake up because - whoops!  You're taking care of her needs now.  Maybe you'll invest in a giant goblin-built sex machine, who knows?  Judging by the bitch's aroused whines, she was probably waiting for the chance to strip for the next person to find her up the mountain.  It's about time she let herself go, after all.");
        ...
        if (player.torso.cocks.count > 0) DisplayText("hot, white seed");
    ...
    public stayForPretendNTRGoodWinjobSex() {
        ...
        if (player.stats.lust >= 100) {
            ...
            if (player.torso.vaginas.get(0).wetness >= 4 || player.cumQ() >= 1000) DisplayText(" - figuratively, and literally, as well, as your fluids drip off the edge to add to the rain of sexual fluids the giantess is laying in the midst of");
            DisplayText(".");
        }
    ...
    public conclusionOfGiantessGoodEnd() {
        ...
        if (Flags.list[FlagEnum.PC_ENCOUNTERED_CHRISTMAS_ELF_BEFORE] === 0) DisplayText("\n\nYou have no idea what that is.");
