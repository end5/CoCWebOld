...
export class Valentines {
    ...
    public cuddleWithScyllaVDay() {
        ...
        //[if (hasVagina = true)]
        if (player.torso.vaginas.count > 0) DisplayText("  Your [vagina] is starting to get moist, making a small mess in your underclothes.");
        //[if (hasCock = true)]
        if (player.torso.cocks.count > 0) DisplayText("  You realize it'd probably be a done deal if you didn't resist the pull of your hardening cock towards one of Scylla's orifices, but you decide to spare her any temptations and endure yourself.");
        DisplayText("  In time, your ignited lust slowly changes to constant arousal, and Scylla just sitting there, watching the sun with a beautiful smile on her face, clearly content to just have someone in her arms on this evening is helping you control yourself a lot better.");
        ...
        player.perks.set(new Perk("PureAndLoving", 0, 0, 0, 0));
        return { next: Scenes.camp.returnToCampUseTwoHours };
    }
    ...
    public makeOutWithScyllaVDay() {
        ...
        //[if (cocks = 1)
        if (player.torso.cocks.count === 1) DisplayText("your [cock] is completely hard and stiff, forming a bulge in your clothing");
        else if (player.torso.cocks.count > 1) DisplayText("[eachCock] is by now completely hard, eager to be touched and wanting to be caressed by the nun you're kissing");
        DisplayText(".");
    ...
    public makeOutWithScyllaVDayII() {
        ...
        if (!player.perks.has(PerkType.SensualLover)) {
            ...
            player.perks.set(new Perk("SensualLover", 0, 0, 0, 0));
        }
    ...
    public feedScyllaVDay() {
        ...
        DisplayText("You feed Scylla what feels like a gallon of jizz!");
        player.orgasm();
        DisplayText("\n\n(<b>You have gained the One Track Mind perk.</b>");
        player.perks.set(new Perk("OneTrackMind", 0, 0, 0, 0));
        return { next: Scenes.camp.returnToCampUseTwoHours };
    }
    ...
    public goVisitAbby() {
        ...
        //[if (hasVagina = false)
        else if (player.torso.vaginas.count > 0) DisplayText("\"<i>Well, that's unexpected. I have certain doubts about you showing me such a good time,</i>\" she says.  You tell her you don't actually require a cock at all to do this, and she seems surprised.");
    ...
    public fuckAbbyVDay() {
        ...
        DisplayText("Figuring that the goblin girl's felt so out of place amid a celebration of love, you might as well show her a little affection the only way she's likely to understand.  Pulling aside your [armor] enough to let [eachCock] spring free, you step up behind the bent-over girl and rest your hands on her thick thighs.  The emerald girl's breath quickens as the heat of your manhood presses between the jiggling globes of her ass cheeks and a whimper escapes her throat before she realizes it.  Coughing, in an attempt to cover the moment of weakness, she turns her head to the side to give you a disapproving eye.  “What is this, romance?  Just jam it in, already!” she commands, gruffly.  Her face is flushed a deeper forest hue, pink irises twinkling in the dim light of her spartan home.");
    ...
    public pleasureAbbyVDay() {
        ...
        //([Naga tongue]
        if (player.torso.neck.head.face.tongue.type === TongueType.SNAKE) DisplayText("  Your fingers make it into her pussy alongside your tongue, and the prehensile thing, longer and more flexible than a human's, ravages the insides of her pussy, tasting everywhere as Abby shakes in her orgasm.  Her pussy eagerly tries to milk both your fingers and your tongue, clenching and gripping at them as it drips her pussy juices all over your face.  Abby screams out in a loud, uninhibited orgasm at last, expressing her ecstasy.");
        //([Demon/Dragon Tongue]
        else if (player.torso.neck.head.face.tongue.type === TongueType.DEMONIC) DisplayText("  Abylon shakes in excitement, pleasure, and fear as you extend your inhuman tongue to reach deep into the recesses of her pussy.  She cannot hold out a moment longer, her cunt gripping your inhuman muscle tightly, the hole and muscles inside quivering as you can taster her orgasmic juices with every single point of your tongue.  She screams, shudders and moans, tongue lolling out from the intense pleasure as you force your own tongue to roll around inside her and stretch her in multiple ways. Only you holding her stops Abylon from slumping forward, it seems...");
        //([Human tongue]
        else DisplayText("  Hooking your fingers at the front and back of her cunny, you plunge your tongue in and wiggle it eagerly.  Abby clamps down on all three invaders and loudly moans out, screaming to her content at last, as your face is flooded with pussy juice.");
    ...
    public goVisitPastyVDay() {
        ...
        if (player.inventory.items.has(consumables.BIMBOCH) || player.inventory.items.has(consumables.BC_BEER) || player.inventory.items.has(consumables.GOB_ALE) || player.torso.vaginas.count > 0) {
            ...
            if (player.torso.vaginas.count > 0) MainScreen.addButton(3, "Pussy", pastieValentineIntro, "vag");
        }
        else {
    ...
    public valentineDrinkPastie(itype: ItemType) {
        ...
        //Corruption 0-20] 
        if (player.stats.cor < 33) DisplayText("  It probably isn't.");
        //[Corruption 21-75]
        else if (player.stats.cor < 75) DisplayText("  You guess as long as she's fine afterwards, it should be okay.");
        //[Corruption 76-100]
        else DisplayText("  As long as she's in the mood to drink more than just the alcohol afterwards, you don't really care.");
    ...
    public rubPastieOnYourWangDawg() {
        ...
        //high cum production:
        else DisplayText(" almost blasting her right out of your hand in the torrent of white that spills forth from your mighty member.");
        DisplayText("  The spunk-plastered pixie shudders in your palm, overwhelmed by the spoo drenching her.  She masturbates, feverishly, climaxing in seconds and then again a moment later.  You watch her for a few minutes, the batter-basted bimbo jilling herself silly from the heightened sensitivity your spoo shower has given her.");
    ...
    public goForAPushayDivePasty() {
        ...
        if (player.torso.cocks.count > 0) DisplayText("your other hand moving downwards to tug on [oneCock], ");
        DisplayText("anticipating the incoming wriggling inside your pussy and an orgasm you'll have with an entire fairy buried inside you.");
        ...
        //[if (hascock = true)
        if (player.torso.cocks.count > 0) DisplayText("  All the while, you continue to jerk off [eachCock], preparing yourself for an orgasm on both sides.");
        DisplayText("  She finally takes a full dive inside, and you moan out in pleasure, feeling her little body roll and explore in there, her tongue tasting every bit of fluid she can from what's moisturizing your inner passage.");
        ...
        //[if (hascock = true)]
        if (player.torso.cocks.count > 0) DisplayText("  At the same time, your cock deposits its own load onto the ground as your hermaphrodite genitals achieve a simultaneous orgasm.");
        //[if (isSquirter = true)
        if (player.torso.vaginas.get(0).wetness >= 4) DisplayText("  She doesn't have much to say in the matter when your female ejaculation truly shoots in, though, sending her rolling out of your [vagina] and onto the ground.");
