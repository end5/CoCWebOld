...
export class XmasJackFrost {
    ...
    public processJackFrostEvent() {
        ...
        //No followers
            ...
            DisplayText("You sigh... this really reminds you of back home... you only wish you had someone to share this feeling with... Well, there is no reason you shouldn't enjoy yourself while it snows, so you set about rolling a big ball of snow to make a snowman out of...");
            //Skip to next day...
        ...
        //Rathazul
        else if (player.statusAffects.has(StatusAffectType.CampRathazul) && Flags.list[FlagEnum.JACK_FROST_PROGRESS] === 2) {
            DisplayText("Rathazul approaches you.  \"<i>[name]?  What is going on?  It hasn't snowed in Mareth for years now.  And why only around the camp?  I wandered out to investigate, but outside there is no snow at all...</i>\" he asks, concerned.");
        ...
        //Izma
            ...
            DisplayText("You wonder where Izma is; as a shark, maybe she doesn't like the cold?  Even as the thought crosses your mind, you note a surprisingly large pile of snow sitting not too far - with a very suspicious striped fin jutting out of it.  You rush over, calling for Izma and asking if she's all right.");
        ...
        //Jojo
        else if ((player.statusAffects.has(StatusAffectType.PureCampJojo) || jojoScene.campCorruptJojo()) && Flags.list[FlagEnum.JACK_FROST_PROGRESS] <= 4) {
            //Pure
            if (player.statusAffects.has(StatusAffectType.PureCampJojo)) {
                DisplayText("Jojo is sitting on his usual rock, one hand out to catch snowflakes with a beatific smile of awe on his face.  \"<i>Look, [name], snow!  I haven't seen snow since I was a very, very small boy.</i>\"  He tells you.");
            ...
            //Corrupt
                ...
                DisplayText("Out in the outskirts of the camp you notice your pet mouse-slut Jojo, casually fucking a small mound of snow like an animal.  You laugh to yourself, now, now... doesn't he know that there are better things to fuck than a pitiful mound of snow on the ground...");
        ...
        //less than 5!
            ...
            if (!amilyScene.amilyCorrupt()) {
                DisplayText("Amily is warily prowling across the icy surface, occasionally glancing all around as if trying to spot an ambush.  Other times, she suddenly leaps up in a flurry, whirling around in mid-air as something catches her over-large ears.  All in all, it's quite obvious she's scared.");
            ...
            //Corrupt
                ...
                DisplayText("Out in the outskirts where your favorite cum-slut lives, you spot her slowly rubbing the snow over her erect nipples, the cold making her shiver in pleasure.");
                ...
                if (player.gender > 0) {
                    ...
                    if (player.torso.cocks.count > 0) {
                        ...
                        else if (player.torso.vaginas.count > 0) DisplayText(", even tickling your [cunt] for added tantalization");
                        DisplayText(".");
        ...
        //Tainted Ember
            ...
            DisplayText("The dragon is wandering your camp in open fascination.  \"<i>What is this weird white stuff?  It's cold and wet... but soft and fluffy, too,</i>\" " + emberScene.emberMF("he", "she") + " says, gently catching a snowflake.");
        ...
        //Marble (Written by TDM himself)
        else if (Flags.list[FlagEnum.JACK_FROST_PROGRESS] <= 7 && player.statusAffects.has(StatusAffectType.CampMarble)) {
            ...
            if (Flags.list[FlagEnum.MARBLE_KIDS] > 0 && Flags.list[FlagEnum.MARBLE_NURSERY_CONSTRUCTION] >= 100) {
                DisplayText("You find Marble simply staring into the sky, a look of wonder covering her face.  Dancing around her in excitement ");
                if (Flags.list[FlagEnum.MARBLE_KIDS] > 1) DisplayText("are ");
                else DisplayText("is ");
                DisplayText("your kid");
                if (Flags.list[FlagEnum.MARBLE_KIDS] > 1) DisplayText("s");
                DisplayText(", playing in the snow, blowing the snow, and experimenting with the snow.  Exactly what ");
                if (Flags.list[FlagEnum.MARBLE_KIDS] === 1) DisplayText("she");
                else DisplayText("they");
                DisplayText(" are up to at any given moment is really hard to say, and you're not even sure that the little girl");
                if (Flags.list[FlagEnum.MARBLE_KIDS] > 1) DisplayText("s");
                DisplayText(" know");
                if (Flags.list[FlagEnum.MARBLE_KIDS] === 1) DisplayText("s herself.");
                else DisplayText(" themselves.");
                ...
                if (Flags.list[FlagEnum.MARBLE_KIDS] === 1) DisplayText("child");
                else DisplayText("children");
                DisplayText(" playing together once more.  A moment later, Marble puts her hand to her expansive backend again, before suddenly exclaiming again in realization.  \"<i>Sweetie, watch our kid");
                if (Flags.list[FlagEnum.MARBLE_KIDS] > 1) DisplayText("s");
                DisplayText(" for a moment, I'll be right back!</i>\" and she leaves the camp as quickly as she can, without slipping and falling on her ass again.");

                DisplayText("\n\nYou certainly don't mind staying behind for now, it is certainly a lot of fun spending time with your kid");
                if (Flags.list[FlagEnum.MARBLE_KIDS] > 1) DisplayText("s");
                DisplayText(" and just having fun.  Your cow-girl daughters are certainly not lacking in energy in the cold snow, their fur covered legs and built in shoes probably help a lot with this.  ");
                if (Flags.list[FlagEnum.MARBLE_KIDS] > 1) DisplayText("They");
                else DisplayText("She");
                DisplayText(" listen");
                if (Flags.list[FlagEnum.MARBLE_KIDS] === 1) DisplayText("s");
                DisplayText(" with rapt attention when you explain what snow is, and about all the things you use to do in it back home.  Hands-on demonstrations are also in high demand.");
                ...
                if (Flags.list[FlagEnum.MARBLE_KIDS] > 1) DisplayText("s");
                DisplayText(" to ride on!  Happy cries of joy fill the air for some time, as you and Marble take turns in giving rides on the sled.");

                //PC strength check:
                //(>=85)
                if (player.stats.str >= 85) {
                    DisplayText("\n\nIt would seem that your strength has won you more favor with ");
                    if (Flags.list[FlagEnum.MARBLE_KIDS] > 1) DisplayText("the girls");
                    else DisplayText("your daughter");
                    DisplayText(" when it comes to giving sled rides, Marble doesn't seem to mind too much though.  Your mate actually decides to join in the riding part after a while!");
                }
                //(60-84)
                    ...
                    DisplayText(" in deciding who gives the best rides.  In the end, a clear winner is never decided, but everyone agrees that it was a lot of fun.");
                }
                //(<=59)
                else {
        ...
        //if she ain't a follower and is a lover, just have her visit.
            ...
            DisplayText("\"<i>Hey, champ, what's going on here?</i>\"  Helia calls as she sees you approach.  Gentle trails of steam are wafting from the salamander's body as her own internal heat melts any snow that builds up on her.");
            ...
            //(If PC has a cock)
            if (player.torso.cocks.count > 0) DisplayText("  A torrent of seed splashes against Helia's taut belly, as [eachCock] throbs and spews its seed while sandwiched between the two of you.");
            //(if PC has a pussy)
            if (player.torso.vaginas.count > 0) DisplayText("  Your [vagina] quivers and spill your juices with the force of a wave.  Moisture falls on the ground to join in with Helia's own discharge.");
        ...
        //Feast time! Just a collection of words on the pleasant day...
            ...
            DisplayText("This small holiday of yours was much needed.  You feel reinvigorated and even more determined to put an end to this struggle with demons.");
            ...
            //(if no sexable follower)
                ...
                if (camp.companionsCount() > 1) DisplayText("s");
                DisplayText(".  You can tell that you were not the only one needing this break... but unfortunately all good things must come to an end, and so you bid your companion");
                if (camp.companionsCount() > 1) DisplayText("s");
                DisplayText(" good night and retire for the night.");
            }
            ////more than one sexable follower.
            else {
                DisplayText("You take care of the preparations and cooking, whipping up a delightful meal for you and your companions.  For a moment you look around - this small band you've formed feels enough like a family that you wouldn't mind having more days like this in the future.  You vow to work extra hard to make this a reality.");
    ...
    public penetradePureMouseWaifu() {
        ...
        if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText(" and painting your belly white with futa-spooge");
        DisplayText(".");
    ...
    public makeOutWithPureMouseWaifu() {
        ...
        if (Flags.list[FlagEnum.AMILY_WANG_LENGTH] > 0) DisplayText("Smiling sheepishly you  tell her to mind the erection... though you're not actually looking for sex, you can't help with how cute she looks...  \"<i>Flirt.</i>\"  Amily giggles, but otherwise doesn't comment on it.  ");
        DisplayText("The mouse promptly walks over and sits down just in front of you, her back facing you and giving you a perfect opportunity to hug her.  Not keen on missing such an opportunity, you do so.");
    ...
    public penetradeCorruptJojoAss() {
        ...
        if (player.cumQ() >= 700) DisplayText(" all over his own cum-engorged gut and ");
        DisplayText("onto the snowy earth underneath him.  He cums and cums until he's exhausted even his prodigious balls, creating a salty quagmire of semi-melted snow and spooge under him that he promptly collapses face-first into.");
