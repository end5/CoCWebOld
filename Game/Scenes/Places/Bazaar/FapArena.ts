...
private fapResults(place:number = 3):void {
	...
	if(place === 3) {
		...
		DisplayText("Your left partner is very professional.  At the very moment your " + Desc.Cock.describeCock(player, x) + " starts shooting its goo, your cock-milker yells in a thundering voice: \"<i>NUMBER " + (num-1) + " SCORING!</i>\"  For a moment, lost in your pleasure, you pay little attention to the world outside of your " + Desc.Cock.describeCock(player, x) + ".  Then you remember the game and shout in a whorish, pleasure-filled voice: \"<i>NUMBER " + num + " CUMMING!</i>\"");
		if(player.torso.vaginas.count > 0) {
		DisplayText("  Your thighs tense a few seconds before your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " squirts too, forming a ");
		...
		//[if player lactates]
		if(player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 1.5) DisplayText("  Your " + Desc.Breast.describeAllBreasts(player) + " also release a milky flood of their own, the soft feeling of cream teasing your nipple only prolonging your orgasm.");
		DisplayText("\n\n");
		
		DisplayText("The cute referee nods in acknowledgement.  Her eyes seem to be filled with longing for your cum-spilling " + Desc.Cock.describeMultiCockShort(player) + ", and you know she'd eagerly finish you off if it weren't for her professionalism.\n\n");
	...
	//[else, the player wins]
		...
		DisplayText("You mentally keep track of all the people still in play.  They are fewer and fewer.  You just have to hold on a little longer.  Four... three... two... that's it.  You made it.  Phew, you have won just in time, your " + Desc.Cock.describeCock(player, x) + " was starting to twitch on its own.  Unhindered by the fear of losing, you completely give up to your orgasm and sigh in utter satisfaction and relief as you release one of the most abundant cumshots you've ever had.  Lost in climax, you carelessly ");
		if(player.cumQ() >= 1000) DisplayText("soak");
		else if(player.cumQ() >= 500) DisplayText("pollute");
		else DisplayText("wet");
		DisplayText(" the floor with your sexual juices and yell in a whorish voice: \"<i>NUMBER " + num+ " CUMMING!</i>\"  With a half-closed eye you notice the referee nodding in acknowledgement and you point your " + Desc.Cock.describeCock(player, x) + " at her, trying to soak her too.  She hardly dodges it and some lands on her arms, but she doesn't seem to notice.");
				if(player.torso.vaginas.count > 0) {
			DisplayText("  Your thighs tense a few seconds before your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " squirts too, forming a ");
			if(player.torso.vaginas.get(0).wetness >= 4) DisplayText("large puddle");
			else if(player.torso.vaginas.get(0).wetness >= 2) DisplayText("puddle");
	...
	//[else, corr >50]
	else DisplayText("Your body tenses and " + Desc.Cock.describeMultiCockSimpleOne(player) + " hardens at the perspective of a collective anal orgy.  You yell in lust like a minotaur in rut, \"<i>BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!</i>\" along with the others.  ");
	//end of global condition about PC's corr, display the following
	...
	if(place <= 1) {
		...
		if(randInt(2) === 0) {
			temp = 3;
			DisplayText("You eagerly grab the luxurious, gelatinous butt cheeks in front of you.  You notice a pussy underneath that fine ass, freely spilling sexual juices.  If the compelling order wasn't resonating deep in your mind, you would easily lose yourself in the herm's glorious depths. Nevertheless, your " + Desc.Cock.describeCock(player, x) + " remembers its job and starts pushing its way between these tender orbs, then into the herm's tight anus.\n\n");
		}
		else {
			DisplayText("You eagerly grab the firm, muscular ass cheeks in front of you, probably belonging to some vigorous male.  You grind your " + Desc.Cock.describeCock(player, x) + " against his back and it automatically starts spilling pre-cum; as soon as the seminal liquid trickles down his shapely butt, you can feel his body tensing, as if his lust reached a new level.  Nevertheless, your " + Desc.Cock.describeCock(player, x) + " remembers its job and starts pushing its way between these gorgeous buttocks, then into the man's tight anus.\n\n");
			temp = 1;
		}
		...
		if((player.torso.cocks.count > 2) || (!tent && player.torso.cocks.count > 1)) {
			...
			DisplayText("own, unhindered by the tight colon holding your first " + Desc.Cock.describeCock(player, x) + ".");
		}
		...
		DisplayText("\n\nWincing in pain and pleasure mixed, your " + Desc.Cock.describeCock(player, x) + " lets out one ultimate spurt of goo before resting, deeply buried into your partner's ass.  You fall over, making the whole chain fall to rest by domino effect.  After this torrid anal session, the butt-fuck train has exhausted its fuel.\n\n");
	}
	//[else if you didn't win]
	else if(place === 2) {
		DisplayText("You almost instantly feel firm arms groping your " + Desc.Butt.describeButt(player) + " and the tip of an erect dick knocking at the door of your colon.  It slowly massages your rectum, and you can't help but giggle a bit as you feel a tingle of pleasure passing through your body.\n\n");
		
		//[1/2 chance of this]
		if(randInt(2) === 0) DisplayText("You eagerly grab the luxurious, gelatinous butt cheeks in front of you.  You notice a pussy underneath that fine ass, freely spilling sexual juices.  If the compelling order weren't resonating deep in your mind, you would easily lose yourself in the herm's glorious depths.  Nevertheless, your " + Desc.Cock.describeCock(player, x) + " remembers its job and starts pushing its way between these tender orbs, then into the herm's tight anus.\n\n");
		else DisplayText("You eagerly grab the firm, muscular ass cheeks in front of you, probably belonging to some vigorous male.  You grind your " + Desc.Cock.describeCock(player, x) + " against his back and it automatically starts spilling pre-cum; as soon as the seminal liquid trickles down his shapely butt, you can feel his body tensing, as if his lust reached a new level.  Nevertheless, your " + Desc.Cock.describeCock(player, x) + " remembers its job and starts pushing its way between these gorgeous buttocks, then into the man's tight anus.\n\n");

		DisplayText("You suddenly hear a muffled moan from behind.  Someone is probably being fucked at the back of the train.  You brace yourself for the imminent anal penetration... and here it comes.  The dong is projected at full force into your " + Desc.Butt.describeButt(player) + " then all the way inside your colon; you're protected only by the copious glaze of pre-cum its bearer had been applying.");
		Mod.Butt.displayStretchButt(player, 5+randInt(15),true,true,false);
		DisplayText("  Instinctively, you reach for the butt awaiting your " + Desc.Cock.describeCock(player, x) + " and push as if your life depended on it.  You notice how your sodomite partner does the same to the ass in front of him, and so on.  Eventually the last one is pushed face down onto the floor, but her butt bumps against the dong fucking her, making the cock's owner bump in response, and it all propagates back to your own pecker; you wince in pleasure as your asshole impales itself further on the turgescent member behind you.  You are pushed and pulled rhythmically, your throbbing member and the one fucking you serving both as vectors in this slow wave of anal pleasure.  Unable to keep control of your thrusts, you let go of your body, enjoying the feeling of being shoved in both directions, each motion bringing either a jolt of pleasure in your crotch or your " + Desc.Butt.describeButt(player) + ".  You are being slowly overwhelmed by the delicious tightness of the butt you're fucking as well as the fat piece of meat pounding your own.  The raw feelings eventually prove too much for your horny body, and you jerk your head back and moan like a whore, waiting for the incoming orgasm.\n\n");
		tentacleFapSpecial(place);
		DisplayText("A pressure builds at your crotch as a milky torrent of seed finds its way into your urethra, down your sodomite partner's colon.  Almost simultaneously, you feel an equally thick spray of spooge being released inside your own " + Desc.Butt.describeButt(player) + ". At the peak of your orgasm, you are unable to think of anything but the steady flow of jism coming in and out of you.");
		tent = tentacleFapCum();
		//[if player has multicocks] 
		if((player.torso.cocks.count > 2) || (!tent && player.torso.cocks.count > 1)) {
			if(player.torso.cocks.count > 3 || (!tent && player.torso.cocks.count > 2)) DisplayText("  Your other cocks also spill ");
		...
		//end of conditions 
		DisplayText("\n\nFinally the strength of all the ejaculations is starting to ebb, as well as your fading orgasm.  Wincing in pain and pleasure mixed, your " + Desc.Cock.describeCock(player, x) + " lets out an ultimate spurt of goo before resting, deeply buried into your partner's ass.  You fall over, making everyone after fall too by domino effect.  As you slowly doze off, you have a special thought about the still semi-erect cock being lodged in your " + Desc.Butt.describeButt(player) + ".  That wagon of butts and dicks... it felt so right, so powerful, so... connected.\n\n",false);
	}
	//[else, if you lost]
	else {
		DisplayText("You almost instantly feel firm arms groping your " + Desc.Butt.describeButt(player) + " and the tip of an erect dick knocking at the door of your colon.  It slowly massages your rectum, and you can't help but giggle a bit as you feel a tingle of pleasure passing through your body.  You prepare yourself for the crazy butt-fuck train, with you being at the end of it.\n\n");
		
		DisplayText("You suddenly hear a muffled moan from behind. Someone is probably being penetrated at the back of the train.  You brace yourself for the imminent anal-fucking... and here it comes.  The dong is projected at full force into your " + Desc.Butt.describeButt(player) + " then all the way inside your colon; you're protected only by the copious glaze of pre-cum its bearer had been applying.");
		Mod.Butt.displayStretchButt(player, 5+randInt(10),true,true,false);
		DisplayText("  You are pushed face down to the floor, bumping your ass against your anal partner's crotch, impaling it further on that fat erect meat.  Hands roughly dig their fingers into the soft flesh of your cheeks as their owner gives you another mad, uncontrolled thrust, an avatar of the train of lust behind him.  Probably the force of thirty people butt-fucking into him was too much for him to bear, so he passes it on to you.  But unlike him, you don't have an asshole to reach and bury your " + Desc.Cock.describeCock(player, x) + " into, so you are smashed on the floor again, and again, and again.  It hurts, yet it feels so... powerful, so strong.  There is only one dong pounding your " + Desc.Butt.describeButt(player) + ", yet it feels like you're being ass-fucked by thirty people at once.  Propelled by the might of thirty thrusts, the huge shaft is tearing its way inside your poor " + Desc.Butt.describeButthole(character.torso.butt) + " and, needless to say, being the end of this line is probably the hardest butt-fuck you'll ever have.\n\n");
		tentacleFapSpecial(place);		
		DisplayText("You feel that towering rod grinding against the rough insides of your colon, the friction providing a shamefully amazing sensation of violation.  The large hands are gripping, pinching and groping your " + Desc.Butt.describeButt(player) + " with ease and feverish lust, and your abused flesh feels every tingle, every prickle of it, bringing you to the edge every time.  You know you are being ferociously dominated by that cock, but the pleasure is too good for you to care.  The raw feelings eventually prove too much for your horny body, and you simply lay there in the dust, waiting for the incoming orgasm.  Eventually you feel an enormous spray of spooge being released in your insides.  Almost instantly, you start unloading thick ropes of jism at a steady rate.  Wallowing in a pool of dirt muddied with sexual juices like a worthless slut, you grind your " + Desc.Butt.describeButt(player) + " against the cock harder, in order to milk every drop of semen coming out of it.  But your colon soon reaches its limits and spooge starts dribbling out in small spurts.");
		tentacleFapCum();
		//[if player has vagina]
		if(player.torso.vaginas.count > 0) {
			DisplayText("  Your feminine part squirts a ");
	...
	//[if you lost]
		...
		DisplayText("You awkwardly remove the dick from your ass, start cleaning yourself from all the dirt and cum before leaving the tent.  Before you go, the nude referee approaches you and gently squeezes your " + Desc.Cock.describeCock(player, x) + "; you still wince from the pounding it took earlier.  \"<i>Awww, looks like you had a rough time, didn't you?  Well, it happens sometimes.  Hopefully you will get better at this.  See you next game!</i>\"");
		player.orgasm();
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
...
private tentacleFapSpecial(place:number):void {
	...
	if(player.torso.cocks.get(x).length >= 40 && player.torso.cocks.get(x).type === CockType.TENTACLE) {
		...
		if(place <= 2) {
			DisplayText("Needless to say, this interference creates a general disruption in the whole butt-fuck wave.  The whole train of peckers and puckers starts moving irregularly, grinding against your " + Desc.Cock.describeCock(player, x) + " at the most unexpected moments, and the pleasure you're getting from your extra tentacle cock doesn't help.  Everyone's butt and dick is affected to some extent; everyone's pleasure is increased because of your audacious initiative, and loud, reluctant moans are heard.");
