...
export class Dreams {
    public static dreamSelect(character: Character): boolean {
        ...
        let temp: number = 0;
        // dream quantity
        let dreamQuantity: number = randInt(Game.player.stats.lib / 10 + Game.player.stats.cor / 20) + Game.player.stats.cor / 20;
        ...
        if (Game.player.minotaurAddicted()) {
            ...
            choices[choices.length] = 6;
            if (Game.player.perks.has(PerkType.MinotaurCumAddict)) choices[choices.length] = 6;
        }
        // Akbal
        ...
        // Exgartuboobs
        if (Game.player.statusAffects.get(StatusAffectType.Exgartuan).value1 === 2) {
            choices[choices.length] = 8;
            choices[choices.length] = 8;
            choices[choices.length] = 8;
        }
        // Exgartucock
        if (Game.player.statusAffects.get(StatusAffectType.Exgartuan).value1 === 1 && Game.player.torso.cocks.count > 0) {
        ...
        if (Flags.list[FlagEnum.NUMBER_OF_TIMES_MET_SCYLLA] > 0 && Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00150] > 0) {
            ...
            choices[choices.length] = 12;
            choices[choices.length] = 12;
            if (Game.player.torso.cocks.count > 0 && Game.player.stats.cor > 50) {
        ...
        if (Game.player.perks.has(PerkType.Lusty)) dreamLust += (dreamQuantity / 3);

        // Well Adjusted cuts by half
        if (Game.player.perks.has(PerkType.WellAdjusted)) dreamLust = (dreamLust / 2);
        ...
        if (dreamQuantity > 5) {
            // Roll for dream!
            daydream = choices[randInt(choices.length)];
            // normal fantasies...
            if (daydream === 0) {
                if (Game.player.stats.cor <= 33) {
                    // randomly 1 of two simple fantasies.
                    if (randInt(2) === 0) DisplayText("tender sex with a sweetheart back home.  ");
                    else {
                        DisplayText("beautiful nude ");
                        if (Game.player.gender <= 1 || (Game.player.gender === Gender.HERM && randInt(2) === 0)) DisplayText("women ");
                        else DisplayText("men ");
                        DisplayText("massaging you, slowly moving their hands over your most intimate places.  ");
                    }
                }
                if (Game.player.stats.cor > 33 && Game.player.stats.cor <= 66) {
                    DisplayText("being violently raped and used by demons.  ");
                }
                if (Game.player.stats.cor > 66) {
                    dreamQuantity = randInt(5);
                    if (dreamQuantity <= 3) DisplayText("giving yourself to the demons fully, allowing yourself to be tied down and owned as your body is warped for their twisted pleasures.  ");
                    else DisplayText("being captured and taken to a rusted building with dark smokestacks that belch sweet purplish smoke.  Inside are rows of multi-dicked, huge breasted humans, approximately thirty of them.  Each is shackled with their legs spread and torso bent over, and each has tight fitting suction tubes fitting over their tits and cocks.  Every set of tubes is pulsing with suction, drawing rivers of sticky white fluids from its slave's over-endowed breasts and balls.  You shudder in horror and arousal as you realize the victims seem to be arranged by age, ending with an empty machine next to the youngest slut-cow.  The inhuman strength of your captors easily overpowers your struggles as you are forced into your shackles, the metal locks clicking with finality.  A funnel is forced into your mouth, force-feeding you slick corrupted fluids that taste like sex and make your head swim.  Your vision fades as you feel heat in your chest and groin, making you swoon from the drugged cocktail and pleasure of your new best friends – the suction tubes. All you can hear is your own desperate moans... no, wait... that was a dream... but it was so twisted and hot that you're still panting with lust.  ");
                }
            }
            // canine
            else if (daydream === 2) {
                // Male-ish dreams
                if (Game.player.gender <= 1 || (Game.player.gender === Gender.HERM && randInt(2) === 0)) {
                    if (randInt(2) === 0) DisplayText("locking a thick knotted cock inside a female, the pheromones of her heat making your maleness twitch and flex, the knot bulging obscenely as you begin to impregnate her.  ");
                    else DisplayText("stroking a knotted doggie-prick, gently stroking and squeezing it, yipping in pleasure as your pointed dog-cock leaks steady streams of fluids.  ");
                }
                // female
                else {
                    // heat dream!
                    if (Game.player.inHeat) {
                        DisplayText("being pregnant, your belly bulging obscenely with the fruits of all your frequent copulations, and your breasts swollen with breast milk.  You imagine your condition enhancing your libido, driving you seek out sexual partners willing to pleasure your distorted form.  ");
                    }
                    // normal dream
                    else {
                        DisplayText("being in heat, your cunt sopping wet with moisture and desire, intense pheromones pouring off you to make all the males rigid and ready.  ");
                    }
                }
            }
            // horse
            else if (daydream === 1) {
                if (Game.player.gender === Gender.MALE || (Game.player.gender === Gender.HERM && randInt(2) === 0)) DisplayText(" running the plains with a harem of beautiful centaur fillies.  Your bloated equine endowments swelling with blood at the sight of their fertile backsides and potent pheromones.   You dream of staying up half the night to service your animalistic brides insatiable desires, plugging them over and over until your baby-batter is running in steady streams from their backsides.  ");
                else DisplayText("running your own farm, complete with a stable of docile, horse-morphed studs.   Of course each had to be captured as he came through the portal, until you had enough studs to keep your cunny happy and your womb filled with equine-spunk.  You saunter out to the barn and smile at the immediate reaction your presence has on your livestock, their leering eyes glued to every curve of your nude body.   Each of them lines up, guiding their rapidly hardening shafts through specially crafted holes in the stalls.  You giggle as your oldest acquisition struggles, trying to get the massive flare of his head through the hole before it gets any bigger.  You dream of walking down the aisle, granting each stud the release he so desires, taking the largest and most worthy to feed your tainted womb's thirst for hot spunk.  ");
            }
            // cow
            else if (daydream === 3) {
                ...
                else DisplayText("roaming the mountain-sides while you hunt for a mate, your turgid shaft and swelling balls aching with the need for release.  ");
            }
            // cat
            else if (daydream === 4) {
                // FEMALE
                if (Game.player.torso.vaginas.count > 0 && (!Game.player.torso.cocks.count > 0 || randInt(2) === 0))
                    DisplayText("being a full cat and getting pounded by another as you mewl with pleasure.  He comes and pulls out, the barbs on his cock rake your insides as you yowl from the sensation. You clean yourself before searching for another cat to pound you, then another, and another...  ");
                // MALE
                else
                    DisplayText("prowling through the forest as a cat.  One dream in particular has you encountering a female cat in heat, and filling her womb to the brim with cum as you rake her insides with the barbs on your cat-cock. You dream that you impregnate every female you come across, making sure you fight off any competing males...  ");
            }

            // demon
            else if (daydream === 5) {
                DisplayText("being used and abused by demons of all varieties.  ");
            }
            // minotaur cum
            else if (daydream === 6) {
                ...
                DisplayText("grinding your rear into his thick cock, your hand reaching behind you to caress Akbal's cheek.  Having his prick between your ass is almost too distracting, but you know you have a duty to perform.  You raise your hands and address the acolytes before you, kneeling and observing.\n\n");
                ...
                DisplayText("\"<i>Glory unto Akbal,</i>\" Buttslut whispers.\n\n");

                DisplayText("You awake with a start, grumbling slightly before turning over and trying to get back to sleep.  Your asshole twitches.");
            }
            // Exgartuboobs
            else if (daydream === 8) {
            ...
            // Exgartucock
                ...
                DisplayText("Hands roaming across your growing bust, you eagerly jiggle it against his prick.  The steaming meat resting in your cleavage feels less like an invader and more like a prisoner, trapped between your massive mounds.  He obviously appreciates your efforts, groaning and grunting as he thrusts into your overwhelming titflesh.  Thick as it is, you can easily feel every vein and bulge in his shaft as it pushes past your sensitive mammaries.\n\n");
                ...
                DisplayText("Your partner licks his lips and sighs in slow release, before pausing.  Looking at you carefully, he speaks.  \"<i>So, is this your dream?</i>\" Exgartuan asks.  \"<i>Or mine?</i>\"\n\n");

                DisplayText("You awake with a start.  Your cock flops over grumpily.");
            }
            // Rubber/Latex Skin
            else if (daydream === 10) {
                ...
                DisplayText("Like before, you drop to the ground after, used.\n\n");

                DisplayText("So it goes, the cycle repeating.  Eternally making your plump O face, unmoving without prompting, and discarded.  The latex sex doll in an unknown harem.  Did they ever know you were once a mortal?\n\n");

                DisplayText("You awake from your dream panting slightly, but not sweating.  You don't do that anymore.\n\n");

                DisplayText("You're not certain how to feel about that.");
            }
            ...
            // Scylla + dom + dong
                ...
                if (Game.player.gender <= 1) {
                    ...
                    // Male Trap Dream #2:
                        ...
                        if (Game.player.torso.cocks.count > 0) DisplayText("  [EachCock] is fully erect and throbbing.");
                        else if (Game.player.torso.vaginas.count > 0) DisplayText("  Your [vagina] dripping wet with need.");
                        DisplayText("  You shudder as you recall the erotic dream you were just having, a hand caressing your aching loins.");
                        // end dream
                        // lust increase
                    }
                ...
                // Dream about living in a borrow and having lots and lots of kids.  Many furred, big eared bodies are running around and you feel the pregnant belly of your mate, or feel your own pregnant belly.
                    ...
                    DisplayText("finding yourself in an underground burrow, with many small furred bodies with large ears scurrying around all over.  The place is warm and cozy, while also filled with the smell of mice and sex.  You love it down here, and you love seeing all your energetic children running around and having fun.\n\n");
                    DisplayText("You have a great larder, able to feed everyone as much as they need.  It has never been a problem to keep it full.  It's a veritable paradise for a rodent such as yourself.  Then you find your way into the master bedroom and see its massive bed, the site where your great family was brought into existence.\n\n");

                    if (Game.player.gender === Gender.MALE || (Game.player.gender === Gender.HERM && randInt(2) === 0)) {
                        DisplayText("Things blur for a moment, and you find your head resting against a pregnant belly, covered in fine fur.  The results of your virility as a father, and soon to be added to the great brood that you helped bring into this world...\n\n");
                    } else {
                        DisplayText("Things blur for a moment, and you find yourself laying on the bed with your hands wrapped around your heavily pregnant belly.  The product of your fertility and the virility of your great mate, and soon to be added to the great brood you've already brought into the world...\n");
                    }
                }
            }
    ...
    public static dayTenDreams() {
        ...
        if (Game.player.torso.cocks.count > 0) {
            if (Game.player.stats.cor < 50) {
                DisplayText("Your task completed, you return through the portal to your home.  Exhausted but happy, the sight of your village fills you with a surge of emotions.  Most prominently, and perhaps most enjoyably: pride.  You did it.\n\n");
                ...
                DisplayText("\"<i>Welcome back,</i>\" she smiles.\n\n");

                DisplayText("\"<i>Thank you, Mom,</i>\" you respond, wrapping your arms around her and kissing her deeply.  Your hands take hold of her rear, gripping-\n\n");

                DisplayText("Shocked awake, you blink as the light of your dying fire flickers dimly across the campsite.  That was...\n\n");

                DisplayText("This place is getting to you.");
            }
            else {
                DisplayText("Your task completed, you return through the portal to your home.  Exhausted but happy, the sight of your village fills you with a surge of emotions.  Most prominently, and perhaps most enjoyably: pride.  You did it.\n\n");
            ...
            if (Game.player.stats.cor < 50) {
                ...
                DisplayText("\"<i>Welcome back,</i>\" he smiles.\n\n");

                DisplayText("\"<i>Thank you, daddy,</i>\" you respond, wrapping your arms around him and kissing him deeply.  Your hands slip into his pants, lowering-\n\n");
                ...
                DisplayText("\"<i>Thank you, daddy,</i>\" you respond, wrapping your arms around him and kissing him deeply.  Your hands slip into his pants, lowering them and exposing his crotch.  Too excited to control yourself, you drag your lips down his chest before wrapping them around his cock and taking him into your mouth.  You pull him deep, teasing his balls with your lower lip.  Though it isn't necessary to gag - you could have swallowed him easily - you do anyway.  Daddy deserves a show.\n\n");

                DisplayText("His cum splatters into your throat.\n\n");

                DisplayText("You smile with glazed teeth as you straddle him in his bed and mount him.  Some disappointment lingers in you that you didn't just let daddy take your ass then and there, but the prospect of his cum flooding your womb and fertilizing you is too delightful to pass up.\n\n");

                DisplayText("\"<i>Do you like it?</i>\" you ask, before moaning his name.\n\n");

                DisplayText("\"<i>You're a good girl,</i>\" he answers.\n\n");

                DisplayText("His balls gurgle as they fill you.  You squeal in delight and lick his chest, demonstrating your affection.  Clenching him tightly, you make sure not to spill daddy's seed, and imagine giving birth to a sexy, busty daughter, teaching her to love him.  One day, teaching her to replace you.\n\n");

                DisplayText("You wake up in a sweat.  Fuck.");
            }
        }
    ...
    public static displacerDreamIII() {
        ...
        let x: number = Game.player.torso.cocks.count;
        while (x > 0) {
            x--;
            if (Game.player.torso.cocks.get(x).sock === "amaranthine" && Game.player.torso.cocks.get(x).type !== CockType.DISPLACER) {
                if (Game.player.torso.cocks.get(x).type !== CockType.DOG) Game.player.torso.cocks.get(x).knotMultiplier = 1.5;
                Game.player.torso.cocks.get(x).type = CockType.DISPLACER;
            }
        }
        return { next: Game.playerMenu };
    }
