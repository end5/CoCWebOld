...
export class OvielixirPregEvent implements IPregnancyEvent {
    public incubationDisplay(player: Character, womb: Womb): void {
        if (player.body.vaginas.length === 0) {
            player.effects.remove(StatusEffectType.Eggs);
    ...
    public birthScene(player: Character, womb: Womb): void {
        ...
        // Small egg scenes
        if (player.effects.get(StatusEffectType.Eggs).value2 === 0) {
            // light quantity
            if (player.effects.get(StatusEffectType.Eggs).value3 < 10) {
            ...
            // High quantity
                ...
                CView.text("you quiver in orgasm, popping out the last of your eggs as your body twitches nervelessly on the ground.   In total you lay " + eggDescript(player) + ".");
                player.orgasm();
            }
        }
        // Large egg scene
        else {
            CView.text("A sudden shift in the weight of your pregnant belly staggers you, dropping you to your knees.  You realize something is about to be birthed, and you shed your " + player.inventory.equipment.armor.displayName + " before it can be ruined by what's coming.  A contraction pushes violently through your midsection, ");
            ...
            if (player.effects.get(StatusEffectType.Eggs).value3 >= 11) CView.text("Your swollen belly doesn't seem to be done with you, as yet another egg pushes its way to freedom.   The stimulation so soon after orgasm pushes you into a pleasure-stupor.  If anyone or anything discovered you now, they would see you collapsed next to a pile of eggs, your fingers tracing the outline of your " + describeVagina(player, player.body.vaginas.get(0)!) + " as more and more eggs pop free.  In time your wits return, leaving you with the realization that you are no longer pregnant.  ");
            CView.text("\n\nYou gaze down at the mess, counting " + eggDescript(player) + ".");
            player.orgasm();
        }
        CView.text("\n\n<b>You feel compelled to leave the eggs behind, ");
        if (player.effects.has(StatusEffectType.AteEgg)) CView.text("but you remember the effects of the last one you ate.\n</b>");
        ...
        player.effects.add(StatusEffectType.LootEgg, 0, 0, 0, 0);
        womb.clear();
    }
}

function eggDescript(player: Character, plural: boolean = true): string {
    let descript: string = "";
