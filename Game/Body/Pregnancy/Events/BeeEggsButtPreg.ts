...
export class BeeEggsButtPregEvent implements IPregnancyEvent {
    ...
    public birthScene(player: Character, buttWomb: ButtWomb): void {
        ...
        if (player.body.chest.reduce(BreastRow.AverageLactation, 0) >= 1 && player.body.chest.sort(BreastRow.Largest)[0].rating > 2) CView.text("Seeking even greater sensation, your hands gather the honey and massage it into your " + describeBreastRow(player.body.chest.get(0)) + ", slowly working up to your nipples.  Milk immediately begins pouring out from the attention, flooding your chest with warmth.  ");
