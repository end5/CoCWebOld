...
export class ImpPregEvent implements IPregnancyEvent {
    ...
    public birthScene(player: Character, womb: Womb): void {
        ...
        if (player.effects.has(StatusEffectType.BirthedImps)) player.effects.get(StatusEffectType.BirthedImps).value1 += 1;
        else player.effects.add(StatusEffectType.BirthedImps, 1, 0, 0, 0);
        ...
        if (player.body.chest.sort(BreastRow.Largest)[0].rating >= 3 && player.body.chest.length > 1 && player.body.chest.reduce(BreastRow.AverageLactation, 0) === 0) {
