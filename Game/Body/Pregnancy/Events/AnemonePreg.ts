...
export class AnemonePregEvent implements IPregnancyEvent {
    ...
    public birthScene(player: Character, womb: Womb): void {
        ...
        // [(if pc has 0-9 existing cocks)
            ...
            CView.text(".  Your " + describeVagina(player, firstVagina) + " quivers and pulses as well, adding ");
            if (firstVagina.wetness < VaginaWetness.SLICK) CView.text("a trickle");
            else if (firstVagina.wetness < VaginaWetness.SLAVERING) CView.text("a squirt");
