...
function horns(character: Character) {
    ...
    if (character.torso.neck.head.horns.type === HornType.DRACONIC_X2) {
        DisplayText("  A pair of " + numToCardinalText(character.torso.neck.head.horns.amount) + " inch horns grow from the sides of your head, sweeping backwards and adding to your imposing visage.");
    }
    // Super lizard horns
    ...
    if (character.torso.neck.head.horns.type === HornType.ANTLERS) {
        if (character.torso.neck.head.horns.amount > 0)
            DisplayText("  Two antlers, forking into " + numToCardinalText(character.torso.neck.head.horns.amount) + " points, have sprouted from the top of your head, forming a spiky, regal crown of bone.");
    }
    // BODY PG HERE
