export class Vapula extends NPCAwareContent implements TimeAwareInterface {
	...
	private talkToVapulaForSomeReason(): void {
		DisplayText().clear();
		DisplayText("\"<i>You want to talk?  Well it's quite rare to see " + player.mf("studs", "girls") + " looking for conversation with the likes of me, but go ahead.  What do you want to talk about?</i>\"");
	...
	private feedVapulaACupOfJizz(): void {
		DisplayText().clear();
		DisplayText("You tell Vapula you're going to reward her for being such a good slut; idly stroking [oneCock] into erection, you command her to get on her knees so she can receive her meal.  The demoness eagerly complies and grabs hold of [oneCock].  She then proceeds to lick it, coating the entirety of your junk with her warm, sweet saliva. With an expert precision, she massages all your most sensitive points, pumping pre-cum up your urethra with ruthless efficiency.  As drops of your juices dribble from the tip of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " she voraciously slurps them with a gourmand's smile.");
		DisplayText("\n\nHer ministrations feel good, but she keeps tickling you with her soft hands and pulpy lips, and her flexible tongue gives you the most infuriating tingles of pleasure.  With a groan of impatience, you grab her head and stuff your whole " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " into her mouth, literally stuffing her full of your junk.  She nearly chokes at first but her old reflexes acquired from a whole life devoted to lewdness and licentiousness start kicking in; Vapula gluttonously deepthroats your meat, softly moaning as she fingers herself with one hand and caresses your ");
		if (player.torso.balls.quantity > 0) DisplayText(BallsDescriptor.describeBalls(true, true, player));
		else DisplayText("groin");
		DisplayText(" with the other.  She sucks like a cock-hungry whore, treating your shaft like a straw as she slurps down your juices; her demonic throat feels incredibly hot and her saliva seems to make your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " grow even harder; her lips squeeze your meat in the most exquisite manner, sending you into short spasms of pleasure every time.");
		DisplayText("\n\nShe works on it for what seems like hours.  You look down at your slut; she keeps staring at you with avid yet playful eyes, never breaking eye contact as she relentlessly throatfucks herself; you feel her lips curve a little as the shape of a ravenous smile appears on her face, and her eyes glitter with an insatiable need.  The naughty bitch! She knows she's going to get what she wants, whether you like it or not.  You wish you were able to control yourself, but the covetous succubus never lets go of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", clinging to it like an animal.");
		DisplayText("\n\nYour stamina is rapidly overwhelmed and you decide to reward her effort; you savagely press Vapula's head against your groin, effectively burying all " + num2Text(Math.round(player.torso.cocks.get(0).length)) + " inches of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " in her waiting throat; her tongue keeps teasing your rod for a few seconds, and then you cum, brutally.  ");
		if (player.torso.balls.quantity > 0) DisplayText("Your " + BallsDescriptor.describeBalls(true, true, player) + " churn");
		else DisplayText("Your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " twitches");
		...
		DisplayText(".  At last, the cum-flow spilling through your urethra starts to ebb and your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " stops throbbing; only then does she removes your junk from her mouth with a loud POP.  Your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " appears to be clean of any spooge: your cock-slut did a very good job.  Satisfied, you pat her head with your cock and let her digest her lavish meal.");
		player.orgasm();
		player.stats.cor += .5;
	...
	private vapulaCeraphThreesomePleaseBoth(): void {
		...
		DisplayText(".");
		DisplayText("\n\nYou feel your ");
		if (player.torso.balls.quantity > 0) DisplayText(BallsDescriptor.describeBalls(true, true, player) + " twitch and your " + CockDescriptor.describeCock(player, x) + " pulsate as they unload their");
		else DisplayText("your " + CockDescriptor.describeCock(player, x) + " twitch and pulsate as it unloads its");
		DisplayText(" essence into your omnibus pet, bringing her to orgasm as she blubbers incoherently, \"<i>Oh yessssssssss, just like that... Yeah... my ass... cum... yessssssssss...</i>\"  Her voice goes so high pitched it trails off, letting you hear Vapula's soft moans below.  You quickly pull out your " + CockDescriptor.describeCock(player, x) + ", still orgasming.  You hear Ceraph feebly protest at the sudden lack of cock filling her hole, \"<i>What are you doing? Put it back! Put it back already!</i>\"");
		...
		if (player.torso.balls.quantity > 0) DisplayText(BallsDescriptor.describeBalls(true, true, player) + " hitting her butt-hole with a faint thud");
		...
		DisplayText("  Your thoughts are quickly drowned in waves of pleasure, and soon your only thought is to ram more of your " + CockDescriptor.describeCock(player, x) + " down your sluts' throats, pumping their heads alternatively around your shaft");
		if (player.torso.cocks.count > 1) DisplayText("s");
		if (player.torso.balls.quantity > 0) DisplayText(" as the girls keep fondling your " + BallsDescriptor.describeBalls(true, true, player));
	...
	private vapulaAndCeraphButtfuckTrainYeehaw(): void {
		...
		if (silly()) DisplayText("screaming at the top of your lungs: \"<i>BUTT-FUCK! BUTT-FUCK! BUTT-FUCK!</i>\", soon joined by your horny anal partners");
		...
		//[if balls]
		if (player.torso.balls.quantity > 0) DisplayText("  As you relentlessly smash your groin against Ceraph's plump rump, your " + BallsDescriptor.describeBalls(true, true, player) + " repeatedly hit her quivering vagina; they are soon utterly coated in fem-spunk.");
		DisplayText("  The entirety of your crotch ends up drenched with a various mix of sexual juices, and your " + CockDescriptor.describeCock(player, x) + " makes squelching noises at each thrust in and out of your omnibus' little ass.");
	...
	private vapulaSophieThreeSome(): void {
		...
		if (player.cockArea(x) > 75) DisplayText("you've filled the entirety of her colon");
		...
		DisplayText(ButtDescriptor.describeButt(player) + ".");
		//[if third cock]
	...
	private jojoButtFuckTrain(): void {
		...
		if (player.cockArea(x) > 100) DisplayText("  You can actually feel his internal organs being bumped against your meat as your " + CockDescriptor.describeCock(player, x) + " stretches his colon beyond its normal size and thrusts in and out like a charging mammoth.");
	...
	private vapulaAndIzma(girls: boolean = true): void {
		...
		//Vapula/Izma threesome (as female)
			...
			DisplayText("\n\nNodding in approval, you pull the dildo out of your tigershark's ass with a wet pop and tell Vapula to keep going.  With your rod still in hand, you walk around Vapula and grab hold of her plush, jiggly ass and without warning stuff your still-dripping strap-on in her ass; unsurprisingly, the anal penetration is much easier with the purple succubus.  She jolts a bit but this doesn't reduce the intensity of her ministrations in the least; her tainted saliva appears to have an aphrodisiac effect on Izma's cock, never letting her get completely soft.  You pump your dildo in and out of Vapula's plump rump, enjoying the pink end stroking your dripping " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " while she still sucks off your tigershark lover, letting her balls fill back up with cum.");
	...
	public vapulaForceFeeds(): void {
		...
		//[if no fuck harem is on and PC hasn't fed/fucked Vapula for 5 days that scene will trigger at night]
			...
			if (player.torso.balls.quantity > 0) DisplayText("  Your " + BallsDescriptor.describeBalls(true, true, player) + " churn");
			else DisplayText("  Your " + CockDescriptor.describeCock(player, x) + " twitches");
			...
			if (player.cumQ() >= 1500) DisplayText(" until she looks 6 months pregnant.  At last, the cum-flow spilling through your urethra starts to ebb and your " + CockDescriptor.describeCock(player, x) + " stops throbbing; only then does she removes your junk from her mouth with a loud POP.  Your tool appears to be clean of any spooge: your cock-slut did a very good job.  Satisfied, you pat her head with your cock and let her digest her lavish meal.");
			player.orgasm();
			player.stats.cor += 1;
			Flags.list[FlagEnum.VAPULA_DAYS_SINCE_FED] = 0;
			Flags.list[FlagEnum.VAPULA_TEASE_COUNT] = 0;
