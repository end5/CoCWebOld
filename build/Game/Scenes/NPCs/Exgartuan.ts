	...
	*/
	...
	public timeChange(): boolean {
		...
		if (player.statusAffects.has(StatusAffectType.Exgartuan)) { //Update Exgartuan stuff
			...
			if (player.statusAffects.get(StatusAffectType.Exgartuan).value1 === 1 && (player.torso.cocks.count <= 0 || player.torso.cocks.get(0).area < 100)) { //If too small dick, remove him
				...
				DisplayText("\n<b>Black milk dribbles from your " + Game.BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + ".  It immediately dissipates into the air, leaving you feeling alone.  It looks like you became too small for Exgartuan!\n</b>");
				player.statusAffects.remove("Exgartuan");
				needNext = true;
			}
			else {
					...
					if (player.statusAffects.get(StatusAffectType.Exgartuan).value1 === 2 && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 12) { //Chick stuff
						if (model.time.hours % 9 === 0) { //Only once every 9 hours or so.
							if (randInt(3) === 0) { //lactation messing with!
								...
								DisplayText("\n<b>");
								if (randInt(2) === 0)
									DisplayText("You feel warm and tingly, good all over.  Wait a second, your hands are playing with your " + Game.BreastDescriptor.describeBreastRow(player.torso.chest.get(0)) + ".  You yank your hands away, but it only makes Exgartuan laugh with demonic pleasure!");
								else {
									DisplayText("Your hands knead and caress your " + Game.BreastDescriptor.describeBreastRow(player.torso.chest.get(0)) + ", eagerly touching every inch of soft flesh.  You gasp when you realize what you're doing and pull them away");
	...
	private drinkFountainEndowment(): void {
		...
		if (randInt(3) === 0 && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 1) {
			DisplayText("\n\nYour " + BreastDescriptor.describeAllBreasts(player) + " balloon, each growing about four bra-sizes larger... they feel so... jiggly and sensitive.  Even your nipples seem to grow with them!  Your " + player.inventory.equipment.armor.displayName + " feels tighter than ever!");
		...
		if (randInt(3) === 0 && player.torso.cocks.count > 0) {
			DisplayText("\n\nYour " + CockDescriptor.describeMultiCockShort(player) + " feels tighter inside your " + player.inventory.equipment.armor.displayName + ", even when flaccid.  You shudder and realize you've probably gained more than a few inches in total length, and who knows how your thickness has changed.");
		...
		if (randInt(4) === 0 && player.torso.vaginas.count > 0) {
			DisplayText("\n\nYour " + VaginaDescriptor.describeClit(player) + " plumps up, visibly parting your lips even when you aren't turned on.  It probably ");
	...
	private exgartuanInfestDick(): void {
		...
		if (player.torso.cocks.get(0).type != CockType.DEMON) {
			...
			DisplayText("\n\nYou look down, and find your demonic cock pointing directly at you, and shaking with... indignation?  The ground hits your " + ButtDescriptor.describeButt(player) + " hard as you fall backwards, too surprised to maintain your footing.  Is your dick talking to you?");
		...
		//(+Demon dick possession – not demondicked)
			...
			DisplayText("\n\nYour " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " puffs up, growing even larger as it absorbs the fountain's essence.  It dribbles pre-cum and twitches about as if sniffing the air, feeling very warm and sensitive.");
			DisplayText("\n\nA voice suddenly splits the air, demanding, \"<i>Satiate me mortal, or I'll make you find someone who will!</i>\"");
			DisplayText("\n\nWhat the hell was that?  You look around, but cannot find the source of the voice.  It speaks again, \"<i>Down here.  What are you, deaf!?</i>\"");
			DisplayText("\n\nYou look down, and find your demonic cock pointing directly at you, and shaking with... indignation?  The ground hits your " + ButtDescriptor.describeButt(player) + " hard as you fall backwards, too surprised to maintain your footing.  Is your dick talking to you?");
			DisplayText("\n\n\"<i>Yes I am.  You should consider yourself lucky – you're now the host of the great demon Exgartuan, and you'd best please me every few hours, or I'll make sure your body finds someone to relieve my building pressure.  But I think you'll do fine.  Come now, I can see a wonderful camp in your mind that we can paint white,</i>\" it suggests.");
	...
	private exgartuanInfestTits(): void {
		...
		DisplayText("\n\nThe ground hits your " + ButtDescriptor.describeButt(player) + " hard as you fall backwards, too surprised to maintain your footing.  Are your breasts really talking to you?");
		DisplayText("\n\n\"<i>Yes I am,</i>\" mutters Exgartuan, spurting a trickle of milk from your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + "s for emphasis, \"<i>and you had better take me back to that lovely camp I can see in your memories and give me a nice long massage.</i>\"");
		DisplayText("\n\nWell now... this was certainly unexpected.  Perhaps there's a way to be rid of this thing?");
	...
	public exgartuanMasturbation(): void {
		...
		if (player.statusAffects.get(StatusAffectType.Exgartuan).value1 === 1) {
			...
			if (player.stats.cor >= 30 && player.stats.cor < 60) DisplayText("You happily remove your " + player.inventory.equipment.armor.displayName + ", eager to pleasure yourself.  Your possessed " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " throbbs happily.\n\n");
			...
			if (player.stats.cor < 33) {
				...
				DisplayText("You sigh, squeezing out thick globs of pre-cum as you fondle the nodule-ringed crown of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", twitching happily against your hands.  Oh gods it feels too good to be true.  Happy coos of pleasure escape your mouth as you try to endure your lewd actions.  You struggle against your possessed loins as they guide your fingertips over the most sensitive places, temporarily stealing away control of your limbs.");
			...
			//Medium corruption characters
				...
				if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 5) DisplayText("shove the bloated demon between your " + BreastDescriptor.describeAllBreasts(player) + ", surrounding it in a home of jiggling flesh.  The wet demon-pre soaks into the skin of your tits, lubricating the passage as you mash them around, vigorously tit-fucking the source of your unholy desires.");
				...
				DisplayText("Your orgasm spreads through the demon like a wave.  You briefly wonder if it's the demon causing this to feel so good, or your own corrupt desires.  The thoughts are scattered by the feeling of a fat gob of spooge splattering over your " + HeadDescriptor.describeFace(player) + ".  You blink your eyes clear in time to see the fat, pulsating tip of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " hovering in front of your face.  Your urethra spreads wide, launching another spurt and splattering it into your " + HeadDescriptor.describeHair(player) + ".\n\n");
			...
			//high corruption characters
				...
				DisplayText("He's so right too.  You could keep making him bigger until it's hard to move and he drags on the ground, but you're sure his demon-magics would entice plenty of corrupted sluts, and haze the minds of any uncorrupted person enough to bring them under the spell of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ".  Gosh it makes you hot!  Just thinking about having a harem of goblin girls massaging your mighty member while another grinds her eager gash against your tip... you NEED to make this happen.  Perhaps you could breed an army of the little sluts, to bear you around on a litter and squeeze out cum anytime you have the barest hint of lust.\n\n");
				DisplayText("Your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " seems quite taken with the idea, spurting hot globules of pre-cum until it's glazed itself in a coating of slick wetness.  Both of your hands rub along the surface, soaking themselves in the copious demon-goo, working it entirely of their own volition.  You manage to control your hips at least, and put them to work thrusting up, increasing the stimulation further, and spurring your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " to drizzle even more of its corrupt fluid.  It feels too good to try to resist, and there really isn't any reason to, is there?\n\n");
				...
				if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 5) DisplayText("your hands smashing your " + BreastDescriptor.describeAllBreasts(player) + " around the " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " and vigorously titfucking it.");
				else DisplayText("your hands repositioning your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " so it lays against your chest.");
		...
		//TITURBATION
			...
			//(ALT 1st PG)
				...
				DisplayText("You shrug off your top eagerly, ready to cooperate with the demon inside your " + BreastDescriptor.describeAllBreasts(player) + " and enjoy a relaxing tit-massage.  You slide the offending material to the side and marvel at the wondrously large orbs on your chest.  Truly any place that can give you such wonderful endowments can't be evil.  You lean back, enjoying the warmth in the air as it flows over every extra-sensitive inch of your mounds, more than ready to get started.\n\n");
			}
			DisplayText("Both hands rise unbidden and begin to caress your " + BreastDescriptor.describeBreastRow(player.torso.chest.get(0)) + ".  They slide over every sensitive inch of ");
			...
			DisplayText("At last, fingers wrap themselves about your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + "s, squeezing them gently and forcing happy gasps from your mouth.  ");
	...
	public exgartuanBored(): void {
		...
		if (player.statusAffects.get(StatusAffectType.Exgartuan).value1 === 1 && player.torso.cocks.get(0).area >= 100) {
			...
			if (select === 0) {
			...
			else if (select === 4) DisplayText("You feel your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " slowly expand and contract, air escaping its urethra and tickling your nether regions.  Did... did your dick just sigh?  Your demon is growing impatient.");
				...
				DisplayText("Your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " has started rubbing against your thighs as a familiar voice chimes in, \"<i>I've got a hot, steaming batch of cum ");
				if (player.torso.balls.quantity > 0) DisplayText("boiling in your " + BallsDescriptor.describeBalls(true, true, player) + ".");
			...
			switch (select) {
				case 0:
					DisplayText("You feel an ominous jiggling in your " + BreastDescriptor.describeBreastRow(player.torso.chest.get(0)) + " as the demon inside you stirs back to wakefulness.  The sound of a yawn being muffled by your " + player.inventory.equipment.armor.displayName + " confirms it.");
					break;
				case 1:
					DisplayText("After a few hours of peace, Exgartuan begins to stir.  You can feel a desire to be touched building within your " + BreastDescriptor.describeAllBreasts(player) + ".  What a needy demon.");
					break;
				case 2:
					DisplayText("Exgartuan wakes, making your " + BreastDescriptor.describeAllBreasts(player) + " wobble pleasantly.  The need to have them groped and fondled slowly grows with the demoness's wakefulness.  She barks out, \"<i>Oi, bitch!  Touch your titties!</i>\"\n\nYou sigh.");
					break;
				case 3:
			...
			//(+Lust in combat) for girls
				...
				switch (select) {
					...
					case 2:
						...
						break;
					case 3:
						DisplayText("You really want to end this fight and bend " + monster.a + monster.short + " over... Why is this taking so long, your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " does NOT want to wait!");
						if (player.stats.cor < 50) DisplayText("  You snap back to reality and curse yourself for falling to the demon's desires.");
						break;
					case 4:
						DisplayText("This fight has gone on long enough; all you really want is to shove your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " into the " + monster.a + monster.short + "'s... Wait, what? Your possessed pecker is getting the better of you.");
						break;
					case 5:
						DisplayText("For a moment, you find it hard to concentrate, visions of impaling the " + monster.a + monster.short + " with your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " etching their way into your mind.  Curse this horny demon!");
						break;
					case 6:
						...
						break;
					case 7:
						DisplayText("Maybe it would be best to just lay down your arms and strip off your " + player.inventory.equipment.armor.displayName + ". Just give yourself up to the " + monster.a + monster.short + " and let fate decide. Surely, " + monster.pronoun1 + " would return the gesture by embracing your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " and... Augh!  The demon is again toying with your judgment.");
						break;
					case 8:
						DisplayText("Tight loving assholes and gaping vaginas, slick long red tongues and breasts like fine china.  Succubi and the hot sexings they bring, these are a few of your favorite things!  ...Wait, what?  Was that musing your own?  And why are you absently stroking your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + "?");
						break;
					case 9:
			...
			//(Taunts Male Foes -enemy lust)
				...
				switch (select) {
					...
					case 5:
						...
						break;
					case 6:
						DisplayText("Your " + player.inventory.equipment.armor.displayName + " unexpectedly opens itself of its own accord, prompting your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " to extend into the open air, \"<i>All the chemicals, potions, and morsels in existence can't help that infinitesimal tool you possess.  The orifices of this world deserve better!  All you are accomplishing this day is their disappointment in delaying the inevitable.</i>\"");
						break;
					case 7:
						DisplayText("All of a sudden, your " + player.inventory.equipment.armor.displayName + " splays open, your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " quickly slithering up and around your back, stopping uncomfortably close to your face while pointing at your opponent, \"<i>Look at this pitiful cretin, " + player.short + ".  We could be out spreading hot dickings to the wanting cunts of the land.  But, no!  This " + monster.a + monster.short + " desires eradication.  Well, champion...oblige him.</i>\"");
						break;
				}
	...
	public exgartuanArmorShift(): void {
		...
		if (player.armor === armors.BEEARMR) {
			...
			DisplayText("The green gel-plate protecting your groin thins and presses tightly against you, molding around your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " in an incredibly lewd way.");
			player.modArmorName = "crotch-hugging glistening gel-armor plates";
			changed = true;
		}
		else if (player.armor === armors.LEATHRA) {
			DisplayText("Your leather armor shifts, pressing tightly against your upper " + LegDescriptor.describeLegs(player) + " and molding itself around your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " to prominently display it.");
			player.modArmorName = "crotch-hugging leather armor segments";
			changed = true;
		}
		else if (player.armor === armors.INDECST) {
			DisplayText("The chainmail bikini of your indecent steel armor rearranges and bends its interlocking rings to best shape itself around your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", leaving very little else to the imagination.");
			player.modArmorName = "crotch-hugging practically indecent steel armor";
			changed = true;
		}
		else if (player.armor === armors.R_BDYST) {
			DisplayText("The thin, transparent material of your red bodysuit begins to firmly press against your groin, perfectly shaping to your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " and every last one of its nubs and nodules.");
			player.modArmorName = "crotch-hugging red, high-society bodysuit";
			changed = true;
		}
		else if (player.armor === armors.SSARMOR) {
			DisplayText("The fine silk that makes up your armor suddenly undoes itself around your crotch, exposing your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " to the open air. The thin strands in the air begin to re-weave themselves around your enormous member, forming a prominent new addition to your protection.");
			player.modArmorName = "crotch-hugging spider-silk armor";
			changed = true;
		}
		else if (player.armor === armors.S_SWMWR) {
			DisplayText("The miniscule piece of swimwear that doubles as a tent to your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " begins to grow and encapsulate it, molding itself perfectly to your manhood.");
			player.modArmorName = "crotch-hugging slutty swimwear";
			changed = true;
		}
		else if (player.armor === armors.FULLCHN) {
			DisplayText("You\'re taken by surprise as the binds of your chainmail begin to flatten and rearrange themselves, doing their best to match the curves of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " and make its presence known.");
			player.modArmorName = "crotch-hugging full-body chainmail";
			changed = true;
		}
		else if (player.armor === armors.CHBIKNI) {
			DisplayText("Your chainmail bikini rearranges and bends its interlocking rings to best shape itself around your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", leaving very little else to the imagination.");
			player.modArmorName = "crotch-hugging revealing chainmail bikini";
			changed = true;
		}
		else if (player.armor === armors.FULLPLT) {
			DisplayText("You begin to clench your fists as your steel platemail heats up around your " + LegDescriptor.describeLegs(player) + " and crotch. Slowly it begins to press itself against your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " and match its every feature.");
			player.modArmorName = "crotch-hugging full platemail";
			changed = true;
		}
		else if (player.armor === armors.SCALEML) {
			DisplayText("The steel scales that make up your armor begin to flap wildly around your crotch. They bend and shift as they attempt to match the profile of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ".");
			player.modArmorName = "crotch-hugging scale-mail armor";
			changed = true;
		}
		else if (player.armor === armors.LTHRROB) {
			DisplayText("Your leather armor shifts, pressing tightly against your upper " + LegDescriptor.describeLegs(player) + " and molding itself around your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " to prominently display it through your robes.");
			player.modArmorName = "crotch-hugging black leather armor surrounded by voluminous robes";
			changed = true;
		}
		else if (player.armor === armors.RBBRCLT) {
			DisplayText("You begin to feel your rubber outfit compressing itself against your upper " + LegDescriptor.describeLegs(player) + " and " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", eliminating any pockets of air or wrinkles that may have existed before.");
			player.modArmorName = "crotch-hugging rubber fetish clothes";
			changed = true;
		}
		else if (player.armor === armors.ADVCLTH) {
			DisplayText("The layer beneath your tunic begins to compress against your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", highlighting every curve and nodule while lifting your package to be clearly visible beneath your outer layers.");
			player.modArmorName = "crotch-hugging green adventurer's clothes";
			changed = true;
		}
		else if (player.armor === armors.OVERALL) {
			DisplayText("The denim of your overalls begins to press tightly against your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", molding itself around your member and its every facet.");
			player.modArmorName = "crotch-hugging white shirt and overalls";
			changed = true;
		}
		else if (player.armor === armors.C_CLOTH) {
			DisplayText("Your clothing shifts, tightening up about your crotch until every curve and nodule of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " is visible through the fabric.");
	...
	public exgartuanLactationAdjustment(): void {
		...
		if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 1) {
			...
			//(Stops) 
				...
				DisplayText("Your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + "s tighten up.  What's that demon up to?  Realization dawns on you when you realize your " + BreastDescriptor.describeAllBreasts(player) + " no longer feel so 'full'.  Your lactation has stopped!");
		...
		//(Not lactating)
			...
			//(START)
			if (randInt(2) === 0) {
				DisplayText("Your chest feels cold.  You touch your " + player.inventory.equipment.armor.displayName + " experimentally and discover a few drops of milk have leaked from your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + "s!  The demon has made you start lactating!");
				player.boostLactation(player.torso.chest.count);
			}
			//Nipple stuff
			else {
	...
	public exgartuanBeeRape(): void {
		...
		DisplayText("\"<i>You sure know your knots!   Speaking of knots, I got an idea...</i>\" exclaims the demon inside your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ".  You wonder at what the demon could have in mind this time as you finish spreading the bee-bitch's legs in preparation.  The scent wafting out from her genitals is unreal, making you simultaneously hungry and incredibly horny.  You pull up the ginormous flanged tip with both hands and rub it over her honey-slicked crotch, mixing your demonic pre-seed with her honey as you smear it all over her downy inner thighs.\n\n");
		...
		DisplayText("You pull out with a satisfied grunt, enjoying the wet 'schlick' sound your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " makes as it pulls free of the bee-girl's once-tight hole.  Where once there was a honey-coated slit now resides a gaping monster, drooling a gooey mixture of slime and your tainted demonic seed.  Well, maybe her queen will have an easier time packing her full of eggs.\n\n");
	...
	private exgartuanSleepSurprise(): void {
		...
		if (player.stats.cor <= 20 && player.perks.has(PerkType.BulgeArmor)) {
			...
			if (player.stats.cor < 33) {
				...
				if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2) DisplayText(" and bouncing your " + BreastDescriptor.describeAllBreasts(player) + " around it");
				DisplayText(".  Moans of pleasure vibrate the " + player.cockHead() + " lodged in your throat while bubbles of wetness begin to slide down the lower half of your esophagus into your gullet.\n\n");
			}
			else {
				...
				if (Flags.list[FlagEnum.TIMES_AUTOFELLATIOED_EXGARTUAN] < 5) {
					...
					if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2) DisplayText("you squeeze your tits around your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " with your biceps");
					else DisplayText("you squeeze and caress your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)));
				...
				//(MED+COR, DONE A LOT:
					...
					DisplayText("You grab hold of the swollen flesh with both hands and start to stroke it, reveling at the feel of your palms sliding along the veiny, pre-cum slicked flesh.  You gurgle happily, your moans of pleasure vibrating the " + player.cockHead() + " in your esophagus.  The swollen cock-tip dumps globules of pre-cum down your stretched throat, directly into your hungry gullet.  With both eyes crossed and your hands absorbed in fondling your member, you quickly forget your irritation at being woken up in such a way.  You get to cum SO HARD when you're fucking yourself like this!");
					if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2) {
						DisplayText("  Your biceps squeeze around your " + BreastDescriptor.describeAllBreasts(player) + ", mashing them into your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " and adding to the pleasure.");
						if (player.torso.chest.hasFuckableNipples() && player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 1) DisplayText("  Milk and lubricants ");
						else if (player.torso.chest.hasFuckableNipples()) DisplayText("  Lubricants ");
						else if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 1) DisplayText("  Milk ");
						if (player.torso.chest.hasFuckableNipples() || player.torso.chest.hasFuckableNipples()) {
							DisplayText(" from your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + " puddle in the bouncing cleavage and turn your chest into a slip-n'-slide for dicks.");
						}
					}
					DisplayText("\n\n");
				}
			...
			if (player.gender === Gender.HERM) {
				...
				//(Herms + Balls)
					...
					DisplayText(" skin of your outer lips is engorged and ready, but with your hands so focused on your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", there's little for your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " to feel.  The closest it gets to pleasure is the sensation of your " + BallsDescriptor.describeSack(player) + " slapping against it with the steady rhythm of your masturbation.  If only you had taken care of yourself earlier!  You might have had enough control over your rebellious tool to let a hand attend to your OTHER needs.\n\n");
				}
			}
			...
			DisplayText(" with obscene, liquid sloshing.  Your " + LowerBodyDescriptor.describeHips(player) + " and " + ButtDescriptor.describeButt(player) + " rise off the ground with your back as your muscles lock");
			if (player.torso.balls.quantity > 0) DisplayText(" and your " + BallsDescriptor.describeBalls(true, true, player) + " pull tight against your groin from the effort");
			DisplayText(".");
			temp = player.cumQ();
			if (temp > 500) {
				...
				if (temp > 2500) DisplayText("  Back-pressure blasts your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " free from your imprisoning oral cavity along with a fountain of seed while you weakly try to cough out enough spooge to breathe.");
			}
			DisplayText("\n\n");
	...
	private exgartuanBulgeTortureII(): void {
		...
		DisplayText("  A little more pushing, pulling, knocking, groping, and stroking confirms what you feared: your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " has been cordoned off from you");
		...
		else if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2) DisplayText("  The " + BreastDescriptor.describeAllBreasts(player) + " lined down your chest should serve a proper conquest.  Your palms set off to work at once, manhandling as much titflesh as they can satisfy at once.  They dart from one row to the next, inside to out, anything to relieve your libido.  You don't appear to be settling down any, however, so you draw your focus on your top row of " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + "s.");
		else DisplayText("  With little else to turn to, your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + "s are your only guiding light to hopefully bringing you the peace you crave.");
		//[if nipplecunts]
		if (player.torso.chest.hasFuckableNipples()) DisplayText("  You waste little time in pushing four fingers into each gaping teat, anxious for any sort of pleasure.  You tickle and tease, push and pull, anything you can think to release you from this torment.");
		else DisplayText("  A little tapping, tickling, and teasing is about all the effort you can seem to muster.");
		DisplayText("  Too preoccupied with whatever's going on down below, you can't seem to focus enough to get the job done right.  Your peer down your body at your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", still flaccid and on lockdown.  The most you can respond is with another deep sigh, retreating your attention to the canvas up above you.  You figure the best course of action from here is to just try and make it until morning when you can find some help.  Confessing shame to some third party seems a much better alternative to going any more rounds with this nonsense.  Your fists clench and you take some deep breaths, prepared to ride out the remainder of the night with your sanity intact.\n\n");
	...
	private exgartuanBulgeTortureIII(): void {
		...
		DisplayText("  Through the blurry, trembling mess that is your vision, you can make out the mighty titan, its figure silhouetted against the blood-red moon behind it.  After what feels like forever, your majestic manhood slowly descends from its great heights towards you, pulsating in time with your frenzied heartbeat.  As if it were a god descending from heaven to bless a mortal, the " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " stops right above your gaze, its urethra slowly widening, continuing to spill pre-cum all over your plagued form.\n\n");
	...
	private exgartuanBulgeTortureIV(): void {
		...
		DisplayText(".  As you rise to your feet, a few streams of cum run down your " + LegDescriptor.describeLegs(player) + ".");
		//[If any followers]
		...
		DisplayText("  You pay one more glance to Exgartuan, the " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " comfortably resting away in your outfit.\n\n");
	...
	private boobgartuanSurprise3(): void {
		...
		if (player.gender > 0) {
			DisplayText("  Your " + LowerBodyDescriptor.describeHips(player) + " begin grinding together, moist with ");
			//[if cock >0]
		...
		DisplayText("  You finally bear down and grasp onto... nothing?  <b>Your nipples are nowhere to be found!</b>  The passion is beginning to flatline as you confusedly grope around your " + Desc.Breast.describeChest(character) + ".  \"<i>What's wrong, champion?</i>\" Exgartuan asks knowingly, \"<i>Is this too hard for you?</i>\" The tips of your fingers comb around your areolas until you discover an inward bend in your " + SkinDescriptor.skin(character) + " where your nipples should be.  Apparently, Exgartuan isn't through having fun with you; the blasted seductress managed to suck your teats inward!  Before you can fathom just how she pulled it off without your knowledge, your humble howitzers start to tingle.  You feel an anxious quiver work its way up your spine before a familiar need begins to course through your being: the need to be milked!");
		//[if corruption <50]
		...
		if (player.gender > 0) {
			...
			DisplayText(" already coating your " + LowerBodyDescriptor.describeHips(player) + ".");
		}
		//else
		else DisplayText(".");
	...
	public exgartuanNagaStoleMyMasturbation(): void {
		...
		//[else]
			...
			DisplayText("  Your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " makes good on its end of the bargain by pumping out its own never-ending supply of lubricant, making your master massage all the more slick and smooth along its surface.  You lay off for a moment to admire your work, your demonic dong throbbing happily now that its full of blood and as tall and thick as a tree.  The corrupt warmth and comfort flowing through your body pleases you to no end.\n\n");
		...
		DisplayText(" load working its way up your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + "... and stop.  You sit up in an involuntary attempt to release the pressure, only to be met by your coiled-up shaft bending to greet your face, its urethra spread wide open...\n\n");
