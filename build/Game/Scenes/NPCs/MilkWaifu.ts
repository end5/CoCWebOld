export class MilkWaifu extends NPCAwareContent {
	...
	private nameZeMilkBath(): void {
		...
		if (mainView.nameBox.text === "Bath Slut") DisplayText("Fuck it, Bath Slut it is.  At least she won't get confused.");
		else if (mainView.nameBox.text === "Biscuit") DisplayText("Fuck it, you may as well butter her buns!");
		//Variable: " + Flags.list[FlagEnum.MILK_NAME] + "
		//Having Named the Girl (Didn't name her Bath Slut)
		else {
			DisplayText("You crouch down beside the newly-named girl, brushing a few stray strands of her dark hair from her cheeks.  \"<i>No more Bath Slut.  You're " + Flags.list[FlagEnum.MILK_NAME] + " now.  " + Flags.list[FlagEnum.MILK_NAME] + "'s your name.</i>\"");
	...
	public milkyMenu(): void {
		...
		if (Flags.list[FlagEnum.FOLLOWER_AT_FARM_BATH_GIRL] === 0) {
			...
			if (Flags.list[FlagEnum.MILK_SIZE] === 0) DisplayText("lying face-down on her massive rack, her plump little ass sticking up in the air for all to see.  Seeing you approach, she brightens up and shifts her titanic bust around to face you, all bright-eyes and smiles.  \"<i>Milk time?</i>\"");
			else if (Flags.list[FlagEnum.MILK_SIZE] === 1) DisplayText("arms crossed under her hefty, milky bust to support their still-sizable weight.  She smiles as you approach, able to stand up under her own power to give you a hug, milky tits pressed tight against you.  \"<i>I-is it milk time?</i>\" she asks, cupping her tits for you.");
			else DisplayText("her long, tanned legs dangling into the tub.  She jumps up when you approach, and though still a bit unsteady with such easy movements, she's quick to leap into your arms, nuzzling into your neck and pressing her firm, perky DD-cup breasts against you, little trickles of milk staining your chest through the little shirt she's wearing.  \"<i>[name],</i>\" she purrs happily.");
		}
		else {
			if (Flags.list[FlagEnum.MILK_SIZE] === 0) {
				DisplayText("[bathgirlname] shakes vaguely out of her boob daze as you pick your way over to her.");

				DisplayText("\n\n“<i>Bath time?</i>”");
			}
			else {
				DisplayText("[bathgirlname] smiles at you as you pick your way over to her.");
		...
		if (Flags.list[FlagEnum.MILK_SIZE] > 0 && player.stats.lust >= 33 && player.torso.cocks.count > 0) MainScreen.addButton(4, "Titfuck", titFuckDatMilkSlut);
	...
	private sendToFarm(): void {
		...
		DisplayText("“<i>Ok,</i>” says [bathgirlname], cautiously testing the idea out. “<i>You’ll come and visit sometimes, right?</i>” Of course. Mollified, the former sand witch slave gets to her feet and cautiously picks her way towards the lake. She won’t be much use protection-wise but she’ll give your milk production a boost.");
	...
	private backToCamp(): void {
		...
		//TODO
		DisplayText("“<i>I want you to head on back to camp,</i>” you tell her. “<i>You’ll be more useful to me there.</i>” [bathgirlName]’s brow crinkles but she seems to accept your instruction.");

		DisplayText("“<i>As you wish.</i>” She wipes her hands before walking slowly down and out of the farm’s gate.");
	...
	private milkWriteFuAppearance(): void {
		...
		if (kGAMECLASS.farm.farmCorruption.hasTattoo("milky")) {
			...
			if (kGAMECLASS.farm.farmCorruption.milkyFullTribalTats()) {
				...
				DisplayText("She is covered from head to tail in tattooed butterflies, as if the pretty insects are attracted to her chocolate skin. When she moves she does it with an extra bounce and flick of the head, admiring how she looks as she goes.");
			}
			else {
				if (kGAMECLASS.farm.farmCorruption.numTattoos("milky") > 1) DisplayText("She has the following tattoos emblazoned across her body:\n");
				else DisplayText("She has ");

				if (Flags.list[FlagEnum.MILKY_TATTOO_COLLARBONE] != 0) DisplayText(Flags.list[FlagEnum.MILKY_TATTOO_COLLARBONE] + "\n");
				if (Flags.list[FlagEnum.MILKY_TATTOO_SHOULDERS] != 0) DisplayText(Flags.list[FlagEnum.MILKY_TATTOO_SHOULDERS] + "\n");
				if (Flags.list[FlagEnum.MILKY_TATTOO_LOWERBACK] != 0) DisplayText(Flags.list[FlagEnum.MILKY_TATTOO_LOWERBACK] + "\n");
				if (Flags.list[FlagEnum.MILKY_TATTOO_BUTT] != 0) DisplayText(Flags.list[FlagEnum.MILKY_TATTOO_BUTT] + "\n");
			}
		}
	...
	private superReductoUsage(): void {
		...
		if (Flags.list[FlagEnum.MILK_SIZE] === 0) {
			...
			if (isabellaFollowerScene.isabellaFollower()) {
				DisplayText(", enough to give Isabella");
				if (player.statusAffects.has(StatusAffectType.CampMarble)) DisplayText(" and Marble");
				DisplayText(" pause");
			}
			else if (player.statusAffects.has(StatusAffectType.CampMarble)) DisplayText(", enough to give Marble pause");
	...
	private milkBathTime(): void {
		...
		if (Flags.list[FlagEnum.MILK_SIZE] === 0) DisplayText("starting to shift her breasts' bulk toward a comfortable spot for a nap");
		else DisplayText("struggling to her feet, legs shaky with the afterglow of a might boobgasm");
		DisplayText(".");
		if (player.stats.lust >= 33) DisplayText("  You could probably masturbate in the tub if you wanted to, or maybe pull the dusky milk-maid in for company.");
		...
		if (player.statusAffects.has(StatusAffectType.PureCampJojo)) count++;
		...
		if (count >= 3) {
			DisplayText("  Then again, if you're getting a bath, maybe the rest of your friends might like to join you?");
	...
	private pullInZeMilkGirl(): void {
		...
		if (player.torso.vaginas.count > 0) {
			...
			if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 1) {
				DisplayText("  She cups your breasts, having to reach so far around both your rack and hers that she's straining her arms to rub your [nipples], but she does so valiantly, stroking them with her incredibly deft fingers.");
	...
	private fuckTheMilkWaifu(): void {
		...
		if (player.torso.cocks.get(0).area < 20) DisplayText("pushing your few inches into her until your hips join, her nice and loose cunt easily taking your length");
		else if (player.torso.cocks.get(0).area < 50) DisplayText("hilting her in one long stroke");
		else DisplayText("your cock gaining as much entrance as your massive member can, the excess dickmeat embraced in cream between you");
		DisplayText(".  With your prick buried in her, " + Flags.list[FlagEnum.MILK_NAME] + " hooks her legs around your [hips] and starts to gently rock her hips, letting you take the initiative.  Smiling at the meek girl, you sink your fingers into milk-yielding titflesh and start to move your hips, thrusting into her with measured ease, letting milk flood into her channel and coat your dick to lubricate each and every stroke.");
	...
	private beARugMunchingMilkDyke(): void {
		...
		DisplayText(".  Her hand slips up your body, brushing your vulva and [clit] before cupping your other breast, delicate fingers wrapping around your [nipple].  With deft, practiced motion, she works your teats between her fingers, working your breast like she might her own");
		if (player.lactationQ() >= 200) DisplayText(", milking you with skill beyond anything you've ever experienced before; and why not, when her entire existence revolved around that self-same skill?");
		else DisplayText(".");
		...
		if (player.lactationQ() >= 200) DisplayText("milky ");
		...
		if (player.lactationQ() >= 200) DisplayText(", spraying milk all over yourself and the milkmaid who caused your explosive pleasure");
		DisplayText(".  You run your fingers through the girl's hair, urging her sexual skills on as your chest heaves and quavers, riding out the massive boobgasm as femcum spurts from your cunt and into the milky pool below.");
	...
	private drinkAndFapturbate(): void {
		...
		if (Flags.list[FlagEnum.MILK_SIZE] === 0) DisplayText("a dinner plate");
		else DisplayText("an open palm");
		DisplayText(" but far more exciting.  After taking a few swallows of her body's milky treat, you reach down to your ");
		if (player.torso.vaginas.count > 0) DisplayText("[vagina] and idly stroke your puffy, lust-engorged vulva");
		else {
			DisplayText("[cock] and idly stroke the turgid mass");
		}
		DisplayText(", inadvertently drawing a lewd moan from yourself.  The fat nipple stuffed in your mouth does an adequate job of muffling your pleasurable vocalizations");
		if (player.torso.cocks.count > 0) DisplayText(", but it does little to hide the swelling of [eachCock] - you have the milk for that");
		DisplayText(".");
		...
		if (player.gender === Gender.HERM) {
			DisplayText("Your [cock] throbs painfully in your hand, so hot and hard that you're sure you must have begun to leak precum, but any fluid is swiftly washed away by the ever-present milk.  ");
			if (player.torso.cocks.count > 1) DisplayText("You make sure to fondle each of your members equally, caressing, squeezing, and stroking to the tempo of your swelling passion.  ");
		...
		//(DA DUDES)
			...
			DisplayText("[OneCock] throbs painfully in your hand, so hot and hard that you're sure you must have begun to leak precum, but any fluid is swiftly washed away by the ever-present milk.");
			if (player.torso.cocks.count > 1) DisplayText("  You make sure to fondle each of your members equally, caressing, squeezing, and stroking to the tempo of your swelling passion.");
			if (player.torso.balls.quantity > 0) DisplayText("  With your offhand, you cradle your [sack], hefting your [balls] as you feel your desire churning to new levels.");
		}
		//(TOGETHER)
		...
		if (player.torso.cocks.count > 0) {
			DisplayText("  [EachCock] release its own gooey load, sputtering and spurting to add to the pearly deluge.");
			if (player.cumQ() >= 1500) DisplayText("  With every torrent of seed you release, you can see it lift partway out of the tub, propelled by your incredible virility towards the nearest female specimen.");
			if (player.cumQ() >= 4000) DisplayText("  Soon, the tub's fluid contents break out of their confines to stain your companion's dusky flesh white, an alabaster glaze that would entice you to further feats of debauchery were it not for the pleased contentment your maleness now radiates.");
		}
		if (player.torso.vaginas.count > 0) DisplayText("  Meanwhile, your juiced-up cunny feels like it's doing backflips inside you, contorting and squeezing as it gushes with fluid, feminine joy.  One brush on your [clit] knocks your [legs] out from underneath you, but thankfully, you float out the rest of your orgasm.");
		...
		if (Flags.list[FlagEnum.MILK_SIZE] === 0) DisplayText("drags her gigantic tits over the puddly, milk-slicked floor");
		else DisplayText("stumbles away, tenderly cupping her bright-red teats");
	...
	private titFuckDatMilkSlut(): void {
		...
		else DisplayText("sink your fingers into her udder-like teats");
