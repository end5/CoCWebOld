export class ShouldraFollower extends NPCAwareContent {
	...
	//Initial Question
	internal function initialShouldersRecruitment() {
		DisplayText().clear();
		DisplaySprite(67);
	...
	private noFollowerShoulders() {
		DisplayText().clear();
		DisplaySprite(67);
		DisplayText("Somewhat bluntly, you inform her of your lack of interest regarding having a ghostly tagalong.  Though she looks a bit disappointed, she nods, scuffing her shoes against the ground and huffing a little sigh.  \"<i>At least I'll see you later, hey?</i>\"");
	...
	private recruitShouldraForCampSexyTimes() {
		DisplayText().clear();
		DisplaySprite(67);
	...
	//Non-Corrupt Vala:
	private shouldraAndValaGetBigger() {
		DisplayText().clear();
		DisplaySprite(67);
		if (Flags.list[FlagEnum.SHOULDRA_MET_VALA] < 1) Flags.list[FlagEnum.SHOULDRA_MET_VALA] = 1;
		DisplayText("You decide to make your way down to the Wet Bitch to introduce Vala to her.  A few minutes into your stay, you see the fairy serving a few tables while holding a tray filled with drinks and empty glasses.  Pointing her out, Shouldra gives a delighted squeal.");
	...
	private pureValaAndShouldraMacroMax() {
		DisplaySprite(67);
		DisplayText().clear();
		player.slimeFeed();
		DisplayText("Before she can do anything, you shout out a rather unusual suggestion, though to you it sounds quite exciting.  Vala or Shouldra, whoever she is, smiles wickedly and nods, lifting you up towards the glittering gash between her legs.");
		...
		if (player.gender > 0) {
			DisplayText("  Inspired by this, your own body has long since responded");
			if (player.torso.cocks.count > 0) DisplayText(", becoming firmly erect.  You happily piston your hips to grind your " + Desc.Cock.describeMultiCockShort(player) + " along the pink-glossed walls, oozing your fluids in delight.");
			else DisplayText(", growing wet, though in this slick fluid, your contribution is a mere drop in the ocean.");
		}
		DisplayText("  The warm fleshy dream is abruptly ended as Shouldra yanks you out by your [feet].  \"<i>I'm gonna put you the whole way in.  Do me a solid and get me off a few times, will ya?</i>\"");
		...
		DisplayText(" against the slick walls.  You push your way towards the pinkish slit in the distance begin to slide your [legs] over the inner edges of her lips, pressing your arms up above you for stability.");
		...
		if (player.torso.cocks.count > 0) {
			DisplayText("  [EachCock] is quickly brought to orgasm by the exotic situation, and you fire off your own load into the deluge of sex-juice.");
			if (player.cumQ() >= 1000) DisplayText("  Somehow, you pour out almost as much spunk as Vala's orgasm, turning the inside of her honeypot into an off-white mess.");
		}
		else if (player.torso.vaginas.count > 0) {
			DisplayText("  Your [vagina] can scarcely resist the allure of the sweet, female flesh pressing against it, and with an explosion of hot bliss, you climax");
			if (player.torso.vaginas.get(0).wetness >= 4) DisplayText(", squirting with joy");
			DisplayText(".");
		}
		...
		if (player.gender === Gender.NONE) DisplayText(" somehow");
		DisplayText(", but she vanishes with a high-pitched giggle.");
	...
	//corrupt vala
	public shouldraMeetsCorruptVala() {
		DisplaySprite(67);
		DisplayText().clear();
		...
		if (player.torso.cocks.count > 0) DisplayText("  Your cock slips past the folds, pressed up against your stomach as the underside is massaged with the gentle tightness within.");
		DisplayText("  Once you're up to your chest, Vala stops, instead grasping your shoulders between her thumb and forefingers as she begins to gently slide you in and out of her.");
	...
	public sandWitchGetsGhostly() {
		...
		if (player.torso.cocks.count > 0 && (player.torso.vaginas.count <= 0 || randInt(2) === 0)) {
			...
			if (!player.torso.chest.find(BreastRow.FuckableNipples)) DisplayText("\"<i>You know, maybe I should give you a couple of those chest-mounted treats.  They're pretty fun,</i>\" Shouldra teases, tweaking your [nipples].  ");
			DisplayText("With a very telling exasperated sigh, you snatch up your [armor] and march back to camp.");
		}
	...
	private shouldraFappinTimes() {
		DisplaySprite(67);
		if (player.torso.cocks.count > 0 && player.statusAffects.has(StatusAffectType.Infested)) {
			shouldraAndWormsYoureGonnaHaveABadTime();
		}
		else if (player.gender === Gender.NONE) genderlessShouldraMasturbation();
		else if (player.torso.cocks.count > 0 && (player.torso.vaginas.count <= 0 || randInt(2) === 0)) maleMasturbationProper();
		else {
			if (randInt(2) === 0) girlyMasturbationWithGhosts();
	...
	private genderlessShouldraMasturbation() {
		DisplayText().clear();
		DisplaySprite(67);
		DisplayText("Shouldra runs your fingers along your ");
		//[if (femininity < 50) 
		if (player.femininity < 50) DisplayText("masculine ");
		else if (player.femininity > 50) DisplayText("feminine ");
		DisplayText("form while processing your request, your eyes glowing yellow as she mulls over her choices.");
		...
		//if event occurrence > 0
			...
			//[if (corruption < 50) 
			if (player.stats.cor < 50) DisplayText("There isn't an answer or outcome to that question that you're in favor of, honestly.");
			else DisplayText("You sure as hell wouldn't have bothered asking if you weren't.");
		}
		DisplayText("  The ghost girl promptly discards your [armor] before hurrying your naked body along into the wilderness away from camp.");
		...
		if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] === 0 && Flags.list[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] === 0) DisplayText("Within lies a rather rustic yet well-built chest.");
		else if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 && Flags.list[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] === 0) DisplayText("The moment the light reveals what lies within, your [butthole] instinctively clenches shut.  \"<i>You didn't think I'd leave my most treasured possessions behind for some goblin to run into, did you?</i>\" It's the ghost girl's chest of dildos, as well-kept as the last time you laid worried eyes on it.");
		else DisplayText("Inside rests the ghost's familiar treasure trove of dildos and other toys you have yet to discover.");
		...
		if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0) DisplayText("  Just what else did she pack in there since last you peered at its contents back at the ruins?");
		...
		if (player.torso.butt.looseness < 3) {
			DisplayText("Your nerves screech out in pain as your rear revenant stretches out her puckered little \"jaw\" to something more fitting for what she has in store.  A few seconds later and the sensations begin to only register as pleasure.");
			//[if (corruption < 50)
			if (player.stats.cor < 50) DisplayText("..  This only worries you further.");
			DisplayText("  It doesn't take her long to <b>prepare your anus for any insidious ploy she's thought up.</b>");
		}
		//if looseness >= 3
		else {
			DisplayText("\"<i>Looks as though you already get plenty of action back here, chief,</i>\" she jests.  Nevertheless, the rear revenant massages her \"jaw\" even more lax than you already are.  Even stranger, each gyration provides more and more pleasurable feedback");
			if (player.stats.cor < 50) DisplayText(", which only serves to confuse you");
			DisplayText(".");
		}
		...
		if (Flags.list[FlagEnum.ZETSUKO_MET] > 0) DisplayText("\"<i>Your run-in with a particular kitsune inspired me,</i>\" Shouldra teases");
		else DisplayText("Shouldra is too caught up in her machination to give you any hint as to what is going on, leaving you confused");
		...
		if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || Flags.list[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] > 0) DisplayText("You recognize it instantly as she pulls it free from her chest: the nearly foot-long floppy horse cock she impaled you on before.");
		//{if SHOULDRA_GENDERLESS_FUCK_COUNT = 0}
		else if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] === 0) DisplayText("She holds her catch up clear for you to see; to your horror, you have clutched onto an enormous horse cock replica.  All 13 inches flop restlessly in your grip, seemingly taunting you with the foreboding future that lays before you.");
		DisplayText("  Shouldra smacks the dildo of doom onto a nearby boulder as she wearily rises you to your [feet].  The inferior, forgotten playthings slide out uselessly from the ghoul's gasping craw as she slowly clambers over top the wiggling horsey wang.");
		...
		//{looseness is now 5} {player receives ectoplasm}
		...
		inventory.takeItem(consumables.ECTOPLS, Scenes.camp.returnToCampUseOneHour);
	}
	...
	private nongenderlessAnalShouldraMasturbation() {
		DisplayText().clear();
		DisplaySprite(67);
		DisplayText("Shouldra runs your fingers along your ");
		//[if (femininity < 50) 
		...
		DisplayText("form while processing your request, your eyes glowing yellow as she mulls over your request.  \"<i>So, let me get this straight.  Despite[if (hasCock = true)  this dandy dick] [if (isHerm = true)  and][if (hasVagina = true)  this serviceable snatch] you want me to work with this?</i>\" the spirit asks, smacking your hand against your [butt].  A smug nod reaffirms your desire.  ");
		//(same paragraph follows)
		//if event occurrence = 0
		if (Flags.list[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] === 0) {
			...
			DisplayText("\"<i>Up for another round then, Champ?</i>\" she asks, your body striking a familiar boxing stance from bouts long since fought.  \"<i>If you think I'll be pulling any punches then you don't have a ghost of a chance.</i>\"  Only if you had control of your body could you respond with even half the appropriate groan.");
		}
		DisplayText("  The ghost girl promptly discards your [armor] before hurrying your naked body along into the wilderness away from camp.");
		...
		//if SHOULDRA_GENDERLESS_FUCK_COUNT > 0 OR event occurrence > 0
		if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || Flags.list[FlagEnum.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] > 0) DisplayText("You recognize it instantly as she pulls it free from her chest: the nearly 2-foot floppy horse cock she impaled you on before.");
		//if SHOULDRA_GENDERLESS_FUCK_COUNT = 0
		else DisplayText("She holds her catch up clear for you to see; to your horror, you have clutched onto an enormous horse cock replica.  All 13 inches flop restlessly in your grip, seemingly taunting you with the foreboding future that lays before you.");
		DisplayText("  Shouldra smacks the dildo of doom onto a nearby boulder as she wearily rises you to your [feet].  The inferior, forgotten playthings slide out uselessly from the ghoul's gasping craw as she slowly clambers over top the wiggling horsey wang.");
		DisplayText("\n\nToo tired to object or plead, you meekly brace for impact as the wraith lines up its target.  Your immense cheeks part way.  Your rectum gapes wide.  Your anal tongue presses firmly against your taint.  \"<i>Down the hatch!</i>\"");
		DisplayText("\n\nThe forest shudders with your scream.  The phantom easily downs six inches on the first go.  She wraps her tongue around the shaft, forcing more and more into you.  You swear your pelvis is bulging.  Muffled moans blended with laughter emit from your belabored behind.  You dare not guess how deep Shoudra drills the cock into you before she climaxes - hard.  Your voluptuous ass quavers with ferocity alongside her gagged yelping, throwing you from your perch atop the rock back down to the earth.  Her fun over, you begin to feel the magic subsiding.  Having no desire to learn of the consequences of leaving a massive wang lodged in your shrinking anus, you muster what little strength you can to plumb it free.  With the experience settling down, you can't help but laugh - closer to a timid chuckle, as its the best your exhausted body can offer.  You think you hear Shouldra saying something to you, but you're too worn out to listen to her boasting.");
	...
	private maleMasturbationProper() {
		DisplayText().clear();
		DisplaySprite(67);
		DisplayText("You are nearly overwhelmed by Shouldra's delight when you allow her to focus on your [cock].  She instantly takes the lead, steering you behind a good-sized boulder and flinging your [armor] piece-by-piece as you go.");
		...
		//if cocksize <200: 
		if (player.torso.cocks.get(0).area < 200) DisplayText("  You stagger a bit at the unaccustomed heft of your junk altering your center of gravity, but you know that will wear off soon enough.");
		DisplayText("  She reaches into her tunic and tosses you a full bottle of ectoplasm, giving you a wink before incorporealizing once more and floating into you.  All's well that ends well.");
		player.orgasm();
		player.stats.sens += -2;
		DisplayText("\n\n");
		shouldraSleeping((10 + randInt(26)), true);
	...
	//Female Masturbation
	private girlyMasturbationWithGhosts() {
		DisplayText().clear();
		DisplaySprite(67);
		...
		DisplayText("\"<i>Tsk!  Saying those kinds of things aloud!  Still... if you want to let it out so badly, I might let you if you beg a little more...</i>\"");
	...
	private shouldraTongueLicksPCs() {
		DisplayText().clear();
		DisplaySprite(67);
		DisplayText("The ghost girl promptly discards your [armor] before hurrying your naked body along into the wilderness away from camp.");
		...
		//if SHOULDRA_GENDERLESS_FUCK_COUNT > 0 || event occurrence > 0
		if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || Flags.list[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] === 0) DisplayText("  The consistency of your forgetfulness while traveling this path leads you to believe that your ethereal inhabitant would rather you not come back here on your own.");
		...
		if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] === 0 && Flags.list[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] === 0) DisplayText("  Within lies a rather rustic yet well-built chest.");
		else if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 && Flags.list[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] === 0) DisplayText("  \"<i>You didn't think I'd leave my most treasured possessions behind for some goblin to run into, did you?</i>\"  It's the ghost girl's chest of dildos, as well-kept as the last time you laid worried eyes on it.");
		else DisplayText("  Inside rests the ghost's familiar treasure trove of dildos and other toys you have yet to discover.");
		DisplayText("  She has you pull the container out from its hiding place, removing the clear tarp she used to protect it from the elements.  Despite the fact that you have yet to control your body once through this entire ordeal, an intrinsic urge to open the container has you bending over to pry into the contents within.");
		...
		if (Flags.list[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] === 0) {
			...
			if (player.torso.head.face.tongueType > TongueType.HUMAN) DisplayText("some of ");
			DisplayText("your tongue out, examining it.  Already, you think you've got a pretty good idea of where she's going to take this.");
		...
		if (player.perks.has(PerkType.Flexibility)) {
			...
			if (Flags.list[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] === 0) {
				...
				DisplayText("\n\nShe sits you down and lets her dildos fall to the ground, forgotten for the moment.  \"<i>How's this one go?</i>\" she asks, leaning down in an awkward attempt to get your face in your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ".  You start to explain the proper 'technique' to what she's trying to do, but before you can even finish your thought she's stuck a leg up in the air and has your mouth to your own snatch.  You doubt you'll ever get used to having her inside your thoughts.");
			}
		...
		//if demon tongue
		else if (player.torso.head.face.tongueType === TongueType.DEMONIC) {
		...
		//else
			...
			if (Flags.list[FlagEnum.SHOULDRA_TONGUE_LICKS_TIMES] === 0) DisplayText("\n\nAlmost shocked enough to wrest control away from your companion, you begin to babble dissent and try to reel your lengthening tongue back in to no avail. Shouldra stops for a moment, quite frustrated by the interruption. Sitting straight up, she consoles you, \"<i>Relax, Champ, it'll stop feeling weird in a minute and you know it's all temporary.</i>\"\n\nGranted a moment to adjust, you let yourself settle down and hand her the reins once more.  She happily goes back to ");
			else DisplayText("\n\n  She happily continues ");
			DisplayText("sliding fingers along the wet appendage, humming a little tune over your tongue until more than a foot of it droops from your mouth.  \"<i>Now we're getting somewhere,</i>\" you hear her thinking as she works her magic.  There doesn't seem to be a limit to how much she can pull out of your mouth, and soon she has more than three feet of pure tongue coiled in your lap.  Furrowing your brow in concentration, she tries to move your tongue about, but can't manage to wiggle more than the tip. \"<i>Too thin,</i>\" she mutters, \"<i>much too weak. Doesn't feel right...</i>\"");
	...
	public shouldraDream1() {
		...
		if (Flags.list[FlagEnum.SHOULDRA_PLOT_LEVEL] === 0) {
			...
			DisplayText("Few things more relaxing than heading home after a day of work.  But as you approach the dilapidated hut, some strange noises from within catch you off guard.  If it's that fucking mouse again...");
	...
	private shouldraYappin() {
		...
		if (Flags.list[FlagEnum.SHOULDRA_TALK_NEEDED] === 1) {
			...
			if (Flags.list[FlagEnum.SHOULDRA_PLOT_LEVEL] === 1) {
				...
				DisplayText("You decide not to beat around the bush and ask about the strange dream you had the other night.  The spirit puts on a surprised reaction, your shoulder returning to normal as she casually walks outside of your body.");
				...
				if (player.perks.has(PerkType.Incorporeality)) DisplayText("  Shouldra bumps you to the side a little as she makes her next point, \"<i>Even with your hoaky quasi-possession power, you don't really know what it's like to actually possess someone.</i>\" You look at her funny, unaware that you weren't really getting the full effect.  Not that you care!");
				...
				if (player.perks.has(PerkType.Incorporeality)) DisplayText(" - and I mean <b>really</b> possess someone -");
			...
			//Talk 2 - First Transformation Discussion
				...
				if (player.gender > 0) {
					DisplayText("  Your [if (hasCock = true) [cock]][if (isHerm = true)  and ][if (hasVagina = true) [vagina]] start to swell up; looking up at the ghost, you realize that she hasn't uttered a single word!");
				}
				else DisplayText("  A fleshy, penis-shaped bulge begins to swell out of your loins; looking up at the ghost, you realized she hasn't uttered a single word!");
				...
				if (player.gender === Gender.NONE) DisplayText("dick");
				DisplayText(" [if (isHerm = false) continues its][if (isHerm = true) continue their] march of magnification while your fingers vigorously explore their new frontiers.");
				...
				if (player.gender === Gender.NONE) DisplayText("burgeoning dickflesh");
				DisplayText(".  It takes some gumption for you to work through the arousal and contribute to the conversation, but you manage to point out how you're pretty sure there are other people in Mareth with the same sort of power, to some degree at least.");
				...
				if (player.gender === Gender.NONE) DisplayText("Your now erect dick is leaking out pre at an alarming rate, which your possessed hands greedly lap up and distribute all along your length.  ");
			...
			//Talk 3 - "Recent" Goblin Possession Discussion
				...
				DisplayText("You stare at Shouldra.  \"<i>Oh, <b>what</b>?</i>\"  You stare at Shouldra.  \"<i>She fucking pissed on my floor!</i>\"  You stare at Shouldra.  The ghost shakes her head, sinking it back inside your skin. The ghost girl quickly flies out of your chest.  \"<i>You don't fuck with my stuff!</i>\"");
				...
				if (player.torso.cocks.count > 0) DisplayText("</i>\"  Shouldra eyes your [cock] gingerly, acting as if she can stare right through your outfit at it.  \"<i>");
				else DisplayText("  ");
				DisplayText("The half-millenium or so of scarcity has really shaped my interests.  I'm at the point now where I just lose it whenever I get these ghostly mitts on a slab of meat.  And the big ones?  ");
				if (player.torso.cocks.sort(Cock.LargestCockArea)[0].area >= 200) DisplayText("Ones like yours?  ");
				...
				DisplayText("\"<i>Oh,</i>\" she says, stepping out of you and sitting herself across from you.  \"<i>You saw that.</i>\"");
		...
		//Generic Oni Discussion!
			...
			DisplayText("On a whim, you decide to have a chat with your spectral companion, who obligingly pops free of your torso to spare you the awkward sensation of conversing with one of your own body parts.");
	...
	public shouldraFollowerScreen() {
		DisplaySprite(67);
		DisplayText().clear();
		if (!shouldraSleeping()) DisplayText("You pulse a desire for interaction with Shouldra, summoning her head to form from your shoulder. \"<i>What's on your mind, Champ? I could've found out on my own, but I was... preoccupied.</i>\"");
		//Follower Screen Quick Text - Shouldra asleep
		else {
			DisplayText("You try and contact Shouldra, but the ghost is still sound asleep within you.  Or whatever ghosts consider sleep after an arousing round of debauchery. Either way, you garner no response.  Best to try again later.");
			return { next: playerMenu };
			return;
		}
		let sex;
	...
	private shouldraSexMenu() {
		DisplayText().clear();
		DisplaySprite(67);
		DisplayText("Shouldra stirs from somewhere deep inside you, excitement evident in the swiftness of her reaction.  \"<i>Is it time for some fun, Champ?</i>\" she asks happily, her increased lust affecting you just a bit.  Do you oblige her?");
		let vala;
		...
		MainScreen.simpleChoices(["GhostMasturbate", "Anal Masturb", "Visit Vala", "", "Back"], [shouldraFappinTimes, anal, vala, null, shouldraFollowerScreen]);
	}

	//Talk Options
	private shouldraTalkMenu() {
		DisplaySprite(67);
		...
		else if (Flags.list[FlagEnum.SHOULDRA_PLOT_LEVEL] >= 2) DisplayText("\n\nShouldra needs some time before she can grow or shrink one of your body parts permanently.");
		MainScreen.simpleChoices(["Normal Talk", "Grow Something", "Shrink Something", "", "Back"], [shouldraYappin, blowShitUp, shrinkShit, null, shouldraFollowerScreen]);
	}
	...
	private shouldraGroPlus() {
		DisplayText().clear();
		DisplaySprite(67);
		DisplayText("You decide to take up Shouldra's offer on natural body enhancement.  Before you tell her as much, the eager spectre quickly assumes control of your body.  \"<i>Alright, Champ, now we're in business.  So, what did you have in mind?</i>\"");
		...
		if (player.torso.balls.quantity > 0) balls = groBallsBiggaGHOSTYSTYLE;
		if (player.torso.cocks.count > 0) cock = shouldraCockBloating101;
		if (player.torso.vaginas.count > 0) clit = shouldraGrowsYoClit;
		choices("Balls", balls, "Breasts", breast, "Clit", clit, "Cock", cock, "Nipples", nipples, "Butt", butt, "", null, "", null, "", null, "Back", shouldraTalkMenu);
	}

	//Balls
	private groBallsBiggaGHOSTYSTYLE() {
		Flags.list[FlagEnum.SHOULDRA_MAGIC_COOLDOWN] = 72;
		DisplayText().clear();
		DisplaySprite(67);
		DisplayText("\"<i>Interested in a bigger ballsack, huh?  Not only will it help pronounce your package, but a big ol' swinging sack of nuts feels great when you're really pounding.</i>\" Shouldra grasps your [balls] with both your hands and begins reciting the appropriate spell.");
		//ballsize + 2 + randInt(4) 20% chance
		if (randInt(5) === 0) {
			player.torso.balls.size += 2 + randInt(4);
		...
		//ballsize + 1 + randInt(2) Else
			...
			player.torso.balls.size += 1 + randInt(2);
		...
		//if ballsize > 10
		if (player.torso.balls.size > 10) DisplayText("  You second guess your decision when your swinging cum barrels seem to impede walking.  Shouldra reassures you that the enhanced libido and baby batter supply is worth the wider gait.");
		player.stats.lust += 10;
		return { next: Scenes.camp.returnToCampUseOneHour };
	}

	//Breast
	private shouldraGrowsYoTits() {
		Flags.list[FlagEnum.SHOULDRA_MAGIC_COOLDOWN] = 72;
		DisplayText().clear();
		DisplaySprite(67);
		DisplayText("\"<i>I'll give you a rack that could hypnotize anyone into a faithful tit slave,</i>\" Shouldra boasts, stretching your shoulders in preparation.  Your hands grope your [fullChest] as the spirit slips into the arcane language you've come to grow so familiar with.\n\n");
		//same growtits() as Gro+
		if (player.torso.chest.count === 1) player.growTits((1 + randInt(5)), 1, true, 1);
		else player.growTits(1 + randInt(2), player.torso.chest.count, true, 1);
		player.stats.lust += 10;
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	//Clit
	private shouldraGrowsYoClit() {
		Flags.list[FlagEnum.SHOULDRA_MAGIC_COOLDOWN] = 72;
		DisplayText().clear();
		DisplaySprite(67);
		DisplayText("\"<i>Looking to give your joy buzzer a little jumpstart?</i>\" Shouldra teases, your hands sliding down to your inviting [cunt].  Your fingers tease your [clit] while the ghost girl spouts her arcane tongue from your mouth.  ");
		//clitLength + 1
		player.torso.clit.length += 1;
		DisplayText("She welcomes the new inch with a quick flick.  The sensitive [clit] twitches happily in response, confirming the success of the spell.");
		player.stats.sens += 5;
		player.stats.lust += 15;
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	//Cock
	private shouldraCockBloating101() {
		Flags.list[FlagEnum.SHOULDRA_MAGIC_COOLDOWN] = 72;
		DisplayText().clear();
		DisplaySprite(67);
	...
	//Nipples
	private shouldraGivesYaSomeFukkinTeats() {
		Flags.list[FlagEnum.SHOULDRA_MAGIC_COOLDOWN] = 72;
		DisplayText().clear();
		DisplaySprite(67);
		DisplayText("Shouldra rubs your hands together in anticipation, \"<i>A beautiful blossoming pair of nipples is what you need to really compliment a nice set of tits.</i>\"  Your hands gleefully tweak your [nipples] as the spirit's incantation gets underway.");
	...
	private shouldrasButtBigginator() {
		...
		//this.player.torso.butt.rating = this.player.torso.butt.rating + 1 + randInt(3)); or so
		player.torso.butt.rating += 1 + randInt(3);
	...
	private shouldraReductoMenu() {
		...
		let butt;
		let nipples;
		if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].nipples.length > .25) nipples = shrinkDemNipplzForYoGhost;
		if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 1) breasts = shouldraReductosYourTits;
		if (player.torso.butt.rating >= 2) butt = shrinkDatBootyForYoGhost;
		if (player.torso.balls.quantity > 0 && player.torso.balls.size > 1) balls = shouldraReductosYourBallsUpInsideYa;
		if (player.torso.cocks.count > 0 && player.longestCockLength() > 4) cock = shouldraMakesCocksDisappear;
		if (player.torso.vaginas.count > 0 && player.torso.clit.length > .25) clit = clittyVanishingActShouldra;
		choices("Balls", balls, "Breasts", breasts, "Clit", clit, "Cock", cock, "Nipples", nipples, "Butt", butt, "", null, "", null, "", null, "Back", shouldraTalkMenu);
	}
	//Balls
	private shouldraReductosYourBallsUpInsideYa() {
		Flags.list[FlagEnum.SHOULDRA_MAGIC_COOLDOWN] = 72;
		DisplayText().clear();
		DisplaySprite(67);
		DisplayText("\"<i>Why would you want to shrivel these delicious cumquats?</i>\"  Shouldra's arsenal of puns doesn't affect your request in the slightest.  The ghost girl moves into a quick spell, savoring your testicles before finishing.");
		//this.player.torso.balls.size = this.player.torso.balls.size - (2 + this.rand(4));
		player.torso.balls.size -= (2 + randInt(4));
		if (player.torso.balls.size < 1) player.torso.balls.size = 1;
	...
	//Breast
	private shouldraReductosYourTits() {
		Flags.list[FlagEnum.SHOULDRA_MAGIC_COOLDOWN] = 72;
		DisplayText().clear();
		DisplaySprite(67);
		DisplayText("Your arms clasp themselves around your [fullChest].  \"<i>Not the gals!  What monster would want to deflate their knockers?</i>\"  This monster, evidently.  Shouldra sighs, holding your hands in a weak casting stance as she gets the heinous act over with.");
		player.shrinkTits();
	...
	//Clit
	private clittyVanishingActShouldra() {
		Flags.list[FlagEnum.SHOULDRA_MAGIC_COOLDOWN] = 72;
		DisplayText().clear();
		DisplaySprite(67);
		DisplayText("\"<i>Aw, really, Champ?  The best part of your snatch?</i>\"  A nod of your head reasserts your desire.  The depressed phantom hastily works through the spell, the notion of shrinking your [clit] not sitting well with her.");
		player.torso.clit.length /= 1.7;
		//Set clitlength down to 2 digits in length
		player.torso.clit.length = int(player.torso.clit.length * 100) / 100;
	...
	private shouldraMakesCocksDisappear() {
		...
		if (player.torso.cocks.count === 1) {
			...
			let choice: number = randInt(10);
			if (choice === 0) DisplayText("murderer");
			else if (choice === 1) DisplayText("bastard");
			else if (choice === 2) DisplayText("jerk");
			else if (choice === 3) DisplayText("monster");
			else if (choice === 4) DisplayText("cretin");
			else if (choice === 5) DisplayText("fiend");
			else if (choice === 6) DisplayText("asshole");
			else if (choice === 7) DisplayText("villain");
			else DisplayText("dick");
			DisplayText("!</i>\" it shouts.  The tiny voice fades away as your [cock] goes still, resting at two thirds of its original size.  A single tear rolls down your cheek; you suspect the melodramatic spirit is at fault here.");
			player.torso.cocks.get(0).length *= 2 / 3;
			player.torso.cocks.get(0).thickness *= 2 / 3;
		}
		//MULTI
		else {
			...
			for (let ii: number = 0; ii < player.torso.cocks.count; ii++) {
				player.torso.cocks.get(ii).length *= 2 / 3;
				player.torso.cocks.get(ii).thickness *= 2 / 3;
			}
		}
		player.stats.sens += -2;
		player.stats.lust += -10;
	...
	//Nipples
	private shrinkDemNipplzForYoGhost() {
		Flags.list[FlagEnum.SHOULDRA_MAGIC_COOLDOWN] = 72;
		DisplayText().clear();
		DisplaySprite(67);
		DisplayText("\"<i>Are you kidding me, Champ?  Nipples are the most important-</i>\"  You cut the ghost girl's argument there, not wanting to leave your decision up for debate.  She instead leaves you with a deep sigh of disapproval before quickly casting the appropriate spell.");
		//Shrink
		if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].nipples.length / 2 < .25) {
	...
	//Butt
	private shrinkDatBootyForYoGhost() {
		Flags.list[FlagEnum.SHOULDRA_MAGIC_COOLDOWN] = 72;
		DisplayText().clear();
		DisplaySprite(67);
		...
		if (player.torso.butt.rating >= 15) {
			player.torso.butt.rating -= 3 + int(player.torso.butt.rating / 3);
	...
	private kickFollowerShouldraOut() {
		DisplayText().clear();
		DisplaySprite(67);
		...
		if (Flags.list[FlagEnum.SHOULDRA_PLOT_LEVEL] < 4 || Flags.list[FlagEnum.SHOULDRA_TALK_NEEDED] > 0) {
			DisplayText("\"<i>Ah, you're booting me out, hey?</i>\"  Shouldra asks innocently, tumbling out of your body and corporealizing. \"<i>Wouldn't be the first, and won't be the last.  I do realize I have certain 'needs' that got cloying to many of my former hosts.</i>\"");
		...
		//She Fucking Explodes
			...
			DisplayText("Are you sure you want to ask her to leave after she opened herself up to you?  It's just a hunch, but she might not take this very well.");
			doYesNo(definitelyKickOutCrazyGhosts, shouldraTalkMenu);
		}
	}

	//if yes
	private definitelyKickOutCrazyGhosts() {
		DisplayText().clear();
		DisplaySprite(67);
		DisplayText("\"<i>What.</i>\"");
	...
	public shouldersWarnings(): boolean {
		...
		//7 days (168 hours) since last Shouldra scene
			...
			if (randInt(3) === 0) {
				...
				if (player.gender === Gender.NONE) DisplayText("libido seems to be going haywire");
				DisplayText(".  Shouldra's unfulfilled sexual needs are starting to blend into your own.  Nothing you do will satisfy your urging until she's seen to her own.\n\n<b>Minimum lust raised by 20.</b>\n");
			}
			else if (randInt(2) === 0) {
	...
	public nightTimeShouldraRapesThePC() {
		...
		if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 1) DisplayText(", circling their entire lengths around your [fullChest]");
		DisplayText(".  It's quite amazing how ecstatic the tour feels, each pre-smeared exploration smothering you in sensual vibes.  Just moving the stiff little devils - particularly at your joints - sends shivers up your arm.");
		...
		DisplayText(", softening the ");
		if (player.torso.cocks.count > 0) DisplayText("magically enlarged member");
		else DisplayText("magically gaped snatch");
		...
		if (player.torso.cocks.count > 0) DisplayText("flailing cock");
		else DisplayText("moist vagina");
		DisplayText(".");
		//if cock
		if (player.torso.cocks.count > 0) {
	...
	private edwardShouldraDickHandsPartII() {
		DisplayText().clear();
		DisplaySprite(67);
	...
	public shouldraWakesUpOrPokesPCsForShitsAndGigglesIdunnoHowLongCanIMakeThisFunctionNameQuestionMark() {
		DisplayText("\n");
		DisplaySprite(67);
		let choices: Array = [];
		let select: number;
		...
		//1 PC with cock - 1
		if (player.torso.cocks.count > 0 && player.torso.cocks.sort(Cock.LargestCockArea)[0].area < 200) {
			choices[choices.length] = 1;
			choices[choices.length] = 1;
			choices[choices.length] = 1;
			choices[choices.length] = 1;
		}
		//2 PC with BIG COCK - if (this.player.torso.cocks.sort(Cock.LargestCockArea)[0].area >= 200)
		...
		if (player.statusAffects.get(StatusAffectType.Exgartuan).value1 === 1 && 9999 === 9999) {
		...
		choices[choices.length] = 7;
		//8 (Tel'Adre unlocked)
		if (player.statusAffects.get(StatusAffectType.TelAdre).value1 >= 1) choices[choices.length] = 8;
		//9 (is preggers)
		...
		//13 (is Naga)
		if (player.torso.hips.legs.isNaga()) choices[choices.length] = 13;
		//14 (Amily is a follower)
		if (kGAMECLASS.amilyScene.amilyFollower()) choices[choices.length] = 14;
		//15 (Pure Jojo is a follower)
		if (player.statusAffects.has(StatusAffectType.PureCampJojo)) choices[choices.length] = 15;
		...
		if (player.statusAffects.get(StatusAffectType.Exgartuan).value1 === 2) choices[choices.length] = 19;
		//20 (PC knows any spells)
		if (player.hasSpells()) choices[choices.length] = 20;
		//21 (PC has anemone dick)
		if (player.torso.cocks.filter(Cock.FilterType(CockType.ANEMONE) > 0)).length choices[choices.length] = 21;
		...
		switch (select) {
			...
			case 1:
				subSelect = randInt(5);
				if (subSelect === 0) DisplayText("Your [cock] tingles, and your [armor] suddenly feels much tighter.  A little giggle from Shouldra reveals her intention.  Though adamant about making you as big and virile as possible, Shouldra does realize it <b>is</b> your body, and your meat shrinks at least three inches to its former glory.");
				else if (subSelect === 1) DisplayText("An uncomfortable stirring in your nether regions has you stop to undo your [armor] and figure out what's the matter.  Your [cock] is wagging enthusiastically.  Shouldra's face quickly sprouts from your shoulder, \"<i>Oh look, he's happy to see you!</i>\" You pay her an irritated glance which she returns with a content snicker.  With your cock under control, you redo your [armor] and continue on your way.");
				else if (subSelect === 2) DisplayText("You're distracted as you notice your [cock] snaking its way down your [leg], quickly growing larger.  You make your displeasure obvious, causing it to recoil back to its original size.  Seems as though your ghostly friend is getting restless.");
				else if (subSelect === 3) DisplayText("You come to a halt as you suddenly lose control of your arms, forcing you to watch your hands dart down your [armor] to fiercely grab at your [cock].  Though you let out a growl for her to stop, it takes Shouldra a minute to return your arms to you.  She must be getting restless.");
				else if (subSelect === 4 && player.torso.cocks.filter(Cock.FilterType(CockType.HORSE) === 0) DisplayText("There's a sudden twisting pain in your [cock], forcing you to undo your [armor] and see what's wrong.  To your dismay, a large equine dong is staring back at you, leaking profusely.  \"<i>Just testing out some new transformations, Champ,</i>\" Shouldra snickers.  You yell out again as your dick quickly turns back to normal.")).length;
				else DisplayText("One of your hands begins playing with your [cock], no doubt spurred on by your paranormal passenger.  \"<i>You know, Champ, I could make you some easy gems.  Just find some know-nothing and introduce them to The Amazing Penis, capable of astounding feats like responding to any vocal coital commands!</i>\"  Playing the part of a sideshow attraction is quite the tangent to your typical daily activities.  Probably best not to think about it.");
				break;
			case 2:
				//PC with BIG COCK 
				subSelect = randInt(5);
			...
			case 6:
				subSelect = randInt(6);
				if (subSelect === 0) DisplayText("While minding your business, you realize faint whispering is coming from your [cock].  Shouldra and Exgartuan are scheming something, probably something you wouldn't be a fan of.  Every so often your wang lets out a chuckle.  Maybe getting these two together wasn't in your best interest.");
				else if (subSelect === 1) DisplayText("Your [cock] is wiggling around inside your [armor], making you incredibly uncomfortable.  You go to undo your outfit when the demonic dong plows its way out anyway, scaring you half to death.  Shouldra and Exgartuan just laugh, too content with one another to pay you much mind.  You never expected them to get along THIS well.");
				else if (subSelect === 2) DisplayText("\"<i>So what other sorts of things can you concoct?</i>\" Exgartuan asks, shuffling around in your armor.  You're incredibly confused by the sudden question.\n\n\"<i>Honestly, that put me in the mood to make all of mother nature our bitch,</i>\" you respond... Shouldra responds.  The spirit is gabbing through your mouth to your possessed pecker.  The two prattle on like this for some time.");
				else if (subSelect === 3) DisplayText("Your hands slip inside your [armor], exploring your [cock].  Your fingers caress each little nub and nodule they come across.  \"<i>That's the ticket,</i>\" Exgartuan gleefully responds, getting rigid in the tight confines of your outfit.  Looks like your ghostly partner is out to make everyone happy.");
				else if (subSelect === 4) DisplayText("\"<i>Why don't you help out during battles, babe?</i>\" Exgartuan asks rather bluntly.\n\n\"<i>Hey, I've offered to help out with Champ's more seductive poses like you do...</i>\"\n\nIt isn't like you have much choice in the matter as far as Exgartuan is concerned!  The two continue their discussion, belittling your apparently meager combat performance.  Maybe your possessors could actually help deal some real damage for a change!");
				else DisplayText("Your [armor] has vanished!  You're butt naked!  It's pretty obvious who's at fau-\n\n\"<i>Really, [name], you don't even need it,</i>\" Exgartuan says, surprising you.\n\nShouldra laughs, \"<i>Right, Champ?  Two sexually-charged tyrants dwelling within you and you insist on sneaking around like common rabble?  Why not let it all hang out?</i>\"\n\nYour hands caress your [cock], exciting the demonic member into attention.  You remind the couple that your outfit isn't <b>just</b> for protection, citing many other-  Oh, your [armor] is back.  Guess they don't want to hear you prattle.");
				break;
			//General flavor text
			case 7:
				subSelect = randInt(6);
				if (subSelect === 0) DisplayText("\"<i>Yoooo, Champ, are we gonna fuck something, or am I just gonna stagnate here?</i>\" Shouldra moans, managing to steal control of an arm long enough to give your [butt] a honk.  You sigh.");
				else if (subSelect === 1) DisplayText("Out of nowhere, a colossal yawn bursts forth from your craw.  \"<i>Come on, Champ. I'm dying to try out some new stuff I've cooked up.</i>\" What is this sense of dread consuming your being?");
				else if (subSelect === 2) DisplayText("It's been pretty quiet recently.  No doubt Shouldra is up to something -- she always is.  Honestly, you've forgotten what it's like to not having your body passing for a bloated something or other.");
				else if (subSelect === 3) DisplayText("As you are going about your business, you swear you make out muttering emanating from somewhere on your person. It sounds like the ghost girl is reciting some magic, but you don't feel any different. This only worries you even more.");
				else if (subSelect === 4) DisplayText("\"<i>Do foot fetishes still apply if it's your own foot?</i>\" Shouldra asks.  For a moment you wonder how such a feat would even be possible.  But only for a brief, forgetful moment...");
				else if (subSelect === 5) DisplayText("A familiar voice works its way from the back of your mind, \"<i>Hey, Champ.  You think there's any way to make armpits sexy?  What about knees or noses?</i>\"  This line of questioning could lead to some very troubling consequences...");
				else DisplayText("\"<i>I like what you did with those racks,</i>\" Shouldra says unexpectedly, her voice losing its mischievous edge for a moment.  \"<i>I'm glad you kept them.</i>\"  You both sit in silence for several moments as you consider her mood shift to tenderness, but the moment passes.  Shouldra chuckles softly.  \"<i>Just thought I'd mention.</i>\"");
				break;
			//Special Flavor Texts (misc. prereqs)
			//(Tel'Adre unlocked)
			case 8:
				DisplayText("Somehow, someway, you can feel Shouldra sifting through your memories.  \"<i>I'd love to visit the bakery in that desert town.  It's been at least a century since I tasted a decent sweet of some sort.</i>\"");
				break;
			//Preggo
			case 9:
			...
			//(is Naga)
			case 13:
				DisplayText("Your tail wiggles around on its own suddenly.  Catching on to how it appears to be sneaking to play around with something on your body, you calmly ask its spiritual captor to knock it off.  An otherworldly snicker emanates from it before it goes limp.");
				break;
			//(Amily is a follower)
			case 14:
			...
			case 17:
				DisplayText("\"<i>");
				if (Flags.list[FlagEnum.IZMA_CHILDREN_SHARKGIRLS] + Flags.list[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] === 1) DisplayText("That shark daughter of yours is");
				else DisplayText("Those shark daughters of yours are");
				DisplayText(" pretty cute you know,</i>\"  Shouldra blurts seemingly from nowhere.  Looks as though she's helping herself to your head.  \"<i>I'd love to get to meet them sometime, Champ.  Put some otherworldly fright in 'em so they grow up right!</i>\"  You doubt Izma would be too onboard with this visit.");
				break;
	...
	private shouldraAndWormsYoureGonnaHaveABadTime() {
		DisplayText().clear();
		DisplaySprite(67);
		...
		if (player.torso.cocks.get(0).length < 48) DisplayText(" and cuts right to the chase, ballooning your [cock] into a rigid, wide, four-foot tall slab of meat");
		DisplayText(".  Unable to contain her delight, she hastily hugs your enormous dick firmly against your body and squeezes vigorously, rubbing your face against your [cockHead].  A warm stream of pre oozes out and onto your possessed face.");
		...
		if (model.time.hours <= 12) DisplayText("morning ");
		else if (model.time.hours <= 5) DisplayText("afternoon ");
		else DisplayText("evening ");
		DisplayText("air.  Your hands clue the eidolon in to further activity, introducing her to your urethra all the way down to your [balls], every square wriggling inch chock-full of the little buggers, exploring their extended living quarters.");
		...
		MainScreen.simpleChoices(["Keep Shouldra", "Keep Worms", "", "", ""], [kickOutWormiesForYourGhostPalPAL, kickOutShouldra4YoWormyBuddies, null, null, null]);
	}
	//Keep Worms
	private kickOutShouldra4YoWormyBuddies() {
		DisplayText().clear();
		DisplaySprite(67);
		...
		if (player.torso.cocks.get(0).length < 48) DisplayText("  With that little ordeal dealt with, you now are left to decide how best to manage the time until your cock returns to its normal size.");
		DisplayText("  Oddly enough, you hope the little buggers appreciate you sticking up for them.");
		//Follower Shouldra leaves
		Flags.list[FlagEnum.SHOULDRA_FOLLOWER_STATE] = 0;
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	//Keep Shouldra
	private kickOutWormiesForYourGhostPalPAL() {
		DisplayText().clear();
		DisplaySprite(67);
	...
	public shouldraAndExgartumonFightGottaCatchEmAll() {
		//40% chance to occur when player has both Shouldra and Cock Exgartuan and is going to sleep
		//Unlocks Argue 2
		DisplaySprite(67);
		DisplayText("Your bedroll looks mighty comfortable after a hard day's ");
		if (player.stats.cor < 50) DisplayText("work");
		else DisplayText("fucking");
	...
	public exgartumonAndShouldraFightPartII() {
	...
	public exgartumonAndShouldraFightPartIII() {
	...
	public exgartuMonAndShouldraShowdown() {
		...
		if (User.settings.silly()) DisplayText("  It's a man's right to choose what's done with his body.  Not anyone else's!");
		DisplayText("  You shout loud enough to get the two to at least shut up and notice you're there.  It's time to end this little battle once and for all, starting with your first - and hopefully last - involvement.");
		//[Keep Exgartuan]	[Keep Shouldra]	[Keep Both!]
		MainScreen.simpleChoices(["Keep Exgartuan", "Keep Shouldra", "Keep Both", "", ""], [keepExgartuanInsteadOfShouldra, keepShouldraAndKickOutExgartuan, keepAllTheGhosts, null, null]);
	}
	//Keep Exgartuan
	private keepExgartuanInsteadOfShouldra() {
		DisplayText().clear();
		DisplaySprite(67);
	...
	//Keep Shouldra
	private keepShouldraAndKickOutExgartuan() {
		DisplayText().clear();
		DisplaySprite(67);
		DisplayText("You decide to jump to Shouldra's side on this one, explaining to your demonic cock your desire to go your separate ways.  Exgartuan heartily laughs at you, your [cock] wiggling in the air with each mighty heave.");
		...
		//PC has human dick
		player.torso.cocks.get(0).type = CockType.HUMAN;
		//Exgartuan departs
	...
	public keepShouldraPartIIExgartumonsUndeatH() {
		DisplayText().clear();
		DisplaySprite(67);
		...
		DisplayText("While slipping into your bedroll to enjoy a peaceful night's sleep, a chill runs down your spine.  You look around to-");
	...
	private keepShouldraPartIIIExgartumonsUndeatH() {
		DisplayText().clear();
		DisplaySprite(67);
		DisplayText("\"<i>Boo!</i>\"");
	...
	private keepAllTheGhosts() {
		...
		if (player.cumQ() >= 1000) DisplayText(", trails of your cum pouring out from them");
		DisplayText(".");
	...
	//Intro
	public shouldraBakeryIntro() {
		DisplayText().clear();
		DisplaySprite(67);
		DisplayText("You set off towards the bakery, and it's not long before the smell of freshly-baked pastries and sugary candies wafts through the air.  Something stirs in your mind as a familiar ghostly presence makes herself known; seems you're not the only one who noticed the scent of cinnamon.");
		...
		//first time: 
		if (Flags.list[FlagEnum.SHOULDRA_BAKERY_TIMES] === 0) DisplayText("You smirk, wondering how exactly a ghost manages to work up a hunger.  Your skepticism is met by an intense stabbing pain in your stomach, forcing you to buckle slightly from the cramping.  It lasts only a moment before it vanishes, however.  \"<i>You feel that, smartass?</i>\" The ghost girl sneers, \"<i>That's what decades of fasting will do to you.");
		else DisplayText("You point out that she's already had something at the bakery.  \"<i>Yeah, I did.  And now I'm hungry again.  Deal with it.");
		DisplayText("  Now, how about a snack?  Or two?</i>\" Her enthusiasm is clear...  and slightly worrying.");
		MainScreen.simpleChoices(["Continue", "Bail Out", "", "", ""], [feedShouldraACake, bailOut, null, null, null]);
	}
	...
	private bailOut() {
		DisplayText().clear();
		DisplaySprite(67);
		DisplayText("You quickly turn around and head back the way you came.  There's a moan of disappointment from Shouldra, before she settles down and goes back to doing whatever she usually does.  You try not to think about it as you leave the city and return to camp.");
		return { next: Scenes.camp.returnToCampUseOneHour };
	}

	//nter
	private feedShouldraACake() {
		DisplayText().clear();
		DisplaySprite(67);
		DisplayText("You decide that there's no use getting cold feet now; you wanted to go the bakery anyway, no harm in letting Shouldra have some fun while you're there.  ");
		//after first time: 
		if (Flags.list[FlagEnum.SHOULDRA_BAKERY_TIMES] > 0) DisplayText("Well, nothing <i>permanent</i>, anyway.  ");
		DisplayText("You press on, soon arriving at the ramp leading down into the bakery's interior.  It's as busy as ever, with patrons milling about, chatting between bites of pastry.");
		...
		//Maddie
		MainScreen.simpleChoices(["Maddie", "", "", "", ""], [shouldraAndMaddieSittingInATree, null, null, null, null]);
	}

	private shouldraAndMaddieSittingInATree() {
		DisplayText().clear();
		DisplaySprite(67);
		...
		if (player.tone >= 60) DisplayText("well-toned");
		else if (player.thickness < 50) DisplayText("trim");
		else DisplayText("curvaceous");
		DisplayText(" figure...");
		//[Go along with it] [Resist]
		MainScreen.simpleChoices(["Go Along", "Resist", "", "", ""], [goAlongWIthShouldrasEatingSpree, resistBeingAFatass, null, null, null]);
	}
	//Go along with it
	private goAlongWIthShouldrasEatingSpree() {
		DisplayText().clear();
		DisplaySprite(67);
		//if repeat scene: 
		if (Flags.list[FlagEnum.SHOULDRA_BAKERY_TIMES] > 0) DisplayText("\"Man, you're not sure you want that kind of impact on your figure again...  \"");
		...
		if (Flags.list[FlagEnum.PC_FETISH] > 0) DisplayText(", though not completely unwelcome");
		DisplayText(".  You blush furiously and back into the corner of the (now rather cold) booth");
		if (Flags.list[FlagEnum.PC_FETISH] > 0) {
			DisplayText(" as your heart races and blood rushes down towards your ");
			if (player.torso.cocks.count > 0) {
				DisplayText("drowsy member");
				if (player.torso.vaginas.count > 0) DisplayText(" and ");
			}
			if (player.torso.vaginas.count > 0) DisplayText("trembling nether-lips");
			if (player.gender === Gender.NONE) DisplayText("groin");
		}
		DisplayText(", before you realize that nobody seems to notice your predicament.  Shouldra reads your thoughts and responds, \"<i>Thought you might appreciate some privacy");
		if (Flags.list[FlagEnum.PC_FETISH] > 0) DisplayText(", even with that kink of yours");
		DisplayText(", Champ.  Here's something extra anyway, just in case.</i>\" One ghostly incantation later, and ");
		if (player.femininity <= 50) DisplayText("a buttoned shirt and pair of belted jeans");
		else DisplayText("a tight-fitting tank top and skirt");
		DisplayText(" adorn your person.");

		if (player.torso.cocks.count > 0) {
			DisplayText("\n\nYou run a hand down your chest, examining the new material.  Silky smooth...  As your hand slides across your stomach, you press in slightly.  You've only made a small dent in the food before you, but there's already a noticeable bulge beneath your belt, and a few slivers of " + Desc.Skin.skinFurScales(player) + " are visible between the buttons.  You rub the modest mound, resulting in a subdued gurgling and a content burp; you've gotten full pretty quickly.  Maybe you should just leave the rest of the food and- \"<i>Not so fast, bucko!</i>\" Shouldra fumes, causing you to jump in your seat.  \"<i>You might be full, but <b>I've</b> barely started!</i>\" Your gut churns violently as the ghost girl's inhuman appetite infects your own.  Oh dear...");
			...
			if (User.settings.silly()) DisplayText("  This is a <i>fucking good eclair</i>.");
			...
			if (player.cumQ() < 200) DisplayText("Modest spurts of semen shoot out into Shouldra's awaiting mouth.  She withdraws, swirls it around her tongue, then swallows your load in one gulp.  \"<i>Hmm...  needs cinnamon,</i>\" she jokes, licking her lips.");
			//if (cumMedium = true)
			else if (player.cumQ() <= 400) DisplayText("Several thick spurts of semen shoot out into Shouldra's awaiting mouth.  She withdraws, swilling the sizeable load around her mouth before swallowing it in a few gulps.  \"<i>Not bad...  could use a dash of ginger though,</i>\" she jokes, licking her lips.");
			//if (cumHigh = true) 
			else if (player.cumQ() <= 1000) DisplayText("Your groin roils as a thick jet of pearly cum floods Shouldra's mouth, followed by another, then another.  The ghost girl barely has time to swallow each load before the next comes, but she manages not to spill a drop of your virility.  \"<i>Mmm, pretty tasty!</i>\"  She says after wiping her mouth in satisfaction.  \"<i>Hints of pineapple, coconut...  delicious.</i>\"");
			//if (cumVeryHigh = true)
			else if (player.cumQ() <= 5000) DisplayText("Your loins tremble and quiver as an immense load of cloying cum bulges along your shaft and explodes in Shouldra's mouth.  A further wave of bulges assault the ghost girl's throat, forcing her to swallow all of your thick virile spunk.  Eventually the wave subsides, leaving a satisfied and somewhat bloated spook in its wake.  \"<i>Soo gooood...</i>\" she drawls, her goofy grin smattered with white.");
			else DisplayText("A cock-distending wad of potent jizm works its way through your throbbing shaft, erupting from your urethra down the hapless ghost's gullet.  As bulge after obscene bulge forces itself inside Shouldra's stomach, her spongy abdomen balloons outwards, soon reaching the size of an adult human.  Streams of excess cum flow down her straining cheeks, and by the time you've stopped cumming the ghostly glutton is lying in a large sticky puddle of your spunk.  She tries to speak, but all that emerge are a sloppy belch and a few flecks of cum, and she resorts instead to hysterical giggling, lost in a cum-addled haze.");
