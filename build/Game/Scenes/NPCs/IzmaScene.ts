...
//Required prequisites for encounter: Encounter a normal shark 
  ...
  //creates Izmacounter and sets value to 1 if it doesn't exist)
		...
		public timeChangeLarge():boolean {
			if (checkedIzmaSophie++ === 0 && sophieBimbo.bimboSophie() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] === 0 && izmaFollower() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_IZMA] === 0 && Flags.list[FlagEnum.IZMA_NO_COCK] === 0 && ((Flags.list[FlagEnum.TIMES_SOPHIE_AND_IZMA_FUCKED] === 0 && randInt(10) === 0) || Flags.list[FlagEnum.TOLD_SOPHIE_TO_IZMA] === 1)) {
				Flags.list[FlagEnum.TOLD_SOPHIE_TO_IZMA] = 0;
				sophieBimbo.sophieAndIzmaPlay();
				return true;
			}
			if (checkedIzmaLatexy++ === 0 && izmaFollower() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_IZMA] === 0 && Flags.list[FlagEnum.IZMA_NO_COCK] === 0 && latexGirl.latexGooFollower() && Flags.list[FlagEnum.FOLLOWER_AT_FARM_LATEXY] === 0 && Flags.list[FlagEnum.TIMES_IZMA_DOMMED_LATEXY] === 0 && (debug || randInt(10) === 0)) {
				izmaDomsLatexy();
				return true;
			}
			return false;
...
public meetIzmaAtLake():void {
	...
	if(Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00233] === 1) {
		...
		MainScreen.simpleChoices(["Borrow", "Fight", "Talk", "Sex", "Leave"], [tradeWithFuckingSharkBitches, fightSharkCunt, talkToASharkCoochie, chooseYourIzmaWeapon, leaveSumSharkPussyOnTheBeach]);
	}
	//(Izmacounter= 1)
		...
		DisplayText("You hesitate a few moments before saying, \"<i>Er... hello?</i>\"\n\n");
		...
		DisplayText("Now you're even more curious; she doesn't seem as imposing and does look a bit more intellectual now that she's calmer – well, from the neck up, anyway.  She's still dressed as if expecting a luau to break out any minute.  You introduce yourself and then look at her pointedly.\n\n");

		DisplayText("\"<i>I'm Izma, a tigershark,</i>\" she replies.\n\n");

		DisplayText("\"<i>Tigershark?</i>\" you ask.\n\n");
		...
		if(player.statusAffects.get(StatusAffectType.TelAdre).value1 >= 1) DisplayText("outside the safety of Tel'Adre");
		else DisplayText("since you arrived");
		...
		DisplayText("You nod in agreement, earning a smile from Izma.  You chat for another short while before you part ways, heading back to your camp.");
		//(Izmacounter +1)
	...
	//[Next 2 encounters with Izma] (Izmacounter = 2 or 3)
		...
		DisplayText("Your exploration of the lakeshore has brought you back to Izma's campsite.  The tigershark is happy to see you, but you can't help but feel she's a little distracted with something; she's constantly playing around with her skirt and grinding her fangs.\n\n");

		DisplayText("\"<i>So, what can I interest you in?</i>\" she asks.\n\n");
		//[Trade] [Talk] [Leave] - has special text
		MainScreen.simpleChoices(["Borrow", "Talk", "", "", "Leave"], [tradeWithFuckingSharkBitches, talkToASharkCoochie, null, null, leaveSumSharkPussyOnTheBeach]);
	}
	//-----------------------------------------
	//[After 3 encounters] (Izmacounter = 4)
		...
		DisplayText("\"<i>Don't get the wrong idea here, I'm not gonna jump you or anything, I'm just offering.  I mean I could easily catch myself another shark girl or a cultist if I wanted to.  Just... offering, is all,</i>\" Izma says, looking skyward and avoiding eye contact.\n\n");
	...
	//(after 4+ encounters) (Izmacounter >= 5)
		...
		if(Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00238] === -1) {
			DisplayText("As you stroll along the lake, you find yourself coming across a familiar looking sea-chest.  It looks like you've stumbled into the path of your tigershark lover, Izma, and sure enough, she promptly emerges dripping from the waters of the lake.  She smiles in delighted surprise at seeing you.\n\n");
			
			DisplayText("\"<i>" + player.short + "! It's so good to see you!</i>\" she greets, both of you exchanging a quick hug. She sits on a rock beside her trunk, grinning from ear to ear. \"<i>So, what do you want to do today?  Have you thought about bringing your beta with you?</i>\"\n\n");
		...
		//[After 5 consecutive wins against Izma] (encountered with Izmafight >= 5)
			...
			DisplayText("You step on a brittle shell as you advance and it crunches loudly beneath your feet, drawing Izma's attention.  She looks at you and her mood seems to instantly brighten, though her hands are still fidgeting nervously.  Smirking, you hold up a hand as a greeting and ask what's up.\n\n");
			...
			//[Accept][Stay]
			MainScreen.simpleChoices(["Accept", "Stay", "", "", ""], [acceptIzmaAsYourBitch, IzmaStayAtTheLakeBitch, null, null, null]);
		}
		//Normal
		else {
			DisplayText("Izma sees you coming from a long way off and waves you over.  \"<i>Hey, " + player.short + "!  Come by to chat, or did you want to borrow a book?  Or... did you want to help me with my 'problem'?  I'm afraid it's becoming rather chronic lately,</i>\" she says, grinning, as she uncrosses her legs and a bulge lifts the front of her skirt.\n\n");
			//[Trade] [Talk] [Sex] [Fight] [Leave]
			MainScreen.simpleChoices(["Borrow", "Fight", "Talk", "Sex", "Leave"], [tradeWithFuckingSharkBitches, fightSharkCunt, talkToASharkCoochie, chooseYourIzmaWeapon, leaveSumSharkPussyOnTheBeach]);
		}
	}
...
private tradeWithFuckingSharkBitches():void {
	DisplaySprite(32);
	DisplayText().clear();
	DisplayText("Izma opens up her wooden trunk, and lays out some old books for you to look at.  An interesting and varied selection, if a small one; they've evidently been used before with their turned up corners and worn-looking pages.  Still good, of course.\n\n");
...
//Use only 10w30 jism.
	DisplayText().clear();
	DisplayText("You point to a pile of books which has a note on top declaring them to be combat manuals, feeling any combat tips you can get will be invaluable in this land.  \"<i>Those?</i>\" the shark asks.  \"<i>They're okay, I guess.  Mostly for beginners, but there are a few worthwhile tricks in each one.  20 gems to borrow one.</i>\"");
	if(player.stats.gems < 20) {
		DisplayText("\n\n<b>You haven't got that much.</b>");
		return { next: tradeWithFuckingSharkBitches };
	}
	//[Yes/No]
	else MainScreen.doYesNo(readSharkCuntManual2,tradeWithFuckingSharkBitches);
}
...
private readSharkCuntManual2():void {
	...
	DisplayText("Handing Izma the gems she asked for, you pick up one of the many issues of 'Combat Manual'. Izma takes a moment to count and store the gems you've given her, while you move over to a nearby rock to have a quick read of the volume.\n\n");
	...
	if(choice === 0) {
		DisplayText("You learn a few new guarding stances that seem rather promising.");
		//(+2 Toughness)
		player.stats.tou += 2;
	}
	else if(choice === 1) {
		DisplayText("After a quick skim you reach the end of the book. You don't learn any new fighting moves, but the refresher on the overall mechanics and flow of combat and strategy helped.");
		//(+2 Intelligence)
		player.stats.int += 2;
	}
	else {
		DisplayText("Your read-through of the manual has given you insight into how to put more of your weight behind your strikes without leaving yourself open.  Very useful.");
		//(+2 Strength)
		player.stats.str += 2;
	}
	...
	if(Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00238] != 1) DisplayText("head back to your camp.");
	else DisplayText("leave the shark-girl to her books.");
	//(Izmacounter +1)
	Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00230]++;
	return { next: Scenes.camp.returnToCampUseOneHour };
	return;
...
private sharkEdgingGuideLOL():void {
	...
	//[Yes/No]
	else MainScreen.doYesNo(readSharkEdgingGuideLOL,tradeWithFuckingSharkBitches);
}
...
private readSharkEdgingGuideLOL():void {
	...
	statScreenRefresh();
	DisplayText().clear();
	DisplayText("You hand Izma the gems she asked for and then pick up a copy.  Izma takes a moment to count the gems, while you sit down near her.\n\n");

	DisplayText("You peruse the strange book in an attempt to refine your manners, though you're almost offended by the stereotypes depicted within.  Still, the book has some good ideas on how to maintain chastity and decorum in the face of lewd advances.\n\n");
	//(-2 Libido, -2 Corruption)
	player.stats.lib += -2;
player.stats.cor += -2;
	
	DisplayText("After reading through the frilly book you give it back to Izma who delicately places it back in the trunk.  You say your goodbyes and then ");
	if(Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00238] != 1) DisplayText("head back to your camp.");
	else DisplayText("leave the shark-girl to her books.");
	//(Izmacounter +1)
	Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00230]++;
...
private sharkgirlPronz():void {
	...
	//[Yes/No]
	else MainScreen.doYesNo(readSharkgirlPornzYouFuckingPervertAsshole,tradeWithFuckingSharkBitches);
}
...
private readSharkgirlPornzYouFuckingPervertAsshole():void {
	...
	statScreenRefresh();
	DisplayText().clear();
	DisplayText("Izma colors brightly as you flamboyantly produce the requested gems and present them to her, but dutifully hands over the bound illustrations.  While she fumbles with the gems you move down a few feet to examine the pornographic material.\n\n");
	...
	if(player.stats.cor < 33) {
		DisplayText("  You give a bit of a start.  \"<i>S-sorry,</i>\" she says.  At a loss for words, you hand her the porn and make a hasty retreat");
		if(Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00238] != 1) DisplayText(" back to your camp");
		DisplayText(".");		
	}
	//(31-69 Corruption)
	else if(player.stats.cor < 66) {
		DisplayText("  You smile at her and pass her the book, with a heavy-lidded glance and a quip about how it wasn't a bad read but the real deal is much better.  She blushes a bit and claps her knees together.  Thanking Izma for the read, ");
		if(Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00238] != 1) DisplayText("you head back to camp.");
		else DisplayText("you turn back to the center of your camp.");
	}
	//(70+ corruption)
	else DisplayText("  You nonchalantly glance at Izma, and mention that it doesn't really compare to your own fantasies and experiences.  With that, you hold the closed book out and tuck it neatly into the cleavage of her breasts!  Keeping your hand on it, you quirk an eyebrow at her; she shivers, colors deeply, and turns around, snatching the book from you.  \"<i>You... perv,</i>\" she teases back.  \"<i>Why don't you write a book yourself then?</i>\" As you go to leave, you notice her grass skirt has shifted to the front and lies taut against the contours of her butt.  Too tempting!  You plant an open-palmed smack on it and take off running as she shouts after you.");
	//(Izmacounter +1)
	Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00230]++;
	return { next: Scenes.camp.returnToCampUseOneHour };
	return;
...
private talkToASharkCoochie():void {
	...
	if(Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00232] === 0) {
		DisplayText("You sit down on the rocks beside Izma, and the two of you exchange bits of gossip and information. Izma then tells you a strange tale of a mysterious island she's seen on the horizon of the lake, along with a strange smoke-belching shape she's seen on the nearby mountain in the past.  ");
		//(If player hasn't done the Demon Factory quest) 
		if(!player.statusAffects.has(StatusAffectType.DungeonShutDown)) DisplayText("You scratch your chin in thought, feeling that this matter warrants further investigation.");
		//(If the player has done the Demon Factory)
		else DisplayText("You smile, and detail just what that factory is and what you went on to do in this factory.");
	}
	//(repeat: factory not cleared)
	else if(!player.statusAffects.has(StatusAffectType.DungeonShutDown)) {
...
private leaveSumSharkPussyOnTheBeach():void {
	DisplaySprite(32);
	DisplayText().clear();
	DisplayText("Having no business with Izma for the time being, you head off back to your camp.");
...
private fightSharkCunt():void {
	...
	//(Izmacounter +1)
	Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00230]++;
	DisplayText("Izma smiles widely and retrieves a pair of hooked metal gauntlets from her chest, donning them and clenching her fist a few times.  ");
	//(If Izmafight = 0) 
	if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] === 0) DisplayText("\"<i>All right, show me just what a Champion can do!</i>\" she says, entering a fighting stance.");
	//(If Izmafight = 1-2) 
	else if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] > 0 && Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] <= 2) DisplayText("Izma's eyes narrow at you, and she assumes a fighting stance.  \"<i>You won't get so lucky this time.</i>\"");
	//(If Izmafight = 3+)
	else if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] > 2) DisplayText("Izma seems uncertain with herself as she prepares for battle. \"<i>Go a little easier on me this time... please?</i>\"");
	//(If Izmafight = -1 or -2 )
	else if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] < 0 && Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] >= -2) DisplayText("\"<i>Hm, really?  Well, maybe you'll get lucky this time,</i>\" she mocks, gesturing at you to strike first.");
	//(If Izmafight = -3 to -4)
	else DisplayText("Izma laughs slightly and shakes her head.  \"<i>If you insist.  At least TRY this time, will ya?</i>\"");
...
internal function defeatIzma():void {
	...
	if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] <= 0) {
		DisplayText("Izma falls back into the sand, her ");
		if(monster.stats.HP < 1) DisplayText("injuries");
		else DisplayText("lust");
		DisplayText(" preventing her from fighting on.  She growls at you in annoyance, \"<i>Fine.  You win... this time.</i>\"\n\n");
	}
	else if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] > 0) {
		//(Izmafight = 1 or 2) 
		if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] < 3) {
			DisplayText("Having incapacitated Izma through ");
			if(monster.stats.HP < 1) DisplayText("physical");
			else DisplayText("sexual");
			DisplayText(" prowess, you stand over the defeated tigershark.  \"<i>Okay, okay!  You win!  Geez... let's get on with it, I feel like I'm gonna go crazy.</i>\"  She begins removing her clothing.\n\n");
		}
		//(Izmafight = 3+) 
		else DisplayText("Izma falls into the sand in an exaggerated fashion.  \"<i>Oh no!  I seem to have lost!  Please don't ravish me again!</i>\" she proclaims loudly as she undresses, her bad acting almost making you laugh your ass off.\n\n");
	}
	DisplayText("Which part of your body will you claim her with?");
	let penis:Function = null;
	if(player.torso.cocks.count > 0) penis = victoryPenisIzma;
	let vag:Function = null;
	if(player.torso.vaginas.count > 0) vag = useVagooOnIzmaWin;
	//[use penis][use vag][use ass][Leave]
	MainScreen.simpleChoices(["Use Penis", "Use Vagina", "Use Ass", "", "Leave"], [penis, vag, takeItInZeButtVictoryLikeFromIzma, null, leaveIzmaVictoryTease]);
}
...
internal function IzmaWins():void {
	...
	//(without worms)
		...
		//(Izmafight =0)
		if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] >= 0) DisplayText("Izma chuckles slightly as she prowls around your defeated form.  \"<i>Well, as far as things around here stand, you made for a decent fight.  Still no match for me, though.</i>\"");
		//(Izmafight = -1 or -2) 
		else if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] >= -2) DisplayText("\"<i>Ya know, just because we're friends doesn't mean you need to hold back... you were holding back, right?</i>\" Izma asks, placing her hands on her hips.");
		//(Izmafight = -3 or -4) 
		else DisplayText("Izma sighs and shakes her head at you, letting a foot rest on your stomach  \"<i>You're doing this on purpose, aren't you?  Hm, fine.  If you love my cock so much, I think you'd make for a decent mate...</i>\"");
		//TO THE SECKS!
...
private loseToIzma():void {
	...
	if(player.gender <= 2) {
		...
		if(player.stats.HP < 1) {
			DisplayText("One of Izma's metal-clad fists cracks hard against your stomach, sending you crashing onto the sandy earth - you aren't seriously hurt, but you're too weak to continue fighting, and Izma knows it.\n\n");
			DisplayText("She grins at you, a rather fierce expression given she's still got her shark-teeth out, but her tone is gentle enough.  \"<i>Hah! Looks like I win this round, kid!  Now, I believe there's something you owe me, and I intend to have it...</i>\"\n\n");
		}
		//(If player is raised to 100 Lust:) 
		else {
			DisplayText("Your legs buckle, and your mind fogs with arousal; you are too turned on to continue fighting and collapse bonelessly into a shivering heap.\n\n");
			DisplayText("Izma shakes her head, grinning wryly.  \"<i>Looks like somebody needs to get more discipline before they try a sex-off.</i>\"\n\n");
		}
		...
		else DisplayText("pucker");
		DisplayText(" of yours!</i>\"  ");
		//[(If Izmafight is -1 or -2)
		if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] >= -2) DisplayText("With some reluctance, but driven by your promise, you remove pieces of your " + player.inventory.equipment.armor.displayName + " until you stand naked before the hungrily ogling tigershark.");
		//(If Izmafight is -3 or less)
		else DisplayText("You are already undressed before Izma has finished speaking.  The tigershark is clearly surprised, and, to be honest, a part of you is surprised too... but it's drowned out by the need to give yourself over to Izma's lusts.");
		DisplayText("\n\n");
		
		DisplayText("Your 'cute little ");
		if(player.torso.vaginas.count > 0) DisplayText("cunt");
		else DisplayText("pucker");
		DisplayText("' clenches tight in ");
		if(player.stats.HP < 1) DisplayText("fear");
		else DisplayText("anticipation");
		DisplayText(" at the thought of something that huge forcing its way into it... but you did make a promise and you are ");
		if(player.stats.HP < 1) DisplayText("too weak to resist");
		else DisplayText("feeling very horny");
		DisplayText(", so you comply.  You remove the last bits of your " + player.inventory.equipment.armor.displayName + ", and position yourself on the ground, your " + ButtDescriptor.describeButt(player) + " facing towards Izma.  You can hear her stalking across the sand toward you, but it still sends tingles up your spine when her hands - now free of her metal gauntlets - fall onto your " + ButtDescriptor.describeButt(player) + ".\n\n");
		
		DisplayText("\"<i>Ooh, looking good from this angle.  I'm going to enjoy taking you like this... after all, if you want to be on top, then you gotta fight for it.</i>\"\n\n");
	...
	if(player.gender <= 1) {
		...
		if(player.analCapacity() < 26) {
			DisplayText("You can't help but yell in pain at the sudden sensation of something so huge forcing its way into your " + ButtDescriptor.describeButthole(character.torso.butt) + ".");
			...
			//(If Izmafight <= -4)
			if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] <= -4) DisplayText("  You find yourself pushing back to speed up the process, desperate for Izma to fill you again.");
		...
		//(If player has middling anus:) 
			...
			DisplayText("You can feel every inch of her cock as it sinks steadily into your anus, swelling into your guts as inexorably as the flood tide.\n\n");
			
			DisplayText("\"<i>Ahhhh... now that's a nice little hole!  Did you lose on purpose?</i>\" she asks, and you can hear the grin in her voice.");
			//(If the player has lost 4+ times)  
			if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] <= -4) DisplayText("  You find yourself wondering that as well...");
			player.displayStretchButt(monster.torso.cocks.get(0).area,true,true,false);
		}
		//(If player has a loosey goosey:)
		else {
			DisplayText("Izma's cock may be fairly impressive, but you've taken bigger in your time, and it shows; Izma's first experimental thrust sees her sink up to the hilt into your bowels, and you moan with the pleasure of being filled again even as her four balls slap against your " + ButtDescriptor.describeButt(player) + ".\n\n");

			DisplayText("\"<i>The heck!?  What kind of monsters have you been running into?</i>\" she wonders aloud.");
		}
		...
		DisplayText("Harder and faster she thrusts, building up a rhythm that grows in pace, her balls slapping audibly against your " + ButtDescriptor.describeButt(player) + " as she bucks back and forth.  You can feel her huge male organ in your depths, rubbing against your prostrate, stretching your inner walls, her boiling erection against your burning heat.  You moan in pleasure; you can't help but enjoy this");
		if(player.torso.cocks.count > 0) DisplayText(", and your own male organ is hard and throbbing from the stimulation");
		DisplayText(".\n\n");
		...
		//(If Izmafight = -4)  
		if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] <= -4) DisplayText("  If this is how the sharks do it, you could really get used to it...");
		DisplayText("\n\n");
		...
		DisplayText("You groan as well, ");
		//[(male)
		if(player.torso.cocks.count > 0) {
			DisplayText(CockDescriptor.describeMultiCockSimpleOne(player) + " disgorging ");
			if(player.cumQ() < 25) DisplayText("a trickle");
			else if(player.cumQ() <= 150) DisplayText("several squirts");
			else DisplayText("a steady stream");
			DisplayText(" of semen onto the sandy earth below you, but it pales in comparison to the tide flooding into your guts.  Hot and slick, it surges and flows into you, pumping and pumping into your depths.");
		}
		//(unsex)
		else DisplayText("your own muscles spasming from the immense pleasure.");
	...
	//Female Loss:
		...
		if(player.vaginalCapacity() < 26) {
			DisplayText("You can't help but yell in pain at the sudden sensation of something so huge forcing its way into your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ".\n\n");
			
			DisplayText("\"<i>Whoah-!  ");
			if(player.torso.vaginas.get(0).virgin) DisplayText("First time, huh?");
			else DisplayText("That's tight!");
		...
		//(If player has ordinary, everyday cunt:)
			...
			DisplayText("You can feel every inch of her cock as it sinks steadily into your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ", filling your moist folds as inexorably as the rising tide.\n\n");
		...
		//(If player is loose:)
			...
			DisplayText("Izma's cock may be fairly impressive, but you've taken bigger in your time, and it shows; Izma's first experimental thrust sees her sink up to the hilt into your crotch, and you moan with the pleasure of being filled again even as her four balls slap against your taint.\n\n");
		...
		DisplayText("\"<i>Oh, somebody likes it, eh?  Well, don't worry, you wanted to test your luck, so I'm not going to hold back!  You're getting the whole experience, sweetheart!</i>\" Izma growls.  Her hands suddenly shift from your buttocks to holding onto your " + Desc.Breast.describeChest(character) + ", and you howl in a mixture of pain and pleasure as Izma suddenly gives your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + "s a good hard tug.  \"<i>Stiff nipples?  You so wanted this...</i>\" Izma teases, licking at your neck and causing you to moan in pleasure.");
		//(If the player has lost 4+ times) 
		if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] <= -4) DisplayText("  If this is how the sharks do it, you could really get used to it...");
		DisplayText("\n\n");
		...
		DisplayText("\"<i>That's how shark people have sex,</i>\" she tells you.  \"<i>Of course, it's different if you're the one who won... but you'll need to come back again and beat me if you want to see what that's like.</i>\"  She leans down and gives you a small peck on the lips, then drops the tigershark tooth beside you.");
		if(player.pregnancyIncubation === 0 || player.pregnancyIncubation > 180) DisplayText("  From her oak chest she fetches an odd leaf, which she pushes past your lips.  \"<i>Birth control herbs.  No way I'm giving kids to someone who's not my mate,</i>\" Izma says, making sure you swallow the plant before diving into the water, most likely to clean herself off.\n\n");

		DisplayText("You remain where you are, waiting for the strength to flow into your limbs and for some of the abundance of sexual fluids to vacate your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " before you dress yourself and leave.  You had no idea that Izma could take charge in such a fierce manner... but, at the same time, you find yourself actually liking it.  A part of you wonders if you could see her that way again...");
		//(Izmafight minus 1)
		Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON]--;
	}
	//Loss Scene- Herm 
		...
		//(If player is raised to 100 Lust:) 
			...
			DisplayText("Your legs buckle, and your mind fogs with arousal; you are too turned on to continue fighting and collapse bonelessly into an aroused heap.\n\n");
			
			DisplayText("Izma shakes her head, grinning wryly.  \"<i>Looks like somebody needs to get more discipline before they try a sex-off.</i>\"\n\n");
		}
		DisplayText("As you collapse in defeat, you're aware of the pretty tigershark stalking around you and removing her clothing.  She grabs hard on your " + player.inventory.equipment.armor.displayName + " and undresses you with minimal effort, revealing your ");
		if(player.stats.lust < 40) DisplayText("limp cock and barely-engorged cunt");
		else if(player.stats.lust < 70) DisplayText("turgid erection and moist cunt");
		else DisplayText("throbbing erection and slavering cunt.");
		DisplayText("\n\n");

		DisplayText("\"<i>Dirty little minx, ain't ya?  You wanted this, didn't you...</i>\" Izma teases, shoving two fingers into your moist nether-lips, to test the waters.  The peneration of your needy cunt does serve to make you whimper softly, almost begging to just be filled.  ");
		//(Tight/Virgin vagina)
		if(player.vaginalCapacity() < 26) {
			DisplayText("\"<i>My oh my, quite a tight little slit you got back here. Not for long...</i>\" Izma says, her fingers roaming around within your cunt.");
		}
		//(If player has ordinary, everyday cunt:)
		else if(player.vaginalCapacity() < 60) {
			DisplayText("Izma giggles slightly from just how easy it is to move around your moist folds.  \"<i>Well, this'll make things slightly easier for you I suppose.</i>\"");
		}
		else {
			DisplayText("Izma's eyes widen as her entire hand seems to slip into your cavernous vagina. \"<i>Holy shit... what's been up here?</i>\" she mumbles, laughing nervously in amazement.");
		}
		DisplayText("\n\n");
		...
		//(1-10 inch penis)
		if(player.torso.cocks.get(x).length <= 10) DisplayText("Izma manages to supress a snort of laughter at the sight of your cock.  \"<i>Um... wow?  It's simply... heh, huge...</i>\"");
		//(10-19 inches)
		else if(player.torso.cocks.get(x).length <= 19) DisplayText("\"<i>Not bad, I'm actually impressed,</i>\" Izma says, nodding slightly in approval.");
		//(20+ inches)
		else DisplayText("Izma gives a low whistle at the sight of your " + CockDescriptor.describeCock(player, x) + ".  \"<i>Now THAT'S a cock.  Looks like you've got a third leg down there!</i>\"");
		...
		//(If Izmafight <= -4) 
		if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] <= -4) DisplayText("  You're not even really ashamed to admit that such is the case anymore.");
		DisplayText("\n\n");
		...
		DisplayText("Within minutes, Izma gives one final, powerful thrust and roars loudly, cum pumping into your womb and spraying out onto the sands.  " + CockDescriptor.describeMultiCockSimpleOne(player, true) + " twitches and pulses, ready to blow.  Izma quickly takes hold and points ");
		if(player.torso.cocks.count === 1) DisplayText("it");
		else DisplayText("them");
		DisplayText(" toward your face, stroking you to your own climax.  Jets of your own cum splatter across your face and body as you writhe, protesting. \"<i>Tch, you really thought I'd let you cum on ME?  Maybe if you actually managed to beat me I'd give you the honor,</i>\" Izma says, pulling free with a loud *SCHLICK* sound.  She releases your " + LowerBodyDescriptor.describeFeet(player) + ", allowing your " + ButtDescriptor.describeButt(player) + " to hit the sand with a plop, and gets to work redressing while you lie still.");
		if(player.pregnancyIncubation === 0 || player.pregnancyIncubation > 150) {
			DisplayText("  Moving over to her oak chest, she returns with a strange leaf in her hands, which she pushes past your lips.\n\n");

			DisplayText("\"<i>Anti-pregnancy herb.  Can't have someone bearing my litter if they're not my mate, can I?</i>\"  Izma explains, moving over to the shoreline.  ");
		}
		else DisplayText("\n\n");
	...
	//zmafight -1: 
	if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] >= -1) DisplayText("  You realize what you're thinking and shudder, forcing the submission-tinged desires down. Where did they come from, anyway?");
	//Izmafight -2: 
	else if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] >= -2) DisplayText("  Though you manage to force them away, the dreams of submitting to Izma are starting to haunt you, their power and seductive allure growing. But, still, you can control them.");
	//Izmafight -3: 
	else if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] >= -3) DisplayText("  You hum idly to yourself and enjoy the mental visions for a while, then, with some reluctance, you push them aside. Still, you're sure you can bring them out again when you want to enjoy them.");
	//Izmafight -4: 
	else if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] >= -4) DisplayText("  You have only the vaguest thought that maybe you shouldn't be thinking about Izma in that way, but it's so tempting to just immerse yourself in the sexiness of having a hot herm shark-girl dominate you so thoroughly. After all, it's not as if there's any harm in doing so, is there?");
	//Izmafight -5: 
	else DisplayText("  You embrace the dreams fully, desperate to cling to them as long as you can. It's getting so hard to care about your former mission anymore; why fight the demons when you can just give it up and surrender yourself to Izma? Yes... such a strong, powerful, worthy alpha she is; Izma is all you need. Let her take control of your life, why don't you?");
...
private finalIzmaSubmission():void {
	DisplayText().clear();
	DisplayText("You collapse onto your hands and knees, defeated by Izma once again.  You feel you may not have fought as hard as you could have though, out of a deep-rooted desire for Izma to dominate you again.\n\n");
	
	DisplayText("Izma sniggers as she advances on you, happy to oblige your desire, casually undoing your " + player.inventory.equipment.armor.displayName + " after scattering her own clothing to the rocks.  \"<i>Aw, was that all? Honestly...</i>\" she mocks, walking behind you, \"<i>you're quite the little bitch, aren't ya?  You just love submitting to Izma, don'tcha?</i>\"  You whimper slightly and nod your head in response.  As Izma leans into your back, her rock-hard nipples and cock press into your ");
	if(!player.torso.hips.legs.isTaur()) DisplayText("spine");
	else DisplayText("flank");
	DisplayText(".\n\n");
	
	DisplayText("Some of Izma's hot pre-cum dribbles out onto your back and runs down to your " + ButtDescriptor.describeButt(player) + ".  It causes your eyes to roll back in your head; you can only imagine the taste of her salty pre running down your throat.  Izma snickers, aware of her hold over you, before tightening her grip.");
	if(player.torso.cocks.count > 0 || player.torso.vaginas.count > 0) {
		DisplayText("  The tight embrace makes ");
		if(player.torso.cocks.count > 0) {
			DisplayText(CockDescriptor.describeMultiCockSimpleOne(player) + " stiffen");
			if(player.torso.vaginas.count > 0) DisplayText(" and ");
		}
		if(player.torso.vaginas.count > 0) DisplayText("your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " moisten considerably");
	}
	DisplayText(".\n\n");
	...
	DisplayText("She keeps you held in place as she positions the tip of her cock against your " + ButtDescriptor.describeButthole(character.torso.butt) + ".  With no warning, she forces her meat-pole into you, making you squeal in delight from the familiar heat of her cock.  ");
	player.displayStretchButt(monster.torso.cocks.get(0).area,true,false,true);
	DisplayText("She keeps forcing her way in your backdoor, inch after inch, and you only feel weaker against Izma the further in she goes.  Eventually she manages to bury her entire dick into your pucker, sighing in delight as her meaty quads press against your " + ButtDescriptor.describeButt(player) + ".\n\n");
	...
	DisplayText("As if sensing your complete submission, Izma's hands trail down toward your " + BreastDescriptor.describeBreastRow(player.torso.chest.get(0)) + ", mauling and groping at them, tweaking your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + "s in an almost-painful fashion.  It only manages to turn you on further though, groaning and squirming from intense pleasure.  Izma makes you orgasm first, your muscles twitching and spasming as you briefly lose control over yourself");
	if(player.torso.vaginas.count > 0 || player.torso.cocks.count > 0) {
		DisplayText("; ");
		if(player.torso.cocks.count > 0) {
			DisplayText(CockDescriptor.describeMultiCockSimpleOne(player) + " begins spurting jizz all over the sandy shore");
			if(player.torso.vaginas.count > 0) DisplayText(" and ");
		}
		if(player.torso.vaginas.count > 0) DisplayText("your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " starts squirting fluids liberally");
	}
	DisplayText(".\n\n");
	...
	if(player.race() != "shark-morph") DisplayText("  \"<i>But we'll have to make some changes to your body, of course...</i>\"");
	DisplayText("\n\n");
	player.slimeFeed();
	player.orgasm();
...
private IzmaBadEnd():void {
	...
	DisplayText("You arise eagerly and follow after her like a love-struck puppy.  Your muscles do feel quite sore from today's adventuring, but the fact that Izma uses you to help her in combat, instead of just keeping you as a sex toy");
	if(player.gender >= 2) DisplayText(" and a fertile womb");
	DisplayText(", is wonderful.  It gives you a real sense of purpose, serving your alpha in such a way.\n\n");
...
private victoryPenisIzma():void {
	...
	DisplayText("You watch the defeated tigershark closely and a grin forms on your face.  You touch a hand to her forehead and push her onto her back with minimal effort, slipping her clothing off.  She hardly lacks the strength to stop you, but she knows full well what the terms of the battle were, so she's not going to go back on her word.  You remove your " + player.inventory.equipment.armor.displayName + " and spread Izma's legs wide, " + CockDescriptor.describeMultiCockSimpleOne(player) + " almost painfully erect as you lift her quartet of balls up to look at her glistening womanhood.\n\n");
	
	DisplayText("Not wanting to waste any time on foreplay, you push your " + CockDescriptor.describeCock(player, cockIndex) + " into Izma's slit as far as you can manage, making Izma gasp sharply and writhe against you.  You snicker and start thrusting into her, the odd little tendrils inside her cunt teasing and massaging your cock.  The walls themselves are so tight and smooth that her pussy conforms to you like a glove.  It almost feels like Izma's snatch was made just for you.");
	//[(If Izmafight = 3+)
	if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] >= 3) DisplayText("  Hell, maybe it was made for you, given just how eager Izma seems to be whenever she sees you.  It's like she loses to you on purpose.");
	DisplayText("\n\n");
	...
	if(player.gender === Gender.MALE) {
		DisplayText("After a lengthy fuck, you grunt loudly as your " + CockDescriptor.describeCock(player, cockIndex) + " swells, blasting streamers of jizz into Izma's womb");
		//[(multi) 
		if(player.torso.cocks.count > 1) DisplayText(" and onto her groin");
		DisplayText(", causing Izma to cry out loudly.");
		//[(big skeet)
		if(player.cumQ() >= 500) {
			DisplayText("  Her belly swells as you empty your impressive load into her");
			if(player.cumQ() >= 1500) DisplayText(".  Eventually it can swell no more and each new squirt forces cum out from her stuffed pussy, trickling past her asshole");
			DisplayText(".");
		}
		DisplayText("  You sigh happily and push back from her, weakly getting back on your " + LowerBodyDescriptor.describeFeet(player) + " and redressing. Izma scrambles to her chest and takes out some sort of leaf, then eats it.\n\n");
	}
	//(Herm)
	else {
		DisplayText("After a lengthy fuck, you grunt loudly as your " + CockDescriptor.describeCock(player, cockIndex) + " swells, blasting streamers of jizz into Izma's womb");
		...
		DisplayText("  You sigh happily and push back from her, weakly getting to your " + LowerBodyDescriptor.describeFeet(player) + ".  You're not done yet though, not fully.\n\n");
		
		DisplayText("Izma gasps again as you plant your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " onto her face, grinding against the angular features and moaning loudly as her obliging tongue darts past your lips.  You could really get used to this feeling.  You ride her face for another few minutes before an orgasm rocks your female parts, splattering girlcum onto Izma's face.  You sigh happily and weakly get to your feet, redressing.  You see Izma fishing something from her storage chest - a plant of some sort - and munching it down.\n\n");
	}
	player.orgasm();
	//[(if Izmafight <=4)
	if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] <= 4 || Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00235] > 0) {
		DisplayText("You say your goodbyes to the pretty tigershark and leave once she hands you your tooth-shaped reward.");
		Flags.list[FlagEnum.BONUS_ITEM_AFTER_COMBAT_ID] = consumables.TSTOOTH.id;
		cleanupAfterCombat();
	}
...
private useVagooOnIzmaWin():void {
	...
	DisplayText("Deciding you've had enough foreplay, you mount her and slide down her cock.");
	player.displayStretchVagina(monster.torso.cocks.get(0).area,true,true,false);
	DisplayText("  You start grinding and gyrating atop her, ");
	//[(taur)
	if(player.torso.hips.legs.isTaur()) DisplayText("your weight pinning her to the sand and preventing her from taking control.");
	//(non-taur and height > 4') 
	else if(player.tallness > 48) DisplayText("and pin her hands above her head to stop her from trying to change the position. Her heaving breasts rub against your " + BreastDescriptor.describeAllBreasts(player) + " as you ride her in this posture.  She needs to know who's in charge here, after all.");
	//(non-taur and height <= 4')
	else DisplayText("but small as you are, you can't stop her as she reaches up and grabs onto your " + ButtDescriptor.describeButt(player) + ", then begins bouncing you like a goblin cocksleeve.");
	DisplayText("\n\n");
	
	DisplayText("Izma seems to do her best, moving and jerking her cock up as much as she can, earning gasps of pleasure from you.");
	//[(no-taur)
	if(!player.torso.hips.legs.isTaur()) DisplayText("  To reward your eager partner for her efforts you reach back with one free hand to massage and grope at her testes.  Izma bites her lip and starts growling loudly, pushing her hips up as far as possible, eager to cum for you.  You decide to return the favor, mashing your pussy down with increasing speed.");
	DisplayText("\n\n");
...
private eatIzmasLeafAfterRapinHer():void {
	DisplayText().clear();
	DisplayText("You accept the leaf gingerly and eat it.  Izma smiles.");
	//Set loot
	Flags.list[FlagEnum.BONUS_ITEM_AFTER_COMBAT_ID] = consumables.TSTOOTH.id;
	//[(if Izmafight <=4) 
	if(Flags.list[FlagEnum.IZMA_TIMES_FOUGHT_AND_WON] <= 4 || Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00235] > 0) {
...
private dontEatIzamsLeafAfterRape():void {
	DisplayText().clear();
	DisplayText("You rankle at the offered herb and give her a haughty glare.  \"<i>You're going to tell ME what to do? Me, your proven superior?</i>\"\n\n");

	DisplayText("Izma cringes.  \"<i>Sorry!  I just don't want to go fathering children with someone who's not my mate!  Please, please take it!</i>\"\n\n");
...
private takeItInZeButtVictoryLikeFromIzma():void {
	DisplayText().clear();
	player.slimeFeed();
	DisplayText("You smirk as Izma slumps down, unable to fight against you anymore.\n\n");
	...
	DisplayText("She grunts and huffs as you slide down, and you too feel a strain from her iron-hard dick despite the various fluids lubricating it.  But gradually pain turns to pleasure and you're both moaning loudly and calling each other's names as you ride her.");
	player.displayStretchButt(monster.torso.cocks.get(0).area,true,true,false);
	DisplayText("\n\n");
...
private leaveIzmaVictoryTease():void {
	DisplayText().clear();
	DisplayText("Izma collapses to the sand and leans back.  \"<i>Ahhh... you won.  Come get your prize,</i>\" she says, beginning to undress.  You stand there, considering for a moment, as she idly strokes herself.\n\n");

	DisplayText("\"<i>I'm ready, you know... we can start... any time?</i>\" she puts forth cautiously as you remain still.\n\n");
	...
	DisplayText("\"<i>We're going to play the orgasm denial game,</i>\" you declare.  \"<i>You're not to masturbate, drink Lust Drafts, or have sex until you can beat me in a fight or I say you can.</i>\"\n\n");

	DisplayText("\"<i>What?!</i>\" she yelps.  \"<i>But....</i>\"  You cut her off with a hand held palm-out.\n\n");
...
private victorzChoice():void {
	DisplayText("Izma looks at you, panting from the sex.  \"<i>S-so... that was good... want your reward now?</i>\" she asks, holding the tigershark tooth out to you.  You stare at it, thinking.  Do you want another one of those, or do you want something else?");
	//[Tooth][Gloves]
	MainScreen.simpleChoices(["Tooth", "Gloves", "", "", ""], [chooseIzmaTooth, chooseIzmaGloves, null, null, null]);
}
//[Tooth]
private chooseIzmaTooth():void {
	DisplayText().clear();
	DisplayText("You accept the tooth from her with a polite word of thanks.");
	//(gain 1 t-shark toof)
	Flags.list[FlagEnum.BONUS_ITEM_AFTER_COMBAT_ID] = consumables.TSTOOTH.id;
	cleanupAfterCombat();
}
...
private chooseIzmaGloves():void {
	DisplayText().clear();
	DisplayText("You look her dead in the eye and say, in a flat monotone, \"<i>I want your gloves.</i>\"\n\n");

	DisplayText("\"<i>W-what?</i>\" she asks, dazed.  You point at the hooked gauntlets, currently laying discarded on the beach after the sex.  \"<i>But that's my weapon!  I need it to-</i>\"\n\n");
	...
	DisplayText("Meekly, she picks up the gloves and hands them to you.");
...
private chooseYourIzmaWeapon():void {
	if(player.gender === Gender.NONE) nonFightIzmaSmexASS();
		...
		DisplayText("Which of your genitals will you focus on?", true);
		MainScreen.simpleChoices(["Male", "Female", "", "", ""], [nonFightIzmaSmexPAINUS, nonFightIzmaSmexCUNTPUSSYSNATCHQUIM, null, null, null]);
	}
}
...
private nonFightIzmaSmexPAINUS():void {
	...
	DisplayText("You respond in kind, shedding your garments as her lips purse and her eyes roam up and down your form.  ");
	//Single Normal dicks
	if(player.torso.cocks.count === 1 && player.torso.cocks.get(0).type.Index < 9) {
		//[Human dick in slot 0]
		if(player.torso.cocks.get(0).type === CockType.HUMAN) DisplayText("Izma seems surprised to see your genitalia is similar to hers.  \"<i>Huh. Thought that every land-dweller usually bumbled into a dick transformation around here.</i>\"");
		//[Horse cock in slot 0] 
		else if(player.torso.cocks.get(0).type === CockType.HORSE) DisplayText("Seeing your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " causes Izma to lick her lips slowly. \"<i>Well... Equinum is pretty popular, isn't it?</i>\"");
		//[Dog cock]
		else if(player.torso.cocks.filter(Cock.HasKnot).length > 0) DisplayText("Izma stares intently at your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", as if trying to make up her mind about it. \"<i>Well, it is a rather cute look. Just be careful with the knot, will ya?</i>\"");
		//[Tentacle] 
		else if(player.torso.cocks.get(0).type === CockType.TENTACLE) DisplayText("A puzzled look plays across Izma's face as she stares at your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ".  \"<i>Wow... I've read a lot about the corrupt plants.  Didn't think people could wind up like that.</i>\"");
		//[Demon Dick] 
		else if(player.torso.cocks.get(0).type === CockType.DEMON) DisplayText("Izma looks shocked as she lays eyes on your perverted pecker, having never seen anything quite like it before. \"<i>Um... that looks... awkward. Like having a truncheon between your legs.</i>\"");
		//[cat dork] 
		else if(player.torso.cocks.get(0).type === CockType.CAT) DisplayText("She glances at your barbed pecker and winces.  \"<i>You ate the fruit around here, didn't you?  One of my friends did that, too.</i>\"");
		//[lizardick]
		else if(player.torso.cocks.get(0).type === CockType.LIZARD) DisplayText("Her eyes goggle as you reveal a bulbous purple pecker.  \"<i>Oh wow.  Do you have a green one too?</i>\"");
		//[Captain Nemo] 
		else if(player.torso.cocks.get(0).type === CockType.ANEMONE) DisplayText("She grimaces as you unveil a " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ".  \"<i>God, not one of those.  You wind up hornier after cumming than you were before you started touching it.</i>\"");
		//[Kangaroo Jack] 
		else if(player.torso.cocks.get(0).type === CockType.KANGAROO) DisplayText("She looks on blankly as you reveal your sheath, then gives a start as your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " slides out of it after a few strokes.  \"<i>Woah... it's so... thin and pointy.</i>\"");
	}
	//Multi or weird new shit.
	else DisplayText("Her eyes bug out as you reveal " + CockDescriptor.describeMultiCockSimpleOne(player) + ".  \"<i>Woah, jeez, look at that one... oh man, and that one's... daaamn.</i>\"");
	DisplayText("\n\n");
	
	//[If the player is masculine but has B-cups or larger]
	if(player.mf("m","f") === "m" && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2) {
		DisplayText("The fact that you have breasts seems to confuse Izma quite a bit as well, causing her to tilt her head.  \"<i>You're a guy... with boobs.  Did you honestly think they were necessary, or did you just want something to play with?</i>\"");
		//[If player is lactating] 
		if(player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 2) DisplayText("  Catching sight of some milk oozing from your nipples makes Izma step back slightly.  \"<i>You're lactating too?!  Man, there are way too many things that are weird about this...</i>\"  Still, you doubt Izma is as turned off as she acts, what with the raging erection between her legs.");
		DisplayText("\n\n");
	}
	...
	DisplayText("In an effort to further pleasure your tigershark partner, you occasionally reach up to fondle her swollen sack and finger her tight, damp cunt.  Izma gasps sharply and shudders from the pleasure you're giving her.  As if to reward you for your efforts, Izma picks up more speed with her sucking, drenching every inch of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " with her saliva, and as you're so eager for her magic tongue you start thrusting your hips up to reach as far into her mouth as you can.\n\n");
	
	DisplayText("Eventually, the two of you manage to bring each other to a powerful simultaneous orgasm, Izma's hot cum shoots down your eager throat, and you yourself fire loads into Izma's cheeks");
	if(player.torso.cocks.count > 1) DisplayText(" and hair");
	DisplayText(".  ");
	//[(big/mega skeet)
	if(player.cumQ() >= 500) DisplayText("You continue to shoot off stroke after stroke long after her mouth has filled up and she's pulled away.  \"<i>For the love of Marae, " + player.short + ", I can't swallow all that!</i>\"  ");
	DisplayText("The two of you roll away from each other, and while you catch your breath you're surprised at the strong taste of Izma's cum.  You don't think you'd mind getting another load of that wonderful spunk in your stomach...\n\n");
	...
	Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00230]++;
	player.orgasm();
	inventory.takeItem(consumables.TSTOOTH, Scenes.camp.returnToCampUseOneHour);
}
...
private nonFightIzmaSmexASS():void {
	...
	DisplayText("Izma sighs and places a hand on your shoulder.  \"<i>I'm sorry... I came off as harsh there.  Look, it's... this isn't gonna work out unless one of us is on top.  Just... I dunno... go gulp some incubus drafts or succubi milk and come back.  Or challenge me.</i>\"  You nod and redress, deciding to take your leave.\n\n");
	
	//(If Int is 50+) 
	if(player.stats.int >= 50) {
		DisplayText("You stop a moment as a realization occurs to you.  \"<i>Wouldn't you be getting just as much pleasure if I was on top?</i>\" you ask.\n\n");

		DisplayText("Izma rolls her eyes.  \"<i>I've dominated with my holes before.  You ride 'em hard enough, they don't feel as much pleasure.  Now, fuck off.</i>\"\n\n");
	}
	return { next: Scenes.camp.returnToCampUseOneHour };
}
...
private nonFightIzmaSmexCUNTPUSSYSNATCHQUIM():void {
	...
	DisplayText("\"<i>Your turn now.  Fair's fair,</i>\" she says, watching you with seductively-lidded eyes.\n\n");

	DisplayText("You nod and remove your " + player.inventory.equipment.armor.displayName + ", not doing it with quite as much showmanship as Izma herself.  You expose your " + BreastDescriptor.describeAllBreasts(player) + " to the air; your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + "s grow stiff from the breeze.  The bottom half of your clothing comes off, exposing your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)));
	...
	DisplayText("Izma gives a muffled groan of pleasure, her cum blasting down your throat and filling your cheeks as you pull your mouth up.  It's not too long after that when you cry out in orgasm, your girlcum spattering onto the tigershark's face");
	if(player.torso.cocks.count > 0) DisplayText(" and " + CockDescriptor.describeMultiCockSimpleOne(player) + " shooting a weak load across her chest");
	DisplayText(".  You roll off her and try to catch your breath, swallowing residual cum.  The taste is incredible, you have to admit... it's hard to define just what it is that makes it so good.  Something in the water Izma swims in?\n\n");
	player.slimeFeed();
...
private IzmaStayAtTheLakeBitch():void {
	...
	DisplayText("You give her a small kiss on the lips and then leave for camp.");
...
private acceptIzmaAsYourBitch():void {
	...
	if(Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00236] === 0 && Flags.list[FlagEnum.AMILY_FOLLOWER] === 1) {
		DisplayText("You smile warmly and place your hands on her shoulders, before pulling her in and kissing her deeply.  Practically melting at your touch and moaning into your mouth, she wraps her arms and tail around your waist.\n\n");
		...
		DisplayText("Izma tilts her head to the side and adjusts her spectacles to get a better look at Amily.  \"<i>Well, I'm a tigershark actually,</i>\" she says flatly.  \"<i>I'm also " + player.short + "'s beta.  Who're you?  Oh!  You're one of those mouse people, right?</i>\"\n\n");

		DisplayText("Amily just looks at her quizzically.  \"<i>Yes, I am a mouse.</i>\"  Her voice is ringing with a tone that just screams 'isn't it obvious?'.  \"<i>I'm sorry, I've seen some striped shark-girls before, but I didn't know your kind had a specific term... wait, what was that about being " + player.short + "'s beta?  What's a beta?</i>\"\n\n");

		DisplayText("Izma gives a frown and scuffs the ground with her bare feet.  You give a nervous chuckle and try your best to explain the events of how you met Izma, your various sparring matches and how she eventually submitted fully to you after several victories, declaring you as her alpha.\n\n");
		...
		DisplayText("Then she looks thoughtful.  \"<i>Hey, come to think of it, if you're a shark-girl - sorry, tigershark, then how are you going to survive out here?  Don't you need to spend about two-thirds of the day underwater to live?</i>\" Amily asks, puzzled.\n\n");
		...
		DisplayText("You apologize, but tell her that you won't just throw Izma out now.  She's here to stay, and the mouse-morph had better make her peace with that - fighting wouldn't work out well for anyone.\n\n");

		DisplayText("\"<i>I'll be polite to her, don't worry about that...  but are you absolutely sure we can trust her?  I'd be very hesitant to accept anything or anyone coming out of that lake,</i>\" Amily asks you, watching Izma as she curiously starts poking around at your mutual belongings.\n\n");
	...
	else if(Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00237] === 0 && player.statusAffects.has(StatusAffectType.CampMarble)) {
		...
		DisplayText("Marble nods and seems to force a smile.  \"<i>Just give us a moment,</i>\" she tells Izma, before taking you by the hand and pulling you to another part of camp rather forcefully. Oh great, it's going to be one of those talks.\n\n");
		...
		DisplayText("Marble grits her teeth and shake her head. \"<i>I don't like it, sweetie.  I don't like it and I certainly don't support it.  Just keep her under control and away from me, and then we'll see,</i>\" the cow-girl says, before clopping off in a huff.\n\n");
		...
		if(player.perks.has(PerkType.MarblesMilk)) DisplayText("You couldn't break your addiction, and now you'll die without a steady supply of her milk.");
		//(If the player broke their addiction) 
		else DisplayText("Thankfully, though you were able to break her milk's hold, and you realized you love Marble despite all that transpired between you, letting her move in with you.");
		DisplayText("\n\n");

		DisplayText("Izma grits her teeth and sucks air through them as you finish your story.  \"<i>That... damn cow...</i>\" she growls, making you gulp at her barely contained rage.  \"<i>She took you, my perfect alpha, and ");
		//(Fatally addicted)
		if(player.perks.has(PerkType.MarblesMilk)) DisplayText("turned you into some weak drug-addict");
		//(Broke addiction)
		else DisplayText("drugged you into some twisted relationship");
...
public newAmilyMeetsIzma():void {
	DisplaySprite(32);
	DisplayText().clear();
	DisplayText("You and Amily seem to talk non-stop on the way back, both excited at the prospect of living together.  Then she realizes you're not alone, once she sees Izma seated on her trunk, and looks puzzled. \"<i>" + player.short + "... there's a shark-girl in your camp!</i>\" she hisses, looking ready to fight or flee if the situation warrants.\n\n");
	...
	DisplayText("\"<i>Ah, well, the lake isn't too far from here,</i>\" Izma counters.  \"<i>And there is a stream up here.  So long as I get some water on my body routinely, I'm golden.  Plus it's nice to get access to some cleaner water!  Well, I've got some reading to catch up on, so I'll leave you be,</i>\" the pretty tigershark says.\n\n");
...
public newMarbleMeetsIzma():void {
	DisplaySprite(32);
	DisplayText().clear();
	DisplayText("You and Marble are deep in conversation all the way to your camp, mainly inconsequential things to pass the time.  Marble enjoys the talk all the same, but she comes to a halt once she sees Izma sitting around your camp.  \"<i>Ah... who's that, sweetie?</i>\" she asks.\n\n");
	...
	DisplayText("Marble nods and seems to force a smile.  \"<i>Just give us a moment,</i>\" she tells Izma, before taking you by the hand and pulling you to another part of camp rather forcefully.\n\n");
	...
	DisplayText("Marble grits her teeth and shakes her head.  \"<i>I don't like it, sweetie.  I don't like it and I certainly don't support it.  Just keep her under control and away from me, and then we'll see,</i>\" the cow-girl says, before clopping off in a huff.\n\n");
...
private izmaLakeTurnedDownCampSex():void {
	DisplaySprite(32);
	DisplayText().clear();
	DisplayText("You give Izma a smirk and tell her that, if she's in the mood, you could use a little 'relief'.\n\n");
	...
	MainScreen.simpleChoices(["Equals", "Dominate", "Submit", "", "Back"], [izmaLakeSexAsEquals, izmaLakeDominate, submitToLakeIzma, null, meetIzmaAtLake]);
}
...
private izmaLakeSexAsEquals():void {
	DisplaySprite(32);
	DisplayText().clear();
	DisplayText("You tell Izma that you would like to have sex as equals; you and she.  She smiles and asks you what part you'd like to use.\n\n");
	//((If player is Genderless) 
	if(player.gender === Gender.NONE) {
		DisplayText("Izma looks you over, and then shakes her head sadly. \"<i>I'm sorry, " + player.short + ", but... you still don't really have anything for me to play with.  If you want us to do it as equals, you'll need to grow a cock or a pussy.  How about you have one of us take charge, instead?</i>\"\n\n");
		//[dom/sub])
...
private izmaLakeDominate():void {
	DisplaySprite(32);
	DisplayText().clear();
	DisplayText("Izma blushes fiercely and a lewd smile comes over her face at the suggestion.  \"<i>O-okay,</i>\" she says, stripping off her bikini top and gently removing her grass skirt, allowing her breasts and monster cock to pop free.  She sits down and smiles at you.  \"<i>After you...</i>\"\n\n");
	let x:number = player.cockThatFits(65);
	if(x < 0) x = 0;
	//[Male/Herm]
	if(player.torso.cocks.count > 0 && (player.gender === Gender.MALE || randInt(2) === 0)) {
		DisplayText("You remove your " + player.inventory.equipment.armor.displayName + " and spread Izma's legs wide, " + CockDescriptor.describeMultiCockSimpleOne(player) + " almost painfully erect as you lift her quartet of balls up to look at her glistening womanhood.\n\n");
 
		DisplayText("Not wanting to waste any time on foreplay, you push your " + CockDescriptor.describeCock(player, x) + " into Izma's slit as far as you can manage, making Izma gasp sharply and writhe against you.  You snicker and start thrusting into her, the odd little tendrils inside her cunt teasing and massaging your cock.  The walls themselves are so tight and smooth that her pussy conforms to you like a glove.  It almost feels like Izma's snatch was made just for you.\n\n");
 
		DisplayText("You start to pick up speed as you mash your hips against Izma's own, earning moans from the pretty tigershark which only seem to get louder with every subsequent thrust.  Izma quickly starts to return the gesture, moving her hips up to meet your own thrusts every time.  It's while she's doing this that you notice her throbbingly erect cock wobbling around.\n\n");
		//NEW RADAR STUFF, GAO
		DisplayText("Do you tend to her prick?");
		doYesNo(izmaLakeDominateContinueVanilla,noWankingForIzmaRadarSaysSo);
		return;
	}
	//[Female]
	else if(player.torso.vaginas.count > 0) {
		DisplayText("You strip off your " + player.inventory.equipment.armor.displayName + " and spread Izma's legs wide, licking your lips at the sight of her throbbing erection and meaty quads.  You give Izma's massive cock a few test strokes, earning some pleasured groans from the tiger shark.\n\n");
		...
		if(player.torso.hips.legs.isTaur()) DisplayText("your weight pinning her to the sand and preventing her from taking control.");
		///(non-taur and height > 4')
		else if(player.tallness > 48) DisplayText("and pin her hands above her head to stop her from trying to change the position. Her heaving breasts rub against your " + Desc.Breast.describeChest(character) + " as you ride her in this posture.  She needs to know who's in charge here, after all.");
		//(non-taur and height <= 4')
		else DisplayText("but small as you are, you can't stop her as she reaches up and grabs onto your " + ButtDescriptor.describeButt(player) + ", then begins bouncing you like a goblin cocksleeve.");
		DisplayText("\n\n");
		...
		DisplayText("\"<i>Birth control,</i>\" she explains. You give her a half-hearted slap across the face and tell her not to be so audacious, but you can see by her smiling expression that she's pleased to have stolen a march on you.");
		player.slimeFeed();
	}
	//[Genderless]
	else {
		...
		DisplayText("She grunts and huffs as you slide down, and you too feel a strain from her iron-hard dick despite the various fluids lubricating it.  But gradually pain turns to pleasure and you're both moaning loudly and calling each others' names as you ride her.");
		player.displayStretchButt(30,true,true,false);
		DisplayText("\n\n");
 
		DisplayText("The shark grits her teeth and gives a roar as she cums, blowing a massive, hot load straight up your " + ButtDescriptor.describeButthole(character.torso.butt) + ", bloating you slightly as she empties her quads inside you.  Your muscles twitch and contract, and you can swear you see stars as she ejaculates.  It takes you a while to catch your breath as you slide off her slowly softening meat pole and crawl onto the sand.");
																																				  
		DisplayText("The two of you get dress after awhile, have some small talk, and then you make your way back to camp.");
		player.slimeFeed();
	}
	player.orgasm();
	player.stats.sens += -1;
...
private izmaLakeDominateContinueVanilla(vanilla:boolean = true) :void
{
	...
	if(player.gender === Gender.MALE) {
		...
		DisplayText(", causing Izma to cry out loudly.");
		//[(big skeet)
		if(player.cumQ() >= 500) DisplayText("  Her belly swells as you empty your impressive load into her.");
		//[(mega), 
		if(player.cumQ() >= 1500) DisplayText("  Eventually it can swell no more and each new squirt forces cum out from her stuffed pussy, trickling past her asshole.");
		DisplayText("  You sigh happily and push back from her, weakly getting to your " + LowerBodyDescriptor.describeFeet(player) + " and redressing. Izma scrambles to her chest and takes out some sort of leaf, then eats it.");
	}
	//(Herm)
	else {
		DisplayText("After a lengthy fuck, you grunt loudly as your " + CockDescriptor.describeCock(player, cockIndex) + " swells, blasting streamers of jizz into Izma's womb");
...
private noWankingForIzmaRadarSaysSo():void {
	...
	MainScreen.simpleChoices(["Stop Her", "LetHerWank", "", "", ""], [noWankingForIzma, letIzmaWankLikeABitch, null, null, null]);
}
//[Let her masturbate]
private letIzmaWankLikeABitch():void {
	...
	//let cockIndex:number = player.cockThatFits(65);
	//if(cockIndex < 0) cockIndex = 0;
	DisplayText("Grinning at her sudden need for penile release, you dart in for an intimate kiss and further inflame her passion, soliciting a series of sated moans as she reciprocates while increasing the speed of her stroking.\n\n");
...
private izmaCumsAfterRadarStuffHere(denied:boolean):void {
	let cockIndex:number = player.cockThatFits(65);
	if(cockIndex < 0) cockIndex = 0;
	if(!denied) DisplayText("The double stimulation pushes Izma past her limit very quickly, and she starts shooting thick jets of spunk into the air, which begin to rain down on her face and breasts.  Her vaginal walls clamp down on your " + CockDescriptor.nounCock(player.torso.cocks.get(cockIndex).type) + " almost painfully as the orgasm wracks her female genitalia too.\n\n");
	//[Remove her hands] 
	else DisplayText("Without another word, or another moment of tormenting impediment, Izma howls with unbridled fury, releasing her hot, sticky load into the air and onto her forehead.  Convulsing with utter euphoria as she finally blows her wad, the walls of her vagina clamp down hard in concert with her feelers, trying ever so valiantly to push you over the edge.  Unfortunately, (for Izma anyway), the unconscious efforts of her body fail to bring you to release. Remarking to Izma that she came so soon, you coo down to her that you are pleased with her ability to speedily heed your commands.\n\n");
...
private submitToLakeIzma():void {
	...
	DisplayText().clear();
	//(starter -all sexes)
	DisplayText("You give Izma a smile and begin to slowly, sensuously remove your " + player.inventory.equipment.armor.displayName + ", littering the sand around you.  As the tigershark watches, you strike a few poses, and then sink onto your hands and knees with your rear facing towards her.  You twist around to look at her over your shoulder, ");
	if(player.hasLongTail() > 0) DisplayText("your tail waving enticingly, ");
	DisplayText("before you give your " + ButtDescriptor.describeButt(player) + " a seductive wiggle.  You tell her that you think it's her turn to play.\n\n");
	...
	//(female)
		...
		if(player.vaginalCapacity() < 26) {
 		...
 		//(If player has ordinary, everyday cunt:)
			...
			DisplayText("\"<i>Ahhhh~  Now that's a nice little hole!  Did you lose on purpose?</i>\" she asks, and you can hear the grin in her voice.  You find yourself wondering that as well...");
		}
	...
	//(herm)
		...
		//(20+ inches)
		else DisplayText("Izma gives a low whistle at the sight of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ".  \"<i>Now THAT'S a cock.  Looks like you've got a third leg down there!</i>");
		DisplayText("\n\n");
		
		DisplayText("Seemingly done appraising you, Izma roughly grabs your " + LowerBodyDescriptor.describeFeet(player) + " and pulls your rear upwards, forcing your weight onto your spine and making you cry out from the uncomfortable position.  She doesn't waste time on foreplay, simply deciding to bury her cock into you slowly, inch by painstaking inch until she's pushing against the entrance to your womb.");
		player.displayStretchVagina(30,true,true,false);
		DisplayText("\n\n");
		
 		DisplayText("She starts thrusting in and out of you, gradually increasing the speed and force, her hot pre-cum and your feminine juices acting like a lubricant to make things easier.  \"<i>Ahh~  You're a lovely cock-sleeve... you like being treated like this, don't ya, weakling?</i>\" Izma taunts, slamming in and out of your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ".  You're not even really ashamed to admit that such is the case anymore.\n\n");
		...
		if(player.pregnancyIncubation === 0) {
			DisplayText("releases your " + LowerBodyDescriptor.describeFeet(player) + ", allowing your " + ButtDescriptor.describeButt(player) + " to hit the sand with a plop, and gets to work redressing while you lie still.  She returns with an anti-pregnancy herb in her hands, and then flips a leaf into her mouth. Before you can wonder why she did that, she bends down and kisses you fiercely, her tongue pushing it past your lips and down your throat.");
		
			DisplayText("\"<i>I'd be happy to make some babies with you... but <b>after</b> you accept me as a mate.\"</i>  She smirks, giving you a second, decidedly unchaste kiss before she helps you up.  You dress yourself and head back to camp feeling very sated.");
		}
		else DisplayText("gives you a decidedly unchaste kiss before she helps you up.  You dress yourself and head back to camp feeling very sated.");
	}
	player.orgasm();
	player.stats.sens += 2;
	player.slimeFeed();
...
public izmaFollowerMenu():void {
	...
	//Izma pops 'em out!
		...
		DisplayText("You call for Izma and she approaches, asking, \"<i>What can I do for my Alpha?</i>\"\n\n");
		//Izma Pregnancy Stages:
		switch (pregnancy.event) {
			case 2: DisplayText("You can't help noticing that Izma seems very nauseous this morning; she's literally green around the gills.  When you investigate, though, she waves you off, insisting that she's fine; she just has a bit of a stomach bug.");
					break;
			case 3: DisplayText("Izma comes up to you, looking concerned.  \"<i>" + player.short + ", do you think I've gained weight?</i>\" she asks.  Looking at her, particularly at the stomach she's holding her hands over, you have to confess that it is starting to bulge out in a noticeable paunch.  At her crestfallen look, you suggest that maybe she's pregnant.  At that, she looks delighted.  \"<i>You really think so?</i>\" she asks, hopefully.  You assure her that you're certain of it; after all, she's very good at watching her weight.  Pleased, she kisses you and then heads off for a swim.");
					break;
			case 4: DisplayText("There is no doubt about it now; Izma's pregnant.  She's grown gravid more rapidly than any of the expecting mothers you remember seeing back in your village, but she seems to be having all of the same aches and pains.  She's grown lethargic and irritable, and complains about not being able to fit into her old clothes.  Still, despite that, she seems happy; she's always rubbing her belly with unmistakable pride.  You also think you've noticed her casting you \"<i>come hither</i>\" looks more frequently than before.");
					break;
			case 5: DisplayText("Izma's started to go around completely naked now.  She insists that she's grown too big to fit into her clothes, but you're kind of skeptical about that; she's as big as the women in your village got when they were near the end, but they managed to fit into their clothes, and theirs were a lot more restrictive than a bikini and grass skirt combo.  Still, you're not objecting to the view it provides, and she definitely seems to enjoy that; you never knew a shark tail could be wiggled in an enticing manner over shapely buttocks until you came to this world...");
					break;
			case 6: DisplayText("Izma's certain the baby will come soon.  You're inclined to believe her; she's huge now.  She still wears no clothing, but there's definitely a practical reason for it");
					if (Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText("- you don't think her skirt would be able to cope with how often she gets erect now, her huge cock rubbing along");
					else DisplayText(", her juices staining");
					DisplayText(" the underside of her swollen belly...  Izma spends much of her time in the water, now; she says it's to soothe her skin.  Given you've seen her explosively cum all over herself from ");
					if (Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText("the friction of her cock against her bulge, you think it has more to do with avoiding being splattered in sexual juices all day long.");
					else DisplayText("the friction of her thighs as she moves, you think it has more to do with avoiding being splattered in sexual juices all day long.");
					break;
			default:
		}
	}
	...
	{
		DisplayText("Izma smiles and puts away her book at your approach.");
...
private sendToFarm():void
{
	...
	DisplayText("You tell your beta that she is to head towards the lake, find a farm, present herself to the lady who works there and do as she says. Izma’s brow furrows as she takes this in.");
...
private backToCamp():void
{
	...
	DisplayText("You tell her to head back to camp; she will be more use to her alpha there.");
...
private gatASharkTooth():void {
	...
	if(Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00246] > 1) {
		DisplayText("Izma smiles apologetically and says, \"<i>I'm sorry, but I won't be able to get my hands on one of those until tomorrow.</i>\"");
		return { next: izmaFollowerMenu };
	}
	else {
		DisplayText("Izma smiles as she pulls a tooth from her chest.  She hands it to you with a pleased expression.  \"<i>Anything for you, my Alpha.</i>\"  ");
...
private izmaPearance():void {
	DisplaySprite(32);
	DisplayText().clear();
	DisplayText("Izma is a 6-foot 2-inch tall tigershark, with a very toned athletic build; her muscles are covered by coarse red sharkskin, marked with black stripes across her body.  She is currently wearing a bikini top with a grass skirt");
	
	if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00235] === 0) DisplayText(" and wielding iron gauntlets as a weapon.");
	else DisplayText(".");
	...
	DisplayText("She has a pair of DD-cup breasts, with a single 0.5 inch nipple on each breast.\n\n");
	...
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) {
		...
		DisplayText("She has a loose fuck-hole placed between her legs, with a 0.2 inch clitoris.  Occasionally, beads of lubricant appear on her cunt, her lips slightly parted.\n\n");
	}
	else DisplayText("In her crotch, Izma has a puffy, impressively wet pussy that frequently dribbles lubricants - enough that if she doesn't take care, it will stain her grass skirt.  The lips are dark red and will frequently part as she becomes aroused, revealing her swollen love-button.  Though she doesn't look loose, her flesh looks soft and pliant enough to spread quite wide.\n\n");

	DisplayText("She has a tight asshole placed between her toned butt-cheeks, right where it belongs.");
	return { next: izmaFollowerMenu };
}
private izmaSexMenu():void {
	...
	if(player.torso.vaginas.count > 0 && Flags.list[FlagEnum.IZMA_NO_COCK] === 0) getVagooFucked = inCampRideIzmasDickDongTheWitchIsDead;
	//69 - non-0 gender
...
private followerIzmaMountsPC(lastHalf:boolean = false):void {
	...
	if(!lastHalf) {
		...
		DisplayText("With a smirk, you make a show of removing your " + player.inventory.equipment.armor.displayName + " and settling yourself down on your back, " + CockDescriptor.describeMultiCockSimpleOne(player) + " pointing into the air excitedly.  You give Izma a coy look and tell her that you're thinking of letting her have control this time... if she thinks she can dominate with her pussy");
		if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText(" instead of her cock");
		DisplayText(".  Izma watches you eagerly and licks her lips.  \"<i>I think that can be arranged,</i>\" she purrs, straddling you and then slowly sliding down onto your erect member.  The tiny tendrils inside her cunt tickle and massage your cock all the while.\n\n");
	
		DisplayText("You groan, lie yourself back down, and give yourself over to your aquatic girlfriend's ministrations; you don't know why she has naughty little tentacles inside her pussy, but you're certainly not going to complain about them.  You can feel them caress and stroke your " + CockDescriptor.describeCock(player, x) + ", eagerly pulling it deeper and deeper into her warm, wet, inviting depths.\n\n");
		...
		DisplayText("It does feel pretty good, and so you settle back down, one hand cautiously lifting up to caress the mark of your lover's bite.  \"<i>Hehe, don't worry; you're not bleeding or anything.  Just marking you as mine,</i>\" she whispers, before giving a loud moan.  ");
		if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText("  Your stomach starts to feel wet, and as you look down you realize her pre is starting to soak into your " + player.skinFurScales() + ".\n\n");
		else DisplayText("  Your stomach starts to feel wet, and as you look down you realize her feminine lubricant is starting to soak into your " + player.skinFurScales() + ".\n\n");
	
		DisplayText("You wonder if you should be more actively moving your hips to her thrusts, but then, this is supposed to be about letting her be in charge.");
		if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) {
			DisplayText("  Still, the sight and sensation of her pre-cum-dribbling cock slapping against your belly gives you an idea.  ");
			DisplayText("You could reach up, grab her meaty member and jerk it off... or you can just enjoy the sex as is.  What will you do?");
			...
			return;
			//DisplayText("You remove your hand from her mark and instead place it squarely on her cock, wrapping your fingers around its two-inch-thick girth.\n\n");
		}
		else DisplayText("  Still, the sight of her curvaceous cleavage bouncing gives you an idea.  You reach up to cup your strong lover's hard bosom, enjoying the sight of her slightly-upturned nipples pointing up at the sky.  You flick them gently as she rides you, sometimes squeezing and tugging encouragingly.\n\n");
	}
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) {
		if(lastHalf) DisplayText().clear();
		DisplayText("You remove your hand from her mark and instead place it squarely on her cock, wrapping your fingers around its two-inch-thick girth.\n\n");
	...
	else DisplayText("Izma bites her lip in response, clearly wanting to enjoy her time on top just a little bit longer.  But the grunts and moans of ecstasy make it clear that the tigershark can't hold back much longer.  Her vaginal walls clamp down hard against your cock.  She squeals loudly and thin streams of her girl-cum run from your belly, the wriggling cilia in her twat going at your dick like crazy.  Her juices run so fast and so freely that it soaks most of your belly, [hips], [butt], and [legs].\n\n");
	

	DisplayText("Even if you hadn't found Izma's impromptu cum bath a bit kinky, your ");
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText("own ");
	DisplayText("cock has been just as teased and tormented - and as her inner walls grip down, her tentacles squeezing as hard as they can, it's too much. You let out a cry of your own as you flood her interior with your spooge");
	//[(if vagina)
	if(player.torso.vaginas.count > 0) {
		DisplayText(", your cunt ");
		if(player.torso.vaginas.get(0).wetness <= VaginaWetness.SLICK) DisplayText("dripping");
		else if(player.torso.vaginas.get(0).wetness <= VaginaWetness.DROOLING) DisplayText("gushing");
		else DisplayText("cascading");
		DisplayText(" feminine fluids onto the sticky ground beneath you both");
	}
	DisplayText("; you were just as eager to release as she was.\n\n");
	izmaPreg();
	//(Standard cum protozoan) 
	if(player.cumQ() < 500) DisplayText("You can feel Izma's womb fill with your cum, and she shudders from the sheer pleasure.");
	//(High cum production) 
	else DisplayText("Izma squeals loudly as you cum inside her, almost having a second orgasm as her belly is bloated and distended by your inhuman amount of cum.");
	DisplayText("  Once you've finished, Izma rolls off you and continues breathing heavily.  \"<i>That was amazing... my Alpha.</i>\"\n\n");
...
private izmaPreg():void {
	...
	//PREGGO CHANCES
	if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00249] === 1 && !pregnancy.isPregnant && randInt(100) < (10 + Math.round(player.cumQ() / 100))) {
...
private followerIzmaTakesItInPooper():void {
	...
	player.slimeFeed();
	DisplayText().clear();
	DisplayText("When you tell Izma you want her to screw you this time, she exclaims, \"<i>Ooh! It's my turn to be alpha!</i>\"  Her look of deviant joy fades into a blush of fondness for a moment, and she holds you and whispers into your ear, \"<i>It's really nice that you let me take charge, too.  Thanks for thinking of it.</i>\"\n\n");

	DisplayText("You're about to reply when she grabs your wrists and, holding them behind your back, ");
	//[[if [player height>6' or player is centaur] 
	if(player.tallness > 72 || player.torso.hips.legs.isTaur()) {
		DisplayText("pulls you down to kiss you fiercely");
	}
	//[else if player<4.5'] 
	else if(player.tallness < 54) DisplayText("pulls you up off the ground in a bear hug to kiss you fiercely");
	//[else] 
	else DisplayText("pulls you in to her body and kisses you fiercely");
	DisplayText(". She lets her shark teeth graze your lips, and you taste blood.\n\n");

	DisplayText("You start to kiss her back, but she's still in charge, and seems intent on screwing your mouth with her tongue. You feel her stiffening tool ");
	//[[if player has legs] 
	if(player.torso.hips.legs.isBiped() || player.torso.hips.legs.isTaur()) DisplayText("between your legs");
	//[else] 
	else DisplayText("against your lower body");
	DisplayText(", and give up on fighting. Each time her tongue thrusts into your mouth, you try to suck it a little, or flick it with your own tongue.\n\n");

	DisplayText("Izma gives a happy moan as she feels you relax into her large breasts, and a satisfied chuckle when you start fellating her tongue.  Pulling back from your panting mouth, she pushes you down to the ground and presents you with her cock.\n\n");

	DisplayText("\"<i>Get it nice and slippery, then, if you're so eager.</i>\"\n\n");

	DisplayText("You start kissing along her fifteen-inch length, but Izma's in more of a hurry than that, because she grabs your " + HeadDescriptor.describeHair(player) + " and pulls you back to the tip.  With her smooth glans resting at your lips, you look up at her. She grins, showing all her teeth, and starts pushing your head down.\n\n");
	
	DisplayText("As her cock slides past your lips and down your throat, it takes a lot of effort not to gag, but you feel proud and happy when she's the first one to look away, tilting her head back and grunting in pleasure as she bottoms out in your mouth, her balls tapping against your chin.\n\n");
	...
	//[(if player has penis)
	if(player.torso.cocks.count > 0) {
		DisplayText("You expect to feel her penetrate you when she thrusts forward, but instead she runs her cock along the surface of your asshole");
		//[(if player has balls)
		if(player.torso.balls.quantity > 0) DisplayText(" and " + BallsDescriptor.describeSack(player));
		DisplayText(" until her dark-red cock is lined up with your " + CockDescriptor.describeMultiCockShort(player) + ".\n\n");
	
		DisplayText("She takes your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " and presses it against her own, holding both of them together in one hand and rubbing slightly.  Her other hand reaches down below, to rub her double set of balls");
		//[(if player has balls)
		if(player.torso.balls.quantity > 0) DisplayText(" against your own " + BallsDescriptor.describeBalls(true, true, player));
		//(if no balls and vag)
		else if(player.torso.vaginas.count > 0) DisplayText(" against your already-dripping " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)));
		//(else)
		else DisplayText(" against your perineum");
		DisplayText(", humiliating you more than she stimulates you.\n\n");

		//[if dick0 is <14\"] 
		if(player.torso.cocks.get(0).length < 14) DisplayText("\"<i>Hmm...</i>\" she sighs, as she looks at her glans sticking out past the tip of your dick.  \"<i>It's nice to know who's in charge.</i>\"");
		//[dick0 >14 but <16] 
		else if(player.torso.cocks.get(0).length < 16) DisplayText("\"<i>It's nice that we keep things fair around here,</i>\" murmurs Izma, smiling at your similarly-sized cocks.");
		//[If dick0 >16]
		else DisplayText("\"<i>Oh fuck,</i>\" she says, her eyes filling with lust as she sees your cock extending well past her own.  She tilts forward and rubs the base of her cock hard against the base of your " + CockDescriptor.nounCock(player.torso.cocks.get(0).type) + ", jerking the both of you off together.  You moan helplessly when she stops, and she sighs, \"<i>It feels so good to be alpha over a powerful cock like yours.</i>\"");
		DisplayText("\n\n");
	}
	
	DisplayText("She slowly moves her hips away and lifts your ass up off the ground, letting her head trail along your body until it drops into place at your anus.  ");
	//[[if player is anal virgin]
	if(player.torso.butt.looseness === 0) {
		DisplayText("You gasp in pain as she starts pushing her giant cock against your tightly-puckered ass, whimper as your sphincter gives way and, with a pop, her cock head is resting inside of you.  Izma looks at you with a confused face, and then seems to realize what just happened.\n\n");
		DisplayText("\"<i>You... let me be the first to take you? You're sweeter than I thought!</i>\"  She strokes your body with surprising gentleness as she eases herself the rest of the way into you.  Each inch is easier than the last, and by the fifteenth inch you're squirming and ready for more.");
	}
	//[else if player has tight anus]
	else if(player.analCapacity() <= 26) {
		DisplayText("You moan as Izma's cockhead pushes past your tight sphincter.  \"<i>That's it, you beta slut,</i>\" she says as she slides the rest of her cock into you.  \"<i>Moan for your alpha.</i>\"");
	}
	//[else if player has loose anus]
	else {
		DisplayText("Izma starts to push and, meeting no resistance from your well-trained ass, slams her hips against yours with a groan of satisfaction.\n\n");
	}
	player.displayStretchButt(30,true,true,false);
	DisplayText("\n\n");

	DisplayText("With her hips securely pressed against yours, she leans over you to grab your wrists and hold them to the ground, her elbows hooked");
	if (player.torso.hips.legs.isBiped()) DisplayText(" behind your knees to immobilize you");
	else DisplayText(" around your [legs] to immobilize you");
	DisplayText(" with your ass in the air.  Her cock feels warm inside of you, and you could swear she's making it throb deliberately, just to feel you twitch in response.\n\n");

	DisplayText("She starts to screw you deliberately, pulling out just short of all the way before thrusting back in with practiced expertise.  You're lost in the pleasure of being used so roughly, and can't remotely keep track of how long she screws you.");
	//[if player has penis: 
	if(player.torso.cocks.count > 0) DisplayText("  You try to reach for your cock and get off, but her hands keep your wrists securely pinned to the ground.");
	DisplayText("\n\n");

	DisplayText("Her thrusts become shorter, faster, and wilder, and you see her panting and focusing.  Reaching forward with your mouth, you take one of her dangling nipples in your mouth and start suckling and nibbling.  Her half-closed eyes flash open and she slams into you with a shocked expression.\n\n");

	DisplayText("She yells as you bite down on her nipple, and you're rewarded with hot spurts of cum that fill your belly as Izma's quad balls empty into you.  ");
	//[if player has no penis)
	if(player.torso.cocks.count <= 0) DisplayText("The feeling of her cock throbbing finally puts you over the edge and you orgasm, your tight pucker helping milk the last of her cum into you.\n\n");
	//(if player has penis)
	else {
		DisplayText("She collapses onto you, spent, and you close your eyes and relish the feeling of her hot cock pulsing in your ass and her warm belly lying against your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ".  When she sits back up, she gives you a stern look as she lets go of your wrists, making it clear that they should stay right where they are.  You look at her pleadingly as she trails her fingers along your " + CockDescriptor.nounCock(player.torso.cocks.get(0).type) + ".\n\n");
		//[(if penis 0 length <14 inches)
		if(player.torso.cocks.get(0).length < 14) {
			DisplayText("Izma grabs your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)));
			if(player.torso.cocks.count > 1) DisplayText(" first, then your other " + CockDescriptor.describeCock(player, 1) + ", synchronizing her movements");
			DisplayText(", and starts jerking you off.  It feels incredible, and when she says, \"<i>You've been a good beta for me, but now I want to see you come,</i>\" you can't help but obey; you orgasm messily onto your own " + Desc.Breast.describeChest(character));
			//(huge cum)
			if(player.cumQ() >= 1500) DisplayText(" over and over again until you're a sticky mess lying in a pool of your own semen.");
			//[(big cum)
			else if(player.cumQ() >= 500) DisplayText(" until cum pours off of your body and pools on the ground.");
			else DisplayText(".");
			DisplayText("\n\n");
		}
		//(else if penis 0 length <16 inches)
		else if(player.torso.cocks.get(0).length < 16) {
			DisplayText("When Izma guides " + CockDescriptor.describeMultiCockSimpleOne(player) + " between her breasts, you're not sure what you're feeling at first; smooth in one direction, rough in the other direction.  Pushing her breasts tight around your " + CockDescriptor.describeMultiCockShort(player) + ", she starts to move up and down.  The contrast quickly brings you over the edge and you cum all over her face and breasts, ");
			//[(normal cum)
			if(player.cumQ() < 300) DisplayText("leaving her looking spattered but pleased");
			else if(player.cumQ() < 500) DisplayText("leaving her looking wet but impressed");
			else DisplayText("showering her in so much cum that she leans down to lick it off of her breasts");
			DisplayText(".\n\n");
		}
		//[(else if penis 0 length >15 inches)
		else {
			DisplayText("Your sigh of relief soon becomes a moan of pleasure when you feel Izma take the head of your " + CockDescriptor.nounCock(player.torso.cocks.get(0).type) + " into her mouth.  Despite the hidden presence of her ring of sharp teeth, you feel nothing but bliss as she sucks, her own cock still buried deep inside of you.  She nestles the shaft of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " between her breasts and starts moving her upper body up and down against your cock.  You're startled to feel that her breasts are smooth in one direction and rough in the other, and the alternating sensations soon overwhelm you.  You manage a shuddering yell to warn Izma before you start pumping cum into her mouth");
			if(player.torso.cocks.count > 1) DisplayText(" and onto your combined bodies");
			DisplayText(".\n\n");
			
			//<1 L] 
			if(player.cumQ() < 1000) DisplayText("She swallows dutifully and, when your cock has stopped throbbing between her breasts, takes her mouth off of you, licks her lips, and smiles, showing so many teeth that your cock shrinks a little bit faster than normal.\n\n");
			//[<5 L] 
			else if(player.cumQ() < 3000) DisplayText("She swallows dutifully at first, and when you keep pumping her with cum, gives up on swallowing. Instead, she grabs you by the hips and, by leaning down and pulling you up, gets your cock down her throat so you can deliver your seed directly.  It looks difficult but feels amazing, and when you're done and she pulls her head back, she looks very proud of herself.\n\n");
			//[>5 L] 
			else DisplayText("When your first enormous load hits her mouth, she splutters, and some leaks out, but Izma refuses to be defeated.  Instead, she grabs you by the hips and, by leaning down and pulling you up, gets your cock down her throat so you can deliver your seed directly.  It looks difficult but feels amazing, and you gladly keep filling her until a look of mild alarm appears on her face.  Her belly is swelling and she needs air.  Finally she pulls back with a gasp and, with a look of admiration, leans back and has you empty the rest onto her breasts, occasionally putting her mouth back on the tip to sample a load.  When you're done, she flops backward into the pool of cum you've created.\n\n");
		}
	}
	//(10; resume here for dickless)
	if(player.torso.cocks.count <= 0) DisplayText("After a few minutes, she pulls her shrinking cock out of you and gives you a deep kiss. \"<i>Thanks for that,</i>\" she says, \"<i>...my Alpha.</i>\"  She jogs off to the river to clean off.  After a minute you follow her, with her cum dripping out of your well-used anus.");
	player.orgasm();
	player.stats.sens += -1;
	return { next: Scenes.camp.returnToCampUseOneHour };
}
...
private followerIzmaTakesItInVagoo():void {
	...
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) player.slimeFeed();
	DisplayText().clear();
	DisplayText("Admiring the view of the tigershark's ornamented crotch, you lower the bottoms of your " + player.inventory.equipment.armor.displayName + " and reveal the base of your " + CockDescriptor.describeMultiCockShort(player) + ", something that instantly draws Izma's eyes.  You ask how she'd like her alpha to tend to her oft-neglected pussy.  Izma smiles eagerly and spreads her legs");
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText(", gently lifting her meaty quads up so");
	else DisplayText(", gently parting her muscled thighs so");
	DisplayText(" you can better see her damp nether-lips.  \"<i>Okay lover... if anyone knows how to make me feel good down there, it's you.</i>\"\n\n");

	DisplayText("You give her a lecherous grin back and stalk towards her, watching as she slowly seats herself on her bedroll and lies back for you; one hand playing with her breasts, ");
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText("the other holding her maleness out of the way, teasing her cock with her thumb and flexing her fingers to roll her four balls around in the palm of her hand.");
	else DisplayText("the other slowly spreading the folds of her vulva, teasing her clit with her thumb and flexing her interior muscles to make the pink opening wink at you.");
	DisplayText("  You shed your own " + player.inventory.equipment.armor.displayName + " and strike a pose for her. Izma watches you closely for a few moments, before growling, \"<i>Oh stop toying around and fuck me already!</i>\" She gives a gasp and covers her mouth. \"<i>O-oh, sorry... I was out of line talking to my Alpha like that!</i>\"\n\n");

	DisplayText("Sauntering over, you settle yourself on your elbows and knees before her.  Reaching out with one hand, you start to stroke her pussy, feeling its intense heat and letting her juices squelch and ooze between your fingers.  Casually, nonchalantly, you tell Izma that she was out of line for talking to you like that, and suggest that maybe you shouldn't put anything into her, if that's the mood she's in.\n\n");

	DisplayText("Izma moans and writhes slightly as you rub her cunt.  \"<i>N-noooo,</i>\" she whines, looking at you with puppy-dog eyes.  \"<i>Please, I need you...</i>\"  You enjoy her struggles for a little while, then give her your wickedest grin.  Well, if that's the way she feels about it...  You surge forward, rising up between her legs until your pelvises are positioned properly.  You position your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " against her labia and then slide it home, enveloping it in her hot, wet depths.  She moans loudly and goes limp, gasping and panting from the pleasure.  The strange tendrils in her cunt waste no time massaging and teasing your cock");
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText(", while Izma herself reaches around to jerk herself off.\n\n");
	else DisplayText(", while Izma herself reaches up to massage her heavy breasts.\n\n");

	DisplayText("You wonder for a second if maybe it would be more dominant to stop her from pleasuring herself");
	//RADAR XPACK FORK
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) {
		DisplayText(".");
		MainScreen.simpleChoices(["Let Her", "Deny Her", "", "", ""], [radarIzmaXPackLetHer, radarIzmaXpackDenyHer, null, null, null]);
		return;
	}
	...
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText(", and gains speed as she jerks her meat pole");
	DisplayText(".\n\n");
	
	DisplayText("Seeing that she is speeding up, you accelerate your ");
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText("own ");
	DisplayText("thrusts, trying to match her pace.  The tendrils inside her vagina writhe and flex, an experience unlike anything you've had before - you don't think any creature other than a succubus could give you this sort of pleasure!\n\n");

	DisplayText("Izma continues to moan and groan loudly, and starts to mash her own hips against yours, trying to work in as many inches of your " + CockDescriptor.nounCock(player.torso.cocks.get(0).type) + " as she can.  ");
	...
	else DisplayText("As she fucks you, you can see long streams of girlish-goo dribbling from her pussy, and you know full well she's close to her limit.  Unable to resist, even as you feel that familiar pressure signaling the approach of your own climax, you bend upward to plant a kiss on one of Izma's hard nipples, immediately switching to a brief, gentle suckle.\n\n");

	DisplayText("This sends a shudder through Izma's whole body.  She cries loudly as she orgasms");
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText(", a hot jet of spunk going straight into your mouth, and several subsequent shots hitting you in the lips and face.  ");
	else DisplayText(", a small explosion of fem-cum splattering your belly, and several subsequent squirts dripping down your thighs.  ");
	DisplayText("Her vagina clamps down on your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", making you grunt as the feelers in her pussy milk you for all you've got. You groan deep in your throat, the scent of Izma's hot ");
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 1) DisplayText("girl-");
	DisplayText("cum filling your nostrils and the feeling of her milking, hungry feelers is too much.  You cum, blasting everything you have deep into Izma's cunt.  ");
	//[(Normal cum production)
	if(player.cumQ() < 750) DisplayText("You pump straight into her womb, enough to make a trickle of your baby batter dribble from her freshly used cunt when you pull out.");
	//(High cum production)
	else DisplayText("Your load is so vast that it bloats and distends her belly, almost giving her a second orgasm from the sensations.");
	DisplayText("\n\n");

	DisplayText("The two of you roll apart, panting and gasping for breath.  \"<i>Mmm, that was amazing,</i>\" Izma says, giving you a long drawn-out kiss and ");
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText("licking some of her seed off of your face.  \"<i>Come fill me whenever you need to.</i>\"  You grin at Izma and order her to eat up every drop.\n\n");
	else DisplayText("licking you face affectionately.  \"<i>Come fill me whenever you need to.</i>\"  You grin back at Izma knowingly.\n\n");

	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText("She giggles and runs her tongue over the mess she's made; her tongue lingers on your lips for a few moments before she gives you another long kiss and lets you go.");
	 else DisplayText("She giggles and runs her tongue over your lips, her tongue lingering for a few moments before she gives you another long kiss and lets you go.");
	//PREGGO CHANCES
	izmaPreg();
	player.orgasm();
	player.stats.sens += -2;
...
private radarIzmaXPackLetHer():void {
	...
	DisplayText("Seeing that she is speeding up, you accelerate your own thrusts, trying to match her pace.  The tendrils inside her vagina writhe and flex, an experience unlike anything you've had before - you don't think any creature other than a succubus could give you this sort of pleasure!");
	//(leads to Where's it going?)
	wheresItGoing();
}
private wheresItGoing(denied:boolean = false):void {
	DisplaySprite(32);
	DisplayText("\n\nIzma continues to moan and groan loudly, and starts to mash her own hips against yours, trying to work in as many inches of your " + CockDescriptor.nounCock(player.torso.cocks.get(0).type) + " as she can.  As she ");
	if(!denied) DisplayText("jerks off ");
	else DisplayText("moans fiercely ");
	DisplayText("you can see long streams of pre dribble down her red cock, and know full well she's close to her limit.  Her cock stands at attention, ready to release its sticky load in your direction.");
	DisplayText("\n\nIt seems Izma cannot hold out much longer... where will you direct her orgasm?");
	//[Your face]   [Your chest]   [Her face]
	MainScreen.simpleChoices(["Your Face", "Your Chest", "Her Face", "", ""], [facialWhereItGoesRadarIzmaXpack, RadarIzmaCumInYourChest, IzmaSelfFacialWheeRadar, null, null]);
}
...
private facialWhereItGoesRadarIzmaXpack():void {
	...
	DisplayText().clear();
	DisplayText("This sends a shudder through Izma's whole body.  She cries loudly as she orgasms");
	DisplayText(", a hot jet of spunk going straight into your mouth, and several subsequent shots hitting you in the lips and face.  ");
	DisplayText("Her vagina clamps down on your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", making you grunt as the feelers in her pussy milk you for all you've got. You groan deep in your throat, the scent of Izma's hot ");
	DisplayText("cum filling your nostrils and the feeling of her milking, hungry feelers is too much.  You cum, blasting everything you have deep into Izma's cunt.  ");
	//[(Normal cum production)
	if(player.cumQ() < 750) DisplayText("You pump straight into her womb, enough to make a trickle of your baby batter dribble from her freshly used cunt when you pull out.");
	...
	DisplayText("The two of you roll apart, panting and gasping for breath.  \"<i>Mmm, that was amazing,</i>\" Izma says, giving you a long drawn-out kiss and ");
	DisplayText("licking some of her seed off of your face.  \"<i>Come fill me whenever you need to.</i>\"  You grin at Izma and order her to eat up every drop.\n\n");

	DisplayText("She giggles and runs her tongue over the mess she's made; her tongue lingers on your lips for a few moments before she gives you another long kiss and lets you go.");
	//PREGGO CHANCES
	izmaPreg();
	player.orgasm();
	player.stats.sens += -2;
...
private RadarIzmaCumInYourChest():void {
	...
	if(player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 1) DisplayText("in between your " +  Desc.Breast.describeChest(character) + " and coats ");
	DisplayText("against your chest");
	if(player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating < 1) DisplayText(", coating it");
	...
	//[(Normal cum production)
	if(player.cumQ() < 750) DisplayText("liberally lubricating her fuckhole with steamy cum and causing your beta to cry out in blissful pleasure.");

	//(High cum production)
	else DisplayText("flooding her love canal with so much sperm that a slew of wet, sloppy noises ring out from her pussy.  Izma's expression betrays a sense of utter disbelief as you stuff her full of your spooge; which begins to flow out of her at a steady rate as each contraction of your dick's muscles fires more and more into her.");
...
private IzmaSelfFacialWheeRadar():void {
	...
	DisplayText("\n\nYour own seed ");
	//[(Normal cum production)
	if(player.cumQ() < 750) DisplayText("liberally lubricates her fuckhole with steamy cum and causes your luscious beta to cry out in blissful pleasure.");
	//(High cum production)
	else DisplayText("floods her love canal with so much sperm that a slew of wet, sloppy noises ring out from her pussy.  Izma's expression betrays a sense of utter disbelief as you stuff her full of your spunk, some of which begins to flow out of her at a steady rate, each contraction of your dick's muscles firing more and more into her.");
	...
	DisplayText("When she concludes with cleaning her Alpha, she takes a few moments to show you her appreciation for mating with her, kissing your chest and working her way up to your neck.");
	//PREGGO CHANCES
	izmaPreg();
	player.orgasm();
	player.stats.sens += -2;
...
private followerIzmaTakesIt69():void {
	...
	player.slimeFeed();
	DisplayText().clear();
	DisplayText("Izma smiles and nods, swinging her hips from side to side as she makes her way over to you, as if to tease you further. She positions her head at your groin, rubbing her own crotch against your head.  \"<i>Alright, let's do this,</i>\" Izma says happily, and wastes no time ");
	//[(Male/herm)
	if(player.torso.cocks.count > 0) {
		DisplayText("gorging herself on your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", her tongue wrapping around it and caressing it like jewelry");
		//[(if herm)
		if(player.torso.vaginas.count > 0) DisplayText(" while pushing two fingers into your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)));
	}
	//(Female)
	else if(player.torso.vaginas.count > 0) {
		DisplayText("licking and eating you out, paying special attention to your " + VaginaDescriptor.describeClit(player));
	}
	DisplayText(".\n\n");
	...
	else DisplayText("You wrap your lips around Izma's puffy, inhuman lower lips, sucking and swallowing her fragrant fem-juices as she jerks and twists her hips around.  Her muffled moans seem to assure you that you're doing a great job, and Izma responds by increasing the speed of her head-bobbing.  Your vision is a little obscured by Izma's sinuous tail, but if anything this serves to arouse you further.\n\n");
	...
	DisplayText(" with her saliva, and as you're so eager for her magic tongue you start thrusting your hips up ");
	if(player.torso.cocks.count > 0) DisplayText("to reach ");
	else DisplayText("to push your " + VaginaDescriptor.describeClit(player));
	DisplayText(" as far into her mouth as you can.\n\n");
 
	if(player.torso.cocks.count > 0) DisplayText("Eventually, the two of you manage to bring each other to a powerful simultaneous orgasm, Izma's hot cum shoots down your eager throat, and you ");
	else DisplayText("Eventually, the two of you manage to bring each other to a powerful simultaneous orgasm, Izma's slippery cum drenching your face, and you ");
	if(player.torso.cocks.count > 0) {
		DisplayText(" fire loads into Izma's cheeks");
		//[(multi)
		if(player.torso.cocks.count > 1) DisplayText(" and hair");
		DisplayText(".");
		//[(big/mega skeet)
		if(player.cumQ() >= 750) DisplayText("  You continue to shoot off stroke after stroke long after her mouth has filled up and she's pulled away.  \"<i>For the love of Marae, " + player.short + ", I can't swallow all that!</i>\"");
	}
	else if(player.torso.vaginas.count > 0) DisplayText("drool a sticky mess over Izma's cheeks.");
	DisplayText("  The two of you roll away from each other, and while you catch your breath you're surprised at the strong taste of Izma's cum.  You don't think you'd mind getting another load of that wonderful ");
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 1) DisplayText("lady-");
	DisplayText("spunk in your stomach...\n\n");
...
private talkWivIzma():void {
	...
	if(Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00239] === 0) {
		DisplayText("You take a seat beside Izma on the rocks, and smile warmly at her.  She returns the smile, though she seems a little flustered and confused by the affection.  \"<i>What's on your mind?</i>\" she asks, focusing on you.  You shrug and ask her to tell a bit about herself.\n\n");
		...
		DisplayText("She heaves a sigh, and you place a hand on her shoulder, bringing a small blush to her face.  \"<i>Thanks, but I don't mind. Really.</i>\"  She gives you a quick kiss. \"<i>Thanks for listening.</i>\"\n\n");
	...
	//[Talk option 2]
		...
		DisplayText("You take a seat beside Izma on the rocks, and smile warmly at her.  She returns the smile, though she seems a little flustered and confused by the affection.  \"<i>What's on your mind?</i>\" she asks, focusing on you.  You shrug and ask her to talk a bit.\n\n");
		...
		DisplayText("Curious, you ask if Izma has any personal encounters with either race.\n\n");
		...
		DisplayText("You thank Izma for being willing to talk to you, then you politely excuse yourself and leave her to her thoughts.");
		//(Slight lust gain)
		player.stats.lust += 5;
		Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00239]++;
		return { next: Scenes.camp.returnToCampUseOneHour };
	...
	//[Talk option 3]
		...
		DisplayText("\"<i>Um... well, whatever food you have in camp, of course.  Or anything that swims upstream,</i>\" she replies.  A part of you feels frustrated at Izma's evasive answers. Changing tack, you ask if she eats only fish or other kinds of meat as well?\n\n");

		DisplayText("\"<i>I'm not really picky.</i>\"  She shrugs languorously.  \"<i>It's just instinct - you catch food, you eat it before something comes along and takes it off you... are you feeling okay?  Why are you asking such weird questions?</i>\"  You take a deep breath and let it out slowly; then ask: has she ever eaten a person?\n\n");
		...
		DisplayText("\"<i>Ah... I guess we don't have a good reputation,</i>\" she says, misinterpreting your expression and smiling with tight lips. You give her a quick kiss... on the cheek.  Morbid curiosity forces out the question hanging in the air; just what do they taste like?\n\n");
	...
	//[Talk topic 4.]
		...
		DisplayText("You thank her for telling you such personal things.  As you get ready to walk away, though, you stop and turn back to her. She mentioned wanting kids with 'someone special'.  Out of curiosity, you ask what sort of family she wants for herself.\n\n");
		...
		if(Flags.list[FlagEnum.IZMA_NO_COCK] === 1) DisplayText("she used to have a ");
		else DisplayText("she's got a ");
		DisplayText("perfectly functional set of male genitalia.\n\n");
		...
		else DisplayText("\"<i>Oh, nah.  Pfft, as if that's a challenge,</i>\" she says.  It's interesting to see how Izma, as a hermaphrodite, thinks of herself when it comes to the most basic of gender roles.  She scratches her chin in thought.  \"<i>Well, sexually, I was always more accustomed to my penis, since it was a bit easier to dominate with and the lake life seemed to fixate on it.  For you though, I'll do as you command, my Alpha.</i>\"\n\n");
		DisplayText("You could just leave it at this, or you could try flirting with Izma - you might get more than you bargained for if you did that, though.  Which will you do?");
		
		Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00239]++;
		//[Leave] [Flirt]
		MainScreen.simpleChoices(["Flirt", "", "", "", "Leave"], [chooseToFlirtWithIzma, null, null, null, chooseNotToFlirtWithIzma]);
		return;
	}
	//[Talk option 5]
		...
		DisplayText("When you realize Izma is staring at you, waiting patiently, you can't resist asking her why she is so... well, so sedate compared to the other shark-girls.\n\n");
		...
		DisplayText("She blushes bright red and hugs herself close to you.  \"<i>You're so nice to me, my Alpha...</i>\"  You wrap her back, enjoying her tender embrace.");
		if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText("..  and patiently ignoring the fact you can feel her cock starting to stiffen against your thigh.");
		DisplayText("  You release her, trying to avoid turning her on too much.  Thanking her for the talk, you leave as she buries herself back into her books.");
		Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00239]++;
		return { next: Scenes.camp.returnToCampUseOneHour };
		return;
	}
	//[Talk Option 6]
		...
		DisplayText("You tell Izma you're curious; you've seen shark-girls before, but you've never seen one quite like her before. Are there a lot other shark herms like her?\n\n");
		...
		DisplayText("\"<i>Um...</i>\" Izma says, rubbing her temples as she tries to remember exactly what happened.  \"<i>When I was there... yeah, there were some men and some women who underwent the change.  We were all shocked but... well, the endowments were certainly pleasing.</i>\"\n\n");

		DisplayText("So, both genders became herms.  The obvious question fills the space between you.  Was Izma originally a boy?  Or a girl?\n\n");
		...
		DisplayText("\"<i>I'm not all that sure on the nature of the chemicals, but I think they're designed expressly to make people into herms,</i>\" she replies.  \"<i>The more dilute chemicals just affected peoples' minds and genitalia.</i>\"\n\n");

		DisplayText("Puzzled, you ask her; then why do herms seem to be so rare?  You haven't seen any hermaphroditic sharks other than Izma herself; all of the other shark people you've met have been girls.  Doesn't that mean that most of the men were turned into shark-girls?\n\n");
...
private chooseNotToFlirtWithIzma():void {
	DisplaySprite(32);
	DisplayText().clear();
	DisplayText("You thank Izma for the interesting conversation, even if it did end up meandering a bit, and then walk away, leaving her to her private thoughts.\n\n");
	return { next: Scenes.camp.returnToCampUseOneHour };
}
//Flirt]
private chooseToFlirtWithIzma():void {
	DisplaySprite(32);
	DisplayText().clear();
	DisplayText("You smile at Izma and tell her you appreciate the offer and that you think she'll be beautiful whether she becomes a mommy or a daddy.  In fact, the mental image of her heavily pregnant, ");
	if (Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText("her huge cock barely able to poke past her baby-filled belly, ");
	DisplayText("her DD-cups full of milk, watching as she waddles around the camp, is a very attractive one.  She smirks at you .  \"<i> You're such a perv.</i>\"");
	
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText("  But she glances between her legs and sees her massive cock poking out of her skirt.");
	else DisplayText("  But she glances down to behold her rock-hard nipples jutting out.");
	
	DisplayText("  \"<i>Though, you do paint a rather vivid image with words...</i>\"\n\n");

	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) {
		DisplayText("Smiling back, you ask about her mental image of you being pregnant with her babies.  \"<i>Um...</i>\"  She trails off.");
		//[(If player is male/genderless)
		if(player.gender <= 1) {
			DisplayText("  \"<i>That's kinda weird, man... I'm not sure the god in charge of this world would allow something like that.  It seems too silly.</i>\"");
			return { next: Scenes.camp.returnToCampUseOneHour };
			return;
		}
		///(Female/herm)
		else DisplayText("  \"<i>Oh... you'd be so big and curvy, so sexy,</i>\" she purrs.  You advance towards her.  Leaning in, you mention you're willing to try and make a baby, right here and now.  \"<i>I... oh, wow.  We could certainly try,</i>\" she replies, grabbing your neck and pulling you in for a kiss.");
	}
	izmaSexMenu();
}
...
private childToggle():void {
	...
	if(Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00248] === 0) {
		DisplayText("You ask Izma if she's ever thought about the two of you having children together.\n\n");
	 
		DisplayText("Izma looks a little sheepish, but then her expression settles into a determined one.  \"<i>Yes, I have.  I'd like us to start a family, but if you don't think you're ready yet, I'll keep taking my herbs.</i>\"\n\n");
	 
		DisplayText("Do you tell her to stop taking her contraceptives?");
	...
	//[=Yes=] (TURN ON KID-MODE
		...
		if(Flags.list[FlagEnum.IZMA_CHILDREN_SHARKGIRLS] + Flags.list[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] > 0) {
			...
			DisplayText("You give Izma a knowing look and tell her that you're ready to start having children - if she is.\n\n");
...
private firstTimeDeclineIzmasKdiddlezlijfhdjkfhslkfbsjkhfbs():void {
	DisplaySprite(32);
	DisplayText().clear();
	DisplayText("You tell Izma, with a shake of your head, that you aren't ready to be a parent yet.  The tigershark looks a little disappointed, but she gives a soft sigh and nods in understanding.  \"<i>You're the boss.  So, what did you want to do?</i>\" she asks.\n\n");
...
private IzmaPoopsBabies():void {
	...
	DisplayText("\"<i>Here we go, huh?</i>\"  She tries to laugh.  That's Izma for you; always trying to be brave.\n\n");
	...
	DisplayText("\"<i>Fondle me,</i>\" Izma suddenly says.  As you look at her in surprise, she nods vaguely in the direction of her crotch. \"<i>Please, start ");
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText("jerking me off or ");
	DisplayText("playing with my cunt; we learned it helps with the pain of birth a long time ago.</i>\"\n\n");
 
	DisplayText("Nodding your head, you reach around with one hand.  ");
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText("Divvying your time between her raging erection, quartet of sizable balls and dilating cunt while she strains under you is an awkward experience, but you persevere; when Izma stops hissing and gasping in pain and starts instead moaning lightly, it seems more worthwhile.\n\n");
	else DisplayText("Divvying your time between her dilating cunt and engorged clit while she strains under you is an awkward experience, but you persevere.  When Izma stops hissing and gasping in pain and starts instead moaning lightly, it seems more worthwhile.\n\n");
	...
	DisplayText("Quickly you take hold of its hand, then help Izma take it up into her arms once she's regained her strength.  You are now the proud parents of a baby ");
	if(randInt(100) <= 59) {
		DisplayText("shark-girl");
		Flags.list[FlagEnum.IZMA_CHILDREN_SHARKGIRLS]++;
	}
	else {
		DisplayText("tigershark");
	...
	DisplayText("You help Izma climb back onto shore with your new baby, then lay them both down to rest.  Once they're settled down comfortably, cuddling up against each other, you turn and head back to camp, looking to dry yourself off and catch a quick nap.");
	return { next: Scenes.camp.returnToCampUseOneHour };
}
...
public pcPopsOutASharkTot():void {
	...
	if(Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00238] != 1) {
		...
		player.displayStretchVagina(100,true,true,false);
		DisplayText("\n\n");

		DisplayText("Finally, she's out; you quiver weakly as she crawls over to you and it takes all your effort to lift your " + player.inventory.equipment.armor.displayName + " and expose your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + "s to her.  As she sucks greedily at them, the haze begins to clear from your vision.  For the first time, you can actually make out distinct features on your new daughter; she's a ");
		if(randInt(100) <= 59) {
			DisplayText("shark-girl");
			Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00412]++;
		}
		else DisplayText("tigershark");
		DisplayText(", quickly growing and filling out as she takes in milk.  She finishes up, looking rather like a pre-teen already, and glances around nervously.  The amniotic fluid is evaporating rapidly, and she's scratching idly at her gills as more and more of her skin is exposed directly to the dry air.\n\n");
		...
		DisplayText("After making a quiet wish for your child, you slip into a doze.\n");
		...
		DisplayText("You wake up suddenly to strong pains and pressures in your gut.  As your eyes shoot wide open, you look down to see your belly absurdly full and distended.  ");
		if(player.torso.vaginas.count === 0) {
			DisplayText("You feel a terrible pressure in your groin... then an incredible pain accompanied by the rending of flesh.  You look down and behold a vagina.  ");
			player.createVagina();
			player.updateGender();
		...
		DisplayText("You cry out as the first wrenching labor pain begins in earnest, unthinkingly hoping for somebody to come and help you.  To your relief, Izma comes running over to you from out of the gloom.  \"<i>" + player.short + "!  You've gone into labor?</i>\" she asks; something of a stupid question, but she's evidently as off-guard as you are.\n\n");
 
		DisplayText("\"<i>Do you think you can walk?  Make it to the stream?</i>\" she asks.\n\n");
 
		DisplayText("You throw her a dirty look, and make it quite emphatically clear that with how strong these contractions are, you don't think you could stand, never mind walk that way.\n\n");
 
		DisplayText("\"<i>Right, sorry.</i>\" She has the courtesy to look chagrined, at least.\n\n");
 
		DisplayText("She unhesitatingly steps up and kneels before you, one hand reaching out to stroke your swollen orb of a belly, the other trailing around the edges of your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " in a manner at once sensual and professional. \"<i>Don't worry, " + player.short + "; I'm going to help you get through this.</i>\" The tigershark promises you.\n\n");
 
		DisplayText("You point out it's the least she can do, seeing as how she's the one who put this thing in you in the first place, then turn your attention back fully to the task of bringing your offspring into the world.\n\n");
 
		DisplayText("Time slips away; you're too overwhelmed by the pain of your womb contracting and the pleasure as Izma ministers to your " +             VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)));
		if(player.torso.cocks.count > 0) DisplayText(" and " + CockDescriptor.describeMultiCockShort(player));
		...
		if(randInt(100) <= 59) {
			...
			DisplayText("black-striped orange");
			type = 1;
			Flags.list[FlagEnum.IZMA_CHILDREN_TIGERSHARKS]++;
		}
		DisplayText(" skin, it's obvious she's a baby ");
		if(type === 0) DisplayText("shark-girl");
		else DisplayText("tigershark");
		...
		DisplayText("She latches onto the " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + " and starts to suckle for all she's worth, waves of pleasure both sexual and maternal filling you, overwhelming you with bliss. Oblivious to everything else, you hold her against your breast, allowing her to sate her incredible appetite. She drinks your breast dry, and then immediately latches onto the next one.");
		if(player.torso.chest.count > 1) DisplayText("  She then starts on your third breast, and while she slows down at the fourth breast, she still finishes it off.");
		//(If player has six breasts: 
		if(player.torso.chest.count > 2) DisplayText("  She seems to almost be struggling to finish off your third pair, but she manages to suck all six of your breasts dry.");
		DisplayText("  Once she is done, she lets go with a wet popping sound, then starts to gasp for breath before letting out a surprisingly loud, deep belch for such a little thing.\n\n");
 
		DisplayText("...Okay, maybe not so little. Before, she was the size of a human baby. Now, you're holding a fairly grown young ");
		if(type === 0) DisplayText("shark-girl");
		else DisplayText("tigershark");
...
player.stats.sens += 1;
	...
	Mod.Breast.boostLactation(character, .01);
}
...
private izmaKidsPlaytime():void {
	...
	let choice:number = choices[randInt(choices.length)];
	if(model.time.hours < 10 && Flags.list[FlagEnum.ANEMONE_KID] > 0 && randInt(2) === 0) {
		anemoneScene.kidAWatchesSharks();
		return;
	}
	
	
	DisplayText("Izma shows you to an isolated bend in the stream; this is where your children have chosen to settle themselves.  Though they have the physiques and minds of pre-teens already, it will still be several years before they are fully grown and ready to leave you and Izma.  ");
	if (Flags.list[FlagEnum.IZMA_CHILDREN_SHARKGIRLS] + Flags.list[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] === 1) DisplayText("There is ");
	else DisplayText("There are ");
	if (Flags.list[FlagEnum.IZMA_CHILDREN_SHARKGIRLS] > 0) {
		if (Flags.list[FlagEnum.IZMA_CHILDREN_SHARKGIRLS] === 1) DisplayText("one shark-girl");
		else DisplayText(num2Text(Flags.list[FlagEnum.IZMA_CHILDREN_SHARKGIRLS]) + " shark-girls");
		if (Flags.list[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] > 0) DisplayText(" and ");
	}
	if (Flags.list[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] > 0) {
		if (Flags.list[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] === 1) DisplayText("one tigershark");
		else DisplayText(num2Text(Flags.list[FlagEnum.IZMA_CHILDREN_TIGERSHARKS]) + " tigersharks");
	}
	DisplayText(" living here; the fruits of your love with Izma.\n\n");
	...
	//NOPE
	if(choice === 1) {
		if (Flags.list[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] === 1) DisplayText("Your lone tigershark daughter is off by herself, experimentally stroking and fondling her already-impressive cock.  It doesn't look like it'll be long before she cums; her face is already screwed up in an expression of curious pleasure, gasping softly at the sensations.");
		//(Requires 2-3 Tigersharks: 
		else if (Flags.list[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] < 4 && Flags.list[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] > 0) DisplayText("Your tigershark daughters are seated together, each experimentally stroking and fondling her sizable erection.  It looks like they're competing to see who will cum first.");
		//(Requires 4+ Tigersharks:
		else if (Flags.list[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] > 0) DisplayText("Your tigershark daughters are seated together, each experimentally jerking off either her own cock or one of her sisters.  Some look to be competing to see who can jerk themselves off first, others are evidently seeing who's better at handjobs.");
		if (Flags.list[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] > 0) DisplayText("  Izma blushes and explains that your children have strong libidos, so it's natural for them to experiment like this.  She assures you that they won't do any real sexual penetration with a family member.\n\n");
	}
	...
	//NOPE
		...
		//Other children:
		if (Flags.list[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] + Flags.list[FlagEnum.IZMA_CHILDREN_SHARKGIRLS] > 2) DisplayText("  Your other kids are gathered around, watching with amusement and a little envy as one of your tigershark offspring plows an excited goblin.  \"<i>Oh, yeah, baby, now you folks know how to live!</i>\" she cheers.");
		if (Flags.list[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] > 2) DisplayText("  \"<i>Don't worry; you'll get a chance to use a beautiful big cock on me, too - just wait your turn!</i>\" she laughs looking around the circle.  Your other daughters look like they can't wait.");
	...
	//YUS
	if(choice === 4) {
		DisplayText("As you look around, a sudden violent splashing erupts from the lake.  You ready yourself to fight, but it turns out to be ");
		if (Flags.list[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] + Flags.list[FlagEnum.IZMA_CHILDREN_SHARKGIRLS] === 1) DisplayText("your daughter");
		else DisplayText("one of your daughters");
	...
	if(choice === 5) {
		DisplayText("Your ");
		if (Flags.list[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] + Flags.list[FlagEnum.IZMA_CHILDREN_SHARKGIRLS] === 1) DisplayText("child perks up to see Izma and comes running over to her, throwing herself into her arms");
		else DisplayText("children perk up to see Izma and come running over to her, throwing themselves into her arms");
		DisplayText(".  She laughs in delight and sits down on the bank, reaching for a book she has in a bag at her side.  Your offspring ");
		if (Flags.list[FlagEnum.IZMA_CHILDREN_TIGERSHARKS] + Flags.list[FlagEnum.IZMA_CHILDREN_SHARKGIRLS] === 1) DisplayText("cries out in joy at the prospect of storytime and eagerly seats herself in front of her, listening as she starts to read.");
		else DisplayText("cry out in joy at the prospect of storytime and eagerly seat themselves in front of her, listening as she starts to read.");
 		DisplayText("  You remain there for a while, listening, but eventually you think of your duties as champion and you have to take your leave of them.");
	}
...
private inCampRideIzmasDickDongTheWitchIsDead():void {
	DisplaySprite(32);
	DisplayText().clear();
	DisplayText("You direct a flushed look at Izma and rub your ");
	if(player.torso.vaginas.get(0).wetness > 3) DisplayText("dripping ");
	DisplayText("fuckhole meaningfully, telling her to lie down and let you give her fifteen inches and four balls a run.\n\n");
 
	DisplayText("Izma gives a happy squeal in response, before blushing and clearing her throat.  \"<i>Ah ha... of course, my Alpha,</i>\" she replies curtly.  She lies down on the ground and her giant dong stands straight to attention, waiting for your " +             VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ".\n\n");
 
	DisplayText("Slowly and sensuously you remove your " + player.inventory.equipment.armor.displayName + ", exulting in the way Izma hungrily drinks in every inch of exposed " + player.skinFurScales() + ".  Her eyes devour your " + BreastDescriptor.describeAllBreasts(player) + ", then slide eagerly down to your crotch to feast upon");
	if(player.torso.cocks.count > 0) {
		...
		DisplayText(" before going to the main attraction:");
	}
	DisplayText(" your pussy.  You sashay over to the prone tigershark, your " + LowerBodyDescriptor.describeHips(player) + " swaying in a fashion that has her hypnotised.  By the time you reach her, her cock is visibly bubbling pre from the slit and drooling it down onto her four swollen balls, which sit in a steadily-growing puddle of feminine juices. Smiling hungrily, you gently stroke her long, crimson-hued cock, playfully asking what such a little girl is doing with such a big, hard thing.\n\n");
 
	DisplayText("Izma pants and giggles in response, licking her lips at the little show you're putting on for her.  \"<i>Big?  Me?  Haha...</i>\"  She gasps and moans as you rub at her head.  \"<i>There are much bigger fish in those lake waters...</i>\" she pants, winking at you.  You don't know if she's joking or not.\n\n");
 
	DisplayText("You purse your lips thoughtfully, and feign entertaining the idea that maybe you should throw this small fry back... then you shake your head and grin.  No, you declare, this is plenty big enough for you.\n\n");

	DisplayText("\"<i>Oh, thank you... I feel like I'll go crazy if I have to wait much longer,</i>\" Izma purrs, placing her hands behind her head and thrusting her chest out at you to entice you.\n\n");
 
	DisplayText("You smile and bend over, hungrily kissing and suckling at her impressive breasts, squeezing them firmly to squeals of delight from the tigershark.");
	if(player.torso.cocks.count > 0) {
		DisplayText("  Your " + CockDescriptor.nounCock(player.torso.cocks.get(0).type) + " rubs against her shaft, sending delicious sparks surging through you.");
	}
	DisplayText("  Tiring of the foreplay, you slither up her body and position yourself over her, slipping down onto her shark-sausage just enough that the head is poking teasingly into your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ", but not enough to really penetrate you.");
	if(player.torso.vaginas.get(0).wetness >= 5) DisplayText("  Not that this stops your nether-lips from drooling your lubricant all over Izma's crotch.");
	DisplayText("\n\n");
 
	DisplayText("Izma seems to tense beneath you, her breath caught in her throat from anticipation.  She bites her lip as she awaits for you to slide onto her cock, but she doesn't dare move without your permission.\n\n");
 
	DisplayText("Finally deciding that ");
	if(player.stats.cor < 66) DisplayText("you've teased her enough");
	else DisplayText("you've delayed your pleasure long enough");
	DisplayText(", you sink down onto her, stifling a yelp as she fills your depths.");
 
	//(Tight/Virgin vagina)
	if(player.vaginalCapacity() < 20 || player.torso.vaginas.get(0).virgin) DisplayText("  Holy shit, that hurts like a motherfucker! It's as if you're trying to stuff a baguette into your vag!");
	//(Loose)
	else if(player.vaginalCapacity() < 50) DisplayText("  You groan and shudder slightly as you slide down the generous 15 inches of dick, the slight pain being drowned out by the sensation of being filled as full as you could possibly hold.");
	//(Gaping)
	else DisplayText("  You shudder and lick your lips. Izma may be big but you've had bigger, and you fit her cock like a glove.");
	player.displayStretchVagina(30,true,true,false);
	...
	if(player.torso.cocks.count > 0) {
		DisplayText(", your erect cock");
		if(player.torso.cocks.count > 1) DisplayText("s");
		DisplayText(" bouncing against her belly with audible slaps");
	}
	DisplayText(".\n\n");
	...
	DisplayText("Izma's cock twitches and she places her hands on your hips.  \"<i>Ah... I'm gonna...</i>\"  She grunts and bites her lip, trying to hang onto the moment.\n\n");
 
	DisplayText("You laugh in delight, complimenting Izma on what a good beta she is; the alpha gets to cum first, after all.  ");
	if(player.stats.cor < 33) DisplayText("Feeling merciful, you tell her that you're almost there... just a little more.  ");
	DisplayText("Finally, the orgasm that's been hanging just out of reach, tantalizing you, flows free and you cry out in pleasure as femcum ");
	if(player.torso.vaginas.get(0).wetness < 4) DisplayText("spatters");
	else if(player.torso.vaginas.get(0).wetness < 5) DisplayText("gushes");
	else DisplayText("fountains");
	DisplayText(" onto the tigershark under you");
	if(player.torso.cocks.count > 0) {
		DisplayText("; " + CockDescriptor.describeMultiCockSimpleOne(player) + " spraying all over her belly, breasts and face");
	}
	DisplayText(".  Izma, feeling the twitching of your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ", finally gives a loud roar as she climaxes inside you, filling your womb up with sharky spunk to the point where it makes your stomach bloat out from the sheer volume of her deposit.\n\n");
	...
	DisplayText("Izma moans weakly from beneath you, trying hard to keep her eyes open.  \"<i>If, ah... if that's what you want.</i>\"\n\n");
 
	DisplayText("You simply give her your wickedest grin, already starting to get into the rhythm of rising and falling...\n\n");
	player.orgasm();
 	player.stats.sens += -2;
	...
	DisplayText("Izma waves at you and smirks as you go to leave, and you have to wonder if Izma was only acting helpless in order to get off...");
...
private IzmaCampBooks():void {
	...
	//DisplayText("Izma starts to lazily fish through her trunk for her reading material while she waits on you.  She looks up and says, \"<i>Oh, I was just organizing my library.</i>\"  You ask her if she'd like some company.  \"<i>Oh, that would be lovely... would you like to read something?</i>\"\n\n");
 
 	DisplayText("You tell Izma you feel like doing some reading and she starts to lazily fish through her trunk for her reading material.  She looks up and says, \"<i>I'll need to organize this later... So, what would you like to read with me?</i>\"\n\n");
...
player.stats.cor += -2;
	...
	DisplayText("As time passes you realize that you do have other things to do.  You thank Izma for her company and get up to leave.  \"<i>All right, thanks for sitting with me " + player.short + ".  You go on ahead, I'm just going to read some more of this,</i>\" she replies, not even looking up from the pages of her book.\n\n");
	return { next: Scenes.camp.returnToCampUseOneHour };
}
//[Porn]
private stephenHawkingPorn():void {
	...
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText("continuously to keep the beast in her skirt pinned down.\n\n");
	else DisplayText("nervously in an effort to keep from moistening it.\n\n");
	Flags.list[FlagEnum.IZMA_TIME_TILL_NEW_BOOK_AVAILABLE] = 11;
	DisplayText("By the time you're done reading, Izma certainly seems turned on.  She tries to hide it and sit primly - ");
...
private findLostIzmaKidsII():void {
	DisplayText().clear();
	DisplayText("You embrace your daughter, both of you half-laughing half-crying into each other's shoulders.  After a few moments of this, you pull away a little and blurt out a few urgent questions about her well-being and mental health.  She in turn inquires about you and her \"father.\"\n\n");
...
private removeIzmasPenis():void {
	...
	if(Flags.list[FlagEnum.ASKED_IZMA_ABOUT_WANG_REMOVAL] === 0) {
		DisplayText("You ask Izma if she wouldn't mind getting rid of her dick for you, her Alpha.  She recoils at the mention, a haughty look on her angular visage.  \"<i>Tch, why would I want to get rid of my pride and joy?  My dick feels so good... I would never want to lose it,</i>\" Izma answers with a hint of rebellion.");
		...
		DisplayText("You bring up the idea of removing her dick again, and Izma just sighs \"<i>Why?  Why can't you just... be happy with me like I am?  I don't want to be just another shark-girl!</i>\"");
	...
	if(succMilk + pMilk < 5) {
		...
		MainScreen.simpleChoices(["Remove Dick", "", "", "", "Back"], [izmaDickToggle, null, null, null, izmaFollowerMenu]);
	}
}
...
private fuckIzmasPussyDominate():void {
	...
	if(x < 0) x = 0;
	let y:number = x + 1;
	DisplayText("Fixating on Izma's ");
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText("cock ");
	else DisplayText("cunt ");
	DisplayText("like a moth to an open flame, you saunter over towards her, sporting a ridiculous grin as thoughts of impaling yourself in her tight pussy takes hold of your mind, driving you into a frenzied lust.  With her skirt now thrown indifferently to the ground, her bare sex");
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText("es");
	DisplayText(" now stand");
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 1) DisplayText("s");
	DisplayText(" exposed to the cool air, drooling ");
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText("their sexual lubricants");
	else DisplayText("its sexual lubricant");
	...
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 1) DisplayText("shark-girl");
	...
	if(player.torso.hips.legs.isNaga() || player.torso.hips.legs.isTaur() || player.torso.hips.legs.isGoo()) DisplayText("With her tail now against your shoulder blade and jutting up in the air, your path to her fuckhole is now clear.  ");
	//(If human or Harpy legs are present: 
	else DisplayText("Lifting your right leg over her rump, you give yourself the superior position needed to ravage her cunt.  ");
	...
	if(player.torso.hips.legs.isGoo() && Flags.list[FlagEnum.IZMA_NO_COCK] === 0) {
		...
		DisplayText("Will you tend to her dick?");
		//[Goo job]    [Leave it, leads to [Spanking scene])
		MainScreen.simpleChoices(["Goo Job", "Leave It", "", "", ""], [gooJob, radarIzmaSpanking, null, null, null]);
	}
	DisplayText("\n\n");
	radarIzmaGasm();
}
...
private gooJob():void {
	DisplayText().clear();
	DisplayText("Concentrating on moving your mass forward, you manage to envelop Izma's entire back side.  Slowly but surely, you are able to harden your goo around Izma's rock hard cock, and begin to pump her penile member with gusto. Now that she's been jerked off, and the fact that she's ");
	radarIzmaSpanking(false);
	player.statusAffects.set(new StatusAffect("Goojob",0,0,0,0)));
}
//[Spanking and orgasm]
private radarIzmaSpanking(newSentence:boolean = true):void {
	...
	DisplayText("being subjected to double penetration and powerful bucks, ");
	radarIzmaGasm();
}
private radarIzmaGasm():void {
	...
	if(player.cumQ() <= 750) DisplayText("shooting your hot load into Izma's fuckhole, enveloping the walls of her pussy with your seed.");
	//(Cum production High: 
	else DisplayText("unleashing a cascade of steamy jizz into Izma's well fucked hole.  Your volume is so large that it causes a back draft of semen to rush out of her within mere seconds.");
	//(If PC gave Izma a "goo job":
	if(player.statusAffects.has(StatusAffectType.Goojob)) {
		player.statusAffects.remove("Goojob");
		DisplayText("  The sensation of being injected with her lover's seed is all the encouragement Izma's cock needs to finally erupt, shooting thick streams of steamy cum into your central mass; something your gooey form appreciates as it unconsciously works her load into your see through gut.");
	}
	DisplayText("\n\nYour exhausted partner swoons as she cradles her ");
	if(player.cumQ() > 750) DisplayText("bloated ");
	DisplayText("belly, clamping her legs shut in ");
	if(player.cumQ() <= 750) DisplayText("a ");
	else DisplayText("an ultimately ");
	...
	if(Flags.list[FlagEnum.IZMA_NO_COCK] === 0) DisplayText(" cock");
	else DisplayText("self");
	DisplayText(" down at the river... but judging by how tired she looks, you doubt that will happen anytime soon.");
	player.orgasm();
	player.stats.sens += -1;
	izmaPreg();
	return { next: Scenes.camp.returnToCampUseOneHour };
...
private radarIzmaAnalDominant():void {
	...
	if(player.torso.vaginas.count > 0) DisplayText("Your [clit] flushes with");
	else if(player.torso.cocks.count > 0) DisplayText("[EachCock] bobs in");
	else DisplayText("You flush with");
	...
	DisplayText("\n\nLooking back over your shoulder, you tell her that she... won't be dominant; in fact, she won't even lay a hand on you.  Izma's passionate, eager expression turns to one of confusion at your words; something you rectify by taking hold of her lengthy dick and caressing it with your " + ButtDescriptor.describeButthole(character.torso.butt) + ", allowing the head of her cock to be roughed around from tip to belly in the contours of your " + ButtDescriptor.describeButthole(character.torso.butt) + " and " + ButtDescriptor.describeButt(player) + ".  The callous treatment of her dick surges through Izma, sending a visible shiver through her as her member electrifies her body with sexual pleasure.  You remark to Izma how cute she looks from this angle, in a manner that denotes your playful yet condescending control over her; all the tigershark can do is blush fiercely as she whimpers, in what seems to be both a need for better intimacy and sensual overload.  With an almost mean spirited disregard for her need to warm up first, you begin to force her freakishly large member inside of your " + ButtDescriptor.describeButthole(character.torso.butt) + ", ");
	//(if Virgin to Tight:
	if(player.analCapacity() < 35) DisplayText("painfully rending the tight shape of your anus on it, causing both of you to moan in pain as you envelop more and more of her dick inside of you.  Izma slams into you forcefully as she unleashes a huge gasp, face utterly flooded with ecstasy as she stares into the sky behind closed eyes."); 
	//(If loose to gaping: 
	else DisplayText("slipping past the fleshy \"gates\" of your anus, requiring little stretching or working of your " + ButtDescriptor.describeButthole(character.torso.butt) + ".  You're worried that this might not work the way you want it to, given how easy it was to take Izma; a series of wild, erratic thrashings against your ass as the penile intruder tunnels deeper seems to dispel that notion.");
	player.displayStretchButt(42,true,true,false);

	DisplayText("\n\nWith a cool, yet skittish breath of air, you tell Izma that you will be working her cock with your ass ");
	if(player.torso.hips.legs.isGoo()) DisplayText("and goo");
	...
	else DisplayText("violated anus");
	DisplayText(" to masturbate as you do so.");
	
	DisplayText("\n\nDespite Izma's near orgasmic scare, she soldiers on through your brutal and \"agonizing\" stimulation of her giant red dick");
	if(player.torso.hips.legs.isGoo()) DisplayText("; taking her firm ass into your slippery body's grip, you begin to force Izma to gyrate against you, driving the shark-morph mad with pleasure");
	DisplayText(".  Your pumping of your ");
	if(player.torso.vaginas.count > 0) DisplayText(VaginaDescriptor.describeClit(player));
	else if(player.torso.cocks.count > 0) DisplayText(CockDescriptor.describeCock(player, player.torso.cocks.get(0)));
	else DisplayText("anus");
	DisplayText(" increases in ferocity as you slam your " + ButtDescriptor.describeButt(player) + " against her lap, her quadrate of testicles slapping against your ");
	
	//(if PC is fingering their pussy: 
	if(player.torso.vaginas.count > 0) DisplayText("feverish hands as you fingerfuck your " +             VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ".");
	//(If PC has balls: 
	else if(player.torso.balls.quantity > 0) DisplayText("swinging [balls] with each gyration of your hips.");
	//(If PC is genderless: 
	else DisplayText("thighs, thoroughly \"tenderizing\" them with every thunderous impact.");
	...
	if(player.gender > 0) {
		...
		//(If PC has vagina: 
		if(player.torso.vaginas.count > 0) DisplayText("intent on burying your " + VaginaDescriptor.describeClit(player) + " in her face and having her finish you off.  Izma tries her best to pleasure you, but she's still too wracked with bliss to be any help... still, you manage to find stimulation by grinding yourself against her spaced-out face, eventually orgasming and drenching her face in your lady fluids as she weakly strokes the remaining cum out of her cock.");
		//(If PC has cock(s): 
		else if(player.torso.cocks.count > 0) DisplayText("stroking furiously along your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " to bring an explosive end to this passionate act.  Izma is still off in another world of orgasmic fulfillment to note the impending facial coming her way.  It takes her a few moments to register it, but she coos lovingly as the scent of semen hits her nostrils, finally realizing that you have blown your steamy load all over her face.");
		//(genitalia end: 
		DisplayText("  Falling back on your " + ButtDescriptor.describeButt(player) + ", you breathe a sigh of relief, both thanking and lamenting the fact that your orgasms have reached fruition.  Despite still being able to get up and walk around, Izma is another story.");
	
		DisplayText("\n\nYou could carry her back to her bedding and tuck her in... or you could just let her sleep it off.");
		
		//[Tuck in]   [Leave her]
		MainScreen.simpleChoices(["Tuck In", "Stay", "", "", ""], [radarIzmaAnalDomResultTuckIn, radarIzmaLeaveHerInTheDirtAfterAnalDom, null, null, null]);
	}
	else {
...
private radarIzmaAnalDomResultTuckIn():void {
	...
	if(player.stats.str < 70) {
		DisplayText("Despite your good intentions, you can't lift Izma off the ground for long before needing to set her back down.  Imza quietly remarks that you should just let her rest; causing you to tell her not to talk back.  Alternating between dragging her through the grass and \"carrying\" her over some rocks, ");
		if(player.fatigue < 70) {
			DisplayText("you eventually get her back to her bedroll.  She smiles sheepishly as you lay her on the bedding to sleep off your passionate throes.  She's comfortable, but the effort is exhausting.");
			player.stats.cor += -2;
			fatigue(30);
		}
		else DisplayText("you try and get her to her bedroll, but you are simply too exhausted.  Izma tells you not to worry about her and go get some rest; that she'll be okay in the grass.  You groggily agree as you set her down, telling her she better be ready for more sex if you want it; something she approves of as she hums and drifts off to sleep.");
	}
	else {
...
private radarFucksIzmasAss():void {
	...
	if(player.torso.cocks.get(x).type != CockType.TENTACLE) {
		...
		if(y >= 0) DisplayText(" as you gently place your other dick against her labia");
		DisplayText(".");
		...
		if(y === -1) DisplayText("her anus");
		else DisplayText("both of her holes");
		DisplayText(".  The sound of Izma's pained cry of agony rises as her head does, guided by your arm as you drag her so far back that she ends up howling up into the sky.  Like a tongue reaching down into a candy wrapper to lick up delicious chocolate, you twist and grind the end of your ");
		if(y >= 0) DisplayText("cocks against the unyielding ends of her anus and pussy, unleashing a weak whimper of bliss from under you.");
		else DisplayText("cock against the unyielding end of her anus, unleashing a weak whimper of bliss from under you.");
		...
		DisplayText(".  All too quickly though, your loins ache with the pained vice-grip of orgasm.  As much as you would like to continue dominating Izma, you cannot hold out.  The only choice you have now is where to direct your impending release.");
		//[Internal, Global option for non-TD and TD scenes]    [External, Non-TD scenes]
		MainScreen.simpleChoices(["Internal", "External", "", "", ""], [null, null, null, null, null]);
	}
	//!Tentacle dick scenes!
		...
		//(PC has one TD: 
		if(player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE) === 1) DisplayText("begins to slowly caress her pussy in tandem with your thrusts, bringing an expression of intense bliss to her face as her eyes clamp shut and her mouth opens wide to let out a deep moan.")).length;
		//(PC has 2 TD's and 30 INT: 
		if(player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE) >= 2) DisplayText("tries to finger her pussy; the thought of her neglecting one of your rock hard cocks earns your ire, and with some degree of focus and concentration, you jut your tentacle cock out and suddenly slap her soft hand away from her clit; a gasp full of frustration rings out from your partner as you do so... deciding that she needs something substantial to moan about. Before she can react, your direct your thoughts to the tentacle cock that blocks her clit, and in a dizzying array of twists and bends, pulls back a few inches from her pussy before diving into her with lightning speed.  The way she thrusts her head upwards in the air is a pretty clear indicator of her discomfort at the act; whether it being painful or pleasurable isn't clear, but the howl she lets loose seems to point to the former.  The hermaphrodite shark morph claws fiercely at the ground as she grits her teeth, spasming like an crippled junebug caught in the burning rays of the sun.  You tauntingly ask the submissive tigershark if that hurt; something your lover can only answer with passionate groans as you resume your dual fucking of her back side.  You arrogantly respond by telling her THAT was the correct answer.")).length;
		//(Subsequent scenes require TD's to have 4 feet of length)
		//(PC has three TD's, and 40 INT: 
		if(player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE) >= 3) DisplayText("\n\nAmused at how lost in sexual pleasure Izma is, you mockingly ask her if she can handle another one of your dicks.  By now, Izma seemingly knows to hold her tongue and rely on inferring your meaning, the realization dawning on her as she visibly braces herself for another tentacle dicked assault on her now sweat drenched body.  You don't disappoint her, as you snake your lengthy appendage past her thigh and up her stomach, coming to a sudden rest between her breasts before beginning to brutally tit fuck her.")).length;
		if (player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE) >= 4)).length {
			DisplayText("\n\nLaughing heartily, you inform the seemingly overwhelmed tigershark that you still have ");
			if (player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE) === 4) DisplayText("a cock that needs")).length;
			else DisplayText("cocks that need");
			DisplayText(" a little loving from her; something that visibly registers on her face as her expression goes wide in shock, her eyes seemingly asking you where THIS one is going next.");
			//[Dick]    [Blowjob]   
			MainScreen.simpleChoices(["Dick", "Blowjob", "", "", ""], [null, null, null, null, null]);
			return;
		}
		//Go to buttsex
		//analFuckRadarEnding();
...
private radarIzmaJustDick(choice: number = 0):void {
	...
	if(choice === 0) {
		...
		player.statusAffects.set(new StatusAffect("JerkingIzma",0,0,0,0)));
	}
	//[(Just) Blowjob]: 
	else if(choice === 1) {
		...
		player.statusAffects.set(new StatusAffect("IzmaBlowing",0,0,0,0)));
	}
	//[Both]
	else {
...
private internalIzmaAnalCumShot():void {
	...
	if(player.cockThatFits2(65) >= 0) DisplayText("  The feelers in her pussy glide along the glans of your " + CockDescriptor.describeCock(player, player.cockThatFits2(65)) + ", driving your dick's unconscious efforts to inseminate her into overdrive as you feel each subsequent stream of semen increase in velocity.");
}
//[External, Non-TD scenes]
private backJizzShot():void {
	DisplayText().clear();
	DisplayText("Pulling out of Izma, you begin to stroke [eachCock] furiously, bobbing your shaky hands up and down ");
	if(player.torso.cocks.count === 1) DisplayText("your shaft ");
	else if(player.torso.cocks.count === 2) DisplayText("two shafts ");
	else DisplayText("three shafts");
	DisplayText(" as you struggle to cum.  Rolling over onto her thoroughly abused ass, the feminine tigershark twists her head back to get her hair out of her face, and opens up wide to receive your load.  Arcing like lightning in the sky, your orgasm flows forcefully out of your " + CockDescriptor.describeMultiCockShort(player) + ", ");
	//(Cum volume normal: 
	if(player.cumQ() < 500) DisplayText("firing thin streams of semen from [eachCock], lathering Izma's luscious red body and making it glisten in the sun.");
	//(Cum volume high:
	else if(player.cumQ() <= 1000) DisplayText("streaming forth like fierce rain down a tree, repeatedly striking Izma across the face and breasts as your orgasm continues unabated for several moments.");
	//(Cum volume very high: 
	else {
		DisplayText("creating a earthquake like seizure in [eachCock] as you painfully ejaculate a torrent of sperm all over Izma, drowning that bright red body out with a thick layer of clear white goop.  Your abundant release is so large that you, quite literally, cover Izma head to toe with sticky, sloppy cum.");
...
private izmaDomsLatexyPartI():void {
	if(Flags.list[FlagEnum.TIMES_IZMA_DOMMED_LATEXY] === 0) DisplayText().clear();
	else DisplayText("\n\n");
	DisplayText("Izma growls while she pumps away at her erection, saying, \"<i>Yeah, you like that, don't ya ink-slut?</i>\"");
	
	DisplayText("\n\n" + Flags.list[FlagEnum.GOO_NAME] + " bites her lower lip and ");
	if(latexGirl.gooObedience() > 50) DisplayText("eagerly");
	else DisplayText("sheepishly");
	DisplayText(" nods, gushing a few fresh dribbles of moisture around her wrist at the admission.");
	...
	if(Flags.list[FlagEnum.GOO_DICK_LENGTH] > 0) DisplayText("cock");
	else DisplayText("clit");
	DisplayText(" with a toe, drawing a pleasured gasp from the submissive.");
	...
	if(latexGirl.gooObedience() < 50) DisplayText("  A look of annoyance flashes across her face as soon as she says it, like she didn't really want to say that so openly.");
	else if(latexGirl.gooObedience() <= 80) DisplayText("  A conflicted look flashes across her face as soon as the words slip out, but there's no taking them back.");
	else DisplayText("  A hungry look flashes in her eyes as soon as the words leave her mouth, and she licks her lips hungrily.");
	DisplayText("  A particularly powerful thrust inside herself has " + Flags.list[FlagEnum.GOO_NAME] + " whining, her eyelids fluttering closed as she cries, \"<i>Oh, [name]!</i>\"");
	...
	//OBEDIENCE FAIL
	if(latexGirl.gooObedience()/5 + latexGirl.gooFluid()/5 + randInt(20) < 10) {
...
private stopIzmaLatexy():void {
	DisplayText().clear();
	DisplayText("You step into the middle of it before it can progress any farther.  " + Flags.list[FlagEnum.GOO_NAME] + " is yours, and Izma is not to play at being a mistress to YOUR pet.");
...
private izmaLatexySubmenu():void {
	DisplayText().clear();
	DisplayText("\"<i>Oh, I wouldn't worry about her, [name],</i>\" Izma says while her tail flicks back and forth.");
	menu();
...
private izmaLatexyToggle():void {
	...
	if(Flags.list[FlagEnum.IZMA_X_LATEXY_DISABLED] === 0) {
		DisplayText("You tell Izma you don't want her feeding " + Flags.list[FlagEnum.GOO_NAME] + " any more.");
		...
		DisplayText("You tell Izma that you've changed your mind, and she can keep " + Flags.list[FlagEnum.GOO_NAME] + " fluid topped off.  The shark-girl hugs you with a predatory grin splitting her face.  \"<i>I can't wait.</i>\"");
