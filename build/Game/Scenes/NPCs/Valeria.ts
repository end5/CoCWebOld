export class Valeria extends NPCAwareContent implements TimeAwareInterface {
	...
	public timeChange(): boolean {
		if (player.statusAffects.get(StatusAffectType.GooStuffed).value1 > 0) {
	...
	public pcWinsValeriaSparDefeat() {
		...
		if (player.stats.HP < 1) DisplayText("in too much pain");
		else DisplayText("too turned on");
		DisplayText(" to fight any longer.  Valeria's on you in a heartbeat, her gooey greatsword pressed to your throat.");
		...
		if (player.stats.HP < 1 || player.gender === Gender.NONE) {
		...
		//[else; If PC lost via lust & has a gender: 
			...
			DisplayText(".  \"<i>Well, since you're so... eager... I might as well get my daily fluids while we're here.</i>\"");
	...
	private followersValeriaSex(display: boolean = true) {
		...
		if (display) {
			DisplayText().clear();
			DisplayText("With a lusty grin, you ask your gooey friend if ");
			if (player.gender > 0) DisplayText("she's interested in a little fluid exchange");
			else DisplayText("she wouldn't mind helping you get some release, despite your genderlessness");
			DisplayText(".");
	...
	private valeriaDickToggle() {
		...
		if (Flags.list[FlagEnum.VELARIA_FUTA] === 1) {
			DisplayText("Before you do anything, you ask Valeria if she wouldn't mind hiding her facsimile of a dick, at least when you aren't having sex.");
		...
		//[If Valeria doesn't have a dick:] 
			...
			DisplayText("Struggling to find a less-than-awkward way of phrasing this, you ask Valeria if she wouldn't mind growing a certain extra appendage for you.");
	...
	private valeriaGetFucked() {
		...
		if (player.torso.vaginas.count > 0) DisplayText("stroking the slick lips of your [vagina], ask Valeria to fuck you.");
		else DisplayText("posing seductively, ask Valeria to fuck you.");
		DisplayText("\n\nShe grins.  \"<i>You just lie back and let me take care of everything, partner.</i>\"  You do as she asks, flopping onto your back ");
		if (player.torso.hips.legs.isBiped()) DisplayText("and spreading your legs ");
		DisplayText("as Valeria ");
		if (Flags.list[FlagEnum.VELARIA_FUTA] === 0) DisplayText("forms a thick, gooey cock");
		else DisplayText("grabs her thick, gooey cock");
		DisplayText(" and looms over you.");
		...
		//If PC has no gender 
		if (player.gender === Gender.NONE) DisplayText("poking the shaft of her cock gingerly, teasingly against your [asshole]");
		DisplayText(".");
		...
		if (player.torso.cocks.count > 0) DisplayText(", as she rhythmically flexes her ass cheeks, shifting her fluidity around your shaft within her with a feeling like lotion-covered hands gently stroking you");
		DisplayText(".");
		...
		//(If Male/Herm)
			...
			if (player.torso.vaginas.count > 0) DisplayText("and vag ");
			DisplayText("with warm, sticky goop.");
		}
		...
		if (player.gender > 0) DisplayText("patting her belly full of your juices, ");
		DisplayText("and looms over you.  \"<i>That was fun, partner,</i>\" she says, leaning down to give you a wet peck on the cheek. \"<i>Let's do that again soon, all right?</i>\"");
	...
	private gooFlation(clearText: boolean = true) {
		...
		if (clearText) {
			DisplayText().clear();
			DisplayText("\"<i>Just relax, partner,</i>\" she grins, reaching up to run a hand through your [hair].  \"<i>Just let ol' Valeria take care of everything...</i>\"");
		...
		if (clearText) {
			DisplayText("\n\nYou collapse, goop flowing freely from your abused body.  Laughing, Valeria pours out of your lap, ");
			if (player.gender > 0) DisplayText("patting her belly full of your juices, ");
			DisplayText("and looms over you.  \"<i>That was fun, partner,</i>\" she says, leaning down to give you a wet peck on the cheek. \"<i>Let's do that again soon, alright?</i>\"");
			player.orgasm();
			player.stats.sens += 1;
			HPChange(25, false);
			return { next: Scenes.camp.returnToCampUseOneHour };
	...
	private valeriaSexDominated() {
		...
		if (player.torso.vaginas.count > 0 && (player.torso.cocks.count <= 0 || randInt(2) === 0)) {
			...
			if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 1) {
				...
				if (player.lactationQ() > 0) DisplayText("  To her delight, a spray of warm milk jets out of your sore nipples, milky white mixing into blue goo like oil in water.  \"<i>Mmm, tasty!</i>\" she teases, massaging more and more from you.");
			}
			...
			if (player.vaginalCapacity() > 80 || player.analCapacity() > 80) DisplayText(" and your own receptibility");
			DisplayText(".  Hugging your back, the goo-girl grabs at your [chest], squeezing your tits and pinching at your nipples.  \"<i>Moan for me, bitch,</i>\" Valeria growls into your ear, giving your [nipples] a particularly violent tug.  You let out a lewd, whorish moan.  The goo-girl grins, her tongue lashing out to lick your ear as she rewards you with a single long thrust into your depths, filling and stretching your [vagina] and [asshole] completely before withdrawing.");
			...
			if (player.lactationQ() > 0) DisplayText("and milk ");
			DisplayText("swirling around inside her.  \"<i>We'll do this again sometime,</i>\" she adds, walking off to another part of camp with a wink.");
		}
		//(PC has Dick)
		else {
	...
	private talkWithValeria() {
		...
		if (player.race() != "human") DisplayText(" were");
		DisplayText(".  I just, I dunno, changed, I guess.</i>\"");
		DisplayText("\n\nChanged?");
		...
		if (player.gender > 0) MainScreen.simpleChoices(["Flirt", "Accept", "Gross", "", ""], [flirtWithValeria, acceptValeriasNeeds, declineValeriasNeeds, null, null]);
		else MainScreen.simpleChoices(["", "Accept", "Gross", "", ""], [null, acceptValeriasNeeds, declineValeriasNeeds, null, null]);
	}
	...
	private flirtWithValeria() {
		...
		if (player.torso.cocks.count > 0) DisplayText("brushing along the length of your rapidly-hardening cock");
		else DisplayText("brushing a wet, warm knuckle across your [clit]");
		DisplayText(".");
	...
	private acceptValeriasNeeds() {
		...
		if (player.statusAffects.has(StatusAffectType.SlimeCraving)) DisplayText(" after all, you have the same ones");
		DisplayText(", as long as they don't interfere with your mission as Champion.");
	...
	public valeriaAndGooThreeStuff() {
		...
		let list: Array = ["[hips]"];
		if (player.torso.balls.quantity > 0) list.push("[balls]");
		if (player.torso.cocks.count > 0) list.push("[multiCockDescriptLight]");
		if (player.torso.vaginas.count > 0) list.push("[vagina]");
		list.push("[asshole]");
		list.push("[nipples]");
		DisplayText(formatStringArray(list) + " with liquid-soft caresses, almost ephemeral and yet still so perfectly all-consuming, filling every tiny imperfection in your [skin] with blue, gooey goodness and teasing whatever entrances she can get at. ");
		/*
			clearList();
			addToList("[hips]");
			if(player.torso.balls.quantity > 0) addToList("[balls]");
			if(player.torso.cocks.count > 0) addToList("[multiCockDescriptLight]");
			if(player.torso.vaginas.count > 0) addToList("[vagina]");
			addToList("[asshole]");
			addToList("[nipples]");
			DisplayText(outputList() + " with liquid-soft caresses, almost ephemeral and yet still so perfectly all-consuming, filling every tiny imperfection in your [skin] with blue, gooey goodness and teasing whatever entrances she can get at. ");
		*/
		if (player.torso.cocks.count > 0) {
			DisplayText("You sigh as [eachCock] stands at full erection. Bluish coating");
			if (player.torso.cocks.count > 1) DisplayText("s");
			DisplayText(" barely thicker than a condom stretch");
			if (player.torso.cocks.count === 1) DisplayText("es");
			DisplayText(" around ");
			if (player.torso.cocks.count === 1) DisplayText("it as it distends");
			else DisplayText("them as they distend");
			DisplayText(" Valeria's increasingly liquescent form. Dripping off the tip");
			if (player.torso.cocks.count > 1) DisplayText("s");
			DisplayText(", sapphire slime falls in long gooey stands, yet the whole time you feel as if a dozen hands are dragging lubricant-coated feathers over the whole of your sensitive organ");
			if (player.torso.cocks.count > 1) DisplayText("s");
			DisplayText(".");
		}
		...
		DisplayText(" juices slowly meld together until you cannot tell one from the other, and they firm up into a semi-translucent, flat-bottomed sphere, trapping you there. Your debauched and hopelessly aroused state is visible to any who would wander by, stroked by dark-colored currents that wrap around your ");
		if (player.torso.cocks.count > 0 || player.torso.vaginas.count > 0) DisplayText("genitals");
		else DisplayText("[asshole]");
		DisplayText(" with sinful tightness. An involuntary shiver of excitement traverses up your spine, though your body is held almost completely stock-still in its sloshing vice.");
		...
		DisplayText("\n\nYou ");
		if (Flags.list[FlagEnum.TIMES_VALERIA_GOO_THREESOMED] === 0) DisplayText("demand that she release you");
		else DisplayText("playfully comment that she really ought to release you");
	...
	private valeriaGooRapeII() {
		...
		if (Flags.list[FlagEnum.TIMES_VALERIA_GOO_THREESOMED] === 0) DisplayText(", unexpected");
		else DisplayText(" but expected");
		...
		if (player.torso.cocks.count > 0) {
			...
			if (player.torso.cocks.count > 1) DisplayText("s");
			DisplayText(" in place, and something even warmer than the slippery container");
			if (player.torso.cocks.count > 1) DisplayText("s");
			DisplayText(" pushes at your head");
			if (player.torso.cocks.count > 1) DisplayText("s");
		...
		if (Flags.list[FlagEnum.TIMES_VALERIA_GOO_THREESOMED] > 0) DisplayText("You grin in eager anticipation, waiting to feel the flood of fluid that you know is coming. It won't take long.... You feel it in your butt first.");
		else DisplayText("At first, you don't realize what it is. You're floating in a bit of a sexual daze by now and barely cogent of what's going on, but it does feel quite nice, whatever it is. The spreading heat in your posterior is what finally clues you in to what's happening.");
		DisplayText(" Pumping deep into your bowels, the hardened protrusion is no longer simply stuffing you. It's pumping something slick and wet and oh so gooey straight into your [asshole], filling you up so strangely that you aren't sure what to make of beyond how... well, good it feels to be stuffed like this.");
		...
		if (player.torso.vaginas.count > 0) {
			...
			if (player.torso.vaginas.get(0).wetness <= 2) DisplayText("leaking");
			else if (player.torso.vaginas.get(0).wetness <= 3) DisplayText("dripping");
		...
		if (player.torso.cocks.count > 0) {
			...
			if (player.torso.cocks.count > 1) DisplayText("s");
			DisplayText(" in [eachCock] ");
			if (player.torso.cocks.count > 1) DisplayText("do not");
			else DisplayText("does not");
			DisplayText(" differ greatly from Valeria's other liquid attentions. ");
			if (player.torso.cocks.count > 1) DisplayText("They start");
			else DisplayText("It starts");
			DisplayText(" by thickening, stretching your cumvein");
			if (player.torso.cocks.count > 1) DisplayText("s");
			DisplayText(" as wide as ");
			if (player.torso.cocks.count > 1) DisplayText("they");
			else DisplayText("it");
			DisplayText("'ll go without pain and then rolling thick pulses of gelatinous matter into your urethra");
			if (player.torso.cocks.count > 1) DisplayText("s");
			DisplayText("s");
			DisplayText(". The sleeve");
			if (player.torso.cocks.count > 1) DisplayText("s");
			DisplayText(" squeeze");
			if (player.torso.cocks.count === 1) DisplayText("s");
			DisplayText(" down on your length");
			if (player.torso.cocks.count > 1) DisplayText("s");
			DisplayText(" harder, starting to slide around in ways that would be impossible for a vagina, and you find yourself helplessly blowing your load straight into the dick-plug");
			if (player.torso.cocks.count > 1) DisplayText("s");
			DisplayText(", spurting bliss that can't even hope to escape from your loins. You feel ");
			if (player.torso.balls.quantity === 0) DisplayText("something expanding as you cum, filling up with spunk or slime or some mixture of the two");
			else DisplayText("your [balls] expanding as you cum, filling up with spunk or slime or some mixture of the two, weighing down your sack with ever-increasing heaviness. Tender licks of tongue roll across the surface as you fill beyond completely");
			DisplayText(".");
		}
		//Butt fillinggasm
		...
		if (player.torso.cocks.count > 0 && player.torso.balls.quantity > 0) {
			...
			DisplayText(" Your balls look positively swollen with libidinous juices.");
		}
		DisplayText(" Your belly is stuffed into a parody of a pregnant woman's");
		if (player.torso.vaginas.count > 0) DisplayText(", so gravid from both holes that a real mother would be shamed by your obscene \"pregnancy.\"");
		else DisplayText(".");
		...
		//v3 = cunt fill?
		//v4 = tit fill?
		player.statusAffects.set(new StatusAffect("GooStuffed", 10 + randInt(300))), 0, 0, 0);
	...
	public birthOutDatGooSlut() {
		...
		if (player.skin.type === SkinType.FUR) DisplayText("furry ");
		...
		if (player.statusAffects.get(StatusAffectType.GooStuffed).value4 > 0) {
			...
			if (player.torso.chest.count > 2) DisplayText(", and so on");
			DisplayText(", working out every drop of thick, body-filling slime while your ass is still busy draining. Even after they empty, they don't seem to lose a single bit of the size they've gained....");
		}
		if (player.torso.cocks.count > 0 && player.statusAffects.get(StatusAffectType.GooStuffed).value2 > 0) {
			...
			if (player.torso.balls.quantity > 0) DisplayText("ball-clenching");
			else DisplayText("body-clenching");
			...
			if (player.statusAffects.get(StatusAffectType.GooStuffed).value2 >= 2) DisplayText("  Your balls keep their enhanced sized. Odd.");
		}
		if (player.torso.vaginas.count > 0 && player.statusAffects.get("$1").value3 > 0) {
