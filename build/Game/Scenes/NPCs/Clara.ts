export class Clara extends Character {
	...
	protected claraDrugAttack() {
		let temp2: number = randInt(2);
		let color: string = "";
		if (temp2 === 0) color = "red";
		if (temp2 === 1) color = "black";
		//Throw offensive potions at the player
		DisplayText("Clara suddenly snatches something from a pouch at her belt. \"<i>Try this, little cutie!</i>\" She snarls, and throws a vial of potion at you.");
		//Dodge chance!
	...
	protected claraTeaseAttack() {
		...
		else if (randInt(2) === 0) DisplayText("Clara seems to relax for a moment and bounces her breasts in her hands.  \"<i>Come on, you know how good it is to drink cow-girl milk, just give up!</i>\" she coos.  Despite yourself, you can’t help but remember what it was like, and find yourself becoming aroused.");
		else DisplayText("Instead of attacking, Clara runs her hands up and down her body, emphasizing all the curves it has.  \"<i>You were made to be the milk slave of this, stop fighting it!</i>\" she says almost exasperated.  Even so, you find your gaze lingering on those curves against your will.");
		DisplayText("\n");
	...
	public claraCastsBlind() {
		DisplayText("Clara glares at you, clearly being worn down.  Then strange lights start dancing around her hand and she points it in your direction.");
		//Successful: 
		if (player.stats.int / 5 + randInt(20) + 1 < 14) {
	...
	override protected performCombatAction() {
		if (player.statusAffects.has(StatusAffectType.ClaraFoughtInCamp) && player.statusAffects.get(StatusAffectType.ClaraCombatRounds).value1 >= 10) {
			HP = 0;
			combatRoundOver();
		}
		if (HP < 50 && randInt(2) === 0) {
			notMurbleEnjoysTheLacticAcid();
		}
		else if (player.statusAffects.has(StatusAffectType.Blind)) {
			claraGropesBlindPCs();
		}
		else {
			let actions: Array = [eAttack, claraDrugAttack, claraTeaseAttack, claraCastsBlind];
			let action: number = randInt(actions.length);
			trace("ACTION SELECTED: " + action);
			actions[action]();
		}
		if (!player.statusAffects.has(StatusAffectType.ClaraCombatRounds)) player.statusAffects.add(StatusAffectType.ClaraCombatRounds, 1, 0, 0, 0);
			else player.addStatusValue(StatusAffects.ClaraCombatRounds, 1, 1);

		//Bonus damage if not in camp
		if (HP > 0 && lust < 100 && !player.statusAffects.has(StatusAffectType.ClaraFoughtInCamp)) claraBonusBaseLustDamage();
	}
	public defeated(hpVictory: boolean) {
		...
		if (player.statusAffects.has(StatusAffectType.ClaraFoughtInCamp) && player.statusAffects.get(StatusAffectType.ClaraCombatRounds).value1 >= 10) { }
			...
			DisplayText().clear();
			//PC wins via health
			if (HP <= 0) DisplayText("The pissed off cowgirl finally collapses to the ground.  She tries to stand up again, but finds that she can’t.  \"<i>Noo!</i>\" she cries out in frustration, \"<i>You were the perfect slave!  We were meant to be toogether!</i>\"\n\n");
			//PC wins via lust
			else DisplayText("The fury and anger finally give out to the overwhelming lust that you’ve help Clara feel.  She can’t fight anymore, and falls onto her backside.  She starts feeling herself up, and desperately asks you to fuck her.\n\n");
		}
