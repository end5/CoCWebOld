export class ShouldraScene extends NPCAwareContent implements TimeAwareInterface {
	...
	public timeChange(): boolean {
		...
		if (shouldraFollower.followerShouldra()) {
			if (player.statusAffects.get(StatusAffectType.Exgartuan).value1 === 1 && player.torso.cocks.count > 0 && randInt(10) === 0) {
	...
	internal function shouldraGreeting() {
		...
		if (Flags.list[FlagEnum.TIMES_MET_SHOULDRA] < 3) {
			...
			//([if first time] 
			if (Flags.list[FlagEnum.TIMES_MET_SHOULDRA] === 0) DisplayText("\"<i>You're the asshole that took my racks, aren't you?  I was hoard-...er, I was keeping an eye on those!</i>\" she accuses, eyes narrowing angrily.  ");
			DisplayText("Finally, she acknowledges your readiness to fight. With a sigh and a shrug, she raises her arms and drops into an orthodox boxing stance. Once again, you heft your " + player.weaponName + ".");
		...
		//(after three encounters with her)
			...
			if (Flags.list[FlagEnum.TIMES_BEATEN_SHOULDRA] >= 3) {
				DisplayText("\"<i>Or... maybe not?  Truth be told, I think you've proven your ability to pummel me enough.  Wanna just skip to the good part?</i>\"\n\n");
				//([if first time] 
				if (Flags.list[FlagEnum.TIMES_BEATEN_SHOULDRA] === 3) DisplayText("You can't say you saw that coming.  It seems she has offered to forego the fight in lieu of sexings.  ");
				DisplayText("Would you like to accept her offer, or do you want to fight her regardless?");
				//Now back to the good part!
				MainScreen.simpleChoices(["Accept", "Fight", "", "", ""], [yankeeEchoPapa, novemberAlphaHotel, null, null, null]);
			}
			else {
				//FIGHT!
				startCombat(new Shouldra());
	...
	internal function defeatDannyPhantom() {
		...
		//(defeat by lust)
			...
			DisplayText("The girl glances down at her leggings, then does a double take, realizing as if for the first time she was completely overwhelmed by her lust. She sinks to her knees, dragging her breeches down and pressing a hand against her pubic mound.  ");
			if (Flags.list[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] < 1) DisplayText("Strangely, the action seems to have little to no effect.");
			else DisplayText("Of course, the motion has no effect on her libido, due to her ghostly state.");
			DisplayText("  Looking up at you with what can only be described as desperation, she pants, \"<i>Please, hero... I need release... let me in?</i>\"  You take a step forward, stroking your chin softly in contemplation.  ");
			//([if first time]
			if (Flags.list[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] < 1) {
				DisplayText("What does she mean, \"<i>let me in?</i>\" Do you want to find out?");
				doYesNo(littlePigLittlePigLetMeIn, noSlimingSlimer);
			}
			else {
				DisplayText("What do you do?");
				MainScreen.simpleChoices(["Let Her In", "Deny", "", "", ""], [littlePigLittlePigLetMeIn, noSlimingSlimer, null, null, null]);
			}
		}
	...
	private noSlimingSlimer() {
		...
		//([if first time] 
		if (Flags.list[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] === 0) DisplayText("not feeling up to heed the strange girl's whims.");
		else DisplayText("mystified at the thought of letting a ghost possess your body.");
		DisplayText("You whirl about and stomp away.  She rises, screaming, \"<i>Fine! I'll go and take over a fucking goblin, again!</i>\" and storms the other direction. Before you move out of earshot, you hear a sniffle from the departing ");
		//([if first time]
	...
	private littlePigLittlePigLetMeIn() {
		...
		if (Flags.list[FlagEnum.TIMES_POSSESSED_BY_SHOULDRA] === 0) DisplayText("you can afford being a little adventurous with the obviously desperate girl");
		else DisplayText("having a spiritual tag-along might be a bit of fun");
		DisplayText(", and agree to her request to possess you. She staggers to her feet and undresses, panting and shivering with unfulfillable sexual need. You reach out to her, drawing her");
		//[if <~7' tall]
		if (player.tallness < 84) DisplayText(" into your arms for a tight hug");
		else DisplayText(" up into your arms for a tight hug");
		...
		else if (player.gender === Gender.FEMALE) gender = ghostGinaWinSexings;
		else if (player.gender === Gender.HERM) gender = hermaphroditeGhostsCumEctoplasm;
	...
	private nowOnVickiLake() {
		...
		if (ooze != null && shark != null) {
			...
			MainScreen.simpleChoices(["Shark", "Ooze", "", "", ""], [shark, ooze, null, null, null]);
		}
		else if (ooze != null) return { next: ooze };
	...
	private sharkbustersVictory() {
		...
		if (player.torso.cocks.count > 0 && player.torso.vaginas.count > 0) {
			DisplayText("Which gender would you like to focus on?");
			//male / female)
			MainScreen.simpleChoices(["Male", "Female", "", "", ""], [ghostIzmaPenis, sharkyEctoginas, null, null, null]);
		}
		else if (player.torso.vaginas.count > 0) return { next: sharkyEctoginas };
		else if (player.torso.cocks.count > 0) return { next: ghostIzmaPenis };
	}
	...
	private sharkyEctoginas() {
		...
		//[if repeat]
		if (Flags.list[FlagEnum.TIMED_SHARKGINAS] > 0) DisplayText(" again...");
		DisplayText("</i>\" she pants, brow suddenly furrowed with worry.  ");
		if (Flags.list[FlagEnum.TIMED_SHARKGINAS] === 0) DisplayText("You begin to ask what she's talking about, but the words die in your mouth when");
		else DisplayText("You can only shake your head resignedly as");
		...
		if (player.hasKeyItem("Deluxe Dildo") >= 0) DisplayText(" by the biggest object you've experienced since Tamani's deluxe dildo");
		...
		DisplayText("Some time later, with another, much softer kiss, you draw the spirit back into yourself, leaving a very tired and surprisingly content shark girl in her wake. She reveals her consciousness throughout the experience, going so far as to apologize ");
		//(if repeated: again) 
		if (Flags.list[FlagEnum.TIMED_SHARKGINAS] > 0) DisplayText("again ");

		DisplayText("to the ghost girl for disrupting the spell—though the spirit would have nothing of that, claiming that though the spell was not what she had meant—or even readily knew how—to cast, she enjoyed the experience as well.  With a friendly wave, the now-naked shark girl dives back into her habitual home.\n\n");
	...
	private ghostIzmaPenis() {
		...
		//[if first time]
		if (Flags.list[FlagEnum.TIMES_SHARKPENISED] === 0) DisplayText("whose expression reveals she's as clueless as you are.");
		else DisplayText("who only smiles as she realizes her 'mistake' in her spellcasting once again.");
		DisplayText("  \"<i>That wasn't nearly what the spell should have been,</i>\" the girl muses, licking her lips ");
		if (Flags.list[FlagEnum.TIMES_SHARKPENISED] === 0) DisplayText("absently as she contemplates what could have caused the lengthening of your loins");
		else DisplayText("in anticipation for the experience you both are about to undergo, yet again");
		DisplayText(". Your jaw drops as her lips puff out, suddenly looking much more sultry and far fuller than before. Your spiritual seductress swiftly takes note, as well, and her lips curl up into a mischievous smile ");
		if (Flags.list[FlagEnum.TIMES_SHARKPENISED] === 0) DisplayText("as she deduces the nature of the spell. \"<i>I think... oh my... my host's saliva may have expansive qualities on... sexual aspects of the body,</i>\" she explains. \"<i>So, if I were to do this...</i>\"");
		else DisplayText("\"<i>Would you look at that, my magic spittle still works! How about I just...</i>\"");
		DisplayText("\n\n");

		DisplayText("Before you can stop her, she positions her head over your twitching shaft and opens her mouth wide. Out pours a torrent of drool amassed from a mixture of her arousal and undoubtedly magical influence to splatter against your maleness");
		if (player.torso.balls.quantity > 0) DisplayText(" and softly churning sack");
		...
		if (player.torso.balls.quantity > 0) DisplayText("  Your enchanted testes give an audible groan as you reach climax.");
		else DisplayText("  You feel something stirring in your groin, and you squirm as the tingling intensifies.");
	...
	private ghostGooGurlzDuckfaces() {
		...
		if (player.gender === Gender.HERM) {
			...
			else if (randInt(2) === 0) slimeyPenorsAhoy();
			else slimeGinaFlation();
		}
		else if (player.torso.cocks.count > 0) slimeyPenorsAhoy();
		else slimeGinaFlation();
	}
	...
	private slimeGinaFlation() {
		...
		//([if corruption <50]
		if (player.stats.cor < 50) DisplayText("you beg her to stop before this gets out of hand.");
		...
		DisplayText("Content in taking second place, your anus also stretches as the bulb flows into it. Moments later, before you are even allowed to recover from the first two, your jaw is forced farther open to make room for the bulbous deposit. You hum unhappily as your throat widens, not unlike a croaking frog. Your already-huge belly burgeons with the new additions until it's at least twice as large as ");
		if (player.statusAffects.has(StatusAffectType.MeetWanderer)) DisplayText("both of Markus's testicles, combined!");
	...
	private slimeyPenorsAhoy() {
		...
		if (player.torso.balls.quantity > 0) DisplayText("and teasing your quaking testes ");
		...
		DisplayText("Even if you were to attempt to assist her with the lovemaking, at this point, the ghost girl is too out of it to realize. Keeping your arms and legs pinned, she vigorously goes at her work, her extra-large ass ");
		if (player.torso.hips.legs.isNaga()) DisplayText("slapping against your tail");
		else if (player.torso.hips.legs.isGoo()) DisplayText("smooshing against your goo");
		else DisplayText("swiftly bruising your thighs");
		DisplayText(".  A bestial groan rumbles from the goo-ghost as she continues to ride you, and despite everything, you begin to enjoy the treatment, going so far as to begin bucking your hips in rhythm.");
		if (player.torso.balls.quantity > 0) DisplayText("  The near-vacant-minded girl rewards you with two slime-tentacles to fondle your scrotum.  You hiss in a sharp breath at the sudden stimulation, but swiftly come to welcome it.");
		...
		DisplayText("In your current overfilled state, it takes a mere two minutes of her frenzied ministrations to cause your ");
		if (player.torso.balls.quantity > 0) DisplayText("balls to churn");
		else DisplayText("body to prepare itself for release");
		...
		if (player.torso.balls.quantity > 0) DisplayText("waddle over to where the mewling ghost girl is twitching on the ground; your now-shrinking testicles hindering your gait.");
		else DisplayText("limp over to where the mewling ghost girl is twitching on the ground; your now-shrinking prostate hinders your gait.");
	...
	private genderlessGhostBuster() {
		...
		if (Flags.list[FlagEnum.SHOULDRA_GENDERLESS_FUCK_COUNT] === 0) DisplayText("Curious as to its contents, you glance inside—it's a chest of dildos, of every shape and size. Before you can ask why the hell a ghost would carry a CHEST of dildos, ");
		...
		if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating < 1) DisplayText("your tits, along with the elongated nipples, receding back into a flat chest.");
	...
	private penisLossThatIsntALoss() {
		...
		if (Flags.list[FlagEnum.SHOULDRA_PENIS_DEFEAT_TIMES] === 0) DisplayText("She abruptly gasps in excitement and begins moving your hands towards your groin, though you swiftly reclaim control. \"<i>You have a penis,</i>\" she telepathically pulses to you with an anxious and excited edge to her voice. \"<i>My hosts... they rarely... they...</i>\"");
		else DisplayText("She chuckles softly as she recalls the times you've shared, and she hungrily goes for your maleness once more, though you throw her off of it.");
		DisplayText("  The communication suddenly breaks as the ghost girl's will surges against yours, all thoughts of submissiveness vanishing from her mind.  ");
		//[if intelligence <50] 
		if (player.stats.int < 50) DisplayText("It's not very hard for her to break down your fortifications, and you soon feel yourself being pushed aside by her vast mental capacity.");
		else if (player.stats.int < 90 || player.cockThatFits(80) === -1) DisplayText("You begin to gain an edge, but the ghost girl is craftier than you expected. She begins to pulse mental intrusions at you, scrambling your thoughts just long enough for her to overtake you and push you to the side.  \"<i>You are one hundred years too early to challenge me,</i>\" she chides. \"<i>My intellect has long surpassed most any living creature on this plane.</i>\"");
		else {
			...
			if (player.torso.cocks.count > 1) {
				...
				else DisplayText("two other shafts and pumps them vigorously.");
			}
			DisplayText("\n\n");

			DisplayText("The ghost girl goes to town on your genitals, the combination of your will and her still-rising lust fueling her movements.\n\n");
			...
			if (player.torso.cocks.count === 2) DisplayText("Taking her hand away from your other dick, the ");
			...
			DisplayText("The strange feeling focuses on your testes, making it a bit hard to concentrate on keeping the ghost girl subdued.  Your management of willpower does not waver until you feel your legs being pushed farther apart.  A curious glance turns into a horrified stare as you gaze upon your now ");
			if (player.torso.balls.size >= 9) DisplayText("gargantuan");
			else DisplayText("basketball");
			DisplayText("-sized balls.\n\n");

			DisplayText("Taking advantage of your distraction, your former mind slave pushes you over and jumps on top of you.  Her impotently sopping box leaves streaks of clear fluid across your belly as she continues her work on your cock");
			...
			MainScreen.simpleChoices(["LetHerCum", "KeepHerOut", "", "", ""], [letShouldraIn, kickShouldraOut, null, null, null]);
			return;
		}
		DisplayText("\n\n");
	...
	private ghostBJFinisher(inside: boolean) {
		...
		if (inside) {
			...
			DisplayText("You watch her move off into the ruins with a new spring in her step, chuckling as she goes before heading back to camp yourself.");
		}
	...
	private penisGartuanGhostSmexVictory() {
		...
		if (Flags.list[FlagEnum.SHOULDRA_EXGARTUAN_SPIRIT_SEX_COUNT] === 0) DisplayText("\"<i>Spirit sex... I can't say I've ever tried that,</i>\" she admits. \"<i>Damn if it isn't good.</i>\"\n\n");
		else DisplayText("\"<i>Damn,</i>\" she sighs, \"<i>that is still a great ride, I'd say...</i>\"\n\n");
	...
	private ghostGinaWinSexings() {
		...
		if (Flags.list[FlagEnum.SHOULDRA_VAGINAL_POSSESSIONS] === 0) DisplayText("She 'hmm's several times inside your head, but you can feel a surge of elation as she apparently reaches a solution. \"<i>Of course!</i>\" she cries excitedly.  \"<i>I actually have never done this before!</i>\"\n\n");
		else DisplayText("\"<i>Oh well, I suppose we can always just do that again,</i>\" she muses. \"<i>It worked well enough the first time.</i>\"\n\n");
	...
	private hermaphroditeGhostsCumEctoplasm() {
		...
		//[if repeated] 
		if (Flags.list[FlagEnum.SHOULDRA_HERMSEX_COUNT] > 0) DisplayText("She chuckles softly as she recalls the times you've shared, and she hungrily goes for your maleness once more, though you mentally throw her off of it.  ");
		DisplayText("The communication suddenly breaks as the ghost-girl's will surges against yours, all thoughts of submissiveness vanishing from her mind.  ");
		if (player.stats.int < 50) DisplayText("It's not very hard for her to break down your fortifications, and you soon feel yourself being pushed aside by her vast mental capacity.");
		else if (player.stats.int < 90 || player.cockThatFits(80) === -1) DisplayText("You begin to gain an edge, but the ghost girl is craftier than you expected, and she begins to pulse mental intrusions at you, scrambling your thoughts for just long enough for her to overtake you and push you to the side.  \"<i>You are one hundred years too early to challenge me,</i>\" she chides. \"<i>My intellect has long surpassed most any living creature on this plane.</i>\"");
		else {
			x = player.cockThatFits(80);
			DisplayText("The ghost girl puts up a good fight, but you're more than prepared for her mental assault. You feel her panic well as her intrusions are overwhelmed, and though she attempts to disengage the mental link, you easily stop her. She screams mental protests at you, but your ringing ears only strengthen your psychological dominance over the poor spirit. Before long, she's reduced to a whimpering wreck. \"<i>Out,</i>\" you command, tired of the troublesome specter's meddlings. Of course, she won't even try to rebel now, resignedly accumulating her essence and beginning her exit. Within moments the ghost girl flows out of your body and resolidifies before you.\n\n");
			...
			if (player.torso.balls.quantity === 0) DisplayText("The first effects of the spell make themselves known in a big way when an overladen cumsack forms and drops between your legs.  ");
			DisplayText("The strange feeling focuses on your testes, making it a bit hard to concentrate on keeping the ghost girl subdued.  Your management of willpower does not waver until you feel your legs being pushed farther apart.  A curious glance turns into a horrified stare as you gaze upon your now ");
			if (player.torso.balls.size > 10) DisplayText("gargantuan");
			else DisplayText("basketball");
			DisplayText("-sized balls.\n\n");
	...
	internal function loseToShouldra() {
		...
		//[if dick] 
		if (player.torso.cocks.count > 0) DisplayText("Each pulse pushes her a bit further onto your hardening manhood");
		else if (player.torso.vaginas.count > 0) DisplayText("You can't deny the heat slowly welling up in your loins, your love-tunnel getting more lubricated by the moment");
		else DisplayText("You can't deny the heat slowly starting to gather in your bare loins");
		DisplayText(". Finally, the waistband of her poor garment bursts, and several handfuls of butt spill onto your torso.\n\n");

		DisplayText("Wondering what the point of that was, you raise your head and glance up at her face, now at least four inches higher that she just was. She reaches down and gives your ");
		if (player.torso.cocks.count > 0) DisplayText("quivering member");
		else if (player.torso.vaginas.count > 0) DisplayText("sopping gash");
		else DisplayText("sensitive crotch-flesh");
	...
	private ghostGinaLosses() {
		...
		else DisplayText("\"<i>Oh well, I suppose I can always just do that again,</i>\" she muses. \"<i>It worked well enough the first time.</i>\"");
		DisplayText("\n\n");
	...
	private shouldraGiantCockLoss() {
		...
		//[if first time]
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 0) DisplayText("bemused ");
		else DisplayText("excited ");
		DisplayText("girl. With a flair, she finishes the incantation, then moves to the other goblin and repeats the process. The first goblin, meanwhile, ");
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 0) DisplayText("searches her body for any signs of change, and when she finds none, begins pouting.");
		else DisplayText("resists the urge to smash her tits together, waiting with as much patience as she can muster.");
		DisplayText("  By this time, the ghost girl has finished with the spell, ");
		//[if first time] 
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 0) DisplayText("and instead of offering an explanation, she merely ");
		else DisplayText("and, to the green girl's obvious delight, ");
		DisplayText("reaches over and kneads the goblin's ");
		//([if first time]
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 0) DisplayText("small, firm B-cups");
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 1) DisplayText("perky, sizeable C-cups");
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 2) DisplayText("well-formed, jiggly D-cups");
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 3) DisplayText("big and squishy E-cups");
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 4) DisplayText("oversized, heaving F-cups");
		else DisplayText("gigantic and openly-bared G-cups");
		DisplayText(". The goblin ");
		//[if first time] 
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 0) DisplayText("instantly stops her protests, suddenly too stimulated by the massage to worry about any magical favoritism, ");
		else DisplayText("cries out in ecstasy at the still-incredible sensation, ");
		//[if first time]
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 0) DisplayText("although that doesn't stop the second goblin to begin voicing her own dissent");
		else DisplayText("and you can't help but notice the excited blush running across the second goblin's face");
		DisplayText(". The ghost girl pinches a quickly-stiffening nipple and pulls it out, ");
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 0) DisplayText("and... pulls the breast along with it?");
		else DisplayText("and pulls the breast along with it.");
		DisplayText("  You inwardly gasp ");
		//[if first time] 
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 0) DisplayText("as the effects of the magic are revealed");
		else DisplayText("despite seeing the magic before, always enthralled by the process");
		DisplayText("; it has turned the goblin's breasts into an almost putty-like substance");
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 0) DisplayText(", evidently!");
		else DisplayText(" once again");
		DisplayText(". The ghost girl begins to speed the process up, softly laying the goblin on the ground and kneading out the green girl's tits, adding at least half a cup size with every tug. The second goblin, watching intently behind your back, ");
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 0) DisplayText("gasps in amazement, ");
		else DisplayText("can't restrain her urges any longer, ");
		DisplayText("swiftly reaching down to roughly grab at her own ");
		//[if first time] 
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 0) DisplayText("petite boobs");
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 1) DisplayText("modest bosom");
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 2) DisplayText("sizable chest");
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 3) DisplayText("large endowments");
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 4) DisplayText("huge tits");
		else DisplayText("titanic melons");
		DisplayText(", crying in excitement as her breasts also begin to billow out under her own ministrations.\n\n");

		DisplayText("The first goblin's breasts, at this time, have reached at least ");
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 0) DisplayText("E-cups");
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 1) DisplayText("F-cups");
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 2) DisplayText("G-cups");
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 3) DisplayText("H-cups");
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 4) DisplayText("I-cups");
		else DisplayText("J-cups");
		DisplayText(", and the ghost girl speeds up the massage ever further. Not satisfied with the progress, she raises the goblin's head to your lips and blows a steady gust of wind down the little woman's throat, ballooning her glistening mammaries out much more efficiently. This continues until the goblin nearly collapses under the weight of her new ");
		//[if first time] 
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 0) DisplayText("HH-cups");
		//[if repeated once] 
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 1) DisplayText("II-cups");
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 2) DisplayText("JJ-cups");
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 3) DisplayText("KK-cups");
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 4) DisplayText("LL-cups");
		else DisplayText("MM-cups");
		DisplayText(", and the ghost girl turns your attention to the second goblin, whose boobs are embarrassingly uneven due to her...unpracticed skills. The ghost girl moves you over to her, mercifully evening the currently ");
		//([if first time] 
		if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 0) DisplayText("DD-cup (and C-cup, on the other side)");
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 1) DisplayText("EE-cup (and D-cup, on the other side)");
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 2) DisplayText("FF-cup (and E-cup, on the other side)");
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 3) DisplayText("GG-cup (and F-cup, on the other side)");
		else if (Flags.list[FlagEnum.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] === 4) DisplayText("HH-cup (and G-cup, on the other side)");
	...
	private genderlessShouldrasLossRapes() {
		...
		DisplayText("when she reaches in and rummages about, producing two eight-inch vibrators with bumpy demon-shafts. She moves back to the center of the room, launching into a short spell as she goes. She sets the dildos on the same nightstand and opens the carton, pulling out four large white eggs.\n\n");
	...
	private superCoolPaladinSexStuffDudeGasmJizzMonkies() {
		...
		DisplayText("You look up to gaze into her moderately attractive face. Is it?");
	...
	public paladinModeFollowup() {
		...
		if (model.time.hours < 6 || model.time.hours >= 23) DisplayText("\n\nYou rage fap and consider going back to bed.");
		player.orgasm();
	...
	private slimeGhostGalForHerms(clearOut: boolean = false) {
		...
		//{event occurrence === 0}
		if (Flags.list[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] === 0) DisplayText("I've got this thing all figured out.");
		//event occurrence =1
		else if (Flags.list[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] === 1) DisplayText("I've got this thing all figured out.  For real this time.");
		//event occurrence >=2
		else if (Flags.list[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] === 2) DisplayText("I've got this thing all figured out.  For real this time.  Promise.");
		//event occurrence >=3
		else DisplayText("Well... don't worry TOO much.");
		DisplayText("</i>\"");
		...
		//event occurrence === 0
		if (Flags.list[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] <= 2) DisplayText("Maybe she really does have a firm grip on the reigns of this beast.");
		//event occurrence >=3
		else DisplayText("Regardless, you've been through this dog and pony show enough times to know that something is bound to go wrong anyway.");
		...
		//if cockLength(0) > 24
		if (player.torso.cocks.get(0).length > 24) DisplayText("However... it isn't exactly up to your immense scale.  ");
		...
		//looseness === 0
		if (player.torso.vaginas.get(0).looseness === 0) DisplayText("tight");
		//looseness === 1
		else if (player.torso.vaginas.get(0).looseness === 1) DisplayText("forgiving");
		else if (player.torso.vaginas.get(0).looseness === 2) DisplayText("loose");
		else if (player.torso.vaginas.get(0).looseness === 3) DisplayText("gaping");
		else DisplayText("cavernous");
		DisplayText(", velvety scabbard.  Hands too busy latched onto one another's hips, ");
		if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 0) DisplayText("both you and the spooky slime press your tits together, your swaying thrusts only enhancing the mammary meltdown.");
		...
		if (model.time.hours < 11) DisplayText("morning");
		else if (model.time.hours < 17) DisplayText("afternoon");
		...
		DisplayText("\"<i>Yea, I jumped ship from that senseless slime the moment it finished filling you up to christen the S.S.  Ballsac,</i>\" she explains, her tone free of any responsibility.  \"<i>You're lucky I was around to pick up the slack for you.  ");
		if (Flags.list[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] === 0) DisplayText("You couldn't spank it even to save your life, as we've learned today.</i>\"");
		else DisplayText("Evidently, you haven't put in any meaningful practice since last time,</i>\" the specter chides you while making an exaggerated jerking-off motion.");
		if (Flags.list[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] === 1) DisplayText("  \"<i>I was afraid you'd have... given up the ghost.</i>\"");
		else if (Flags.list[FlagEnum.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] > 1) DisplayText("  \"<i>But I'm more than willing to do it again next time with a conclusion like that!</i>\"");
