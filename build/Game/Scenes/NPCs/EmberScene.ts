...
export class EmberScene extends NPCAwareContent implements TimeAwareInterface {
	...
	public timeChange(): boolean {
		...
		//Ember fuck cooldown
		if (player.statusAffects.get(StatusAffectType.EmberFuckCooldown).value1 > 0) {
			player.addStatusValue(StatusAffects.EmberFuckCooldown, 1, -1);
			if (player.statusAffects.get(StatusAffectType.EmberFuckCooldown).value1 < 1) player.statusAffects.remove("EmberFuckCooldown");
		}
		//Ember napping
		if (player.statusAffects.has(StatusAffectType.EmberNapping)) {
			player.addStatusValue(StatusAffects.EmberNapping, 1, -1);
			if (player.statusAffects.get(StatusAffectType.EmberNapping).value1 <= 0) player.statusAffects.remove("EmberNapping");
		}
		if (followerEmber() && !player.statusAffects.has(StatusAffectType.EmberNapping)) {
			//Mino cum freakout - PC partly addicted!
			if (Flags.list[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] === 1 && !player.perks.has(PerkType.MinotaurCumAddict) && Flags.list[FlagEnum.EMBER_CURRENTLY_FREAKING_ABOUT_MINOCUM] === 0) {
	...
	public timeChangeLarge(): boolean {
		if (!player.statusAffects.has(StatusAffectType.EmberNapping) && followerEmber() && !player.statusAffects.has(StatusAffectType.EmberFuckCooldown)) {
			//Ember get's a whiff of fuckscent and knocks up PC!
			if (player.torso.vaginas.count > 0 && player.inHeat && player.pregnancyIncubation === 0 && randInt(10) === 0 && (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3)) {
				emberRapesYourHeatness();
				return true;
			}
			else if (player.torso.cocks.count > 0 && player.inRut && !pregnancy.isPregnant && randInt(10) === 0 && Flags.list[FlagEnum.EMBER_GENDER] >= 2) {
				emberRapesYourHeatness();
				return true;
			}
		}
	...
	public emberCampMenu() {
		...
		//Moderate Affection:
		else if (emberAffection() <= 75) DisplayText("Ember fidgets as " + emberMF("his", "her") + " tail starts to sway from side to side, " + emberMF("he", "she") + " looks at you and asks, \"<i>What is it?</i>\"");
		else DisplayText("Ember's eyes light up as you close in on " + emberMF("him", "her") + ", and " + emberMF("he", "she") + " smiles nervously.  \"<i>Y-Yes?</i>\"");
		//OPTIONS HERE
		//[APPEARANCE]
	...
	private emberSexMenu(output: boolean = true) {
		if (output) {
			...
			if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) {
				DisplayText(emberMF("His", "Her") + " cock ");
				if (Flags.list[FlagEnum.EMBER_INTERNAL_DICK] === 1 || Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0) DisplayText("is poking out of " + emberMF("his", "her") + " slit");
				else DisplayText("is starting to swell with blood");
				DisplayText(".  ");
			}
			if (Flags.list[FlagEnum.EMBER_GENDER] >= 2) DisplayText("You can see tiny rivulets of moisture starting to run down Ember's inner thighs as they rub together, barely hiding her precious treasure from your hungry eyes.  ");
			DisplayText("Well, " + emberMF("he", "she") + " is a sexy beast; you ask what naturally comes to mind.");
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) {
			...
			if (player.stats.lust >= 33 && player.torso.vaginas.count > 0) {
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] >= 2) {
			...
			if (player.torso.cocks.count > 0 && player.stats.lust >= 33) penetrateHer = penetrateEmbrah;
		}
		if (player.torso.vaginas.count > 0 && player.stats.lust >= 33) getEatenOut = getEatenOutByEmbra;
	...
	public findEmbersEgg() {
		...
		if (Flags.list[FlagEnum.TIMES_FOUND_EMBERS_EGG] === 0) {
			...
			DisplayText("You spot a familiar cave partially hidden behind the mossy vegetation and decide to confirm your suspicion.");
		...
		MainScreen.simpleChoices(["Take It", "Destroy It", "", "", "Leave"], [takeEmbersEggHomeInADoggieBag, destroyBabyEmberYouMonster, null, null, leaveEmbersAssOutToDry]);
	}
	...
	private destroyBabyEmberYouMonster() {
		...
		//[Eat][Leave]
		simpleChoices("Eat It", eatEmbersYolkLikeAnEvenBiggerDick, "", null, "", null, "", null, "Leave", Scenes.camp.returnToCampUseOneHour);
	}
	...
	private eatEmbersYolkLikeAnEvenBiggerDick() {
		...
		//(+20 Corruption)
		//(also slimefeed!)
		dynStats("str", 5 + randInt(5), "tou", 5 + randInt(5), "int", 5 + randInt(5), "cor", 20);
		player.slimeFeed();
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	private takeEmbersEggHomeInADoggieBag() {
		...
		if (player.tallness < 84) DisplayText("  Lifting it isn't as much of a problem as you thought; it's surprisingly light.  It is, however, very big and very awkward to carry.");
		else DisplayText("  Between the egg's surprising lightness, and your own size and wide arms, you can easily carry the egg.");
	...
	public emberCampDesc() {
		...
		//NOT AN EGG! HAHA!
			...
			if (choice === 0) DisplayText("Ember is lying in front of " + emberMF("his", "her") + " excavated den, watching the camp and resting.  Every once in a while " + emberMF("his", "her") + " eyes dart in your direction, before " + emberMF("he", "she") + " quickly looks away.\n\n");
			else if (choice === 1) DisplayText("Ember doesn't seem to be around " + emberMF("his", "her") + " excavated den... it doesn't take long before Ember lands in front of it and then takes off again, apparently occupied with darting around in the sky for fun.\n\n");
			else DisplayText("Ember appears to be fighting to stay awake; sometimes " + emberMF("he", "she") + " falls into a doze, but quickly snaps back to consciousness.\n\n");
		}
	}
	...
	public emberEggInteraction() {
		...
		if (Flags.list[FlagEnum.EMBER_COR] <= 25) {
			DisplayText("  As you observe the egg, it glows with soft, metered pulses and you're overcome with a sense of calm and peace.  Watching them, you feel serene, as if untouched by this world's taint.");
		}
		//(else If the egg Corruption level is 26-74, aka \"<i>Tainted</i>\")
		else if (Flags.list[FlagEnum.EMBER_COR] <= 75) {
			DisplayText("  As you observe the egg, it glows with bright, gaudy pulses and you're overcome with a sense of arrogance and strength.  You feel somehow as if you could do anything, as if you were superior to everything, and no one could dare say otherwise.");
		}
		//(If the egg Corruption level is 75-100, aka \"<i>Corrupt</i>\")
		else {
			DisplayText("  As you observe the egg you realize what you had taken for its pulses are actually just its normal color; the egg is actually 'glowing' with a black light!  As you stare, mesmerized, you begin to consider the pleasing contrast that would result if you covered it in your ");
			if (player.gender === Gender.NONE) DisplayText("cum, if you had any... ");
			else {
				if (player.torso.cocks.count > 0) DisplayText("white ");
				else DisplayText("glistening girl-");
				DisplayText("cum... ");
			}
			DisplayText(" You stop yourself and shake your head.  Where did that thought come from?");
	...
	private useIncubusDraftOnEmber(purified: boolean = false) {
		...
		DisplayText("Uncorking the vial, you drizzle the slimy off-white fluid onto the pointed cone of the egg.  It oozes slowly across the surface, then seeps through the shell, leaving not a drop of moisture.");
		if (Flags.list[FlagEnum.EMBER_GENDER] === 3 || Flags.list[FlagEnum.EMBER_GENDER] === 0) {
			DisplayText("  The egg's shell slowly changes to a soft, pastel blue.");
			Flags.list[FlagEnum.EMBER_GENDER] = 1;
		}
		else if (Flags.list[FlagEnum.EMBER_GENDER] === 2) {
			DisplayText("   The egg's shell slowly changes to a lavender hue.");
	...
	private useSuccubiMilkOnEmber(purified: boolean = false) {
		...
		DisplayText("Popping the cap off of the milk bottle, you pour the contents onto the egg - the porous shell soaks up the milk as fast as you dump it, spilling not a drop.");
		//(If Unsexed or Herm:
		if (Flags.list[FlagEnum.EMBER_GENDER] === 3 || Flags.list[FlagEnum.EMBER_GENDER] === 0) {
			DisplayText("  The egg's shell slowly changes to a muted pink color.");
	...
	private useLactaidOnEmber() {
		...
		//max uses 1
		DisplayText("Feeling a little bemused, you pour the creamy fluid onto the egg.  It is absorbed through the shell, and a spiderwork of creamy yellow vein-like markings suddenly forms on the shell's surface.");
	...
	private hairExtensionSerum() {
		...
		//max uses 2
		DisplayText("Wondering at your motivations, you pour the goblin gunk onto the egg.  Most rolls harmlessly off of the shell, leaving you annoyed at the waste... until you see ");
		if (Flags.list[FlagEnum.EMBER_HAIR] === 0) {
			DisplayText("a narrow tiger-stripe pattern suddenly develop");
			Flags.list[FlagEnum.EMBER_HAIR] = 1;
		}
		else {
			DisplayText("the tiger-stripes multiply");
			Flags.list[FlagEnum.EMBER_HAIR] = 2;
		}
		DisplayText(" on the egg.");
		Flags.list[FlagEnum.EMBER_EGG_FLUID_COUNT]++;
		return { next: playerMenu };
	}
	...
	private giveEmberBludSausages() {
		...
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0) {
			DisplayText(" but the egg just glows its excitement, as if it wanted still more.");
			Flags.list[FlagEnum.EMBER_INTERNAL_DICK] = 1;
		}
		else {
			DisplayText(", but nothing does.  How disappointing.");
	...
	private masturbateOntoAnEgg() {
		...
		if (player.gender === Gender.MALE) {
			...
			else DisplayText("wave");
			DisplayText(" of cum onto the egg.");
		...
		//Female Version (Z)
			...
			if (player.torso.vaginas.get(0).wetness <= 3) DisplayText("few drops");
			else if (player.torso.vaginas.get(0).wetness < 5) DisplayText("squirt");
			else DisplayText("torrent");
			DisplayText(" of girlcum jumps from your pussy onto the egg.");
	...
	private hatchZeMuzzles() {
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] === 0) DisplayText("white ");
		else if (Flags.list[FlagEnum.EMBER_GENDER] === 1) DisplayText("blue ");
		else if (Flags.list[FlagEnum.EMBER_GENDER] === 2) DisplayText("pink ");
		else DisplayText("purple ");
		DisplayText("dust.");
		//FURRAH
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0) {
			//Male Anthro (Z)
			if (Flags.list[FlagEnum.EMBER_GENDER] === 1) {
				...
				if (Flags.list[FlagEnum.EMBER_MILK] > 0) DisplayText(", drooling nipples notwithstanding");
				DisplayText(".");
			}
			//Female Anthro (Z)
			else if (Flags.list[FlagEnum.EMBER_GENDER] === 2) {
				...
				DisplayText(" the monster's crotch; there, you see that the fine scales actually separate to reveal a slick-looking pussy.  She's clearly a female, with no noteworthy 'additions' that you can see.");
			}
			//Herm Anthro (Z)
			else {
				DisplayText("\n\nIt's huge, standing 7 feet tall at the very least.  Its build is lean and slender, with powerful arms and legs that end in reptilian claws, complete with splay-toed feet capped by menacing talons.");
		...
		//Boring version
			...
			if (Flags.list[FlagEnum.EMBER_GENDER] === 1) {
				...
				if (Flags.list[FlagEnum.EMBER_MILK] > 0 || Flags.list[FlagEnum.EMBER_HAIR] > 0) DisplayText(", complete with ");
				if (Flags.list[FlagEnum.EMBER_MILK] > 0) {
					DisplayText("huge breasts, easily F-cups");
					if (Flags.list[FlagEnum.EMBER_HAIR] > 0) DisplayText(" and ");
				}
				if (Flags.list[FlagEnum.EMBER_HAIR] > 0) DisplayText("long, feminine locks of hair");
				DisplayText(", but his maleness is undeniable.  Especially when you spot ");
				if (Flags.list[FlagEnum.EMBER_INTERNAL_DICK] > 0) DisplayText("the slit in his pelvis that disgorges a foot and a half-long inhuman penis");
				else DisplayText("a foot and a half long human penis that sways between his legs");
				DisplayText(", completed by apple-size nuts held inside a fleshy sack.");
			}
			...
			//Herm Monstergirl (Z)
				...
				if (Flags.list[FlagEnum.EMBER_INTERNAL_DICK] === 0) DisplayText("huge, human prick");
				else DisplayText("huge inhuman cock hanging from some kind of internal sheath");
				DisplayText(" and apple-size nuts held inside a fleshy sack slung under - the ensemble hangs nearly level with her knees.  She... or he? is obviously a hermaphrodite.");
			}
		}
		//Aftermath (Z)
	...
	private embersAppearance() {
		...
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0) {
			...
			if (Flags.list[FlagEnum.EMBER_GENDER] === 1) DisplayText(" not overly muscled.");
			else DisplayText(" feminine.");
			...
			if (Flags.list[FlagEnum.EMBER_GENDER] === 1) {
				...
				if (Flags.list[FlagEnum.EMBER_MILK] > 0) DisplayText(" dribbling breasts,");
				DisplayText(" snout, wings, and horns, gives him the silhouette of a prince from your village's stories: dashing and handsome.");
			}
			//(Female/Herm)
			else {
			...
			if (Flags.list[FlagEnum.EMBER_HAIR] === 1) {
				if (Flags.list[FlagEnum.EMBER_GENDER] === 1) DisplayText("  Short ");
				else DisplayText("  Shoulder-length ");
				DisplayText("steel-gray hair sprouts from " + emberMF("his", "her") + " head.  You'd think that a dragon with hair would look weird, but it actually compliments Ember's looks very well.");
			}
			//(if Ember has a level 2 mane)
			else if (Flags.list[FlagEnum.EMBER_HAIR] === 2) {
				DisplayText("  Tracing " + emberMF("his", "her") + " spine, a mane of hair grows; starting at the base of " + emberMF("his", "her") + " neck and continuing down " + emberMF("his", "her") + " tail, ending on the tip of " + emberMF("his", "her") + " tail in a small tuft.  It is the same color as the hair on " + emberMF("his", "her") + " head, but shorter and denser; it grows in a thick vertical strip, maybe two inches wide.  It reminds you vaguely of a horse's mane.");
			}
			...
			DisplayText("  The ");
			if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0) DisplayText("little ");
			DisplayText("exposed flesh of Ember's body is a light shade of pink; but flushes when " + emberMF("he", "she") + "'s aroused, drawing your eyes towards " + emberMF("his", "her") + " most sexual parts.");

			switch (pregnancy.event) {
				case 2: DisplayText("  At the moment there's a slight pudgyness to her golden belly.");
					break;
				case 3: DisplayText("  Her golden belly is a little larger and firmer than usual.");
					break;
				case 4:
				case 5: DisplayText("  Her golden belly has grown quite a bit.  Ember often rests her hand on it, especially when she sees you looking at it.");
					break;
				case 6: DisplayText("  Her large pregnant belly has forced Ember to change her posture.  It looks like she has a big golden bullseye painted on her stomach.  Ember looks tired but happy.");
					break;
				case 7: DisplayText("  Her swollen belly is as large as that of any pregnant woman you can remember from Ingnam and you " + (Flags.list[FlagEnum.EMBER_OVIPOSITION] > 0 ? "can easily make out the egg's outline under her golden skin" : "occasionally see movement as the baby shifts in Ember's womb") + ".");
					break;
				case 8:
				case 9: DisplayText("  Ember's golden belly is stretched taut by the large " + (Flags.list[FlagEnum.EMBER_OVIPOSITION] > 0 ? "egg" : "baby") + " in her womb.  You're sure she'll give birth very soon, there just isn't room for the " + (Flags.list[FlagEnum.EMBER_OVIPOSITION] > 0 ? "egg" : "baby") + " to grow any larger.");
					break;
				default:
			}
			...
			if (Flags.list[FlagEnum.EMBER_MILK] > 0 || Flags.list[FlagEnum.EMBER_GENDER] >= 2) DisplayText("F-cup soft, pillowy breasts");
			else DisplayText("flat manly pecs");
			DisplayText(" covered in fine scales excepting " + emberMF("his", "her") + " areolas; 0.5 inch nipples protrude from the center of the ");
			if (Flags.list[FlagEnum.EMBER_MILK] > 0 || Flags.list[FlagEnum.EMBER_GENDER] >= 2) DisplayText("generous mounds");
			else DisplayText("masculine pectorals");
			DisplayText(".");
		...
		//Dragon-girl Appearance (By Radar) (Z)
			...
			DisplayText("Ember is a 7' 3</i>\" tall ");
			if (Flags.list[FlagEnum.EMBER_GENDER] === 1) DisplayText("male");
			else if (Flags.list[FlagEnum.EMBER_GENDER] === 2) DisplayText("female");
			else DisplayText("hermaphrodite");
			DisplayText(" dragon-" + emberMF("boy", "girl") + ", with slender limbs and a thin frame; " + emberMF("he", "she") + " refuses to wear any kind of clothing and exposes " + emberMF("him", "her") + "self naturally to the world.  " + emberMF("He", "She") + " sports a rather human looking face, but with several key differences.  Where a normal set of human eyes would be located, instead a pair of orange, reptilian eyes stares back at you, filled with immeasurable pride and ferocity.");
			...
			//Short:
			if (Flags.list[FlagEnum.EMBER_HAIR] < 1) DisplayText("is quite short, giving " + emberMF("him", "her") + " that definitive " + emberMF("masculine", "tomboy") + " look.");
			else DisplayText("drops down to " + emberMF("his", "her") + " shoulders, giving " + emberMF("him", "her") + " the look of the " + emberMF("handsome", "beautiful") + " warriors from your village legends.");

			DisplayText("\n\n" + emberMF("His", "Her") + " chest is also human in appearance and houses a pair of ");
			if (Flags.list[FlagEnum.EMBER_MILK] > 0 || Flags.list[FlagEnum.EMBER_GENDER] >= 2) DisplayText("F-cup breasts that support 0.5 inch nipples and hang heavily; you idly wonder if " + emberMF("he", "she") + "'ll develop lower back problems as she spends more time in Mareth");
			else DisplayText("manly pectorals with 0.5 inch nipples");
			DisplayText(".  Just below " + emberMF("his", "her") + " collarbone, in the middle of " + emberMF("his", "her") + " chest, you see what looks like a small, golden, heart-shaped scale; adorning the chest like a birthmark of some sort.");
			...
			if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) {
				if (Flags.list[FlagEnum.EMBER_INTERNAL_DICK] === 0) DisplayText("  " + emberMF("He", "She") + " sports a flaccid penis and a pair of apple-sized balls that sit dangerously exposed to the elements, let alone to naked blades or a heavy blow.  Yet " + emberMF("he", "she") + " doesn't seem concerned about that in the least, almost daring anyone to focus on them.  While " + emberMF("he", "she") + " isn't aroused right now, Ember's penis can reach a length of approximately 16 inches, and it looks to be about 2 inches thick.");

				else DisplayText("  " + emberMF("He", "She") + " sports what looks like a protective slit of some sort, protecting " + emberMF("his", "her") + " dick from the elements as well as stray blows.  You can't see it right now; but you remember it to be about 16 inches long and 2 inches thick.");
			}
			//Ember has a pussy:
			if (Flags.list[FlagEnum.EMBER_GENDER] >= 2) DisplayText("  The inviting lips of a human-looking pussy purse between her legs; some moisture seems to have gathered on her labia, giving it a slick look that just begs for attention.");
	...
	private talkToEmber() {
		...
		DisplayText().clear();
		DisplayText("What will you talk about?");
		//Else you can pick one of three topics:
		//Talk about Dragons
		//Talk about Exploring
		//Talk about Yourself
		MainScreen.simpleChoices(["Dragons", "Exploring", "Yourself", "", "Back"], [talkToEmberAboutDragonzzz, discussExplorationWithEmber, talkToEmberAboutYourself, null, emberCampMenu]);
	}
	...
	private talkToEmberAboutDragonzzz() {
		...
		if (choice === 0) {
			...
			if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 1) DisplayText(", a blush creeping onto " + emberMF("his", "her") + " cheeks");
			DisplayText(".");
	...
	private discussExplorationWithEmber() {
		...
		DisplayText("You ask Ember for news of anything interesting " + emberMF("he", "she") + " has seen during " + emberMF("his", "her") + " aerial explorations of Mareth.");
		...
		if (choice === 0) {
			...
			if (subChoice === 0) DisplayText("imps. Only a few imps, and they seemed very nervous. I'm guessing that whatever naturally lives there is something that they really don't want to get involved with.");
			else if (subChoice === 1) DisplayText("two goblins in the foothills, arguing with each other. One was saying that the other shouldn't go up into the hills, as apparently the minotaurs that live there are too much for an untrained girl like her to take without being split open. The second goblin just laughed at her, called her a 'wimp' and told her that she's going to get herself 'some juicy bull-cock and tasty mino-spooge'. Ugh, disgusting little creatures.");
			else if (subChoice === 2) DisplayText("a pair of muscle-bound bull-men beating on each other with their bare fists. They spent over an hour smashing each other into a bloody pulp, and then the winner promptly started fucking the loser up the ass.  I had seen more than enough by that point, so I left.");
			else if (subChoice === 3) DisplayText("this... creature... that looked kind of like a human woman, but with a big dick where her clit should be.  She was walking around stark naked, 'cept for a bunch of piercings, and leading this bull-man along like a pet by a chain attached to a ring anchored into his cockhead.");
			else if (subChoice === 4) DisplayText("a couple of goblins sharpening scissors on some rocks outside of a cave with a door on it. Weird. Wonder what they could be doing in there?");
			else if (!player.statusAffects.has(StatusAffectType.WormsOff) && randInt(2) === 0) DisplayText("a horrible swarm of slimy white worms, clumped together into a mockery of a human form and squelching along.  It managed to latch onto this two-headed dog-creature and... ugh!  The worms started forcing their way into both of its cocks!  I've never seen anything so disgusting!");
			else if (subChoice === 5) DisplayText("this two-headed dog-morph loping around; it spotted an imp, dropped to all fours, then gave chase. Managed to catch the ugly little demon, whereupon it ass-raped it, then ate it.");
		}
		else if (choice === 1) {
			subChoice = randInt(5);
			...
			if (subChoice === 0) DisplayText("a whole tribe of imps, just lounging around in a glade, jerking themselves off or squabbling over food.  Nasty little things, but easily dispatched.");
			else if (subChoice === 1) DisplayText("a  goblin with a huge pregnant belly, laughing to herself and swilling down that ale they brew, slopping it all over herself.  Little hedonists.");
			else if (subChoice === 2) DisplayText("this strange bee-woman creature... she made this, this music that started messing with my head.  I spat a tongue of flames at her and she flew away in fright, luckily.");
			//(If player has impregnated Tamani)
			else if (subChoice === 3 && Flags.list[FlagEnum.TAMANI_NUMBER_OF_DAUGHTERS] > 0) {
				DisplayText("that green-skinned baby-making whore, Tamani.  She was letting some of her daughters suckle from her and grinning ear to ear as she named the 'prize catch' she got to father them, exhorting them to hunt him down.");
			...
			if (subChoice === 0) {
				DisplayText("a pair of shark-women - well, one was a woman, the other had breasts but also had a cock");
				if (Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText(" like me");
				DisplayText(". They were on the beach, fucking each other's brains out.");
			}
			else {
				//(If Whitney's Farm is not yet in the Places menu)
				if (randInt(3) === 0 && player.statusAffects.get(StatusAffectType.MetWhitney).value1 <= 1) DisplayText(" a big farm on the shoreline.  I saw some sort of cow-woman tending to fields of peppers, and a dog-woman herding cattle. They looked pretty fat and tasty, too... What?  I meant the cattle, stupid!  And I wouldn't have taken them - it's no fun snatching up livestock.  I prefer to chase down a deer or a wild auroch or something like that.");
				//(if Fetish Cult is encounterable)
				else if (randInt(3) === 0 && player.statusAffects.has(StatusAffectType.FetishOn)) DisplayText("a pair of humans, arguing about sex.  They both wanted it, but the guy said he wanted to do the 'naughty schoolboy and female teacher' routine, while the girl wanted to do the 'noblewoman and love-servant' routine.  Weird; their clothes kept changing back and forth as they argued.");
				//(If Beautiful Sword has not been acquired)
				else if (randInt(3) === 0 && !player.statusAffects.has(StatusAffectType.TookBlessedSword) && !player.statusAffects.has(StatusAffectType.BSwordBroken)) DisplayText("a sword jammed into a tree.  Weird; what kind of idiot would stick a weapon there like that?  And what kind of weakling wouldn't be able to take it out?");
				else if (randInt(3) === 0 && !player.statusAffects.has(StatusAffectType.CampRathazul)) DisplayText("a smelly rat-man moping around while some weird equipment bubbled and boiled.  I think maybe he was an alchemist.");
				else DisplayText("a great blob of green goo, sliding along and minding its own business.  I could swear it looked up at me once, and grew a penis... that can't be right, though.");
			}
		}
		else if (choice === 3) {
			subChoice = randInt(4);
			...
			if (subChoice === 0) DisplayText("a woman with four big breasts, squeezing milk out of her tits and into the sand.  I didn't know breasts could hold that much milk!");
			else if (subChoice === 1) DisplayText("a whole tribe of demons, lounging around an oasis.  Would have been too much bother to kick the crap out of them, so I left them alone - well, alright, I did buzz them to make them scatter like scared sheep for fun.");
			//(if player hasn't solved Marcus & Lucia's argument)
			else if (randInt(2) === 0 && !player.statusAffects.has(StatusAffectType.WandererDemon) && !player.statusAffects.has(StatusAffectType.WandererHuman)) DisplayText("a human with balls so big he had to carry them in a wheelbarrow, trundling through the wasteland with a succubus.  They were arguing about whether or not he should become an incubus.");
			else {
				DisplayText("this strange creature, like a woman with a snake's tail for legs, slithering through the sand.  ");
				if (player.torso.hips.legs.isNaga()) DisplayText("She looked a lot like you.");
				else DisplayText("I've never seen anything like her before.");
			}
		}
		DisplayText("</i>\"");
		return { next: Scenes.camp.returnToCampUseOneHour };
	...
	private talkToEmberAboutYourself() {
		...
		//(If PC Str < 50)
		if (player.stats.str < 50) DisplayText("  \"<i>You could use more training; you look pretty weak...</i>\" Ember says, critically.");
		//(If PC Str < 90)
		else if (player.stats.str < 90) {
			DisplayText("  \"<i>You're well on your way to being as strong as a dragon, but there's still room for improvement.</i>\" Ember says, pleased.");
			//(+1 point)
			points++;
		}
		//(If PC Str >= 90)
		else {
			DisplayText("  \"<i>With muscles like these, you could easily lift anything you wanted... even me...</i>\" Ember trails off dreamily, almost purring.  \"<i>N-Not that I'd want you to carry me in your arms or anything...</i>\"");
			//(+2 points)
			points += 2;
		}
		DisplayText("\n\nNext, Ember pokes at your chest and your ribs.  ");
		//(If PC Tou < 50)
		if (player.stats.tou < 50) DisplayText("\"<i>I don't know how you could hope to survive out there.  You look like the wind could blow you away.</i>\"");
		//(If PC Tou < 90)
		else if (player.stats.tou < 90) {
			DisplayText("\"<i>Not bad; some more training and you could be as tough as me!</i>\" Ember says, pleased.");
			//(+1 point)
			points++;
		}
		//(If PC Tou >= 90)
		else {
			DisplayText("\"<i>Your body's as tough as rock, you should show off more frequently...  N-Not that I'd want you to go naked!  You should always wear something to protect yourself,</i>\" " + emberMF("he", "she") + " quickly adds.");
			//(+1 Affection)
			emberAffection(1);
			//(+2 points)
			points += 2;
		...
		//(If PC Int < 50)
		if (player.stats.int < 50) DisplayText("\"<i>You're still very naive... anyone or anything could trick you at any time...</i>\"  Ember frowns.");
		//(If PC Int < 90)
		else if (player.stats.int < 90) {
			DisplayText("\"<i>I see wisdom in your eyes, but you could always use more lessons.</i>\"");
			//(+1 point)
			points++;
		}
		//(If PC Int >= 90)
		else {
			DisplayText("Ember's eyes widen.  \"<i>I see great wisdom in your eyes...</i>\"  Ember's face grows more absent by the minute as " + emberMF("he", "she") + " looks at you in deep thought, until the trance breaks.  " + emberMF("He", "She") + " blows at your face, brushing a claw on your forehead.  \"<i>Y-You had something in your face... I wasn't staring into your steely eyes.</i>\"");
			//(+2 points)
			points += 2;
		}
		...
		//(If PC Spd < 50)
		if (player.stats.spe < 50) DisplayText("You try to block the pebble, but you're not quick enough.  It hits your belly, not hard enough to hurt.  \"<i>You need to work on your reactions; anything could surprise you out there and you'd be helpless.</i>\"");
		//(If PC Spd < 90)
		else if (player.stats.spe < 90) {
			DisplayText("You successfully deflect the pebble.  \"<i>Not bad!  Next time try to catch it,</i>\"  Ember says, pleased.");
			//(+1 point)
			points++;
		}
		//(If PC Spd >= 90)
		else {
			DisplayText("You easily catch the pebble and throw it back at Ember, surprising " + emberMF("him", "her") + ".  \"<i>With reflexes like these, you could even...</i>\" Ember mumbles.  \"<i>D-Don't even think about it!</i>\" " + emberMF("he", "she") + " snaps.");
			//(+2 points
			points += 2;
		}
		...
		if (player.nagaScore() >= 3 || RaceScore.lizardScore(player) >= 3 || player.dragonScore() >= 3) {
			DisplayText("  Ember's eyes linger on your form.  After a moment of awkward silence, you clear your throat.  " + emberMF("he", "she") + " blinks and says hurriedly.  \"<i>Sorry... I was just admiring you-</i>\" Realizing what " + emberMF("he", "she") + " was about to say,and quickly blurts out.  \"<i>I mean the weather! Yes, nice day today isn't it?</i>\"  You're not convinced, but let it slide. Ember recomposes and clears " + emberMF("his", "her") + " throat before saying.");
			//(+1 Affection)
			points++;
		}
		DisplayText("\n\n\"<i>All right, here's what I think about you.</i>\"  ");
		if (points < 2) {
			DisplayText("\"<i>This is not good at all; it's a miracle you even managed to survive in this world thus far.  With the things I've seen roaming the land...</i>\" Ember trails off, concerned.  \"<i>You should stay in the camp; I'll help you train.</i>\"  You don't much like the assessment.");
		}
		//(else if points < 6)
		else if (points < 6) {
			DisplayText("\"<i>You're doing fine, but make sure you don't slack off, and keep training.</i>\"  You thank Ember for sharing " + emberMF("his", "her") + " thoughts.");
		}
		//(else)
		else {
			DisplayText("\"<i>You're quite a catch... if you strolled down the street you'd have dragons fawning all over you...</i>\"  Realizing what " + emberMF("he", "she") + " just said, Ember coughs furiously.  \"<i>I-I mean... lesser dragons might fawn all over you.  You don't meet my standards!</i>\"");
			emberAffection(5);
		}
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	private manEmberBitchesAboutPCPregnancy() {
		...
		//(Low Affection)
		if (emberAffection() <= 25) DisplayText("The two of you talk about nothing in particular.  It's light, airy and pointless.  When you finish up, though, you realize something odd; Ember was doing " + emberMF("his", "her") + " best all throughout the conversation to avoid looking at your pregnant belly - almost as if " + emberMF("he", "she") + " were upset by it?");
		//(Moderate Affection)
		else if (emberAffection() < 75) DisplayText("Ember stares coldly at your gravid midriff as icebreaker after icebreaker from you falls flat.  You ask what's wrong.  \"<i>Nothing.  Nothing is wrong,</i>\" Ember states, flatly.");
		//(High Affection)
		else DisplayText("The expression Ember gives you is very cold.  \"<i>What is the meaning of this!?</i>\" " + emberMF("he", "she") + " says, motioning towards your bloated belly.  \"<i>You're supposed to help <b>me</b> breed more dragons; not slut around with trash!</i>\"  Ember blows out an indignant puff of smoke and walks away.");
	}
	...
	private emberIsPregnantFirstTimeTalkScene() {
		...
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 1) DisplayText(" and blushes");
		DisplayText(", one of her hands rubbing her belly lovingly and tenderly.  \"<i>W-what is it?  Why are you looking at me like that?  Something wrong?</i>\"");

		DisplayText("\n\nYou merely give her a faint smile and tell her that she's beautiful when pregnant.");
		if (player.stats.cor >= 66) DisplayText("  You almost choke on those cheesy words; still, she'll never put out for you if you don't butter her up.");
		...
		if (player.stats.cor >= 66) DisplayText("  Is she seriously buying this nonsense?  Your smile would be miles wide if you let your lip curl even a bit, so you keep a straight face with due dilligence.");
		...
		if (player.stats.cor < 30) DisplayText("  Given her attitude and nature, you know how lucky you are to have someone like her want to be your mate.");
	...
	public emberGetOverFreakingOutAboutMinoJizz() {
		...
		//(High Affection)
			...
			//(Male/HermEmber:
			if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText(", especially for sucking cock,");
			DisplayText(" you should come to me instead; I'll help you.</i>\"  Then as if realising what " + emberMF("he", "she") + "'s just implied, Ember looks away; and hurriedly adds, \"<i>But not the way you're thinking...</i>\"");
	...
	private bloodForTheBloodGod() {
		...
		//(Medium Affection)
			...
			if (player.stats.cor < 50) DisplayText(", expressing your gratitude; gently, you kiss " + emberMF("his", "her") + " fingers, then the cut, letting its cool, iron-tinted tang roll across your lips and tongue.  You carefully lick, trying to avoid causing pain as you drink that which " + emberMF("he", "she") + " has so generously offered you.");
			else DisplayText(", quickly drawing it to your mouth and forming a seal around the cut with your lips, anxious to let not a drop escape.");
			...
			//[Continue][Stop]
			MainScreen.simpleChoices(["Continue", "Stop", "", "", ""], [drinkDeeplyOfDagronBlud, noMoDagronBlud, null, null, null]);
		}
		//(High Affection)
		else {
	...
	private noMoDagronBlud() {
		...
		if (emberAffection() < 75) {
			...
			DisplayText("You decide to stop for now and pull away.  Ember licks " + emberMF("his", "her") + " lips, draws " + emberMF("his", "her") + " tongue back into " + emberMF("his", "her") + " mouth and purrs with delight.  When " + emberMF("he", "she") + " realizes what " + emberMF("he", "she") + "'s doing, though, " + emberMF("he", "she") + " sobers.  \"<i>D-Don't get any strange ideas...</i>\"");
	...
	private emberTFs() {
		...
		//Gain Dragon Dick
		if (changes < changeLimit && player.dragonCocks() < player.torso.cocks.count && randInt(3) === 0) {
			temp = 0;
			let choices: Array = [];
			let select: number;
			...
			while (temp > 0) {
				temp--;
				if (player.torso.cocks.get(temp).type != CockType.DRAGON) choices[choices.length] = temp;
			}
			...
			changes++;
			//Apply the TF
			player.torso.cocks.get(select).type = CockType.DRAGON;
			player.torso.cocks.get(select).knotMultiplier = 1.3;
		}
		//Gain Dragon Head
		if (changes < changeLimit && randInt(3) === 0 && player.torso.head.face.type != FaceType.DRAGON && Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0) {
			...
			player.torso.head.face.type = FaceType.DRAGON;
			changes++;
		}
		//-Existing horns become draconic, max of 4, max length of 1'
		if (player.torso.head.hornType != HornType.DRACONIC_X4_12_INCH_LONG && changes < changeLimit && randInt(5) === 0) {
			//No dragon horns yet.
			if (player.torso.head.hornType != HornType.DRACONIC_X2 && player.torso.head.hornType != HornType.DRACONIC_X4_12_INCH_LONG) {
				//Already have horns
				if (player.torso.head.horns > 0) {
					//High quantity demon horns
					if (player.torso.head.hornType === HornType.DEMON && player.torso.head.horns > 4) {
						...
						player.torso.head.horns = 12;
						player.torso.head.hornType = HornType.DRACONIC_X4_12_INCH_LONG;
					}
					else {
						...
						player.torso.head.hornType = HornType.DRACONIC_X2;
						if (player.torso.head.horns > 13) {
			...
			//ALREADY DRAGON
				...
				if (player.torso.head.hornType === HornType.DRACONIC_X2) {
					if (player.torso.head.horns < 12) {
						if (randInt(2) === 0) {
							DisplayText("\n\nYou get a headache as an inch of fresh horn escapes from your pounding skull.");
							player.torso.head.horns += 1;
						}
						else {
							...
							player.torso.head.horns += 2 + randInt(4);
						}
						if (player.torso.head.horns >= 12) DisplayText("  <b>Your horns settle down quickly, as if they're reached their full size.</b>");
						changes++;
					}
					//maxxed out, new row
					else {
		...
		//Gain Dragon Ears
		if (changes < changeLimit && randInt(3) === 0 && player.torso.head.earType != EarType.DRAGON) {
			player.torso.head.earType = EarType.DRAGON;
		...
		//Gain Dragon Legs
		if (player.hips.legs.type != LegType.DRAGON && changes < changeLimit && randInt(3) === 0) {
			//(if drider)
			if (player.hips.legs.type === LegType.DRIDER_LOWER_BODY)
			...
			//(If hoofed legs)
			else if (player.hips.legs.type === LegType.HOOFED) {
			...
			player.hips.legs.type = LegType.DRAGON;
			changes++;
		}
		//Gain Dragon Tail
		 ...
		 */
		//Grow Dragon Wings
		if (player.torso.wings.type != WingType.DRACONIC_LARGE && changes < changeLimit && randInt(3) === 0) {
			if (player.torso.wings.type === WingType.NONE) {
				...
				player.torso.wings.type = WingType.DRACONIC_SMALL;
				player.torso.wings.desc = "small, draconic";
			}
			//(If Small Dragon Wings Present)
			else if (player.torso.wings.type === WingType.DRACONIC_SMALL) {
		...
		if (player.dragonScore() >= 4 && changes < changeLimit && !player.perks.has(PerkType.Dragonfire)) {
	...
	private emberIsAnEggFactory() {
		...
		if (emberAffection() <= 25) {
			...
			if (player.stats.cor >= 40) DisplayText("or sell it for quick cash ");
			DisplayText("anyway, but you don't tell her that.");
			...
			if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) {
				DisplayText(" and her ");
				if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0 || Flags.list[FlagEnum.EMBER_INTERNAL_DICK] > 0) DisplayText("dragon-dick jutting from its slit,");
				else DisplayText("veiny, bulbous penis fully");
				DisplayText(" erect");
			}
			DisplayText(".  One of her hands gently teases her clit, as another traces her outer labia; she bites her lips, trying to stifle her moans of pleasure, but it's useless... every touch brings a sigh.");
			...
			//[(if Ember has a dick)
			if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText(" and her throbbing cock is soft enough to hide");
			DisplayText(" before she presents the egg to you.");
			...
			inventory.takeItem(consumables.DRGNEGG, Scenes.camp.returnToCampUseOneHour);
		}
		//(Medium Affection)
		else if (emberAffection() < 75) {
			DisplayText("\n\nEmber gasps");
			if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 1) DisplayText(" and her cheeks grow red with embarrassment");
			DisplayText(".  \"<i>T-This...</i>\"  She can't quite bring herself to finish the sentence and settles for just looking at the ground.");
		...
		//(High Affection)
			...
			if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText(", and her cock points you accusingly");
			DisplayText(".");
	...
	private dontWatchEmberLayEgg() {
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText(".. it even drips with some kind of off-white fluid.");
		DisplayText("  \"<i>H-here's your egg.  Use it while it's fresh, okay?</i>\"  Her eyes glaze over a bit at the suggestion, and she giggles.  ");
	...
	private watchMediumAffectionEmberEggLay() {
		...
		//(if Ember has a cock:
		if (Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText(", cock poking straight out");
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0 || Flags.list[FlagEnum.EMBER_INTERNAL_DICK] > 0) DisplayText(" of its hiding place");
		DisplayText(".  \"<i>Okay... but you're only allowed to watch!  If you do anything weird, I swear I'll smack you!</i>\"");
		DisplayText("\n\nYou promise her that you won't do anything");
		if (player.stats.lib >= 50) DisplayText(" she doesn't ask you to, beaming a salacious grin");
		DisplayText(".");
		...
		if (player.gender > 0) {
			DisplayText("; ");
			if (player.torso.cocks.count > 0) {
				DisplayText("the bulge inside your [armor]");
				if (player.torso.vaginas.count > 0) DisplayText(" and ");
			}
			if (player.torso.vaginas.count > 0) DisplayText("wetness gathering in your own pussy");
			DisplayText(" more than indicate");
			if (player.torso.cocks.count <= 0 || player.torso.vaginas.count <= 0) DisplayText("s");
			DisplayText(" your desire to break your promise.");
		}
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText(" and caressing her throbbing cock");
		DisplayText(".");
		DisplayText("\n\nWith a deep moan, Ember shakes and orgasms.");
		if (Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText("  Ropes of jism spew out of her cock, arching into the air to hit the ground nearby.");
		DisplayText("  The egg plugs her wet pleasure hole, preventing any liquid from escaping, until finally with a wet, squelching pop,it flies out of her pussy and into your waiting hands; releasing a flood of juices.");
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText(" and her cock hanging out");
		DisplayText(", it's hard to see her as the same angrily defensive ice queen from before.");
	...
	private getMilkFromEmber() {
		...
		if (emberAffection() <= 25) {
			...
			//noEmberkids:
			if (Flags.list[FlagEnum.EMBER_EGGS] === 0 && emberChildren() === 0) DisplayText(emberMF("he", "she") + " has no offspring to feed them with");
			//(1+ EmberEggs:
			else if (Flags.list[FlagEnum.EMBER_EGGS] > 0) DisplayText(emberMF("his", "her") + " children haven't hatched yet");
			else DisplayText(emberMF("his", "her") + " children don't drink nearly enough to properly empty " + emberMF("him", "her") + "), so is " + emberMF("he", "she") + " just going to suffer with " + emberMF("his", "her") + " breasts so full and aching?");
			...
			if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0) DisplayText("scales");
			else DisplayText("skin");
			DisplayText(".  " + emberMF("He", "She") + " lets out a cute gasp of pleasure, eyes darting furiously around to look at anything but you. You beam a smile and " + emberMF("he", "she") + " turns her head away in embarrassment.  \"<i>T-They're not getting any emptier like this, y'know!?</i>\"");
			...
			if (Flags.list[FlagEnum.EMBER_GENDER] != 2) {
				...
				if (!player.torso.hips.legs.isTaur()) DisplayText("belly");
				else DisplayText("back");
				DisplayText(" and look down at Ember's engorging shaft.  Smiling around Ember's nipple, you start to gently stroke and squeeze the partially-erect draconic dick " + emberMF("he", "she") + "'s presenting.  If " + emberMF("he", "she") + "'s enjoying this so much, well, why not make it all the more fun for " + emberMF("him", "her") + "?");
			}
			//(else)
			else {
			...
			if (player.statusAffects.has(StatusAffectType.DragonBreathCooldown)) {
				...
				DisplayText("  Your throat feels soothed as the scratching and soreness die down; you feel like you could shout to the mountaintops!");
			}
		...
		//(High Affection)
			...
			if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) {
				DisplayText("the faintest hint of " + emberMF("his", "her") + " cock ");
				if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0 || Flags.list[FlagEnum.EMBER_INTERNAL_DICK] > 0) DisplayText("emerging");
				else DisplayText("hardening");
			}
			else DisplayText("small beads of moisture gathering on her honeypot");
			DisplayText(".  \"<i>Ok... I suppose I could use some help draining myself, but no funny moves!</i>\"");
			...
			if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) {
				DisplayText(emberMF("his", "her") + " erecting dragon-prick, gently stroking its");
				if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0 || Flags.list[FlagEnum.EMBER_INTERNAL_DICK] > 0) DisplayText(" strangely ridged, almost scaly");
				DisplayText(" surface");
			}
			else DisplayText("her gently-dripping cunt, sliding in between the lips to stroke the wet interior");
			DisplayText(".");
			...
			if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText("stroke " + emberMF("his", "her") + " shaft");
			else DisplayText("probe gently into her slick depths");
			DisplayText(".  Ember squirms and moans, humming softly, while you suckle and tease, delighting as much in the pleasure you are giving as in the delicious milk cascading down your thirsty throat.");
			...
			DisplayText("  You tell your dragon that you're finished.  You're up and turning to leave when a looping tail around your waist stops you.");
	...
	public beatEmberSpar() {
		...
		//Medium affection
			...
			DisplayText("Ember lies on " + emberMF("his", "her") + " back, while you stand over the defeated dragon triumphantly.  You extend a helping hand, intent on helping " + emberMF("him", "her") + " up; " + emberMF("he", "she") + " takes it.");
		...
		//High affection
			...
			DisplayText("Ember lies on " + emberMF("his", "her") + " back, while you stand over the defeated dragon triumphantly.  You extend a helping hand, intent on helping " + emberMF("him", "her") + " up; " + emberMF("he", "she") + " gladly accepts your help.");
	...
	public loseToEmberSpar() {
		...
		//Low affection
		if (emberAffection() <= 25) {
			DisplayText("Panting, you drop your fighting stance and sit on the floor, lifting a hand and saying you've had enough.");
	...
	private catchAnal() {
		...
		if (emberAffection() < 75) {
			...
			if (Flags.list[FlagEnum.EMBER_INTERNAL_DICK] === 1 || Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0) DisplayText(" of its hiding place");
			DisplayText(", growing slowly but steadily.");
		...
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] > 0) DisplayText("mouth");
		else DisplayText("maw");
		DisplayText(" and sliding " + emberMF("his", "her") + " cock meaningfully against your pucker.  \"<i>You sure you want this?  Last chance to back out.</i>\"  As if you're going to turn away now, so close to filling that hunger for a nice, hot cock in your ass.  Rearing your rump up, you show " + emberMF("him", "her") + " that you're ready if " + emberMF("he", "she") + " is.");
		...
		if (player.torso.butt.looseness === 0 || player.analCapacity() < 25) DisplayText(", leaving both of you wincing as the sensitive head of " + emberMF("his", "her") + " cock struggles to make room for its insertion.  \"<i>I can't believe you really want me to put it here... you're so tight it almost hurts,</i>\" " + emberMF("he", "she") + " whimpers.");
		//(PC has a loose asshole:)
		if (player.analCapacity() < 50) {
			DisplayText(", causing you to gasp as " + emberMF("his", "her") + " ");
			if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0 || Flags.list[FlagEnum.EMBER_INTERNAL_DICK] === 1) DisplayText("draconic ");
			DisplayText("dick slips inside of you; stretching you and making you wince a bit in pain as " + emberMF("his", "her") + " spit rubs off along the interior of your anus.  \"<i>It feels pretty snug... good thing I prepared for this,</i>\" " + emberMF("he", "she") + " remarks with a gasp of pleasure.");
		}
		...
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0 || Flags.list[FlagEnum.EMBER_INTERNAL_DICK] === 1) DisplayText("draconic ");
		DisplayText("prick in and out, reaching for your deepest recesses to rub on the right spots.");
		...
		if (player.torso.vaginas.count > 0) {
			...
			if (player.torso.balls.quantity > 0) DisplayText("over your [balls]");
			DisplayText(" to puddle underneath you");
			if (player.torso.cocks.count > 0) DisplayText(", mingling with your pre to form an ever-growing pool of mixed sexual fluids.");
		}
		...
		//(if PC has balls:
		if (player.torso.balls.quantity > 0) DisplayText("  " + emberMF("His", "Her") + " own balls swing into yours, and every time they connect a shrill wave of pleasure flows through you.");
		...
		if (pregnancy.event > 3) {
			if (Flags.list[FlagEnum.EMBER_OVIPOSITION] === 0) DisplayText("swollen ");
			else DisplayText("egg-laden ");
		}
		DisplayText("belly, and start to rise and sink, pistoning your ass up and down. You have total control over the sex, and you intend to use that.");
		...
		if (player.torso.vaginas.count > 0) DisplayText("your cunt gushing feminine lubricant to glaze Ember's belly");
		if (player.torso.vaginas.count > 0 && player.torso.cocks.count > 0) DisplayText(" and ");
		if (player.torso.cocks.count > 0) {
			DisplayText("your cock ");
			if (player.cumQ() < 250) DisplayText("spurting gouts of cum onto Ember's upper torso");
			else if (player.cumQ() < 1000) DisplayText("raining spunk across the supine dragon");
			else DisplayText("cascading your man-cream across your blissed-out lover until " + emberMF("he", "she") + " is completely glazed in a sperm coating");
		}
		DisplayText(".");

		DisplayText("\n\nEmber hugs you close, enjoying the afterglow");
		if (player.gender > 0) DisplayText(" and rubbing the results of your own orgasm against you");
		DisplayText(".  You sink readily into the dragon's embrace, rubbing against " + emberMF("him", "her") + ", feeling sleepy and blissful after a good, hard fuck.  Sadly, this moment of respite doesn't last long; you feel a certain discomfort as Ember's body heats up.  Looking up, you see " + emberMF("his", "her") + " eyes are wide open.  Before you can ask " + emberMF("him", "her") + " what's wrong, " + emberMF("he", "she") + " knocks you off with a start.");
	...
	private suckEmberCock() {
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1) {
			if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0 || Flags.list[FlagEnum.EMBER_INTERNAL_DICK] === 1) DisplayText("his single genital slit");
			else DisplayText("his genitals");
		}
		else {
			if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0 || Flags.list[FlagEnum.EMBER_INTERNAL_DICK] === 1) DisplayText("her twin genital slits");
			else DisplayText("her genitals");
		}
		DisplayText(", " + emberMF("his", "her") + " ");
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0 || Flags.list[FlagEnum.EMBER_INTERNAL_DICK] === 1) DisplayText("draconic ");
		DisplayText("prick already hard and throbbing.  The dragon taps a clawed finger idly on the stump, awaiting your attention.");
		...
		if (player.torso.cocks.count > 0) DisplayText("  [EachCock] hardens, spearing aimlessly into the ground in your arousal.");
		if (player.torso.vaginas.count > 0) DisplayText("  Your [vagina] starts to seep with feminine juices, your [clit] hardening in anticipation as your excitement dribbles down onto the thirsty ground.");
		...
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0) DisplayText("scaly");
		else DisplayText("soft-skinned");
		DisplayText(" lower belly");
		if (pregnancy.event > 3) DisplayText(", though the gravid orb resists you");
		DisplayText(", and discover a surge of pride in you that you got all sixteen inches of it down.");
		...
		if (player.stats.cor < 80) DisplayText("amused");
		else DisplayText("increasingly leery of " + emberMF("his", "her") + " batty behavior");
		DisplayText(", then pick yourself up to head back to the camp.");
	...
	private stickDickInKnifeDrawer() {
		...
		if (!player.torso.hips.legs.isNaga()) DisplayText("seat yourself on a conveniently lichen-covered boulder.");
		else DisplayText("coil up on your serpentine tail) and start peeling off the clothes on your lower half, exposing [eachCock] to the air and awaiting Ember's response.");
		...
		if (player.torso.cocks.count > 1) {
			DisplayText("  " + emberMF("He", "She") + " selects the biggest cock and gives it a gentle stroke,");
		}
		else DisplayText("  " + emberMF("He", "She") + " takes your [cock " + y + "] in hand and begins gently stroking it,");
		DisplayText(" sniffing your musk with obvious delight.");

		//(if Ember chose a dragon cock)
		if (player.torso.cocks.get(x).type === CockType.DRAGON) DisplayText("\n\n\"<i>Such a magnificent, beautiful cock... it feels powerful... and familiar.</i>\"");
		...
		if (Flags.list[FlagEnum.EMBER_HAIR] === 0 && Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0) DisplayText("scaly pate");
		else DisplayText("hair");
		DisplayText(", telling " + emberMF("him", "her") + " that " + emberMF("he", "she") + " certainly has your attention and you're looking forward to finding out what " + emberMF("he", "she") + " has in mind.");
		...
		if (player.cumQ() < 250) {
			...
			if (!player.torso.hips.legs.isTaur()) DisplayText("a finger");
			else DisplayText("the thumb of her wing");
			DisplayText(" into your ass to tickle your prostate, drawing a few last drops out of you.");
		}
		//(Medium Cum Amount)
		...
		if (player.stats.cor < 75) DisplayText(" and chuckle; you know " + emberMF("he", "she") + " loves you, really.");
		else DisplayText(", folding your arms; " + emberMF("his", "her") + " dementia is getting worse...");
	...
	private stickItInEmbersButt() {
		...
		//(High Affection)
			...
			if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) {
				DisplayText(emberMF("his", "her") + " stiffening cock");
				if (Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText(" and ");
			}
			if (Flags.list[FlagEnum.EMBER_GENDER] === 2) DisplayText("her ");
			if (Flags.list[FlagEnum.EMBER_GENDER] >= 2) DisplayText("dripping pussy");
			DisplayText(" make it obvious that the idea is at least partially arousing.");
		}
		//(first time and corr < 40)
		if (Flags.list[FlagEnum.TIMES_BUTTFUCKED_EMBER] === 0) DisplayText("\n\nYou ask if " + emberMF("he", "she") + "'s really so certain that it won't fit; you're eager to give it a try if " + emberMF("he", "she") + " is, but you won't push if " + emberMF("he", "she") + "'s really that scared of the idea...");
		...
		if (player.torso.vaginas.count > 0) DisplayText("and [vagina] ");
		DisplayText("to the dragon.  Once you're fully undressed, you turn to look at Ember, to see " + emberMF("him", "her") + " openly masturbating; " + emberMF("he", "she") + " pants as " + emberMF("he", "she") + " strokes " + emberMF("his", "her") + " ");
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText("cock ");
		if (Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText("and ");
		if (Flags.list[FlagEnum.EMBER_GENDER] >= 2) DisplayText("pussy ");
		DisplayText("with a hand, while " + emberMF("he", "she") + " suckles on the fingers of the other.");
		...
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0) DisplayText("scaly ");
		DisplayText("cheeks and rubbing the base of " + emberMF("his", "her") + " long, flexible tail to try and convey just how wild " + emberMF("his", "her") + " ass is driving you.");
		...
		else DisplayText("the base of your spine");
		DisplayText(", and you groan to Ember that you're going to cum, soon.");
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText(", jet upon jet of spunk arcing over " + emberMF("his", "her") + " head to paint the ground below");
		if (Flags.list[FlagEnum.EMBER_GENDER] >= 2) DisplayText("; a veritable gush of juices spews forth from her forgotten love-hole to splash against your lower body and over herself");
		DisplayText(".  The force of " + emberMF("his", "her") + " orgasm makes the dragon clench " + emberMF("his", "her") + " ass, sending waves of renewed pleasure through you.");
		...
		if (player.stats.str < 70) {
			DisplayText("desperately try to grab any of the flying pieces, but Ember keeps you pinned with " + emberMF("his", "her") + " tail");
			DisplayText(".");
		...
		//(else str >=80)
			...
			DisplayText("grab " + emberMF("his", "her") + " tail, pulling the dragon off " + emberMF("his", "her") + " feet and throwing " + emberMF("him", "her") + " to the ground as you roll over... and causing " + emberMF("him", "her") + " to drop them everywhere as " + emberMF("he", "she") + " falls.");
			...
			//New
			DisplayText("\"<i>What do you think you're doing!?</i>\" Ember yells at you, obviously mad at your little stunt, one hand reaching behind to prevent any more cum from spilling.");
	...
	private slurpDraggieCunnies() {
		...
		if (emberAffection() <= 25) {
			...
			if (!silly()) DisplayText("with me");
			else DisplayText("on my affection meter");
			DisplayText(" just because you're offering to do this,</i>\" the dragon hastily adds.");
		}
		//(Medium Affection)
		else if (emberAffection() < 75) {
			...
			if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText(" and slowly hardening shaft");
			DisplayText(".");
		}
		//(High Affection)
		else {
			...
			if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText(" and shaft");
			DisplayText(".  \"<i>Ok... I suppose I would like that,</i>\" Ember replies, trying to calm herself down, lest she seem too excited by the idea.");
		}
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText("  With Ember's cock jutting out above your head, you can't resist sliding your tongue along the underside of her shaft, just for an extra teasing motion.");
		...
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] > 0) DisplayText("blushing all the while and ");
		DisplayText("panting from the stimulation.");
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText("  You think droplets of pre are starting to slide down her girl-cock and drip onto your head... but that's part and parcel of loving a herm, so you pay it no attention.");
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText("  Her throbbing shaft, not intending to be left out of the fun, records the event by marking the tree bark with white dragon spooge.");
		...
		if (player.torso.cocks.count > 0) DisplayText("stiffened shaft");
		if (player.gender === Gender.HERM) DisplayText(" and ");
		if (player.torso.vaginas.count > 0) DisplayText("moist cunt");
		if (player.gender === Gender.NONE) DisplayText("empty crotch");
		DisplayText(" in sympathy, you head back to camp.");
	...
	private getEatenOutByEmbra() {
		...
		if (!player.torso.hips.legs.isNaga()) DisplayText(" with your [legs] spread wide");
		DisplayText(", displaying your [vagina].  You tell Ember that " + emberMF("he", "she") + "'ll need to make the next move from here, smiling as you do so.");
		...
		if (player.torso.cocks.count > 0) DisplayText(", as well as the musk emanating from your drooling cock");
		DisplayText(".  Ember's eyes close as " + emberMF("he", "she") + " savors your scent, committing it to memory and licking " + emberMF("his", "her") + " lips in preparation for the task ahead.");
		...
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0) DisplayText("maw");
		else DisplayText("mouth");
		DisplayText(", kissing your contracting box as if it were a long-lost lover, pushing " + emberMF("his", "her") + " tongue ever deeper inside you, as if intent on reaching your womb.");
		...
		if (player.torso.vaginas.get(0).wetness >= 4) DisplayText("  However, your squirting spatters make this an impossible task.  Ember doesn't seem to care, even as " + emberMF("his", "her") + " face is completely covered in femcum.");
	...
	private penetrateEmbrah() {
		...
		//(High Affection)
			...
			if (Flags.list[FlagEnum.EMBER_GENDER] === 3) {
				if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0 || Flags.list[FlagEnum.EMBER_INTERNAL_DICK] === 1) DisplayText(" and her cock peek out of its protective slit");
				else DisplayText(" and her cock begin hardening");
			}
			DisplayText(" in anticipation.  \"<i>Yeah... maybe... why do you ask?</i>\"");
		}
		//(First Time)
		if (Flags.list[FlagEnum.EMBER_PUSSY_FUCK_COUNT] === 0) {
			...
			if (player.stats.cor > 75) DisplayText("  As if bitches like her could qualify as maidens in any but the most literal sense.");
			...
			if (player.stats.cor <= 75) DisplayText("cute... but very troublesome");
			else DisplayText("irritating as hell");
		...
		DisplayText("\n\nYou ");
		if (player.stats.cor < 50) DisplayText("hasten to undress yourself for the impatient dragoness, quickly pulling");
		else DisplayText("languidly denude yourself, making sure to exaggerate every movement as you pull");
		DisplayText(" off your [armor] until you are standing naked before her, letting her see what you have to offer in terms of phallic delights.");
		...
		if (player.cockThatFits(emberVaginalCapacity()) === -1) {
			...
			if (player.torso.cocks.count === 1) DisplayText("that... that... monster inside me,");
			else DisplayText("any of those... those... monsters inside me,");
			DisplayText(" you'll tear me apart!</i>\"  Ember finishes by exhaling a puff of smoke in frustration... clearly she wants this as much as you.");

			DisplayText("\"<i>Find some way to shrink ");
			if (player.torso.cocks.count === 1) DisplayText("that");
			else DisplayText("those");
			DisplayText(" and then come back!</i>\"  Ember turns on her heels and walks away, moodier than usual.");
			return { next: Scenes.camp.returnToCampUseOneHour };
			return;
		}
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) {
			DisplayText("; her ");
			if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0 || Flags.list[FlagEnum.EMBER_INTERNAL_DICK] > 0) DisplayText("draconic");
			else DisplayText("humanoid");
			DisplayText(" prick slaps wetly against your belly with each thrust, smearing pre on you as the engorged tool bounces back and forth");
		}
		DisplayText(".");
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText("; her own erect cock spurts a small jet of pre");
		DisplayText(".");
		...
		if (player.cumQ() < 250) DisplayText("dribble");
		else if (player.cumQ() < 1000) DisplayText("flow");
		else DisplayText("gush");
		DisplayText(" from her used cunt and you sit on the ground, while Ember remains limp on the ground, panting. Still suffering from the afterglow of your recent orgasm, you dimly realize that Ember herself doesn't seem to have cum yet; the only fluids dripping from her pussy are yours");
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText(", and her cock, while engorged and swollen, hasn't poured forth yet");
		DisplayText(".");
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText("; her cock jumps and unloads on the ground below, marking this as the spot of your lovemaking");
		DisplayText(".  Her pussy grabs at you like a vice, constricting you with such pressure that you have trouble moving, even though her cunt is well-lubed from your mixed fluids.");
		...
		//(High Cum Amount)
			...
			if (player.torso.balls.quantity === 0) DisplayText("prick is ");
			else DisplayText("[balls] are ");
			DisplayText("finished, Ember doesn't only look more than a few months pregnant; she looks absolutely huge, like her water could break at any moment!");
		}
	...
	private penetrateEmbrahPartII() {
		...
		if (player.cumQ() < 1000) {
			...
			player.cumMultiplier += 1 + randInt(3);
		}
	...
	private getPenetratedByEmberLastSexSceneWoooo() {
		...
		//(High Affection)
			...
			DisplayText("Ember's cock jumps ");
			if (Flags.list[FlagEnum.EMBER_ROUNDFACE] > 0 && Flags.list[FlagEnum.EMBER_INTERNAL_DICK] === 0) DisplayText("to attention");
			else DisplayText("out of its protective slit");
			DisplayText(" at the mere mention of what you expect " + emberMF("him", "her") + " to do.  \"<i>I'm more than enough to handle two... no... three of you!  And if you want proof all you have to do is ask!</i>\" Ember boasts proudly.");
		}
		...
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0 || Flags.list[FlagEnum.EMBER_INTERNAL_DICK] > 0) DisplayText("draconic ");
		else DisplayText("human-like ");
		DisplayText("cock begins making its way past your labia.");
		...
		if (player.torso.cocks.count > 0) {
			DisplayText("  Your cock explodes, belching cum to ");
			if (player.cumQ() < 250) DisplayText("spatter");
			else if (player.cumQ() < 1000) DisplayText("drench");
			else if (player.cumQ() < 2000) DisplayText("completely cover");
			else DisplayText("utterly soak");
			DisplayText(" your draconic partner.");
		}
		...
		if (player.perks.has(PerkType.HistoryWhore) || player.perks.has(PerkType.HistorySlut)) DisplayText("  Using the skills you've honed, you make the muscles in your vagina ripple and wrinkle, teasing the cock caught inside you in a way few women can.");
		DisplayText("  Mmm... you can feel your own second orgasm coming in hot.  Maybe after another 4 or 5, you'll let the dragon go...");
	...
	private getPenetratedByEmberLastSexSceneWooooPartII() {
		...
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0 || Flags.list[FlagEnum.EMBER_INTERNAL_DICK] === 1) DisplayText("protective slit");
		else DisplayText("limp cock and balls");
		DisplayText(".  \"<i>You used me like a fucktoy!</i>\"");
		...
		if (emberAffection() < 75) DisplayText(emberMF("his", "her") + " cock was unlike anything else in this world?");
		else DisplayText(emberMF("he", "she") + " could handle two or even three of you at the same time?");
		DisplayText("  You expected more resistance if that was the case.");
	...
	public emberRapesYourHeatness() {
		...
		if (player.torso.vaginas.count > 0 && player.inHeat && (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3))
			DisplayText("your [vagina] aching to be filled and your womb put to its proper purpose");
		else DisplayText("[eachCock] burning to be seeding ripe, ready wombs");
		DisplayText(".");

		DisplayText("\n\nEmber responds by ");
		if (player.torso.vaginas.count > 0 && player.inHeat && (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3))
			DisplayText("humping against you, smearing your thigh with dragon pre.");
		else DisplayText("tightly pressing her drooling, puffy netherlips on your thighs.");
		DisplayText("  \"<i>I need you, [name].  I need you so badly... can you see how badly I need you?</i>\" Ember asks, panting in barely contained lust.  \"<i>I want to fuck you so badly... Let's make a baby now!</i>\"");
		dynStats("lus", 10 + player.stats.lib / 10);
		DisplayText("\n\nWhat do you say?");
		//[Accept] [Deny]
		MainScreen.simpleChoices(["Accept", "Deny", "", "", ""], [timeToPuffTheMagicDragon, fuckOffEmberIWantANap, null, null, null]);
	}
	...
	private fuckOffEmberIWantANap() {
		...
		DisplayText(" so yearn");
		if (player.gender === Gender.MALE || player.gender === Gender.FEMALE) DisplayText("s");
		DisplayText(" to take Ember up on " + emberMF("his", "her") + " offer! ...But you are better than this; you are not some mindless animal, a slave to your lusts.  You fight down the arousal gnawing at you and tell Ember you don't want to fuck " + emberMF("him", "her") + ".");
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText("protruding prick");
		if (Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText(" and ");
		if (Flags.list[FlagEnum.EMBER_GENDER] >= 2) DisplayText("creaming wet vagina");
		DisplayText(".  No doubt " + emberMF("he", "she") + "'s off to take care of " + emberMF("his", "her") + " needs by " + emberMF("him", "her") + "self.  You doubt " + emberMF("he", "she") + "'s going to be happy once " + emberMF("his", "her") + " head is clear... still, " + emberMF("he", "she") + "'ll get over it.");
		emberAffection(-10);
		//Set some cooldown so this doesn't proc all the goddamn time!
		player.statusAffects.add(StatusAffectType.EmberFuckCooldown, 12, 0, 0, 0);
		return { next: playerMenu };
	}
	...
	private timeToPuffTheMagicDragon() {
		...
		if (player.pregnancyIncubation === 0 && !pregnancy.isPregnant && Flags.list[FlagEnum.EMBER_GENDER] === 3 && player.gender === Gender.HERM) {
			DisplayText("\n\n(Who should bear the kids?)");
			//[Ember] [You]
			MainScreen.simpleChoices(["Ember", "You", "", "", ""], [breedEmberPregnantAsIfThereWasAnyOtherKindOfBreeding, getKnockedUpByEmbrahBroBaby, null, null, null]);
		}
	...
	private getKnockedUpByEmbrahBroBaby() {
		...
		else DisplayText(emberMF("his", "her") + " soft, squeezable boobs");
		DisplayText(" and savoring the kiss.  Emboldened, you poke your tongue into " + emberMF("his", "her") + " mouth, to see how " + emberMF("he", "she") + " will react.");
		...
		if (Flags.list[FlagEnum.EMBER_OVIPOSITION] === 0) DisplayText("little dragons");
		else DisplayText("dragon eggs");
		DisplayText(", my beautiful.</i>\"  Ember resumes " + emberMF("his", "her") + " kissing; " + emberMF("his", "her") + " own hands roaming all over your body, as if mapping you out through touch, looking for the best places to touch and tease.");
		...
		if (Flags.list[FlagEnum.EMBER_OVIPOSITION] === 0) DisplayText("dragonlings");
		else DisplayText("dragon eggs");
		DisplayText(".  You want to breed!");
		...
		DisplayText("  You groan throatily, your need burning through you, and start trying to grind yourself against Ember's shaft.  " + emberMF("He", "She") + " pins you down, though, making it impossible for you to actually do anything more than rub " + emberMF("his", "her") + " glans against your needy netherlips.");
		...
		DisplayText("You growl throatily, feeling that same need burning inside you. You clutch and claw against the dragon, fingers grasping deeply into " + emberMF("his", "her") + " scaly limbs as you drag, scrape, and thrust yourself against " + emberMF("him", "her"));
		if (player.torso.cocks.count > 0) DisplayText(", your neglected [cock] drooling pre-cum that is being smeared all across your belly and " + emberMF("his", "hers") + " with every move you make");
		DisplayText(".  Gods, you can feel " + emberMF("him", "her") + " filling you...");
		...
		DisplayText("Your contracting walls milk the dragon's sensitive cock, trying to coax even more seed out of your panting dragon lover.  \"<i>I-it's time,</i>\" Ember mutters, crashing into your wet bedroll and thrusting deep inside you.");
		...
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 1) DisplayText("blushing ");
		DisplayText("face.  \"<i>But if you must know, I do love you.</i>\"   With a final sigh Ember sets down on your bedroll for a quick nap.");
		...
		if (Flags.list[FlagEnum.EMBER_OVIPOSITION] === 0) DisplayText("baby dragon");
		else DisplayText("dragon egg");
		DisplayText(".  Yawning, you curl up to the dragon for a quick nap of your own.");
		player.orgasm();
		player.stats.sens += -2;
		...
		player.statusAffects.add(StatusAffectType.EmberFuckCooldown, 36, 0, 0, 0);
		return { next: createCallBackFunction(emberBreedingAfterMathWatchOutForRadioactiveFallout, false) };
	}
	...
	private breedEmberPregnantAsIfThereWasAnyOtherKindOfBreeding() {
		...
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0) DisplayText("scale-covered ");
		DisplayText("orbs gently squish ");
		if (Flags.list[FlagEnum.EMBER_MILK] > 0) DisplayText("and leak milk ");
		DisplayText("under your grasp.");
		...
		if (Flags.list[FlagEnum.EMBER_MILK] > 0) DisplayText("the slowly trickling rivulets of milk along with ");
		DisplayText("Ember's pleased moans are enough to make you push away such thoughts, you're glad she has those!");

		DisplayText("\n\nYou hungrily kiss her ");
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0) DisplayText("draconic ");
		else DisplayText("oh-so-human ");
		DisplayText("face, nipping lightly at her lip, rolling your tongue over hers and savoring her inhuman tastes; Ember answers in kind with a kiss of her own, purring and moaning into your mouth.  Her vibrating breasts massage your hands as you knead, lift, and then roll them about.");

		//(if Ember has a dick)
		if (emberHasCock()) {
			DisplayText("\n\nYou feel something poke your [chest] and look down to see Ember's painfully erect ");
			if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0 || Flags.list[FlagEnum.EMBER_INTERNAL_DICK] === 1) DisplayText("dragon ");
			DisplayText("prick.  With a mischievous curl of your lips, you pull Ember deeper into your kiss and grab her cock, stroking it vigorously.");
		}
		...
		if (emberHasCock()) DisplayText("her cock spurts a few jets of cum onto your [chest]; ");
		DisplayText("her eyes roll to the back of her head.  With a muffled roar she finally caves, a veritable wave of dragon juices crashing into your lower body and pooling on your bedroll, filling the whole tent with the delicious smell of sex and pheromones.");

		//(If Ember has a dick)
		if (emberHasCock()) DisplayText("  Ember's cock wets the side of the tent with continuous jets of cum, only adding to the already overpowering smell of sex as she empties herself on top of you.");
		...
		if (emberHasCock()) DisplayText("her spent cock throb; ");
		DisplayText("her eyes shut tight in pleasure as she reaches her second orgasm.  You muffle her groan of pleasure with one of your own.  A second wave of dragon fluids joins the first in wetting your lower body, along with your bedroll.");
		if (emberHasCock()) DisplayText("  Little gobs of cum spill from her cock to slide down her shaft and gather on your belly button.");
		DisplayText("  Her tail coils around you, helping her stay in position as she melts on top of you like putty.");
		...
		//(Low Cum Amount)
		if (player.cumQ() < 250) DisplayText("  Though you usually don't cum that much, Ember's contracting walls literally suck the cum out of you; forcing you to give up more than you usually would... something you're quite glad to do!");
		//(Medium Cum Amount)
		else if (player.cumQ() < 1000) DisplayText("  Your generous helping of cum works its way towards her awaiting womb.  In fact, due to the delicious massage your shaft is receiving from Ember's contracting, almost sucking vaginal walls, you find yourself pumping out even more cum that you would usually have, quickly filling her to the brim and beyond.  It's not until Ember's belly is slightly distended that you stop.");
		//(High Cum Amount)
		else DisplayText("  You dump a huge load of jism into Ember's awaiting belly, but her contracting walls seem to milk you for ever more!  Ember is filled to the brim, her belly already slightly distended, but you still continue to fill her with your seed.  Ember's tightly sealed pussy lips means the cum has nowhere to go, so instead it gathers in her womb, distending her belly until she looks at least a few months pregnant; and by the look of pleasure on Ember's panting face, you don't think she minds it.");

		DisplayText("\n\nWith a gasp and a sigh of utter relief, you slump into Ember's arms, collapsing onto her ");
		if (emberHasCock()) DisplayText("cum-slick ");
		DisplayText("belly and laying there to regain your strength, ");
		...
		player.orgasm();
		player.stats.sens += -2;
		return { next: createCallBackFunction(emberBreedingAfterMathWatchOutForRadioactiveFallout, true) };
	}
	...
	private emberBreedingAfterMathWatchOutForRadioactiveFallout(emberPregged: boolean = true) {
		...
		DisplayText("\n\nEmber's cheeks ");
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0) DisplayText(" are probably blushing under " + emberMF("his", "her") + " scales.");
		else DisplayText("blush.");
		DisplayText("  \"<i>You're welcome, but this is the only time I'll clean up after you!  I'm not your personal maid!</i>\"");
		...
		if (!emberPregged) DisplayText("touch your belly");
		else DisplayText("point at Ember's belly");
		DisplayText(", and you ask the dragon if " + emberMF("he", "she") + " thinks it \"<i>took</i>\".");

		DisplayText("\n\nEmber puffs " + emberMF("his", "her") + " chest and proudly boasts, \"<i>Of course it did!  We're both very virile!  And after all the cum you ");
		if (emberPregged) DisplayText("pumped into me...");
		else DisplayText("had me pump into you...");
		DisplayText("</i>\" Ember trails off, and you can see " + emberMF("his", "her") + " hand move to " + emberMF("his", "her") + " crotch to caress " + emberMF("his", "her") + " ");
		if (emberPregged) DisplayText("slit");
		else DisplayText("cock");
		DisplayText(", as if remembering the intense sex session you two just had.");
		...
		DisplayText("\n\nEmber's ");
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 1) DisplayText("blush");
		else DisplayText("discomfort");
		DisplayText(" deepens, and " + emberMF("he", "she") + " frowns at you.  \"<i>A-are you questioning my virility!?  Well, if you want, I can go again, right now!</i>\"");
		...
		DisplayText("\n\nEmber ");
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 1) DisplayText("blushes");
		else DisplayText("frowns");
		...
		//(if PC now pregnant:
		if (!emberPregged) DisplayText("  You touch your belly with a smirk.  Very certain they took indeed...");
		return { next: playerMenu };
	}
		...
		switch (pregnancy.eventTriggered()) {
			...
			case 4: DisplayText("\nEmber's belly has grown quite a bit.  Anyone can tell she's pregnant with a single glance.  ");
				if (Flags.list[FlagEnum.EMBER_OVIPOSITION] > 0) {
					DisplayText("Ember notices you looking.  \"<i>W-what? Never seen a pregnant woman before?</i>\" she asks indignantly, although she can't hide her smile as you continue to look.\n");
				}
				else {
					DisplayText("Ember catches you looking" + (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 1 ? " and blushes" : "") + ".  \"<i>W-what is it?</i>\"  You reply merely that she carries a baby bump very well; she looks good pregnant.  \"<i>Oh, uh...  Thanks I guess?</i>\" she replies, looking away and flicking her tongue out nervously.\n");
				}
				return true;
	...
	public emberGivesBirth() {
		...
		if (Flags.list[FlagEnum.EMBER_OVIPOSITION] === 0) {
			...
			if (emberHasCock()) {
				if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0 && Flags.list[FlagEnum.EMBER_INTERNAL_DICK] === 0) DisplayText("  Her human-like cock dangles heavily in front of her pussy, partially erect from the muscular spasms.");
				else DisplayText("  Her draconic cock has been pushed from its internal sheathe by the pressure, but it's too painful for her to be erect.");
			}
			...
			if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0) DisplayText("n anthro dragon");
			else DisplayText(" hybrid of human and dragon");
			DisplayText(".  It's a beautiful strong, healthy little ");
			let roll: number = randInt(100);
			if (roll < 40) DisplayText("boy");
			else if (roll < 80) DisplayText("girl");
			else DisplayText("herm");
			DisplayText(".");

			DisplayText("\n\nYou cradle ");
			if (roll < 40) DisplayText("him");
			else DisplayText("her");
			DisplayText(" in your arms, uncaring of the stains as the fluid drenching its little body soaks into your [armor], gently soothing ");
			if (roll < 40) DisplayText("him");
			else DisplayText("her");
			DisplayText(".  Then, seeing Ember is recovering from her recent exertions, you grin proudly at her and gently hold your ");
			if (roll < 40) DisplayText("son");
			else DisplayText("daughter");
			DisplayText(" out for her to take.");
			...
			if (emberChildren() > 1) DisplayText("  Your other draconic offspring quietly peek into the lair, having vacated the place when Ember went into labor.  They smile at the sight of their new sibling, gently slipping in to curl up beside the mother and new child.");
			DisplayText("  With a contented expression on your face, you leave Ember and your ");
			if (emberChildren() <= 1) DisplayText("new child");
			else DisplayText("newly expanded draconic brood");
			DisplayText(" to get some rest, leaving the den.\n");
		...
		//Ember Lays Egg
			...
			if (emberHasCock()) {
				if (Flags.list[FlagEnum.EMBER_ROUNDFACE] > 0 && Flags.list[FlagEnum.EMBER_INTERNAL_DICK] === 0) DisplayText("  Her human cock dangles heavily in front of her pussy, partially erect from the muscular spasms.");
				else DisplayText("  Her draconic cock has been pushed from its internal sheathe by the pressure, but it's too painful for her to be erect.");
			}
			DisplayText("\n\nMind racing, you bend your head in close and stick out your tongue, sliding it back in and out of your mouth to get it nice and wet before delivering a long, sloppy lick to her inner walls.  You can taste her, and the strange salty-blood taste of amniotic fluid mingles with her natural lubricants.  It's an unusual taste but not unbearable, and you begin to lick with greater fervor and purpose.  Your intention is to try and drown the pain of her contractions with pleasurable stimulus.  It's a crazy idea, but it makes sense in a place like Mareth.");
		...
		player.statusAffects.add(StatusAffectType.EmberNapping, 12, 0, 0, 0);
	}
	...
	public giveBirthToEmberKids() {
		...
		if (Flags.list[FlagEnum.EMBER_OVIPOSITION] === 0) {
			...
			if (player.torso.vaginas.count <= 0) {
				DisplayText("A terribly painful ripping feeling comes from your crotch. Reaching down to touch the tender spot you feel a spike of pleasure and moistness.  <b>You've grown a vagina!</b>\n\n");
				player.createVagina();
			}
			...
			DisplayText("on the boundary between pleasure and pain.  You aren't sure how much more you can take.");
			...
			DisplayText(" was getting hungry and I didn't know what to do,</i>\" Ember explains.");
			if (Flags.list[FlagEnum.EMBER_MILK] > 0) {
				DisplayText("  \"<i>I tried to feed ");
				if (roll < 40) DisplayText("him");
				else DisplayText("her");
				DisplayText(" myself, but ");
				if (roll < 40) DisplayText("he");
				else DisplayText("she");
			...
			DisplayText(" face and starts to cry softly.  You gently help ");
			if (roll < 40) DisplayText("him");
			else DisplayText("her");
			DisplayText(" up onto your shoulder and gently pat ");
			if (roll < 40) DisplayText("him");
			else DisplayText("her");
			DisplayText(" on the back between ");
			if (roll < 40) DisplayText("his");
			else DisplayText("her");
			DisplayText(" little wings, eliciting a burp that clearly leaves ");
			if (roll < 40) DisplayText("him");
			else DisplayText("her");
			DisplayText(" feeling better.  ");
			if (roll < 40) DisplayText("He");
			else DisplayText("She");
			DisplayText(" coos, giggles and nuzzles into your neck, clearly happy to be here in the real world at last.");
		...
		//PC Lays Egg
			...
			DisplayText("withdraws, licking " + emberMF("his", "her") + " lips of a stray droplet of milk.  \"<i>Sorry, it's just that you looked so full, and all that milk would've been wasted...  So, I thought I could help myself, not that I've been wanting to drink your milk or anything like that.</i>\"");
			...
			if (Flags.list[FlagEnum.EMBER_EGGS] > 0) DisplayText("s hatch");
			else DisplayText(" hatches");
			DisplayText(", anyway.  The dragon ");
			if (Flags.list[FlagEnum.EMBER_ROUNDFACE] > 0) DisplayText("blushes and then ");
			DisplayText("scurries away, even as you pull yourself upright and get ready to go about your business.\n");
			Flags.list[FlagEnum.EMBER_EGGS]++;
		}
		player.statusAffects.add(StatusAffectType.EmberNapping, 5, 0, 0, 0);
		player.updateGender();
	}
	...
	private highAffectionEmberLustFuck() {
		...
		let x: number = player.cockThatFits(emberVaginalCapacity());
		if (x < 0) x = player.torso.cocks.sort(Cock.SmallestCockArea)[0];
		DisplayText("You strip your [armor] and watch Ember as " + emberMF("he", "she") + " appraises your naked body.  You can see " + emberMF("his", "her"));
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) {
			DisplayText(" dragon cock ");
			if (emberInternalDick()) DisplayText("poking out of " + emberMF("his", "her") + " protective slit");
			else DisplayText("growing erect");
		}
		if (Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText(" and");
		...
		if (Flags.list[FlagEnum.TIMES_EMBER_LUSTY_FUCKED] === 0) {
			...
			DisplayText("\"<i>All of them,</i>\" you state.");
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1) {
			...
			DisplayText("\n\nYour ");
			if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0) DisplayText("she-dragon");
			else DisplayText("dragon-girl");
			DisplayText(" moans throatily and gives you a rapturous look.  \"<i>I love you, [name]... gods, I love you so much, and I'm so happy I can finally say that to your face,</i>\" she tells you.");
		...
		//else if Ember has a pussy and the PC more than one cock:
			...
			DisplayText(" of those?  I-I know it's not exactly part of the pose and all, but...</i>\"");
			...
			if (player.torso.cocks.count === 2) DisplayText("both ");
			else DisplayText("two of ");
			DisplayText("your cocks enveloped in slick tightness and warmth is almost enough to make you fill her up with your seed right then and there, but somehow you manage to hold on.");
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] >= 2 && player.torso.cocks.count > 1) DisplayText(" and ass");
		DisplayText(" rippling and squeezing your intruding member");
		if (Flags.list[FlagEnum.EMBER_GENDER] >= 2 && player.torso.cocks.count > 1) DisplayText("s");
		DisplayText(".");
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] >= 2) DisplayText("vagina");
		else DisplayText("ass");
		DisplayText(" of " + emberMF("his", "hers") + " the pounding it deserves.");
		...
		else DisplayText(" tight ass");
		DisplayText(" and blow your load.");

		DisplayText("\n\nEmber howls exultantly as your steaming load gushes inside of " + emberMF("him", "her") + ".  " + emberMF("His", "Her") + " ");
		if (Flags.list[FlagEnum.EMBER_GENDER] >= 2) {
			DisplayText("cunt shudders, splashing femcum all over your intruding shaft");
			if (Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText(", and her ");
			else DisplayText(", ");
		}
		if (Flags.list[FlagEnum.EMBER_GENDER] === 3 || Flags.list[FlagEnum.EMBER_GENDER] === 1) {
			DisplayText("cock erupts, spraying draconic cum up into the air to rain back down upon " + emberMF("his", "her") + " body, spattering " + emberMF("him", "her") + " in " + emberMF("his", "her") + " own juices and ");
		}
		DisplayText("leaving " + emberMF("him", "her") + " gasping for breath as the climax ebbs away.");
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] >= 2 && player.torso.cocks.count > 1) DisplayText("s");
		DisplayText(", letting a satisfying stream of white mat the leaves below.  You smile at Ember, shaft");
		if (player.torso.cocks.count > 1) DisplayText("s");
		DisplayText(" still flagging above " + emberMF("him", "her") + ".");
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1) DisplayText("ass");
		else if (player.torso.cocks.count === 1) DisplayText("pussy");
		else DisplayText("holes");
		DisplayText(" with a squelch.  It feels so good... taking " + emberMF("him", "her") + " one time after the other.");
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] >= 1 && player.torso.cocks.count > 1) DisplayText("s");
		DisplayText(".  The dragon lets out an exultant cry as " + emberMF("his", "her") + " cum-slimed ");
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1) DisplayText("ass squeezes");
		else if (player.torso.cocks.count === 1) DisplayText("pussy squeezes");
		else DisplayText("holes squeeze");
		DisplayText(" you, milking your ");
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1) DisplayText("shaft as his cock spurts dragon-seed onto the leaves below him, filling the air with the scent of spunk and matting them into a steaming morass.");
		//{twin shafts} as {her cunt spasms wetly, drenching your lap with fresh femcum} {and/or} {[her] cock spurts dragon-seed onto the leaves below {her}, filling the air with the scent of spunk and matting them into a steaming morass}
			...
			if (player.torso.cocks.count === 1) DisplayText("shaft as her cunt spasms wetly, drenching your lap with fresh femcum");
			else DisplayText("twin shafts as her cunt spasms wetly, drenching your lap with fresh femcum");
			if (Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText(" and her cock spurts dragon seed onto the leaves below her, filling the air with the scent of spunk and matting them into a steaming morass");
			DisplayText(".");
		}
		...
		//Else:
			...
			if (Flags.list[FlagEnum.EMBER_GENDER] >= 2) DisplayText("her cunt starts to drip with moisture");
			if (Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText(" and ");
			if (Flags.list[FlagEnum.EMBER_GENDER] === 3 || Flags.list[FlagEnum.EMBER_GENDER] === 1) DisplayText(emberMF("his", "her") + " cock begins to grow erect again");
			DisplayText(", amazement giving way to arousal.  " + emberMF("He", "She") + " licks " + emberMF("his", "her") + " lips with " + emberMF("his", "her") + " inhuman tongue, clearly eager to begin again.");
		}

		DisplayText("\n\nEmber crawls over towards you, gripping the base of your shaft");
		if (player.torso.cocks.count > 1 && Flags.list[FlagEnum.EMBER_GENDER] >= 2) DisplayText("s");
		DisplayText(" tenderly yet firmly, stroking you slowly.  \"<i>I can't believe how hot I get when I see you sporting ");
		if (player.torso.cocks.count === 1) DisplayText("this");
		else DisplayText("these");
		DisplayText(".  It's just so... intoxicating... your scent, the way you look at me, everything really.</i>\"  You pat Ember's head");
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 1 || Flags.list[FlagEnum.EMBER_HAIR] > 0) {
			DisplayText(", ruffling " + emberMF("his ", "her "));
			if (Flags.list[FlagEnum.EMBER_HAIR] >= 2) DisplayText("mane");
			else DisplayText("hair");
		}
		DisplayText(".  " + emberMF("He", "She") + " leans against your hand, hugging your midriff and letting your shaft");
		if (player.torso.cocks.count > 1) DisplayText("s");
		DisplayText(" brush against " + emberMF("his", "her") + " cheek.");
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1) DisplayText("depths");
		else DisplayText("nethers");
		if (Flags.list[FlagEnum.EMBER_GENDER] >= 2 && player.torso.cocks.count > 1) DisplayText(" and depths");
		DisplayText(".  \"<i>Ahn.  D-deeper...</i>\"  You hook your arm under " + emberMF("his", "her") + " knee and pull " + emberMF("his", "her") + " leg up, nearly throwing the dragon off balance.  \"<i>Ah!  D-do you like it when I let you take charge?  Ugh- oh!  Well, I think maybe I kind of like letting you be in charge, too...</i>\"  You barely pay attention to " + emberMF("his", "her") + " teasing remarks, instead focusing on exploring " + emberMF("his", "her") + " cummy ");
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1) DisplayText("ass");
		else if (player.torso.cocks.count === 1) DisplayText("pussy");
		else DisplayText("love-holes");
		DisplayText(".  \"<i>Uhn... yes...  take me again.</i>\"  " + emberMF("He", "She") + " lets " + emberMF("his", "her") + " tongue loll out as " + emberMF("he", "she") + " pants in pleasure.");
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] >= 2 && player.torso.cocks.count > 1) DisplayText("both of your dicks");
		else DisplayText("your dick");
		if (Flags.list[FlagEnum.EMBER_GENDER] >= 2) DisplayText(", moisture drenching the cock buried in her cunt as it slobbers greedily across your burning flesh.");
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText("  " + emberMF("His", "Her") + " cock throbs and pulsates, ripples of arousal giving way to cumvein-bulging jets that shoot from " + emberMF("his", "her") + " prick to splatter wetly against the wall.");
		DisplayText("  Wobbling unsteadily, the dragon becomes increasingly dependent on you to hold " + emberMF("him", "her") + " upright - a climax is coming, and it looks like it's going to be big...");
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) {
			DisplayText(emberMF("his", "her") + " cock fountaining seed across the wall, practically whitewashing it in steaming hot dragon-spunk");
		}
		if (Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText(" and ");
		if (Flags.list[FlagEnum.EMBER_GENDER] >= 2) DisplayText("her cunt gushing female ejaculate, almost soaking you with the cascade of feminine fluids and puddling wetly against your [feet]");
		DisplayText(".");
		...
		DisplayText(", feeling the excess slide out around your cock");
		if (Flags.list[FlagEnum.EMBER_GENDER] >= 2 && player.torso.cocks.count > 1) DisplayText("s");
		DisplayText(".  \"<i>Ahh... more seed from my lover...</i>\" " + emberMF("he", "she") + " whispers, nearly passing out from the pleasure.  You feel yourself grow dizzy with pleasure and tumble onto the soft grass covering the den's floor, bringing Ember along.  Your dick");
		if (Flags.list[FlagEnum.EMBER_GENDER] >= 2 && player.torso.cocks.count > 1) DisplayText("s");
		DisplayText(" slide");
		if (player.torso.cocks.count === 1) DisplayText("s");
		DisplayText(" out of " + emberMF("his", "her") + " with a wet slurp, allowing your deposit to leak its way under your prone forms.");
	...
	private noStayingForCuddlesPostLustFuck() {
		...
		if (player.stats.cor < 50) DisplayText("thank her for being so understanding");
		else DisplayText("grunt an acknowledgement");
		DisplayText(" and then gather your things before heading off to wash yourself down.");
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	private stayWithEmberAfterLustFuck() {
		...
		let x: number = player.cockThatFits(emberVaginalCapacity());
		if (x < 0) x = player.torso.cocks.sort(Cock.SmallestCockArea)[0];
		DisplayText("With a smile, you tell " + emberMF("him", "her") + " that you'd be happy to.  \"<i>Great, come here...</i>\" " + emberMF("he", "she") + " croons, scooting over towards you.  You open your arms and allow the dragon to snuggle up against you, folding your arms comfortably under " + emberMF("his", "her") + " wings.");
		...
		//else if Ember is female:
			...
			if (Flags.list[FlagEnum.EMBER_MILK] > 0) DisplayText("milk-filled ");
	...
	private frottingWithEmber(clear: boolean = true) {
		...
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] > 0) DisplayText(emberMF("-boy", "-girl"));
		DisplayText("'s overly long tongue sticks out as pleasure overrides " + emberMF("his", "her") + " senses.  You groan as you slide your cock against Ember's, the thought occurring to you that this is going to be pretty messy when the inevitable comes, but you are too overwhelmed to actually say so.");
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] >= 2) DisplayText("  Her moist dragon pussy is not helping the mess in the least.  You can smell her femcum pooling along with her pre.");
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] >= 2) DisplayText("  Her other hand frigs her pussy as she comes down from her orgasm, gushing female juices and leaking some of the cum you've pumped into her earlier.");

		DisplayText("\n\nThe dragon");
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] > 0) DisplayText(emberMF("-boy", "-girl"));
		DisplayText(" uses what strength " + emberMF("he", "she") + " still has to crawl over to you and slide back between your arms.  \"<i>Sorry, just had to get that last bit out of my system.</i>\"  " + emberMF("He", "She") + " yawns lowdly.  \"<i>[name]?  How about a quick nap?</i>\"  " + emberMF("He", "She") + " asks not even bothering to hear your reply before exhaustion gets " + emberMF("his", "her") + " and " + emberMF("he", "she") + " falls asleep, snoring lightly.  You smile at " + emberMF("him", "her") + " and stroke " + emberMF("his", "her"));
		if (Flags.list[FlagEnum.EMBER_HAIR] === 0 && Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0) DisplayText(" head");
		else if (Flags.list[FlagEnum.EMBER_HAIR] === 2) DisplayText(" mane");
		else DisplayText(" hair");
	...
	private penetrateWithEmber(clear: boolean = true) {
		...
		let x: number = player.cockThatFits(emberVaginalCapacity());
		if (x < 0) x = player.torso.cocks.sort(Cock.SmallestCockArea)[0];
		DisplayText("\"<i>Go on.</i>\"  She moves her arms around you and into a hug.  \"<i>Enter me.</i>\"  With no further prelude needed, you slide yourself into the damp interior of her cunt, the organ eagerly accepting you back for the fourth time.");
		...
		if (Flags.list[FlagEnum.EMBER_MILK] > 0) DisplayText(", milk seeping across your fingers,");
		DisplayText(" and promise you will, sliding slowly in until you have hilted yourself yet again.");

		DisplayText("\n\nThe dragon");
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] > 0) DisplayText("-girl");
		DisplayText(" moans at your ministrations");
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) {
			DisplayText(", her draconic prick pressing tightly against you as it leaks a steady stream of pre, matting ");
			if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 1) DisplayText("your [chest], ");
			DisplayText("your belly");
			if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 1) DisplayText(",");
			DisplayText(" and Ember's own.");
		}
		DisplayText("  \"<i>Do you like playing with my body?</i>\"  Ember asks, gazing at you through half-lidded eyes.  You give her a playful smirk and run your fingers expertly over her nipples, making her moan and mewl at the stimulation and saying more than mere words could on the matter.");
		...
		if (player.torso.tailType > TailType.NONE) DisplayText("the base of your tail, tugging lightly on it and stroking it for a moment, then her hands move on to ");
		DisplayText("your [butt], grabbing the ");
		...
		if (player.torso.wings.type > WingType.NONE) DisplayText("stopping momentarily to stroke along your " + player.torso.wings.desc + " wings before continuing up and ");
		DisplayText("stopping at the back of your head.");

		DisplayText("\n\nSlowly she guides you towards one of her erect nipples.  \"<i>");
		if (Flags.list[FlagEnum.EMBER_MILK] > 0) DisplayText("Drink my milk, I've been saving it up specially for you,");
		else DisplayText("I may not have any milk in my breasts right now, but maybe if you keep suckling I'll be able to make you some,");
		DisplayText("</i>\" Ember teases you.  You smile at her and accept the nipple, rolling it between your lips and exerting gentle, teasing pressure with your teeth.");
		if (Flags.list[FlagEnum.EMBER_MILK] > 0) DisplayText("  You savor the resultant gush of sweet dragon's milk as it squirts obediently down your throat.");
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText("  A small dollop of pre escapes Ember's dragon-dick at the pleasure of the movement.");
		DisplayText("  \"<i>And don't forget about my other breast either.</i>\"  She takes your hand in her own and guide it to her other mound, helping you knead it.");
		...
		if (Flags.list[FlagEnum.EMBER_ROUNDFACE] > 0) DisplayText("-girl");
		DisplayText(" roaring towards the ceiling, then turning to look at you as her pussy constricts and milks you for all you're worth.  Already spent from your past three orgasms, you just sit back and enjoy yourself being milked by her strong vaginal muscles one last time.");
		...
		DisplayText(".");
		if (Flags.list[FlagEnum.EMBER_GENDER] === 3) DisplayText("  Her draconic cock throbs all the way through your orgasm, shooting blanks a few times before spurting a couple ropes of pre onto her belly.");
	...
	private emberJizzbangbangEnding() {
		...
		DisplayText(", letting you see the whiteness on " + emberMF("his", "her") + " tongue, before tipping " + emberMF("his", "her") + " head back and gulping it down, licking " + emberMF("his", "her") + " lips and moaning as if " + emberMF("he", "she") + " was tasting a fine wine.  \"<i>Thanks for the snack, my mate.</i>\" " + emberMF("He", "She") + " croons at you, a teasing expression on " + emberMF("his", "her") + " face.");
		...
		if (Flags.list[FlagEnum.EMBER_GENDER] === 1 || Flags.list[FlagEnum.EMBER_GENDER] === 3) {
			...
			if (Flags.list[FlagEnum.EMBER_ROUNDFACE] === 0) DisplayText("anthro");
			else DisplayText("monster-" + emberMF("boy", "girl"));
			DisplayText(" to come clean.  \"<i>Cut me some slack... I'm not made of stone, you know.  I was just giving my mate a blowjob, you can't seriously expect me not to react to that...</i>\"  " + emberMF("He", "She") + " crosses " + emberMF("his", "her") + " arms and blows a puff of smoke as " + emberMF("he", "she") + " looks away, obviously flustered.");
