export class SophieFollowerScene extends NPCAwareContent {
	...
	//Un-Bimbo*
	internal function unbimboSophie() {
		DisplayText().clear();
	...
	private yesDebimboSophie() {
		...
		DisplayText("\n\n\"<i>Wha... what did you DO TO ME!?!?!</i>\" Sophie screams, a look of sheer feral rage splayed across her still-bimbofied face, full-lips drawn into a snarl beneath gleaming eyes and long golden hair.  Her shiny yellow feathers ruffle as she stomps up to you, putting her face right up to yours.  \"<i>You " + Desc.Gender.mf(player, "bastard", "bitch") + "!  WHAT DID YOU DO TO ME?</i>\"");
	...
	private bimboSophieAgain() {
		...
		if (Game.inCombat)
			return { next: Scenes.camp.returnToCampUseOneHour }; //(Display Sophie's normal options.You monster)
		else sophieBimbo.approachBimboSophieInCamp(false);
	}

	//Beat Her (You Monster)*
	private beatSophieAroundYouMonster() {
		DisplayText().clear();
	...
	internal function beatUpDebimboSophie() {
		DisplayText().clear();
	...
	internal function debimboSophieBeatsYouUp() {
		DisplayText().clear();
	...
	private whyIDidItToDebimboSophie() {
		DisplayText().clear();
	...
	private apologizeToDebimboSophie() {
		DisplayText().clear();
	...
	public sophieDaughterDebimboUpdate() {
	...
	public followerSophieMainScreen() {
		DisplayText().clear();
		...
		if (sophieBimbo.sophieIsInSeason() && player.torso.cocks.count > 0 && Flags.list[FlagEnum.FOLLOWER_AT_FARM_SOPHIE] === 0) {
			...
			DisplayText(".  \"<i>Ohhh, come on, [name]!  Just give it to me like a good " + Desc.Gender.mf(player, "boy", "girl") + ", would you?  I could even give you a special treat.  <b>Just give. It. To. Me.</b></i>\"");
	...
	private fuckFollowerSophie() {
		...
		DisplayText("\n\nSophie gives your " + Desc.Cock.describeCock(player, x) + " an appreciative swat before pirouetting around, bent over with her hands on her knees and her butt up in the air.  She wiggles back and forth, her lasciviously luscious backside jiggling, the cheeks clapping audibly at the apex of each sway.  Her voice purrs, \"<i>Come and get it, if you think you're " + Desc.Gender.mf(player, "man", "woman") + " enough!</i>\"");

		DisplayText("\n\nYou prowl forward, but Sophie backs up, wings flapping, her rump pushing right into you.  Her bubble-butt drives you back against one of the many standing stones, pinned between a cunt and a hard place.  Squishing noisily, your " + Desc.Cock.describeCock(player, x) + " slides right into Sophie's ");
		...
		DisplayText(", her words still affect you.  Her pussy just feels... so... exquisite.  It's so hot and wet that you almost feel like you could cum right now.  Your [legs] wobble, but Sophie just pushes back even harder, holding you up by your " + Desc.Cock.describeCock(player, x) + " and the friction of your [butt] on the boulder.  The harpy watches your expression as her hips continue their slow roll, feeling your fingers twitch and knead her immense backside when the pleasure begins to mount.  You try to stay composed, but it's just so gods-damned good!  Her silken slit is so warm, wet, and just plain... erotic.  Slowly, the point where you would normally orgasm approaches, but instead of the spasming clenches that signal release, your jizz stays bottled up inside you tighter than sperm in a goblin's womb.");

		DisplayText("\n\nSophie begins to grunt and move more forcefully.  Her slit batters hard up against you as she cries, \"<i>Fuck yes!  Oooh, yeah that feels good.  You're a good " + Desc.Gender.mf(player, "boy", "girl") + ", not cumming just yet.</i>\"  The compelling tone returns.  \"<i>Massage my butt and ");
		...
		if (y >= 0) DisplayText("You shift to bring your " + Desc.Cock.describeCock(player, y) + " up in place, just in time for Sophie's wiggling rectum to spread around it.  It slides on in without so much as a whimper.");
		...
		DisplayText("  Then, you black out, with your " + Desc.Cock.describeCock(player, x));
	...
	//[Next]
	private sophieVagFollowerFollowup() {
		DisplayText().clear();
		let x: number = player.cockThatFits(sophieBimbo.sophieCapacity());
		if (x < 0) x = player.torso.cocks.sort(Cock.SmallestCockArea)[0];
		let y: number = player.cockThatFits2(sophieBimbo.sophieCapacity());
		...
		else if (x.area <= 5) DisplayText("Not too bad for a tiny girl-dick!");
	...
	//Blowjob*
	//Sucking dicks.
	private sophieFollowerGivesBlowjobs() {
		DisplayText().clear();
		let x: number = player.cockThatFits(sophieBimbo.sophieCapacity());
		if (x < 0) x = player.torso.cocks.sort(Cock.SmallestCockArea)[0];

		DisplayText("Sophie cocks her head to the side and chuckles, \"<i>You wanna feel Momma Sophie's lips around that ");
		if (x.area <= 5) DisplayText("tiny clit you call a penis");
		else if (x.area <= sophieBimbo.sophieCapacity() / 2) DisplayText("needy boy-cock");
	...
	private sophieBlowsSixtyNine() {
		...
		if (player.torso.cocks.count > 1) DisplayText("s, searching for her target of the day.  At last, she settles on your " + Desc.Cock.describeCock(player, x) + ", giving it an affectionate squeeze");
		else DisplayText(", giving it an affectionate squeeze");
		DisplayText(".  She exhales on it with breath so hot and moist you almost think the blowjob has begun, but when her lips truly meet your " + Desc.Cock.describeCockHead(x) + ", that notion is shattered by the slippery-sweet embrace.  With your senses so deprived by snatch, smothered in quim, the touch of Sophie's golden cock-pillows on you seems multiplied seven-fold.  Your back arches instinctively, trying to lift your [hips] higher, but the matron's gentle, firm hand pushes you back down.  Her tongue flicks out to dance along the underside of your urethra while her cushiony mouth slips ever-lower.");
		...
		if (player.torso.head.face.tongue.type > TongueType.HUMAN && player.torso.head.face.tongue.type <= TongueType.DRACONIC) DisplayText("Longer and longer, your tongue continually unspools from your mouth, plunging deeper into the slippery channel with each passing moment.  Her taste saturates your many taste buds, and the velvety folds of her interior feel wonderful as you repeatedly slither your inhuman muscle through that syrupy passage.");
		...
		if (x.area <= 60) DisplayText("to the [sheath]");
		...
		if (player.cumQ() >= 500) DisplayText("  She quickly gives up on trying to swallow it after she starts showing some cum-pudge, but her hand pumps you energetically as she milks your " + Desc.Cock.describeCock(player, x) + ", squeezing the voluminous ropes of cum all over your [chest].");
	...
	private forceSophieBlowjob() {
		...
		DisplayText("\n\nSophie grumbles, but one of her hands does nestle itself down between her feathery thighs, her other going to your groin to fish out your " + Desc.Cock.describeCock(player, x) + ".  She looks up at you through angry eyebrows softened slightly by concealed lust.  \"<i>I'll suck you off... this time,</i>\" she declares, ");
		...
		DisplayText(" your legs.  Sophie's eyes roll back slightly - not all the way but enough to proudly display her penultimate pleasure as she cums.  Her syrupy girl-cum splatters off the waste-land earth below, though the sound changes to a wetter, muddy splashing after a few seconds.  Squeezing tightly, the harpy's throat convulses around you.  Her lips lock tight as she hums in ecstasy, languidly beginning to suckle your " + Desc.Cock.describeCock(player, x) + " as her own orgasm dies down.  You force her to move faster, pushing her head to pop back and forth, too eager for release to care for her comfort.  Judging by the steady, squelching pumping that begins anew, she doesn't care that much either.");
		DisplayText("\n\nSophie suddenly compresses her gold-lacquered cock-pillows around you and sucks harder.  She flicks her eyes up at you to beg for your cum with a sluttier expression than you thought the harpy matron capable, inaudibly demanding you release inside her.  Your [hips] lurch forward to pound your " + Desc.Cock.describeCock(player, x) + " deep into her face, and your " + Desc.Cock.describeCockHead(x) + " tingles, expanding in orgasm.");
	...
	public sophieSmallDongTeases() {
		...
		if (player.torso.chest.get(0).rating >= 1) DisplayText(" and squeezes your " + Desc.Breast.breastCup(player.torso.chest.get(0)) + " breast roughy.");
		else DisplayText(" and lets the talons dig into your " + Desc.Skin.skinFurScales(player) + " just enough to hurt a little.");

		DisplayText("\n\nSophie licks her shining lips as she looks down at the little spit-soaked pole that's twitching between her legs, just inches away from her pussy.  \"<i>Oooh, does Momma Sophie's favorite girl" + Desc.Gender.mf(player, "-boy", "y-girl") + " want her little bitch-stick played with?</i>\"  She touches it, right below the [cockHead smallest], her warm fingertip sliding in slow, sensuous circles until you begin to twitch at her touches.  \"<i>You know what I think?</i>\" she purrs, bending over to look you face to face- an amazing feat of flexibility considering her foot is locked on your shoulder.  \"<i>I think you like it when I play with this tiny girl-cock.  I think some depraved, sick, nasty part of your brain makes you cum harder when I tease you about how feminine this bitch-cock is.</i>\"");
		...
		//{BOTH END HERE}
		DisplayText("\n\nThe older harpy giggles, when it ends.  \"<i>I love sensitive " + Desc.Gender.mf(player, "lady-boys", "girls") + " like you...  You let me feel so powerful...</i>\"");
	...
	//Similar to bimbo sophie
	public sophieFollowerGoesIntoSeas() {
	...
	public sophieFertilityKnockedUpExpired() {
	...
	//[special]
	private sophieSpecial() {
		DisplayText().clear();
		let x: number = player.cockThatFits(sophieBimbo.sophieCapacity());
		if (x < 0) player.torso.cocks.sort(Cock.SmallestCockArea)[0];
		DisplayText("\"<i>Ahhh, so you do want a bit of Momma Sophie's special treatment... such a brave " + Desc.Gender.mf(player, "boy", "girl") + ", aren't you?</i>\" She leans in closer as she starts to get rid of your [armor].  \"<i>Very brave... we might have to put you back into place.  Momma Sophie will milk this " + Desc.Cock.describeCock(player, x) + " until the only thing that even allows you to cum anytime in the future will be the fact I drugged you with my lipstick.</i>\"");

		DisplayText("\n\nSlowly, she sinks to her knees, fishing out your " + Desc.Cock.describeCock(player, x) + " from your [armor], licking her lips eagerly.  \"<i>First, we cover it with a lot of my special lipstick, so you'll stay hard no matter what.  That, and, well, so you can't resist when Momma Sophie wants you to squirt.</i>\"");

		DisplayText("\n\nIndeed, her plush golden lips wrap about your " + Desc.Cock.describeCock(player, x) + ", slathering it with both her saliva and lipstick as she bobs her head up and down on your member, causing it to become painfully hard as you thrust your [hips] instinctively, seeking more of this delicious mouth.  She grumbles, displeased at you moving and tries to hold you in place by grabbing your [ass] and trying to hold you in place; however, this proves to be rather uneffective as you begin to shake from her oral assault, her tongue licking up and down your " + Desc.Cock.describeCock(player, x) + ", covering it completely in a mix of slutty spit and drugged lipstick... as a matter of fact, you feel light in the head");
		...
		DisplayText(".  Sophie seems to sense it and chuckles, pulling her mouth off your " + Desc.Cock.describeCock(player, x) + ", causing you to whine as you seek out her mouth again.  \"<i>There, there, my little cum pump.  Momma Sophie wouldn't be so mean as to leave you and your ");
		if (x.area <= 5) DisplayText("squirting clitty");
		else if (x.area <= 30) DisplayText("needy boy-cock");
		...
		DisplayText(Desc.Cock.describeCock(player, x) + " inside Sophie's schlong-milking passage, causing her to moan in satisfaction as the first of your virile cum hits her inner walls.");
		...
		if (x.area <= 5) DisplayText("dainty little fem-cock");
		else if (x.area <= 30) DisplayText("nice, sweet cum-squirter");
		...
		DisplayText("\n\nWith a grunt, you manage to withhold from her command, and Sophie coos with delight at your resistance.  \"<i>Look at my " + Desc.Gender.mf(player, "boy", "girl") + ", all grown up now, resisting the pleasures of Momma Sophie's fertile, cum-thirsty cunny!  You shouldn't do that... though, maybe we could cum together if you do... Okay, baby, once you feel Momma Sophie's pussy squeeze you extra hard, you will cum and you won't try to hold back, okay?</i>\"  You grunt and nod, but already find it hard to hold back when you imagine how Sophie will look knocked up with your kids. Once you cum inside her a second time, there's probably no way it won't hold.");

		DisplayText("\n\nYou hear Sophie's shlicking vagina swallow your entire " + Desc.Cock.describeCock(player, x) + " and release it repeatedly, her hips and soft, plump ass bouncing off of your [legs] as she continues to fuck herself on your " + Desc.Cock.describeCock(player, x) + " harder and harder.  The way her inner folds lap and massage at your drug-infused cock is exquisite, and it takes every ounce of willpower to indeed stop yourself from cumming inside her repeatedly like you were broken down there.  Finally, Sophie slaps her hips into your hardness, moaning out loud as her cunt squeezes and tugs at your member violently, temporarily stopping the flow of cum out of your " + Desc.Cock.describeCock(player, x) + ", but only for a split second.  It churns and boils in your [balls], raising through your " + Desc.Cock.describeCock(player, x) + " to be fired into Sophie's wet, ravenous cunt and baby-wanting womb.  Sophie's own orgasm and yours blend into one and prolong one another as you release ");
		...
		DisplayText("\n\n\"<i>Ahhh... ahhh... good " + Desc.Gender.mf(player, "boy", "girl") + "... very good... I'm definitely a momma now.  But... buuuut... You should squirt out more just to make sure... and as punishment for trying to resist when Momma Sophie ordered you to cum!</i>\"");
		...
		DisplayText("\n\nShe says that, but every lift and fall of her hips is a torture on your thoroughly milked, overly sensitive " + Desc.Cock.describeCock(player, x) + ".  Her pussy is trying to draw out every drop of fluid you have in your body, and soon, it will succeed at doing just that it seems...");
	...
	//Get Out
	private getOutSophieRecruitment() {
		DisplayText().clear();
	...
	//If Sure
	private sophieRecruitmentFinale() {
		DisplayText().clear();
	...
	internal function sophieIncestInHerCooterOrSomethingIDunno() {
		DisplayText().clear();
		DisplayText("Approaching the harpy nest you call out for one of your daughters.  ");
		if (Flags.list[FlagEnum.SOPHIE_ADULT_KID_COUNT] > 1) DisplayText("The prospect of their " + Desc.Gender.mf(player, "father", "mother") + "'s attention causes a small scuffle to break out between your feathery daughters. Thankfully, the small fight is soon ended and one of your daughters flies down and stands before you.  ");
		...
		if (daughterCap === 0) {
			...
			if (Flags.list[FlagEnum.DAUGHTER_ONE_BIMBO] > 0 && Flags.list[FlagEnum.SOPHIES_DAUGHTERS_DEBIMBOED] === 0) DisplayText("\"<i>Like, how do I look, " + Desc.Gender.mf(player, "Daddy", "Mommy") + "?  Aren't my breasts so totally soft?  Don't you just want to suck and squeeze them?  Ooohhh!  " + Desc.Gender.mf(player, "Daddy", "Mommy") + ", can you please touch me?  I want you so bad!  My pussy gets sooo wet when I, like, think about you pounding my cunny!</i>\"");
			//normal speech:
			else DisplayText("\"<i>Don't you like how big and soft my breasts are, " + Desc.Gender.mf(player, "Daddy", "Mommy") + "?  Don't you just want to squeeze them and suck on them?  Ooohh, " + Desc.Gender.mf(player, "Daddy", "Mommy") + ", please, I want you so bad.  I get so hot and wet when I think of you!  I just want your [cock] inside me, pounding my hot cunt until I scream!</i>\"");
			...
			if (Flags.list[FlagEnum.DAUGHTER_ONE_BIMBO] > 0 && Flags.list[FlagEnum.SOPHIES_DAUGHTERS_DEBIMBOED] === 0) DisplayText("\n\n\"<i>Ohhh, oooohhhh!  " + Desc.Gender.mf(player, "D-Daddy", "M-Mommy") + "!  L-like, I can't take so much teasing!  Please, Gimme your h-haaard cock, " + Desc.Gender.mf(player, "Dad", "Mom") + "!<i>\"");
			//[normal speech:
			else DisplayText("\n\n\"<i>Ohhh, oooohhhh!  " + Desc.Gender.mf(player, "D-Daddy", "M-Mommy") + "!  Nnnnn... I can't take so much teasing!  Please, please give it to me, fuck me as hard as you can, " + Desc.Gender.mf(player, "Daddy", "Mommy") + "!<i>\"");
		}
		//second daughter: 
			...
			if (Flags.list[FlagEnum.DAUGHTER_TWO_BIMBO] > 0 && Flags.list[FlagEnum.SOPHIES_DAUGHTERS_DEBIMBOED] === 0) DisplayText("\"<i>Mmmm... doncha want to just like, pound my sexy ass, " + Desc.Gender.mf(player, "Daddy", "Mommy") + "?  It's so big, it's just begging to be spanked and squeezed till it's totally red and sensitive</i>\"");
			else DisplayText("\"<i>Come on, " + Desc.Gender.mf(player, "Daddy", "Mommy") + ", touch me, spank me like the bad girl I am, lusting after my " + Desc.Gender.mf(player, "Daddy", "Mommy") + " like this.  Give it to me hard, spank my big round ass and pound my hot tight cunt like I know you want</i>\"");
		...
		//third daughter:
			...
			DisplayText("\n\nYou clap your hands and applaud your daughter's work.  The curvy little harpy is working so hard to please her " + Desc.Gender.mf(player, "daddy", "mommy") + " and entice him into her nest...  ");
			if (Flags.list[FlagEnum.DAUGHTER_THREE_BIMBO] > 0 && Flags.list[FlagEnum.SOPHIES_DAUGHTERS_DEBIMBOED] === 0) DisplayText("\"<i>L-like, did I do a good job, " + Desc.Gender.mf(player, "Daddy", "Mommy") + "?  I so totally did my best for you.  I know I'm not super sexy like Mom and my sisters... but, " + Desc.Gender.mf(player, "Daddy", "Mommy") + ".. I will like, still please you, like, totally the best that I can!</i>\"");
			else DisplayText("\"<i>W-well, what did you think, " + Desc.Gender.mf(player, "Dad", "Mommy") + "?  Did I do a good job?  I know I'm not as hot as Mom, but I promise I'll please you as best I can!</i>\"");
			...
			if (Flags.list[FlagEnum.DAUGHTER_FOUR_BIMBO] > 0 && Flags.list[FlagEnum.SOPHIES_DAUGHTERS_DEBIMBOED] === 0) DisplayText("\"<i>Come on " + Desc.Gender.mf(player, "Daddy", "Mommy") + ", can't you see how totally hot I am?  Don't you just want to let loose and like, just fuck me till I scream.</i>\"");
			else DisplayText("\"<i>Just look at me, " + Desc.Gender.mf(player, "Daddy", "Mommy") + ", so curvy, so ready, so hot and wet for you.  Just let loose and show me your love, show me how much you love me, " + Desc.Gender.mf(player, "Daddy", "Mommy") + ", fuck me hard, make me scream and claim me as yours.</i>\"");
			...
			//bimbo speech
			if (Flags.list[FlagEnum.DAUGHTER_FOUR_BIMBO] > 0 && Flags.list[FlagEnum.SOPHIES_DAUGHTERS_DEBIMBOED] === 0) DisplayText("  \"<i>Ahh... ohhh... oh, " + Desc.Gender.mf(player, "Daddy", "Mommy") + "!  You like my butt, don't you; isn't it nice?  It's so good to grab and squeeze.  I bet it would be fun to, like, spank it too....</i>\"");
			//[normal speech:]
			else DisplayText("  \"<i>Mmmm... Oh, " + Desc.Gender.mf(player, "Daddy", "Mommy") + ", yessss...  That's it, touch me, touch me and play with me, do whatever you want with me.</i>\"");
	...
	private phaseTwoOfIncest(daughter: number) {
		DisplayText().clear();
		let x: number = player.cockThatFits(sophieBimbo.sophieCapacity());
		if (x < 0) x = player.torso.cocks.sort(Cock.SmallestCockArea)[0];
		daughter++;
		...
		if (bimbo) DisplayText("\n\n\"<i>Yessss!  Please, " + Desc.Gender.mf(player, "Daddy", "Mommy") + "!  Give it to me!  Like, fuck my brains out till I can't think of anything but your, like, totally amazing cock!</i>\"");
		else DisplayText("\n\n\"<i>Yesssss! Please, " + Desc.Gender.mf(player, "Daddy", "Mommy") + ", I've been wanting this so badly!  Give it to me, fuck me like the stud you are and fuck my cunt full of spunk!</i>\"");
		DisplayText(" she croons, her words dripping with raw, passionate need for you and your incestuous love.  The sight of her so needy, so wanting, her body aflame with an unquenchable hunger for you is just too tempting.  Your own lust boils over as a thick dollop of pre-cum bursts from your " + Desc.Cock.describeCockHead(x) + " and splatters across your daughter's sodden snatch, adding to the lube already waiting to welcome you into her body.");

		DisplayText("\n\nTaking your sensitive, throbbing cock in hand, you give yourself a stroke and let out a groan at how ready you are; how hard you are for the incestuous fuck you're going to enjoy.  With as much grace as a fuck-hungry beast, you align yourself with your daughter's tender folds and plunge yourself in, your " + Desc.Cock.describeCock(player, x) + " sliding into her slippery cunt easily thanks to all the sweet juices oozing from her.  The sudden hard thrust into her body sends shockwaves of pleasure through your over-aroused daughter, her legs wrapping around your waist as soon as you hilt yourself inside her.  Not waiting for her to get used to you, you start to buck your hips, pistoning your " + Desc.Cock.describeCock(player, x) + " into her over and over as you keep her pinned to the ground.  Her bird-like cries of delight fill the air and mix with the hard, slippery slaps of your hips against hers, telling you all you need to know about her pleasure and how she loves to be fucked by her " + Desc.Gender.mf(player, "father", "mother") + ".");
		...
		if (bimbo) DisplayText("  \"<i>Oh, " + Desc.Gender.mf(player, "Daddy", "Mommy") + ", oh, " + Desc.Gender.mf(player, "Daddy", "Mommy") + "...  So good!  My " + Desc.Gender.mf(player, "Daddy", "Mommy") + "'s such a hot, sexy " + Desc.Gender.mf(player, "stud", "herm") + ".  I so totally thought I was going to like, lose my mind for a second.  I love you, " + Desc.Gender.mf(player, "Daddy", "Mommy") + ".</i>\"");
		else DisplayText("  \"<i>Oh, " + Desc.Gender.mf(player, "Daddy", "Mommy") + ", oh, " + Desc.Gender.mf(player, "Daddy", "Mommy") + "... so good, you're such a fucking " + Desc.Gender.mf(player, "stud", "perfect breeder") + "!  I could just get addicted to you.  I love you, " + Desc.Gender.mf(player, "Daddy", "Mommy") + ",</i>\"");
