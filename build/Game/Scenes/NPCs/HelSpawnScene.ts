export class HelSpawnScene extends NPCAwareContent {
	...
	//New Variable: " + Flags.list[FlagEnum.HELSPAWN_NAME] + ", the name of Hel’s daughter
	...
	public helspawnFollower(): boolean {
		return Flags.list[FlagEnum.HELSPAWN_AGE] === 3;
	}
	...
	internal function heliasAppearanceScreen(): void {
		...
		if (Flags.list[FlagEnum.HEL_BONUS_POINTS] < 150) DisplayText("bright orange glow");
		else DisplayText("bright white glow");
		DisplayText(" that lets off a pleasant heat, though it never seems to burn you.  Hel is wearing her scale bikini and a leather thong, and using her scimitar as a weapon");
		//if Isabella is cool: 
		if (Flags.list[FlagEnum.HEL_ISABELLA_THREESOME_ENABLED] >= 1) DisplayText("; she’s got her blue bandanna wrapped around her head, mostly obscured by her fiery hair");
		...
		if (Flags.list[FlagEnum.HELSPAWN_NAME] != 0) {
			DisplayText("  A dark trio of scars run down Hel’s thighs, left by " + Flags.list[FlagEnum.HELSPAWN_NAME] + "’s youthful claws.");
		}
		else {
			switch (kGAMECLASS.helScene.pregnancy.event) {
				case 1: //She's pregnant, but no special text yet
					break;
				case 2: DisplayText("  Hel's just starting to show a little bulge of pregnancy.");
					break;
				case 3: DisplayText("  Hel’s belly is starting to look fairly bloated, swelling with her child.");
					break;
				case 4: DisplayText("  Hel’s belly is positively gravid, full of a little salamander child.");
					break;
				default: //She's not pregnant, check if she's ready for it
					if (Flags.list[FlagEnum.HEL_BONUS_POINTS] >= 150) DisplayText("  Hel’s long, reptilian tail is currently burning white-hot, signaling her body’s ready for motherhood.");
			}
		}
	...
	private helSpawnsAppearanceScreen(): void {
		...
		DisplayText("salamander daughter, " + Flags.list[FlagEnum.HELSPAWN_NAME] + ", stands about six-foot-six tall, with pale skin and thick, ");
		if (Flags.list[FlagEnum.HELSPAWN_DADDY] === 1) DisplayText("chitin-black");
		else DisplayText("bright-red");
		DisplayText(" scales covering her arms and legs, though she has a normal human face and torso.  A long leathery tail swishes happily behind her, surrounded by a dim fiery aura that emits a pleasant heat, though thankfully it isn’t hot to the touch, seemingly incapable of harming you.  " + Flags.list[FlagEnum.HELSPAWN_NAME] + " is wearing ");
		if (Flags.list[FlagEnum.HELSPAWN_PERSONALITY] >= 50) DisplayText("a scale bikini, just like her mother’s");
		else DisplayText("a short skirt, thigh-high boots, and a sky-blue blouse, in stark contrast to her mother’s sluttier attire");
		if (Flags.list[FlagEnum.HEL_ISABELLA_THREESOME_ENABLED] >= 1) DisplayText(", with a brightly-colored scarf wrapped around her neck");
		DisplayText(". She has a human face, with ");
		if (Flags.list[FlagEnum.HELSPAWN_DADDY] === 0) DisplayText("golden");
		else if (Flags.list[FlagEnum.HELSPAWN_DADDY] === 2) DisplayText("bright green");
		else DisplayText("crimson");
		DisplayText(" eyes, girlish features, and a smattering of pale red scales on her cheeks, like freckles.  Her short, dark-red hair cuts off just below her ");
		if (Flags.list[FlagEnum.HELSPAWN_DADDY] === 2) DisplayText("puppy-dog");
		else DisplayText("reptilian");
		DisplayText(" ears, and it’s adorned with a pretty pink bow.  She has surprisingly large hips, and a big, squishy butt behind her.  Her reptilian legs are covered with thick, dark scales, ending in short claws atop soft, leathery soles.");
	...
	public heliaBonusPointsAward(): void {
		...
		if (camp.companionsCount() === 1) DisplayText("You cock an eyebrow, saying that you're already alone, but she shakes her head, saying she needs somewhere private, away from prying eyes.  ");
	...
	private noYouDontLoveHeliaYouMonster(): void {
		...
		//[HelLove to NO]
		Flags.list[FlagEnum.HEL_LOVE] = -1;
		heliaLoveFollowup();
	}
	...
	private heliaLoveFollowup(): void {
		...
		if (player.torso.vaginas.count > 0) DisplayText("even another woman, it seems, ");
		DisplayText("we... change.</i>\"");
		...
		//If HelLove = No:
		if (Flags.list[FlagEnum.HEL_LOVE] === -1) DisplayText("Even if you don't love me, you'd love our child, right?  You can fuck me full of kids, or even... even get someone else to, I don't care.  I just </i>need<i> it, [name].  More than anything else in the whole wide world.");
		else {
			DisplayText("I love you, [name].  I love you so much.  I want to share this with you.  ");
			if (player.torso.cocks.count > 0) DisplayText("You'll fuck me full of kids, right?  Please?  Give me your seed, [name].  Give it all to me until I'm fucking gravid with your salamander babies.");
	...
	private HaveAHellKidPartII(): void {
		...
		if (player.tallness >= 90) DisplayText("up ");
		else if (player.tallness <= 72) DisplayText("down ");
		DisplayText("at you, holding you tight for a long moment before stepping away, still holding your hands.  \"<i>This is all new to me, [name].  I never pictured myself as a mother, with a mate and a stable, safe home - or as stable and safe as anything these days - but with you by my side, there's nothing I can't take on.</i>\"");
	...
	private getAnotherDad(): void {
		...
		if (player.torso.cocks.count <= 0) {
			DisplayText("You tell Helia you'd love to a share a child with her, but you're not... properly equipped for the endeavor.  \"<i>That's fine!  I can... I can wait, a little.  I-if you want to go grow one, I mean.  If not, then we can find someone with a cock.  ");
		...
		//[Another Dad] (PC has a dick)
			...
			DisplayText("\"<i>W-wha?</i>\" Hel says, cocking an eyebrow.  \"<i>You sure, [name]?  I-if you don't want to be the dad, I guess I understand.  But why?  Am I not... no, nevermind.  I don't want to know.  Okay, uh, we can find someone with a cock.  ");
			if (player.statusAffects.get(StatusAffectType.TelAdre).value1 >= 1) DisplayText("There's Miko and Mai from the bar.  Mai's said she wanted a kid, but can't take care of one... she'd probably be willing to fuck one into me!  If that's not alright, then... lemme think.  ");
		...
		if (player.torso.cocks.count > 0 && player.cockThatFits(helFollower.heliaCapacity()) >= 0) MainScreen.addButton(2, "I Will", haveAKid);
		else if (player.torso.cocks.count <= 0) MainScreen.addButton(2, "I Will", growingDicks4Hel);
	...
	private maiWouldBeTheBestInseminator(): void {
		...
		Flags.list[FlagEnum.HEL_NTR_TRACKER] = 1;
		Flags.list[FlagEnum.HELSPAWN_DADDY] = 2;
	...
	private spiderboyWouldBeBestDad(): void {
		...
		Flags.list[FlagEnum.HEL_NTR_TRACKER] = 1;
		Flags.list[FlagEnum.HELSPAWN_DADDY] = 1;
		return { next: playerMenu };
	}
	...
	private noKidsHel(): void {
		...
		if (player.torso.cocks.count > 0 && player.cockThatFits(helFollower.heliaCapacity()) >= 0) DisplayText("You couldn't if you wanted to anyway - you're too big to fit.  ");
		DisplayText("As the words leave your lips, you can see Hel's shoulders slump, a crestfallen look spreading across her face.  \"<i>A-are you sure?  Please, [name], I really, really want a child.  For </i>us<i> to have one.</i>\"");
	...
	public helGotKnockedUp(): void {
		...
		if (Flags.list[FlagEnum.HELSPAWN_DADDY] === 2) DisplayText("warm ");
		else DisplayText("wan ");
		DisplayText("smile as you approach, scooting over to allow you to sit beside her.  She almost seems distant, though she's quick to wrap her tail around your waist, hugging you close against her.");
		...
		else if (Flags.list[FlagEnum.HELSPAWN_DADDY] === 1) {
	...
	private sureHelGimmeSpidahBoyDetails(): void {
		...
		if (followerKiha()) DisplayText(", toward hotwings' old place");
		DisplayText(". Took a while, but eventually I just sort of stumbled on this couple of spidergirls, just as cute as could be.  They wander up to me, all cautious, and ask if I was serious about the whole baby thing.  So I say, 'Yeah, sure,' and they get this big, dopey grin, the both of 'em.</i>\"");
	...
	private youWantABoy(): void {
		...
		if (player.torso.cocks.count > 0 && player.torso.vaginas.count <= 0) DisplayText("If I were a guy, I'd want a big strong son to hang out with, too.  Take him fishing, teach him how to fight the way you do... you'll make a great dad, my love.  I'm sure you will.");
		else DisplayText("Would be nice to have a man around here, you know?  I miss hanging around the boys back home, watching 'em strut like peacocks for every passing girl.");
		DisplayText(" And any son of mine is going to be a real lady killer, mark my words.  We're going to have to fight off whole hordes of goblin sluts, all looking for a piece of our handsome little boy before you know it.</i>\"");
	...
	private youWantAGirl(): void {
		...
		else DisplayText("We're going to be a gaggle of tittering girls before you know it, though.  Salamanders grow up so fast, [name]...  I just hope she doesn't turn out like me, you know?  I don't know if I could stomach seeing my little girl becoming a wanton slut like her mom.  I might get jealous, after all...");
		DisplayText("  Any daughter of ours is going to be a real beauty, mark my words. She'll make a succubus look like a toad before she's out of swaddling.</i>\"");
	...
	private encouragePregalia(): void {
		...
		if (Flags.list[FlagEnum.HEL_LOVE] === 1) DisplayText("You love Hel");
		else DisplayText("Hel's your friend");
		DisplayText(", and if her child follows in her footsteps, so much the better.  Your lover manages a half-hearted laugh, blushing at your words.  Her tail tip brushes your cheek as its owner cuddles up against you, smiling.  \"<i>Thanks, lover mine.  I just hope I - we - can do right by the kid.  That's all.</i>\"");
	...
	private helsLifestyle(): void {
		...
		if (Flags.list[FlagEnum.HEL_LOVE] === 1) DisplayText("and I hope that's part of why you love me");
		else DisplayText("even though I know that's probably part of why you can't love me back");
		DisplayText(".  I'll try and rein it in, but maybe... it'd be better if you were the one who raised my child, [name].  God knows I'm the least qualified person to do it.</i>\"");
	...
	private applyHelspawnName(): void {
		...
		if (kGAMECLASS.testingBlockExiting) {
			...
			DisplayText().clear();
			if (mainView.nameBox.text === "") DisplayText("<b>You must select a name.</b>");
			else if (mainView.nameBox.text === "Hellgirl") DisplayText("\"<i>Hey, that's copyright infringement.  Probably.</i>\"");
			else if (mainView.nameBox.text === "Kid") DisplayText("\"<i>Wow, what are you, five?</i>\"");
			else if (mainView.nameBox.text === "Phoenix") DisplayText("\"<i>Oh hell no.  You're not naming my little girl after one of those... things!  Sorry Kiri, but still!</i>\"");
			else if (mainView.nameBox.text === "Savin") DisplayText("\"<i>That's a boy's name, dumbass.</i>\"");
		...
		Flags.list[FlagEnum.HELSPAWN_NAME] = mainView.nameBox.text;
		mainView.nameBox.visible = false;
		//[Name Field]
		//Easter Egg Names:
		DisplayText().clear();
		if (Flags.list[FlagEnum.HELSPAWN_NAME] === "Helspawn") DisplayText("\"<i>That's what I was gonna call her!  Except not at all.  God dammit, seriously, [name]?</i>\"\n\n");
		if (Flags.list[FlagEnum.HELSPAWN_NAME] === "Jayne") DisplayText("\"<i>That sounds like a hero's name... I like it.  A name that demands respect!</i>\"\n\n");
		if (Flags.list[FlagEnum.HELSPAWN_NAME] === "Hesper") DisplayText("\"<i>Ah, the Evening Star.  She'll be the star of my life, alright...</i>\"\n\n");
		if (Flags.list[FlagEnum.HELSPAWN_NAME] === "Kiri") DisplayText("\"<i>Aw, that's great, lover.  Sis'll shit her pants when she hears her niece is named after her!  Gonna get confusing, though...</i>\"\n\n");
		if (Flags.list[FlagEnum.HELSPAWN_NAME] === "Mai") DisplayText("\"<i>Aw, that's great, lover.  Mai'll be honored, I know it.  And it is a great name...</i>\"\n\n");
		if (Flags.list[FlagEnum.HELSPAWN_NAME] === "Tanis") DisplayText("\<i>I don't... I don't know if I want her to have that name, lover.  It's got a lot of baggage, but... but it does my mother an honor.  More than I could ever give her.</i>\"\n\n");
		//Syn: 
		if (Flags.list[FlagEnum.HELSPAWN_NAME] === "Syn") DisplayText("\"Awesome.  I was hoping you'd choose that one, lover mine.</i>\"\n\n");
		if (Flags.list[FlagEnum.HELSPAWN_NAME] === "Chara") DisplayText("\"<i>Awesome.  You liked my ideas, after all!</i>\"\n\n");

		DisplayText("\"<i>So... " + Flags.list[FlagEnum.HELSPAWN_NAME] + " it is.  That's a good name, my love.  A strong name.  ");
		if (Flags.list[FlagEnum.HELSPAWN_DADDY] === 0) DisplayText("Our");
		else DisplayText("my");
		DisplayText(" little girl's going to grow up big and strong with a name like that... and with you around to guide her,</i>\" Hel says, kissing your cheek.  \"<i>Why don't you give me a minute to catch my breath and get the little one situated, and let's talk.  Alright?</i>\"");

		DisplayText("\n\nYou nod and help Helia to her feet, still holding " + Flags.list[FlagEnum.HELSPAWN_NAME] + " to her breast.  Your lover gives you a wink before walking bow-legged back toward her part of camp, and the little crib she's built beside her hammock.");
		helSpawnsSetup();
	...
	private helSpawnsSetup(): void {
		...
		Flags.list[FlagEnum.HELSPAWN_PERSONALITY] = 50;
		if (Flags.list[FlagEnum.HELSPAWN_DADDY] === 2) Flags.list[FlagEnum.HELSPAWN_PERSONALITY] += 10;
		//>If Spiderbro is daddy: +10 HelspawnChaste
		if (Flags.list[FlagEnum.HELSPAWN_DADDY] === 1) Flags.list[FlagEnum.HELSPAWN_PERSONALITY] -= 10;
		if (Flags.list[FlagEnum.HELSPAWN_DADDY] === 0) {
			//>If Corrupt (66+ Corr) PC is daddy: +10 Helspawn Slutty
			if (player.stats.cor >= 66) Flags.list[FlagEnum.HELSPAWN_PERSONALITY] += 10;
			//>If Pure (33> Corr) PC is daddy: +10 Helspawn Chaste
			if (player.stats.cor <= 33) Flags.list[FlagEnum.HELSPAWN_PERSONALITY] -= 10;
		}
		Flags.list[FlagEnum.HELSPAWN_AGE] = 1;
	...
	internal function heliaTalkEight(): void {
		...
		DisplaySprite(68);
		DisplayText("Watching your little daughter crawl around, you and Hel sit down together arm in arm, smiling as the little salamander girl plays with bits and pieces of Hel's gear.");
		...
		if (Flags.list[FlagEnum.HELSPAWN_DADDY] === 0) DisplayText("parental ");
		else DisplayText("care giver's ");
		DisplayText("pride, either.  She really is growing up faster, many times faster than a human girl.  And, by the sound of things, many times faster than a salamander ought to.  You ask Hel if that's right, and she nods gravely.");
		...
		DisplayText(" little girl's going to be a big girl damn soon.  Let's enjoy it while we can, eh?</i>\"");

		DisplayText("\n\nYou nod to your lover, and the both of you walk over to spend some quality time playing with " + Flags.list[FlagEnum.HELSPAWN_NAME] + ".");
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	internal function playWithYourKid(): void {
		...
		if (Flags.list[FlagEnum.HELSPAWN_DADDY] != 0) DisplayText("ward ");
		else DisplayText("daughter ");
		DisplayText("cries out in surprise and glee.  Soon, you're bouncing her, then down on the ground crawling around with her, playing peek-a-boo and a dozen other games as you and " + Flags.list[FlagEnum.HELSPAWN_NAME] + " laugh and hug.  Helia herself slips down with you before you know it, holding her daughter close as she tries to communicate, teaching the ever-growing girl a few words at a time.");

		DisplayText("\n\n\"<i>Mommy!</i>\" " + Flags.list[FlagEnum.HELSPAWN_NAME] + " eventually cries, bopping her mother on the nose. Hel laughs, but it turns into a sigh as her daughter reaches down and fiddles with her top, obviously hungry.");

		DisplayText("\n\n\"<i>We've got to wean this one before she bites my tits off,</i>\" Hel groans as " + Flags.list[FlagEnum.HELSPAWN_NAME] + " latches on, starting to suckle.  \"<i>Those teeth came in fast...</i>\"");

		DisplayText("\n\nLaughing, you rustle " + Flags.list[FlagEnum.HELSPAWN_NAME] + "'s fiery hair and leave mother and daughter to finish the meal.");
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	public helSpawnGraduation(): void {
		...
		DisplaySprite(68);
		Flags.list[FlagEnum.HELSPAWN_GROWUP_COUNTER] = 1;
		Flags.list[FlagEnum.HELSPAWN_AGE]++;
		DisplayText("You wake up to a scream, high-pitched and terrified.  You bolt up out of bed, tangling up in your bedroll for a minute before stumbling over to Hel and... " + Flags.list[FlagEnum.HELSPAWN_NAME] + "?");
		DisplayText("\n\nYou do a double take as you lay eyes on the ");
		if (Flags.list[FlagEnum.HELSPAWN_DADDY] === 1) DisplayText("black");
		else DisplayText("red");
		DisplayText("-scaled figure curled up next to Helia; Hel herself is looking wide-eyed down at the drowsy figure on her breast, just barely opening her eyes.  \"<i>Y-you're... you're huge!</i>\" Hel manages to gasp out, pushing herself back and away from her daughter.");
		...
		DisplayText("\n\n\"<i>Ugh, what's wrong?</i>\" " + Flags.list[FlagEnum.HELSPAWN_NAME] + " groans, looking up at Hel, then to you.  \"<i>" + championRef() + ", why's mom screaming?</i>\"");
		...
		DisplayText("\n\nYou have to admit, the change is sudden and drastic... but the girl's scales, her eyes, they're certainly " + Flags.list[FlagEnum.HELSPAWN_NAME] + "'s.  ");
		if (Flags.list[FlagEnum.HELSPAWN_DADDY] === 2) DisplayText("And the puppy ears, now more wolf-like, are a dead giveaway.  Utterly unique to her.  ");
		DisplayText("You sit down beside the obviously confused girl and stroke her hair, telling her it's all right, her mother's just startled, that's all.");

		DisplayText("\n\n\"<i>It's just me, mom,</i>\" " + Flags.list[FlagEnum.HELSPAWN_NAME] + " says, sliding over to sit beside her mother, who's still wide-eyed with shock.  Your girl tries for a few moments to talk to Hel, but seeing that venture prove fruitless - Hel seems utterly insensate, paralyzed by shock - she instead curls back up beside her mother, head rested on Helia's shoulder.  Eventually, warily, Hel reaches out a trembling hand to brush her daughter's cheek.");
	...
	public helspawnDiscoversBooze(): void {
		...
		if (player.race() != "cat-boy" && player.race() != "cat-girl") DisplayText("a cat");
		else DisplayText("the cat you are");
	...
	private scoldHelSpawn(): void {
		...
		DisplayText("\n\n\"<i>Alright, " + championRef() + ",</i>\" " + Flags.list[FlagEnum.HELSPAWN_NAME] + " says, curling up beside you, head resting on your [chest].");
		//{HelspawnChaste +10}
		Flags.list[FlagEnum.HELSPAWN_PERSONALITY] -= 10;
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	//Encourage Her
	private encourageHelspawn(): void {
		...
		DisplayText("\n\n\"<i>Wha...?</i>\" " + Flags.list[FlagEnum.HELSPAWN_NAME] + " starts to say, before catching on as you drink, inviting her to join you.");
		...
		DisplayText("\n\n\"<i>Uh...</i>\" you groan, slumping over onto your [ass].  " + Flags.list[FlagEnum.HELSPAWN_NAME] + "'s right behind you, collapsing on top of you in a pile of ");
		if (Flags.list[FlagEnum.HELSPAWN_DADDY] != 1) DisplayText("red ");
		else DisplayText("black ");
		DisplayText("scales and the smell of a burning brewery.");
		...
		DisplayText("\n\n\"<i>I saved some for ya, mom,</i>\" " + Flags.list[FlagEnum.HELSPAWN_NAME] + " groans, retching.");
	...
	public helSpawnChoosesAFightingStyle(): void {
		...
		if (camp.companionsCount() > 3) DisplayText("apparently having cleared out most of your other followers for the moment, ");
		DisplayText("her sword held at a guard pose between herself and a training dummy. " + Flags.list[FlagEnum.HELSPAWN_NAME] + " stands a few feet behind her mother, watching intently as Hel paces around the dummy, breathing hard.");
	...
	private snipermanders(): void {
		...
		DisplayText("You tell Hel to take a break: you've got this handled. Grabbing your bow and quiver from over your shoulder, you hand them over to ");
		if (Flags.list[FlagEnum.HELSPAWN_DADDY] === 0) DisplayText("your");
		else DisplayText("Hel's");
		DisplayText(" eager daughter. " + Flags.list[FlagEnum.HELSPAWN_NAME] + " takes them up with surprising reverence, holding them as gingerly as glass.  Grinning, you clasp her shoulder and tell her how to put the quiver on, and get her stance right for shooting.  It takes her a moment to get set up: her scaled, reptilian legs don't naturally stand in the shooting pose you've adopted");
		if (player.hips.legs.type === LegType.LIZARD) DisplayText(", even with your own lizard legs");
		DisplayText(", and you find yourself having to correct her grip on the haft several times.  But, finally, you get her ready to shoot.");

		DisplayText("\n\nYou slip behind " + Flags.list[FlagEnum.HELSPAWN_NAME] + ", putting your arms on hers as you guide her through nocking an arrow, head resting on her fist, a lone finger outstretched toward the dessicated dummy.");
		...
		//If Rath is @ camp:
		if (player.statusAffects.has(StatusAffectType.CampRathazul)) DisplayText("  You hear your old alchemist friend suddenly putting up a ruckus as the sounds of breaking glass echo throughout camp.  " + Flags.list[FlagEnum.HELSPAWN_NAME] + " looks up at you nervously, but you ruffle her fiery hair and tell her to try again. Rath is probably just fine.");
		//else if Valeria/Latexy is at camp: 
		else if (Flags.list[FlagEnum.VALARIA_AT_CAMP] === 1) DisplayText("  You hear a sudden yelp of pain from across camp. Valeria slithers up to you with an irritated look on her gooey face, pointing an accusing finger at an arrow sticking out of her tit.  \"<i>Dammit, [name], I'm a googirl, not a pin cushion!</i>\"  You wave her off, and tell " + Flags.list[FlagEnum.HELSPAWN_NAME] + " to try again.");
		else if (latexGooFollower()) DisplayText("  You hear a sudden yelp of pain from across camp.  " + Flags.list[FlagEnum.GOO_NAME] + " slithers up to you with an irritated look on her gooey face, pointing an accusing finger at an arrow sticking out of her tit.  \"<i>Dammit, [name], I'm a googirl, not a pin cushion!</i>\"  You wave her off, and tell " + Flags.list[FlagEnum.HELSPAWN_NAME] + " to try again.");

		DisplayText("\n\n\"<i>I-I dunno, " + championRef() + ",</i>\" " + Flags.list[FlagEnum.HELSPAWN_NAME] + " whines, biting her lip as she traces the arrow's path with her eyes.  \"<i>Maybe mom was right. I should just stick to a sword. That's easy....</i>\"");

		DisplayText("\n\nYou sigh and tell her to try again.  She can't expect to be perfect on the first try, after all.  It takes a little convincing, but soon you've got " + Flags.list[FlagEnum.HELSPAWN_NAME] + " back into form and nocking an arrow.  You put your hands on hers, helping the young salamander take aim, drawing down on her straw-stuffed target.  Barely audible over " + Flags.list[FlagEnum.HELSPAWN_NAME] + "'s nervous breathing, you whisper, \"<i>Take it easy.  Relax.... Good.  Take aim... now breathe out.  Exhale and loose.</i>\"");

		DisplayText("\n\nAgain, " + Flags.list[FlagEnum.HELSPAWN_NAME] + " lets an arrow fly - and this one strikes true.  You watch the missile streak across camp, slamming into the training dummy's crotch with deadly force.  It sways, buckles, and falls to pieces.");

		DisplayText("\n\n\"<i>Ouch,</i>\" Hel laughs, sipping on her flask behind you.  \"<i>What'd that poor dummy ever do to you?</i>\"");

		DisplayText("\n\n\"<i>I did it!</i>\" " + Flags.list[FlagEnum.HELSPAWN_NAME] + " shouts, pumping her fist in the air. Her mother reaches over to ruffle her fiery hair, and you both shower her with congratulations.");

		DisplayText("\n\nBefore she can get cocky, though, you push the bow back into her hands.  \"<i>Lesson 2, kiddo: one shot is never enough.</i>\"");

		DisplayText("\n\nBy the time " + Flags.list[FlagEnum.HELSPAWN_NAME] + " has her shooting stance back, her mother's already set the dummy back up, ready for round two.  It's several hours long before the three of you quit: time spent drilling " + Flags.list[FlagEnum.HELSPAWN_NAME] + " on stance and aim, even going a round with her and a pair of daggers to remind her that enemies will eventually close in if she's not careful.  She learns quickly, though, and soon she hardly needs your help to plant an arrow in the target's chest.");

		DisplayText("\n\nA few hours like this every night, and ");
		if (Flags.list[FlagEnum.HELSPAWN_DADDY] === 0) DisplayText("your ");
		else DisplayText("Hel's ");
		DisplayText("daughter will be a warrior worthy of her parent");
		if (Flags.list[FlagEnum.HELSPAWN_DADDY] === 0) DisplayText("s");
		DisplayText(".");
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	private swordAndBoardmander(): void {
		...
		DisplayText("You tell Hel to take a break: you've got this handled.  You grab some kindling and string from your supplies, and a few minutes later you've got a pair of small, vaguely sword-like practice weapons pieces together.  Tossing one to the young salamander, you tell her to come get some.");

		DisplayText("\n\n" + Flags.list[FlagEnum.HELSPAWN_NAME] + " gives the blade a few practice swings before grabbing the wooden blade in both hands and charging at you.  You put your guard up, catching her first overhand blow and turning it aside, nearly toppling the girl over.  She rolls with it, spinning out of your guard and trying a strike from the side.  You parry it, catch an opening, and give her a nice wallop on the shoulder.  A real blade would have nearly cut her arm off, but " + Flags.list[FlagEnum.HELSPAWN_NAME] + " merely stumbles back, yelping in pain.");
		...
		DisplayText(" daughter braces herself, drawing a breath just a second before she launches her attack, giving herself away.  You parry three quick blows, stepping back as " + Flags.list[FlagEnum.HELSPAWN_NAME] + " leaps into another overhand strike.  You dodge, spinning out of the attack and slicing her right across her bare belly.");

		DisplayText("\n\n\"<i>Gah!</i>\" she cries, stumbling to her knees.  \"<i>I-I can't...</i>\"");

		DisplayText("\n\nYou pull " + Flags.list[FlagEnum.HELSPAWN_NAME] + " to her feet and ruffle her hair.  \"<i>C'mon kiddo, don't give up.  Here,</i>\" you say, grabbing a dinner plate and some leather straps from your kit.  In a moment, you've got a rudimentary shield cobbled together.  You fit it over the young salamander's free arm and give her a few pointers on stance.  Nervously, she brings her makeshift shield to bear against you, fidgeting under the heft of it as you make a few slow, easy to parry swings for her to get used to it.  Steadily you pick up the pace, swinging your training sword faster and harder, forcing " + Flags.list[FlagEnum.HELSPAWN_NAME] + " to parry and block, retreating against your onslaught.  You keep it up for a full minute before relenting, letting the kid seize the initiative.  She swings at you, rolling with each parry you make, cleaving into another swing, turning your ripostes aside with her shield and forcing you back.");
		DisplayText("\n\nYou wince in pain as " + Flags.list[FlagEnum.HELSPAWN_NAME] + " manages to nick you, turning one of your blows aside with her shield and striking through the opening left in your guard.  \"<i>Alright, alright,</i>\" you laugh, rubbing your shoulder.  \"<i>Good job, " + Flags.list[FlagEnum.HELSPAWN_NAME] + ". You're getting the hang of that shield!</i>\"");

		DisplayText("\n\n\"<i>Thanks, " + championRef() + "!</i>\" the little salamander grins, hugging you.");

		DisplayText("\n\n\"<i>Alright, alright,</i>\" Hel says, grabbing the dummy sword from your hand.  \"<i>Now it's time to fight a REAL warrior, " + Flags.list[FlagEnum.HELSPAWN_NAME] + ". Come get some!</i>\"");

		DisplayText("\n\nIt's several hours long before the three of you quit: time spent drilling " + Flags.list[FlagEnum.HELSPAWN_NAME] + " on stance and parrying, trading off with Helia every few rounds to keep ");
		if (Flags.list[FlagEnum.HELSPAWN_DADDY] != 0) DisplayText("her");
		else DisplayText("your");
		DisplayText(" daughter on her toes.  She learns quickly, though, and soon she hardly needs reminding to keep her shield up and stance braced against Helia's overwhelming assaults.");
	...
	public helspawnIsASlut(): void {
		...
		if (player.weaponName != "fists") DisplayText("clutch your [weapon]");
		else DisplayText("ball your fists");
		DisplayText(" as the intruder passes by, ready to defend yourself and your loved ones.  The moment they're out of reach, you roll onto your [feet], looking around for signs of a massed demon attack.  Instead, you see the outline of " + Flags.list[FlagEnum.HELSPAWN_NAME] + " in the dying embers of the fire light, leading what looks like a spider girl by the hand toward her bed.");

		DisplayText("\n\nYou creep closer, trying to get a closer look at what your ");
		if (Flags.list[FlagEnum.HELSPAWN_DADDY] === 0) DisplayText("daughter ");
		else DisplayText("ward ");
		DisplayText("is doing.  Well, it doesn't take a genius to figure that out, really, but still, you have to be sure.  You slide right up to them, just out of their lines of sight, as " + Flags.list[FlagEnum.HELSPAWN_NAME] + " turns around, letting the spider-kin unclasp her bra and grab her tits, squeezing the small mounds as " + Flags.list[FlagEnum.HELSPAWN_NAME] + " suppresses a moan, reaching back to slip a hand into the spider's crotch, grabbing her - his dick.");
	...
	private helSpawnStopFucking(): void {
		...
		DisplayText(". " + Flags.list[FlagEnum.HELSPAWN_NAME] + " gives a shrill cry of surprise and tumbles away from you, trying to cover herself as best she can; the spider trap squirms in your unrelenting grasp, one set of arms covering his rapidly-wilting erection as the other flails around, trying to get you off of him.  Not happening.");

		DisplayText("\n\n\"<i>You!</i>\" you say, pointing an accusing finger at " + Flags.list[FlagEnum.HELSPAWN_NAME] + ".  \"<i>What the hell, kid?  Who the fuck is this?</i>\"");

		DisplayText("\n\n\"<i>That's, uh.... Alex, meet ");
		if (Flags.list[FlagEnum.HELSPAWN_DADDY] === 0) DisplayText("my dad");
		else DisplayText("[name]");
		DisplayText(". " + championRef() + ", Alex.</i>\"");
		...
		DisplayText("\n\n\"<i>" + championRef() + "...</i>\" " + Flags.list[FlagEnum.HELSPAWN_NAME] + " groans as you grab the spider boy and toss him out of camp, watching with hawk-like eyes as he slinks back to his jungle.  When he's gone, you sigh and walk over to " + Flags.list[FlagEnum.HELSPAWN_NAME] + ", wiping a tear from her cheek.  Putting an arm around her shoulder, you give her a long, long talk about not being a whore.");

		DisplayText("\n\n\"<i>B-but mom...</i>\"");

		DisplayText("\n\n\"<i>That's why you </i>shouldn't<i> be bringing men home.  Or women.  Or anything else, kiddo.");
		//If No Hel Love:
		if (Flags.list[FlagEnum.HEL_LOVE] != 0) DisplayText("  You don't want to be a whore like your mother, do you?  The girl that everyone's had a ride with.");
		//if HelLove:
		if (Flags.list[FlagEnum.HEL_LOVE] === 1) DisplayText("  I'm not going to a raise a whore, " + Flags.list[FlagEnum.HELSPAWN_NAME] + ".  Your mother's made mistakes, sure, but...</i>\" you sigh.  \"<i>");
		DisplayText("  You don't have to be that person, " + Flags.list[FlagEnum.HELSPAWN_NAME] + ".  Don't just give yourself out to the first person that asks, you know?  Save it for someone you really care about.  Trust me, it'll be worth it.</i>\"");
	...
	public helspawnAllGrownUp(): void {
		...
		DisplayText("like a woman.  The time to raise her, to shape her development, is over.  You can only hope you've done a good enough job to keep her safe in this hellish world; to give her sound judgment and the abilities she'll need to protect herself in the years and adventures to come.");

		DisplayText("\n\n<b>" + Flags.list[FlagEnum.HELSPAWN_NAME] + " has been added to the Followers menu!</b>");
		Flags.list[FlagEnum.HELSPAWN_AGE] = 3;
		Flags.list[FlagEnum.HELSPAWN_GROWUP_COUNTER] = 0;
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	public helspawnsMainMenu(): void {
		...
		else DisplayText("throwing her arms around you and hugging you tightly");
	...
	private hugHelspawn(): void {
		...
		if (player.tallness <= 72) {
			DisplayText("You reach up and pull " + Flags.list[FlagEnum.HELSPAWN_NAME] + " down to your level, wrapping your arms tight around her slender shoulders.  Grinning from ear to ear, the towering girl squeezes you right back, her powerful muscles just about crushing your smaller body.  Her tail slips around your [hips], the warm leather binding you to your beloved ");
			if (Flags.list[FlagEnum.HELSPAWN_DADDY] === 0) DisplayText("daughter ");
			else DisplayText("ward ");
			DisplayText("as she nuzzles up against you.");

			DisplayText("\n\n\"<i>Love you too, " + championRef() + ",</i>\" she laughs, planting a quick kiss on your cheek before letting you go.");
		}
		//{If PC is 6' or taller:}
		else {
			DisplayText("You grab " + Flags.list[FlagEnum.HELSPAWN_NAME] + " and pull her into a tight hug.  She lets out a girlish giggle as she falls into your arms and squeezes you right back.  Her powerful arms threaten to crush your bigger body, smothering you in affection.  Her tail slips around your [leg], the warm leather binding you to your beloved ");
			if (Flags.list[FlagEnum.HELSPAWN_DADDY] === 0) DisplayText("daughter ");
			else DisplayText("ward ");
			DisplayText(" as she nuzzles into your [chest].");
	...
	private talkToHelspawn(): void {
		...
		if (Flags.list[FlagEnum.HAD_FIRST_HELSPAWN_TALK] === 0) {
			...
			//{if Sluttymander:
			if (Flags.list[FlagEnum.HELSPAWN_PERSONALITY] >= 50) DisplayText("  Even if she wants to be just like mom, she shouldn't be bringing monsters home.  Some of them may be dangerous, and she's not a warrior quite like you and Helia yet.");
			else DisplayText("  She should know better than to bring monsters back.  She's not her mother; lust doesn't have to dominate her life like that.");
		...
		//{Kiha must be at camp}
			...
			DisplayText("\"<i>Oh hey, " + championRef() + "!  You gotta come see what I was making!</i>\"");
			...
			DisplayText("\n\n\"<i>C'mon, " + championRef() + ", try it!</i>\" " + Flags.list[FlagEnum.HELSPAWN_NAME] + " adds happily.  \"<i>It's aunt Kiha's favorite recipe.</i>\"");
		...
		//Talk 3
		//{Needs Rath at camp]
		else if (temp <= 1 && player.statusAffects.has(StatusAffectType.CampRathazul)) {
			DisplayText("\"<i>" + Flags.list[FlagEnum.HELSPAWN_NAME] + "!</i>\" you hear a ragged old voice call, \"<i>Get back here this instant!  I'm not done with you yet!</i>\"");

			DisplayText("\n\n\"<i>Coming!</i>\" " + Flags.list[FlagEnum.HELSPAWN_NAME] + " calls back as Rathazul shuffles into view, waving around what looks like a tiny hammer.  Whispering, " + Flags.list[FlagEnum.HELSPAWN_NAME] + " says, \"<i>Could you come with me, " + championRef() + "? Rathazul weirds me out.</i>\"");

			DisplayText("\n\nChuckling, you nod and follow " + Flags.list[FlagEnum.HELSPAWN_NAME] + " as she returns to Rath's makeshift laboratory and seats herself on one of his tables.  Grumbling about kids, the old rat proceeds to rap his little hammer on her knee - nearly getting kicked in the face for his trouble - before asking her to hold her breath as he listens to her heartbeat.  She's nearly blue in the face before he nods approvingly and tells her she can go. " + Flags.list[FlagEnum.HELSPAWN_NAME] + " goes running, happy to get away as Rath turns to you, shaking his head.");
			...
			if ((amilyScene.amilyFollower() && !amilyScene.amilyCorrupt())) DisplayText("Little Amily's solution was makeshift at best, and the children are beyond my reach to study.  Still.  ");
			DisplayText("Imagine whole legions of children reared and raised in the time it takes a goblin to do the same. We might have a fighting chance for once, with numbers to match the demons.</i>\"");
		...
		else if (temp <= 2 && milkSlave() && Flags.list[FlagEnum.MILK_SIZE] > 0) {
			DisplayText("\"<i>Hey, have you seen " + Flags.list[FlagEnum.MILK_NAME] + " around anywhere, " + championRef() + "?</i>\" " + Flags.list[FlagEnum.HELSPAWN_NAME] + " asks, looking around the camp.  \"<i>Mom asked me to milk her, but I haven't seen her around.  She wouldn't have run off, would she?</i>\"");
			if (Flags.list[FlagEnum.MILK_SIZE] === 1) DisplayText("\n\nYou tell her it'd be more of a wobble, but");
		...
		//{Isabella must be at camp}
			...
			if (player.race() != "human") DisplayText(" and you yourself have suffered as well");
			DisplayText(".  You smile as Isabella seats herself beside " + Flags.list[FlagEnum.HELSPAWN_NAME] + ", joining their voices together for the final chorus that leaves them both shaking as " + Flags.list[FlagEnum.HELSPAWN_NAME] + " strums the last, desperate notes, nearly clawing through the strings in her passion.");

			DisplayText("\"<i>");
			if (!isabellaAccent()) DisplayText("Sehr gut");
			else DisplayText("Very good");
			DisplayText(" little " + Flags.list[FlagEnum.HELSPAWN_NAME] + ",</i>\" Isabella says, pulling the young 'mander into a great big hug that threatens to smother her betwixt the cowgirl's massive bosoms.  Grinning, you congratulate " + Flags.list[FlagEnum.HELSPAWN_NAME] + " on her performance, showering your girl in praise before you depart, leaving her to Isabella's instruction.  As you walk away, you can't help but notice Helia standing a short ways off, rubbing her eyes.");

			DisplayText("\n\nWhen you approach, your lover smiles at you, saying, \"<i>I always wanted to be a bard when I was a little girl.  I'm... I'm glad " + Flags.list[FlagEnum.HELSPAWN_NAME] + "'s getting the chance, at least.</i>\"");
			return { next: Scenes.camp.returnToCampUseOneHour };
		}
		else {
			DisplayText("Unfortunately, there doesn't seem to be anything in particular to talk about at the moment.  The two of you spend the time in companionable quiet, making smalltalk.");
			return { next: Scenes.camp.returnToCampUseOneHour };
		}
	}
	...
	private dontFuckAlex(): void {
		...
		if (Flags.list[FlagEnum.HELSPAWN_PERSONALITY] >= 50) {
			DisplayText("\n\n" + Flags.list[FlagEnum.HELSPAWN_NAME] + " scoffs, shaking her head.  \"<i>But I </i>like<i> mom's way.  I love jilling off, the feeling of my cunt getting stretched and my ass being torn apart by the big toys I buy in town.  That's who I am, " + championRef() + ".  I'm like mom, and I don't think... no, I know I don't want to change.  I love sex, even if you stopped me and Alex from it.  I want to feel a gang of minotaurs raping me, I want to jump on a drider's cock and ride him down.  I want to do the things I heard mom doing, and that's just what I'm going to do. I'm sorry.</i>\"");

			DisplayText("\n\nYou try to say something, but " + Flags.list[FlagEnum.HELSPAWN_NAME] + " steps away, calling back, \"<i>I love you, " + championRef() + ", but I can't be what you want me to be.  I'm going to be a slut like mom, and there's nothing you can do to stop me!  It's who I was born and raised to be.</i>\"");

			DisplayText("\n\nYou may have made a horrible mistake.");
		}
		//{If Chastemander:}
		else {
			DisplayText("\n\n" + Flags.list[FlagEnum.HELSPAWN_NAME] + " nods slowly, taking in your words.  \"<i>I... I know, " + championRef() + ".  I'm sorry I brought Alex home last night.  It won't happen again.  I don't... I love mother, but I don't want to be like her.  A whore.  She's a great woman, but the things she does... they disgust me.</i>\"");
	...
	private incestWithHelspawn(): void {
		...
		Flags.list[FlagEnum.HELSPAWN_INCEST] = 1;
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	private ohGodEwKihaAndHelspawnSuckAtCooking(): void {
		...
		DisplayText("\n\n\"<i>K-Kiha?</i>\" " + Flags.list[FlagEnum.HELSPAWN_NAME] + " yelps, watching the dragoness collapse into a pile of limp scales and leather.  \"<i>Are you okay?  Is she okay, " + championRef() + "?</i>\"");
		...
		DisplayText("\n\nYou and " + Flags.list[FlagEnum.HELSPAWN_NAME] + " are soon working away, stirring up your own special blend of your rations and the spices she and Kiha gathered for the first attempt.  Once you've got the basics in, you step back and hand the ladle over, telling the young 'mander to go wild.  Grinning like a kid, she goes to work, mixing in a wild combination of ingredients.  The cauldron's full to bursting by the time she's done, and no sooner does she ladle out the first bowel of it than Kiha sits up, groaning.");

		DisplayText("\n\n\"<i>The fuck happened?</i>\" the dragoness hisses, rubbing her temple.  \"<i>Feel like I got hit with a hammer.</i>\"");

		DisplayText("\n\n\"<i>You, uh, fell off the edge, aunt Kiha.  And spilled all the stew.  Here, we remade it for you!</i>\" " + Flags.list[FlagEnum.HELSPAWN_NAME] + " lies, presenting \"aunt Kiha\" with a bowl full of an aromatic brown goop, full of carrots and spices floating atop it like boats on a placid lake.  At least it smells better.");
	...
	private umYum(): void {
		...
		DisplayText("\n\nCringing, you set the offered bowl back down, squeaking out that it's delicious, thank you very much, but you have something to take care of right now; maybe you can have more later.  Kiha rolls her eyes and shoos you off before she and " + Flags.list[FlagEnum.HELSPAWN_NAME] + " spoon out their lunch.");
	...
	private helSpawnMilkHerNow(): void {
		...
		DisplayText("\n\n" + Flags.list[FlagEnum.HELSPAWN_NAME] + " gives a rebellious huff as she clambers down, slipping down to the bottom and giving " + Flags.list[FlagEnum.MILK_NAME] + " a gentle nudge.  \"<i>C'mon, cutey, wake up.  Time to relieve some pressure.</i>\"");

		DisplayText("\n\nYawning powerfully, " + Flags.list[FlagEnum.MILK_NAME] + " rises to her knees and rubs the sleep from her eyes");
		if (Flags.list[FlagEnum.MILK_SIZE] === 2) DisplayText(", already working to unfasten her shift");
		DisplayText(".  \"<i>Suckle?</i>\" she asks, turning her full teats toward " + Flags.list[FlagEnum.HELSPAWN_NAME] + ".");

		//If Sluttymander:
		if (Flags.list[FlagEnum.HELSPAWN_PERSONALITY] >= 50) DisplayText("\n\n\"<i>Wouldn't have it any other way!</i>\" the salamander answers with an eager grin, easing herself into " + Flags.list[FlagEnum.MILK_NAME] + "'s arms and wrapping her lips around one of her prominent teats. " + Flags.list[FlagEnum.MILK_NAME] + " gives a pleasured little shudder as " + Flags.list[FlagEnum.HELSPAWN_NAME] + " starts to suckle like a babe, drinking down the dusky maid's seemingly endless supply of milk.");

		//If Chastemander:
		else DisplayText("\n\n\"<i>Sorry, " + Flags.list[FlagEnum.MILK_NAME] + ", you know I'm not really into that.  Just turn around and sit back, alright?</i>\"  With a sigh, " + Flags.list[FlagEnum.MILK_NAME] + " does as she's asked, leaning back into " + Flags.list[FlagEnum.HELSPAWN_NAME] + "'s arms as the young salamander goes to work, clawed fingers gently caressing her prominent nubs until a white flow springs fort, splashing her knees as the dusky maid gives a little gasp of pleasure.");

		DisplayText("\n\n<i>They seem to get on all right</i>, you think as you watch the pair of them.  It's nice to have someone else around to help keep " + Flags.list[FlagEnum.MILK_NAME] + "'s production under control.  Poor thing just never stops lactating.");
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	internal function loseSparringToDaughter(): void {
		...
		if (Flags.list[FlagEnum.HELSPAWN_PERSONALITY] >= 50) {
			...
			if (player.stats.lust > 99) DisplayText("succumbing to your own lusts");
			else DisplayText("unable to withstand her unending hail of attacks");
			DisplayText(", " + Flags.list[FlagEnum.HELSPAWN_NAME] + " quickly sweeps your [legs] out from under you, dropping you right on your ass.  You collapse with a grunt, ");
			if (player.weaponName != "fists") DisplayText("weapon tumbling out of hand");
			else DisplayText("unable to even clench your fists anymore");
			DisplayText(", and a moment later " + Flags.list[FlagEnum.HELSPAWN_NAME] + "'s on you, straddling you with her powerful legs, pushing you down into the dirt.");
		...
		//Else If Chastemander:
			...
			DisplayText(", " + Flags.list[FlagEnum.HELSPAWN_NAME] + " grabs your arm, catching you before you can fall.  \"<i>Easy there, " + championRef() + ", I got you,</i>\" she says, pulling you into a quick hug.");
			...
			if (player.stats.HP < 1) DisplayText("recover from that ass kicking");
	...
	internal function beatUpYourDaughter(): void {
		...
		if (Flags.list[FlagEnum.HELSPAWN_PERSONALITY] >= 50 && monster.lust > 99) {
			DisplayText("\"<i>N-no more...</i>\" the slutty little salamander moans, slumping down to the ground, arms wrapping around herself.  \"<i>Fuck, you're sexy... so horny...</i>\" she groans, hands slipping down to her soaked bikini bottom.");
			...
			//If No Incest: 
			if (Flags.list[FlagEnum.HELSPAWN_INCEST] === 0) DisplayText("  You wash your hands of the defeated slut and head back to camp, leaving her to work through her tension herself.");
			else {
				DisplayText("  You reach down and give your lovely, lusty daughter a pat on her expansive rear, telling her she'll always look her best with her ass in the air, begging for ");
				if (player.torso.cocks.count <= 0) DisplayText("sex");
				else DisplayText("your cock");
				DisplayText("... and that if she's lucky, you might tend to her when you've cooled off.");
			}
		}
		else {
			DisplayText(Flags.list[FlagEnum.HELSPAWN_NAME] + " stumbles back, dropping her weapon and waving her arms, \"<i>I give, I give.</i>\"");
			...
			if (monster.stats.HP < 1) DisplayText(", rubbing her many bruises");
			else DisplayText(", blushing brightly after your slutty display");
	...
	public maiVisitsHerKids(): void {
		...
		DisplayText("\n\n\"<i>Hi, [name],</i>\" Mai says with a demure smile, arms wrapped around " + Flags.list[FlagEnum.HELSPAWN_NAME] + "'s belly, trying to hold the daughter that's near a foot taller than she is.  Finally, laughing, she lets the young salamander go.  " + Flags.list[FlagEnum.HELSPAWN_NAME] + " drops down to sit beside her mother and father, and quickly has both a smooth and scaly hand stroking her hair.");
		//If Chastemander:
		if (Flags.list[FlagEnum.HELSPAWN_PERSONALITY] < 50) DisplayText("\n\n" + Flags.list[FlagEnum.HELSPAWN_NAME] + " happily leans her head against Mai's leg, accepting the affection with her tail swishing gaily behind her, foxy ears twitching as Mai brushes them.");
		else DisplayText("\n\n" + Flags.list[FlagEnum.HELSPAWN_NAME] + " leans back, pulling out a flask and taking a deep swill before passing it to her father, who takes it with a knowing smile and knocks it back like a champ.");
		...
		DisplayText("\n\n\"<i>Heeeeeyyyyyy,</i>\" " + Flags.list[FlagEnum.HELSPAWN_NAME] + " whines, crossing her arms with a huff.");
		...
		DisplayText("\n\n" + Flags.list[FlagEnum.HELSPAWN_NAME] + " lights up as Mai slips the gold chain around her daughter's neck.  \"<i>Something to remember me by when I'm not around,</i>\" she says with a wink, letting her kid skip off to find a mirror.");

		DisplayText("\n\n\"<i>And you, [name],</i>\" she says, turning to you, \"<i>Thanks for taking care of " + Flags.list[FlagEnum.HELSPAWN_NAME] + ".  I-I mean, I know I was really making her for you and Hel anyway, but still... she's a good kid, and I love her anyway.  Keep her safe, alright?</i>\"");

		DisplayText("\n\nYou promise that you will, and with a quick nod, Mai runs after " + Flags.list[FlagEnum.HELSPAWN_NAME] + " to say goodbye for the day.  As she leaves, Helia grins, holding you tight against her.  \"<i>She's a good girl, [name].  Couldn't have picked a better father for our girl.</i>\"");
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	public spiderBrosGift(): void {
		...
		DisplayText("As you're getting ready to head out for the day, you notice " + Flags.list[FlagEnum.HELSPAWN_NAME] + " prancing around camp with a long, brightly-colored scarf wrapped around her neck, standing painfully in contrast with her midnight-black scales and pale flesh.");
		...
		//if Isabella: 
		if (isabellaFollower()) DisplayText(", or aunt Isabella might have knitted me a new one, I don't know");
		DisplayText(".</i>\"");
	...
	public hakonAndKiriComeVisit(): void {
		...
		DisplayText("As you're returning to camp, you notice Helia running around like a chicken with her head cut off, fussing about damn near everything in her part of the camp");
		if (camp.companionsCount() > 1) DisplayText(" and everyone else's too, much to their chagrin");
		DisplayText(". As she's furtively polishing off her giant still, trying to get the dingy old thing to shine, you approach and clear your throat. She gives a sharp yelp and spins around, but seems to relax as she sees it's just you.");
		...
		DisplayText("\n\nIt hardly matters, as a few minutes later, you hear a cry of unabashed glee from the edge of camp, and turn to see " + Flags.list[FlagEnum.HELSPAWN_NAME] + " running up to meet a pair of oncoming figures.  You wave as Hel's father swoops his granddaughter up into a great bear hug, hefting the half-breed 'mander off her feet in his powerful arms.  " + Flags.list[FlagEnum.HELSPAWN_NAME] + " squeals with delight as she's practically carried into camp, until Hakon sets her down and hugs her mother, pulling Hel into a tight hug before she can finish saying hello.");
		...
		DisplayText("\n\n\"<i>And you should see how strong she is!</i>\" Hel grins.  \"<i>Go on, " + Flags.list[FlagEnum.HELSPAWN_NAME] + ", show the old scales what you've got.</i>\"");

		DisplayText("\n\n\"<i>Old scales!?  I'll show you old, you little brat.  Come on, hit me!</i>\" Hakon says, just before " + Flags.list[FlagEnum.HELSPAWN_NAME] + " punches him right in the chest, knocking him right on his ass.  He tumbles back with a grunt, shaking his head with a wry laugh.  \"<i>Ha!  Maybe these scales are getting old after all.  This one's gonna be a hell of a fighter, though.  Should take her hunting with us one of these days.  What do you say, kid?</i>\"");

		DisplayText("\n\n\"<i>Sure!  ");
		if (Flags.list[FlagEnum.HELSPAWN_WEAPON] === "scimitar and shield") DisplayText(championRef() + "'s been teaching me how to fight.");
		else DisplayText("Mom's been teaching me how to fight.");
		DisplayText("  I'm not as good as mom and " + championRef() + " yet, but I'd love to come along.</i>\"");

		DisplayText("\n\n\"<i>That's the spirit.  Now we just have to convince your aunt Kiri to come along.  And ");
		if (Flags.list[FlagEnum.HELSPAWN_DADDY] === 0) DisplayText("your old man");
		else DisplayText("[name]");
		DisplayText(", too.</i>\"");
