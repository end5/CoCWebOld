...
export class UrtaPregs extends NPCAwareContent {
	...
	*/
	...
	private urtaSexMenu() {
		let spank;
		if ((player.torso.cocks.count > 0 && player.cockThatFits(urta.urtaCapacity()) >= 0) || player.hasKeyItem("Deluxe Dildo") >= 0) spank = urta.spankTheShitOutOfUrtaAndMakeHerCreamHerselfFromProstateStimulationAloneLikeTheHornyDrunkenSlutSheReallyIs;
		//NOT Drunk
		if (!urta.urtaDrunk()) {
			//Lover
	...
	internal function urtaPregAppearance() {
		switch (pregnancy.event) {
			case 1:
			case 2: DisplayText("Urta is sitting at her usual table, looking more clear-eyed and sober than usual.  She looks extremely happy, tail wagging gently behind her, and she gives you a thrilled look when she sees you, smiling blissfully and patting her belly in a meaningful gesture.");
				break;
			case 3:
			case 4:
			case 5:
			case 6:
			case 7: DisplayText("Urta has moved from her usual table to a more private stall, apparently for the comfort of its padded seats.  Her bulging belly makes it obvious she's pregnant - and the wide grin that seems fixed to her face makes it obvious she's ecstatic about it.");
				break;
			case 8:
			case 9: DisplayText("Urta's pregnancy has caused her to outgrow the stall; she's moved herself to an entirely new position in a quiet, out of the way part of the bar.  Looking at the sheer size of her, she's doubtlessly going to pop any day now.");
			default:
		}
	}

	//In Urta Menu
	internal function urtaPreggoApproached() {
		DisplayText().clear();
		
		switch (pregnancy.event) {
			case 1: DisplayText("As you approach and take your usual seat, Urta chugs down a glass of something orange-colored and fizzy, which definitely doesn't smell like her usual taste in drinks.  She sees you looking at it and smiles, shrugging casually.  \"<i>Well, I can't drink alcohol while pregnant, can I, [name]?</i>\"  She then steals a kiss from you; she tastes sweet and a little tart - must be some sort of fruit-juice...");
				//Horny:
				if (Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] === 0) {
					DisplayText("  Her cock audibly clonks against the table's underside, but she just grins at the sound, fully confident in who she is now.  \"<i>Wanna come back to my place, lover" + Desc.Gender.mf(player, "-boy", "") + "?  They say sex is good for the baby - and really good for the mommy...</i>\"  She croons, teasingly rubbing your [legs] with her foot.");
					urtaSexMenu();
				}
				else {
					DisplayText("  The lack of cock-on-wood sounds or motion on Urta's part makes it clear she's not feeling amorous right this moment.  Breaking the kiss, she licks her glossy lips clean and looks at you with a good-natured smirk.  \"<i>Feeling up to talking for a little while?</i>\"");
	...
	//[=Refuse=]
	private IAintGotTimeForUrtaBirfs() {
		DisplayText().clear();
		DisplayText("The centaur looks perplexed at your refusal.  \"<i>I - what?  Really?</i>\"");
		...
		Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] = randInt(3) + 1;
		...
		if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] === 3 || Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] === 1) {
			if (randInt(2) === 0) Flags.list[FlagEnum.URTA_LATESTBORN_COCKTYPE] = 2;
	...
	public getUrtaLetter() {
		...
		if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] === 1) DisplayText("son");
		else if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] === 2) DisplayText("daughter");
		else DisplayText("hermaphrodite daughter");
		DisplayText(", though.\n");
	...
	//[=Go=]
	private goVisitUrtaBirfs(withUrta: boolean = false) {
		DisplayText().clear();
		pregnancy.knockUpForce(); //Clear Pregnancy
		Flags.list[FlagEnum.URTA_PREGNANT_DELIVERY_SCENE] = 0;
		Flags.list[FlagEnum.NEW_BABY_ANNOUNCED] = 0;
		if (!withUrta) DisplayText("The trip to the clinic where Urta is giving birth to your child is a blur; all that matters is getting there as fast as possible.  The centaur abandons you at the hospital doors, and you make your own way inside, approaching the counter inside.");

		//[1st time:
		if (Flags.list[FlagEnum.URTA_TIMES_BIRTHED] === 0) {
			DisplayText("\n\nA perky-looking young female mouse-morph, her nurse's outfit stretched a little tighter over her breasts than is considered professional, a small name tag with \"Splinter\" clipped onto one hem, looks up at you with a welcoming grin.  \"<i>Hello, " + Desc.Gender.mf(player, "mister", "miss") + "; please state the nature of the problem.</i>\"  You explain your reason and her eyes widen with understanding.  \"<i>Ah, I see.  Please, follow me, [name]; I will take you to Urta's room.</i>\"  She promptly gets up out of her seat - allowing you to see a rather unladylike bulge at her crotch, though you have no idea if she's a herm or merely an effeminate male - and starts walking, with you following close behind.");
		}
		//Else:
		else {
		...
		if (gender === 2) DisplayText("girl!");
		else if (gender === 1) DisplayText("boy!");
		else DisplayText("boy...?  Well... isn't that unusual, looks like you actually gave birth to a healthy herm!");
		DisplayText("</i>\"");
		...
		if (gender === 1 || gender === 3) {
			if (randInt(2) === 0) {
				DisplayText("\n\n\"<i>Well, now... ");
				if (gender === 3) DisplayText("s");
				DisplayText("he's a perfectly normal little fox,</i>\" the doctor notes, and you can catch a quick glimpse of a somewhat bigger than normal vulpine sheathe on your ");
				if (gender === 1) DisplayText("son");
				else DisplayText("daughter");
				DisplayText("'s groin from where you are.");
			...
			//[Horse Dick:
				...
				DisplayText("\n\n\"<i>...My, my; looks like ");
				if (gender === 3) DisplayText("s");
				DisplayText("he takes after ");
				if (gender === 3) DisplayText("her");
				else DisplayText("his");
				DisplayText(" mother,</i>\" the doctor comments.  Snatching a quick glance reveals the leathery sheath of an equine phallus swaying between the newborn's legs.  Although infantile, it already promises that ");
				if (gender === 1) DisplayText("he");
				else DisplayText("she");
				DisplayText(" is going to be quite a big ");
				if (gender === 1) DisplayText("boy when he grows up.");
				else DisplayText("girl when she grows up.");
		...
		if (gender >= 2) DisplayText("her");
		else DisplayText("him");
		DisplayText(" to the breast, watching with joyous awe as the little fox-morph roots for her black nipple and then latches on.  Tears begin to roll down her cheeks at the sight.  \"<i>Our ");
		if (gender >= 2) DisplayText("daughter");
		else DisplayText("son");
		DisplayText("... look, [name], ");
		//[she's]
		if (gender >= 2) DisplayText("she's");
		else DisplayText("he's");
		DisplayText(" really here; our beautiful, beautiful little baby...</i>\" She sobs, ");
		if (urtaKids() === 0) DisplayText("overwhelmed by the fact she's a mother at long last");
		else DisplayText("still as delighted as ever to have a new addition to your growing brood");
		DisplayText(".");
		...
		DisplayText(" mother's breast.  \"<i>Looks like my work here is done for now... I'll be back later to give a quick check-up on the baby, but from what I can see you have nothing to worry about.  Excuse me then.</i>\"  The doctor gets up and leaves the room, letting you have some privacy with the fox-herm.");
	...
	//=Yes=]
	private stayAfterUrtaBirf() {
		DisplayText().clear();

		DisplayText("Of course, if that's what she wants, you'd be happy to oblige.  The vixen smiles at you thankfully.  \"<i>You won't regret it, [name]; I have an eventful day planned for us,</i>\" she finishes with a mischievous grin and a giggle of delight.");
	...
	private wakeUpWithUrtaAfterStaying() {
		DisplayText().clear();
		model.time.days++;
		model.time.hours = 6;
		statScreenRefresh();
		...
		//PC Wakes with Urta
		DisplayText("As the morning sun shines on the blinds, you open your eyes.  Then you remember the events of the last day.  You spent the whole day with Urta, not having sex, just walking together and buying stuff for your newborn ");
		if (Flags.list[FlagEnum.URTA_KIDS_MALES] === 1) DisplayText("son");
		else DisplayText("daughter");
		DisplayText(", then you went home and spent the better part of the evening making out with Urta which, of course, led to hot vixen sex in her bedroom.  After that... you probably fell asleep on her bed, and considering the current state of affairs, she didn't mind it one bit.");
		...
		DisplayText("\n\nYou hold the shaft ");
		if (player.stats.cor < 50) DisplayText("gently,");
		else DisplayText("tightly,");
		DisplayText(" earning a ");
		if (player.stats.cor < 50) DisplayText("whimpering moan");
		else DisplayText("gasping shudder");
		DisplayText(" from the vixen, then begin stroking her and pushing her to the side.  The fox grumbles and shudders, rolling over to the side.  \"<i>Mmm... [name], while that's nice, I don't really wanna have to do laundry right after getting up,</i>\"  she mumbles in a sleepy voice.");
		...
		//1st time: 
		if (Flags.list[FlagEnum.URTA_TIMES_BIRTHED] === 1) DisplayText("You look at her questioningly; she's breast-feeding, so she shouldn't be drinking alcohol.  \"<i>Oh, don't worry; even I don't drink booze in the morning.  It's a little pick-me-up of my own invention; totally non-alcoholic.  It's mostly fruit juice with a few spices thrown in.</i>\"  Urta laughs.  ");
		DisplayText("  Your only reply is to reach behind her balls and gently insert a wandering finger into her pussy.");
		...
		if (urtaKids() === 1) DisplayText("a child");
		...
		if (urtaKids() === 1) DisplayText("new ");
		else DisplayText("newest ");
		DisplayText("little monster,</i>\" she notes, already walking off to the nursery, indifferent to any need for clothes.  After all, it's not like the baby will mind that she's not wearing pants while feeding ");
		if (urtaKids() === 1) DisplayText("him");
		else DisplayText("her");
		DisplayText(".");
		if (urtaKids() === 2) DisplayText("  The other kid is staying at Edryn's, so there's no risk of you two needing to answer... complicated questions.");
		if (urtaKids() > 2) DisplayText("  The other kids are staying at Edryn's, so there's no risk of you two needing to answer... complicated questions.");
	...
	//[=Leave=]
	private noNeedForTitMilkTodayUrta() {
		DisplayText().clear();
	...
	//[=Drink=]
	private drinkSomeUrtaPostBirthTitMilk() {
		DisplayText().clear();
		...
		if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] === 1) DisplayText("his");
		else DisplayText("her");
		...
		DisplayText(" together... but that's a poor substitute if you don't ever come back.  Please, be careful with yourself, [name],</i>\" she tells you solemnly.  You kiss her goodbye, giving her shaft a teasing grope then step out of the room.");
	...
	public urtaPregooUpdates(): boolean {
		...
		//4: 
			...
			if (pregnancy.type != PregnancyType.PLAYER) DisplayText("sounding somewhat jealous that it's you carrying and not her.");
			else DisplayText("sounding a little smug as she caresses her own gravid tummy.");
			DisplayText("  You shake your head, but considering how far you are into the pregnancy, it shouldn't take long... if Urta keeps visiting, you're pretty sure it'll be kicking the next time she comes around.");
		...
		//5
			...
			DisplayText("\n\nUrta squeals in glee and immediately glomps onto your bloated belly, rubbing her hands and her cheek excitedly across its " + Desc.Skin.skinFurScales(player) + " surface in hopes of feeling her child moving around inside you.  You groan at Urta's tight hug, she throws you slightly off-balance, but you manage to remain steady.  From inside you, you feel the baby shifting in your increasingly tight womb, and Urta looks up at you in shock, glee dancing in her eyes.  \"<i>I felt it!  It's moving in there!");
			if (urtaKids() === 0) DisplayText(" Oh, [name], we really are going to be parents...");
			DisplayText("</i>\" She babbles happily, tail wagging so enthusiastically you wonder if it's going to fall off.");
	...
	//[=No=]
	private noHelpForUrtaPreggoPC() {
		DisplayText().clear();
		DisplayText("You caress her cheek before saying a flat, \"<i>No</i>\".  Urta actually gives you puppy dogs eyes at that, then lets out a wounded-sounding huff.  \"<i>All right, [name]; I guess I better get back to work, then.  Now you take care of yourself; you've got a little one on board, now.</i>\"  She chastises you.");
		return { next: Scenes.camp.returnToCampUseOneHour };
	}

	//[=Yes=]
	private getUrtaHelpPreggoPC() {
		DisplayText().clear();
		DisplayText("Good, you hope this means she'll be having plenty of time, because you sure could use a fair share of vixen right now.  \"<i>As long as you want me, I'll be here.</i>\"  Urta laughs already starting to fiddle with the straps of her armor.  \"<i>Do you have any preferences for what we do?</i>\"");
	...
	//[=Sex=]
	private getUrtaSexWhenPreggoz() {
		DisplayText().clear();
		DisplayText("Well... you could use a break from minding your passenger, so how does she plan to help you take your mind off your current predicament?  You ask with a knowing smile.");
	...
	//[=Massage=]
	private getAnUrtaMassageWhenPreggo() {
		DisplayText().clear();
		DisplayText("You tell her that a backrub sounds fine, but you'd rather not lie down.  It could hurt the baby.  \"<i>Right, sorry, what was I thinking?  Well, I think I can be flexible...</i>\"  Urta grins, cracking her fingers. \"<i>Now, let's just find somewhere comfortable to put you first...</i>\"");
	...
	//[=Decline=]
	private declineUrtaLevelSixPreggoHelps() {
		DisplayText().clear();
		DisplayText("You thank Urta for the offer, but you'll have to decline.  You still have business to take care of, so you'll just rest for a while longer then be on your way.  Besides, it wouldn't look good for her to be absent for too long, she has a city to take care of.");
	...
	//[=Accept=]
	private acceptUrtaLevelSixPreggoHelps() {
		DisplayText().clear();
		DisplayText("You're feeling really tired right now, and besides, spending a whole day with Urta would be nice, so you gladly accept her offer.");
	...
	public PCGivesBirf() {
		...
		if (gender === 1) {
			...
			DisplayText(" sheath between his legs that looks like it's going to be bigger than normal when he grows up.");
		...
		//Herm:
			...
			DisplayText(" sheath between her legs that looks like it's going to be bigger than normal when she grows up.");
		...
		DisplayText(" out here.  I know you wouldn't let anything bad happen to ");
		if (gender >= 2) DisplayText("her");
		else DisplayText("him");
		DisplayText(", but, well, you can't really take care of ");
		if (gender >= 2) DisplayText("her");
		else DisplayText("him");
		DisplayText(" properly in this wilderness, now can you?</i>\"");

		DisplayText("\n\nYou are eventually forced to admit that it would probably be better for the baby if ");
		if (gender >= 2) DisplayText("she");
		else DisplayText("he");
		DisplayText(" was brought up in Tel'adre.  Urta smiles thankfully and gives both of you one particularly eager cuddle.  \"<i>Thank you for understanding, [name].  ");
		//[She]
		if (gender >= 2) DisplayText("She");
		else DisplayText("He");
		DisplayText(" will be brought up safely, I promise.</i>\"  With that, she carefully bundles up the sleeping child and quietly vanishes into the wilderness, leaving you to take yourself off to bed for a proper rest.\n");
	...
	public urtaIsHappyAboutPregnancyAtTheBar() {
		DisplayText().clear();
	...
	public urtaIsAPregnantCopScene() {
		DisplayText().clear();
		...
		if (kGAMECLASS.telAdre.edryn.pregnancy.isPregnant) DisplayText("  Edryn's pregnant belly isn't helping her, either.");
	...
	public urtaSpecialDeliveries() {
		DisplayText().clear();

		DisplayText("As you enter through the gates, Edryn clops up to you, a small basket swinging from the crook of one arm");
		if (kGAMECLASS.telAdre.edryn.pregnancy.isPregnant) DisplayText(" and her pregnant belly jiggling from the motions");
		DisplayText(", a faint smile on her face.  \"<i>Hey there, [name]; can you do me a favor, please?  Urta's been getting deliveries to her place, but I got an unexpected call, so I'm too busy to take her groceries to her today - you're her " + Desc.Gender.mf(player, "boyfriend", "girlfriend") + "; can you please take this for me?</i>\"  The centauress asks.");
		...
		if (kGAMECLASS.telAdre.edryn.pregnancy.isPregnant) DisplayText("as quickly as a pregnant mare can, anyway,");
		DisplayText(" leaving you to head on to Urta's house.");
		...
		if (pregnancy.event >= 7) DisplayText("heavily ");
		DisplayText("pregnant vixen is snoring softly, laying on her side under the covers, forming a sizable bump in the soft, fluffy quilt that practically envelops her, almost invisible amidst the big plump cushions that serve to keep her comfortable as she sleeps. ");
	...
	internal function talkToUrtaAboutHerHouse() {
		DisplayText().clear();
		DisplayText("You've been wondering why Urta keeps her old apartment if she has a nicer and bigger house now.  It would be much better for her to put the lease money towards something more productive than keeping that apartment...");
		if (player.stats.cor < 50) DisplayText(" despite it having some emotional value to you.");
		...
		if (urtaKids() > 1) DisplayText("or two ");
		DisplayText("pipes up asking what we're doing.</i>\"  She shakes her head with amused smirk.  \"<i>No, I'd rather keep the old place for our little love nest; we can have our fun there, then come home and be all respectable around the kids.</i>\"");
	...
	//[=Yes=]
	private yesUrtaHouseSex() {
		DisplayText().clear();
		DisplayText("You smile seductively at her and raise a brow.");
		...
		if (player.stats.lib > 50) DisplayText("  Sometimes you almost give up on waiting to get to her apartment and instead strip and do her right then and there... but it wouldn't bode well for the Watch captain to get a fine for public indecency.");
	...
	//[=No=]
	private noUrtaHouseSex() {
		DisplayText().clear();
		DisplayText("You smile at her and apologize, you're just not in the mood right now.");
	...
	private talkAboutLiana() {
		DisplayText().clear();
		DisplayText("You tell Urta that you need to talk with her about something.");
		...
		if (urtaKids() > 1) DisplayText("ren have");
		else DisplayText(" has");
		DisplayText(" a special place in her heart.</i>\"");
	...
	//Pregnant Urta Sex
	private preggoUrtaSmexOrSomething() {
		DisplayText().clear();
		DisplayText("You let Urta know that you're for helping her vent some steam, if that's what she wants.");

		DisplayText("\n\nUrta grins lecherously, precum ");
		switch (pregnancy.event) {
			case 7: DisplayText("dripping wetly onto the floor");
				break;
			case 8:
			case 9: DisplayText("slopping into the filling bucket");
				break;
			default: DisplayText("staining her dress");
		}
	...
	//Belly Massage
	private massagePregnantUrtasBelly() {
		DisplayText().clear();
		...
		if (Flags.list[FlagEnum.TIMES_MASSAGED_URTA_BELLY] === 0) DisplayText("  She looks at you and her newly cream-adorned belly contemplatively.  \"<i>I never knew my sweet tooth would bring me to this...</i>\"");
	...
	private urtaMassageClimax() {
		...
		//Low Cor:
		if (player.stats.cor < 33) DisplayText("  You walk up to her and carefully remove her condom, tying it up with a knot to prevent any of her seed from escaping, then dispose of it.");
		//Medium Cor:
		else if (player.stats.cor < 66) DisplayText("Well, whatever, she's probably too sleepy to do anything about it either way.");
		//High Cor:
		else DisplayText("You walk up to her with a smirk and remove her condom, then pour down its contents over her belly, chuckling to yourself.  Now that's a great cream topping for your lovely fox-herm.");
		DisplayText("  You walk out of her house, taking care to lock the door on your way out.");
	...
	private urtaBoobWorshipScene() {
		DisplayText().clear();
		DisplayText("You gaze at Urta's breasts, heavy with their milky load, a few droplets already escaping her black nipples.  \"<i>Oh, you like these, do you?  You like a nice, big pair of foxy-boobs, huh?</i>\"  She jokes, jiggling them for your amusement, before wincing at their sensitivity.");
		...
		if (player.torso.head.face.type === FaceType.SHARK_TEETH || player.torso.head.face.type === FaceType.SNAKE_FANGS || player.torso.head.face.type === FaceType.SPIDER_FANGS) DisplayText(", always minding your sharp teeth");
		...
		if (player.torso.cocks.count > 0) DisplayText(", tasting a bit of yourself,");
		DisplayText(" and tell her you'll be back to check up on her later.  She gives you a sleepy smile and nods absently, already practically asleep.  You quickly find your [armor] and don them, leaving Urta to rest for a spell in her apartment.");
		return { next: Scenes.camp.returnToCampUseOneHour };
	}

	//Nurse
	private nurseFromMommaUrtasBooBees() {
		DisplayText().clear();
	...
	//if Stroke:
	private titWorshipAndStroke() {
		DisplayText().clear();
		DisplayText("You smile mischievously around Urta's breast and eagerly grab her shaft with a hand, careful not to hurt her, but roughly enough for her to feel what you just did.  Urta lets out a surprised groan.  \"<i>Lover?  What are you...?</i>\"  She moans again as your fingers shift on her distended dick.");
	...
	//Else:
	private ignoreUrtaBonerWhenWorship() {
		DisplayText().clear();
		DisplayText("You have other things to worry about right now, but you're glad she seems to be enjoying your ministrations.");
		finishTitWorshipWivUrta();
	}

	private finishTitWorshipWivUrta() {
	...
	//Milk 
	private milkPregnantUrta() {
		DisplayText().clear();
		DisplayText("You tell her that you feel like playing with her toys.  Urta smiles and nods.  \"<i>Sure, though I prefer a more... personal touch... myself.  They're in that cupboard over there,</i>\" she finishes, pointing it out.");
		...
		if (player.torso.cocks.count > 0) DisplayText("Urta promptly stops stroking your dick, though whether it's because she wants to be helpful or because she's annoyed you've stopped is anyone's guess.  ");
		...
		if (player.torso.cocks.count > 0 && !player.torso.hips.legs.isTaur() && player.cockThatFits(urta.urtaCapacity())) {
	...
	//[=Go=]
	private goHomeHorsecock() {
		DisplayText().clear();
		DisplayText("You nod at Urta and gather your clothes, redressing in an instant.  Before going you ask if Urta needs anything.  She smiles and shakes her head, a soft smile on her face.  You bid her a good rest and leave her apartment.");
		return { next: Scenes.camp.returnToCampUseOneHour };
	}

	//[=Stay=]
	private stayForHorseDickings() {
		DisplayText().clear();
		DisplayText("You yawn yourself, and suggest that maybe Urta would like some company.  You're kinda sleepy yourself.  She smirks and reclines on the bed, \"<i>Help yourself, lover - there's plenty of me to cuddle,</i>\" she giggles softly as she pats her swollen belly.");
	...
	private urtaPregCravings() {
		DisplayText().clear();
		DisplayText("You decide to ask Urta if she has any particular cravings right now.  The pregnant fox visibly thinks about, then surreptitiously eyes your crotch.");
		//noDick:
		if (player.torso.cocks.count <= 0) {
		...
		if (player.lactationQ() >= 200) DisplayText(", which automatically causes milk to trickle forth");
		DisplayText(".  \"<i>Nope, not it either...");
		if (player.lactationQ() >= 200) DisplayText(" sweet, though.");
		...
		if (player.torso.balls.quantity > 0) DisplayText(" with your [balls]");
		...
		if (player.torso.cocks.count > 2) DisplayText("The rest of your " + Desc.Cock.describeMultiCockShort(player) + " are as erect as your [cock biggest] dribbling pre along their lengths as they slap Urta during her frenzied bobs.  ");
		DisplayText("You tell Urta that you're getting close; if she keeps up with this you won't last much longer.");
		...
		if (player.torso.balls.quantity > 0 || player.torso.vaginas.count > 0) {
			DisplayText(", a hand playing expertly with ");
			if (player.torso.balls.quantity > 0) {
				DisplayText("your balls");
				if (player.torso.vaginas.count > 0) DisplayText(" and another with ");
			}
			if (player.torso.vaginas.count > 0) DisplayText("your pussy");
		}
		DisplayText(".  She suddenly pops free, licking her lips.  \"<i>Come on, lover.  I thought you offered to help me with my craving?  And yet you won't give me any of that salty load of yours?  So hungry... gimme!</i>\"  She pleads, looking up at you with hopeful eyes even as she continues to stroke your shaft.");
	...
	//[=Blow=]
	private getBlownByPregnantUrta() {
		DisplayText().clear();
		DisplayText("If she wants it so bad, she can have it, you think to yourself, bucking against her hand as you finally feel ");
		if (player.torso.balls.quantity > 0) DisplayText("your balls churn and ");
		DisplayText("cum travelling down your urethra, expanding your shaft with the force of their passage, and finally jetting out of your [cockHead biggest] to arch gracefully through the air and splatter against Urta's face.");
		...
		if (player.torso.cocks.count > 1) {
			DisplayText("  Even if it did, you wouldn't be able to do anything about your other cock");
			if (player.torso.cocks.count > 2) DisplayText("s as they join");
			else DisplayText(" as it joins");
			DisplayText(" in on the onslaught.");
		}
		...
		//(Low Cum Amount)
		if (player.cumQ() < 250) {
			DisplayText("  Soon, all too soon, you're spent.  Urta is left with strands of cum covering her naked body, only a few having made it inside her open maw.  You pant as you slump down, satisfied with the outcome.");
	...
	//[=Hold On=]
	private holdOnYouAintGettingYerCumYetYouNaughtyFox() {
		DisplayText().clear();
		DisplayText("You're not going to give in so easily.  If she wants it she'd best work hard for it.  You smile at her mischievously, even as you hump against her grip.");
		...
		if (player.torso.cocks.count > 1) {
			...
			if (player.torso.cocks.count > 2) DisplayText("s bulging ominously as they too prepare to deposit their loads on the willing fox-herm.");
			else DisplayText(" bulging ominously as it too prepares to deposit its load on the willing fox-herm.");
		}
		DisplayText("  Urta eagerly gulps and slurps, doing her best to swallow every drop.");

		//(Low Cum Amount)
		if (player.cumQ() < 250) {
			DisplayText("\n\nYou feed her all you can, ");
			if (player.torso.cocks.count > 1) {
				DisplayText("even splashing some cum over her face as your other cock");
				if (player.torso.cocks.count === 2) DisplayText(" convulses");
				else DisplayText("s convulse");
				DisplayText(" in orgasm, ");
			}
			DisplayText("but ultimately you reach your limit and can do nothing but slump as she continues sucking on your [cock biggest] like a straw, hungry for your male milk.");
		...
		//(Medium Cum Amount)
			...
			if (player.torso.cocks.count > 1) {
				DisplayText("Your other cock");
				if (player.torso.cocks.count === 2) DisplayText(" contributes");
				else DisplayText("s contribute");
				DisplayText(" by making a complete mess of her as ");
				if (player.torso.cocks.count > 2) DisplayText("they randomly spatter");
				else DisplayText("it randomly spatters");
				DisplayText(" her with cum, not that she seems to mind.");
			}
		...
		//(High Cum Amount)
			...
			if (player.torso.cocks.count > 1) {
				DisplayText("  Your other cock");
				if (player.torso.cocks.count > 2) DisplayText("s shoot their loads");
				else DisplayText(" shoots its load");
				DisplayText(" erratically, some of it splattering against Urta's face, but most of it winds up making a complete mess of the room.");
			}
			...
			if (player.hasKnot(player.torso.cocks.sort(Cock.LargestCockArea)[0])) DisplayText(", just above your knot,");
			DisplayText(" with both hands, trying to pinch it shut long enough to inhale a couple of deep breaths, then goes back to trying to drink every last drop; she must really have a hankering for what you're packing.");
		...
		DisplayText(" drained of your cum.");
		...
		if (player.cumQ() >= 500) DisplayText(", audibly gurgling");
		DisplayText(" belly, clearly feeling tired even though she wasn't the one being pleasured this time.");
	...
	private weirdUrtaCravingsConclusion() {
		DisplayText().clear();
		...
		if (player.cumQ() < 1000) DisplayText("completely clean.");
		else DisplayText("lacking the extra bit of paunch you gave her earlier.");
	...
	private urtaRaepsJoo() {
		...
		if (player.torso.cocks.count > 0) DisplayText("Every time her swollen tip rubs against your prostate you feel droplets of pre escape your dilating cum-hole.  ");
		if (player.torso.vaginas.count > 0) DisplayText("Each slap of her balls against your [vagina] sends electric waves of pleasure that expand throughout your body.  ");
		...
		if (player.torso.cocks.count > 0) {
			if (player.torso.cocks.count > 1) DisplayText("  [EachCock] tenses in unison");
			else DisplayText("  [EachCock] throbs, tensing in preparation");
			//(Low Cum Amount)
			if (player.cumQ() < 250) DisplayText(" and you unload on the floor, spilling rope after rope of cum, forming a small puddle where you stand.  An undeniable testament to your enjoyment of the kinkiness of the act, however small.");
			//(Medium Cum Amount)
			else if (player.cumQ() < 1000) DisplayText(" and you deliver your payload right there on the floor.  Jet after jet splash against it, and you have no choice but to continue moaning and spilling your cum as Urta remains firmly lodged in your depths, no longer able to move now that you're squeezing her shaft so deliciously tight.  By the time you're done, the puddle you've formed is big enough that you can see your own face on the reflection, enraptured in pleasure and enjoyment of Urta's equine meatstick.");
			//(High Cum Amount)
			else {
				DisplayText(" and you let your prodigious load escape the confines of your ");
				if (player.torso.balls.quantity > 0) DisplayText("churning balls");
				else DisplayText("being");
		...
		if (player.torso.vaginas.count > 0) {
			...
			DisplayText(" juices that paint Urta's balls, which are resting flush against your [vagina].  The spilled juice slides down Urta's balls");
			if (player.torso.vaginas.get(0).wetness >= 4) DisplayText(" to join your previous discharge on");
			else DisplayText(" and down towards");
			DisplayText(" the floor, where it gathers in a ");
			DisplayText("puddle of its own.");
		}
		//Genderless:
		if (player.gender === Gender.NONE) {
			...
			DisplayText(" anal muscles milk Urta's shaft much like a pussy would.");
			if (player.torso.butt.wetness > 0) DisplayText(" It even secretes its own wetness to slide lovingly down the fox-herm's balls and drip on the floor, an undeniable proof of your enjoyment.");
		}
		...
		let puddle: number = 0;
		if (player.torso.vaginas.count > 0) {
			if (player.torso.vaginas.get(0).wetness >= 4) puddle += 2;
			else puddle++;
		}
		if (player.torso.cocks.count > 0) {
		...
		if (puddle > 0) {
			if (puddle === 1) DisplayText("small");
			else if (puddle === 2) DisplayText("reasonable");
			else DisplayText("enormous");
			DisplayText(" puddle of cum you created");
		}
		else DisplayText("floor");
		DisplayText(" and close your eyes, letting sleep take you over even as the fox behind you drools on your back and continues to pound your ass raw.");
	...
	private partTwoOfUrtaBoning() {
		...
		if (player.torso.cocks.count > 1) {
			...
			if (!player.torso.hips.legs.isTaur()) DisplayText("  So close that you can almost feel its heat against your [cockHead].");
			DisplayText("  \"<i>Oh, don't feel embarrassed; I know what it's like to wake up with morning wood,</i>\" she jests, clearly unphased by your erect state.");
		}
		if (player.torso.vaginas.count > 0) {
	...
	private acceptUrtaRimJobbies() {
		...
		if (player.torso.cocks.count > 1) DisplayText("  Especially not if your " + Desc.Cock.describeMultiCockShort(player) + " have anything to say about it, throbbing at the mere feeling of Urta's wet tongue.");
		else if (player.torso.cocks.count === 1) DisplayText("  Especially not if your [cock] has anything to say about it, throbbing at the mere feeling of Urta's wet tongue.");
		else if (player.torso.vaginas.count > 0) DisplayText("  Especially not if your [pussy] has any say in the matter, winking at Urta, hoping for a licking of its own.");
		DisplayText("  She slowly begins deepening the pseudo-kiss, pressing more and more of her wet tongue against you until she is taking slow, languid licks up and down the lengths of the chasm, slathering your burning skin with her cooling goo.");
	...
	internal function visitKidsFirstTime() {
		DisplayText().clear();
		DisplayText("You ask Urta if she can take you to see your child");
		if (urtaKids() > 1) DisplayText("ren");
		DisplayText(".");

		DisplayText("\n\n\"<i>You want to see ");
		if (urtaKids() === 1) DisplayText("our child");
		else DisplayText("our babies");
		DisplayText("?</i>\"  Urta asks, tail wagging excitedly at the thought.  \"<i>Why, of course, [name] - " + urtaKidsText("he'll", "she'll", "they'll") + " be so happy to see " + urtaKidsText("his", "her", "their") + " " + Desc.Gender.mf(player, "father", "other-mother") + "!  Come on, let's get going!</i>\"  She insists, immediately getting up from her seat");
		if (Flags.list[FlagEnum.URTA_TIME_SINCE_LAST_CAME] === 0) DisplayText(", erection already diminishing as she forgets about her horniness in her parental pride");
		DisplayText(".  You waste no time in following her; indeed, she seems liable to leave you behind in her excitement to go and see her offspring.");
		...
		if (urtaKids() === 1) DisplayText("child");
		else DisplayText("children");
		DisplayText(".  One door leads out to the back courtyard, while another looks like it closes off stairs leading down to a basement.");
	...
	private pickKidSceneHere() {
		...
		else if (randInt(2) === 0) takeTheKidsForWalk();
		//Bathtime
	...
	private newBabyComing() {
		DisplayText().clear();
		DisplayText("You announce to your ");
		if (urtaKids() === 1) DisplayText("child");
		else DisplayText(numToCardinalText(urtaKids()) + " kids");
		DisplayText(" that you and Urta have something important to tell them.");

		//If UrtaKids <8:
		if (urtaKids() < 8) {
			DisplayText("\n\nAs ");
			if (urtaKids() === 1) {
				if (Flags.list[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) DisplayText("she");
				else DisplayText("he");
				DisplayText(" approaches");
			}
			else DisplayText("they gather");
			DisplayText(" and curiously look");
			if (urtaKids() === 1) DisplayText("s");
			DisplayText(" at you, Urta gives ");
			if (urtaKids() > 1) DisplayText("them");
			else if (Flags.list[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) DisplayText("her");
			else DisplayText("him");
			DisplayText(" a loving smile.");
			...
			//One person preg:
				...
				if (pregnancy.type === PregnancyType.PLAYER) DisplayText("her");
				else DisplayText("your");
				DisplayText(" belly.");
			}
			DisplayText("\n\nYour child");
			if (urtaKids() > 1) DisplayText("ren are");
			else DisplayText(" is");
			DisplayText(" silent for a moment, digesting this information.  Then ");
			if (urtaKids() === 1) DisplayText("a small fluffy bundle is");
			else DisplayText("several small fluffy bundles are");
			DisplayText(" doing ");
			if (urtaKids() === 1) DisplayText("its");
			else DisplayText("their");
			DisplayText(" best to hug you both; ");
			if (urtaKids() > 1) DisplayText("they");
			else if (Flags.list[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) DisplayText("she");
			else DisplayText("he");
			if (urtaKids() === 1) DisplayText(" is");
			else DisplayText(" are");
			DisplayText(" clearly delighted at the prospect.  You and Urta smile at each other over ");
			if (urtaKids() > 1) DisplayText("their");
			else if (Flags.list[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) DisplayText("her");
			else DisplayText("his");
			DisplayText(" head");
			if (urtaKids() > 1) DisplayText("s");
			DisplayText(", then return the embrace, pleased that ");
			if (urtaKids() > 1) DisplayText("they");
			else if (Flags.list[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) DisplayText("she");
			else DisplayText("he");
			if (urtaKids() === 1) DisplayText(" is");
			else DisplayText(" are");
			DisplayText(" excited at the prospect.");
		}
		//If UrtaKids =>8:
		else {
	...
	private hugAttackScene() {
		DisplayText().clear();
		DisplayText("Before you can exchange any further pleasantries the sound of running padded feet fills the house.  Lianna simply looks at the approaching foxling");
		if (urtaKids() > 1) DisplayText("s");
		DisplayText(" from the safety of the kitchen, leaving you to handle the incoming charge.  It takes a bit of effort to keep from losing your balance as your brood tackles you, hugging you in affection.  \"<i>All right, all right, let your " + Desc.Gender.mf(player, "father", "other-mother") + " go, or do you not want to play?</i>\" Urta says, smirking with amusement as she closes the front door.  With a groan and a bit of prying the little fox");
		if (urtaKids() > 1) DisplayText("es do");
		else DisplayText(" does");
		DisplayText(" eventually clamor" + urtaKidsText("s", "s", "") + " off of you, but not before taking your hands in " + urtaKidsText("his", "her", "their") + " own.  ");
		if (urtaKids() > 1) DisplayText("They");
		else if (Flags.list[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) DisplayText("She");
		else DisplayText("He");
		DisplayText(" insistently pull");
		if (urtaKids() === 1) DisplayText("s");
		DisplayText(" you towards the backyard, while your lover chuckles at your predicament.");
	...
	private stayToPlay() {
		DisplayText().clear();
		DisplayText("You nod your head in acceptance.  Urta smiles in delight.  \"<i>Take a seat, please, make yourself comfortable - I'll go and get it,</i>\" she says happily, tail wagging as she saunters off to the kitchen.");
	...
	private bathtime() {
		DisplayText().clear();
		DisplayText("You tell Lianna you just wanted to check in on your ");
		if (urtaKids() > 1) DisplayText("kids");
		else if (Flags.list[FlagEnum.URTA_FIRSTBORN_GENDER] >= 2) DisplayText("daughter");
		else DisplayText("son");
		DisplayText(".  \"<i>Sure, but they need a bath, maybe you'd like to help me bathe them?</i>\"");
		//[Yes] [No]
	...
	//[=No=]
	private noBathTiemPlease() {
		DisplayText().clear();
		DisplayText("\"<i>I see, in that case excuse me,</i>\"  She says with a blank expression.  Looks like you don't have any business here for now, so you decide to leave.");
		//(Back to TA/Camp)
	...
	private giveTheKidsABath() {
		...
		if (urtaKids() === 1) {
			if (Flags.list[FlagEnum.URTA_FIRSTBORN_GENDER] === 1) DisplayText("your son");
			else DisplayText("your daughter");
		}
		else DisplayText("the kids");
		DisplayText(".</i>\"  You contemplate what to do, then remove your outer layer of clothes, carefully placing them on a shelf in case they get splashed.  Now you just need to wait for Lianna to return.");

		DisplayText("\n\nLianna enters the bathroom followed by your ");
		if (urtaKids() === 1) {
			if (Flags.list[FlagEnum.URTA_FIRSTBORN_GENDER] === 1) DisplayText("son");
			else DisplayText("daughter");
		}
		else if (urtaKids() < 8) DisplayText("kids");
		else DisplayText("army of kids");
		DisplayText(".  \"<i>All right, ");
		if (urtaKids() > 1) DisplayText("everyone ");
		DisplayText("strip up and hop in.</i>\"  ");
		if (urtaKids() === 1) {
			if (Flags.list[FlagEnum.URTA_FIRSTBORN_GENDER] === 1) DisplayText("He");
			else DisplayText("She");
		}
		else DisplayText("They");
		DisplayText(" mutter" + urtaKidsText("s", "s", "") + " rebelliously, but then perk" + urtaKidsText("s", "s", "") + " up at the sight of you.  Immediately " + urtaKidsText("he", "she", "they") + " start" + urtaKidsText("s", "s", "") + " pulling off " + urtaKidsText("his", "her", "their") + " clothes and then, naked as the day " + urtaKidsText("he was", "she was", "they were") + " born, swarm" + urtaKidsText("s", "s", "") + " you, hugging you tightly.  You dole out comforting pats and playful strokes, then instruct " + urtaKidsText("him", "her", "them") + " to get into the tub.  Mournful eyes look at you, but, seeing you won't be swayed, " + urtaKidsText("he leaps", "she leaps", "they leap") + " with a splash into the bathtub.  Once in the water, " + urtaKidsText("he", "she", "they") + " start" + urtaKidsText("s", "s", "") + " to enjoy " + urtaKidsText("himself", "herself", "themselves") + ", merrily kicking and splashing about in what is, for " + urtaKidsText("him", "her", "them") + ", a decent-sized pool.  This gives you an opportunity to note the sex of your little rugrat" + urtaKidsText("", "", "s") + ".");

		//If any UrtaKids=Male:
		if (Flags.list[FlagEnum.URTA_KIDS_MALES] > 0) {
			DisplayText("\n\nYou have " + numToCardinalText(Flags.list[FlagEnum.URTA_KIDS_MALES]) + " vulpine son");
			if (Flags.list[FlagEnum.URTA_KIDS_MALES] > 1) DisplayText("s");
			DisplayText(".");
			if (Flags.list[FlagEnum.URTA_FIRSTBORN_GENDER] === 1) {
				DisplayText("  Your firstborn is ");
				if (Flags.list[FlagEnum.URTA_KIDS_MALES] === 1) DisplayText("the only one,");
				else DisplayText("among them,");
				DisplayText(" sporting a");
				if (Flags.list[FlagEnum.URTA_FIRSTBORN_COCKTYPE] === 2) DisplayText(" vulpine penis");
				else DisplayText("n equine phallus, similar to what Urta has");
				DisplayText(".");
			}
			if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] === 1 && urtaKids() > 1) {
				DisplayText("  Your newest is ");
				if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] === 1) DisplayText("also ");
				DisplayText("a boy, with a");
				if (Flags.list[FlagEnum.URTA_LATESTBORN_COCKTYPE] === 2) DisplayText(" vulpine penis");
				else DisplayText("n equine phallus, just like his mom's");
				DisplayText(".");
			}
			DisplayText("  You have a feeling ");
			if (Flags.list[FlagEnum.URTA_KIDS_MALES] > 1) DisplayText("they're");
			else DisplayText("he's");
			DisplayText(" going to be quite developed when ");
			if (Flags.list[FlagEnum.URTA_KIDS_MALES] > 1) DisplayText("they hit");
			else DisplayText("he hits");
			DisplayText(" puberty, if not as big as Urta.  Urta is quite proud of ");
			if (Flags.list[FlagEnum.URTA_KIDS_MALES] > 1) DisplayText("them");
			else DisplayText("him");
			DisplayText(", and often boasts that ");
			if (Flags.list[FlagEnum.URTA_KIDS_MALES] > 1) DisplayText("they");
			else DisplayText("he");
			DisplayText(" will grow up to be ");
			if (Flags.list[FlagEnum.URTA_KIDS_MALES] > 1) DisplayText("strong fighters like she is.");
			else DisplayText("a strong fighter like she is.");
		}
		//If any UrtaKids=Herm:
		if (Flags.list[FlagEnum.URTA_KIDS_HERMS] > 0) {
			DisplayText("\n\nYou have " + numToCardinalText(Flags.list[FlagEnum.URTA_KIDS_HERMS]) + " vulpine, hermaphroditic daughter");
			if (Flags.list[FlagEnum.URTA_KIDS_HERMS] > 1) DisplayText("s");
			DisplayText(".");
			if (Flags.list[FlagEnum.URTA_FIRSTBORN_GENDER] === 3) {
				DisplayText("  Your firstborn is a hermaphrodite, sporting a");
				if (Flags.list[FlagEnum.URTA_FIRSTBORN_COCKTYPE] === 2) DisplayText(" vulpine penis");
				else DisplayText("n equine phallus, a peculiarity she inherited from her mother");
				DisplayText(".");
			}
			if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] === 3 && urtaKids() > 1) {
				DisplayText("  Your latest addition is ");
				if (Flags.list[FlagEnum.URTA_FIRSTBORN_GENDER] === 3) DisplayText("also ");
				DisplayText("a herm, with a");
				if (Flags.list[FlagEnum.URTA_LATESTBORN_COCKTYPE] === 2) DisplayText(" vulpine penis");
				else DisplayText("n equine phallus.  No doubt such a characteristic was passed from her mother");
				DisplayText(".");
			}
			DisplayText("  You have a feeling ");
			if (Flags.list[FlagEnum.URTA_KIDS_HERMS] > 1) DisplayText("they're");
			else DisplayText("she's");
			DisplayText(" going to be quite developed when ");
			if (Flags.list[FlagEnum.URTA_KIDS_HERMS] > 1) DisplayText("they hit");
			else DisplayText("she hits");
			DisplayText(" puberty, if maybe not quite as big as Urta.  Urta particularly dotes on ");
			if (Flags.list[FlagEnum.URTA_KIDS_HERMS] > 1) DisplayText("them");
			else DisplayText("her");
			DisplayText(", maybe because she remembers how much her parents hated her for being a herm.");
		}
		//If any UrtaKids=Female:
		if (Flags.list[FlagEnum.URTA_KIDS_FEMALES] > 0) {
			DisplayText("\n\nYou have " + numToCardinalText(Flags.list[FlagEnum.URTA_KIDS_FEMALES]) + " vulpine daughter");
			if (Flags.list[FlagEnum.URTA_KIDS_FEMALES] > 1) DisplayText("s");
			DisplayText(".");
			if (Flags.list[FlagEnum.URTA_FIRSTBORN_GENDER] === 2) {
				DisplayText("  Your firstborn is a girl.");
			}
			if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] === 2 && urtaKids() > 1) {
				DisplayText("  Your latest addition is ");
				if (Flags.list[FlagEnum.URTA_FIRSTBORN_GENDER] === 2) DisplayText("also ");
				DisplayText("a girl.");
			}
			DisplayText("  Urta loves to play with ");
			if (Flags.list[FlagEnum.URTA_KIDS_FEMALES] > 1) DisplayText("them");
			else DisplayText("her");
			DisplayText(", and swears ");
			if (Flags.list[FlagEnum.URTA_KIDS_FEMALES] > 1) DisplayText("they will");
			else DisplayText("she will");
			DisplayText(" be quite the heartbreaker");
			if (Flags.list[FlagEnum.URTA_KIDS_FEMALES] > 1) DisplayText("s");
			DisplayText(" when ");
			if (Flags.list[FlagEnum.URTA_KIDS_FEMALES] > 1) DisplayText("they grow");
			else DisplayText("she grows");
			DisplayText(" up.");
		}
		...
		if (urtaKids() > 1) DisplayText("nearest ");
		DisplayText("furry little form of your child and gently tug them over to you.  ");
		if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) DisplayText("She");
		else DisplayText("He");
		DisplayText(" kicks and splashes with ");
		if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) DisplayText("her");
		else DisplayText("his");
		DisplayText(" little legs, but smiles up at you, clearly willing to cooperate, giving you no problem as you spurt a generous dollop of the soapy goo into ");
		if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) DisplayText("her");
		else DisplayText("his");
		DisplayText(" hair and start to massage it... now, just how do you shampoo somebody covered from head to toe in fur...?");
		...
		if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) DisplayText("daughter");
		else DisplayText("son");
		DisplayText(" flicks ");
		if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) DisplayText("her");
		else DisplayText("his");
		DisplayText(" tail, splashing Lianna with water.  The skunkette, however, fails to display any reaction at this.");

		DisplayText("\n\nYou apologize for that, noting that " + urtaKidsText("he", "she", "they") + " got you the same way earlier.  You then flick your ");
		if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) DisplayText("daughter");
		else DisplayText("son");
		DisplayText("'s ear and tell ");
		if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) DisplayText("her");
		else DisplayText("him");
		DisplayText(" to apologize to Lianna for that.  \"<i>It's all right, [name].  " + urtaKidsText("He's just a kid", "She's just a kid", "They're just kids") + ", you can't expect to bathe kids and not get wet.</i>\"  You think you have a pretty clear idea of why Urta picked Lianna to care for the children.  Casually, you ask if Lianna's got any younger siblings; she seems like she's got a lot of practice at this.  She shakes her head, \"<i>This is the first time I got a job where kids are involved.  Either way, it's a lot easier than taking care of minotaurs.</i>\"  You can't help but wonder if she really did somehow take care of minotaurs at one point, or if this is just some colloquial expression.");

		DisplayText("\n\n\"<i>Here, let me help you.</i>\"  She squeezes a handful of shampoo on her hands and begins scrubbing your ");
		if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) DisplayText("daughter");
		else DisplayText("son");
		DisplayText("'s chest, tickling ");
		if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) DisplayText("her");
		else DisplayText("him");
		DisplayText(" sides as she does so.  The little fox giggles and squirms, clearly finding the actions quite ticklish, and you have to hold on a little harder to keep ");
		if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) DisplayText("her");
		else DisplayText("him");
		DisplayText(" from escaping.  \"<i>Now all you have to do is rinse off.</i>\"  She grabs a small bucket nearby, scooping up some water.  \"<i>Close your eyes, dear.</i>\"  Your child does as Lianna instructs; this is evidently a familiar enough routine that ");
		if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) DisplayText("she");
		else DisplayText("he");
		DisplayText(" has no intention of winding up with soap in ");
		if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) DisplayText("her");
		else DisplayText("his");
		DisplayText(" eyes.  Lianna turns the bucket, letting the water cascade over ");
		if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) DisplayText("her");
		else DisplayText("his");
		DisplayText(" head.  Your little fox squeals and then shakes wildly to fling the water off of ");
		if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) DisplayText("her");
		else DisplayText("his");
		DisplayText(" head, kicking up a cascade of water as ");

		if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) DisplayText("she");
		else DisplayText("he");
		DisplayText(" does so");
		if (urtaKids() > 2) {
			DisplayText(", to the delight of ");
			if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) DisplayText("her");
			else DisplayText("his");
			DisplayText(" siblings, who are safely watching from the other side of the bath");
		}
		DisplayText(".");
		...
		DisplayText(" hair is done, shaking some of the water out of your own face.");
		...
		if (urtaKids() === 1) {
			...
			DisplayText(" tail.  Get out of the tub.</i>\"  The skunkette instructs.  The young fox nods and scrabbles at the side of the bath, hauling ");
			if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) DisplayText("her");
			else DisplayText("him");
			DisplayText("self up and out so Lianna can start tending to ");
			if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) DisplayText("her");
			else DisplayText("his");
			DisplayText(" tail.  You promptly make yourself useful and start scrubbing as well...");
		}
		else {
			DisplayText("\n\n\"<i>Good, now who's next?</i>\"  \"<i>Me!</i>\"  ");
			if (urtaKids() > 2) DisplayText("One of your other children");
			else DisplayText("Your other child");
			DisplayText(" pipes up.  You let go of the ");
			if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) DisplayText("girl");
			else DisplayText("boy");
			DisplayText(" you're currently holding so ");
			if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) DisplayText("she");
			else DisplayText("he");
			DisplayText(" can swim away, allowing the new volunteer to take ");
			if (Flags.list[FlagEnum.URTA_LATESTBORN_GENDER] >= 2) DisplayText("her");
			else DisplayText("his");
			DisplayText(" place.  You promptly make yourself useful and start scrubbing as well...");
		}
	...
	//[=Yes=]
	private getABackWashFromLianna() {
		DisplayText().clear();
		DisplayText("You tell Lianna that if she is willing, you wouldn't object to her washing your back.");
		...
		DisplayText("\n\nLianna returns to her scrubbing and massaging, no longer self-conscious about letting her breasts rub against your back.  You groan appreciatively; not only is Lianna quite skilled at easing your tension, but you can feel her soft, pillowy breasts rubbing all over your " + Desc.Skin.skinFurScales(player) + ", the perky nipples stirring up not-unwelcome feelings in your loins.  \"<i>Ok, all done here.</i>\"  She pats you on the back.  You get up and stretch, listening to your joints cracking, and thank her for her efforts.  \"<i>You're welcome, now it's your turn,</i>\" she declares, taking a seat on the bench herself.  \"<i>Use the floral shampoo, please.</i>\"");
	...
	//Take the kids for a walk - Special thanks to Belin
	//Randomly picked when selecting Kids option in Urta's house.
	private takeTheKidsForWalk() {
		DisplayText().clear();
		DisplayText("\"<i>You'd like to see " + urtaKidsText("him", "her", "them") + "?  Sure, but first I need to get " + urtaKidsText("him", "her", "them") + " ready.</i>\"   You ask her where she plans on taking " + urtaKidsText("him", "her", "them") + ".");
		...
		if (urtaKids() > 1) DisplayText("ren");
		DisplayText(" are suitably dressed for a traipse around a desert city, you usher your kit");
		if (urtaKids() > 1) DisplayText("s");
		DisplayText(" out the door and into the street.  \"<i>Can " + urtaKidsText("I", "I", "we") + " get some ice cream?!</i>\"");
		if (urtaKids() > 1) DisplayText("  \"<i>Yeah, can we " + Desc.Gender.mf(player, "daddy", "mommy") + "?!</i>\"");
		DisplayText("  Your child");
		if (urtaKids() > 1) DisplayText("ren plead");
		else DisplayText(" pleads");
		DisplayText(".  Well, the vendors down at the market would make for a suitably far destination, which should give Lianna plenty of time for herself, and you yourself wouldn't mind some ice cream either at that, so it's decided.  \"<i>Yay! You're the best " + Desc.Gender.mf(player, "dad", "mom") + " ever!</i>\" ");
		if (urtaKids() > 1) DisplayText("a chorus of excited cheers sounds out.");
		else DisplayText("an excited cheer sounds out.");
		DisplayText("  It's hard not to want to live up to that title with such ");
		if (urtaKids() === 1) DisplayText("a happy face");
		else DisplayText("happy faces");
		DisplayText(" staring up at you.");
		...
		if (urtaKids() > 1) DisplayText("ren");
		DisplayText(" pick out flavours while the shopkeeper scoops them up onto wafer cones.  Once he's done, you thank the wolf and leave the shop followed by your ");
		if (urtaKids() > 10) DisplayText("army of ");
		DisplayText("child");
		if (urtaKids() > 1) DisplayText("ren");
		DisplayText(".");
		...
		DisplayText("\n\nYour child");
		if (urtaKids() > 1) DisplayText("ren");
		DisplayText(" don't bother waiting or savouring their ice cream and neither do you, it's damned hot walking around this desert.  There's less cause for mischief too, " + urtaKidsText("his", "her", "their") + " face");
		if (urtaKids() === 1) DisplayText("buried in cone");
		else DisplayText("s buried in their cones");
		DisplayText(" along with your own, greedily licking up melting droplets.  There's just a bit of cone left as you return to Urta's house, and you quickly polish that off, upon entering.  \"<i>Oh, hey.  You're back.</i>\"  Lianna greets you in her usual inexpressive fashion.");

		DisplayText("\n\n\"<i>How did it go?</i>\"");

		DisplayText("\n\nWithout a hitch, you reply.  The " + numToCardinalText(urtaKids() + 1) + " of you stopped for some ice cream along the way.  \"<i>Good, now off you go.  Bathtime.</i>\"  She states, ushering your little fox");
		if (urtaKids() > 1) DisplayText("es");
		DisplayText(" towards the bathroom.  There is a " + urtaKidsText("shrill protest", "shrill protest", "chorus of shrill protests") + ", but " + urtaKidsText("he", "she", "they") + " clearly understand");
		if (urtaKids() === 1) DisplayText("s");
		DisplayText(" that Lianna is not one to be trifled with and so " + urtaKidsText("he", "she", "they") + " march");
		if (urtaKids() === 1) DisplayText("s");
		DisplayText(" off to the bathroom.  You can't help but note how good Lianna is at making kids do what she says.");
	...
	public visitTheHouse() {
		...
		if(Flags.list[FlagEnum.URTA_INCUBATION] >= 192 && randInt(2) === 0) {
			...
			if(urtaKids() > 1) DisplayText("s");
			DisplayText(" for a walk.  Be back soon!  - Lianna");
	...
	private talkToLiana(output: boolean = true) {
		if (output) {
			DisplayText().clear();
			DisplayText("You tell her you'd like to talk her.  She adjusts her glasses.  \"<i>Oh?  What about?</i>\"");
		}
	...
	//Her Job
	private talkToLianaAboutHerJob() {
		DisplayText().clear();
		DisplayText("You ask her about her jobs, both for Urta and what she intends to do afterwards.");
	...
	//Her Research
	private talkToLianaAboutHerResearch() {
		DisplayText().clear();
		DisplayText("You ask her why she's so eager to get samples from you, a human.  What sort of research is she doing that she needs human samples?");
		...
		if (player.stats.cor >= 50) DisplayText("however half-heartedly it may be, ");
		DisplayText("and ask if that's what pushed her to studying humans and Marethians.");

		DisplayText("\n\nShe shrugs.  \"<i>I guess, though what I have is genuine curiosity.  Maybe someday one of your kids will ask why they don't look like " + Desc.Gender.mf(player, "daddy", "mommy") + ".  Wouldn't you want to have an answer to that question?</i>\"  She smiles.");
	...
	private talkAboutLiannasLubeSpray() {
		DisplayText().clear();
		Flags.list[FlagEnum.DISCUSSED_LUBE_SPRAY] = 1;
		DisplayText("You ask why it is that she produces a lubricating liquid rather than a noxious one, like you would expect of a skunk.");
	...
	private lubeSpraySideEffects() {
		DisplayText().clear();
		DisplayText("So, if she's experimented on herself in the past, that must have gone wrong for her at least once or twice in the past, you comment.");
		...
		if (player.stats.cor < 33) DisplayText("mild");
		else if (player.stats.cor < 66) DisplayText("some");
		else DisplayText("great");
		DisplayText(" reluctance.");
	...
	private sexWithLianna() {
		DisplayText().clear();
		DisplayText("\"<i>So you want to help me with my experiments?  Great!</i>\"  She takes your hand and basically drags you into her room.  You allow her to lead you, wondering what she has in mind.");
		...
		if (player.torso.cocks.count > 1) DisplayText("  Your lower garments suddenly feel much tighter.");
		...
		if (player.torso.cocks.count > 0) MainScreen.addButton(1, "HarvestManly", liannaHandjobbies);
		MainScreen.addButton(4, "Back", visitTheHouse);
	}

	//[=Refuse=]
	private refuseGenderlessInjection() {
		DisplayText().clear();
		DisplayText("Lianna's face falls at your refusal, but she sighs.  \"<i>All right, I can understand; I mean, you must have your reasons.  Still, if ever you get a gender, remember that I'd be happy to use some of your seed, please.</i>\"");
	...
	//[=Accept=]
	private acceptARandomGenderFromASkunk() {
		DisplayText().clear();
		DisplayText("You decide that it's worthwhile to go along and indicate that you'll let Lianna experiment on you with her \"genderiser\".");
		...
		//{if Centaur: 
		if (player.torso.hips.legs.isTaur()) DisplayText("  Or at least hold on to something, this might sting a little and I don't need you kicking down anything in here.");
		DisplayText("</i>\"");
		...
		//If Tight:  
		if (player.torso.butt.looseness <= 2) DisplayText("\n\n\"<i>Going to need some lube for this...</i>\" she remarks.");
		else DisplayText("\n\n\"<i>Pretty loose, I think I could just stick it in, but I'll apply some lube to make this as comfortable as possible.</i>\"");
		DisplayText("  You really should be more surprised, but, sadly, this is kind of predictable; you were more hoping than anything that this would be an oral delivery, or at least an injection.");
		...
		if (randInt(10) <= 3) {
			...
			player.torso.cocks.get(0).length = 4;
			player.torso.cocks.get(0).thickness = 0.75;
		...
		//if PC becomes Female:
			...
			if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating < 1) {
				...
				if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].nipples.length < .5) {
					DisplayText("  Your nipples erect into small half-inch nubs, complementing the transformation.");
					player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].nipples.length = .5;
				}
				player.torso.chest.get(0).rating = 1;
			}
			player.createVagina();
		...
		//if PC becomes Herm:
			...
			if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating < 3) {
				...
				if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating < 1) DisplayText("s");
				DisplayText(" swelling into a soft C-Cup.");
				if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].nipples.length < 0.5) {
					DisplayText("  Your nipples erect into small, half-inch nubs, complementing the transformation.");
					player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].nipples.length = 0.5;
				}
				DisplayText("  You watch the soft mounds in your chest, unable to do anything but pant.");
	...
	private getEggsHarvested() {
		...
		if (Flags.list[FlagEnum.LIANNA_HAVESTED_LADIES] === 0) {
			DisplayText("confusion, wondering what she's playing at");
			Flags.list[FlagEnum.LIANNA_HAVESTED_LADIES]++;
		}
		else DisplayText("remembrance, knowing what's coming next");
		DisplayText(".  She unceremoniously begins shoving it inside without even a hint of pleasure or discomfort; the dildo slides in without resistance, spreading her buttcheeks apart and penetrating her with a wet squelch.  You watch the display, not sure whether or not you should find this kinky.");
		...
		if (player.vaginalCapacity() >= 50) DisplayText("  \"<i>Hmm, I might not have had to lube it up after all...</i>\"  The skunkette remarks.");
		DisplayText("  She aligns the tip of the dildo with you and without saying a word she begins pushing in.  Instinctively you moan and try to spread yourself as wide as possible.");
		...
		if (player.torso.vaginas.get(0).looseness >= 4) DisplayText("; your loose pussy doesn't hurt, either");
		DisplayText(".  Finally, you can feel it stop inside of you, the tip pushing against your cervix, which is what has brought it to a halt.");
		...
		if (player.torso.cocks.count > 0) {
			DisplayText(" except for the ");
			if (player.cumQ() < 50) DisplayText("small");
			else if (player.cumQ() < 250) DisplayText("normal");
			else if (player.cumQ() < 1000) DisplayText("sizable");
			else DisplayText("huge");
			DisplayText(" splatters of cum from your neglected penis");
		}
		DisplayText("; the dildo has absorbed it all.  You can feel it clenching tighter, shrinking down so that it no longer fills you up so perfectly, leaving you panting in release as Lianna gently frees it from inside you.");
	...
	//Handjob
	//Needs cock.
	private liannaHandjobbies() {
		DisplayText().clear();
		Flags.list[FlagEnum.LIANNA_HAVESTED_MALES]++;
		DisplayText("\"<i>All right, let's get this show on the road.</i>\"  She ducks out into her closet once more, and comes out carrying what looks like a condom and a lollipop.  \"<i>Here's how this works.  You put on this condom and I milk your seed out of you.  Simple, yes?</i>\"");
		...
		if (player.torso.cocks.count > 1) DisplayText("  \"<i>However I think milking just one is good enough.</i>\"");
		DisplayText("  She motions to the bed.");
		...
		if (player.torso.cocks.count > 1) {
			...
			DisplayText("\n\nShe ducks close to you and analyses your " + Desc.Cock.describeMultiCockShort(player) + ".  \"<i>I think this one should do.</i>\"  She says, caressing your [cock biggest].  You murmur appreciatively at the stimulus, but you don't forget what's coming next even as your traitorous flesh erects itself for her.  She fishes for a cock-ring and attaches it to your other dick");
			if (player.torso.cocks.count > 2) DisplayText("s");
			DisplayText(", effectively binding them at their base.  You groan hollowly at this; though this certainly will mean less mess, you're not so sure you're going to enjoy the sensation of trying to cum through the " + numToCardinalText(player.torso.cocks.count - 1) + " cocks that have been blocked up.  Satisfied with her work, she looks at your [cock biggest].");
		}
		...
		if (player.torso.balls.quantity > 0) DisplayText("occasionally stopping to caress your [balls], ");
		DisplayText("while the other massages your [cockHead biggest], rolling the steadily growing bubble of pre around as she kneads it.");
		...
		if (player.cumQ() < 1000) { }
		else if (player.cumQ() < 2000) DisplayText("big");
		else if (player.cumQ() < 3000) DisplayText("huge");
		else DisplayText("giant");
