export class Raphael extends NPCAwareContent implements TimeAwareInterface {
	...
	public timeChange(): boolean {
		...
		if (Flags.list[FlagEnum.RAPHAEL_DRESS_TIMER] > 1 && player.inventory.gems >= 5) Flags.list[FlagEnum.RAPHAEL_DRESS_TIMER]--;
		if (Flags.list[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] > 1 && player.inventory.gems >= 5) Flags.list[FlagEnum.RAPHEAL_COUNTDOWN_TIMER]--;
	...
	public timeChangeLarge(): boolean {
		if (checkedRussetRogue++ === 0 && model.time.hours === 6 && Flags.list[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] >= 0 && player.hasKeyItem("Camp - Chest") >= 0 && player.inventory.gems >= 5 && player.statusAffects.get(StatusAffectType.TelAdre).value1 >= 1) {
	...
	public RaphaelLikes(): boolean {
		if (Flags.list[FlagEnum.LOW_STANDARDS_FOR_ALL]) {
			if (player.gender === Gender.FEMALE || player.gender === Gender.HERM)	// you at least need a vagoo for raphael to fuck you. Otherwise the sex scenes will be rather broken.
				return true;
		}
		...
		if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 12) return false;
		//({If player has below C cup breasts} 
		if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating < 3) return false;
		//({If player has grown less than girly hips}
		if (player.torso.hips.rating < 6) return false;
		//({If player has gotten a massive butt} 
		if (player.torso.butt.rating >= 16) return false;
		...
		if (player.torso.balls.quantity > 0 || player.torso.cocks.count > 0) return false;
		//(For now: 
		//({If player has lost all gender}
		if (player.gender === Gender.NONE) return false;
		return true;
	}
	...
	private meetRaphael() {
		...
		DisplayText("Suddenly, you sit up straight and something occurs to you. If you're sleeping, then who's opening the chest?\n\n");

		DisplayText("You rush out of bed and into camp, concerned for your stash.\n\n");
		...
		DisplayText("\"<i>Ha-hah!</i>\" it exclaims boastfully with a sharp, young, dashing voice while standing tall and proud on top of its perch. \"<i>Another daring caper committed by...</i>\"  The being takes the time to strike a pose. \"<i>...the Russet Rogue!</i>\"\n\n");
	...
	//~~~ Next Page ~~~
	private meetRaphaelPtII() {
		DisplayText().clear();
		DisplayText("Suddenly, Raphael's features grow soft and surprised as he looks down upon you. You get the feeling he's eyeing you up and catching a peek at your cleavage, but you can't be sure.\n\n");

		DisplayText("\"<i>Marae must have cursed me for my audacity, for I am growing blind even at my young age.</i>\" The fox puts his hand to his forehead and pretends to faint. He rights himself just before he hits the ground, however and lands in a kneel before your feet.\n\n");

		DisplayText("You can't help but smile a little at the amount of theatrical flourish.\n\n");
		...
		//[Talk] [Slap] [Swoon]
		MainScreen.simpleChoices(["Talk", "Slap", "Swoon", "", ""], [RaphaelFirstMeetingTALK, RaphaelFirstMeetingSLAP, RaphaelFirstMeetingSWOON, null, null]);
	}

	//{When Player chooses Slap/refuse after the first encounter}
	private RaphaelFirstMeetingSLAP() {
		DisplayText().clear();
		DisplayText("With an offended scowl you throw a flat palm across his face. You make sure to catch his nose and sharp snout, sending him a clear message.\n\n");

		DisplayText("It takes Raphael completely by surprise. The fox keeps his head in the wake of the blow for a good five seconds in disbelief, but tries once more when he recovers.\n\n");

		DisplayText("\"<i>I can assure you señorita, that I had no intention of tainting your honor.</i>\" He pats your hand.\n\n");
		...
		DisplayText("Raphael curtsies and tips his hat before making his escape. \"<i>You have my condolences.</i>\"\n\n");

		DisplayText("You resolve to wash your hand. You're sure you've not seen the last of the russet rogue, but it will be time in coming before that happens with the severity of your rejection.");
	...
	private RaphaelFirstMeetingSWOON() {
		DisplayText().clear();
		DisplayText("You snicker softly, shift your weight on one leg and blush a little. He's quite the charmer; almost good enough to forgive him for robbing you. The attention he showers you with doesn't leave you cold either.\n\n");

		DisplayText("\"<i>Normally I rob opulent merchants and criminals. I have no idea what drew me into your camp. Perhaps destiny willed this fateful meeting?</i>\" He looks up and burrs with a suave accent. \"<i>Then again, I am attracted to extraordinary splendor.</i>\" He gazes at you with an emerald shimmer in his rich green eyes.\n\n");

		DisplayText("Curious, you finally ask him who he really is.\n\n");
		...
		DisplayText("In a blink of an eye, the red fox jumps back up the wall. \"<i>We will meet again!</i>\" He exclaims in a hushed tone, while slinking over to the other side of the wall.\n\n");

		DisplayText("You hold the hand he touched close to your chest.");
	...
	private RaphaelFirstMeetingTALK() {
		DisplayText().clear();

		DisplayText("You squint your eyes at him, pulling on your arm to wrestle it from his hold. When you remark he stole from you and ask him to return your goods, Raphael simply throws you a vulpine smirk.\n\n");
		...
		DisplayText("You insist he return all of it, but the Russet Rogue merely smiles as he walks back towards you. \"<i>My, whatever do you mean, pretty señorita? Are you accusing me of being less than forthright?</i>\" He raises his shoulders.\n\n");
		...
		DisplayText("\"<i>My, aren't we frisky. What happened to foreplay? Shouldn't you be buying me dinner before you ravish me? Breakfast perhaps?</i>\" Raphael clucks. You jump away as you realize you've had your hands all over his body.\n\n");
		...
		DisplayText("In a blink of an eye, the red fox jumps back up the wall. \"<i>We will meet again!</i>\" He exclaims in a hushed tone, while slinking over the wall to land on the other side.\n\n");
	...
	//{Second encounter.} 
	//Again at bedtime
	private RaphaelDress() {
		DisplayText().clear();
		DisplayText("A small pebble hits the ground near you, waking you up. When a second one hits, you're sure someone is trying to draw your attention.\n\n");

		DisplayText("Rubbing your eyes, you pull yourself out of bed, wondering what's going on. Sticking your head through the front wall of your tent, you take a curious peek outside, but find no one around.\n\n");
		...
		DisplayText("\"<i>I happened across this beautiful ensemble and was instantly reminded of the one thing in Mareth that makes its splendor pale in comparison. Fair trappings, to fit around a beautiful body.\n\n");

		DisplayText("Yours faithfully,\n  - The Russet Rogue\n\n");

		DisplayText("P.S. The clothes are my gift to you. I would delight to see you wear it whenever you desire to meet. Ownership of the ruby however, remains to be seen.</i>\"\n\n");

		DisplayText("You hold the ruby pendant to the light. It must be priceless. Its crimson shimmer is enough to make you forget to ask yourself where Raphael could have gotten it from. You wonder what he means by ownership. It very clearly is in your possession now.\n\n");

		DisplayText("Soon enough you put it down and begin to inspect the equally lustrous garment he left you. You unfold it and marvel at the Bordeaux colored clothes.");
		//nxt page
		return { next: RaphaelDressPtII };
	}

	//~~~ Next Page ~~~
	private RaphaelDressPtII() {
		DisplayText().clear();
		...
		DisplayText("All over your body where the gossamer isn't reinforced with jacket or corset, the density of the delicate velvet web varies. Although the silk hugs across your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " and through the crack of your ass as a triple layer that guards against prying eyes, the surface of your hips and legs is clearly seen through the transparent motif of flowers swirling across the lace. The cheeks of your " + Desc.Butt.describeButt(player) + " feel equally exposed despite the presence of four sweeping rosebranches stitched across them, but at least the jacket trails past your lower back and partly covers your buttocks with its parted tailflaps.");
		//({If player has tail}
		if (player.torso.tailType > TailType.NONE) DisplayText("  Your tail peeks out through the cut.");
		DisplayText("  You still can't help but feel that anyone standing behind you is given a generous glimpse of your ornate ass, however. The same goes for your " + Desc.Breast.describeBreastRow(player.torso.chest.get(0)) + "; cupped, lifted and presented as they are to the outside world by grasping silk. Their ample curve and tender flesh are clearly visible through the red lace. The only thing saving their modesty is the tactical application of a sea of organic patterns across the lower half, with the curl of two roses covering your " + Desc.Breast.describeNipple(character, character.torso.chest.get(0)) + "s. The ensemble comes with a pair of red stiletto high heels, but you're not sure you're ready for them. Wearing them would only perk up your noticeable posterior even more. That your " + Desc.Breast.describeBreastRow(player.torso.chest.get(0)) + " contrast above a slender waist is enough for now. Maybe on special occasions.\n\n");

		DisplayText("You blush as the wind breezes by, and with the exception of the upper jacket, feel like you're wearing nothing at all. This sensation is only aggravated when you can't help but slip a finger across your inner thigh and feel it glide up effortlessly across the textile. It's like you've only become more sensitive for wearing it. Much to your amazement, the triple layer across your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " doesn't provide quite as much protection as you assumed earlier; at least not so much against roving fingers. You find the fabric across your womanhood has a hidden opening to it. Rubbing through it is enough to part the velvet folds and set your finger upon your own. It's not apparent, but anyone aware of this shameful split would have easy access to your depths without even disrobing you. You feel nude.\n\n");
	...
	private RaphaelEncounterIIDressFollowup() {
		//{Encounter two}
		...
		if (RaphaelLikes()) {
			DisplayText("\"<i>How long ago was it, that you had a decent breakfast, hhhmmm?</i>\" He smirks. \"<i>One that didn't include imp gut and hellhound testicles. A worn out camp like this one is no place for a lady. Let me at least endeavor to give a woman of your caliber a taste of the good life.</i>\"\n\n");

			DisplayText("When you inspect the rest of his body, it's clear that Raphael is still the sly fox. Your ruby pendant hangs from his belt. He has once again stolen it from you after gifting it earlier, like it was some game. When he notices your interest, he flicks his tail over it, causing the necklace to disappear.\n\n");

			DisplayText("\"<i>The temporary price of admission for a moment of wonder.</i>\" He assures you. \"<i>Trust me that it'll be worth it. Join me, and I might even teach you the tricks of the trade. That is, unless you're cunning enough to frisk me for it.</i>\" He smiles playfully.\n\n");

			DisplayText("What do you do?");
			Flags.list[FlagEnum.RAPHAEL_DISGUSTED_BY_PC_APPEARANCE] = 0;
			//[Reject] [Frisk] [Date]
			MainScreen.simpleChoices(["Reject", "Frisk", "Date", "", ""], [RaphaelChooseReject, RaphaelChooseFrisk, RaphaelSelectDate, null, null]);
		}
		//({If player does not meet the first encounter requirements:}
		else {
			DisplayText("When he catches sight of you, he spurts out the sip.\n\n");

			DisplayText("\"<i>Mon Dieu!</i>\" he states in shock and stands up on top of his wall.\n\n");

			DisplayText("\"<i>What terrible tragedy! The land has taken its toll on the once so beautiful.</i>\" He looks down on you.\n\n");

			//({If player has no legs, or a centaur body.}
			if (player.hips.legs.type === LegType.NAGA || player.hips.legs.type === LegType.CENTAUR || player.hips.legs.type === LegType.GOO || player.hips.legs.type === LegType.PONY)
				DisplayText("\"<i>You're missing half your body!</i>\" He refers to your morphed legs.\n\n");
			//({If player has above E cup breasts} 
			if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 7) DisplayText("\"<i>Your female curves... replaced with such... udders!</i>\" He looks at your bosom. \"<i>No woman could be elegant with such monstrosities up front!</i>\"\n\n");
			//({If player has below C cup breasts} 
			if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating < 3) DisplayText("\"<i>Your female curves... gone!</i>\" He looks at your bosom. \"<i>It's hard to tell you apart from a little girl!</i>\"\n\n");
			//({If player has grown less than girly hips}
			if (player.torso.hips.rating < 6) DisplayText("\"<i>What happened to that fine hourglass shaped figure? Those comely hips?</i>\"\n\n");
			//({If player has gotten a massive butt} 
			if (player.torso.butt.rating >= 13) DisplayText("\"<i>Oh... my... Marae! " + player.short + ", look at your butt. It is so big! You look one of those cat guy's girlfriends. Who understands those cat guys? You look like a total prostitute. I mean, your butt. It's just so big. I can't believe it's just so round and so out there! Gross!</i>\"  The fox shakes his head and breaks it down. \"<i>I hate big butts! So vulgar.</i>\"\n\n");
			//({If female player has gotten bigger than 6 feet}
			if (player.tallness > 72) DisplayText("Raphael rolls his eyes across your giant body and looks intimidated. \"<i>I can forget about remaining unseen when I take someone of your size somewhere!</i>\"\n\n");
			//({If female player has gotten smaller than 4 feet}
			if (player.tallness < 48) DisplayText("Raphael squints like he has trouble seeing you from there, because of your dimunitive size. \"<i>I've dated goblins once... didn't work out.</i>\"\n\n");
			//({If player has grown ANY cock and balls}
			if (player.torso.balls.quantity > 0 || player.torso.cocks.count > 0) DisplayText("\"<i>What is that bulge below the tight outfit I gave you?</i>\" The fox inspects your groin. \"<i>No, never mind. I don't want to know.</i>\"\n\n");
			//(For now: 
			//({If player has lost all gender}
			if (player.gender === Gender.NONE) DisplayText("\"<i>There's... something different about you today. Your smell, it has changed.</i>\"\n\n");
			...
			DisplayText("You clench your jaw as he vanishes, more than a bit offended.");
	...
	private RaphaelSelectDate() {
		DisplayText().clear();
		DisplayText("You smile bashfully, not expecting this sort of gesture from anyone in Mareth. You hardly care about the pendant and you nod at him.\n\n");

		DisplayText("\"<i>Ha-ha!</i>\" Raphael hops to his feet on top the wall and gloats at you. \"<i>You're sure you're ready for this journey? Because I'm going to show you the time of your life!</i>\" He kneels down on top his wall and extends you his hand on approach.\n\n");

		DisplayText("It almost feels like you're consenting when you reach for his soft, cushioned padded furry paw, after which he pulls you up and helps you over the wall.");
		//{Unlocks Picnic}
		return { next: RaphaelPicnic };
	}
	//{When player chooses [Reject] after second encounter}
	private RaphaelChooseReject() {
		DisplayText().clear();
		DisplayText("You sigh at the fox and shake your head. You're not interested in him or his advances.\n\n");

		DisplayText("He flicks his tail past his body again and within the blink of an aye, the glass of wine is exchanged for the stolen pendant.\n\n");

		DisplayText("\"<i>The Russet Rogue is no thief, señora.</i>\" He states seriously and tosses the bauble back at your feet. You drop your frown.\n\n");
	...
	private RaphaelChooseFrisk() {
		Flags.list[FlagEnum.FUCK_YOU_GOT_MINE_RAPHAEL] = 0;
		DisplayText().clear();
		DisplayText("You flash a playful smile and express the desire to get your jewelry back.\n\n");
		...
		DisplayText("Noticing how Raphael leaves more room between his hands and his head than necessary, you reach beind his neck to retrieve the pilfered pendant, but when you do, there is none to be found. It's the only place he could have put it, but it's gone and he isn't using his arms!\n\n");

		/// Int/Spe Variables ///
		//({Int or Spe below 24 and Cor is not higher than 19} 
		if ((player.stats.int < 24 && player.stats.spe < 24) && player.stats.cor < 19) {
			DisplayText("You frown, befuddled and eyeing the body you hold close to yours. He must be a magician, you conclude.\n\n");
			...
			DisplayText("\"<i>Forgive me...</i>\" He growls softly with low tone. \"<i>You're just so cute when consternated.</i>\"\n\n");

			DisplayText("\"<i>But the day is young!</i>\" He proclaims before slipping from your hold and up the wall, extending you his paw. \"<i>Join me! Together we shall paint the forest russet red!</i>\" He winks at you while pulling you up. \"<i>Maybe I could even show you how I do it, huh?</i>\"\n\n");
			//{Next scene picnic}
			return { next: RaphaelPicnic };
		}
		//({Int or Spe between 24-36 and Cor is not higher than that}
			...
			if (player.stats.cor <= 15) DisplayText("You have the stronge urge to grab hold of the oddly enlarged bulge between his legs and retrieve the necklace from within his pants, but the lewdness of the gesture keeps you from it.\n\n");
			//{If player corruption is at or higher than 15} 
			else DisplayText("With no intention to relent, you grab him by the oddly enlarged bulge in his tight leather pants and squeeze softly, discovering how several of his jewels are harder than others. He hid your gem amongst his own!\n\n");

			DisplayText("\"<i>My, aren't we frisky. What happened to foreplay? Shouldn't you be buying me dinner before you ravish me? Breakfast perhaps?</i>\" Raphael clucks. \"<i>Although frankly.</i>\" He growls softly with low tone. \"<i>I would gladly suffer through a thousand indignities, for a mere touch from one such as you.</i>\"\n\n");

			DisplayText("\"<i>But the day is young!</i>\" He proclaims before slipping from your hold and up the wall, extending you his paw. \"<i>Join me! Together we shall paint the forest russet red!</i>\" He winks at you while pulling you up. \"<i>You have skill though. Stick with me and maybe I can teach you.</i>\"\n\n");
			//{Next scene picnic}
			return { next: RaphaelPicnic };
		}
		//({Int or Spe are above 35 and Cor is not higher than that}
		else if (player.stats.cor < 35) {
			DisplayText("Figuring it out easily, you notice how the fox is moving his tail about. The sneaky rogue is using his fifth limb to move the pendant about his body! Deducing the only way he could do that without moving his arms, you reach for the heel of his lifted foot and intercept the string before he can pass it onto his leg.\n\n");

			DisplayText("\"<i>Nice try!</i>\" You comment with a triumphant smile, when you quickly retrieve the necklace and walk away from Raphael before he realizes you've outwitted him. You hug the jewelry tight.\n\n");
			...
			DisplayText("How do you respond?");
			//Reject] [Accept]
			MainScreen.simpleChoices(["Reject", "Accept", "", "", ""], [friskRejectChoice, friskAcceptChoice, null, null, null]);
		}
		//{If player's corruption is higher than 19 and higher than Intelligence.}
			...
			DisplayText("\"<i>My, such an eager little thing.</i>\" He comments, pleased.\n\n");

			DisplayText("You ask yourself the age old question if he's just happy to see you, but you suspect differently when you reach out for the firm lump and cup it.\n\n");

			DisplayText("What do you do?");
			//[Squeeze] [Fondle]
			MainScreen.simpleChoices(["Squeeze", "Fondle", "", "", ""], [friskSqueezeChoice, friskFondleChoice, null, null, null]);
		}
	}

	//[Accept]
	private friskAcceptChoice() {
		Flags.list[FlagEnum.FUCK_OFF_THEIVING_RAPHAEL] = 1;
		DisplayText().clear();
		DisplayText("\"<i>Come on then!</i>\" He proclaims before shooting up the wall, extending you his paw. \"<i>Join me! Together we shall paint the forest russet red!</i>\"\n\n");

		DisplayText("You waive his paw and choose to walk around the 8 foot long wall, instead of struggling to climb over it. Raphael flashes a grin of delight when you do. \"<i>I can see I've caught on to a smart one! Stick with me and I could even finish your education. I promise that graduation will be something... special.</i>\"\n\n");
		//{Next scene picnic}
		return { next: RaphaelPicnic };
	}

	//[reject]
	private friskRejectChoice() {
		DisplayText().clear();
		DisplayText("You scoff, cross your arms and reject the offer by telling Raph your answer from across your shoulder, with your back still turned to him aloofly. Pleased with yourself, you add that he's nothing more than a carnie with an accent and that you won't be needing his help.\n\n");

		DisplayText("\"<i>As you say, my desert rose. It is clear that I have no more to teach you then.</i>\"\n\n");

		DisplayText("You startle when you hear the fox bang against your chest. When you turn around, he's once again stealing from you!\n\n");

		DisplayText("\"<i>There's no teaching those of lesser stock a sense of grace and refinement. Those are still decidedly my domain!</i>\" He guffaws with a bag of goods across his back and runs off.\n\n");

		DisplayText("\"<i>Even without a belt on?</i>\" You ask him, holding up the leather strap he wore across his waist a mere minute ago. You took the time to swipe it along with the pendant.\n\n");

		DisplayText("As Raphael bolts, he has a look of surprise on his face. He doesn't last long before his pants drop to his ankles and he falls flat on his face.\n\n");

		DisplayText("He yelps when he loses hold of the bag, which drops to the ground. The fox uses both hands to protect his modesty by holding up his pantaloons, slipping over the spilt gems a few good times and flailing about, before finally making it to the wall with nothing to show for it.\n\n");

		DisplayText("\"<i>I will regain my honor!</i>\" He exclaims while waving his hands in a theatrical flourish, causing his pants to drop once more.\n\n");

		DisplayText("You laugh as he blushes, beating a sound retreat. You're sure this won't be the last you'll see of him however.");
		//{Game removal}
		Flags.list[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] = 14;
		Flags.list[FlagEnum.REJECTED_RAPHAEL] = 1;
		return { next: playerMenu };
	...
	private friskFondleChoice() {
		...
		DisplayText("\"<i>Such a good girl you are.</i>\" He smirks lewdly. \"<i>You have a lot of talent in those fingers. A distinct gift. There's a lot I could show a feisty little minx such as you.</i>\"\n\n");

		DisplayText("You deftly remove the sweat stained, fogged up stones and tug them away between your breasts.\n\n");

		DisplayText("\"<i>And I in turn, will have to retrieve those.</i>\" He growls low.\n\n");
	...
	private friskSqueezeChoice() {
		...
		DisplayText("\"<i>You have some balls stealing from me fox.</i>\" You comment and grin. \"<i>Just checking. Is that too soft or should I squeeze harder?</i>\"\n\n");

		DisplayText("Raphael shakes his head while a tear wells up in his eyes. When you roll his privates into the sharp edges, he finally lets out a cry loud enough to hear from miles away.\n\n");

		DisplayText("You chuckle and look around you to see if anyone heard. When you look back to see him squirming between your fingers however, all you're holding onto is the ruby pendant. Raphael has pulled off an amazing switching trick and has escaped. Instead, he stands on top the wall panting and defiant.\n\n");

		DisplayText("\"<i>I'll be sure to repay the favor.</i>\" He warns you with a high pitched voice while clasping his poor genitals. \"<i>Nobody touches my goods like that and gets away with it.</i>\"\n\n");

		DisplayText("He raises just a single finger to the air and groans painfully. \"<i>We will meet again!</i>\"\n\n");
	...
	private RaphaelPicnic() {
		...
		DisplayText("\"<i>Mademoiselle?</i>\" Raphael inquires your readiness. \"<i>Breakfast, is served!</i>\" he exclaims while uncorking a bottle of wine.  He has neatly arranged a series of plates, each filled with small delicacies.\n\n");
	...
	private RaphaelPicnicII() {
		...
		//[Discuss] [Skill] [Flirt]
		MainScreen.simpleChoices(["Fencing", "Thieving", "Flirt", "", ""], [RaphaelPicnicSkill, RaphaelPicnicChooseThieving, null, null, null]);
	}
	private RaphaelPicnicEnd() {
		DisplayText().clear();
		...
		DisplayText("\"<i>Are you sure, señorita? In your presence, I would gladly spend an eternity.</i>\"\n\n");
		...
		DisplayText("However, just before he vanishes, the fox gives you a wink and a bow. \"<i>My Lady, it has been a true delight, and it would be to my great fortune if you'd allow me to see you again.</i>\"\n\n");

		DisplayText("He seems so hasty that you'd begin to question it, but your doubt is taken away as the fox takes the time to stare patiently at you from the forest's edge. \"<i>How could I resist, when you're dressed and made so gorgeously?</i>\" He smiles, referring to your elegant red suit.\n\n");

		if (Flags.list[FlagEnum.FUCK_OFF_THEIVING_RAPHAEL] === 0) {
			DisplayText("A minute of silence follows and just when it has been long enough to reassure you, he throws you something and slips away and vanishes.\n\n");

			DisplayText("You catch the object: he has returned the priceless ruby pendant. You recollect yourself and head back to camp.");
		}
		else {
			Flags.list[FlagEnum.FUCK_OFF_THEIVING_RAPHAEL] = 0;

			DisplayText("A minute of silence follows and just when it has been long enough to reassure you, he makes his excuses and sets about heading back to... whatever it is the sly fox spends his days doing.\n\n");

			DisplayText("You recollect yourself and head back to camp.");
		}
		return { next: playerMenu };
	}

	//{Player chooses [Skill]}
	private RaphaelPicnicSkill() {
		DisplayText().clear();
		...
		if (Flags.list[FlagEnum.RAPHAEL_RAPIER_TRANING] === 0) {
			...
			DisplayText("There's a moment of awkward silence as you realize the young man places more pride in his sword than you thought. However, Raphael recollects himself with a smile and chuckles at himself like he's been had by someone who just doesn't understand.\n\n");

			DisplayText("\"<i>No, my lady. This is a man's weapon in every definition of the word.</i>\" He beams and picks it up, pulling the blade from the sheath with a cold iron swish. \"<i>Notice how its long hard length stands tall and fierce upon a sturdy hilt designed for deep thrusts.</i>\"\n\n");

			DisplayText("Holding it up, it indeed gleams sensually in the sun. However, Raphael then points it downwards and directly at you, carefully tracing the point across your outfit. You can feel the cool edge glide across. \"<i>Notice the roving tip, how she searches for an opening upon a quivering body's wavering guard.</i>\"\n\n");
			...
			DisplayText("\"<i>Could I learn to use it?</i>\" You ask Raphael, who then begins to snicker into his fist and snorts unseemly.\n\n");

			DisplayText("When you don't flinch and just stare at him in earnest, the fox looks up with surprise. \"<i>Oh, you are serious about that?</i>\" He remarks and snaps out of it by adopting the coy frown typical of the wileful fox.\n\n");
			...
			DisplayText("\"<i>Shall we?</i>\" He jumps to his feet and extends you an arm.\n\n");

			DisplayText("When you ask him if you shouldn't be wearing armor instead of wafer-thin silk bodystockings, he laughs. \"<i>I assure you. For the Rapier, you are dressed perfectly.</i>\"\n\n");
		...
		if (player.stats.spe < 30 || Flags.list[FlagEnum.RAPHAEL_RAPIER_TRANING] === 0) {
			DisplayText("The two of you amble over to a mossy field. Raphael is up front and waving about his rapier playfully, grazing the tip through the flora and even slashing at a wasp. You can't seem to spot the insect flying around after he does that.\n\n");

			DisplayText("When you reach the center of the field however, Raphael throws the weapon to the ground with temperament and tells you to prepare.\n\n");

			DisplayText("\"<i>Aren't we going to use the weapon?</i>\" You ask him, causing him to smirk.\n\n");

			DisplayText("\"<i>So eager.</i>\" He snarls exitedly. \"<i>Whatever happened to foreplay?</i>\"\n\n");

			DisplayText("You try to explain that this isn't what you meant, but the fox shakes his head. \"<i>The sword is merely an extension of the rest of our body. The sharp of the edge is meaningless, when the arm that aims it and the feet that drive it suffer from inferior technique. It is... footwork.</i>\"\n\n");

			DisplayText("\"<i>Observe!</i>\" He exclaims and motions you to keep your distance. For just a moment, Raphael stands still as though caught in a moment of intense preparation.\n\n");
			...
			DisplayText("Raphael has the energy left to manage a theatrical flourish. He swerves his feet about, moves his hands with temperament to strike a few poses, before he puts a hand on the hip of his one straight leg and raises his sword into the air with victorious bravado. \"<i>Footwork!</i>\" He proclaims.\n\n");
			...
			DisplayText("\"<i>Join me my fine fae, and I will teach you to dance.</i>\" He states while discarding the blade.\n\n");

			DisplayText("\"<i>Dance?</i>\" You mumble with a smile, walking up to him and taking him by his hand.\n\n");

			DisplayText("\"<i>Dance!</i>\" He snarls certain, wrapping his paw around your waist and pulling you close to him.\n\n");
			...
			DisplayText("\"<i>I was very impressed by your earlier show.</i>\" You walk over and reassure him. Raphael looks somewhat discouraged.\n\n");

			DisplayText("\"<i>That was the most beautiful thing I ever saw. Do you really think I could ever master that much elegance and refinement, mister fox?</i>\" You chime appreciatively, running a teasing finger down his neck.\n\n");

			DisplayText("Raphael's smile returns for him. \"<i>Raphael!</i>\" He burrs. \"<i>And for someone like you I'd gladly suffer a dozen broken toes at once!</i>\" He boasts. \"<i>I will teach you grace becoming of your beauty yet!</i>\"\n\n");

			DisplayText("\"<i>Again!</i>\" He stands up and extends you an arm with renewed energy.");
			dynStats("str", -1, "tou", -1, "spe", 3, "sen", 1, "lus", 25);
		...
		//{Fencing practice variables: Speed 30-39 Must play out at least once}
			...
			DisplayText("When you ask him if he's sure that this is part of a dance. He chortles. \"<i>M'lady, it takes two to tango!</i>\"\n\n");

			DisplayText("\"<i>Vamanos!</i>\" The fox moves his feet about, leading you into dance.\n\n");
			...
			DisplayText("However, you dig your heels crudely into the moss and stop Raphael from moving any further.  Worried over the inclusion of the razor-sharp weapon, you object to the lack of safety.\n\n");
			...
			DisplayText("You gasp.  For the first time, Raphael forbids you something with harsh tone.  It's quite the departure.  You hang limply from his grasp, your " + Desc.Breast.describeBreastRow(player.torso.chest.get(0)) + " squashed against his chest like dough.\n\n");
		...
		//{Fencing practice variables: Speed 40-49, Must play out at least once}
			...
			DisplayText("<i>\"I think you have found the proper posture now.\"</i> he comments austerely, holding you by the " + Desc.Breast.describeBreastRow(player.torso.chest.get(0)) + " once more to make sure the angle of your back is correct. You push your rear a bit deeper into the fox's loins without needing to be guided in, enjoying the heat pulsing off the seat upon his manhood and how shamelessly you're grinding into him.  You actually begin to wonder if he himself doesn't notice and you struggle to contain the urge to wiggle around and bring attention to it.\n\n");

			DisplayText("<i>\"Think you can remember how this feels?\"</i> Raphael asks. Your flustered acknowledgement comes out in the form of a soft groan.\n\n");
		...
		//{Warning when you have 50 speed, played through all 3 variables and choose {Skill} at the opening picnic}
			...
			DisplayText("Regardless of the picnic's splendor, you are quickly drawn to Raphael's rapier again, having gained real feelings for the weapons during your previous sessions with it.  The fox chuckles.  <i>\"My fair fae; she is a woman of taste.  She knows what she wants.\"</i>\n\n");

			DisplayText("The weapon is lying between you.  You stare at it, Raphael stares at you.\n\n");

			DisplayText("<i>\"Would you like to own it?\"</i> he suddenly asks you.\n\n");

			DisplayText("You look up and smile, cautious about the offer.  You've built up a good rapport with the fox, but there's something mischievous about him, that still makes you playfully hesitant to completely trust him.  You ask him what you'd have to do for it.\n\n");

			DisplayText("<i>\"Spar with me over it.\"</i> The master fencer challenges you to what could only be an impossible fight.  He has taught you everything you know about fencing.\n\n");

			DisplayText("What do you do?");
			Flags.list[FlagEnum.RAPHAEL_RAPIER_TRANING] = 4;
			//[Fence] [Discuss]
			MainScreen.simpleChoices(["Fence", "Discus", "", "", ""], [fenceRaphaelSexily, fenceOfferChangeToDiscuss, null, null, null]);
			return;
		}
		return { next: RaphaelPicnicEnd };
	}

	private fenceOfferChangeToDiscuss() {
		DisplayText("You blush, intimidated, and change the subject.\n\n", true);
		//{Leads to conversation intro}
		RaphaelPicnicChooseThieving(false);
	}

	//{Fence leads to the final sex fencing scenes} 
	private fenceRaphaelSexily() {
		DisplayText().clear();
		//[sexy fencing finale]
		...
		DisplayText("The tall, fiery fox stands elegantly in the middle as a vivid, lean apparition around whom the bated fogs part.  Of course, you're not doing so bad either in your classy thin outfit, drifting through it like a crimson spirit.\n\n");
		...
		DisplayText("<i>\"En garde,\"</i> the fox states with confidence, calm playfulness, and certain arrogance.\n\n");
		...
		DisplayText("With a soft metallic scrape, the sword falls into his sheath and you bump clumsily into his body, while Raphael continues to stand firm and proud.  When he holds your wrist to the scabbard, he has effectively disarmed you.  You can't believe it's over when you get back up, using his body as a support.\n\n");
		//~~~ Next Page ~~~
		return { next: fenceRaphaelSexilyPtII };
	}

	private fenceRaphaelSexilyPtII() {
		DisplayText().clear();
		DisplayText("You're a little embarrassed at how easily the fox outdid you.  When you look up and stare him in the eyes however, Raphael is possessing of fantastic sportsmanship.\n\n");

		DisplayText("<i>\"My lady,\"</i> the fox states delicately, <i>\"that was truly magnificent.  You are a natural, I can tell.\"</i>\n\n");

		DisplayText("You stammer and are not so sure of that yourself, but the confidence carried in his voice quiets all self-doubt.  Maybe you were good.  Perhaps you could have beaten him in any other way, but just not yet with his weapon of choice.\n\n");
		...
		if (player.stats.int > 49) {
			...
			DisplayText("<i>\"I have never met a woman with your skills in the rapier,\"</i> he sighs at you.  <i>\"My lady, I must have you.  It would only be right.\"</i>\n\n");

			DisplayText("<i>\"Oh my, are you soliciting me, mister fox?  Right here in the wilds, crudely upon the moss?\"</i> you tease coyly.  <i>\"Wouldn't that mean I lose the game?\"</i>\n\n");

			DisplayText("<i>\"Some games are meant to be lost.\"</i>  He nuzzles you, putting his snout to your ear.  <i>\"It will be... exquisite. Lose yourself, in my capable hands.\"</i>");

			DisplayText("Do you let him?");
			//[Yes] [No])
			//yes to sex
	...
	//{speedsex}
	private RaphaelPostFenceSex() {
		DisplayText().clear();
		DisplayText("It's already too late to say no; you open your mouth to receive his agile tongue, accompanied by the slow approach of an inquisitive snout and slow breaths.  An intimate embrace follows as you sink slightly into the maw of his muzzle, to wrestle with his limber tongue.  The angle and pressure of the harsh leather sheath between your flushed lower lips might as well have tipped you into his body as you wrap your arms around his neck.  When Raphael drops the harsh intruder to the ground, his paws begin to roam freely on your body.  You can feel them travel everywhere, these soft hands of a swindler, through the thin silk of your outfit.  When they stop to rest on your " + Desc.Butt.describeButt(player) + ", he softly fondles your haunches.  You're already lost when he begins to nibble you softly on the neck, sometimes rearing up and whispering things past your ear in a dialect you still can't understand.\n\n");

		DisplayText("His whiskers feel smooth and soft when passing you by into a position behind you.  You must admit, you like this stance best as he lays his impertinent snout on your shoulder and licks your neck with slow laps.  When he starts to cup your " + Desc.Breast.describeBreastRow(player.torso.chest.get(0)) + " with the gentle touch of his vulpine paws, you throw your head into his shoulder, reach up to hold him around your neck and stretch your torso.  It brazenly presents all you have to offer while you arch your spine back, your " + Desc.Breast.describeBreastRow(player.torso.chest.get(0)) + " more sensitive to his circling swerves.  When your corset is uncinched and the zipper pulled down enough for the fox to flip the fabric down your tits, the touch of the cushions below his skillful fingers is exquisite.  They have a rough sandy texture - every brush an acute sensation.  Raphael wields it with such finesse however, that they feel like the cat's tongue upon a maiden's skin when he brushes by.  When he pinches your " + Desc.Breast.describeNipple(character, character.torso.chest.get(0)) + " between two such pads and flicks it about briskly enough to barely leave an impression on the edge of madness, you feel as though you just climaxed from them.  You shiver when he runs one of those paws down your crotch.  Mercifully, Raphael does not part the silken opening of your suit. Your " + Desc.Vagina.describeClit(player) + ", sensitive and erect, is spared his unique ministrations.\n\n");
		...
		DisplayText("When Raphael notices your attention, the time seems right for one of his one-liners.  'Do not fear the blade', 'look how the length stands firm upon the hilt', a lecture on the art of parrying, or the like is not forthcoming, however.  The fox says nothing instead and merely smiles knowingly at you from the side, knowing silent action is enough.  With his one remaining free arm, he claws around the silk of your womanhood and does indeed part the subtle opening of your interlapping folds. For a moment you gasp as his hot, slick cock falls freely into the denuded skin of your quaking " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ".  It shouldn't come as a surprise that he knows of the secret opening in the clothes: he gifted them to you after all, perhaps planning it all along.\n\n");

		DisplayText("You tremble as Raphael shifts back, angles his cock into the furrow of your womanhood and takes your moist opening in a single inward incursion.  ");
		Mod.Vagina.displayStretchVagina(player, 12, true);
		...
		DisplayText("It is how you spend the rest of that morning, filled a thousands times over and constantly driven past the edge of orgasmic bliss by the master fencer's trained thrusts.  His civilized smile has long since given way to the mean smirk of a sexual victor driving his victim to the edge of madness.");
		player.orgasm();
		return { next: postRaphaelCoitus };

	}

	private postRaphaelCoitus() {
		DisplayText().clear();
		Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00149] = 1;
		DisplayText("When you wake up on a bed of soft moss, Raphael has disappeared completely.\n\n");

		//({When player had reached the SPE fencing apex}
		if (Flags.list[FlagEnum.RAPHAEL_RAPIER_TRANING] === 4) {
			DisplayText("The only thing left behind is his rapier, sticking out of the moss.  He's bound it with his red sash around the length like a ribbon, as though he has now gifted it to you.  Perhaps it is his way of congratulating you.\n\n");
		...
		DisplayText("You return to camp, having cleaned up the picnic and taking the rations that were left with you.  You can't help but wonder if you'll ever see him again though.\n\n");
		//[Removes Raph from the game. 7 days later, the Quicksilver scene plays out.]
		Flags.list[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] = 7;
		//Next button if not taking Rapier
		if (Flags.list[FlagEnum.RAPHAEL_RAPIER_TRANING] != 4) return { next: playerMenu };
	}
	...
	private declinePuttingOutForRogues() {
		DisplayText().clear();
		DisplayText("<i>\"No.\"</i> You shake your head, dropping the atonal monosyllable as if it were the last note in a musical play.\n\n");

		DisplayText("The horny fox frowns.  It's like raising an invincible shield between you.  He reaches out towards your face and body, but he cannot touch anymore.\n\n");
		...
		DisplayText("When he gets off your body, you've won.  You've caught Raphael in his own game.  You've led him around.  He'd probably do anything for it, but cannot obtain it.\n\n");

		DisplayText("<i>\"Fine,\"</i> he mumbles.  He rolls off and sits sulking a few paces away from you.  It's the first time the young man fails to keep a cheerful attitude.\n\n");

		DisplayText("You smile, a bit sorry for the way you broke him, but you try to convince him that he shouldn't mope.  The two of you had fun.  It's getting late and you occupy yourself with cleaning up the picnic.\n\n");

		DisplayText("After a small while, the fox finally speaks.  <i>\"It seems like there isn't anything more I can teach you,\"</i> the fox claims.  <i>\"... I'm proud of you, my greatest student.\"</i>\n\n");

		DisplayText("You turn around to smile at him.  However, Raphael has vanished.\n\n");

		//({When player had reached the SPE fencing apex}
		if (Flags.list[FlagEnum.RAPHAEL_RAPIER_TRANING] === 4) {
			DisplayText("The only thing left behind is his rapier, sticking out of the moss. He's bound it with his red sash around the length like a ribbon, like he has now gifted it to you. Perhaps it is his way of congratulating you.\n\n");
			//[Weapon: Rapier. Speed, instead of strength, influences the damage rating. Never as strong as the heavier weapons or sword, but works great with speed & evasion, encouraged by the rapier.])
			inventory.takeItem(weapons.RRAPIER, playerMenu);
		...
		DisplayText("You return to camp, having cleaned up the picnic and taking the rations that were left with you.  You can't help but wonder if you'll ever see him again.\n\n");
	...
	private RaphaelPicnicChooseThieving(newl: boolean = true) {
		if (newl === true) DisplayText().clear();
		...
		if (Flags.list[FlagEnum.RAPHAEL_INTELLIGENCE_TRAINING] === 0) {
			...
			DisplayText("<i>\"Certainly not!\"</i>  The fox grins.  <i>\"People often underestimate how tiring proper lovemaking is.  I restrict myself to a mere tryst a day.  No more, often less.  You ladies would be the death of me otherwise.\"</i>\n\n");
			...
			DisplayText("<i>\"What else would you like revealed?\"</i> Raphael quips suggestively, smiling.\n\n");
		...
		if (player.stats.int < 30 || Flags.list[FlagEnum.RAPHAEL_INTELLIGENCE_TRAINING] === 0) {
			DisplayText("<i>\"How do pickpockets do it?\"</i> you blurt out bashfully; the first thing coming to mind is how it's possible for them to reach into pockets without the victims even sensing it.\n\n");
			...
			DisplayText("When you ask him what's the matter, Raphael merely blinks and smiles genuinely.  <i>\"Only now do I notice, just how sexy you are.\"</i> He manages a surprising amount of baritone for a voice as young as his.\n\n");
			...
			DisplayText("You respond with a embarrassed \"<i>uh-huh</i>\" as you turn your back to the fox and shield your breasts. The taste of his saliva is still on your tongue, surprisingly sweet. You would have protested, if you hadn't asked for a demonstration of his skills earlier.\n\n");
		...
		//{Picnic Thieving 30-39 Int. Must play out at least once.}
			...
			DisplayText("<i>\"Are rogues born to be what they are?\"</i> you ask him curiously.\n\n");
			...
			DisplayText("<i>\"Moves like the ones you're employing right now?\"</i> you ask the fox cunningly.  Raphael smiles like you caught him, but after flashing you his flawless white teeth through the curl of his lips, simply continues.\n\n");
			...
			DisplayText("When you ask him what he means, Raphael merely winks.  <i>\"You've come this far, I'm sure you'll figure it out. A proper rogue knows it by heart.\"</i>\n\n");
			dynStats("str", -1, "tou", -1, "int", 3, "sen", 1, "lus", 40);
		...
		//{Picnic Thieving at 40-49 Int. Must play out at least once.}
			...
			DisplayText("<i>\"Are you always this much of a charmer?\"</i> you wonder about Raphael, and how he always manages to keep his cool.\n\n");
			DisplayText("He draws a weary smile, not completely disheartened.  <i>\"It is my natural state of being... though I must confess; it takes effort to stay on top of my game.\"</i>\n\n");

			DisplayText("When you ask him what he does for relaxation, the rogue raises his shoulders.  <i>\"Being less than on the very top of my game.  An audience will never know the difference however.  Working the crowds is vital.\"</i>  When you ask him if that includes you, he merely grins.\n\n");
			...
			DisplayText("You crack up and laugh openly at his antics.  Suddenly, you don't have as hard a time imagining Raphael relaxing.\n\n");
		...
		//{Int Thieving Apex Warning: When the PC's intelligence is at 50 or higher.}
			...
			DisplayText("<i>\"Ready for another chat, mademoiselle?\"</i> he drawls lusciously.\n\n");

			DisplayText("You realize that he's preparing for the long haul.  You might not get off quite as easily as earlier, should you decide to flirt with him again.\n\n");

			DisplayText("It occurs to you to change the subject to that of the rapier at his side but you are equally drawn to engaging the fox and satisfying your... curiosity.\n\n");
			...
			MainScreen.simpleChoices(["Fencing", "Flirt", "", "", ""], [RaphaelPicnicSkill, thieveryEnding, null, null, null]);
			return;
		}
		return { next: playerMenu };
	}
	...
	private thieveryEnding() {
		...
		DisplayText("Raphael himself remains silent.  There is no need to move as he looks upon you with a dreamy glaze in his eyes, sideways from his pointed snout.  He's enjoying his sunbath.\n\n");
		...
		DisplayText("You hold your breath as the rogue closes in on you, putting his face close to yours.  He growls softly at you and you moan back at him, but only after allowing yourself to do so.\n\n");
		...
		DisplayText("As he kisses you across the leg, the thing you feel most clearly are his whiskers and soft wet nose gliding across your thin silk attire.  He keeps a respectable distance from your groin, skipping past it with a feathery touch.  Instead, his inquisitive snout sniffs past your tummy and " + Desc.Breast.describeAllBreasts(player) + ".  Meanwhile, you have the opportunity to admire him, his soft fur, his skillful touches and his fine wardrobe.  The sash falls in over your body while the supple brown and red leather gives the graceful body moving in over you a refined quality.  Spending time with him has rubbed off on you.  Only now do you appreciate the value of certain baubles that cling to his leathered threads, thinking like a pickpocket.\n\n");

		DisplayText("A brooch that can only be described as priceless decorates the soft silk sash.  ");
		//({When player has not reached the apex of fencing lessons/Optional: and also has less than 60 speed}
		if (Flags.list[FlagEnum.RAPHAEL_RAPIER_TRANING] != 4) DisplayText("You try to reach for the magnificent ornament hanging off his hips, but as the fox continues to move, you lack the nimbleness to remove it.\n\n");
		//({When player has also reached the apex of fencing lessons
		else DisplayText("You reach and fondle the magnificent ornament hanging off his hips. With a few nimble flicks of your fingers, you manage to get it off.\n\n");

		DisplayText("Realizing you've allowed yourself to be distracted, you refocus.  Raphael has his paws on your breasts and is about to kiss the top of them.  Throwing him off, you sigh, brush his hands away while raising your own towards your head.  Like this, you stretch out your upper body across the moss, and cross your hands to signify helplessness.  It presents your " + Desc.Breast.describeBreastRow(player.torso.chest.get(0)) + " even more invitingly, but you've elegantly dispersed his efforts.  To touch them again on such short notice would be to flaw his otherwise refined lovemaking.  Instead, the fox crawls in over you completely on hands and knees and looks you in the eyes.  He realizes you're still very much in the game and to take you roughly on the moss instead of sweeping you off your feet would be a thing unbecoming of the refined fox.  He needs your consent to win the game.\n\n");

		DisplayText("You're enjoying yourself and so is Raphael, for as long as you keep the illusion up he might manage to woo his way into your pants.  He's so fragile right now.  You're almost literally holding the horny fox's desires and pride in the palm of your hand.  You are in complete control.\n\n");

		DisplayText("<i>\"I have never before met a woman of your breathtaking insight.  Of such deep beauty.\"</i>  He sighs at you.  <i>\"My lady, I must have you,\"</i> he burrs.\n\n");

		DisplayText("<i>\"Oh my, are you soliciting me, mister fox? Right here in the wilds, crudely upon the moss?\"</i> you answer coyly.  <i>\"Wouldn't that mean I lose the game?\"</i>\n\n");
	...
	private RaphaelThieverySmex() {
		DisplayText().clear();
		DisplayText("You need no words.  You place your hand on the back of Raphael's neck and pull his lisping tongue closer to your ears.  After that, you lose yourself to the weight of a man atop you as the fox starts tending to your quivering body.\n\n");

		DisplayText("Finally the fragile tension between you breaks.  Raphael undoes your zipper and roughly pulls your bodysuit down, revealing your " + Desc.Breast.describeAllBreasts(player) + ". You're stunned; the fine, abrasive texture of his nimble fingers' pads run in eager circles across your sensitive neck, from whence moans have begun to flow. His soft fur brushes across your body as the fox moves downwards.  Gingerly, he puts his maw down on your right breast and suckles once, but in the span of an agonizingly long time.  The " + Desc.Breast.describeNipple(character, character.torso.chest.get(0)) + " is sucked into his toothy mouth, which he puts down on the nub carefully.  The little points scrape by sensually, before your nipple slips through them.\n\n");

		DisplayText("You gasp when he spreads your legs by placing his in between and parting them. When he fondles you down there, fingers rubbing into your flushed  " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", you suddenly feel the wind passing through the hot bare inners of your parted folds.  Raphael has opened the silken opening of your suit. It shouldn't come as a shock.  He gifted them after all, perhaps planning for it all along.  Another surprise follows as you can feel something hot and rigid standing off his body and lying on top of yours. Raphael's vulpine cock is resting in the hollow of your bellybutton, through the fly of his pants.  The bright red, smooth tip rides up your tummy admiringly.  It lacks the mushroom shaped dome that human men have and instead, his cock is pointed and tapered, much like the weapons he prefers.  You can also feel a subtle, but noticeable canine bulb at the base, throbbing against your sensitive loins.\n\n");
	...
	private RaphaelThieverySmexPtII() {
		...
		if (Flags.list[FlagEnum.RAPHAEL_INTELLIGENCE_TRAINING] === 4) {
			...
			//[Optional Perk: Misdirection. Intelligence adds to the chance to evade. Turns you into a true rogue together with the bodysuit.])
			player.createPerk(PerkLib.Misdirection, 0, 0, 0, 0);
			DisplayText("(Gained Perk: Misdirection!)\n\n");
		}
		DisplayText("You return to camp, having cleaned up the picnic and taking the rations that were left with you. You can't help but wonder if you'll ever see him again though.");
	...
	//[Quicksilver scene]
	private quiksilverFawkesEndGame() {
		DisplayText().clear();
		DisplayText("You wake up to the sound of an ominous cry in the distance, like that of a howling wolf.  It can only mean trouble and you jump out of bed.\n\n");
		...
		DisplayText("<i>\"Please, you must help me!\"</i> he begs of you.  <i>\"They're trying to frame me!  I'm innocent!\"</i>\n\n");

		//({If player rejected him earlier}
		if (Flags.list[FlagEnum.REJECTED_RAPHAEL] === 1) {
			DisplayText("You're immediately confused and you ask him what's going on, but the fox is out of breath.  What's more, a similar howl now calls out from the opposite direction too!  Whatever these sounds are or this fog is, they have the camp surrounded!\n\n");
		}
		...
		DisplayText("<i>\"They hound me night and day!  I can't outrun them any longer!\"</i> he wheezes.  <i>\"Please!  You don't know what they'll do to me when they catch me!\"</i>\n\n");

		DisplayText("You don't have the chance to respond before the sounds of snapping branches and rustling leaves near camp.  Raphael jumps up and dives into the barrel you keep in camp for the storage of water, hiding himself and leaving you to face the oncoming threats by yourself.\n\n");
		...
		DisplayText("<i>\"Smelled of elderberries, huh?\"</i> it snarls.  <i>\"And what was that about my mother?\"</i> When you open your eyes again, you're greeted by a row of razor-sharp white teeth, set in a pitch-black face. Above them, two smoldering yellow eyes stare down on you, infuriated.\n\n");

		DisplayText("<i>\"Hati!\"</i> an identical voice calls out from elsewhere.\n\n");

		DisplayText("Suddenly, the teeth above you vanish and the eyes grow soft and surprised.  <i>\"Oh!\"</i> the shadow exclaims, its expression apologetic and startled.\n\n");

		DisplayText("It removes itself from atop your body.  After having the breath knocked out of you, you take a moment to recover it and finally manage to get back up and meet the two monsters of the mist.");

		//~Next Page~
		return { next: QuiksilverFawkesPtII };
	}

	private QuiksilverFawkesPtII() {
		DisplayText().clear();
		...
		DisplayText("You jump up when one of them nudges his nose into your ass and sniffs you.\n\n");
		...
		DisplayText("<i>\"We're tracking down a fugitive, a red fox.  Urta's orders,\"</i> he explains.  <i>\"Goes by the name of Raphael. Raphael Fagin Soleil. Don't suppose you've seen him?\"</i>  He stares at you with inquisitive yellow eyes beneath broad brows that give him a permanent serious frown.\n\n");

		DisplayText("When you ask him what crimes Raphael has committed, the wolf shakes his head.  <i>\"Debauchery.\"</i>  You raise one eyebrow, surprised that debauchery could be a crime in a place like Mareth.\n\n");

		DisplayText("<i>\"Must have gone on a real romp, that one,\"</i> you quip.\n\n");

		DisplayText("Skoll responds, amused.  <i>\"We need to set standards for ourselves and he crossed them.  It's how we separate ourselves from the demons.  Petty theft, highway robbery, and breaking and entering are counted amongst his other crimes.  Not that their severity should matter to the course of our investigation.\"</i>\n\n");

		//({If player removed him earlier}
		if (Flags.list[FlagEnum.REJECTED_RAPHAEL] === 1) DisplayText("<i>\"I can smell traces of him,\"</i> Hati states, nearing the waterbarrel, <i>\"but it's hard to tell.  There're too many other scents here.\"</i>\n\n");
		//Played around/special ending
		else DisplayText("Hati breaks into the conversation.  <i>\"She must know.  The entire camp reeks of him. She has his scent on her.  I can smell strong trails leading towards a nearby meadow.  A bit stale.  Might have been a while ago.\"</i>  He nears the water barrel.  <i>\"...but it's hard to tell. There's too many other scents here.\"</i>\n\n");

		DisplayText("Your heart jumps as Hati manages to discover a pile of Raphael's clothes, near the barrel.  It's irrefutable proof.  Raphael must have put them there to throw the wolves off.  Hati gives you a look of dissapointment.  <i>\"He's nude. They must have slept together.\"</i>");
		//({If player is wearing red bodysuit}
		if (player.inventory.equipment.armor.displayName === "red, high-society bodysuit") DisplayText("  Suddenly, his eyes and ears perk.  <i>\"Isn't that the stolen clothing of the widow Camembert that she's wearing? You know, before she mysteriously dropped the charges after a nightly visit from the fox? Should we confiscate it anyway?\"</i>");
		DisplayText("\n\n");

		DisplayText("<i>\"Come now, Hati. No need to concern ourselves with that,\"</i> Skoll reproves.  <i>\"I'm sure we can rely on our upstanding citizens not to actively harbor a criminal.\"</i>  He looms in over you.\n\n");
		...
		DisplayText("You glance at your waterbarrel.  Hati is keeping an obsessive watch over the water's surface, like he's already closed in on the fox.\n\n");

		DisplayText("You could sell Raphael out, or you could cover for him.  What do you do?");

		//[Cover] [Sell out] 
		MainScreen.simpleChoices(["Cover", "Sell Out", "", "", ""], [coverForRaphael, betrayRaphael, null, null, null]);
	}

	//{PC chooses to cover for Raphael}
	private coverForRaphael() {
		DisplayText().clear();
		DisplayText("You stare back into the wolf's yellow eyes, keep a stiff back and answer him.  <i>\"I'm not seeing him right now.\"</i>\n\n");

		//({Player corruption below 30}
		if (player.stats.cor < 30) DisplayText("You instantly feel bad for lying to the wolf. More so because he doesn't get troubled, disappointed or vexed.  He seems to instantly believe you... or at least, he wants to believe you, even though being a detective probably keeps him cynical.  You'll only be adding to that cynicism if he discovers your lie.\n\n");

		DisplayText("<i>\"Cute answer,\"</i> his partner remarks, taking a break from staring at the water barrel.\n\n");
		...
		//({If cor is below 30}
		if (player.stats.cor < 30) DisplayText("  You slowly shake your head, tortured by the trusting twinkle in the wolf's soft yellow eyes.");
		DisplayText("\n\n");

		DisplayText("<i>\"The trail is strongest over towards the meadows.  He might have gone there.\"</i> Hati remarks after circling your camp one more time.\n\n");

		DisplayText("<i>\"I hope you will let us know when you see him again...\"</i>  Skoll nods at you, his feral features pulling away from you.\n\n");

		DisplayText("Skoll picks up Raphael's clothes.  The two brothers then get on all four of their limbs and speed off into the fog.  Fierce, dangerous beasts they are.\n\n");
		//~~~ Next page ~~~
		return { next: coverForRaphaelII };
	}

	private coverForRaphaelII() {
		DisplayText().clear();

		DisplayText("It doesn't take more than a minute for Raphael to make use of the wolves' departure.  Not even bothering to climb out with dignity, he tips the barrel over as he rushes out of the water.  Like a drowned cat, he climbs out coughing and gasping for air on all fours.\n\n");

		DisplayText("He hasn't lost his ways however.  The first words out of him are as suave and composed as ever.\n\n");

		DisplayText("<i>\"Merci, mademoiselle,\"</i> he wheezes softly.  <i>\"You have whisked me away from the snatches of great injustice, as well as a fair share of humiliation.  Normally I am capable of outrunning them myself, but it seems that faith has blessed me with a guardian angel on the day I find myself less capable.\"</i>\n\n");
		...
		DisplayText("<i>\"No.  It is nothing like that,\"</i> he splutters, in response to one of your rants.  <i>\"We go way back.  The wolves and Urta, they carry out a personal vendatta against me that has no bearing on the law.\"</i>\n\n");

		DisplayText("You spend some time considering the dubiosity of this revelation.\n\n");
		...
		DisplayText("<i>\"Mademoiselle, this day, I owe you a great boon.  The russet rogue never forgets a debt.  Visit Tel'Adre after things die down and look for the orphanage.  You will be welcomed like a queen! Ah-ha!\"</i> he exclaims, before hopping down the wall.\n\n");

		DisplayText("You blink as he fades off into the mist and you're left to absorb the morning's events by yourself.  For a minute you're concerned with the wolves, but at least you've managed to keep Raphael as a friend.  Though, you are curious what he means by 'orphanage'.");
		//Concludify things
		Flags.list[FlagEnum.RAPHEAL_COUNTDOWN_TIMER] = -2;
		return { next: playerMenu };
	}
	//{PC chooses to betray Raphael}
	private betrayRaphael() {
		DisplayText().clear();
		DisplayText("<i>\"The barrel,\"</i> you admit, the answer slipping from your lips in face of the wolf.\n\n");
		...
		DisplayText("The two twins look at each other, then grin playfully.  One of them leans in over the barrel and puts his head directly over the water's surface, peering down.\n\n");
		...
		DisplayText("<i>\"Tell it to Urta and the judge,\"</i> one of the wolves announces loudly, causing the other to chuckle.  <i>\"You have the right to remain silent. Anything you say will make us hold something against you, not necessarily in the court of law.\"</i> Sullenly, Raphael quiets down inside the sealed barrel.\n\n");

		DisplayText("Effortlessly, one of the wolves lifts the barrel up onto his shoulder.  Before he carries it off, he throws one more look backwards and beams at you. <i>\"Check in with Urta.  She will reimburse you for the barrel, along with rewarding you with the outstanding bounty on our foxy friend.  Visit even later than that and you might even be allowed to enjoy his sentence.\"</i>\n\n");

		DisplayText("The other spends a little longer regarding you, wagging his tail playfully and smiling a friendly smile at you.  He then looks back and sees his brother heading in the wrong way.  <i>\"We're not heading back to Tel'Adre right away?\"</i>  His voice is almost as low as his brother's.\n\n");

		DisplayText("<i>\"We caught him in record time,\"</i> Skoll remarks.  <i>\"I'm sure Urta won't mind it if we keep him for ourselves for just a day.  You know... collect evidence... interrogate the culprit...\"</i>  His voice is carried off by the mist.\n\n");

		DisplayText("Hati throws you one more amicable glance before turning, getting on all fours, and running off.\n\n");

		DisplayText("You blink as he fades and you're left to reflect on these morning's events by yourself.  For a minute you're concerned for the fox, but the wolves don't seem like the types to harm him... much.  You are curious what they plan to do with him and what his 'sentence' might be, though.");
	...
	private raphaelIntroPageTwo() {
		DisplayText().clear();
		DisplayText("\"<i>This place isn't really an orphanage, is it?</i>\" you ascertain sagely, still waiting for a reply.");
	...
	private getTheFuckOutOfDodge() {
		DisplayText().clear();
		DisplayText("You politely inform Raphael that you have to go, and though he seems crestfallen, he demurs, \"<i>Very well then, ma chère.  I shall await you in the hours of late evening, in case you change your mind.</i>\"");
		DisplayText("\n\nWell, that went well.");
	...
	private kissRaphaelFirstTimeOrphanage() {
		...
		Mod.Vagina.displayStretchVagina(player, 5, true, true, false);
	...
	//After Sex1
	private followupToFirstTimeOrphanageRaphSex() {
		DisplayText().clear();
	...
	//Cunnilingus
	private cunnilingusWithRaphael() {
		DisplayText().clear();
		...
		DisplayText("\n\nWith his paws, Raphael raises up your butt and crotch just a few inches into the air, as if elevating it for a special occasion.  It encourages you to bend your knees and spread your legs, opening yourself up and presenting your womanhood fully.  Lacking all shame and still looking you into the eyes, the fox then does the lewdest thing.  Maintaining eye contact and without blinking, he rolls out his long red tongue and brushes it against your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ".  The sensation of the bawdy rough thing lapping up your sensitive gash's gathered dew takes you completely by surprise and you let your head fall back and moan.  It feels like he's worshipping you by lapping through the trembling fruit.  At the very zenith of his lashes, he takes extra time to twirl around your " + Desc.Vagina.describeClit(player) + " in a vortex of swirling tastebuds.");
		...
		if (player.torso.vaginas.get(0).wetness < 3) DisplayText("your cunt fluttering");
		//({Medium wetness multiplier}
		else if (player.torso.vaginas.get(0).wetness < 4) DisplayText("sending out a small spurt of femcum landing on your belly");
		//({High wetness multiplier}
		else DisplayText("gushing a torrent of femcum into the air");
		DisplayText(".  Raphael backs off a bit afterwards, giving you a few more laps of admiration around the throbbing fissure and cleaning you of any spillage. He ends with a tiny kiss, just inside your inner thigh.");
		player.orgasm();
		player.stats.sens += -2;
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	private girlOnTopOfRedFoxesOhMy() {
		DisplayText().clear();
		...
		DisplayText("\n\nYou swipe the condom out of the fox's hand and waste no time applying it.  In fact, you manage to do a good job of giving it a sensual part within your lovemaking, which makes you feel all smart.  Between spread legs and right in front of your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", you lay it in within your hand and slowly roll your fist across his cock; giving him a taste of what is to come.  Raphael gasps softly at the sight as you do it. The condom fogs up instantly around his muzzled red bone.");
		...
		DisplayText("\n\nRaphael's cock is something else. It must be the bone running through it that gives it its firmness.  Instead of stuffing yourself with a nondescript roll of meat, you can clearly feel him penetrating you with his well defined and rock-hard shaft.  With the faintest twitch of the hip, you can change its angle and run it through in an entirely different manner and you eagerly start experimenting with what feels best.  Eventually you settle on jutting your ass backwards and pressing the middlemost of his solid length against the front wall of your [vagina] while the tip bottoms out against the back.  The position does not go unnoticed to Raph, and he trails his paws along your " + Desc.Butt.describeButt(player) + ".  You begin to bounce around on top him, jamming him into you more firmly and more securely with every rut in.  Raphael supports you by either grabbing you by the ass or cupping your tits, playing with them with his sandy paws.  He sinks in heavily, smoothly, with every gulp of your tightening cunt accompanied by a little wet slush around his hard manhood.");

		DisplayText("\n\nDesperate for that little bit more, you reach down and start fondeling your " + Desc.Vagina.describeClit(player) + " too.  Your button brushing in against Raphael's soft white pubes tickles.");
	...
	private talkWithRedFoxLooksOutHesASpy() {
		DisplayText().clear();
	...
	private raphaelOrphanageSexMenu() {
