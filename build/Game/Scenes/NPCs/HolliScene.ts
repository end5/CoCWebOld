export class HolliScene extends NPCAwareContent {
		...
		if (Flags.list[FlagEnum.FOUGHT_HOLLI] > 0) {
			...
			if (output) {
				...
				if (Flags.list[FlagEnum.HOLLI_SUBMISSIVE] === 1) {
					DisplayText("\n\n\"<i>Welcome back, [name].  ");
					if (Flags.list[FlagEnum.TIMES_FUCKED_FLOWER] > 0 && randInt(3) === 0) DisplayText("Did you want to fertilize me again?  My flower is ready and waiting...");
					else if (Flags.list[FlagEnum.TIMES_RIDDEN_FLOWER] > 0 && randInt(3) === 0) DisplayText("Did you want to use my tentacles again?  They've been restless without you...");
					else if (randInt(2) === 0) DisplayText("Did you come for a drink?  I've built up lots of nectar for you.  Surely, a sip would be refreshing for you...");
					else DisplayText("How may I serve you?  Come, give me a command so that I can fulfill it for you.");
					DisplayText("</i>\"");

				}
				else {
					DisplayText("\n\n\"<i>Welcome back, [name].  ");
					if (Flags.list[FlagEnum.TIMES_FUCKED_FLOWER] > 0 && randInt(3) === 0) DisplayText("Did you want to fertilize me again?  Gardening is such exhausting work, but so rewarding...");
					else if (Flags.list[FlagEnum.TIMES_RIDDEN_FLOWER] > 0 && randInt(2) === 0) DisplayText("Did you come back to play again?  My tentacles are so restless.");
					else if (randInt(2) === 0) DisplayText("Did you come for a drink?  I've built up lots of nectar for you.  Surely a hungry adventurer like yourself needs a sip, right?");
					else DisplayText("Do you want to play?  Come on, taste Mother Marae's finest creation.");
					DisplayText("</i>\"");
				}
				//Extra line on description:  
				if (Flags.list[FlagEnum.HOLLI_FRUIT] > 0) {
					if (Flags.list[FlagEnum.HOLLI_FRUIT] === 1) DisplayText("\n\nA large, purple fruit hangs from Holli's branches, twisting softly in the breeze.  It looks succulent and ripe.");
					...
					//Optional Addition: 
					if (Flags.list[FlagEnum.HOLLI_FRUIT_EXPLAINED] > 0) DisplayText("  A very small, strange part of you actually feels proud to have made something like that with the tree-woman.");
				}
			}
	...
	private fertilizeHolli(cock: boolean = true): void {
		...
		if (!cock && player.torso.vaginas.count > 0) {
		...
		if (odds >= randInt(101)) {
			Flags.list[FlagEnum.HOLLI_FRUIT] += 1 + randInt(Math.floor(odds / 10));
			trace("Holli pregged!");
		}
	}
	...
	public getASprout(): void {
		...
		if (Flags.list[FlagEnum.CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE] === 2) DisplayText("  She even promised to plant a tree in your camp...");
		DisplayText("  You look closer and see purple veins on the undersides of the leaves, throbbing with what can only be the tainted liquids that suffuse much of this planet.  Fully grown, it might spread its corruption to your camp.\n\n<b>Do you destroy it?</b>");
	...
	public fuckPlantGrowsToLevel2(): void {
		...
		if (player.gender === Gender.NONE) DisplayText("  Sucks to be it!  You don't waste time with stupid shit, like genitals.");
		DisplayText("  Is this Marae's idea of a gift?");
		if (player.gender > 0) DisplayText("  Well, you could give it a whirl... just being around it seems to slowly turn you on.");
	...
	private fuckFuckingFuckFlowerP2(): void {
		...
		DisplayText(" and form a sticky puddle.  Now moaning, you grab the stem and start to pump the sheath up and down, using the plant like the cock-toy it was meant to be.  Lewd, wet squishes ring out through the air,");
		...
		if (player.cumQ() > 1500) DisplayText("  Soon, the meager stem cannot swallow it all.  Spunk froths and squirts out around the base of your tool for a moment, and then a veritable tide of the stuff gushes out to soak  you.");
	...
	private rideDatFuckingFukkFlowerP2(): void {
		...
		if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 1) DisplayText("ir");
		DisplayText(" swell as you search for the lust-tightened skin of your [nipples].  They seem almost as sensitive as little clits, and as soon as you find them, you immediately set to circling them slowly, occasionally tugging and twisting when a particularly powerful sensation works through your pleasure-wracked body.");
		if (player.lactationQ() > 50) {
			DisplayText("  One after the other, they begin to ");
			if (player.lactationQ() < 150) DisplayText("trickle");
			else if (player.lactationQ() < 500) DisplayText("leak");
			else DisplayText("spray torrents of");
			DisplayText(" milk.");
		}
		DisplayText("  You moan in unabashed delight, your mouth hanging open in a mindless, blissful 'o' in between vocalizations.  If your friends back home could see you now, writhing in the dirt, face open in simple, animal pleasure, and perhaps worst of all, fucking a plant, you'd die of ");
		if (Flags.list[FlagEnum.PC_FETISH] === 0 && player.stats.cor + player.stats.lib < 150) DisplayText("embarrassment");
		...
		if (player.torso.cocks.count > 0) DisplayText("  [EachCock] explodes in release, pumping heavy flows of seed into the petals, even as y");
		else DisplayText("Y");
		DisplayText("our quivering pussy, already sopping wet, releases the last of your juices in a soaked ");
		if (player.cumQ() < 50) DisplayText("drizzle");
		else if (player.cumQ() < 250) DisplayText("stream");
		else if (player.cumQ() < 750) DisplayText("spray");
		else DisplayText("torrent");
		DisplayText(".  The tentacles inside you curl in on themselves and slowly pull back from the well-creamed quim, scraping out as much fluid as possible from your oversensitive honey-pot.  The binding on your [clit] releases, which of course sets off a whole new wave of rippling pleasure-contractions in your cunt.");
	...
	public flowerGrowsToP3(): void {
		...
		if (player.torso.vaginas.count > 0 && player.stats.lust >= 33) ride = rideTheWalrusP3;
		//[Fuck flower] [Drink Sap] [Ride Tentacles] [{Torch It}] [Leave It]
		MainScreen.simpleChoices(["Fuck Flower", "Drink Sap", "Ride Tentacle", "Torch It", "Leave It"], [fuck, drinkThePlantGirlsSap, ride, burnIt, playerMenu]);
	}
	...
	private fuckTheFlower(): void {
		...
		if (player.cumQ() > 1500) DisplayText("  A wave of your thick gunk rolls out from under your pumping cock - you've filled the tree beyond even its capacity, and the thick puddle forming beneath your [feet] is all the proof you need of your virility.");
		if (player.torso.vaginas.count > 0) {
			DisplayText("  Your hungry cunt, long denied in this exchange, trembles as it ");
			if (player.torso.vaginas.get(0).wetness < 4) DisplayText("leaks");
			else DisplayText("sprays");
			DisplayText(" your own female fluids in sympathy.");
		}
		DisplayText("  After what feels like forever, the spurting seed no longer flows from your long-abused dick, and as your member softens, the tentacles swing you away, setting you down softly.");
	...
	private torchP3Tree(): void {
		...
		//[(nerdfire)
		if (player.statusAffects.has(StatusAffectType.KnowsWhitefire)) DisplayText("Closing your eyes to focus, you gather your energies, and unleash your white, supernatural flames upon the thing.");
		//(bromancefire)
		else if (player.perks.has(PerkType.FireLord)) DisplayText("Sucking in your breath, you inflate your chest and stir the embers of the jaguar demon's gift.  Dredging up an enormous pine-green fireball, you exhale and launch it at the tree.");
		else DisplayText("Sucking in your breath, you inflate your chest and stir the embers of your fiery gift.  Dredging up an enormous fireball, you exhale and launch it at the tree.");
		DisplayText("  It goes up by the torch, though the fire starts to fade sooner than you would expect.  You huff, but repeat the action, layering a new blaze atop the previous one; you put all your concentration into this new conflagration, stoking the inferno with every ounce of your ");
		if (player.statusAffects.has(StatusAffectType.KnowsWhitefire)) DisplayText("magical will");
		else DisplayText("breath");
		DisplayText(".");
	...
	public treePhaseFourGo(): void {
		...
		if (Flags.list[FlagEnum.TIMES_FUCKED_FLOWER] + Flags.list[FlagEnum.TIMES_RIDDEN_FLOWER] > 0) DisplayText("Did you enjoy feeding me?  You should have seen the expression on your face when you were cumming in me.  Your eyes were rolling back and your mouth was just hanging open.  Do you know what it made me want to do?");
		else DisplayText("Did you enjoy watching me grow?  You should have fed me.  It was so much harder to grow without proper nutrients.  Do you know what I thought about during all the time I spent waiting?  Do you know what it made me want to do?");
		DisplayText("</i>\"");
	...
	public JojoTransformAndRollOut(): void {
		...
		if (player.stats.cor < 25) DisplayText("Not willing to lie to your friend, you also admit that she seems to have changed after your actions in the factory - that, though you were trying to render the tanks permanently inoperable, the spillage you created pushed the cthonic spirit over the edge.");
		else DisplayText("Though, you do leave out the precise, needless details of what caused the change.");
		DisplayText("  The monk's eyes widen in surprise and surmise.");
		...
		else DisplayText("And even knowing that this could be the case, you still chose to fornicate with it?  [name]... I cannot say that I am not disappointed.  This place is not safe for me anymore.  For now, I will return to the forest.");
		DisplayText("</i>\"");

		DisplayText("Jojo looks at you sadly; any argument you try to make is met with a resigned shake of his head.  \"<i>If the creature truly is the demonic offspring of a corrupted earth spirit, doubtless it will be powerful and the wrath of its parent severe.");
		if (player.level < 20) DisplayText("  Nonetheless, if you find yourself in a position where you must confront this demon or perish, call upon me.  I will render what aid I can, though I prefer it not be in vain.");
		else DisplayText("  Though, a mortal of your stature and resources may be able to destroy her and escape the reprisal.  I would counsel against direct confrontation until you are sure of victory, but if you must fight, then call upon me and you will have what aid I can give.");
		DisplayText("</i>\"  So saying, he presses a small talisman into your hand.");

		DisplayText("\"<i>Until then, you will find me in the forest if you need my assistance.</i>\"  Jojo makes a polite bow and departs.");
		DisplayText("(<b>Jojo is no longer a companion!</b>)");
	...
	private slapAmilysWhoreFace(): void {
		...
		if (Flags.list[FlagEnum.AMILY_NOT_FURRY] === 0) DisplayText("mouse-");
		DisplayText("girl says, eyes wet with tears, \"<i>but I don't.  Goodbye.</i>\"");
	...
	private fuckHolliInZeFlowerPuss(): void {
		...
		if (player.torso.cocks.count > 1) {
			...
			else DisplayText("The rest of your penises splay");
			DisplayText(" to the side, smearing furrows of the stuff around as they slide over the butter-smooth skin.");
		}
		DisplayText(" The demon dryad's hand reaches below to ");
		...
		if (player.cumQ() >= 1500) DisplayText("  Before you know it, there's a lake around the two of you and a small river running downhill away from camp.");
		DisplayText("  A mischievous grin breaks out under the goopy facial, followed by an unnaturally long tongue that pulls gobs of the stuff into the dryad's hungry mouth.  Once she can see again, the girl gives her tits a squeeze and affectionately kisses your cock, practically worshipping it with sloppy-sweet licks.");
		...
		if (player.tallness < 60) DisplayText("tits");
		else DisplayText("arms");
		DisplayText(".  [EachCock], instead of slamming into the bark painfully, slots into something warm and wet, pulsating and pliant.  A clingy, tight hold that ripples gently around your member");
		if (player.torso.cocks.count > 1) DisplayText("s, massaging the bundle of cocks with uncommon ferver");
		else DisplayText(", massing your tool with uncommon fervor");
		DisplayText(".  Your grunt of irritation vanishes as soon as your tender sex is forced into the heavenly hole, and with the soft breasts pressing into ");
		if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2) DisplayText("your own");
	...
	private haveAMapleSyrupSnack(): void {
		...
		else DisplayText("eagerly, Holli asks, \"<i>Care for a little pick-me-up?  They're getting very engorged, and I'm sure it would be a refreshing snack for you.</i>\"");
	...
	private level4RideHollisTentacruels(): void {
		...
		if (!player.torso.hips.legs.isBiped()) DisplayText("you'd be walking bowlegged for a week, if you had legs.");
		else DisplayText("you'll be walking bowlegged for a week!");
	...
	private eatHolliFruit(): void {
		...
		else DisplayText("one of the fruits");
		DisplayText(".  ");
		if (Flags.list[FlagEnum.HOLLI_FRUIT_EXPLAINED] === 0) {
			DisplayText("Holli smiles over at you and asks, \"<i>Enjoying the fruits of our union?</i>\"");
		...
		DisplayText("Biting into it, sweet juices seem to explode from the flesh, dribbling down your chin.  It tastes like a dessert and you chow down, happily munching away.  In no time flat, you're down to just a core.  You toss it and wipe your [face] clean, then burp.  Damn, that was good!");
		//TF CHANCES
		if (randInt(2) === 0 && player.torso.cocks.count > player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE))).length {
			let choices: Array = [];
			temp = 0;
			while (temp < player.torso.cocks.count) {
				if (player.torso.cocks.get(temp).type != CockType.TENTACLE) choices[choices.length] = temp;
				temp++;
			}
			...
			player.torso.cocks.get(temp).knotMultiplier = 1.3;
			player.stats.sens += 3;
	...
	private askHolliToWatch(): void {
		...
		if (Flags.list[FlagEnum.HOLLI_DEFENSE_ON] === 0) {
			...
			if (Flags.list[FlagEnum.THREATENED_HOLLI] > 0) DisplayText("<b>  You could always just slap the cunt.</b>");
		...
		//Stop Guarding (edited)(C)
			...
			DisplayText("You tell Holli you'd like her to stop watching at night.  She laughs, \"<i>You just want a chance to come back and beg some more, don't you?  I thought you were supposed to be tough shit, not a simpering little submissive.</i>\"");
			Flags.list[FlagEnum.HOLLI_DEFENSE_ON] = 0;
			return { next: treeMenu };
		}
	}
	...
	private domUpSomeHolli(): void {
		...
		if (player.torso.head.horns > 0) domPowah += 3;
	...
	private askBrokenHolliToGuard(): void {
		...
		else DisplayText("sleeping, instead");
		DisplayText(".</i>\"");
	...
	private toggleBrokenHolliGuard(): void {
		...
		//Guard Off
			...
			DisplayText("\"<i>All right, they're yours to smite or play with,</i>\" Holli says with a wink.  You give her a glare, to which she responds with a shrug.  \"<i>What?  I know how it is!  You mortals can't fight worth a damn when you're sleeping.</i>\"");
	...
	private holliGetsDickDommed(): void {
		...
		else DisplayText(", swallowed as deep as Holli's distended neck can handle.");
		...
		if (player.torso.balls.quantity > 0) DisplayText("[sack] feel taut and ready to burst");
		else DisplayText("body feel strung tight like an over-tuned guitar");
		...
		if (player.cumQ() > 1000) DisplayText("  A trickle of sperm washes out of the demoness's entrance, somehow escaping the heavy suction.");
		if (player.cumQ() > 2000) DisplayText("  A moment later, that trickle turns into a river.");
		if (player.cumQ() > 3000) DisplayText("  You swear that her trunk must be absolutely stuffed with spunk.");
		if (player.cumQ() > 5000) DisplayText("  A spooge-spout sprays out as the pressure goes through the roof, confirming your suspicions in the messiest way possible.");
	...
	private fullOnTentacleTasticGangBangForHolli(): void {
		...
		if (player.torso.balls.quantity > 0) DisplayText("  Your [balls] draw up tight to your groin and squeeze, rhythmically discharging their heavy loads.");
		...
		if (player.cumQ() > 500) DisplayText(", and the next");
		if (player.cumQ() > 1400) DisplayText("... and the next");
	...
	private vaginalDomHollisTentacruels(): void {
		...
		if (!player.torso.chest.hasFuckableNipples() && player.lactationQ() < 50) DisplayText(" and pay special attention to your [nipples], tugging them in time with every sluggish thrust in your slit.");
			...
			else if (player.lactationQ() < 800) DisplayText("sprays");
			else if (player.lactationQ() < 2000) DisplayText("outpouring");
			else DisplayText("thick fountains");
			DisplayText(" of milk that escape with every tug, timed to match the sluggish thrusts at your slit.");
		}
		else DisplayText(" and sink your fingers into your [nipples], fucking them to the sluggish tempo of the thrusting tentacle.");
		DisplayText("  A loud groan escapes your mouth, and you rock your [hips] against the undulating tendril.  Your body's eagerness for more pleasure is unparalleled, but you control yourself as efficiently as you control the dryad.");
		...
		if (player.torso.clit.length >= 3.5) DisplayText(" for a moment before wrapping around and jerking on it");
		DisplayText(".");
		...
		if (player.torso.vaginas.get(0).wetness < 4) DisplayText("onto the ground");
		else DisplayText("into the leavings of your squirting orgasm");
	...
	private threatenHolli(): void {
		...
		if (player.level < 30) DisplayText("Mighty as you are, Marae is likely still beyond your power.  Perhaps you should just ignore this demon for the time being.");
		else DisplayText("Marae is likely beyond your power yet, but... you feel like she might not pressure you too hard if you can make a suitably credible threat.");
		...
		if ((player.gender > 0 && player.statusAffects.has(StatusAffectType.JojoNightWatch) && player.statusAffects.has(StatusAffectType.PureCampJojo)) || Flags.list[FlagEnum.ANEMONE_WATCH] > 0) {
	...
	private callDatJojo(): void {
		...
		Flags.list[FlagEnum.FOUGHT_HOLLI] = 1;
		startCombat(new Holli());
		monster.statusAffects.add(StatusAffectType.JojoIsAssisting, 0, 0, 0, 0);
	}
	...
	internal function defeatHolli(): void {
		...
		if (monster.findStatusAffect(StatusAffects.JojoIsAssisting) < 0) {
			DisplayText("Weary, the succubus godling folds into her tree, unwilling to allow you the sight of her defeat.  With her energy suppressed, you're free to ");
			if (player.weaponName != "large axe") DisplayText("fetch an axe and ");
			DisplayText("hack at her trunk, laboriously peeling bark and making wedge-shaped cuts in it as you try to fell the abomination.  It takes nearly eight hours, but eventually the gnarled tree topples.  No sooner does it fall than it shrivels and turns to ash.");
			temp = 7;
			while (temp > 0) {
				temp--;
				model.time.hours++;
		...
		//Win with Jojo -Z
			...
			DisplayText("The fire-blackened mouse cheers at you as the succubus groans and retreats completely into her bark.  \"<i>Well done, [name]!  Help me carry tinder and we can burn this abomination to the ground!</i>\"");
	...
	internal function enjoyYourBadEndBIYAAAATCH(): void {
		...
		if (player.gender === Gender.MALE || (randInt(2) === 0 && player.torso.cocks.count > 0)) {
			...
			if (player.torso.balls.quantity > 0) DisplayText(", and your [balls] draw close to your body to unload");
			DisplayText(".  Your eyes roll as you flex and flex, but nothing comes out.  As you conclude your dry orgasm, you begin to feel a pressure on your prostate.");
		...
		//branch of vagina-having
			...
			if (player.torso.hips.legs.isTaur()) DisplayText(" and turning you around");
			DisplayText(", the demon buries her face in your [vagina], penetrating it with her wet, flexible tongue.");
			...
			DisplayText("\n\nSomething ");
			if (!player.torso.hips.legs.isGoo() && !player.torso.hips.legs.isNaga()) DisplayText("slips between your thighs and ");
			...
			if (player.torso.cocks.count > 0) DisplayText(" and a glob of precum from [eachCock]");
			DisplayText(".");
			...
			if (player.torso.cocks.count > 0) DisplayText("  [OneCock] drools precum down its length and onto the flower, triggering another spasm and another gush from your prick.  Holli laughs as you continue to self-stimulate with your uncontrollable fluids.");
			...
			if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 1) DisplayText("wrapping around and under your breasts to keep them permanently jutting out in a lurid display");
			else DisplayText("tracing a lattice of slender, criss-crossing ivy over your chest that hangs just far enough over your [nipples] that a stiff breeze will drag it into them");
			...
			if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 1) DisplayText("your breasts cupped and pressed together by vines, ");
			if (!player.torso.hips.legs.isNaga() && !player.torso.hips.legs.isGoo()) DisplayText("your thighs spread wide open, ");
			DisplayText("and your nectar-drooling flower-sex exposed with your [clit]");
			if (player.torso.cocks.count > 0) DisplayText(" and [eachCock]");
			DisplayText(" bobbing in a state of permanent semi-arousal.");
			...
			if (player.torso.butt.wetness > 0) DisplayText("Betraying your depravity, the slick crack opens wide and your greedy, wet asshole practically welcomes the demon's entire hand, sucking it up and making you shiver with repressed delight.  \"<i>Oh, mother!  It seems as though this is not a new lesson to you, is it?  My little whore, will you beg and moan as I fill you?</i>\"");
			else if (player.analCapacity() < 30) DisplayText("With some difficulty, she forces it into your pucker.  \"<i>I think we'll just use this as our inroads, instead.  It may be slow going, but the journey is half the fun.</i>\"");
			...
			else if (player.analCapacity() < 30) DisplayText("Your jaw hits your collarbone as Holli angles the florid head of a long, narrow penis into your field of view.  Judging by how far it comes up her chest, it's almost a foot and half long, and it's drooling sticky sap all over your back.  With a twisted smile, the demon pushes it along your spine until it reaches your [butt], then shoves it toward the hole.  ");
			else DisplayText("The demon presses up to your back, brushing her nipples against your skin and shoving her new growth between your thighs.  Looking down, you can see a fat, green shaft sticking from your crotch, glistening and dripping with sticky slime.  With another moan, Holli pulls it back from between your legs, then pushes apart your asscheeks and plunges it into your [asshole].  ");
			DisplayText("The sap oozing from her member coats your passage, ");
			if (player.torso.butt.wetness > 0) DisplayText("mingling with your own lubrication and ]easing her thrusts.  Holli's tempo picks up quickly, and soon you can feel her twitch inside you as she nears her climax.");
			...
			//[(buttslut or loose)
			if (player.torso.butt.wetness > 0 || player.analCapacity() > 30) DisplayText("spilling a wave of her deposited fluids down your [legs].  ");
			else DisplayText("and your once-tight pucker reflexively squeezes shut as best it can, trapping all but a few trickles of Holli's deposit inside you.  ");
			...
			DisplayText(".  They continue past your face, reaching over your head to form a canopy similar to Holli's own.  Struggle as you may, when they're done growing you're left unable to budge and locked in a rather artful pose, head turned over your shoulder with your [ass] thrust out");
			if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 1) DisplayText(", your breasts exposed from the sides,");
			DisplayText(" and your [legs] concealed below your buttcheeks by a thick tree trunk.");
	...
	private girlsGetANiceManToBadEnd(): void {
		...
		if (player.torso.balls.size > 10 && player.torso.balls.quantity > 0) DisplayText("lifting the hefty testicles to reveal a wet, eager pussy.");
		else DisplayText("spreading the labia and exposing the eager, clenching hole inside.");
		...
		if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 1) {
			DisplayText("reaches up and grabs two handfuls of breast, passing his thumb over a nipple and watching the ");
			if (player.gender === Gender.FEMALE) DisplayText("woman");
			else DisplayText("hermaphrodite");
			DisplayText("'s mouth open and close in bliss.  ");
		}
		else DisplayText("leans down and kisses the [woman/hermaphrodite]'s smooth chest, causing her to arch her back in pleasure.  ");
		if (player.torso.cocks.count > 0) DisplayText("With a nervous glance downward, he slides one hand to her [cock smallest].  \"<i>It's not gay if it's a chick's,</i>\" he mumbles to himself, and then begins to stroke haltingly, playing his finger along the urethra and drawing a squirt of precum from his shivering partner.  ");
		else {
			DisplayText("His hand slides to the [clit] above her wet fuck-hole; collecting some of the frothing lube from their wet sex, ");
			if (player.torso.clit.length < 6) DisplayText("he smears it over the little button and then begins to trace up and down the underside with a finger.  ");
			else DisplayText("he coats the swollen organ with it and begins to jerk it off valiantly, like a cock.  ");
			DisplayText("\"<i>Does that feel good?</i>\" he asks, upping the pace of his thrusts.  ");
		}
		DisplayText("As if in reply, the ");
		if (player.gender === Gender.FEMALE) DisplayText("woman");
		else DisplayText("hermaphrodite");
		DisplayText(" in the tree arches her back and her [vagina] clamps down on his tool");
		if (player.torso.cocks.count > 0) {
			DisplayText("; ");
			if (player.torso.balls.quantity > 0) DisplayText("her balls draw closer to her body and ");
			DisplayText("her trembling dick");
			if (player.torso.cocks.count === 1) DisplayText(" throbs");
			else DisplayText("s throb");
			DisplayText(" as she nears climax");
		}
		DisplayText(".");
		...
		if (player.torso.cocks.count > 0) {
			...
			//else cock biggest > 80)
				...
				DisplayText("her body is easily dwarfed by her male sex's massive size, and globs of sperm shoot into her canopy to splatter among her leaves and branches.  ");
				if (player.cumQ() >= 1000) DisplayText("The overproductive hermaphrodite's body pushes out so much seed that the entire canopy soon turns white and drips constantly as though it were in a tropical rainforest.  ");
			}
		}
		//(vagina and no cock)
		else {
			DisplayText("\n\nA ");
			if (player.torso.vaginas.get(0).wetness < 3) DisplayText("gush");
			else if (player.torso.vaginas.get(0).wetness < 4) DisplayText("rush");
			else DisplayText("torrent");
		...
		DisplayText("Pushed past his limit by her lewd display, the young man groans, shudders, and begins to unload his pent-up cargo.  Falling on top of his partner's slick ");
		if (player.torso.cocks.count === 1) DisplayText("cock");
		else if (player.torso.cocks.count > 1) DisplayText("cocks");
		else DisplayText("chest");
		DisplayText(", he goes limp and slack-jawed as his dick squirts, filling her vagina.");
		...
		DisplayText("'s stomach.  \"<i>In fact, she seems to have enjoyed it so much that she would have me bestow a blessing on you.</i>\"");
