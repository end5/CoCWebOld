...
//		public currentCallNext():Function { return callNext; }
		...
		public itemGoNext():void { if (callNext != null) return { next: callNext }; }
		...
		public inventoryMenu():void {
			...
			DisplayText("<b>Weapon</b>: " + player.weaponName + " (Attack - " + player.weaponAttack + ")\n");
			DisplayText("<b>Armor : </b>" + player.inventory.equipment.armor.displayName + " (Defense - " + player.armorDef + ")\n");
			if (player.keyItems.length > 0) DisplayText("<b><u>\nKey Items:</u></b>\n");
			for (x = 0; x < player.keyItems.length; x++) DisplayText(player.keyItems[x].keyName + "\n");
			menu();
			for (x = 0; x < 5; x++) {
...
//Gone			menuLoc = 1;
		...
		public takeItem(itype:ItemType, nextAction:Function, overrideAbandon:Function = null, source:ItemSlotClass = null):void {
			...
			if (temp >= 0) { //First slot go!
				...
				DisplayText("You place " + itype.longName + " in your " + inventorySlotName[temp] + " pouch, giving you " + player.itemSlots[temp].quantity + " of them.");
				itemGoNext();
				return;
			}
			...
			if (temp >= 0) {
				...
				DisplayText("You place " + itype.longName + " in your " + inventorySlotName[temp] + " pouch.");
				itemGoNext();
				return;
			}
		...
		public returnItemToInventory(item:Useable, showNext:boolean = true):void { //Used only by items that have a sub menu if the player cancels
			...
			if (showNext)
				return { next: callNext }; //Items with sub menus should return to the inventory screen if the player decides not to use them
		...
		public giveHumanizer():void {
			if(Flags.list[FlagEnum.TIMES_CHEATED_COUNTER] > 0) {
				DisplayText("<b>I was a cheater until I took an arrow to the knee...</b>", true);
				Game.gameOver();
				return;
			}
			DisplayText("I AM NOT A CROOK.  BUT YOU ARE!  <b>CHEATER</b>!\n\n", true);
		...
		private useItemInInventory(slotNum: number):void {
			...
			if (player.itemSlots[slotNum].itype is Useable) {
				...
				DisplayText("You cannot use " + player.itemSlots[slotNum].itype.longName + "!\n\n");
			}
...
//				if (!item.hasSubMenu()) itemGoNext(); //Don't call itemGoNext if there's a sub menu, otherwise it would never be displayed
		...
		private takeItemFull(itype:ItemType, showUseNow:boolean, source:ItemSlotClass):void {
			DisplayText("There is no room for " + itype.longName + " in your inventory.  You may replace the contents of a pouch with " + itype.longName + " or abandon it.");
			menu();
			...
			MainScreen.addButton(9, "Abandon", callOnAbandon); //Does not MainScreen.doNext - immediately executes the callOnAbandon function
		}
		...
		private replaceItem(itype:ItemType, slotNum: number):void {
			...
			if (player.itemSlots[slotNum].itype === itype) //If it is the same as what's in the slot...just throw away the new item
				DisplayText("You discard " + itype.longName + " from the stack to make room for the new one.");
			else { //If they are different...
				if (player.itemSlots[slotNum].quantity === 1) DisplayText("You throw away " + player.itemSlots[slotNum].itype.longName + " and replace it with " + itype.longName + ".");
				else DisplayText("You throw away " + player.itemSlots[slotNum].itype.longName + "(x" + player.itemSlots[slotNum].quantity + ") and replace it with " + itype.longName + ".");
...
*/
		...
		private armorRackDescription():boolean {
			if (itemAnyInStorage(gearStorage, 9, 18)) {
				...
				DisplayText("  It currently holds " + formatStringArray(itemList) + ".");
				return true;
			}
			return false;
		}
		...
		private pickItemToPlaceInStorage(placeInStorageFunction:Function, typeAcceptableFunction:Function, text:string, showEmptyWarning:boolean):void {
			DisplayText().clear(); //Selects an item to place in a gear slot. Rewritten so that it no longer needs to use numbered events
			hideUpDown();
			DisplayText("What item slot do you wish to empty into your " + text + "?");
			menu();
			let foundItem:boolean = false;
			for (let x: number = 0; x < 5; x++) {
		...
		private placeIn(storage:Array, startSlot: number, endSlot: number, slotNum: number):void {
			...
			for (x = startSlot; x < endSlot && qty > 0; x++) { //Find any slots which already hold the item that is being stored
				if (storage[x].itype === itype && storage[x].quantity < 5) {
					...
					DisplayText("You add " + temp + "x " + itype.shortName + " into storage slot " + num2Text(x + 1 - startSlot) + ".\n");
			...
			for (x = startSlot; x < endSlot && qty > 0; x++) { //Find any empty slots and put the item(s) there
				if (storage[x].quantity === 0) {
					...
					DisplayText("You place " + qty + "x " + itype.shortName + " into storage slot " + num2Text(x + 1 - startSlot) + ".\n");
					qty = 0;
					return;
				}
			}
			DisplayText("There is no room for " + (orig === qty ? "" : "the remaining ") + qty + "x " + itype.shortName + ".  You leave " + (qty > 1 ? "them" : "it") + " in your inventory.\n");
