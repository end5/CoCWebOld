...
export class Desert {
	...
	public exploreDesert(): void {
		...
		if ((player.exploredDesert === 20 && !player.statusAffects.has(StatusAffectType.TelAdre)) || (randInt(20) === 0 && player.statusAffects.get(StatusAffectType.TelAdre).value1 === 0)) {
			kGAMECLASS.telAdre.discoverTelAdre();
			return;
		}
		if (sandWitchScene.pregnancy.event === 2 && randInt(4) === 0) {
		...
		if (randInt(4) === 0 && player.stats.int > 50 && Flags.list[FlagEnum.FOUND_WIZARD_STAFF] === 0) {
			DisplayText().clear();
			DisplayText("While exploring the desert, you see a plume of smoke rising in the distance.  You change direction and approach the soot-cloud carefully.  It takes a few moments, but after cresting your fourth dune, you locate the source.  You lie low, so as not to be seen, and crawl closer for a better look.\n\n");
			...
			inventory.takeItem(weapons.W_STAFF, Scenes.camp.returnToCampUseOneHour);
			return;
		}
		//Possible chance of boosting camp space!
		if (player.hasKeyItem("Camp - Chest") < 0 && (randInt(100) < 10)) {
		...
		let select: number = randInt(choices.length);
		if (args[select] === -8008) {
			choices[select]();
		}
	...
	private walkingDesertStatBoost(): void {
		...
		if (randInt(2) === 0) {
			...
			if (randInt(2) === 0 && player.stats.str < 50) {
				DisplayText("The effort of struggling with the uncertain footing has made you stronger.");
				player.stats.str += .5;
			}
			//Toughness
			else if (player.stats.tou < 50) {
				DisplayText("The effort of struggling with the uncertain footing has made you tougher.");
				player.stats.tou += .5;
			}
		}
		return { next: Scenes.camp.returnToCampUseOneHour };
