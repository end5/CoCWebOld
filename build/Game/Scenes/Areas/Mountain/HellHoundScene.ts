...
export class HellHoundScene {
	...
	public hellhoundRapesPlayer(): void {
		...
		DisplayText(" and positions himself behind you. He grips you with his clawed hands on your shoulders and easily holds you in place. The powerful creature is easily able to get past your " + player.inventory.equipment.armor.displayName + " and you feel one of his rods brush against your " + ButtDescriptor.describeButt(player) + ".  A shudder shoots through your body as he brings his two heads down to either side of your own, his hot canine breath blowing past your ears. He stays there for a few moments, completely still... before suddenly pulling his heads back and ");
		if (player.torso.vaginas.count > 0 && player.hips.legs.type != LegType.NAGA) DisplayText("slamming his twin dicks into your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " and your " + ButtDescriptor.describeButthole(character.torso.butt) + ".  ");
		else DisplayText("slamming his lower dick into your " + ButtDescriptor.describeButthole(character.torso.butt) + ", while rubbing the other between the cheeks of your " + ButtDescriptor.describeButt(player) + ".  ");
		if (player.torso.vaginas.count > 0 && player.hips.legs.type != LegType.NAGA) {
			if (player.vaginalCapacity() < monster.torso.cocks.get(0).area) DisplayText("You feel an intense mixture of sensations in your lower body as your " + ButtDescriptor.describeButthole(character.torso.butt) + " feels like it is being torn apart, while your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " is filled with an intense pleasure at being filled with the creature's large member.  ");
			else DisplayText("Your lower body explodes with pain as the hellhound forces himself in too quickly for either your " + ButtDescriptor.describeButthole(character.torso.butt) + " or your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " to handle.  ");
			if (player.torso.vaginas.get(0).virgin) DisplayText("<b>You are no longer a virgin!  </b>");
			if (player.displayStretchVagina(monster.torso.cocks.get(0).area, false)) DisplayText("The beast howls as your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " is stretched to accommodate the large shaft.  ");
			DisplayText("The hellhound pants for a few seconds before continuing.  ");
			if (player.torso.vaginas.get(0).wetness < VaginaWetness.WET) DisplayText("The pause gives your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " time to moisten, ensuring the next thrust won't be quite as rough.  ");
			DisplayText("This time the beast starts pumping in and out more gently, only a short distance at a time as the hellhound continues panting.  As the pain in your " + ButtDescriptor.describeButthole(character.torso.butt) + " fades, you start feeling pleasure to match the sensations rising from your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ". Each thrust of the beast's twin manhoods bring you closer and closer to your peak.\n\n");
			//Cum
			...
			DisplayText("Your lower body explodes with pain as the hellhound forces himself in too quickly for your " + ButtDescriptor.describeButthole(character.torso.butt) + " to handle. He pauses and pants for a few seconds, but you barely have enough time to catch your breath before the beast starts thrusting in and out again. However, this time the beast starts pumping in and out more gently, only a short distance at a time as the hellhound continues panting.  As the pain in your " + ButtDescriptor.describeButthole(character.torso.butt) + " fades, you start feeling strong pleasure from it. Each thrust of the beast's lower manhood, combined with the upper one tickling and teasing your " + ButtDescriptor.describeButt(player) + " above it brings you closer and closer to your peak.\n\n");
			//Cum
	...
	public hellHoundGetsRaped(): void {
		...
		if (player.torso.cocks.count === 1 && player.torso.vaginas.count === 0) DisplayText("The twin heads of the hellhound eagerly lick your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " for a while.  The large, flat tongues give you a rather unusual feeling, but it is definitely effective. As you approach your peak, one of the heads tries to take your manhood into its mouth. But the other head doesn't seem all that pleased with this, and the two start to nip at each other in an effort to claim the right to be the one to push you over the edge. Annoyed at being ignored, you give both heads a smack with your hand. After a moment of recoil, they look down apologetically before resuming their licking. As you peak, the heads pull back and try to catch all the cum that erupts from your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " in their mouths. The fierce competition between the two means that most of the cum ends up outside their eager maws, but they are nontheless satisfied.  With a pair of happy barks, the hellhound turns and runs away.");
		//[if player has only a vagina and no dick] 
		if (player.gender === Gender.FEMALE) DisplayText("The twin heads of the hellhound eagerly lick your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " for a while.  The large, flat tongues give you a rather unusual feeling, but it is definitely effective. As you approach your peak, one of the heads tries to force the other out of the way and push its snout into your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ". But the other head doesn't seem all that pleased with this, and the two start to nip at each other in an effort to claim the right to be the one to push you over the edge. Annoyed at being ignored, you give both heads a smack with your hand. After a moment of recoil, they look down apologetically before resuming their licking. As you peak, the heads resume their quarrel and try to drink as much as the fluids coming from your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " as they can.  You don't know which got the most, but before long your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " is definitely quite clean.  With a pair of happy barks, the hellhound turns and runs away.");
		//[if player has a dick and a vagina] 
		if (player.gender === Gender.HERM && player.torso.cocks.count === 1) DisplayText("One of the heads eagerly starts licking your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " while the other starts working on your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ". The large, flat tongues give you a rather unusual feeling, but it is definitely effective.  As you approach your peak, the head that had been licking your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " suddenly takes it into its mouth while the other pushes its snout into your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ". The pleasure from this new arrangement quickly overwhelms you, leaving you dazed.  When your senses return to you, you look up to see the hellhound's tongues running all over their mouths, clearly very happy.  With a pair of happy barks, the hellhound turns and runs away.");
		//If player has multiple endowments
		if (player.torso.cocks.count > 1) DisplayText("One of the heads eagerly starts licking your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " while the other starts to work on your " + CockDescriptor.describeCock(player, 1) + ". The large, flat tongues give you a rather unusual feeling, but it is definitely effective.   As you approach your peak, the heads quickly take your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " and " + CockDescriptor.describeCock(player, 1) + " into their mouths. The pleasure from this new arrangement quickly overwhelms you, leaving you dazed.  When your senses return to you, you look up to see the hellhound's tongues running all over their mouths, clearly very happy.  With a pair of happy barks, the hellhound turns and runs away.");
	...
	private giveALLTHELETHICITES(): void {
		...
		if (player.torso.cocks.filter(Cock.FilterType(CockType.DOG) >= 2)).length {
			...
			if (player.torso.cocks.count > 2 || player.cockThatFits(60) < 0) {
				DisplayText("He then reaches around your waist and takes a hold of your " + CockDescriptor.describeMultiCockShort(player) + ".  \"<i>Before we get started, let's make sure you're just right for Cremera.</i>\"  He then reforms your body to have twin doggy pricks of appropriately sized.  \"<i>Now.</i>\"\n\n");
			...
			DisplayText("After what feels like hours of painful pleasure without release, you're smacked on your rear one last time, and it feels like Cimer bursts through your body completely.  This finally frees the floodgates, and you feel like you're releasing everything in your body in the most powerful orgasm you've ever felt.  You pant for a moment and look down in front of you.  You can see your " + player.BreastDescriptor.describeAllBreasts(player) + " and your pregnant belly in front of you. You also see a package of demonic dicks sticking out in front of you, with a quad of hot balls resting underneath.  In front of you is a large pool of white steaming fluid, with a pink crystal resting in the middle.\n\n");

			DisplayText("<i>Well my new pet, what do you think?</i>\"  A voice calls out in your head.  You look to the side, and see one of Cimer's heads on your right, and the other on your left.  Your hands seem to move on their own, and reach down to touch your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ".  Another voice calls out in your mind, <i>They are wonderful master.</i>\"\n\n");
		}
	...
	public hellHoundPropahRape(): void {
		...
		//--- ELSE (CORRUPTION >= 20) ---
			...
			DisplayText("  Riding the hound for a couple of minutes, you feel the dick in your " + ButtDescriptor.describeButthole(character.torso.butt) + " extending, giving you a good stretching. Its brother in your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " does the same, pushing more and more girl-fluids out of your slippery slit while stimulating your sensitive clit from the inside. You lean forward, running your hands through your fuckpet's dense midnight black fur, feeling his animal body warmth. The hellhound is moaning and panting beneath you, unsure whether to fear you or to enjoy what you are doing to him.\n\n");
			...
			if (player.torso.cocks.count > 0) {
				...
				//--- IF ONE PENIS ---
				if (player.torso.cocks.count === 1) DisplayText("  Soon your own " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " reacts, quickly growing into a full throbbing erection. You leave your member alone, and it keeps flapping against the hellhound's belly while you ride him.");
				//--- ELSE ---
				else DisplayText("  Soon " + CockDescriptor.describeMultiCockSimpleOne(player) + " reacts, quickly growing into a throbbing erection.  You leave your members alone, and they flap against the hellhound's belly while you ride him.");
			}
			//--- IF PC HAS BALLS ---
			...
			if (player.torso.cocks.count > 0) {
				// --- IF ONE PENIS ---
				if (player.torso.cocks.count === 1) DisplayText("Your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " twitches as it ejects its sticky load.");
				//--- ELSE ---
				else DisplayText("Your " + CockDescriptor.describeMultiCockShort(player) + " twitch as they eject their sticky loads.");
