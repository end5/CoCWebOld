export class Salon implements TimeAwareInterface {
	...
	public hairDresser(): void {
		DisplayText("While exploring the mountain, you find a cleverly concealed doorway.  From inside you can hear the sound of blades being sharpened.  Do you enter the doorway?", true);
		doYesNo(salonGreeting, Scenes.camp.returnToCampUseOneHour);
	}
	...
	private buyMinoCum(): void {
		if (player.stats.gems < 60) {
			DisplayText("You can't afford any minotaur cum right now!", true);
			...
			DisplayText("You happily give Lynnette 60 gems and pick up the bottle full of glistening, heavenly cum.  ", true);
			statScreenRefresh();
	...
	private hairDresserGreeting(): void {
		...
		if (player.torso.cocks.count === 0) {
			...
			DisplayText("  I'll even do you the favor of letting you blow it in my mouth, I've already got a bun in the oven.  So what do you say?  Want a spooge and a haircut?  Or would you rather go get your payment from one of the gloryholes in the back, you " + player.mf("kinky boy", "naughty girl") + "?</i>\"\n\n");
	...
	private hairDresserRepeatGreeting(): void {
		...
		//Standard repeats
			...
			DisplayText("Lynnette the goblin answers the door and lets you in, waving you deeper into her shop.  Her shining black dress barely contains her fertile-hips and jiggling chest as she greets you, \"<i>Welcome back, honey!  ", true);
			if (player.torso.cocks.count === 0) {
	...
	private gloryholeMinotaur(): void {
		...
		DisplayText("Driven on by the minotaur's reactions, you take more and more of the minotaur cock into your mouth. As you work your mouth around the first ring, you open up the top section of your " + player.inventory.equipment.armor.displayName + ", freeing up your " + BreastDescriptor.describeAllBreasts(player) + ". With each pass, you're able to take more and more of the bull dick down your throat. You caress your " + Desc.Breast.describeBreastSize(character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + " while moaning around the dick. When you get your mouth around the second ring, the minotaur's cock throbs sharply, and his cum explodes in your mouth. Your eyes bulge in shock, and you pull off the cock as quickly as you can. You swallow the first several shots of thick, rich minotaur cum, while the rest shoots all over your face and tits. You ");
	...
	private goblinHairDresserFacefuck(): void {
		DisplaySprite(38);
		DisplayText("Lynnette licks her lips and practically tears her way into your " + player.inventory.equipment.armor.displayName + ", having your crotch exposed in seconds.  Your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " flops out immediately, slapping her on the nose as it grows hard.  She wraps both hands around you and begins pumping with practiced ease, flicking her tongue over your crown and wrapping her lips ", true);
	...
	private cutShort(): void {
		...
		DisplayText("Lynnette and her daughters crowd around you with razor-sharp scissors, effortlessly paring down your " + HeadDescriptor.describeHair(player) + ".  When they've finished, you're left with ", true);
		player.torso.neck.head.hair.length = 1;
		DisplayText(HeadDescriptor.describeHair(player) + ".");
	...
	private hairGrow(): void {
		...
		DisplayText("Lynnette grabs a bottle and squirts a white fluid into your hair.  You really hope it isn't your payment.  But it must not be, as within short order you feel the added weight of ", true);
		temp = randInt(3) + 3;
		Flags.list[FlagEnum.HAIR_GROWTH_STOPPED_BECAUSE_LIZARD] = 0;
		player.torso.neck.head.hair.length += temp;
		DisplayText(num2Text(temp) + " more inches of " + player.torso.neck.head.hair.color + " hair.");
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
	...
	private minotaurCumBukkakeInSalon(): void {
		...
		else if (player.torso.cocks.count > 0) DisplayText(" and turning " + CockDescriptor.describeMultiCockSimpleOne(player) + " into a turgid, pulsating mass.\n\n");
		else DisplayText("\n\n");
		...
		if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2) {
			DisplayText("  You wrap the pillowy flesh of your " + BreastDescriptor.describeAllBreasts(player) + " around the new member, pleasantly surprised by its girth and wide, already-flared tip.  Maybe you could have gotten him off with your hands after all?  ");
		...
		DisplayText("You lower and raise yourself, bouncing up and down on your " + LegDescriptor.describeLegs(player) + " to enhance the ");
		...
		if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2) DisplayText("The sudden girth change sends an enticing ripple through your " + BreastDescriptor.describeAllBreasts(player) + " that's pleasurable enough to make you moan into his steadily-widening urethra.  ");
		...
		//(buttchange here: 90)
		player.displayStretchButt(90, true, false);
		...
		else DisplayText("presses on your prostate as it squeezes by, making " + CockDescriptor.describeMultiCockSimpleOne(player) + " drip and spurt freely until you can bear it no longer and cum, shaking and shuddering like a wanton whore.  Jizz drips and pours from " + CockDescriptor.describeMultiCockSimpleOne(player) + " in a steady stream that pools on the floor, slowly rolling towards a drain that doubtless empties into a tank or greedy goblin cunt.");
		DisplayText("\n\n");

		DisplayText("The strength goes completely out of your " + LegDescriptor.describeLegs(player) + ", but you manage to hold yourself up long enough for your stud to flex his cock inside you and fill up your backdoor with more potent addiction.  You slide off, nerveless and still orgasming as jizz rains on your back from the abandoned cock, rolling off you to add to the ever-widening puddle of fluids on the floor.  Smiling vacantly and addled on more minotaur-cum than any one person has reason to handle, you stumble up on all fours and crawl to the next one.  More...");
		//[NEXT]
		player.orgasm();
		player.stats.lib += 2;
		player.stats.sens += 2;
	...
	private minotaurSalonFollowUp(): void {
		...
		else DisplayText("You're woken up with a slap to the face, and still muzzy from your cum-induced glory-hole orgy, you stagger up to your " + LowerBodyDescriptor.describeFeet(player) + ".  Lynnette shakes her stinging palm and apologizes, \"<i>Sorry I have to keep doing that, but you're damned hard to wake after you go on these binges!  Anyways, you've earned some hair treatment if you want it.  Of course you could always buy more of your favorite fluid...</i>\"\n\n");
		//Menu
		DisplayText("Lynnette offers and explains their options, \"<i>So what'll it be hun?  We could cut it down or give you a lengthening treatment. Or you can get a hair-dye to use on your own.  Just remember to come back in a few days for a touchup.</i>\"");
		salonPurchaseMenu();
	}
	...
	*/
	...
	private fuckLynnette(): void {
		...
		if (player.cockArea(x) > 50) {
			...
			if (player.cockArea(x) <= 80) DisplayText("her belly.");
		...
		if (player.cockArea(x) > 24) DisplayText("up against her slightly-yielding cervix");
		...
		if (player.cockArea(x) <= 90) DisplayText("completely inside once more");
