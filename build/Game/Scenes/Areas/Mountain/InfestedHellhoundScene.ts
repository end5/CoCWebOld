...
export class InfestedHellhoundScene {
	...
	public infestedHellhoundEncounter(): void {
		...
		if (player.hips.legs.type === LegType.GOO) DisplayText("making your jello-like body jiggle");
	...
	public infestedHellhoundLossRape(): void {
		...
		if (player.torso.cocks.count > 0 && player.statusAffects.has(StatusAffectType.Infested)) {
			...
			//(HP) 
			else DisplayText("Too wounded to stand, you drop down to all fours in order to keep yourself off the ground.   Too late you realize your mistake – the snarling beast is behind you and its razor-sharp teeth easily sink into your " + player.inventory.equipment.armor.displayName + " before tearing away the offending armor, exposing your " + ButtDescriptor.describeButt(player) + " and " + CockDescriptor.describeMultiCockShort(player) + ".  A cold mountain breeze blows across your now-exposed " + player.skinDesc + ", reminding you just how utterly vulnerable you are to the alien lusts of this symbiotic monstrosity. With a brutal lunge it knocks you off your " + LowerBodyDescriptor.describeFeet(player) + " and onto your back.\n\n");

			DisplayText("The beast takes a sniff at your groin, then backs away, looking confused.  You glance down and realize just how hard you've become.  A few of your worms are hanging from the " + player.cockHead() + " of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", starting to flow out in a steady stream.  It feels better than it has any right to.   A shadow falls across you as the hellhound moves over you, its imposing twin members hard and pulsating above you.  Hot splatters of jism drip onto your chest as the beast's worms begin escaping, forcing thick globules of dog-semen out along with them.\n\n");
		...
		//[PLAYER'S COCKS ARE BIG ENOUGH TO BE INFECTED]
		else if (!player.statusAffects.has(StatusAffectType.Infested) && player.torso.cocks.biggestCocks[0].area >= 40 && player.torso.cocks.count > 0) {
			//(LUST)
			if (player.stats.lust > 99) {
				DisplayText("No amount of shame from the act of submitting to such a beast can overpower the furnace of lust raging in your loins.  ");
				if (player.stats.cor < 33) DisplayText("In spite of your revulsion ");
			...
			//(HP)
				...
				DisplayText("Too wounded to stand, you drop down to all fours in order to keep yourself off the ground.   Too late you realize your mistake – the snarling beast is behind you and its razor-shark teeth easily sink into your " + player.inventory.equipment.armor.displayName + " before tearing it off, exposing your " + ButtDescriptor.describeButt(player) + " and " + CockDescriptor.describeMultiCockShort(player) + ".  A cold mountain breeze blows across your now exposed " + player.skinDesc + ", reminding you just how utterly vulnerable you are to the alien lusts of this symbiotic monstrosity.  With a brutal lunge it knocks you off your " + LowerBodyDescriptor.describeFeet(player) + " and onto your back.\n\n");
			}
			if (player.torso.cocks.count > 1) {
				...
				DisplayText("each of your " + CockDescriptor.describeMultiCockShort(player) + ".  It feels good, better than it has any right to.  ");
				DisplayText("Every single one of your " + CockDescriptor.describeMultiCockShort(player) + " hardens under the stimulation, happy to be so well-treated.\n\n");
				...
				DisplayText("Gods, it's shoving its infected cocks up your urethras!  It's trying to infect you with those worms!  The dog-demon keeps pushing further and further, sliding deep inside you, the outline of its members easily visible through the skin of your " + CockDescriptor.describeMultiCockShort(player) + ".  It starts feeling good, the wriggling parasite-infested dicks begin sliding in and out, fucking your urethras in earnest and depositing their wormy cargo deep inside you.\n\n");

				DisplayText("Your ");
				if (player.torso.balls.quantity > 0) DisplayText(BallsDescriptor.describeBalls(true, true, player) + " shiver as hot doggie-cum and wiggling worms are pumped directly into them.  ");
				...
				DisplayText("The infested hound repositions itself, blocking out the sun with its dark fur, leaving you with only the pale flames surrounding its fuzzy sack to look at.   The warm wetness of its smooth tongue starts sliding over ");
				DisplayText("your " + CockDescriptor.describeMultiCockShort(player) + ".  It feels good, better than it has any right to.  ");
				DisplayText("Your " + CockDescriptor.describeMultiCockShort(player) + " hardens under the stimulation, happy to be so well-treated.\n\n");
				...
				DisplayText("Gods, it's shoving its infected cock up your urethra!  It's trying to infect you with those worms!  The dog-demon keeps pushing further and further, sliding deep inside you, the outline of its member easily visible through the skin of your " + CockDescriptor.describeMultiCockShort(player) + ".  It starts feeling good, the wriggling parasite-infested dick begins sliding in and out, fucking your urethra in earnest and depositing its wormy cargo deep inside you.\n\n");
		...
		//[HAS PUSSY AND NO DICK BIG ENOUGH TO BE INFECTED]
		else if (player.torso.vaginas.count > 0 && player.torso.cocks.biggestCocks[0].area < 40 && player.hips.legs.type != LegType.NAGA) {
			//(LUST) 
			if (player.stats.lust > 99) {
				DisplayText("No amount of shame from the act of submitting to such a beast can overpower the furnace of lust raging in your loins.  ");
				if (player.stats.cor < 33) DisplayText("In spite of your revulsion ");
			...
			//(HP) 
				...
				DisplayText("Too wounded to stand, you drop down to all fours in order to keep yourself off the ground.   Too late you realize your mistake – the snarling beast is behind you and its razor-shark teeth easily sink into your " + player.inventory.equipment.armor.displayName + " before tearing it off, exposing your " + ButtDescriptor.describeButt(player) + " and " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ".  A cold mountain breeze blows across your now exposed " + player.skinDesc + ", reminding you just how utterly exposed you are to the alien lusts of this symbiotic monstrosity.\n\n");
			}
			...
			if (player.statusAffects.has(StatusAffectType.WormPlugged))
				...
				player.statusAffects.set(new StatusAffect("WormPlugged", 1 + randInt(5))), 0, 0, 0);
			player.knockUpForce(PregnancyType.WORM_STUFFED, 100 + player.statusAffects.get(StatusAffectType.WormPlugged).value1); //Will be cleared when the WormPlugged effect ends
			player.orgasm();
			player.stats.lib += 1;
			player.stats.cor += 1;
			cleanupAfterCombat();
		...
		//[GENDERLESS OR MALE WITH DICK TOO SMALL]
			...
			//(HP) 
				...
				DisplayText("Too wounded to stand, you drop down to on your elbows in order to keep yourself off the ground.   Too late you realize your mistake – the snarling beast is behind you and its razor-shark teeth easily sink into your " + player.inventory.equipment.armor.displayName + " before tearing it off, exposing your " + ButtDescriptor.describeButt(player) + ".  A cold mountain breeze blows across your now exposed " + player.skinDesc + ", reminding you just how utterly exposed you are to the alien lusts of this symbiotic monstrosity.\n\n");
			}
			DisplayText("The hell-hound rises up, looking nearly humanoid in shape for a moment before it drops down, planting its paws on your shoulders.  ");
			...
			DisplayText("The demon-dog tries to penetrate you all at once, but it isn't lined up properly, and all it manages to do is bump against your taint and rub its other dick on your back.   You gasp in pain, momentarily thankful not to be impaled by such a bestial implement, but knowing all too well the brief reprieve will be over all too soon.   As if it can read your thoughts, the demon-hound repositions itself and slams forward, this time managing to line its dog-dicks up with your " + ButtDescriptor.describeButthole(character.torso.butt) + " correctly.  It feels far larger than it looked, filling you with a painful suddenness that brings tears to your eyes.\n\n");
			DisplayText("Incredible heat radiates from the thick black rod inside you, making your whole body break out in a cold sweat in an attempt to deal with the situation.  You're rocked back and forth as the beast begins fucking you in earnest, slamming its fur-covered hips against your " + ButtDescriptor.describeButt(player) + " with animalistic intensity.  Thankfully, the fucking isn't that painful, but the small mercy is ruined by the hot fluid you can feel starting to pour into your " + ButtDescriptor.describeButthole(character.torso.butt) + " and onto your back.\n\n");

			DisplayText("Oh gods no, you can feel something squirming inside you!  It's dripping worms into your intestines!   Its hot spittle and doggie-cum drips onto your back as it ruts hard and fast, spurting and dripping a small portion of its wormy cargo inside you.  They're wriggling and squirming around, rubbing you in such an obscene way, bringing you closer and closer to orgasm in spite of the budding horror gnawing at your gut.   The beast on your back seems oblivious to it all, pushing more roughly into your " + ButtDescriptor.describeButthole(character.torso.butt) + " with every stroke until with a burst of pressure, he manages to bottom out his throbbing, black fuck-stick.\n\n");
