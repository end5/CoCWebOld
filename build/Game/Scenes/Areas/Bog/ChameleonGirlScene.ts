...
export class ChameleonGirlScene {
    ...
    public encounterChameleon(player: Character): NextScreenChoices {
        DisplayText().clear();
    ...
    public loseToChameleonGirl(player: Character): NextScreenChoices {
        DisplayText().clear();
        DisplaySprite(SpriteName.Chameleon);
        // -Lose by lust
        ...
        if (player.gender === Gender.HERM && randInt(2) === 0) return loseToChameleonWithCockAnBallsAnCunt();
        else if (player.gender === Gender.MALE || (player.gender === Gender.HERM && randInt(2) === 0)) return dudesLoseToChamChams();
        else if (player.torso.vaginas.count > 0) return loseToChamChamWithPCCunt();
        else {
    ...
    // -Male Loss (Z edited)
    private dudesLoseToChamChams(player: Character): NextScreenChoices {
        const x: number = randInt(player.torso.cocks.count);
        DisplaySprite(SpriteName.Chameleon);
        DisplayText("\n\nRecognizing that you're no danger, her skin shifts back into its natural " + monster.skin.tone + " coloration as she walks towards you.  She swings her hips seductively as she splashes slowly through the water and licks her lips with a loud smack of her tongue, then reaches out and forcefully pulls your " + player.inventory.equipment.armor.displayName + " above your waist before you can even try to resist her advances, and pushes you back against a tree.  With a few quick pulls of fabric she's stripped her own clothes, and she tosses her thong over a low-hanging branch.  The other band of silk she uses to quickly bind your wrists.  Were you in any shape to fight back you could pull free, but you're too ");
        ...
        if (x.area <= monster.vaginalCapacity()) DisplayText("to the base of its shaft");
    ...
    // -Female Loss (Z edited)
    private loseToChamChamWithPCCunt(player: Character): NextScreenChoices {
        DisplaySprite(SpriteName.Chameleon);
    ...
    // VICTORY SCENES INTRO(Z edited)
    public defeatChameleonGirl(player: Character): NextScreenChoices {
        DisplayText().clear();
        DisplaySprite(SpriteName.Chameleon);
        // -Win by lust
        if (monster.lust > 99) {
        ...
        // -Win by HP
            ...
            DisplayText("Too weak to continue fighting, the chameleon girl drops to her knees, exhausted.  Her skin returns to its usual " + monster.skin.tone + ", unable to maintain the camouflage.");
    ...
    // -Herm Victory (Z edited)
    private fuckDatChameleonAsACoolGuyGirlHerm(player: Character) {
        DisplayText().clear();
        DisplaySprite(SpriteName.Chameleon);
        ...
        DisplayText("\n\nTaking the hint, she starts playing with your dripping gash again, sometimes teasing your " + Desc.Vagina.describeClit(player) + " with her fingers.  Your throbbing cock aches for attention, though, and an interesting idea pops into your head; you pick her tail up off the ground and, though she tries to twitch it away instinctually, you hold tight.  It relaxes, and you guide it towards your needy member, then gently wrap its end around a few times.  With a mischievous smile, the girl tightens her grip more than you expected, slowly pumping your shaft up and down with her strong tail.  Pre-cum begins to leak from the tip of your " + Desc.Cock.describeCock(player, x) + " and your pussy drools with sexual fluids as the extra the stimulation affects your body.");
        DisplayText("\n\nNot ");
    ...
    private manFucksChameleonWithBiggishWang(player: Character) {
        DisplayText().clear();
        DisplaySprite(SpriteName.Chameleon);
    ...
    // -Female (for herms without dicks) (Z edited)
    private femaleHasWinSexWithChamCham(player: Character) {
        DisplayText().clear();
        DisplaySprite(SpriteName.Chameleon);
        DisplayText("You shed your " + player.inventory.equipment.armor.displayName + " and approach the fallen chameleon girl");
        if (monster.lust > 99) DisplayText(", her moans of pleasure growing more desperate as you approach");
        ...
        if (monster.lust > 99) DisplayText("  She coos lustily at the sensation.");
    ...
    // Item Use Scenes Intro (Victory) (Z edited)
    private useAnItemOnTheChamcham(player: Character) {
        DisplayText().clear();
        DisplaySprite(SpriteName.Chameleon);
        ...
        let milk;
        let drafts;
        if ((player.inventory.items.has(consumables.SUCMILK) || player.inventory.items.has(consumables.P_S_MLK)) && player.torso.cocks.count > 0) milk = giveTheChameleonASuccubiMilk;
    ...
    private giveTheChameleonASuccubiMilk(player: Character) {
        DisplayText().clear();
        DisplaySprite(SpriteName.Chameleon);
        ...
        if (player.hasLongTail() > 0) DisplayText(" with your tail,");
        else DisplayText(" by straddling her,");
        DisplayText(" your cock aimed for her pillowy tits.  She pushes and squirms beneath you, but cannot get enough leverage to move you.  You take her breasts in your hands and shimmy up her torso towards them, squeezing them around your " + Desc.Cock.describeCock(player, x) + ".");
    ...
    // -Lust and Sens Draft (you you you you you need to find more ways to start sentences without using pronouns, she?) (Z edited)
    private doseDatChameleonWithLustAndSensitivityDrafts(player: Character) {
        DisplayText().clear();
        DisplaySprite(SpriteName.Chameleon);
        ...
        if (player.torso.cocks.count <= 0 || player.cockArea(player.torso.cocks.sort(Cock.SmallestCockArea)[0]) > monster.vaginalCapacity()) {
