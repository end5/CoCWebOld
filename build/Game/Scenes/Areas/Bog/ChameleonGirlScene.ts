...
export class ChameleonGirlScene {
	...
	public loseToChameleonGirl(): void {
		...
		if (player.gender === Gender.HERM && randInt(2) === 0) loseToChameleonWithCockAnBallsAnCunt();
		else if (player.gender === Gender.MALE || (player.gender === Gender.HERM && randInt(2) === 0)) dudesLoseToChamChams();
		else if (player.torso.vaginas.count > 0) loseToChamChamWithPCCunt();
		else {
	...
	private loseToChameleonWithCockAnBallsAnCunt(): void {
		...
		//[if(cockarea <40)
		if (player.cockArea(x) < 40) DisplayText(" and occasionally lifting it to take the shaft between two dexterous toes and work it more delicately");
	...
	private dudesLoseToChamChams(): void {
		let x: number = randInt(player.torso.cocks.count);
		DisplaySprite(89);
		...
		if (player.cockArea(x) <= monster.vaginalCapacity()) DisplayText("to the base of its shaft");
	...
	public defeatChameleonGirl(): void {
		...
		MainScreen.simpleChoices(["Use Dick", "Use Pussy", "Herm Style", "Use Item", "Leave"], [dick, pussy, herm, item, cleanupAfterCombat]);
	}
	...
	private manFucksChameleonWithBiggishWang(): void {
		...
		if (player.torso.balls.quantity > 0) DisplayText("your " + BallsDescriptor.describeBalls(true, true, player) + " clench and ");
		DisplayText("the semen erupts from the end of your " + CockDescriptor.describeCock(player, x) + " spilling out hot spunk over her folded tongue.  She gasps audibly and arches her back as the thick ropes of your jizz coat her tongue, shoving her long fingers as far as she possibly can into her dripping pussy, and flashes so brightly pink she nearly glows.");
	...
	private giveTheChameleonASuccubiMilk(): void {
		...
		DisplayText(" your cock aimed for her pillowy tits.  She pushes and squirms beneath you, but cannot get enough leverage to move you.  You take her breasts in your hands and shimmy up her torso towards them, squeezing them around your " + CockDescriptor.describeCock(player, x) + ".");
