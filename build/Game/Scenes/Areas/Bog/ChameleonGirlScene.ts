...
export interface ChameleonGirlFlags {
...
export function encounterChameleon(player: Character): NextScreenChoices {
    ...
    CombatManager.beginBattle(player, [], [monster]);
...
export function loseToChameleonGirl(player: Character, chameleonGirl: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Chameleon);
    // -Lose by lust
    if (player.stats.lust > 99) {
        DisplayText("Losing control to your own growing arousal, you fall to your knees and desperately start working to get at your needy body beneath your " + player.inventory.equipment.armor.displayName + ".");
    }
    // Lose by HP
    else DisplayText("Too weak to keep fighting, you stagger back and fall to your knees.");
    ...
    if (player.gender === Gender.HERM && randInt(2) === 0) return loseToChameleonWithCockAnBallsAnCunt(player);
    else if (player.gender === Gender.MALE || (player.gender === Gender.HERM && randInt(2) === 0)) return dudesLoseToChamChams(player, chameleonGirl);
    else if (player.torso.vaginas.count > 0) return loseToChamChamWithPCCunt(player, chameleonGirl);
    else {
        ...
        player.combat.stats.loseHP(20, chameleonGirl);
...
function loseToChameleonWithCockAnBallsAnCunt(player: Character): NextScreenChoices {
    ...
    DisplayText("\n\nAs you set the last bit of your " + player.inventory.equipment.armor.displayName + " aside in the water, you're suddenly struck in your chest and pushed to the ground.  The chameleon girl's wide foot keeps you pinned down, and she looks at you almost apologetically.  She shakes the worry from her face, and tries to appear stern once again.  \"<i>I'll... I'll show you who's the boss around here!</i>\"  Her tail darts towards you and uncoils at its end.  The tip starts probing around your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", brushing across your " + Desc.Vagina.describeClit(player) + " as it feels around for your entrance.");
...
// -Male Loss (Z edited)
function dudesLoseToChamChams(player: Character, chameleonGirl: Character): NextScreenChoices {
    const randCock = player.torso.cocks.random();
    DisplaySprite(SpriteName.Chameleon);
    DisplayText("\n\nRecognizing that you're no danger, her skin shifts back into its natural " + chameleonGirl.skin.tone + " coloration as she walks towards you.  She swings her hips seductively as she splashes slowly through the water and licks her lips with a loud smack of her tongue, then reaches out and forcefully pulls your " + player.inventory.equipment.armor.displayName + " above your waist before you can even try to resist her advances, and pushes you back against a tree.  With a few quick pulls of fabric she's stripped her own clothes, and she tosses her thong over a low-hanging branch.  The other band of silk she uses to quickly bind your wrists.  Were you in any shape to fight back you could pull free, but you're too ");
    if (player.stats.lust > 99) DisplayText("overwhelmed by lust");
    else DisplayText("battered");
    DisplayText(" to even rip the makeshift binding.");
    ...
    if (randCock.area <= chameleonGirl.vaginalCapacity()) DisplayText("to the base of its shaft");
    else DisplayText("as far as you can go into her");
    ...
    DisplayText("\n\nApparently, the chameleon girl is nearing the breaking point too.  A few inches of her tongue loll out of her mouth with more slipping out every time she pants or moans in pleasure, and her eyes look almost crossed as she stares blankly towards the base of your shaft.  Her hips begin to move a bit more erratically and she takes more of you into and out of her with each push.  Soon she's abandoned the slow, teasing motions and is just riding up and down your cock, slamming it in and out of herself in a desperate attempt to get off.  Her breasts bounce wildly and her head rolls back, with just a bit more of her tongue escaping each time she comes down on you.  The pleasure is too great, and you feel as if you could hardly stand it another moment.  The need to cum blares in your mind, pushing all other thoughts out.  The moans of the girl riding your " + Desc.Cock.describeCock(player, randCock) + " hardly register with you as you release your load into her");
    if (player.cumQ() > 500) DisplayText(", filling her with so much cum that it leaks out of her and drips to the water below");
    DisplayText(".  The chameleon girl's pace slows once again, and both of your breaths come in deeply as you each recover from orgasm.  She sucks her hanging tongue most of the way back into her mouth, grimacing as she wipes some mud from it.");
...
// -Female Loss (Z edited)
function loseToChamChamWithPCCunt(player: Character, chameleonGirl: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Chameleon);
...
// VICTORY SCENES INTRO(Z edited)
export function defeatChameleonGirl(player: Character, chameleonGirl: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Chameleon);
    // -Win by lust
    if (chameleonGirl.stats.lust > 99) {
        DisplayText("Unable to control her arousal, the chameleon girl collapses to her knees and begins masturbating underneath her thong, having lost all capacity to fight you; she moans and throws her head back as her hand splashes in and out of the water she's kneeling in.  Her skin returns to its usual " + chameleonGirl.skin.tone + " and then keeps going, shifting closer and closer to pink as her moans increase in both volume and volubility.");
        if (player.stats.lust >= 33) DisplayText("  Do you help the horny girl to get you both off?  Or maybe you could punish her bad attitude with something from your bag...");
    }
    // -Win by HP
    else {
        DisplayText("Too weak to continue fighting, the chameleon girl drops to her knees, exhausted.  Her skin returns to its usual " + chameleonGirl.skin.tone + ", unable to maintain the camouflage.");
        if (player.stats.lust >= 33) DisplayText("  Do you use the girl in her weakened state to get yourself off?  Or maybe you could punish her bad attitude with an item from your bags...");
    }
    if (player.stats.lust < 33 || player.gender === Gender.NONE) {
        return { next: Scenes.camp.returnToCampUseOneHour };
    }
...
function fuckDatChameleonAsACoolGuyGirlHerm(player: Character, chameleonGirl: Character): NextScreenChoices {
    ...
    if (player.stats.cor < 50) DisplayText("and guiltily ");
    DisplayText("roll her onto her back and get up to collect your things, rinsing yourself with the water and shaking the mud out before putting your " + player.inventory.equipment.armor.displayName + " back on and heading back to camp.");
    player.orgasm();
    return { next: Scenes.camp.returnToCampUseOneHour };
}
...
function manFucksChameleonWithBiggishWang(player: Character, chameleonGirl: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Chameleon);
    const largestCock = player.torso.cocks.sort(Cock.LargestCockArea)[0];
    ...
    if (player.torso.balls.quantity > 0) DisplayText("your " + Desc.Balls.describeBalls(true, true, player) + " clench and ");
    DisplayText("the semen erupts from the end of your " + Desc.Cock.describeCock(player, largestCock) + " spilling out hot spunk over her folded tongue.  She gasps audibly and arches her back as the thick ropes of your jizz coat her tongue, shoving her long fingers as far as she possibly can into her dripping pussy, and flashes so brightly pink she nearly glows.");
    if (player.cumQ() > 750) DisplayText("  The torrent of cum bursting from your cock just doesn't stop, and her eyes roll back into her head as your semen coats her face and runs off her tongue, drenching her breasts and belly in white.  Her head falls back and her tongue goes limp as she's sent into ecstasy by the flood.");
    ...
    if (player.cumQ() > 750) DisplayText("  Even the murky water around her is turned white as the pools of your cum on her body run off into it.");
    DisplayText("  Satisfied, you put your " + player.inventory.equipment.armor.displayName + " back on and head back to camp.");
...
// -Female (for herms without dicks) (Z edited)
function femaleHasWinSexWithChamCham(player: Character, chameleonGirl: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Chameleon);
    DisplayText("You shed your " + player.inventory.equipment.armor.displayName + " and approach the fallen chameleon girl");
    if (chameleonGirl.stats.lust > 99) DisplayText(", her moans of pleasure growing more desperate as you approach");
    DisplayText(".  You look her up and down, following the smooth curves of her thin body with your eyes.  Squatting down beside her, you take her weak body by the shoulders in one arm and trail your fingertips lightly up her torso with the other, feeling the soft skin of her belly.");
    if (chameleonGirl.stats.lust > 99) DisplayText("  She coos lustily at the sensation.");
    else DisplayText("  Her eyes widen, and she looks from your hand to your face and back again.");
    DisplayText("  When your hand reaches the silk cloth covering her chest, you pull it off with a light tug and cast it off to the side.  You cup her small breast in your palm, and as you squeeze it her skin tone changes perceptibly, flashing to a much brighter shade of the colors it was.");
    ...
    if (player.stats.cor < 50) DisplayText("and somewhat guiltily ");
    DisplayText("roll her onto her back and get up to collect your things, shaking out the mud and water before putting your " + player.inventory.equipment.armor.displayName + " back on and heading to camp.");
    // send player back to camp, reset hours since cum, add gems and time
    player.orgasm();
    return { next: Scenes.camp.returnToCampUseOneHour };
}
...
// Item Use Scenes Intro (Victory) (Z edited)
function useAnItemOnTheChamcham(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Chameleon);
    DisplayText("Looking at the poor girl kneeling pathetically before you, you almost feel sorry for her.  But, she did try to rape you and claim that you were on 'her' territory.  You rummage through your bags, trying to find a fun way to punish her.  Fun for you, at least.");
    ...
    let milk;
    let drafts;
    if ((player.inventory.items.has(ConsumableName.SuccubiMilk) || player.inventory.items.has(ConsumableName.SuccubiMilkPure)) && player.torso.cocks.count > 0) milk = giveTheChameleonASuccubiMilk;
    if (player.inventory.items.has(ConsumableName.SensitivityDraft) && (player.inventory.items.has(ConsumableName.LustDraft) || player.inventory.items.has(ConsumableName.LustDraftEnhanced))) drafts = doseDatChameleonWithLustAndSensitivityDrafts;

    return { choices: [["SuccMilk", milk], ["LustnSensD.", drafts], ["Back", defeatChameleonGirl]] };
}
...
function giveTheChameleonASuccubiMilk(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Chameleon);
    DisplayText("You find a bottle of Succubi Milk tucked away in a pouch.  A wicked smile comes to your face as you imagine filling her so full of the stuff that her tits grow to the point where she can't even move, but you ultimately decide that just one dose will suffice for now.");
    ...
    if (player.hasLongTail()) DisplayText(" with your tail,");
    else DisplayText(" by straddling her,");
    DisplayText(" your cock aimed for her pillowy tits.  She pushes and squirms beneath you, but cannot get enough leverage to move you.  You take her breasts in your hands and shimmy up her torso towards them, squeezing them around your " + Desc.Cock.describeCock(player, largestCock) + ".");
...
// -Lust and Sens Draft (you you you you you need to find more ways to start sentences without using pronouns, she?) (Z edited)
function doseDatChameleonWithLustAndSensitivityDrafts(player: Character, chameleonGirl: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Chameleon);
    ...
    if (player.torso.cocks.count <= 0 || player.torso.cocks.sort(Cock.SmallestCockArea)[0].area > chameleonGirl.vaginalCapacity()) {
