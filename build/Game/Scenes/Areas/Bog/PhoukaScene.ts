...
export class PhoukaScene implements TimeAwareInterface {
    ...
    public timeChange(player: Character): boolean {
        if (player.statusAffects.get(StatusAffectType.PhoukaWhiskeyAffect).value1 > 0) {
            player.statusAffects.get(StatusAffectType.PhoukaWhiskeyAffect).value1 = -1; // Count down hours until player is not drunk
            if (player.statusAffects.get(StatusAffectType.PhoukaWhiskeyAffect).value1 <= 0) {
    ...
    public phoukaNameText(known: string, unknown: string): string { // Helper function, For any text that depends on whether or not the player knows what to call a phouka
        return (Flags.list[FlagEnum.PHOUKA_LORE] === 0 ? unknown : known);
    }
    ...
    public phoukaEncounter() { // General entry point for everything except halloween special encounter
        phoukaForm = PHOUKA_FORM_FAERIE; // Reset to faerie form at the start of any encounter
        let choiceChance: number = 0;
        if (Flags.list[FlagEnum.PHOUKA_ENCOUNTER_STATUS] === 0) { // Guarantee first Phouka encounter is with faerie fire
            Flags.list[FlagEnum.TREACLE_MINE_YEAR_DONE] = date.fullYear - 1; // If you've never encountered phoukas before then we can safely set all this stuff here
            Flags.list[FlagEnum.BIRTHS_PHOUKA] = 0;
            Flags.list[FlagEnum.BIRTHS_FAERIE] = 0;
        }
        else if ((date.month > 4) && (date.month < 11))
            choiceChance = randInt(7); // All through the late summer and autumn they might be masturbating on berries
        else choiceChance = randInt(6);
        if (choiceChance < 2)
            phoukaFaerieFire();
    ...
    protected phoukaFaerieFire(player: Character) { // In old Irish legends the Phouka are responsible for Will-o’-the-wisps. In the deep woods it’s Kitsune, but in the bog it’s a Phouka
        DisplayText().clear();
        DisplayText("As you’re wading through the bog a flickering light off in the distance catches your eye.  ");
        if (Flags.list[FlagEnum.PHOUKA_ENCOUNTER_STATUS] !== 0) {
            DisplayText("You recall your last encounter with one of these 'lanterns' out here in the bog.\n\nPerhaps it would be best to leave this faerie fire alone.");
        }
        else { // Special text for the first encounter
            DisplayText("It seems to be moving around in one of the dense parts of the bog, where the canopy of dead trees and creeping vines create shadows as deep as twilight even at noon.\n\nYou can't make out a figure holding the light, but you decide to follow.  Spurred on by the thought that it may be someone else like yourself, or perhaps some agent of the demons, you keep trying to catch up with the light up ahead.  This trip into the bog is about to pay off.");
        ...
        if (Flags.list[FlagEnum.PHOUKA_ENCOUNTER_STATUS] !== 0 || player.hasKeyItem("Traveler's Guide") >= 0) MainScreen.addButton(4, "Leave", phoukaFaerieFireLeave);
        MainScreen.addButton(0, "Follow", phoukaStuck);
    }

    protected phoukaFaerieFireLeave() {
        if (Flags.list[FlagEnum.PHOUKA_ENCOUNTER_STATUS] === 0)
        ...
        return { next: Scenes.camp.returnToCampUseOneHour };  // Return to camp, 1 hour used
    }

    protected phoukaStuck(player: Character) {
        DisplayText().clear();
        const hasAlcohol: boolean = player.inventory.items.has(consumables.P_WHSKY, 1);
        DisplayText("You follow some firmer ground into one of the many copses of dead trees.  The bog is quiet and still here.  Up ahead you see something glint in the hollow of a dead tree, but before you can investigate the firm ground beneath your feet liquefies and you begin to sink.\n\nYou try to leap to another muddy lump of exposed ground, but even before you land it has gone soft as well.  You are soon encased in mud almost to your waist.\n\nThat's when you feel something slick and warm dart past you in the muck.  Wherever it goes it drags a thick net behind it.  Your ");
        if (player.torso.hips.legs.isNaga()) DisplayText("tail is");
        else DisplayText("legs are");
        DisplayText(" soon tangled in a solid web of intertwined roots.  Once you are bound in place, a large black eel launches itself free of the muck and lands in a nearby puddle.\n\nYour attacker seems to melt, then reforms, becoming a five inch tall " + phoukaName() + ".  His skin and wings are coal black, and his eyes are green and shiny like those of a cat.  He isn't wearing any clothes, and a fully erect cock almost an inch long juts out of his groin.  Apart from that and a lack of breasts, his body looks feminine, much like the faeries of the woods.\n\nThe " + phoukaName() + " leers at you, clearly deciding what he wants to do next.");
        if (hasAlcohol) DisplayText("\n\nYou suspect that if you offer up that whiskey in your pack the " + phoukaName() + " probably won’t try to rape you.");
        if (Flags.list[FlagEnum.PHOUKA_ENCOUNTER_STATUS] === 0) Flags.list[FlagEnum.PHOUKA_ENCOUNTER_STATUS] = 1;

        if (hasAlcohol) MainScreen.addButton(0, "Offer Drink", phoukaStuckOfferWhiskey);
        MainScreen.addButton(1, "Talk", phoukaTalk);
        MainScreen.addButton(2, "Fight", phoukaStartFight);
    }

    protected phoukaStuckOfferWhiskey(player: Character) {
        DisplayText().clear();
        DisplayText("You pull out the bottle and ask the " + phoukaName() + " if he might be interested in a drink.  He looks at the bottle and says <i>“Well we both know where ya picked that up, hmmm?”</i>  Then he smiles and adds <i>“Course that’s some other phouka’s problem if he let ye beat him.”</i>");
        if (Flags.list[FlagEnum.PHOUKA_LORE] === 0) Flags.list[FlagEnum.PHOUKA_LORE] = 1; // Now you know what to call them
        player.inventory.items.consumeItem(consumables.P_WHSKY, 1);
        ...
        if ((player.pregnancyIncubation === 0) && (player.buttPregnancyIncubation === 0)) {
            ...
            MainScreen.addButton(1, "Drink", phoukaDrinkAccept);
        }
        else phoukaDrinkWhilePregnant(true);
    }

    protected phoukaMasturbating() { // Old Irish folklore again - Phoukas were supposed to be responsible for the berries rotting away at the end of the year
        DisplayText().clear();
    ...
    protected phoukaStartFightSilenced(player: Character) {
        startCombat(new Phouka(phoukaName()));
        player.statusAffects.add(StatusAffectType.WebSilence, 0, 0, 0, 0);
    }

    protected phoukaTalk(player: Character) {
        DisplayText().clear();
        DisplayText("You ask the " + phoukaName());
        if (player.level < 10)
            DisplayText(" if perhaps there's some way he could release you.");
        else DisplayText(" if he really wants a taste of what you're going to give him.");

        if (player.stats.cor + randInt(125) <= 110) { // Players with 0% corruption have a 12% chance that the Phouka is willing to talk, players with 100% corruption have a 92% chance
            // The phouka is not in the mood for talk, start the fight gagged as punishment for trying to talk
            DisplayText("  You open your mouth to say something more to the ");
            if (player.stats.cor < 34)
                DisplayText("vile little monster");
            else if (player.stats.cor < 67)
                DisplayText("perverted " + phoukaName());
            else DisplayText("fuckable little " + phoukaName());
            DisplayText(" but he must not feel like listening.  He launches a ball of black mud at your face, sealing your mouth with sticky and [if (corruption <= 50)ewww -][if (corruption > 50)mmmm -] salty muck.");
            startCombat(new Phouka(phoukaName()));
            player.statusAffects.add(StatusAffectType.WebSilence, 0, 0, 0, 0);
        }
        else { // The phouka would rather talk
            DisplayText("\n\nThe " + phoukaName());
            switch (player.level < 10 ? randInt(8) : randInt(12)) { // Low level characters don't get the chance to be offered whiskey
                case 0:
                case 1:
                case 2:
                case 3:
                ...
                case 6: // Offers you a chance to submit, which leads to the various defeat sex scenes
                    ...
                    if (player.torso.vaginas.count > 0)
                        DisplayText("<i>“Maybe yer inta bunnies, hrm?  Big bunny with a nice hard cock and strong legs, I can do that fer ya.  Fill that tight little muff o' yers up with a big load o' spunk!”\n\n“Or maybe ");
                    else
                        DisplayText("<i>“");
                    DisplayText("I could change inta a goat for ya.  That suit yer fancy?  Is it a fond dream o' yers to let a goat blow its load in yer cornhole?");
                    if (player.torso.vaginas.count > 0)
    ...
    protected phoukaDrinkWhilePregnant(player: Character, playerOfferedTheBooze: boolean) {
        ...
        if (acceptable < 0) {
            ...
            if (acceptable === -4) DisplayText("  One of them seems to crave the whiskey, but the other is a more normal child.");
            DisplayText("  Sure, some of the elders around the village said a beer or some light cider was all right for a woman heavy with child, but you doubt that whiskey would be acceptable.  You politely refuse the drink, hoping the phouka won't be offended.");
        }
        else {
            DisplayText("\n\nYou put one hand on your belly, about to refuse the drink.  Then you realize that yes, this is exactly what your ");
            if (acceptable <= 2)
                DisplayText("baby needs.  Some light kicks inside your " + (acceptable === 2 ? "distended colon" : "womb") + " suggest that your child knows");
            else
                DisplayText("babies need.  Some light kicks inside your womb and your distended colon suggest that your children know");
            DisplayText(" what's up for grabs.  <i>“Go on and have some already. I try not to drink alone.  Always end up fucked by an imp or a drider or something that way.”</i>");
        }

        MainScreen.addButton(0, "Refuse", (playerOfferedTheBooze ? phoukaDrinkRefuseAfterOffer : phoukaDrinkRefuse));
        if (acceptable) MainScreen.addButton(1, "Drink", phoukaDrinkAccept);
    }

    protected phoukaDrinkAccept(player: Character) { // In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
        DisplayText().clear();
        ...
        switch (consumables.P_WHSKY.phoukaWhiskeyDrink(player)) {
            case 0: // Player isn't pregnant
                dynStats("lus", randInt(5) + 1, "cor", randInt(2) + 1); // Minor lust and corruption gains (since you aren’t drinking it all yourself). These gains are permanent
                break;
            case 1: // Child is a phouka or satyr, loves alcohol
                DisplayText("A warm feeling in your belly lets you know your passenger enjoyed the meal as well.");
                break;
        ...
        if (player.stats.tou < 70) {
            DisplayText("  It looks like the phouka is holding his liquor about as well as you are.");
        }
        else {
            DisplayText("  It seems you can handle your liquor better than the phouka.");
        }
        DisplayText("  Soon both of you start to share stories about your adventures in Mareth.");
        phoukaDrinkTalk(false);
    }

    protected phoukaDrinkRefuse(player: Character) { // In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
        DisplayText().clear();
        if ((player.pregnancyIncubation === 0) && (player.buttPregnancyIncubation === 0) && (player.pregnancyIncubation <= 100)) { // Pregnancy is obvious to the phouka
            DisplayText("The rabbit morph sits down heavily on the wet ground and sips his booze.  <i>“Don't want to fuck, don't want to drink.  What gives?  Live a little and let me have fun with your pregnant pussy.  I’ll get ya off, and it’s not like I can knock you up again.  Course, that'd be fun too.”</i>");

            phoukaSexAddStandardMenuChoices();
            MainScreen.addButton(0, "Leave", phoukaDrinkRefusePregnantLeave);
        }
        else { // Less obvious pregnancy or player not pregnant
            DisplayText("<i>“You damn prude.  That does it! I don't have to sit around listening to this.  I try to be friendly like some people say ya should instead of just raping ya.  What's it get me, huh?”</i>");
            if ((player.stats.lust > 66) || ((player.stats.cor > 50) && (player.stats.lust > 33))) {
                ...
                MainScreen.addButton(0, "Leave", phoukaDrinkRefuseLeave);
        }
    }

    protected phoukaDrinkRefuseLeave() { // In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
        DisplayText().clear();
        DisplayText("You extract yourself from the mud and get out of the bog before the phouka changes its mind.");
        return { next: Scenes.camp.returnToCampUseOneHour };  // Return to camp, 1 hour used
    }

    protected phoukaDrinkRefusePregnantLeave() { // In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
        DisplayText().clear();
        DisplayText("<i>“Ah fuck.  You're a killjoy.  Don't you know the best time to get porked is when you've already got a bun in the oven?”</i>\n\nThe black bunny waves you away dismissively. <i>“Fine, yeah, go home.  I hope the little bastard hurts when it pops out.  I would have stretched you out but good.”</i>");
        return { next: Scenes.camp.returnToCampUseOneHour };  // Return to camp, 1 hour used
    }

    protected phoukaDrinkRefuseAfterOffer(player: Character) {
        DisplayText().clear();
    ...
    protected phoukaDrinkTalk(player: Character, playerNotDrinking: boolean) {
        if (Flags.list[FlagEnum.PHOUKA_LORE] === 1) {
            Flags.list[FlagEnum.PHOUKA_LORE]++;
        ...
        else if (Flags.list[FlagEnum.PHOUKA_LORE] === 2) {
            Flags.list[FlagEnum.PHOUKA_LORE]++;
        ...
        else if (Flags.list[FlagEnum.PHOUKA_LORE] === 3) {
            Flags.list[FlagEnum.PHOUKA_LORE] += 1;
        ...
        else if (Flags.list[FlagEnum.PHOUKA_LORE] === 4) {
            // More can come later, right now we don’t increment here //Flags.list[FlagEnum.PHOUKA_LORE] += 1;
        ...
        const playerIsInebriated: boolean = (playerNotDrinking === false) && (player.stats.tou < 70);
        if (playerIsInebriated) {
            ...
            MainScreen.addButton(0, "Leave", phoukaDrinkLeave);
        }
        else {
            ...
            DisplayText("  He lets out a little giggle and begins to shrink. Tendrils of black smoke rise from the bunny morph’s black fur as he shrinks back to the size and shape of a faerie.");
            phoukaForm = PHOUKA_FORM_FAERIE;
            if (player.stats.lust < 33)
                DisplayText("  It looks like he needs a nap and you take that as your cue to get out of here.");
            else {
                DisplayText("\n\nYou look at the sleeping phouka");
                if (player.torso.cocks.count > 0)
                    DisplayText(", [eachCock] beginning to swell" + (player.torso.vaginas.count > 0 ? " and your [vagina] starting to moisten" : "") + ".");
                else if (player.torso.vaginas.count > 0)
                    DisplayText(", your [vagina] starting to moisten.");
                else DisplayText("and you feel your sphincter twitch as your brain thinks about what you could do with this phouka now."); // Genderless
            }

            MainScreen.addButton(0, "Leave", phoukaDrinkLeave);
            if (player.stats.lust >= 33) {
                MainScreen.addButton(1, "Ride Faerie", phoukaSexFaerieNonCombat); // Works for all
                if (player.torso.cocks.count > 0) MainScreen.addButton(2, "Fuck Faerie", phoukaSexFaerieFemaleNonCombat); // Male or Herm Only
            }
        }
    }

    protected phoukaDrinkLeave(player: Character) {
        if (player.stats.lust < 33)
            DisplayText("\n\nIf you were a bit hornier");
        else
            DisplayText("\n\nIf you wanted");
        DisplayText(" you could take advantage of the phouka’s predicament, but right now you’d rather go and check on the portal.  Thanks to the whiskey, the water soaking your [legs] doesn't matter to you quite so much as you slog back towards camp.");
        return { next: Scenes.camp.returnToCampUseOneHour };  // Return to camp, 1 hour used
    }

    public phoukaHalloween(player: Character) {
        Flags.list[FlagEnum.TREACLE_MINE_YEAR_DONE] = date.fullYear;
        DisplayText().clear();
        ...
        if (player.stats.str < 40) {
            DisplayText("You find it very difficult to operate and have to squeeze it with your thighs, but you manage to pump a respectable amount before passing it on.");
        }
        else if (player.stats.str < 80) {
            DisplayText("You find that treacle pumping is as hard as the phouka said, but you keep at it.  With what you pumped you must be as strong as they are.");
        }
        else {
            DisplayText("You don't know why they said this was difficult.  You have no problem pumping the treacle; in fact they have to stop you partway through to change barrels.");
        }
        ...
        if (player.stats.int > 80) DisplayText("\n\nYou're about to accept when you notice how a few of the phouka in the back are looking at you.  It's a look you recognize from merchants who used to stop in Ingnam to sell their wares.  The 'a sucker born every minute' look.  You realize none of the phouka have so much as tasted any treacle and you start to wonder what this sugary substance really is.");

        MainScreen.addButton(0, "Taste", phoukaHalloweenTreacleEater);
        if (player.stats.int > 80) MainScreen.addButton(4, "Leave", phoukaHalloweenLeave);
    }

    protected phoukaHalloweenLeave() {
        DisplayText("\n\nFeeling you're better off not finding out you excuse yourself, telling them they can enjoy the treacle amongst themselves.  As you walk away you just catch the boss telling the others <i>“Watch out fer that one, " + Desc.Gender.mf(player, "He", "She") + "'s a smart one.”</i>");
        return { next: Scenes.camp.returnToCampUseOneHour };  // Return to camp, 1 hour used
    }

    protected phoukaHalloweenTreacleEater() {
        DisplayText().clear();
    ...
    public phoukaPregBirth(player: Character) {
        ...
        if (Flags.list[FlagEnum.PREGNANCY_CORRUPTION] > 6) { // You’ve been drinking like a fish, haven’t you?
            ...
            fatigue(5);
            Flags.list[FlagEnum.BIRTHS_FAERIE]++;
        }
    }

    public phoukaPregUpdate(player: Character): boolean { // Belly size doesn't change, instead you get updates on what's going on
        if (player.pregnancyIncubation === 170) { // Stage 1:
            if (Flags.list[FlagEnum.PREGNANCY_CORRUPTION] > 0)
        ...
        if (player.pregnancyIncubation === 100) { // Stage 3:
            ...
            DisplayText(" You've noticed that this pregnancy doesn't seem to be affecting your breasts. It's as if the child inside you has no use for your milk.\n");
            return true;
        }
        if (player.pregnancyIncubation === 60) { // Stage 4:
            if (Flags.list[FlagEnum.PREGNANCY_CORRUPTION] > 6)
        ...
        if (player.pregnancyIncubation === 36) { // Stage 5:
            ...
            if (Flags.list[FlagEnum.PREGNANCY_CORRUPTION] > 6)
                DisplayText("Despite the small size of your belly you spend most of your time feeling deeply ill and you can't wait to force this thing out.  You can feel a constant dull pain from your womb and ovaries, probably the result of the tainted fluid inside you.  Only drinking alcohol settles your stomach.\n");
            else if (Flags.list[FlagEnum.PREGNANCY_CORRUPTION] > 0)
                DisplayText("Despite the relatively small size of your belly, you spend most of your time feeling mildly ill and you can't wait to force this thing out.\n");
            else DisplayText("Despite being pregnant you still don't feel restricted or slowed in any way.  It's as if the tiny life inside is living in harmony with you.\n");
            return true;
        }
        return false;
    }

    protected phoukaSexAddStandardMenuChoices(player: Character) { // This happens several times so it's broken out here in case additional options get added later
        if (player.torso.vaginas.count > 0) {
            MainScreen.addButton(1, "Bunny", phoukaSexBunnyChoice);
            MainScreen.addButton(2, "Horse", phoukaSexHorseChoice);
        }
        MainScreen.addButton(3, "Goat", phoukaSexGoatChoice);
    }

    private phoukaSexBunny(player: Character, postCombat: boolean, lustLoss: boolean) { // Player must have a vagina for bunny-morph sex
        DisplayText().clear();
        if (postCombat) {
            DisplayText("As you collapse the " + phoukaName());
            if (phoukaForm === PHOUKA_FORM_BUNNY)
                DisplayText(" hops over and stands in front of you");
            else
                DisplayText(" begins to shapeshift in front of you.  He changes into various forms, looks you over and finally changes into a black furred bunny-morph");
            DisplayText(".  You watch as the bunny's erection stiffens, your ");
            if (lustLoss)
                DisplayText("mind so overcome with lust you can't");
            else DisplayText("body too bruised to");
            DisplayText(" resist any further.");
        }
        else {
            if (player.stats.cor <= 50) {
                DisplayText("It might not be the wisest move, but you decide to let the " + phoukaName() + " have his way with you.  You tell him you would prefer a little bunny loving and the " + phoukaName() + " grins from ear to ear. <i>“Comin' right up,”</i> he says");
                if (phoukaForm !== PHOUKA_FORM_BUNNY) DisplayText(", and morphs into a rabbit");
                DisplayText(".  Hopefully he'll be gentle with a willing partner.");
            }
            else {
                DisplayText("You can't think of any good reason to refuse a roll in the hay (or the mire) with this " + phoukaName() + ".  You tell him that you want to feel his big bunny cock deep inside you.  The " + phoukaName() + " grins as you begin seductively stripping off you armor.");
                if (phoukaForm !== PHOUKA_FORM_BUNNY) DisplayText("  He goes through a short series of warm up exercises and then transforms himself into the rabbit you desire.");
            }
        }
        phoukaForm = PHOUKA_FORM_BUNNY;
        ...
        if ((player.stats.lust > 80) || (player.stats.cor > 50))
            DisplayText(" You can only moan in response.  The thought of this rabbit stuffing you has your [vagina] leaking already.");
        else
            DisplayText(" You are powerless to stop the bunny-morph as he undoes the last few straps holding your [armor] in place.");
        ...
        if (player.hasVirginVagina())
            DisplayText("You scream as the " + phoukaName() + "'s cock tears your hymen apart.  He stops thrusting and says <i>“A virgin? Here? Good goddess, missy, ya should have told me.”</i> In a lower tone he adds <i>“so I could have enjoyed it more.”</i> Your whole vagina burns as every fold is stretched and widened by the passage of the rabbit's cock.");
        else if (postCombat && (player.torso.vaginas.get(0).looseness < 2))
            DisplayText("You nearly pass out from the pain as the " + phoukaName() + "'s cock lengthens and thickens inside you.");
        else if (!postCombat || (player.stats.cor > 50))
            DisplayText("Even as the invader lengthens and thickens you push back against it. <i>“Good girl. Good little fuck slut,”</i> whispers the " + phoukaName());
        Mod.Vagina.displayStretchVagina(player, 50, true);
    ...
    protected phoukaSexBunnyChoice() {
        phoukaSexBunny(false, false);
    }

    private phoukaSexGoat(postCombat: boolean, lustLoss: boolean) { // Player does not have a vagina or has chosen anal
        DisplayText().clear();
        if (postCombat) {
            if (lustLoss)
                DisplayText("Panting with need, you begin to strip off your [armor].  The " + phoukaName() + " watches the show with an evil smile.");
            else
                DisplayText("Your body falls to the ground, too weak to fight back against the " + phoukaName() + " any longer.");
            if (phoukaForm !== PHOUKA_FORM_GOAT) DisplayText("  As you lay there, the " + phoukaName() + "'s body stretches and morphs until it takes the form of a large, black furred goat-morph.");
        }
        else {
            DisplayText("You tell the " + phoukaName() + " you're willing to let him take your ass in his goat form.  He smiles and lands near you.  His body warps into the form of a black furred goat morph.  ");
            if (player.stats.cor <= 50)
                DisplayText("You steel yourself for an asspounding while you remove your [armor].");
            else DisplayText("Then you put on a show, taking your clothes off piece by piece.  You strip off your final undergarment, bend over and wink your asshole at the goat.");
        }
        ...
        if (postCombat)
            DisplayText("He mounts you immediately, giving you no chance to adjust to his invading cock.  The goat-morph's first powerful thrust drives your face into the muck as his tries to force his narrow cockhead into your ass.");
        else
            DisplayText("The goat rubs his cockhead against your pucker, spreading a thick layer of pre over your hole before slowly pressing home his cock.  His first few thrusts are gentle enough that you have the chance to spread your ass and get used to this invader.");
        DisplayText("  Disturbingly you feel his front 'hooves' split into hard little fingers that he wraps around your [if (isTaur = true)belly][if (isTaur = false)[chest]] to hold onto you even tighter.\n\n");
        if (postCombat)
            DisplayText("The " + phoukaName() + " shows no interest in you except as a receptacle for his cum.  After stabbing his hips at you several times you feel the full length of his cock penetrate your pucker.");
        else
            DisplayText("Once your [butt] has adjusted, the goat morph begins to thrust more quickly.  You are surprised when you feel his balls slapping against your [if (isNaga = true)tail][if (isNaga = false)inner thighs] - he can't already be in that deep, can he?");
        DisplayText("  Once the goat's cock is inside, he stops moving and tenses up.  You feel his cock growing larger and larger until you fear your abused anus is going to split apart.");
        Mod.Butt.displayStretchButt(player, 45, true);
        ...
        if (postCombat)
            DisplayText("The goat-morph waits only a few seconds before unceremoniously yanking his deflating cock from your ass.");
        else
        ...
        dynStats("cor", randInt(1) + (postCombat ? 1 : 3)); // Extra two corruption for being enough of a pervert to want to fuck the phouka
        player.orgasm();
        if (postCombat) DisplayText("  While you're recovering the goat-morph reaches into your gem pouch and takes a handful.");
    ...
    protected phoukaSexGoatChoice() {
        phoukaSexGoat(false, false);
    }

    private phoukaSexHorse(postCombat: boolean, lustLoss: boolean) { // Player has a vagina
        DisplayText().clear();
        if (postCombat) {
            DisplayText("As you collapse the " + phoukaName());
            if (phoukaForm === PHOUKA_FORM_HORSE)
                DisplayText(" circles you in a slow trot, his top half shaping into a centaur form before finally stopping directly in front of you.");
            else
                DisplayText(" begins to shapeshift in front of you.  It grows and grows, finally taking the form of a massive, black, centaur stallion with an equally massive prick.");
        }
            ...
            if (player.stats.cor <= 50)
                ...
                DisplayText("The more you think about it the more you decide you could use a real fucking.  You smile at the " + phoukaName() + ", telling him you want the biggest cock he can muster.  The " + phoukaName() + "morphs into the form you expected, that of a black, centaur stallion [if (tallness <= 96)taller than you are][if (tallness > 96)that's about your height].  Maybe that tree trunk between his legs can scratch your itch.");
        }
        ...
        if (!postCombat || lustLoss)
            DisplayText("Considering the fire in your vagina that's crying out to be quenched, you don't care how big this horse's cock is.  You just need something to fill you NOW.");
        else
            DisplayText("You're hardly in any condition to refuse.  This stallion is going to pound you with that meatstick no matter what you do[if (corruption <= 50), so you steel yourself for what is to come][if (corruption > 50).  The thought of being wrecked by such an enormous staff has your cunt dripping girl-cum already.]");
        ...
        if (player.torso.hips.legs.isTaur())
            ...
            if (player.torso.hips.legs.isNaga())
                DisplayText("He walks in front of you, turns your way and then simply walks over you, forcing your upper body to bend backward over your tail.  Thankfully your snake-like spine means bending into a 'U' shape isn't that uncomfortable.");
            else
                DisplayText("He walks up behind you and then simply walks right over top of you, his chest knocking you to the ground.  You find yourself on all fours with a massive cockhead grinding against your ass.");
            DisplayText("  You hear the " + phoukaName() + " say <i>“Now line me up with yer cunt lass, or I'll lay down on top of ya and have a nap.”</i> Not wanting to be crushed under that kind of bulk, you reach [if (isNaga = true)up][if (isNaga = false)behind you] and grab his member.  You place his cock at the entrance to your [vagina] and the stallion steps forward.  [if (vagCapacity < 50)His cock bends and then snaps free of your cunt, slapping against his belly. <i>“Again! And do it right this time!”</i> he shouts.  You line it up once more and press against it with your hands so it has nowhere to go but in.]");
        }
        DisplayText("  You're convinced you're going to be split in half by this monster horse cock.\n\n");
        if (player.hasVirginVagina())
            DisplayText("You scream as the " + phoukaName() + "'s cock tears your hymen apart.  He stops mid-thrust and asks <i>“A virgin? Here? Good goddess missy ya should have told me.”</i> In a lower tone he adds <i>“so I could have enjoyed it more.”</i> Your whole vagina burns as every fold is stretched and widened by the passage of the " + phoukaName() + "'s huge horse cock.  You think he's delighted to have taken a woman from virgin to gaping in just one thrust.  ");
        else if ((postCombat) && (player.torso.vaginas.get(0).looseness < 3))
            DisplayText("You nearly pass out from the pain as the " + phoukaName() + "'s cock drives deeper and deeper inside you.  With such a wide head forcing its way in, you're sure the stretching will be permanent.  ");
        else if (!postCombat || (player.stats.cor > 50))
            DisplayText("Even as the invader thrusts deeper you push back against it. <i>“Good girl, ya love horse cock, don't ya?”</i> whispers the " + phoukaName() + ".  ");
        Mod.Vagina.displayStretchVagina(player, 80, true);
        ...
        if ((player.stats.lust > 80) || (player.stats.cor > 50))
            DisplayText("You pant and moan, waiting for the inevitable flood from his cantaloupe-sized nuts.  You're ready to get off just from the feeling of being so stuffed, so violated.");
        else {
            DisplayText("You whimper in pain, hoping the " + phoukaName() + " will soon be done with you.  ");
            if (player.torso.hips.legs.isTaur())
                DisplayText("You feel so stretched and full.  What worries you is that the pain is giving way to pleasure.  Some base instinct of your centaur body is telling you that as a mare you were meant to be filled like this.  Your cunt was built for cocks like this so you could carry lots of little centaur foals.");
            else
                DisplayText("You're so stretched that your blood flow is being cut off and you can't feel [if (isNaga = true)the end of your tail][if (isNaga = false)your legs] anymore.");
        }
        DisplayText("  You feel those cum-stuffed balls twitch once, twice, three times.  The pressure must be building in the " + phoukaName() + "'s prostate.  He groans <i>“[if (isPregnant = true)Time te feed the baby.][if (isPregnant = false)Maybe today I'll leave ye a present.]”</i>  When he cums it's not a series of ripples moving up his urethra, it's a single long blast of stallion seed.");
        phoukaSexPregnate(postCombat);
    }

    protected phoukaSexHorseChoice() {
        phoukaSexHorse(false, false);
    }

    protected phoukaSexFaerie(postCombat: boolean) {
        DisplayText().clear();
        ...
        if (player.torso.vaginas.count > 0) {
            DisplayText("[vagina].");
            Mod.Vagina.displayStretchVagina(player, 20, true);
            ...
            DisplayText("[butt].");
            Mod.Butt.displayStretchButt(player, 20, true);
            ...
            if (player.torso.hips.legs.isTaur())
                DisplayText("You sigh in frustration as the " + phoukaName() + " pulls free from your ass, obviously about to leave.  He notices your condition and thinks better of it. <i>“Just gimme a sec.  I’ll get ye off,”</i> he says. <i>“After all, I do want ye to come back fer more.”</i>\n\nHe starts jacking himself off, whispering something to himself under his breath.  When his cock looks nice and hard he grabs your flanks and drives the full length back inside you.  As he pistons into your rectum, you feel that denied orgasm building and at last you cum.");
            else
                DisplayText("Not yet satisfied, you wait for the flood to stop, then begin to really ride your man-sized faerie.\n\nYou don't give him the chance to go soft.  Your [if (isNaga = true)tail][if (isNaga = false)legs] feel rubbery and your muscles are burning when you finally cum.");
            if (player.torso.cocks.count > 0) DisplayText("  [EachCock] fires several blasts of cum into the bog [if (isTaur = true)beneath you][if (isTaur = false)and all over your faerie partner].");
            ...
            dynStats("cor", randInt(1) + 3); // Extra two corruption for being enough of a pervert to want to fuck the phouka
            if (postCombat) {
                kGAMECLASS.clearStatuses(false);
                kGAMECLASS.awardPlayer(); // This will provide loot and return to camp, 1 hour used
            }
            else return { next: Scenes.camp.returnToCampUseOneHour }; // Return to camp, 1 hour used
        }
    }

    protected phoukaSexFaerieNonCombat() {
        phoukaSexFaerie(false);
    }

    protected phoukaSexFaeriePostCombat() {
        phoukaSexFaerie(true);
    }

    protected phoukaSexFaerieFemale(postCombat: boolean) {
        ...
        if (player.torso.cocks.sort(Cock.LargestCockArea)[0].length < 8)
            DisplayText("You push the last inch of your " + int(player.torso.cocks.sort(Cock.LargestCockArea)[0].length) + " prick into the moaning slut.");
        else if (player.torso.cocks.sort(Cock.LargestCockArea)[0].length < 15)
            DisplayText("You feed the last inch of your " + int(player.torso.cocks.sort(Cock.LargestCockArea)[0].length) + " cock into her.  Your cockhead bumps against her cervix and the slut beneath you begs for more.");
        else {
            DisplayText("You drive as much of your cock as you can into the little faerie slut, but there's just too much meat for her to take in.");
            if (player.stats.cor <= 50)
                DisplayText("  It still feels amazing and you give her even more pleasure by rubbing your cockhead against her cervix.");
            else
        ...
        if (player.stats.cor <= 33)
            DisplayText("You certainly don't want a phouka to learn where your campsite is.  You help her into the bushes and tell her she just needs to hide out for a few days.");
        else if (player.stats.cor <= 66)
            DisplayText("You laugh and remind her that's what she's wanted to do to you.");
        else
            DisplayText("Why shouldn't the other phouka get the chance to enjoy her?  Before she can fully recover you grab some tree limbs and vines.  You pin her to the ground and use them to tie her up tight.  With her arms and legs lashed together and her ass in the air she's an inviting target.");
        DisplayText("  Satisfied, you march back to camp.");
        dynStats("cor", randInt(1) + 3); // Extra two corruption for being enough of a pervert to want to fuck the phouka
        if (postCombat) {
            kGAMECLASS.clearStatuses(false);
            kGAMECLASS.awardPlayer();  // This will provide loot and return to camp, 1 hour used
    ...
    protected phoukaSexFaerieFemaleNonCombat() {
        phoukaSexFaerieFemale(false);
    }

    protected phoukaSexFaerieFemalePostCombat() {
        phoukaSexFaerieFemale(true);
    }

    protected phoukaSexPregnate(postCombat: boolean) { // Whether by horse, bunny or (male) faerie sex it all ends up here if the PC has a vagina
        if (player.isPregnant()) {
            ...
            if (player.stats.cor > 50)
                DisplayText("  The feeling of all that seed flowing into you and then out of you again finally brings you to your own orgasm.[if (hasCock = true)  [EachCock] fires long strands of cum into the bog water beneath you.]");
            else
                DisplayText("  As the thick grey cum leaks from your cunt you finally give in and get yourself off.  There's no way you could have made it back to camp feeling like that.");
            if (phoukaForm === PHOUKA_FORM_FAERIE) DisplayText("\n\nNow that you've got your fill you [if (isTaur = true)pull away from][if (isTaur = false)roll off] your faerie lover.  He shrinks back to his normal size and launches himself into the air.  You're too tired to stop him from buzzing into the bushes and you quickly lose track of him.");
        ...
        if (phoukaForm === PHOUKA_FORM_FAERIE)
            ...
            DisplayText("\n\nHe");
        if (postCombat) DisplayText(" rummages around in your gem pouch and then");
        DisplayText(" takes to the air and hovers nearby as if waiting for something.\n\n");
        if (player.isPregnant()) {
            const isFirstTime: boolean = (Flags.list[FlagEnum.BIRTHS_PHOUKA] === 0) && (Flags.list[FlagEnum.BIRTHS_FAERIE] === 0);
            Flags.list[FlagEnum.PREGNANCY_CORRUPTION] = 2; // Start off with a corrupt little phouka. It will take three drinks of pure honey to turn it into a faerie
            DisplayText("You're about to lift yourself up when you feel " + (isFirstTime ? "an unusual" : "a familiar") + " sensation inside your belly.  All that cum is hardening!  You can feel it set, almost like mortar left out in the sun!  You stand and some clear water drains out, your belly shrinking in size to that of a watermelon.  Whatever's left inside your belly is solid, like you're carrying a smooth ball of wood inside you.\n\nYou run a hand over your belly.  Thankfully it isn't any heavier than a normal pregnancy, nor does it hurt at all.\n\n<i>“Yes!”</i> shouts the " + phoukaName() + ". <i>“I'm gonna be a daddy!  Thanks fer letting me fill ya up girl, there’s plenty o’ space in there fer the little guy.  I gotta tell all the other phouka about ya");
            if (Flags.list[FlagEnum.PHOUKA_LORE] === 0) Flags.list[FlagEnum.PHOUKA_LORE] = 1; // Now you know what to call them
            if (!postCombat || (player.stats.cor > 50)) DisplayText(", yer a real prize slut");
            DisplayText(".”</i> The phouka gains more altitude and flies off deeper into the bog, whistling happily to himself.  ");
            if (isFirstTime)
                DisplayText("You have to wonder how long this pregnancy will last. If this were a human child people would judge you to be at least four months along.");
            else DisplayText("At least you know what to expect from this form of pregnancy now.");
        }
        else DisplayText("When you pull yourself up your belly begins to deflate and leak its gooey grey payload into the mud.  The " + phoukaName() + " says <i>“Ah well, there's always next time,”</i> and buzzes away to some other part of the bog.");
        phoukaSexPregnateEnd(postCombat);
    }

    protected phoukaSexPregnateEnd(postCombat: boolean) { // Everything after the sex. Handles awards, gem loss and text for player leaving the bog
        player.orgasm();
        dynStats("cor", randInt(1) + (postCombat && (phoukaForm !== PHOUKA_FORM_FAERIE) ? 1 : 3)); // Extra two corruption for being enough of a pervert to want to fuck the phouka
        if (phoukaForm === PHOUKA_FORM_FAERIE) { // In this case postCombat means you need an award because you must have won to get faerie sex
            ...
            if (player.stats.cor > 50) DisplayText("  But either way you plan to return and give all of them that lesson.");
            if (postCombat) {
                kGAMECLASS.clearStatuses(false);
                kGAMECLASS.awardPlayer();  // This will provide loot and return to camp, 1 hour used
            }
            ...
            if (postCombat && player.inventory.gems > 0) DisplayText("  While you're recovering the " + phoukaName() + " reaches into your gem pouch and takes a handful.");
            ...
            DisplayText("\n\nYou ");
            if (phoukaForm === PHOUKA_FORM_HORSE && !player.torso.hips.legs.isTaur()) DisplayText("wait for the throbbing pain in your pelvis to subside.  Then you ");
            if (postCombat) {
                DisplayText("pull your [if (isNaga = true)tail][if (isNaga = false)legs] free from the cool muck before you get chilled to the bone.");
                return { next: Scenes.camp.returnToCampUseOneHour };
            }
            else {
                DisplayText("collect your clothes and begin the long march out of the bog.  ");
                return { next: Scenes.camp.returnToCampUseOneHour }; // Return to camp, 1 hour used
            }
            if (player.stats.cor <= 50)
                DisplayText("As you trudge back to camp you have to wonder - why did you decide to visit the bog again?");
            else if (phoukaForm === PHOUKA_FORM_HORSE && player.torso.hips.legs.isTaur())
                DisplayText("  It was quite the ride and you find yourself looking forward to your next trip into the bog.");
            else DisplayText("Not what you expected, but at least you got off.");
        }
    }
