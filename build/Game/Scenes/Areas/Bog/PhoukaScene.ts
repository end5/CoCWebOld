 ...
 */
export class PhoukaScene implements ITimeAware {
...
export interface PhoukaFlags {
    PHOUKA_LORE: number;
    PHOUKA_ENCOUNTER_STATUS: number;
    TREACLE_MINE_YEAR_DONE: number;
    BIRTHS_PHOUKA: number;
    BIRTHS_FAERIE: number;
...
User.flags.set(FlagType.Phouka, phoukaFlags);
...
export function phoukaNameText(known: string, unknown: string): string { // Helper function, For any text that depends on whether or not the player knows what to call a phouka
    return (phoukaFlags.PHOUKA_LORE === 0 ? unknown : known);
}
...
export function phoukaEncounter() { // General entry point for everything except halloween special encounter
    ...
    if (phoukaFlags.PHOUKA_ENCOUNTER_STATUS === 0) { // Guarantee first Phouka encounter is with faerie fire
        phoukaFlags.TREACLE_MINE_YEAR_DONE = Time.fullYear - 1; // If you've never encountered phoukas before then we can safely set all this stuff here
        phoukaFlags.BIRTHS_PHOUKA = 0;
        phoukaFlags.BIRTHS_FAERIE = 0;
    }
    else if ((Time.month > 4) && (Time.month < 11))
...
function phoukaFaerieFire(player: Character) { // In old Irish legends the Phouka are responsible for Will-o’-the-wisps. In the deep woods it’s Kitsune, but in the bog it’s a Phouka
    DisplayText().clear();
    DisplayText("As you’re wading through the bog a flickering light off in the distance catches your eye.  ");
    if (phoukaFlags.PHOUKA_ENCOUNTER_STATUS !== 0) {
    ...
    if (phoukaFlags.PHOUKA_ENCOUNTER_STATUS !== 0 || player.inventory.keyItems.has("Traveler's Guide")) MainScreen.addButton(4, "Leave", phoukaFaerieFireLeave);
    MainScreen.addButton(0, "Follow", phoukaStuck);
}

function phoukaFaerieFireLeave() {
    if (phoukaFlags.PHOUKA_ENCOUNTER_STATUS === 0)
...
function phoukaStuck(player: Character) {
    DisplayText().clear();
    ...
    if (phoukaFlags.PHOUKA_ENCOUNTER_STATUS === 0) phoukaFlags.PHOUKA_ENCOUNTER_STATUS = 1;
...
function phoukaStuckOfferWhiskey(player: Character) {
    DisplayText().clear();
...
function phoukaMasturbating() { // Old Irish folklore again - Phoukas were supposed to be responsible for the berries rotting away at the end of the year
    DisplayText().clear();
...
function phoukaStartFightSilenced(player: Character) {
    startCombat(new Phouka(phoukaName()));
    player.statusAffects.add(StatusAffectType.WebSilence, 0, 0, 0, 0);
}

function phoukaTalk(player: Character) {
    DisplayText().clear();
    ...
    if (player.stats.cor + randInt(125) <= 110) { // Players with 0% corruption have a 12% chance that the Phouka is willing to talk, players with 100% corruption have a 92% chance
        ...
        DisplayText("\n\nThe " + phoukaName());
        switch (player.stats.level < 10 ? randInt(8) : randInt(12)) { // Low level characters don't get the chance to be offered whiskey
            case 0:
            case 1:
            case 2:
            case 3:
...
function phoukaDrinkWhilePregnant(player: Character, playerOfferedTheBooze: boolean) {
...
function phoukaDrinkAccept(player: Character) { // In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
    DisplayText().clear();
    ...
    phoukaFlags.PREGNANCY_CORRUPTION++; // Faerie or phouka babies become more corrupted, no effect if the player is not pregnant or on other types of babies
...
function phoukaDrinkRefuse(player: Character) { // In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
    DisplayText().clear();
...
function phoukaDrinkRefuseLeave() { // In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
    DisplayText().clear();
    DisplayText("You extract yourself from the mud and get out of the bog before the phouka changes its mind.");
    return { next: Scenes.camp.returnToCampUseOneHour };  // Return to camp, 1 hour used
}

function phoukaDrinkRefusePregnantLeave() { // In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
    DisplayText().clear();
...
function phoukaDrinkRefuseAfterOffer(player: Character) {
    DisplayText().clear();
...
function phoukaDrinkTalk(player: Character, playerNotDrinking: boolean) {
    if (phoukaFlags.PHOUKA_LORE === 1) {
        phoukaFlags.PHOUKA_LORE++;
    ...
    else if (phoukaFlags.PHOUKA_LORE === 2) {
        phoukaFlags.PHOUKA_LORE++;
    ...
    else if (phoukaFlags.PHOUKA_LORE === 3) {
        phoukaFlags.PHOUKA_LORE += 1;
    ...
    else if (phoukaFlags.PHOUKA_LORE === 4) {
        // More can come later, right now we don’t increment here //phoukaFlags.PHOUKA_LORE += 1;
...
function phoukaDrinkLeave(player: Character) {
    if (player.stats.lust < 33)
...
export function phoukaHalloween(player: Character) {
    phoukaFlags.TREACLE_MINE_YEAR_DONE = date.fullYear;
    DisplayText().clear();
...
function phoukaHalloweenTreacleEater() {
    DisplayText().clear();
...
function phoukaLeaveOnLustWin(newScreen: boolean = true) {
...
function phoukaPlayerWins(player: Character, hpVictory: boolean) {
    DisplayText().clear();
...
export function phoukaPregBirth(player: Character) {
    ...
    if (phoukaFlags.PREGNANCY_CORRUPTION > 6) { // You’ve been drinking like a fish, haven’t you?
...
function phoukaSexAddStandardMenuChoices(player: Character) { // This happens several times so it's broken out here in case additional options get added later
    if (player.torso.vaginas.count > 0) {
...
function phoukaSexBunny(player: Character, postCombat: boolean, lustLoss: boolean) { // Player must have a vagina for bunny-morph sex
    DisplayText().clear();
    if (postCombat) {
    ...
    if (player.torso.vaginas.find(Vagina.Virgin))
...
function phoukaSexBunnyChoice() {
    phoukaSexBunny(false, false);
}

function phoukaSexGoat(postCombat: boolean, lustLoss: boolean) { // Player does not have a vagina or has chosen anal
    DisplayText().clear();
    if (postCombat) {
        if (lustLoss)
...
function phoukaSexGoatChoice() {
    phoukaSexGoat(false, false);
}

function phoukaSexHorse(postCombat: boolean, lustLoss: boolean) { // Player has a vagina
    DisplayText().clear();
    if (postCombat) {
        DisplayText("As you collapse the " + phoukaName());
...
function phoukaSexHorseChoice() {
    phoukaSexHorse(false, false);
}

function phoukaSexFaerie(postCombat: boolean) {
    DisplayText().clear();
...
function phoukaSexFaerieNonCombat() {
    phoukaSexFaerie(false);
}

function phoukaSexFaeriePostCombat() {
    phoukaSexFaerie(true);
}

function phoukaSexFaerieFemale(postCombat: boolean) {
    ...
    if ((phoukaFlags.FREED_VALA === 1) || (phoukaFlags.TIMES_PC_DEFEATED_VALA > 0) || (phoukaFlags.TIMES_FUCKED_VALA_IN_DUNGEON > 0) || (phoukaFlags.INVESTIGATED_VALA_AFTER_ZETAZ_DEFEATED > 0)) // If any of these is true you've had some contact with Vala
...
function phoukaSexFaerieFemaleNonCombat() {
    phoukaSexFaerieFemale(false);
}

function phoukaSexFaerieFemalePostCombat() {
    phoukaSexFaerieFemale(true);
}

function phoukaSexPregnate(postCombat: boolean) { // Whether by horse, bunny or (male) faerie sex it all ends up here if the PC has a vagina
...
function phoukaSexPregnateEnd(postCombat: boolean) { // Everything after the sex. Handles awards, gem loss and text for player leaving the bog
    player.orgasm();
