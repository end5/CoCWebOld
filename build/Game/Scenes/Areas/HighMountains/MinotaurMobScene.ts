export class MinotaurMobScene implements TimeAwareInterface {
	...
	public meetMinotaurSons(): void {
		...
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00325] === 1) {
			...
			if (!player.minotaurAddicted()) {
				...
				DisplayText("You ready your " + player.weaponName + " and adopt a fighting pose, muscles already quivering from the intensity of the lust-inducing, mind-numbing musk of so many minotaurs packed so closely together.  They aren't the first aphrodisiac-exuding beast-men you've dealt with, and though you've ");
				...
				if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] > 3) DisplayText("some of ");
				...
				MainScreen.simpleChoices(["Fight", "Submit", "Negotiate", "", "Leave"], [fightOTaurs, submitToMinotaurMob, negotiate, null, runFromMinotaurs]);
			}
			//(Addicted) 
			else {
				DisplayText("You explore the hilly, almost cliff-like high mountains, searching for the next fight, item, or discovery that will put you closer to your goal.  The hot sun beats down on your back like a baleful flame, seeming to cook you in your " + player.inventory.equipment.armor.displayName + " with every step you take.  You raise an arm to mop away the sweat that streams down your brow, but as you lower it, you're confronted with a most unwelcome sight.\n\n");
		...
		//Repeat meetings
			...
			if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] < 20) {
				...
				if (!player.minotaurAddicted()) {
					...
					else if (player.pregnancyIncubation > 0 && player.pregnancyIncubation <= 216) DisplayText("Huh... Looks like Mommy couldn't keep her legs closed, and she got knocked up by some vermin.  You're such a disappointment to your beloved family.  Don't worry, though, mom, we'll still fuck you.");
					...
					if (player.pregnancyIncubation === 0 || player.pregnancyIncubation > 216) DisplayText("soon-to-be ");
				...
				//[Addicted Repeat] 
					...
					if (player.pregnancyIncubation > 0 && player.pregnancyIncubation <= 216 && player.pregnancyType === PregnancyType.MINOTAUR) DisplayText("Pregnant with another brother for us, and you're already back for another fix?  Why even leave in the first place?");
					else if (player.pregnancyIncubation > 0 && player.pregnancyIncubation <= 216) DisplayText("Huh... you couldn't keep your legs closed, eh, mom?  Looks like you're all knocked up with vermin, too!  You're a disappointment to our family... don't worry, though, we'll still give you a fix.");
				...
				//[Fight] [Submit] [Run]
				MainScreen.simpleChoices(["Fight", "Submit", "", "", "Run"], [fightOTaurs, submitToMinotaurMob, null, null, runFromMinotaurs]);
			}
			//Tribe sized
			else {
				...
				if (!player.minotaurAddicted()) {
					...
					if (player.pregnancyIncubation === 0 || player.pregnancyIncubation > 216) DisplayText("Come back to add to the tribe?");
					else if (player.pregnancyType === PregnancyType.MINOTAUR) DisplayText("Come back to show off the lil' bro yer growin'?");
				...
				//[Repeat Tribe-Size Addict] 
					...
					if (player.pregnancyIncubation === 0 || player.pregnancyIncubation > 216) DisplayText("Come for a baby? Or do you just want to be spunk-stuffed?");
					else if (player.pregnancyType === PregnancyType.MINOTAUR) DisplayText("Come back for some more spunk? I bet your next son comes out as a spooge-slurping minitaur.  We could use another cock-sleeve around here.");
				...
				//[Fight] [Submit]
				MainScreen.simpleChoices(["Fight", "Submit", "", "", ""], [fightOTaurs, submitToMinotaurMob, null, null, null]);
			}
		}
	}
	...
	private runFromMinotaurs(): void {
		...
		if ((player.canFly() && player.stats.spe > randInt(40)) || (!player.canFly() && player.stats.spe > randInt(60))) {
			DisplayText("A furry arm nearly catches your " + LegDescriptor.describeLeg(player) + ", but you slip free and quickly escape your lusty brood.");
			return { next: Scenes.camp.returnToCampUseOneHour };
		}
		//FAIL:
		else {
			DisplayText("A furry arm catches hold of your " + LegDescriptor.describeLeg(player) + " and yanks on it hard enough to slam you into the cliff face.  Disoriented and bleeding, you wobble free, dazed.  It's a fight!");
			//go to combat
			fightOTaurs();
		}
	}
	...
	private nonAddictMinotaurGangBang(): void {
		...
		if (player.torso.neck.head.horns > 0) DisplayText("horns ");
		...
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] <= 3) DisplayText(" the minotaur that gave you your first dose.  Maybe he's ready to give you more? He smirks and slaps you across the face with his soft shaft.  You giggle drunkenly and start to lick his balls - they HAVE to have more for you!  ");
		else {
	...
	private loseToMinoMobVeryPregnant(): void {
		...
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] < 8) {
			...
			if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] > 3) {
				if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] === 4) DisplayText("Sorry bro, it looks like Mom's done.  You'll have to snag a shot at her later - she's pretty fucking pregnant, and I don't want her broken.");
		...
		//(Long) 
			...
			if (player.pregnancyType === PregnancyType.MINOTAUR) DisplayText("we wouldn't want the next member of our tribe injured.");
	...
	private analSpearSemiPregMinotaurGangbang(): void {
		...
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] >= 20) DisplayText("to make the tribe even bigger");
		...
		if (player.pregnancyType === PregnancyType.MINOTAUR) DisplayText("our little brother");
		...
		if (player.torso.chest.hasFuckableNipples()) {
			...
			if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] < 7) DisplayText("shakes his head.  \"<i>You're only saving up trouble for later, little cunt.  Your hole after Mom's, remember.</i>\"  The minitaur wistfully withdraws his cock and crawls a few paces off, where he sits in a sulk.");
	...
	private victoryMinotaurGangTitFuck(): void {
		...
		if (player.minotaurAddicted()) DisplayText("Of course it helps that pleasing them will give you exactly what you want - more of that delicious, wondrous cock-cream.  ");
		...
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] >= 10) {
			...
			if (player.minotaurAddicted()) DisplayText("I'd hate to see you go mad with need around a lesser beast.");
		...
		//(SMALL) 
			...
			if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] === 3) DisplayText("You grab the minitaur's cock and drag the shy beast-man over your cum-slicked tits, wasting no time in burying the shaft in mommy's mammaries, where it belongs.");
				...
				if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] === 4) DisplayText("the remaining brother");
			...
			if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] > 3) DisplayText("His glaze spurts up along the opening created by his larger brother's shaft, and soon he too is cumming, obediently orgasming for you.  ");
			...
			if (!player.minotaurAddicted()) DisplayText("Maybe we can get Mom completely hooked.  Then we won't have to fight her?");
			...
			if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] < 5) DisplayText("The minotaurs let you get dressed while they sigh happily, clearly pleased with the outcome.");
			else {
				...
				if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] > 5) {
					...
					if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] >= 6) DisplayText("s");
				}
	...
	private victoryAllThePenetrationsMinotaurGangBang(): void {
		...
		if (monster.lust > 99) DisplayText(" nearly instantly, too aroused to turn down the idea of their mother doing ANYTHING with their hard shafts.");
		...
		if (player.torso.chest.hasFuckableNipples() && Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] > 4) {
			...
			if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] === 4) DisplayText("He licks his fingers clean, and a moment later, a cock is spearing one of your nipples, disappearing deep into the tingling passages concealed within your chest.\n\n");
		...
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] < 20) DisplayText("gang");
	...
	private forceMinitaurToGiveOral(choice: number = 0): void {
		...
		if (choice === 0) {
			...
			if (player.torso.hips.legs.isTaur()) {
				DisplayText("turn ");
				if (player.torso.tailType > TailType.NONE) DisplayText("and lift your tail ");
	...
	private victoryBJNippleFuckMinotaurGang(): void {
		...
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00326] === 3) DisplayText("the minitaur, much to the smaller monster's chagrin.");
