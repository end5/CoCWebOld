...
import { Basilisk, basiliskSpeed } from './Basilisk';
...
export interface BasiliskFlags {
    ENCOUNTERED: number;
}
...
User.flags.set(FlagType.Basilisk, basiliskFlags);

// Intros and Fight Texts.
export function basiliskGreeting(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Basilisk);
    DisplayText().clear();
    // First encounter:
    if (basiliskFlags.ENCOUNTERED === 0) {
        DisplayText("You are carefully trailblazing up a steep pass in the jagged mountain peaks when a furious screech from high above you makes you start.\n\n");
        ...
        // (spd loss)BasiliskBasilisk
        basiliskSpeed(player, 5);
        basiliskFlags.ENCOUNTERED++;
        return CombatManager.beginBattle(player, [], [new Basilisk()]);
    }
    // Standard encounter:
    else {
        DisplayText("You notice a large boulder ahead.  There is something curiously shaped about it. A small, wet grey shape on it catches your eye...\n\n");

        DisplayText("You look away in the nick of time, and ready yourself to fight as the basilisk slides from its hiding place and advances upon you, its deadly eyes and sharp claws glinting coldly in the sunlight.\n\n");
        basiliskFlags.ENCOUNTERED++;
        return CombatManager.beginBattle(player, [], [new Basilisk()]);
    }
}
...
// wins
export function defeatBasilisk(player: Character, monster: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Basilisk);
    DisplayText().clear();
    let evil;
    let eggs;
    if (player.canOvipositSpider()) eggs = { func: driderPCEggLaysBasilisk, args: [monster] };
    if (player.canOvipositBee() && player.gender > 0) eggs = { func: layBeeEggsInABasilisk, args: [monster] };
    if (player.torso.cocks.find(Cock.CockThatFits(monster.analCapacity())) && player.stats.cor >= 66) evil = { func: defeatBasiliskAndAnal, args: [monster] };
    // Player HP victory:
    if (monster.stats.HP < 1) DisplayText("Unable to stand anymore, the basilisk shakily sinks down on one knee, drops its head and looks at the ground, evidently demonstrating submission.");
    // Player Lust victory:
    else DisplayText("No longer able to control its raging erection, the basilisk closes its eyes and sinks to one knee.  It would probably be attempting to signal its submission to you if it weren't furiously masturbating its long, purple cock, which has emerged straining from the creature's genital slit.");
    ...
    if (player.stats.lust >= 33 && player.gender > 0) {
        ...
        // [Tongue][Ass]
        return { choices: [["Tongue", tongueBasiliskSmex], ["Ass", evil], ["Lay Eggs", eggs]], persistantChoices: [["Leave", Scenes.camp.returnToCampUseOneHour]] };
    }
...
// Player Victory sex:
// "<i>Tongue"<i>
function tongueBasiliskSmex(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Basilisk);
    DisplayText().clear();
    ...
    DisplayText("It is as you are taking in this latter article that the basilisk swallows nervously, and you get a glimpse inside its mouth at a long, sticky, pale purple tongue, another gift of its reptilian ancestors. It's the only part you've seen of it so far which looks... soft.  Pliable.  Your smile widens and you step forward, slowly loosening your " + player.inventory.equipment.armor.displayName + ".\n\n");
    ...
    if (player.torso.cocks.count > 0) {
        DisplayText("\"<i>Wrap it around me,</i>\" you demand gruffly, poking your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " lightly against the end of its tongue.  Slowly, the basilisk does as you ask.  It is clumsy at first, taking time to get a sense of your girth and hardness as it tastes and licks blindly, but it quickly grows into the task.  Starting from just above your head, it curls its tongue right around your prick and then inches its way upwards, spiraling its long, incredibly flexible organ around you.  The more you feel this thing's tongue, the more you know you made the right choice here; the shifting, wet sensation as it snakes its way upwards towards your base is incredible, and gets better the more of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " is swathed in mouth muscle.  It is unlike any tongue you've felt before... there is some kind of adhesive property in the basilisk's saliva which makes its soft flesh stick like a leech once it is on you.\n\n");

        // Single cock:
        if (player.torso.cocks.count === 1) {
            DisplayText("Once it has wrapped around as much of your cock as it can, you take a moment to control yourself in this dizzying, soft yet tight vice grip your serpent sub now has you in.  \"<i>Squeeze.  Work it, nice and slow,</i>\" you say huskily.  The basilisk's tongue slides up and down your length, its saliva slathering every inch, the sticky, sucking sensation encompassing your cock on all sides.  It squeezes you softly and then relaxes... squeezes you, then relaxes, as it continues to spiral around and around.  It is everything you can do not to go over the edge straight away; you hold back desperately, staring upwards.  To blow your load before properly enjoying what is turning out to be a truly unbelievable suck would be a terrible crime.  You look downwards and manage to pull yourself back a bit by laughing hugely at the sight.  With the basilisk's tongue spiralled around you, your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " looks like nothing so much as an obscene candy cane.  The basilisk seems to take your hysterical laughter for anger or contempt, because it hastily increases its pace, forcing its sticky muscle up and down as fast as it can.  You lean back and lose yourself in the blissful, sucking, enveloping sensation.\n\n");

            DisplayText("You last for as long as you can before, with a euphoric sigh, you reach your peak.  Your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " bulges and then rockets out stream after stream of jizz.  Directly in the line of fire, the blind basilisk takes its minute-long facial with as much dignity as it can muster- which is to say, none at all.\n\n");
        }
        // Multicock:
        else {
            DisplayText("The basilisk wraps itself around part of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + "... and then stops.  Confused and frustrated, you look down.  The tip of its tongue has touched your " + Desc.Cock.describeCock(player, player.torso.cocks.get(1)) + ", and cautiously, it begins to inch its way downwards and around, working towards your second tip. \"<i>Oh, aren't you a clever girl,</i>\" you sigh, revelling in the sensation of both of your cocks trapped in the dizzying, soft yet tight vice grip your serpent sub now has you in. The basilisk grunts at the humiliating commentary. \"<i>Squeeze. Work them, nice and slow,</i>\" you say huskily.  The basilisk's tongue slides up and down your length, its saliva slathering every inch, the sticky, sucking sensation encompassing your cocks on all sides.  It squeezes you softly and then relaxes... squeezes you, then relaxes, as it continues to spiral around and around.  It bends forward, investing more of its tongue around you, inching its way further up your " + Desc.Cock.describeCock(player, player.torso.cocks.get(1)) + ".  Its tip comes off you with a sucking sound, and it begins to tease the head of your second prick, circling it and lavishing it with attention as it continues to work your two lengths.  \"<i>How... where did you learn to do that?!</i>\" you gasp, and are immediately grateful the basilisk can't respond to your query.  It is everything you can do not to go over the edge straight away; you hold back desperately, staring upwards.  To blow your load before properly enjoying what is turning out to be a truly unbelievable suck would be a terrible crime.  You look downwards and manage to pull yourself back a bit by laughing hugely at the sight.  With the basilisk's tongue spiralled around you, your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " looks like nothing so much as an obscene candy cane.  The basilisk seems to take your hysterical laughter for anger or contempt, because it hastily increases its pace, forcing its sticky muscle up and down as fast as it can.  You lean back and lose yourself in the blissful, sucking, enveloping, teasing sensation.\n\n");
            DisplayText("You last for as long as you can before, with a euphoric sigh, you reach your peak.  Your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " and " + Desc.Cock.describeCock(player, player.torso.cocks.get(1)) + " both bulge and then rocket out streams of jizz in concert.  Directly in the line of fire, the blind basilisk takes its minute-long double facial with as much dignity as it can muster- which is to say, none at all.\n\n");
        }
        // Both go to:
...
function defeatBasiliskAndAnal(player: Character, monster: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Basilisk);
    DisplayText().clear();
    ...
    if (player.torso.cocks.count > 0) {
        ...
        // [(13 inches or less:)
        if (cockThatFits.length <= 13) {
            DisplayText("Soon your " + Desc.Hip.describeHips(player) + " are clapping a staccato rhythm against its warm, muscly butt,");
            if (player.torso.balls.quantity > 0) DisplayText(" your " + Desc.Balls.describeBalls(true, true, player) + " slapping against it,");
            DisplayText(" the creature taking every inch of your length before you pull out and thrust all the way in again, forcing ragged gasps from the reptile's still throat.\n\n");
        }
        // (More than 13 inches:)
        else {
            DisplayText("You force as much of your huge member as you can into the creature's anus, clutching onto the creature's warm, muscly butt as you pull out and thrust yourself in again, brutally driving a bit more of yourself in each time, forcing ragged gasps from the reptile's still throat.  ");
            // [(if balls)
            if (player.torso.balls.quantity > 0) DisplayText("Your " + Desc.Balls.describeBalls(true, true, player) + " swing heavily underneath your shaft, swelling as your rhythm picks up.  ");
            DisplayText("At the end of your reach you push against something spongy and yielding.  The basilisk emits a dry moan and underneath it you see that you have forced its long, thin, shining cock from its genital slit.  With a cruel smile you thrust into it again and again, holding onto your depth at the height of your thrust just a little each time to put pressure on the helpless basilisk's prostate, forcing the creature into an involuntary, straining erection.  You slip one hand under its frozen legs and begin to pump the creature in time with your rhythm, leaning over it as you do, whispering every dark thought that bubbles up through your corrupt mind into its ear; telling it what a pathetic slutty little fuck toy it is, how turned on it is by your assault on it, how grateful it should be that you have deigned to give it your cock, knowing your words are sinking into its hypnotically stilled and pliable mind as easily as your " + Desc.Cock.describeCock(player, cockThatFits) + " is into its cum-oiled hole.\n\n");
        }
        ...
        // <(13 inches or less:)
        if (cockThatFits.length <= 13) DisplayText("gripping its hips and pushing every inch of yourself in and reaching a glorious, skin tingling peak.");
        // (More than 13 inches:)
        else DisplayText("gripping its hips and pushing as much of your huge, bulging cock into it as you can, its stomach bulging as you reach a glorious, skin tingling peak.");
...
// Player Defeated:
export function loseToBasilisk(player: Character, monster: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Basilisk);
    DisplayText().clear();
    // Speed 0 loss:
    ...
    if (player.statusAffects.has(StatusAffectType.BasiliskSlow)) {
        player.stats.spe += player.statusAffects.get(StatusAffectType.BasiliskSlow).value1;
        player.statusAffects.remove(StatusAffectType.BasiliskSlow);
    }
    player.stats.spe += -3;
    player.stats.lust += 399;
    // Bad end
    if (player.stats.spe < 5) {
        return basiliskBadEnd(player);
    }
    // choose between loss rapes
    if (player.torso.vaginas.count > 0 && (player.statusAffects.has(StatusAffectType.Heat) || player.perks.has(PerkType.Oviposition) || player.perks.has(PerkType.BasiliskWomb) || player.pregnancy.womb.pregnancy.type === PregnancyType.OVIELIXIR_EGGS))
        return basiliskHasVagEggStuff(player, monster);
    else return defaultBasiliskRape(player);
}
// Loss, vag rape conditions not met:
function defaultBasiliskRape(player: Character): NextScreenChoices {
    DisplayText("Working briskly, the basilisk tears off your " + player.inventory.equipment.armor.displayName + " until you are entirely naked.  It then rummages through your pockets; it carelessly discards everything it finds without apparent interest.  It grabs a handful of gems from your purse and then prowls back to you.\n\n");

    // Male/Herm:
    if (player.torso.cocks.count > 0) {
        DisplayText("With surprising gentleness and deftness, the basilisk rubs your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " with one palm");
        if (player.torso.vaginas.count > 0) DisplayText(" and sticks some of the smaller fingers of its other hand in your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)));
        DisplayText(", thankfully angling its sickle claw away.  You can't do anything against it, and some of its mental compulsion remains; a backwash of erotic images from your past fill your head, and you can't even grit your teeth as the gentle, insistent pressure brushing your prick makes you rock hard.  It stops when you are erect and then, with the very faintest of smiles playing over its cruel mouth, leaves. You're naked, your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " is begging for release; you're utterly helpless... you can only hope that the spell will wear off, and before anything else in the mountain finds you.\n\n");
    }
    // Female:
    else if (player.torso.vaginas.count > 0) {
        DisplayText("With surprising gentleness and deftness, the basilisk slips the smaller fingers of one hand into your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", and carefully flicks at your " + Desc.Vagina.describeClit(player) + " with the other, thankfully holding its sickle claws away from you.  You can't do anything against it, and some of its mental compulsion remains; a backwash of erotic images from your past fill your head, and you can't even grit your teeth as the gentle, insistent caresses make you wet.  It stops when you are beading moisture involuntarily onto its hand and then, with the very faintest of smiles playing over its cruel mouth, leaves. You're naked, your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " begs to be filled; you're utterly helpless. You can only hope that its spell will wear off, and before anything else in the mountain finds you...\n\n");
    }
    // Genderless:
    else DisplayText("Staring into your eyes, the basilisk moves its smaller fingers onto your groin... and then stops.  It looks downwards, and then back into your face.  You aren't very good at reading lizard facial expressions, but the creature looks distinctly baffled.  Finally with a slight shake of its head, it slowly turns and leaves.  It has left you paralyzed and naked to the open air, your skin prickling from the exposure.  You can only hope that its spell will wear off, and before anything else in the mountain finds you...\n\n");

    // More to go here?
    const scene: number = randInt(5);
    if (scene === 0) basiliskAdvantageNobody();
    else if (scene === 1) basiliskAdvantageHarpy(player);
    else if (scene === 2) basiliskAdvantageImp(player);
    else if (scene === 3) basiliskAdvantageGoblin(player);
    else basiliskAdvantageMinotaur(player);
...
function basiliskHasVagEggStuff(player: Character, monster: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Basilisk);
    player.slimeFeed();
    DisplayText("The basilisk is breathing heavily as it tears your " + player.inventory.equipment.armor.displayName + " from your body, its warm exhalations rolling over your naked flesh.  It seems to be having difficulty controlling itself; from your frozen gaze you can see it constantly shifting its dreadful slit eyes back to your frame as it searches through your pockets with claws that tremble.  Eventually it throws down your attire and stares back into your eyes.  There is something else in there now; a pulsing lust, hints of red at the edges of that great, grey sea, a rapacious tide gathering.  You wish you could look away but there is more chance of you moving mountains.");
    // (Heat:
    if (player.statusAffects.has(StatusAffectType.Heat)) DisplayText("  You are more aware than ever of an invisible scent simmering off you, of your wet vagina clenching and wetting itself in anticipation, your body begging this male creature to fulfil its genetic objective upon you.  Your eyes have betrayed you, your body is betraying you, and whatever else you are is a tiny, ignored voice screaming in between.");
    DisplayText("  The basilisk suddenly breaks away and kneels down in front of you.  Out of sight of your petrified eyes you cannot see what it is doing; however a moment later, you can feel, as a warm, sticky sensation slavers over your abdomen.");
    // (egg preg:
    if (player.pregnancy.womb.isPregnant && player.pregnancy.womb.pregnancy.type === PregnancyType.OVIELIXIR_EGGS) DisplayText("  The basilisk licks your bulging belly hungrily, pushing against and testing for the eggs you are carrying.  Your sensitive cargo shifts around under its hungry attention; you'd squirm, but that is, of course, impossible.");
    // (heat or perk:
    if (player.statusAffects.has(StatusAffectType.Heat) || player.perks.has(PerkType.Oviposition) || player.perks.has(PerkType.BasiliskWomb)) DisplayText("  The basilisk licks your belly hungrily, its sticky tongue crawling like a warm tentacle across your sensitive underside.  You'd squirm, but that is impossible.  The creature is making you feel everything it is forcing upon you.");
    DisplayText("\n\n");
    ...
    DisplayText("The creature suddenly raps something out in its strange, dry tongue, and you feel something - something deep and red - flinch in your mind.  Suddenly, you are wet, wetter than you've ever been, your pussy slavering so badly you can hear the pitter-patter of your juices hitting the ground beneath you.  The basilisk says something else, more softly this time as it slides in close, its long claws reaching around to clutch your " + Desc.Butt.describeButt(player) + ".  With a kind of horror you feel your limbs move involuntarily, your arms reaching around the thing's thin, muscled back, your bottom half slackening until you are supported entirely by the wiry strength of the lizard.  You expose your crotch in complete submission as the basilisk walks forward, and you feel the head of its cock teasingly touch your dripping, treasonous cunt.  As soon as your limbs are where it wants them to be, they lock in position again; you heave at them desperately, but once again your ability to do anything with your own body is simply not there.  You are less of a statue and more of a fuck toy, an extension of the basilisk's lust-maddened will.  Your jailer keeps walking until you feel the rough surface of a boulder against your back, and using this support the basilisk pushes itself straight into your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ".");
    Mod.Vagina.displayStretchVagina(player, monster.torso.cocks.get(0).area, true, true, false);
    DisplayText("\n\n");

    DisplayText("Once it has you pinioned, the uncontrollable rut your body has instilled in the basilisk really takes hold and it begins thrusting against you with abandon, its long, thin reptilian cock sliding in and out of your eager, slavering cunt, its hot breath pushing against your face and shoulders.  ");
    // <(Tight:
    if (player.torso.vaginas.get(0).looseness < 4) DisplayText("Though it is not girthy, it is a perfect fit for your tight hole, and its long length coupled with your drawn up position has you panting as it touches your deepest, most sensitive depths.");
    // (Loose:
    else DisplayText("At first the sensation is not great, its thin penis quickly lost in your vast, accommodating twat, but then, in between breaths, the basilisk barks out more harsh words. Once again you feel that helpless, red flex in your mind, and suddenly your vagina tightens around it, beginning to eagerly milk the reptile.  You desperately wish it didn't and at the same time are hopelessly glad it did; the sensation of your walls pushing and pulling the long, smooth prick in loving synchronisation is unbearably pleasurable.");
    DisplayText("  The rock wall the basilisk is fucking you against grates your back and ass and, lost in its daze the creature's claws dig into your flesh, but these discomforts only serve to heighten the pleasure the creature is forcing you to feel.  Its hard stomach beats a steady rhythm against your own as you slather its thighs and crotch with girl cum with the first of many involuntary orgasms, your " + Desc.Vagina.describeClit(player) + " twitching eagerly for more.\n\n");
    ...
    player.pregnancy.womb.knockUp(new Pregnancy(PregnancyType.BASILISK, IncubationTime.BASILISK));
    // Egg change - 100% chance
    if (player.pregnancy.womb.pregnancy.type === PregnancyType.OVIELIXIR_EGGS) {
        ...
        player.pregnancy.womb.knockUp(new Pregnancy(PregnancyType.BASILISK, IncubationTime.BASILISK - 150), 0, true); // Convert Ovi Elixir eggs to Basilisk eggs
    }
...
export function basiliskBirth(player: Character) {
    DisplaySprite(SpriteName.Basilisk);
    DisplayText("\n");
    if (player.torso.vaginas.count === 0) {
        DisplayText("You feel a terrible pressure in your groin... then an incredible pain accompanied by the rending of flesh.  <b>You look down and behold a new vagina</b>.\n\n");
        player.torso.vaginas.add(new Vagina());
    }
    if (player.perks.has(PerkType.BasiliskWomb)) {
        ...
        if (player.torso.cocks.count > 0) DisplayText(" and [eachCock] starting to leak pre-cum");
        DisplayText(" as you find yourself being moved to climax by the birthing.  You see no point in resisting and reach down to begin fiddling with yourself, moaning in pain-spiked pleasure as the stimulus overwhelms you. With an orgasmic cry, you release your eggs into the world amidst a gush of femcum");
        if (player.torso.cocks.count > 0) DisplayText(" and a downpour of hermcum");
        DisplayText(".");

        DisplayText("\n\nWhen you find yourself able to stand, you examine what it is you have birthed; ");
        // (eggNumber)
        DisplayText(numToCardinalText(Math.floor(player.totalFertility() / 10)));
        DisplayText(" large, jade-colored eggs, the unmistakable shape of reptile eggs. You pick up one and hold it gently against your ear; inside, you can hear a little heart, beating strong and quick.  You put it down carefully with its fellows and stare at your clutch, a queasy tangle of emotions tugging at you.");
        ...
        if (Flags.list[FlagEnum.BENOIT_EGGS] > 0) DisplayText(" as well");
        DisplayText(".  You place the egg back down and gather them all up, moving them closer to the campfire to stay warm while you recover from your exertions.");
        ...
        if (player.stats.cor < 33) DisplayText("You find yourself shaken by the experience, and deeply disquieted by the thought of the clutch of monsters you have unleashed on this world.  You pick yourself up, rub yourself down and leave, promising yourself fervently you'll be more careful around basilisks in the future.\n\n");
        else if (player.stats.cor < 66) DisplayText("You pick yourself up, rub yourself down and leave.  You feel conflicted about what just happened; on the one hand you feel disquieted about the dozen monsters you just unleashed on this world, on the other you cannot help feel oddly proud of them and yourself.\n\n");
        else DisplayText("With a soft smile, you get up and leave, enjoying the sensation of green slime trickling down your legs.  You cannot wait to get pregnant again, for your stomach to bulge with eggs, to release more delightful creatures into this world which can grow up to fuck you and everyone else in turn, so everyone can enjoy life as much as you do.");
    }
    DisplayText("\n");
}
...
function basiliskBadEnd(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Basilisk);
    DisplayText().clear();
    ...
    if (player.gender === Gender.HERM) DisplayText("  You moan through your still throat as you feel blood rushing to your groin, your " + Desc.Cock.describeMultiCockShort(player) + " stiffening and your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " beginning to drip.");
    if (player.gender === Gender.MALE) DisplayText("  You moan through your still throat as you feel blood rushing to your groin, your " + Desc.Cock.describeMultiCockShort(player) + " stiffening.");
    if (player.gender === Gender.FEMALE) DisplayText("  You moan through your still throat as you feel blood rushing to your groin, your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " beginning to drip.");
    DisplayText("  Every erotic thing that has ever happened to you crowds your head, a dozen sexual sensations are forced upon your senses, and you feel yourself helplessly pushed, fucked, inundated towards an incredible, glorious orgasm...\n\n");
    ...
    DisplayText("You have no voice, and you must scream.");
    return { next: Menus.GameOver };
}
// Defeated, Taken Advantage of: nobody
function basiliskAdvantageNobody() {
    DisplaySprite(SpriteName.Basilisk);
...
function basiliskAdvantageHarpy(player: Character) {
    DisplaySprite(SpriteName.Basilisk);
    DisplayText("Time stretches by at an agonizingly slow pace as you stand there, a bizarre, motionless flesh statue.  You have no way of measuring how much time is passing; the sun is not in your direct line of vision.   You try to move any and every part of yourself in turn, but it is hopeless.  Your body is a cage, and you begin to hate the basilisk less because it paralyzed you and more because it left your mind entirely aware of it.  Every so often another unbidden backwash of erotic memories overwhelms your senses, keeping you helplessly aroused and reminded of who did this to you.  Coupled with the unscratchable itches and the aching in your limbs the experience is one of sensational hell.\n\n");
    ...
    // Male/Herm:
    if (player.torso.cocks.count > 0) {
        DisplayText("The harpy's eyes zero in on your erect cock greedily.  Stepping back from you she raises her head and lets out a screech which echoes around the mountains; as answering calls roll back to her she closes in, threads her arms around your neck and scalp and kisses you roughly. She pushes her golden lips against yours and squeezes her rough bird tongue into your mouth.  You feel your lips tingle and you raggedly moan against the savage frenching, her hot breath pushing down your throat as she circles your still tongue with her own, before exploring further down towards your tonsils.  By the time she has finished with you your whole body feels like it is glowing red from the effect of her lipstick, " + Desc.Cock.describeMultiCockSimpleOne(player) + " straining.\n\n");

        DisplayText("You wonder vaguely how she and her no-doubt-soon-to-arrive sisters are going to take advantage of you as you are; you find out a moment later when with no preamble whatsoever the harpy shoves you roughly in the chest.  ");
        if (player.torso.hips.legs.isBiped()) DisplayText("You teeter horribly on your frozen heels for a moment and then fall onto your back like a collapsing statue.  ");
        else DisplayText("You feel yourself slowly and horribly lose your balance before flopping onto your side like a collapsing statue.  ");
        DisplayText("Unable to brace yourself, you bang your head painfully; as you are lying there dazed, you feel something build at the back of your mind.  Involuntary sensations prickle your skin and groin as, once again, the intermittent mental backwash that the basilisk's hypnosis has forced upon your mind hits you.  Memory after memory of sexually-charged encounters, daydreams or fantasies crowd your consciousness.  It is made worse, much worse by the pheromones the harpy has pushed into your mouth; your body rides the chemical glow at the same time as imaginings of soft skin, tight muscle and musk overload your senses.  You groan raggedly as " + Desc.Cock.describeMultiCockSimpleOne(player) + " bulges and leaks pre-cum, almost screaming for attention.  When you finally, woozily come to your senses, you find that all vision has been blotted out by a big, wobbly harpy bum, her moistening lips rubbing impatiently against your mouth.\n\n");

        DisplayText("The bang on the head you took, the harpy lipsticks trilling in your bloodstream and the involuntary tide of erotic memories which ebbs and flows over you mean the next couple of hours goes by for you in a haze of forceful sex.  You can't control your cock");
        if (player.torso.cocks.count > 1) DisplayText("s");
        DisplayText(", your sex drive or what you are thinking about; you are a prisoner of lust and you quickly subside under the sexual concussion, hoping eventually the harpies and your own body will stop fucking you.  It does sink in around your second involuntary orgasm, your aching " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " spurting ribbon after ribbon of jizz into a harpy's clenching warmth, that the two arriving harpies are not happy with the first one; with your mouth and hands frozen you can't pleasure them with anything but your manhood, which leaves two of them fighting each other for time with your groin whilst a third grinds frustratedly at your face, mainly using your nose to rub at her inner walls and clit, forcing her juices down your nostrils and making you cough raggedly, struggling to breathe.\n\n");

        // Single cock:
        if (player.torso.cocks.count === 1) DisplayText("Their inability to properly satisfy themselves on your frame raises their tempers and frustration to the point where the three of them are physically fighting each other, treading all over you as they scream, bite and tear, their feathers flying all over the place.  Eventually the biggest of the trio drives the other two off, flapping and screeching into the mountains, before throwing herself onto her scratched and battered prize, her eyes lit up and wild.  She fucks you with a gusto born of bloodlust, her petit breasts bouncing up and down as she slams her powerful thighs into your " + Desc.Hip.describeHips(player) + ", picking up the pace.  She quickly forces you to another achingly pleasurable peak, your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " spurting more of your seed deep into her, and then just keeps on working you. Your cock seems incapable of going soft.  You feel the involuntary erotic backwash build in your skull again...\n\n");
        // Multicock:
        else {
            DisplayText("The three of them do eventually work out a compromise however, once they discover you have more than one manpole to your name.  One pulls your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " forwards painfully until it is almost pointing towards your face before squatting over it, whilst another clutches at your " + Desc.Cock.describeCock(player, player.torso.cocks.get(1)) + " as she spreads her legs and works her way inwards until she is spearing herself on you. You can't see them - you can't see anything except pink wobbly flesh and lavender feathers - but you can hear them shift impatiently around each other as they find a position which is comfortable to them both, their inner walls rubbing you from every direction as they move.  The third continues to thrust her needy sex into your face as best she can");
            if (player.torso.cocks.count >= 3) DisplayText(", crowded out by the bodies of the others despite her transparent desire for the unoccupied man meat flopping against them,");
    ...
    // Genderless:
        ...
        DisplayText("The harpy comes to a halt behind you and begins to eagerly run her cold but soft hands over your bottom half, stroking your thighs and squeezing your " + Desc.Butt.describeButt(player) + ", as if appraising a piece of meat.  Whilst caressing your neck she runs her hands between your legs and grabs around your groin, searching for something that isn't there.  She lets loose a squawk of pure frustration and wheels around you to glare in your eyes angrily.  The sex-crazed harridan is clearly deeply irritated about finding such a helpless victim only to further discover they lack a cock which she can abuse.  She shifts her eyes to your mouth, but quickly arrives at the same conclusion you've already come to: unable to move your mouth, you can't even be forced to give oral pleasure.  You feel a bizarre sense of triumph over the creature; you stare into space smugly as the harpy paces in front of you, glaring, thwarted but unwilling to give up her prize.  Perhaps eventually she will leave you alone...?\n\n");
...
function basiliskAdvantageGoblin(player: Character) {
    DisplaySprite(SpriteName.Basilisk);
    DisplayText("Time stretches by at an agonizingly slow pace as you stand there, a bizarre, motionless flesh statue.  You have no way of measuring how much time is passing; the sun is not in your direct line of vision.   You try to move any and every part of yourself in turn, but it is hopeless.  Your body is a cage, and you begin to hate the basilisk less because it paralyzed you and more because it left your mind entirely aware of it.  Every so often another unbidden backwash of erotic memories overwhelms your senses, keeping you helplessly aroused and reminded of who did this to you.\n\n");
    ...
    if (player.torso.cocks.count > 0) {
        ...
        DisplayText("Eventually, after what seems like hours of forcible ejaculation, you notice that your dick is no longer trapped in sucking wet.  You come out of your daze you find that the goblin has left, leaving your petrified form in a mingled pool of her juices and your own jizz. After another ten or twenty minutes of being forced to lie there and marinate in your own shameful memories, you find with great relief you can begin to move your fingers again.  Eventually with some effort you manage to work power into each corner of your body and finally shake free of the basilisk's curse; quickly, you pick yourself up and redress before anything else finds you and woozily begin to make your way back down the mountain.  The smell of horny goblin on you is a lingering reminder of what just happened to you.");
        player.orgasm();
    }
    // Unsexed:
    else if (player.torso.vaginas.count <= 0) {
        ...
        DisplayText("\"<i>A " + Desc.Body.describeRace(player) + "!</i>\" she yelps.  Hesitantly at first, then with increasing confidence as how vulnerable you are sinks in, she does a round circuit of you, taking in every angle of your frozen, helpless flesh.\n\n");

        DisplayText("\"<i>Get caught with our pants down by a basilisk, did we?</i>\" the goblin purrs.  \"<i>Well, not to worry. I'll take good c- hey, what gives?!</i>\" From behind you, you feel her hands thread their way around your " + Desc.Leg.describeLegs(player) + " before feeling and slapping all around your featureless groin.  \"<i>This is fucking bullshit!</i>\" she howls.  \"<i>I get a toy that can't stop me all to my lonesome, and it's some colossal jerkoff who thinks it's funny to have no sex!</i>\"  She stomps around to your front and glares at you, simmering with rage.  \"<i>I bet you think you're a real smartass, you bastard... you bitch... you... whatever!  Ooh, I'll fix you!</i>\"  With a look of complete disgust she storms off.  Having faced down this bizarre rant, you allow yourself to feel a tiny bit of relief.  You were worried for a moment there she'd go for your ass... a short distance away, you hear the goblin calling to someone.\n\n");
        ...
        DisplayText("The huge bull-man is not one to look a gift fuck in the mouth.  Without bothering to take you in, bar a long wet sniff of your " + Desc.Head.describeHair(player) + ", he sticks his cock between your ass cheeks.  He grunts as he forces his head past your sphincter, squirting pre-cum into your passage as he does so.  Your body is incapable of clenching instinctively against the invasion, but there is no escaping how huge the dong feels as he impatiently forces your ass open.");
        Mod.Butt.displayStretchButt(player, 60, true, true, false);
        if (player.analCapacity() < 60) DisplayText("  The giant cock stretches you out painfully, and everything else blots out as your body attempts to accommodate the beast.  As he begins to thrust more of his length up you, he grunts and beads more of his drugged pre-cum, lubricating your anus.  This thankfully makes his cock easier to take, but also increases the pace of his thrusting as your hole becomes more receptive to it.");
        // V loose/Buttslut:
        else DisplayText("  Your well-worn ass is a perfect fit for the giant cock and accepts it eagerly, every bit as welcoming as a moist vagina.  As the minotaur rubs against your tender inner walls he grunts and beads more of his drugged pre-cum, turning your hole into a helplessly wet, clenching ass cunt.");
        ...
        DisplayText("Eventually the minotaur's balls swell against your " + Desc.Butt.describeButt(player) + " and with a long, satisfied moo, he reaches his peak, holding you down so his cum jets forth deep inside you.  The sensual high it has already instilled in you amplifies by tenfold as the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, your anus helplessly milking the creature for all it can get.\n\n");

        DisplayText("The minotaur holds onto you until he has finished spurting his last into you, before abruptly setting you back down on your frozen feet, pulling his slimy, receding member out of your abused anus and with a satisfied snort, takes his leave.  You feel his cum drooling out of you and down your " + Desc.Hip.describeHips(player) + ", but in your hazy, druggy state the feeling is almost sensual.");
        // (Addict:
        if (User.flags.get<PlayerFlags>(FlagType.Player).MINOTAUR_CUM_ADDICTION_STATE > 0 || player.perks.has(PerkType.MinotaurCumAddict)) DisplayText("  You're incredibly frustrated that you can't clench yourself and hold the magical substance deep inside you, so you can savor the wonderful, soft elation it blossoms inside of you for as long as you can.");
        DisplayText("\n\n");

        DisplayText("Eventually, after another thirty or so minutes of being forced to stand there and savor the cum trickling down your legs, you find with great relief you can begin to move your fingers again. With some effort you manage to work power into each corner of your body and finally shake free of the basilisk's curse; quickly, you shake the aching out of your " + Desc.Leg.describeLegs(player) + " and re-dress before anything else finds you and woozily begin to make your way back down the mountain, trying to ignore the feeling of ooze dripping out of you.");
        player.orgasm();
        player.slimeFeed();
    }
    // Female:
        ...
        DisplayText("\"<i>Get caught with our pants down by a basilisk, did we slut?</i>\" she purrs.  \"<i>Well, not to worry.  I'll take good care of you.</i>\"  You somehow doubt her good intentions, and your suspicions are confirmed when, after completing her sauntering circumference of you, she shoves your bottom half as hard as she can. You fall slowly like a wooden board; being unable to brace yourself, the sensation of dropping is horrible, and, once your skull connects with the ground, painful.  Before you can clear your swimming vision you feel the goblin's soft, dense weight on your chest, and then her eager lips upon yours, sucking and tonguing every inch of your frozen mouth that she can reach.  A tingling sensation spreads from your mouth downwards as her pheromone-laced lipstick takes effect.  By the time she has finished with your mouth and worked her way downwards, your paralyzed body feels warm and receptive, and your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " is wet with need.  You feel small fingers caressing your mons before slipping their way inside, circling your " + Desc.Vagina.describeClit(player) + " before testing your depths.  You wish you could cringe away or even thrust yourself forward to try and make the goblin better address your growing need, but that is impossible; you stare in front of you, petrified, as the fingers continue their lazy tour of your sex until your juices are running like a river in spring.\n\n");

        DisplayText("\"<i>You are well up for this, aren't you hun?</i>\" coos the goblin as she begins to flick your sopping clit, sounding delighted with the involuntary heat she has stoked within you.  \"<i>It's such a shame that I don't have the big cock you obviously want and need.  Don't worry though; I've got the next best thing!</i>\"  You hear her rummage around in her satchel, which is followed by the sounds of her wetly sucking on something with obvious enthusiasm.  You shrink inwardly as you make a strong guess as to what she's holding and what she intends to do with it.  Sure enough, a moment later you feel a dildo pushing unceremoniously into your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ".");
        Mod.Vagina.displayStretchVagina(player, 30, true, true, false);
        // [(tight)
        if (player.vaginalCapacity() < 30) DisplayText("  You didn't get the wet end, and you groan as you feel the thing begin to puff up as it reacts with your eager juices until it is pushing almost painfully against your inner walls.");
        else DisplayText("  You didn't get the wet end, and you groan as you feel the thing begin to puff up as it reacts with your eager juices until it fills your well-used passage almost perfectly.");
        DisplayText("  You feel the goblin crawl on top of you, pulling the dildo upwards so it rubs against your " + Desc.Vagina.describeClit(player) + " and then with a squealing giggle impales herself on the other end.  Her lustful movements translate through the artificial cock buried in your dripping sex, sending tremors and waves of pleasure to your very core.  A gush of female fluid splatters over your own sex as the goblin works herself right down the shaft connecting you until she is squelching against you wetly.  The goblin twists, grinding and scissoring her thighs, the hard bud of her clit rubbing back and forth over your own.  By now your passage feels as stuffed as it ever has been, crammed totally full of the squishy expanded double-dong.  Every motion the tiny slut makes is amplified directly into the fuck-stick plugging your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ".  Judging by how wonderful it feels rubbing and twisting against your sensitive walls, the aphrodisiac it is leaking into you is definitely having an effect.  You whine at the goblin twists herself and her dildo around, rubbing and riding you so effectively, the involuntary tide of erotic memories building again...\n\n");
...
function basiliskAdvantageMinotaur(player: Character) {
    DisplaySprite(SpriteName.Basilisk);
    DisplayText("Time stretches by at an agonizingly slow pace as you stand there, a bizarre, motionless flesh statue.  You have no way of measuring how much time is passing; the sun is not in your direct line of vision.   You try to move any and every part of yourself in turn, but it is hopeless.  Your body is a cage, and you begin to hate the basilisk less because it paralyzed you and more because it left your mind entirely aware of it.  Every so often another unbidden backwash of erotic memories overwhelms your senses, keeping you helplessly aroused and reminded of who did this to you.\n\n");
    ...
    DisplayText("Eventually the minotaur's balls swell against your " + Desc.Butt.describeButt(player) + " and with a long, satisfied moo, it reaches its peak, holding you down so its cum jets forth deep inside you.  ");
    if (player.gender === Gender.MALE) DisplayText("The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark and you moan as you spontaneously ejaculate, ropes of your own cum spattering on the ground and your own nerveless " + Desc.Leg.describeFeet(player) + ".");
    else if (player.gender === Gender.FEMALE) DisplayText("The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, your juices spattering your thighs and calves.");
    else if (player.gender === Gender.HERM) DisplayText("The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, ropes of your own jizz spattering on the ground and girl cum dripping down your thighs.");
    else DisplayText("The sensual high it has already instilled in you amplifies by tenfold as the warm, oozing fluid finds its mark and you moan as you spontaneously orgasm, your anus helplessly milking the creature for all it can get.");
    DisplayText("\n\n");
...
function driderPCEggLaysBasilisk(player: Character, monster: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Basilisk);
    DisplayText().clear();
    DisplayText("Leaning back on your carapace-clad abdomen, you try and look over your defeated opponent.  Even slumped over in ");
    if (monster.stats.HP < 1) DisplayText("exhaustion");
    else DisplayText("arousal");
    DisplayText(", the basilisk is still striving to meet your eyes with its own.  Clenching your teeth, you avert your eyes and stomp forward blindly, colliding roughly with the spined lizard, knocking him flat on his back on the rocky ground.");
    ...
    if (User.settings.silly()) DisplayText("Pinning him between a rock and a hardening place, ");
    DisplayText("Shoving him up against the cool stone, ");
    ...
    DisplayText(" up against his half-open mouth, smearing your free-running juices across his scaled face.  You wait a few seconds, just enjoying the feeling of his hot breath against your oversensitive sex, before you state your intentions in an imperious tone.  \"<i>Lick.</i>\"");
    ...
    if (player.torso.cocks.count > 0) DisplayText("wrap tightly around your waiting prick");
    else DisplayText("burrow deep into your needy slit");
    DisplayText(".  You coo in delight at the sensation, and begin to grind yourself against his face as you wait for your egg-laying to begin.");
    DisplayText("\n\nThe monster's stomach bloats as a final rush of goo fills his bowels in preparation for your ");
    if (player.pregnancy.ovipositor.fertilizedEggs === 0) DisplayText("un");
    DisplayText("fertilized orbs, providing you with a moment's warning before the ecstasy-inducing ripple of an egg sliding deep into him rolls your eyes backwards in pleasure.  You twitch and squirm atop the bound basilisk, mashing your ");
    if (player.torso.cocks.count > 0) DisplayText("dick");
    else DisplayText("cunt");
    DisplayText(" against his face while your egg-laden tube pumps away, each pulse forcing another egg through his squeezing ring and sending trills of intoxicating euphoria up your spine.  You lose yourself in the act, your heartbeat racing in time with contractions of your ovipositor, spraying ");
    if (player.torso.cocks.count > 0) DisplayText("semen");
    else DisplayText("femcum");
    DisplayText(" across your partner's face and neck.");
    ...
    // (if corruption >= 50
    if (player.stats.cor >= 50) DisplayText(" before heading off towards your camp. The harpies will probably free him from your webs... eventually.");
    else DisplayText(".  You reach up and slice him free from the webbing, carefully placing him on the ground below.  The blindfold stays, though.  You're kind, not stupid.  With your good deed for the day complete, you gather yourself up and head back to camp.");
    player.pregnancy.ovipositor.dumpEggs();
    player.orgasm();
    return { next: Scenes.camp.returnToCampUseOneHour };
}

function layBeeEggsInABasilisk(player: Character, monster: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Basilisk);
    DisplayText().clear();
    DisplayText("Completely destroyed, the basilisk lays face down on the ground, struggling pathetically to get up.  You giggle at the attempt as you ");
    if (player.torso.wings.type === WingType.BEE_LIKE_SMALL || player.torso.wings.type === WingType.BEE_LIKE_LARGE) DisplayText("buzz lightly over to the basilisk's writhing body");
    else DisplayText("saunter arrogantly over to the basilisk's crumpled up figure");
    DisplayText(" and lightly nudge him in his ribs with your [foot].  His eyes are closed from to the ");
    ...
    if (player.torso.hips.legs.isTaur() || player.gender === Gender.NONE) DisplayText("\n\nYou drool messily on your hand and cup it to the basilisk's hole; he hisses in displeasure - though he must have had an inkling of what was coming, the visceral sensation of having his asshole lubed for entry is probably sending his imagination careening off in reckless, tormenting anticipation.");
        ...
        DisplayText("; rubbing enthusiastically, you start at the base but end in a gentle stroke at the sensitive tip.  With you doing your best to milk as much pre as you can, the viscous ooze that dribbles out of your dong");
        if (player.torso.cocks.count) DisplayText("s");
        DisplayText(" lands with soft pitters on the overgrown lizard's back, the intense mountain sun baking the gunk onto the scales.  You sigh in languor, but remember you're doing this for a reason.");
    }
    // (else if pc has vag:
    ...
    if (player.torso.hips.legs.isTaur()) DisplayText("above");
    else DisplayText("behind");
    ...
    if (player.pregnancy.ovipositor.fertilizedEggs) DisplayText("fertilized offspring");
    else DisplayText("burdensome load");
    DisplayText(".");
    ...
    if (player.torso.hips.legs.isTaur() || player.gender === Gender.NONE) {
        DisplayText("\n\nThere's nothing to distract you from the pleasure; ");
        // [(U)
        if (player.gender === Gender.NONE) DisplayText("no needy cock or cunt demanding a caress - except the basilisk's, and you care not a bit about that one.");
        else if (player.torso.hips.legs.isTaur()) DisplayText("even if you wanted to reach your genitals, your arms resolutely refuse to bend that way, so you focus on the slippery, silky feelings travelling along your ovipositor.");
    }
    if (player.torso.cocks.count > 0) {
        DisplayText("\n\nYour seeping member");
        if (player.torso.cocks.count > 1) DisplayText("s");
