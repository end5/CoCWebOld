...
export class BasiliskScene {
	...
	private tongueBasiliskSmex() {
		...
		if (player.torso.cocks.count > 0) {
			...
			//Multicock: 
				...
				DisplayText("The basilisk wraps itself around part of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + "... and then stops.  Confused and frustrated, you look down.  The tip of its tongue has touched your " + Desc.Cock.describeCock(player, 1) + ", and cautiously, it begins to inch its way downwards and around, working towards your second tip. \"<i>Oh, aren't you a clever girl,</i>\" you sigh, revelling in the sensation of both of your cocks trapped in the dizzying, soft yet tight vice grip your serpent sub now has you in. The basilisk grunts at the humiliating commentary. \"<i>Squeeze. Work them, nice and slow,</i>\" you say huskily.  The basilisk's tongue slides up and down your length, its saliva slathering every inch, the sticky, sucking sensation encompassing your cocks on all sides.  It squeezes you softly and then relaxes... squeezes you, then relaxes, as it continues to spiral around and around.  It bends forward, investing more of its tongue around you, inching its way further up your " + Desc.Cock.describeCock(player, 1) + ".  Its tip comes off you with a sucking sound, and it begins to tease the head of your second prick, circling it and lavishing it with attention as it continues to work your two lengths.  \"<i>How... where did you learn to do that?!</i>\" you gasp, and are immediately grateful the basilisk can't respond to your query.  It is everything you can do not to go over the edge straight away; you hold back desperately, staring upwards.  To blow your load before properly enjoying what is turning out to be a truly unbelievable suck would be a terrible crime.  You look downwards and manage to pull yourself back a bit by laughing hugely at the sight.  With the basilisk's tongue spiralled around you, your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " looks like nothing so much as an obscene candy cane.  The basilisk seems to take your hysterical laughter for anger or contempt, because it hastily increases its pace, forcing its sticky muscle up and down as fast as it can.  You lean back and lose yourself in the blissful, sucking, enveloping, teasing sensation.\n\n");
				DisplayText("You last for as long as you can before, with a euphoric sigh, you reach your peak.  Your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " and " + Desc.Cock.describeCock(player, 1) + " both bulge and then rocket out streams of jizz in concert.  Directly in the line of fire, the blind basilisk takes its minute-long double facial with as much dignity as it can muster- which is to say, none at all.\n\n");
			}
			//Both go to: 
	...
	private defeatBasiliskAndAnal() {
		...
		if (player.torso.cocks.count > 0) {
			...
			//[(13 inches or less:)
			if (player.torso.cocks.get(x).length <= 13) {
			...
			//<(13 inches or less:) 
			if (player.torso.cocks.get(x).length <= 13) DisplayText("gripping its hips and pushing every inch of yourself in and reaching a glorious, skin tingling peak.");
			//(More than 13 inches:)
	...
	public loseToBasilisk() {
		...
		if (player.torso.vaginas.count > 0 && (player.inHeat || player.perks.has(PerkType.Oviposition) || player.perks.has(PerkType.BasiliskWomb) || player.pregnancyType === PregnancyType.OVIELIXIR_EGGS))
			basiliskHasVagEggStuff();
		else defaultBasiliskRape();
	}
	//Loss, vag rape conditions not met:
	private defaultBasiliskRape() {
	...
	private basiliskHasVagEggStuff() {
		DisplaySprite(75);
		player.slimeFeed();
		...
		//(Heat: 
		if (player.inHeat) DisplayText("  You are more aware than ever of an invisible scent simmering off you, of your wet vagina clenching and wetting itself in anticipation, your body begging this male creature to fulfil its genetic objective upon you.  Your eyes have betrayed you, your body is betraying you, and whatever else you are is a tiny, ignored voice screaming in between.");
		...
		//(egg preg:
		if (player.pregnancyIncubation > 1 && player.pregnancyType === PregnancyType.OVIELIXIR_EGGS) DisplayText("  The basilisk licks your bulging belly hungrily, pushing against and testing for the eggs you are carrying.  Your sensitive cargo shifts around under its hungry attention; you'd squirm, but that is, of course, impossible.");
		//(heat or perk:
		if (player.inHeat || player.perks.has(PerkType.Oviposition) || player.perks.has(PerkType.BasiliskWomb)) DisplayText("  The basilisk licks your belly hungrily, its sticky tongue crawling like a warm tentacle across your sensitive underside.  You'd squirm, but that is impossible.  The creature is making you feel everything it is forcing upon you.");
		DisplayText("\n\n");
	...
	private basiliskAdvantageNobody() {
		DisplaySprite(75);
	...
	private basiliskAdvantageHarpy() {
		DisplaySprite(75);
		DisplayText("Time stretches by at an agonizingly slow pace as you stand there, a bizarre, motionless flesh statue.  You have no way of measuring how much time is passing; the sun is not in your direct line of vision.   You try to move any and every part of yourself in turn, but it is hopeless.  Your body is a cage, and you begin to hate the basilisk less because it paralyzed you and more because it left your mind entirely aware of it.  Every so often another unbidden backwash of erotic memories overwhelms your senses, keeping you helplessly aroused and reminded of who did this to you.  Coupled with the unscratchable itches and the aching in your limbs the experience is one of sensational hell.\n\n");
		...
		if (player.torso.cocks.count > 0) {
			...
			//Multicock: 
				...
				DisplayText("The three of them do eventually work out a compromise however, once they discover you have more than one manpole to your name.  One pulls your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " forwards painfully until it is almost pointing towards your face before squatting over it, whilst another clutches at your " + Desc.Cock.describeCock(player, 1) + " as she spreads her legs and works her way inwards until she is spearing herself on you. You can't see them - you can't see anything except pink wobbly flesh and lavender feathers - but you can hear them shift impatiently around each other as they find a position which is comfortable to them both, their inner walls rubbing you from every direction as they move.  The third continues to thrust her needy sex into your face as best she can");
	...
	private basiliskAdvantageGoblin() {
		DisplaySprite(75);
		DisplayText("Time stretches by at an agonizingly slow pace as you stand there, a bizarre, motionless flesh statue.  You have no way of measuring how much time is passing; the sun is not in your direct line of vision.   You try to move any and every part of yourself in turn, but it is hopeless.  Your body is a cage, and you begin to hate the basilisk less because it paralyzed you and more because it left your mind entirely aware of it.  Every so often another unbidden backwash of erotic memories overwhelms your senses, keeping you helplessly aroused and reminded of who did this to you.\n\n");
		...
		if (player.torso.cocks.count > 0) {
			...
			DisplayText("\"<i>A " + player.race() + "!</i>\" she yelps.  Then, after shifting her startled attention downwards, \"<i>A cock!</i>\"  Hesitantly at first, then with increasing confidence as how vulnerable you are sinks in, she does a round circuit of you, taking in every angle of your frozen, helpless flesh.\n\n");
		...
		//Unsexed: 
			...
			DisplayText("\"<i>A " + player.race() + "!</i>\" she yelps.  Hesitantly at first, then with increasing confidence as how vulnerable you are sinks in, she does a round circuit of you, taking in every angle of your frozen, helpless flesh.\n\n");
			...
			//(Addict: 
			if (Flags.list[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] > 0 || player.perks.has(PerkType.MinotaurCumAddict)) DisplayText("  You're incredibly frustrated that you can't clench yourself and hold the magical substance deep inside you, so you can savor the wonderful, soft elation it blossoms inside of you for as long as you can.");
			DisplayText("\n\n");
	...
	private basiliskAdvantageMinotaur() {
		DisplaySprite(75);
		DisplayText("Time stretches by at an agonizingly slow pace as you stand there, a bizarre, motionless flesh statue.  You have no way of measuring how much time is passing; the sun is not in your direct line of vision.   You try to move any and every part of yourself in turn, but it is hopeless.  Your body is a cage, and you begin to hate the basilisk less because it paralyzed you and more because it left your mind entirely aware of it.  Every so often another unbidden backwash of erotic memories overwhelms your senses, keeping you helplessly aroused and reminded of who did this to you.\n\n");
	...
	private driderPCEggLaysBasilisk() {
		DisplaySprite(75);
		DisplayText().clear();
		...
		if (player.fertilizedEggs() === 0) DisplayText("un");
	...
	private layBeeEggsInABasilisk() {
		...
		if (player.fertilizedEggs()) DisplayText("fertilized offspring");
