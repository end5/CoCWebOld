...
import { NextScreenChoices, ClickOption, ClickFunction } from "../../../ScreenDisplay";
...
export interface MinervaFlags {
    MINERVA_LOVE: number;
    MINERVA_BACKSTORY: number;
    MINERVA_BACKSTORY_LEARNED: number;
    TIMES_MINERVA_SEXED: number;
    TIMES_BUTTFUCKED_MINERVA: number;
    TIMES_MINERVA_COWGIRLED: number;
    TIMES_MINERVA_LAPSEXED: number;
    MINERVA_LEZZES_OUT: number;
    MET_MINERVA: number;
}
...
const minervaFlags = {
    MINERVA_LOVE: 0,
    MINERVA_BACKSTORY: 0,
    MINERVA_BACKSTORY_LEARNED: 0,
    TIMES_MINERVA_SEXED: 0,
    TIMES_BUTTFUCKED_MINERVA: 0,
    TIMES_MINERVA_COWGIRLED: 0,
    TIMES_MINERVA_LAPSEXED: 0,
    MINERVA_LEZZES_OUT: 0,
    MET_MINERVA: 0,
};

User.flags.set(FlagType.Minerva, minervaFlags);
...
function minervaRomanced(): boolean {
    return (minervaFlags.MINERVA_LOVE === 1);
}
...
function minervaAppearance(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    ...
    return minervaMainMenuChoices(player, false, true, true);
}

function minervaMainMenuChoices(player: Character, appearance?: boolean, sex?: boolean, spar?: boolean): NextScreenChoices {
    return {
        choices: [
            appearance ? ["Appearance", minervaAppearance] : ["", undefined],
            ["Talk", minervaTalkSelect],
            sex ? ["Sex", player.stats.lust > 33 ? minervaSexMenu : undefined] : ["", undefined],
            ["Eat", eatSomethingYouCunt],
            ["Drink", getADrinkYouBitch],
            spar ? ["Spar", fightMinerva] : ["", undefined],
        ],
        persistantChoices: [["Leave", Scenes.camp.returnToCampUseOneHour]]
    };
}

export function encounterMinerva(player: Character): NextScreenChoices {
    if (minervaFlags.MET_MINERVA > 0) minervaFlags.MET_MINERVA++; // Increment counter by 1 if previously met Minerva.
    if (minervaFlags.MET_MINERVA === 0) return firstMinervaEncounter(player);
    else if (minervaFlags.MET_MINERVA === 3) return minervaThirdPlusEncounter(player);
    else if (minervaFlags.MINERVA_BACKSTORY_LEARNED > 0 && player.gender > 0 && minervaFlags.MINERVA_LOVE === 0 && randInt(2) === 0) return minervaAfterFiveTalksRomancing(player);
    else {
        if (randInt(2) === 0) return minervaThirdPlusEncounter(player);
        else return repeatEncounterMinerva(player);
    }
}

// Basic intros
// 1-1 First Encounter.
function firstMinervaEncounter(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplayText("Returning to the mountains, you begin to climb; the usual search for loot and, perhaps, some harpies driving your upward efforts.  Perhaps you'll run into");
    // If Sophie Bimbo=0:
    if (User.flags.get<SophieFlags>(FlagType.Sophie).SOPHIE_BIMBO === 0 && User.flags.get<SophieFlags>(FlagType.Sophie).SOPHIE_DISABLED === 0 && !Scenes.npcs.sophieFollowerScene.sophieFollower()) DisplayText(" that matronly harpy, Sophie");
    ...
    return { choices: [["Take Path", takeMinervasPath], ["Ignore Path", ignoreMinervasPath]] };
}
// [Ignore Path]
function ignoreMinervasPath(): NextScreenChoices {
    DisplayText().clear();
    DisplayText("Deciding to play it safe, you turn away from the new path and continue your search elsewhere.");
    // PC returns to camp.
    return { next: Scenes.camp.returnToCampUseOneHour };
}
// [Take Path]
function takeMinervasPath(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    DisplayText("Knowing that a new path could lead to great things, you can't help but wonder where this one could lead.");
    ...
    if (player.tallness <= 96) DisplayText(" one hand");
    else DisplayText(" both hands");
    DisplayText(", bodily swinging you out over the abyss, with only her grip keeping you from plunging back down the mountainside.");
    // Low corruption:
    if (player.stats.cor < 50) DisplayText("  \"<i>All right, you!  Who are you and what are you doing here?!  I don't get any visitors besides those horny harpies, so what are you after?  Hm?");
    // High corruption:
    else DisplayText("  \"<i>All right, you corrupted beast!  Who are you and what are you doing here?  Your presence is not welcome in this sanctuary... answer quickly or I'll drop you to your death!");
    // Demon Score=3>:
    if (RaceScore.demonScore(player) >= 3) DisplayText("  Filthy demon!  Come to finish your work, have you?!  You won't take me without a fight!  Maybe if you beg for your life I'll give you a chance. Speak!  Convince me to not simply kill you here and now!");
    DisplayText("</i>\" the creature demands, clearly threatening to drop you if you don't answer to her liking.");
    ...
    if (player.torso.hips.legs.isGoo()) {
        ...
        // Corruption=75>:
        if (player.stats.cor >= 75) DisplayText("... \"<i>yet</i>\", you think to yourself.");
    ...
    // High corruption:
    if (player.stats.cor > 66) DisplayText("  She is wise not to fully trust you.");

    DisplayText("\n\nWith");
    if (player.torso.hips.legs.isGoo()) DisplayText(" your own");
    else DisplayText(" a");
    DisplayText(" relieved sigh, you take in the sight of the strange shark... harpy... harpy-shark... sharpy?  What an odd combination!  Looking the woman up and down, you take in her whole appearance.");
    ...
    minervaFlags.MET_MINERVA = 1;
    // [Follow] [Leave]
    return { choices: [["Follow", followMinerva], ["Don'tFollow", leaveMinervasFirstEncounter]] };
}

// [Follow]
function followMinerva(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    DisplayText("Deciding that you want to know more about Minerva, you follow her into the tower.  \"<i>So, now that you're inside, would you like to talk?  Or perhaps to partake in some of the peaches that grow on my trees?</i>\" she asks, looking at you with interest.");
    // [Talk] [Drink] [Eat] [Leave]

    // MainScreen.addButton(9, "Leave", leaveMinervasFirstEncounter);
    // addButton(0,"Appearance",minervaAppearance);
    // MainScreen.addButton(1, "Talk", minervaTalkSelect);
    // addButton(2,"Sex",minervaSexMenu);
    // MainScreen.addButton(3, "Eat", eatSomethingYouCunt);
    // MainScreen.addButton(4, "Drink", getADrinkYouBitch);
    return minervaMainMenuChoices(player, false, false, false);
}

// [Leave](for first encounter options)
function leaveMinervasFirstEncounter(): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
...
function repeatEncounterMinerva(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    ...
    // No romance:
    if (!minervaRomanced()) DisplayText("  \"<i>Ah, [name], you have returned!  What can I do for you?</i>\"");
    else DisplayText("  \"<i>Mmm... my lover has come to see me!  Oh, how I have missed you, my sweet!  Won't you join me for a bath?  Or... is there anything else I can do for you?</i>\"");

    // [Talk] [Sex] [Eat] [Fight] [Leave]

    // MainScreen.addButton(0, "Appearance", minervaAppearance);
    // MainScreen.addButton(1, "Talk", minervaTalkSelect);
    // if (player.stats.lust >= 33) MainScreen.addButton(2, "Sex", minervaSexMenu);
    // MainScreen.addButton(3, "Eat", eatSomethingYouCunt);
    // MainScreen.addButton(4, "Drink", getADrinkYouBitch);
    // MainScreen.addButton(5, "Spar", fightMinerva);
    // MainScreen.addButton(9, "Leave", Scenes.camp.returnToCampUseOneHour);
    return minervaMainMenuChoices(player, true, true, true);
}

// 1-3 Repeat Encounter 2.  Use if 3rd+ time visiting Minerva.
function minervaThirdPlusEncounter(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    ...
    // No romance:
    if (!minervaRomanced()) DisplayText("  \"<i>Welcome back, [name]!  It's good to see you again.  Have you come to give me some company, or would you like to partake in some of the fruit?  Or are you maybe here to sate your thirst?</i>\"");
    else DisplayText("  \"<i>Darling!  Coming to visit me all the way out here!  How sweet of you!</i>\"");
    DisplayText("  Minerva says as she strides toward you, her generous hips swaying back and forth, as if to entice you into her arms.");
...
function minervaAfterFiveTalksRomancing(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    minervaFlags.MET_MINERVA++;
    ...
    if (!player.torso.hips.legs.isTaur() && !player.torso.hips.legs.isDrider()) {
        ...
        DisplayText("  Looking up, you see the grinning face of Minerva, clearly playing with you.  \"<i>Well, hello there, [name]!  I was wondering when you would come and visit me.  I can be away from the oasis for a little while, so... want to go for a flight, or are you scared of heights?</i>\"  she asks playfully.");
        // [Flight] [Land]
        return { choices: [["Flight", goForASkyRideWithMinervaHeartBaring], ["Land", dontFlyWithWithMeYouCrazyBitch]] };
    }
    else {
...
function goForASkyRideWithMinervaHeartBaring(player: Character): NextScreenChoices {
    ...
    // Goo:
    if (player.torso.hips.legs.isGoo()) DisplayText(" while you're barely able to maintain your slimy cohesion");
...
// Opening her heart
function minervaOpensHerHeartAndDisplaysHerVentricals(): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    ...
    return { choices: [["Kiss Her", yeahSureMinervaILikeYouAndShit], ["Reject", rejectMinervasLove]] };
}

// Kiss
function yeahSureMinervaILikeYouAndShit(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    ...
    minervaFlags.MINERVA_LOVE = 1;

    // {Male: [NEXT] / Female/Herm: [NEXT]}
    // Sex Menu
    if (player.gender === Gender.MALE) {
        if (player.torso.cocks.find(Cock.CockThatFits(minervaVCapacity()))) return { next: minervaCowgirlSex };
        else return { next: letMinervaSuckYouOff };
    }
    else return { next: minervaLapSex };
}

// [Reject]
function rejectMinervasLove(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    DisplayText("You stare at Minerva awkwardly as she waits for your response, her electric blue eyes looking at you with hope.  ");
    if (player.stats.cor < 45) DisplayText("The look in the siren's eyes only makes this harder for you as you look into them; you can't help but feel bad that you really don't feel anything special for her.");
    else DisplayText("You can tell she really wants this - the siren is almost desperate for your attention.  Unfortunately for the love starved herm, she won't find the love she craves with you.");
    DisplayText("  You look away from her and pull back a bit before speaking.  You confess to her that, while she is a nice person, you do not feel that sort of bond with her, and you apologize if she was misled by your actions, but that you would like to be friends with her still.");
...
// [Land]
function dontFlyWithWithMeYouCrazyBitch(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
...
function fightMinerva(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Minerva);
    return CombatManager.beginBattle(player, [], [new Minerva()]);
}

// 1-2 Repeatable Cute, Romantic Encounter. Only if PC accepted Minerva's feelings. Add to normal encounters.
function repeatableMinervaRomanceScene(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
...
function minervaTalkSelect(player: Character, bath: boolean = true): NextScreenChoices {
    const choices: ClickFunction[] = [talkingToMinervaAboutBackstory];
    if (minervaFlags.MINERVA_BACKSTORY_LEARNED === 0) {
        // Force first time to talk about her early backstory!
        if (minervaFlags.MINERVA_BACKSTORY === 0) {
            return talkingToMinervaAboutBackstory();
        }
    ...
    if (minervaRomanced() && minervaFlags.MINERVA_BACKSTORY_LEARNED === 1) choices[choices.length] = talkWithMinervaAboutSharkGirlDaughter;
...
function talkingToMinervaAboutBackstory(): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    // PART 1
    if (minervaFlags.MINERVA_BACKSTORY === 0) {
        minervaFlags.MINERVA_BACKSTORY = 1;
    ...
    // PART 2
    else if (minervaFlags.MINERVA_BACKSTORY === 1) {
        minervaFlags.MINERVA_BACKSTORY = 2;
    ...
    // PART 3
        ...
        minervaFlags.MINERVA_BACKSTORY = 0;
        ...
        // No Romance:
        if (!minervaRomanced()) DisplayText("  \"<i>You're a good friend to have, and you know that, [name].</i>\"");
        // Romance:
        else DisplayText("  \"<i>You're a wonderful lover, you know.  I couldn't be happier that you found me.</i>\"");
        // PC returns to camp.
        minervaFlags.MINERVA_BACKSTORY_LEARNED = 1;
    }
    return { next: Scenes.camp.returnToCampUseOneHour };
}
...
// 3-2 Talk Scene 2 - talks about the spring
// -repeatable
function talkAboutTheSpringWithMinerva(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
...
// 3-3 Talking Scene 3 - talks about her shark girl daughter
function talkWithMinervaAboutSharkGirlDaughter(): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    // - requires that backstory has been told
    // - if already romanced Minerva shamefully confesses she may have fathered a granddaughter with her first daughter -repeatable
...
// 3-4 Talk Scene 4 - talks about her corruption
// - romance yes only or if you trick her into thinking you love her. - Repeatable until Minerva is purified or corrupted
function talkToMinervaAboutHerCorruption(): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    DisplayText("Telling Mineva that you just want to talk brings a smile to her face, the siren eager to speak with you.  Guiding you over to a soft spot of moss by the spring, she sits next to you and looks into the pond, her smile slowly fading as she looks into the clear waters.");
    ...
    if (minervaFlags.MINERVA_BACKSTORY_LEARNED === 1) DisplayText("  You remember my firstborn, the shark girl I had?  Well... not long after she reached maturity, she... well, she got a bit amorous...  She seduced me and I... I mated with her.  I mated with her over and over that night.  I enjoyed it... it was like I couldn't get enough...  I probably got her pregnant...  So shameful...  I impregnated my own daughter and made her bear my grandchild,");
    DisplayText("</i>\" she declares sourly as she stares into the pure waters of the spring.");
...
// 3-5 Motherhood
// -talks about how she wishes to be a real mother, have an actual loving family and not made from being raped- repeatable
function minervaMotherhood(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    ...
    if (minervaRomanced()) DisplayText("lover");
    else DisplayText("friend");
    ...
    if (player.torso.vaginas.count > 0) {
        ...
        // friend/lover
        if (!minervaRomanced()) DisplayText("friend");
    ...
    if (player.gender === Gender.MALE) DisplayText(" and those goblins");
    // PC vagina:
    else if (player.gender === Gender.FEMALE) DisplayText(" and that bee-girl");
    // PC herm:
    if (player.gender === Gender.HERM) DisplayText(", goblins and that bee-girl");
    DisplayText(", if the stories were real, I'm sure you gave them plenty of children.  I wish to one day have kids... to have a great, loving family to call my own.</i>\"");

    DisplayText("\n\nShaking your head, you grin and shuffle over to Minerva.  Sitting down with her, you");
    // Romance:
    if (minervaRomanced()) DisplayText(" hug her tightly and kiss her cheek, telling her that she will have children one day, and perhaps you will have to see about helping her with that 'problem.'");
    // No romance:
    else DisplayText(" place a hand on her shoulder and give her a comforting smile before telling her that she will have that one day - you're sure of it.");
    DisplayText(" Hearing your words brings a smile to her black glossy lips.  \"<i>Thank you so much; that means a lot to me, really,</i>\" she coos, her hands gingerly stroking yours before pulling away.  \"<i>But... I guess we have been here for a while.  I'm sure you have a great deal of work to do as such a brave champion.</i>\"");
...
// 3-6 Bath Time - romance only
function bathTimeTalkWithMinerva(): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    ...
    // [Yes] [No]
    return { yes: bathTimeWithMinerva, no: noBathTime };
}

// [No] // add and take to another talking scene
function noBathTime(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplayText("Looking down, she sighs, a brief moment passes before she returns her attention to you.  \"<i>Well, that's all right, maybe next time.  So you wanted to talk; what would you like to know?</i>\"");
    return { next: partial(minervaTalkSelect, player, false) };
}

// [Yes]
function bathTimeWithMinerva(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
...
function minervaSexMenu(player: Character, display: boolean = true): NextScreenChoices {
    if (display) {
        DisplayText().clear();
        DisplaySprite(SpriteName.Minerva);
        DisplayText("You tell Minerva that, this time, you'd like to have sex with her.");
        // Default, first time:
        if (minervaFlags.TIMES_MINERVA_SEXED === 0) {
            ...
            // PC cock, add:
            if (player.torso.cocks.count > 0) DisplayText(" just... not in my vagina, all right?  That's... special.");
            DisplayText("  Did you have anything specific in mind?");
            // PC female, add: "
            if (player.torso.vaginas.count > 0) DisplayText("  Would you mind how we do it?  You're the one asking, so... would you want me to fuck your ass?  Or... do you want me to try to fit between those sexy hips of yours?");
            DisplayText("</i>\"");
        }
        // Dick Too Big - PC exceed capacity
        else if (randInt(2) === 0 && player.torso.cocks.count > 0 && !player.torso.cocks.find(Cock.CockThatFits(minervaACapacity())))
        ...
        // {Repeated/No romance:
            ...
            if (player.torso.vaginas.count > 0) DisplayText("  Since you don't seem to mind... would you like me to fuck your ass or to squeeze into your cute cunny?");
            DisplayText("</i>\"");
        }
        // Romanced:
        else {
            ...
            // PC cock, add:
            if (player.torso.cocks.count > 0) DisplayText("  Since we're together, I'll even let you have my pussy, I bet you like the sound of that!");
            // PC female, add:
            if (player.torso.vaginas.count > 0) DisplayText("  I'd be happy to pound that cute butt of yours and stuff your hot pussy - which would you rather have, my dear?");
            DisplayText("</i>\"");
        }
    }

    const choices = [];
    if (player.torso.cocks.count > 0 && player.torso.cocks.find(Cock.CockThatFits(minervaACapacity())))
        choices.push(["FuckHerButt", fuckMinervasAsshole]);
    if (player.torso.cocks.count > 0 && player.torso.cocks.find(Cock.CockThatFits(minervaVCapacity())) && minervaRomanced()) {
        choices.push(["FuckCowgirl", minervaCowgirlSex]);
        choices.push(["RestrainFuck", fuckMinervaWithHerHandsBehindHerBack]);
    }
    if (player.torso.vaginas.count > 0)
        choices.push(["TakeHerDick", minervaLapSex]);
    choices.push(["EatHerOut", goDownOnAHermAndLoveItYouDirtySlutYou]);
    if (player.torso.cocks.count > 0)
        choices.push(["Get BJ", letMinervaSuckYouOff]);
    return { choices, persistantChoices: [["Leave", repeatEncounterMinerva]] };
}

// 4-1= sex scene 1 male/herm:  Anal
// -pc pushes Minerva down onto her chest ass up and fucks her ass = needs penis
function fuckMinervasAsshole(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    let cockThatFits = player.torso.cocks.find(Cock.CockThatFits(minervaVCapacity()));
    if (!cockThatFits) cockThatFits = player.torso.cocks.sort(Cock.SmallestCockArea)[0];
    minervaFlags.TIMES_MINERVA_SEXED++;
    ...
    if (player.torso.vaginas.count > 0) DisplayText(" and your [vagina]");
    DisplayText(" to the open air.");
    DisplayText("\n\nFeeling Minerva's bare ass rubbing against you helps coax your ");
    if (player.torso.cocks.count > 1) DisplayText("shafts");
    ...
    DisplayText("\n\nYou draw yourself back, dragging your " + Desc.Cock.describeCock(player, cockThatFits) + " inch by inch through her grinding, squeezing walls, then thrust yourself firmly back inside her.  You repeat the process again, and then again, picking up speed as you pull and push.  The siren's lyrical moans slowly start to fill the air, her body rocking with your thrusts, pressing and grinding her pierced nipples against the ground as you pound her slick rear.  The longest of her cock tentacles manage to reach back and grabs hold of your " + Desc.Cock.describeCock(player, cockThatFits) + ", feverishly stroking you as you thrust into her.  The naughty little things sting you hard, oozing their aphrodisiac venom into your " + Desc.Cock.describeCock(player, cockThatFits) + " instinctively.  You cry out, shivering as the potent venom immediately streams through your body, setting your blood aboil with lust, and causing you to piston into her ass with ever-increasing speeds.  More and more toxin floods you, combining with the incredible sensations of her anal walls gripping and squeezing you to turn you into a mindlessly rutting beast.");
    ...
    if (!minervaRomanced()) {
        // First time ending:
        if (minervaFlags.TIMES_BUTTFUCKED_MINERVA === 0) {
    ...
    minervaFlags.TIMES_BUTTFUCKED_MINERVA++;
    player.orgasm();
    player.stats.sens += -1;
    return { next: Scenes.camp.returnToCampUseOneHour };
}
...
function minervaCowgirlSex(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    let cockThatFits = player.torso.cocks.find(Cock.CockThatFits(minervaVCapacity()));
    if (!cockThatFits) cockThatFits = player.torso.cocks.sort(Cock.SmallestCockArea)[0];
    minervaFlags.TIMES_MINERVA_SEXED++;
    DisplayText("You look the attractive siren up and down and");
    if (player.tallness <= 60) DisplayText(", with some trepidation, given the difference in size,");
    DisplayText(" ask if Minerva would like to give her female parts some pleasure, promising to allow her control of the pace.");
    // First time:
    if (minervaFlags.TIMES_MINERVA_COWGIRLED === 0) {
        DisplayText("  Minerva looks unsure, the siren biting her lip and looking a little away from you, \"<i>I don't know.  That's... special...  After the things I have been through, I don't want to just do it with anyone.  You... </i>have<i> been really nice to me though... I guess I can trust you.</i>\"");
    }
    // Repeated:
    else DisplayText("  Minerva runs a hand through her hair and smiles, \"<i>Well... the last time was quite nice, I think it could be fun.  Sure, let's do it.</i>\"");

    DisplayText("\n\nWith a smile, you gently remove your clothes and put them aside, giving Minerva a chance to see your naked body.  Your " + Desc.Cock.describeCock(player, cockThatFits) + " already starting to stiffen, you lie back down on the ground, patiently waiting for the siren to make the first move.  You don't have to do so for long, as soon, Minerva's tight tube-top and short shorts are pulled off, her firm, pierced DD's bouncing free of their fabric prison.  Clearly excited by the thrill of actually having regular sex, you can see her male endowment quickly growing hard between her legs, the forearm-length appendage ready to go despite the fact it won't be used for the main event.  Looking down at you, Minerva approaches, swaying her hips to and fro.  She kneels down, crawling over you like some predator, one of her hands briefly stroking your " + Desc.Cock.describeCock(player, cockThatFits) + " before sliding up to feel your [chest].  \"<i>I promise not to get too excited,</i>\" she says before grabbing hold of your maleness and stroking it again.  Bringing her own male organ up against yours, her tentacles encircle your " + Desc.Cock.describeCock(player, cockThatFits) + ", stinging it with her potent venom to really get you fired up as she teases you.  \"<i>This stuff is always floating around in me - that's why I get so damn hard so fast...  I hope you like my special venom.</i>\"");

    DisplayText("\n\nYou groan softly, feeling the venom already coursing through you.  Your " + Desc.Cock.describeCock(player, cockThatFits) + " surges erect, the venom-induced lust making you swell larger than usual, ready for her to do with as she pleases, your limbs numb and useless from the lust-inducing toxins coursing through your body.  You are completely helpless and at her mercy...  And part of you seems to like that.");

    DisplayText("\n\nMinerva seems to enjoy seeing you helpless like this, as her dick seems to just get harder and harder while her tentacles caress your member, though they no longer sting; instead, they just tease you with their slick and gentle touch.  \"<i>You like that, don't you?  You're so hard I can feel your need growing more and more.  You want my tight pussy so badly,</i>\" she says with a lecherous grin before rising her body up and pressing against your " + Desc.Cock.describeCock(player, cockThatFits) + " with her surprisingly cool cunt.  Already her lips are soaked, her slick juices running down your dick as she teases you even further.");

    DisplayText("\n\nFinally, she ends your torment and brings your " + Desc.Cock.describeCockHead(cockThatFits) + " to her refreshingly wet entrance before pushing down, letting you penetrate her with agonizing slowness, her longer tentacles still stroking and caressing your length as she pulls you into the vicelike grip of her snatch.  Inside, you can feel smaller tendrils starting to caress and squeeze you, clearly another trait she was gifted with from the shark girls.");
    ...
    if (player.tallness < 96) DisplayText(", as well as her superior size,");
    ...
    DisplayText("\n\nYou moan and shudder, shivering violently as Minerva's poisons boil through your blood and countless tendrils stoke your lust to a fever pitch.  Your brain seethes and roils with pleasure as you are surrounded, engulfed.  You don't think you can hold back from cumming for very long, and gasp a warning to the siren that you will cum inside her if she isn't careful.  Clearly hearing your response, a sinister grin grows on Minerva's face as two of her longest and strongest tentacles wrap around the base of your " + Desc.Cock.describeCock(player, cockThatFits) + " and squeeze tightly, \"<i>Ah-ah-ah... no cumming for you, not until I get mine... I'm the one in charge, here, remember?</i>\"");
    DisplayText("\n\nYou can't help but beg and plead for her to show mercy on you, you can't bear this much longer!  Unfortunately for you, the only response you get from Minerva is a smirk, and the tentacles tightening around your throbbing maleness as she draws you in deeper.  The venom and the aggressive treatment makes your dick swell even more, stretching her tight squirming walls around you, the sensation drawing a sharp gasp and soft moan from your siren lover.  The siren's balls come to a rest just above your " + Desc.Cock.describeCock(player, cockThatFits) + ", the heavy, swollen orbs a reminder that she is enjoying this just as much as you.");

    DisplayText("\n\nBiting her lip, she pushes down harder, sinking her soothingly cool pussy down until your whole cock is engulfed in her tight, cilia-filled snatch.  \"<i>Mmm, oh yesss... don't you just love how your " + Desc.Cock.describeCock(player, cockThatFits) + " stretches me... and all those little things in there, stroking you like little tongues.  Tell me how much you need to cum.  Tell me and I'll let you blow your pent-up load deep inside me.  I doubt you could get me pregnant, I'm probably too unstable to breed with someone like you.  Or maybe you'd like to take a gamble and try?  Do it, fill my pussy with your cum!</i>\"");
    ...
    if (player.cumQ() >= 500) {
        ...
        if (player.torso.balls.quantity > 0) DisplayText(" your balls and");
        DisplayText(" the base of your " + Desc.Cock.describeCock(player, cockThatFits) + ", stinging you again and again like an overzealous jellyfish, pumping more of her aphrodisiac venom into your genitals in mind-numbing quantities.");
        if (player.torso.vaginas.count > 0) DisplayText("  A few even slither down into your [vagina] to sting you there as well, making sure you are, at least temporarily, turned into a sex-hungry beast.");
    }
    ...
    if (player.cumQ() < 500) DisplayText(", Minerva's belly visibly bloated by the time you are finished");
    DisplayText(".");
    if (player.cumQ() < 500) {
    ...
    if (player.cumQ() >= 500 && player.cumQ() < 4000) {
        ...
        if (minervaRomanced()) DisplayText(" with your child");
        DisplayText(".");
    }
    // Very High:
    if (player.cumQ() >= 4000) {
        ...
        if (!minervaRomanced()) DisplayText("  \"<i>Oohhh...I'm so... so big... It's almost like you've gotten me pregnant.</i>\"");
        else DisplayText("  \"<i>Oohhh... I'm so... so big... cumming so much, I didn't know you were so desperate to get me pregnant.</i>\"");
    }
    // Low-to-moderate:
    ...
    minervaFlags.TIMES_MINERVA_COWGIRLED++;
    player.orgasm();
    player.stats.sens += -1;
    return { next: Scenes.camp.returnToCampUseOneHour };
}
...
function minervaLapSex(player: Character): NextScreenChoices {
    let cockThatFits = player.torso.cocks.find(Cock.CockThatFits(minervaVCapacity()));
    if (!cockThatFits) cockThatFits = player.torso.cocks.sort(Cock.SmallestCockArea)[0];
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    ...
    // PC herm:
    if (player.gender === Gender.HERM) DisplayText("  One of her hands trail down to your hard herm " + Desc.Cock.describeCock(player, cockThatFits) + ", stroking you softly.");
    ...
    // Nonvirgin:
    if (!player.torso.vaginas.find(Vagina.Virgin)) DisplayText("  The thick siren prick plunges into your soaked cunt and fills your sweltering hole with her oddly cool cock, both of you groaning in delightful pleasure.");
    // Virgin:
    else DisplayText("  The thick siren prick plunges into your soaked cunt, filling your sweltering pussy with her oddly cool cock, in the process tearing right through your hymen, claiming your first fuck for herself as you bite back a pained yelp, though it is quickly displaced by the pleasure from the stimulation and venom.");
    ...
    // PC herm:
    if (player.torso.cocks.count > 0) {
        DisplayText(", your own throbbing phallus");
        if (player.torso.cocks.count > 1) DisplayText("es");
        DisplayText(" cumming alongside,");
        // {Low-moderate cum:
        if (player.cumQ() < 500) DisplayText(" gushing your cum all over both of your bellies");
        // High cum volume:
        else if (player.cumQ() < 4000) DisplayText(" splattering thick splooge all over you and Minerva, caking your torsos in fertile jizz");
        // Very high cum:
        else DisplayText(" exploding out onto your unsuspecting bodies, thick ropes of starkly white sperm gushing over your bodies, quickly dousing both of you in buckets of cum and pooling on the ground");
    }
    DisplayText(".");
    ...
    if (!minervaRomanced()) {
        // Default, first time:
        if (minervaFlags.TIMES_MINERVA_LAPSEXED === 0) {
            DisplayText("\n\nMinerva actually seems a little sad that you're leaving.  \"<i>I guess you are a busy person.  But I'm glad we did this, it was a lot of fun.  And... you know...  I wouldn't mind if we did this again, soon,</i>\" She suggests with a small, hopeful smile on her face.  You promise her that you'll try to come back again soon.");
        }
        // Default, repeated:
        else {
    ...
    player.slimeFeed();
    minervaFlags.TIMES_MINERVA_LAPSEXED++;
    player.orgasm();
    player.stats.sens += -1;
    return { next: Scenes.camp.returnToCampUseOneHour };
}
...
function goDownOnAHermAndLoveItYouDirtySlutYou(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    minervaFlags.TIMES_MINERVA_SEXED++;
    ...
    if (player.tallness < 96) DisplayText("your fingers not even able to fit around the herm's hefty girth");
    else DisplayText("your fingers barely wrapping around the meaty member, the fat thing throbbing gently");
    DisplayText(" as you stroke it slowly.  Your touches are clearly adding to your partner's pleasure, her thick masculinity pulsating under your grip, her tentacles reacting to your presence, the dexterous things curling around your hands and fingers, touching you gently, but not stinging you.");
    ...
    return { choices: [["Balls", lickMinervasBalls], ["No Balls", noBallsMinervaAndContinuation]] };
}

// Balls
function lickMinervasBalls(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    ...
    return noBallsMinervaAndContinuation(player, false);
}

// No Balls and continuation
function noBallsMinervaAndContinuation(player: Character, clear: boolean = true): NextScreenChoices {
    if (clear) DisplayText().clear();
    else DisplayText("\n\n");
    DisplaySprite(SpriteName.Minerva);
    DisplayText("Your careful ministrations soon have the poor siren shuddering and squirming, looking about ready to pop from your skilled hands and mouth.");
    if (!clear) DisplayText("  Retreating from her throbbing balls, you");
...
function letMinervaSuckYouOff(player: Character): NextScreenChoices {
    let cockThatFits = player.torso.cocks.find(Cock.CockThatFits(minervaVCapacity()));
    if (!cockThatFits) cockThatFits = player.torso.cocks.sort(Cock.SmallestCockArea)[0];
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    ...
    // If naga:
    if (player.torso.hips.legs.isNaga()) DisplayText(" lounge against the rock, spreading open your protective slit and letting your " + Desc.Cock.describeCock(player, cockThatFits) + " hang free.");
    else DisplayText(" sit down, spreading your [legs] and opening up the crotch of your [armor] to the air, letting your " + Desc.Cock.describeCock(player, cockThatFits) + " hang free.");

    DisplayText("\n\nAnxious and ready, Minerva looks down at you before kneeling, placing a hand on your [hips] as she takes hold of your " + Desc.Cock.describeCock(player, cockThatFits) + " and slowly starts to stroke it.  Her eyes looking up at you, she leans in and starts to gently lick your hardening flesh with the tip of her tongue.  Under her careful, deliberate attentions your manhood is quickly brought to full attention.");

    // if cock area 21+:
    if (cockThatFits.area < 21) { }
    else if (cockThatFits.area < 34) {
    ...
    DisplayText("\n\nTaking a relaxing breath, Minerva dives right into it, her long tongue slithering out to lick your hard, sensitive flesh, wrapping around the tip and giving the most sensitive part of your dick a good squeeze.  The heat-quenching chill of her wet tongue feels remarkable on your hard prick, the tight squeeze pulling a gasp from your mouth.  Your hands move over her head and into her dark red hair, and you're suddenly relieved that she can't envenom you this way.  Wanting to show you what she's made of, your siren leans in, dragging her cool wet tongue up your " + Desc.Cock.describeCock(player, cockThatFits) + " from base to tip before planting a kiss right on the " + Desc.Cock.describeCockHead(cockThatFits) + " of your throbbing, needy dick.");

    // If balls:
    if (player.torso.balls.quantity > 0) DisplayText("  Her hand on your hip moves down between your legs, sliding inwards to gently caress and massage your cum-filled [balls] to increase your pleasure even more, her hands trying their best to get your nuts worked up and prepare a good load for her at the end.");
    ...
    // If balls:
    if (player.torso.balls.quantity > 0) DisplayText(" before licking and teasing your [balls]");
    DisplayText(".");

    DisplayText("\n\nWith your " + Desc.Cock.describeCock(player, cockThatFits) + " firmly captured by her tongue, Minerva leans down, opening her mouth wide and taking you inside its cool, wet confines, even as her tongue squeezes you and slides around your warm, sensitive flesh.  Looking up at you with a playful twinkle in her eyes, she starts to suck and squirm her tongue around on you, the long, prehensile appendage switching between licking over your throbbing cockmeat and squeezing it in a snakelike embrace.");

    DisplayText("\n\nPuckering her glossy black lips, Minerva starts to bob her head, taking in more and more of you with each downwards motion");
    // If cockarea 50+:
    if (cockThatFits.area >= 50) DisplayText(", your huge cock deep inside her mouth, and soon far down her throat.  With some difficulty, her slightly angular nose presses down against your crotch, every inch of your " + Desc.Cock.describeCock(player, cockThatFits) + " trapped inside her cool, wet mouth and throat");
    DisplayText(".");
    ...
    if (player.torso.balls.quantity > 0) DisplayText(", while your [balls] lurch and swell, rising up as you reach your climactic explosion");
    DisplayText(".");

    DisplayText("\n\nGripping Minerva's head tightly, you let out a long deep moan, your " + Desc.Cock.describeCock(player, cockThatFits) + " bloating with each ejaculation.  Eager to please, Minerva does her best to drink down every drop you give her, her hands gripping your [hips] and pressing her nose against your crotch, puckering her full black lips around your [sheath].");
...
function fuckMinervaWithHerHandsBehindHerBack(player: Character): NextScreenChoices {
    let cockThatFits = player.torso.cocks.find(Cock.CockThatFits(minervaVCapacity()));
    if (!cockThatFits) cockThatFits = player.torso.cocks.sort(Cock.SmallestCockArea)[0];
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    ...
    DisplayText("\n\nWith her so wet now, it's the perfect time to strike!  Deftly pulling your [armor] from your body, you toss the useless pieces away and expose [eachCock] to the air.  Taking hold of [oneCock], you drag its length across Minerva's thighs and rump just to tease her a little more before giving the horny herm the hard dicking she craves.  Reaching down, you lift and spread her curvy rear before aligning yourself, your " + Desc.Cock.describeCockHead(cockThatFits) + " pressing and kissing her netherlips for a moment, drawing a lusty whimper from the siren before tensing your hips and plowing your iron-hard breeding meat into her with body-jerking power.");

    DisplayText("\n\nDespite your brutal thrust, you slide in with ease, Minerva's tight walls so slickened with her arousal.  You squeeze into the deliciously tight herm like butter, easily ramming every inch of your " + Desc.Cock.describeCock(player, cockThatFits) + " into her with only a few hard, deep thrusts.  With your hips pressed against Minerva's accepting body, you grind against her, making sure she can feel every inch moving inside her clinging cunt.");
    // If balls:
    if (player.torso.balls.quantity > 0) DisplayText("  As you continue to pound the siren, your [balls] swing between your legs, the sperm-filled orbs churning with your fertility, preparing to fill the needy siren with every drop you can muster when you let loose your eventual orgasm.");
    ...
    // Holy shit!:
        ...
        if (player.torso.balls.quantity > 0) DisplayText("  Minerva groans and lets out mindless gurgles as you unload your overproductive balls into her.");
        DisplayText("  To even your amazement, her hungry womb is able to contain much of your sperm before she can take no more, each blast of potent cum now gushing from between your bodies, the floor soon looking like someone dumped a barrel full of raw milk on the ground before you're finally done.");
...
function pcGetsEatenOutByMinerva(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    DisplayText("Thinking about just what kind of heated, heart-pounding, lusty fun you want to have with your siren lover, your eyes look the shark-like harpy up and down, taking in her features.  Starting with her long legs, and moving up to her wide, flared hips, your eyes travel past the thick bulge in her shorts, going up her scarred belly to the siren's perky DD-cup breasts before finally reaching her face.");
    ...
    if (player.torso.cocks.count > 0) DisplayText(", and, perhaps, your more male parts while she's down there");
    DisplayText(".");
    ...
    if (player.torso.balls.quantity > 0) DisplayText(", one of her hands lifting your [balls] up before giving each of the fertile nuts a kiss");
    DisplayText(" and moving down to your sweet cunt, just waiting for her attentions.");
    ...
    DisplayText("\n\n");
    // If dick:
    if (player.torso.vaginas.count > 0) DisplayText("Thanks to her handy stimulation, your musky maleness throbs and dilates along your femininity, the thick cockvein feeding your bloating member as your seed rushes out, splattering up and onto Minerva's back, defiling her smooth skin with your fertility.  ");
    ...
    minervaFlags.MINERVA_LEZZES_OUT++;
    // PC returns to camp
    player.stats.sens += -2;
    player.orgasm();
    return { next: Scenes.camp.returnToCampUseOneHour };
...
function loseToMinerva(player: Character, minerva: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    // PC loss by physical
    if (player.stats.HP < 1) {
        ...
        minerva.statusAffects.add(StatusAffectType.PeachLootLoss, 0, 0, 0, 0);
        return player.inventory.items.createAdd(player, ItemType.Consumable, ConsumableName.PurityPeach, Scenes.camp.returnToCampUseOneHour);
    }
    // PC loss by Lust
    else {
        ...
        // Take PC to sex menu
        return { choices: minervaSexMenu(player, false).choices, persistantChoices: [["Leave", Scenes.camp.returnToCampUseOneHour]] };
    }
}

// Victory text for tainted
function beatUpDatSharpie(player: Character, minerva: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    // Win by damage
    if (minerva.stats.HP < 1) {
...
function eatSomethingYouCunt(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    // Acquiring:
    DisplayText("You walk over to the fruit trees surrounding the spring, examining the strange treats.  You decide that one ripe fruit, one resembling a peach, is the best choice, and pluck it from the tree.  Thanking Minerva for letting you have it, you stow it away safely and head back to camp.\n\n");
    return player.inventory.items.createAdd(player, ItemType.Consumable, ConsumableName.PurityPeach, Scenes.camp.returnToCampUseOneHour);
}
...
*/
// Spring Water
function getADrinkYouBitch(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Minerva);
    DisplayText("You ask Minerva if she's okay with allowing you to drink from her spring.  She replies with a nod, a smile and a casual wave towards the pool.  Making it quite clear that you can drink your fill.");
    // [Bottle] [Drink]
    // [Drink]
    ...
    if (player.stats.cor > 75) player.stats.cor += -1;
    return { next: Scenes.camp.returnToCampUseOneHour };
}
/*
