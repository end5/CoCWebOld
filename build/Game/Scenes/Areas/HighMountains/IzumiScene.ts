...
export class IzumiScene {
	...
	protected enterFirstTime(): void {
		...
		if (player.tallness >= 108) {
			...
			if (player.torso.neck.head.hornType != HornType.COW_MINOTAUR) {
		...
		if (player.tallness < 108) {
			...
			if (player.minoScore() > 3) DisplayText("plus it looks like you're thinking with the head on top of your neck, rather than the head between your legs, unlike the stupid brutes who usually wander around these parts.  ");
			...
			else if (player.goblinScore() > 3) DisplayText("plus you don't seem to be as insufferable as the rest of those little green whores who trollop around these parts, hunting their next lay.  ");
			else if (player.demonScore() > 3) DisplayText("plus you're not forever going on about corruption this, enslavement that, demonic taint the other, unlike the insufferable clods who usually wander around these parts.  ");
			else if (player.harpyScore() > 3) DisplayText("plus you're not screeching incessantly, like the feathery sluts who usually flap their way around these parts.  They're lucky they </i>can<i> fly, 'cause I'd give them something to really squawk about if I could get my hands on them.  ");
	...
	protected smokeEffect(smokedPipe: boolean): void {
		...
		player.statusAffects.set(new StatusAffect("IzumisPipeSmoke", SMOKE_DURATION, deltaSpd, deltaSns, deltaLib)));
	...
	protected surrenderSmallCock(): void {
		...
		if (Flags.list[FlagEnum.MINOTAUR_CUM_ADDICTION_STATE] >= 1) {
			...
			DisplayText("“<i>Hold on, I want to watch this.</i>”  She says, shifting position until her head is placed on one side of your hip, staring at your [cock biggest]");
	...
	protected surrenderOhGodTheFemaleSurrenderContentIsFuckingHugeSendHelp(): void {
		...
		if (!player.torso.hips.legs.isTaur()) {
			...
			DisplayText("As you squirm in Izumi’s grip, it begins to dawn on you that you’re not getting away from this.  Your resistance begins to lessen until eventually you begin to relax, finally admitting to yourself that this really doesn’t feel half bad...\n\n");
		}
		// Taur variant
		else {
			...
			DisplayText("As you squirm under Izumi’s body, it begins to dawn on you that you’re not getting away from this.  Your resistance begins to lessen until eventually you begin to relax, finally admitting to yourself that this really doesn’t feel half bad....\n\n");
		}
	...
	protected surrenderFemaleNonExhibitionVariant(): void {
		...
		if (player.hasMuzzle()) DisplayText(" maw");
	...
	protected surrenderFemaleLookitYouTheCenterOfAttentionYouSlut(): void {
		...
		DisplayText("“<i>Hey out there!  Turns out my friend here is just too big a slut to resist the smell of some juicy cock, and you got her all riled up.  So, with that in mind,</i>” She announces, loudly.  “<i>She’s up for public use.  Who’s first?</i>”\n\n");
	...
	protected noDickNoVagNoService(): void {
		...
		DisplayText("“<i>Well, " + this.heightDesc() + ", you really are weird... no dick </i>or<i> pussy.  I, err...</i>”  She says, the confusion caused your evident lack of equipment spreading across her face.  “<i>I gotta say, I’m fresh out of ideas over here.</i>”\n\n");
	...
	public touchThatFluffyHorn(): void {
		...
		if ((player.stats.HP / player.maxHP()) <= 0.1) DisplayText(" the brief reprieve giving both of you a moment to recover.\n\n");
		else if (monster.lust >= 100) DisplayText(" the Oni’s cheeks tinged a rosy peach in contrast to her alabaster skin.\n\n");
		...
		if (player.torso.cocks.count > 0) {
			...
			if (player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE) >= 8) MainScreen.addButton(5, "Tentacruel", tentacruelRevengeFux)).length;
		}
	}
	...
	protected tentacruelRevengeFux(): void {
		...
		if (player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE) >= 10) DisplayText("  You decide to test the theory, snaking another prehensile appendage into each of her open, outstretched palms.  Sure enough, after a few fumbling attempts, Izumi manages to grasp the concept of a handjob and awkwardly starts to jerk them off.")).length;
	...
	protected sayThatsAPrettyFaceBeAShameIfSomebodyJizzedAllOverIt(): void {
		...
		let cockIndex: number = player.torso.cocks.sort(Cock.LargestCockArea)[0];

		if (player.torso.cocks.get(cockIndex).type === CockType.HORSE) DisplayText("  Visibly unsure of where to put herself, Izumi finally settles for brushing her lips against the exact center of the flared mass that makes up the head of your stallion-prick.");
		else if (player.torso.cocks.get(cockIndex).type === CockType.DOG) DisplayText("  Her eyes darting nervously from side to side, Izumi leans forwards and takes the very tip of your dogcock into her mouth, pressing her lips against the main bulk of what passes for your head.");
