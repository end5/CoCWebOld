...
export class Lake {
	...
	public exploreLake() {
		...
		if (player.level >= 2 && player.statusAffects.has(StatusAffectType.DungeonShutDown))
			choice[choice.length] = 3;
		//Slimes/Ooze = level >= 2
		if (player.level >= 2)
			choice[choice.length] = 4;
		//Izma
		if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00230] > 0 && (player.exploredLake >= 10) && (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00233] === 0 || !player.statusAffects.has(StatusAffectType.Infested)) && Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00238] <= 0)
		...
		//UNCOMMON EVENTS
		//Goo finding!
		if (randInt(30) === 0 && Flags.list[FlagEnum.GOO_TFED_MEAN] + Flags.list[FlagEnum.GOO_TFED_NICE] > 0 && Flags.list[FlagEnum.GOO_SLAVE_RECRUITED] === 0) {
		...
		if (temp > 0 && player.torso.cocks.sort(Cock.LongestCocks)[0].length >= player.tallness - 10 && player.totalCockThickness() >= 8)
		...
		//Sword Discovery
		if (!player.statusAffects.has(StatusAffectType.TookBlessedSword) && !player.statusAffects.has(StatusAffectType.BSwordBroken))
			choice[choice.length] = 10;
		//Pre-emptive chance of finding the boat
		if (!player.statusAffects.has(StatusAffectType.BoatDiscovery))
			choice[choice.length] = 11;

		//CHOOSE YOUR POISON!
		select = choice[randInt(choice.length)];
		...
		//Chance of dick-dragging! 10% + 10% per two foot up to 30%
			...
			if (player.statusAffects.has(StatusAffectType.FactoryOverload) && randInt(10) === 0) {
				gooGirlScene.spyOnGooAndOozeSex();
				return;
			}
			//Else pretty rare.
			else if (randInt(25) === 0) {
				gooGirlScene.spyOnGooAndOozeSex();
				return;
			}
			let girlOdds: number = 50;
			//50% odds of slime-girl, 75% if shutdown factory
			if (player.statusAffects.has(StatusAffectType.DungeonShutDown) && !player.statusAffects.has(StatusAffectType.FactoryOverload))
				girlOdds += 25;
			if (player.statusAffects.has(StatusAffectType.FactoryOverload))
				girlOdds -= 25;
			//Slimegirl!
			if (randInt(100) <= girlOdds) {
			...
			//OOZE!
				...
				if (player.stats.int >= 25) {
					DisplayText("A soft shuffling sound catches your attention and you turn around, spotting an amorphous green mass sliding towards you!  Realizing it's been spotted, the ooze's mass surges upwards into a humanoid form with thick arms and wide shoulders.  The beast surges forward to attack!", true);
					startCombat(new GreenSlime());
					if (player.statusAffects.has(StatusAffectType.DungeonShutDown) && !player.statusAffects.has(StatusAffectType.FactoryOverload)) DisplayText("\n\n<b>You are amazed to encounter a slime creature with the factory shut down - most of them have disappeared.</b>");
					return;
				}
			...
			DisplayText("Your quick walk along the lakeshore feels good.", true);
			if (player.stats.spe >= 50) {

			}
			else {
				DisplayText("  You bet you could cover the same distance even faster next time.\n");
				player.stats.spe += .75;
			}
			...
			//No boat, no kaiju
			if (player.level >= 5 && Flags.list[FlagEnum.KAIJU_DISABLED] === 0 && player.statusAffects.has(StatusAffectType.BoatDiscovery)) {
				kaiju.kaijuMeeting();
				return;
			}
			DisplayText("Your stroll around the lake increasingly bores you, leaving your mind to wander.  ", true);
			if (player.stats.cor > 30 || player.stats.lust > 60 || player.stats.lib > 40) DisplayText("Your imaginings increasingly seem to turn ");
			...
			if ((player.stats.cor > 30 && player.stats.cor < 60) || (player.stats.lust > 60 && player.stats.lust < 90) || (player.stats.lib > 40 && player.stats.lib < 75)) {
				DisplayText("to thoughts of sex.");
				dynStats("lus", (5 + player.stats.lib / 10));
				displayed = true;
			}
			if (((player.stats.cor >= 60) || (player.stats.lust >= 90) || (player.stats.lib >= 75)) && !displayed) {
				DisplayText("into daydreams of raunchy perverted sex, flooding your groin with warmth.");
		...
		//Find whitney or equinum
			...
			//Find Whitney
				...
				if (player.statusAffects.has(StatusAffectType.MetWhitney)) {
					...
					if (player.statusAffects.get(StatusAffectType.MetWhitney).value1 > 1) {
			...
			if (!player.statusAffects.has(StatusAffectType.FetishOn)) {
				player.statusAffects.add(StatusAffectType.FetishOn, 0, 0, 0, 0);
			...
			DisplayText("OH SHIT! LAKE EXPLORE BE BROKED.  SELECT: " + select + ".  You should probably go to fenoxo.com and click the link to report a bug and tell Fen about it.");
		}
	}
	...
	private findLakeLoot() {
		...
		if (randInt(2) === 0) {
			...
			inventory.takeItem(consumables.EQUINUM, Scenes.camp.returnToCampUseOneHour);
		}
		else {
			DisplayText("You find an odd, fruit-bearing tree growing near the lake shore.  One of the fruits has fallen on the ground in front of you.  You pick it up.\n");
	...
	private eggChoose(eggType: number) {
		...
		switch (eggType) {
			case 0: DisplayText("brown"); break;
			case 1: DisplayText("purple"); break;
			case 2: DisplayText("blue"); break;
			case 3: DisplayText("pink"); break;
			case 4: DisplayText("white"); break;
			default: DisplayText("black"); break;
		}
		DisplayText(" light.  Immediately it flows into your skin, glowing through your arm as if it were translucent.  It rushes through your shoulder and torso, down into your pregnant womb.  The other lights vanish.");
	...
	private ottahGirl() {
		...
		if (Flags.list[FlagEnum.MET_OTTERGIRL] === 1) {
			DisplayText("Your exploration of the lakeside takes you further than you've gone before.  The water here is almost completely still, its waters ");
			if (!player.statusAffects.has(StatusAffectType.FactoryOverload)) DisplayText("crystal clear, giving you a stunning view of the lakebed");
			else DisplayText("only slightly clouded, giving you an obscured view of the lakebed");
			DisplayText(".  Fish dart to and fro within the waters, caring little for your explorations above the waves.");
		...
		//Repeats
			...
			DisplayText("Your explorations of the lake lead you back to Callu, the otter girl. She sits lazily on the beach; fishing rod in hand, as usual. She gives a friendly wave as you approach, and pats the sandy patch of lakeside next to her.");
		...
		if (player.stats.lust < 33) DisplayText("\n\nYou aren't aroused enough to fuck her.");
			...
			if (player.torso.cocks.count > 0) {
				if (player.shortestCockLength() > 48) DisplayText("\n\nUnfortunately, you don't think she can quite handle your cock.");
				else MainScreen.addButton(0, "Fuck Her", ottergirlLikesDongs);
			}
			if (player.torso.vaginas.count > 0 || player.torso.cocks.count <= 0) MainScreen.addButton(1, "Facesitting", ottersForGals);
		}
	...
	private ottergirlLikesDongs() {
		...
		while (temp < player.torso.cocks.count) {
			if (player.torso.cocks.get(temp).length < 48) {
				if (y < 0) y = temp;
				else if (player.torso.cocks.get(temp).length > player.torso.cocks.get(y).length) y = temp;
			}
			temp++;
		}
		...
		//{(lil dicks)
		if (player.torso.cocks.get(x).length < 6) DisplayText("little, wanna-be cock");
		...
		//[equine]
		if (player.torso.cocks.get(x).type === CockType.HORSE) DisplayText("to your flare, sucking, teasing and ");
		//[canine]
		else if (player.hasKnot(x)) DisplayText("to the base of your cock, planting sloppy kisses on your knot, ");
		//[demonic]
		else if (player.torso.cocks.get(x).type === CockType.DEMON) DisplayText("to the demonic nodules ringing your cock, ");
		//[anemone]
		else if (player.torso.cocks.get(x).type === CockType.ANEMONE) DisplayText("to the little wriggling tentacles ringing the head and base of your cock, ");
		else DisplayText("to the sensitive little spot on the underside of the head, ");
		DisplayText("lavishing it with attention.  Precum and saliva practically pour down the length of your shaft, tickling your ");
		if (player.torso.balls.quantity > 0 && player.torso.vaginas.count > 0) DisplayText("balls and cunt");
		...
		if (player.cumQ() >= 250) {
			DisplayText("  There's so much of it, ");
			if (player.cumQ() < 500) DisplayText("some of it begins to dribble down your cock, forming a puddle right under your ass cheeks");
			else if (player.cumQ() < 1000) DisplayText("it begins to spray out of the edges of your cock, like water coming out of a blocked faucet");
			else if (player.cumQ() < 2000) DisplayText("Callu's stomach begins to visibly inflate, even from your point of view");
			else DisplayText("Callu's stomach inflates to a huge degree. She suddenly looks to be about eight months pregnant, though she doesn't seem bothered by this in the least");
			DisplayText(".");
		}
		DisplayText("  Her womb greedily takes everything it can, until you fall back onto the ground, exhausted.");

		DisplayText("\n\nTo your surprise, Callu simply picks up where she left off");
		if (player.cumQ() >= 2000) DisplayText(", despite the huge belly she now sports");
		DisplayText(".  Gyrations, thrusts and the constant cadence of her cunt work together to keep you unbearably hard.  Apparently she's not satisfied.");
		...
		if (player.hasKnot(x)) DisplayText("fails, held fast by the knot tying the two of you together. She looks at you in surprise, but eventually smiles and leans back down. The two of you cuddle for half an hour, until your knot deflates enough to let her *pop* off of it. She stands and ");
		DisplayText("slips her bikini bottoms into a canvas bag.");
		...
		player.orgasm();
		player.stats.sens += -1;
		inventory.takeItem(consumables.FISHFIL, Scenes.camp.returnToCampUseOneHour);
	}
	...
	private ottersForGals() {
		...
		if (player.torso.vaginas.count > 0) DisplayText("folds");
		...
		if (player.torso.vaginas.count > 0) DisplayText("clit");
		else DisplayText("unnatural bare crotch");
		DisplayText(" down to your pucker, she leaves a trail of sloppy smooches.  You mirror her movements, attempting to give her the same experience she's giving you.  A low murr escapes her lips, and she squirms above you slightly as your tongue hits the right spots.");
		...
		if (player.torso.vaginas.count > 0) DisplayText("quivering quim");
		else DisplayText("rump");
		...
		if (player.torso.vaginas.count > 0) DisplayText("Your snatch clenches and clamps down on thin air, flooding Callu's face with your feminine juices in a tasty, refreshing spray.");
		else DisplayText("Your asshole clenches and spasms randomly, aching to be filled by something, anything in your quest for release.");
		DisplayText("  The orgasmic bliss makes you collapse forwards, dropping you onto all fours. However, your blonde lover grips your thighs firmly, clearly intent on fully repaying her debt.");
