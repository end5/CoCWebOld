...
export interface DriderFlags {
    UNKNOWN_FLAG_NUMBER_00277: number;
}

const driderFlags: DriderFlags = {
    UNKNOWN_FLAG_NUMBER_00277: 0
};

User.flags.set(FlagType.Drider, driderFlags);

// *Encounter Intro (done)
export function driderEncounter(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Drider);
    DisplayText().clear();
    // Load monster stats so we can display things like eye/hide color!
    const drider: CorruptedDrider = new CorruptedDrider();
    // First time
    if (driderFlags.UNKNOWN_FLAG_NUMBER_00277 === 0) {
        ...
        DisplayText("\"<i>My, what lovely prey I seem to have found tonight,</i>\" croons the drider.\n\n");

        DisplayText("Her tone makes it quite clear you'll have to run or fight your way out of this!");
    }
    // repeat intro:
    else {
        ...
        DisplayText("As far as you're aware, it's unlikely you'll be able to scare your stalker off, and so you simply wait for them to show themselves.  You search with your eyes and, noisily clearing your throat, make it clear you're aware of their presence.  Soft chuckling chimes around you, seeming to originate from several places in the scrub, but nothing appears.  You turn about and call again, raising your " + player.inventory.equipment.weapon.displayname + ".  A gentle tap on the shoulder answers you, but when you look the vista is equally as desolate as the first time.  And where the hell is that laughing coming from?\n\n");

        DisplayText("Spinning again, you lose your patience and loudly demand your ambusher present itself.  A soft tug on your ");
        if (player.torso.tails.count > 0) DisplayText("tail ");
        else DisplayText("hair ");
        DisplayText("pulls you back halfway, but as you open your mouth to yell, the ground gives a quiet thump.  Just barely in your periphery, you see a white apparition with a yawning mouth stretching out to bite, and your reflexes take over, dropping your weight and rolling you away from it.\n\n");
        ...
        DisplayText("You've yet to meet a wild drider that let you walk away without some sadistic display of power, and this one looks to be no different.");
    }
    driderFlags.UNKNOWN_FLAG_NUMBER_00277++;
    return CombatManager.beginBattle(player, [], [drider]);
}

// *Victory Intro: (done)
export function defeatDriderIntro(player: Character, monster: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Drider);
    DisplayText().clear();
    DisplayText("All eight of the drider's black and " + monster.torso.neck.head.hair.color + " striped legs collapse under her weight while she struggles ");
    if (monster.stats.HP < 1) DisplayText("to try to continue the fight, her movements slowed by pain and weakness.");
    ...
    if (player.torso.cocks.find(Cock.CockThatFits(monster.analCapacity()))) buttFuckBUTTFUCKBUTTTFUCKBUTTFUCK = buttFuckADriderOhBaby;
    let vagFuck;
    let fuckSpinner;
    let careful;
    if (player.torso.cocks.count > 0 && player.stats.lust >= 33) {
        if (player.torso.cocks.find(Cock.CockThatFits(monster.vaginalCapacity()))) {
            ...
            if (player.torso.tails.find(Tail.FilterType(TailType.SPIDER_ABDOMEN)) || player.inventory.items.has(MaterialName.ToughSpiderSilk)) {
        ...
        if (player.torso.cocks.find(Cock.CockThatFits(12))) {
    ...
    let rideOvi;
    let bikiniTits;
    if (player.torso.vaginas.count > 0 && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 4 && player.inventory.equipment.armor.name === ArmorName.LustyMaidensArmor) bikiniTits = partial(lustyMaidenPaizuri, player, monster);
...
// Lose to drider
export function loseToDrider(player: Character, monster: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Drider);
    // Build array of choices
    let select = 0;
    if (player.torso.cocks.count > 0) select++;
    if (player.torso.vaginas.count > 0) select++;
    // Genderless scene has low chance for gendered folks
    if (player.gender === Gender.NONE || (player.gender > 0 && randInt(3) === 0)) select++;

    // SELECT SCENE HERE
    const choice: number = randInt(select);
    // *Loss Ride Ovi Anal unsexed - gats (done)
    if (choice === 2) return loseVSDriderGetAnalledOvi(player, monster);
    // *Loss Vaginal Impregnation and forced BJ - Symphonie (done)
    else if (choice === 1) return loseVSDriderAndVaginallyImpreggleOvi(player, monster);
    // *Loss Ride & Impregnate Butt (done) MALE
    else return loseVSDriderAndGetFuckedANDAnalImpreggegity(player, monster);
}

// *Victory Rape: PC Penis goes in Vagina (done)
function winDriderPCDickInSpiderCunt(player: Character, monster: Character): NextScreenChoices {
    ...
    DisplayText("Eager to plumb the depths of this greedy spider's snatch, you practically tear your way out of your " + player.inventory.equipment.armor.displayName + " in your hurry to get naked.  Through it all ");
    if (monster.stats.lust > 99) {
        DisplayText("she looks ");
        if (player.tallness > 72) DisplayText("up ");
        DisplayText("at you with her " + monster.skin.tone + " eyes opened wide and her tongue hanging from her mouth, panting lewdly.  Carapace-clad digits sink into her dripping honey-pot over and over while she watches, deep in her own lusty haze.  She retracts her engorged demon-cock so as to have better access to that feminine hole.");
    }
    else {
        DisplayText("she looks ");
        if (player.tallness > 72) DisplayText("up ");
        DisplayText("at you with a look of disgust in her " + monster.skin.tone + " eyes.  She folds her arms across her many-bosomed, quadratically endowed chest with disdain");
        if (monster.inventory.equipment.piercings.nipples.count > 0) DisplayText(", the chains jingling loudly as she does so");
        DisplayText(".  In spite of keeping a stiff upper lip about it, her lower ones visibly quiver while you strip.");
    }
    DisplayText("  You saunter over to the limp-legged drider and run your hands down her inhuman, arachnid body, exploring the smooth, shiny carapace.\n\n");
    ...
    if (monster.stats.HP < 1) DisplayText("giving up little, lewd moans");
    else DisplayText("breathing out lurid moans");
    DisplayText(" of pleasure and her ");
    if (monster.inventory.equipment.piercings.nipples.count > 0) DisplayText("chain-pierced-");
    DisplayText("nipples and clit look as hard as her natural body-armor.  You boldly grope one of her ");
    if (monster.inventory.equipment.piercings.nipples.count > 0) DisplayText("pierced ");
    ...
    DisplayText("Still fondling one of the four nicely-sized tits, you push the spider-bitch back, admiring the drider's flexibility as she leans back over her inhuman lower body, giving you an even better view of her intoxicatingly sweet-smelling pussy.  You take " + Desc.Cock.describeMultiCockSimpleEach(player) + " in hand and rub the " + Desc.Cock.describeCockHead(cockThatFits) + " along the slick lips of her pussy, shuddering slightly as her wetness rubs the sensitive glans.  Gasping, you slowly spread her molten-hot pussy around your girth, reveling in the tight embrace of her slick walls.  The drider squirms in your grip with her breasts bouncing deliciously and whole body trembling.  Spreading as easily as her cunt, the drider's venom-glossed lips slowly open into a wordless 'o' of rapturous pleasure.\n\n");

    DisplayText("Half an inch at a time, you sink ever-deeper inside the seemingly bottomless pit that is the spider-girl's steamy sex.  Reveling in the taming of the aggressive predator's sticky snatch, you postpone the eventual hilting as long as possible.  For her part, she doesn't seem to mind you having your way with her.  All eight of her spindly legs are scrabbling pitifully in the dirt, doing little more than nervelessly convulsing in uncontrolled pleasure.  The spider-woman's arms close behind your back, the sharp tips of her nails trying to claw deep furrows into your " + Desc.Skin.skinFurScales(player) + ".  At long last, your waist presses against her own to feel the oozing slipperiness of her nethers.  Your " + Desc.Cock.describeCock(player, cockThatFits) + " is totally buried in her tunnel, and now that you're holding still, you can feel the tiny folds inside her vagina slowly moving and gliding along your length, stroking your " + Desc.Cock.describeCock(player, cockThatFits) + " like dozens of tongues.\n\n");

    DisplayText("Compelled by your body's instincts, you draw back your " + Desc.Hip.describeHips(player) + " to expose the bulk of your pussy-soaked shaft.  The spider-woman moans disappointedly, her voice filled with so much wanton desire that your " + Desc.Cock.describeCockHead(cockThatFits) + " twitches in response.  You fondle the drider's lower pair of breasts with both hands, circling and squeezing the delectable mounds while you begin to repeatedly impale her squelching pussy, watching the upper row of pale breasts bounce and jiggle above your busy fingers.");
    if (monster.inventory.equipment.piercings.nipples.count > 0) DisplayText("  The chains binding her nipples make a delightful clinking noise with each squeeze and stroke.");
    DisplayText("\n\n");

    DisplayText("Meanwhile, the drider's swollen clit prods at your loins with each penetrating thrust, leaving lines of sticky spider-juice all over your crotch.  ");
    ...
    else DisplayText("Your " + Desc.Cock.describeCock(player, cockThatFits) + " seems to bulge inside her, giving itself over to the silken caresses of her glorious vagina.");
    ...
    DisplayText("While you're distracted by the repeated contrast of tension and relief emanating from " + Desc.Cock.describeMultiCockSimpleOne(player) + ", the drider leans up and kisses your neck, humming against your throat as you squirt the last of your orgasmic goop into her vagina.  You slump into her arms, spent by the effort of fucking her so fast and cumming so hard.  As you lie there, her lips sucking hard enough to leave hickeys on your neck with every crushing, passionate kiss, you feel the patterns of glossy fluid tingling on your " + Desc.Skin.skin(player) + ".  A jolt of fear runs up your spine, and you try to push away from her envenomed affections.  It doesn't work!  You can barely lift your arms, let alone pull out of the amorous arachnid's embrace!\n\n");
    ...
    else DisplayText("No, that's not right.  You're not some rutting beast, you're the champion.  Sure, you've got your " + Desc.Cock.describeCock(player, cockThatFits) + " locked hilt-deep inside her squirming slit, but that doesn't mean you're some kind of " + Desc.Gender.mf(player, "stud", "slut") + " who jumps on a cunt without a second thought, does it?  You blink your eyes while you try to think.  She does have a point though, if you hadn't been in such a hurry for some sex, you wouldn't be sagging into her tits and feeling so awfully relaxed.\n\n");
    ...
    if (player.stats.cor > 80 && player.stats.lib > 50) DisplayText("You grumble out a denial.  You're not dumb, you just like to take charge and fuck. There's no reason to resist jumping on a nice wet pussy and making it squirt while you pack it full... but you can't quite get your limbs to move.\n\n");

    else DisplayText("You mumble out a denial.  You're not a beast... you just like to take charge and fuck.  It's just so hard to resist jumping on a nice wet pussy and making it squirt while you pack it full... no!  That's not what...\n\n");
    ...
    DisplayText("A plaintive whining, almost exactly like that of an animal, vocalizes subconsciously.  Your " + Desc.Cock.describeCock(player, cockThatFits) + " feels afire, constantly leaking pre-cum while you're held immobile in your spidery mistress' arms; you lick your lips and start to beg, mumbling out your desire to cum for her, anything for her, so long as she lets your dick orgasm again and again.  You want to climax so bad, but you want her to control your cock so much more.  It's always getting you in trouble, and it feels so good to let her use you as her cum-drooling mount.\n\n");
    ...
    if (player.stats.cor > 80 && player.stats.lib > 50) DisplayText("Gods it was a bad idea not to bind her lips... ");
    ...
    DisplayText("You mewl out an affirmation, forgetting how to speak with all the venom pumping through you and the mind-crushing levels of arousal pouring out of your " + Desc.Cock.describeCock(player, cockThatFits) + ".\n\n");
    ...
    DisplayText("The first spurt of pent-up seed splatters the spider's womb before you even realize she gave you permission, painting the squirming walls white.  You growl and cum, releasing spurt after spurt of spunk while her pussy seems to drink it all down");
    if (player.cumQ() > 1000) DisplayText(", even though you've packed it full once already");
    DisplayText(".  Her legs skitter and tremble, her pussy convulses, and your dick gets the squeezing, sucking orgasm of a lifetime.  It feels like you're pumping out two or three orgasms worth of spooge, each dick-stretching explosion of cream dilating your slit almost painfully wide.");
    if (player.cumQ() > 1000) {
        DisplayText("  The stuff dribbles out around your tool along with a mix of girl-lube, too much even for her prodigious pussy to contain.");
        if (player.cumQ() > 2000) DisplayText("  The flow thickens into a river, and soon the puddle of cum looks more like a pond, turning the dirt into a soup of muddy cum.");
    }
    DisplayText("\n\n");

    DisplayText("While it seems to go on and on, your orgasm eventually ends, leaving you to hang limp while your still-hard dick is massaged by a much wetter spider-cunt.  The " + monster.torso.neck.head.hair.color + "-haired beauty dreamily says, \"<i>Mmmm... good job, pet.  Now, let that nice hard dick do the thinking for you and keep fucking my hot cunt until it goes dry.  Maybe a few hours after you finish, you'll be able to remember that you used to be a " + Desc.Gender.mf(player, "man", "woman") + " before you became my livestock.</i>\"\n\n");

    DisplayText("The blissful arachnid rolls onto her side with you still locked inside her and falls asleep, her " + monster.torso.neck.head.hair.color + " tresses ");
    if (player.cumQ() <= 1000) DisplayText("splayed across the ground");
    else DisplayText("soaking in the white puddle of spooge");
    DisplayText(".  Even unconscious, her vagina continues its rhythmic contractions around your cock, working you towards another burst of orgasmic pleasure while you wait for your strength to come back...\n\n");
    // [NEXT]
    player.orgasm();
    player.stats.lib += 1;
    player.stats.sens += 1;
    return { next: partial(winDriderPCDickInSpiderCunt2, player, monster) };
}

// Part 2 of the above
function winDriderPCDickInSpiderCunt2(player: Character, monster: Character): NextScreenChoices {
    const cockThatFits = player.torso.cocks.find(Cock.CockThatFits(monster.vaginalCapacity()));
    DisplaySprite(SpriteName.Drider);
    DisplayText().clear();
    ...
    DisplayText("A low growl rips through your throat as you pull back and plaster her face white, coating the caked-on residue of a few previous loads with fresh moisture.  You shift position and force your " + Desc.Cock.describeCock(player, cockThatFits) + " between her two pairs of breasts, sandwiching it in there for the titty-fuck of a lifetime.  Pistoning back and forth, humping the glorious orbs relentlessly, you give yourself over completely to your sore dick, letting your world dissolve into the feelings of each of the spider-woman's orifices around your member.\n\n");
...
function winVSDriderTakeItsCockInCunt(player: Character, monster: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Drider);
    DisplayText().clear();
    ...
    DisplayText("A moan escapes your lips as her nine-inch pleasure-rod ");
    if (player.vaginalCapacity() < monster.torso.cocks.get(0).area) DisplayText("bottoms out");
    ...
    if (monster.inventory.equipment.piercings.nipples.count > 0) DisplayText("pierced nipples, circling the metallic stud");
    else DisplayText("nipples, circling the moist bud");
    DisplayText(" with your tongue");
    if (player.torso.neck.head.face.tongue.type > TongueType.HUMAN) DisplayText(" as the rest of its demonic length lashes out and wraps around her pillowy bosom");
    ...
    // GET KNOCKED UP
    // Pregnancy Goes Here
    player.pregnancy.womb.knockUp(new Pregnancy(PregnancyType.DRIDER_EGGS, IncubationTime.DRIDER), 151);
    player.slimeFeed();
    player.orgasm();
    player.stats.cor += .5;
    return { next: Scenes.camp.returnToCampUseOneHour };
...
function victoryVSDriderRideOviVaginal(player: Character, monster: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Drider);
    DisplayText().clear();
    ...
    player.slimeFeed();
    DisplayText("Eyeing the small lump of the spider's retracted ovipositor, you resolve to have a little fun with it, even if it means giving the spider exactly what she wants in a way that she doesn't.  You tear off your " + player.inventory.equipment.armor.displayName + " while the drider watches with ");
    if (monster.stats.HP < 1) DisplayText("hate");
    ...
    DisplayText("Like this, you're treated to a perfect view of ALL of the web-woman's endowments.  Her four D-cup tits flatten slightly against her torso now that she's on her back, hanging slightly to the side.  At her crotch, you can see her glistening, black-lipped sex puffing up, the tip of her large button already peeking out of her tempting box.  Just below that, the black and " + monster.torso.neck.head.hair.color + "-striped exoskeleton of her spider-half starts, merging down into an bulbous, arachnid shape.  At the very bottom of it all is the huge bulk of her abdomen, covered with numerous spinnerets and with a wide, mostly-retracted organ near the back.\n\n");
    ...
    if (player.torso.cocks.count > 0) {
        ...
        else DisplayText("unending");
        DisplayText(" jizz-ropes on your groin and the base of the drider's slimy stalk.");
    }
    ...
    DisplayText("You watch her for a time, until her orgasms finally die down.  By that point, you've recovered enough to get dressed.  You walk away a little bow-legged, with a new, heavy weight in your belly, just waiting to be born.");
    // GET KNOCKED UP
    // Pregnancy Goes Here
    player.pregnancy.womb.knockUp(new Pregnancy(PregnancyType.DRIDER_EGGS, IncubationTime.DRIDER), 151);
    player.slimeFeed();
...
function victoryVSDriderRideOviAnal(player: Character, monster: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Drider);
    ...
    // ([not mans]
    if (player.torso.cocks.count <= 0) DisplayText("\"<i>I never would've thought you were an anal-pregnancy slut,</i>\" hisses the spider in pleasure, flexing her organ harder while it curls through your guts, so deep inside you that you wonder how much further she can go.");
    else DisplayText("\"<i>Just because I have an exoskeleton doesn't mean I can't feel you dripping on me.  I thought I was the corrupt bitch here, but you... you take the cake.</i>\"");
    DisplayText("\n\n");

    DisplayText("With a final hard shove, the last few segments pop inside you");
    if (player.torso.cocks.count > 0) DisplayText(", nearly crushing your prostate from the tight squeeze");
    DisplayText(".  ");
    ...
    if (player.torso.cocks.count > 0) DisplayText("while your " + Desc.Cock.describeMultiCockShort(player) + " dribbles white, wasted goo onto the black and " + monster.torso.neck.head.hair.color + " striped carapace below");
    else if (player.torso.vaginas.count > 0) DisplayText("while your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " dribbles fem-slime onto the black and " + monster.torso.neck.head.hair.color + " striped carapace below");
    else DisplayText("while your fingers tweak your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s and your starved sphincter sucks at its slimy, squirming stuffing");
    DisplayText(".  Without warning, the ovipositor dilates, pulling your " + Desc.Butt.describeButthole(player.torso.butt) + " even wider.  An explosion of warmth goes off in your guts, and you realize that your term as an incubator has begun with a sense of perverse delight.\n\n");
    ...
    DisplayText(", and travels deep inside you, bulging your belly the slightest bit once it's deposited.  With a groan of deep satisfaction, you start humping the fluid-slicked chitin beneath you, barely registering the long, seemingly unending series of eggs that infiltrate your ass");
    if (player.torso.cocks.count > 0) DisplayText(" and milk your prostate as they push past");
    DisplayText(".\n\n");

    DisplayText("The stream of slime only spreads as the stimulation of the sliding spheres slewing into your sphincter sets off");
    if (player.gender > 0) {
        DisplayText(", simultaneously, ");
        if (player.torso.cocks.count > 0) {
            DisplayText("a storm of semen-squirting spurts from your shaft");
            if (player.torso.cocks.count > 1) DisplayText("s");
        }
        else if (player.torso.vaginas.count > 0) DisplayText("a siege of spray-scattering spasms from your snatch");
        DisplayText(" and");
    }
    ...
    // GET KNOCKED UP
    player.pregnancy.buttWomb.knockUp(new Pregnancy(PregnancyType.DRIDER_EGGS, IncubationTime.DRIDER - 200), 10); // Butt pregnancy goes faster
    player.slimeFeed();
    player.orgasm();
    player.stats.cor += .5;
    return { next: Scenes.camp.returnToCampUseOneHour };
...
function victoryVSDriderStickDickInSpinneret(player: Character, monster: Character): NextScreenChoices {
    ...
    else DisplayText("  Her throat bulges from the sheer amount of cum you deposit into her mouth, and she nearly chokes as she attempts to swallow all of it.");
    if (player.torso.cocks.count > 1) {
        DisplayText("  Your other ");
        if (player.torso.cocks.count > 2) DisplayText("cocks give ");
        else DisplayText("cock gives ");
    ...
    DisplayText("\n\n");
    DisplayText("Your lust sated, you disengage from her with a little pop, leaving her to lick her lips happily and groan unintelligible babble.  You wipe your " + Desc.Cock.describeCock(player, smallestCock) + " off on her hair to clean it off a bit, then redress and head out.  With a glance at the snoozing drider over your shoulder, you head out of the smelly swamp and start back towards camp.");
    player.orgasm();
    player.stats.cor += .5;
    return { next: Scenes.camp.returnToCampUseOneHour };
}
...
// *Loss Ride Ovi Anal unsexed - gats (done)
function loseVSDriderGetAnalledOvi(player: Character, monster: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Drider);
    DisplayText().clear();
    ...
    if (player.stats.HP < 1) DisplayText("relax in order to reduce the discomfort");
    else DisplayText("calm your racing thoughts and resist showing how eager you are");
    ...
    DisplayText("\"<i>Well, aah, enough of my fun,</i>\" the drider pants, groping her pillowy breasts and stroking herself to maintain her stiff erection.  \"<i>Time to get down to business.</i>\"\n\n");
    ...
    else DisplayText("chest, trickling between your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s");
...
// *Loss Vaginal Impregnation and forced BJ - Symphonie (done)
function loseVSDriderAndVaginallyImpreggleOvi(player: Character, monster: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Drider);
    DisplayText().clear();
    ...
    if (!player.torso.hips.legs.isTaur()) DisplayText("wrists");
    else DisplayText("forelegs");
    DisplayText(" and leaving you pinned and spread-eagled on the ground.  \"<i>That should stem that.</i>\"\n\n");
    ...
    if (player.torso.cocks.count > 0 && monster.inventory.equipment.piercings.nipples.count > 0) DisplayText("  Chains brush over " + Desc.Cock.describeMultiCockSimpleOne(player) + " as her chest passes your crotch.");
    DisplayText("  Her fangs trail teasingly along your belly, her fingers crawling up your sides.");
    if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 3) DisplayText("  Venom dribbles down the cleavage of your " + Desc.Breast.describeBreastSize(player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + " as she approaches your vulnerable face.  Her own quartet of matronly tits comes to rest on them as her torso appears over their horizon.");
    DisplayText("\n\n");
    ...
    if (player.torso.cocks.count > 0) DisplayText("  Worse still, its blood-swollen protuberance is met with no reaction from the drider.");
    ...
    else DisplayText("strain your neck suddenly with the dead weight of it");
    DisplayText(", and you nearly spit up, but the drider's abuse remains welcome.\n\n");
    ...
    DisplayText("She steps over you, satisfied and still groaning slightly.  Not fully retracted, her ovipositor drags over your belly and chest as she departs, smearing your pussy juices over you.\n\n");

    DisplayText("You can tell that the bindings at your limbs are already slackening, but you're so weak, so exhausted - simply being able to breathe seems like a delicacy that had not been afforded you until just this moment.  You can't leave, just yet.  You just need... to rest... a little...");
...
function loseVSDriderAndGetFuckedANDAnalImpreggegity(player: Character, monster: Character): NextScreenChoices {
    const cockThatFits = player.torso.cocks.find(Cock.CockThatFits(monster.vaginalCapacity()));
    DisplaySprite(SpriteName.Drider);
    ...
    if (player.stats.lust > 99) DisplayText("Unable to resist the desire coursing through your body, you strip off your " + player.inventory.equipment.armor.displayName + " and start stroking " + Desc.Cock.describeMultiCockSimpleOne(player) + ", displaying it in the hope that it will draw the drider's attention, while at the same time just trying to get yourself off and attain release.");
        ...
        DisplayText("member");
        if (player.torso.cocks.count > 1) DisplayText("s");
        DisplayText(" to the air.  She squeezes " + Desc.Cock.describeMultiCockSimpleEach(player) + " in her hand and pumps it, smiling while you stiffen and twitch precipitously in her grasp.");
    }
    DisplayText("  With a strength beyond what her lithe frame would imply, the spider-woman's shiny, exoskeletal arms slip under your shoulders and lift you up, hugging you into both pairs of breasts");
    if (monster.inventory.equipment.piercings.nipples.count > 0) DisplayText(", the crossed chains digging into your " + Desc.Breast.describeChest(player));
    ...
    DisplayText("Lines of webbing burst from the arachnid abdomen, snapping into tree limbs before pulling taut.  The drider's eight legs clitter and clack, four of them folding around your back while your world rotates 90 degrees.  The two of you are pulled into the trees by the sticky strands, hanging sideways, almost upside-down, in the leaves.  The four-eyed monster-girl blinks asynchronously and smiles, shifting you slightly to bring " + Desc.Cock.describeMultiCockSimpleEach(player) + " against her moist lips.  You exhale, an intense, animalistic thrill shivering through you from the feeling of having your " + Desc.Cock.describeCock(player, cockThatFits) + " lodged in a woman's entrance.  She leans down and bites into your neck, releasing a surge of toxin into your bloodstream.  It boils like fire, making you twitch and squirm in her many-limbed clutches.  The pain fades quickly, but in its place it leaves behind a molten, liquid lust, filling " + Desc.Cock.describeMultiCockSimpleOne(player) + " to its absolute maximum size, as hard as it can possibly be.\n\n");

    DisplayText("The " + monster.torso.neck.head.hair.color + "-haired dominatrix pulls back, a single drop of blood clinging to one of her fangs while she hisses, \"<i>Mmm, fresh meat is always so tasty... not to mention... firm...</i>\"\n\n");

    DisplayText("You look at her with fear, but your " + Desc.Hip.describeHips(player) + " start to twitch against her, rubbing your " + Desc.Cock.describeCockHead(cockThatFits) + " up and down the slippery slit.  A slow exhale of pleasure escapes the spider's purple lips while her legs squeeze tighter against you, trying to push your " + Desc.Cock.describeCock(player, cockThatFits) + " home.  The drider gasps when at last, you slip between her juicy outer labia, slurping deep into her cunny with a lurid squelch.  Finally held within her depths, your muscles all slacken, save for those you need to pump your hips against her, futilely trying to fuck her hot quim even though you have zero leverage.\n\n");
    ...
    DisplayText("  You shudder from the anal violation and twist against the legs holding you in place, but with your " + Desc.Cock.describeCock(player, cockThatFits) + " locked inside her, it's a useless endeavor.  It isn't shaped like a dick, and besides, that's currently dripping on your belly.  Oh gods... she mentioned eggs.\n\n");

    DisplayText("The strange organ keeps forcing its way deeper, slipping the steadily-widening, rib-like segments of its bulk into your " + Desc.Butt.describeButthole(player.torso.butt) + ".  ");
...
export function birthSpiderEggsFromAnusITSBLEEDINGYAYYYYY(player: Character) {
    DisplaySprite(SpriteName.Drider);
    ...
    if (player.torso.vaginas.count > 0) {
        ...
        DisplayText(" into the dirt, getting off along with your " + Desc.Butt.describeButthole(player.torso.butt) + ".  ");
    }
...
export function driderPregVagBirth(player: Character) {
    DisplaySprite(SpriteName.Drider);
    ...
    if (player.torso.hips.legs.isNaga()) DisplayText("At last, when no more eggs seem to be coming, you flop over onto your back, careful not to crush any of your dormant offspring.");
    else DisplayText("Looking between your legs you can see another slick, green egg slide from your body to join the sizable pile behind you.  At last, when no more seem to be following it, you flop over onto your back, careful not to crush any of your dormant offspring.");
    DisplayText("  As you lie panting, your revery is interrupted by a small cracking noise.  You sit up with a start and scan the area for someone approaching.  The cracking noise begins to multiply and you realize that it's coming from the pile of eggs nearby.  You begin watching with interest as your eggs start to hatch.\n\n");
    ...
    if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2) {
        DisplayText("One drider girl cautiously makes her way over to you.  Her legs tickle your stomach as she climbs up to one of your " + Desc.Breast.describeBreastRow(player.torso.chest.get(0)) + ".  Grasping the flesh tightly in her tiny hands she begins to suckle at your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + ".");
    ...
    DisplayText("Around you, your many drider children fervently explore their surroundings.  One climbs up your back and perches atop your head.  She raises one hand to her forehead to block the sun's glare and stretches all four of her jewel-like eyes wide, delighting in the wondrous world she surveys.");
    if (player.torso.neck.head.face.eyes.type !== EyeType.FOUR_SPIDER_EYES) DisplayText("  Suddenly she flops forward, leaning against your eyebrows and causing you to cross your eyes in an attempt to focus on her.  She seems confused by the way your eyes are different to hers.");
    if (player.torso.hips.legs.isNaga()) DisplayText("  A few baby driders crawl ponderously across your scaled tail.  You give your tail a little wiggle and suddenly they go wild crawling around it and letting out the faintest, cute little squeals.");
    else DisplayText("  A few baby driders sit on each of your legs, facing each other.  Some gesture and squeak at each other in wonder while others pull faces.");
    DisplayText("\n\n");
    ...
    DisplayText(".  After remembering where you are, the next thing you notice is the absence of your drider children.  Only a pile of empty, broken egg shells and some messy webbing in the trees remain.  As you stand up and prepare to move on, the sun reflecting off something in the corner of your eye catches your attention.\n\n");

    DisplayText("Between two nearby trees is a large spider web.  While not out of the ordinary, when the sunlight hits it at a certain angle the gossamer strands spell out a simple message:  \"<i>THANK YOU MOMMY.</i>\"");
    player.orgasm();
}

// Win Drider Male Anal (OH BABY)
function buttFuckADriderOhBaby(player: Character, monster: Character): NextScreenChoices {
    ...
    DisplayText("Now that she's calmed down, you give her a hard shove, putting her on her drider-belly so you're not nearly thrown off her every other moment.  With that done, you put your hands on her human back, pushing her forward and exposing her human-like bubble-butt, seated just above where woman meets spider. Greedily, you pull her cheeks apart, revealing the tiny star of her puckered asshole.  You pull your " + Desc.Cock.describeCock(player, cockThatFits) + " out of your " + player.inventory.equipment.armor.displayName + ", already hardening from the thought of you pounding her tight little backdoor.\n\n");

    DisplayText("\"<i>Oh, you want that hole, do you? How very kinky,</i>\" the drider chides, but you aren't fooled by her tone – you can see her anus relaxing, preparing itself for your arrival.  With a grin, you grab your " + Desc.Cock.describeCock(player, cockThatFits) + " and push it against her tight hole, leaning into the penetration until you feel it slip past her sphincter and into her hot, snug passage.  As you slide more and more of your cockmeat into her, you reach up from her cheeks and grope her breasts, squeezing her pert nipples between your fingers as you finally bottom out.  Since you can go no further up her butt, you start to piston your hips forward, slapping your hips to her ass with short, forceful strokes.\n\n");
    ...
    DisplayText("Without warning, the drider cums.  She cries out and grabs her fully-engaged corrupted cock, hosing down the trees in front of her with spider-cum as her butt squeezes down on your " + Desc.Cock.describeCock(player, cockThatFits) + ", milking your dick with her anal passage.  You pick up the tempo of your fucking to break-pelvis speeds, ramming your cock into her backdoor and letting loose a torrent of your spunk, shooting it deep inside her");
    // (if Vagina:
    if (player.torso.vaginas.count > 0) {
        DisplayText(" while your unused cunt spurts girlcum across her carapace");
        if (player.torso.cocks.count > 1) DisplayText(" and");
    }
    else if (player.torso.cocks.count > 1) DisplayText(" as");
    if (player.torso.cocks.count > 1) {
        DisplayText(" your spare cock");
        if (player.torso.cocks.count === 2) DisplayText(" coats");
        else DisplayText("s coat");
        DisplayText(" her human back with a thick white layer of spooge");
    }
    DisplayText(".\n\n");
...
function driderVagSmartFuck(player: Character, monster: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Drider);
    DisplayText().clear();
    ...
    const cocksThatFit = player.torso.cocks.reduce(Cock.CocksThatFitOrSort(monster.vaginalCapacity(), 2, Cock.SmallestCockArea), []);

    DisplayText("You keep your distance from the downed spidermorph, still wary of her dripping, toxin-coated lips despite her weakened state and your own rising desire to breed her.  Trailing your eyes over her sweaty, splayed body you feel a familiar rush below the belt, your hardening " + Desc.Cock.describeCock(player, cocksThatFit[1]) + " pressing against the inside of your [armor] as her breasts rise and fall seductively with each panting breath.  Desire threatens to overpower your caution as your gaze lingers on the vulnerable woman before you, her netherlips ");
    if (monster.stats.HP < 1) DisplayText("parted slightly and glistening with beads of sweat from the adrenaline of battle.");
    else DisplayText("stuffed with her fingers, femcum streaming down the front of her spider-half as she desperately plays with herself.");
    ...
    if (player.torso.tails.find(Tail.FilterType(TailType.SPIDER_ABDOMEN))) {
        DisplayText("\n\nA plan quickly forms in your mind and you feel your spinnerets twitch with anticipation as you ");
        if (player.torso.hips.legs.isDrider()) DisplayText("scuttle");
        else DisplayText("saunter");
        DisplayText(" over to the collapsed drider.");
        return driderTiedUpPartTwo(player, monster, false);
    }
    // (Champion has some Tough Spider Silk on them)
    else if (player.inventory.items.has(MaterialName.ToughSpiderSilk)) {
        ...
        // [Yes] [No]
        return { yes: partial(driderTiedUpPartTwo, player, monster, true), no: partial(dontSilkRapeDrider, player, monster) };
    }
}

// [No]
function dontSilkRapeDrider(player: Character, monster: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Drider);
    DisplayText().clear();
    ...
    // (Return to "win" menu)
    return { next: partial(defeatDriderIntro, player, monster) };
}

function driderTiedUpPartTwo(player: Character, monster: Character, useItem: boolean): NextScreenChoices {
    DisplaySprite(SpriteName.Drider);
    const cocksThatFit = player.torso.cocks.reduce(Cock.CocksThatFitOrSort(monster.vaginalCapacity(), 2, Cock.SmallestCockArea), []);
    if (useItem) {
        DisplayText().clear();
        player.inventory.items.consumeItem(MaterialName.ToughSpiderSilk);
        DisplayText("Smirking to yourself, you advance on the collapsed girl, " + Desc.Cock.describeCock(player, cocksThatFit[1]) + " straining against your clothing in anticipation of what's to come.");
    }
    DisplayText("\n\nDarting forwards suddenly, you seize her wrists, pulling her arms above her head");
    if (monster.stats.lust > 99) DisplayText(" and away from her quivering snatch");
    ...
    DisplayText("\n\nThe thought is definitely enticing, causing your " + Desc.Cock.describeCock(player, cocksThatFit[1]) + " to twitch as you thrust it against the girl's back, smearing pre-cum on her already-slick skin.  However, you've already got a plan in mind and you feel that it's about time that you finish teasing the poor thing and finally put it into action.  Taking one last luxury you slide your free hand around her hip, suckling her neck as your searching fingers spread across her lower stomach.");
    ...
    if (!useItem) DisplayText("Shifting yourself, you withdraw your hand from her quivering slit, reaching back to collect another handful of stretchy silk.");
    else DisplayText("Reaching back into your pack, you grab the remainder of the bundle of silk that you tied the girl's hands with.  Looking over the mass of white-grey strands, you smile as you see that there seems to be more than enough left for what you have in mind.");

    // New Page
    return { next: partial(driderTiedUpPartThree, player, monster) };
}

function driderTiedUpPartThree(player: Character, monster: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Drider);
    const cocksThatFit = player.torso.cocks.reduce(Cock.CocksThatFitOrSort(monster.vaginalCapacity(), 2, Cock.SmallestCockArea), []);
    DisplayText().clear();
    DisplayText("The drider seems to be completely oblivious, apparently too focused on ignoring your hand still roaming over her chest to realize your intentions.  You start to prepare the ball of silk with one hand, keeping the other on her chest to distract her from your actions.  You notice that the silk is starting to glisten slightly as the juices on your hand begin to stain the material and the sight gives you a devious idea.  Wrapping the strands around your hand, you dip it down in between your bodies, trying to absorb as much pre-cum as possible.  You run it over your slick stomach, as well as the small of the spider-taur's back, making her squeak slightly at the strange, wet sensation.  As you brush the dampening sheet over your still-leaking length, you have to choke back a moan as the soft, slick, sticky silk sticks slightly to your " + Desc.Cock.describeCock(player, cocksThatFit[1]) + ".");
    ...
    DisplayText("\n\nDriven on by the sight of the thoroughly defeated girl, you move your hands to grip her sides and start pistoning your bodies together.  She can only moan pathetically while you use her, the way your length presses against her sensitive insides all too much for her exhausted frame.  Hammering into her, you dart a thumb across to sweep over her throbbing clit, bringing about a particularly loud cry of pleasure that makes your " + Desc.Cock.describeCock(player, cocksThatFit[1]) + " jump delightfully even as she clenches especially tightly around you.  You feel yourself getting close, the mixture of such a tight, wet cunt and your unbelieveable arousal leaving you smacking into her pelvis with a single goal in mind: filling her with your potent seed.");
    ...
    player.orgasm();
    player.stats.sens += -1;
    return { next: partial(driderTiedUpPartFour, player, monster) };
}

function driderTiedUpPartFour(player: Character, monster: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Drider);
    const cocksThatFit = player.torso.cocks.reduce(Cock.CocksThatFitOrSort(monster.vaginalCapacity(), 2, Cock.SmallestCockArea), []);
    DisplayText().clear();
