...
export class CorruptedDriderScene {
	...
	public defeatDriderIntro() {
		...
		if (player.torso.cocks.count > 0 && player.stats.lust >= 33) {
			if (player.cockThatFits(monster.vaginalCapacity()) >= 0) {
				...
				if (player.torso.tailType === TailType.SPIDER_ABDOMEN || player.inventory.items.has(useables.T_SSILK)) {
		...
		if (player.stats.lust >= 33) {
			...
			choices("Butt Fuck", buttFuckBUTTFUCKBUTTTFUCKBUTTFUCK, "Fuck Pussy", vagFuck, "Bondage Fuck", careful, "FuckSpinner", fuckSpinner, "Ride Cock", rideCock,
				"Ride Ovi", rideOvi, "RideOviAnal", rideOviAss, "", null, "B.Titfuck", bikiniTits, "Leave", Scenes.camp.returnToCampUseOneHour);
		}
	...
	public loseToDrider() {
		...
		let select: Array = [];
		if (player.torso.cocks.count > 0) select[select.length] = 0;
		if (player.torso.vaginas.count > 0) select[select.length] = 1;
		//Genderless scene has low chance for gendered folks
		if (player.gender === Gender.NONE || (player.gender > 0 && randInt(3) === 0)) select[select.length] = 2;

		//SELECT SCENE HERE
		let choice: number = select[randInt(select.length)];
	...
	private winDriderPCDickInSpiderCunt() {
		...
		if (monster.lust > 99) {
			...
			if (monster.nipplesPierced > 0) DisplayText(", the chains jingling loudly as she does so");
		...
		if (monster.nipplesPierced > 0) DisplayText("chain-pierced-");
		...
		//[NEXT]
		return { next: winDriderPCDickInSpiderCunt2 };
		player.orgasm();
		player.stats.lib += 1;
		player.stats.sens += 1;
	}
	...
	private victoryVSDriderRideOviVaginal() {
		...
		DisplayText("Like this, you're treated to a perfect view of ALL of the web-woman's endowments.  Her four D-cup tits flatten slightly against her torso now that she's on her back, hanging slightly to the side.  At her crotch, you can see her glistening, black-lipped sex puffing up, the tip of her large button already peeking out of her tempting box.  Just below that, the black and " + monster.hair.color + "-striped exoskeleton of her spider-half starts, merging down into an bulbous, arachnid shape.  At the very bottom of it all is the huge bulk of her abdomen, covered with numerous spinnerets and with a wide, mostly-retracted organ near the back.\n\n");
	...
	private loseVSDriderAndGetFuckedANDAnalImpreggegity() {
		...
		if (player.stats.lust > 99) DisplayText("Unable to resist the desire coursing through your body, you strip off your " + player.inventory.equipment.armor.displayName + " and start stroking " + Desc.Cock.describeMultiCockSimpleOne(player) + ", displaying it in the hope that it will draw the drider's attention, while at the same time just trying to get yourself off and attain release.");
			...
			DisplayText("member");
			if (player.torso.cocks.count > 1) DisplayText("s");
			DisplayText(" to the air.  She squeezes " + Desc.Cock.describeMultiCockSimpleEach(character) + " in her hand and pumps it, smiling while you stiffen and twitch precipitously in her grasp.");
		}
	...
	public driderPregVagBirth() {
		...
		if (player.torso.neck.head.face.eyeType != EyeType.FOUR_SPIDER_EYES) DisplayText("  Suddenly she flops forward, leaning against your eyebrows and causing you to cross your eyes in an attempt to focus on her.  She seems confused by the way your eyes are different to hers.");
	...
	private driderTiedUpPartTwo(useItem: boolean) {
		...
		if (useItem) {
			...
			DisplayText("Smirking to yourself, you advance on the collapsed girl, [cock " + y + "] straining against your clothing in anticipation of what's to come.");
		}
		DisplayText("\n\nDarting forwards suddenly, you seize her wrists, pulling her arms above her head");
		if (monster.lust > 99) DisplayText(" and away from her quivering snatch");
