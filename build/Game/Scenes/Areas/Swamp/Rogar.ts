...
export interface RogarFlags {
    ROGAR_PHASE: number;
    ROGAR_DIRT: number;
    ROGAR_WARNING: number;
    ROGAR_DISABLED: number;
    ROGAR_FUCKED_TODAY: number;
}

const rogarFlags: RogarFlags = {
    ROGAR_PHASE: 0,
    ROGAR_DIRT: 0,
    ROGAR_WARNING: 0,
    ROGAR_DISABLED: 0,
...
User.flags.set(FlagType.Rogar, rogarFlags);
...
export function encounterRogarSwamp(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Rogar);
    if (rogarFlags.ROGAR_PHASE === 0) {
        ...
        if (player.stats.cor < 70) DisplayText("You nod at him, wiping at the sludge on your body before frowning at yourself.  Ro'gar rumbles thoughtfully, eyeing you over again.  \"<i>My house's over yonder.  Ain't nobody but m'self 'round here, but I kin find ya somethin' to wipe yer face with.</i>\"  Ro'gar offers his friendly smile again, his formidable rows of teeth defeating the gesture.  Though vainly, you try to ignore the teeth and smile back at the generosity.  Ro'gar's thick finger points to the ground.  \"<i>Ya jus' wait right here an' I'll be back,</i>\" he says as he turns, trotting through the mud casually.  Should you really wait for him?\n\n");
        ...
        // ((Corruption 70 or more))
            ...
            if (player.torso.hips.legs.isNaga() || player.torso.tails.count > 0) DisplayText("  Your tail flicks about happily.");
            DisplayText("  Should you really wait for him?");
        }
        return { yes: waitForChunkyOrcLoe, no: dontWaitForRogar };
    }
    // ((Ro'gar phase = 1)) (edited)
    else if (rogarFlags.ROGAR_PHASE === 1) {
        DisplayText("With map in hand you wander the swamp for some time, squinting at the rough scrawls Ro'gar had scribed for you.  You can't help but stop and scratch your head as you begin to wonder if you are hopelessly lost.\n\n");
        ...
        if (player.stats.int <= 30) {
            ...
            DisplayText("Eventually one comes out on top, pinning the other to force it to lie down and remain still.  They pant heavy breaths as the overpowered man's exhausted body finally gives in to the victor.  With a nasty snarl the victor grips the defeated body, tossing him a few feet to land in the mud with a splash.  The defeated man slowly struggles to his feet as the winner points his finger and shouts authoritatively.\n\n");

            DisplayText("\"<i>GIT!</i>\"\n\n");

            DisplayText("Your eyes widen as you recognize that voice!  You move from your hiding spot to get closer.  \"<i>Ro'gar?</i>\" You call out as you approach, but slip on a wet root and stumble forward.  Before you feel the wet embrace of mud, strong hands grip you around the chest and right you.\n\n");
        ...
        // ((intellect 30 or more))
            ...
            // [(lib > 50)
            if (player.stats.lib >= 50) DisplayText("  Your loins burn");
            else DisplayText("  Your nose crinkles");
            DisplayText(" at the remains of Ro'gar's pleasure, and you make a mental note that the orc must be lonely.  You figure someone spends many nights entertaining himself here, alone.\n\n");
        ...
        if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2 && player.stats.lib >= 50) {
            ...
            // set Ro'gar phase = 2
            rogarFlags.ROGAR_PHASE = 2;
            player.stats.lust += 30;
            return { next: Scenes.camp.returnToCampUseTwoHours };
            return;
        }
        // ((high femininity or breasts >=B-cup, libido less than 50))
        else if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2) {
            DisplayText("You grin as you chat with Ro'gar mindlessly.  ");
            // [(int <30)
            if (player.stats.int < 30) DisplayText("Ro'gar apologizes for wrestling for his territory in your sight.");
            // (int>=30)
            else DisplayText("Ro'gar apologizes for how messy his hut is.");
        ...
        // ((androgynous or masculine and breasts <= A-cup, libido less than 50))
            ...
            if (player.stats.int < 30) DisplayText("leftovers from that fight in the swamp still linger on his body.");
            else DisplayText("it makes you wonder what Ro'gar was up to.");
            DisplayText("  Ro'gar follows your gaze, then his eyes glint oddly as he snickers.  \"<i>Care ta... return the favor?</i>\"  He slyly winks at you, eyes flicking from you to the bucket and back again.");
            // goto sexy choices
        }
    ...
    // Not sure what determines this yet
    else if (rogarFlags.ROGAR_PHASE === 2) {
        ...
        if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2) {
            ...
            rogarFlags.ROGAR_PHASE = 3;
        }
        // ((androgynous or masculine and breasts <= A-cup))
        else {
            DisplayText("Remembering your way from memory, you find yourself on Ro'gar's doorstep. You gently knock with your fist, and soon you hear footsteps on the other side of the crude door. It opens with a snap and you are greeted with a wide grin of jagged, razor-like teeth.\n\n");
            // (Dirt Mc Girt = 2)
            if (rogarFlags.ROGAR_DIRT === 2) DisplayText("\"<i>" + player.desc.short + ", y'all took off on me real quick like,</i>\" he says through a snicker.  The joke falls flat and the orc seems to blush lime green as you stare at him uncomprehendingly.  He rubs the back of his neck with a hand before continuing.  \"<i>Sorry about that.  I was plumb tuckered out afterwards.</i>\"\n\n");
            // (merge sex and non-sex, new PG)
            DisplayText("\"<i>Come on in.</i>\"  Ro'gar extends a hand, welcoming you inside.  You quickly take a seat and fold your arms");
            if (player.stats.lib > 50) {
                DisplayText(" over your musk-fueled, growing ");
                if (player.torso.cocks.count > 0) DisplayText("bulge");
            ...
            if (player.statusAffects.get(StatusAffectType.TelAdre).value1 <= 0) {
            ...
            // (if player has been to Tel'Adre)
                ...
                DisplayText("\"<i>You'll come an' find me, right?  After I get there?</i>\" Ro'gar asks as he puts a hand on your shoulder.  You nod quickly. \"<i>Good, then I'll be seein' you there.</i>\"\n\n");

                // (if Dirt Mc Girt = 1)
                if (rogarFlags.ROGAR_DIRT === 1) {
                    DisplayText("You leave Ro'gar to finish packing his things for now.");
                    // set Ro'gar phase = 3
                    rogarFlags.ROGAR_PHASE = 3;
                    return { next: Scenes.camp.returnToCampUseOneHour };
                }
                // (if Dirt Mc Girt = 2)
                    ...
                    DisplayText("As you exit the door you suddenly feel a strong grip on your wrist, making you turn around.");
                    // (if player height is shorter than 4 feet)
                    if (player.tallness < 48) DisplayText("  Hands hook under your armpits to lift you up gently.");
                    // (if player height is taller than 7 feet)
                    if (player.tallness >= 84) DisplayText("  A hand on your shoulder tugs you to lower your head.");
                    DisplayText("  Lips press to yours and you taste Ro'gar's long, orcish tongue flicking out, trying to coax its way into your mouth.  With a faint moan you open wide, locking tongues with him as your eyes close in bliss.  You wrap your arms around the orc's neck in embrace, your hands curling around to hold him near.  His own strong hands carefully settle at your " + Desc.Hip.describeHips(player) + " roughly clenching down as his fingertips press over your " + Desc.Butt.describeButt(player) + ".  The two tongues wrestle in your mouth before Ro'gar slowly pulls away.  \"<i>Thanks fer everythin', " + player.desc.short + ".  Y'all take care.  I'll be keepin' my eyes peeled for ya.</i>\"  With his usual toothy grin he waves you off.\n\n");

                    DisplayText("You wander toward your camp through the swamp, making it there without any problems beside the heat in your loins.");
...
// ((No))
function dontWaitForRogar(): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Rogar);
    DisplayText("Time passes with no sign of Ro'gar.  You decide it would be a waste of time to sit and wait for him, and head off in hopes of finding something interesting.\n\n");

    DisplayText("Instead, you find only more mud.  You return to camp.");
    // <set Crying Game = 1>
    rogarFlags.ROGAR_DISABLED = 1;
    return { next: Scenes.camp.returnToCampUseOneHour };
}

// ((Yes))
function waitForChunkyOrcLoe(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Rogar);
    if (rogarFlags.ROGAR_PHASE === 0) {
        ...
        if (player.stats.lib < 50) DisplayText("The hulking orc is surprisingly gentle as he cleans you like a young child.  The warm waters and careful touch soothe your tired muscles.  Your skin becomes textured with goose bumps as the tickles of droplets roll down your body and the filth is washed down to the waters below.  You find yourself sighing in relaxation from the treatment.  A smile curls your lips as you look down at Ro'gar, and he returns the pleased look with a toothy smirk.\n\n");
        // ((If libido is 50 or more))
        else {
            DisplayText("  Something about the burly orc rubbing you down with warm water excites you.");
            if (player.torso.vaginas.count > 0) {
                DisplayText("  Your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " starts to quiver as the warm water rolls over its lips, moistening in excitement as your skin is rubbed over by strong hands.  Your chest heaves as it relaxes to his touch, your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s becoming stiff under his rubbing.");
                DisplayText("  Even your " + Desc.Leg.describeFeet(player) + " and fingers curl as you smell the sweat and musk of Ro'gar's closeness, the animalistic scent arousing you.");
            }
            else if (player.torso.cocks.count > 0) DisplayText("  " + Desc.Cock.describeMultiCockSimpleOne(player, true) + " stirs to life, making it obvious how much you are enjoying the rub down, then begins to drool with lust.  Through lidded eyes you faintly make out Ro'gar's toothy smirk becoming a hungry grin.");
            player.stats.lust += 5 + player.stats.lib / 20 + player.stats.sens / 20;
            DisplayText("\n\n");
        }
        DisplayText("\"<i>Clean as a whistle.</i>\"  Ro'gar chuckles as he drops the towel in the bucket with a splash.  His face beams with pride at his work, chest puffing out as he looks you over.  He eyes you up and down again, and your curious look just makes him snicker.  \"<i>Don't meet too many friendly folk 'round here, but it would be nice to have a " + Desc.Gender.mf(player, "feller", "lass") + " to talk with.</i>\"  After a few moments of awkward rummaging through his pocket he pulls out a scrunched up piece of paper and hands it to you.  \"<i>This's so you can find me.</i>\"  Ro'gar grins from ear to pointy ear.  You unfold the paper, and see a crudely drawn map that vaguely explains how to get to Ro'gar's hut.  You wonder if making this map was why he took so long.  \"<i>Y'all take care now, y'hear!</i>\" he hollers, turning summarily and walking away, waving over his shoulder at you as he tromps through his natural habitat.  You wander back to your camp, wondering if you should see him again.\n\n");

        DisplayText("<b>You can now find Ro'gar's hut when wandering the swamp occasionally!</b>");
        // pass 2 hours, set Ro'gar phase flag = 1
        rogarFlags.ROGAR_PHASE = 1;
        return { next: Scenes.camp.returnToCampUseTwoHours };
    }
}

// sexy choices
function sexyChoices(): NextScreenChoices {
    DisplaySprite(SpriteName.Rogar);
    return {
        choices: [
            ["Ewww", ewwwRogarIsGay],
            ["NoThnxBro", noSlowBroIDontWantPokeSex],
            ["Lick Clean", okayBroLetsHaveAGayCarwash]
        ]
    };
}
// ((Ewww))
function ewwwRogarIsGay(): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Rogar);
...
// ((No thanks bro))
function noSlowBroIDontWantPokeSex(): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Rogar);
    ...
    rogarFlags.ROGAR_DIRT = 1;
    rogarFlags.ROGAR_PHASE = 2;
    return { next: Scenes.camp.returnToCampUseTwoHours };
}

// ((Ok))
function okayBroLetsHaveAGayCarwash(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Rogar);
    ...
    DisplayText(" aches, each step of the way rubbing the sensitive");
    if (player.gender > 0) DisplayText(", drooling");
    DisplayText(" parts against the fabric of your clothes.\n\n");
    // <Lust increased, slimefeed,
    player.slimeFeed();
    player.stats.lust += 40;
    // set Dirt Mc Girt flag = 2 and Ro'gar phase = 2>
    rogarFlags.ROGAR_DIRT = 2;
    rogarFlags.ROGAR_PHASE = 2;
    if (player.stats.int < 30) return { next: Scenes.camp.returnToCampUseTwoHours };
...
export function rogarThirdPhase(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Rogar);
    ...
    if ((player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2)) {
        if (rogarFlags.ROGAR_WARNING === 0) {
            DisplayText("Judging the patron's figure based on how imposingly filled out the cloak is, he or she is clearly very muscled.  You spy a green hand reach for a can and a spark of recognition hits you.  You're completely unsure how to break the ice, but tug on the hem of the cloak.  The figure turns toward you, and you're greeted with the green, toothy smile of Ro'gar, the orc from the swamp!  \"<i>H'lo, " + player.desc.short + "!  I'm s'rprised to see ya here...</i>\"\n\n");
        ...
        // return to bar menu
        return { next: Scenes.telAdre.barTelAdre };
    }
    // ((androgynous or masculine and breasts <= A-cup))
    else {
        DisplayText("After taking a few moments to look over the familiar cloth of the cloak, your lips curl into a small smile.  Walking up quietly, ");
        // [(if Ro'roh Raggy flag >=1)
        if (rogarFlags.ROGAR_WARNING >= 1) {
            DisplayText("you tap him on the shoulder.  He turns around, then assumes a pleasant smile.  \"<i>Well, h'lo.  Who might you be?</i>\"  Momentarily confused, it quickly comes to you that you were quite different at your prior meetings; you reintroduce yourself with a small bow.  \"<i>Wow.  " + player.desc.short + ", ya look completely different than before!</i>\"  You nod, slightly abashed at having so much attention drawn to your changes, and point out that this world can have some unusual effects on people.  He nods eagerly.  \"<i>Well, ya look jus' great.  How 'bout havin' a sit and chattin' with me fer a while?</i>\"\n\n");
        }
        // (else if Ro'roh Raggy < 1 and player height >= 5 feet)
        else if (player.tallness >= 60) {
            DisplayText("you carefully wrap your arms around the figure's head and cover his eyes with your hands.  A playful giggle escapes your lips as you murmur for him to \"<i>guess who.</i>\"  A familiar voice growls, but quickly relaxes as orcish hands cup over yours.  \"<i>Dag nabbit " + player.desc.short + ", don't sneak up on a feller like that.</i>\"  Ro'gar's low tone fills your ears as he turns to face you with a wide grin.\n\n");
        }
        // else if Ro'roh Raggy < 1 and player height < 5 feet)
        else {
            DisplayText("you gently tug on the figure's cloak, mentioning aloud that it seemed to have held up all right with your sewing job.  The figure's back straightens sharply and a deep snicker is heard.  \"<i>Ya did a mighty fine job, " + player.desc.short + ".  Been waitin' for ya.</i>\"  Ro'gar turns and grins at you.\n\n");
        }
        // ((Paths combine here))
        ...
        if (player.skin.tone === "green") DisplayText("  \"<i>Hey... Have y'ever had somebody randomly run up an' ask ya ta 'Waaagh!'?</i>\" he mutters quietly.  You give him a perplexed look.  The orc laughs off your confusion dismissively.  \"<i>Nothin', nothin'.</i>\"");
        DisplayText("\n\n");
        ...
        // variable ending
        // [(if Dirt Mc Girt = 0)
        if (rogarFlags.ROGAR_DIRT === 0) {
            DisplayText("You chat for a while longer before separating.  Ro'gar extracts a promise from you to come drink with him again before waving you off with his standard grin.");
            // set Ro'gar phase = 4 and Ro'roh Raggy = 0
            rogarFlags.ROGAR_PHASE = 4;
            rogarFlags.ROGAR_WARNING = 0;
            return { next: Scenes.camp.returnToCampUseOneHour };
        }
        // (else if Dirt Mc Girt = 1)
        else if (rogarFlags.ROGAR_DIRT === 1) {
            DisplayText("Ro'gar gives a satisfied huff, relaxing his body as he takes another drink from his can.  \"<i>You been nothin' but good ta me ever since I met ya, " + player.desc.short + ".  I do ya one favor and you been a real friend ever since.</i>\"  Ro'gar's tone grows lower and more serious as a calloused finger rubs over his damp can.  You rest your arms against the polished wood of the bar as you listen.  \"<i>Yer special ta me,</i>\" he mutters quietly, just loud enough for you to hear him.  You smile warmly and rest a hand against his arm, coaxing him to turn his head to look at you.  \"<i>Here, I got ya sumthin.</i>\" Ro'gar digs through his pants pocket and pulls out a tin can.  Tugging your hand closer, he puts it in your palm and grins widely at you.  \"<i>It's what I'm drinking now.</i>\"  He taps his can.  \"<i>One've my favorites.  Thanks, " + player.desc.short + ".  You kin drink with me any time.</i>\"  The pair of you spend some time together at the bar, drinking and laughing as you empty mugs.  Eventually you part ways and wander back into the city.\n\n");
            // set Ro'gar phase = 4 and Ro'roh Raggy = 0, Acquire 1x Bro Brew
            rogarFlags.ROGAR_PHASE = 4;
            rogarFlags.ROGAR_WARNING = 0;
            player.inventory.items.createAdd(player, ItemType.Consumable, ConsumableName.BroBrew, Scenes.camp.returnToCampUseOneHour);
        }
        // (else if Dirt Mc Girt = 2)
        else {
            DisplayText("Your heart flutters as you realize he is still only wearing the ragged shorts beneath his hooded cloak, and your eyes rake over his musculature as he models.  \"<i>How's about you an' I go visit the baths sometime?</i>\"  Ro'gar asks coyly, relaxing his body as he takes another drink.  \"<i>You been nothin' but good ta me ever since I met ya, " + player.desc.short + ".  I do ya one favor and you been a real friend ever since.</i>\"  Ro'gar's tone grows lower and more serious as a calloused finger rubs over his damp can.  You rest your arms against the polished wood of the bar as you listen.  \"<i>Yer special ta me.</i>\" he mutters quietly, just loud enough for you to hear him.  You smile warmly and rest a hand against his arm, coaxing him to turn his head to look at you.  His tongue slides over his lips as he gives you a seductive stare, with his hand reaching between your legs.");
            // [(female or unsexed)
            if (player.gender === Gender.FEMALE || player.gender === Gender.NONE) {
                ...
                DisplayText("  " + Desc.Cock.describeMultiCockSimpleOne(player, true) + " gets a caress from him as he explores, arousing it to mild stiffness.  \"<i>" + player.desc.short + ",</i>\" he grunts.  \"<i>Ah hope ah'll be seein' more of ya.</i>\"  Ro'gar breaks the contact and tips back the remains of his drink while you ");
                if (player.stats.cor < 50) DisplayText("blush");
                else DisplayText("fantasize about those strong hands touching every part of you");
                DisplayText(".  I drink here whenever I'm not werkin' out or on a job.  I'll be waitin' for you.</i>\"  He lowers his eyelids suggestively at you, then gets up; you smile dumbly as he leaves the bar, watching him go with a blush in your cheeks");
                // [(If player has a tail)
                if (player.hasLongTail() && player.torso.hips.legs.isNaga()) DisplayText(", and you feel your tail flicking rapidly in excitement");
                DisplayText(".\n\n");
...
export function rogarPhaseFour(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Rogar);
    // (if high femininity or breasts <=B-cup and Ro'roh Raggy = 0)
    if ((player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2) && rogarFlags.ROGAR_WARNING === 0) {
        ...
        if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2) DisplayText(" and your " + Desc.Breast.describeChest(player));
    ...
    // (high fem or breasts >=B-cup and Ro'roh Raggy =1)
    else if ((player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2) && rogarFlags.ROGAR_WARNING === 1) {
    ...
    // (andro or masculine with tits <= A)
        ...
        DisplayText("Ro'gar is tipping his drink back when he catches sight of you, and waves you over.  \"<i>Ah, good ta see ya, " + player.desc.short + "!");
        // [(Ro'roh Raggy = 1)
        if (rogarFlags.ROGAR_WARNING === 1) {
            DisplayText("Yer lookin' much better today; ya been workin' out?</i>\"  You smile at the compliment, and ");
        }
        else DisplayText("</i>\"  You ");
        ...
        // [(if F or U)
        if (player.gender === Gender.FEMALE || player.gender === Gender.NONE) {
            DisplayText("  It reaches the center without interruption, and Ro'gar colors deeper.  \"<i>Uh... ferget it,</i>\" he mumbles.  \"<i>Didn't mean ta be indecent with you or anythin', miss.</i>\"  He nods to you and quickly departs before you can recover from the surprise.");
            return { next: Scenes.camp.returnToCampUseOneHour };
        }
        else {
            DisplayText("  " + Desc.Cock.describeMultiCockSimpleOne(player, true) + " trembles under his strong touch, as he increases the pressure and looks into your eyes meaningfully.  ");
            // [(if Dirt Mc Gurt flag = 1)
            if (rogarFlags.ROGAR_DIRT === 1) DisplayText("</i>\"I know ya probably ain't ever imagined hangin' out with a feller like me, but... I could really use the company tonight.</i>\"  ");
            DisplayText("He's clearly in the mood for some fun; do you want to head back to his place and do something?");
            // + lust, if M or H, set Ro'roh Raggy = 0
            rogarFlags.ROGAR_WARNING = 0;
            // and give choices
            // [Get Anal][Give Anal][Frot][Drink with your Bro! (requires 1x Bro Brew)][No Thanks]
            player.stats.lust += 10;
            return rogarFuckMenu(player);
        }
    }
}

function rogarFuckMenu(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Rogar);
    let getAnal;
    let giveAnal;
    let frot;
    ...
    if (player.inventory.items.has(ConsumableName.BroBrew)) brodown = brobrobrobro;
    ...
    return {
        choices: [
            ...
            ["GetTiedAnal", getTied],
            ["OrcRubDown", rubdub],
            ["No Thanks", noTHanksRogarIAintGayDudeIjustLickedThatMudToBeNice],
        ]
    };
}

// [No Thanks]
function noTHanksRogarIAintGayDudeIjustLickedThatMudToBeNice(): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Rogar);
    DisplayText("You smile at the orc, but tell him that you're not really in the mood to hang out right now.  He looks crestfallen, but nods at you.  \"<i>Well, I understan'.  I'll be seein' ya later, then.</i>\"");
    // Bar menu!
    return { next: Scenes.telAdre.barTelAdre };
}

// (([Get Anal], Scenario 1a: give up the buttcherry (buttvirgins only)))
function loseButtGinity(player: Character): NextScreenChoices {
    rogarFlags.ROGAR_FUCKED_TODAY = 1;
    DisplayText().clear();
    DisplaySprite(SpriteName.Rogar);
    // BUTTCHERRIES
    if (player.torso.butt.looseness === 0) {
        ...
        if (player.stats.cor < 33) DisplayText("shyly ");
        else if (player.stats.cor > 66) DisplayText("eagerly ");
        DisplayText("over the threshold, and then look around the apartment.  He points out the small kitchen area and then, rather sheepishly, the bedroom.  You're examining it when something catches your eye...\n\n");
        ...
        DisplayText("He yanks your wrists behind your back, ignoring your faint gasps.  With a rustle of cloth, your wrists are bound together behind your back tightly; your fingers feel over the bonds but you struggle against them in vain.  Looking over your shoulder in confusion at Ro'gar, you only see his lust-ridden face dipping down to kiss at your lips.  His tusks press against your cheeks with a demanding snarl and his tongue into your mouth; your " + Desc.Butt.describeButt(player) + " receives a smack, forcing you to yelp.  His tongue quickly invades, and his free hand unfastens your " + player.inventory.equipment.armor.displayName + " to strip you forcefully.  Rough, calloused fingers dance over your body and fasten onto your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s, coaxing moans from your throat with each pinch.  Your pleased noises are muffled by Ro'gar's mouth, his tongue and lips hushing each whimper and gasp you breathe.  A bare, firm chest presses against your exposed back, and you pant as the heat from his body burns against you, only intensified by the thin layer of oil on his skin.  Ro'gar's blue eyes watch you like a predator as your face flushes.  He breaks the kiss and cups his hands under your chin to hold your face in place.  \"<i>Such a perdy face yer makin', " + player.desc.short + ".  Yer lookin' at me like a proper slut, an' it's makin' me hard seein' ya so needy to be bred.</i>\"  He growls in his throat as his mouth nips over your neck, leaving red marks along your skin where his tusks rub against you.  You can feel the oil and sweat smearing over your back, and your senses go wild.  The room is soon filled with your needy whimpers as " + Desc.Cock.describeMultiCockSimpleOne(player) + " oozes and throbs against the sheets, making the fabric damp with your lust.\n\n");

        DisplayText("With an abrupt grunt you feel hands gripping your " + Desc.Hip.describeHips(player) + " and pulling your rear into the air, making you balance on your face and knees.  Ro'gar begins to play with your " + Desc.Butt.describeButt(player) + "; each hand massages a cheek between his fingers, squeezing and kneading them apart.  A finger rubs over your sensitive entrance and you shudder softly.  The orc tests its resistance with his finger, pressing in and threatening to penetrate, rubbing harder around the tight, puckered flesh.  Your virgin hole's taut flesh remains resistant to all the coaxing Ro'gar's finger lavishes on it. \"<i>Tarnation, " + player.desc.short + ".  You're so tight...</i>\"  You feel your cheeks blush with bright color and in a low voice you admit you've never had anyone inside you.\n\n");

        DisplayText("It's as if time freezes: everything stops, including your erstwhile lover.  The only sound in the room is your own breath for almost half a minute, until Ro'gar rolls you over by your " + Desc.Hip.describeHips(player) + ", forcing your body to lay out over your back.  The orc's face comes down close enough that you feel his breath washing over your skin.  \"<i>You serious?</i>\" he asks with eagerness you've never seen before.  Caught off guard, you simply nod your head as his eyes go wide.  With a rip you hear the shorts torn from Ro'gar's body.  You feel his hot, throbbing length grinding over your " + Desc.Butt.describeButt(player) + " before you see it, the deep green head towering over your groin menacingly, each vein pulsing as the mammoth cock belches pre-cum to leak down the orcish shaft.  Your skin is soon slick with his spurting pre as he groans out.  \"<i>Oh fuck... a bona-fied virgin ass.  Pure an' untainted, ripe for the pluckin'... or maybe the fuckin'.</i>\"  Ro'gar's breath becomes labored as he pants heavily and eagerly, his green length humping against your virgin rosebud, making you moan out in an excess of sensation.  His hands hold your cheeks apart with a strong grip as you begin to leak your own pre-cum onto your stomach.");
        // (if player has balls)
        if (player.torso.balls.quantity > 0) DisplayText("  Ro'gar's dark green cockhead rubs over your " + Desc.Balls.describeBalls(true, true, player) + " and coats the plump sack in his musky goo, like an alpha male marking his territory.");
        DisplayText("\n\n");

        DisplayText("Deep, guttural hisses echo out from the orc.  \"<i>Easy fella...</i>\" Ro'gar mutters, trying to restrain himself from ripping you apart with his fat cock.  He leans down and kisses you softly before continuing.  \"<i>I'll give ya a proper first time, all gentleman-like.</i>\"  With a playful wink he gets down on his knees, squatting near the foot of the bed.  His hand hoists your " + Desc.Hip.describeHips(player) + " up to his mouth.  You can feel him kiss your tight ring, and soon a warm sensation shoots up your spine, causing you to moan.  His hot, slick tongue swirls over your puckered flesh, slathering your hole while his tusks rub along your cheeks.  You're left panting out hot, needy breaths as Ro'gar works over your virgin anus, his tongue gently easing your tight ring of flesh open.  The pleasure makes your legs go limp over his shoulders and your eyes shut in a blissful daze.  Ro'gar's wet slurps and growls make your ears twitch, and you feel him slowly press in with his orcish tongue, beginning to tonguefuck your entrance forcefully.  With patience, your pucker gives in under Ro'gar's touch and his tongue slides into your untouched confines.  You give a sharp gasp, this strange feeling overwhelming you as you flick your eyes open.  Your " + Desc.Butt.describeButt(player) + " wriggles in pleasure around his tongue, and the orc gives a pleased grunt as his tongue swirls around your inner walls, sliding out to pull your lime flavor into his mouth before roughly thrusting back in.  Your back arches as you cry out a surprised moan.");
        if (player.hasLongTail() || player.torso.hips.legs.isNaga()) DisplayText("  Your tail wraps around Ro'gar's arm for some sort of stability, the tip flicking lazily as it coils and squeezes around a bicep.");
        DisplayText("\n\n");

        DisplayText("Ro'gar produces harsh snarls, eagerly tonguefucking you, eating you out as his lips work over your ass.  His fingers grip your " + Desc.Hip.describeHips(player) + " roughly as he ravages your hole in a manner beyond your wildest dreams.  You hear slapping as Ro'gar's hips smack involuntarily against the side of the bed.  His aching erection drools globs of thick slime over the sheets, barely restraining his desire to fuck you senseless.  \"<i>Relax!</i>\" Ro'gar barks in a commanding tone that shocks you.  You try to comply with a whimper, feeling something pressed against you.  Ro'gar's fat finger prods your spit-slicked pucker, slipping in with a wet shlick.  You shudder as his digit slides into you with some resistance, but the relentless finger rubs over your walls as he begins to pull your rim open, forcing another finger inside and making you bite your lip from the stinging sensation.  Working the pair of fingers together inside of you, he slowly moves them apart and stretches your hole wide.");
        DisplayText("  Fingers and " + Desc.Leg.describeFeet(player) + " curl in pleasure as your previously untouched boycunt is forced open by Ro'gar's steady coaxing.");
        ...
        DisplayText("Your mind races with Ro'gar's words as they seemed to be coming true.  You can't help but focus on the wonderful feeling of Ro'gar's cock throbbing within you.  Maybe this is where you belong?  With your tight ass impaled on Ro'gar's massive tool?  You moan loudly as Ro'gar picks up the pace, hips ramming into you with enough force to shift your body over the sheets.  He really starts to break in your hole with huffed grunts and groans as he hilts in you; his cockhead prods your prostate with each thrust, causing you to cry out as electric shocks shoot up your spine.  Every nerve in your body lights up like a Christmas tree and make your entire body squirm with pleasure.  You feel spurts of cum begin to leak from your " + Desc.Cock.describeMultiCockShort(player) + " and glaze your belly with pre-cum.");
        if (player.hasLongTail() || player.torso.hips.legs.isNaga()) DisplayText("  Your tail, still gripping Ro'gar's arm, squeezes tight.");
        // (If player has balls)
    ...
    // (([Get Anal] Scenario 1b: Get fucked (normal)))
        ...
        if (player.tallness >= 84) DisplayText("  You lean down as Ro'gar tilts up, your lips connecting with his, both mouths open to let tongues wrestle.");
        // (If player height is less than 7 feet)
        if (player.tallness < 84) DisplayText("  You lean up into Ro'gar's mouth, his hand caressing your cheek.");
        DisplayText("  You can feel his demanding tongue pressing into your mouth and coaxing moans from the wanton entrance.  His hand slides down your waist to cup your " + Desc.Butt.describeButt(player) + " in his strong grip.  Ro'gar breaks the kiss with a devious grin.  \"<i>Let's get you out of these clothes.</i>\"  His voice rumbles deep in his chest, the resonations vibrating against your skin.  He takes the initiative and pulls off your garments, leaving you completely nude in his lap.  One of the orc's hands wanders over your body, playing with your nipples while the other slides down toward your " + Desc.Cock.describeMultiCockShort(player) + ".  Ro'gar's head nestles in the nape of your neck, gently nipping at your skin and reddening it with a slight sting.  Where he nibbles he quickly follows with a tender lap of warm tongue over the abused flesh, bringing slight shudders through your body as his smooth tusks rub over you.\n\n");

        DisplayText("You are left panting for breath from Ro'gar's continued teasing, your skin on fire from every touch as his thin oil smears over it.  You spread your legs lewdly on Ro'gar's lap, blood rushing south as you feel yourself stiffen and throb - though you aren't the only one sporting wood, as you soon feel Ro'gar's hot manhood pressed up to your side.  He groans into your shoulder, bucking his hips to hump your flesh.  Pre-cum dribbles from Ro'gar's swollen cockhead and smears you in his need.  \"<i>Come here,</i>\" he whispers directly into your ear.  His hands grip your bottom as he pulls you closer to face him.  Your chest presses to his as you straddle Ro'gar's hips.  Ro'gar's throbbing ten inches of man meat tucks between the cheeks of your " + Desc.Butt.describeButt(player) + ". He gives you a playful wink, bucking his hips to bounce you in his lap.  The feeling of his hot, thick length rubbing over your " + Desc.Butt.describeButthole(player.torso.butt) + "  and wetting it with copious pre makes you gasp like a bitch in heat.  The dripping orc spooge snakes down in between your cheeks, and you feel your body shudder with anticipation; Ro'gar lets the liquid pour over his own hard-on.  \"<i>My favorite boy ready for the ride of his life?</i>\"  He grins impishly as he grips the head of his thick, uncut orc cock and pushes it against your boy cunt.  A gasp of pleasure escapes your lips and your hands clasp onto Ro'gar's shoulders, wriggling your " + Desc.Butt.describeButt(player) + " over the green mile.\n\n");

        DisplayText("With a buck of Ro'gar's hips his slick cockhead slips into your " + Desc.Butt.describeButthole(player.torso.butt) + ".  Your lips part wide, letting out a sharp gasp as he nestles within you.  \"<i>Git ridin', boy!</i>\" he barks, with a hard smack against your " + Desc.Butt.describeButt(player) + ".  You yelp at the rough treatment, easing yourself down slowly to slide Ro'gar's stiff flesh further into you, and feel yourself growing hard as the throbbing manhood stretches you open.  " + Desc.Cock.describeMultiCockSimpleOne(player, true) + " swells with blood as it brushes against Ro'gar's stomach, and the sensitive urethra picks up a thin layer of his slime from his skin, making you burn with sensitivity.  Your hips shift into Ro'gar's body more, rubbing your " + Desc.Cock.describeMultiCockShort(player) + " tightly against him.");
        if (player.torso.cocks.count > 1) {
            DisplayText("Lines of pre-cum are drawn with your erections, adulterating the sweat and oil on Ro'gar's body as you bob your hips along Ro'gar's mast.   ");
        }
        DisplayText("Your spunk dribbles over Ro'gar's green skin and collects in his navel as his mouth attacks your neck with an assault of nips and licks, each nibble rubbing his tusks over your sensitized skin.  Hands roughly grip your " + Desc.Butt.describeButt(player) + ", spreading your cheeks apart manually and kneading the flesh like dough.  He coaxes you to move faster with demanding nudges of your hips, growling deeply with each thrust of his cock inside you.  Hot fluid sticks to your inner walls, arousal making you buck your hips helplessly as you become more and more sensitive with each thrust.  You're soon bouncing up and down on Ro'gar's prick, and to further your wild ride, Ro'gar begins bucking his hips up to meet your " + Desc.Butt.describeButt(player) + ".\n\n");

        // [(If player is fat or has very defined muscles)
        if ((player.thickness >= 75 && player.tone < 33) || player.tone >= 75) {
            DisplayText("Riding Ro'gar's stiff manhood as fast as you can muster, you drive the orc man wild with need.  His thick girth stretches your " + Desc.Butt.describeButthole(player.torso.butt) + " wide around it, making you a wonderful sleeve for his orc cock.  He hilts within you, and you cry out in pleasure as his cockhead rams your prostate.  Ro'gar wrests control from you as he hears you cry out like a bitch in heat.  His strong hands at your " + Desc.Hip.describeHips(player) + " hold you in place as he pounds your " + Desc.Butt.describeButthole(player.torso.butt) + ", ramming against you with brutal force and making you moan loud enough that your voice grows a little hoarse.  " + Desc.Cock.describeMultiCockSimpleOne(player, true) + " twitches from the abuse to your trigger");
            if (player.torso.balls.quantity > 0) DisplayText(" and your " + Desc.Balls.describeBalls(true, true, player) + " clench tight as you feel your limit reached");
            DisplayText(".");
            // [(if player has toes)
            DisplayText("  You curl your fingers and " + Desc.Leg.describeFeet(player) + " in ecstasy as Ro'gar drives you over the edge.");
            DisplayText("  Your bucks become ragged, and the orc takes over as " + Desc.Cock.describeMultiCockSimpleOne(player) + " jerks and spasms, shooting out countless ropes of sticky spunk onto the glistening green skin, painting the orc white with your lust.  With a low, deep, tone, Ro'gar groans out as you feel his warmth overtake your lower body, flooding you full of his fertile man cream.  Ro'gar continues to fuck you with decreasing vigor, slowing to a stop as his spent cock pops from your drooling pucker.  His spunk floods out of your filled boycunt, the spooge running down Ro'gar's half-hard cock to drip onto the floor lewdly.\n\n");
        }
        // (else)
        else {
            DisplayText("\"<i>Enough horsin' around.</i>\"  Ro'gar snarls as his arms quickly yank back, sneaking under your legs and hoisting you up as he gets to his feet.  You grip Ro'gar's shoulders tight as he lifts you with a strained grunt.  You delight with sharp little moans as he begins to fuck you skyward, bouncing you off of his hips with each slam of his cock into you.  Slowly Ro'gar steps towards a wall, taking his time to be sure not to drop you, and you soon feel the cool surface pressed to your back.  Caught between a cock and a hard place, you stare into Ro'gar's predatory gaze as he holds you hoisted in the air and pinned, making you feel vulnerable.  The orc takes advantage of you and begins to abuse your " + Desc.Butt.describeButthole(player.torso.butt) + ", making your head tilt back to moan out helplessly.");
            DisplayText("  " + Desc.Cock.describeMultiCockSimpleOne(player, true) + " lays out over your chest, dribbling pre-cum on your skin as you make a mess of your torso in lust.");
            // [(if player has tail)
            if (player.hasLongTail() || player.torso.hips.legs.isNaga()) DisplayText("  Your tail flicks helplessly below you, urgently wrapping around Ro'gar's thigh and trying to pull him deeper into you.");
            DisplayText("  You cry out as Ro'gar finds your sweet spot.  \"<i>Now I got yer button, boy,</i>\" he snickers, wearing his usual predatory grin, the sharp, jagged teeth on full display as you shudder from the relentless pounding against your " + Desc.Butt.describeButt(player) + ".  The sound of flesh smacking flesh is loud in your ears as you squirm from Ro'gar's cock.  Each thrust drives you mad with pleasure, your mind in a lust-induced haze as your prostate is prodded and poked.");
            // [(If player has balls)
            if (player.torso.balls.quantity > 0) DisplayText("  You feel your " + Desc.Balls.describeBalls(true, true, player) + " pull up tight in your sack as your labored breath comes out in pants.");
...
// ((Scenario 2, [Give Anal]: Fuck Ro'gar))
function fuckRogarsButtPussyBoyCuntManMoundSissySlitQueerQuim(player: Character): NextScreenChoices {
    rogarFlags.ROGAR_FUCKED_TODAY = 1;
    DisplayText().clear();
    DisplaySprite(SpriteName.Rogar);
    ...
    if (player.stats.cor > 70 && player.torso.vaginas.count > 0) DisplayText("Ro'gar flicks his tongue toward your " + Desc.Cock.describeMultiCockShort(player) + ", but you have a better idea and nudge him lower to guide him to your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ".  His eyes look up at you in confusion.  You can tell he'd rather not have his face buried there by the way he moves his mouth away, but you urge him on; telling him to get licking, you push your hips down over his face.  With his head caught between your legs and the bed his mouth presses to your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + "without a choice.  His whimpers of displeasure bring a grin to your face as you rub over his mouth with your netherlips.  He reluctantly flicks his tongue out to lap over your mound.  You shiver at the attention, growing moist and slick as you drip over Ro'gar's mouth.  The orc gives a whine of disgust as you rudely force him to pleasure your pussy, and you let out a loud moan as his unwilling licks turn you on; excited, you stroke your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ".  Ro'gar focuses his attention on that instead, watching you grow stiff and swollen, his eyes following your pumping hand.  You generously rub the slick, drooling pre-cum over " + Desc.Cock.describeMultiCockSimpleOne(player) + ", your vaginal walls clenching around his tongue as you pleasure yourself, working the goo over your flesh with your hand.  You look down and grin devilishly at him.\n\n");
    // (If player doesn't have vagina but has 70+ corruption)
    else if (player.stats.cor > 70) DisplayText("Ro'gar flicks his tongue over your " + Desc.Cock.describeMultiCockShort(player) + ", but you have a better idea and nudge him lower to guide him to your " + Desc.Butt.describeButthole(player.torso.butt) + " .  With a lusty growl Ro'gar attacks your pucker, tongue swirling over the surface and coaxing a pleased moan from your lips.  You grow stiff with arousal, bringing a hand down your shaft to answer your burning lust with a few teasing strokes.  You smear the pre-cum pouring out of your " + Desc.Cock.describeMultiCockShort(player) + " into an even coating of spunk as you work your hand.  Ro'gar looks over at the pumping hand and soon becomes distracted from his work, his tongue pressed lazily against your " + Desc.Butt.describeButthole(player.torso.butt) + ".  You look down and grin devilishly at him.\n\n");
    ...
    if (player.torso.cocks.sort(Cock.LongestCocks)[0].length < 4) {
        DisplayText("\"<i>Yer serious?</i>\" Ro'gar asks with a quirked eyebrow.  You blink as he suddenly gets up to stare you in the eyes.  \"<i>Not to be rude, " + player.desc.short + ", but I ain't gonna feel nothin'.</i>\"  You can't help but feel a little offended, saying something about motions of large bodies of water.  Ro'gar doesn't seem convinced as he buries his forehead in a palm.  \"<i>Listen, this ain't a really good time for this.  I'm already losin' my mojo.  You come back another time, okay?</i>\"  With that, you are ushered out as politely as possible.  You can't help but feel a little embarrassed as you walk the city streets like this is some sort of walk of shame.");
        return { next: Scenes.camp.returnToCampUseOneHour };
        return;		// end if too small
    }
    // ((else If smallest cock is over 18 inches))
    else if (player.torso.cocks.sort(Cock.ShortestCocks)[0].length > 18) {
        DisplayText("\"<i>E-easy there...</i>\" Ro'gar actually stutters, looking at you with concern.  You are impossibly huge for him, and despite your titanic head rubbing pre-cum over Ro'gar's green hole it just won't fit.  You let out an annoyed grumble and begin to rub your length over Ro'gar's body and orc cock.  With lustful grunts Ro'gar bucks his hips into your " + Desc.Cock.describeMultiCockShort(player) + ", grinding the stiff masts together as they leak and drool over one another.\n\n");
        // [(If multiple cock)
        if (player.torso.cocks.count > 1) DisplayText(Desc.Cock.describeMultiCockSimpleOne(player, true) + " grinds over every inch of Ro'gar's front, and his dark olive skin soaks in your glistening fluids.  Ro'gar's length is lost in the forest of your meat.  With his green monster.assaulted by multiple angles and dwarfed in size he whimpers softly.  \"<i>Ain't never felt nothin' like this.  Almost like fuckin' a dick.</i>\"");
        // (else If single cock)
        else DisplayText("Your " + Desc.Cock.describeMultiCockShort(player) + " bucks against Ro'gar's orcish pride.  The pair of hard cocks press tightly together as you dwarf Ro'gar in size.  You hump his orc sex with your " + Desc.Cock.describeMultiCockShort(player) + " and grunt in strain; your hands reach down to grip the pair of manhoods tightly, jerking them fast with squeezed pumps.");
        // (If player has balls)
        if (player.torso.balls.quantity > 0) DisplayText("  Ro'gar squirms under you, his chest heaving with labored breath while his musky scrotum presses against your own.");
        DisplayText("  He reaches down, sweaty palm wrapping around his meaty length and roughly choking it, then strokes along his orchood hastily as he groans out, spunk spilling over " + Desc.Cock.describeMultiCockSimpleOne(player) + " and leaving it sopping wet with hot seed.  Your own hands rub Ro'gar's strong smelling sperm over yourself, coating your " + Desc.Cock.describeMultiCockShort(player) + " in his lust.  The sight of the heaving orc beneath you drives you over the edge,and your pent-up load splatters over Ro'gar's sweaty body and covers the orc in your thick volleys of cum.");
        DisplayText("\n\n");
    }
    // ((If cock is between 18 and 4 inches))
    else {
        DisplayText("\"<i>E-easy there.</i>\"  Ro'gar actually stutters, looking at you with concern. Your lips curl into a devious grin as you roughly press into Ro'gar's puckered hole.  You slide in with an abrupt grunt, ignoring Ro'gar's yelps of pain at the sudden entry and reasoning that it couldn't have hurt too much, judging by how your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " was literally coated in pre-cum.  But still, you find it amusing to push the orc's button and make him squirm for once.  Slipping in further, you bury yourself in Ro'gar's hole until he groans out and prematurely spurts more orc cum onto his stomach.");
        // [(If using feline cock)
        if (player.torso.cocks.get(0).type === CockType.CAT) DisplayText("  You grind around in his green pucker, the barbs working over Ro'gar's inner walls.  \"<i>W-what in tarnation?</i>\" Ro'gar grunts as you watch him squirm under you.  You buck your hips, digging deeper into orcflesh with each thrust, his muscled ass tensing with each slam of your hips.");
        // (If horse cock)
        else if (player.torso.cocks.get(0).type === CockType.HORSE) DisplayText("  The flared head of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " nestles in Ro'gar's hole.  His breath quickens as you push in, the girthy, flared head stretching Ro'gar's insides impossibly.  You simply thrust and buck you way deeper within the green pucker to force more of your lengthy shaft past that olive rosebud each time.\n\n");
        // (If dog cock)
        else if (player.torso.cocks.find(Cock.HasKnot)) DisplayText("  Your pointed tip slips in easily and you begin to plunge into Ro'gar's depths, and the burly orc gives satisfied grunts as his strong hand reaches down to touch himself.  You begin to fill his orc hole, the pre-slicked cock working as a lubing tool as you begin to build a nice rhythm within Ro'gar.  Your knot bulges as your climax approaches and begins to ram into Ro'gar's green pucker, the tight ring unable to allow entry as you swell bigger and bigger.");
        // (If other cock)
        else DisplayText("  Breaking Ro'gar's orc hole open, you thrust deep within his hot, tight depths.  The burly orc pants with heated breaths, humid air fluttering out of his tusked mouth.  His musclegut heaves, each breath punctuating the thrusts of your hips.  You plow his ass roughly, pulling out until only the head remains inside him before plunging back in further than before.  Ro'gar grunts low in his chest with each bit of progress made, his strong, calloused hand stroking his drooling cock.  \"<i>Mmmf, " + player.desc.short + ",</i>\" he groans as you finally hilt yourself within him.");
        // end racialdicks, still inside right fit path

        DisplayText("\n\nYou grip his hips and begin a steady pace, slapping flesh loudly against his muscled ass.  His thick, powerful legs twitch in the air as you pound into his orcish hole, and the orc's toes curl visibly with each brutal thrust you make into him.  You groan as his musk makes your " + Desc.Cock.describeMultiCockShort(player) + " throb all the more within him.  To further his humiliation, you whisper in his pointed ear how good he feels wrapped around your length.  He blushes a lime green before you make him groan out your name with a rough smack over his ass.");
        if (player.torso.cocks.count > 2) DisplayText("  Your other cocks grind over that firm orc rump, drooling over his skin and making his rear glisten with your sex.");
        else if (player.torso.cocks.count > 1) DisplayText("  Your other cock grinds over that firm orc rump, drooling over his skin and making his rear glisten with your sex.");
        DisplayText("You decide to see how loud you can make him moan and begin ramming deep within him.  The thrusts slam hard enough to make Ro'gar's back arch, your bulbous cockhead prodding his buried prostate with each hilt.  His big hand beats furiously at his massive orc cock, strained groans bellowing out loudly as his heavy sack clenches tight against the base of his green rod.  Orc spunk flies out over his chest, hitting his chin and splattering over his heaving pecs.  His cum collects between his bulging muscles and splashes over his head.  Ro'gar's anal walls clench down tight around you as you pick up the pace of your brutal fuck; your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " is milked hard as you feel your climax rapidly approaching.  Sweat drips from your body onto Ro'gar's skin, and you finally feel yourself hit the edge.  With a burst of ecstasy you hilt completely within your orc fuck toy.");
        // [(If dog cock)
        if (player.torso.cocks.find(Cock.HasKnot)) DisplayText("  Your knot forces past Ro'gar's tight, anal ring to pulse inside him, joining the pair of you together.");
        // [(if player has balls)
        if (player.torso.balls.quantity > 0) DisplayText("  With short jerks of your hips, you begin to unload your pent up orbs into the orc's innards.");
        DisplayText("  Wave after wave of thick seed spills from your cock into Ro'gar as his head falls back at the blissful sensation.");
        // [(if multiple cock)
        if (player.torso.cocks.count > 1) DisplayText("  The other meat rubbing over his olive skin outside adds to the coating of spunk, making a giant mess of Ro'gar's body.  In seconds he resembles a well-paid whore beneath your " + Desc.Cock.describeMultiCockShort(player) + ".");
        DisplayText("\n\n");
    }
    // end right fit path
    // ((Combine too big, and right size paths))
    DisplayText("Ro'gar's cum-coated body lies panting under you.  You slowly soften and lay limp against his olive skin, then pull away and clean yourself off.  Ro'gar stirs slowly, only rising to a seated position by the time you are dressed.  \"<i>That was really... something.  I don't really know what got inta ya, " + player.desc.short + ", but ya really know how ta surprise a feller.</i>\"  With a sly smirk you intone that he had it coming and finish putting on your clothes, then walk over to smack Ro'gar's cum-covered ass with your hand.  The giant orc looks stunned a moment before giving a playful grin.  \"<i>Frisky, aren't'cha?</i>\"  He snickers low in his rumbling chest, then gets up to wave you off, but you're already out the door and heading back into the city with a spritely skip added to your step.");
    // <Lust sated>
    player.orgasm();
    player.stats.sens += -1;
    return { next: Scenes.camp.returnToCampUseOneHour };
...
// ((Scenario 3, [Drink! (sorta)]: Fun with Bro Brew(Requires one bro brew per visit, uses it and turns player into a Bro)))
function brobrobrobro(): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Rogar);
    ...
    return { choices: [["FukkinGrabIt", takeDatBroBrewFromDaBigMeanOlOrc], ["ReversePsych", rogarIsDumb]] };
}
// [reverse psychology]
function rogarIsDumb(): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Rogar);
    ...
    DisplayText("The playful mood broken, you hang out for a while and shoot the breeze, then head back.");
    // end
    return { next: Scenes.camp.returnToCampUseOneHour };
}

// [Fukkin' Grab It!]
function takeDatBroBrewFromDaBigMeanOlOrc(player: Character): NextScreenChoices {
    rogarFlags.ROGAR_FUCKED_TODAY = 1;
    player.inventory.items.consumeItem(ConsumableName.BroBrew);
    ...
    DisplayText("Leaving Ro'gar's apartment you wander out into the streets.");
    if (!player.perks.has(PerkType.BroBody)) {
        ...
        if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 1) {
            if (player.torso.chest.count > 1) {
                while (player.torso.chest.count > 1) {
                    player.torso.chest.remove(player.torso.chest.count - 1);
                }
            }
            player.torso.chest.get(0).rating = 0;
            player.torso.chest.get(0).nipples.count = 1;
            player.torso.chest.get(0).nipples.fuckable = false;
            if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].nipples.length > .5) player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].nipples.length = .25;
            player.torso.chest.get(0).lactationMultiplier = 0;
        }
        if (player.torso.cocks.count > 0) {
            if (player.torso.cocks.get(0).length < 10) {
                if (player.torso.cocks.get(0).thickness < 2.75) {
                    player.torso.cocks.get(0).thickness = 2.75;
                }
                cocked = true;
        ...
        if (player.femininity > 0) {
            Mod.Body.displayModFem(player, 0, 100);
        }
        // max tone.  Thickness + 50
        if (player.tone < 100) {
            Mod.Body.displayModTone(player, 100, 100);
        }
        if (player.thickness < 100) {
            Mod.Body.displayModThickness(player, 100, 50);
        }
    ...
    if (changed) {
        DisplayText("  You realize you look much stronger, bigger, and sexier");
        // (if dick grew)
        if (cocked) DisplayText(", and you smile to yourself as your now-larger endowment rubs against garments not suited to it");
        DisplayText(".  Who needs book smarts anyway?  Nerds.");
        // lose lust, gain bro traits, slimefeed
        player.perks.add(PerkType.BroBrains);
        player.perks.add(PerkType.BroBody);
...
function kinkyWithDaOrc(player: Character): NextScreenChoices {
    rogarFlags.ROGAR_FUCKED_TODAY = 1;
    DisplayText().clear();
    DisplaySprite(SpriteName.Rogar);
    DisplayText("You nod eagerly, and the orc rewards you with another caress.  \"<i>Well then... let's head on out,</i>\" he says.  The two of you walk down the desert city's streets until you reach a discrete, out-of-the-way building labeled 'Steel Hogan Apartments'.  \"<i>This here's where I'm stayin',</i>\" Ro'gar says, handing you through the doorway.  He leads you down a hallway to his own lodgings, then pushes open the door for you; you step ");
    ...
    DisplayText("over the threshold, and then look around the apartment.  He points out the small kitchen area and then, rather sheepishly, the bedroom.  You're examining it when something catches your eye...\n\n");
    DisplayText("You bend over the bed to grip a small bottle lying in the soft fabric of the sheets.  Turning it over, you tilt your head as you read the label.  It's... a bottle of lubricant.  Suddenly, there is a loud thud behind you and the bottle drops from your hand as your torso is pressed into the cushioned surface of the bed.  Something presses into your back.  \"<i>Gotcha...</i>\" the familiar voice snickers in your ear.  Ro'gar's head leans over your shoulder and a long, orcish tongue licks your cheek.  \"<i>Was waitin' for this...</i>\" he purrs; the resonance of his voice rumbles against your ear.  Ro'gar chuckles and grips around your wrists.  \"<i>Now that yer here, it's time ta break in this here new bed with sumthin' I've been cravin' since I washed ya down in that swamp, boy.</i>\"  Ro'gar's tone suddenly takes a demanding, hissing tone.  His tongue flicks over your ear as he speaks.  Your skin begins to grow goosebumps as shivers crawl down your spine.\n\n");
    DisplayText("He yanks your wrists behind your back, ignoring your faint gasps.  With a rustle of cloth, your wrists are bound together behind your back tightly; your fingers feel over the bonds but you struggle against them in vain.  Looking over your shoulder in ");
    if (player.stats.lib < 33) DisplayText("feigned confusion ");
    else if (player.stats.lib > 66) DisplayText("clear lust ");
    DisplayText("at Ro'gar, you only see his lust-ridden face dipping down to kiss at your lips.  His tusks press against your cheeks with a demanding snarl and his tongue into your mouth; your " + Desc.Butt.describeButt(player) + " receives a smack, forcing you to yelp.  His tongue quickly invades, and his free hand unfastens your " + player.inventory.equipment.armor.displayName + " to strip you forcefully.  Rough, calloused fingers dance over your body and fasten onto your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s, coaxing moans from your throat with each pinch.  Your pleased noises are muffled by Ro'gar's mouth, his tongue and lips hushing each whimper and gasp you breathe.  A bare, firm chest presses against your exposed back, and you pant as the heat from his body burns against you, only intensified by the thin layer of oil on his skin.  Ro'gar's blue eyes watch you like a predator as your face flushes.  He breaks the kiss and cups his hands under your chin to hold your face in place.  \"<i>Such a perdy face yer makin', " + player.desc.short + ".  Yer lookin' at me like a proper slut, an' it's makin' me hard seein' ya so needy to be bred.</i>\"  He growls in his throat as his mouth nips over your neck, leaving red marks along your skin where his tusks rub against you.  You can feel the oil and sweat smearing over your back, and your senses go wild.  The room is soon filled with your needy whimpers as " + Desc.Cock.describeMultiCockSimpleOne(player) + " oozes and throbs against the sheets, making the fabric damp with your lust.\n\n");
    ...
    if (player.torso.butt.looseness < 3) DisplayText("\"<i>Tarnation, " + player.desc.short + ".  You're so tight...</i>\"\n\n");
    else if (player.torso.butt.looseness < 5) DisplayText("\"<i>My favorite boy ready for the ride of his life?</i>\"\n\n");

    DisplayText("Ro'gar rolls you over by your " + Desc.Hip.describeHips(player) + ", forcing your body to lay out over your back.  The orc's face comes down close enough that you feel his breath washing over your skin.  With a rip you hear the shorts torn from Ro'gar's body.  You feel his hot, throbbing length grinding over your " + Desc.Butt.describeButt(player) + " before you see it, the deep green head towering over your groin menacingly, each vein pulsing as the mammoth cock belches pre-cum to leak down the orcish shaft.  Your skin is soon slick with his spurting pre as he groans out. \"That's much better.\"  Ro'gar's breath becomes labored as he pants heavily and eagerly, his green length humping against your rosebud, making you moan out in an excess of sensation.  His hands hold your cheeks apart with a strong grip as you begin to leak your own pre-cum onto your stomach.");
    // (if player has balls)
    if (player.torso.balls.quantity > 0) DisplayText("  Ro'gar's dark green cockhead rubs over your " + Desc.Balls.describeBalls(true, true, player) + " and coats the plump sack in his musky goo, like an alpha male marking his territory.");
    DisplayText("\n\n");

    DisplayText("Deep, guttural hisses echo out from the orc.  \"<i>Easy fella...</i>\" Ro'gar mutters, trying to restrain himself from ripping you apart with his fat cock.  He leans down and kisses you softly before continuing.  \"<i>I'll give ya a kinky fun ya won't forget.</i>\"  With a playful wink he gets down on his knees, squatting near the foot of the bed.  His hand hoists your " + Desc.Hip.describeHips(player) + " up to his mouth.  You can feel him kiss your tight ring, and soon a warm sensation shoots up your spine, causing you to moan.  His hot, slick tongue swirls over your puckered flesh, slathering your hole while his tusks rub along your cheeks.  You're left panting out hot, needy breaths as Ro'gar works over your anus, his tongue gently easing your tight ring of flesh open.  The pleasure makes your legs go limp over his shoulders and your eyes shut in a blissful daze.  Ro'gar's wet slurps and growls make your ears twitch, and you feel him slowly press in with his orcish tongue, beginning to tonguefuck your entrance forcefully.  With patience, your pucker gives in under Ro'gar's touch and his tongue slides into your untouched confines.  You give a sharp gasp, this strange feeling overwhelming you as you flick your eyes open.  Your " + Desc.Butt.describeButt(player) + " wriggles in pleasure around his tongue, and the orc gives a pleased grunt as his tongue swirls around your inner walls, sliding out to pull your lime flavor into his mouth before roughly thrusting back in.  Your back arches as you cry out a surprised moan.");
    if (player.hasLongTail() || player.torso.hips.legs.isNaga()) DisplayText("  Your tail wraps around Ro'gar's arm for some sort of stability, the tip flicking lazily as it coils and squeezes around a bicep.");
    DisplayText("\n\n");
    ...
    if (player.torso.butt.looseness < 3)
        DisplayText(" with some resistance, but the relentless finger rubs over your walls as he begins to pull your rim open, forcing another finger inside and making you bite your lip from the stinging sensation.  Working the pair of fingers together inside of you, he slowly moves them apart and stretches your hole wide.");
    else if (player.torso.butt.looseness < 5)
        DisplayText(" and rubs over your walls as he pulls your rim open, forcing another finger inside and making you moan like a bitch in heat.  Working the pair of fingers together inside of you, he slowly moves them apart and stretches your hole wide.");

    DisplayText("  Fingers and " + Desc.Leg.describeFeet(player) + " curl in pleasure as your boycunt is forced open by Ro'gar's steady coaxing.");
    ...
    if (player.torso.butt.looseness < 3) DisplayText("the tight hole.  Your teeth clench as he stretches you wide around his length, though he continues without hesitation, and finally you cry out with the sting of the bulbous head squeezing into your tiny hole, though it falls on deaf ears as Ro'gar merely groans at your tightness.");
    else if (player.torso.butt.looseness < 5) DisplayText("your well-used hole. You moan with need as he stretches you wide around his length, continuing without hesitation until he's inside.");
    Mod.Butt.displayStretchButt(player, 25, true, true, false);
    ...
    DisplayText("You can't help but focus on the wonderful feeling of Ro'gar's cock throbbing within you.  Maybe this is where you belong?  With your ass impaled on Ro'gar's massive tool?  You moan loudly as Ro'gar picks up the pace, hips ramming into you with enough force to shift your body over the sheets.  He really starts to pound your hole with huffed grunts and groans as he hilts in you; his cockhead prods your prostate with each thrust, causing you to cry out as electric shocks shoot up your spine.  Every nerve in your body lights up like a Christmas tree and make your entire body squirm with pleasure.  You feel spurts of cum begin to leak from your " + Desc.Cock.describeMultiCockShort(player) + " and glaze your belly with pre-cum.");
    if (player.hasLongTail() || player.torso.hips.legs.isNaga()) DisplayText("  Your tail, still gripping Ro'gar's arm, squeezes tight.");
    // (If player has balls)
...
function rubdubWithDaOrcWithoutDucky(player: Character): NextScreenChoices {
    rogarFlags.ROGAR_FUCKED_TODAY = 1;
    DisplayText().clear();
    DisplaySprite(SpriteName.Rogar);
    DisplayText("By the time you reach Ro'gar he has already reached his bedroom, his shorts on the floor. You eye over his olive green skin, all of it laid bare before you including his limp orc cock hanging over his hefty nutsack.\n\n");
    DisplayText("\"<i>I purchased these oils to relax my muscles after spending so much time in the gym.</i>\" Your orc lover motions to a few bottle of oils on his night stand. \"<i>How about you give me a hand... or two.</i>\" Ro'gar grins at you, his eyebrows twitching suggestively. You need no further provocation as you strip from your " + player.inventory.equipment.armor.displayName + ", you let it fall into a pile on the floor as you make for the bottle. Ro'gar grins and rolls onto his stomach.\n\n");
    ...
    DisplayText("Without any hesitation you straddle the orc's hips again, his engorged orc cock brushing against the bare skin of your " + Desc.Butt.describeButt(player) + ". Your hands get to work smearing the relaxing oil over Ro'gar's body. The orc stares at you with a lurid gaze as you massage his bulging pectorals, teasing his darkened nipples with your fingers. You grin feeling Ro'gar's thick cock pressing more against your " + Desc.Butt.describeButt(player) + " as his chest rumbles under your hands.\n\n");
    DisplayText("\"<i>You're good at this " + player.desc.short + ", have you done this before?</i>\" Ro'gar purrs as you work over his abdominals, tracing over every gap with your fingertips, the oils glistening over your lover's body. Ro'gar's hips buck gently, bumping your " + Desc.Butt.describeButt(player) + " with his throbbing cock with a chuckle, his mouth curls into a coy smile. You watch as Ro'gar reaches his arms back, resting his head on his hands as he bares his underarm and pits.\n\n");
    // [Play with pits][Ignore pits]
    return { choices: [["Play with pits", sexyOrcPitsAreSexy], ["Ignore pits", lickSomeMoreOrcButNoPits]] };
}
// [Play with pits]
function sexyOrcPitsAreSexy(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Rogar);
    DisplayText("You lean forward enticed by Ro'gar's arm pits. The smile on his lips grows broader as your tongue sinks into his musky sweat centers. You can feel his fat orc dick drooling precum as you lap your tongue over the wet olive skin.\n\n");
...
// [Ignore pits]
function lickSomeMoreOrcButNoPits(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Rogar);
    DisplayText("With a tilt of your head you press your mouth to Ro'gar's. His lips part for yours and your tongue dances with his, his hand stroking your sides as he growls low in need into your mouth. His hot lips and tongue pressing hard to your own. He tries to dominate your mouth, his tongue wrestling with yours to push it where he wills. His teeth catch and nip your bottom lip with a low possessive growl.\n\n");
    DisplayText("\"<i>So skilled with your mouth and hands, care to give me a good finish? I'll be sure to tip.</i>\" He winks playfully. With a smile on your face you back up a bit, dragging your " + Desc.Cock.describeMultiCockShort(player) + " over his own. Abruptly Ro'gar blurts out a moan, his breath labored as his fat green dick humps your own " + Desc.Cock.describeMultiCockShort(player) + ". An idea fresh in your mind as you hump back, driving the orc male wild as he grunts and pants. His slime and oil slick body madly rubbing over your own. A gasping and moaning mess of muscle and orc flesh, he is left writhing under you, his mouth hung open in needy groans desperate for your touch, your warmth, your body.\n\n");
