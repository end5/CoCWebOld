...
((Scenario 4, [Frot]))*/
	...
	public encounterRogarSwamp(): void {
		...
		if (Flags.list[FlagEnum.ROGAR_PHASE] === 0) {
			...
			//((Corruption 70 or more))
				...
				if (player.torso.hips.legs.isNaga() || player.torso.tailType > TailType.NONE) DisplayText("  Your tail flicks about happily.");
		...
		//((Ro'gar phase = 1)) (edited)
			...
			if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2 && player.stats.lib >= 50) {
				...
				if (player.stats.int <= 30) {
					...
					if (player.torso.vaginas.count > 0) DisplayText("  When you witnessed him make that other beast submit with strength alone it created a demanding need in your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " during the match, and the memory is doing much the same now.");
					//((if player has cock(s))
					if (player.torso.cocks.count > 0) DisplayText("  You feel " + CockDescriptor.describeMultiCockSimpleOne(player) + " shift as you remember how hard you got from behind the tree, finding yourself absent mindedly stroking yourself with an open palm as pre-cum soaks through your clothing.");
		...
		//Not sure what determines this yet
			...
			//((androgynous or masculine and breasts <= A-cup))
				...
				//(Dirt Mc Girt = 2)
				if (Flags.list[FlagEnum.ROGAR_DIRT] === 2) DisplayText("\"<i>" + player.short + ", y'all took off on me real quick like,</i>\" he says through a snicker.  The joke falls flat and the orc seems to blush lime green as you stare at him uncomprehendingly.  He rubs the back of his neck with a hand before continuing.  \"<i>Sorry about that.  I was plumb tuckered out afterwards.</i>\"\n\n");
		...
		if (Flags.list[FlagEnum.ROGAR_PHASE] === 0) {
			...
			//((If libido is 50 or more))
				...
				if (player.torso.vaginas.count > 0) {
					DisplayText("  Your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " starts to quiver as the warm water rolls over its lips, moistening in excitement as your skin is rubbed over by strong hands.  Your chest heaves as it relaxes to his touch, your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + "s becoming stiff under his rubbing.");
					DisplayText("  Even your " + LowerBodyDescriptor.describeFeet(player) + " and fingers curl as you smell the sweat and musk of Ro'gar's closeness, the animalistic scent arousing you.");
				}
				else if (player.torso.cocks.count > 0) DisplayText("  " + CockDescriptor.describeMultiCockSimpleOne(player, true) + " stirs to life, making it obvious how much you are enjoying the rub down, then begins to drool with lust.  Through lidded eyes you faintly make out Ro'gar's toothy smirk becoming a hungry grin.");
	...
	private sexyChoices(): void {
		DisplaySprite(86);
		MainScreen.simpleChoices(["Ewww", "NoThnxBro", "Lick Clean", "", ""], [ewwwRogarIsGay, noSlowBroIDontWantPokeSex, okayBroLetsHaveAGayCarwash, null, null]);
	}
	//((Ewww))
	...
	private okayBroLetsHaveAGayCarwash(): void {
		...
		if (player.torso.cocks.count > 0) DisplayText(CockDescriptor.describeMultiCockSimpleOne(player));
		else if (player.torso.vaginas.count > 0) DisplayText("your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)));
		else DisplayText("your " + ButtDescriptor.describeButthole(character.torso.butt));
	...
	public rogarThirdPhase(): void {
		...
		//((androgynous or masculine and breasts <= A-cup))
			...
			//else if Ro'roh Raggy < 1 and player height < 5 feet)
				...
				DisplayText("you gently tug on the figure's cloak, mentioning aloud that it seemed to have held up all right with your sewing job.  The figure's back straightens sharply and a deep snicker is heard.  \"<i>Ya did a mighty fine job, " + player.short + ".  Been waitin' for ya.</i>\"  Ro'gar turns and grins at you.\n\n");
			}
			//((Paths combine here))
			...
			//(else if Dirt Mc Girt = 2)
				...
				if (player.gender === Gender.FEMALE || player.gender === Gender.NONE) {
					...
					DisplayText("  " + CockDescriptor.describeMultiCockSimpleOne(player, true) + " gets a caress from him as he explores, arousing it to mild stiffness.  \"<i>" + player.short + ",</i>\" he grunts.  \"<i>Ah hope ah'll be seein' more of ya.</i>\"  Ro'gar breaks the contact and tips back the remains of his drink while you ");
					...
					if (player.hasLongTail() > 0 && player.torso.hips.legs.isNaga()) DisplayText(", and you feel your tail flicking rapidly in excitement");
					DisplayText(".\n\n");
	...
	public rogarPhaseFour(): void {
		...
		if ((player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2) && Flags.list[FlagEnum.ROGAR_WARNING] === 0) {
			...
			if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2) DisplayText(" and your " + Desc.Breast.describeChest(character));
		...
		//(high fem or breasts >=B-cup and Ro'roh Raggy =1)
		else if ((player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 2) && Flags.list[FlagEnum.ROGAR_WARNING] === 1) {
		...
		//(andro or masculine with tits <= A)
			...
			DisplayText("Ro'gar is tipping his drink back when he catches sight of you, and waves you over.  \"<i>Ah, good ta see ya, " + player.short + "!");
			//[(Ro'roh Raggy = 1)
			if (Flags.list[FlagEnum.ROGAR_WARNING] === 1) {
			...
			if (player.gender === Gender.FEMALE || player.gender === Gender.NONE) {
				...
				DisplayText("  " + CockDescriptor.describeMultiCockSimpleOne(player, true) + " trembles under his strong touch, as he increases the pressure and looks into your eyes meaningfully.  ");
				//[(if Dirt Mc Gurt flag = 1)
				if (Flags.list[FlagEnum.ROGAR_DIRT] === 1) DisplayText("</i>\"I know ya probably ain't ever imagined hangin' out with a feller like me, but... I could really use the company tonight.</i>\"  ");
	...
	private loseButtGinity(): void {
		...
		if (player.torso.butt.looseness === 0) {
			...
			DisplayText("  Fingers and " + LowerBodyDescriptor.describeFeet(player) + " curl in pleasure as your previously untouched boycunt is forced open by Ro'gar's steady coaxing.");
			...
			player.displayStretchButt(25, true, true, false);
			...
			DisplayText("Your mind races with Ro'gar's words as they seemed to be coming true.  You can't help but focus on the wonderful feeling of Ro'gar's cock throbbing within you.  Maybe this is where you belong?  With your tight ass impaled on Ro'gar's massive tool?  You moan loudly as Ro'gar picks up the pace, hips ramming into you with enough force to shift your body over the sheets.  He really starts to break in your hole with huffed grunts and groans as he hilts in you; his cockhead prods your prostate with each thrust, causing you to cry out as electric shocks shoot up your spine.  Every nerve in your body lights up like a Christmas tree and make your entire body squirm with pleasure.  You feel spurts of cum begin to leak from your " + CockDescriptor.describeMultiCockShort(player) + " and glaze your belly with pre-cum.");
			if (player.hasLongTail() > 0 || player.torso.hips.legs.isNaga()) DisplayText("  Your tail, still gripping Ro'gar's arm, squeezes tight.");
			//(If player has balls) 
		...
		//(([Get Anal] Scenario 1b: Get fucked (normal)))
			...
			if ((player.thickness >= 75 && player.tone < 33) || player.tone >= 75) {
				DisplayText("Riding Ro'gar's stiff manhood as fast as you can muster, you drive the orc man wild with need.  His thick girth stretches your " + ButtDescriptor.describeButthole(character.torso.butt) + " wide around it, making you a wonderful sleeve for his orc cock.  He hilts within you, and you cry out in pleasure as his cockhead rams your prostate.  Ro'gar wrests control from you as he hears you cry out like a bitch in heat.  His strong hands at your " + LowerBodyDescriptor.describeHips(player) + " hold you in place as he pounds your " + ButtDescriptor.describeButthole(character.torso.butt) + ", ramming against you with brutal force and making you moan loud enough that your voice grows a little hoarse.  " + CockDescriptor.describeMultiCockSimpleOne(player, true) + " twitches from the abuse to your trigger");
				if (player.torso.balls.quantity > 0) DisplayText(" and your " + BallsDescriptor.describeBalls(true, true, player) + " clench tight as you feel your limit reached");
				DisplayText(".");
				//[(if player has toes)
				DisplayText("  You curl your fingers and " + LowerBodyDescriptor.describeFeet(player) + " in ecstasy as Ro'gar drives you over the edge.");
				DisplayText("  Your bucks become ragged, and the orc takes over as " + CockDescriptor.describeMultiCockSimpleOne(player) + " jerks and spasms, shooting out countless ropes of sticky spunk onto the glistening green skin, painting the orc white with your lust.  With a low, deep, tone, Ro'gar groans out as you feel his warmth overtake your lower body, flooding you full of his fertile man cream.  Ro'gar continues to fuck you with decreasing vigor, slowing to a stop as his spent cock pops from your drooling pucker.  His spunk floods out of your filled boycunt, the spooge running down Ro'gar's half-hard cock to drip onto the floor lewdly.\n\n");
			}
			//(else)
			else {
				DisplayText("\"<i>Enough horsin' around.</i>\"  Ro'gar snarls as his arms quickly yank back, sneaking under your legs and hoisting you up as he gets to his feet.  You grip Ro'gar's shoulders tight as he lifts you with a strained grunt.  You delight with sharp little moans as he begins to fuck you skyward, bouncing you off of his hips with each slam of his cock into you.  Slowly Ro'gar steps towards a wall, taking his time to be sure not to drop you, and you soon feel the cool surface pressed to your back.  Caught between a cock and a hard place, you stare into Ro'gar's predatory gaze as he holds you hoisted in the air and pinned, making you feel vulnerable.  The orc takes advantage of you and begins to abuse your " + ButtDescriptor.describeButthole(character.torso.butt) + ", making your head tilt back to moan out helplessly.");
				DisplayText("  " + CockDescriptor.describeMultiCockSimpleOne(player, true) + " lays out over your chest, dribbling pre-cum on your skin as you make a mess of your torso in lust.");
				//[(if player has tail) 
				if (player.hasLongTail() > 0 || player.torso.hips.legs.isNaga()) DisplayText("  Your tail flicks helplessly below you, urgently wrapping around Ro'gar's thigh and trying to pull him deeper into you.");
				...
				//[(If player has balls) 
				if (player.torso.balls.quantity > 0) DisplayText("  You feel your " + BallsDescriptor.describeBalls(true, true, player) + " pull up tight in your sack as your labored breath comes out in pants.");
	...
	private fuckRogarsButtPussyBoyCuntManMoundSissySlitQueerQuim(): void {
		...
		//((If cock is between 18 and 4 inches))
			...
			//(If horse cock)
			else if (player.torso.cocks.get(0).type === CockType.HORSE) DisplayText("  The flared head of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " nestles in Ro'gar's hole.  His breath quickens as you push in, the girthy, flared head stretching Ro'gar's insides impossibly.  You simply thrust and buck you way deeper within the green pucker to force more of your lengthy shaft past that olive rosebud each time.\n\n");
			//(If dog cock)
			else if (player.torso.cocks.filter(Cock.HasKnot).length > 0) DisplayText("  Your pointed tip slips in easily and you begin to plunge into Ro'gar's depths, and the burly orc gives satisfied grunts as his strong hand reaches down to touch himself.  You begin to fill his orc hole, the pre-slicked cock working as a lubing tool as you begin to build a nice rhythm within Ro'gar.  Your knot bulges as your climax approaches and begins to ram into Ro'gar's green pucker, the tight ring unable to allow entry as you swell bigger and bigger.");
			//(If other cock)
			...
			//[(if multiple cock)
			if (player.torso.cocks.count > 1) DisplayText("  The other meat rubbing over his olive skin outside adds to the coating of spunk, making a giant mess of Ro'gar's body.  In seconds he resembles a well-paid whore beneath your " + CockDescriptor.describeMultiCockShort(player) + ".");
			DisplayText("\n\n");
		}
		//end right fit path
	...
	private brobrobrobro(): void {
		...
		MainScreen.simpleChoices(["FukkinGrabIt", "ReversePsych", "", "", ""], [takeDatBroBrewFromDaBigMeanOlOrc, rogarIsDumb, null, null, null]);
	}
	//[reverse psychology]
			...
			if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 1) {
				if (player.torso.chest.count > 1) {
					while (player.torso.chest.count > 1) {
						player.removeBreastRow(player.torso.chest.count - 1, 1);
					}
				}
	...
	private frotWithRogar(): void {
		...
		//(If single cock)
			...
			DisplayText("Your cock presses tight against Ro'gar's fat orc cock. He growls low as he grinds himself over you, his thick, globbing pre-cum drooling onto your " + CockDescriptor.describeMultiCockShort(player) + ".  Muffled groans escape Ro'gar's mouth as you kiss him roughly, tongues locked all the while in writhing combat.  ");
			//[(If player has balls)
			if (player.torso.balls.quantity > 0) DisplayText("Your " + BallsDescriptor.describeBalls(true, true, player) + " slap audibly against Ro'gar's heavy green sack.  ");
	...
	private kinkyWithDaOrc(): void {
		...
		DisplayText("  Fingers and " + LowerBodyDescriptor.describeFeet(player) + " curl in pleasure as your boycunt is forced open by Ro'gar's steady coaxing.");
		...
		DisplayText("You can't help but focus on the wonderful feeling of Ro'gar's cock throbbing within you.  Maybe this is where you belong?  With your ass impaled on Ro'gar's massive tool?  You moan loudly as Ro'gar picks up the pace, hips ramming into you with enough force to shift your body over the sheets.  He really starts to pound your hole with huffed grunts and groans as he hilts in you; his cockhead prods your prostate with each thrust, causing you to cry out as electric shocks shoot up your spine.  Every nerve in your body lights up like a Christmas tree and make your entire body squirm with pleasure.  You feel spurts of cum begin to leak from your " + CockDescriptor.describeMultiCockShort(player) + " and glaze your belly with pre-cum.");
		if (player.hasLongTail() > 0 || player.torso.hips.legs.isNaga()) DisplayText("  Your tail, still gripping Ro'gar's arm, squeezes tight.");
		//(If player has balls) 
