...
export class MaleSpiderMorphScene {
	...
	public defeatSpiderBoy(): void {
		...
		if (player.torso.cocks.count > 0) {
			...
			if (player.torso.cocks.biggestCocks[0].area > monster.analCapacity()) frot = victoryFrotTheSpoidah;
		}
	...
	private victoryButtFuck(): void {
		...
		DisplayText("  With a wide smile on your face, you grab a handful of his pale butt-cheek and squeeze your fingers into the soft flesh.  It spreads his dainty derriere just wide enough for you to see the puckered star of his rectum.  " + CockDescriptor.describeMultiCockSimpleOne(player, true) + " ");
		...
		DisplayText("  Meanwhile, you grab " + Desc.Cock.describeMultiCockSimpleEach(character) + " and line it up with the spider-boy's twitching rectum, rubbing the " + player.cockHead(x) + " over the tight little hole while you wait for it to slowly loosen.  A fresh dollop of pre-cream bubbles out of his cum-frothing foreskin in response, letting you know that in spite of his protests, your reluctant lover's body loves the anal stimulation.\n\n");
	...
	private victoryFrotTheSpoidah(): void {
		...
		DisplayText(".  You sway your " + LowerBodyDescriptor.describeHips(player) + " to drag your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " all over his body, and the defeated boy's pale white skin slowly colors pink with a full-body blush.\n\n");
		...
		DisplayText("You hump forward and smash your " + player.cockHead() + " against the spider-boy's face, popping a bubble of pre-cum on his lips before you pull back far enough to reveal the soaked foreskin that hides his cock-tip.  He's probably filled the entirety of that little flesh-balloon with his leavings, and as you slide back up you feel more of his pre-cum bursting out, confirming your suspicions.  A pair of hard, chitinous arms encircle your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " in a tight hug, squeezing into a slippery pleasure-vice for your penis.\n\n");
	...
	private victoryCowgirlRidingOnSpiderBoi(): void {
		...
		DisplayText("It takes some time to regain your strength with all the orgasmic aftershocks rolling through your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ".  Once you've mastered yourself, you climb off your conquest and get dressed, ");
		...
		else DisplayText("paying no heed to the thick flows of spunk that drip down your " + LegDescriptor.describeLegs(player) + ".");
		player.slimeFeed();
		//Pregnancy Goes Here
	...
	private spiderBoyLossMaleButtfuck(): void {
		...
		DisplayText("Finished with his foreplay, the spider-boy grabs your " + LowerBodyDescriptor.describeHips(player) + " and raises your ");
		if (player.torso.hips.legs.isTaur()) DisplayText("hindquarters slightly");
		else DisplayText(LegDescriptor.describeLegs(player) + " up and out of the way");
		DisplayText(", displaying surprising strength as he holds your " + ButtDescriptor.describeButthole(character.torso.butt) + " at waist height.  You look up at the gloating arachnid ");
		...
		DisplayText("  His only response is to squeeze tight on your " + ButtDescriptor.describeButt(player) + " and press forward until his pre-cum-soaked foreskin is glazing your rim in preparation for the coming penetration.  ");
		if (player.stats.lust > 99) DisplayText(CockDescriptor.describeMultiCockSimpleOne(player, true) + " bounces atop your belly, reacting strongly to the tingling sensation radiating from your " + ButtDescriptor.describeButthole(character.torso.butt) + ".");
		else {
			...
			DisplayText(", reacting strongly to the tingles radiating up from your " + ButtDescriptor.describeButthole(character.torso.butt) + ".");
		}
		DisplayText("\n\n");
		...
		else DisplayText("quite easy due to how well-stretched your " + ButtDescriptor.describeButthole(character.torso.butt) + " is.  Watching his fat little cock disappear into your roomy backside actually brings a smile to your lips.");
		...
		DisplayText(" out from " + CockDescriptor.describeMultiCockSimpleOne(player) + " over your belly, weakly leaking out in a slow, steady flow.\n\n");
		...
		DisplayText("  Pumping his hips slowly, the spider-morph watches a few more weak trickles of seed squirt out.  He gives your ass a hearty smack and muses out loud, \"<i>That's a good " + player.mf("boy", "girl") + ".  You make a");
		...
		DisplayText(", you're absolutely drowning in a sea of bliss.  The waves of sexual indulgence threaten to drag you under their frothing crests.  Slowly, your self-control washes away, eroded by the all-encompasing pleasure " + CockDescriptor.describeMultiCockSimpleOne(player) + " is experiencing.\n\n");
		...
		if (player.cumQ() < 500) DisplayText(CockDescriptor.describeMultiCockSimpleOne(player, true) + " bounces on your belly, leaking one last drop from its tip, utterly draining every drop from your " + BallsDescriptor.describeBalls(true, true, player) + ".");
		else DisplayText(CockDescriptor.describeMultiCockSimpleOne(player, true) + " explodes again, dumping every remaining drop from your " + BallsDescriptor.describeBalls(true, true, player) + " directly onto your " + Desc.Breast.describeChest(character) + " and belly.");
	...
	private loseToSpiderBoyVagFucked(): void {
		...
		DisplayText(" you, your " + LegDescriptor.describeLegs(player) + " grow shaky, then give out entirely.  You flop down on your back");
		...
		if (player.torso.chest.hasFuckableNipples()) DisplayText("into his mouth before plunging his tongue into the tit-pussy's passage");
		...
		DisplayText("  The monstrous man handles your body with ease, playing your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + " like a finely tuned instrument, and you pant out little");
		...
		DisplayText(" mons, teasing against your vulva until your " + VaginaDescriptor.describeClit(player) + " emerges from its hood, ");
		if (player.torso.clit.length >= 3) DisplayText("frotting against him");
		else DisplayText("grinding along the underside of his dick");
		DisplayText(".  You start rocking your " + LowerBodyDescriptor.describeHips(player) + " encouragingly, trying to snare his marvelous maleness, but every time you catch his tip within your lips, he changes the angle and swivels away, teasing you.\n\n");
		...
		DisplayText(", your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + "s ");
		if (player.torso.chest.hasFuckableNipples()) DisplayText("drip their own slippery juices");
		...
		DisplayText(", ");
		if (player.torso.cocks.count > 0) DisplayText(CockDescriptor.describeMultiCockSimpleOne(player) + " becomes so full and hard it feels like it could burst, ");
		DisplayText("and your " + VaginaDescriptor.describeClit(player) + " ");
		...
		DisplayText("The rippling, fat cock rubs your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " perfectly, the foreskin dragging along your walls as soon as you start reciprocating and humping your wonderful mate's dripping dick.\n\n");

		DisplayText("You fuck him like an animal, a wanton beast lost in the throes of artificial heat.  The spider-boy hangs onto you for dear life, clutching tightly to your torso while your frenzied motions bounce him into the air on top of you, sliding his cock in and out of the quivering vice that is your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ".  Loud, wet slaps echo through the sex-musk-filled air while you put all of your body's strength and endurance into milking the spider-boy's cock.  Your thoughts vanished some time ago, replaced with hunger for orgasm and fantasies of getting an injection of hot spider-spunk.\n\n");

		DisplayText("At long last, the twin needles withdraw from your neck, dripping out the last of the spider's venom while he throws his head back, closes his eyes, and mewls quietly.  You throw your hips into him desperately, eager to catch every drop of his seed with your womb.  His cum squirts out, and blossoms of slippery warmth squirt through your cunt.  Your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " squeezes and begins to ripple, sending spasms of pleasure up your spine until you're screaming out loud, moaning and grunting in complete, utter bliss.  Absolute happiness washes through you, the pleasure that can only come from giving into your body's demands and letting this male fill you with his spunk.\n\n");
	...
	public spiderPregVagBirth(): void {
		...
		DisplayText("You shudder violently, your stomach growling fiercely at you. It feels as if something is moving around in your womb, and you bend over on all fours, raising your ass into the air as if it is the most natural thing in the world. A slimy green fluid starts leaking out of your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ", making a small puddle on the ground. The sensation is extremely pleasurable as you feel it running down your thighs. Groaning, you start to push out a small, smooth green ball, and the action makes your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " so much wetter.  You push out another, and another, each ball rubbing against your inner walls before dropping into the slimy pool of goo. After the sixth, you orgasm, ");
		//[if male/herm]
