 ...
 */
// Greeting
export function greetMaleSpiderMorph(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Spiderguy);
    ...
    if (randInt(2) === 0) {
        ...
        // - auto fight
        return CombatManager.beginBattle(player, [], [new MaleSpiderMorph()]);
    }
    // NICE GUY MODE:
    else {
        ...
        // [Fight] [Talk] [Leave]
        return { choices: [["Fight", fightSpiderBoy], ["Talk", talkToSpiderBoy]], persistantChoices: [["Leave", Scenes.camp.returnToCampUseOneHour]] };
    }
}

function fightSpiderBoy(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Spiderguy);
    return CombatManager.beginBattle(player, [], [new MaleSpiderMorph()]);
}

// Talk
function talkToSpiderBoy(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Spiderguy);
...
// *Victory Pretext:rr
export function defeatSpiderBoy(player: Character, monster: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Spiderguy);
    let mount;
    let buttfuck;
    ...
    if (player.torso.cocks.count > 0) {
        if (!player.torso.cocks.find(Cock.CockThatFits(monster.analCapacity()))) buttfuck = victoryButtFuck;
...
// Loss selector
export function loseToMaleSpiderMorph(player: Character, monster: Character): NextScreenChoices {
    let choices = 0;
    if (player.torso.cocks.count > 0) choices++;
    if (player.torso.vaginas.count > 0) choices++;
    if (player.gender === Gender.NONE) {
        DisplaySprite(SpriteName.Spiderguy);
        DisplayText().clear();
        DisplayText("The raven-haired spider-guy tears off your " + player.inventory.equipment.armor.displayName + ", and disgusted by what he finds, he kicks you in the head, knocking you out.");
        player.stats.HP = 1;
        return { next: Scenes.camp.returnToCampUseOneHour };
    }
    const select: number = randInt(choices);
    if (select === 0) return spiderBoyLossMaleButtfuck(player, monster);
    else return loseToSpiderBoyVagFucked(player, monster);
}

// *Victory Buttfucko
function victoryButtFuck(player: Character, monster: Character): NextScreenChoices {
    ...
    if (monster.stats.lust > 99) DisplayText("He's too distracted by his foreskin fondling to pay much attention to your anal-focused attentions.");
    ...
    if (monster.stats.lust <= 99) DisplayText("after he starts touching himself.");
    else DisplayText("while he fondles and caresses the sensitive flesh.");
    DisplayText("  Meanwhile, you grab " + Desc.Cock.describeMultiCockSimpleEach(player) + " and line it up with the spider-boy's twitching rectum, rubbing the " + Desc.Cock.describeCockHead(cockThatFits) + " over the tight little hole while you wait for it to slowly loosen.  A fresh dollop of pre-cream bubbles out of his cum-frothing foreskin in response, letting you know that in spite of his protests, your reluctant lover's body loves the anal stimulation.\n\n");

    DisplayText("After a little more gentle pressure and a few more gentle caresses of his testicles, the defeated male finally relaxes enough for you to put the first inch inside him.  His tunnel is so hot that it feels like your " + Desc.Cock.describeCock(player, cockThatFits) + " is going to melt inside his sweltering embrace, and much worse, his sphincter clamps down hard enough to trap your " + Desc.Cock.describeCockHead(cockThatFits) + " inside his toasty tail-hole.  Holding tightly to his ass and balls, you yank your clenching, anal slut down hard, forcing him to stretch while the rest of your " + Desc.Cock.describeCock(player, cockThatFits) + " burrows deep inside him");
...
function victoryFrotTheSpoidah(player: Character, monster: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Spiderguy);
    DisplayText("You push the ");
    if (monster.stats.lust > 99) DisplayText("masturbating");
    ...
    if (monster.stats.lust > 99) DisplayText("He pants in surprise and pleasure, clearly enjoying the weighty shaft rubbing against his drooping dick-skin");
...
// *Victory Cowgirl
function victoryCowgirlRidingOnSpiderBoi(player: Character, monster: Character): NextScreenChoices {
    ...
    DisplayText(" to summon a response.  How irritating.  You push him over with your " + Desc.Leg.describeFoot(player) + " to roll him onto his back, giving you the access you need to get at his ");
    ...
    DisplayText(" your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s, struggling to get off while the spider-boy cums again, this time without any warning at all.\n\n");
    ...
    player.slimeFeed();
    // Pregnancy Goes Here
    player.pregnancy.womb.knockUp(new Pregnancy(PregnancyType.SPIDER, IncubationTime.SPIDER), 151);
    player.orgasm();
    return { next: Scenes.camp.returnToCampUseOneHour };
}
...
function spiderBoyLossMaleButtfuck(player: Character, monster: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Spiderguy);
    ...
    DisplayText(", displaying surprising strength as he holds your " + Desc.Butt.describeButthole(player.torso.butt) + " at waist height.  You look up at the gloating arachnid ");
    ...
    if (player.stats.lust > 99) DisplayText(Desc.Cock.describeMultiCockSimpleOne(player, true) + " bounces atop your belly, reacting strongly to the tingling sensation radiating from your " + Desc.Butt.describeButthole(player.torso.butt) + ".");
    else {
        ...
        DisplayText(", reacting strongly to the tingles radiating up from your " + Desc.Butt.describeButthole(player.torso.butt) + ".");
    }
    DisplayText("\n\n");
    ...
    else DisplayText("quite easy due to how well-stretched your " + Desc.Butt.describeButthole(player.torso.butt) + " is.  Watching his fat little cock disappear into your roomy backside actually brings a smile to your lips.");
    Mod.Butt.displayStretchButt(player, monster.torso.cocks.get(0).area, true, true, false);
    DisplayText("  A firm, unforgiving hand grabs hold of " + Desc.Cock.describeMultiCockSimpleEach(player) + " and starts pumping it with short, fast strokes.  The slippery carapace gliding along your length feels absolutely marvelous, and in no time you're utterly relaxed against his invading member, dripping rivulets of sticky pre-cream onto your belly.  A round, hard digits circles your frenulum with a soft, repetitive motion, and you nearly blow your load on the spot.\n\n");
    ...
    else DisplayText(Desc.Cock.describeMultiCockSimpleOne(player, true) + " explodes again, dumping every remaining drop from your " + Desc.Balls.describeBalls(true, true, player) + " directly onto your " + Desc.Breast.describeChest(player) + " and belly.");
...
function loseToSpiderBoyVagFucked(player: Character, monster: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Spiderguy);
    DisplayText("As your ");
    ...
    DisplayText("  He pounces you, lightning fast, quick enough that you don't even react until he's on top of you.  Chitin-clad legs brush your " + Desc.Hip.describeHips(player) + ", the hard knees sinking deep into the swampy loam.  Grinning happily, the spider-boy leans over you and sucks a " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + " ");
    ...
    DisplayText("You arch your back from the sensation, lifting your " + Desc.Breast.describeChest(player) + " to provide him better access.  ");
    ...
    DisplayText("  The monstrous man handles your body with ease, playing your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + " like a finely tuned instrument, and you pant out little");
    ...
    DisplayText("The spider-morph's eyes seem to grow purple in the dim swamp light as he lifts himself away from your " + Desc.Breast.describeChest(player) + ", looking you in the eye.  \"<i>I knew you wanted this,</i>\" he says, rubbing his floppy foreskin against your entrance hard enough for you to feel the stiffness of the member hidden within the loose sheath.  His solid-black shaft feels wonderful against your ");
    ...
    DisplayText(", your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s ");
    ...
    player.slimeFeed();
    player.orgasm();
    player.pregnancy.womb.knockUp(new Pregnancy(PregnancyType.SPIDER, IncubationTime.SPIDER));
    return { next: Scenes.camp.returnToCampUseOneHour };
}

export function spiderPregVagBirth(player: Character) {
    DisplayText("\n");
    DisplaySprite(SpriteName.Spiderguy);
