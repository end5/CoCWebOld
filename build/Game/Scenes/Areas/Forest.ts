...
export default class Forest {
    ...
    public exploreDeepwoods(character: Character): void {
        character.locations.get(LocationName.Deepwoods).timesVisited++;

        const chooser = randInt(5);

        // Faerie
        if (chooser === 0) {
            Scenes.forest.faerie.encounterFaerie(character);
            return;
        }
        // Tentacle monster
        if (chooser === 1) {
        ...
        if (chooser === 2) {
            if (randInt(4) === 0) {
                this.trappedSatyr(character);
                return;
            }
            this.corruptedGlade.intro(character);
        }
        if (chooser === 3) {
            this.akbalScene.supahAkabalEdition(character);
        }
        else if (chooser === 4) {
            if (randInt(3) === 0) this.kitsuneScene.kitsuneShrine(character);
            else this.kitsuneScene.enterTheTrickster(character);
        }
    }

    // Explore forest
    public exploreForest(character: Character): void {
        character.locations.get(LocationName.Forest).timesVisited++;

        const chooser: number = randInt(4);
        // Chance to discover deepwoods
        if ((character.locations.get(LocationName.Forest).timesVisited >= 20) && !character.locations.get(LocationName.Deepwoods).visited) {
            character.locations.get(LocationName.Deepwoods).timesVisited++;
            DisplayText().clear();
    ...
    // Will be standalone
    private trappedSatyr(character: Character): void {
        DisplayText().clear();
        ...
        // (Player has penis:
            ...
            // [Yes] [No]
            MainScreen.displayChoices(["Ravage"], [this.rapeSatyr], ["Leave"], [this.ignoreSatyr]);
        }
    }

    // [=No=]
    private ignoreSatyr(character: Character): void {
        DisplayText().clear();
        DisplaySprite(SpriteName.Satyr);
    ...
    // Player returns to camp
    private rapeSatyr(character: Character): void {
        DisplayText().clear();
        DisplaySprite(SpriteName.Satyr);
        ...
        character.orgasm();
        // [Again][Leave]
        MainScreen.displayChoices(["Again"], [this.secondSatyrFuck], ["Leave"], [this.dontRepeatFuckSatyr]);
    }

    // [=Leave=]
    private dontRepeatFuckSatyr(character: Character): void {
        DisplayText().clear();
        DisplaySprite(SpriteName.Satyr);
    ...
    // [=Again=]
    private secondSatyrFuck(character: Character): void {
        const biggestCocks = character.torso.cocks.sort(Cock.LargestCockArea);
        const x = biggestCocks[0];
        DisplayText().clear();
