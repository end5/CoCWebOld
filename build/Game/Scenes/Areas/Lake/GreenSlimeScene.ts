...
function serviceLowCorruption(player: Character, monster: Character) {
    DisplayText().clear();
    DisplayText("You seem unable to pull your eyes away from the creature, clearly helpless and distressed in its current state.  Carefully, you step forward, the slime barely registering your presence any longer.  It momentarily grimaces as if in agony, and you decide that you can't simply leave it like this.");
...
function serviceLowCorruptionHighLust(player: Character, monster: Character) {
    player.slimeFeed();
    DisplayText().clear();
    ...
    // Tall peeps
    if (player.tallness >= 82) DisplayText("  You grip the head of the member firmly with one of your large hands and tease the tip with your thumb for a moment before squeezing and sliding your hand to its base.");
    // Short peeps
    else DisplayText("  You run a finger up and down the creature's member a few times before tightly wrapping your hands around the tip and running them to its base.");
    ...
    if (player.torso.vaginas.count > 0) {
        DisplayText("As you rub the creature's shaft you toy with the idea of inserting the thing into your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", biting your lip slightly and sliding forward a bit to rub yourself against the slime's " + Desc.Cock.describeCockShort(monster.torso.cocks.get(0)) + ".  You moan a bit and lose yourself for a moment, caught between desire ");
        ...
        if (player.vaginalCapacity() <= 18) DisplayText("  On top of that, you are unsure if the thick, foot and a half long appendage would even fit inside of you.");
    ...
    // Next PG
    // Get it hard
    DisplayText("As you continue to work the creature's mass its erection gains more and more definition, slowly but surely turning into an unmistakable human penis.  ");
    // And here begins the giant goddamn height split.  Hoo boy.
    // Smaller characters!
    if (player.tallness < 82) {
        // Hand gets coated in sensitive slime\n
        DisplayText("A light green fluid begins to coat your hand as you stroke the slime's cock, making your fingers feel warm and sending shivers of arousal through your entire body.\n\n");
        ...
        // female or herm version (if the herm didn't trigger male)
            ...
            DisplayText("\n\n");
            // Female cum!
            DisplayText("Finally, you feel a tsunami of pleasure begin to wash over you as you orgasm, and as your entire body shakes you unconsciously try to squeeze the creature between your legs.  It pulls back slightly and with a final mighty pulse, a green fluid explodes from the tip of its " + Desc.Cock.describeCockShort(monster.torso.cocks.get(0)) + ", splashing over your entire body.  Everywhere it touches seems to catch fire with pleasure and your already reeling mind almost collapses, and you let yourself fall back against the slime's body as wave after wave of pleasure flows through you.  ");
            // Herm bonus cum!
            if (player.torso.cocks.count > 0) {
                // Multidicked herms
        ...
        DisplayText("After a few moments the creature slides out from under you, slinking back into the waters of the lake as you recover from the ordeal.  When your wits return, there is nothing to do but stop back by camp.\n");
    }
    // PHEW! that was a big one.  now...  we do it all again for large players, whee! and this is why this thing is so goddamn long.
        ...
        // Female/Herm Masturbation
            ...
            DisplayText("You barely notice as the creature's erection recedes and it slides out from under you, leaving you utterly satisfied.  When you do finally recover your senses you find no other traces of its presence.");
        }
    }
    player.orgasm();
    player.stats.sens += 3;
...
function maleRapesOoze(player: Character) {
    DisplayText().clear();
    ...
    if (randInt(2) === 0) {
        ...
        // Continue the setup...
        DisplayText("You pull back for a moment and aim your penis downward, resting the ");
        // !! PENISTYPE
        switch (player.torso.cocks.get(0).type) {
            case 0: // human cock
                DisplayText("tip");
                break;
            case 1: // horse cock
                DisplayText("flared tip");
                break;
            case 2: // dog cock
                DisplayText("pointed tip");
                break;
            case 4: // tentacle cock
                DisplayText("rounded tip");
                break;
            default: // error text
                DisplayText("tip");
                break;
        ...
        if (player.torso.cocks.get(0).length >= 48) DisplayText(" and all the way through to its hips,");
        DisplayText(" and grunt approvingly.  You begin to slowly pull yourself out and then thrust yourself back in, feeling a moistness as some fluid surrounds your cock.  ");
        // player doesn't care about monster's wellbeing
        if (player.stats.cor >= 60) DisplayText("You wonder for a moment if you've broken the creature's membranous skin, then shrug and decide you don't care.");
        // player shows concern for monster
        else DisplayText("You wonder for a moment if you've broken the creature's membranous skin, but the continuing presence of the creature's throbbing erection implies that it's in relatively little distress.");
        // New PG
        DisplayText("\n\n");
...
function femaleRapesOoze(player: Character) {
    player.slimeFeed();
    ...
    if (player.torso.vaginas.get(0).virgin) {
        ...
        // higher corruption tosses it aside
            ...
            DisplayText("  You smile as you ram yourself onto the creature, finally removing your virginity.  You quickly start moving and give a pleased gasp as the creature's fluids coat your interior with a cool, soothing fluid.  ");
        }
    }
    else if (player.vaginalCapacity() >= 18) {
        DisplayText("  You enthusiastically lower yourself onto the slime's cock, letting out a small gasp of surprise as it slides inside of you.  The creature quickly understands your intentions and reaches up with its hands, beginning to pump you furiously.  ");
    }
    else {
    ...
    if (player.torso.cocks.count > 0) {
        if (player.torso.cocks.count === 1) {
            ...
            DisplayText("  Your cock grows hard as you are penetrated, and with the creature handling the movement you reach down, jerking off a different cock with each hand.");
        }
    }
    DisplayText("\n\n");
...
function oozeButtRapesYou(player: Character, monster: Character) {
    player.slimeFeed();
    ...
    // Girls get fondled.
    if (player.torso.vaginas.count > 0) DisplayText(Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " and " + Desc.Butt.describeButthole(player.torso.butt));
    // No coochie
    else {
        // Ballsy
        if (player.torso.balls.quantity >= 2) DisplayText(Desc.Butt.describeButthole(player.torso.butt) + " and " + Desc.Balls.describeBalls(false, true, player, true));
    ...
    if (player.analCapacity() < 18 || player.torso.butt.looseness === 0) {
        ...
        if (player.stats.cor <= 50) DisplayText("You feel a bit of fear as you tremble under the creature's weight, worried about the damage that its massive size might do to your ass.  ");
        else if (player.stats.cor < 100) DisplayText("In spite of your worries about the creature's size, you can't help but feel a little bit excited at the idea of having this beast take you from behind.  ");
        // and something special for the total whorebags that have somehow not already wrecked their asses.
        else DisplayText("You almost lick your lips in anticipation and push your hips back greedily, eager to feel the monster's cock stretch your asshole wide.  ");
    }
    ...
    // cock arousal
    if (player.torso.cocks.count > 0) DisplayText("You feel pangs of pleasure run through your body as its member repeatedly strokes your prostate from the inside, stimulating it.  ");
    DisplayText("Its dick shifts slightly, continually growing and shrinking in tiny amounts in a continual, heavy throb.  ");
    // masturbation begin
    // Cawkz!
    if (player.torso.cocks.count > 0) {
        // Single
...
function oozeRapesYouOrally(player: Character) {
    player.slimeFeed();
    ...
    if (player.stats.cor < 20) DisplayText("you blush as ");
    DisplayText("a large erection shaped like a human's dick forms at the bottom of its torso.  The creature thrusts its hips forward, pushing this new appendage against your face and making its intent very clear.\n");
    // slutty
    if (player.stats.cor >= 70) {
        ...
        if (player.torso.vaginas.count > 0) {
            ...
            if (player.torso.cocks.count < 1 || player.stats.lust <= 40) DisplayText("You try to reach for yourself to masturbate, but find yourself unable to reach around the slime's massive bulk.");
        }
        ...
        if (player.stats.lust > 40) {
            DisplayText("The creature withdraws as you begin to masturbate, furiously working yourself to a quick, but powerful, orgasm.  By the time you recover, it is long gone.");
            player.orgasm();
        }
        else DisplayText("The creature withdraws while you're distracted, leaving no traces of its presence save for what it left on your body.");
    }
    // standard
        ...
        DisplayText("The beast moves off of you and you roll over onto your hands and knees, coughing and spitting up green fluid as you recover your senses.  When you finish you look around for the creature but find no traces of it except the fluid on the ground and a slight taste of mint in your mouth.");
    }
    // resistant
    else {
        DisplayText("You close your mouth tightly as the penis rubs against your face, the soft, moist skin leaving a trace of green fluid around your mouth.  You try to turn away and the thing forms a third arm from its chest, pressing it over your mouth.  ");
        if (player.stats.int > player.stats.tou) DisplayText("You force yourself to keep your mouth shut in spite of the lack of air, holding out for as long as you can.  Eventually you become light headed, and pass out.  When you wake up there is no trace of the creature, save for the fluid covering your mouth, chest, and the ground around you.");
        else {
...
function oozeRapesYouVaginally(player: Character, monster: Character) {
    player.slimeFeed();
...
export function rapeOozeWithMilk(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplayText("You look over the ooze, wondering what to do about your need to nurse now that it has lost cohesion. After a while of puzzling things out, you decide to wing it, removing the top of your " + player.inventory.equipment.armor.displayName + " and pressing the mess of a monster to your " + Desc.Breast.describeBreastRow(player.torso.chest.get(0)) + " and giving it a squeeze to get the milk to it. The slime responds almost immediately, applying pressure from the base of your " + Desc.Breast.describeBreastRow(player.torso.chest.get(0)) + " to the tip of your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + ", earning it a shot of milk to your immense satisfaction. As it tends to your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + ", it slowly works its way down your body, almost lovingly ");
    ...
    DisplayText("Unfortunately, you seem to have done a little too much to it in your battle before. The ooze slides off, leaving you hanging on the edge of orgasm. Deciding you just won't stand for this, you scoop up most the mess left by the monster.desc.and use it as a masturbation aid, achieving sweet release by ");
    ...
    // [if genderless-
    else DisplayText("rubbing it over your " + Desc.Butt.describeButthole(player.torso.butt) + ".  The cool feel of it contrasts enough to send you over the edge relatively quickly, and you drop it as you feel your muscles spasm with the phantom orgasm ");

    DisplayText("while using the rest of it to coat your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + " in order to give it the last of your milk.\n\n");
...
export function slimeVictoryRape(player: Character, monster: Character) {
    // Service for lower corruption
    if (player.stats.cor <= 33) {
        DisplayText().clear();
        if (player.stats.lust < 60) serviceLowCorruption(player, monster);
        else serviceLowCorruptionHighLust(player, monster);
    }
    // higher corruption raaaaeeeep
    else {
        DisplayText().clear();
        // male or futa
        if (player.gender === Gender.MALE || (player.gender === Gender.HERM && randInt(2) === 0)) maleRapesOoze(player);
        // female or futa
        else if (player.gender > 0) femaleRapesOoze(player);
    }
    return { next: Scenes.camp.returnToCampUseOneHour };
}

export function slimeLoss(player: Character, monster: Character) {
    DisplayText().clear();
    if (player.gender === Gender.FEMALE || (player.gender === Gender.HERM && randInt(2) === 0)) {
        const temp = randInt(3);
        if (temp === 0) oozeRapesYouVaginally(player, monster);
        if (temp === 1) oozeRapesYouOrally(player);
        if (temp === 2) oozeButtRapesYou(player, monster);
    }
    // Everybody else
    else {
        const temp = randInt(2);
        if (temp === 0) oozeRapesYouOrally(player);
        if (temp === 1) oozeButtRapesYou(player, monster);
    }
    return { next: Scenes.camp.returnToCampUseOneHour };
}
