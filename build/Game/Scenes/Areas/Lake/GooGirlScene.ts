...
export interface GooGirlFlags {
    GOOGIRL_BIRTHS: number;
    GOOGIRL_CONSECUTIVE_LOSSES: number;
    TIMES_FUCKED_NORMAL_GOOS: number;
}

const gooGirlFlags: GooGirlFlags = {
    GOOGIRL_BIRTHS: 0,
    GOOGIRL_CONSECUTIVE_LOSSES: 0,
    TIMES_FUCKED_NORMAL_GOOS: 0,
};
...
function coreDropChance(player: Character) {
    if (randInt(4) === 0 && player.statusAffects.has(StatusAffectType.SlimeCraving) && !player.perks.has(PerkType.SlimeCore) && player.torso.hips.legs.isGoo() && RaceScore.gooScore(player) >= 4) {
        ...
        DisplayText("(<b>Gained New Perk: Slime Core - Moisture craving builds at a greatly reduced rate.</b>)");
        player.perks.add(PerkType.SlimeCore, 0, 0, 0, 0);
    }
}
// Color types are presented as [Blue slimes/Purple Slimes/Clear Slimes]
export function gooColor(): string {
    // blue, purple, or crystal
    return skinTone;
}

// [azure/plum/crystalline]
export function gooColor2(): string {
...
export function gooColor3(): string {
...
export function gooColor4(): string {
...
export function gooColor5(): string {
...
export function gooColor6(): string {
    if (skinTone === "blue") return "sapphire";
    else if (skinTone === "purple") return "amethyst";
    else return "diamond";
}
...
export function gooColor7(): string {
...
export function gooColor8(): string {
...
export function gooColor9(): string {
...
export function gooColor10(): string {
...
export function gooColor11(): string {
...
function gooGirlBadEnd(): NextScreenChoices {
    DisplayText().clear();
    ...
    // [Next]
    return { next: this.gooGirlBadEnd2 };
}

function gooGirlBadEnd2(player: Character): NextScreenChoices {
    DisplayText().clear();
...
export function slimeBadEnd(): NextScreenChoices { // Another gooey bad end; you should have drunk more fluids
    ...
    return { next: this.slimeBadEnd2 };
}

function slimeBadEnd2(): NextScreenChoices {
    DisplayText().clear();
    DisplayText("One year later...");
...
// DEFEAT
// ===============
export function getBeatByGooGirl(player: Character, monster: Character): NextScreenChoices {
    gooGirlFlags.GOOGIRL_CONSECUTIVE_LOSSES++;
    gooGirlFlags.TIMES_FUCKED_NORMAL_GOOS++;
    if (gooGirlFlags.GOOGIRL_CONSECUTIVE_LOSSES >= 5 && RaceScore.gooScore(player) >= 4) return gooGirlBadEnd();
    else if (player.gender === Gender.NONE) return genderlessLoseToGooGal(player);
    else if (player.gender === Gender.MALE) return dudeLoseToGooGal(player, monster);
    else if (player.gender === Gender.FEMALE) return femaleLoseToGooGal(player);
    else return hermLoseToGooGal(player);
}

// Defeat – Neuter
function genderlessLoseToGooGal(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplayText("You stumble, nearly falling to your knees and the slime recognizes her victory with a smile that nearly bisects her face. Clapping her hands in a wet splash, she oozes up to you and wraps her dripping, puddle-like lower body around yours, quickly stripping your " + player.inventory.equipment.armor.displayName + " off. There's no strength left in your limbs to resist, the goo-girl's insistent tugging guides you into the lapping waves of the lake. The cool water eases the excited heat of the girl's slimy embrace and you barely notice the depth until she's dragged you up to your chin. A surge of panic thrills up your spine, but she seems comfortable enough here, so she halts to investigate her prize.\n\n");
...
function hermLoseToGooGal(player: Character): NextScreenChoices {
    ...
    if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 2) DisplayText("  The mouths sucking at your " + Desc.Breast.describeAllBreasts(player) + " ripple against your flesh until milk sprays from your mammaries, her body suckling the entirety of your lactate-heavy chest, cleavage to teat.");
    DisplayText("  Your spurting ecstasy floods the girl's jelly body, coloring her " + gooColor3() + " body white as she drinks deeply, coaxing a few seconds of gasping spasms into long, seed-spewing minutes of climaxing euphoria.");
...
// Defeat – Female
function femaleLoseToGooGal(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplayText("You sway, finding it difficult to maintain your balance. When you fall, your " + Desc.Butt.describeButt(player) + " splashes wetly in the enveloping folds of the goo-girl's eager slime. You weakly hold your hand up to keep her back, but the wide-eyed victor disregards the gesture with playful disdain. She slithers against you, moist muck slurping at your flesh with a hungry heat. Her semi-solid hands take yours, pulling your arms apart to bare your chest. She cranes her dripping head around your shoulders, skillfully removing your " + player.inventory.equipment.armor.displayName + " with only her mouth. Your " + Desc.Breast.describeChest(player) + " heave as the irrepressible heat of the slime sears your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s until beads of sweat well up on your " + Desc.Skin.skin(player) + " and trickle down your curves, leaving a wet sheen over your body.\n\n");

    // [If the player is pregnant]
    if (player.pregnancy.womb.isPregnant()) {
        DisplayText("The goo-girl squishes her lurid " + gooColor() + " body against yours, yielding membrane rippling across your flesh in anticipation. She leans in, inches from your face, when her expression turns quizzical. She seems surprised by something and studies you carefully. Her head cranes to one side, as if listening to something. Her face lights up in cheerful glee and she lowers her head to your belly, feeling the vibrations from within. By way of demonstration, she strokes her belly and it bloats to a sloshing pudge, making her appear to be in a state of advanced pregnancy. Nodding vigorously, she points to her breast and you notice that what you took for a single slime core, is actually two hearts pressed against one another! With happy bounces of excitement, she embraces you in a motherly hug.\n\n");
    }
    // 	[If the player is not obviously pregnant/small breasts/not lactating]
        ...
        DisplayText("Disparagingly, she pokes your " + Desc.Breast.describeAllBreasts(player) + " and shakes her head, an expression of disappointment on her face. She smiles and strokes your cheek, patting you comfortingly on the head with a dripping appendage. She quirks her head to the side and her blank eyes light up in the excitement of inspiration. Grabbing the sides of her hair, she lifts two of the gooey hair clumps and sculpts them into tiny cow horns. With a focused determination, she bites down on her lower lip and balls her mitten-like hands into fists, inflating her chest larger and larger, until it begins to dwarf the rest of her body. Her arms stretch out, around the titanic chest and grab the back of your head, pulling your mouth against her wobbling breast. Pulling your lips apart, she seals your face around a shimmering nipple.\n\n");

        DisplayText("Tentatively, you take an experimental swallow, a thin trickle of " + gooColor7() + " gel rolling warmly down your throat. The taste is vaguely familiar, but you can't quite place it. Taking another gulp, you roll the milky goo around in your mouth, trying to identify the flavor, but it continues to elude you. For some reason, it seems very important to determine the maddening fluid and you suck as hard as you can, steady streams of slimy cream filling your belly with a blossoming heat. When the goo-girl pulls you off, you're a little surprised to see that your breasts have grown! The liquid weight in your stomach seems to have expanded your chest. You tweak a " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + " and find that milk gushes out, even at the slightest stimulation. You are lactating! Seemingly satisfied that your body is now fit to birth a child, the goo-girl nods her head, beaming.");
        // [player breast size increase, lactation activated/ amount increases]
        player.torso.chest.get(0).rating++;
        Mod.Breast.boostLactation(player, 1);
    }
    // [Pregnancy continued]
    if (player.pregnancy.womb.isPregnant()) {
        DisplayText("The fluid woman squeezes her goopy breasts against yours and the globes jiggle with their liquid weight. The " + gooColor() + " girl wriggles against you with an ecstatic expression, delighting in the quivering vibrations of your body. She presses her palms to your breasts and the warm goo engulfs your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + " with a wet plop. She plays with your tits, rolling them in her hands while the slurping goop of her body sucks at your swollen mammaries. She bends down to pepper your pulsing tit-flesh with tiny, moist kisses that leave " + gooColor() + " lip prints on your " + Desc.Skin.skin(player) + ". Rocking her body against yours, the girl's expression mimicks the helpless, agonized bliss of your face, her cheeks even flushing indigo to imitate your blush. She darts in and gives you a deep, full-bodied kiss of pregnant joy and your " + Desc.Breast.describeAllBreasts(player) + " burst with creamy milk, even as your climax leaves your body as quivering as the goo-girl's. Feeding on your sexual release, she arches her back and her hearts flicker wildly, her body losing some of its focus as it appears to melt under her orgasmic heat.\n\n");

        DisplayText("Thick trails of milk squirt into the slime's hands and down her arms, filling the girl's " + gooColor() + " body with your creamy milk.");
        if (player.lactationQ() <= 1000) DisplayText("  She drains you in no time, hands hungrily sucking for more in goopy kisses on your overtaxed nipples.");
        // [medium milk only:
        else if (player.lactationQ() <= 2000) DisplayText("  She drinks gratefully, nursing you with her permeable body until the pout of her belly sloshes heavily with the pale ivory of your motherly bounty.");
        else DisplayText("  You bite your lip and let go, torrents of rich, pearl fluid rushing through her arms and inflating the surprised girl before your eyes. The milk gushing into her fills the goo from base to breast, giving the once-" + gooColor5() + " girl an opalescent shine, her over-filled body bloated as if she were carrying triplets.");
        DisplayText("  She gives you a fond bop on the chin and a loving stroke on your belly before sloshing back into the lake, her seething heat still clinging to your body like a lingering hug.");
    }
    // [If the player isn't pregnant]
    else {
        DisplayText("The goo-girl flashes a broad smile and tickles under your chin to get your attention. Apparently trying to communicate her intent, she puffs out her cheeks and pats her belly, jiggling her amorphous frame in the process. Next, she taps your nose and pats your belly, slapping your " + Desc.Skin.skin(player) + " with a moist splat of her hand. Your expression remains blank and she sighs soundlessly, turning cold in annoyance. The slime presses you on your back and lifts your " + Desc.Leg.describeLegs(player) + " into the air. In a splattering motion, the goo lets her lower half sink into the thick sludge of her feet until only her upper torso remains. Then, with impatient force, she descends on your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", her yielding breasts rolling across your lower body.\n\n");

        DisplayText("The girl's face finds your all-too-eager pussy swiftly enough, her tongue tracing every fleshy fold of your labia while the warming slime of her lips suck at the moisture of your rapidly lubricating slit. She pushes a little harder, forcing the oozing length of her tongue inside of you, its pulsing surface expanding and contracting, leaving you squirming and gasping, biting at one of your knuckles while the goo prods the limits of your womanhood. Seemingly satisfied, she screws her face into an expression of intense focus and slaps her head down, between your thighs. The delicate features of her nose and face press against your lower lips with an insistence that practically melts you before, shockingly, her head begins to squeeze into your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + "!\n\n");

        DisplayText("You try to stop the girl's progress, frantically, wagging your legs and clenching your inner walls together, but your strength has left you and her " + gooColor() + " skin is much too slippery to gain ground against. A moment later, all thoughts of stopping the invading goo are lost to the waves of gushing heat pouring into your body as her neck feeds into your aching cunt, filling your uterus with the sentient slime. She braces her hands at the widening mouth of your sex before slipping those in as well, working her arms and shoulders into your now gaping body.  ");
        // [Small vaginal capacity:
        if (player.vaginalCapacity() <= 30) DisplayText("She bottoms out quickly, filling every inch of your pussy with her squished form, wriggling inside of you happily, her membrane molding to your birth canals every crevasse.");
        else DisplayText("More and more of her ooze thrusts into you, shivers of overstimulation tearing through your mind like fists through wet parchment. Your eyes roll aimlessly as the pressure makes it difficult to breathe. With a squelching slurp, the last of the girl's form is pulled inside your slavering pussy.");
        ...
        if (player.pregnancy.womb.isPregnant()) DisplayText("  Everything she pumped into you runs back out after.  For some reason or another, it didn't take.");
        else {
            player.pregnancy.womb.knockUp(new Pregnancy(PregnancyType.GOO_GIRL, IncubationTime.GOO_GIRL), 1, true);
        }
    }
    player.orgasm();
    player.stats.sens += 4;
...
export function gooPregVagBirth(player: Character) {
    gooGirlFlags.GOOGIRL_BIRTHS++;
    DisplayText("\n");
...
export function beatUpGoo(player: Character): NextScreenChoices {
    ...
    // Victory – Neuter
    if (player.gender === Gender.NONE || player.stats.lust < 33) {
        ...
        DisplayText("With the goo-girl defeated, her unusual body is at your mercy.  What do you do?");
        let sex1S: string = "";
        let sex1N;
        let sex2S: string = "";
        let sex2N;
        ...
        if (player.torso.cocks.count > 0) {
            if (player.torso.cocks.sort(Cock.SmallestCockArea)[0].length < 24) {
                sex1S = "DickSex";
                sex1N = { func: gooMaleRape, args: [2] };
            }
            if (player.torso.cocks.sort(Cock.LongestCocks)[0].length >= 24) {
                sex2S = "BigDickSex";
                sex2N = { func: gooMaleRape, args: [1] };
            }
            if (player.torso.vaginas.count > 0) {
                sex3S = "Herm Sex";
        ...
        let gooTF;
        // corrupt chances
        if ((Flags.list[FlagEnum.GOO_TFED_MEAN] === 0 && Flags.list[FlagEnum.GOO_TFED_NICE] === 0) && gooGirlFlags.TIMES_FUCKED_NORMAL_GOOS >= 2) {
            if (
                player.stats.cor < 50 &&
                (
            ...
            else if (Flags.list[FlagEnum.PC_KNOWS_ABOUT_BLACK_EGGS] > 0) {
                // Recruitment:
                // Notice After Victory:
                if (Flags.list[FlagEnum.TIMES_THOUGHT_ABOUT_GOO_RECRUITMENT] === 0) {
                    ...
                    Flags.list[FlagEnum.TIMES_THOUGHT_ABOUT_GOO_RECRUITMENT]++;
                }
                // (Repeat)
                else {
...
// Victory – Male
function gooMaleRape(player: Character, type: number = 1): NextScreenChoices {
    gooGirlFlags.TIMES_FUCKED_NORMAL_GOOS++;
    DisplayText().clear();
    let selectedCock = player.torso.cocks.sort(Cock.LargestCockArea)[0];
    if (player.torso.cocks.sort(Cock.LongestCocks)[0].length >= 24 && player.torso.cocks.sort(Cock.ShortestCocks)[0].length < 24) {
        if (type === 1) selectedCock = player.torso.cocks.sort(Cock.LongestCocks)[0];
        else selectedCock = player.torso.cocks.sort(Cock.ShortestCocks)[0];
    }
    ...
    // [Dick under 2 ft]
    if (selectedCock.length < 24) {
        DisplayText("Dropping to the goo-girl's level, you roughly grab her pulsing breasts, fingers sinking an inch into the slime before her surface tension gives you something to grip. Her membrane squishes between your fingers with lewd squelches as you squeeze harder, rolling the " + gooColor() + " orbs in your palms, her " + gooColor2() + " nipples bulging outward the harder you press. She noiselessly coos at your stimulation and shivers of delight ripple through her body. Stroking your body with soft hands, the goo-girl leaves " + gooColor9() + " film of warm slime across your " + player.inventory.equipment.armor.displayName + " that quickly leaves you too hot to stay clothed. You strip and the buxom slime flashes a happy smile of victory.\n\n");

        DisplayText("Noticing your hardening length, the goo-girl claps her hands in gushing pleasure and gently pushes you back onto your " + Desc.Butt.describeButt(player) + ". Rubbing her moist cheek against yours, she slides down your " + Desc.Breast.describeChest(player) + " and perches over your groin. Slowly, she moves back and forth, stroking your shaft with the perky wetness of her stiff nipples. She hovers over your manhood for a moment before dropping down, her sloping cleavage stroking your cock between her slimy pillows with a slurping squelch. Pressing her balled-up fists to either side, she squeezes your " + Desc.Cock.describeCock(player, selectedCock) + " between her breasts and begins to slowly pump your stiffening organ. The natural lubrication of her gooey skin makes the tit-fuck pleasantly slick, softness sucking your flesh with wet gulps as she rolls from base to tip, your cockhead popping between her cleavage at a brisk pace, cooled by the air for only a moment before it is swallowed once more by the seething heat of her chest. Through her transparent skin, you can see your " + Desc.Cock.describeCock(player, selectedCock) + " being massaged by the gooey embrace of her tits. Your body trembles to the precipice of an orgasm, muscle spasms and a shortness of breath makes each pump of your slime-oiled hardness more exquisite than the last, as if the girl's murky breasts were pulling the air from your lungs.\n\n");
...
// Victory – Herm
function victoryHermSex(player: Character): NextScreenChoices {
    const largestCock = player.torso.cocks.sort(Cock.LargestCockArea)[0];
    gooGirlFlags.TIMES_FUCKED_NORMAL_GOOS++;
    DisplayText().clear();
    ...
    DisplayText("Quickly stripping, you let your " + Desc.Cock.describeMultiCockShort(player) + " slap against her turbulent membrane, the warm muck gurgling wetly at your turgid stiffness. Sinking your fingers into the girl's yielding breasts, you push forward, pulsing flesh sliding into the " + gooColor2() + " depths of her expanded cleavage. The goo-girl's chest sucks eagerly at you, pliant slime drooling warm " + gooColor5() + " beads of unnecessary lubrication as you pick up your pace. Staring at your " + Desc.Cock.describeMultiCockShort(player) + " moving in and out of the girl's chest through her transparent skin, you are surprised to see her body shifting further under your pounding. A vertical slit opens between her breasts and her membrane quickly folds itself in delicate petals around the edges. Your next thrust pushes into the new cavity and the organic texture confirms your suspicion: she's created a vagina between her breasts for you to fuck!\n\n");

    DisplayText("Slowing yourself to enjoy the experience, you note that she seems to have made her new cunny too tight- every inch you push in displaces her mass, forming a bulge beneath her abdomen. Without even pausing, you lower yourself to sit atop the girl's lap and jam your " + Desc.Cock.describeCock(player, largestCock) + "  into her narrow tunnel. The bulge rises into your dripping pussy, her moist shaft conforming to your inner walls for a moment before stiffening into a replica of your own dick.  ");
    Mod.Vagina.displayStretchVagina(player, player.torso.cocks.get(0).area, true, false, true);
    DisplayText("Every time you withdraw from the squirming slime, she pulls out of you and every time you slam back in, your pussy fills with the aching delight of the gooey copy-cock. Through the girl's mimicking body, you've found a way to fuck yourself by proxy! The thought fuels your pace as you accelerate to a fevered pitch. Slamming the " + gooColor5() + " mirror of your " + Desc.Cock.describeCock(player, largestCock) + " into your sloppy cunt, you ride yourself with reckless abandon.  The force of your fucking is too much for the goo-girl and she silently cries out in ecstatic bliss before the pulsing scarlet heart in her chest is violently ejected from her body, splashing a hundred yards away in the lake's depths. With no core, the empty girl's face becomes wanton, little more than a lusty slut unable to sate her need to fuck, her tongue dangling out of the side of her mouth.\n\n");
...
// FEMALE VICTORIES
// [Feeder perk]
function victoryRapeAGooGalAsFeeder(player: Character): NextScreenChoices {
    gooGirlFlags.TIMES_FUCKED_NORMAL_GOOS++;
    DisplayText().clear();
    ...
    if (player.torso.vaginas.count > 0) DisplayText("Even without stimulation, your clit practically vibrates under your climax, squeezing your body to its limits until everything fades into a pearl haze and you lose yourself to the euphoria.\n\n");
    else if (player.torso.cocks.count > 0) if (player.torso.vaginas.count > 0) DisplayText("Even without stimulation, your cock practically vibrates under your climax, squeezing your body to its limits until everything fades into a pearl haze and you lose yourself to the euphoria.\n\n");
    else if (player.torso.vaginas.count > 0) DisplayText("Even without stimulation, your tingling anus practically vibrates under your climax, squeezing to its limits until everything fades into a pearl haze and you lose yourself to the euphoria.\n\n");
...
export function spyOnGooAndOozeSex(player: Character): NextScreenChoices {
    DisplayText().clear();
    ...
    DisplayText("You hurry away before the five oozes take an interest in you next.");
    player.stats.lust += (4 + player.stats.cor / 10);
    return { next: Scenes.camp.returnToCampUseOneHour };
}

function layBeeEggsInGoo(player: Character): NextScreenChoices {
    DisplayText().clear();
