...
export class GooGirlScene {
    ...
    public encounterGooGirl() {
        ...
        DisplayText(this.gooColor() + " slime body sculpting itself into a humanoid shape. The girl curiously tilts her head to one side, as if trying to figure out why you're backing away, before she happily surges forward!");
        return { next: playerMenu };
    }

    // New Perk – Slime Core (requires goo player, random drop rate?)
    private coreDropChance(player: Character) {
        if (randInt(4) === 0 && player.statusAffects.has(StatusAffectType.SlimeCraving) && !player.perks.has(PerkType.SlimeCore) && player.torso.hips.legs.isGoo() && player.gooScore() >= 4) {
    ...
    public slimeBadEnd() { // Another gooey bad end; you should have drunk more fluids
    ...
    // DEFEAT
    // ===============
    public getBeatByGooGirl(player: Character) {
        Flags.list[FlagEnum.GOOGIRL_CONSECUTIVE_LOSSES]++;
        if (Flags.list[FlagEnum.GOOGIRL_CONSECUTIVE_LOSSES] >= 5 && player.gooScore() >= 4) this.gooGirlBadEnd();
        else if (player.gender === Gender.NONE) this.genderlessLoseToGooGal();
        else if (player.gender === Gender.MALE) this.dudeLoseToGooGal();
        else if (player.gender === Gender.FEMALE) this.femaleLoseToGooGal();
        else this.hermLoseToGooGal();
        Flags.list[FlagEnum.TIMES_FUCKED_NORMAL_GOOS]++;
    }

    // Defeat – Neuter
    private genderlessLoseToGooGal(player: Character) {
        DisplayText().clear();
    ...
    private hermLoseToGooGal(player: Character) {
        ...
        DisplayText("  Your spurting ecstasy floods the girl's jelly body, coloring her " + this.gooColor3() + " body white as she drinks deeply, coaxing a few seconds of gasping spasms into long, seed-spewing minutes of climaxing euphoria.");
    ...
    // Defeat – Female
    private femaleLoseToGooGal(player: Character) {
        DisplayText().clear();
        ...
        if (player.pregnancyIncubation <= 280 && player.pregnancyIncubation >= 1) {
            ...
            else DisplayText("  You bite your lip and let go, torrents of rich, pearl fluid rushing through her arms and inflating the surprised girl before your eyes. The milk gushing into her fills the goo from base to breast, giving the once-" + this.gooColor5() + " girl an opalescent shine, her over-filled body bloated as if she were carrying triplets.");
        ...
        // [If the player isn't pregnant]
            ...
            if (player.pregnancyIncubation >= 1) DisplayText("  Everything she pumped into you runs back out after.  For some reason or another, it didn't take.");
            else {
                player.knockUp(PregnancyType.GOO_GIRL, PregnancyType.INCUBATION_GOO_GIRL, 1, 1);
            }
        }
        player.orgasm();
        player.stats.sens += 4;
    ...
    public gooPregVagBirth(player: Character) {
        Flags.list[FlagEnum.GOOGIRL_BIRTHS]++;
        DisplayText("\n");
    ...
    // VICTORY
    public beatUpGoo(player: Character) {
        Flags.list[FlagEnum.GOOGIRL_CONSECUTIVE_LOSSES] = 0;
        DisplayText().clear();
        ...
        if (player.gender === Gender.NONE || player.stats.lust < 33) {
            ...
            let sex4S: string = "";
            let sex4N;
            let valeria: Function = kGAMECLASS.valeria.valeriaAndGooThreeStuff;
            if (player.inventory.equipment.armor.displayName !== "goo armor" || player.isButtPregnant() || player.isPregnant()) valeria = null;
            let eggs;
            if (player.canOvipositBee()) eggs = this.layBeeEggsInGoo;
            if (player.torso.cocks.count > 0) {
                if (player.torso.cocks.list[player.torso.cocks.sort(Cock.SmallestCockArea)[0]].cockLength < 24) {
                    sex1S = "DickSex";
                    sex1N = createCallBackFunction(this.gooMaleRape, 2);
                }
                if (player.torso.cocks.sort(Cock.LongestCocks)[0].length >= 24) {
                    sex2S = "BigDickSex";
                    sex2N = createCallBackFunction(this.gooMaleRape, 1);
                }
                if (player.torso.vaginas.count > 0) {
                    sex3S = "Herm Sex";
            ...
            let gooTF;
            // corrupt chances
            if ((Flags.list[FlagEnum.GOO_TFED_MEAN] === 0 && Flags.list[FlagEnum.GOO_TFED_NICE] === 0) && Flags.list[FlagEnum.TIMES_FUCKED_NORMAL_GOOS] >= 2) {
                if (player.stats.cor < 50 && (player.inventory.items.has(consumables.SUCMILK) || player.inventory.items.has(consumables.P_S_MLK)) && (player.inventory.items.has(consumables.BLACKEG) || player.inventory.items.has(consumables.L_BLKEG))) {
    ...
    // Victory – Male
    private gooMaleRape(player: Character, type: number = 1) {
        Flags.list[FlagEnum.TIMES_FUCKED_NORMAL_GOOS]++;
        DisplayText().clear();
        let x: number = player.torso.cocks.sort(Cock.LargestCockArea)[0];
        if (player.torso.cocks.sort(Cock.LongestCocks)[0].length >= 24 && player.shortestCockLength() < 24) {
        ...
        // [Dick under 2 ft]
        if (player.torso.cocks.get(x).length < 24) {
    ...
    // Victory – Herm
    private victoryHermSex(player: Character) {
        const x: number = player.torso.cocks.sort(Cock.LargestCockArea)[0];
        Flags.list[FlagEnum.TIMES_FUCKED_NORMAL_GOOS]++;
        DisplayText().clear();
    ...
    // FEMALE VICTORIES
    // [Feeder perk]
    private victoryRapeAGooGalAsFeeder(player: Character) {
        Flags.list[FlagEnum.TIMES_FUCKED_NORMAL_GOOS]++;
        DisplayText().clear();
    ...
    // [Exhibitionist Perk]
    private exhibitionismGooGirlVictoryRape(player: Character) {
        Flags.list[FlagEnum.TIMES_FUCKED_NORMAL_GOOS]++;
        DisplayText().clear();
    ...
    public spyOnGooAndOozeSex(player: Character) {
        monster = new GooGirl();
        DisplayText().clear();
        ...
        dynStats("lus", (4 + player.stats.cor / 10));
        return { next: Scenes.camp.returnToCampUseOneHour };
    }

    private layBeeEggsInGoo(player: Character) {
        DisplayText().clear();
