...
export function zealotBoat(player: Character) {
    if (player.statusAffects.has(StatusAffectType.FetishOn) && player.statusAffects.get(StatusAffectType.FetishOn).value1 === 1) {
        return zealotRepeat(player);
    }
    player.statusAffects.get(StatusAffectType.FetishOn).value1 = 1;
    DisplayText().clear();
    ...
    DisplaySprite(SpriteName.Fetish_Zealot);
    return CombatManager.beginBattle(player, [], [new FetishZealot()]);
}

// Regular encounter
// This is the regular pre combat text for wandering zealots (they'll be a regular mob at the swamp)
function zealotRepeat(player: Character) {
    DisplayText().clear();
...
// Raping the player
export function zealotLossRape(player: Character) {
    DisplayText().clear();
    const broseph: string = Desc.Gender.mf(player, "dude", "chick");
    ...
    if (broseph === "dude") {
        ...
        if (player.inventory.equipment.armor.value <= 0) {
            player.inventory.equipment.armorDescMod = "formal vest, tie, and crotchless pants";  // can you think of a better way of putting this?
        }
    ...
    // no its a girl, or it looks like one
        ...
        if (player.inventory.equipment.armor.value === 0) {
            player.inventory.equipment.armorDescMod = "backless female teacher's clothes";  // again, change this if you've got a better name
        }
    ...
    if (player.stats.int < 10) {
        ...
        return { next: Scenes.lake.fetishCultistScene.cultistBadEnd2 };
    }
    ...
    player.stats.lust += player.stats.cor / 20 + player.stats.lib / 10;
    return { next: Scenes.camp.returnToCampUseOneHour };
}

export function zealotDefeated(player: Character, cultist: Character) {
    // Defeated by health
    if (cultist.stats.HP < 1) DisplayText("The zealot collapses from his wounds, too hurt to continue controlling his powers.");
    // Defeated by lust
    ...
    if (player.stats.lust >= 33 && player.gender > 0) {
        ...
        let bikiniTits;
        if (player.torso.vaginas.count > 0 && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 4 && player.inventory.equipment.armor.displayName === "lusty maiden's armor") bikiniTits = () => lustyMaidenPaizuri(player, cultist);
        return { choices: [["Yes", this.zealotWinRape], ["B.Titfuck", bikiniTits], ["Leave", Scenes.camp.returnToCampUseOneHour]] };
    }
    else return { next: Scenes.camp.returnToCampUseOneHour };
}

// Raped by the player
function zealotWinRape(player: Character, cultist: Character) {
    DisplayText().clear();
    ...
    // If player has a vagina, again, if both, chose one at random
        ...
        Mod.Vagina.displayStretchVagina(player, cultist.torso.cocks.get(0).area, true);
    }
    player.orgasm();
    return { next: Scenes.camp.returnToCampUseOneHour };
}
