...
export class FetishZealotScene extends AbstractLakeContent {
	...
	public zealotLossRape(): void {
		...
		if (broseph === "dude") {
			...
			if (player.torso.cocks.count > 0) {
				DisplayText(CockDescriptor.describeMultiCockShort(player) + ".  You feel yourself grow hard ");
			...
			if (player.torso.cocks.count > 0) {
				DisplayText(", and your " + CockDescriptor.describeMultiCockShort(player) + " start");
		...
		DisplayText("\"<i>Ok,</i>\"  you say nervously, \"<i>I can give you a lesson on pleasing " + player.mf("men", "women") + ".</i>\"  He jumps up excitedly and runs around to the other side of the desk saying \"<i>Ok, what do I have to do?</i>\"  Feeling a bit put on the spot, you stop for a moment to take in the situation.  ");
		//again, is that a guy?  Or just look like one?
		if (broseph === "dude") {
			DisplayText("You're standing in front of your student behind your desk in your empty classroom.  You are in regular teacher's attire: an open vest and tie over your otherwise bare " + player.BreastDescriptor.describeAllBreasts(player) + ", with formal dress pant legs attached to your vest with suspenders, plus your black dress shoes for dancing.  Your student is wearing the standard male student uniform, a collection of pieces of material held together with straps, that will all fall away easily when one gets pulled; but otherwise gives full coverage.\n\n");
			...
			if (player.gender != 0) {
				...
				DisplayText("You stand up and indicate to him that it is now his turn, and while he is disappointed that the blowjob ended before he came, he still eagerly kneels down in front of your exposed " + CockDescriptor.describeMultiCockShort(player) + " and licks his lips eagerly.  He tries to imitate your performance on his manhood onto your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", but he sometimes messes up the order, or does something too fast or slow.  Fortunately, he is a good student, and under your guidance he is soon sucking cock like a pro.  He tickles you with his tongue in just the right way, and gives just the right amount of attention to each part of your length.  ");
				if (player.torso.vaginas.count > 0) DisplayText("You slip one of your fingers inside your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ", wishing that it could get some attention, but this lesson is about pleasing <i>men</i>.  ");
				...
				//jerking off contest
				DisplayText("He eagerly stands up and wraps his hands around his fine cock, you quickly try to do the same with your " + CockDescriptor.describeMultiCock(player) + ".  \"<i>I won't lose to you so easily, professor!</i>\" he declares to you, \"<i>Don't act so confidently on your first time.</i>\" you reply.  Then the two of you are madly jerking yourselves off in a mad effort to get off before the other, your respective cocks already moist from the blowjobs they just received.  ");
				//just one cock
				if (player.torso.cocks.count === 1) {
					DisplayText("You piston your hands on your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", ");
				}
				//nope two
				else if (player.torso.cocks.count === 2) {
					DisplayText("With one hand on each of your " + CockDescriptor.describeMultiCockShort(player) + ", you piston them like mad, ");
				}
				//oh no, is more than that
				else {
					DisplayText("Rapidly moving your hands between each rod in your " + CockDescriptor.describeMultiCockShort(player) + ", you manage to piston each of them at an incredible rate, ");
				}
			...
			if (player.gender != 0) {
				...
				DisplayText("You take a hold of his nicely sized balls in one hand and the base of his cock in the other.  You give the tip a few licks before running your tongue down its length,  making sure to coat every bit of it in your saliva along the way.  His aroused groans let you know that you are giving a good demonstration.  You give his balls a soft rub as you stick the end inside your mouth and run your tongue around it; the taste of his pre is almost like candy to you.\n\n");
				//cunalinguss
				DisplayText("You stand up and indicate that it is now his turn, and while he is disappointed that the blowjob ended before he came, he still eagerly kneels down in front of your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ".  You gingerly lift up your skirt ");
				if (player.torso.balls.quantity > 0) {
					DisplayText("and move your " + BallsDescriptor.describeBalls(true, true, player) + " out of the way ");
				}
				DisplayText("so he has access to your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ".  He sets to work, and you give him careful instructions on where to lick, how hard, and such.  He easily catches on, and soon he is probing all the right places, and giving the right amount of attention to every part of you.  ");
				if (player.torso.cocks.count > 0) {
					DisplayText("You give your " + CockDescriptor.describeMultiCockShort(player) + " a few gentle strokes, wishing he would give that a little attention too, but this is a lesson on how to please a <i>woman</i>.  ");
				}
				DisplayText("You feel yourself getting close to the edge and tell him to stop.  He looks up at you uncertain for a moment, and you tell him \"<i>It's time for the main event.</i>\"\n\n");
				//prick in the twat
				DisplayText("He eagerly stands up and starts to push you back against your desk, his fine cock already inside your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ".  He pushes you onto your back and grabs ahold of your legs, starting to make strong thrusts deep inside you, his cock seeming to fit your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " perfectly.  Any pretense of professionalism is lost in the moment, as all you can care about is the feeling of his exquisite cock perfectly filling you, pushing in and out.  All too soon, you hear him gasp, and the wonderful feeling of sweet release fills you.  ");
				if (player.torso.cocks.count > 0) {
					DisplayText("Your " + CockDescriptor.describeMultiCockShort(player) + " spasms and covers you with a liberal amount of your own fluids.  ");
				}
	...
	private zealotWinRape(): void {
		...
		if (player.torso.cocks.count > 0 && (player.gender != 3 || randInt(2) === 0)) {
			...
			if (player.stats.cor < 50) DisplayText("You stop and stare at him for a moment, in complete disbelief at this bizarre 'prayer'.  You consider just leaving him alone, but sensing your hesitation, the zealot looks up to you with a horrified expression.  \"<i>You must violate me!</i>\"  He cries out to you, \"<i>Please, let me finish my prayer, put your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " in my ass.</i>\"  Well, he did ask you...\n\n");
			else DisplayText("You chuckle at him, he <i>accepts</i> his punishment?  Oh, you are going to enjoy this so much.\n\n");
			DisplayText("You remove your " + player.inventory.equipment.armor.displayName + " and stride up behind him, and grab his rear end to line up your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " with his loose hole as he continues his prayer: \"<i>Soon a man will violate my ass, like so many others have done in your holy worship.  Woe is me, to be treated like this by someone who doesn't follow us.</i>\" You shake your head at this absurdity and plunge your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " inside his waiting hole.  Amazingly, his hole somehow manages to fit you perfectly.  Since he seems to have no problem taking you, you waste no time in getting the anal rape on.\n\n");
