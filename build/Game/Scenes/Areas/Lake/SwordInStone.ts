...
export class SwordInStone {
    public findSwordInStone(player: Character) {
        if (!player.statusAffects.has(StatusAffectType.FactoryOverload)) {
            ...
            doYesNo(tryToTakeSwordInStone, Scenes.camp.returnToCampUseOneHour);
        }
        else {
    ...
    private tryToTakeSwordInStone(player: Character) {
        DisplayText().clear();
        // if corrupted...
        ...
        // If not corrupted...
            ...
            inventory.takeItem(weapons.B_SWORD, Scenes.camp.returnToCampUseOneHour);
