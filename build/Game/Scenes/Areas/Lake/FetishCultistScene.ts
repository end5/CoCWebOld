...
export class FetishCultistScene {
	...
	public cultistRapesYou(): void {
		...
		if (player.hips.legs.type === LegType.CENTAUR || player.hips.legs.type === LegType.NAGA) {
			...
			if (player.hips.legs.type === LegType.CENTAUR && player.torso.balls.quantity > 0 && player.torso.cocks.count > 0 && randInt(4) < 3) {
				if (player.torso.cocks.get(0).type === CockType.HORSE) {
					...
					//(ending after all cultist rape scenes)
						...
						if (player.armorValue > 0) DisplayText("You find your " + player.inventory.equipment.armor.displayName + " back on your body with no sign of the strange clothes you were wearing before.  ");
			...
			//Naga
			else if (player.hips.legs.type === LegType.NAGA) {
				...
				//boy's surprise
					...
					DisplayText("your " + CockDescriptor.describeMultiCockShort(player) + " poke out of your pelvic slit, growing to full erectness.  You strain against your bonds, desperate to bring release to ");
					if (player.torso.cocks.count > 1) DisplayText("each of ");
					DisplayText("your " + CockDescriptor.describeMultiCockShort(player) + ", but there is nothing you can do and you keeper laughs at you.  \"<i>Oh so eager to bury that eager cock of yours inside my snatch?  Too bad, I'm the one who gets to choose what goes in there, and it's not your cock.</i>\" she says to you, clearly enjoying the frustration you're going through.\n\n");
				}
				//girl's surprise
				else if (player.gender === Gender.FEMALE) {
					DisplayText("and to your shame, you feel your pelvic slit open up to reveal your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ", already wet from the sight.  You pull against your bonds a little, hoping to find some way to get some release from your arousal.  \"<i>Sorry girly, that slit of yours isn't supposed to be filled now,</i>\" she says, before pointing at her cunt, \"<i>this one is.</i>\"  She is clearly enjoying the frustration you're going through.\n\n");
				}
				//herm's frustrations
				else {
					DisplayText("and with a shudder you feel your pelvic slit open up so that your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " is exposed and ");
					if (player.torso.cocks.count > 1) DisplayText("each of ");
					DisplayText("your " + CockDescriptor.describeMultiCockShort(player) + " can thrust up into the air.  You make a few futile thrusts into the air, desperate to find some way to get release from your mixed genitals.  \"<i>Hmm, sorry snakey, but I can't pick one of those to play with, maybe if you had only one...</i>\" she says to you with a snark, but you know she is lying and is clearly enjoying your frustration.\n\n");
				}
		...
		if (randInt(2) === 0 && !sexed) {
			...
			if (player.hips.legs.type === LegType.CENTAUR) {
				...
				if (player.torso.cocks.count > 0) DisplayText("  As you see your " + CockDescriptor.describeMultiCockShort(player) + ", 'incredibly large' is the only qualifier you can think of.");
			}
			DisplayText("\n\n");
			...
			if (player.torso.chest.hasFuckableNipples()) {
				...
				//[if breasts are already lactating] 
					...
					Mod.Breast.boostLactation(character, .6);
				}
			...
			if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 1 && !changed) {
				...
				if (player.torso.chest.count === 2) DisplayText("Once about a quarter of the fluid has been injected into the first breast, she moves on to the second and you feel a similar sharp pain followed by the glowing pleasure.  She continues this until all of your " + player.BreastDescriptor.describeAllBreasts(player) + " have been injected.  ");
				//[if the player has three rows of breasts]
				if (player.torso.chest.count > 2) DisplayText("Once about a sixth of the fluid has been injected into the first breast, she moves on to the second and you feel a similar sharp pain followed by the glowing pleasure.  She continues this until all of your " + player.BreastDescriptor.describeAllBreasts(player) + " have been injected.  ");
				DisplayText("Soon the glowing pleasure fills your breasts completely and you feel an orgasm shoot through your body as your " + player.BreastDescriptor.describeAllBreasts(player));
				//[if breasts are already lactating]
			...
			if (player.torso.vaginas.count > 0 && !changed) {
				...
				DisplayText("Soon you have a fully formed " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " standing tall and proud where your old femininity used to lie.  The nurse sets the syringe aside and climbs on top of your legs. She giggles softly before lowering herself and taking your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " between her breasts and starts rubbing and playing with them while running your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " between them.  It is an exquisite experience having her tit fuck your brand new " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", and you notice that she seems to be enjoying it just as much as you are.  It doesn't take long for her ministrations to make you cum between her breasts, and cum you do.  Satisfied, the nurse rises from you and says \"<i>Looks to me like its working properly.  For now, I've got to go take care of another patient. Don't you do anymore running about until I get back, ok?  I'll be back to check on your progress as soon as I'm able!</i>\"  She gives you a wink, and turns to walk out the door.");
				player.stats.sens += 3;
				changed = true;
				player.updateGender();
				changedCock = true;
			...
			if (player.gender === Gender.NONE && !changed) {
				...
				player.torso.cocks.add(new Cock());
				DisplayText("Soon you have a fully formed " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " standing tall and proud where your bare crotch used to lie.  The nurse sets the syringe aside and climbs on top of your legs. She giggles softly before lowering herself and taking your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " between her breasts and starts rubbing and playing with them while running your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " between them.  It is an exquisite experience having her tit fuck your brand new " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", and you notice that she seems to be enjoying it just as much as you are.  It doesn't take long for her ministrations to make you cum between her breasts, and cum you do.  Satisfied, the nurse rises from you and says \"<i>That was fun, maybe later we can practice more.  For now, I've got to go take care of another patient. Don't you do anymore running about until I get back, ok?  I'll be back to check on your progress as soon as I'm able!</i>\"  She gives you a wink, and turns to walk out the door.");
				player.stats.sens += 3;
				changed = true;
				player.updateGender();
				changedCock = true;
			...
			if (player.torso.vaginas.count > 0 && player.torso.cocks.count > 0) {
				DisplayText("You see your " + player.inventory.equipment.armor.displayName + " becoming a nice, comfortable outfit of a white shirt and black pants that make you look well-suited to work in a manor... if they didn't have an exposed crotch and butt that puts your " + CockDescriptor.describeMultiCockShort(player) + ", your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ", and your " + ButtDescriptor.describeButt(player) + " on full display.  Any undergarments you had are now gone.\n\n");
				...
				DisplayText("After a minute of this, she stops you and says, \"<i>Good; I am now ready to have you inside of me, toy.  Lie down and start fingering your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " while I use the " + CockDescriptor.describeMultiCockShort(player) + " that I keep you here for.</i>\"\n\n");
				DisplayText("With your hands slightly trembling in anticipation, you lie down and grab at your very erect " + CockDescriptor.describeMultiCockShort(player) + " just before you slip your fingers into your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ", ready to please your mistress the best way you can.  You watch your mistress rise off her chair and slowly lower herself onto your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", moaning in pleasure.  Her interior is so familiar and so wonderful.  You increase the pace that your fingers plunge into your depths, moaning in the way you know your mistress loves. She starts to rock back and forth, your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " bringing her to the height of ecstasy. ");
				//[if player has 2 dicks] 
				if (player.torso.cocks.count === 2) DisplayText("Your " + CockDescriptor.describeCock(player, 1) + " rubs against her skin pleasantly.  ");
				//[if player has more than 2 dicks] 
				if (player.torso.cocks.count > 2) DisplayText("The rest of your " + CockDescriptor.describeMultiCockShort(player) + " rubs against her skin pleasantly.  ");
				...
				if (player.torso.cocks.count > 0 && (player.torso.vaginas.count === 0 || randInt(2) === 0)) {
					DisplayText("  You see that your " + player.inventory.equipment.armor.displayName + " have become a nice looking, if simple, frilled white shirt and bow tie along with black pants and matching shoes.  They make you look like you would be at home in a manor attending the needs of a noble, if it were not for the flaps on the front and back of your pants that give easy access to your " + CockDescriptor.describeMultiCockShort(player));
					if (player.torso.vaginas.count) DisplayText(", your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ",");
					DisplayText(" and your " + ButtDescriptor.describeButt(player) + ".  Any undergarments you had are now gone.\n\n");
					...
					DisplayText("After a minute of this, she stops you and says, 'Good, I am now ready to have you inside of me boy.  Show me your " + CockDescriptor.describeMultiCockShort(player) + " that I keep you here for.'\n\n");
					DisplayText("With a cry of joy you lift the front flap on your pants revealing your very erect " + CockDescriptor.describeMultiCockShort(player) + " and lie on the ground ready to please your mistress the best way you can.  You watch your mistress rise off her chair and slowly lower herself onto your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " moaning in pleasure.  Her interior is so familiar and so wonderful.  ");
					if (player.torso.vaginas.count > 0) DisplayText("You feel desire start to well up in your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ", but you resist the urge to act on it; that's not why your mistress keeps you. ");
					DisplayText("She starts to rock back and forth, your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " bringing her to the height of pleasure. ");
					if (player.torso.cocks.count === 2) DisplayText("Your " + CockDescriptor.describeCock(player, 1) + " rubs against her skin pleasantly.  ");
					if (player.torso.cocks.count > 2) DisplayText("The rest of your " + CockDescriptor.describeMultiCockShort(player) + " rub against her skin pleasantly.  ");
				...
				//(variant 2: maid, requires vagina)
					...
					if (player.torso.cocks.count > 0) DisplayText(", your " + CockDescriptor.describeMultiCockShort(player) + ",");
					DisplayText(" and your " + ButtDescriptor.describeButt(player) + ".\n\n");
					...
					DisplayText("'I'm glad to see you remember your place.  I wish you to first rub my feet, then move up my legs, and finally, start relieving the tension in my lower lips with your capable hands.'\n\n");
					DisplayText("Eager to please your mistress, now reclining on a lovely chair, you crawl to her, remove her shoes, and start to massage her feet.  Your hands seem to move with the will of some outside force and they expertly work to please your mistress.  Pushed on by her approving groans, you work your way up her legs, removing clothing as you go, until you reach her hips.  There, to your mistress' great delight, you run your hands over her hips, butt, and then start to rub her hips, lower lips, and clit.\n\n");
					DisplayText("After a minute of this, she says, 'Good.  Now finger yourself before me, girl.  Show me that " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " that I keep you here for.'\n\n");
					DisplayText("You lay back on the ground and raise your skirt, showing off your engorged " + VaginaDescriptor.describeClit(player) + " and " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ".  With a pleased squeal you plunge your fingers into your waiting " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " and start wriggling them around in the way that your mistress loves.  ");
					//[if player has a dick]
					if (player.torso.cocks.count > 0) DisplayText("You feel a desire rising in your erect " + CockDescriptor.describeMultiCockShort(player) + " but you ignore it; that's not what your mistress told you to play with.  ");
	...
	public playerRapesCultist(): void {
		...
		if (monster.armorName === "pervy nun's clothing") {
			...
			if (player.torso.cocks.count > 0 && (player.gender != 3 || randInt(2))) {
				...
				if (player.hips.legs.type === LegType.CENTAUR) DisplayText("You command her to get on all fours, then cover her and push ");
				...
				DisplayText("your ");
				DisplayText(CockDescriptor.describeMultiCockShort(player) + " into her waiting love hole.  ");
				if (player.torso.cocks.count > 1) {
					DisplayText("You thrust once, before using one of your hands to guide the second of your " + CockDescriptor.describeMultiCockShort(player) + " inside her.  You are rather surprised at how easily she seems to accommodate your size");
			...
			else if (player.torso.vaginas.count > 0 && player.hips.legs.type != LegType.CENTAUR) {
				DisplayText("She meekly lies back on the ground and spreads her legs, one slightly in the air.  You have a clear view of her needy hole through the large opening in her clothing.  \"<i>My lord, please scissor me!</i>\" she begs you.  You stand and stride over to her. \"<i>Yes little girl, accept my divine lips on your pussy,</i>\" you reply.  You grab the leg she had in the air and pull yourself between her legs, pressing your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " against her own.  With a firm tug on her leg, you begin to push and rub your pussies together in earnest, and her eager moans soon fill the air.  The cultist climaxes unexpectedly quickly, and the feeling soon rushes through you as well.  You pull back and the cultist slumps onto the ground contentedly.  Looking at the cultist you wonder if she was able to make you spontaneously orgasm just from having an orgasm herself, or if she has the power of a real sex god?\n\n");
			}
			//Horsey
			if (player.torso.vaginas.count > 0 && player.hips.legs.type === LegType.CENTAUR) {
			...
			MainScreen.simpleChoices(["FuckHerVag", "Vibrator", "", "", "Leave"], [fuckVag, vibe, null, null, cleanupAfterCombat]);
			return;
		}
		else {
			...
			//(if player has a dick)
			if (player.torso.cocks.count > 0 && (player.gender != 3 || randInt(2))) {
				DisplayText("Telling her that she is going to please you whether she wants to or not, you reveal your " + CockDescriptor.describeMultiCockShort(player) + ".  She screams for help and starts struggling against your grip, but you tell her that no one is going to save her now.  Easily overpowering her, you force her to the ground with no care for her comfort and start tearing off her clothes.  Once you have a clear view of her love hole, you look back up at her face and see that she has stopped struggling and is now crying.  ");
				//[if not corrupted] 
				...
				DisplayText("Filled with new determination, you push your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " into her love hole and start to rape her roughly.  Despite the tears, she starts moaning, clearly enjoying the rough treatment.  A little while into the rape, she starts to move against you, almost lovingly.  It is quite clear that she has a lot of experience, but is giving you free reign to do whatever you want.   Feeling in complete control, you decide that this game isn't too bad.  Before long you reach your orgasm and you blow your load deep inside her.");
			...
			//(if player has a vagina)
				...
				DisplayText("Telling her that she is going to please you whether she wants to or not, you reveal your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ".  Pushing your " + VaginaDescriptor.describeClit(player) + " to her mouth, you order her to lick you if she wants you to leave her mother alone.  ");
	...
	private plugSwimsuitVag(): void {
		...
		DisplayText("When you push your " + CockDescriptor.describeCock(player, x) + " inside her, she goes absolutely insane underneath you.  She squirms and wriggles all over your " + CockDescriptor.describeCock(player, x) + ", as you ");
		if (player.cockArea(x) < 50) DisplayText("push further and further until you're all the way inside her.");
		else if (player.cockArea(x) < 200) DisplayText("push further and further, until to your great delight, she manages to take all of your massive member without any trouble!");
		...
		DisplayText("After all the foreplay, the two of you are already reaching your peaks thanks to her incredible, tight cunt and its ministrations to your member.  The cultist is screaming in pleasure, telling you to look at her, to fondle her, and to fuck her; you happily oblige her.  Her interior contracts about your " + CockDescriptor.describeCock(player, x) + " as you loose the proof of your orgasm deep inside her.");
		if (player.torso.cocks.count > 1) {
	...
	private swimsuitVibrators(): void {
		...
		if (player.vaginalCapacity() <= 30) {//PC does not have a very loose or looser vagina)
			...
			DisplayText("she puts the vibrating toy to the entrance to your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ".  The next instant leaves you on your back as the cultist roughly shoves the thing inside you, forcing you to painfully stretch around the toy.");
			//if (PC was a virgin)
			if (player.torso.vaginas.get(0).virgin) {
				DisplayText("\n\n<b>Hell of a way to lose your virginity.</b>\n\n");
				player.displayStretchVagina(50, false, false, false);
			}
			else {
		...
		//It fits!
			...
			DisplayText("The cultist shows you how to activate the toy and they hum to life.  Quivering slightly, the two of you push the vibrating toys deep inside of yourselves.  The feeling of the toy filling up your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " is quite an experience.  ");
	...
	public fetishCultistHasAMilkFetish(): void {
		DisplayText().clear();
		DisplayText("You remove your " + player.inventory.equipment.armor.displayName + ", standing naked in front of the beaten fetish-girl with your lactating " + BreastDescriptor.describeBreastRow(player.torso.chest.get(0)) + " dripping milk. You pick the cultist up and sit down on the ground with her resting on your lap, her soft warm ass up against your thighs. You hold the nipple up to her mouth; she suckles on it without the slightest sign of rebellion.\n\n");
		...
		DisplayText("\"<i>Thank you " + player.mf("Master", "Mistress") + " for letting me have some of your special milk after doing all my chores.</i>\"\n\n");

		DisplayText("What is she talking about? " + player.mf("Master", "Mistress") + "? Chores? What the hell is going on? Your mind races with confusion as you try to figure out what's happening. The scene around you then changes back to the lake. The cultist, who looks like she normally would, sees your confusion.\n\n");
		...
		DisplayText("Soon her thong becomes wet with arousal and her mouth comes off your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + " to ask, \"<i>Please " + player.mf("Master", "Mistress") + ", will you fix the aching in my loins?</i>\"\n\n");
		...
		if (player.torso.cocks.count > 0) {
			...
			DisplayText("our " + CockDescriptor.describeMultiCockShort(player) + " becomes fully erect as it rubs up against her warm ass cheeks, which is enough to fulfill its desire.   Her cum bubbles into the palm of your hand while your cum drizzles her ass with sticky whiteness.\n\n");
		}
		//[ELSE Pussy]
		else if (player.torso.vaginas.count > 0) {
