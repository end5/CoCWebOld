...
export class FetishCultistScene {
    ...
    */
    ...
    public cultistRapesYou(player: Character) {
        cultist.lust = 1;
        cultist.stats.HP = 10;
        ...
        if (player.torso.hips.legs.type === LegType.CENTAUR || player.torso.hips.legs.type === LegType.NAGA) {
            ...
            if (player.torso.hips.legs.type === LegType.CENTAUR && player.torso.balls.quantity > 0 && player.torso.cocks.count > 0 && randInt(4) < 3) {
                if (player.torso.cocks.get(0).type === CockType.HORSE) {
                    ...
                    if (player.stats.int <= 10) this.cultistBadEnd();
    ...
    public playerRapesCultist(player: Character, cultist: Character) {
        DisplayText().clear();
        ...
        if (cultist.armorName === "pervy nun's clothing") {
            ...
            let vibe;
            let fuckVag;
            if (player.torso.vaginas.count > 0) vibe = this.swimsuitVibrators;
            if (player.torso.cocks.count > 0) fuckVag = this.plugSwimsuitVag;
    ...
    private plugSwimsuitVag(player: Character) {
    ...
    private swimsuitVibrators(player: Character, cultist: Character) {
        DisplayText().clear();
        ...
        if (player.vaginalCapacity() <= 30) {// PC does not have a very loose or looser vagina)
            ...
            User.flags.get<PlayerFlags>(CharacterType.Player).COMBAT_BONUS_XP_VALUE = cultist.XP;
            player.orgasm();
            player.stats.lib += 1;
            player.stats.sens += -3;
            return { next: Scenes.camp.returnToCampUseOneHour };
    ...
    private cultistBadEnd2(player: Character) {
    ...
    public fetishCultistHasAMilkFetish(player: Character) {
        DisplayText().clear();
