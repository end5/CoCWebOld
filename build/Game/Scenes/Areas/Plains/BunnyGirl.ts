...
export interface BunnyGirlFlags {
    MET_BUNBUN: number;
}

const bunnyGirlFlags: BunnyGirlFlags = {
    MET_BUNBUN: 0
};

User.flags.set(FlagType.BunnyGirl, bunnyGirlFlags);

export function bunnbunbunMeet(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Easter_Bunny);
    if (bunnyGirlFlags.MET_BUNBUN === 0) {
        DisplayText("A ");
        if (player.tallness > 64) DisplayText("short ");
        ...
        if (player.stats.lib > 50) DisplayText("and you find yourself wondering what sort of treasures she conceals behind it.");
        else if (player.stats.cor < 40) DisplayText("and you find yourself thankful for her modesty.");
        else DisplayText("and you find yourself considering tearing away the basket so you can see what she's hiding down there.");
        DisplayText("\n\n");

        DisplayText("Even though nearly a minute has passed, the bunny-lass is STILL frozen and staring.  She hasn't done anything since realizing that you're looking at her.  Well, it looks like the ball's in your court.  What do you do?");
        // [Talk] [Rape Her]
        return { choices: [["Talk", talkToBunnyBunBun], ["Rape Her", rapeBunBun]], persistantChoices: [["Leave", Scenes.camp.returnToCampUseOneHour]] };
    }
    // Met her
    else {
        // Bunbunsplosions!
        if (randInt(6) === 0 || (isEaster() && randInt(3) === 0)) {
            return adjathaEggsplosions(player);
        }
        ...
        DisplayText("You lean forwards for a better look, dislodging a small pebble with your " + Desc.Leg.describeFoot(player) + " while shifting position.  The bunny-girl leaps to her feet in a panic until her terrified irises lock onto your " + Desc.Face.describeFace(player) + ".  Her panic turns to embarrassment and in a flash every inch of her tanned skin is colored red.  She asks, \"<i>Ummm, since I'm still in s-season, c-could we b-breed or something?  Maybe just a little fuck?</i>\"\n\n");

        DisplayText("She claps both hands over her mouth, leaving her swollen prick to bounce precipitously while she awaits your response.\n\n");

        DisplayText("(If you're going to sex her, which of her body parts will you use?");
        player.stats.lust += 5 + player.stats.lib / 20;
        let DickInV;
        let Vagina;
        let sixtyNine;
        let eggs;
        if (player.torso.cocks.find(Cock.CockThatFits(40))) {
            Vagina = bunbunGetsFucked;
            DisplayText("  Fuck her vagina?");
        }
        else if (player.torso.cocks.count > 0) DisplayText("  <b>You're too to big fit inside her...</b>");

        // Dick requires one 40 area or smaller.
        if (player.torso.vaginas.count > 0) {
            DickInV = bunbunFucksYourVag;
            DisplayText("  Her dick in your vagina?");
        }
        if (player.gender > 0) {
            sixtyNine = bunbun69;
            DisplayText("  Sixty-nine her?");
        }

        if (player.pregnancy.ovipositor.canOviposit() && player.stats.lust >= 33) {
            if (!player.canOvipositBee() || randInt(2) === 0)
                eggs = ovipositBunnyEaster;
            else
                eggs = layEggsInBunbuns;
        }

        DisplayText("  Her dick in your ass?)");
        // let Ass:number = 0;
        ...
        return {
            choices: [
                ...
                ["69", sixtyNine],
                ["LayYourEggs", eggs],
                ["Your Ass", bunbunFucksPCInAss]],
            persistantChoices: [["Leave", Scenes.camp.returnToCampUseOneHour]]
        };
    }
}
// [Talk]
function talkToBunnyBunBun(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Easter_Bunny);
    bunnyGirlFlags.MET_BUNBUN++;
    DisplayText().clear();
    ...
    else if (player.torso.cocks.count > 0) DisplayText("  <b>You're too big to fit inside her...</b>");
    if (player.gender > 0) {
        sixtyNine = bunbun69;
        DisplayText("  Sixty-nine her?");
    }
...
function bunbunFucksYourVag(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Easter_Bunny);
    DisplayText().clear();
    DisplayText("A anticipatory grin widens across your " + Desc.Face.describeFace(player) + " as you speedily disrobe, discarding your " + player.inventory.equipment.armor.displayName);

    if (player.inventory.equipment.weapon.displayname !== "fists") DisplayText(" and " + player.inventory.equipment.weapon.displayname);
    DisplayText(" in a hurry.   Meanwhile the bunny looks on in a semi-aroused stupor, stroking her length with one hand while her other teases one of the hard nubs of her nipples.  You sigh with excitement as you position yourself above her, lining up her somewhat angular crown with the entrance to your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " before you start to drop.");
    if (player.torso.vaginas.get(0).wetness >= VaginaWetness.DROOLING) DisplayText("  Moisture leaks in a steady drizzle, mixing with bunny-pre to totally slick the soon-to-be invading member.");
    DisplayText("\n\n");

    DisplayText("Gasping hotly, you swallow her pointed tip into your depths, feeling the rabbit-like girl's tip swell and pump out a few more drops of lube");
    if (player.torso.vaginas.get(0).wetness >= VaginaWetness.SLICK) DisplayText(", not that you needed it");
    DisplayText(".  You don't need any more encouragement.  The long, slow slide down her thick bunny-dick is heavenly, as the veins on its surface scratch your 'itch' in just the right way.  ");
    if (player.vaginalCapacity() < 20) DisplayText("By the time you're getting close to the bottom you have to work to encompass her length and girth within your velvet folds and grunt with discomfort from each new inch of throbbing fuck-stick, but you take it all.  ");
    else if (player.vaginalCapacity() < 50) DisplayText("By the time you get to the bottom you're panting and moaning, delighting in the feeling of being so perfectly impaled on a rigid fuck-stick.   With her entirely inside you, you're ready to fuck her in earnest.  ");
    else DisplayText("It doesn't take long to slide down the more-than footlong meat-pole, and to be honest you wish she was a bit bigger.  Your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " is so ready for big dicks that this disproportionate member is average at best.  Still, you clench your thigh muscles to wring it a little tighter, and swoon from the feeling of warm maleness filling your loins.  ");
    Mod.Vagina.displayStretchVagina(player, 35, true);
    DisplayText("\n\n");

    DisplayText("While the bunny was content to remain passive up until this point, a fire lights in her eyes now that she's had a taste of pussy.  Her petite hands grab hold of your " + Desc.Butt.describeButt(player) + " while she pulls her powerful, oddly-jointed legs underneath of her.  A split second later she pushes up with enough force to bounce you a few inches into the air before you drop down on top of her.  ");
    if (player.tallness >= 72) {
        DisplayText("Your sheer weight makes her grunt in discomfort when you land back atop her, but the squish of copious sexual fluids nearly drowns it out. She even squirts another thick gout of pre inside you.  ");
        if (player.stats.cor < 33) DisplayText("You'll have to try to slow your descent next time!  ");
        else DisplayText("If she's going to fuck you like this, she can handle your weight!  ");
    }
    else DisplayText("Your bodies clap together with a loud squish of copious sexual fluids, and another thick burble of pre warms your cunny.  ");
    DisplayText("How much pre-cum can she squirt?!\n\n");

    DisplayText("You reach down and grab the bunny's hips for support as she bounces you again, higher this time.  A solid five or six inches of her length slide in and out of your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " before you slap into her, and a split second later, you're airborne again.  She bounces you harder and harder until each thrust of her muscled thighs is launching you nearly a foot off her loins and letting gravity guide you back down her shaft.");
    if (player.torso.clit.length >= 4) DisplayText("  Your " + Desc.Vagina.describeClit(player) + " bounces on her belly, tingling like mad every time it slaps into her tanned, sweat-slicked skin.");
    if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 1) DisplayText("  Milk begins to bead on your " + Desc.Breast.describeAllBreasts(player) + " from the sensation of the bunny's brutal, almost mechanical fucking.");
    DisplayText("  The eager girl moans, \"<i>Ooooh fuckfuckyes... gotta fuck... gotta breed... ungh... eggs eggs eggs!</i>\"\n\n");

    DisplayText("Eggs?  Is she going to knock you up with eggs?  The confusion she's caused actually distracts you from the sex long enough to push back your orgasm, but the amorous bunny-gal pushes herself over the edge with one last thrust, hard enough to nearly launch you from her tumescent cock.  You slide back down the exhausted bunny, but don't feel the telltale spurting and warmth you'd expect to be bursting inside your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ".  Sure, there is a trickle of warmth, but your convulsing lover's rod hasn't spurted like a male's organ should.   You turn to get a better look at her and notice that she STILL seems lost in orgasm, and her balls are GONE!\n\n");

    DisplayText("Your eyes cross from a sudden, massive change in thickness of your lover's pole.  It starts out at your lower lips, spreading them until they're positively gaping.  Muscular contractions in the bunny's shaft make her dick pulse inside you, slowly pushing the bulge upwards and stretching your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + "'s walls as it travels towards your womb.  More and more fluid leaks inside of your channel until your " + Desc.Leg.describeLegs(player) + " relax and go limp.  It isn't just pre-cum anymore – it's dulling the pain and relaxing your body!  You can actually feel your cervix starting to open involuntarily as the bulge passes the halfway point.  From the distended spot downwards, your cunt hangs loosely around the swollen cock, but another knot emerges at the base and starts to slide up your abused tunnel.  ");
    // (Cunt Change HERE)
    Mod.Vagina.displayStretchVagina(player, 70, true);
    DisplayText("\n\n");

    DisplayText("The girl underneath you is thrashing and moaning, chanting, \"<i>Yes... eggs eggs EGGS! YES!</i>\" while her ovipositor-like cock robs you of your strength and slides egg-shaped bulges inside you.  Amazingly, your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " is awash with pleasure, and you reach down to ");
    if (player.torso.clit.length >= 4) DisplayText("fondle your " + Desc.Vagina.describeClit(player) + " a moment before wrapping your hand around it and stroking the girl-cock excitedly.");
    else DisplayText("caress your " + Desc.Vagina.describeClit(player) + " a moment before you start to circle it with a finger, teasing it expertly.");
    if (player.torso.cocks.count > 0) {
        DisplayText("  " + Desc.Cock.describeMultiCockSimpleOne(player, true) + " twitches and drools ");
        if (player.torso.cocks.get(0).length < 10) DisplayText("on the bunny's belly");
        else if (player.torso.cocks.get(0).length < 18) DisplayText("between the bunny's tiny tits");
        else if (player.torso.cocks.get(0).length < 28) DisplayText("on the bunny's face");
        else DisplayText("past the bunny's head");
        DisplayText(", and your free hand wastes no time in tending to your male ");
        if (player.torso.cocks.count === 1) DisplayText("half's needs, slathering it in pre-cum and stroking hard and fast.");
        else DisplayText("halfs' needs, slathering them in pre-cum and stroking hard and fast.");
    }
    DisplayText("  The futanari rabbit's dick-head swells as the egg reaches her tip, completely opening your womb, and then with one last explosive push, launches it deep inside you.\n\n");

    DisplayText("You get off hard.  Despite the drug-induced relaxation that's overwhelmed you from the waist down, rippling convulsions erupt up and down your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", squeezing the egg-spurting cock tightly.  This only speeds the passage of the bunny's second bulge, and in seconds her distended cock-head is stretched wider than before, locking her inside you for a moment.   The pair of you rock and grind your hips together futilely until the final egg erupts inside of you, accompanied by a huge wave of whatever it is that's made your lower body so nerveless.");
    if (player.torso.cocks.count > 0) {
        DisplayText("  At last, " + Desc.Cock.describeMultiCockSimpleOne(player) + " blasts sticky ropes of seed everywhere, cumming with you as your body slides sideways off the exhausted bunny into the dirt.  A few thick strands hit her tits and face, but judging by her half-closed eye-lids and questing tongue, she doesn't mind.");
        if (player.cumQ() > 500) DisplayText("  Not content with that, " + Desc.Cock.describeMultiCockSimpleOne(player) + " continues to pump until the bunny is coated with a generous layer of seed");
        if (player.cumQ() > 2000) DisplayText(" and the excess is running down the hill in a small river");
        if (player.cumQ() > 500) DisplayText(".");
    }
    DisplayText("\n\n");
    ...
    if (RaceScore.bunnyScore(player) < 4) DisplayText("It's so good to finally think straight!  I don't know why I went into heat right now, but it's so much easier to think now that I've gotten rid of those eggs.  Don't worry, you won't be popping out rabbits since you aren't an easter-bun.  Your body will probably absorb them in a few days.  Just don't be surprised if you feel a little bloated or feel a little weird after.</i>\"\n\n");
    // Pregnant already
    else if (player.pregnancy.womb.isPregnant()) DisplayText("It's so good to finally think straight!  I don't know why I went into heat right now, but it really is a shame you're pregnant.  Your body will destroy the little eggs before they can even do anything!  Well, at least I feel sooo much better...</i>\"\n\n");
    // Preggers
    else DisplayText("It's so good to finally think straight!  I don't know why I went into heat right now, but it feels so good to pour my babies into a nice, ready breeder-mom like yourself.  I bet the kids come out so cute!  Oh, I feel so much better.</i>\"\n\n");

    DisplayText("She walks away, her puffy tail twitching with the breeze while your eyes drift closed.");
    // (Easter vag-preg + 8 hours pass)
    if (RaceScore.bunnyScore(player) < 4) {
        player.pregnancy.womb.knockUp(new Pregnancy(PregnancyType.BUNNY, IncubationTime.BUNNY_EGGS));
    }
    else {
        player.pregnancy.womb.knockUp(new Pregnancy(PregnancyType.BUNNY, IncubationTime.BUNNY_BABY), 60);
    }
    player.orgasm();
    player.stats.lib += 1;
    player.stats.sens += -3;
...
// [Take Dick In A]
function bunbunFucksPCInAss(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Easter_Bunny);
    DisplayText().clear();
    DisplayText("You get a very naughty idea and silently discard your " + player.inventory.equipment.armor.displayName + ", tossing it aside as you turn around and present your " + Desc.Butt.describeButt(player) + " to the bunny-girl.  With a lewd shake, you tease her and give her a good view of your " + Desc.Butt.describeButthole(player.torso.butt) + " while it lowers closer and closer to the turgid bunny-cock, just inches away.  Drops of clear pre-cum roll down her shaft as it twitches eagerly, and the girl watches you through a gleam of sexual excitement while her shaft gets closer and closer to your rear entrance.  You let your " + Desc.Leg.describeLegs(player) + " relax a little until it's pressing tightly against your sphincter, feeling it slather pre-cum in preparation for the coming penetration.\n\n");

    DisplayText("Looking over your shoulder at the bunny's beet-red face, you let a little of your weight down and start to spread around the bunny-herm's cock.  It slides inside you easily, aided by her copious drops of pre-cum, but ");
    if (player.analCapacity() < 20) DisplayText("it stretches you dangerously wide as you slide down the thick shaft.  The further down it gets, the more you have to work to relax your muscles and push yourself along.  With enough time, patience, and copious bunny-pre, you manage to take it completely inside you.  It twitches happily while you adjust to the intrusion, but you know you're just getting started.");
    else if (player.analCapacity() < 40) DisplayText("you have to keep pausing as you slide down to let more of her pre-cum bubble out and lubricate the fourteen inch shaft.  It fills you nicely, pressing on your innards in all the right ways as you slowly engulf the bunny-cock with your " + Desc.Butt.describeButthole(player.torso.butt) + ".  After bottoming out, your nervous lover's prick pulsates happily inside you, but you're just getting started.");
    else {
        DisplayText("you wish it was a bit bigger.  A few seconds is all it takes to get her thick shaft completely inside your " + Desc.Butt.describeButthole(player.torso.butt) + ".  You clench and squeeze your muscles around it as you sit on the bunny-");
        if (player.stats.cor < 50) DisplayText("girl");
        else DisplayText("bitch");
        DisplayText("'s lap, giggling as you feel it twitch happily inside you, but you're just getting started.");
    }
    // (BUTT CHANGE HERE)
    Mod.Butt.displayStretchButt(player, 35, true);
    DisplayText("\n\n");
    ...
    if (player.torso.cocks.count > 0) DisplayText("Each time she bumps and slides past your prostate a dollop of your pre-cum spurts from " + Desc.Cock.describeMultiCockSimpleOne(player) + " onto the ground, and you have a hard time not using your new lover like an anal toy and masturbating yourself.  ");
    DisplayText("The long-eared slut makes a display of touching her nipples, pulling and twisting on them as you watch her over your shoulder.\n\n");

    DisplayText("Without meaning to, you begin to pick up the speed of your up and down strokes.  Your body is feeling horny and warm from all the sex, and having such a strange, attractive lover mating with your backside isn't helping.  ");
    if (player.torso.vaginas.count > 0) {
        if (player.torso.vaginas.get(0).wetness >= VaginaWetness.SLICK) DisplayText("Juices begin to run from your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", but you ignore the empty feeling and focus on the throbbing meat inside your backdoor.  ");
    }
    DisplayText("Fingers find their way to your nipples without conscious thought, and begin ");
    if (!player.torso.chest.find(BreastRow.FuckableNipples)) {
        DisplayText("pulling and tugging on them");
        if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 1) {
            DisplayText(", spurting out ");
            if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 4) DisplayText("sprays");
            else DisplayText("drops");
            DisplayText(" of milk");
        }
        DisplayText(".");
    }
    else DisplayText("sliding inside them, fucking their cunt-like interiors as you start to lose yourself to the pleasure.");
    DisplayText("  The bunny reaches down to your " + Desc.Butt.describeButt(player) + " and gives it a gentle caress and squeeze.  A moment later both her hands are holding your butt-cheeks, guiding you up and down as you bounce atop her faster and faster.\n\n");
    ...
    if (player.torso.cocks.count > 0) {
        DisplayText(", and " + Desc.Cock.describeMultiCockSimpleOne(player) + " pouring incredibly sticky pre-cum each time she squeezes your prostate");
    }
    else if (player.torso.vaginas.count > 0) DisplayText(", and your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " clenching and dripping on the verge of orgasm");
    DisplayText(".");
    if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 1) {
        DisplayText("  Milk ");
        if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier < 2) DisplayText("leaks");
        else if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier < 4) DisplayText("drips");
        else DisplayText("pours");
        DisplayText(" from your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s, turning your front to a leaky mess as you ride the very edge of pleasure.");
    }
    DisplayText("\n\n");
    ...
    DisplayText("You gasp in pain as the thickness at the base of your lover's shaft doubles.  It stretches your " + Desc.Butt.describeButthole(player.torso.butt) + " wide, almost painfully wide as something starts to push up her dick from the inside.  Before you can pull yourself off her, a massive gush of fluid drizzles inside of you, quickly numbing any pain and stealing the strength from your " + Desc.Leg.describeLegs(player) + " and muscles.  You twist back and forth, but the pleasure intensifies as the bulge slips past your sphincter, leaving your body slightly agape in its wake.  You find the rest of its passage to be more comfortable ");
    if (player.torso.cocks.count > 0) {
        DisplayText("and as it squeezes past your prostate, " + Desc.Cock.describeMultiCockSimpleOne(player) + " spurts thick cum on the ground, though you don't QUITE orgasm.");
    }
    else DisplayText("and your arousal returns in force.");
    // (BUTT CHANGE HERE)
    Mod.Butt.displayStretchButt(player, 70, true);
    DisplayText("\n\n");

    DisplayText("A moment before the bulge reaches the tip of the rabbit-cock, you feel another sliding through your " + Desc.Butt.describeButthole(player.torso.butt) + ".  The bunny-dick's head thickens for a moment before it dwindles down to its normal shape.  You feel something warm and orb shaped inside you – it's an egg!  The strangeness of the insemination can't hold back the excitement your body built up or the feelings the second egg's passage is leaving in its wake, and you cum hard.  Somehow your body gets enough control to squeeze the girl's penis tightly, but all it accomplishes is speeding the eggs passage as it spurts into your rectum along with another big burst of pleasant, relaxing fluids.");
    if (player.torso.cocks.count > 0) {
        DisplayText("  " + Desc.Cock.describeMultiCockSimpleOne(player, true) + " erupts in truth, spraying and splattering white goop over the grass and ground.");
        if (player.cumQ() > 500) DisplayText("  The splattering of your seed only increases in volume as it makes a rather large puddle");
        if (player.cumQ() > 2000) DisplayText(", but you just keep going until you create a lazily winding river of cum that flows its way through the grasses");
        if (player.cumQ() > 500) DisplayText(".");
    }
    if (player.torso.vaginas.count > 0) DisplayText("  Your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " clenches, empty, drooling, and neglected, wishing it something inside it.");
    DisplayText("\n\n");
    ...
    player.pregnancy.buttWomb.knockUp(new Pregnancy(PregnancyType.BUNNY, IncubationTime.BUNNY_EGGS), 1, true);
    player.orgasm();
    player.stats.lib += 1;
    player.stats.sens += 1;
    return { next: Scenes.camp.returnToCampUseEightHours };
...
function bunbunGetsFucked(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Easter_Bunny);
    DisplayText().clear();
    const cocksThatFit = player.torso.cocks.filter(Cock.CocksThatFit(40));
    const firstCockThatFits = cocksThatFit[0];
    const secondCockThatFits = cocksThatFit[1];
    DisplayText("You disrobe and toss your " + player.inventory.equipment.armor.displayName + " to the side, immediately forgetting about it as the bunny-girl lifts her sack to expose the bright pink flesh of her femininity.  It glistens, practically steaming up the air with her plentiful lubricants.  She runs a slender finger around the moist hole, beckoning you to plunge inside and fill her hungry flesh with your " + Desc.Cock.describeCock(player, firstCockThatFits) + ".  Lust burns through you as you drop to your knees and line your " + Desc.Cock.describeCock(player, firstCockThatFits) + " up with that ready opening");
    if (secondCockThatFits) DisplayText(", taking care to align your " + Desc.Cock.describeCock(player, secondCockThatFits) + " with her tightly puckered backdoor as well.");
    else DisplayText(", barely able to restrain yourself as the time for penetration approaches.");
    DisplayText("\n\n");

    DisplayText("The nubile bunny's eyes cross as you push forwards, sliding your " + Desc.Cock.describeCockHead(firstCockThatFits) + " over her love-slicked lips until it has a generous coating and pressing harder.  ");
    if (secondCockThatFits) DisplayText("She squeaks at the pressure on her tender asshole, but doesn't object as your pre-cum smears over it.  ");
    DisplayText("At once, her greedy breeding hole relaxes ");
    if (secondCockThatFits) DisplayText("along with her anus ");
    DisplayText("and your " + Desc.Cock.describeCock(player, firstCockThatFits));
    if (secondCockThatFits) DisplayText(" and " + Desc.Cock.describeCock(player, secondCockThatFits) + " sink");
    else DisplayText(" sinks");
    DisplayText(" into her slippery, warm depths.  She pants and groans with happiness, going so far as to vocalize her pleasure, \"<i>Oooh yes, I don't know why but it feels just as good as breeding season!  If you keep this up, " + Desc.Gender.mf(player, "stud", "sexy") + ", you might... ahhhh.. make me egg!</i>\"\n\n");

    DisplayText("You don't try to make sense of it and instead plunge further forward, watching with delight as more and more of your length is devoured by the horny bunny's body.  ");
    if (firstCockThatFits.area >= 25) DisplayText("Her belly bulges pleasantly, displaying the vaguely cylindrical shape of a cock through her skin in a way that makes you feel utterly dominant over this 'female'.  ");
    ...
    if (secondCockThatFits) DisplayText("Her tight asshole slowly loosens around your " + Desc.Cock.describeCock(player, secondCockThatFits) + ", and so much lubricant splatters from her cunt with each thrust that the sensations of both your dicks become nigh-identical, save for the tight ring gripping the bottom one.");
    else {
        DisplayText("So much lubricant splatters from her cunt that your thighs");
        if (player.torso.balls.quantity > 0) DisplayText(", belly, and balls");
        else DisplayText(" and belly");
        DisplayText(" are soon sticky with the stuff, but it only spurs you on to fuck her harder.");
    }
    DisplayText("\n\n");
    ...
    if (secondCockThatFits) DisplayText("  Surprisingly, her asshole stays about the same, but it was already a little tighter and you know you'll be pumping it full of goo in a second.");
    DisplayText("\n\n");
    ...
    DisplayText("You bottom out with brutal force");
    if (player.torso.balls.quantity > 0) DisplayText(", slapping your balls against her ass");
    DisplayText(", as your ");
    if (player.torso.cocks.find(Cock.HasSheath)) DisplayText("sheath");
    else DisplayText("base");
    DisplayText(" rubs her passion-inflamed pussy-lips.  " + Desc.Cock.describeMultiCockSimpleOne(player, true) + " unloads, splattering out ropes of cum into ");
    if ((player.torso.cocks.count === 2 && !secondCockThatFits) || player.torso.cocks.count > 2) DisplayText("and onto ");
    DisplayText("the insensate bunny-breeder.  Her silken pussy milks and squeezes as you cum, seeming to draw your seed from your cock itself and pull it deeper inside, leaving none to waste.");
    if (secondCockThatFits) {
        DisplayText("  Her ass gurgles from your deposit");
        if (player.cumQ() > 500) DisplayText(", and her stomach gains a bit of pudge from the copious jism deluge you pump into her");
        DisplayText(".");
    }
    if (player.cumQ() > 500) DisplayText("  You keep cumming, slamming blast after blast of spooge into her welcoming nethers, delighted that none seems to leak out.");
    if (player.cumQ() >= 1500) DisplayText("  Somehow her womb never seems to fill or bulge, even though you must be pumpings gallons of fertile baby-batter into her cunt.  By the time your orgasm winds down, you feel a little confused by it all, but sated.");
    DisplayText("\n\n");
    ...
    if (player.cumQ() > 100) DisplayText("  A few seconds later another rolls in, cramming three into her increasingly obscene looking 'sack'.");
    if (player.cumQ() > 250) DisplayText("  One more follows, giving her quads");
    let z: number = 8 + randInt(player.cumQ() / 100);
    if (z > 50) z = 50;
    if (player.cumQ() > 700) DisplayText(", but she's not done.  Her gut rumbles loudly, and then one after another egg after egg pours into the sack, stretching it out until you're sure she has at least " + z + " in there and will have trouble walking.");
    else DisplayText(".");
    DisplayText("  She moans contentedly and closes her eyes, sighing blissfully.\n\n");

    DisplayText("You shrug and pick up one of her eggs, noticing that it's turned neon pink in color.  The bunny mumbles, \"<i>Have it, iz good for youuuu...</i>\" before she starts to snore and murmur out a sexual dream.\n\n");
...
function bunbun69(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Easter_Bunny);
    DisplayText().clear();
    // Centaur
    if (player.torso.hips.legs.isTaur()) {
        if (randInt(2) === 0) {
            ...
            DisplayText("Without thinking it over beyond 'that sounds hot', you declare your intention to 69 the bunny girl.  She stands there, mouth hanging open and heat briefly forgotten.\n\n");

            DisplayText("\"<i>What?</i>\"\n\n");

            DisplayText("By now you've realized the absurdity of your statement, but you're too proud to admit it and simply restate your goal.  You cross your arms to try to look as serious as possible.  She looks down at your equine portion while idly stroking her cock, making sure your \"equipment\" is where she thinks it is.\n\n");

            DisplayText("\"<i>You're weird,</i>\" declares the rabbit.\n\n");

            DisplayText("Before you can return with a witty remark, the bunny has hopped off, leaving you alone in the field with your face in your palm.\n\n");
            // - Intelligence
            // + Lust
            player.orgasm();
            player.stats.int += -2;
            ...
            DisplayText("Without thinking it over too hard beyond 'that sounds hot', you declare your intention to 69 the bunny girl.  She bursts into laughter.  You're a little perturbed by this and make it clear you're quite serious by restating your goal.\n\n");

            DisplayText("\"<i>No, no, stop – ahaha - you're killing me - haha!</i>\" she splutters.  \"<i>Hahaha - oh gods - I'm going to egg from laughter - HAHAHA!</i>\"\n\n");
            ...
            DisplayText("\"<i>Hahah! Egg on your face! Ahahah!</i>\"\n\n");

            DisplayText("You wipe off what you can of the sticky goop and trot off with a scowl, leaving the hysterical rabbit to calm herself down.\n\n");
            // - Intelligence
            player.orgasm();
            player.stats.int += -2;
            // + Lust
    ...
    if (player.torso.cocks.count > 0) {
        ...
        if (isEaster()) {
            ...
            DisplayText(", whiskers vibrating with a happy little hum, her buck teeth massaging your root enthusiastically, impatient for her Champion-cream filling. Her hands slide from the back of your head, along your neck, down your back, and to your ass, fingers wrapping around to find your " + Desc.Body.assholeOrPussy(player) + ". Slipping both middle fingers in, she strokes your ");
            if (player.torso.vaginas.count <= 0) DisplayText("prostate");
            else DisplayText("inner walls");
            DisplayText(" until your orgasm overpowers your saccharine-fat daze and you peak, egg-bloated body spurting in creaming jets of pale seed to paint the brown bunny's insides white.\n\n");
            ...
            if (!player.torso.tails.find(Tail.FilterType(TailType.BUNNY))) {
                // [No Tail:
                if (player.torso.tails.count === 0) DisplayText("  The heat around your lower back clutches at your tail bone and you feel a fluffy explosion swell around your butt cheeks as <b>a fluffy bunny's tail pops out of your back</b>, twitching excitedly!");
                // [Existing tail:
                else DisplayText("  Your tail jiggles and shrinks, slowly sucking back into your body until only a tiny nub remains which quickly explodes with fluff into <b>your new cuddly, twitching bunny tail!</b>");
            }
            if (player.torso.neck.head.ears.type !== EarType.BUNNY) {
                DisplayText("  The top of your head is next, it seems.");
                // [No antennae:
            ...
            DisplayText("With a happy sigh, the girl slides off of your cock and lets it flop against your leg, not a single drop of cum wasted. Her belly has ");
            if (player.cumQ() < 250) DisplayText("a slight swell");
            else if (player.cumQ() < 1000) DisplayText("a noticeable girth");
            else DisplayText("bloated obscenely");
        ...
        // REGULAR SHOW
            ...
            if (player.torso.balls.quantity > 0) DisplayText("balls");
            else if (player.torso.vaginas.count > 0) DisplayText("clit");
            else DisplayText("groin");
            DisplayText(", whiskers vibrating with a happy little hum, her buck teeth massaging your root enthusiastically, impatient for her own creamy meal. Her hands slide from the back of your head, along your neck, down your back, and to your ass, fingers wrapping around to find your " + Desc.Body.assholeOrPussy(player) + ". Slipping both middle fingers in, she strokes your ");
            if (player.torso.vaginas.count <= 0) DisplayText("prostate");
            else DisplayText("inner walls");
            DisplayText(" until your orgasm overpowers your bunny-fucked daze and you peak, your egg-bloated body spurting in gushing jets of pale seed to paint the olive girl's insides white.\n\n");
            ...
            if (!player.torso.tails.find(Tail.FilterType(TailType.BUNNY))) {
                // [No Tail:
                if (player.torso.tails.count === 0) DisplayText("  The heat around your lower back clutches at your tail bone and you feel a fluffy explosion swell around your butt cheeks as a fluffy bunny's tail pops out of your back, twitching excitedly!");
                // Existing tail:
                else DisplayText("  Your tail jiggles and shrinks, slowly sucking back into your body until only a tiny nub remains which quickly explodes with fluff into a cuddly, twitching bunny tail!");
            }
            if (player.torso.neck.head.ears.type !== EarType.BUNNY) {
                DisplayText("  The top of your head is next, it seems.");
                // [No antennae:
                DisplayText("  Your eyebrows feel like they're being drawn upwards, your eyes getting larger and larger until you're almost painfully aware of every color and sound around you. You shake your head and large, floppy bunny ears bounce in front of your eyes.");
            }
            DisplayText("\n\n");
            ...
            DisplayText(", your spunk calming the egg-laying passion. She pulls her shrinking phallus from your mouth with a wet slurp, the taste of her strange cum sweet on your lips. She rises to a crouch and gives your ");
            if (player.torso.neck.head.ears.type !== EarType.BUNNY) DisplayText("new ");
    ...
    // CUNTS
        ...
        DisplayText("With a happy sigh, the girl rubs your slick honey off of her face and gives your clit a final, pleased lick. She pulls her shrinking phallus from your mouth with a wet slurp, the taste of her cum sweetly icy on your lips. She rises to a crouch and gives your ");
        if (player.torso.neck.head.ears.type !== EarType.BUNNY) DisplayText("new ");
        DisplayText("ears a playful tweak between her thumb and forefinger. \"<i>Sorry about that, I don't know what came over me! I certainly didn't expect this, though! Kind of makes me want to stick around and see if you and I could pop out more bunnies,</i>\" she winks. \"<i>But unfortunately, I've got to get going! Maybe I'll try to find you again, some time down the line.</i>\" She gives you a moist little kiss and hops away, still energetic after all that. You groan, still recovering from the eggs.");
        player.torso.tails.add(new Tail(TailType.BUNNY));
        player.torso.neck.head.ears.type = EarType.BUNNY;
...
export function layEggsInBunbuns(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplayText("The displayed lapin in front of you has you excited, but not for a simple roll in the grass.  Your ");
    if (player.pregnancy.ovipositor.eggs < 20) DisplayText("eager egg-tube slips free, ready to show the rabbit you can play the egg game too.");
    else if (player.pregnancy.ovipositor.eggs < 40) DisplayText("distended bug part quivers in anticipation, the ovipositor pushing into view with a drop of honeyed pre on its tip.");
    else DisplayText("low-hanging bee abdomen drools honey-like ichor down its carapace from the bulge where the thick black organ shows its need to relieve your burden.");
    DisplayText("  The bunny gasps upon noticing it, the herm's hips wiggling. \"<i>Oh my, could... could you mount me with that?  I need to egg so badly.</i>\"  Not needing to be urged further, you arch your bee-section up underneath you while lifting the lusty bunny against you.");
    ...
    if (player.gender === Gender.NONE) DisplayText(" against the empty place of your crotch.");
    else {
        if (player.torso.cocks.count > 0) DisplayText(" alongside [eachCock]");
        if (player.torso.vaginas.count > 0) DisplayText(" and her hefty oval balls rubbing the [vagina] tucked between your thighs");
        DisplayText(".");
    }
    DisplayText("  She braces herself with arms around your shoulders, trying to nearly jump onto you as you ram your engorged ovipositor into her tight ass.");
    ...
    if (player.gender > 0) {
        ...
        DisplayText(" lathering the bunny-girl in ");
        if (player.torso.cocks.count > 0) DisplayText("spunk");
        if (player.gender === Gender.HERM) DisplayText(" and ");
        if (player.torso.vaginas.count > 0) DisplayText("femcum");
    }
    DisplayText(".  Stuffing a warm hole with egg after egg is not only fulfilling (for <i>both</i> involved) but is also damn fun, which the rabbit agrees with!");
    ...
    if (player.gender > 0) {
        DisplayText("  You add to the mess, ");
        if (player.torso.cocks.count > 0) DisplayText("spraying a long, thick load against the bunny's soft tits from your [eachCock]");
        if (player.gender === Gender.HERM) DisplayText(" and ");
        if (player.torso.vaginas.count > 0) DisplayText("splashing her balls with your girlcum");
        DisplayText(".");
    }
...
export function ovipositBunnyEaster(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplayText("Smiling a little over-eagerly, you suggest, \"<i>You know, if you really like eggs, I know where you can get a few...</i>\"");
    ...
    if (player.canOvipositBee()) DisplayText("insect");
    else DisplayText("arachnid");
    DisplayText(" claiming an incubator.  When she feels your groin pressing on the small of her back, the fluffy hare grows concerned, looking over her shoulder and declaring, \"<i>You... you can't put eggs in me like THAT!</i>\"");
    ...
    // {Plenty of eggs}
        ...
        if (!player.canOvipositBee()) DisplayText("arachnid");
        else DisplayText("insectile");
        DisplayText(" cargo, but you aren't empty, so you force more and more inside her.  She's so stretchy that it's easy to do, and before long she has a nicely rounded, pregnant belly (even if it is a little bit bumpy).  Shuddering, you release a few long, thick squirts of lubricant over your leavings and sag down, spent.");
    }
    // {Lots of Eggs!}
    else {
        ...
        if (User.settings.silly()) DisplayText("like an energizer bunny");
        else DisplayText("like some kind of endless, pregnancy-inducing fuck-beast");
...
// -----------
function adjathaEggsplosions(player: Character): NextScreenChoices {
    DisplayText().clear();
    ...
    if (!isEaster()) DisplayText("dusky");
    else DisplayText("chocolate");
    DisplayText(" skinned woman.  Atop her ");
    if (!isEaster()) DisplayText("tanned");
    else DisplayText("caramel");
    ...
    if (!isEaster()) DisplayText("thin, gushing pre-cum");
    else DisplayText("thick, syrupy cream");
    DisplayText(". It's a bunny-girl, certainly, but who would go through the effort of tying her up and then just leave?");
    ...
    if (!isEaster()) DisplayText("olive");
    else DisplayText("cinnamon");
    DisplayText(" skin.  \"<i>Oh thank goodness,</i>\" she pants, her tongue lolling under her tiny buck teeth.  \"<i>You have to help me,</i>\" the ");
    if (!isEaster()) DisplayText("coffee");
    else DisplayText("toffee");
    ...
    let eggFlated;
    if (player.torso.vaginas.count > 0) eggFlated = getEggflated;
    // [Fuck Her] (Male/Futa Only)
    let fuckHer;
    if (player.torso.cocks.count > 0) {
        if (player.torso.cocks.find(Cock.CockThatFits(40))) fuckHer = fuckTheEggBoundBun;
        else DisplayText("  <b>You're too big to fuck her!</b>");
    }
    return {
        choices: [
            ["Free Her", freeHerOhGodWhyDidYouDoThis],
            ["Get Egged", eggFlated],
            ["Fuck Her", fuckHer]
        ],
        persistantChoices: [["Leave", Scenes.camp.returnToCampUseOneHour]]
...
// [Free Her] (Any gender)
function freeHerOhGodWhyDidYouDoThis(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplayText("Taking a chance, you step up to the wriggling bunny and inspect her bonds.  The girl's body is covered in a ");
    if (!isEaster()) DisplayText("sheen of sweat");
    else DisplayText("shimmering glaze");
    DisplayText(", her urgent lust pulsing in every inch of her ");
    if (!isEaster()) DisplayText("freckled brown");
    else DisplayText("pudding rich");
    DisplayText(" skin.  Her abdomen is slightly rounded with a pot-bellied pout that you assume must be the seed of the previous Samaritans.  Due to her inability to climax, her pussy must be clenching so tightly that only a thin trickle of cum is able to escape.  Her ");
    if (isEaster()) DisplayText("cony ");
    DisplayText("cock jerks in the air with every hyperventilating breath, ");
    if (!isEaster()) DisplayText("bubbling pearls");
    else DisplayText("gooey marshmallows");
    DisplayText(" of pre dribbling from the narrow, spear-like crest. The lustful girl's balls are even more lewd close up, gargantuan orbs obscenely stuffed with ");
    if (!isEaster()) DisplayText("the carnal fruit of her unwelcome, monstrous couplings");
    else DisplayText("candied plenty, her saccharine treats swimming amid a warm sea of honeyed spunk");
    DisplayText(". She bites down on her lower lip with her prominent front two teeth, her eyes following your motions intently, hoping for the best but braced for the worst.");
    ...
    if (!isEaster()) DisplayText("splattering spoo");
    else DisplayText("climaxing cream");
    DisplayText(".  The showering spray continues in an unbroken outpouring, a river of alabaster arcing and gushing in a throbbing surge that white-washes everything within ten feet of her frontal arc.  Countless ");
    if (!isEaster()) DisplayText("bright pink eggs");
    else DisplayText("candy-coated confections");
    DisplayText(" issue forth amid the ");
    if (!isEaster()) DisplayText("cum");
    else DisplayText("icing");
    DisplayText(" geyser, plopping moistly here and there.  Some stop in plain sight, others roll into hidden hollows, left for some curious treasure hunter.");
    ...
    if (!isEaster()) DisplayText("pert");
    else DisplayText("gum drop");
    DisplayText(" nipples as her big, soft toes dig rutting furrows into the ground beneath her.  Her orgasmic throes send spasms of bliss through her whole body, her clenched inner muscles finally relaxing enough to release the jism inside her womb, vulgar globules oozing down her trembling inner thighs.");
...
// [Fuck Her] (Male/Futa Only)
function fuckTheEggBoundBun(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplayText("It's not often you find a treat gift wrapped so neatly for you; what kind of ingrate would you be if you just turned it down?  You step up to the wriggling bunny and inspect her carefully.  The girl's body is covered in a ");
    if (!isEaster()) DisplayText("sheen of sweat");
    else DisplayText("shimmering glaze");
    DisplayText(", her urgent lust pulsing in every inch of her ");
    if (!isEaster()) DisplayText("freckled brown");
    ...
    if (!isEaster()) DisplayText("dusky");
    else DisplayText("caramel-skinned");
    DisplayText(" bunny-girl with an easy smile.  Loosening your [armor] just enough to let your stiffening cock");
    ...
    if (!isEaster()) DisplayText("supple mounds");
    else DisplayText("taut dough");
    ...
    if (!isEaster()) DisplayText("yielding flesh");
    else DisplayText("semi-spongy hotcakes");
    ...
    if (player.stats.cor < 33) DisplayText("grimace at the vulgar goo that washes over your flesh");
    else if (player.stats.cor < 66) DisplayText("swallow a gulp as the lewd spoo squelches around your member");
    else DisplayText("revel in the profane stimulation of bestial batter anointing your length in impure, liquid virility");
    DisplayText(".");
    ...
    if (!isEaster()) DisplayText("darkly freckled thighs ivory");
    else DisplayText("savory, chocolate thighs with pale cream");
    DisplayText(".  Reaching your own threshold, you lean forward, pressing your cheek against hers, the intake of your breath hissing between your teeth.  She instinctively understands, slamming her ass against your [hips] with as much force as she can muster.");

    DisplayText("\n\nYou orgasm, [oneCock] erupting inside the frenzied woman's womb with thick gouts of heavy seed ");
    if (!isEaster()) DisplayText("flooding her semen-gorged cunt with fresh issue");
    else DisplayText("glazing her cream-filled center with your silky spunk");
    DisplayText(".  She bucks helplessly as her gut swells under your load, ");
    if (player.cumQ() < 250) DisplayText("steady streams of cum seeping from her pussy");
    else if (player.cumQ() < 1000) DisplayText("bulging enough that her belly button turns outward as the cum oozing from her pussy gushes between the two of you");
    else DisplayText("growing larger and larger as cum splatters in thick streams from her saturated pussy. Your seemingly endless gallons of flooding cum wash the remnants of her previous partners away, immersing the caramel woman in the fluid wealth of your liquid possession.");
...
// [Get Egged] (Female/Futa only)
function getEggflated(player: Character): NextScreenChoices {
    DisplayText().clear();
    ...
    if (!isEaster()) DisplayText("twitching length");
    else DisplayText("succulent delicacy");
    ...
    if (!isEaster()) DisplayText("ring");
    else DisplayText("donut");
    DisplayText(" around her conical cock.  The engorged flesh is swollen with pent-up need and you almost swear it gets a bit bigger when your fingertips press against her bindings.  Slowly, carefully, you start to remove the cock ring.  Luckily, the steady flow of her ");
    if (!isEaster()) DisplayText("pre-cum");
    else DisplayText("milky cream");
    ...
    if (!isEaster()) DisplayText("frothy juices");
    else DisplayText("slick honey");
    DisplayText(", her spear-like head slips easily past your labia, fervent flesh filling your trembling cavity. The girl's ");
    if (!isEaster()) DisplayText("thick pole");
    else DisplayText("sweet shaft");
    DisplayText(" sends ripples of hot and cold through your gut as her peculiar, almost numbing jism bubbles up inside you.  In another moment, you find yourself completely impaled on her turgid length, despite the throbbing girth of her ");
    if (!isEaster()) DisplayText("swollen mast");
    else DisplayText("bloated baster");
    DisplayText(".  Pressing her backwards, the two of you lean against her ");
    if (!isEaster()) DisplayText("supple");
    else DisplayText("marshmallow");
    DisplayText(" testes, their soft, pliant surface enveloping her as you straddle the helpless bunny.  Testing the tension of her scrotum, you find that the egg-filled sac is firm enough to support both of your weights without causing the girl any pain.  Gleefully, you wrap your arms around her waist and push forward, rolling up and over until the two of you rest atop the gargantuan balls like spherical, ");
    if (!isEaster()) DisplayText("well-padded bedding");
    else DisplayText("sponge cakes baked for a giant");
    DisplayText(".");
    Mod.Vagina.displayStretchVagina(player, 28, true, true, false);
    ...
    if (!isEaster()) DisplayText("gurgling seed");
    else DisplayText("saccharine seed");
    DisplayText(", you realize there's really only one thing left to do.  Sliding a hand between her waist and the tender flesh the two of you are balanced upon, your fingers seek out the ");
    if (!isEaster()) DisplayText("twitching tuft");
    else DisplayText("cotton candy poof");
    DisplayText(" of her fluffy tail.  You find the junction where it meets the base of her spine and bear down on the sensitive point with your knuckles, eliciting a shrieking squeal of bliss from the over-burdened bunny-girl.  An intense pressure floods your cunt as her ");
    if (!isEaster()) DisplayText("swollen dick");
    else DisplayText("candied cock");
    DisplayText(" distorts under the heft of the emerging eggs.  As the first oval bulge travels up her shaft, you grind and slam deeper on her until the narrow point of her ");
    if (!isEaster()) DisplayText("jizz-burbling");
    else DisplayText("jelly-bubbling");
    ...
    if (!isEaster()) DisplayText("tingling, liquid passion");
    else DisplayText("fattening, cream filling");
    DisplayText(".  Another egg rushes up, followed by another, the pace accelerating as you bounce atop the vivacious hare with fast, short strokes that rub your [clit] against her cum-bloated belly.");

    DisplayText("\n\nEgg after egg fills your womb, swelling your abdomen with the heavy ");
    if (!isEaster()) DisplayText("load");
    else DisplayText("treat");
    DisplayText(". You pant, finding it difficult to draw a breath with the burden massing in your inner recess, your tongue hanging freely  and drool trickling down the side of your mouth.");
    // Futa:
    if (player.torso.cocks.count > 0) {
        DisplayText("  Quite without realizing it, [eachCock] spasms in messy orgasm, spurting ropes of spunk  across the bunny girl's tawny skin like a gooey glaze.");
    }
    DisplayText("  Your vision swims and when you blink back the dizziness, you find your gut has bloated to the size of ");
    if (!isEaster()) DisplayText("advanced pregnancy");
    else DisplayText("gluttonous excess");
    ...
    if (!isEaster()) DisplayText("cum");
    else DisplayText("cream");
    DisplayText(" leaking from your freshly stuffed snatch.  With a guiding hand, you steer her surging spout to your [ass] and drop your full ");
    if (!isEaster()) DisplayText("egg-loaded");
    else DisplayText("candy fattened");
    DisplayText(" weight on the conical cock.  With fresh space to fill, the eggs flow into you once more, one pushing another in a procession up your large intestine.");
    // Futa:
    if (player.torso.cocks.count > 0) DisplayText("  The steady movement of the orbs pushing past your prostate drills another, even messier orgasm from your " + Desc.Cock.describeMultiCockShort(player) + ", cum arcing into the air to shower down on the two of you in pearl beads.");

    DisplayText("\n\nTime seems to flow from one moment to the next, your consciousness fading in and out as a small deluge of ");
    if (!isEaster()) DisplayText("bunny seed");
    else DisplayText("marshmallow and chocolate");
    DisplayText(" loads up your belly as fully as your twitching, clenching womb.  You can practically taste the sweetness in the back of your throat by the time the bunny-girl's orgasm ends.  Your body is inflated far beyond anything you could've imagined, eggs and spunk leaking from your gravid abdomen.  The bunny girl is lost to the world, dazed in a function bliss as her basketball-sized balls slowly push the last few eggs out of her dusky stalk with splattering plops.  You couldn't move if you wanted to, so you just remain on the ground, enjoying the feeling of utter, immobile bounty.  Gradually, the eggs in your belly and womb begin to melt, losing their firmness in favor of a fresh flux of raw warmth that wrings a final climax from your overtaxed body.");

    DisplayText("\n\n");
    // Futa:
    if (player.torso.cocks.count > 0) {
        DisplayText("Your exhausted " + Desc.Cock.describeMultiCockShort(player) + " somehow find" + (player.torso.cocks.count > 1 ? "" : "s") + " the strength for a gushing orgasm of " + (player.torso.cocks.count > 1 ? "their" : "its") + " own, strands of spoo spilling across your own body, white hot spunk bringing a fresh sheen to your sweat-slick [chest].  ");
    }
    ...
    DisplayText(Mod.Body.displayModThickness(player, 100, 3));
    DisplayText(Mod.Body.displayModTone(player, 0, 3));
    player.orgasm();
    player.stats.lib += 1;
    player.stats.sens += -3;
    return { next: Scenes.camp.returnToCampUseEightHours };
