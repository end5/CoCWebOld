...
export class GnollSpearThrowerScene {
	...
	public hyenaSpearLossAnal(): void {
		...
		//<Paragraph for Non-Taurs>
			...
			if (player.torso.balls.quantity > 0) DisplayText("clamping onto your " + BallsDescriptor.describeSack(player) + " and squeezing until ");
			else if (player.torso.vaginas.count > 0) DisplayText("grabbing your " + VaginaDescriptor.describeClit(player) + " until ");
			else DisplayText("plunging into your " + ButtDescriptor.describeButthole(character.torso.butt) + " and pinching until ");
		...
		DisplayText("Sharp talons press against your back, leaving eight lines of sensation as they trace down your body and along your " + LowerBodyDescriptor.describeHips(player) + ".  You feel the heat of her breath just above your ear.  \"<i>The queens of the savanna demand submission.  You are only welcome in these lands so long as you pay tribute to us in the most primal of ways.</i>\"  ");
		//<For characters with non-naga tails>
		if (player.torso.tailType > TailType.NONE) DisplayText("One furred paw wraps around your tail before pulling up sharply, exposing your " + ButtDescriptor.describeButthole(character.torso.butt) + " to the hot, savanna air.  Sharp fangs clamp onto the tail, holding it up to rob you of any remaining modesty.");
		//<For characters without tails>
		else DisplayText("One furred paw disappears before a single claw presses roughly against your " + ButtDescriptor.describeButthole(character.torso.butt) + " until she hears your breath catch in your throat.");
		...
		DisplayText("  The air fills with the sound of mocking laughter as the warrior forces herself deep into your passage.  Your ass is forced wide as your body tries desperately to cope with the sudden intrusion until her hips finally press against your " + ButtDescriptor.describeButt(player) + ".\n\n");
		...
		//<Male> 
		if (player.torso.cocks.count > 0) DisplayText("Your cum spurts from your " + CockDescriptor.describeMultiCockShort(player) + ", spraying your seed over the dry ground, giving life to the savanna.");
		//<Females>
		else if (player.torso.vaginas.count > 0) DisplayText("Your own juices flow freely from your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ", coating your thighs and dripping onto the dry grass below you.");
		DisplayText("  Muscles deep within you ripple, spasming around the strange intruder buried deep within you.  The hyena roars her pleasure, slamming deep into your " + ButtDescriptor.describeButthole(character.torso.butt) + " as your body milks her hard cunt for the juices flowing from her.\n\n");
	...
	private hyenaVictoryRapeFellatio(): void {
		...
		if (player.torso.cocks.get(x).length > 7) DisplayText("The tip of your member presses hard against the back of her mouth for a long moment until she swallows, letting your " + CockDescriptor.describeCock(player, x) + " slide into her tight throat.  ");
		...
		//<Two Cocks>
			...
			DisplayText("Even as the most incredible sensations flow through you from the feel of her muzzle, a spotted hand leaves one pendulous breast to wrap around your unattended " + CockDescriptor.describeCock(player, y) + ".  Her powerful grip feels like a vice as she begins to slide the paw along the length.  Moving in time to the rippling of her throat, the hand tries hard to match the flood of sensations radiating from her muzzle.");
			//<If character has Scrotum> 
			if (player.torso.balls.quantity > 0) DisplayText("  Her second paw finally cups your " + BallsDescriptor.describeBalls(true, true, player) + ", squeezing and massaging to add to the powerful sensations.");
			DisplayText("\n\n");
		}
		//Three or More Cocks>
		if (player.torso.cocks.count > 2) {
			DisplayText("Even as the most incredible sensations flow through you from the feel of her muzzle, a spotted hand leaves one pendulous breast to wrap around your unattended " + CockDescriptor.describeCock(player, y) + ".  Soon, her second hand slides along ");
		...
		//<Cock 0 is 7\" or greater> 
		if (player.torso.cocks.get(x).length >= 7) DisplayText("The muzzle starts to pull away from your " + CockDescriptor.describeCock(player, x) + " until the tip finally slips from her throat, letting her gasp for breath.  ");
		...
		if (player.torso.cocks.get(x).length >= 7) {
			DisplayText("Orgasm crashes through your body and mind as time loses its meaning.  The gnoll's muzzle plunges forward once more, enveloping the pulsing shaft deep into her maw.  The powerful muscles of her throat ripple as she swallows, milking your " + CockDescriptor.describeCock(player, x) + " for each spurt of your potent seed.");
			//<Two Cocks> 
			if (player.torso.cocks.count === 2) DisplayText("  Your second " + CockDescriptor.describeCock(player, y) + " throbs in her hand, spraying her face and throat with thick ropes of cum.");
			//<Three or More Cocks>
			if (player.torso.cocks.count >= 3) DisplayText("  Your " + CockDescriptor.describeMultiCockShort(player) + " throb in her paws, coating her muzzle, neck, and breasts with thick ropes of cum.");
		...
		//<Cock 0 is less than 7\"> 
			...
			DisplayText("Orgasm crashes through your body and mind as time loses its meaning.  The gnolls's cheeks sink in as she starts to suck as hard as she can, increasing the glorious pressure on your trapped member.  The powerful muscles of her muzzle ripple as she swallows, milking your " + CockDescriptor.describeCock(player, x) + " as each spurt of your potent seed splashes across her tongue.");
			//<Two Cocks> 
			if (player.torso.cocks.count === 2) DisplayText("  Your " + CockDescriptor.describeCock(player, y) + " throbs in her hand, spraying her face and throat with thick ropes of cum.");
			//<Three or More Cocks>
			if (player.torso.cocks.count >= 3) DisplayText("  Your " + CockDescriptor.describeMultiCockShort(player) + " throb in her paws, coating her muzzle, neck, and breasts with thick ropes of cum.");
