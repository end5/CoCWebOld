...
export interface SatyrFlags {
    SATYR_KIDS: number;
}

const satyrFlags: SatyrFlags = {
    SATYR_KIDS: 0
};

User.flags.set(FlagType.Satyr, satyrFlags);

export function satyrEncounter(player: Character, location: number = 0): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Satyr);
    if (randInt(2) === 0 || player.pregnancy.womb.isPregnant() || player.pregnancy.buttWomb.isPregnant() || player.gender === Gender.NONE) {
        ...
        return CombatManager.beginBattle(player, [], [new Satyr()]);
    }
    ...
    // Pregnant PCs can't get this encounter.
        ...
        // [Yes][No]
        if (location === 0) return { yes: partial(consensualSatyrFuck, player, 0), no: Scenes.camp.returnToCampUseOneHour };
        else return { yes: partial(consensualSatyrFuck, player, 0), no: Scenes.camp.returnToCampUseOneHour };
    }
}

// [=Yes=]
function consensualSatyrFuck(player: Character, loc: number = 0): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Satyr);
    ...
    return { choices: [["Trick Him", trick], ["Keep Drinking", keepDrinking], ["Skip Foreplay", foreplay]], persistantChoices: [["Leave", leavePartySatyr]] };
}

// [=Keep Drinking=]
function keepDrinking(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Satyr);
...
// [=Leave=]
function leavePartySatyr(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Satyr);
    ...
    return CombatManager.beginBattle(player, [], [satyr]);
}
// [=Trick Him=]
function trickZeSatyr(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Satyr);
...
// [=Skip Foreplay=]
function skipForeplay(): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Satyr);
...
function loseToSatyr(player: Character, monster: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Satyr);
    // [Lust loss
...
function defeatASatyr(player: Character, monster: Character): NextScreenChoices {
    ...
    if (player.stats.lust >= 33 && player.gender > 0) {
        ...
        // [Male][Female][Leave]
        if (player.torso.cocks.count > 0 && player.torso.cocks.find(Cock.CockThatFits(monster.analCapacity()))) butt = malesTakeAdvantageOfSatyrs;
    ...
    if (
        player.torso.vaginas.count > 0 &&
        player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 4 &&
        player.inventory.equipment.armor.name === ArmorName.LustyMaidensArmor
    ) bikiniTits = partial(lustyMaidenPaizuri, player, monster);
    return { choices: [["FuckHisButt", butt], ["Ride Face", faces], ["B.Titfuck", bikiniTits]], persistantChoices: [["Leave", Scenes.camp.returnToCampUseOneHour]] };
}
// Female (Z)
function femaleTakesAdvantageOfSatyr(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Satyr);
...
// Male (Z)
function malesTakeAdvantageOfSatyrs(player: Character, monster: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Satyr);
    let cockThatFits = player.torso.cocks.find(Cock.CockThatFits(monster.analCapacity()));
    ...
    DisplayText("\n\nThe moment your fingers slide inside the helpless satyr he bleats, not in horror, but in delight.  His insistent struggles to get away from you make a complete U-turn, and instead he struggles to get more of your hand inside his gripping hole; his cock throbs and grows even harder, spurting pre on the ground below, as his balls churn and slosh eager for the prostate massage you'll be giving him with your own " + Desc.Cock.describeCock(player, cockThatFits) + ".  You can't help but laugh at the satyr's change of heart; what a slut.");
...
function willinglyBoneSatyr(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Satyr);
    ...
    if (player.torso.vaginas.count <= 0 && satyrFlags.SATYR_KIDS === 0) {
...
// Pregnancy Stuff (Z)
function satyrPreggo(player: Character) {
    if (player.torso.vaginas.count > 0) {
        player.pregnancy.womb.knockUp(new Pregnancy(PregnancyType.SATYR, IncubationTime.SATYR)); // Satyrs can now fertilize eggs for ovipositing players
    }
    else {
        player.pregnancy.buttWomb.knockUp(new Pregnancy(PregnancyType.SATYR, IncubationTime.SATYR), 1, true);
    }
}

// Birthing (Z)
// Baby is always male.
export function satyrBirth(player: Character, vag: boolean) {
    DisplaySprite(SpriteName.Satyr);
