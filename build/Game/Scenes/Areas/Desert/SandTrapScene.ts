export class SandTrapScene {
	...
	private dickwieldersLoseToSandTarps(): void {
		...
		if (player.torso.vaginas.count > 0) DisplayText("  Ignored entirely, your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " quivers and orgasms in tandem, flexing and wetting fruitlessly against the dry water pressing against it.");
	...
	public birfSandTarps(): void {
		...
		if (player.gender === Gender.MALE) {
			...
			if (!player.torso.hips.legs.isTaur() || (player.tallness * (5 / 6) < player.torso.cocks.list[player.longestCock()].cockLength)) DisplayText("  Your oily hands descend upon your cock, and you massage your shaft as you feel the pressure in your bowels intensify again.");
		}
		//Female: 
		else if (player.gender === Gender.FEMALE) {
			DisplayText("  You roll your eyes to the sky and moan, your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " moistening as you push out the egg.");
			//[(no fukken horses from here)]
			if (!player.torso.hips.legs.isTaur()) DisplayText("  Your oily hands push softly into your cleft, fingering your needy " + VaginaDescriptor.describeClit(player) + " as you feel the pressure in your bowels intensify again.");
		}
		//Herm: 
			...
			//[(no horses)]
			if (!player.torso.hips.legs.isTaur()) DisplayText("  Your oily hands descend upon your genitals and you begin to slowly pump your shaft and finger your needy " + VaginaDescriptor.describeClit(player) + " as you feel the pressure in your bowels intensify again.");
		}
	...
	private useSandTarpsHands(): void {
		...
		if (player.gender === Gender.MALE) {
			...
			if (player.torso.cocks.get(0).length < 6) {
				// balls:
				if (player.torso.balls.quantity === 0) DisplayText("\n\nThe sandtrap only needs one hand to pump your diminutive cock; it barely moves its wrist as it jerks you, occasionally wriggling its oily fingers along the bottom of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " to stimulate you even further.");
				else DisplayText("\n\nThe sandtrap only needs one hand to pump your diminutive cock; it barely moves its wrist as it jerks you, occasionally wriggling its oily fingers along the bottom of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " to incite you even further, whilst it uses its free hand to gently cup your [balls].");
			...
			//Two cocks: 
				...
				DisplayText("  As the sandtrap begins to pump you faster, your disregarded " + CockDescriptor.describeCock(player, 1) + " bobs in hard, eager concert with your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ".  You gasp as the trap slows down momentarily to grasp your second sex.  It is clumsy at first as it moves its lubricated palms in tandem, then with increasing confidence as it finds a position and rhythm that suits it, until it is jerking you off furiously, clutching your butt cheeks so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.");
				DisplayText("\n\nThe warm, oily double sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn't long before you reach a toe-curling peak and, with a wordless bark, spurt out line after line of jizz.  The trap flinches slightly when your first cumshots hit it in the face and chest, but then goes back to gently rubbing you, guiding you up and down your warm, slippery orgasm.");
			}
			//Three cocks: 
			else if (player.torso.cocks.count === 3) {
				DisplayText("\n\nAs the sandtrap begins to pump you faster, your two other, disregarded cocks bob in hard, eager concert with your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ".  You gasp as the trap slows down momentarily to reach one of its hands down to grasp your second sex, and then stops entirely as  one of its hands clutching your ass releases and snakes back around to take hold of your " + CockDescriptor.describeCock(player, 2) + ".  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking all three of your cocks in rhythm.");
			...
			//Four cocks: 
				...
				DisplayText("\n\nAs the sandtrap begins to pump you faster, your three other, disregarded cocks bob in hard, eager concert with your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ".  You gasp as the trap slows down momentarily to reach one of its hands down to grasp your second sex, and then stops entirely as both of its hands clutching your ass release and snake back around to take hold of your " + CockDescriptor.describeCock(player, 2) + " and " + CockDescriptor.describeCock(player, 3) + ".  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking all four of your cocks in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels; it glances up at you and giggles shyly as you look into each other's eyes and share a thought.  \"<i>At last, I've found somebody who complements my body perfectly.</i>");
			...
			//More than four cocks: 
				...
				DisplayText("\n\nAs the sandtrap begins to pump you faster, your many other, disregarded cocks bob in hard, eager concert with your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ".  You gasp as the trap slows down momentarily to reach one of its hands down to grasp your second sex, and then stops entirely as its two hands clutching your ass snake back around to take hold of your " + CockDescriptor.describeCock(player, 2) + " and " + CockDescriptor.describeCock(player, 3) + ".  It is clumsy at first as it works its way around the incredibly complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking four of your cocks in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels, and the trap giggles slightly as well despite itself.  Hazily you wonder if it is too much to wish that this creature, with its excellent wrist action, had just one or two more hands to its name.");
				DisplayText("\n\nThe warm, oily quadrupled sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn't long before you reach a toe-curling peak and, with a wordless bark, spurt out line after line of jizz.  The trap flinches slightly when your first shots hit it in the face and chest, but then goes back to gently rubbing you, guiding you up and down your warm, slippery multiple orgasm as you spurt like a cum sprinkler in front of it.");
			}
		}
		//Female: 
		...
		//Herm: 
			...
			DisplayText("\n\nOne pair of the creature's hands slowly brush up your inner thighs whilst the other two feel around your \"<i>armor</i>\", clumsily loosening clasps and belts as it finds them.  One hand finds your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " and, after pausing for a split second, slides its fingers around your shaft and begins to slowly rub up and down.  You sigh as you quickly harden in the creature's smooth, dry, enveloping grasp.  You look into the sandtrap's face as it works; blushing, it looks down from your gaze and concentrates upon what it is doing.  It begins to pump you faster and you hiss slightly at the abrasion this causes.  At this the sandtrap makes a throaty sound.  Curiously you look down in time to see it bring its other pair of hands up to its mouth and deposit a great dollop of clear ooze into its cupped palms.  Your disgust turns to delight as it swaps its hands and begins to massage its oil into your length, the warm lubricant allowing the creatures' fingers to glide around your shaft and head like they were made of silk.  As it moves its first hand up and down, it tentatively moves its second further between your legs.  It finds your [vagina] and, after pausing for a split second, slides its slender fingers into your moistness.  Although it is clumsy at first, the oil which coats its digits make them glide over your shaft and labia as if they were entirely frictionless.  It quickly gains confidence and begins to gently frig you as it slides its warm, slippery grasp up and down your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", circling your [clit] whilst occasionally dipping into your dripping hole.  You sigh and begin to lose yourself in the wet, silky sensation.");
			//Cock less than 6 inches:  
			if (player.torso.cocks.get(0).length < 6) {
				//[no balls: 
				if (player.torso.balls.quantity === 0) DisplayText("\n\nThe sandtrap only needs one hand to pump your diminutive cock; it barely moves its wrist as it jerks you, occasionally wriggling its oily fingers along the bottom of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " to stimulate you even further.");
			...
			//Cock 6-16 inches: 
				...
				if (player.torso.balls.quantity === 0) DisplayText("\n\nThe sandtrap glides its hand up and down your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", picking up a warm smooth rhythm.");
				else DisplayText("\n\nThe sandtrap glides its hand up and down your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", picking up a warm smooth rhythm as it gently cups your [balls] with its free hand.]  It slows down intermittently to move its fingers around your length, letting every inch of your dick feel the tips of its delicate fingers.");
			}
			else {
			...
			//More than three cocks: 
				...
				DisplayText("\n\nAs the sandtrap begins to pump you faster, your many other, disregarded cocks begin to harden in eager concert with your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ".  You gasp as the trap slows down momentarily to reach both of its hands clutching your ass back around to take hold of your " + CockDescriptor.describeCock(player, 1) + " and " + CockDescriptor.describeCock(player, 2) + ".  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking three of your cocks whilst fingering you in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels, and the trap giggles slightly as well despite itself.  Hazily you wonder if it is too much to wish that this creature, with its excellent wrist action, had just one or two more hands to its name.");
	...
	private stickWangInSandgina(): void {
		...
		if (silly()) DisplayText(", though with fewer pouting blue lips and doe eyes");
	...
	private nagaThreesomeWithSandTrap(): void {
		...
		if (player.torso.cocks.count > 0) {
			...
			//14 inches or more: 
				...
				if (player.torso.balls.quantity > 0) DisplayText("your " + BallsDescriptor.describeBalls(true, true, player) + " swinging heavily in front of the sandtrap's nose, ");
