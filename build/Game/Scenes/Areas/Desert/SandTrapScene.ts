export class SandTrapScene {
	...
	public encounterASandTarp() {
		DisplayText().clear();
		DisplaySprite(97);
		if (Flags.list[FlagEnum.SANDTRAP_LOSS_REPEATS] >= 2 && player.torso.neck.head.face.eyeType === EyeType.BLACK_EYES_SAND_TRAP && player.torso.wings.type === WingType.GIANT_DRAGONFLY) {
			sandTrapBadEnd();
			return;
		}
		...
		if (Flags.list[FlagEnum.TIMES_ENCOUNTERED_SAND_TRAPS] === 1) {
			...
			if (player.stats.spe / 10 + randInt(20) >= 16) {
				...
				//Fight]/[Leave]
				simpleChoices("Fight", startSandTarpFight, "", null, "", null, "", null, "Leave", Scenes.camp.returnToCampUseOneHour);
			}
			//Speed check fail: 
			else {
	...
	public sandtrapmentLoss(clear: boolean = false) {
		if (Flags.list[FlagEnum.SANDTRAP_LOSS_REPEATS] >= 2 && player.torso.neck.head.face.eyeType === EyeType.BLACK_EYES_SAND_TRAP && player.torso.wings.type === WingType.GIANT_DRAGONFLY) {
		...
		if (player.torso.cocks.count > 0) return { next: dickwieldersLoseToSandTarps };
		else if (player.torso.vaginas.count > 0) return { next: chicksLoseToSandTarp };
		else return { next: genderlessLoss };
	}

	internal function pcBeatsATrap() {
		DisplayText().clear();
		DisplaySprite(97);
	...
	//Male/Herm loss (Z)
	private dickwieldersLoseToSandTarps() {
		DisplayText().clear();
		DisplaySprite(97);
		...
		monster.statusAffects.add(StatusAffectType.Fertilized, 0, 0, 0, 0);
		sandTrapPregChance();
		player.orgasm();
		player.stats.lib += 1;
		player.slimeFeed();
	...
	private chicksLoseToSandTarp() {
		...
		if (monster.statusAffects.has(StatusAffectType.Fertilized)) DisplayText("  \"<i>A cute little wwworker hasz fallen into my home.  Wwwell, that isz ok.  Fortunately for hwyou, I already have some eggsz ready.  Hhyou will get to be a queen! Iszn't that exciting?</i>\"");
		...
		if (monster.statusAffects.has(StatusAffectType.Fertilized)) sandTrapPregChance();
		player.orgasm();
		player.stats.lib += 1;
		player.slimeFeed();
		//reduce lust, increase lib, slimefeed, reset hours since cum
	...
	private genderlessLoss() {
		...
		if (monster.statusAffects.has(StatusAffectType.Fertilized)) DisplayText("\"<i>Not a worker OR a drone! Wwwell, that isz ok.  Even ants who hhhave nothing can be someone for somebody.  And hhyou can be mommy for my childrrhen!</i>\"");
	...
	private useSandTarpsHands() {
		...
		if (player.gender === Gender.MALE) {
			...
			//Four cocks: 
				...
				DisplayText("\n\nAs the sandtrap begins to pump you faster, your three other, disregarded cocks bob in hard, eager concert with your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ".  You gasp as the trap slows down momentarily to reach one of its hands down to grasp your second sex, and then stops entirely as both of its hands clutching your ass release and snake back around to take hold of your " + Desc.Cock.describeCock(player, 2) + " and " + Desc.Cock.describeCock(player, 3) + ".  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking all four of your cocks in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels; it glances up at you and giggles shyly as you look into each other's eyes and share a thought.  \"<i>At last, I've found somebody who complements my body perfectly.</i>");
			...
			//More than four cocks: 
				...
				DisplayText("\n\nAs the sandtrap begins to pump you faster, your many other, disregarded cocks bob in hard, eager concert with your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ".  You gasp as the trap slows down momentarily to reach one of its hands down to grasp your second sex, and then stops entirely as its two hands clutching your ass snake back around to take hold of your " + Desc.Cock.describeCock(player, 2) + " and " + Desc.Cock.describeCock(player, 3) + ".  It is clumsy at first as it works its way around the incredibly complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking four of your cocks in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels, and the trap giggles slightly as well despite itself.  Hazily you wonder if it is too much to wish that this creature, with its excellent wrist action, had just one or two more hands to its name.");
				DisplayText("\n\nThe warm, oily quadrupled sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn't long before you reach a toe-curling peak and, with a wordless bark, spurt out line after line of jizz.  The trap flinches slightly when your first shots hit it in the face and chest, but then goes back to gently rubbing you, guiding you up and down your warm, slippery multiple orgasm as you spurt like a cum sprinkler in front of it.");
			}
		}
		//Female: 
		...
		//Herm: 
			...
			//More than three cocks: 
				...
				DisplayText("\n\nAs the sandtrap begins to pump you faster, your many other, disregarded cocks begin to harden in eager concert with your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ".  You gasp as the trap slows down momentarily to reach both of its hands clutching your ass back around to take hold of your " + Desc.Cock.describeCock(player, 1) + " and " + Desc.Cock.describeCock(player, 2) + ".  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking three of your cocks whilst fingering you in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels, and the trap giggles slightly as well despite itself.  Hazily you wonder if it is too much to wish that this creature, with its excellent wrist action, had just one or two more hands to its name.");
	...
	private stickWangInSandgina() {
		...
		DisplayText("\n\n\"<i>Hhhhyes!  That'sz it lion, oh do me, do it to me hard!  Give me yyyyour strong spunk - uh! give hhit to me szo I can sink yyour children into evvryone I catch - yes!  Hhhhso the whole world can know hhwhat a " + Desc.Gender.mf(player, "man", "woman") + " you arrrrrhhhhhe!</i>\"  The creature's words subside into a high fluttering cry as it cums.  Its ovipositor spasms and clenches, milking you down your " + Desc.Cock.describeCock(player, x) + " for all it is worth.  Even if you wanted to, you could not pull back from the brink.  You come with near-painful intensity, your jizz shooting out of you as if your dick were trapped in a vacuum, losing yourself in ecstasy - you feel nothing but the grasping wet around your " + Desc.Cock.describeCock(player, x) + ", your cream spurting out of you like liquid gold, and the sun beating down on your back.");
	...
	//Naga Threesome (Z)
	private nagaThreesomeWithSandTrap() {
		DisplayText().clear();
		DisplaySprite(97);
		//Requirements: Player is naga with tail and fangs, has met desert naga as naga at least once
	...
	//Lose fight:
	private loseLastFightWithSandTrap() {
		DisplayText().clear();
		DisplaySprite(97);
