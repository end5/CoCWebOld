...
export interface SandTrapFlags {
    TIMES_ENCOUNTERED_SAND_TRAPS: number;
    SANDTRAP_NAGA_3SOME: number;
...
export function encounterASandTarp(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Sandtrap);
    if (sandTrapFlags.SANDTRAP_LOSS_REPEATS >= 2 && player.torso.neck.head.face.eyes.type === EyeType.BLACK_EYES_SAND_TRAP && player.torso.wings.type === WingType.GIANT_DRAGONFLY) {
        return sandTrapBadEnd(player);
    }
    sandTrapFlags.TIMES_ENCOUNTERED_SAND_TRAPS++;
    if (sandTrapFlags.TIMES_ENCOUNTERED_SAND_TRAPS === 1) {
        // femininity <= 65:
        if (player.femininity <= 65) {
            ...
            if (player.stats.int > 65) DisplayText("  Something about this scene puts you on edge.  How come you've never encountered quicksand in this desert before, with no water and its steady, stubborn, but not violent winds?  There's something odd about the woman, too - although she can't be a sand witch with that hair, you can't quite make her features out clearly through the heat haze.");
            DisplayText("  You think you could just about reach her with your hand if you were careful.  If you're going to save her, it has to be now; even in the short time you've been here she has sunk down to her collarbone.  She stares at you with plaintive despair.");
        }
        else {
            ...
            DisplayText("You think you could just about reach him with your hand if you were careful.  If you're going to save him, it has to be now; even in the short time you've been here he has sunk down to his collarbone.  He stares at you with plaintive despair.");
        }
        return { choices: [["Save", saveTheSandTarps], ["Don't Save", dontSaveTheTarps]] };
    }
    else {
        // Standard encounter:
        ...
        if (player.stats.spe / 10 + randInt(20) >= 16) {
            ...
            // Fight]/[Leave]
            return { choices: [["Fight", startSandTarpFight]], persistantChoices: [["Leave", Scenes.camp.returnToCampUseOneHour]] };
        }
        // Speed check fail:
        else {
            ...
            return CombatManager.beginBattle(player, [], [new SandTrap()]);
        }
    }
}
...
function saveTheSandTarps(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Sandtrap);
    // (femininity <= 65):
    if (player.femininity <= 65) {
    ...
    sandTrap.statusAffects.get(StatusAffectType.Level).value1 = 2;
    return CombatManager.beginBattle(player, [], [sandTrap]);
}

// [Don't Save]:
function dontSaveTheTarps(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Sandtrap);
    DisplayText("You carefully step backwards from the quicksand, not taking your eyes off the stricken ");
    if (player.femininity <= 65) DisplayText("woman");
    else DisplayText("man");
    DisplayText(".  \"<i>Please!</i>\" ");
    if (player.femininity <= 65) DisplayText("she");
    else DisplayText("he");
    DisplayText(" cries, neck sinking into the drift.  \"<i>Don't let me die like this... oh, very well.  Have it YOUR way!</i>\"  The ");
    if (player.femininity <= 65) DisplayText("woman");
    else DisplayText("man");
    DisplayText(" suddenly thrusts upwards and plunges a fist into the fluid sand surrounding ");
    if (player.femininity <= 65) DisplayText("her");
    else DisplayText("him");
    DisplayText(".  The dune you are standing on caves inwards, turning into a shifting slide to the morass below you.  You stumble and hop backwards as fast as you can, managing to stay just ahead of the ");
    if (player.femininity <= 65) DisplayText("woman");
    else DisplayText("man");
    DisplayText("'s spell.  In front of you, sand collapses downwards into a great conical depression, at the epicentre of which is your supposed rescuee.  With ");
    if (player.femininity <= 65) DisplayText("her");
...
function startSandTarpFight(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Sandtrap);
    return CombatManager.beginBattle(player, [], [new SandTrap()]);
}

export function sandtrapmentLoss(player: Character, sandTrap: SandTrap, clear: boolean = false): NextScreenChoices {
    if (sandTrapFlags.SANDTRAP_LOSS_REPEATS >= 2 && player.torso.neck.head.face.eyes.type === EyeType.BLACK_EYES_SAND_TRAP && player.torso.wings.type === WingType.GIANT_DRAGONFLY) {
        return loseLastFightWithSandTrap(player);
    }
    sandTrapFlags.SANDTRAP_LOSS_REPEATS++;
    if (clear) DisplayText().clear();
    else DisplayText("\n\n");
    DisplaySprite(SpriteName.Sandtrap);
    if (sandTrap.trapLevel() === 1) DisplayText("You are sunk to your belly in the depthless sand at the bottom of the pit, and are still falling fast.  The sun above you is blotted out by a shape which leans downwards towards you, smiling triumphantly.  Desperately, you try to keep your arms free so you can swing a blow at it, but with consummate ease, the sandtrap grabs your wrists with one set of hands while pushing you downwards with the others.  Within moments you are up to your armpits in the stuff, staring helplessly up at the strange androgynous creature which has you entirely at its mercy.");
    // PC lust loss:
    else DisplayText("You feel as radiant and molten as the sun above you... you just want to sink into the warm sand surrounding you forever.  Why are you struggling against it again?  You can't remember; with a sigh, you fall backwards onto the soft powder and allow yourself to be carried right down to the bottom.  The sandtrap chuckles softly as it envelopes you in its waiting arms.  \"<i>Good " + Desc.Gender.mf(player, "boy", "girl") + "...</i>\"");

    if (player.torso.cocks.count > 0) return { next: { func: dickwieldersLoseToSandTarps, args: [sandTrap] } };
    else if (player.torso.vaginas.count > 0) return { next: { func: chicksLoseToSandTarp, args: [sandTrap] } };
    else return { next: { func: genderlessLoss, args: [sandTrap] } };
}

function pcBeatsATrap(player: Character, monster: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Sandtrap);
    sandTrapFlags.SANDTRAP_LOSS_REPEATS = 0;
    // PC HP victory:
    if (monster.stats.HP < 1) DisplayText("The sandtrap groans and collapses forwards into the dirt.  With the creature no longer controlling it, the sand underneath you feels a great deal more stable... climbing out won't be too difficult.");
    // PC lust victory:
    else DisplayText("The sand around you stops sinking.  Overpowered with lust, the sandtrap moans and gives up control over itself and its pit to desperately run one set of hands over its flat chest whilst the other pushes into the sand to masturbate... whatever it has down there.  With your foe in this state, climbing out won't be so difficult now.");
    ...
    let bikiniTits;
    if (player.torso.vaginas.count > 0 && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 4 && player.inventory.equipment.armor.displayName === "lusty maiden's armor") bikiniTits = { func: lustyMaidenPaizuri, args: [monster] };
    // Requirements: Player is naga with tail and fangs, has met desert naga as naga at least once
    if (player.torso.hips.legs.isNaga() && player.statusAffects.has(StatusAffectType.Naga) && player.gender > 0 && player.torso.neck.head.face.type === FaceType.SNAKE_FANGS) nagaThreeSome = nagaThreesomeWithSandTrap;

    // Requires: Penis and str requirement
    if (player.torso.cocks.count > 0 && player.stats.str >= 60) putYourDickInIt = stickWangInSandgina;

    // \"<i>Ride</i>\" (Z)
    if (player.torso.vaginas.count > 0) rideDatSantTrap = rideDatSandTarpLikeIts1999;
    // \"<i>Hands</i>\" (Z)
    ...
    if (player.stats.lust >= 33) {
        ...
        return {
            choices: [["Naga3Some", nagaThreeSome], ["UseYourCock", putYourDickInIt], ["RideVaginal", rideDatSantTrap], ["HandJob", useSandTarpsHand], ["B.Titfuck", bikiniTits]],
...
// Male/Herm loss (Z)
function dickwieldersLoseToSandTarps(player: Character, sandTrap: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Sandtrap);
    ...
    if (!player.torso.hips.legs.isTaur()) DisplayText("armpits");
    else DisplayText("forelegs");
    DisplayText(".  You try and turn your head to see what it is doing but it gently but insistently pushes your face into the yielding sand.  \"<i>You arrrrhe a fine little drone.  I wwill thank you for yourrh hard work... with the fruits of ssame hard work.</i>\" What does it mean by...?");
    ...
    DisplayText("  The creature pushes your head down, making soothing shushing noises as it feeds more of its bulbous length into your ass.");
    if (player.analCapacity() < 40) DisplayText("  Although the oil which coats your thoughts and reflexes with sensual supplication makes things easier, the feeling is still incredibly intense, and you grit your teeth as something round travels down the creature's ovipositor and stretches your tight hole wide.");
    else DisplayText("  Your accommodating ass feels like it was made for this purpose, and with the creature's oil soothing your thoughts and reflexes you savor the creature taking you like this as if it were your quim.  You moan as something round travels down the creature's ovipositor and into your wet ass, the sensation making [eachCock] hard again.");
    ...
    if (player.torso.vaginas.count > 0) DisplayText(" and cunt");
    DisplayText(" flexing out more moisture into the sand beneath you.  The oil-induced serenity and the sexual marathon the sandtrap has put you through are too much and, even with the creature still relentlessly pumping away at you, you pass out.");
    ...
    sandTrap.statusAffects.add(StatusAffectType.Fertilized, 0, 0, 0, 0);
    sandTrapPregChance(player, sandTrap);
    player.orgasm();
    player.stats.lib += 1;
    player.slimeFeed();
...
// Female loss (Z)
function chicksLoseToSandTarp(player: Character, sandTrap: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Sandtrap);
    ...
    if (sandTrap.statusAffects.has(StatusAffectType.Fertilized)) DisplayText("  \"<i>A cute little wwworker hasz fallen into my home.  Wwwell, that isz ok.  Fortunately for hwyou, I already have some eggsz ready.  Hhyou will get to be a queen! Iszn't that exciting?</i>\"");
    else DisplayText("  \"<i>A cute little wwworker hasz fallen into my home, but I have no fertile eggsz to givve it.  Wwwell, that isz ok.  Hwwe can still have fun, can't we?</i>\"");
    ...
    if (player.analCapacity() < 30) DisplayText("Although the oil which coats your thoughts and reflexes with sensual supplication makes things easier, the feeling is still incredibly intense, and you grit your teeth as something round travels down the creature's ovipositor and stretches your tight ass wide.");
    else DisplayText("Your accommodating ass feels like it was made for this purpose, and with the creature's oil soothing your thoughts and reflexes you savor the creature taking you like this as if it were your second quim.  You moan as something round travels down the creature's ovipositor and into your wet ass.");
    DisplayText("  Slowly the creature begins to pick up its rhythm again, sliding in and out of your pussy as it breeds you.");
    ...
    if (sandTrap.statusAffects.has(StatusAffectType.Fertilized)) sandTrapPregChance(player, sandTrap);
    player.orgasm();
    player.stats.lib += 1;
    player.slimeFeed();
    // reduce lust, increase lib, slimefeed, reset hours since cum
...
// Unsexed (Z)
function genderlessLoss(player: Character, sandTrap: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Sandtrap);
    ...
    if (sandTrap.statusAffects.has(StatusAffectType.Fertilized)) DisplayText("\"<i>Not a worker OR a drone! Wwwell, that isz ok.  Even ants who hhhave nothing can be someone for somebody.  And hhyou can be mommy for my childrrhen!</i>\"");
    else DisplayText("\"<i>Not a worker OR a drone! Wwwell, that isz ok.  Hwwe can still have fun, can't we?</i>\"");
    ...
    DisplayText("  Slowly the creature begins to pick up its rhythm again, sliding in and out of your butt as it breeds you.");
...
function sandTrapPregChance(player: Character, monster: Character): void {
    // Only happens if PC aint already buttpregged
    if (monster.statusAffects.has(StatusAffectType.Fertilized))
        player.pregnancy.buttWomb.knockUp(new Pregnancy(PregnancyType.SANDTRAP_FERTILE, IncubationTime.SANDTRAP), 1, true);
    else
        player.pregnancy.buttWomb.knockUp(new Pregnancy(PregnancyType.SANDTRAP, IncubationTime.SANDTRAP), 1, true);
}

export function birfSandTarps(player: Character) {
    DisplaySprite(SpriteName.Sandtrap);
    DisplayText("\nYour eyes widen as a gout of oil suddenly gushes from your ass.  Before panic can set in, an incredible light-headedness overtakes you.  Dreamily, you discard your [armor] and squat.  More oil oozes out of you, and in your hazy euphoria, you scoop some of it up and rub it dreamily into your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s.  Part of you is disgusted at yourself, questioning what you are doing, but that is one voice in a million-strong chorus crooning you into total relaxation... the oil clings to your skin and seems to radiate warmth and softness.  Something round stretches your rectum wide, but in your state the sensation is practically orgasmic.");
    // [Male:
    if (player.gender === Gender.MALE) {
        DisplayText("  You roll your eyes to the sky and moan, [eachCock] growing hard as you push out the egg.");
        // [(mans and qualified horses only)]
        if (!player.torso.hips.legs.isTaur() || (player.tallness * (5 / 6) < player.torso.cocks.sort(Cock.LongestCocks)[0].length)) DisplayText("  Your oily hands descend upon your cock, and you massage your shaft as you feel the pressure in your bowels intensify again.");
    }
    // Female:
    else if (player.gender === Gender.FEMALE) {
        DisplayText("  You roll your eyes to the sky and moan, your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " moistening as you push out the egg.");
        // [(no fukken horses from here)]
        if (!player.torso.hips.legs.isTaur()) DisplayText("  Your oily hands push softly into your cleft, fingering your needy " + Desc.Vagina.describeClit(player) + " as you feel the pressure in your bowels intensify again.");
    }
    // Herm:
    else if (player.gender === Gender.HERM) {
        DisplayText("  You roll your eyes to the sky and moan, [eachCock] growing hard and your [vagina] moistening as you push out the egg.");
        // [(no horses)]
        if (!player.torso.hips.legs.isTaur()) DisplayText("  Your oily hands descend upon your genitals and you begin to slowly pump your shaft and finger your needy " + Desc.Vagina.describeClit(player) + " as you feel the pressure in your bowels intensify again.");
    }
...
// \"<i>Hands</i>\" (Z)
// Requires: Genitals
function useSandTarpsHands(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Sandtrap);
    ...
    if (player.gender === Gender.MALE) {
        ...
        if (player.torso.cocks.get(0).length < 6) {
            // balls:
            if (player.torso.balls.quantity === 0) DisplayText("\n\nThe sandtrap only needs one hand to pump your diminutive cock; it barely moves its wrist as it jerks you, occasionally wriggling its oily fingers along the bottom of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " to stimulate you even further.");
            else DisplayText("\n\nThe sandtrap only needs one hand to pump your diminutive cock; it barely moves its wrist as it jerks you, occasionally wriggling its oily fingers along the bottom of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " to incite you even further, whilst it uses its free hand to gently cup your [balls].");
        ...
        // Cock 6-16 inches:
            ...
            // [no balls:
            if (player.torso.balls.quantity === 0) DisplayText("\n\nThe sandtrap glides its hand up and down your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", picking up a warm smooth rhythm.");
            // Balls:
            else DisplayText("\n\nThe sandtrap glides its hand up and down your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", picking up a warm smooth rhythm as it gently cups your [balls] with its free hand.]  It slows down intermittently to move its palm around your length, letting every inch of your dick feel the tips of its delicate fingers.");
        }
        // Cock more than 16 inches:
        else DisplayText("\n\nThe sandtrap quickly comes to the realisation it needs both hands for this.  Still cupping your behind, it moves one oily hand up the top end of your massive shaft, while the other works the other half.  You sigh with deep satisfaction, enjoying the all-too-rare sensation of having every inch of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " immersed in pleasure.  The creature screws its face up and looks away in distaste at the size of your straining dick, which is in danger of poking it in the nose.  As you enjoy the warm, slippery sensation inundating you, you find you couldn't care less what it thinks.");
        // number of cock fork
        // One cock:
        if (player.torso.cocks.count === 1) {
            DisplayText("  The sandtrap pumps you faster and faster until its slippery hands are a blur of motion.  It clutches onto your butt cheeks so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.");
        ...
        // Two cocks:
            ...
            DisplayText("  As the sandtrap begins to pump you faster, your disregarded " + Desc.Cock.describeCock(player, player.torso.cocks.get(1)) + " bobs in hard, eager concert with your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ".  You gasp as the trap slows down momentarily to grasp your second sex.  It is clumsy at first as it moves its lubricated palms in tandem, then with increasing confidence as it finds a position and rhythm that suits it, until it is jerking you off furiously, clutching your butt cheeks so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.");
            DisplayText("\n\nThe warm, oily double sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn't long before you reach a toe-curling peak and, with a wordless bark, spurt out line after line of jizz.  The trap flinches slightly when your first cumshots hit it in the face and chest, but then goes back to gently rubbing you, guiding you up and down your warm, slippery orgasm.");
        }
        // Three cocks:
        else if (player.torso.cocks.count === 3) {
            DisplayText("\n\nAs the sandtrap begins to pump you faster, your two other, disregarded cocks bob in hard, eager concert with your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ".  You gasp as the trap slows down momentarily to reach one of its hands down to grasp your second sex, and then stops entirely as  one of its hands clutching your ass releases and snakes back around to take hold of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(2)) + ".  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking all three of your cocks in rhythm.");
            if (player.torso.balls.quantity > 0) DisplayText("  It grins up at you shyly and, with its final hand, cups your [balls].  ]You laugh with an almost hysterical edge to the sky at how mindboggling getting a triple handjob feels, and the trap giggles slightly as well despite itself.");
        ...
        // Four cocks:
        else if (player.torso.cocks.count === 4) {
            DisplayText("\n\nAs the sandtrap begins to pump you faster, your three other, disregarded cocks bob in hard, eager concert with your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ".  You gasp as the trap slows down momentarily to reach one of its hands down to grasp your second sex, and then stops entirely as both of its hands clutching your ass release and snake back around to take hold of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(2)) + " and " + Desc.Cock.describeCock(player, player.torso.cocks.get(3)) + ".  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking all four of your cocks in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels; it glances up at you and giggles shyly as you look into each other's eyes and share a thought.  \"<i>At last, I've found somebody who complements my body perfectly.</i>");
        ...
        // More than four cocks:
            ...
            DisplayText("\n\nAs the sandtrap begins to pump you faster, your many other, disregarded cocks bob in hard, eager concert with your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ".  You gasp as the trap slows down momentarily to reach one of its hands down to grasp your second sex, and then stops entirely as its two hands clutching your ass snake back around to take hold of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(2)) + " and " + Desc.Cock.describeCock(player, player.torso.cocks.get(3)) + ".  It is clumsy at first as it works its way around the incredibly complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking four of your cocks in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels, and the trap giggles slightly as well despite itself.  Hazily you wonder if it is too much to wish that this creature, with its excellent wrist action, had just one or two more hands to its name.");
            DisplayText("\n\nThe warm, oily quadrupled sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn't long before you reach a toe-curling peak and, with a wordless bark, spurt out line after line of jizz.  The trap flinches slightly when your first shots hit it in the face and chest, but then goes back to gently rubbing you, guiding you up and down your warm, slippery multiple orgasm as you spurt like a cum sprinkler in front of it.");
        }
    }
    // Female:
        ...
        if (player.tone < 60) DisplayText("  It picks up the pace gradually and soon it is doubly frigging you as fast as its fingers will allow, its other hands sinking deep into your soft, pillowy behind as it clutches onto you.");
        else DisplayText("  It picks up the pace gradually and soon it is doubly frigging you as fast as its fingers will allow, its other hands clutching onto your tight, muscled behind so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.");
    ...
    // Herm:
        ...
        DisplayText("\n\nOne pair of the creature's hands slowly brush up your inner thighs whilst the other two feel around your \"<i>armor</i>\", clumsily loosening clasps and belts as it finds them.  One hand finds your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " and, after pausing for a split second, slides its fingers around your shaft and begins to slowly rub up and down.  You sigh as you quickly harden in the creature's smooth, dry, enveloping grasp.  You look into the sandtrap's face as it works; blushing, it looks down from your gaze and concentrates upon what it is doing.  It begins to pump you faster and you hiss slightly at the abrasion this causes.  At this the sandtrap makes a throaty sound.  Curiously you look down in time to see it bring its other pair of hands up to its mouth and deposit a great dollop of clear ooze into its cupped palms.  Your disgust turns to delight as it swaps its hands and begins to massage its oil into your length, the warm lubricant allowing the creatures' fingers to glide around your shaft and head like they were made of silk.  As it moves its first hand up and down, it tentatively moves its second further between your legs.  It finds your [vagina] and, after pausing for a split second, slides its slender fingers into your moistness.  Although it is clumsy at first, the oil which coats its digits make them glide over your shaft and labia as if they were entirely frictionless.  It quickly gains confidence and begins to gently frig you as it slides its warm, slippery grasp up and down your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", circling your [clit] whilst occasionally dipping into your dripping hole.  You sigh and begin to lose yourself in the wet, silky sensation.");
        // Cock less than 6 inches:
        if (player.torso.cocks.get(0).length < 6) {
            // [no balls:
            if (player.torso.balls.quantity === 0) DisplayText("\n\nThe sandtrap only needs one hand to pump your diminutive cock; it barely moves its wrist as it jerks you, occasionally wriggling its oily fingers along the bottom of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " to stimulate you even further.");
        ...
        // Cock 6-16 inches:
            ...
            if (player.torso.balls.quantity === 0) DisplayText("\n\nThe sandtrap glides its hand up and down your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", picking up a warm smooth rhythm.");
            else DisplayText("\n\nThe sandtrap glides its hand up and down your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", picking up a warm smooth rhythm as it gently cups your [balls] with its free hand.]  It slows down intermittently to move its fingers around your length, letting every inch of your dick feel the tips of its delicate fingers.");
        }
        else {
        ...
        // Two cocks:
            ...
            DisplayText("\n\nAs the sandtrap begins to pump you faster, your disregarded " + Desc.Cock.describeCock(player, player.torso.cocks.get(1)) + " bobs in eager concert with your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ".  You gasp as the trap slows down momentarily to reach one of the hands gripping your ass back around to grasp your second manhood.  It is clumsy at first as it moves its lubricated palms in tandem, then with increasing confidence as it finds a position and rhythm that suits it, it is jerking and flicking you off furiously, clutching your butt cheek with its final hand for what seems like dear life.");
            DisplayText("\n\nThe warm, oily triple sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn't long before you reach a toe-curling peak and, with a harsh moan, spurt out line after line of jizz whilst gushing femcum coat its hands and your thighs.  The trap flinches slightly when your first shots hit it in the face and chest, but then goes back to gently rubbing and frigging you as you ride your glorious orgasms out.");
        }
        // hree cocks:
        else if (player.torso.cocks.count === 3) {
            DisplayText("\n\nAs the sandtrap begins to pump you faster, your two other, disregarded cocks bob in eager concert with your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ".  You gasp as the trap slows down momentarily to reach both hands clutching your ass back around to take hold of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(1)) + " and " + Desc.Cock.describeCock(player, player.torso.cocks.get(2)) + ".  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking three of your cocks whilst fingering you in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels; it glances up at you and giggles shyly as you look into each other's eyes and share a thought.  At last, I've found somebody who compliments my body perfectly.");
        ...
        // More than three cocks:
            ...
            DisplayText("\n\nAs the sandtrap begins to pump you faster, your many other, disregarded cocks begin to harden in eager concert with your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ".  You gasp as the trap slows down momentarily to reach both of its hands clutching your ass back around to take hold of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(1)) + " and " + Desc.Cock.describeCock(player, player.torso.cocks.get(2)) + ".  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking three of your cocks whilst fingering you in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels, and the trap giggles slightly as well despite itself.  Hazily you wonder if it is too much to wish that this creature, with its excellent wrist action, had just one or two more hands to its name.");
...
function rideDatSandTarpLikeIts1999(player: Character): NextScreenChoices {
    ...
    if (player.torso.cocks.count > 0) DisplayText("  [EachCock] flexes in delicious concert with your quim, spurting jizz onto the trap's chest and face.");
...
// \"<i>Vagina</i>\" (Z)
function stickWangInSandgina(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Sandtrap);
    // Requires: Penis and str requirement
    let cockThatFits = player.torso.cocks.find(Cock.CockThatFits(80));
    ...
    DisplayText("\n\nYou take a moment to look your strange conquest over.  Even up close you can't tell from its thin, fey beauty whether it is male or female.  Although it is affecting to look angry at you, its sculpted cheeks are quite flushed and there is a definite undercurrent of desire in its eyes; you suspect your display of strength and dominance appeals to it on some perverse, animalistic level.  Still smiling victoriously, you bend your face into it, deliberately invading its personal space, silently demonstrating who is in charge here.  It can't hold your gaze and, blushing, looks downwards.  This submissive demureness sends heat rushing towards your " + Desc.Cock.describeMultiCockShort(player) + "; the desire to fuck builds in you, to plough, to sink your cock into this strange, eerily beautiful creature, to establish once and for all who is giving and who is receiving around here... the only question is, how?  Its slim, androgyne upper body is devoid of obvious sexual features.  Your eyes roam steadily downwards.  Where its insect half meets its human half, where its cock should be, it instead has a genital slit; from this is emerging a long, black prong, dripping with clear oil.  Your show of dominance is turning this creature on, and it is powerless to disguise it.  It looks up at you with timid hopefulness... but you are in no mood for that.  There is a definite air of femininity to this creature, a pheromone scent which speaks directly to your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", makes you hard");
    if (player.torso.balls.quantity > 0) DisplayText(", makes your " + Desc.Balls.describeBalls(true, true, player) + " swell in readiness,");
    DisplayText(" and fills you with a deep, red urge; you grit your teeth in frustration at its essentially male upper half.  Its female half must be what is buried beneath the sand.  <i>Well,</i> says your dick.  <i>You've come this far...</i>");
    ...
    if (User.settings.silly()) DisplayText(", though with fewer pouting blue lips and doe eyes");
    ...
    DisplayText("\n\n\"<i>Hhhhyes!  That'sz it lion, oh do me, do it to me hard!  Give me yyyyour strong spunk - uh! give hhit to me szo I can sink yyour children into evvryone I catch - yes!  Hhhhso the whole world can know hhwhat a " + Desc.Gender.mf(player, "man", "woman") + " you arrrrrhhhhhe!</i>\"  The creature's words subside into a high fluttering cry as it cums.  Its ovipositor spasms and clenches, milking you down your " + Desc.Cock.describeCock(player, cockThatFits) + " for all it is worth.  Even if you wanted to, you could not pull back from the brink.  You come with near-painful intensity, your jizz shooting out of you as if your dick were trapped in a vacuum, losing yourself in ecstasy - you feel nothing but the grasping wet around your " + Desc.Cock.describeCock(player, cockThatFits) + ", your cream spurting out of you like liquid gold, and the sun beating down on your back.");

    DisplayText("\n\nYou come to your senses some time later.  Your ");
    if (player.torso.balls.quantity > 0) DisplayText("balls ache and your ");
    DisplayText("vision swims a bit as you slowly recede from the sandtrap, dripping shared sex as you go.  You give the top half of the creature a careful berth as you pick up your [armor] and slowly dress, but it is in no shape for retaliation.  The black haired androgyne seems to be in a deeply satisfied post-coital trance, humming and fluttering softly to itself as it trails patterns in the sand.  It blinks unsteadily at you as you begin to clamber your way out of the hole.  Behind you you hear sand being flung as it begins to go about the task of re-immersing itself.");
...
// Naga Threesome (Z)
function nagaThreesomeWithSandTrap(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Sandtrap);
    // Requirements: Player is naga with tail and fangs, has met desert naga as naga at least once
    ...
    // First encounter:
    if (sandTrapFlags.SANDTRAP_NAGA_3SOME === 0) {
    ...
    if (player.torso.cocks.count > 0) {
        ...
        if (player.torso.cocks.get(0).length >= 14) DisplayText("  You're not sure which is the more pleasing sight; her, or the rhythmic bulging in the sandtrap's neck.");
        DisplayText("  Mindlessly, the sandtrap's four arms clutch and stroke the warm scales which swaddle its upper body as the two of you continue to fuck it.");

        DisplayText("\n\nThe naga is soon deep within the sandtrap's oil and she slows her thrusting down as it relaxes her and amplifies what she can feel, smiling euphorically as she slowly rotates on the creature's leaking pseudo-cock.  You slow yourself down in sympathy; although it's difficult not to blow your load from the worshipful treatment the sandtrap is giving your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", there is no harm in prolonging this quality sex and you've had enough oil rubbed into you to feel quite dreamy yourself.  Eventually though, your peer begins to pick up the pace, clutching your hands as she bucks, her breasts bouncing up and down, gasping and moaning.   Finally she clenches both you and the sandtrap tight as she rolls her eyes at the sky and screams, femcum spurting out around the sandtrap's base; the sight is enough for you to finally push to your own eye-crossing peak.  You grip the naga's hands, lean your head back and unload your cum straight down the sandtrap's throat.");
        if (player.cumQ() > 1000) DisplayText("  The long, sensuous blowjob has pushed your body into overdrive and you spurt endlessly down the creature's tight gullet, panting and sweating as you discharge a veritable flood.  The hypnotised sandtrap gladly drinks what you give it, but it's far too much and soon it is coughing your jizz back up, through its nose and around your still ejaculating " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ".");
    }
    // Female:
    else {
        DisplayText("\n\nYou press your moistening " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " against the creature's lips.  Its eyes still far away, the sandtrap opens its mouth and pushes its soft tongue into your cleft.  It explores your labia and hole before turning its attention to your " + Desc.Vagina.describeClit(player) + ", circling it with gentle pressure whilst giving it the occasional faintest of flicks.");
        // [Squirtr, jotun of flooding:
        if (player.torso.vaginas.get(0).wetness < 4) DisplayText("  Its gentle worship of your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " soon has you streaming rivulets of girlcum from your oversexed hole, dribbling down the sandtrap's face as you eagerly press yourself down for more.");
        ...
        DisplayText("\n\nThe naga is soon deep within the sandtrap's oil fuge and she slows her thrusting down as it relaxes her and amplifies what she can feel, smiling euphorically as she slowly rotates on the creature's leaking pseudo-cock.  You pace yourself in sympathy; having already reached one ecstatic high you're more than happy to simply sit back and enjoy the sandtrap's worshipful ministrations, tending to every corner of your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ".  You've also had enough oil rubbed into you to feel quite dreamy yourself; the sand, the sun and even your own vagina seem bigger and more encompassing than they were before.");
        ...
        if (player.cumQ() > 1000) DisplayText("  The long, sensuous tongue bath has pushed your body into overdrive and the waterfall you unleash is even more uncontrollable than the last; you wonder dizzily if you haven't just lost control of your bladder.  The hypnotised sandtrap gladly drinks what you give it, but it's far too much and soon it is coughing your girlcum back up, through its nose and back onto your still contracting " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ".");
    }
...
function sandTrapBadEnd(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Sandtrap);
    ...
    if (player.torso.cocks.count > 0) DisplayText("[EachCock] hardens and your thin frame feels aflame, barely understood impulses deep within you begging you to walk down and submit to the Sandtrap's will.  ");
    // Female:
    else if (player.torso.vaginas.count > 0) DisplayText("Your [vagina] moistens and your thin frame feels aflame, barely understood impulses deep within you begging you to walk down and submit to the Sandtrap's will.  ");
    ...
    return { choices: [["Fight", sandTrapBadEndFight], ["Desztiny", desztiny]], persistantChoices: [["Leave", leaveSandTrapBadEnd]] };
}

// Fight:
function sandTrapBadEndFight(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Sandtrap);
...
// Leave:
function leaveSandTrapBadEnd(): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Sandtrap);
    DisplayText("With some effort you break your stare with the Sandtrap, turn away and step back towards camp, resolving to leave this disturbing scenario with its disturbing thoughts behind.");
...
// Desztiny:
function desztiny(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Sandtrap);
    DisplayText("The Sandtrap is right.  What your body and subconscious are telling you is so right.  At your acceptance of what the creature is saying warmth floods through your body; your mind bubbles as blood rushes towards your skin, making you feel incredibly sensitive, incredibly sexual.  ");
    // Male:
    if (player.torso.cocks.count > 0) DisplayText("Your cock strains upwards and you moan; feeling as sensitive as you do, you are both desperate to be touched and desperate not to be.  ");
    // Female:
    else if (player.torso.vaginas.count > 0) DisplayText("You feel but barely hear the drip of moisture on the sand, both your [asshole] and [vagina] beading with oily need.  ");
    ...
    DisplayText("\n\n\"<i>Good " + Desc.Gender.mf(player, "boy", "girl") + ",</i>\" says the Sandtrap, giggling slightly as it caresses you.  \"<i>Of course, that word doesn't mean anything to you anymore.</i>\"  You blearily look into its thin, beautiful face to ask what it means by that, and the creature takes the opportunity to plunge its tongue into your unresisting mouth.");
    return sandTrapBadEndFinale(player);
}

// Lose fight:
function loseLastFightWithSandTrap(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Sandtrap);
...
// Both go to:
function sandTrapBadEndFinale(player: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Sandtrap);
    ...
    if (player.gender > 0) {
        ...
        if (player.torso.cocks.count > 0) DisplayText("cock harden");
        else DisplayText("vagina dribble");

    }
    DisplayText(".  Yes.  You remember your quest.");
    ...
    if (player.torso.cocks.count > 0) DisplayText(" and masturbating [oneCock] desperately");
    else if (player.torso.vaginas.count > 0) DisplayText(" and fingering your [vagina] desperately");
    DisplayText(".  But, of course, as a fresh dominant urge takes you and drags you reluctantly from your needy genitals, there is much still left to be done.");
