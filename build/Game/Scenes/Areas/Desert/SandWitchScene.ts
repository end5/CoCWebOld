...
export interface SandWitchFlags {
    EGG_WITCH_TYPE: PregnancyType;
...
class SandWitchPregnancy implements ITimeAware {
    ...
    public timeChange(): boolean {
        sandWitchFlags.EGG_WITCH_COUNTER--;
        if (sandWitchFlags.EGG_WITCH_COUNTER <= 0) {
            clearPregnancy();
        }
        return false;
    }
...
TimeManager.add(new SandWitchPregnancy());

function clearPregnancy() {
    sandWitchFlags.EGG_WITCH_TYPE = undefined;
    sandWitchFlags.EGG_WITCH_COUNTER = 0;
}
...
export function encounter(player: Character): NextScreenChoices {
    ...
    if (player.statusAffects.get(StatusAffectType.Exgartuan).value1 === 1 && player.torso.cocks.get(0).area > 100 && player.statusAffects.get(StatusAffectType.Exgartuan).value2 === 0) {
        ...
        else DisplayText("turns crimson");
        DisplayText(" as you yank your " + player.inventory.equipment.armor.displayName + " back into place.  You're in charge here, not some possessed appendage!   Exgartuan yells something snide, but it's muffled too much to understand.  You look up in time to sidestep an attack from the Sand Witch.  It looks like you'll have to fight her!");
...
function allowSandWitchMagic(player: Character): NextScreenChoices {
    ...
    if (player.torso.neck.head.hair.color === "sandy blonde") {
        DisplayText("She smiles wickedly and intones, \"<i>Tresed eht retaw llahs klim ruoy.</i>\"\n\n");
        if (player.torso.chest.count === 0 || player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating === 0) {
            ...
            if (player.torso.chest.count === 0) player.torso.chest.add(new BreastRow());
            player.torso.chest.get(0).count = 2;
            player.torso.chest.get(0).rating = 3;
            if (player.torso.chest.get(0).nipples.count < 1) player.torso.chest.get(0).nipples.count = 1;
            player.stats.sens += 2;
            player.stats.lust += 1;
        }
        if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 1 && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating <= 2) {
        ...
        if (player.torso.chest.get(0).nipples.count < 1) {
            DisplayText("Two dark spots appear on your chest, rapidly forming into sensitive nipples.  ");
            player.torso.chest.get(0).nipples.count = 1;
            player.stats.sens += 2;
            player.stats.lust += 1;
        }
        if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 0) {
            ...
            if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier < 2)
                DisplayText("powerful jets of milk spray from your nipples, spraying thick streams over the desert sands.  You moan at the sensation and squeeze your tits, hosing down the tainted earth with an offering of your milk.  You blush as the milk ends, quite embarassed with your increased milk production.  ");
            if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 2 && player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier <= 2.6)
            ...
            if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier < 3) {
                Mod.Breast.boostLactation(player, .75);
        ...
        if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier === 0) {
            DisplayText("A pleasurable release suddenly erupts from your nipples!  Streams of milk are spraying from your breasts, soaking into the sand immediately.  It stops all too soon, though the witch assures you that you can lactate quite often now.  ");
            Mod.Breast.boostLactation(player, 1);
            player.stats.lib += .5;
            player.stats.sens += 1;
            player.stats.lust += 10;
        }
        DisplayText("The sand-witch smiles and thanks you for your offering.  You notice her dress is damp in four spots on the front.  ");
        if (User.flags.get<PlayerFlags>(FlagType.Player).sand === 0)
            DisplayText("You wonder at what her robes conceal as she vanishes into the dunes.");
        if (User.flags.get<PlayerFlags>(FlagType.Player).sand === 1) {
            if (player.stats.cor <= 33)
                DisplayText("You are glad to avoid servicing her again as she vanishes into the dunes.");
            else if (player.stats.cor <= 66)
                DisplayText("You wonder if you should've resisted and tried for some sex as she departs.");
            else DisplayText("You wish you had said no, so you could fuck with her and her magnificent quartet of breasts some more.");
        }
    }
    else {
        DisplayText("She smiles wickedly and intones, \"<i>nuf erutuf rof riah ydnas, nus tresed eht sa ydnas.</i>\"\n\nYou feel a tingling in your scalp, and realize your hair has become a sandy blonde!");
...
function sandwitchRape(player: Character): NextScreenChoices {
    ...
    if (player.stats.lust > 99) {
        ...
        if (player.torso.neck.head.hair.color.indexOf("sandy blonde") !== -1) {
            ...
            if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 0 && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating < 3) {
                DisplayText("Your breasts suddenly balloon outwards, stopping as they reach a perfectly rounded C-cup.  ");
                player.torso.chest.get(0).rating = 3;
                player.stats.sens += 1;
                player.stats.lust += 1;
            ...
            if (player.torso.chest.reduce(BreastRow.AverageNipplesPerBreast, 0) < 1) {
                ...
                for (const breastRow of player.torso.chest) {
                    ...
                    if (breastRow.nipples.count < 1) breastRow.nipples.length = .2;
            ...
            if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 0) {
                DisplayText("A strong pressure builds in your chest, painful in its intensity.  You yank down your top as ");
                if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier < 2) DisplayText("powerful jets of milk spray from your nipples, spraying thick streams over the desert sands.  You moan at the sensation and squeeze your tits, hosing down the tainted earth with an offering of your milk.  You blush as the milk ends, quite embarassed with your increased milk production.  ");
                if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 2 && player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier <= 2.6) DisplayText("eruptions of milk squirt from your nipples, hosing thick streams everywhere.  The feeling of the constant gush of fluids is very erotic, and you feel yourself getting more and more turned on.  You start squeezing your breasts as the flow diminishes, anxious to continue the pleasure, but eventually all good things come to an end.  ");
                if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 2.6 && player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier < 3) DisplayText("thick hoses of milk erupt from your aching nipples, forming puddles in the sand.  You smile at how well you're feeding the desert, your milk coating the sand faster than it can be absorbed.  The constant lactation is pleasurable... in a highly erotic way, and you find yourself moaning and pulling on your nipples, totally outside of your control.  In time you realize the milk has stopped, and even had time to soak into the sands.  You wonder at your strange thoughts and pull your hands from your sensitive nipples.  ");

                if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 3) DisplayText("you drop to your knees and grab your nipples.  With a very sexual moan you begin milking yourself, hosing out huge quantities of milk.  You pant and grunt, offering as much of your milk as you can.  It cascades down the dune in a small stream, and you can't help but blush with pride... and lust.  The erotic pleasures build as you do your best to feed the desert all of your milk.  You ride the edge of orgasm for an eternity, milk everywhere.  When you come to, you realize you're kneeling there, tugging your dry nipples.  Embarrassed, you stop, but your arousal remains.  ");
                if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier < 3) {
                    Mod.Breast.boostLactation(player, .7);
                    DisplayText("Your breasts feel fuller... riper... like your next milking could be even bigger.  ");
                }
                player.stats.lib += 1;
                player.stats.sens += 4;
            ...
            if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier === 0) {
                DisplayText("A pleasurable release suddenly erupts from your nipples!  Twin streams of milk are spraying from your breasts, soaking into the sand immediately.  It stops all too soon, though the witch assures you that you can lactate quite often now.  ");
                Mod.Breast.boostLactation(player, 1);
                player.stats.lib += .5;
                player.stats.sens += 1;
                player.stats.lust += 10;
            ...
            DisplayText("The Sand Witch smiles wickedly and intones, \"<i>nuf erutuf rof riah ydnas, nus tresed eht sa ydnas.</i>\"\n\nYou feel a tingling in your scalp, and realize your hair has become a sandy blonde!");
            player.torso.neck.head.hair.color = "sandy blonde";
            DisplayText("\n\nThe witch whispers into your ear, \"<i>I ALWAYS get what I want, dear...</i>\"");
        }
        // RAEP
        ...
        player.stats.lib += 1;
        player.stats.sens += 5;
        if (User.flags.get<PlayerFlags>(FlagType.Player).sand === 0) User.flags.get<PlayerFlags>(FlagType.Player).sand = 1;
        return { next: Scenes.camp.returnToCampUseOneHour };
    }
    // HP DEFEAT
    else {
...
function sandWitchBadEnd(): NextScreenChoices {
    DisplayText().clear();
...
function sandWitchBadEndPartTwo(): NextScreenChoices {
    DisplayText().clear();
...
function sandWitchBadEndPartThree(player: Character): NextScreenChoices {
    ...
    if (player.perks.has(PerkType.MarblesMilk)) {
        ...
        DisplayText("The last thing you hear is the Sand Witch screaming \"<i>What the hell is going on?  I ALWAYS get what I want!</i>\"");
    }
    else {
...
function sandwitchRaped(player: Character, sandWitch: Character): NextScreenChoices {
    ...
    if (player.gender === Gender.MALE || player.gender === Gender.HERM) {
        ...
        if (player.stats.cor < 50) {
            ...
            DisplayText("Your attention wanders down to the damp sand underneath her legs, and her musky scent hits your nose. Which do you go after?\n\n");
            return { choices: [["Breast", sandwitchBewbs], ["Sex", sandwitchSex]] };
        }
        // Really corrupt
        else {
            // Single cock
    ...
    // Femfem
        ...
        DisplayText("You smile at the sand witch, trying to seem enticing. A tiny smile plays across her face even though she supports a worried frown. She reaches up to you, and you allow her to gently pull you down to her. You marvel at her multiple breasts, each perfectly rounded. Each has a nipple approximately two inches long, enough to suck if you wanted to.  ");
        DisplayText("Your attention wanders down to the damp sand underneath her legs, and her musky scent hits your nose.\n\n");
...
function sandwitchCentaurBoning(player: Character, sandWitch: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Sandwich);
    player.orgasm();
    DisplayText("The Sand Witch is panting before you, her clothes in disarray ");
    // [win via HP]
    if (sandWitch.stats.HP < 1) DisplayText("and torn in many places. ");
    // [win via lust]
    ...
    if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 3) {
        ...
        // [win via HP]
        if (sandWitch.stats.HP < 1) DisplayText("is unable to stop herself from slowly licking her lips. ");
        // [win via lust]
        else DisplayText("sensually licks her parted lips. ");
    }
    // [lactating]
    if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 1) {
        DisplayText("Her eyes widen when she sees the beads of milk already dripping from your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s as her hand drops between her legs and she begins to slowly masturbate.");
    }
    DisplayText("\n\n");
    // [Corruption < 70]
    if (player.stats.cor < 70) {
        ...
        // [has breasts]
        if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 0) DisplayText("; your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s  become stiff");
        // [cock only]
        ...
        if (SWCentaurMore(player, 3)) return { next: Scenes.camp.returnToCampUseTwoHours };
        DisplayText("\n\n");
        ...
        DisplayText("tease your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s, and you can feel her grinding herself against your strong shoulders. ");
        ...
        if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 0) {
            // [lactating]
            if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 1) {
                DisplayText("Your own " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s  are leaking as well, ");
                // [light-med lactation]
                ...
                // [heavy lactation]
                else DisplayText("torrents gushing out ");
                DisplayText("under her expert hands. ");
            }
        }
        ...
        // [GoTo: SW-1]
        if (SWCentaurMore(player, 1)) return { next: Scenes.camp.returnToCampUseTwoHours };
    }
    // [Corruption > 70]
    else {
        ...
        // [STR > 50]
        if (player.stats.str >= 50) DisplayText("easily tear them off of her. ");
        // [STR < 50]
        else DisplayText("drag her a bit until they come off. ");
        ...
        // [GoTo: SW-2]
        if (SWCentaurMore(player, 2)) return { next: Scenes.camp.returnToCampUseTwoHours };
    }
}
...
function SWCentaurMore(player: Character, argument: number): boolean {
    ...
    // [SW_1]
    if (argument === 1) {
        DisplayText("You realize too late that she has been casting a spell for the past few minutes. Before you can react, everything grows dark.");
    ...
    if (argument === 2) {
        ...
        if (player.torso.cocks.count > 0 || (player.gender === Gender.HERM && randInt(2) === 0)) {
            ...
            // [1 cock, non-tentacle]
            if (player.torso.cocks.count === 1 && player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE)).length === 0) DisplayText("With a single thrust, you push deep into one of her cunts, ");
            // [1 cock, tentacle]
            if (player.torso.cocks.count === 1 && player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE)).length === 1) DisplayText("Your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " caresses her anus teasingly, causing her to whimper in a mixture of arousal and denial.  With a single savage thrust, you push past her clenched muscles, ");
            // [two cocks]
            ...
            // [largest cock is wide]
            if (player.torso.cocks.get(0).thickness >= 3) {
                DisplayText("It is hard to believe just how tight she is, though if her cries serve as any indication, she will not be after you are through with her.  Turned on even more, you thrust in with increasing vigor and try to widen her as much as possible. ");
            }
            // [2+ cocks]
            ...
            // [1 cock, tentacle]
            if (player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE)).length === 1 && player.torso.cocks.count === 1) {
            ...
            if (player.torso.cocks.get(0).length >= 12 || player.torso.cocks.get(0).thickness >= 3) {
                DisplayText("; her cries have long since died down to whimpers. ");
            }
            ...
            if (player.cumQ() >= 250) DisplayText("gushing massive amounts of your cum ");
            // [regular cum production]
            else DisplayText("pushing your sperm ");
            DisplayText("deep into her ");
            // [1 cock, tentacle]
            if (player.torso.cocks.count === 1 && player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE)).length === 1) DisplayText("anus");
            else {
                // [largest cock is not wide and/or long]
                if (player.torso.cocks.get(0).length >= 12 || player.torso.cocks.get(0).thickness >= 3) {
                    // [1 cock]
                    if (player.torso.cocks.count === 1) DisplayText("cunt");
                    // [2+ cocks]
                    if (player.torso.cocks.count >= 2) DisplayText("cunts");
                }
                // [largest cock is wide and/or long]
                else {
                    DisplayText("womb");
        ...
        // [% no cocks]
            ...
            if (player.torso.vaginas.count > 0) {
                ...
                DisplayText("penetrates your " + Desc.Butt.describeButthole(player.torso.butt));
                DisplayText(".");
            }
            // [does not has cunt]
            else {
                DisplayText(Desc.Butt.describeButthole(player.torso.butt) + ", she fingers herself even harder than you had been a moment ago.  Her tongue drives into your " + Desc.Butt.describeButthole(player.torso.butt) + ", licking enthusiastically as she squeezes her nipples. ");
                // [anus size is small]
                ...
                // [anus size is normal]
                else if (player.analCapacity() < 20) DisplayText("Unsatisfied with just her buffet, you soon feel her fingers thrust hard into you and hear her muttering what sounds like obscene words of depraved lust. ");
                // [anus size is large]
                else DisplayText("Unsatisfied with just her buffet, the witch grabs one of her breasts and squirts milk into your massive hole.  You realize she was lubricating it even as you feel her entire hand driving into you.  The sensations are too intense, and your screams of pleasure drown out her lustful muttering. ");
                DisplayText("Her mumbling finishes even as an orgasm overwhelms you.");
                DisplayText("\n\n");
                // [SW_1]
    ...
    if (argument === 3) {
        ...
        // [STR > 50]
        if (player.stats.str > 50) DisplayText("pick her up and toss her onto your back. ");
        // [STR < 50]
        else DisplayText("offer her your hand and help her onto your back. ");
        DisplayText("Your strong hooves carry you across the clear desert at a light trot.  Without her robes to protect her, you can feel the witch's bare pussies grinding against your back as she presses her quad of breasts against your shoulders.  Her moans intensify more and more as you pick up the pace.");
    }
    return false;
}
...
function beatSandwitch(player: Character, monster: Character): NextScreenChoices {
    DisplaySprite(SpriteName.Sandwich);
    DisplayText().clear();
    if (monster.stats.lust > 99) DisplayText("You smile in satisfaction as the " + monster.desc.short + " drops down on all fours and begins masturbating feverishly.  Sadly you realize your own needs have not been met.  Of course you could always fuck the horny witch...\n\nDo you rape her?");
    else DisplayText("You smile in satisfaction as the " + monster.desc.short + " drops down on all fours and struggles to rise.  Sadly you realize your own needs have not been met.  Of course, you could always fuck the witch...");
    ...
    let tamaniDildo;
    let missingoSexScene;
    let shouldra;
    let ovi;
    if (User.settings.silly()) missingoSexScene = missingoSex;
    if (player.inventory.keyItems.has("Deluxe Dildo")) tamaniDildo = Scenes.forest.tanami.sandwitchGetsDildoed;
    if (Scenes.npcs.shouldraFollower.followerShouldra() && player.gender > 0) shouldra = Scenes.npcs.shouldraFollower.sandWitchGetsGhostly;
    // doYesNo(sandwitchRaped, Scenes.camp.returnToCampUseOneHour);
    if (player.gender > 0 && player.pregnancy.ovipositor.canOviposit()) ovi = ovipositSandWitches;

    return {
        choices: [
            ["Yes", { func: sandwitchRaped, args: [monster] }],
            ["Dildo Rape", tamaniDildo],
            ["Use 3i@-", missingoSexScene],
            ["Use Shouldra", shouldra],
            ["Lay Eggs", { func: ovi, args: [monster] }],
            ["Taunt Her", { func: sandwitchSpanking, args: [monster] }]
        ],
...
function missingoSex(): NextScreenChoices {
    DisplaySprite(SpriteName.Sandwich);
    DisplayText().clear();
    ...
    return { choices: [["N*xt", missingoSex2]] };
}
// {Next Page}
function missingoSex2(): NextScreenChoices {
    DisplaySprite(SpriteName.Sandwich);
    DisplayText().clear();
    ...
    DisplayText("\"I can't belie^e you just tried that.\" she says with a )_=f. \"You're goi(g to get and in all s))ts o& uncomfortable places ^& you ke-_p that up.\"\n\n");

    DisplayText("\"Still...\" she says, ##ok%7g at you slyly. \"Why don't you (ull b=c( and try that again....\"");

    // [N*xt]
    return { choices: [["N*xt", missingoSex3]] };
}

// {Next Page}
function missingoSex3(): NextScreenChoices {
    DisplaySprite(SpriteName.Sandwich);
    DisplayText().clear();
    ...
    DisplayText("\"@ @a@'t b@lie^e y_u j@st _rie_ th_t.\" s@e sa_s @?th a )_=f. \"Yo_'re @oi(g to g_t a_d in all s))ts o& u_c_mfo_table pla_es ^& yo_ ke-_p t@at u@.\"\n\n");

    DisplayText("\"@_i@_...\" s_e says, ##o@%7g @t yo_ s_@_y. \"W_@ d_n't @o_ (ull b=c( an_ @r_ _h_@ a_@in....\"\n\n");

    // [@*xt]
    return { choices: [["@*xt", missingoSex4]] };
}

// {Next Page}
function missingoSex4(): NextScreenChoices {
    DisplaySprite(SpriteName.Sandwich);
    DisplayText().clear();
    ...
    DisplayText("\"@ @o@'o o@ooo^o o_o o@oo _ooo_ oo_o.\" o@o oo_o @?oo o )_=o. \"oo_'oo @oo(o oo o_o o_o oo ooo o))oo o& o_o_ooo_ooooo ooo_oo ^& oo_ oo-_o o@oo o@.\"\n\n");

    DisplayText("\"@_o@_...\" o_o oooo, ##o@%7o @o oo_ o_@_o. \"o_@ o_o'o @o_ (ooo o=o( oo_ @o_ _o_@ o_@oo....\"");
    // [Fuck this!]
    return { choices: [["Fuck This!", missingoSex5]] };
}

// {Next Page}
function missingoSex5(): NextScreenChoices {
    DisplaySprite(SpriteName.Sandwich);
    DisplayText().clear();
    ...
    // Restore]
    return { choices: [["Restore", missingoSex6]] };
}
// {Next Page}
function missingoSex6(player: Character): NextScreenChoices {
    ...
    player.orgasm();
    DisplayText().clear();
    DisplayText("You successfully restore your game. Everything's intact. At least, you thought it was. You must have messed up the password somewhere along the line, but oddly enough, the only difference is the clock being set an hour ahead and a few extra gems and EXP in your pocket.\n\n");

    DisplayText("It is like fucking christmas up in here.");
...
function ovipositSandWitches(player: Character, monster: Character): NextScreenChoices {
    ...
    if (monster.stats.HP < 1) DisplayText("fright as she clamors to get away.");
    else DisplayText("an uneasy curiosity to see what exactly you have planned.");
    ...
    if (player.canOvipositBee()) DisplayText("bee-morphs");
    else DisplayText("driders");
    DisplayText(" can do to their partners and unsure if this is really what she wants.  The poor girl seems to need a little 'encouragement'... of course, you could just skip all that regardless if you really wanted.");

    // [Foreplay 1st]    [Get Fucking]
    return { choices: [["Foreplay 1st", eggwitchForeplay], ["Get Fucking", getToFuckingWithZeEggsInWitch]] };
}
...
function eggwitchForeplay(player: Character): NextScreenChoices {
    ...
    if (player.stats.cor > 50) DisplayText("enough ");
    DisplayText("to giving your way of breeding a shot, you quickly get into position on top of the sand witch.  ");
    if (player.canOvipositBee()) DisplayText("Sitting");
    else DisplayText("Laying");
    DisplayText(" against your mate, you both take the moment to revel in the anticipation of coitus.");
    player.stats.lust += 25;
    player.stats.cor += -.35;
...
function laySomeEggsInThatWitchFinally(player: Character): NextScreenChoices {
    ...
    if (cunt) DisplayText("folds of her vulva");
    else DisplayText("ring of her anus");
    DisplayText(".  Like something out of a fever dream, the bizarre appendage you wield burrows further and further into her ");
    if (cunt) DisplayText("sex");
    else DisplayText("ass");
    DisplayText(", the ");
    if (cunt) DisplayText("ribbed contours of her pussy");
    else DisplayText("tight fit of her innards");
    DisplayText(" enrapturing your egg-tube with every passing inch.  You begin the process of lubricating the antsy witch's ");
    if (cunt) DisplayText("love-");
    else DisplayText("anal ");
    DisplayText("canal for the delivery of your young; the witch barely contains her moans and utter glee at being filled with such a hot, humid substance.  In perverted delight the sand witch coos between kisses, her moans betraying a curiosity to explore just how pleasant mating this way can truly be.");
    ...
    if (cunt) DisplayText("opening of your lover's cervix");
    else DisplayText("deepest recesses of her sphincter");
    DisplayText(".  The joining of oviduct to ");
    if (cunt) DisplayText("womb");
    ...
    if (cunt) DisplayText("at the waiting lips of the sand witch's vagina");
    else DisplayText(" through her rectal walls");
    DisplayText(".  With a meticulous push, the forerunner of your young manages to infiltrate her passage, working its way valiantly into her depths with every convulsion.");
    ...
    if (cunt) DisplayText("into her waiting womb");
    else DisplayText("as far as you can safely tuck it");
    DisplayText(", nestled and sound until the scramble for life can begin.");

    DisplayText("\n\n\"<i>More... give me more...</i>\" the sand witch whimpers, caressing your ");
    // (Bee, non-horse:
    if (player.canOvipositBee() && !player.torso.hips.legs.isTaur()) DisplayText("torso");
    else DisplayText("pelvis");
    ...
    else if (facial === 1) DisplayText("the interior of her mouth with sweltering streams of your cum");
    DisplayText(".  Having finally achieved release, and also having run out of eggs, you pull yourself free of the desert woman with a loud pop, the ");
    if (player.canOvipositBee()) DisplayText("honey-like");
    else DisplayText("goopy");
    ...
    if (player.pregnancy.ovipositor.fertilizedEggs > 0) {
        if (player.canOvipositBee()) {
            sandWitchFlags.EGG_WITCH_TYPE = PregnancyType.BEE_EGGS;
            sandWitchFlags.EGG_WITCH_COUNTER = 192;
        }
        else {
            sandWitchFlags.EGG_WITCH_TYPE = PregnancyType.DRIDER_EGGS;
            sandWitchFlags.EGG_WITCH_COUNTER = 192;
        }
    }
...
export function sammitchBirthsDriders(player: Character): NextScreenChoices {
    ...
    if (player.stats.cor < 50) DisplayText("Intending to help");
    else DisplayText("Curious");
    DisplayText(", you slide down the dune that you are on and come to a stop next to the distressed woman.  However, upon seeing her face you realize that this is the very sand witch that you had a hot and steamy tumble with!");
...
function helpZeWithBirfBabies(player: Character): NextScreenChoices {
    ...
    if (player.torso.hips.legs.isTaur() || player.torso.hips.legs.isDrider()) {
        ...
        if (player.torso.hips.legs.isDrider()) DisplayText("skitter");
        else DisplayText("gallop");
        DisplayText(" across the dry desert sea...");
    }
...
function reluctantlyHelpZeWitch(player: Character): NextScreenChoices {
    ...
    // PC is a drider/centaur/whatever:
    if (player.torso.hips.legs.isDrider() || player.torso.hips.legs.isTaur()) {
...
function sandwitchBirthsYourMonstrosities(player: Character): NextScreenChoices {
    ...
    if (player.stats.cor < 60) DisplayText(", with a quick kiss on the lips");
    DisplayText(", whisper in her ear that you'll see her again...");
...
function sandwitchSpanking(player: Character, monster: Character): NextScreenChoices {
    ...
    else DisplayText("  You can't even stop masturbating, can you?");
    DisplayText("</i>\"  You kick her hand away, and she tumbles down into a distraught heap, crying softly");
    if (monster.stats.lust > 99) DisplayText(" but still masturbating");
    DisplayText(".  It's time for some fun!");
    ...
    if (player.stats.cor < 33) DisplayText("and though you feel kind of bad about using her this way, she likely would have done worse to you.");
    else if (player.stats.cor < 66) DisplayText("and though you aren't sure you ought to be using her this way, she likely would have done worse to you.");
    else DisplayText("and though you used to find things like this distasteful, you don't really give a shit anymore.");
    ...
    if (player.torso.cocks.count > 0) DisplayText("Worse still, [eachCock] is now unprotected and pressed against the rough stone by your body-weight.  It's less than pleasant.  ");
    else if (player.torso.vaginas.count > 0) DisplayText("Worse still, your [vagina] is pressed up against the rough stone.  It's less than pleasant.  ");
    ...
    DisplayText(".  Fingers tickle the " + Desc.Skin.skin(player) + " of your bottom, and then, without warning, your [butt] is slapped!  The sound of the hit rings out across the desert, hard enough to make moisture bead at the corners of your eyes.");
    ...
    else DisplayText("What the hell is she on about?  Does she actually think her little torture session is going change your attitude?");
    ...
    if (player.torso.vaginas.count > 0) DisplayText(" and [vagina]");
    DisplayText(".  Bruised and orgasmed into exhaustion, you pass out in the sand with your [armor] around your ankles.  Maybe next time you won't waste your chances gloating.");
    monster.stats.lust = 98;
    monster.stats.HP = 2;
    player.stats.lust = 100;
    User.flags.get<PlayerFlags>(FlagType.Player).COMBAT_BONUS_XP_VALUE = monster.stats.XP;
    player.orgasm();
    player.stats.sens += 5;
    return { next: Scenes.camp.returnToCampUseOneHour };
}
