export class SandWitchScene implements TimeAwareInterface {
	...
	private allowSandWitchMagic() {
		...
		if (player.torso.neck.head.hair.color === "sandy blonde") {
			...
			if (player.torso.chest.count === 0 || player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating === 0) {
				...
				if (player.torso.chest.count === 0) player.createBreastRow();
				player.torso.chest.get(0).breasts = 2;
				player.torso.chest.get(0).rating = 3;
				if (player.torso.chest.get(0).nipplesPerBreast < 1) player.torso.chest.get(0).nipplesPerBreast = 1;
				player.stats.sens += 2;
				player.stats.lust += 1;
			}
			...
			if (player.torso.chest.get(0).nipplesPerBreast < 1) {
			...
			if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 0) {
				...
				if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier < 3) {
					Mod.Breast.boostLactation(character, .75);
			...
			if (player.torso.neck.head.hair.color.indexOf("sandy blonde") != -1) {
				...
				if (player.torso.chest.reduce(BreastRow.AverageNipplesPerBreast, 0) < 1) {
					...
					while (temp > 0) {
						...
						if (player.torso.chest.get(0).nipplesPerBreast < 1) player.torso.chest.get(0).nipples.length = .2;
						player.torso.chest.get(0).nipplesPerBreast = 1;
					}
					player.stats.sens += 2;
					player.stats.lust += 1;
	...
	private sandwitchRaped() {
		...
		if (player.gender === Gender.MALE || player.gender === Gender.HERM) {
			...
			if (player.stats.cor < 50) {
				...
				MainScreen.simpleChoices(["Breasts", "Sex", "", "", ""], [sandwitchBewbs, sandwitchSex, null, null, null]);
			}
			//Really corrupt
			else {
				//Single cock
	...
	private sandwitchCentaurBoning() {
		...
		//[win via HP]
		if (monster.stats.HP < 1) DisplayText("and torn in many places. ");
		//[win via lust]
	...
	private SWCentaurMore(argument: number): boolean {
		...
		if (argument === 2) {
			...
			if (player.torso.cocks.count > 0 || (player.gender === Gender.HERM && randInt(2) === 0)) {
				...
				//[1 cock, tentacle]
				if (player.torso.cocks.count === 1 && player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE) === 1) DisplayText("anus")).length;
				else {
					//[largest cock is not wide and/or long]
					if (player.torso.cocks.get(0).length >= 12 || player.torso.cocks.get(0).thickness >= 3) {
						//[1 cock]
	...
	private ovipositSandWitches() {
		...
		MainScreen.simpleChoices(["Foreplay 1st", "Get Fucking", "", "", ""], [eggwitchForeplay, getToFuckingWithZeEggsInWitch, null, null, null]);
	}
	...
	private sandwitchSpanking() {
		...
		if (monster.lust > 99) DisplayText(" but still masturbating");
