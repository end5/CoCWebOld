...
export function nagaEncounter(player: Character) {
    DisplaySprite(SpriteName.Naga);
    ...
    if (player.torso.hips.legs.type === LegType.NAGA) {
        ...
        player.statusAffects.get(StatusAffectType.Naga).value1 = 1;
        // Not met as naga before
        if (player.statusAffects.get(StatusAffectType.Naga).value2 === 0) {
            player.statusAffects.get(StatusAffectType.Naga).value2 = 1;
        ...
        // Genderleast
            ...
            // (Otherwise)
            // else
            DisplayText("You move your tongue faster and faster into her pussy, reaching as far in as you possibly can.  ");
    ...
    if (player.statusAffects.get(StatusAffectType.Naga).value1 > 0) {
        ...
        player.statusAffects.get(StatusAffectType.Naga).value1 = 0;
...
function gooNagaRape(player: Character, naga: Character) {
    DisplayText().clear();
    player.orgasm();
    DisplayText("You look over at the prone form of the naga lying in the sand, her ");
    if (naga.stats.HP < 0) DisplayText("weak ");
...
// 3) Victory male
function nagaVictoryMale(player: Character, naga: Character) {
    DisplayText().clear();
    player.orgasm();
    // Male or 50% herms
    if (player.torso.cocks.count > 0) {
        ...
        // Uncorrupted
            ...
            // [if cock fits]
            if (player.torso.cocks.get(0).area <= naga.vaginalCapacity()) {
...
function nagaVictoryFemale(player: Character, naga: Character) {
    DisplayText().clear();
    // 4) Victory female
...
function nagaVictoryGenderless(player: Character) {
    DisplayText().clear();
    // c)Centaur
    if (player.torso.hips.legs.type === LegType.CENTAUR) {
...
function nagaFUCKSJOOOOOO(player: Character, naga: Character) {
    DisplayText().clear();
    ...
    if ((player.perks.has(PerkType.BimboBrains) || player.perks.has(PerkType.FutaFaculties)) && player.torso.vaginas.count > 0) {
        ...
        DisplayText("  The scaly skin rubs at your soft vaginal walls like nothing else you have experienced, and the feeling begins to drive you mad with pleasure. It only takes a few minutes of this gentle tail-fucking to drive you to another climax. This time, however, the naga doesn't wait for your recovery. Instead, she responds by plunging deeper, forcing her ever-thickening tail deeper inside your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ". As she stuffs more and more of her scaly tail inside you, her hands begin to work on your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s, gently caressing and massaging them.\n\n");

        // [If Lactating]
        if (player.lactationQ() >= 50 && player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 1) DisplayText("Your warm milk is leaking from your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + "s the entire time, coating your skin and soaking her hands. At one point, she withdraws a hand and licks the milk from it. You can only assume that she liked it, as she begins to focus even more on your breasts.\n\n");
        // -------

        DisplayText("The naga lifts you from her body with her tail, placing you onto the warm sand where she had been laying. She moves her own body above yours, her tail buried deep inside your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " the entire time. She holds you in place with one hand, her other hand moving to resume playing with your " + Desc.Breast.describeAllBreasts(player) + ". She moves her head in, her hot breath washing over your neck, and looks up into your eyes for a moment. She smiles, giggles, and then wraps her mouth around your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + ", letting her slender tongue tickle your sensitive areola.\n\n");
        // -----
    ...
    // b) female //http://nekomimichan.org/mg/src/12844585127.jpg
        ...
        DisplayText("Then the naga lies flat on the sandy ground, pulling your body down onto on hers, face up, with your head resting between her soft breasts. She reaches around your body, placing her hands on your own " + Desc.Breast.describeBreastSize(player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + " gently holding you in place as her tail moves closer to your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ". As she begins to rub her presently-slick tail against your slit, the scaly tip brushes your " + Desc.Vagina.describeClit(player) + " and you moan with passion in response.  There is no holding it back; this feeling is one of the best you have experienced. Her tail continues to flick and rub your " + Desc.Vagina.describeClit(player) + " as you begin to buck your hips gently. The naga pokes and rubs at your " + Desc.Vagina.describeClit(player) + " for some time, eventually driving you to the first of many coming orgasms. After your breath has become even again and your muscles have become relaxed, the naga resumes her advances. She begins to rub the tip of her tail up and down along your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", and you finally understand her intentions.\n\n");

        DisplayText("As the tail begins to penetrate you, you cannot help but let out another passionate moan. The scaly skin rubs at your tender vaginal walls like nothing else you have experienced, and the feeling begins to drive you mad with pleasure. It only takes a few minutes of this gentle tail-fucking to drive you to another climax. This time, however, the naga doesn't wait for your recovery. Instead, she responds by plunging deeper, forcing her ever-thickening tail further into your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ". As she stuffs more and more of her scaly tail inside you, her hands begin to work on your " + Desc.Breast.describeBreastSize(player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + ", gently caressing and massaging them.");
        // (If fuckable nipples)
        if (player.torso.chest.find(BreastRow.FuckableNipples)) {
            DisplayText("  As her hands rub across your " + Desc.Breast.describeBreastSize(player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + ", the naga takes notice of the size of your nipples. She begins to press on them with her fingers, and, to her surprise, her fingers begin to disappear inside your breasts. It doesn't take her long to figure it out, and before you know it she is finger-fucking both your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + " in sync with the thrusts of her tail.");
        }
        // (If lactating)
        if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 1) {
            DisplayText("  Your warm milk is leaking from your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + " the entire time, coating your " + player.skin.desc + " and soaking her hands. At one point, she withdraws a hand and licks the milk from it. You can only assume that she liked it, as she begins to focus even more on your breasts.");
        }
        DisplayText("\n\n");

        DisplayText("The naga lifts you from her body with her tail, placing you onto the warm sand where she had been laying. She moves her own body above yours, her tail buried deep inside your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " the entire time. She holds you in place with one hand, her other hand moving to resume playing with your " + Desc.Breast.describeBreastSize(player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + ". She moves her head in, her hot breath washing over your other breast, and looks up into your eyes for a moment. She smiles, giggles, and then wraps her mouth around your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + ", letting her slender tongue tickle your sensitive skin there.");
        // (If fuckable nipples)
        if (player.torso.chest.find(BreastRow.FuckableNipples)) {
            DisplayText("It isn't long before she decides to take it one step further, pushing her tongue inside your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + " and wriggling it around inside. Meanwhile, her other hand begins to work its way into your other " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + ", finger-fucking it for all it's worth.");
        }
        // (If lactating)
        if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 1) {
            DisplayText("  As her mouth sucks at and plays with your " + Desc.Breast.describeNipple(player, player.torso.chest.get(0)) + ", your warm, sweet milk begins to flow forth once again, filling her mouth with your delectable cream. Her hand is also absolutely covered by it, and the excess drips down to the dry sand below, absorbed in seconds by the thirsty earth.");
        }
        DisplayText("\n\n");
...
function nagaRapeChoice(player: Character, naga: Character) {
    DisplayText().clear();
    if (naga.stats.HP < 1) DisplayText("You've defeated the naga!  ");
    else DisplayText("The naga writhes in the sand, masturbating feverishly!  She's completely forgotten about fighting you.  ");
    ...
    const hermChoices = {
        choices: [
            ["As Male", nagaVictoryMale],
            ["As Female", nagaVictoryFemale],
            ["Gooey Rape", gooNagaRape],
            ["Lay Eggs", eggs],
            ["Leave", Scenes.camp.returnToCampUseOneHour]
        ]
    ...
    if (player.stats.lust >= 33) {
        ...
        if (player.torso.hips.legs.type !== LegType.GOO) {
            choices[2][1] = undefined;
            hermChoices[3][1] = undefined;
        }
        if (player.gender === Gender.NONE) choices[0][1] = nagaVictoryGenderless;
        if (player.gender === Gender.MALE) choices[0][1] = nagaVictoryMale;
        if (player.gender === Gender.FEMALE) choices[0][1] = nagaVictoryFemale;
        if (player.gender === Gender.HERM) return hermChoices;
        return { choices };
    }
    return { next: Scenes.camp.returnToCampUseOneHour };
}

function eggUpANagaSpiderLike(player: Character) {
    DisplayText().clear();
...
function beePositANagaPlease(player: Character) {
    DisplayText().clear();
    ...
    // {empty venom}
    player.torso.tails.find(Tail.FilterType(TailType.BEE_ABDOMEN)).vemon = 0;
    ...
    if (player.pregnancy.ovipositor.eggs > 30) DisplayText(" from the outside at least.  You stuffed her so full that she looks positively pregnant, bulging to a picture of utter fertility");
    ...
    if (player.pregnancy.ovipositor.fertilizedEggs === 0) DisplayText("egg-bearer");
