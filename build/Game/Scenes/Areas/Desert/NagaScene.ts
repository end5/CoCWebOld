export class NagaScene {
		...
		if (player.hips.legs.type === LegType.NAGA) {
			...
			if (player.gender === Gender.MALE || (player.gender === Gender.HERM && randInt(2) === 0)) {
				...
				//[Cock isn't too big]
					...
					DisplayText("The kiss continues and both of your bodies rub together sensually, your tails continuing to wrap around one another. There is a pulsing in your " + CockDescriptor.describeMultiCockShort(player) + " as ");
					if (player.torso.cocks.count === 1) DisplayText("it starts ");
					else DisplayText("they start ");
					DisplayText("to grow hard against the naga's soft belly. Feeling this, the naga gently pulls away, slowly letting your tongue out of her mouth as she does so. Her hands make their way over your shoulders, down your abs, and stop at your " + CockDescriptor.describeMultiCockShort(player) + ".\n\n");
					//[player has one dick]
					if (player.torso.cocks.count === 1) DisplayText("Gently she starts to stroke the length of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " with one hand while circling the tip with the other. ");
	...
	private gooNagaRape(): void {
		...
		if (player.stats.cor > 50) {
			...
			if (player.gender === Gender.MALE) {
				DisplayText("your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " growing harder at the thought.");
			}
			//(if corrupt female)
		...
		if (player.torso.cocks.count > 0) {
			...
			else DisplayText("one of your " + CockDescriptor.describeMultiCockShort(player));
		...
		//(if herm) 
			...
			if (player.torso.cocks.count === 1) DisplayText("your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)));
			else DisplayText("your " + CockDescriptor.describeMultiCockShort(player));
		...
		if (player.gender === Gender.HERM) {
			...
			DisplayText(" and your " + CockDescriptor.describeMultiCockShort(player));
			//(normal)
	...
	private nagaVictoryMale(): void {
		...
		if (player.torso.cocks.count > 0) {
			...
			if (player.hips.legs.type === LegType.CENTAUR) {
				DisplayText("As your massive bulk looms over the defeated naga, your eyes gleam with desire. Your heart pounds in your ears as lusty thoughts echo in your head. You lift the snakewoman up into your arms, bringing her close before licking her neck and tasting her smooth scaly skin as her tail dangles down to the sand. As you touch her, she recovers, and her tail slowly wraps around your equine half to support her own weight. She returns your attentions by running her long forked tongue down your chest. Her scaly tail rubs sensually over your flesh as she continues to wind around you, caressing your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + "  with her tip, making you hard in a flash. The naga wriggles free from your embrace and coils behind you. Her entire length is now wrapped around various portions of both your human and equine halves, and she uses her grip to dangle underneath you. Her hands reach out to take a firm hold of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ".  You shiver as her tongue darts out and slides around your cock, pre-cum dripping from your head as she licks you. The agile naga lowers more of herself beneath you, spreading her pussy wide open before sliding onto your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " and letting out a hissy moan as you penetrate her. Your body tingles as her entire length quivers in pleasure, sending ripples over the flesh her tail is wrapped around.  Jolts of pleasure run through you as the naga woman grinds her hips against your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " , driving it into her as far as she can take.  With a loud and drawn-out moan, ");
				//[normal cum load]
				...
				//[SUPER CUM]
				else DisplayText("you explode with huge fountains of cum spurting from your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", gushing out from where the two of you are joined to rain on the sands below, ");
			...
			//Uncorrupted  
				...
				if (player.torso.cocks.get(0).area <= monster.vaginalCapacity()) {
					...
					//[Spermageddon]
					else DisplayText("your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " erupting, blasting a violent torrent of semen deep inside her petite frame, filling her up completely as both your voices rise in screams of pleasure.");
				}
				//[If cock too large]
				else {
					DisplayText("You draw away from her lips and return to your upright posture. Relaxed as she is, the naga looks nervously at your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", fully aware that it won't fit inside her small body. This occurs to you as well, and you begin to formulate a different plan. You move up onto her body so that you're straddling her stomach, fitting your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " neatly between her small C-cup breasts. You gently coax her hands to her breasts, and she begins to comprehend your intentions. She begins to squeeze your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " between her two warm mounds of flesh, doing her best to keep it snugly between them. Of her own accord, the naga begins to run her tongue across the " + player.cockHead() + ", obviously understanding what you want. She looks up at you with an unsure look, as if asking whether or not she is doing it right. You nod to her, and that's all the encouragement she needs. She begins to stroke your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " with her breasts as if it came naturally to her, covering as much of your " + player.cockHead() + " as she can with her warm and trembling mouth.\n\n");
					...
					DisplayText("  The naga removes your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " from her mouth");
					//[lots of cum OR more]
		...
		if (player.torso.vaginas.count > 0) {
			...
			if (player.hips.legs.type === LegType.CENTAUR) {
				...
				if (monster.lust > 99) {
					...
					if (player.gender === Gender.HERM) DisplayText("  As her tail gently squeezes your body, it draws your own " + CockDescriptor.describeMultiCockShort(player) + " up against your body, her soft underbelly stimulating the sensitive skin.");
					DisplayText("  With the added support, you are able to move your arm away from her stomach, moving your hand down to her covered snatch. It isn't hard to find as there is practically a river of her juices dripping from it. You peel away the scaly covering, and reveal her soaking wet lovehole. As you begin to play with her engorged clit, her soft moans begin to become more pronounced and you can feel her tail begin to wrap more tightly around your body. You feel a strange sensation as her tail brushes past your own " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ", causing you to tense up a bit. It seems that she noticed, because she does it again a few seconds later. As she begins to rub your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " with her tail, your body begins to betray your own lusts.\n\n");
					DisplayText("She begins to gently toy with your " + VaginaDescriptor.describeClit(player) + " using the very end of her tail, flicking it back and forth. You can't help but start moaning softly yourself. You respond by slowly teasing two of your fingers inside her waiting pussy, slowly beginning to move them in and out. As you begin to accelerate the motion of your fingers, the naga steps it up as well, sliding the tip of her tail inside your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ".  ");
					player.displayStretchVagina(20, true, false, true);
	...
	internal function nagaFUCKSJOOOOOO(): void {
		...
		if ((player.perks.has(PerkType.BimboBrains) || player.perks.has(PerkType.FutaFaculties)) && player.torso.vaginas.count > 0) {
			...
			DisplayText("The naga then lies flat on the sandy ground, pulling you down with her. She lays you down on her body, face up, with your head resting on her soft breasts. She reaches around your body, placing her soft hands on your own " + player.BreastDescriptor.describeAllBreasts(player) + ", gently holding you in place as her tail moves closer to your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ". Her now slick tail begins to rub against your clit and as the scaly skin runs along your " + VaginaDescriptor.describeClit(player) + ", you begin to moan air-headedly. There is no holding it back, this feeling is one of the best you have experienced. Her tail continues to flick and rub your clit as you begin to buck your hips gently. The naga pokes and rubs at your hard clit for some time, eventually driving you to the first of many coming orgasms. After your breath has become even again and your muscles have relaxed, the naga resumes her advances. She begins to rub the tip of her tail up and down along your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ", and you FINALLY get what she is trying to do.\n\n");
			...
			//PROPER stretched.
			player.displayStretchVagina((player.vaginalCapacity() + 5), true, true, false);
			DisplayText("\n\n");
		...
		//too)
			...
			//(if defeated by lust)
			if (player.stats.lust > 99) DisplayText("all your resistance seems to do is make your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " harder.");
			//if defeated by KO)
			...
			if (player.stats.cor > 75) DisplayText("  The thought somehow makes your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " throb even more.");
			DisplayText("\n\n");
			DisplayText("You close your eyes and ready yourself to meet your fate. Instead, you feel the hand of the naga at your crotch, sliding your " + player.inventory.equipment.armor.displayName + " down to give her better access to your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ". A gasp of surprise escapes you as the naga grabs ahold of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " and starts to stroke it, licking at the " + player.cockHead() + " with her serpentine tongue. You moan as she quickens her pace, causing a wave of heat to rise through your shaft.\n\n");

			//(Cock fits)
			if (player.torso.cocks.get(0).area <= monster.vaginalCapacity()) {
				DisplayText("She stops suddenly and you look at her, a pleading look in your eyes despite yourself. The naga isn't done with you yet, and she makes this known by opening up the scaly slit at her crotch to reveal her moist cunt. She rubs the opening on to the " + player.cockHead() + " of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", teasing your throbbing member before slowly sliding it in. She carefully lowers herself onto you, swallowing up the first few inches of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ". Her hips slide back, before bringing them back down and sliding more of your cock into her. She continues to gently swallow more of you inside of her, the folds of her love canal massaging you with each progressive thrust. Finally there is no more of you that she can push into her, her stomach presses against you in an attempt to try to bring more of you inside her.\n\n");
				DisplayText("You feel the hands of the snake woman firmly grab your shoulders and watch as she pulls away from you, shouting with pleasure as she thrusts herself down on the full length of your shaft. Unable to move or do anything to help her, you watch helplessly as she manipulates your body, her slick netherlips completely enveloping your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ".  Again and again she thrusts her hips onto your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", her pussy squeezing you almost as tightly as her tail. She continues to impale herself on your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", her thrusts becoming more intense by the moment. Finally she gives one last squeeze and tilts her head back with a yell. Unable to hold back, her tight walls begin to convulse, squeezing your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " tighter than ever before. You can feel a pressure welling up inside as her soaking pussy attempts to milk your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " of its tasty seed. You try to hold back, but it's no use. As she continues to pump at your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", a torrent of warm, sticky cum ");
				//Cum variants
			...
			//(Cock too big)
				...
				DisplayText("As she continues to stroke your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", her nimble fingers doing their best to wrap around your prodigious girth, she moves her head in closer to wrap her lips around the tip of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ". She begins to gently suck at the head of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", her thin, reptilian tongue dancing across the slick surface. ");
				//(if PC has balls)
				if (player.torso.balls.quantity > 0) {
					DisplayText("She shifts one of her hands down from your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", to your " + BallsDescriptor.describeBalls(true, true, player) + ", cupping them in her slick palm.  ");
				}
				DisplayText("As her tongue continues to slide across the " + player.cockHead() + " of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", her slender tongue wanders ever so closely to the center. You feel a strange sensation as her tongue wanders inside the hole at the tip of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", just barely tickling the inside. You try to pull away, but her tail binds you too tightly to move much. The naga notices your reaction and begins to exploit it as much as she can. She begins to force the rest of her tongue inside of your urethra, her forked tip tickling you from the inside as she pushes deeper. The naga, now that her tongue is completely buried inside your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", turns her attentions to the actual shaft, attempting to sandwich it between her modest C-cup breasts. She begins to stroke them up and down along your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", sucking at the " + player.cockHead() + " and tickling your urethra at the same time.\n\n");
				...
				DisplayText("  The naga pulls away, her tongue slowly sliding from your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " and dripping with cum, allowing the last few drops of cum to splash across her face and breasts. Her cheeks are still full of seed and  as she moves in closer, you wonder what she has planned. She grabs your shoulders with her still-slick hands, and leans in all the way for a final, passionate kiss. As your lips are pushed apart by her own, you feel a large amount of your still-warm sperm enter your mouth. You try to resist, but she begins to massage your throat, forcing you to swallow it all.\n\n");
		...
		//b) female //http://nekomimichan.org/mg/src/12844585127.jpg
			...
			//(if uncorrupted)
			else DisplayText("and while you know you probably won't like it, your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " says otherwise.  ");
			...
			if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 1) {
				DisplayText("  Your warm milk is leaking from your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + " the entire time, coating your " + player.skinDesc + " and soaking her hands. At one point, she withdraws a hand and licks the milk from it. You can only assume that she liked it, as she begins to focus even more on your breasts.");
			}
			DisplayText("\n\n");

			DisplayText("The naga lifts you from her body with her tail, placing you onto the warm sand where she had been laying. She moves her own body above yours, her tail buried deep inside your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " the entire time. She holds you in place with one hand, her other hand moving to resume playing with your " + Desc.Breast.describeBreastSize(character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating) + ". She moves her head in, her hot breath washing over your other breast, and looks up into your eyes for a moment. She smiles, giggles, and then wraps her mouth around your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + ", letting her slender tongue tickle your sensitive skin there.");
			//(If fuckable nipples)
			if (player.torso.chest.hasFuckableNipples()) {
				DisplayText("It isn't long before she decides to take it one step further, pushing her tongue inside your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + " and wriggling it around inside. Meanwhile, her other hand begins to work its way into your other " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + ", finger-fucking it for all it's worth.");
			}
			//(If lactating)
			if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier > 1) {
