...
export class Oasis {
	...
	public oasisEncounter(): void {
		...
		DisplayText("You wander in the desert for what seems like hours, sweating profusely in the sweltering heat. Eventually you come across a small watering hole surrounded by scrappy trees and shrubs. It would be foolish not to take this opportunity to drink, freshen up and paddle your " + LegDescriptor.describeLegs(player) + " in the cooling water, so you settle into what little shade you can find for a quick break.\n\n", true);
		//Demons approach!
		...
		MainScreen.simpleChoices(["Talk", "Fight", "", "", "Leave"], [oasisTalk, chooseToFight, null, null, oasisRunAway]);
	}
	...
	private oasisRunAway(): void {
		...
		if (player.stats.spe > 15 && player.stats.spe / 2 > randInt(40)) {
			DisplayText("You bolt out from under your bush and scramble away over the sand. Before long the swishing sounds of pursuit fade away and looking back you see the few demons with the gusto to follow you tramping back to the oasis.", true);
			return { next: Scenes.camp.returnToCampUseOneHour };
		}
		else {
			DisplayText("You scramble away from the demons, but are too late. A swift demon with canine features tackles you to the ground.  Luckily he loses his grip as you tumble onto the sand and you slither free, stand up and wheel to face the host of leering demons which begin to advance with malicious intent.", true);
			startCombat(new DemonPack());
	...
	private oasisTalk(): void {
		...
		MainScreen.simpleChoices(["Stay", "", "", "", "Leave"], [oasisTalkAccept, null, null, null, oasisTalkDecline]);
	}
	...
	private oasisTalkAccept(): void {
		...
		if (player.hips.legs.type === LegType.CENTAUR) DisplayText("your rear legs losing their balance and sending you crashing on your flank.  ");
	...
	internal function oasisSexing(): void {
		...
		if (player.torso.cocks.count > 0) {
			...
			DisplayText("your " + CockDescriptor.describeMultiCockShort(player) + " and begin to slide up and down before another demon pushes them out of the way and you ");
			if (player.torso.cocks.get(0).length > 25) DisplayText("feel your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " slide into a hell-girl's sloppy cunt. The demoness moans and pants in pleasure as your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " bottoms out in her gaping pussy and she begins to ride you like the motionless whore you momentarily are, her gigantic breasts threatening to concuss you as she slides the huge distance up and down your cock in her thirst to fill her cavelike cunt.  ");
			else {
				DisplayText("see a smaller and younger demon lowering her very tight pussy onto your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ". It's a tight fit, but her almost-virginal pussy is dripping wet. She bottoms her pussy out ");
				if (player.torso.cocks.get(0).length > 17) DisplayText("with hardly any of you inside her ");
				else if (player.torso.cocks.get(0).length > 12) DisplayText("about halfway down your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " ");
				else if (player.torso.cocks.get(0).length > 6) DisplayText("with almost all of you inside of her ");
				else if (player.torso.cocks.get(0).length <= 6) DisplayText("as her pussy lips touch the base of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " ");
				...
				if (player.torso.cocks.get(0).length > 6) {
					DisplayText("The realisation that her pussy is slowly stretching over your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " gives you a thrill that can only be described as positively evil.  ");
					player.stats.cor += 1;
				}
				DisplayText("Soon the slender cock-whore begins to cum and she starts on what is about to be the first of several shuddering orgasms. You smile evilly as she cums around your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " and when her eyes roll back to the front of her head you know that there is more to come.  ");
			}
			//Orgasmzzzzzzz
			...
			if (player.torso.cocks.count > 1) {
				...
				if (player.torso.cocks.count === 2) DisplayText(CockDescriptor.describeCock(player, 1) + " is");
		...
		DisplayText("Soon you feel hands and dicks grabbing and jabbing at your " + ButtDescriptor.describeButt(player) + ", edging inwards bit by bit and jostling for position as a slippery cock positions itself at the opening of your " + ButtDescriptor.describeButthole(character.torso.butt) + ". It pauses for a second and then dives straight into your waiting asshole.  ");
		...
		if (player.torso.chest.count > 0) {
			if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 1) {
				DisplayText("  Demonic hands grab and squeeze at your " + BreastDescriptor.describeAllBreasts(player) + " and lips fasten around your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + " and begin to lick and suck like there's no tomorrow.");
			}
		}
		//Newline for oral!
		...
		if (player.pregnancyIncubation === 0 && player.gender > 1) {
			...
			DisplayText("\n\n");
			DisplayText("You do your best to keep a vague mental catalogue of what has been in where, but eventually it becomes impossible to remember the type or number of demonic dicks that have filled you with their cum. The sand below your ass is wet with seed that has spilled out of your overflowing " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " and there is every indication of more to come.\n\n");
		}
		//If you got here by winning combat!
		if ((monster.stats.HP < 1 || monster.lust > 99) && Game.inCombat) {
		...
		else if ((player.stats.HP < 1 || player.stats.lust > 99) && Game.inCombat) {
			//►Oasis Demons Defeat PC as part of antm
			//Antmorph stuff
			if (monster.statusAffects.has(StatusAffectType.phyllafight)) {
	...
	private oasisBadEnd(): void {
		...
		//[If female/herm]
		if (player.hips.legs.type != LegType.CENTAUR) DisplayText("Your feet are locked up in shackles, though with a chain long enough to leave you room to move or walk without problems. ");
		if (player.hips.legs.type === LegType.CENTAUR) DisplayText("Your four legs are locked up in shackles, though with a chain long enough to leave you room to move or walk without problems. ");
		...
		DisplayText("\"<i>There will no longer be any of that back talk, slave. As for what I'm talking about... After all of our encounters with you, my tribesmen and I have made a decision.</i>\" The leader's smirk widens as he reaches down and grips your face tightly, forcing you to look up at him. His eyes look you over appraisingly as he pulls out a black collar with a long chain attached and fastens it to your neck. His other hand reaches down to one of your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + "s and grabs it roughly, giving it a hard squeeze and causing you to let out a small moan of pain and pleasure. He relinquishes his grip and walks around you, continually looking you over and scrutinizing your body from every angle. \"<i>We have decided that you would make an excellent addition to our group, and have laid claim to you as our sex slave... to be used in any way that we want.</i>\" He stops in front of you and continues, \"<i>We were lucky to catch you while you were still unconscious, before you could sneak off on us. I will personally enjoy using you for my own purposes. Maybe I'll even use you to carry my children once you're properly broken in.</i>\"\n\n");
		//[If herm] 
		if (player.gender === Gender.HERM) {
			if (player.hips.legs.type === LegType.CENTAUR) DisplayText("\"<i>Everyone will have a great time playing with you,</i>\" the leader says, smirking wider still as he stares at your centaur body. He reaches out and gives your " + ButtDescriptor.describeButt(player) + " a hard smack, causing you to yelp and quickly stand to your feet. You stagger a bit as your legs strain against the shackles as you stand. Without warning, he sticks his fingers inside of your " + ButtDescriptor.describeButthole(character.torso.butt) + " and stretches it out as his free hand reaches between your legs to play with both your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " and " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ". You gasp and moan in pleasure as your cock becomes erect in his hand. He laughs as he lets go and moves back in front of you. \"<i>Plus we could use you to carry our heavy loads like a pack animal. You have the perfect body for it.</i>\"\n\n");
			else DisplayText("His smirk widens even more as he stares down at your uncovered " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " laying flaccidly between your legs against the hot desert sand. You gasp in surprise as he suddenly reaches down and takes a firm hold of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ". He strokes it and gives it a rough squeeze, making you whimper sensually as it becomes fully erect in his hand. He laughs as he lets it go, \"<i>Our girls will definitely be having fun with you, slave.</i>\"\n\n");
		}
	...
	private oasisBadEndEpilogue(): void {
		DisplaySprite(46);
		DisplayText("After one year", true);
