...
export class Oasis {
	...
	private chooseToFight() {
		startCombat(new DemonPack());
		DisplaySprite(46);
		playerMenu();
	}

	private oasisRunAway() {
		DisplaySprite(46);
	...
	internal function oasisSexing() {
		...
		if (player.torso.cocks.count > 0) {
			...
			if (player.torso.cocks.count > 1) {
				...
				if (player.torso.cocks.count === 2) DisplayText(Desc.Cock.describeCock(player, 1) + " is");
		...
		if (player.torso.chest.count > 0) {
			if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 1) {
				DisplayText("  Demonic hands grab and squeeze at your " + Desc.Breast.describeAllBreasts(player) + " and lips fasten around your " + Desc.Breast.describeNipple(character, character.torso.chest.get(0)) + " and begin to lick and suck like there's no tomorrow.");
			}
		}
		//Newline for oral!
