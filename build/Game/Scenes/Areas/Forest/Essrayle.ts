...
export default class Essrayle {
    ...
    public constructor() {
        User.flags.set(CharacterType.Essrayle, this.data);
    }
    ...
    public essrayleMeetingI(character: Character): void {
        DisplayText().clear();
        if (this.data.met === 0) {
        ...
        this.data.met++;
        // [Yes] [No]
        MainScreen.doYesNo(this.plantsForMe, this.noPlantsForMe);
    }
    ...
    private noPlantsForMe(): void {
        ...
        this.data.turnedDownFirstMeeting = 1;
        MainScreen.doNext(Scenes.camp.returnToCampUseOneHour);
    }

    // >If Yes
    private plantsForMe(character: Character): void {
        DisplayText().clear();
        if (this.data.turnedDownFirstMeeting === 0) {
            this.data.turnedDownFirstMeeting = 1;
        ...
        if (character.torso.chest.get(0).rating >= 1) {
            ...
            // [Yes] [No]
            MainScreen.doYesNo(this.plantsForMe2BE, this.plantsForMe2NoBE);
        }
        // Else
        else {
            // [Next]
            MainScreen.doNext(this.plantsForMe2NoMentionBE);
        }
    }

    private plantsForMe2NoMentionBE(character: Character) {
        this.plantsForMe2(character, 1);
    }

    private plantsForMe2BE(character: Character) {
        this.plantsForMe2(character, 1);
    }

    private plantsForMe2NoBE(character: Character) {
        this.plantsForMe2(character, 2);
    }

    private plantsForMe2(character: Character, BE: number = 0): void {
        DisplayText().clear();
        // Yes
        if (BE === 1) DisplayText("Essy grins, nodding.  \"<i>Precisely.  I don't doubt they'll be even more fun given enough exposure here.</i>\"\n\n");
        // If No=
        else if (BE === 2) DisplayText("\"<i>Heh, suit yourself.</i>\"\n\n");
    ...
    // Look Closer
    public approachTrappedEssy(character: Character): void {
        DisplayText().clear();
        if (this.data.dungeonFucked === 0) {
        ...
        if (character.gender > 0) MainScreen.getBottomButton(0).modify("Feed Her", this.feedTrappedEssy);
        MainScreen.addBackButton("Leave", Menus.Player);
    }

    // [Feed Her]
    private feedTrappedEssy(character: Character): void {
        DisplayText().clear();
        ...
        this.data.dungeonFucked++;

        if (character.gender === 1) {
            DisplayText("\n\n");
            this.hasCockFeedEssy(character);
        }
        if (character.gender === 2) {
            DisplayText("\n\n");
            this.hasPussyFeedEssy(character);
        }
        if (character.gender === 3) {
            DisplayText("  How best to make use of this floral beauty?");
            MainScreen.displayChoices(["Cock", "Pussy"], [this.hasCockFeedEssyClear, this.hasPussyFeedEssyClear]);
        }
    }

    private hasCockFeedEssyClear(character: Character) {
        DisplayText().clear();
        this.hasCockFeedEssy(character);
    }

    // (Cock)
    private hasCockFeedEssy(character: Character): void {
    ...
    private hasPussyFeedEssyClear(character: Character) {
        DisplayText().clear();
        this.hasPussyFeedEssy(character);
    }

    private hasPussyFeedEssy(character: Character): void {
    ...
    public essyWitchVictory(): void {
        ...
        this.data.escapedDungeon = 1;
        MainScreen.doYesNo(this.acceptEssyPrizes, this.declineEssyPrizes);
    }

    // [No]
    private declineEssyPrizes(character: Character): void {
        DisplayText().clear();
    ...
    // [Yes]
    private acceptEssyPrizes(character: Character): void {
        DisplayText().clear();
        ...
        MainScreen.doYesNo(this.yesGimmeGiantNipplesEssy, this.noGimmeGiantNipplesEssy);
    }

    // [Yes]
    private yesGimmeGiantNipplesEssy(character: Character): void {
        DisplayText().clear();
    ...
    //         DisplayText("\n\nThat'll do.");
    //     }
    //     this.data.toldMotherToRelease = 1;
    //     MainScreen.doNext(Menus.Player);
    // }
}
