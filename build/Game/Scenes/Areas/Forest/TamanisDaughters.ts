import { Goblin } from "../../Explore/Goblin";
import { CharacterType } from "../../../Character/CharacterType";
import { randInt } from "../../../../Engine/Utilities/SMath";
import { User } from "../../../User";
import { FlagType } from "../../../Utilities/FlagType";
import { TamaniFlags } from "./TamaniScene";
import { Weapon } from "../../../Items/Weapons/Weapon";
import { WeaponName } from "../../../Items/Weapons/WeaponName";
import { ArmorName } from "../../../Items/Armors/ArmorName";
import { Armor } from "../../../Items/Armors/Armor";
import { HipRating } from "../../../Body/Hips";
import { ButtRating, ButtWetness, ButtLooseness } from "../../../Body/Butt";
import { StatusAffectType } from "../../../Effects/StatusAffectType";
import { BreastCup, BreastRow } from "../../../Body/BreastRow";
import { VaginaLooseness, VaginaWetness, Vagina } from "../../../Body/Vagina";
import { CharacterDescription } from "../../../Character/CharacterDescription";

// private midRoundMadness() {
//     let selector: number = randInt(4);
//     if (selector === 0) {
//         DisplayText("A slender hand reaches inside your " + player.inventory.equipment.armor.displayName + " and gives your ");
//         if (player.torso.balls.quantity > 0) {
//             if (randInt(2) === 0) DisplayText(player.Desc.Cock.describeMultiCockShort(player));
//             else DisplayText(player.Desc.Balls.describeBalls(true, true, player));
//         }
//         else DisplayText(player.Desc.Cock.describeMultiCockShort(player));
//         DisplayText(" a gentle squeeze.  You twist away but your breathing gets a little heavier.\n\n");
//     }
//     else if (selector === 1) {
//         DisplayText("A girl latches onto your " + Desc.Leg.describeLegs(player) + " and begins caressing your body lovingly, humming happily.  You quickly shake her loose but the attention makes you blush a little more.\n\n");
//     }
//     else if (selector === 2) {
//         DisplayText("One of your daughters launches onto your back and presses her hard, pierced nipples against your neck.  She whispers in your ear, \"<i>Twist my nipples dad!</i>\"\n\n");
//         DisplayText("You reach back and throw her off, but her perverted taunts still leave you feeling a little hot under the collar.\n\n");
//     }
//     else DisplayText("A daughter lays down in front of you and starts jilling herself on the spot.  It's impossible to not glance down and see her or hear her pleasured moans.  You step away to remove the distraction but it definitely causes some discomfort in your " + player.inventory.equipment.armor.displayName + ".\n\n");
//     game.dynStats("lus", 1 + player.stats.lib / 15 + randInt(player.stats.cor / 30));
// }

// private tamaniShowsUp() {
//     if (TamainsDaughtersScene.tamaniPresent) {
//         if (randInt(4) === 0) goblinDrugAttack(); //Tamani already there - chance of potion
//     }
//     else if (randInt(6) === 0) {
//         TamainsDaughtersScene.tamaniPresent = true;
//         DisplayText("A high-pitched yet familiar voice calls out, \"<i><b>So this is where you skanks ran off to---wait a second.  Are you trying to poach Tamani's man!?</b></i>\"\n\n");
//         DisplayText("You can see Tamani lurking around the rear of the goblin pack, visibly berating her daughters.  On one hand it sounds like she might help you, but knowing goblins, she'll probably forget about her anger and help them subdue you for more cum...\n\n");
//         //(+5 mob strength)
//         str += 5;
//         //(+5 mob toughness)
//         tou += 5;
//         HP += 10;
//         //(-20 mob lust)
//         lust -= 20;
//         //append combat desc
//         long += " <b>Tamani lurks in the back of the crowd, curvier than her brood and watching with a mixture of amusement and irritation.  She runs a hand through her pink and black hair, waiting for an opportunity to get involved...</b>";
//     }
// }

// override protected performCombatAction() {
//     let select: number = 1;
//     //mid-round madness!
//     midRoundMadness();
//     tamaniShowsUp();

//     if (special1 != null) select++;
//     if (special2 != null) select++;
//     if (special3 != null) select++;
//     switch (randInt(select)) {
//         case 0:
//             statusAffects.set(new StatusAffect("Attacks", int(Flags.list[FlagEnum.TAMANI_NUMBER_OF_DAUGHTERS] / 20))), 0, 0, 0); //Tamani's Daughters get multiattacks!
//             eAttack();
//             break;
//         case 1:
//             special1();
//             break;
//         case 2:
//             special2();
//             break;
//         default:
//             special3();
//             break;
//     }
//     combatRoundOver();
// }

// public defeated(hpVictory: boolean) {
//     game.forest.tamaniDaughtersScene.combatWinAgainstDaughters();
// }

// public won(hpVictory: boolean, pcCameWorms: boolean) {
//     if (pcCameWorms) {
//         DisplayText("\n\nYour foes seem visibly disgusted and leave, telling you to, \"<i>quit being so fucking gross...</i>\"");
//         game.return { next: Scenes.camp.returnToCampUseOneHour };
//     } else {
//         game.forest.tamaniDaughtersScene.loseToDaughters();
//     }
// }
export class TamanisDaughters extends Goblin {
    public constructor() {
        super(CharacterType.TamaniDaughters);
        this.description = new CharacterDescription(this, "Tamani's daughters", "A large grouping of goblin girls has gathered around you, surrounding you on all sides.  Most have varying shades of green skin, though a few have yellowish or light blue casts to their skin.  All are barely clothed, exposing as much of their flesh as possible in order to excite a potential mate.  Their hairstyles are as varied as their clothing and skin-tones, and the only things they seem to have in common are cute faces and curvy forms.  It looks like they want something from you.", true, "the group of ");
        this.torso.vaginas.add(new Vagina(VaginaWetness.DROOLING, VaginaLooseness.TIGHT));
        this.statusAffects.add(StatusAffectType.BonusVCapacity, 40, 0, 0, 0);
        this.torso.chest.add(new BreastRow(BreastCup.D));
        this.torso.butt.looseness = ButtLooseness.TIGHT;
        this.torso.butt.wetness = ButtWetness.DRY;
        this.statusAffects.add(StatusAffectType.BonusACapacity, 25, 0, 0, 0);
        this.tallness = 40;
        this.torso.hips.rating = HipRating.AMPLE + 1;
        this.torso.butt.rating = ButtRating.NOTICEABLE + 1;
        this.skin.tone = "greenish gray";
        this.torso.neck.head.hair.color = "pink";
        this.torso.neck.head.hair.length = 16;
        this.baseStats.str = 55;
        this.baseStats.tou = 30;
        this.baseStats.spe = 45;
        this.baseStats.int = 50;
        this.baseStats.lib = 70;
        this.baseStats.sens = 70;
        this.baseStats.cor = 50;
        this.inventory.equipment.defaultWeaponSlot.equip(new Weapon("fists" as WeaponName, undefined, "fists", "tiny punch", 0));
        this.inventory.equipment.defaultArmorSlot.equip(new Armor("leather straps" as ArmorName, undefined, "leather straps", 0));
        this.baseStats.bonusHP = 50 + (Math.floor(User.flags.get<TamaniFlags>(FlagType.Tamani).TAMANI_NUMBER_OF_DAUGHTERS / 2) * 15);
        this.baseStats.lust = 30;
        this.baseStats.lustVuln = .65;
        // this.temperment = TEMPERMENT_RANDOM_GRAPPLES;
        this.baseStats.level = 8 + (Math.floor(User.flags.get<TamaniFlags>(FlagType.Tamani).TAMANI_NUMBER_OF_DAUGHTERS / 20));
        this.inventory.gems = randInt(15) + 5;
        // this.drop = new WeightedDrop().
        //     add(consumables.GOB_ALE, 5).
        //     addMany(1, consumables.L_DRAFT,
        //         consumables.PINKDYE,
        //         consumables.BLUEDYE,
        //         consumables.ORANGDY,
        //         consumables.PURPDYE);
        // this.special1 = goblinDrugAttack;
        // this.special2 = goblinTeaseAttack;
    }
}
