...
export default class ErlKingScene {
    ...
    public constructor() {
        User.flags.set("Erlking", this.data);
    }

    public encounterWildHunt(character: Character) {
        if (this.data.wildHuntEncounters === 0) {
            this.firstWildHuntEncounter();
        }
        else if (!character.inventory.keyItems.has("Golden Antlers")) {
            this.repeatWildHuntEncounter(character);
        }
        else if (character.inventory.keyItems.has("Golden Antlers")) {
            this.encounterPrincessGwynn(character);
        }

        this.data.wildHuntEncounters++;
    }

    public characterHuntScore(character: Character): number {
		...
		Conditional modifiers: +20 for Evade
				...
				+10 Fast Perk
				+10 Incorporeality.  (Increases to +20 if character has wings)
				+10 Lunging Attacks
		...
		*/
        ...
        console.trace("Wild Hunt Points = " + baseVal);

        return baseVal;
    }

    public firstWildHuntEncounter() {
        DisplayText().clear();
        ...
        DisplayText("A shiver of unnatural fear runs up your spine, just as a hunting horn sounds from the distance.  You gasp, your breath materializing as a puff of fine, white mist.  Just as the echoes of the horn fade, a chorus of canine howls breaks through the");
        if (Time.hour >= 0 && Time.hour <= 10) DisplayText(" chill morning");
        else if (Time.hour >= 11 && Time.hour <= 13) DisplayText(" unusually cold daytime");
        else if (Time.hour >= 14 && Time.hour <= 17) DisplayText(" brisk afternoon");
        else if (Time.hour >= 18 && Time.hour <= 24) DisplayText(" freezing night");
        DisplayText(" air. Your eyes twitch and ears ring at the sound of hooves pounding through the forest.\n\n");
        ...
        MainScreen.displayChoices(["Wait", "Run"], [this.firstWildHuntChaseWaited, this.firstWildHuntChaseRan]);
    }

    private firstWildHuntChaseWaited(character: Character) {
        this.firstWildHuntChase(character, true);
    }

    private firstWildHuntChaseRan(character: Character) {
        this.firstWildHuntChase(character, false);
    }

    protected firstWildHuntChase(character: Character, waited: boolean = false) {
        DisplayText().clear();

        if (waited === false) {
            DisplayText("You stumble your way through the woods, but no matter which way you turn, you are greeted by bone-chilling fog.  Soon, canine snarls come from all sides.\n\n");
            DisplayText("You’re surrounded.\n\n");
        }
        else {
            ...
            if (character.stats.cor >= 40) {
                if (character.torso.cocks.count > 0 && character.torso.vaginas.count <= 0) {
                    ...
                    DisplayText("  Your asshole twitches in response, aching to be filled.");
                }
            }
            DisplayText("\n\n");
        }

        DisplayText("But before the dogs move, the curtain of fog parts, and a figure on horseback rides into view.  The hounds go silent, as the tall figure pulls back his hood to look at you.\n\n");
        ...
        DisplayText("“<i>I am the Erlking, Master of the Wild Hunt,</i>” he says in a deep, hollow voice. “<i>and you... you are a very unusual [race].  My hounds can scent it on you.</i>” He gestures toward the stock-still hounds surrounding you. \n\n");

        DisplayText("His gaze runs up and down your form, and despite yourself, you shiver at his attention.\n\n");

        if (waited) {
            // [If Stand Your Ground Selected]
            DisplayText("“<i>You’re a brave one, aren’t you?  Standing your ground in the face of the Wild Hunt?  I applaud your audacity.  I’m going to enjoy hunting you....</i>”\n\n");
        }
        else {
            // [If Run Selected]
            DisplayText("“<i>I’m afraid you’re going to have to learn to run faster if you hope to elude me in the future.  I do hope you won’t bore me.</i>”\n\n");
        }

        DisplayText("Before you can say another word, the fog closes in, leaving nothing but the red glow from the eyes of the Erlking and his Hounds.  One by one, they vanish, the Erlking’s last, leaving you alone in the darkness, the mist, and the cold.\n\n");
    ...
    public repeatWildHuntEncounter(character: Character) {
        DisplayText().clear();
        ...
        MainScreen.displayChoices(["Run", "Wait"], [this.repeatWildHuntChase, this.repeatWildHuntWait]);
    }

    protected repeatWildHuntWait(character: Character) {
        DisplayText().clear();
        ...
        DisplayText("Unfazed, you cross your arms and stand firm, staring down the menacing fog. \n\n");

        DisplayText("The sounds of the hunt grow louder and louder until the trees themselves appear to be shaking with the sounds of the approaching hunt.  You stand tall, refusing to play the Huntsman’s twisted game.\n\n");

        DisplayText("The Hunt’s deafening approach abruptly quiets.  Like a tide going out, the fog drains away from around you, leaving the forest clear and calm. Slowly, the sounds of birdsong and insects return to the woods around you.  \n\n");
    ...
    protected repeatWildHuntChase(character: Character) {
        const pScore: number = this.characterHuntScore(character);
        if (pScore > 150) {
            this.repeatWildHuntEscaped(character);
        }
        else {
            this.repeatWildHuntCaught(character, pScore);
        }
    }

    protected repeatWildHuntEscaped(character: Character) {
        DisplayText().clear();
    ...
    protected repeatWildHuntCaught(character: Character, pScore: number) {
        DisplayText().clear();
        ...
        DisplayText("You run through the woods, heart pounding so hard you feel it might leap out of your throat.  Despite your best efforts, though, the fog still closes in.  With it comes the sound of the hounds, running alongside you, hidden in the thick haze. \n\n");

        DisplayText("With the Hounds on your right, you juke left, nearly running into a tree, but stumble past it, only to hear the Hounds again on your left.  You turn right, still running, unable to shake the feeling that you’re being driven, but too panicked to figure a way out. \n\n");

        DisplayText("Your lungs are burning as you run at top speed, the Hounds driving you this way and that.  Something happens as you breathe the fog - it’s getting harder and harder to think.  You just need to run, <b>run</b> from the predators after you.\n\n");
        ...
        if (RaceScore.bunnyScore(character) >= 4 || RaceScore.kitsuneScore(character) >= 4 || RaceScore.harpyScore(character) >= 4 || pScore > 100)
            this.repeatWildHuntAWinnerIsYou(character);
        else this.repeatWildHuntGivenToTheHounds(character);
    }

    public repeatWildHuntGivenToTheHounds(character: Character) {
        ...
        DisplayText("“<i>Perhaps next time,</i>” he sighs.  He turns, his dark cloak flaring.  Is it your imagination or do his golden antlers seem a bit more dull?\n\n");

        DisplayText("You allow yourself a brief moment of relief as he turns his attention from you.\n\n");

        DisplayText("“<i>But it would be wrong to not reward my Hounds for a job well done.</i>”  His cane flashes with golden light as he slashes it against the support rope.\n\n");

        DisplayText("The fog swallows the Erlking as you drop to the ground.  The impact against the mossy forest floor doesn’t injure you, but it <b>does</b> knock the wind from you.  As you struggle to regain your breath, you inhale the icy fog, and a cascade of terror... and something else... runs through you.\n\n");
    ...
    protected repeatWildHuntAWinnerIsYou(character: Character) {
        ...
        DisplayText("“<i>And so, my hind, my prey, you have a choice,</i>” he says, his refined tones ringing through the air.  Through the net you can see the fog receding from the forest floor.  You take a few experimental breaths, and feel your head begin to clear.  \n\n");

        DisplayText("He grasps the rope holding you up, and with strength surprising for his slim frame, lowers you down to the forest floor.  “<i>You have my thanks for a grand( hunt,</i>” he says as, hand-over-hand, you are returned to the ground.	\n\n");

        DisplayText("As you touch down on the moss, the net falls away from you, and the Erlking offers you a hand up.  “<i>I’d like to offer you something.</i>”\n\n");

        DisplayText("You look up to see the slim, elegant form of the Erlking looking down at you.  Under his hunting gear, you can see the outline of a thick, equine cock, complete with triple prepuce rings straining against the black leathers.\n\n");

        DisplayText("A reward for all this trouble would definitely be welcome, and as much as you would appreciate a gift, you’d even welcome the opportunity to unwind after the exertion of the hunt.  \n\n");

        DisplayText("You pause for a moment to consider his words and realize he might be offering you more. If you’re feeling brave, you could ask for him to stop the hunt once and for all.  Or, if the hunt has finally broken your will, you might just submit to the huntsman forever.\n\n");
        ...
        character.stats.fatigue += 10;
        MainScreen.displayChoices(["Sex", "Prize?", "Stop", "Surrender", "Revenge"], [this.predatoryPrey, this.whatsMyPrize, this.stopTheMadness, this.surrenderToTheHounds, this.howDareYou]);
    }

    protected whatsMyPrize(character: Character) {
        DisplayText().clear();
        ...
        DisplayText("“<i>Of course, of course!</i>” laughs the Erlking, reaching into one of his saddle bags, and retrieving a small bundle.  He tosses it to you.  “<i>Better luck to you on the next hunt!</i>”\n\n");

        DisplayText("“<i>Next hunt?</i>” you begin, but before you can get the words out, the Erlking is already on his horse, thundering away through the trees.\n\n");
    ...
    protected stopTheMadness() {
        this.data.erlkingDisabled = 1;
        DisplayText().clear();

        // [This ends all the Erlking Encounters]

        DisplayText("You have had enough of this maniac and his insane hunt.\n\n");

        DisplayText("Supremely irritated, you tell him to stop hunting you. \n\n");

        DisplayText("“<i>Stop?</i>” he asks, the red light in his eyes dimming.\n\n");

        DisplayText("Narrowing your eyes, you begin to tell him exactly how fed up you are with this never-ending hunt.\n\n");

        DisplayText("“<i>As you wish,</i>” says the Erlking.  The fog rolls in once more, engulfing the Erlking and his steed.  It clears a moment later, leaving you alone in the forest.\n\n");
    ...
    protected surrenderToTheHounds(character: Character) {
        // [Bad End]
        DisplayText().clear();

        DisplayText("You sit there, shivering, unable to shake off the effects of the fog, looking up in utter fear at the huntsman who has pursued you for so long.\n\n");

        DisplayText("In a quavering voice, you bow your head, and ask him to take you as his prey once and for all.\n\n");

        DisplayText("“<i>My poor hind,</i>” he mutters.  You feel his gloved hand on your head, running through your [hair] as your shoulders slump.  “<i>Perhaps I hounded you too fiercely,</i>” he murmurs, cupping your cheek.\n\n");

        DisplayText("You shiver at his touch, completely broken.\n\n");

        DisplayText("“<i>Look at me,</i>” he commands.  Completely obedient to the Hunter’s words, you look up, meeting his red-ember eyes.  “<i>I shall make amends,</i>” he says softly. \n\n");
    ...
    protected predatoryPrey(character: Character) {
        DisplayText().clear();

        DisplayText("You stand, unable to take your eyes from the Erlking’s slim body and erect dick.\n\n");

        DisplayText("You smile seductively, asking the Erlking exactly what he’s offering.\n\n");
        ...
        if (!character.torso.hips.legs.isTaur()) {
            if (character.torso.vaginas.count > 0 && character.torso.cocks.count <= 0) {
                ...
                DisplayText("You wrap your arms around him, clinging to him as he shifts his grip, bearing you up as you quake with aftershocks of pleasure.  One arm holds you up, close to his muscular chest, his other gloved hand strokes your [hair], as the fog rolls in.\n\n");

                DisplayText("You feel drowsy as the air thickens with chill fog, though the Erlking’s body keeps you warm.  Despite your best efforts, you find yourself drifting to sleep in his arms.  \n\n");
                ...
                DisplayText("You wrap your arms around him, clinging to him as he shifts his grip, holding you up as you quake with aftershocks of pleasure.  One arm holds you up, close to his now-sticky, muscular chest, his other gloved hand still slowly stroking your cock, as the fog rolls in.\n\n");
            ...
            if (character.torso.vaginas.count > 0 && character.torso.cocks.count <= 0) {
                ...
                DisplayText("He allows you a moment to catch your breath, the pulls out.  You hear his cock slap wetly against his thigh.  A strong hand takes yours, guiding you across the clearing to a fallen log. Dazed, you follow him, and he sits, guiding you to do the same next to him.  \n\n");

                DisplayText("Reaching into his belt pouch, he pulls out a small bottle of salve.  One hand strokes your hair as the other begins to work the cream into your scratched [chest].  The cream is cool and soothing, and the Erlking is attentive.  You soon fall asleep, your head leaning against his chest.\n\n");
                ...
                DisplayText("The Erlking responds by stroking you faster, his hand tightening, increasing the tingling pleasure from the cream.  You groan, gripping the tree trunk, rubbing your chest against it for extra stimulation, the rough bark scratching harshly against your [nipples].  \n\n");

                DisplayText("The Erlking gives a final, tight squeeze, a fingertip pressed against the tip of your cock.  You moan, cumming in his hand, your cum jetting against his fingertips, spraying, hose-like, against the mossy forest floor.  \n\n");
                ...
                DisplayText("Sleepily, you’re only half aware as he guides you down to a grassy patch, where you quickly fall asleep.  You wake up an hour later in the clearing.  The Erlking is gone and but your cock gives a twitch as you remember his touch.  You shakily climb to your feet, making your way back to camp.\n\n");
            }
        }
    ...
    protected howDareYou(character: Character) {
        DisplayText().clear();
        ...
        DisplayText("He stumbles backward, pained as if you’d struck him directly.  A look of confusion crosses his ridiculous deer face.  “<i>Wait - my cane - it fortifies me against the rigors of this land... </i>” he gasps, suddenly staggering.\n\n");

        DisplayText("You sneer and stalk forward, another well-placed shove to his shoulders knocking him from his feet.  He drops to his knees with a groan.  At the edge of the clearing, the Hounds sit silently, awaiting the outcome of the confrontation.\n\n");

        DisplayText("“<i>Please... my cane...</i>” he pleads.  On his knees, his trembling arms hold him up.  He seems to be telling the truth - without the cane, he’s as weak as a newborn.\n\n");

        DisplayText("“<i>You taunt me, you hunt me, and now you ask for favors?</i>”  you snort.  “<i>No, no, you’re about to be taught a very lasting lesson,</i>” you snarl.\n\n");
        ...
        if (character.torso.vaginas.count > 0 && character.torso.cocks.count <= 0) {
            ...
            DisplayText("“<i>Lick it, Huntsman!</i>” you order, pulling on his antlers, his dark muzzle pushing hard against your pussy.  You snarl, feeling the timid push of his tongue against your pussy lips.\n\n");

            DisplayText("“<i>Deeper!</i>” you command.  You feel his tongue explore deeper into your wet snatch, and you guide his wet deer nose to rub against your clit. \n\n");

            DisplayText("“<i>Not a king anymore, are you?</i>” you growl, smiling.  “<i>You’re more like a Princess now.  A slutty, little Princess,</i>” you grin.  You see a tremble run down his chest and front, his cock straining against his tight leathers.  “<i>Eat my pussy, Princess,</i>” you order.\n\n");

            DisplayText("Your princess moans, shuddering and submissive, shoving his flat tongue into your pussy.  It might be a trick of the light, but you swear his fur just got a little lighter.  His hips writhe, drawing attention to how slim and girlish they really are.  He’s obviously responding very well to the abuse.\n\n");
            ...
            DisplayText("“<i>On all fours, slut,</i>” you smirk, shoving the huntsman back.  You throw his horns to the side as he stumbles to the mossy ground, visibly weakened without his cane.  Somehow, you don’t think it’s only the loss of his cane making him so submissive.  He looks more like a doe now with his horns gone.\n\n");
            ...
            DisplayText("“<i>Not a king anymore, are you?</i>” you growl, smiling.  “<i>You’re more like a Princess now.  A slutty, little Princess,</i>” you grin.  You see a tremble run down his back, and his soft, fluffy tail twitches in your hand.  “<i>Say it,</i>” you order.\n\n");

            DisplayText("“<i>I... I’m a Princess,</i>” he mutters, his voice noticeably higher.  You smile, wondering what else the loss of his cane will do to your princess.\n\n");

            DisplayText("“<i>Good girl,</i>” you coo, pressing your [cock] against his pucker.  The new princess moans, shuddering  as you do, shoving his ass back against you.\n\n");
            ...
            DisplayText("He shivers below you, tensing, as he grinds his hips back against you on every downstroke.  He bucks and moans, and you hear a splashing noise from beneath him.  From the looks of the sticky white pool growing, the slut is shooting his load HARD.\n\n");

            DisplayText("The sight of his total submission turns you on so much that you shudder, cumming deep inside his ass, filling him up with your hot, sticky seed.  As you grip his ass, you pant, driving in as deep as possible, marking him as your bitch forever.\n\n");

            DisplayText("He moans, slumping down, cheek to the ground, and you bend over him, grinding his face to the ground.\n\n");

            DisplayText("“<i>Dirty little slut,</i>” you growl.  “<i>Did you like that?</i>”\n\n");

            DisplayText("“<i>Y-yes...</i>” purrs the once-Lord.  “<i>I... I loved it...</i>” He shudders again, and you notice that the lighter fur is no illusion.  Your new doe princess is no longer midnight black, but a sandy-tan, and still slowly lightening.\n\n");

            DisplayText("“<i>I don’t expect we’ll have any more problems, will we?</i>” you ask, sliding out of him.  You rise, watching as his fur takes on a curiously pink hue.\n\n");
    ...
    protected encounterPrincessGwynn(character: Character) {
        DisplayText().clear();
        ...
        DisplayText("“<i>Muh-M’lord?</i>” she asks softly.  Her lithe arms push through the low branches and trees as she steps closer to you.  She’s timid and twitchy, quite different from the ominous and powerful huntsman she’d once been.  In a moment, you see why.\n\n");

        const selector: number = randInt(6);

        if (selector === 0) DisplayText("The Princess’s pink fur is slathered in smears of saps and juices, and angry red stings pepper her body.  It’s very obvious that she’s fallen prey to the roaming tentacle monsters that haunt the forests.\n\n");
        else if (selector === 1) DisplayText("The Princess winces as low branches drag across her ass, and you can see angry red claw marks criss-crossing her pert ass.  It seems she’s run afoul of Akbal, the feline lord of the Deepwoods.\n\n");
        else if (selector === 2) DisplayText("The Princess coughs, a trickle of cum running down her lips and chin.  You see bruises on her throat and chin, and tiny claw marks across her body.  It looks like she was caught by a roving gang of imps.\n\n");
        else if (selector === 3) DisplayText("The Princess’ pink fur is stained with multicolored blotches, and she sways a little bit, apparently woozy.  Goblin attacks can leave a person that way for quite some time.\n\n");
        else if (selector === 4) DisplayText("The Princess’ neck and wrists bear bruises, as if someone had bound her up roughly and repeatedly.\n\n");
        else if (selector === 5) DisplayText("As she turns to squeeze between two trees, you see that the Princess has numerous claw marks up and down her back.  It looks like, since her transformation, her Hounds have turned on her.\n\n");
        ...
        if (this.data.timesEncounteredPrincessGwynn === 0) {
            DisplayText("“<i>I’m so happy you helped me get rid of that nasty old cane,</i>” she says, waving a pink-furred arm vaguely at the forest.  “<i>It may have kept out the corruption, but it was giving me a </i>weird<i> idea of fun,</i>” she bubbles.  “<i>No more hunting for me - no, sir!</i>”\n\n");
            DisplayText("She touches her white fingers to her chest and purrs demurely, “<i>You can call me Gwynn, now.  But I’ll still be your princess!</i>”\n\n");
        }

        this.data.timesEncounteredPrincessGwynn++;
        ...
        if (character.torso.cocks.count > 0) {
            MainScreen.getBottomButton(0).modify("Suck Me", this.gwynnSucksDicks);
            MainScreen.getBottomButton(1).modify("Assfuck", this.gwynnGetsButtfuxed);
        }
        if (character.torso.vaginas.count > 0) {
            MainScreen.getBottomButton(2).modify("Eat Me", this.gwynnNomsDaCunts);
        }
        MainScreen.getBottomButton(3).modify("Milk Dick", this.gwynnGetsDickmilked);
        MainScreen.getBottomButton(4).modify("Gifts", this.gwynnGibsGifts);
    }

    protected gwynnSucksDicks(character: Character) {
        DisplayText().clear();
        ...
        DisplayText("Her slim tongue rubs against the underside of your shaft, massaging it in time to the bobbing of her head.  Her index finger and thumb form a slim O at the base of your dick, pumping it counter to her head bobs, giving you continuous stimulation as she hums.\n\n");

        DisplayText("She hums as she works, sounding like the happiest little slut in the world.  She expertly works your cock, stroking and pumping your meat with her wet, hungry mouth and you feel yourself breathing heavily as her slurping grows louder and more eager.  \n\n");

        DisplayText("The vibration of her chirpy voice and pressure of her lips around your cock soon push you over the edge, and with a relaxed shudder, you cum in her mouth.  She takes the first jet down her throat, then pulls her mouth from your dick with a satisfied gasp, taking the next jet across her face.\n\n");

        DisplayText("“<i>Oooooo...</i>” she moans, shivering in delight, her tongue lapping at the dripping cum, her fingers delicately wiping the thick spooge from her cheeks and depositing it in her mouth.  You notice that she’s shoved two of her slim fingers up her cute, perky ass as she slurps away at your spilled seed.\n\n");

        DisplayText("Satisfied, you pat her head, ruffling her short, pink hair, leaving her happy and smiling.\n\n");

        DisplayText("“<i>Thank you, M’lord!</i>” she calls as you walk off.\n\n");
    ...
    protected gwynnGetsButtfuxed(character: Character) {
        DisplayText().clear();
        ...
        DisplayText("“<i>Princess Gwynn is always prepared!</i>” she chirps happily.\n\n");

        DisplayText("She drops her shoulders to the ground and raises her white rump in the air, her pink, tufted tail twitching excitedly.  “<i>Your Princess is ready for you, m’Lord!</i>”\n\n");
        ...
        DisplayText("She moans and purrs, eyes rolled back and tongue hanging out as you have your way with her.  When you finally cum, her pink-and-white cock spills out her own load, strangely raspberry-scented, onto the forest floor.  Her hole doesn’t stop squeezing you, and continues to milk you until you’re completely spent.\n\n");

        DisplayText("You withdraw from her and she sits up, giggling, spinning on her knees to slurp at your cock, cleaning you off.  Just as when you first turned her, she cleans you completely, then helps you dress, giggling happily as you kiss her cheek farewell.\n\n");
    ...
    protected gwynnNomsDaCunts(character: Character) {
        DisplayText().clear();
        ...
        DisplayText("Her tongue eagerly plunges between your folds, running up and down the length of your pussy. Each time her muzzle bobs up, her candy-pink nose rubs against your clit, and she purrs, closing her eyes and nuzzling it.  She slowly laps at your slit, gradually building speed.\n\n");

        DisplayText("You sigh happily, bracing yourself with your arms and leaning back.  Princess Gwynn slurps noisily at your muff, her tongue moving faster and faster.  You gasp, pleasure building in your whole body as she withdraws her tongue from your pussy, wrapping her lips around your pleasure button.\n\n");
    ...
    protected gwynnGetsDickmilked(character: Character) {
        DisplayText().clear();

        DisplayText("“<i>My Lord, are you sure?</i>” she says, tilting her head to the side.\n\n");

        DisplayText("You assure her that this is what you want and order her onto her back, lying on the soft, mossy ground.  One lithe arm is draped over her head, the other crosses over her chest, as she bites her lip, looking up at you nervously.\n\n");

        DisplayText("Her dick is already rigid, a foot long and mottled pink and white, like clouds at dawn.  You kneel down next to her and grasp it slowly, your fingers finding natural handholds along her triple prepuce rings.\n\n");
        ...
        DisplayText("“<i>Oh, nuh-nooo...</i>” she gasps, her knees now planted on either side of her head.\n\n");

        DisplayText("“<i>Oh, yes,</i>” you coo wickedly, shoving the lithe doe’s cock into her own mouth.  She blathers a muffled protest before you give her tufted tail another sharp tug, drawing out her orgasm..\n\n");

        DisplayText("Her cock spurts into her own mouth, immediately ballooning out her cheeks.  Her eyes roll back in her head as she gurgles around her cock.  Slightly-pink, raspberry-scented cum spills out from her pouty lips, as she coughs and nearly chokes on her own cum.  \n\n");
        ...
        DisplayText("Satisfied that she’ll be fine, you stand up, leaving the slutty doe to clean herself up.\n\n");
    ...
    protected gwynnGibsGifts(character: Character) {
        DisplayText().clear();

        DisplayText("“<i>Do you have any presents for your Master?</i>” you ask casually.\n\n");

        DisplayText("Princess Gwynn claps her hands, bouncing on one foot, then the other.  “<i>Yes I do, M’Lord!  Yes I do!</i>”  She stops bouncing long enough to rifle through her purse, pulling out a small, pink bottle. \n\n");
