...
function flowerFun(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Corrupted_Glade);
    if (player.torso.cocks.count > 0) { // Sex scenes for those with cawks
        if (player.torso.cocks.count === 1) { // Single Cawk
            DisplayText("You grin to yourself as you decide to see just how close to a pussy these perverted little flowers are.  The thick stem bends with ease as you grasp it and bend it towards your groin, your other hand fumbling to open your " + player.inventory.equipment.armor.displayName + ".  In seconds you free yourself and gingerly bring the folds closer, the musky scent that fills the air rapidly bringing you to a full, throbbing hardness.  The first touch of petals to your skin slicks you with the flower's silky secretions, allowing you to easily slip between the petals.  Though the flower looks fairly deep, you quickly feel yourself bottom out inside the petal's slippery grip.  Shrugging, you decide to make the best of it and begin thrusting into the plant, enjoying the unusual sensations along the front-most parts of your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ".  As you pound away, you begin to notice a change in the rear of the flower.\n\n");

            DisplayText("It feels as if something is opening up, and the tip of your cock begins slipping through a tight ring, bulging the plant's stem noticeably.  The sudden change worries you enough to pull back for a moment, your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " nearly clearing the opening before dozens of tiny whip-like tendrils burst from the flower, wrapping your maleness with painful tightness.  They constrict further and with a burst of movement, slam the flower down onto your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ", pulling you further and further into the stem with painful force.  You struggle briefly but the pain it causes your over-stimulated member is too much, so you just give up, letting the pussy-like plant draw the last of you inside its stem, the silken flowers cupping around your ");
            ...
            DisplayText("You feel a flood of wetness surge up from the depths of the plant, surrounding your member with even more fluid as the stem begins constricting and squeezing.  Gently at first, and then with increasing insistence, a suction builds inside the stem, drawing more and more blood into your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + ".  The stem, now heavily distended by your massive member, continues rippling, squeezing, and sucking your over-engorged meat-pole, overwhelming your mind with sensation far beyond normal.  You'd wonder just what kind of tactile-enhancing fluids that plant excretes, if you weren't already mindlessly pistoning against the tainted plant, still locked inside it by tight little tentacles.\n\n");
        }
        else { // Multicock
            DisplayText("You grin to yourself as you decide to see just how close to a pussy these perverted little flowers are.  The thick stems bend with ease as you grab a few with your hand and pull them towards your groin, your other hand fumbling to open your " + player.inventory.equipment.armor.displayName + ".  In seconds you free yourself, and gingerly bring the folds closer.  The musky scent filling the air rapidly brings your " + Desc.Cock.describeMultiCockShort(player) + " to a full, throbbing hardness.  The first touch of petals to your skin slicks you with the flower's silky secretions, allowing you to easily slip between the petals.  Though the flowers look fairly deep, you quickly feel yourself bottom out inside the petals' slippery grip.  Shrugging, you decide to make the best of it and begin thrusting into the plant, enjoying the unusual sensations along the front-most parts of your " + Desc.Cock.describeMultiCockShort(player) + ".  As you pound away, you begin to notice a change in the rear of the flowers.\n\n");

            DisplayText("They seem to be gradually opening up, allowing the smallest of your cock-tips to begin slipping through an opening in the backs of the flowers and into the stems.  Shocked by this unexpected development, you pull the bundle of flowers from your " + Desc.Cock.describeMultiCockShort(player) + ", but whiplike tendrils shoot forth from deep within the flowers, wrapping tightly around your manhoods, painfully squeezing as they drag your " + Desc.Cock.describeMultiCockShort(player) + " back into the tight vaginal openings.  They pull tighter as they force you deeper inside the plant, pulling the full length of each of your members into the constricting stalks.   Wrapped tightly around your base, the tendrils form effective cock-rings, making each of your " + Desc.Cock.describeMultiCockShort(player) + " overfill with blood.\n\n");
        ...
        DisplayText("You cum, and cum, and cum, the evidence of your pleasure devoured by the plant's sucking, squeezing gullet.  The orgasm drags on for what feels like forever, your " + Desc.Leg.describeLegs(player) + " eventually giving out, your hips the only muscle that seems to work as they twitch into the air, as if begging for more.  You are milked of a few last big spurts, at last collapsing.\n\n");

        DisplayText("The tendrils encircling your genitals do not release; instead they pull tighter, one of " + (player.torso.cocks.count > 1 ? "each plant's tiny" : "the tiny plant's") + " appendages penetrating your urethra" + (player.torso.cocks.count > 1 ? "s" : "") + ", squirming up your cum slicked passage" + (player.torso.cocks.count > 1 ? "s" : "") + " with uncomfortable slowness.  You lay there, too weak to resist it or fight, hoping that whatever the plants are doing won't hurt much.  You feel it twisting and coiling inside you... until it stops.  You feel a sharp pinch, and then it withdraws, seemingly satisfied.  The tendrils unwrap, allowing the plants to spring back up, exposing your still over-engorged and sensitive member" + (player.torso.cocks.count > 1 ? "s" : "") + ".\n\n");

        DisplayText("You lay there for some time until your muscle control returns, your cock" + (player.torso.cocks.count > 1 ? "s" : "") + " still slightly over-large " + (player.torso.balls.quantity >= 2 ? "and your " + Desc.Balls.describeBalls(true, true, player) : "and") + " feeling sore from the exertion.  At least you hope it's just from the exertion and not from whatever the plant did.\n\n");
        if (player.cumQ() < 25)
        ...
        if (player.torso.balls.quantity === 0)
            booster += 3;
        else if (player.torso.balls.size < 6)
            booster += 2;
        if (player.perks.has(PerkType.MessyOrgasms) && player.cumMultiplier < 3) booster += 1;
...
function tentacleFun(player: Character): NextScreenChoices {
    DisplayText().clear();
    DisplaySprite(SpriteName.Corrupted_Glade);
    if (player.torso.vaginas.count > 0 && randInt(2) === 0) { // Vaginal Variant 50% of the time
        DisplayText("You saunter over to a dangling group of perverted looking vines, discarding your " + player.inventory.equipment.armor.displayName + " along the way. Running your fingertips along the bulbous-tipped tentacle-like vines, you find one that looks ");
        ...
        if (tentacle === 0) { // Small
            ...
            switch (player.torso.vaginas.get(0).looseness) {
        ...
        // resume secksings
        DisplayText(Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ". Pulling gently, you manage to yank a bit more vine free, allowing it to brush against the damp forest loam. That same soft earth makes the perfect cushion for you as you lay down, spreading your legs. With both hands you grasp the vine, guiding it towards the entrance of your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ". The beaded moisture that covers the vine tingles tantalizingly at the first contact with your lips.\n\n");

        if (tentacle === 0) // Small
            DisplayText("With a sexy little sigh, you slip the mushroom-like tip between your nether-lips, feeling it bulge a little as it penetrates you. The vine's lubricants combine with your own, turning your horny cunt into a sloppy little slip-and-slide. You take it all the way to your cervix, easily handling its smaller size as you begin to use it like a favorite dildo. Deep inside your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + ", the vine's lubricants begin to make your passage tingle, intensifying until your entire channel is overloaded with clit-like levels of sensation.\n\n");
        ...
        player.stats.cor += 2;
        // Xforms
        if (randInt(3) === 0 && player.torso.neck.head.hair.color !== "green") { // Change hair to green sometimes
            ...
            player.torso.neck.head.hair.color = "green";
        }
        if (randInt(4) === 0 && player.torso.hips.rating <= 10) { // +hip up to 10
            DisplayText("A strange shifting occurs below your waist, making your " + player.inventory.equipment.armor.displayName + " feel tight.  <b>Your hips have grown larger</b>, becoming " + Desc.Hip.describeHips(player) + ".  ");
            player.torso.hips.rating += randInt(3) + 1;
            player.fertility++;
        }
    }
    else {
        ...
        if (player.torso.cocks.count > 0) { // Cocks (and maybe vagina)
            DisplayText(player.torso.cocks.count === 1 ? Desc.Cock.describeCock(player, player.torso.cocks.get(0)) : Desc.Cock.describeMultiCockShort(player));
            if (player.torso.vaginas.count > 0) DisplayText(" and " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)));
        }
        else if (player.torso.vaginas.count > 0) // Vagina
        ...
        if (randInt(3) === 0 && player.torso.neck.head.hair.color !== "green") {
            DisplayText("You don't get far before you realize all the hair on your body has shifted to a verdant green color.  <b>You now have green hair.</b>  ");
            player.torso.neck.head.hair.color = "green";
        }
        // +butt up to 10
        if (randInt(4) === 0 && player.torso.butt.rating <= 10) {
            DisplayText("A strange shifting occurs on your backside, making your " + player.inventory.equipment.armor.displayName + " feel tight.  <b>Your butt has grown larger</b>, becoming a " + Desc.Butt.describeButt(player) + ".  ");
            player.torso.butt.rating += randInt(3) + 1;
        }
        // Rarely change one prick to a vine-like tentacle cock.
        if (randInt(3) === 0 && player.torso.cocks.count > 0 && player.torso.neck.head.hair.color === "green") {
            if (player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE)).length < player.torso.cocks.count) {
                if (player.torso.cocks.count === 1) { // Single cawks
                    DisplayText("Your feel your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " bending and flexing of its own volition... looking down, you see it morph into a green vine-like shape.  <b>You now have a tentacle cock!</b>  ");
                    // Set primary cock flag
                    player.torso.cocks.get(0).type = CockType.TENTACLE;
                }
                if (player.torso.cocks.count > 1) { // multi
                    DisplayText("Your feel your " + Desc.Cock.describeMultiCockShort(player) + " bending and flexing of their own volition... looking down, you watch them morph into flexible vine-like shapes.  <b>You now have green tentacle cocks!</b>  ");
...
function treeBoobFun(player: Character): NextScreenChoices {
    ...
    player.stats.cor += .5;
    player.slimeFeed();
    if (!player.statusAffects.has(StatusAffectType.LustyTongue)) {
        if (randInt(4) === 0) { // 25% Chance of sensitive mouth status – increased lust gain/hour due to licking your lips :3
            ...
            player.statusAffects.add(StatusAffectType.LustyTongue, 24, 0, 0, 0);
        }
    ...
    if (randInt(2) === 0) { // 50% Chance of breast-growth
        if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 5) {
            ...
            Mod.Breast.growSmallestBreastRow(player, 1, 1 + randInt(3), false);
            // character.growTits(1, 1 + randInt(3), false, 1);
        }
        else {
            if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating === 0) { // No tits yet
                DisplayText("  As you redress, you realize you have grown a pair of luscious breasts!  Perhaps this was a good idea after all...");
                Mod.Breast.growSmallestBreastRow(player, 1, 2 + randInt(3), false);
