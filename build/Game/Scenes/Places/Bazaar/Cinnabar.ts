...
private cinnabarGreeting():void {
	...
	if(flags[FlagEnum.CINNABAR_NUMBER_ENCOUNTERS] === 0) {
		...
		//(Small Cock)
		else if(player.torso.cocks.biggestCocks[0].area < 100) {
		...
		//(Big enough Cock) 
			...
			if(player.torso.cocks.count > 1 && player.torso.cocks.biggestCocks[0].cockArea2() >= 75) DisplayText("those things");
			...
			//[Fuck] [Leave]
			if (player.torso.cocks.count > 2 && player.torso.cocks.biggestCocks[0].cockArea2() >= 75)
	...
	//Not first time
		...
		//(Big enough Cock) 
			...
			if(player.torso.cocks.count > 2 && player.torso.cocks.biggestCocks[0].cockArea2() >= 75) DisplayText("those things");
			else DisplayText("that thing");
...
private cinnabarNonHugeDickings():void {
	...
	statScreenRefresh();
	DisplayText("Once you give your assent, Cinnabar's paws dive into your " + player.inventory.equipment.armor.displayName + ", gliding over your " + player.skinDesc + " before re-emerging above your gem-pouch.  She deftly grabs her payment and snakes back through, tickling your belly with her soft, well-groomed fur.  You look at her, then yourself, then her again.  How the hell was she flexible enough to do that?  Cinnabar giggles and curls her tail around your " + LegDescriptor.describeLegs(player) + ", looping it like a long, flexible rope.  She hugs you and murmurs, \"<i>Where will my " + player.mf("master","mistress") + " take " + player.mf("his","her") + " slutty, fuck-hungry companion?</i>\"  Her tongue flicks your ear like a snake tasting its prey, and though her attitude sounds submissive and weak, your horny 'companion' is clearly hiding a fiery, aggressive streak.\n\n");
	
	DisplayText("Before you can make an answer, Cinnabar's hands are back inside your " + player.inventory.equipment.armor.displayName + ", tugging your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + " towards an unmarked wagon that's clearly her home.  You smirk and rub her between her legs, feeling the moistness of her sloppy cunt through her now-dampened dress.  Before she can tease you any more, you hook a finger up, pushing her dress into her hungry snatch.  It's easy to lead the giggling rat-whore like this, and when you reach the door she slides her tail into the lock, clicking it open with ease.\n\n");
	...
	DisplayText("Too shocked to keep your grip, you listen dumbly as Cinnabar groans, \"<i>" + player.mf("Master","Mistress") + " is pleased?  This one knows how to use portals for their intended purpose.  Watch slut make more sexy portals.</i>\"  True to her words, many of the other mirrors change, showing both sets of nipples");
	if(player.torso.vaginas.count <= 0) DisplayText(" and your " + ButtDescriptor.describeButthole(character.torso.butt));
	else DisplayText(", your " +             VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ", and your " + ButtDescriptor.describeButthole(character.torso.butt));
	...
	if(player.torso.vaginas.count > 0) DisplayText("grab your " + VaginaDescriptor.describeClit(player) + ", rolling it between her thumb-pads with a hair too much pressure");
	else DisplayText("slip two digits into your " + ButtDescriptor.describeButthole(character.torso.butt) + ", pulling it apart far enough to cause you discomfort");
	...
	//Finger poke
	if(player.torso.vaginas.count > 0) player.displayStretchVagina(8,true,true,false);
	else player.displayStretchButt(8,true,true,false);
	if(player.torso.cocks.count > 0 && player.torso.vaginas.count <= 0) DisplayText("  Unbidden, " + CockDescriptor.describeMultiCockSimpleOne(player) + " begins to harden, stiffening with every brush against your prostate.");
	...
	DisplayText("  You rock back, slamming your " + ButtDescriptor.describeButt(player) + " against the wall and moaning with need.  She's quite skilled, and ");
	...
	DisplayText("leave you panting like a bitch in heat.  She urges you on, practically begging, \"<i>Yes!  Stuff my hole!  Fist me, " + player.mf("master","mistress") + "; stretch me with your hand until I'm too loose for a minotaur!  Please... please... fist my ass too.  Ruin my holes, " + player.mf("master","mistress") + "!  Break fuck-toy's body!</i>\"\n\n");
	...
	if(player.torso.cocks.count > 0 && player.torso.vaginas.count <= 0) DisplayText("You gasp and moan when you feel her other hand wrap around your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + ", pulling and tugging on it until it starts to leak a trail of squeezed-out man-slime on the mattress.  ");
	...
	if(player.torso.cocks.count > 0 && player.torso.vaginas.count <= 0) {
		DisplayText("  Her soft pads grip your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)) + " tightly while she  presses on your prostate, milking your cum from you with professional skill.  Rather than coming in its usual spurts, it drips in a long, liquid chain of white-goo that drops into a submissive puddle on the bed.");
		if(player.cumQ() >= 250) {
	...
	if(player.torso.vaginas.count > 0) {
		DisplayText("  She licks your " + VaginaDescriptor.describeClit(player) + ", and your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " convulses around the intruding digits, making you shiver and moan.  Not wanting to be left out, your " + ButtDescriptor.describeButthole(character.torso.butt) + " clamps and puckers with it, wringing the rat's ");
		if(player.vaginalCapacity() < 30) DisplayText("fingers");
		else DisplayText("hand");
		DisplayText(" in its hungry embrace.");
	}
	//(No dick/cunt:
	if(player.gender === Gender.NONE) DisplayText("  Her soft pads rub your " + ButtDescriptor.describeButthole(character.torso.butt) + " just right, setting off your climax with professional skill.  You grunt and squeeze, wringing her fingers inside your hungry hole as if you could milk cum from them.");
...
private cinnabarGetsFUKKKKED():void {
	...
	if(flags[FlagEnum.CINNABAR_FUCKED_CORRUPT_PREVIOUSLY] > 0) DisplayText("  Just be gentler than last time, okay " + player.mf("stud","baby") + "?");
	...
	DisplayText("Flexible, determined fingertips are buried deep into your crotch before the entry has closed behind.  Cinnabar's eyes fixate on the massive, tenting bulge between your " + LegDescriptor.describeLegs(player) + ", her soft finger-pads shaking while they try to set you free.  You appreciate her efforts, but she nearly got you into some trouble out there.  She needs to settle down, or at least be a little more mindful of your wishes.  You yank her arms away from your bouncing, newly-freed " + CockDescriptor.describeCock(player, x) + ", groaning from sensation while you wrestle to deny Cinnabar her desire.\n\n");
	...
	if(player.cockArea(x) < 200) DisplayText("centaur mare wince");
	...
	if(player.torso.balls.quantity > 0) DisplayText(BallsDescriptor.describeSack(player) + " and ");
	DisplayText(CockDescriptor.describeCock(player, x) + " before pulling tight.  You ignore the onslaught of pressure and line your fat prick up with her inviting, fist-gaped hole.  It still doesn't look like it'll fit, but you press forwards anyway – she wanted it inside her and you've got way too much blood in your dick to do any logical thinking.\n\n");
	
	DisplayText("Your " + player.cockHead(x) + " hits those plush, juice-gushing lips and forces them wide.  She's tight, so goddamned tight, but you've only got the barest portion of your tip inside.  Cinnabar squeaks in pleasure and pain, rocking her ass up and down to try and loosen up her poor, abused entrance.\n\n");
...
private fuckCinnabarHugerAndCorruptedEr():void {
	...
	DisplayText("\"<i>T-too fast... " + player.mf("Master","Mistress") + ", too fast!  Please go slower... please... ungghhAAAHHH... Godda-ah-aH-AH-AMNIT!</i>\" she protests.\n\n");
	...
	DisplayText(", squirting juices from the callous, cunt-piercing thrust.  The " + player.cockHead(x) + " of your " + CockDescriptor.describeCock(player, x) + " can be seen just below her neck");
	...
	else DisplayText(", and only now do you realize you blew past her cervix long ago, the tight ring hugging your " + CockDescriptor.describeCock(player, x) + " in a clutching embrace.");
	DisplayText("\n\n");
	...
	DisplayText("  A half-second later your " + CockDescriptor.describeCock(player, x) + " joins it, your internal muscles clenching and convulsing, heaving the jizz from your body in one huge, gooey blast.\n\n");
	
	DisplayText("Cinnabar pulls her dirty paw out and grins like a mouse with a chunk of cheese, massaging her distorting neck while you empty every ounce of cum from your " + BallsDescriptor.describeBalls(true, true, player) + " into her over-stretched womb-filled body.  She squeaks, \"<i>No-ohhhh-ormally I LIKE being a mewling slut for " + player.mf("studs","bitches") + " like you, but if you won't treat a cum-sleeve like me with respect, then I'm going to teach it to you.</i>\"  You nod dumbly, still locked in an climax that ");
...
private cinnabarHuger():void {
	...
	DisplayText("True to her words, she starts squeezing you through her fur, massaging your " + CockDescriptor.describeCock(player, x) + " by dragging her stretchy puss along your length with her fingertips.  It's too much, her body is fucking perfect for your " + CockDescriptor.describeCock(player, x) + ", and you feel warmth building inside you.  Her tail twists around your ");
	if(player.torso.balls.quantity > 0) DisplayText(BallsDescriptor.describeBalls(true, true, player));
	else DisplayText("base");
	DisplayText(" to prod at your " + ButtDescriptor.describeButthole(character.torso.butt) + ", sliding in to push your prostate.  She hits it hard enough to make you pump cum with enough force to distort her already-ballooned body, and you go into an orgasmic frenzy, dragging your " + CockDescriptor.describeCock(player, x) + " back to her entrance before jamming in all ");
	if(player.torso.cocks.get(x).length < 24) DisplayText(num2Text(Math.round(player.torso.cocks.get(x).length)) + " inches");
	else DisplayText(num2Text(Math.round(player.torso.cocks.get(x).length/12)) + " feet");
	...
	DisplayText("Her body is squirming, writhing from the complete and total dick-domination.  You push her hands off your " + CockDescriptor.describeCock(player, x) + ", and jack yourself off while you cum inside her, watching with fascination as each blast bulges her belly before it vanishes inside her with a wet slosh.  Her huge, stretched lips leak like a sieve, splattering girlcum everywhere while your " + BallsDescriptor.describeBalls(true, true, player) + " pack");
	if(player.torso.balls.quantity === 0) DisplayText("s");
	DisplayText(" her with goo, claiming her ruined body.  Cinnabar pants and screams in between pleasure-squeaks, \"<i>F-fuck... fuck your fuck-toy!  Stuff her cunt... YES!  S-slut is so fucked... Cum, " + player.mf("master","mistress") + ", then toss slut aside l-l-like a discarded, broken toy.  YOU'VE RUINED ME " + player.mf("MASTER","MISTRESS") + "!</i>\"\n\n");
	
	DisplayText("Those wonderful, perverse words milk your " + BallsDescriptor.describeBalls(true, true, player) + " dry as effectively as her massively-gaped cunt and broken body, but you keep pumping until you're sore and completely emptied.  ");
	if(player.cumQ() >= 500) {
	...
	DisplayText("At last, your " + player.cockHead(x) + " pops from her pussy, showing just how completely you've worked over her body.  Her hips are stretched damn near a foot apart, and you can see deep enough inside her ");
...
private cinnabarHuge():void {
	...
	DisplayText().clear();
	DisplaySprite(91);
	DisplayText("You're almost there, and though you can feel the burgeoning warmth growing underneath your " + CockDescriptor.describeCock(player, x) + ", you're determined to push the whole way inside her before you blow.  Cinnabar crams her muzzle through a particularly large mirror and locks onto your lips, ramming her tongue deeper into your throat, pushing harder with each inch of tightly-packed cock you shove inside her.  At last you bottom out, your " + player.cockHead(x) + " bumps the tight ring of her cervix while her tail pulls tight about your ");
	if(player.torso.balls.quantity > 0) DisplayText(BallsDescriptor.describeBalls(true, true, player));
	...
	DisplayText("your " + LegDescriptor.describeLegs(player) + ", and ");
	if(!player.torso.cocks.hasSheath()) DisplayText("the base of your twitching cock");
	else DisplayText("your twitching cock-sheath");
	DisplayText(".  You rub your hand over her belly, feeling your " + player.cockHead(x) + " through her distended, dick-dominated fur.  Each fiery, pumping load of cum boils underneath the rat's fur, bubbling between your fingers as it sloshes into her womb.  Cinnabar is thrashing, squeaking and crying, though it's quite clear her tears are tears of joy.  She gasps, \"<i>So big");
	...
	else DisplayText(BallsDescriptor.describeBalls(true, true, player) + " feel sore and empty, completely drained.");
	if(player.cumQ() > 500) {
		DisplayText("  The rat-girl has gained a nice, ");
...
private cinnabarMultiCockPortalFuckFest():void {
	...
	DisplayText("Cinnabar's hands fly through your " + player.inventory.equipment.armor.displayName + ", diving through your undergarments to squeeze " + CockDescriptor.describeMultiCockSimpleOne(player) + ".  ");
	...
	DisplayText("  Your " + LegDescriptor.describeLegs(player) + " go wobbly from the delicate touches dancing through your " + CockDescriptor.describeMultiCockShort(player) + ", making it hard to stand.  Cinnabar slides ");
	if(player.tallness >= 72) DisplayText("under your arm");
	else DisplayText("up against you");
	DisplayText(", supporting you while she asks, \"<i>Do we need to get inside and get these taken care of, big " + player.mf("boy","girl") + "?  Ohhh, that's just too bad.  I think I'll just play with you in the street until you're ready to pop, and then leave you like that until you go soft.  Then we can start over.</i>\"\n\n");
	...
	DisplayText("The sensation is absolutely divine, but primal need takes over.  You begin to rock your " +  LowerBodyDescriptor.describeHips(player) + " in and out, triple-penetrating the whore");
	if(player.torso.cocks.count >= 4) DisplayText(" while you tit-fuck her swollen breasts");
	DisplayText(".  Slippery-soft warmth wreathes your " + CockDescriptor.describeCock(player, x) + ", the plush black love-tunnel contracting slightly to happily squeeze you every few seconds.  The darker, sinfully tight star of Cinnabar's anus glazes with your pre-cum, her ass-cheeks wobbling perfectly in your hands with each rhythmic penetration.");
	...
	DisplayText("  The " + CockDescriptor.describeCock(player, z) + " in her mouth feels so good it nearly makes you jump, the long pink tongue licking around it while a wet muzzle buries itself in your crotch.  Gods, it's too much!\n\n");
	
	DisplayText("You feel yourself cumming, that tell-tale warmth in your gut mixing with a few involuntary muscle-contractions that bounce your " + CockDescriptor.describeMultiCockShort(player) + " inside Cinnabar.  You turn your head to watch her, " + num2Text(player.torso.cocks.count) + " penises pumping from holes in the wall to ravage her body completely and totally, her eyes rolled back in complete bliss.   Heat blooms in your " + CockDescriptor.describeMultiCockShort(player) + ", the pumping gouts of seed deep into all of the slut's holes simultaneously.  She screams into your " + CockDescriptor.describeCock(player, z) + ", vibrations making your orgasm that much stronger.  It drags on and on, the spit-roasted rat shaking while she's completely taken in every way");
	...
	DisplayText("The slut-rat gargles in irritation around your sensitive, cummed-out cock, clearly displeased that you're still inside her.  You step back from the wall, your " + LegDescriptor.describeLegs(player) + " giving out from the sensory overload of unsheathing your " + CockDescriptor.describeMultiCockShort(player) + " simultaneously to drop you flat on your " + ButtDescriptor.describeButt(player) + ".  At the same time, Cinnabar starts coughing and groaning, clearly exhausted from the one-" + player.mf("man","woman") + " gangbang.  You understand how that feels; just touching " + CockDescriptor.describeMultiCockSimpleOne(player) + " makes your head swoon.\n\n");
	
	DisplayText("An incessant licking reaches your ears, getting your attention.  You stand up, careful about your " + CockDescriptor.describeMultiCockShort(player) + " while you look towards the bed.  The portals are closed down, and Cinnabar is bent double, folded in half like a contortionist");
	...
	DisplayText("\"<i>You're still here?  Go on, I've got a lot of cleaning up to do before my next customer!</i>\" grunts Cinnabar before returning to her task.  You run your hand through your " + HeadDescriptor.describeHair(player) + ", exasperated, but quite satisfied.  You'll definitely have to visit her again sometime.\n\n");
