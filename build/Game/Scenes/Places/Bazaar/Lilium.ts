...
private approachLilium():void {
	...
	if(player.stats.gems < 200) DisplayText("<b>You remember that you haven't got the 200 gems for her services right now.  Maybe next time.</b>");
...
private buttFuckTongueJeorb():void {
	...
	DisplayText("Squatting before you, she traces a single gloved finger along the underside of your " + CockDescriptor.describeCock(player, x) + ", teasing it until it grows to its full " + Math.round(player.torso.cocks.get(x).length) + "-inch length.  Once your maleness has sprung to life, she turns around and bends over to brace herself against the wall before looking back over her shoulder at you mischievously and giving her hips a little wiggle.  Taking position behind her, you slide your " + CockDescriptor.describeCock(player, x) + " between her soft thighs.");
	//(If player cock length > 8 inches)
	if(player.torso.cocks.get(x).length > 8) DisplayText("  You can feel Lilium's smaller cock resting atop your " + CockDescriptor.describeCock(player, x) + ".");
	...
	DisplayText("One last time, you slowly draw your " + CockDescriptor.describeCock(player, x) + " from between her thighs and let it flop against her lower back, your hands moving from Lilium's hips to her pearlescent ass cheeks.  Spreading her wide, you let your " + CockDescriptor.describeCock(player, x) + " gradually inch down the cleft between them until the head rests against her back entrance.  With an agonizing slowness you begin to push.  The faintest gasps escape Lilium's lips as her ass finally envelops the head of your cock, followed by a squeak of surprise as you suddenly drive the remaining length in; the force of your " + LowerBodyDescriptor.describeHips(player) + " meeting her ass pushes her entirely up against the wall.\n\n");
	...
	if(player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 1) {
		DisplayText("  A tingle emanates from your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + "s as your " + BreastDescriptor.describeAllBreasts(player) + " slide across her sweat-slicked back with every thrust.");
	...
	//(If naga body)
	if(player.torso.hips.legs.isNaga()) DisplayText("As you both stand there, reeling in the wake of your ecstasy, she leans back against you causing you to flop onto your " + ButtDescriptor.describeButt(player) + " with the demon girl on top of you; you haven't the strength to remain standing.");
	//(else) 
	else DisplayText("As you both stand there, reeling in the wake of your ecstasy, she leans back against you causing you to flop onto your " + ButtDescriptor.describeButt(player) + " with the demon girl on top of you; your wobbling legs have temporarily lost the strength to keep you both upright.");
...
private liliumDickRidah():void {
	...
	DisplayText("You draw yourself up and position yourself above Lilium's twitching cock.  Your fingers part the lips of your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " which are slick with your own arousal.  ");
	//(If virgin/tight)
	...
	else DisplayText("Without a second thought you plunge yourself down on Lilium's fuckrod, your " + ButtDescriptor.describeButt(player) + " jiggling from the sudden impact.  ");
	...
	//(if player has breasts)
	if(player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 1) {
		DisplayText("You turn your attention to your own " + BreastDescriptor.describeAllBreasts(player) + " and how they bounce from the force of your vigorous fucking, supporting the weight of your " + BreastDescriptor.describeBreastRow(player.torso.chest.get(0)) + " with your hands as you continue to ride Lilium's cock.");
		//(if demonic tongue)
		...
		else DisplayText("  You begin to squeeze your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + " between two fingers, sending an electric thrill that flows to your core and back out to the tips of your fingers.");
...
private liliumNippleFuck():void {
	...
	else DisplayText("your " + LowerBodyDescriptor.describeFeet(player));
