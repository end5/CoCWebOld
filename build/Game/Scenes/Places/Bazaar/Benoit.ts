...
private benoitKnocksUpPCCheck():void {
	...
	if (player.pregnancyType === PregnancyType.BASILISK && player.perks.has(PerkType.BasiliskWomb)) player.knockUpForce(PregnancyType.BENOIT, player.pregnancyIncubation);
	//Knock up chances:
	if ((player.pregnancyType === PregnancyType.OVIELIXIR_EGGS || player.perks.has(PerkType.HarpyWomb) || player.perks.has(PerkType.Oviposition) || player.perks.has(PerkType.BasiliskWomb)) && (player.pregnancyIncubation === 0 || player.pregnancyType === PregnancyType.OVIELIXIR_EGGS)) {
		if (player.perks.has(PerkType.BasiliskWomb) && Flags.list[FlagEnum.BENOIT_TESTED_BASILISK_WOMB] === 1) {
...
public benoitKnockUp():boolean
{
	...
	let bounty:boolean = (player.perks.has(PerkType.ElvenBounty)) ? true : false;
	let stud:boolean = (player.perks.has(PerkType.MaraesGiftStud)) ? true : false;
	let alpha:boolean = (player.perks.has(PerkType.FerasBoonAlpha)) ? true : false;
...
public benoitIntro():void {
	...
	else if(Flags.list[FlagEnum.BENOIT_SUGGEST_UNLOCKED] === 0 && player.torso.vaginas.count > 0 && (player.inHeat || player.pregnancyType === PregnancyType.OVIELIXIR_EGGS || player.perks.has(PerkType.HarpyWomb) || player.perks.has(PerkType.Oviposition)) && (player.pregnancyType === PregnancyType.OVIELIXIR_EGGS || player.pregnancyIncubation === 0) && Flags.list[FlagEnum.BENOIT_STATUS] === 0) {
	...
	else if (benoitInClutch() && Flags.list[FlagEnum.FEMOIT_READY_FOR_EGGS] === 0 && (Flags.list[FlagEnum.BENOIT_STATUS] === 1 || Flags.list[FlagEnum.BENOIT_STATUS] === 2))
	{
		...
		DisplayText("As you enter the stall Benoite looks up; though her blind, milky eyes make her harder to read, she looks excited. \"<i>Ah! [name]; it iz good you are here... I had hope zat you would come for a vizit today.</i>\"");
	...
	else if (!benoitInClutch() && !benoitPreggers() && (this.Game.model.time.days - Flags.list[FlagEnum.FEMOIT_NEXTDAY_EVENT] >= 30) && (Flags.list[FlagEnum.BENOIT_STATUS] > 0))
	{
		if (Flags.list[FlagEnum.FEMOIT_FIRST_CLUTCH_MISSED] === 0)
		{
			...
			DisplayText("When you enter the stall, you are greeted by the smell of something cooking. Investigating further brings you to the blind basilisk's small kitchen, where she is busy frying something. Her nose preoccupied with her meal, she doesn't realize you're approaching until you touch her shoulder, yelping in shock.");
		...
		{
			DisplayText("The flat-bellied basilisk is tucking away into a plate laden with a heavy omlette when you arrive.");
	...
	else
	{
		...
		if(benoitAffection() <= 10) DisplayText("Once again, you carefully enter the gloom of Benoit's salvage shop.  The proprietor sniffs the air as you enter, and then looks at you sourly.  \"<i>Well?</i>\" he rasps.");
	...
	if(Flags.list[FlagEnum.BENOIT_WOMB_TALK_UNLOCKED] === 1 && !player.perks.has(PerkType.BasiliskWomb) && Flags.list[FlagEnum.BENOIT_TESTED_BASILISK_WOMB] === 0 && Flags.list[FlagEnum.BENOIT_STATUS] === 0) womb = tryToConvertToBassyWomb;
...
public benoitsBuyMenu():void {
	...
	{
		buyMod = 1.66;
		DisplayText("\"<i>Some may call zis junk,</i>\" says Benoite, indicating her latest wares.  \"<i>Me... I call it garbage.</i>\"");
	}
	else 
	{
		DisplayText("\"<i>Some may call zis junk,</i>\" says Benoit, indicating his latest wares.  \"<i>Me... I call it garbage.</i>\"");
	}
...
private benoitSellMenu():void {
	...
	if (Flags.list[FlagEnum.BENOIT_EXPLAINED_SHOP] === 0)
		...
		DisplayText("\"<i>Let us feel what you are trying to palm off upon me zis time, zen,</i>\" sighs Benoit" + benoitMF("", "e") + ", sitting down and opening " + benoitMF("his","her") + " hand to you.");
	...
	for (let slot: number = 0; slot < 5; slot++) {
		if (player.itemSlots[slot].quantity > 0 && int(player.itemSlots[slot].itype.value / sellMod) >= 1) {
			...
			MainScreen.addButton(slot, (player.itemSlots[slot].itype.shortName + " x" + player.itemSlots[slot].quantity), createCallBackFunction2(benoitSellTransact, slot, sellMod));
			totalItems += player.itemSlots[slot].quantity;
		}
	}
	if (totalItems > 1) MainScreen.addButton(7, "Sell All", createCallBackFunction2(benoitSellAllTransact, totalItems, sellMod));
	MainScreen.addButton(9, "Back", benoitIntro);
}
...
private benoitTransactBuy(slot: number = 1):void {
	...
	if(benoitLover()) DisplayText("After examining what you've picked out with " + benoitMF("his","her") + " fingers, " + benoitMF("Benoit","Benoite") + " hands it over and accepts your gems with a grin.");
	else DisplayText("After examining what you've picked out with " + benoitMF("his", "her") + " fingers, " + benoitMF("Benoit","Benoite") + " hands it over, names the price and accepts your gems with a curt nod.\n\n");
	//(+3 Affection)
	benoitAffection(3);
...
private talkToBenoit():void {
	...
	if(((Flags.list[FlagEnum.BENOIT_TIMES_SEXED_FEMPCS] > 2 && player.stats.int >= 60 && Flags.list[FlagEnum.BENOIT_WOMB_TALK_UNLOCKED] === 0) || Flags.list[FlagEnum.BENOIT_TIMES_SEXED_FEMPCS] === 2) && player.torso.vaginas.count > 0) {
		DisplayText("You ask " + benoitMF("Benoit","Benoite") + " if " + benoitMF("he","she") + " has ever thought about trying to do something to help his people's plight.");
	...
	else if (Flags.list[FlagEnum.FEMOIT_TALKED_TO] === 0 && Flags.list[FlagEnum.BENOIT_STATUS] != 0)
	{
		...
		DisplayText("You ask Benoite if she intends to go back to the mountains now.  She laughs long and hard at this.  One thing the transformation has certainly gifted her is an extraordinarily filthy laugh.");
	...
	//Subsequent Talk
		...
		if(benoitLover() && benoitMF("he","she") === "he" && player.torso.vaginas.count > 0) {
		...
		//(Randomly generated)
		if(choice === 0) {
			DisplayText("You ask if all basilisks talk as " + benoitMF("he","she") + " does.");
			...
			DisplayText("You ask " + benoitMF("Benoit","Benoite") + " about the dog.");
			...
			DisplayText("You ask " + benoitMF("him","her") + " about the sign above the shop.");
			...
			DisplayText("You ask if " + benoitMF("he","she") + " has always been blind.");
			...
			DisplayText("You ask " + benoitMF("Benoit","Benoite") + " how " + benoitMF("he","she") + " got into this line of work.");
			...
			DisplayText("You ask Benoit if " + benoitMF("he","she") + " can tell you anything about the Bazaar.");
			...
			DisplayText("You ask if " + benoitMF("Benoit","Benoite") + " really can tell who you are just by smell.");
			...
			//[(male Benoit only)
			if(benoitMF("he","she") === "he") DisplayText("  There's a hint of longing jealousy in the basilisk's voice and when he lapses into silence you decide not to push the subject.");
		}
		...
		//(Randomly generated.  Added to normal talk options after PC has had sex with Benoit two times or more regardless of womb quest.)
			...
			DisplayText("You ask Benoit there is anything useful he can tell you about the demon strongholds.");
			...
			DisplayText("You ask Benoit if he can suggest anything to help you fight his brethren in the high mountains.");
			...
			DisplayText("You ask Benoit if he really, <b>really</b> can tell who you are just by smell.");
		...
		{
			DisplayText("You ask Benoite how she’s getting on with being the opposite sex.  Benoite stops cleaning the tarnished silver plate in her hands to think.");
		...
		{
			DisplayText("You ask Benoite if she really can tell who you are just by smell.");
...
private benoitAndFemPCTalkAboutEggings():void {
	...
	DisplayText("Benoit seems very on edge today.  He paces up and down, returns your greeting with a stiff nod, and fiddles endlessly with the stock in reach of his counter as you search for a topic.  You ask if there's something wrong.");
...
private eggySuggest():void {
	...
	if(Flags.list[FlagEnum.BENOIT_TIMES_SEXED_FEMPCS] > 0) {
		if(player.torso.hips.legs.isTaur()) {
			DisplayText("You silently reach across the counter to squeeze his hands again.  Benoit grins with deep affection at you and, hand in hand, the two of you quietly debunk to the store room again.");
			...
			DisplayText("You silently reach across the counter to squeeze his hands again.  Benoit beams at you and, hand in hand and without a word, the two of you depart to the store room again.");
		
			DisplayText("\n\nOnce again, you carefully inch your blind charge to a clear cranny and push him against a wooden wall, standing back to slowly peel off your [armor].  You grin as you ostentatiously drop each piece onto the packed earth, allowing him to guess what it is by the sound it makes.  His breathing comes heavier as your undergarments make a feathery sound as they fall.  As you take his hands and lay them upon your naked skin, you think about how you want to go about this.");
		}
		MainScreen.simpleChoices(["Let Him", "Take Charge", "", "", ""], [repeatSexWithBenoitLetHim, repeatBenoitFuckTakeCharge, null, null, null]);
		return;
	}
	Flags.list[FlagEnum.BENOIT_TIMES_SEXED_FEMPCS]++;
	if(player.torso.hips.legs.isTaur()) {
		//Suggest: 
		DisplayText("You reach your fingers across the counter and lightly touch Benoit's hands, saying you don't mind working out a few natural urges, if he's in the mood.");
		...
		if(player.gender === Gender.HERM) DisplayText("  [EachCock] thickens and spurts in sympathy to your female high, spattering the floor with white paint.");
	}
	else {
		DisplayText("You reach your fingers across the counter and lightly touch Benoit's hands, saying you don't mind working out a few natural urges, if he's in the mood.");
	...
	//[Demon: 
		...
		if(player.lactationQ() > 100) {
			DisplayText("  You moan as your breasts reward his attention with spurts of milk; he starts in genuine amazement when they ");
			if(!player.torso.hips.legs.isTaur()) DisplayText("hit him in the face");
			else DisplayText("trickle over his fingers");
			DisplayText(".  \"<i>Mammals are so damn weird,</i>\" he mutters, making you giggle.");
		}
	...
	player.orgasm();
	if ((player.pregnancyType === PregnancyType.OVIELIXIR_EGGS || player.perks.has(PerkType.HarpyWomb) || player.perks.has(PerkType.Oviposition)) && (player.pregnancyIncubation === 0 || player.pregnancyType === PregnancyType.OVIELIXIR_EGGS)) {
		DisplayText("  I would not inflict my children upon you.  Ere, take as much as you like.</i>\"");
		MainScreen.simpleChoices(["Take It", "", "", "", "Leave"], [takeBenoitsContraceptives, null, null, null, dontTakeEggtraceptives]);
	}
	else {
		DisplayText("  I cannot give you babies unless you 'ave eggs.  I guess I should think a bit more before I go digging for things...</i>\"");
...
private takeBenoitsContraceptives():void {
	DisplayText().clear();
	DisplayText("You gladly accept the herbal contraceptive and push it into your mouth, enjoying the pleasantly sharp, citrus flavour.");
...
private dontTakeEggtraceptives():void {
	DisplayText().clear();
	DisplayText("You smile and say you don't mind carrying and laying a few basilisk eggs. \"<i>You... you don't?</i>\" he says hesitantly.  He faces you and for a moment looks like he's going to say something else; but then he shakes his head and puts the bag back into the drawer.");
...
private firstTimeAfterBoningEncounterBenoit():void {
	...
	DisplayText("A strange, faint noise is emanating from the salvage shop.  It's only when you duck cautiously into the stall proper that you realize it's the sound of a basilisk humming.  Benoit stops to sniff the air when you come in, immediately puts down the mug he is polishing, and beckons you inside.");
...
private repeatSexWithBenoitLetHim():void {
	...
	if(player.torso.hips.legs.isTaur()) {
		...
		if(player.torso.hips.legs.isBiped()) DisplayText("knees");
		else DisplayText("lower body");
		DisplayText(".  His hands descend upon your [butt]; grinning, you poke it outwards invitingly, sighing with affected frustration as the blind basilisk painstakingly lines up his next shot with your drooling, sticky sex.");
...
private repeatBenoitFuckTakeCharge():void {
	...
	if(player.torso.hips.legs.isTaur()) {
		...
		DisplayText("\n\n");
		//[Small capacity:]
		if(player.vaginalCapacity() < 30) DisplayText("You slide back down onto him, cooing this time as you feel him bottom out with several inches of him still outside of you.  His thin, long length could not be more perfect for your tight sex; he rubs your walls up and down as you dreamily thrust in and out of his body, sending waves of pleasure flowing through you.");
		//Large capacity: 
		...
		DisplayText("\n\nYour lust ratchets up as his warm hands continue to move over your behind, and at last losing your discipline you begin to thrust into him with abandon, eager now for your peak.  Benoit pants as you really begin to fuck him and thrusts with you; the wetness of your arousal spatters against his thighs as your body slaps into the basilisk's tight muscles.  He makes a harsh, bestial noise when he cums; his blind eyes roll as he clutches your back and shoots surge after surge of cum into your churning depths.  The warmth of his jizz and his helpless bucking thrust you to your own orgasm; irrepressible pulses of pleasure overwhelm your mind and you can do nothing for long minutes except cry and squeal against the basilisk, clutching him back as you work his dick for every last drop.");
		//[Herm: 
		if(player.torso.cocks.count > 0) DisplayText("  [EachCock] thickens and spurts in sympathy to your female high, spattering the floor with white paint.");
...
private tryToConvertToBassyWomb():void {
	...
	//Ingredients in inventory: 
		...
		DisplayText("You ferret out the ingredients you have collected and begin to bang them onto the counter in front of Benoit, telling him that you've got what he needs.  Pierre barks excitedly at the noise.");
		...
		//[No oviposition:
		if(!player.perks.has(PerkType.Oviposition)) DisplayText("  You feel slightly bloated, but otherwise fine; you sense that you can lay eggs of your own volition now.");
		else DisplayText("You feel slightly bloated, but otherwise fine; you sense that were you to get impregnated by basilisk seed, the eggs you produce would be pure basilisk.");
		...
		if(player.stats.lust >= 33) {
			DisplayText("  You grin and say you're not sure it worked, but you suppose there's only one real way of finding out...");
			if(player.tallness <= 78 && !player.torso.hips.legs.isTaur()) DisplayText("  The basilisk is still for a moment, and then with a sudden surge of movement, grabs you by the waist and hoists you over his shoulder.  You squeal in mock terror as he hauls you as fast as he can into the back room, knocking over half his stock as he does.");
			else DisplayText("  The basilisk is still for a moment, and then with a sudden surge of movement, grabs you by the waist and frenetically attempts to hoist you over his shoulder.  You are far too big for him though; after several valiant attempts, he collapses against a shelf.  Laughing, you pick the stricken, panting reptile up, hoist him over your own shoulder, and navigate a path into the back room.");
			return { next: createCallBackFunction(suggestSexAfterBasiWombed,false) };
			return;
		}
		else DisplayText("  You grin and say you're not even sure it worked... but you'll be back at some point to try it out, and he'd better be ready for when you do.  You gently pry yourself out of his grip and leave as deliberately as you can, aware of the beguiling, invisible scent you are leaving for the stunned, silent basilisk to simmer in.");
		return { next: Scenes.camp.returnToCampUseOneHour };
	}
}	
...
private suggestSexAfterBasiWombed(later:boolean = true):void {
	...
	if(later) {
		DisplayText("You smile coquettishly and ask if he's interested in taking your new body for a... test drive.  ");
		//(6'6 or less: 
		if(player.tallness <= 78 && !player.torso.hips.legs.isTaur()) DisplayText("The basilisk is still for a moment, and then with a sudden surge of movement, grabs you by the waist and hoists you over his shoulder.  You squeal in mock terror as he hauls you as fast as he can into the back room, knocking over half his stock as he does.");
		else DisplayText("The basilisk is still for a moment, and then with a sudden surge of movement, grabs you by the waist and frenetically attempts to hoist you over his shoulder.  You are far too big for him though; after several valiant attempts, he collapses against a shelf.  Laughing, you pick the stricken, panting reptile up, hoist him over your own shoulder, and navigate a path into the back room.");
		DisplayText("\n\n");
	...
	if(!player.perks.has(PerkType.Oviposition)) {
...
public popOutBenoitEggs():void {
	...
	if(Flags.list[FlagEnum.BENOIT_EGGS] > 0) DisplayText(" once more");
	DisplayText(".  Well, he will be when this ");
	if(Flags.list[FlagEnum.BENOIT_EGGS] > 0) DisplayText("latest ");
	DisplayText("clutch hatches, anyway.");
...
public femoitInitialTalk():void
{
	...
	DisplayText("You ask Benoit if he has ever thought about trying to do something to help his people's plight.");
...
public benoitFeminise():void
{
	...
	{
		DisplayText("You don't have the necessary ingredients to attempt this yet.");
...
public femoitNextDayEvent():void
{
	...
	DisplayText("A strange, faint sound is emanating from the salvage shop.  It's only when you duck cautiously into the stall proper that you realize it's the sound of a basilisk humming.  Benoit stops to sniff the air when you come in, immediately puts down the mug she is polishing, and beckons you inside.");
...
public femoitFirstTimeYes():void
{
	...
	DisplayText().clear();
	DisplayText("Smiling, you reach across the counter and squeeze Benoite's hands until her nervous babble dies out and she smiles back.  Still holding her hand, you move behind the crates and then gently lead her behind the stall's canopy.");
	...
	if (player.isLactating()) DisplayText(" The pressure and arousal makes your [nipples] dribble milk, spattering fluids across both of your chests.");
	DisplayText("  Soon the two of you are thrashing into each other, Benoite moaning huskily as you batter her against the wagon wall, both seeking your peak together, each pushing the other a bit further upwards.  Your world is lost to everything but the warm tightness around your [cock] and the pressure against your chest.");
...
public femoitSexIntro():void
{
	...
	if (!benoitPreggers())
	{
		...
		if (benoitInClutch()) DisplayText("  Her stomach bulges with clutch and the air is thick with female musk; the sight and smell of overwhelming ripeness speaks to your body in a way which bypasses your higher cognition entirely, sending you growling on top her, barely even hearing her giggle.");
		...
		DisplayText("\n\n");
 		if (!player.torso.hips.legs.isTaur())
		{
			DisplayText("Eventually, sweat dripping off you, you grab her thighs and heave them upwards so that you can really go to town, drawing yourself almost all the way out of her before smacking back into her,");
			if (!benoitInClutch()) DisplayText(" your stomach beating out a slapping rhythm against her own flat abdomen.  ");
			else DisplayText(" your stomach beating out a slapping rhythm against her bulging, gravid abdomen.  ");
		}
 		DisplayText("Benoite moans, squeals and eventually screams to your exertions, her fluids spurting and spattering against your groin");
 		if (player.torso.balls.quantity > 0) DisplayText(" and [balls].  You tumble over your peak as her cunt suddenly tightens around yours, sending surge after surge of cum into her fertile depths, your body seized in a rictus of pleasure.");
 		if (player.cumQ() >= 2500) DisplayText("  The quantity of it is such that it quickly dribbles back out around your cock and pools on the floor.");
	...
	else if (benoitRegularPreggers() && (!player.torso.hips.legs.isTaur() || (player.torso.hips.legs.isTaur() && (player.tallness * (5/6) < player.torso.cocks.list[player.longestCock()].cockLength))))
	{
		...
		if (player.cumQ() >= 2500) DisplayText("  Because her womb is already so jam-packed with eggs, the bulk of your deposit simply oozes messily back out of her, leaving her well and truly creampied.");
	...
	else if (benoitVeryHeavyPreggers() || benoitExtremePreggers())
	{
		...
		if (player.torso.hips.legs.isTaur())
		{
			DisplayText("\"<i>No.  Non!  Absolutely non!</i>\" Benoite insists.  \"<i>You are my amazing stallion, [name], but... it is simply too much.  Wis all zis weight on me, zere is simply no way we can do it.</i>\"");
		...
		else
		{
			...
			{
				DisplayText("Benoite smirks at you.  \"<i>Well, I guess ze last time was enjoyable enough.  Come, then; I am horny and you arrived in time to scratch my itch.</i>\"");
			}
			...
			if (player.torso.balls.quantity === 0) DisplayText(" with a smile, knowing damn well you don't, literally");
			DisplayText(".");
...
// (First time: 
	...
	else
	{
		...
		if (benoitVeryHeavyPreggers() || benoitExtremePreggers()) DisplayText("... which is good, because you can tell there're a lot of them coming");
		DisplayText(".  That done, you squat down behind Benoite, keeping your head down to avoid having it slapped by her swishing tail, and reach up between her legs. She promptly lets out a shriek.");

		if (Flags.list[FlagEnum.FEMOIT_HELPED_LAY] === 1)
		{
			DisplayText("\"<i>What's the matter?</i>\" you blurt, alarmed.");

			DisplayText("\"<i>You... you 'ave cold 'ands!</i>\" she squeals, shuddering in displeasure.");
		}
		else
		{
			DisplayText("\"<i>What have I told you about warming zose hands?</i>\" she snaps angrily.");
		}
	}
	Flags.list[FlagEnum.FEMOIT_HELPED_LAY]++;
		...
		if (Flags.list[FlagEnum.FEMOIT_EGGS] > 1) DisplayText("s");
		DisplayText(" she just laid.  You nod from behind her, and cautiously move around to give her a hug.  She is a very, very brave woman, and an even braver man for making the decision to do this in the first place.");
		...
		if (benoitRegularPreggers()) DisplayText(" She sighs softly.  \"<i>A small clutch, this is... normal for my people, yes, but not very good for my mission, is it?  Still, a small step to freedom is still a step.</i>\"  She manages to smile.  \"<i>My children will be free, and that is something to celebrate.</i>\"");
		else if (benoitHeavyPreggers()) DisplayText(" \"<i>My, a nice big clutch we had together, didn't we?</i>\" she smiles, proudly.  \"<i>Yes, these will be strong children, I zink.</i>\"");
		else if (benoitVeryHeavyPreggers()) DisplayText(" \"<i>...Wow, you're quite ze stud, aren't you?</i>\" Benoite says, giving a throaty growl of lust.  \"<i>I chose well when I decided to let you fertilize my eggs... so many eggs, too.</i>\"");
		...
		if (Flags.list[FlagEnum.FEMOIT_EGGS] > 1) DisplayText("s");
		DisplayText(" away safely, though, and the blind reptilian clearly appreciates the help.  Leaving her to admire her new clutch you head back to camp.");
