class Loppe {
    ...
    */
    ...
    private yesToLoppeMeetingTwo() {
        ...
        //[(corr < 40)]
        if (player.stats.cor < 40) DisplayText(" as politely as possible. It felt nice to see a display like that here; makes it feel a little less like a wasteland of sex-mad monsters and a little more like home");
        DisplayText(".");
    ...
    private chatWithLoppeAboutHerMom() {
        ...
        //(If PC is male:)
        if (player.gender === Gender.MALE) DisplayText("  Well, from what Loppe's told you, Uma wouldn't be attracted to you anyway.");
        ...
        if (Flags.list[FlagEnum.LOPPE_PC_MET_UMA] === 0) {
            ...
            MainScreen.addButton(1, "Mebbe Later", Scenes.camp.returnToCampUseOneHour);
        }
        else {
    ...
    private loppeSexChoice(bakery: boolean = false) {
        ...
        if (Flags.list[FlagEnum.LOPPE_TIMES_SEXED] === 0) {
            ...
            if (!bakery) {
                ...
                //(else)
                    ...
                    //[(lib>40)]
                    if (player.stats.lib >= 40) DisplayText(".. it almost sounds exciting.");
                }
            ...
            DisplayText("It's not very difficult to reach Loppe's place; it happens to be located only a few blocks away from the gym, behind a small shop with the sign \"<i>Kemono's Oriental Clinic</i>\".");
            ...
            //[(corr < 40)]
            if (player.stats.cor < 40) DisplayText("  You tell her they're quite nice... though privately you're curious what the interior looks like.");
            else DisplayText("  They don't seem to work very well - after all, <i>you're</i> here.");
            ...
            if (player.torso.cocks.count > 0) {
                if (player.smallestCockArea() <= loppeCapacity()) DisplayText("Pulling her onto your dick would result in some cowgirl fun, though you'd have to deal with her cock pointed right at you when she came.  ");
                else DisplayText("As good as it would feel to shove your [cock smallest] into her cunt, the nervous looks she's giving it tell you what her response would probably be.  ");
            }
            //[(any cock)
            if (player.torso.cocks.count > 0) DisplayText("You could just whip it out and ask what she thinks of it, compared to hers - it might lead to some cross words and crossed swords, though.  ");
            if (player.torso.vaginas.count > 0) DisplayText("You could take her monster length vaginally, but if her libido is what she claims, you'll probably wind up quite stretched.  ");
            //[(boobs >= boobjob req)
            if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 5) DisplayText("Her hardening horsecock looks like it would fit between your [chest], an act likely to net you a messy demonstration of her enthusiasm.  ");
            DisplayText("There's always the option to receive her anally, though with her vaunted libido, you'd probably end up so flooded with her cum that it would wash from your mouth.  Or you could be a prick-tease and leave.");
            //Display sex options
            //[Cowgirl][Frot][TakeVaginal][Boobjob][TakeAnal][Bail]
        }
        //Repeat Intro (edited)
        else {
            DisplayText("Loppe's other 'workout' sounds pretty good right now.");
            ...
            DisplayText("There's always the option to receive her anally, though with her vaunted libido, you'd probably end up so flooded with her cum that it would wash from your mouth.  Or you could be a prick-tease and leave.");
            //[(loppesexed)
            DisplayText("  Just jerking that cock and squeezing it so hard her balls swell up when she comes, though, that might be fun.  It'd be interesting to see how big they can get.");
        ...
        if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 4) MainScreen.addButton(3, "Boob-job", boobjobLoppe);
        if (Flags.list[FlagEnum.LOPPE_TIMES_SEXED] > 0) MainScreen.addButton(4, "SqueezeJob", loppeSqueezedickWhateverThatIs);
        if (player.torso.hips.legs.isTaur() && player.stats.lust >= 33) MainScreen.addButton(5, "TakeAnal", getAssFuckedByLoppeAsACentaur);
    ...
    private loppeRidesCocks() {
        ...
        if (y >= 0 && player.torso.cocks.count === 2) DisplayText("  \"<i>In fact... I could just eat both of those up.</i>\"");
        //(3+ fit cocks)
        else if (y >= 0 && player.torso.cocks.count === 3) DisplayText("  \"<i>But I'm afraid I'll only be able to handle two of them.</i>\"");
        ...
        if (y >= 0) DisplayText("two of ");
        DisplayText("your cock");
        if (y >= 0) DisplayText("s");
        DisplayText(", rubbing gently.  \"<i>Just sit back and leave everything in my capable hands, sugar.</i>\"");
        ...
        DisplayText(", yet squeezing you as tightly as a velvety vise.  In fact, she's gripping you so hard that you can't seem to push yourself any further into her depths; she's squeezing you much too tightly to move in or out, an outright amazing feat, if Loppe gets half as much action as she claims to.");
        ...
        if (y >= 0) DisplayText("s");
        DisplayText(" with happy squeals of pleasure and happiness, softly accompanied by the wet sounds of your coupling.  You gasp and cry, savoring every motion as she squeezes and wriggles, her inner walls rippling so deliciously around your cock");
        if (y >= 0) DisplayText("s");
        DisplayText(", stroking them in the most delicious way.  You thrust your hips, trying to slip deeper inside her, but still she's holding you at bay.");
        ...
        if (y >= 0) DisplayText(" and watertight ass");
        DisplayText(".  The sensation borders on the thin line between pleasure and pain, and you can't help but moan into Loppe's kiss; you wrap your ");
        if (player.torso.hips.legs.isDrider()) DisplayText("spindly legs");
        else if (player.torso.hips.legs.isTaur()) DisplayText("forelegs");
        else DisplayText("arms");
        DisplayText(" around the bunny-girl's ");
        if (Flags.list[FlagEnum.LOPPE_FURRY] === 0) DisplayText("silky-smooth");
        else DisplayText("softly furred");
        DisplayText(" back and hold her tight against you, her long horse-cock sandwiched between your bellies, smearing both of you with drooling precum, rivers of the stuff flowing out steadily with every thrust and grind the two of you make.");
    ...
    private loppeRidesYouNoFaceJizz() {
        ...
        DisplayText("You let go of the horse-dicked rabbit and she gratefully swivels herself fully upright, cock jutting out over your belly - instinctively, your ");
        if (player.torso.hips.legs.isTaur()) DisplayText("foreleg tucks under her cock, angling it away from your body.");
        else DisplayText("hand lunges forward and places itself underneath, fingers curling around as you push it further up, aiming it over your shoulder.  You can feel it throbbing like mad against your fingers, can feel the impressive bulge of semen as it distends her urethra and surges up towards her flared tip.");
        ...
        if (y >= 0) DisplayText(" while her ass suckles on your second cock with just as much eagerness");
        DisplayText(".");
    ...
    private loppeRidesYouHoseHer() {
        ...
        if (player.torso.cocks.count > 1) DisplayText("s");
        DisplayText(" until you can't help it and cum yourself.");
        loppeRidesPCCockFinal();
    }
    ...
    private loppeRidesPCCockFinal() {
        ...
        if (y >= 0 || player.torso.vaginas.count > 0) {
            ...
            if (player.torso.vaginas.count > 0) {
                ...
                if (player.torso.vaginas.get(0).wetness < 3) DisplayText("drizzles");
                else if (player.torso.vaginas.get(0).wetness < 5) DisplayText("spatters");
                else DisplayText("gushes");
                DisplayText(" femcum onto the bed below you, forgotten during the act but not unsympathetic to her ministrations");
            }
        }
        DisplayText(".  You cry out as you cum long and hard, arching your back off of the bed in your pleasure before finishing and collapsing, gasping for breath as, above you, the laquine comes to her own climactic end.");
        ...
        if (player.torso.hips.legs.isDrider() || player.torso.hips.legs.isTaur()) DisplayText("forelegs");
        else DisplayText("arms");
        ...
        DisplayText(" pull");
        if (y < 0) DisplayText("s");
        DisplayText(" an involuntary shudder from you; the erotic stimulation sends blood pulsing back into your veins, and your shaft");
        if (y >= 0) DisplayText("s rise");
        else DisplayText(" rises");
        DisplayText(" to full mast again, so swiftly ");
        if (y < 0) DisplayText("it almost smacks");
        else DisplayText("they almost smack");
        ...
        if (y >= 0) DisplayText("s");
        DisplayText(" once more.  You moan as once again you follow the lusty dickgirl into the throes of pleasure...");
        ...
        //[(Exhibitionist)
        if (Flags.list[FlagEnum.PC_FETISH] > 0) DisplayText("; it gives you something of a thrill, though nowhere near as much as taking them off for her in the first place did");
        DisplayText(".  Finished, you depart.");
        player.orgasm();
        player.stats.tou += .5;
        player.stats.lib += .5;
        player.stats.sens += -4;
    ...
    private loppeWorshipsDicks() {
        ...
        if (Flags.list[FlagEnum.LOPPE_FURRY] > 0) DisplayText("fluffy ");
        DisplayText("cleavage; her ");
        if (Flags.list[FlagEnum.LOPPE_FURRY] === 0) DisplayText("skin");
        else DisplayText("fur");
        ...
        //[(lib > 40)
        if (player.stats.lib >= 40) DisplayText(" - nor would you want to; she looks just right covered in your fluids");
        DisplayText(".");
        ...
        if (player.torso.balls.quantity > 0) DisplayText(" and balls to balls");
        DisplayText(", the delicious friction pushing you over the edge.");
    ...
    private getFuckedInYerTwatYaCunt() {
        ...
        else DisplayText("exposing your rump and spreading your hind legs in your best effort to make her feel welcome.");

        DisplayText("\n\nLoppe smirks at you and winks.  \"<i>Don't worry; I'll treat you like a ");
        //[(femininity <= 40)
        if (player.femininity > 50) DisplayText("princess.  A sexy, smoking hot princess");
        else DisplayText("prince.  A dashing, dandy prince");
        ...
        if (player.torso.vaginas.get(0).virgin) {
            ...
            //[(low corr)]
            if (player.stats.cor < 50) DisplayText("  You look coy, for her benefit, and cast your eyes down shyly.");
            else DisplayText("  You do your best to warn her away from the topic with your eyes, eager to get on with the fuck.");
            //(not for horsez)
            ...
            DisplayText("\n\nYou reach out ");
            if (player.stats.cor < 50) DisplayText("and tap her playfully on the nose");
            else DisplayText("in your irritation and squeeze her face imploringly");
            DisplayText(".  Would you be here, with your clothes off in Loppe's bed, if you weren't okay with it?");
        ...
        if (player.vaginalCapacity() >= 35) DisplayText(" bottoms out, completely hilted inside you");
        //[else:
        else DisplayText(" bumps against your cervix");
        DisplayText(". You moan in delight as she sinks into you");
        if (player.torso.vaginas.get(0).virgin) DisplayText(", repressing a shiver of pain as she tears through your hymen, taking your virginity");
        ...
        //[(not horse)
        if (!player.torso.hips.legs.isTaur()) DisplayText("  You wrap your [legs] around her hips as best you can, thrusting in your efforts to meet her strong rhythm, feeling your cunt rippling around her wonderfully long, thick horsecock.");
        ...
        if (!player.torso.hips.legs.isTaur()) {
            ...
            if (player.gender >= 3) DisplayText(" and smearing both your bellies with herm-pre");
            DisplayText(".");
        }
        //Centaur:
        else {
        ...
        if (player.torso.cocks.count > 0) {
            DisplayText(", neglected cock fountaining cum ");
            if (player.cumQ() < 500) DisplayText("between you and painting you both");
            else DisplayText("gushing onto the bedsheets and smearing your belly");
            DisplayText(" with this aspect of your dual-sexed orgasm");
        }
        DisplayText(".");
        ...
        if (!player.torso.hips.legs.isTaur() && player.torso.cocks.count > 0) DisplayText(", giving you a quick peck on the cheek and licking a small gob of cum that seems to have stuck to your chin");
        DisplayText(".");
        ...
        //[(low corr)
        if (player.stats.cor < 50) DisplayText("a pleading look... and if there's maybe a trace of horror dancing in the depths of your eyes, well, that's understandable, isn't it?");
        else DisplayText("an incendiary glare, letting her know just what you think of her posturing.");
        DisplayText("  Loppe bursts out laughing.  \"<i>Just kidding, sugar.  If we go another round I think my dick might fall off.</i>\"");
    ...
    private getButtFuckedNonHoarseByLoppe() {
        ...
        //(PC has Anal Wetness)
        if (player.torso.butt.wetness > 0) {
        ...
        if (player.torso.cocks.count > 0) {
            DisplayText(", cock");
            if (player.torso.cocks.count > 1) DisplayText("s");
            DisplayText(" dripping pre");
            if (player.torso.vaginas.count > 0) DisplayText(" and");
        }
        else if (player.torso.vaginas.count > 0) DisplayText(", ");
        if (player.torso.vaginas.count > 0) DisplayText(" pussy oozing femlube");
        DisplayText("... you beg her to go faster, to do it harder!");

        DisplayText("\n\nLoppe happily complies, thrusting herself deeper and stronger.  ");
        //[(cock)
        if (player.torso.cocks.count > 0) DisplayText("One of her hands reaches around to grasp your pre-slickned [cock biggest], stroking you in time with her thrusts.  ");
        if (player.gender === Gender.FEMALE) DisplayText("One of her hands reaches around to tease your dripping [vagina] and erect [clit], dipping a finger in every time she bottoms out.  ");
        DisplayText("Wet slapping noises fill the room, and you gasp in pleasure each time her cum-churning balls slap against ");
        if (player.torso.balls.quantity > 0) DisplayText("yours");
        else DisplayText("your [butt]");
        DisplayText(".  Loppe lays down on top of you, panting hotly on your neck, moaning in pleasure each time your sphincter contracts.");
        ...
        if (player.torso.vaginas.count > 0) {
            DisplayText("  A gush of sexual fluids from your crotch ");
            if (player.torso.vaginas.get(0).wetness < 3) DisplayText("spatters");
            else if (player.torso.vaginas.get(0).wetness < 4) DisplayText("splashes");
            else if (player.torso.vaginas.get(0).wetness < 5) DisplayText("drenches");
            else DisplayText("completely soaks");
            DisplayText(" the bed-linen under you.");
        }
        ...
        if (Flags.list[FlagEnum.SATYR_KIDS] > 0) DisplayText("You laugh softly; hopefully she'll never have to meet the creature that dispelled that same doubt in you.  ");
        DisplayText("\"<i>I think this is time for sleeping, not joking.</i>\"  With a trace of wounded dignity, she lays herself down and curls into your back, one arm wrapped around your gurgling belly.");
        ...
        //(First Time)
        if (Flags.list[FlagEnum.LOPPE_TIMES_SEXED] === 0) {
            DisplayText("Loppe laughs nervously.  \"<i>Sorry about that.  It's just that I usually can't bag a real cutie like you, and I think I might have gone a bit overboard... is there anyway I can make it up to you?</i>\"");
    ...
    private getAssFuckedByLoppeAsACentaur() {
        ...
        //[ass wet)
        if (player.torso.butt.wetness > 0) DisplayText("  She doesn't even have to bother with lube; your ass is wet enough to accommodate her on its own.");
        ...
        if (player.torso.balls.quantity > 0) DisplayText("rs");
        else if (player.torso.vaginas.count > 0) DisplayText("r pussy");
        DisplayText(".");

        DisplayText("\n\nYou moan as she gets into her task");
        if (player.torso.cocks.count > 0) DisplayText(", feeling your erect [cock] press against your underbelly, mashed between your centaur half and the soft carpet of Loppe's room.  You hope she doesn't mind a few stains...  ");
        //(else vag)
        else if (player.torso.vaginas.count > 0) DisplayText("; your cunt is sopping, making a small pool of juices one the floor.  ");
        else DisplayText(".  ");
        DisplayText("She seems more worked up than usual, perhaps because you have a horse-like lower body?  \"<i>Are your instincts acting up?</i>\" you tease her.  \"<i>Or do you just love ass that much?</i>\"");
        ...
        if (player.gender <= 1) DisplayText("  Even if you are the wrong sex entirely for that.");
        ...
        //[PC has cock:
        if (player.torso.cocks.count > 0) DisplayText("  The bumps against your prostate send waves of pleasure running through [eachCock] and spreading along your body, until they reach your throat and come out as a ragged groan.  ");
        DisplayText("If she keeps this up for too much longer, you will pop any moment now...");
        ...
        if (player.torso.cocks.count > 0) DisplayText("  [EachCock] spasms and unloads beneath you, smearing you and the carpet below and pooling thickly with fluid forced out by the prostate stimulation.");
        if (player.torso.vaginas.count > 0) DisplayText("  Your vagina contracts, trying to grip at a phantom member as you spill your fluids on the carpet beneath you.");
        ...
        if (player.skin.type != SkinType.SCALES) DisplayText(" have goosebumps all over your body");
        else DisplayText("r scales are practically standing on end");
        DisplayText(".");

        DisplayText("\n\n\"<i>Ohhh... you are incredible, [name].  ");
        //[PC has dick that fits)
        if (player.cockThatFits(loppeCapacity()) >= 0) DisplayText("Though I hope you won't mind riding me, sometime,</i>\" she laughs.  \"<i>");
        DisplayText("How about another round, hmm?  Big strong 'taur like you should certainly be able to hold a little more loving inside of you, right?</i>\"  She croons happily, running her hands up and down your ass as if trying to entice you.");
        ...
        DisplayText("  Anyway, your clothes are back in my room; sorry to fuck and run, but I've got some errands to run before mom gets home - you'll have to see yourself out, okay?</i>\"");
    ...
    private loppeSqueezedickWhateverThatIs() {
        ...
        if (!player.torso.hips.legs.isTaur()) DisplayText("on");
        else DisplayText("beside");
    ...
    private superLoppeOrgasmDenialGo() {
        ...
        DisplayText("\n\n\"<i>Bye, sugar!");
        if (player.torso.cocks.count > 0) DisplayText("  Maybe next time I'll be the one to give you a bit of a holdout!");
        DisplayText("</i>\"");
    ...
    private boobjobLoppe() {
        ...
        if ((player.stats.str < 50 && !player.torso.hips.legs.isTaur()) || (player.stats.str < 30 && player.torso.hips.legs.isTaur())) {
            DisplayText("  Struggle as you may, you're powerless to stop the drooling laquine as she sends you crashing down on her bed.  The whole piece of furniture shakes ominously with the force of your impact, and for a moment you think it will break apart.");
            ...
            DisplayText("  You hold the laquine's arms and force her onto the floor; she lands with an audible \"<i>oomph</i>\" and you grin as you hold the eager herm down.");
            ...
            //end scene
            DisplayText("\n\n");
            if (player.stats.cor >= 60) DisplayText("You don't really feel like it, but you figure you had better humor the greedy herm if you want to keep fucking her whenever you like.  ");
            DisplayText("Oh, well... you reach down and heft the surprised hybrid off of the floor and onto her spattered bed, then slide yourself into it as best you can, wrapping your arms around her.  She hugs you back, then yawning widely.  \"<i>Naptime...</i>\"");
        ...
        if ((player.stats.str < 50 && !player.torso.hips.legs.isTaur()) || (player.stats.str < 30 && player.torso.hips.legs.isTaur())) DisplayText("- while your stomach is still heavily swollen and gurgles in protest, it's not bad enough to impair you - ");
        DisplayText("before going over and dressing yourself.  Loppe waves goodbye to you as you head out once again.");
