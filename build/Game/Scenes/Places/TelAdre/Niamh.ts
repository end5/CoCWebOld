class Niamh implements TimeAwareInterface {
    ...
    public timeChange(): boolean {
        ...
        if (player.statusAffects.has(StatusAffectType.BimboChampagne)) {
            player.addStatusValue(StatusAffects.BimboChampagne, 1, -1);
            if (player.statusAffects.get(StatusAffectType.BimboChampagne).value1 <= 0) {
    ...
    public telAdreNiamh() {
        if (Flags.list[FlagEnum.MET_NIAMH] === 0) {
            ...
            if (model.time.hours <= 8) DisplayText("massive N-cup breasts, spilling over the table");
            else if (model.time.hours <= 9) DisplayText("huge M-cup breasts, taking up much of the table");
            else if (model.time.hours <= 10) DisplayText("great L-cup breasts, advancing far onto the table");
            else if (model.time.hours <= 11) DisplayText("heavy K-cup breasts, lying on the table for support");
            else if (model.time.hours <= 12) DisplayText("hefty J-cup breasts, resting comfortably on the table");
            else if (model.time.hours <= 13) DisplayText("weighty I-cup breasts, dangling to lie on the table");
            else if (model.time.hours <= 14) DisplayText("squeezable H-cup breasts, settling burdensomely on her chest");
            else DisplayText("moderately manageable G-cups, trembling with every breath she takes");
            DisplayText(".  Smooth, touchable skin covers her chest, face, hands and presumably the rest of her; it seems her animalistic accoutrements stop at kitty ears and tail. She takes a deep swig of her drink, following it up with a cute little burp.");
        }
    ...
    public approachNiamh() {
        ...
        //[Talk] [Get Beer] [Leave]
        MainScreen.simpleChoices(["Talk", "Get Beer", "", "", "Leave"], [talkToNiamh, beer, null, null, leaveNiamh]);
    }

    //[Get Beer]
    public getANiamhBeer() {
        DisplayText().clear();
        DisplayImage(images.showImage("niamh-get-beer"));
        ...
        //Feeder
        if (player.perks.has(PerkType.Feeder)) DisplayText("  You understand exactly what she's going through; the weight of a huge pair of breasts, the sensitivity as they engorge with fluids, the near-maddening sensation of the contents sloshing around underneath your straining skin...");
        //(Player has G-cups or bigger;
        else if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 15) {
            if (player.lactationQ() === 0)
                DisplayText("  Not terribly surprising; you know your tits are sensitive, and you don't even make milk in them.");
            else
                DisplayText("  You can easily imagine just how much trouble she must have, especially given the bulk of her tits are fluid.");
        }
        else if (player.lactationQ() > 0)
            DisplayText("  You know just how distracting it is to have your tits full of fluid waiting to get out.");
        ...
        if (Flags.list[FlagEnum.MET_NIAMH] > 0)
            ...
            DisplayText("the cat-girl ");
        DisplayText("sighs in relief, biting her lower lip slightly.");
        //General Ending
        ...
        //[Here][To Go]
        let togo;
        if (player.inventory.gems >= 2) togo = blackCatBeerToGo;
        else DisplayText("\n\n<b>You're too poor to buy the mug.</b>");
        Flags.list[FlagEnum.GOT_NIAMH_BEER]++;
        MainScreen.simpleChoices(["Here", "To Go", "", "", ""], [drinkNiamhsBeerInTelAdre, togo, null, null, null]);
    }

    //[Here]
    private drinkNiamhsBeerInTelAdre() {
        DisplayText().clear();
        if (player.lactationQ() >= 300 && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 5 && randInt(2) === 0 && Flags.list[FlagEnum.MET_NIAMH] > 0) {
            ...
            if (isHolidays() && date.date >= 31) {
                ...
                if (player.torso.vaginas.count > 0) DisplayText("pussy to wetten");
                if (player.gender === Gender.HERM) DisplayText(" and ");
                if (player.torso.cocks.count > 0) DisplayText("[eachCock] to stiffen");
                DisplayText(".  \"<i>Ye know, I got's an idea.  How's about ye stick by me for the next hour.  I'm sure ye must be diein' to drain those big'uns of yours.  We might as well sell your champagne ta the patrons.  Whaddaya say Lassie?</i>\"");
            ...
            MainScreen.simpleChoices(["SellYourBooze", "", "", "", "Leave"], [sellYourBooze, null, null, null, leaveWithBeerTits]);
            return;
        }
        DisplayText("You yell \"<i>Skoal!</i>\" and upend the mug into your mouth, drinking greedily.  The warmth of the booze quickly follows the liquid itself - potent stuff, this is.");
        //PC is affected by Black Cat Beer item effects
        blackCatBeerEffects(player, false, true);
        //both output
        if (model.time.hours <= 15) DisplayText("\n\n\"<i>Thanks for the business, " + player.mfn("laddie", "lassie", "customer") + "!  Remember, Niamh sells her Black Cat Beer every day from 8 until 4.</i>\"  You nod your head in thanks and step away from the table.");
        //16:00 ending
        else {
            DisplayText("\n\nThanks to your efforts, ");
            if (Flags.list[FlagEnum.MET_NIAMH] === 0) DisplayText("the cat girl's");
            else DisplayText("Niamh's");
    ...
    //[To Go]
    private blackCatBeerToGo() {
        DisplayText().clear();
        DisplayText("\"<i>That'll be two more gems, then,</i>\" the girl says.  \"<i>Though, if your hands get any more skillful I might have to pay you to take it,</i>\" she flirts.  You pass over two more gems, and she gratefully bags them and beams you a smile.");
        player.inventory.gems -= 2;
        statScreenRefresh();
        //both output
        if (model.time.hours <= 15) DisplayText("\n\n\"<i>Thanks for the business, " + player.mfn("laddie", "lassie", "customer") + "!  Remember, Niamh sells her Black Cat Beer every day from 8 until 4.</i>\"  You nod your head in thanks and step away from the table.");
    ...
    private talkToNiamh() {
        ...
        if (Flags.list[FlagEnum.TALKED_NIAMH] === 0) {
            DisplayText("You ask the cat-girl to tell you about her past.");
            ...
            if (model.time.hours < 15) DisplayText("who usually sits over yonder ");
        ...
        //(repeated)
            ...
            if (player.torso.hips.legs.isTaur()) DisplayText("You clop up and stand awkwardly");
            else DisplayText("You pull up a chair and sit");
        ...
        if (player.inventory.gems >= 2)
            beer = getANiamhBeer;
        simpleChoices("Beer", beer, "", null, "", null, "", null, "Leave", Scenes.camp.returnToCampUseOneHour);
    }
    //Leave
    private leaveNiamh() {
        DisplayText().clear();
        DisplayText("You decide that you don't really want to talk to this strange cat-girl and, as politely as possible, excuse yourself.");
    ...
    public blackCatBeerEffects(player: Player, clearScreen: boolean = true, newLine: boolean = false) {
        if (clearScreen) DisplayText().clear();
        if (newLine) DisplayText("\n\n");
        DisplayText("Uncapping the mug, you swill the stuff down in a single swig, gasping as it burns a fiery trail into your belly.  It's rich and sweet, and damn, but it's strong stuff!");
        ...
        HPChange(40 + randInt(21), false);
        let lib: number = 0;
        if (player.statusAffects.has(StatusAffectType.BlackCatBeer)) {
            ...
            dynStats("spe", -1, "int", -1, "lib", lib, "lus", 30 + randInt(player.stats.lib / 4));
        }
        else {
            if (100 - player.stats.lib >= 10) lib = 10;
            else lib = 100 - player.stats.lib;
            player.statusAffects.set(new StatusAffect("BlackCatBeer", 8, lib, 0, 0)));
            dynStats("spe", -5, "int", -5, "lib", lib, "lus", 20 + randInt(player.stats.lib / 4));
        }
        player.slimeFeed();
    }

    //Black Cat Beer Wears Off: This message is displayed eight hours after the last drink.
    public blackCatBeerExpires() {
        dynStats("spe", 4.5, "int", 4.5, "lib", (-1 * player.statusAffects.get(StatusAffectType.BlackCatBeer).value2));
    ...
    private giveNiamphBimboLiquer() {
        taintNiamh(true);
    }

    private giveNiamphSuccubiMilk() {
        taintNiamh(false);
    }
    ...
    private corruptOrBimboNiamhIntro() {
        DisplayText().clear();
        DisplayText("\"<i>Heya, favored customer,</i>\" the busty kitty greets as you approach.  \"<i>I have a favor to ask!  My drink's empty, and I get bored of me own brew,</i>\" Niamh explains.  \"<i>Would ya mind fetchin' me somethin', or are ye gonna force me to sample me own goods once more?</i>\"");
        ...
        if (player.inventory.items.has(consumables.BIMBOLQ))
            DisplayText("'bubbly' for her, in the form of the Bimbo Liqueur you got from the imp lord, or ");
        DisplayText("maybe even a Succubus Milk?  You'll have to consider your options.  On the other hand, of course, something that corruptive would probably mutate her booze-filled boobs, most likely forcing her out of Tel'Adre altogether... and may fall back on your own head, to boot.  What do you do?");
    ...
    public maybeLaterNiamh() {
        ...
        let beer;
        if (player.inventory.gems >= 2) beer = getANiamhBeer;
        else DisplayText("\n\n<b>You're too poor for beer.</b>");
        //[Talk] [Get Beer] [Leave]
        MainScreen.simpleChoices(["Talk", "Get Beer", "", "", "Leave"], [talkToNiamh, beer, null, null, leaveNiamh]);
    }
    ...
    public taintNiamh(bimbo: boolean = false) {
        DisplayText().clear();
        DisplayText("You grab Niamh's glass and one more and head off, making sure to turn a corner before commandeering an empty table.  Plunking the mugs down in front of you, you mutter to yourself as she moves away.");
        //[if sucmilk]
        if (!bimbo) {
        ...
        //[if bimbo liqueur]
            ...
            DisplayText("\n\nHer expression brightens when she notices your approach, and she giggles when she regards the quarter-full mugs of liqueur.  \"<i>What, that's it?</i>\" she quips curiously.  \"<i>This stuff had better pack some punch, " + Desc.Gender.mf(player, "lad", "lass") + ".</i>\"  You assure her that it will knock her socks off.");
            DisplayText("\n\nNiamh reaches for a mug, but you draw it away from her questing hand, instead sinking it into her considerable expanse of cleavage.  A devious grin flits across your features as she subsequently shivers from the cold glass between her fluid-filled bosom.  \"<i>Wh-why you...</i>\" she gasps, quickly snatching the drink and rubbing warmth back into her goosebumped flesh.  \"<i>You scoundrel...</i>\"");
    ...
    public niamhPostTelAdreMoveOut() {
        //Move her into the bazaar
        Flags.list[FlagEnum.NIAMH_MOVED_OUT_COUNTER] = -1;
        DisplayText().clear();
        DisplayText("Upon one of your routine searches, you're given pause as a half-coherent mess of a song comes tumbling across the grasses.  You move toward the cacophony of sound, fairly confident in what you will find.  Indeed, stumbling and shambling towards you comes a ");
        if (Flags.list[FlagEnum.NIAMH_STATUS] === 1)
            DisplayText("demonic-looking");
        else
            DisplayText("hourglass-shaped");
        DisplayText(" Niamh, and it's obvious she's completely hammered.  ");
        if (Flags.list[FlagEnum.NIAMH_STATUS] === 1)
            DisplayText("\"<i>W'hey there " + Desc.Gender.mf(player, "boyo", "big girl") + ", wanna...g'forrra goooo-gfh...time?</i>\"");
        else
            DisplayText("\"<i>W'heeeey, honey!  I never thought I'd see yaaaagain!</i>\"");
        DisplayText(" she slurs, struggling to stay upright between her impaired motor skills and her absolutely overfilled chest.  \"<i>Aaah've been drinkin' me 'shtash',</i>\" she explains, noticing your gaze.  \"<i>Canno' leddit go'tuh waste!</i>\" You nod your agreement, sneaking a glance behind her to observe the twin trails of ");
        if (Flags.list[FlagEnum.NIAMH_STATUS] === 1)
            DisplayText("dark");
        else
            DisplayText("light");
        DisplayText(" fluid she's leaving behind.");
        ...
        MainScreen.simpleChoices(["Yes", "", "", "", "Leave"], [niamhCorruptedMobileSnackDrinkTime, null, null, null, niamhCorruptMobileSnackTurnDown]);
    }

    //[no]
    private niamhCorruptMobileSnackTurnDown() {
        DisplayText().clear();
    ...
    //[yup]
    private niamhCorruptedMobileSnackDrinkTime() {
        DisplayText().clear();
        DisplayText("Of course you do!  You went through the trouble of getting her boobs to their current condition; drinking from the tap is exactly what you had in mind.  A smile brightens Niamh's features, and she plops down heavily on the ground, waving you over to sit next to her.  ");
        //[bimbo version]
    ...
    public bimboChampagne(player: Player, clearScreen: boolean, intro: boolean) {
        if (clearScreen) DisplayText().clear();
        if (intro) {
            if ((player.perks.has(PerkType.FutaFaculties) && player.perks.has(PerkType.FutaForm)) || (player.perks.has(PerkType.BimboBody) && player.perks.has(PerkType.BimboBrains))) {
                DisplayText("You could've swore the stuff worked when you saw Niamh do it to others, but for some reason, it had, like, no effect on you. How weird!");
            }
            else if (!player.statusAffects.has(StatusAffectType.BimboChampagne)) DisplayText("You uncork the bottle and breathe in the fizzy, spicy aroma of the sparkling liquor.  Breathing deeply, you open your mouth and begin pouring the ever-effervescent fluid inside.  It's sweet and slightly gooey, and the feel of it sliding down your throat is intensely... awesome?  Like, totally!");
            else DisplayText("You find yourself falling even further into the dense bimbo mindset.  You do feel, like, super-good and all, though!\n\nMoaning lewdly, you begin to sway your hips from side to side, putting on a show for anyone who might manage to see you.   You just feel so... sexy.  Too sexy to hide it.  Your body aches to show itself and feel the gaze of someone, anyone upon it.  Mmmm, it makes you so wet!  You sink your fingers into your sloppy cunt with a groan of satisfaction.  Somehow, you feel like you could fuck anyone right now!");
        }
        if (player.statusAffects.has(StatusAffectType.BimboChampagne)) {
            ...
            player.statusAffects.add(StatusAffectType.BimboChampagne, 8, 0, 0, 0);
            //(Player has breasts smaller than DD-cup:
            if (player.torso.chest.get(0).rating < 5) {
                ...
                player.changeStatusValue(StatusAffects.BimboChampagne, 2, 5 - player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating);
            ...
            if (player.torso.vaginas.count <= 0) {
                ...
                player.updateGender();
                DisplayText("\n\nYou can feel ");
                if (player.torso.cocks.count > 0) DisplayText("the flesh under your cock[if (hasBalls = true)  and behind your [balls]]");
                else DisplayText("the blank expanse of flesh that is your crotch");
                DisplayText(" start to tingle and squirm... mmm... that feels nice!  There's a sensation you, like, can't describe, and then your crotch feels all wet... but in a good, sticky sorta way.  Oh, wow!  <b>You've, like, just grown a new virgin pussy!</b>  Awesome!");
            ...
            if (player.torso.butt.rating < 12) {
                ...
                player.changeStatusValue(StatusAffects.BimboChampagne, 4, 12 - player.torso.butt.rating);
                player.torso.butt.rating = 12;
                if (player.torso.hipRating < 10) player.torso.hipRating = 10;
            }
            player.updateGender();
            player.stats.spe += -10;
            player.stats.lib += 1;
    ...
    public removeBimboChampagne() {
        ...
        player.stats.spe += 10;
        player.stats.lib += -1;
        if (player.statusAffects.get(StatusAffectType.BimboChampagne).value2 > 0) {
            player.torso.chest.get(0).rating -= player.statusAffects.get(StatusAffectType.BimboChampagne).value2;
            DisplayText("  As the trecherous brew fades, your [chest] loses some of its... bimboliciousness.  Your back feels so much lighter without the extra weight dragging down on it.");
        }
        if (player.statusAffects.get("$1").value3 > 0) {
            DisplayText("  At the same time, your [vagina] slowly seals itself up, disappearing as quickly as it came.  Goodbye womanhood.");
            player.torso.vaginas.remove();
        }
        if (player.statusAffects.get(StatusAffectType.BimboChampagne).value4 > 0) {
            player.torso.butt.rating -= player.statusAffects.get(StatusAffectType.BimboChampagne).value4;
            DisplayText("  Of course, the added junk in your trunk fades too, leaving you back to having a [butt].");
        }
    ...
    public bazaarNiamh() {
        ...
        if (Flags.list[FlagEnum.NIAMH_STATUS] === 2) {
            if (Flags.list[FlagEnum.TIMES_NIAMH_BAZAAR_MET] === 0) {
                DisplayText("As you enter the bazaar, you hear a familiar voice calls out to you. \"<i>Hey!  [name]!  Over here!  Like, it's me!</i>\"");
            ...
            //[Bizarre Bazaar, repeat encounter]
                ...
                DisplayText("Surrounded by blonde-haired, horny women, herms and traps, many of them sporting stomachs swollen with Niamh's copious champagne lactation, the bimbo catgirl enthusiastically waves you over.  \"<i>[name]!  Like, what can I do you for?</i>\" she giggles.");
            }
        }
        //Corrupt:
        else {
            //(General notification:
            DisplayText("Given the pile of snoozing bodies, you can probably hazard a guess as to where Niamh decided to set up shop.");
            //(Approach:
    ...
    private bazaardNiamhDrink() {
        DisplayText().clear();
        player.inventory.gems -= 2;
        DisplayText("Approaching the fallen-from-grace (yet completely content) catgirl, you toss her a couple gems and request a glass of the house drink.");
        //Bimbo:
        if (Flags.list[FlagEnum.NIAMH_STATUS] === 2) {
            ...
            MainScreen.simpleChoices(["Drink", "Bottle", "", "", ""], [drinkFromZeTap, getBimboChampFromNiamh, null, null, null]);
        }
        //Corrupt
        else {
            DisplayText("Succubus milk Niamh Not implemented yet");
            return { next: Scenes.camp.returnToCampUseOneHour };
        }
    }
    //{If player drinks from the tap:}
    private drinkFromZeTap() {
        DisplayText().clear();
    ...
    private getBimboChampFromNiamh() {
        DisplayText().clear();
        ...
        inventory.takeItem(consumables.BIMBOCH, Scenes.camp.returnToCampUseOneHour);
    }
    ...
    public seanBimboBrewing() {
        DisplayText().clear();
        DisplayText("The uncharacteristically gentlemanly incubus raises his eyebrow as you produce an armful of filled flasks and drop them onto his mostly-empty table.  \"<i>I assume you'd like me to do something special with these,</i>\" he quips, somewhat dryly.");
    ...
    //24 hours later
    public getBimboozeFromSean() {
        DisplayText().clear();
        DisplayText("Sean turns as you enter, managing a thin smile as he regards you.  \"<i>Excellent timing, I have just finished this!</i>\"");
        DisplayText("He scoops a familiar-looking bottle from a tabletop, passing it off to you.  \"<i>Be somewhat careful with this,</i>\" he warns as you take it.  \"<i>The original's deleterious effect on the mind has been duplicated, I assure you.</i>\"\n\n");
        DisplayText("You drop the vial into a pouch on your person.\n\n");
    ...
    private leaveWithBeerTits() {
        ...
        player.growTits(2, player.torso.chest.count, false, 2);
        return { next: Scenes.camp.returnToCampUseOneHour };
    }
    ...
    private sellYourBooze() {
        DisplayText().clear();
        player.inventory.gems += 2;
        temp = Math.round(player.lactationQ() / 100);
        if (temp > 40) temp = 40 + randInt(3);
        player.inventory.gems += temp;
        player.inventory.gems = Math.round(player.inventory.gems);
        statScreenRefresh();
        //If player has only two breasts.
        if (player.torso.chest.count === 1) DisplayText("With a chipper giggle Niamh pulls you closer to the bar.  With both hands she helps you heft your tits up and on to the counter.");
        //If player has four or six breasts.
        else DisplayText("Niamh giggles as you try to move your breasts closer to the bar to be milked by the patrons, but it's obvious only the top pair of your tits can easily rest on the counter top.  \"<i>Lassie, it might be unconventional, but givin' the circumstance mayhaps it'd be best if ye laid down on the bar.</i>\"");
        ...
        if (player.torso.vaginas.count > 0) DisplayText("  You feel your excitement dripping down the inside of your thighs as your pussy wettens.");
        dynStats("lus", 10 + player.stats.sens / 5, "resisted", false);
        //[If player has cocks]
        if (player.torso.cocks.count > 0) DisplayText("  You start to feel sympathetic sensations in [eachCock] as the hands and paws of the bar goers milk your nipples.  With their fingers sliding over your sensitive nubs you can easily imagine them sliding up and down your cock");
        if (player.torso.cocks.count > 1) DisplayText("s");
        DisplayText(".");
        //If lust is low
        if (player.stats.lust < 50 || player.gender === Gender.NONE) {
            player.growTits(2, player.torso.chest.count, false, 2);
        ...
        //If lust is high
            ...
            if (player.torso.vaginas.count > 0) DisplayText("You can feel your labia dripping with need.  ");
            if (player.torso.cocks.count > 0) {
                DisplayText("pre-cum drizzles down your " + Desc.Cock.describeMultiCockShort(player) + " as the need to fuck a pussy grows stronger.  A few wandering hands reveals some apparent interest amongst the folks in the bar in seeing your genitals as active as your nipples.");
            }
            //[FUCK THE BAR] [LEAVE]
            MainScreen.simpleChoices(["Fuck Bar", "", "", "", "Leave"], [barBeerOrgyTits, null, null, null, leaveWithBeerTits]);
        }
    }
    ...
    //First Paragraph
    //===============
    private barBeerOrgyTits() {
        DisplayText().clear();
        //If [player has pussy]
        if (player.torso.vaginas.count > 0) DisplayText("The drooling of your [vagina] gets worse as the constant \"tapping of your kegs\" drives your sense of self-restraint to its limits.  The hands of various drunks start moving more aggressively over your breasts, eventually winding their way down between your thighs.");
        //IF [player has cock/s]
        if (player.torso.cocks.count > 0) {
            DisplayText("[EachCock] grows to its full length.  You try to keep ");
            if (player.torso.cocks.count === 1) DisplayText("it");
            else DisplayText("them");
            DisplayText(" discreet but it's impossible.  You try to push ");
            if (player.torso.cocks.count === 1) DisplayText("it");
            else DisplayText("them");
            DisplayText(" down or aside, but other pairs of hands begin stroking you.");
        }
        ...
        if (player.torso.butt.rating >= 20) DisplayText("  She slaps your ass to emphasize her meaning, and the action sends jiggling waves through each immense cheek.");
        ...
        if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 20) DisplayText("Your giant boobs are large enough to temporarily hang off the sides of your body as you're carried.");
        DisplayText("  Your [butt] hits the table first as the crowd puts you down.  Some patrons begin sucking on your nipples directly.");
        // [IF player has four or more giant tits, but not hugely pregnant]
        if ((player.pregnancyIncubation === 0 || player.pregnancyIncubation > 100) && player.torso.chest.count > 1) DisplayText("  You are essentially a pile of " + boozeBoobsType() + " filled tits.");
        // [IF player is hugely pregnant with four or more tits]
        else if (player.torso.chest.count > 1) DisplayText("  You are essentially a giant pregnant belly surrounded by your [chest] on the table, helpless to the sexual advances of the bar goers and you can't get enough of it.");

        //===============
        //Third Paragraph
        //===============
        //IF player has pussy and Urta is in the bar and sex with her is unlocked.
        if (model.time.hours < 15 && Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] > 0 && player.torso.vaginas.count > 0) {
            ...
            if (player.pregnancyIncubation === 0) DisplayText("  Some folks give yelps and cheers when they see the bulge that repeatedly forms in your abdomen each time Urta thrusts herself inside you.");
        ...
        //ELSE IF player has pussy and Urta is not in the bar.
            ...
            if (player.vaginalCapacity() < 60) DisplayText("  One is sporting a cock that is obviously too massive for your pussy, and eventually backs down so that the other one, despite still being on the large size, can take the honor of fucking your cunt silly.");
            //ELSE IF [player has a gaping pussy or bigger]
            else DisplayText("  They're both sporting large canine cocks, but one is particularly massive while the other would likely be a loose fit for a pussy as naturally stretched out as yours.  The largest one wins out and positions himself to give your overly wide fuck hole a much needed stretching.");
        ...
        if (player.pregnancyIncubation > 0 && player.pregnancyIncubation < 100) DisplayText("  \"<i>Oi!  Careful folks, this babe could blow at any time!</i>\"  Niamh pats your belly with chiding affection.");
        ...
        if (player.torso.chest.find(BreastRow.FuckableNipples)) DisplayText("  Eventually some of the randier drunks start whipping out their cocks, and begin pressing the heads against the openings of your nipples.  Within moments you feel your booze bloated breasts being fucked silly by " + numToCardinalText(player.torso.chest.countNipples()) + " hard cocks.  Each thrust causes " + boozeBoobsType() + " to spill out around their shafts.");
        // ELSE[Character doesn't have fuckable nipples]
        else DisplayText("  Some of the hornier customers start shoving cocks of all shapes and sizes into the moist wet cleavages formed by yours and Niamh's breasts.");
        ...
        if (player.torso.chest.find(BreastRow.FuckableNipples)) DisplayText("  The cocks filling your nipples start pumping hot sperm into your breasts, mixing with your " + boozeBoobsType() + " and causing every nerve ending in your nipples to tingle.  Some of them ejaculate so much that it feels as if your breasts are swelling even larger, almost to the point of bursting before the cum spills out.");
        //IF [Player has cocks]
        if (player.torso.cocks.count > 0) {
            DisplayText("  You feel as if your body is exploding everywhere.  Niamh gags momentarily as her mouth fills with your seed, and you realize your cock");
            if (player.torso.cocks.count > 1) DisplayText("s have");
            else DisplayText(" has");
            DisplayText(" begun spurting ");
            if (player.torso.cocks.count === 1) DisplayText("its ");
            else DisplayText("their ");
            DisplayText("jizz.  Niamh swallows it down her throat but not without some of it getting plastered across her face.");
            //IF [character is pregnant]
            if (player.pregnancyIncubation > 0 && player.pregnancyIncubation < 200) DisplayText("  Globs of cum splatter across your gravid belly.");
        }
        ...
        if (player.statusAffects.get(StatusAffectType.Edryn).value1 >= 5 && player.gender === Gender.HERM && model.time.hours === 14 && Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] > 0) {
            ...
            // IF [player has a single cock]
            if (player.torso.cocks.count === 1) DisplayText("  Edryn moans out loud in ecstasy as your [cock] fills her up.");
            // If [player has multiple cocks]
            else if (player.torso.cocks.count > 1) DisplayText("  The heads of your cocks press in between Edryn's labia causing her to cry out, \"<i>Oh gawd!  How many dicks are y-you f-f-fucking meeee with???</i>\"  Edryn's orgasm cuts her voice off and every muscle in her cunt contracts around your " + Desc.Cock.describeMultiCockShort(player) + ".");
            ...
            if (player.pregnancyIncubation > 0 && player.pregnancyIncubation < 200) DisplayText("  Your pregnant belly presses up against Edryn's ass as both Urta's cock and Edryn's pussy keeps you pinned in place between them.");
            // IF[player is not pregnant]
            else DisplayText("  Urta's cock and Edryn's pussy pin you in place between them, and as Urta fucks you deeper her cock head forces your belly to bulge outwards until it presses against Edryn's ass.");
            ...
            if (player.torso.chest.count === 2) DisplayText("  Your second row of tits get thrust out to either side every time Urta's torso presses your own against Edryn.");
            //IF [player has six giant breasts]
            else if (player.torso.chest.count === 3) DisplayText("  Your lowest row of tits slap against all three of your thighs as the love making becomes more brutal.");
            //IF [player has large balls]
            if (player.torso.balls.quantity > 0 && player.torso.balls.size > 14) DisplayText("  Your hanging testicles bounce around between everyone's legs.");
            //IF [player has giant ass]
            if (player.torso.butt.rating >= 22) DisplayText("  The fat of your ass cheeks jiggles and quakes even as Urta practically stuffs her hips into your crack.  \"<i>Damn, you've got a bigger ass than Edryn,</i>\" she chides as she spanks you.  \"<i>Hey!</i>\"  Edryn calls out with a false tone of hurt pride.");
        ...
        // IF [Urta but not Edryn is present in the bar and sex with her is unlocked and character has pussy]
        else if (player.torso.vaginas.count > 0 && model.time.hours < 15 && Flags.list[FlagEnum.URTA_COMFORTABLE_WITH_OWN_BODY] > 0) {
            ...
            if (player.torso.chest.count > 1) DisplayText("  Your rows of breasts shift around as your posture changes, making squelching sounds as they slide around.");
            //If [player not pregnant]
            if (player.pregnancyIncubation === 0) DisplayText("  Urta's small ocean of sperm streams out from your pussy like a river down your legs as you try to stand.");
        ...
        //Generic ending if the first two don't trigger
            ...
            // IF [player has pussy]
            if (player.torso.vaginas.count > 0) DisplayText("  The giant knot of the dog morph finally breaks its way inside causing your pussy to become overstretched.  The pleasurable torture only gets worse as he begins unloading a river of his cum inside you.  It begins squirting out around his cock and soaks both of your thighs in the process.");
            Mod.Vagina.displayStretchVagina(player, 60, true, true, false);

            DisplayText("  Niamh finally has her fill and dismounts you, but no sooner has her pussy left your face than a determined cock belonging to some sort of cat morph fills your mouth.  Your eyes bug out as it thrusts down your throat.");
            // If [player has pussy]
            if (player.torso.vaginas.count > 0) DisplayText("  You gag on it at the same time a new cock fills your cunt, ready to renew the thrusting that's been causing your tits to bounce all over the place.");

            //IF [player has single cock small-med cock]
            if (player.torso.cocks.count === 1 && player.torso.cocks.get(0).area < 75) DisplayText("\n\nA cat girl with six C-cup breasts jumps up onto the table and mounts you.  She grabs your cock and proceeds to shove it up between the folds of her tight pussy.");
            ...
            if (player.torso.cocks.count === 1) DisplayText("  The six titted girl smiles at you, patting her belly while your cum runs down the inside of her thighs.  You only now wonder about how fertile she might be.");
            // IF [player has multiple cocks]
            else if (player.torso.cocks.count > 1) DisplayText("  Various girls around the room are patting their bellies and giving you sultry looks while your cum continues to drip from their pussies.  You passively wonder if any of them might indeed be hauling around some larger pregnant bellies in a few days, but you try to push those thoughts from your head.  At the moment they seem happy.");
            // IF [player has pussy]
            if (player.torso.vaginas.count > 0) {
                DisplayText("  You walk slightly bow legged out of the bar.  Cum is still dripping from your snatch and will likely continue to do so for a while.");
                //IF [player is not pregnant]
                if (player.pregnancyType === 0) DisplayText("  You can't help but wonder how virile those dog morphs might have been as their cum and the cum of other customers sloshes around inside your uterus.");
                temp = randInt(6);
                switch (temp) {
                    case 0:
