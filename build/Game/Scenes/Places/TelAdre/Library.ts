...
class Library {
    ...
    private firstTowerVisit(): void {
        ...
        if (model.time.hours <= 17) { //Don't want to meet Quinn if he's not supposed to be there
            DisplayText("  A single man carefully turns through the pages of one book");
    ...
    private towerFollowUpVisits(): void {
        ...
        if (Flags.list[FlagEnum.TIMES_BEEN_TO_LIBRARY] === -1) { //Return visits before you meet Quinn. Either you meet him or you continue to go to the library at night like some bibliophile vampire
            if (model.time.hours <= 17) {
                DisplayText("You return to the mage's tower.  Entering the main room, you're surprised to see a man carefully turning the pages of one of the tomes");
                commonQuinnTroduction();
            }
            else {
                DisplayText("As before, there's no one here.  At least there's no lack of reading material.  Looking up and down the staircases reveals two locked doors, so it's unlikely you could do anything but study here.");
                Flags.list[FlagEnum.TIMES_BEEN_TO_LIBRARY] = -2; //This will be incremented to -1 by the visitZeMagesTower function after we return
            }
            return;
        }
        ...
        if (model.time.hours <= 17) {
            DisplayText("You return to the mage's tower.  Entering the main room, Quinn is carefully inspecting the pages of a book.  The room looks slightly more organized from when you last saw it, but it looks as though Quinn will be working on it for some time.");
        ...
        //(follow-up visits, 18:00-20:00)
            ...
            DisplayText("You return to the mage's tower.  Entering the main room, Quinn is nowhere to be seen.  The room looks slightly cleaner from when you saw it last, but at the rate he's going it seems like it will be some time before it's finished.");
    ...
    private studyInTA(): void {
        ...
        //[Study, 18:00-20:00]
            ...
            if (Flags.list[FlagEnum.TIMES_BEEN_TO_LIBRARY] === -1) {
                DisplayText("Looking around you decide to spend some time reading");
            }
            else {
                DisplayText("Without Quinn to hassle you and request your absence from the presences, you have some time to read");
            }
            DisplayText(" through some of the literature collected by the Covenant.");
            if (randInt(3) === 0) {
                //magic)
                ...
                //Smart enough for arouse and doesnt have it
                else if (player.stats.int >= 30 && !player.statusAffects.has(StatusAffectType.KnowsHeal)) {
                ...
                //Smart enough for arouse and doesnt have it
                else if (player.stats.int >= 40 && !player.statusAffects.has(StatusAffectType.KnowsMight)) {
                ...
                //Smart enough for arouse and doesnt have it
                else if (player.stats.int >= 25 && !player.statusAffects.has(StatusAffectType.KnowsCharge)) {
                ...
                //Smart enough for arouse and doesnt have it
                else if (player.stats.int >= 30 && !player.statusAffects.has(StatusAffectType.KnowsBlind)) {
                ...
                //Smart enough for arouse and doesnt have it
                else if (player.stats.int >= 40 && !player.statusAffects.has(StatusAffectType.KnowsWhitefire)) {
    ...
    private talkToMali(): void {
        ...
        //[[Mali], player has spellblade]
            ...
            DisplayText("You tell Quinn you're here to see Mali.  He seems intrigued by the wrapped blade you're carrying, but doesn't ask any questions.  Unlocking the second floor as usual, he escorts you to Mali's quarters.");
        ...
        //[[Mali], player does not have spellblade]
            ...
            DisplayText("You tell Quinn you're here to see Mali.  He rolls his eyes but doesn't say anything, unlocking the second floor and leading you up once more.");
