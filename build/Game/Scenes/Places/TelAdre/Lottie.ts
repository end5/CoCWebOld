class Lottie {
    ...
    public lottieAppearance(display: boolean = true): Function {
        if (player.torso.cocks.count <= 0) return null;
        if (!player.statusAffects.has(StatusAffectType.MetMarae)) return null;
        if (model.time.hours >= 15 && model.time.hours <= 23) {
            if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00281] === 0) {
                ...
                if (display) {
                    //[RANDOM INTROS]
                    //[Encouragement 0 – 30]
                    if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00280] < 30 && randInt(6) === 0) {
                        ...
                        if (User.settings.silly()) DisplayText("  What can you say? You love making bacon.");
                    }
                    //[Encouragement 0 – 30]
                    else if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00280] < 30 && randInt(5) === 0) {
                        ...
                        if (User.settings.silly()) DisplayText("  What can you say? Her pork buns are the best.");
                    }
                    //[REPEAT INTROS]
                    else if (randInt(2) === 0) {
                        ...
                        if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 33) DisplayText("voluptuous");
                        else if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 66) DisplayText("plump");
                        else DisplayText("curvy");
                        DisplayText(" pig girl jogs on the spot to wake herself up - unintentionally jiggling her " + lottieCup() + "-cup breasts and soft ass in the process. She walks over to the nearby cafe and sits down on her ");
                        if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 33) DisplayText("bountiful");
                        else if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 66) DisplayText("chubby");
                        else DisplayText("cute");
                        DisplayText(" rump.");
                    }
                    else {
                        ...
                        if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 33) DisplayText("voluminous");
                        else if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 66) DisplayText("chubby");
                        else DisplayText("cute");
                        DisplayText(" rump.");
                    }
    ...
    private encounterLottieCharge() { encounterLottie(true); }

    public encounterLottie(charge: boolean = false) {
        DisplaySprite(36);
        let comedy1;
        ...
        DisplayText().clear();
        DisplayImage(images.showImage("lottie-encounter"));
        if (charge) {
            if (Flags.list[FlagEnum.LIFETIME_GYM_MEMBER] === 0) {
                DisplayText("The centauress working the door walks up to collect her fee, and you drop 10 gems for an hour at the gym into her hand.\n\n");
        ...
        if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00281] === 1) {
            ...
            if (!player.torso.hips.legs.isNaga() && !player.torso.hips.legs.isTaur() && player.tone >= 50) {
                ...
                //[Accept][Decline]
                MainScreen.doYesNo(acceptBeingLottiesHamSandwich, declineBeingLottiesGymHamsandwich);
            }
            //[If Don't Meet Full Requirements]
            else {
                DisplayText("\"<i>Oh, uh... thanks! Hey, listen – I gotta go start my yoga class now, but it was pretty great letting it all out with you. We should run into each other again sometime!</i>\"  The girl flashes a grin and jogs towards the showers.  \"<i>My name's Lottie, by the way!</i>\"\n\n");

                DisplayText("You aren't really doing much else here, so you decide to return home.");
        ...
        //[REPEAT ENCOUNTER]
            ...
            DisplayText("You approach the pig girl ");
            if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00280] >= 50) DisplayText("relaxing");
            else DisplayText("sitting timidly");
            DisplayText(" at a table, wearing her low-cut top and booty shorts that accentuate her ");
            if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 50) DisplayText("plump");
            else DisplayText("curvy");
            DisplayText(" figure.  ");
            if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00297] === 0) DisplayText("Lightly sipping");
            else DisplayText("Slurping down");
            DisplayText(" what appears to be ");
            if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00297] === 0) DisplayText("some juice");
            else DisplayText("a milkshake");
            DisplayText(", her eyes wander over the room when she spots you and waves");
            if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00280] >= 70) DisplayText(" excitedly");
            DisplayText(".  You walk over to her and sit down, listening to Lottie update you on her progress.\n\n");

            //[Repeat If didn't meet requirements]
            if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00299] === 0 && (player.torso.hips.legs.isNaga() || player.torso.hips.legs.isTaur() || player.tone < 50)) {
                DisplayText("\"<i>Hey, " + player.short + "! I...I'm really glad I talked about myself with you, you're a really great friend, y'know. I really think I can achieve something this time, if I work hard!</i>\" You smile and tell her how glad you are that she's doing this. Lottie responds with a delighted expression, and continues to talk. \"<i>Anyway, I've been trying to look for a personal trainer, but it's kinda difficult, heh. I mean... I'm sure there's lots of trainers here and all, but I – I'd prefer someone I know personally, right?</i>\" She looks up at you expectantly, but then looks back towards the floor. </i>\"But I guess we can't always have what we want.</i>\"\n\n");
            ...
            //[Normal Encounter, If became met requirements]
                ...
                DisplayText("\"<i>Hey, " + player.short + "! I was just... I...</i>\"Lottie seems to be at a loss of words, gaping at your new, chiselled physique. You smile and tell her how glad you are that she's doing this, thought it's a shame she hasn't found a personal trainer yet. Lottie responds with a hazy expression and an open mouth. \"<i>Y-yeah... it's a shame...</i>\" She begins, until snapping out of her daze and looking up excitedly at you. \"<i>I- Y'know, I don't know what you've been doing to yourself lately, but... you look a... you're really amazing right now, " + Desc.Gender.mf(player, "big guy", "babe") + "!</i>\" She squeals as you flex at her, giggling at your blatant attempts at showing off. You never really took her as one to appreciate muscles, but you figure her obsession towards the perfect body might extend to other people as well. You take one stern look at the chubby porcine in front of you and grin, asking if she wants you to be her personal trainer. She seems shocked at your proposal. </i>\"Are... are you serious?</i>\"\n\n");
                DisplayText("Well, are you?");
                //[Yes][No][God No]
                if (silly() && player.stats.cor >= 80) comedy1 = gotInShapeComedyDeclineLottie;
                MainScreen.simpleChoices(["Yes", "No", "God No", "", ""], [gotInShapeAcceptTrainingOffer, gotInShapeButTurnedDownLottie, comedy1, null, null]);
                return;
            }
            //[Second Encounter, If fucked Lottie]
                ...
                DisplayText("\"<i>Hey, " + player.short + "! I...I really enjoyed what we did. God, if that's what it takes to get healthy I wouldn't mind! I think I'm falling in love with you already, haha!</i>\" She forces a giggle at her last sentence, albeit rather nervously. You raise an eyebrow, but decide not to push the issue further.");
            }
            //[Second Encounter, If didn't fuck Lottie]
            else if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00281] === 2) {
                DisplayText("\"<i>Hey, " + player.short + "! I...I'm really glad we're doing this, y'know? I really think I can achieve something this time, if I work hard – and if you work hard with me!</i>\" You smile and tell her how glad you are that she's doing this. Lottie responds with a delighted expression as you both return to normal conversation.");
            ...
            //[Encouragement moved from 31 to 30, regular]
                ...
                DisplayText("\"<i>H-hi " + player.short + "... I noticed that you're... y-you're acting like you do when I do something wrong. Did I do something? P-please, I really don't like seeing you so m-mean. Please don't be like this.</i>\" She forces a smile at her last sentence, though you can tell she's incredibly nervous. You raise an eyebrow, but decide not to push the issue further.");
                Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00300] = Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00280];
            }
            ...
            //[Encouragement moved from 79 to 80, first time]
                ...
                if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00304] === 0) {
                    ...
                    DisplayText("You don't know the answer.");
                }
                //[Encouragement moved from 79 to 80, regular]
                else {
                    DisplayText("\"<i>Hey, " + player.short + ".Thanks for being... nicer, again. Sometimes I really worry about you, but... well; I'm fine with putting that behind us. I'm glad, y'know?</i>\" She throws you a smile, confident that you won't mistreat her again. You smile back, silently assuring her that you won't, today.");
                }
            }
            //[Normal Encounter, Encouragement 60 - 100]
            else if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00280] >= 60) {
                //[Figure 0 – 20]
                if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 20 && Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00297] === 0) DisplayText("\"<i>Oh, " + player.short + "! Let's do our best today, I really wanna lose those pounds! I know I can do my best when I'm with you!</i>\" Lottie beams, her massive tits pushed against the table. She seems to be excited for what you've got in store today.");
                //[Figure 0 - 20, convinced her to accept body]
                else if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 20) DisplayText("\"<i>Hey, " + Desc.Gender.mf(player, "stud", "cutie") + "! You shouldn't keep a big girl like me waiting; unless you wanna get crushed by these!</i>\" Lottie grins and grabs her luscious breasts firmly, giggling when she notices you're at full attention now. She seems to be excited for what you've got in store today.");
                //Figure 20 - 40, haven't convinced her to accept body]
                else if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 40 && Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00297] === 0) DisplayText("\"<i>Ah, " + player.short + "! We should probably get started soon; I'm gonna push myself to the limit today!</i>\" Lottie's arms are folded over her chest, subconsciously trying to hide her nervousness, though squishing her pillowy tits in the process. She seems to be excited for what you've got in store today.");
                //[Figure 20 - 40, convinced her to accept body]
                else if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 40) DisplayText("\"<i>Hey, " + Desc.Gender.mf(player, "stud", "cutie") + "! You shouldn't keep me waiting; the only thing I'm losing right now is my patience!</i>\" Lottie grins and punches your arm playfully, squealing when you try to get her back. She seems to be excited for what you've got in store today.");
                //[Figure 40 - 60]
                else if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 60) DisplayText("\"<i>Hey! I was looking for you earlier, y'know? I can't wait to get started.</i>\" She looks happy that you've arrived. Lottie leans over to grab her beverage, giving you an excellent view of her supple mounds. She seems to be excited for what you've got in store today.");
                //[Figure 60 - 80]
                else if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 80) DisplayText("\"<i>Yo, " + Desc.Gender.mf(player, "big guy", "babe") + "! I've been waiting for you, so you better be pumped! Ready to get started?</i>\" Lottie leans over, her creamy tits pushed against the table. She seems to be excited for what you've got in store today.");
                //[Figure 80 - 100]
                else DisplayText("\"<i>Hey, " + Desc.Gender.mf(player, "stud", "cutie") + "! You shouldn't keep me waiting, or I'll put you through the burn twice as hard! You ready?</i>\" Lottie winks at you and punches your arm playfully. She seems to be excited for what you've got in store today.");
            }
            //[Normal Encounter, Encouragement 30 - 60]
            else if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00280] >= 30) {
                //[Figure 0 - 20]
                if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 20) DisplayText("\"<i>Oh, " + player.short + ", hey. I think we should try and get into it today. I don't think I tried hard enough last time, y'know?</i>\" Lottie leans over to grab her beverage, her massive tits pushed against the table. She seems to be intent on trying her best today.");
                //[Figure 20 - 40]
                else if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 40) DisplayText("\"<i>A-ah, " + player.short + "! We should probably get started soon; I... I want to push myself to the limit today!</i>\" Lottie's arms are folded over her chest, subconsciously trying to hide her nervousness, though squishing her pillowy tits in the process. She seems curious as to what you both plan to do today.");
                //[Figure 40 - 60]
                else if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 60) DisplayText("\"<i>H-hey! I was looking for you earlier, y'know? I can't wait to get started.</i>\" She looks at ease now that you've arrived. Lottie leans over to grab her beverage, giving you an excellent view of her supple mounds. She seems curious as to what you both plan to do today.");
                //[Figure 60 - 80]
                else DisplayText("\"<i>Hello " + player.short + "! I've... I've been waiting for you, so you better be ready, hey! Don't hold back!</i>\" Lottie leans over, her creamy tits pushed against the table. She seems to be excited for what you've got in store today.");
            }
            //[Normal Encounter, Encouragement 0 - 30]
            else {
                //[Normal Encounter, Figure 0 – 70]
                if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00279] <= 70) DisplayText("\"<i>Oh, " + player.short + "... I know I haven't been trying the way you want me to. I – I punished myself extra hard last night! Please, let me try again today!</i>\" Lottie leans over and looks at you desperately, her creamy tits pushed against the table. You can see they're redder than before. Has she been trying to slap her fat away? You turn away, not giving her the pleasure of looking at your face. You agree to help her – but don't expect to be looked at by the only person who can stand you if this goes on. Lottie squeals and manages to blurt out a 'th-thank you!' in response.");
                //[Normal Encounter, Figure 70 - 100]
                else DisplayText("\"<i>" + player.short + "! I know you don't like looking at me, but I... I lost some more weight for you today!</i>\" Lottie removes her top and exposes her curvaceous body, plump tits in full view. You stare her down, and she smiles weakly, grateful for your patience. She waits for your response, but you turn away and tell her to cover up her disgusting body. \"<i>But I – oh god, I'm sorry, I'm so sorry, I'll try extra hard today, I'll try so so hard...</i>\" She babbles on while you go to the counter and order some food for yourself.");
            }
        }
        //[Appearance][Talk][Exercise][Give Item][Sex][Hug]
    ...
    private stayLottieBuds4Evah() {
        ...
        DisplayText("A while later and you both sit at the entrance to the gym, Lottie having calmed down a considerable amount. \"<i>Thanks, " + player.short + ", I know I'm just overreacting, but...</i>\" she sighs. \"<i>I don't know. I don't know what I'd do without you, " + Desc.Gender.mf(player, "big guy", "babe") + ".</i>\" You give her one last hug before parting ways. She'll be fine in the morning.");
        return { next: Scenes.camp.returnToCampUseOneHour };
    }

    //[If Decline]
    private declineBeingLottiesGymHamsandwich() {
        DisplaySprite(36);
        DisplayText().clear();
    ...
    private acceptBeingLottiesHamSandwich() {
        ...
        //[Water][Shake]
        MainScreen.simpleChoices(["Water", "Shake", "", "", ""], [drinkWaterWithYourHamhock, drinkLustWithYourHamhock, null, null, null]);
    }

    //Followups to this: DisplayText("\"<i>Hey, " + player.short + "! I was just... I...</i>\"Lottie seems to be at a loss of words, gaping at your new, chiselled physique. You smile and tell her how glad you are that she's doing this, thought it's a shame she hasn't found a personal trainer yet. Lottie responds with a hazy expression and an open mouth. \"<i>Y-yeah... it's a shame...</i>\" She begins, until snapping out of her daze and looking up excitedly at you. \"<i>I- Y'know, I don't know what you've been doing to yourself lately, but... you look a... you're really amazing right now, " + Desc.Gender.mf(player, "big guy","babe") + "!</i>\" She squeals as you flex at her, giggling at your blatant attempts at showing off. You never really took her as one to appreciate muscles, but you figure her obsession towards the perfect body might extend to other people as well. You take one stern look at the chubby porcine in front of you and grin, asking if she wants you to be her personal trainer. She seems shocked at your proposal. </i>\"Are... are you serious?</i>\"\n\n");
    //If No]
    private gotInShapeButTurnedDownLottie() {
        DisplaySprite(36);
    ...
    //[If God No]
    public gotInShapeComedyDeclineLottie() {
        DisplaySprite(36);
        DisplayText().clear();
        DisplayText("Haha, of course you fucking aren't, why would you be? Lottie stares at you, tears welling in her eyes. Oh, whoops – you must've said that out loud. Proceeding to not give two shits about the bawling pig-slut in front of you, you take a good squeeze of her cushiony tits before whipping your " + Desc.Cock.describeCock(player, player.torso.cocks.sort(Cock.LargestCockArea)[0]) + " out and slapping her in the face with it.  \"<i>W-w-why... why are you... *sniff* p-please...!</i>\" The pig girl chokes in between sobs. It almost gives you a hard on - almost. You take one final step backwards, before announcing quite loudly to the other cafe-goers that you're going for a home run. Lottie squeaks before you take your " + Desc.Cock.describeCock(player, player.torso.cocks.sort(Cock.LargestCockArea)[0]) + " and swing at her face, hitting dead on and managing to leave a bit of pre-cum on her newly-christened cheek. You stand triumphantly at your latest work of art, though Lottie seems to have other ideas, running out the door in complete misery, pre-cum and all. You decide you'd better return home too, and head back towards the camp – but not before spending the next hour or so chasing after Lottie and taking grabs at her plush ass.");
        //[PC no longer encounters Lottie.]
        Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00278] = 1;
        return { next: Scenes.camp.returnToCampUseOneHour };
    }
    ...
    //[If Yes]
    private gotInShapeAcceptTrainingOffer() {
        DisplaySprite(36);
        Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00299] = 1;
        DisplayText().clear();
        DisplayText("\"<i>A-ah, really? Y'mean it? This isn't some trick or something, right? You ain't about to go ahead and flake out on me, are you?</i>\" You assure her you're serious if she is, though you make a mental note that she's more likely to flake out than you are, given her history. \"<i>Eeee! Thanks, " + Desc.Gender.mf(player, "big guy", "babe") + " – I can't wait to get started!</i>\" You chuckle, remarking on her enthusiasm. \"<i>Well " + player.short + ", I'm excited because I think the two of us are about to become super close! Do you feel it? Do ya?</i>\" Lottie wiggles in excitement, loosening the already skimpy towel and revealing one of her perky nipples. You definitely feel something, that's for sure. She follows your eyes down to her breasts and reddens as she quickly covers herself up as best as she can - only succeeding in making her pillowy breasts push up against the towel, increasing her cleavage.\n\n");

        DisplayText("You take this opportunity to get a good look at Lottie. She stands at around 4'8</i>\", considerably short on average, ");
        if (player.tallness > 60) DisplayText("and shorter than you");
        else if (player.tallness > 53) DisplayText("but you can't judge, being roughly the same height");
        else DisplayText("taller than you");
    ...
    //[Water]
    private drinkWaterWithYourHamhock() {
        DisplaySprite(36);
        DisplayText().clear();
        ...
        if (Flags.list[FlagEnum.TIMES_HAD_YOGA] === 0) DisplayText("You've never had any experience with the yoga classes first hand, but you tell ");
        else DisplayText("You've had experience with the yoga classes, so you tell ");
        DisplayText("Lottie it's a great way to meet new people.\n\n");
    ...
    //[Shake]
    private drinkLustWithYourHamhock() {
        DisplaySprite(36);
        DisplayText().clear();
        DisplayText("You decide to down the cup of pink liquid, savoring the sweet taste as it pours down your throat. Your thirst quenched, you start to walk back towards the track when you're suddenly overcome with a warm feeling that stretches down to your groin. " + Desc.Cock.describeMultiCockSimpleOne(player, true) + " stirs from beneath your " + player.inventory.equipment.armor.displayName + " – you need to fuck something, and fast.  \"<i>Uh, is something wrong?</i>\" Lottie glances over to you, noticing the bulge in your pants as it strains against your " + player.inventory.equipment.armor.displayName + ". She looks up at your face, concerned about your well being. You grin and tell her you've had enough running.  Confused, Lottie asks what you mean.  You explain that you know a far more effective way to lose weight and stay in shape – in fact, it's how you got to be the way you are now.  You can see Lottie's eyes widen at the idea of such a technique, especially considering your own well-muscled physique.  \"<i>I'm all ears! What is it?</i>\"  She asks.  Clearly not getting the picture, you grab her hand and put it on your crotch.  Lottie quietly squeals and looks around to see if anyone noticed before looking back at you.  \"<i>Oh, um, I don't- I don't know about that, " + player.short + "...</i>\" She mumbles, her hand still remaining on your growing hard-on.  It's perfectly fine - you've trained like this with hundreds of people, you lie.  Lottie swallows and looks down at the sound of 'hundreds'.  Not being one to question your methods, she begins to stroke your hardening length through your " + player.inventory.equipment.armor.displayName + ", using her palm to massage your shaft");
        //[if balls]
        if (player.torso.balls.quantity > 0) DisplayText(" while her fingers nervously trace around your balls");
        DisplayText(". You stop her for now, though not before softly squeezing her hand as it caresses your girth. You know where you two can go 'work out', you tell her. She nods, her eyes glued to your erection.\n\n");

        DisplayText("You take her by the hand and lead her into one of the empty saunas. \"<i>" + player.short + "...</i>\" You grab her by the hips and lean her against the wall, throwing off her towel and revealing her velvety skin underneath. Reaching down, you grab a handful of her luxurious ass, squeezing and slapping it lightly. Lottie moans into your shoulder and reaches towards your groin, removing your " + player.inventory.equipment.armor.displayName + ". She begins stroking " + Desc.Cock.describeMultiCockSimpleEach(character) + " lightly, picking up the pace as you continue to work on her pert rump. You can't help but lust over her full figure, using one of your hands to firmly grope her pillowy jugs and reaching your other towards her glistening pussy. You move your hand over her cushiony rack, before burying your head into her bosom and eagerly sucking on one of her nipples, fondling the other between your thumb and forefinger. Your other hand is already fast at work, fingering her hot cunt. You can feel her legs about to buckle beneath their own weight. Lottie moans in between short, hot breaths, \"<i>Y-you're... really amazing, " + Desc.Gender.mf(player, "guy", "lady") + ".</i>\"  You give her chubby ass a hard slap in agreement.  Lottie squeals in glee, jerking your " + Desc.Cock.describeCock(player, player.torso.cocks.get(0)) + " as it pokes into her soft stomach. The sensation of your tip rubbing against her velvety skin drives you wild.\n\n");
    ...
    //[If Anal]
    public analLottieMilkshake() {
        DisplaySprite(36);
        DisplayText().clear();
        DisplayImage(images.showImage("lottie-fuck-her-in-teh-butzor"));
        let x: number = player.cockThatFits(40);
        if (x < 0) x = 0;
        DisplayText("You brush her grip off of " + Desc.Cock.describeMultiCockSimpleEach(character) + ", only to turn her around and fit your girth between her large cheeks");
        if (player.torso.cocks.count === 2) DisplayText(", managing to squeeze in your other cock along with it");
        else if (player.torso.cocks.count > 2) DisplayText(", the rest of your dicks poking and prodding against her soft flesh");
        DisplayText(".  \"<i>You sure do like my butt, huh?</i>\"  She giggles softly, satisfied that at least some of her fat is being put to use.  You groan as she starts to move her ample hips, massaging your " + Desc.Cock.describeCock(player, x) + " within her jiggling buttocks.  Your tip brushes against her asshole, smearing pre-cum all over the hot entrance.  Lottie starts to push back towards your cock, clearly overcome with lust.  You wait a few seconds as she aimlessly grinds against you before you spread her cheeks and plunge your " + Desc.Cock.describeCock(player, x) + " into her tight backdoor. Lottie squeals against the wall in delight as you fill her stomach and begin pounding away at her relentlessly, her large tits bouncing rhythmically. She continues to drive herself into you, her tight, fiery insides squeezing your length whilst you watch her heavy butt ripple with every thrust.\n\n");
        ...
        DisplayText("\"<i>A-aa-ah, I-I... AHH!</i>\" Lottie oinks noisily, her eyes rolling to the back of her head and her large thighs slapping against your own, pulling your " + Desc.Cock.describeCock(player, x) + " further in. She shudders in orgasm");
        if (player.torso.balls.quantity > 0) DisplayText(", her soaked pussy rubbing against your balls");
        DisplayText(".\n\n");

        //[If Multi]
        if (player.torso.cocks.count > 1) {
            DisplayText("Deciding you aren't done with her bouncy rear, you take ");
            if (player.torso.cocks.count === 2) DisplayText("your other cock");
            else DisplayText("one of your cocks");
            DisplayText(" and push it against her puckered hole. \"<i>H-hey, wait a mome- OHH!</i>\" You ease yourself back into her, with ");
            if (player.torso.cocks.count === 2) DisplayText("your two ");
            else DisplayText("two of your ");
            DisplayText(" throbbing cocks buried deep in her rectum. The pressure against your hot rods is immense, though you continue to slowly push and pull her sweaty ass against you, enjoying the feeling of her meaty fuckpillows as they massage your cock");
            if (player.torso.cocks.count > 1) DisplayText("s");
            DisplayText(".\n\n");
        }
        if (player.torso.balls.quantity > 0) DisplayText("Your " + Desc.Balls.describeSack(player) + " begins to swell");
        else DisplayText("You feel yourself about to explode");
        ...
        if (player.cumQ() >= 1000) DisplayText("overflowing");
        else if (player.cumQ() >= 500) DisplayText("dripping");
        else DisplayText("filled");
        DisplayText(" with your hot, thick spunk. She squeals again as you bring her to orgasm, ramming herself into you to collect as much of your cum as possible. You continue to pound her bouncy buttocks as you release the rest of your seed inside of her");
        //[If large cum production]
        if (player.cumQ() >= 500) DisplayText(", a heavy amount leaking from her ass");
        ...
        if (User.settings.silly()) DisplayText("bacon ");
        DisplayText("shake on the way out.");
        ...
        DisplayText(player.modTone(100, 1));
        player.orgasm();
        return { next: Scenes.camp.returnToCampUseOneHour };
    }

    //[If Oral]
    public getSuckedOffByAHamSandwich() {
        DisplaySprite(36);
        DisplayText().clear();
        DisplayImage(images.showImage("lottie-gives-you-a-blowjob"));
        let x: number = player.torso.cocks.sort(Cock.LargestCockArea)[0];

        DisplayText("You grab Lottie's shoulders and firmly move her down towards your crotch. She kneels down, face to face with your " + Desc.Cock.describeCock(player, x) + ".  Feeling her hot breath against your " + Desc.Skin.skinFurScales(player) + ", you tell her to suck it.\n\n");

        //[If player cock is at or below 4']
        if (player.torso.cocks.get(x).length <= 4) {
            DisplayText("She stares at your " + Desc.Cock.describeCock(player, x) + " and then up at you. \"<i>I... oh, well I guess this is okay.</i>\" She lazily slurps your member into her mouth, and moans at the taste of your " + Desc.Cock.describeCock(player, x) + ". Her agile tongue moves over your entire length, licking gently over your sensitive head and along your shaft. Your " + Desc.Cock.describeCock(player, x) + " sits comfortably inside her warm hole, covered in her saliva");
            if (player.torso.cocks.count > 1) {
                DisplayText(" while your other cock");
                if (player.torso.cocks.count === 2) DisplayText(" pokes");
                else DisplayText("s poke");
                DisplayText(" against her chin");
            }
            DisplayText(".\n\n");
        }
        //[If the player has a cock between 5' – 12']
        else if (player.torso.cocks.get(x).length <= 12) {
            DisplayText("She stares intently at your " + Desc.Cock.describeCock(player, x) + " with hunger in her eyes. A small amount of drool escapes her luscious lips as she moves forward, kissing your ");
            if (player.torso.cocks.count > 1) DisplayText(" biggest head lightly");
            else DisplayText(" head lightly");
            DisplayText(".  Her saliva leaks onto your " + Desc.Cock.describeCock(player, x) + ", though she is quick to lick it up while she works her tongue around the rest of your shaft");
            if (player.torso.cocks.count > 1) DisplayText(", jerking off your " + Desc.Cock.describeCock(player, player.biggestCockIndex2()) + " with both hands");
            DisplayText(".  She places small licks on the tip of your head, and then hungrily devours the rest of your length in one fell swoop.\n\n");
        }
        //[If the player has a cock equal to or over 13']
        else {
            DisplayText("She gasps as you lift your " + Desc.Cock.describeCock(player, x) + " and slap her face. \"<i>It's... wow, " + player.short + "!</i>\" You grin before poking her pudgy cheek with the tip of your ");
            if (player.torso.cocks.count > 1) DisplayText("biggest erection");
            else DisplayText("erection");
            DisplayText(".  A small amount of drool escapes her luscious lips as she moves forward, kissing your head lightly. Her saliva leaks onto your " + Desc.Cock.describeCock(player, x) + ", though she is quick to lick it up while she works her tongue around the rest of your lengthy shaft");
            if (player.torso.cocks.count > 1) DisplayText(" - using her hands to massage your leftover " + Desc.Cock.describeCock(player, player.biggestCockIndex2()));
            DisplayText(". She places small licks on the tip of your head, and without warning stuffs as much of your thick girth as she can into her mouth, coughing around your " + Desc.Cock.describeCock(player, x) + ".\n\n");
        }

        DisplayText("She begins fellating your cock at an incredible rate, throatfucking herself on your " + Desc.Cock.describeCock(player, x));
        if (player.torso.balls.quantity > 0) DisplayText(", your sack slapping against her chin ");
        else if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 2) DisplayText(" while you play with your " + Desc.Breast.describeBreastSize(character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating));
        DisplayText(". You remark on what a hungry little piggy she must be, and the chubby girl moans in response, the vibrations massaging your member. Relishing the feeling of your " + Desc.Cock.describeCock(player, x) + " in between her hungry lips, you start to thrust into her warm mouth hole, grabbing onto her head as you pound her face against your groin.");
        if (player.torso.cocks.count > 1) {
            if (player.torso.cocks.count === 2) DisplayText("  Your extra cock flails wildly, slapping against her cheeks.");
            else if (player.torso.cocks.count > 2) DisplayText("  Your extra cocks flail wildly, slapping against her cheeks.");
        }
        DisplayText("  Lottie's slick tongue desperately wraps itself around your length as you drive in and out of her fuckhole relentlessly, drool beginning to leak from her mouth. She");
        //[if Multi]
        if (player.torso.cocks.count > 1) DisplayText(" stops rubbing your " + Desc.Cock.describeCock(player, x) + " to ");
        //(Otherwise)
        else DisplayText(" reaches down towards her dripping pussy; plunging two fingers into her wetness while using the other hand to ");
        //If Balls]
        if (player.torso.balls.quantity > 0) DisplayText("knead your " + Desc.Balls.describeBalls(true, true, player));
        else DisplayText("reach around and squeeze your " + Desc.Butt.describeButt(player));
        DisplayText(".  You grab the back of her head and pull her in further, bobbing her head while she slurps and sucks at your meatstick. The stimulation almost causes you to explode right then and there, but her bouncing tits give you an idea.\n\n");

        DisplayText("You slowly take your " + Desc.Cock.describeCock(player, x) + " out of her hot mouth, a small *pop* breaking the silence as it escapes her plump, puckered lips. Grabbing one of her hefty tits in your hands, you rub your cockhead against her erect nipple, eliciting a small moan from the sweaty slut. You slap your " + Desc.Cock.describeCock(player, x) + " against her sweaty chest a few times before burying it in between her cleavage, ");
        //[If the player has a cock equal to or over 13']
        if (player.torso.cocks.get(x).length > 12) DisplayText("your sensitive tip brushing against her eager mouth");
        //Otherwise]
        else DisplayText("resting comfortably in between her cushiony fuckpillows");
        DisplayText(". Lottie wraps her tits around your length and begins working at pleasuring your every inch.\n\n");

        //[If the player has a cock equal to or over 13']
        if (player.torso.cocks.get(x).length > 12) DisplayText("Licking at your cockhead, Lottie squishes her breasts against the rest of your member, making sure to lap up the stray beads of pre-cum squirting from your tip. Relishing the taste of your " + Desc.Cock.describeCock(player, x) + " in her mouth she begins to suck at your head, flicking her tongue around your urethra and pressing her full lips against it, creating a vacuum of pleasure around your manhood.\n\n");

        //[If Multi]
        if (player.torso.cocks.count > 1) {
            DisplayText("You decide to make use of ");
            if (player.torso.cocks.count === 2) DisplayText("your");
            else DisplayText("an");
            DisplayText(" extra cock as you push it between her pouty lips, her face brushing against it while she tries to titfuck your main member. Looking up at you, Lottie descends on you and begins fellating your " + Desc.Cock.describeCock(player, player.biggestCockIndex2()) + ", not forgetting to knead her fat tits into your " + Desc.Cock.describeCock(player, x) + ".\n\n");
        }

        DisplayText("Raising her body up and down, you watch as the pig-slut massages her rack tightly around your rod whilst you plunge between her cleavage, thrusting slightly to match her movements. \"<i>" + player.short + ", p-please cum on me, god- I need this!</i>\" she moans, her saliva lubing her chest while she pumps your dick in between her melons");
        if (player.torso.cocks.count > 1) DisplayText(" and chokes on your meat");
        DisplayText(".  You throb against her hot flesh, feeling your spunk bubbling up inside you.  Throwing your head back, you give one final thrust before pulling back and exploding all over your squishy slut, ");
        if (player.cumQ() < 500) DisplayText("thick ropes of cum splattering all over her face and pooling in between her tits");
        else DisplayText("massive globules of cum splattering all over her face and painting her tits");
        DisplayText(".  She continues to move up and down against the thick shaft between her breasts, determined to drain you of your seed. She pulls you out of her bosom and plunges your member back into her mouth, suckling on it for any stray milk left. After a while, you groan as she pops you back out, collecting the ");
        if (player.cumQ() > 500) DisplayText("incredible amounts of ");
        DisplayText("cum you've painted on her flesh and gulping it all down. \"<i>Thanks for the load, " + Desc.Gender.mf(player, "big guy", "babe") + ",</i>\" she remarks, sucking gently on her fingers. \"<i>I'll... I'll see you again, hey!</i>\" You pat her on the head and begin to make your way out of the gym, making sure to grab a ");
        if (User.settings.silly()) DisplayText("bacon ");
        DisplayText("shake on the way out.");
        //LOTTIE FIGURE -1, MUSCLE TONE +1, DRAINS PC'S LUST.
        lottieTone(-1);
    ...
    //[Appearance]
    private lotteryAppearance() {
        DisplaySprite(36);
        DisplayText().clear();
        DisplayImage(images.showImage("lottie-examine-appearance"));
        //[If Figure between 0 – 20]
        if (lottieTone() <= 20) {
            DisplayText("She stands at around 4'8</i>\", considerably short on average, ");
            if (player.tallness > 60) DisplayText("and shorter than you");
            else if (player.tallness > 53) DisplayText("but you can't judge, being roughly the same height");
            else DisplayText("taller than you");
        ...
        //[If Figure between 40 - 60]
            ...
            DisplayText("She stands at around 4'8</i>\", short on average, ");
            if (player.tallness > 60) DisplayText("and shorter than you");
            else if (player.tallness > 53) DisplayText("but you can't judge, being roughly the same height");
            else DisplayText("taller than you");
            DisplayText(". Physique-wise, Lottie is definitely a bit chubbier than most girls, though you couldn't call her fat - of the few pig people you have seen around Tel'Adre, she's definitely the 'thinnest'. The majority of her weight is focused in her cushiony breasts, ample thighs and large ass, though her stomach is rounded nicely to support the weight of her upper body. Her heart-shaped face is complimented by her button nose and soft lips, framed by her pink-gray hair, which ends in small curls. A small, coiled tail rests above her rump.");
    ...
    private talkToPigSlut() {
        ...
        if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00306] === 1) {
            ...
            DisplayText("Her ears perk up at your eagerness to talk, but you can tell she's otherwise gloomy. You ask her what's wrong. \"<i>That obvious, huh?</i>\" Lottie looks down onto the table.  \"<i>Y'see, " + Desc.Gender.mf(player, "big guy", "babe") + ", ever since I... you know, I've actually - I've gained weight!</i>\"  She stands up, her chubby cheeks puffed out in frustration. \"<i>I mean, i-it's not like I didn't enjoy it or anything, but... it seems to be having the opposite effect of what I want!</i>\" She's clearly annoyed, but you place your hand on her shoulder. Do you encourage her, or is it her own fault?");
            //[Encourage][Abuse]
            MainScreen.simpleChoices(["Encourage", "Abuse", "", "", ""], [firstOralReactionChooseEncourage, firstOralReactionChooseAbuse, null, null, null]);
        }
        //[If 1st Time Anal]
        else if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00307] === 1) {
            Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00307] = 2;
            DisplayText("Her ears perk up at your eagerness to talk, she seems to be pretty excited about something. You ask her what's on her mind. \"<i>That obvious, huh?</i>\" Lottie looks up at you and grins.  \"<i>Y'see, " + Desc.Gender.mf(player, "big guy", "babe") + ", ever since we... you know, I've actually – I've lost weight!</i>\" She stands up, pumping her fist in the air and wiggling her plump ass. \"<i>I mean, I know it's not a big difference, but it's still a result! This is amazing!</i>\" She's clearly pleased with herself. You place a hand on her shoulder, and she beams at you. Do you encourage her, or break the bad news?");
            //[Encourage][Abuse]
            MainScreen.simpleChoices(["Encourage", "Abuse", "", "", ""], [firstAnalReactionChooseEncourage, firstAnalReactionChooseAbuse, null, null, null]);
        }
        //FATNESS
        ...
        //[If Gained Weight since Last Encounter, Encouragement 30 - 70]
            ...
            DisplayText("Her ears perk up at your eagerness to talk, but you can tell she's otherwise gloomy. You ask her what's wrong. \"<i>That obvious, huh?</i>\" Lottie looks down onto the table.  \"<i>Y'see, " + Desc.Gender.mf(player, "big guy", "babe") + ", ever since we last met, I've actually - I've gained weight!</i>\" She stands up, her chubby cheeks puffed out in frustration. \"<i>I mean, i-it's not like I'm angry at you or anything, but... I'm just a little frustrated, that's all.</i>\" She's clearly more than a little frustrated, but you place your hand on her shoulder. Do you encourage her, or is it her own fault?");
            //[Encourage][Abuse]
            MainScreen.simpleChoices(["Encourage", "Abuse", "", "", ""], [lottieMediumMoraleWeightGainEncourage, lottieMediumMorealeWeightGainAbuse, null, null, null]);
        }
        //[If Gained Weight since Last Encounter, Encouragement 70 – 100, convinced 'Bigger is Better']
            ...
            DisplayText("Her ears perk up at your eagerness to talk, though she seems rather distracted by something. You ask her what's wrong. \"<i>Oh babe, it's really nothing...</i>\" Lottie looks up and smiles, but your questioning gaze earns another answer. \"<i>Dang, y'see " + Desc.Gender.mf(player, "big guy", "babe") + ", ever since we last met, I've gone ahead and gained a bit more weight...</i>\" She remains in her chair, resting her face between her arms, though her chubby cheeks remain puffed out in frustration. \"<i>I mean, I love my body and I love you, but... it gets to me sometimes, y'know?</i>\" She's clearly more than a little frustrated, but you place your hand on her shoulder, rubbing it gently. Do you encourage her, or is it her own fault?");
            //[Encourage][Abuse]
            MainScreen.simpleChoices(["Encourage", "Abuse", "", "", ""], [lottieHighMoraleWeightGainFATTYLOVESIT, lottieHighMoraleWeightGainFATTYLOVESABUSE, null, null, null]);
        }
        //[If Gained Weight since Last Encounter, Encouragement 70 - 100]
            ...
            DisplayText("Her ears perk up at your eagerness to talk, though she seems rather distracted by something. You ask her what's wrong. \"<i>Oh babe, it's really nothing...</i>\" Lottie looks up and smiles, but your questioning gaze earns another answer. \"<i>Dang, well, y'see " + Desc.Gender.mf(player, "big guy", "babe") + ", ever since we last met, I've gone ahead and gained weight!</i>\" She stands up, her chubby cheeks puffed out in frustration. \"<i>I mean, i-it's not like I'm angry at you or anything, but... I'm just a little frustrated, that's all.</i>\" She's clearly more than a little frustrated, but you place your hand on her shoulder. Do you encourage her, or is it her own fault?");
            //[Encourage][Abuse]
            MainScreen.simpleChoices(["Encourage", "Abuse", "", "", ""], [lottieHighMoraleWeightGainEncourage, lottieHighMoraleWeightGainAbuse, null, null, null]);
        }
        //DEFATNESS
        //[If Lost Weight since Last Encounter, Encouragement 0 - 30]
            ...
            DisplayText("You tell her you want to talk, though the girl only winces in response, shying away from your stern gaze. The nervous pig-slut knows from experience how most of your 'talks' usually end – with a sobbing, miserable little piggy.  In the sweetest voice you can muster you ask her how your favorite girl is doing. Lottie hesitates before facing you again – you know she hates it when you use that voice, but she also seems to be vaguely excited about something. You tell her to spill what's on her mind. \"<i>W-well, you see...</i>\" Lottie looks up at you and manages the tiniest smile. \"<i>Y'see, " + Desc.Gender.mf(player, "sir", "ma'am") + ", since we last m-met, I've actually – I've lost weight!</i>\" Her minute expression turns into a full blown grin, though immediately fades as soon as she realizes what she's done. \"<i>No, I'm – I'm sorry, I didn't... I got ahead of myself again, I was being stupid, I was-</i>\"  You cut her off by placing a finger to her lips. She moans at your touch, though squeals at your intention. Do you congratulate her, or break the bad news?");
            //[Encourage][Abuse]
            MainScreen.simpleChoices(["Encourage", "Abuse", "", "", ""], [lottieLowMoraleWeightLossEncourage, lottieLowMoraleWeightLossAbuse, null, null, null]);
        }
        //[If Lost Weight since Last Encounter, Encouragement 30 - 70]
            ...
            //[Encourage][Abuse]
            MainScreen.simpleChoices(["Encourage", "Abuse", "", "", ""], [lottieMediumMoraleWeightLossEncourage, lottieMediumMoraleWeightLossAbuse, null, null, null]);
        }
        //[If Lost Weight since Last Encounter, Encouragement 70 – 100, convinced 'Bigger is Better']
            ...
            MainScreen.simpleChoices(["Encourage", "Abuse", "", "", ""], [lottieHighMoraleWeightLossFATTYLOVESIT, lottieHighMoraleWeightLossFATTYLOVESABUSE, null, null, null]);
        }
        //[If Lost Weight since Last Encounter, Encouragement 70 - 100]
            ...
            DisplayText("Her ears perk up at your eagerness to talk; she seems to be pretty excited about something. You ask her what's on her mind.  \"<i>That obvious, huh?</i>\" Lottie looks up at you and grins. \"<i>Y'see babe, since the last time we met, I've actually – I've lost weight!</i>\" She stands up, pumping her fist in the air and wiggling her plump ass. \"<i>I mean, I know it's not a big difference, but it's still a result! This is amazing!</i>\" She's clearly pleased with herself. You place a hand on her shoulder, and she beams at you. Do you encourage her, or break the bad news?\n\n");
            MainScreen.simpleChoices(["Encourage", "Abuse", "", "", ""], [lottieHighMoraleWeightLossEncourage, lottieHighMoraleWeightLossAbuse, null, null, null]);
        }
        //[If Encouragement = 0, Figure = 0, first time]
            ...
            comedy1 = null;
            if (User.settings.silly()) comedy1 = lottieFatLoserCulminationLeaveComedyChoice;
            MainScreen.simpleChoices(["To Fuck", "To Humiliate", "To Leave", "", ""], [lottieFatLoserCulminationFuckChoice, lottieFatLoserCulminationHumiliationChoice, comedy1, null, null]);
        }
        //[If Encouragement =100, Figure =0, first time]
            ...
            comedy1 = null;
            if (User.settings.silly()) comedy1 = ewLottieNo;
            MainScreen.simpleChoices(["Love Self", "StickWithIt", "EwLottieNo", "", ""], [lottieHighMoraleFatLoveSelfOneTimeEvent, lottieHighMoraleFatYoullGetThereOneTimeEvent, comedy1, null, null]);
        }
        //[If Encouragement = 0, Figure = 100, first time]
            ...
            DisplayText("You might as well embrace the new member of your group.");
            //[Threesome][Elle][Oh I Get It]
            comedy1 = null;
            if (User.settings.silly()) comedy1 = charlottesWebComedyBullshit;
        ...
        //[If Neither, Encouragement 30 - 70]
            ...
            player.stats.lust += 2;
            DisplayText("Without much to go on, you can't really comment on her progress.");
            return { next: Scenes.camp.returnToCampUseOneHour };
        }
        //[If Neither, Encouragement 70 - 100]
            ...
            DisplayText("<b>SHIT BUGGED OUT SON.</b>");
            return { next: Scenes.camp.returnToCampUseOneHour };
        }
    ...
    //[If Encourage]
    private firstOralReactionChooseEncourage() {
        DisplaySprite(36);
        DisplayText().clear();
    ...
    //[If Abuse]
    private firstOralReactionChooseAbuse() {
        DisplaySprite(36);
        DisplayText().clear();
    ...
    //[If Encourage]
    private firstAnalReactionChooseEncourage() {
        DisplaySprite(36);
        DisplayText().clear();
    ...
    //[If Abuse]
    private firstAnalReactionChooseAbuse() {
        DisplaySprite(36);
        DisplayText().clear();
    ...
    //[If Encourage]
    private lottieLowMoraleWeightGainEncourage() {
        DisplaySprite(36);
        DisplayText().clear();
        ...
        DisplayText("<b>...</b>After a good workout, you decide to return back to camp, exhausted.");
        //ENCOURAGEMENT +2
        lottieMorale(3);
        fatigue(10);
        return { next: Scenes.camp.returnToCampUseOneHour };
    ...
    //[If Abuse]
    private lottieLowMorealeWeightGainAbuse() {
        DisplaySprite(36);
        DisplayText().clear();
    ...
    //[If Encourage]
    private lottieMediumMoraleWeightGainEncourage() {
        DisplaySprite(36);
        DisplayText().clear();
    ...
    //If Abuse]
    private lottieMediumMorealeWeightGainAbuse() {
        DisplaySprite(36);
        DisplayText().clear();
        DisplayText("You tell Lottie your methods never fail, and if something happened that wasn't supposed to, it's her that's at fault. She looks up at you, shocked at your outburst. Lost for words, her eyes begin to well up. \"<i>I... god, I'm so stupid! Of course it was me! It's always me!</i>\" She cries softly to herself, pushing your hand off her shoulder and burying her head in her arms. \"<i>I wasted your time! I'm... I'm...</i>\" Before she finishes her sentence, you walk away from the cafe, content with what you've done. You don't stop hearing her whimpering voice until you leave the gym, returning back to camp – strangely satisfied.");
        lottieMorale(-10);
    ...
    //[If Encourage]
    private lottieHighMoraleWeightGainEncourage() {
        DisplaySprite(36);
        DisplayText().clear();
    ...
    //[If Abuse]
    private lottieHighMoraleWeightGainAbuse() {
        DisplaySprite(36);
        DisplayText().clear();
        ...
        DisplayText("Deciding you're done here, you return back to camp – strangely satisfied.");
        lottieMorale(-10);
        return { next: Scenes.camp.returnToCampUseOneHour };
    }

    //[If Encourage]
    private lottieHighMoraleWeightGainFATTYLOVESIT() {
        DisplaySprite(36);
        DisplayText().clear();
    ...
    //[If Abuse]
    private lottieHighMoraleWeightGainFATTYLOVESABUSE() {
        DisplaySprite(36);
        DisplayText().clear();
    ...
    //[If Encourage]
    private lottieLowMoraleWeightLossEncourage() {
        DisplaySprite(36);
        DisplayText().clear();
    ...
    //[If Abuse]f
    private lottieLowMoraleWeightLossAbuse() {
        DisplaySprite(36);
        DisplayText().clear();
    ...
    //[If Encourage]
    private lottieMediumMoraleWeightLossEncourage() {
        DisplaySprite(36);
        DisplayText().clear();
    ...
    //[If Abuse]
    private lottieMediumMoraleWeightLossAbuse() {
        DisplaySprite(36);
        DisplayText().clear();
        DisplayText("You sneer at Lottie, sternly reminding her that she's nowhere near her goal the way she is now – and a few pounds isn't going to change anything. In fact, she probably just measured herself in the morning, and it's likely she hasn't lost any weight at all, being the pig she is. Lottie doesn't seem to have anything to say. She stares at you, wondering where this all came from. After a few seconds of silence, she breaks into sobs, pushing herself out of her chair and running away from the gym. You know she'll come back, though. Deciding you're done here, you return back to camp – strangely satisfied.\n\n");
        lottieMorale(-10);
    ...
    //[If Encourage]
    private lottieHighMoraleWeightLossEncourage() {
        DisplaySprite(36);
        DisplayText().clear();
    ...
    //[If Abuse]
    private lottieHighMoraleWeightLossAbuse() {
        DisplaySprite(36);
        DisplayText().clear();
    ...
    //[If Encourage]
    private lottieHighMoraleWeightLossFATTYLOVESIT() {
        DisplaySprite(36);
        DisplayText().clear();
    ...
    //[If Abuse]
    private lottieHighMoraleWeightLossFATTYLOVESABUSE() {
        DisplaySprite(36);
        DisplayText().clear();
    ...
    //[Love Yourself]
    private lottieHighMoraleFatLoveSelfOneTimeEvent() {
        DisplaySprite(36);
        DisplayText().clear();
        let x: number = player.cockThatFits(40);
        if (x < 0) x = 0;
        ...
        DisplayText("As you feel yourself about to explode, you keep a tight grip on her hips before thrusting forward, filling her insides with your hot spunk. Lottie trembles in orgasm, the feeling of your cum inside of her sending her over the edge.");
        if (player.torso.cocks.count > 1) {
            DisplayText("  Your other cock");
            if (player.torso.cocks.count === 2) DisplayText(" does");
            else DisplayText("s do");
            DisplayText(" not disappoint, spurting your essence all over the pig-girl.");
        }
        DisplayText("  You both lay on the mat, panting in ecstasy while your " + Desc.Cock.describeCock(player, x) + " continues to ejaculate inside her.\n\n");

        DisplayText("A while later, you've both said your goodbyes for the night.");
        if (User.settings.silly()) DisplayText(" Before you leave, you make sure to head out to the cafe to buy yourself a bacon shake. You love bacon shakes.\n\n");
        else DisplayText("\n\n");

        //[Unlocks Cowgirl]
        DisplayText("(<i>'Cowgirl' unlocked from Lottie's sex menu.</i>)");
        player.orgasm();
        player.stats.sens += -3;
        player.stats.cor += -1;
    ...
    //[You'll get there]
    private lottieHighMoraleFatYoullGetThereOneTimeEvent() {
        DisplaySprite(36);
        DisplayText().clear();
        let x: number = player.cockThatFits(40);
        if (x < 0) x = 0;
        ...
        DisplayText("You follow Lottie out into the pool area, waiting as she walks into the dressing room to get changed. When she comes out however, you're surprised to find that she hasn't changed into anything – in fact, she's removed all her clothes entirely!\n\n");

        DisplayText("The nude pig-girl stands in front of you, turning red at your hungry gaze. \"<i>I'm not comfortable with my body, but... if self confidence is the key to getting what I want, then... will you fuck me, " + player.short + "?</i>\"\n\n");

        DisplayText("In only seconds flat you remove your clothes to jump the piggy, whipping out your " + Desc.Cock.describeCock(player, x) + " and pushing it against Lottie's wet slit, the pig-girl lying on her back while her legs rest upon your shoulders. With a hearty thrust you drive yourself into her, eliciting a loud moan while you pork her insides with your meat. Her thick thighs slap against you as you bend over to take a lustful grab at her melons, sucking at her nipple and pinching the other, her sweet taste enveloping your mouth. Lottie buries you in her cleavage, yelping as you piledrive her tight pussy into submission – but you've got more in store. Flipping her over, Lottie lies face down with her soft butt jutting up in the air, your persistent fucking driving her wild. You continue gripping onto her tits as you pound her flesh, every thrust accompanied by her pleasured grunts. Back and forth you mount her until the familiar sensation in your loins signal your release. You increase your pace, pounding her pussy until you pull your head back and ejaculate inside her inner walls, slamming in harder every few seconds with each burst.");
        if (player.torso.cocks.count > 1) {
            DisplayText("Your other cock");
            if (player.torso.cocks.count === 2) DisplayText(" twitches ");
            else DisplayText("s twitch ");
            DisplayText("in ecstasy, releasing all over her plump butt.");
        }
        DisplayText("\n\n");

        DisplayText("Sweaty and panting, Lottie lies on the floor, exhausted and full of your spunk. You slap her jiggling ass before walking out of the pool area, making sure to grab a ");
        if (User.settings.silly()) DisplayText("bacon ");
        DisplayText("shake on the way out.");
        if (User.settings.silly()) DisplayText("  They're fucking delicious.");
        DisplayText("\n\n");
        //[Unlocks Quickie]
        DisplayText("(<i>'Quickie' has been unlocked from Lottie's sex menu.</i>)");
    ...
    private fuckElleInsteadOfThatFatSlutFirstTime() {
        DisplaySprite(36);
        DisplayText().clear();
        let x: number = player.cockThatFits(40);
        if (x < 0) x = 0;
    ...
    private lottieSuccessOneShot() {
        ...
        if (player.tone > 30 || player.thickness < 70) {
            ...
            DisplayText("Lottie gently asks for you to lie down on your back, to which you comply quite easily. You're comfortable, folding your arms behind your head to give you leverage as you watch your lover walk slowly to you, removing her shorts. Lottie turns around, her thick thighs straddling you as she sits down on your groin with her back facing your direction, giving you a perfect view of her curvy body and plump ass. Your " + Desc.Cock.describeCock(player, x) + " nestles gently between her rosy cheeks, pulsing with lustful intent as the girl grabs her hips and begins to pleasure you through her ass. The silence of the room breaks at your fevered pants and grunts as Lottie's skillful assjob sends you through fits of ecstasy, every curve of her bottom jiggling against the heat of your member. Her pace picks up while she bounces upon you, squeezing and squashing her perfect ass until one final lift in the air is met with her plunging your length up into her hot interior. The feeling of your molten cock inside her immediately sends Lottie into fits of orgasmic bliss, the intense heat filling the pig-girl with pleasure. Her attempts to continue are thwarted by your " + Desc.Cock.describeCock(player, x) + " scraping against her inner walls, her thick thighs clenched together while she slowly moves up and down upon you. Deciding to take things into your own hands, you thrust upwards, almost pushing the girl off with your strength and causing Lottie to squeal as you begin pounding her irresistible hole. Her lush buttcheeks jiggle with every movement you make, the rising and falling of her body moving in accordance to your own will. You pump in and out, keeping a steady rhythm inside the hot and bothered woman.\n\n");

            DisplayText("Inevitably, you can feel your release approaching, and with one final push you grab a hold of the pig-girl's tight ass, slamming against her and pumping your spunk directly into her clenched pussy, your " + Desc.Cock.describeCock(player, x) + " twitching as ropes of cum coat her insides. Lottie yelps, collapsing on the floor and full of your hot seed. You wipe your cock along her ass cleavage, leaving her as a sweaty, cum-filled mess on the floor. Walking out, you make sure to grab a ");
            if (User.settings.silly()) DisplayText("bacon ");
            DisplayText("shake from the cafe. They're fantastic.\n\n");
            //[Unlocks Reverse Cowgirl]
            Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00320] = 1;
            Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00298]++;
            DisplayText("(<i>'Reverse Cowgirl'</i> unlocked in Lottie's sex menu</i>)");
        }
        //[Otherwise]
        else {
            DisplayText("You walk into the yoga area and are met with the smell of sweat and flowers pervading the room. You look around, unable to find your lover until –THUNK! A heavy weight is brought down upon your head, pulling you down to the ground and away from consciousness as everything fades to black.\n\n");
    ...
    //[Leave]
    private leaveLottieInsteadOfYogaSex() {
        DisplaySprite(36);
        DisplayText().clear();
        DisplayText("As excited as you are, your mind is telling you to leave. In blissful ignorance, you walk out the door of the cafe, through the pool area, into the main gym and out the main entrance. Once outside, you figure it's as good a time as any to take a friendly stroll back to the camp.\n\n");
        DisplayText("As you head home, you have the strangest feeling you've missed out on incredibly sexy sex.");
        return { next: Scenes.camp.returnToCampUseOneHour };
    }

    //[Exercise]
    private lottieExercise() {
        DisplaySprite(36);
        DisplayText().clear();
        DisplayText("You tell Lottie it's time to hit the gym, and she responds ");
        if (lottieMorale() >= 66) DisplayText("with an enthusiastic nod");
        else if (lottieMorale() >= 33) DisplayText("with a nod in agreement");
        else DisplayText("by slowly looking up at you and nodding her head");
        DisplayText(". \"<i>Where do you wanna go today, " + Desc.Gender.mf(player, "guy", "ma'am") + "?</i>\" She responds, as you both walk through the cafe's entranceway leading to the rest of the gym. A wall of various weights stands proud amongst the different citizens that make use of it. You spot several jogging tracks, and outside, a large, grassy field mostly occupied by centaurs looking for a nightly run - though other, bipedal gym members try to jog along with them without much success. A large, hidden door hides the entrance to the indoor pool and sauna area, along with the door to the candlelit yoga room. Where do you wish to work out?");
        //[Weights][Jogging][Yoga][Pool]
        //Pool only available when morale is high
        MainScreen.simpleChoices(["Weights", "Jogging", "Yoga", "Pool", ""], [lottieWeights, goJoggingWithLottie, lottieYoga, lottiePool, null]);
    }

    //[Weights]
    private lottieWeights() {
        DisplaySprite(36);
        DisplayText().clear();
        let NTR;
        ...
        if (lottieTone() <= 70) {
            ...
            //[Boast][Train]
            MainScreen.simpleChoices(["Boast", "Train", "NTR Lift", "", ""], [boastAtLottie, trainWithLottie, NTR, null, null]);
        }
        //[If Figure 70 - 100]
        else {
            ...
            //[Boast][Train]
            MainScreen.simpleChoices(["Boast", "Train", "", "", ""], [boastAtFitLottie, trainWithFitLottie, null, null, null]);
        }
    }


    //[Boast]
    private boastAtLottie() {
        DisplaySprite(36);
        DisplayText().clear();
        DisplayText("You laugh as you pick up one of the larger ones and pump mercilessly, looking at her expectantly as if she had been failing out of laziness. She tries harder to pull the heavy object over her head, but eventually she begins to shake and drops it, landing in the sand with a ");
        if (User.settings.silly()) DisplayText("large *pomf =3*");
        else DisplayText("loud clonk");
        DisplayText(". She looks up at you and sniffles, but you just grunt in response, disappointed with her efforts. She sighs and tries her best to do it again.\n\n");

        DisplayText("After a few more attempts at the weights, she eventually buckles under the pressure and gives up, walking home by herself. You snort and continue a few more well-earned reps before dropping down and recharging for the night.");
        fatigue(10);
        player.stats.str += .25;
        lottieMorale(-3);
        return { next: Scenes.camp.returnToCampUseOneHour };
    ...
    private trainWithLottie() {
        ...
        DisplayText("After a few more attempts at the weights, you both decide you've had enough, going your separate ways and waving as you leave the gym.");
        player.stats.str += .1;
        lottieMorale(2);
        lottieTone(2);
        fatigue(10);
    ...
    private goJoggingWithLottie() {
        ...
        if (lottieTone() <= 70) {
            ...
            //[Sprint][Jog]
            MainScreen.simpleChoices(["Sprint", "Jog", "", "", ""], [sprintAndLeaveLottieBehind, jogWithLottie, null, null, null]);
        }
        //[If Figure 70 - 100]
        else {
            ...
            //[Sprint][Run]
            MainScreen.simpleChoices(["Sprint", "Jog", "", "", ""], [sprintAndLeaveFitLottieBehind, jogWithFitLottie, null, null, null]);
        }
    }
    //[Sprint]
    private sprintAndLeaveLottieBehind() {
        DisplaySprite(36);
        DisplayText().clear();
        ...
        DisplayText("A few rounds later, you've exhausted yourself so you decide to return home. You notice that Lottie's nowhere in sight, but it's her loss - not yours.");
        player.stats.tou += .2;
        player.stats.spe += .25;
        lottieMorale(-3);
        fatigue(10);
    ...
    //[Sprint]
    private sprintAndLeaveFitLottieBehind() {
        DisplaySprite(36);
        DisplayText().clear();
        ...
        DisplayText("A few rounds later and you've exhausted yourself, so you decide to return home. You notice that Lottie's nowhere in sight, but it's her loss - not yours.");
        player.stats.tou += .2;
        player.stats.spe += .25;
        lottieMorale(-3);
        fatigue(10);
    ...
    private lottieYoga() {
        ...
        //[Alone] [Together]
        MainScreen.simpleChoices(["Alone", "Together", "", "", ""], [lottieYogaAlone, togetherLottieYoga, null, null, null]);
    }

    //[Alone]
    private lottieYogaAlone() {
        DisplaySprite(36);
        DisplayText().clear();
        ...
        DisplayText("You continue to take on the vaguely sexual positions of the athletic yoga teacher, the class ending before you know it. Your head strangely dizzy from the smell of sweat and pheromones, you walk home by yourself, completely forgetting about Lottie.");
        DisplayText(player.modThickness(30, 1));
        DisplayText(player.modTone(60, 1));
        lottieMorale(-2);
        fatigue(5);
        return { next: Scenes.camp.returnToCampUseOneHour };
    }
    ...
    private togetherLottieYoga() {
        ...
        if (player.torso.cocks.count > 1) DisplayText("s");
        DisplayText(", while you secretly relish in the fact that you're at the back of the class. \"<i>Was... was that because of me?</i>\" Lottie whispers, tactlessly pointing directly at your " + Desc.Cock.describeMultiCockShort(player) + ". You nod in response, Lottie giggling at your aroused reaction to her body. She playfully slaps your stiff length, and returns to her stretches. You groan and move towards an easier, less-exposed position.\n\n");

        DisplayText("The class ends a while later, Lottie continuing to tease you as you walk out of the gym. You note in your head that you'll get her back later, but for now you're exhausted.");
        lottieMorale(4);
        lottieTone(1);
        fatigue(5);
    ...
    private lottiePool() {
        DisplaySprite(36);
        DisplayText().clear();
        DisplayText("You often find Lottie near the pool, so it's only natural to assume she might have a knack for it. You mention swimming to the pig-girl, her ears perking up at the idea of going in the water. \"<i>S-sure! I'll go get ready!</i>\"  Wondering what she might've meant, you wait by the side of the pool, your legs dipping into the cool, blue water. Soon after, Lottie runs out into the area, wearing the daring, black bikini she wore the first time you had met. Her jiggling breasts squish past the fabric, while her chubby butt and thighs hug tightly against the bottom piece. Feeling a familiar stretching in your groin, you look down to find your " + Desc.Cock.describeMultiCockShort(player) + " at full attention, hardly subtle against your " + player.inventory.equipment.armor.displayName + ". A thought passes your mind to cover up, but before you can begin to reflect on the idea the bouncing girl sits her plump ass next to you. You wonder if it's easier to jump straight into the pool or to wait it out – hell, she might even be flattered by your reaction.");
        //[Jump][Wait]
        MainScreen.simpleChoices(["Jump", "Wait", "", "", ""], [jumpInPoolBeforeLottieSeesYourWingWang, waitAndShowLottieYourPoolBoner, null, null, null]);
    }

    //[Jump]
    private jumpInPoolBeforeLottieSeesYourWingWang() {
        DisplaySprite(36);
        DisplayText("", true);
        DisplayText("You can't risk it, so you decide to take a plunge into the water – it's freezing at first, but you quickly warm up. Lottie takes your avoidance to heart and looks visibly discouraged, though you wonder what her reaction would've been if your cockrocket");
        if (player.torso.cocks.count > 1) DisplayText("s");
        ...
        DisplayText(player.modTone(100, 2));
    ...
    //[Wait]
    private waitAndShowLottieYourPoolBoner() {
        DisplaySprite(36);
        DisplayText().clear();
        DisplayText("You figure it can't do much harm to let " + Desc.Cock.describeMultiCockSimpleOne(player) + " fly, so you decide to leave it be, even leaning back so as to accentuate the hardening length. It doesn't take long for Lottie to notice, letting out a little gasp as she observes your ");
        if (player.torso.cocks.sort(Cock.LargestCockArea)[0].area >= 15) DisplayText("pride and joy");
        else if (player.torso.cocks.sort(Cock.LargestCockArea)[0].area < 5) DisplayText("secret shame");
        else DisplayText("painfully average prick");
        DisplayText(". You almost experience a tinge of regret from the now incredibly awkward silence, but it's thrown out the window as soon as you feel a soft hand grab your throbbing member. Looking up, Lottie's face has reddened considerably, but her curiosity overrides her shame as she begins to slowly massage you to full mast. \"<i>I... I did this?</i>\" she asks, facing you with large, wondering eyes. You grin and nod, taking her hand and helping her stroke it further, only to be met with a slap on your wrist as she removes her grip. You look up, frowning as Lottie laughs at the situation. \"<i>I'm – it's nice to know that someone likes me.</i>\" She stutters, smiling timidly. \"<i>But, um, we're here to exercise, " + Desc.Gender.mf(player, "big guy", "babe") + ". We should do that.</i>\" You groan at the sudden cockblock, but you can't really argue. She takes your hand and leads you into the water, but not before you playfully grab a handful of her plush ass.\n\n");

        DisplayText("A while later and you've both finished off several laps of the pool, exhausted from your progress. Lottie flashes you a large smile before you part ways, leaving you still somewhat aroused from the experience.");
        DisplayText(player.modTone(100, 2));
        player.stats.lust += 80;
        lottieTone(2);
        lottieMorale(2);
    ...
    public giveLottieAnItem() {
        ...
        if (player.inventory.items.has(consumables.GROPLUS) && player.statusAffects.has(StatusAffectType.DungeonShutDown)) gro = giveLottieGroPlus;
        ...
        if (Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00321] === 0) {
            ...
            //[LaBova][Gro+][Reducto]
            MainScreen.simpleChoices(["LaBova", "Gro Plus", "Reducto", "", "Back"], [bova, gro, reducto, null, encounterLottie]);
        ...
        //[Give Item – Repeat Encounter]
            ...
            let sammich;
            if (User.settings.silly()) sammich = hamSammitchTimeBIATCH;
    ...
    private feedLottieAWholeCafe() {
        ...
        DisplayText("You secretly know it will.");
        lottieTone(-5);
    ...
    public lottieFuck() {
        ...
        //[If Encouragement 30 – 60]
            ...
            DisplayText("You ask Lottie if she's keen for a little 'extracurricular workout', earning a giggle and a tilt of the head from the pig-girl. \"<i>What do you mean, " + Desc.Gender.mf(player, "big guy", "babe") + "?</i>\" she asks. You sigh and stand up, revealing your hardening member");
            if (player.torso.cocks.count > 1) DisplayText("s");
            DisplayText(". \"<i>I... oh.</i>\" Taking Lottie's hand, you quickly lead her out of the cafe and into the pool area, looking for an empty sauna to sneak into. Considering that the gym's pool is only used by a few semi-aquatic humanoids and the odd ");
            if (User.settings.silly()) DisplayText("street ");
            DisplayText("shark, your search isn't that difficult. Hopping into the toasty room, you quickly undress, Lottie still standing by the doorway. \"<i>H-hey, y'know, I didn't actually agree on this or a-anything...</i>\" she stutters, nervously fiddling with her top. Without missing a beat, you walk towards her and plant a kiss on her full lips, tasting her sweetness as you probe her mouth with your eager tongue. The girl melts underneath your touch, passionately returning your erotic display while you take the chance to slip Lottie out of her top, reaching into her shorts to feel between her creamy hips. She moans as you slowly push two fingers inside her moist wetness, simultaneously pulling out your " + Desc.Cock.describeCock(player, x) + " and pushing her back against the wall. Lottie begins to jerk you off slowly, massaging the length of your " + Desc.Cock.describeCock(player, x) + " with one hand while the other takes care at gently caressing your tip. You start to thrust into her hands on reflex while you claim her mouth, the feel of her sugary tongue wrapped around yours sending you both into fits of needy lust. You use your hands to reach around and squeeze her chubby derriere, lightly slapping it whilst a stray hand heads towards her source of hot desire. You slick your hand against her wetness, Lottie moaning in your mouth while you work towards bringing her to an early climax. However, the sudden roughness of your hands against her mound only causes a quick jerk to your " + Desc.Cock.describeCock(player, x) + ", her soft hands tightening against your shaft. You groan in response, removing yourself from the lips of her mouth and instead placing your own against her erect nipple, sucking forcefully. The pig-slut grabs your head and pushes you towards her heaving chest, relinquishing her grip on your member to hug you closer.\n\n");

            DisplayText("Not wanting to be left unsatisfied, you tell her you've decided you're going to take her for a vigorous work out. She can only pant with anticipation.");
            //[Anal][Oral]
            MainScreen.simpleChoices(["Anal", "Oral", "Tentacle", "", "30Minute"], [analLottieMilkshake, getSuckedOffByAHamSandwich, tentacle, null, thirtyMinute]);
        }
        //[If Encouragement 60 – 100]
        else {
            DisplayText("You take Lottie's hand in your own, causing a small blush from her in response. \"<i>" + player.short + "?</i>\" she begins, only to be cut off by a small peck to the lips. Lottie reddens, giggling slightly and gazing around for any onlookers. \"<i>What's brought this on, babe?</i>\" She beams, twirling and caressing your fingers among her own. You tell her that you thought she just needed a little more 'special exercise' to help get the blood flowing – making sure to emphasise \"<i>special exercise</i>\" with exaggerated air quotes. She playfully pushes away your idiotic gesture and takes you out of your seat, leading the way into the pool area and into one of the empty saunas. Locking the door behind you, she turns around to face you and your hardening crotch. \"<i>I think you're getting a little over excited, babe,</i>\" she purrs, her confidence getting the better of you as she takes a step closer, but you're the one in charge. Grabbing Lottie by the hips, you twirl her around to face the wall, grinding your hardening length");
            if (player.torso.cocks.count > 1) DisplayText("s");
            DisplayText(" against her plump asscheeks. \"<i>W-woah! I... well, okay...</i>\" she moans, reaching back to stroke your " + Desc.Cock.describeCock(player, x) + " through your clothing while you grab a handful of her rosy ass, slapping and squishing it against your palm. You continue to hump relentlessly against Lottie's tight shorts, occasionally reaching around to feel her hot mound, causing the odd squeal or moan to emanate from her pouting lips. Eventually deciding clothes are the only thing between Lottie's lewd figure and your own " + Desc.Cock.describeCock(player, x) + ", you simultaneously remove your outfit while ripping off her sweaty top, causing her breasts to spill out into your eager, groping hand. Despite your sudden stripping, Lottie immediately gets back into position, placing your cock in between her pillow-like asscheeks and moaning while you flick and tease her swollen nipples, rocking back and forth against you.\n\n");

            DisplayText("Lottie whimpers in between short, hot breaths, \"<i>You're driving me crazy here, babe...</i>\" You give her chubby ass a hard slap in agreement. Lottie squeals in glee, jerking your " + Desc.Cock.describeCock(player, x) + " as it pokes up through her ass. The sensation of your tip rubbing against her velvety skin drives you wild.\n\n");

            DisplayText("You turn Lottie around to face you, telling her you've decided you're going to take her for a vigorous work out. \"<i>I'll work you out, babe,</i>\" she moans in response.");
    ...
    //[Doggystyle]
    public lottieRepeatDoggieStyle() {
        DisplaySprite(36);
        DisplayText().clear();
        DisplayImage(images.showImage("lottie-fuck-her-doggie-style-repeat"));
        let x: number = player.cockThatFits(40);
        if (x < 0) x = 0;
        DisplayText("A few minutes later and you've taken your whore to the washrooms. Mirrors line the sides of the walls to allow members to observe how far they've gone and how far they still need to go. It's entirely empty for the night, though you make sure to lock the door just in case. You turn around to see Lottie cowering in the corner. She knows what you want – and you swear on Mareth's giant tree titties you'll get it. With her naked body on full display, Lottie quietly moves down to the ground, positioning herself on all fours with her butt facing towards you. She looks behind herself to see your erect " + Desc.Cock.describeCock(player, x) + " on full display, quickly squealing and looking away. You grin as you stroke yourself into fullness, walking over and placing yourself behind Lottie's full derriere. \"<i>" + player.short + "?</i>\" she gently sobs, \"<i>P-please don't do this.</i>\" With that, you grab her ample thighs and thrust forward, driving your " + Desc.Cock.describeCock(player, x) + " straight into her tense wetness.\n\n");
    ...
    //[Walkies]
    public lottieRepeatWalkies() {
        DisplaySprite(36);
        DisplayText().clear();
        DisplayImage(images.showImage("lottie-fuck-her-in-public-repeat"));
        let x: number = player.cockThatFits(40);
        if (x < 0) x = 0;
        ...
        if (!silly()) DisplayText("Heyyy, is she like *hic* your shex slave er shumthin?");
        else DisplayText("Heyyy, do I shmell bacon?  I fucking love bacon!  BACON BACON BACON!");
        ...
        if (User.settings.silly()) DisplayText("bacon ");
        DisplayText("shake.");
        if (User.settings.silly()) DisplayText("  You sure do love spit roasting a pig, but nothing can beat a bacon shake.");
        player.orgasm();
        player.stats.sens += -2;
        player.stats.cor += 1;
        return { next: Scenes.camp.returnToCampUseOneHour };
    ...
    //[Cowgirl]
    public lottieRepeatCowGirl() {
        DisplaySprite(36);
        DisplayText().clear();
        DisplayImage(images.showImage("lottie-fuck-her-cowgirl-repeat"));
        let x: number = player.cockThatFits(40);
        if (x < 0) x = 0;
        ...
        DisplayText("A while later, you've both said your goodbyes for the night. Before you leave, you make sure to head out to the cafe to buy yourself a ");
        if (User.settings.silly()) DisplayText("bacon ");
        DisplayText("shake.");
        if (User.settings.silly()) DisplayText("  You love bacon shakes.");
        player.orgasm();
        player.stats.sens += -2;
    ...
    public lottieRepeatQuickie() {
        ...
        DisplayText("The nude pig-girl stands in front of you, turning red at your hungry gaze. \"<i>Will you f-fuck my pussy, " + player.short + "?</i>\"\n\n");

        DisplayText("In moments flat you remove your clothes to jump the piggy, whipping out your " + Desc.Cock.describeCock(player, x) + " and pushing it against Lottie's wet slit, the pig-girl lying on her back while her legs rest upon your shoulders. With a hearty thrust you drive yourself into her, eliciting a loud moan while you pork her insides with your meat. Her thick thighs slap against you as you bend over to take a lustful grab at her melons, sucking at her nipple and pinching the other, her sweet taste enveloping your mouth. Lottie buries you in her cleavage, yelping as you piledrive her tight pussy into submission – but you've got more in store. Flipping her over, Lottie lies face down with her soft butt jutting up in the air, your persistent fucking driving her wild. You continue gripping onto her tits as you pound her flesh, every thrust accompanied by her pleasured grunts. Back and forth you mount her until the familiar sensation in your loins signal your release. You increase your pace, pounding her pussy until you pull your head back and ejaculate inside her inner walls, slamming in harder every few seconds with each burst.");
        if (player.torso.cocks.count > 1) {
            DisplayText("  Your other cock");
            if (player.torso.cocks.count === 2) DisplayText(" twitches ");
            else DisplayText("s twitch ");
    ...
    //[Netorare]
    public lottieRepeatSUPERNEATORARES() {
        DisplaySprite(36);
        DisplayText().clear();
        let x: number = player.cockThatFits(40);
        if (x < 0) x = 0;
        ...
        if (player.torso.cocks.count === 1) {
            DisplayText("begins to caress your " + Desc.Cock.describeCock(player, x) + " with two of her hands, while the other sets to work on massaging your ");
        ...
        else DisplayText("uses all four of her hands to handle " + Desc.Cock.describeMultiCockSimpleOne(player) + ", jerking each one with ease");
        ...
        DisplayText("  Elle follows suit, groaning in pleasure as she reaches her own peak, her hole tightening around your length and trapping the rest of your spunk inside. The spinnerets upon her shoulder pulse in orgasm, spurting sticky globs of what appears to be thread up into the air and back down onto the pig, spunk pooling around her cleavage. You both relax, letting the spider-girl down off of you to collapse on the floor. You bend down, looking at Lottie before shoving your " + Desc.Cock.describeCock(player, x) + " inside of her and letting one final spurt of your seed fill her up. \"<i>Satisfactory.</i>\" You mumble, redressing and heading out the door, making sure to grab a ");
        if (User.settings.silly()) DisplayText("bacon ");
        DisplayText("shake on the way out.");
        if (User.settings.silly()) DisplayText("  You only need one of those to satisfy you.");
        player.orgasm();
    ...
    //[Reverse Cowgirl]
    public lottieRepeatBackwardsAssCowgirl() {
        DisplaySprite(36);
        DisplayText().clear();
        let x: number = player.cockThatFits(40);
        if (x < 0) x = 0;
        DisplayText("You both walk into the yoga area, met with the smell of sweat and flowers pervading the room. Lottie gently asks for you to lie down on your back, to which you comply quite easily. You're comfortable, folding your arms behind your head to give you leverage as you watch your lover walk slowly to you. Lottie turns around, her thick thighs straddling you as she sits down on your groin with her back facing your direction, giving you  a perfect view of her curvy body and plump ass. Your " + Desc.Cock.describeCock(player, x) + " nestles gently between her rosy cheeks, pulsing with lustful intent as the girl grabs her hips and begins to pleasure you through her ass. The silence of the room breaks at your fevered pants and grunts as Lottie's skillful assjob sends you through fits of ecstasy, every curve of her bottom jiggling against the heat of your member. Her pace picks up while she bounces upon you, squeezing and squashing her perfect ass until one final lift in the air is met with her plunging your length up into her hot interior. The feeling of your molten cock inside her immediately sends Lottie into fits of orgasmic bliss, the intense heat filling the pig-girl with pleasure. Her attempts to continue are thwarted by your " + Desc.Cock.describeCock(player, x) + " scraping against her inner walls, her thick thighs clenched together while she slowly moves up and down upon you. Deciding to take things into your own hands, you thrust upwards, almost pushing the girl off with your strength and causing Lottie to squeal as you begin pounding her irresistible hole. Her lush buttcheeks jiggle with every movement you make, the rising and falling of her body moving in accordance to your own will. You pump in and out, keeping a steady rhythm inside the hot and bothered woman.\n\n");
    ...
    //[FemDom]
    public lottieRepeatFEMDOMFEMFDOM() {
        DisplaySprite(36);
        DisplayText().clear();
        let x: number = player.cockThatFits(40);
        if (x < 0) x = 0;
        DisplayText("You both walk into the yoga area and are met with the smell of sweat and flowers pervading the room. You look around, unable to find your lover until –THUNK! A heavy weight is brought down upon your head, pulling you down to the ground and away from consciousness as everything fades to black.\n\n");
        ...
        //[If Cum Production High]
            ...
            DisplayText("Unfortunately for her, you have far too much built up to stop now. With another thrust, your " + Desc.Cock.describeCock(player, x) + " pulses in delight, exploding in orgasm as ropes of spunk splatter upon Lottie's face and tits. The girl gasps, the sheer amount of your seed painting her white. \"<i>N-no, stop! Oh god!</i>\" she yelps, before her mouth is filled with your hot essence. She forces herself to swallow, coughing as you grin at the defeated slut. \"<i>O-okay, y-you win, " + player.short + ".</i>\" she looks up at you and smiles, before untying your binds and setting you free. \"<i>But you have to admit, this was pretty fun, right?</i>\" You give her a thumbs up before walking out the door, leaving her to clean your thick mess. On the way out, a milkshake vendor waves over to you – it's the guy from the cafe. You buy a ");
            if (User.settings.silly()) DisplayText("bacon ");
            DisplayText("shake before deciding to head home.");
            if (User.settings.silly()) DisplayText("  You fucking love bacon shakes.");
        }
    ...
    public hugTheShitOutOfYourHam() {
        ...
        if (lottieMorale() <= 30 && player.stats.cor >= 75) {
            DisplayText("You tell Lottie to wake up, slapping her face into consciousness. She yelps at the pain, a strand of drool flying as she jerks awake from her fantasy. She sniffs quietly, keeping her thoughts to herself. \"<i>What kind of useless slut daydreams?</i>\" You smirk.\n\n");
            DisplayText("The kind that wishes for the day that her life will change, Lottie thinks back.");
            lottieMorale(-4);
        }
        return { next: Scenes.camp.returnToCampUseOneHour };
    }
    ...
    //vaginal sex scene.
    public tentaDickLottieButtFuckbuttFuckButtFUCKBUTTFUCKBUTTFUCK() {
        DisplayText().clear();
        DisplayImage(images.showImage("lottie-fuck-her-with-tentacle-dick"));
        let x: number = -1;
        let y: number = -1;
        let z: number = -1;
        let zz: number = -1;
        ...
        DisplayText("It isn't long before you've taken Lottie out to the shed behind the gym - due to the amount of visitors regularly coming in and out of the gym itself, the equipment seems to have been mostly removed - leaving bare shelves and a large concrete floor covered in spare gym mats.\n\n");

        DisplayText("The porcine girl takes the initiative as soon as you enter the room, stripping down and planting her cushiony ass upon the pile of mats in front of you.  \"<i>What're we gonna do in here, babe?</i>\" she asks, turning over onto her soft belly and rolling around, covering herself in an assortment of old, thin mattresses.  \"<i>Heh, I'm a pig in a blanket,</i>\" she squeaks, unaware of you slowly pacing towards her, your hand firmly stroking your " + Desc.Cock.describeMultiCockShort(player) + ".  There's a time for games, you think, and a time for unleashing your myriad of undulating vine-beasts upon your curious pink slut.\n\n");
        DisplayText("Removing your " + player.inventory.equipment.armor.displayName + ", you let your " + Desc.Cock.describeMultiCockShort(player) + " hang loose, a single vine stretching outwards and slapping Lottie upon the ass to remind you of why you're here.  She squeals, trapped underneath the heap of mattresses and doing little to help the situation she's found herself in.  \"<i>H-hey, uh, I've kinda found myself in a little pickle, " + Desc.Gender.mf(player, "big guy", "hon") + " - could you give me a hand?</i>\" Lottie chuckles, squirming around in an attempt to loosen herself from the pile, but inevitably failing.  You walk closer towards her, shifting away a few mattresses here and there to free her limbs.  She smiles apologetically as you lift up the last mattress, crawling back out of the pile only to squeak when you bring down the smallest - albeit, heaviest - mat onto her back, preventing her from moving any further and causing her face to be buried against the mattress below her, the girl's plump ass stretched directly up into the air.  The awkward position restricts the movement of anything outside of her forearms and neck, and the majority of the weight upon her back pushes her chest into her knees, disallowing her from stretching outwards.  Lottie turns her head to the side, giving her time to breathe and assess the situation.  She notices you're stark naked, her eyes drawn down to the " + Desc.Cock.describeMultiCockShort(player) + " laid before her.");
        if (lottieMorale() <= 30) DisplayText("  She gives you a gloomy stare, before turning her head back down and plunging it into the mattress, already accepting her defeat");
        else DisplayText("  She raises an eyebrow, though doesn't probe any further");
        DisplayText(".\n\n");

        DisplayText("You grasp the base of your " + Desc.Cock.describeCock(player, x) + " as it stretches forward, gently caressing Lottie's face before exploring the rest of her body, idly stroking the smooth curves of her hips or brushing against her hardening nipples.  The pig-girl moans quietly from the light petting while the feeling of her soft skin excites your hardening length, reminding you of your true intentions.  You walk behind her, obscuring yourself from Lottie's peripheral vision and crouching down to inspect the view of her perfectly rounded ass pointed conveniently into the air.  " + Desc.Cock.describeMultiCockSimpleOne(player, true) + " twitches in agitation, eager to plow the chubby slut trapped in front of you - and you aren't about to disappoint.\n\n");

        //[if a single, lonely tentapenis]
        DisplayText("You firmly squeeze your ");
        if (player.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE) > 1) DisplayText("first ")).length;
        ...
        if (y >= 0) {
            DisplayText("Seeing that you have extra junk left to be used, and she has some vacant holes, you quickly remedy this problematic situation; guiding your " + Desc.Cock.describeCock(player, y) + " toward her moist butthole, you press the tip of your tree-like beast against her plush buttocks, literally stabbing her flesh with your penile flora.  The butt-slut doesn't seem to care: if anything, she squirms all the more, visibly aroused.  Her ass seems to stretch on its own as it braces itself for the imminent butt-devastation.  You welcome Lottie's efforts to comply to your anal needs and with a single thrust, you push ");
            if (player.cockArea(y) <= 80) DisplayText("the entirety of your " + Desc.Cock.describeCock(player, y));
            else DisplayText("as many inches of your " + Desc.Cock.describeCock(player, y));
            ...
            if (z >= 0) {
                DisplayText("Her constant moaning and screaming reminds you that one of her holes at least isn't being put to use.  Her eyes widen in awe as she sees your " + Desc.Cock.describeCock(player, z) + " inexorably moving to her waiting mouth, her lips forming a wide O as if to greet your tentacle pecker.  Without a word, you plunge your beast in her mouth, burying your prick until she's about to choke.  Fortunately she's used to blowjobs, and you have no trouble fitting a decent amount of inches down her throat.  You can see her neck bulge from the obscene intrusion, and the tip of your " + Desc.Cock.describeCock(player, z) + " swells until her windpipe looks ready to explode.  She can't say anything, but the way her eyes are staring at you tell you more than anything about how she feels.");
                if (lottieMorale() < 40) DisplayText("  This pig slut really loves to be used like that!");
                else DisplayText("  Your pig lover really loves what you're doing to her!");
                ...
                if (zz >= 0) {
                    DisplayText("Surely her twat must have some room left for more cock?  Before you even think of it, your free tentacle prick is already squirming its way to the piggy's love-tunnel, trying to fit as its companion ruthlessly rams her fuckhole.  You feel her whole body clench as you press the tip of your " + Desc.Cock.describeCock(player, zz) + " between her netherlips, stretching them beyond their normal capacity as you keep pushing forward.  If her mouth wasn't so full of dick, you'd be sure she would scream like a harpy in heat.  Once the tip manages to get through her tight vaginal entrance, the rest easily follows, and you effortlessly slide your dickflesh down her now-gaping cunt.  You synchronize your pricks penetrating her cuntwardly, literally ravaging her pussy as you pork her insides with your genitals.\n\n");
                    //[if another extra cock]//to be added to standard text
                    if (zzz >= 0) {
                        DisplayText("Since her dripping twat has been able to take some extra dick, why not try her backside?  Your " + Desc.Cock.describeCock(player, zzz) + " wriggles joyfully as it wraps around Lottie's plump rump, ready to test her anal capacity.  Fortunately, its rubbery texture allows it to be compressed as it grinds against your " + Desc.Cock.describeCock(player, y) + ", sliding toward the pig-girl's wet anus.  Your plant-like dong bounces back against her entrance numerous times, but you know she is being stretched wider at every attempt.  You don't know how many times you poke her pucker with the erect tip of your " + Desc.Cock.describeCock(player, zzz) + ", and you can't imagine how the teasing stimulation feels for the poor prisoner of your tentacle embrace.  At last, with a brutal thrust, your flexible dong somehow manages to clear a path for additional butt-penetration.  Your twin erect pricks are incredibly squeezed against her tight anal walls and her body is about to be torn in half from all the rods impaling her from many directions at once.  That doesn't prevent you from thrusting back and forth very carefully through her colon, never completely pulling out.\n\n");
                        //[if another extra cock]//to be added to standard text
                        if (zzzz >= 0) {
                            DisplayText("No matter how hard you try, you can't fit any more cock in her toy-like body. Your " + Desc.Cock.describeCock(player, zzzz) + " is quivering in frustration, until you notice her fleshy orbs bouncing up and down repeatedly on her chest as you ruthlessly pound her from every hole.  This might be the right occasion to get a good titfuck: your " + Desc.Cock.describeCock(player, zzzz) + " slowly slithers along her sweaty body and slides between her sizable fuckpillows; although the sensation isn't as hot and amazingly rough as the sharp tingles of pleasure received from the dongs pounding her ass, your rod is being subtly tickled and teased by her supple flesh.  Already aroused from the thorough multi-penetration, you start dripping pre-cum all over her torso, smearing her whole body with your sap.  This makes her breasts all the more slick and slippery, and needless to say, the sensation isn't lost on your " + Desc.Cock.describeCock(player, zzzz) + "; it seems to grow even bigger and becomes pinkish, the sudden color change betraying its arousal. You keep fucking her boobs until they are literally glistening from all the juices you've spilled on them.\n\n");

                            //[if another extra cock]//to be added to standard text
                            if (zzzzz >= 0) {
                                DisplayText("As you idly pump Lottie's body with all your junk, you mentally count all the extra ways her body could be used, since you have some tentacle dong left.  Mouth, cunt, ass, tits... wait!  Her hands are still free!  By the way they are twitching convulsively, you can clearly tell they are aching for some dick to handle.  You graciously comply to her cocky needs and shove your " + Desc.Cock.describeCock(player, zzzzz) + " against her palm, coating it with your pre-cum.  She instinctively clings to your rod of flesh as if her life depended on it and starts jerking you off, ferociously rubbing your dickflesh and using a mix of her sweat and your pre-cum as lube.");
                                //[if another extra cock]
                                if (zzzzzz >= 0) DisplayText("  Seeing the effect your junk had on her, you move your " + Desc.Cock.describeCock(player, zzzzzz) + " to her remaining hand and she grips it likewise.");
                                DisplayText("  She keeps stroking you in a compulsive way, her hands clearly acting under the effect of her tremendous lust.");
                                DisplayText("\n\n");
                                //[if another extra cock]//to be added to standard text
                                if (zzzzzzz >= 0) {
                                    DisplayText("With another cock left unsatisfied, you place your thick, tentacled member against the soft flesh of Lottie's ass, driving it to and fro as the sensation of her chubby flesh drives you wild.\n\n");
        ...
        if (lottieMorale() >= 99) {
            ...
            if (User.settings.silly()) DisplayText("  Before you leave, you make sure to grab a bacon shake from the clerk, high-fiving her as you backflip back to camp.");
            DisplayText("\n\n");
        }
        //[if enc. 0]
        else {
            ...
            if (User.settings.silly()) DisplayText("  Unfortunately for the spider-clerk, you've raided her cart of bacon shakes on the way out.  You try to backflip back to camp before accidentally dropping the majority of your shakes onto the floor, ultimately deciding to walk back normally - the last beverage you could save gripped firmly in your hands.");
        }
        player.orgasm();
        player.stats.sens += -2;
        return { next: Scenes.camp.returnToCampUseOneHour };
    ...
    //[Weights]
    public ifrisAndLottieGetsNTRedLiftingWeights() {
        DisplayText().clear();
        DisplayImage(images.showImage("lottie-ifris-threesome"));
        ...
        DisplayText("You stand up and off the bench, holding Ifris so that her mouth is still around the tip of your prick.  Casually tossing your head towards Lottie, you attract her attention and demand, \"<i>Stop shoving your fingers in that fat pussy of yours and pay attention.</i>\"\n\n");
        ...
        if (player.torso.balls.quantity > 0) DisplayText("  Your pace doesn't slow, even when your swollen balls smack loudly against her chin.");
        DisplayText("  You let out a deranged chuckle at the sight of Ifris being thrown about");
        if (player.torso.balls.quantity > 0) DisplayText(", sweaty gonads colliding into her");
        DisplayText(".  Lottie is crying out in ecstasy next to you.  She pleads with you to let her have the next turn as your fuck-toy.  The pressure building up in your sack has finally reached its peak.  You abandon all delusions of control, giving in to the primal desire that has arisen.\n\n");

        DisplayText("Your " + Desc.Leg.describeLegs(player) + " go weak and your arms feel like slime as you push Ifris all the way down one more time.  The first geyser of cum overloads the pretty red gym-goer. She's nearly pushed off of your dick by the force of your first shot.  With no strength left, you let go of her silky hair and let her collapse onto the floor.  Her throat bulges and spunk dribbles out of her mouth.  The second wave overcomes you and you coat the floor with cum.  You barely register grabbing Lottie by the arm and hauling her over to Ifris.  Both women are still fingering their dripping pussies while you prepare one ultimate burst.  Darkness creeps into your vision as you angle your cock above both sluts.  Your urethra dilates and unleashes a barrage of seed; Ifris and Lottie coated in thick ropes of it.  They embrace under the rain of cream, mashing each other's drenched bodies together.\n\n");

        DisplayText("Only swaying for a handful of seconds before collapsing into the two women, you black out.");
        player.orgasm();
        player.stats.lib += 1;
        player.stats.cor += 5;
    ...
    private lottieAndIfrisNTREpilogue() {
        DisplayText().clear();
        DisplayText("A dry wind across your face rouses you from your peaceful sleep.  It seems you're outside the walls of Tel'Adre.  All your equipment is sitting to the right of you, and when you gather the pile of your gear you find a note underneath.\n\n");

        DisplayText("\"<i>Dear " + player.short + ",\n\nSleep well?  I hope so, considering I found you snoring on top of two luscious babes.  Don't worry about getting in trouble at the gym.  Most of the staff and customers were too busy getting themselves off to be angry or offended, though I have to admit I'm a bit jealous.  The little piggy and that tight piece of red ass are both safe in their homes.  ");
        if (!kGAMECLASS.urta.urtaLove()) {
            DisplayText("Of course, I had to fine you for public indecency - you really ought to watch how you behave, lest someone take you for a demon agent.  Next time, try and keep it in your pants.\n\n-Urta, Captain of the Guard");
            if (player.inventory.gems > 0) {
                player.inventory.gems -= 200;
                if (player.inventory.gems < 0) player.inventory.gems = 0;
            }
        }
        else DisplayText("I had to make a show of fining you for public indecency, but you'll note your money is all still there.  Pay me back sometime, okay, hun?\n\n-Always up for you, Urta");
        DisplayText("</i>\"\n\n");

        DisplayText("Satisfied, you get dressed and strut off towards camp.");
        if (User.settings.silly()) DisplayText("  You'll make damn sure to buy Urta a bacon shake tomorrow.");
        return { next: Scenes.camp.returnToCampUseOneHour };
    }

    public lottiesThirtyMinutePigGasm() {
        DisplayText().clear();
        let x: number = player.torso.cocks.sort(Cock.LargestCockArea)[0];
        DisplayText("\"<i>Hey, babe!</i>\"  Lottie greets you in her usual, cheerful fashion.  Though her smile is booming with assumed confidence, the look in her eyes tells a different story.  She has sad, dependent eyes.  She looks up to you!  Then again, why shouldn't she?  You are her personal trainer after all.\n\n");

        DisplayText("Smiling back, you give her a warm greeting.  After the pleasantries are dealt with, you instruct her that it is time for some pre-workout stretches.  Ever eager to please you, Lottie begins.  She spreads her legs as far as she can and then leans down in an attempt to touch her toes.  As she bends over, you get a glimpse of her spacious ass.  Baby got back!  While she fights to touch her toes, her chubby ass wiggles and jiggles; it's the very definition of the term \"<i>more cushion for the pushin'.</i>\"  The more you watch, the more blood rushes to your " + Desc.Cock.describeCock(player, x) + ".  You bite your lip as sweat beads on your skin; you NEED to tame that ass!\n\n");

        DisplayText("Lottie is completely oblivious to that fact that you are ogling her plump rump.  Her face wrenches in frustration as she strains.  Then... victory!  She managed to touch her toes!  A relieved smile crosses her face until... SMACK!  Lottie recoils in pain from the sudden stinging slap to her rear.  She tries to unbend, but your forceful left hand keeps her in place.  Glancing back as much as she can, she notices first your missing " + player.inventory.equipment.armor.displayName + " and then your erect, throbbing cock.  Trembling a little in apprehension, she tries to form a question, but only incoherent stammering comes out of her mouth.\n\n");

        DisplayText("\"<i>Time to pork this bitch!</i>\" you exclaim in triumph.  You forcibly yank down the skimpy bottoms that barely contained her bodacious booty.  Smiling lecherously, you flop " + Desc.Cock.describeMultiCockSimpleEach(character) + " between her pink cheeks.  Now firmly holding her hips, you stroke your manhood between them.  Lottie doesn't so much as put up a fight.  Instead, she trembles and oinks like the pig she is.\n\n");

        DisplayText("Looking down at her with a look most wicked, you immediately stop stroking your length between her chunky cheeks.  Giving her no warning at all, you promptly slam your " + Desc.Cock.describeCock(player, x) + " into her piggy depths, forcibly parting her labia.  Lottie squeals from the sudden impact as her cunt muscles tense around you, giving you unbridled pleasure.  Panting heavily, she just stands there and takes it.  Her face is flustered, blushing rosily as her tongue just hangs out of her mouth, but it matters little to you that she enjoys herself.  FUCK!  Her cunt is a natural born cock milker.  It churns and squeezes your " + Desc.Cock.describeCock(player, x) + ", encouraging you to pound her, and you happily oblige.  Grasping her hips for leverage, you pummel her porky pussy for several minutes until your loins unleash a thick jet of jism, completely coating her insides white.\n\n");
        ...
        DisplayText("DAMN!  Did this pig bitch really just have a half-hour orgasm?  You're a little jealous.");
        player.orgasm();
        return { next: Scenes.camp.returnToCampUseOneHour };
    }
}
