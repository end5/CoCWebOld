...
class Rubi {
    ...
    //const RUBI_HAIR.LENGTH: number = 996;
    ...
    private rubiSprite() {
        if (Flags.list[FlagEnum.RUBI_HORNTYPE] > 0) DisplaySprite(102);
        else DisplaySprite(101);
    }

    private initializeRubi() {
        if (Flags.list[FlagEnum.RUBI_BIMBO] === 0) {
            ...
            flags[FlagEnum.RUBI_NO_CUNT] = 0;
            flags[FlagEnum.RUBI_COCK_SIZE] = 0;
            flags[FlagEnum.RUBI_SHE] = 1;
            flags[FlagEnum.RUBI_HAIR] = 1;
        }
        flags[FlagEnum.RUBI_SETUP] = 1;
    }
    ...
    private rubiBimbo(): boolean {
        return (flags[FlagEnum.RUBI_BIMBO] > 0);
    }
    private rubiIncubus(): boolean {
        return (flags[FlagEnum.RUBI_INCUBUS_PROGRESS] === 3);
    }
    public rubiAffection(delt: number = 0): number {
        if (delt === 0) return flags[FlagEnum.RUBI_AFFECTION];
        flags[FlagEnum.RUBI_AFFECTION] += delt;
        if (flags[FlagEnum.RUBI_AFFECTION] > 100) flags[FlagEnum.RUBI_AFFECTION] = 100;
        else if (flags[FlagEnum.RUBI_AFFECTION] < 0) flags[FlagEnum.RUBI_AFFECTION] = 0;
        return flags[FlagEnum.RUBI_AFFECTION];
    }

    public rubiCock(lust: number = 50): string {
        let cumQ: number = 300;
        if (flags[FlagEnum.RUBI_BLU_BALLS] < 4) {
            cumQ = 10;
        }
        else if (flags[FlagEnum.RUBI_BLU_BALLS] <= 6) {
            cumQ = 150;
        }
        return Appearance.cockDescription(rubiGetCockType(), flags[FlagEnum.RUBI_COCK_SIZE], flags[FlagEnum.RUBI_COCK_SIZE] / 6, lust, cumQ);
    }

    public rubiGetCockType(): CockType {
        return CockType.ParseConstantByIndex(flags[FlagEnum.RUBI_COCK_TYPE]);
    }
    public rubiBreasts(): string {
        let ret: string = "pair of ";


        if (flags[FlagEnum.RUBI_BREAST_SIZE] < 1)
            ret = "chest";
        else {
            ret += Appearance.breastCup(flags[FlagEnum.RUBI_BREAST_SIZE]);
            ret += " ";
            ret += BreastStore.Desc.Breast.describeBreastRow(flags[FlagEnum.RUBI_BREAST_SIZE]);
        }
    ...
    public rubiCapacity(): number {
        let bonus: number = flags[FlagEnum.RUBI_TIMES_ANALLY_TRAINED] * 10;
    ...
    public rubiMF(man: string, woman: string): string {
        if (flags[FlagEnum.RUBI_ADMITTED_GENDER] < 1) return woman;
        else {
            if (flags[FlagEnum.RUBI_SHE] === 1) return woman;
            else return man;
        }
    }
    ...
    private specialRelationship20scene() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("As you're leaving the bakery, you hear the clicking of heels behind you.  Turning back you see Rubi rushing towards you shouting, \"<i>Hey, wait a second!</i>\"  You do, and wait for her to catch up.  Stopping in front of you, she puts her hands on her dainty knees and catches her breath.");
        ...
        flags[FlagEnum.RUBI_ADMITTED_GENDER] = 1;
        //(Accept) or (Reject)
        MainScreen.simpleChoices(["Accept", "Reject", "", "", ""], [acceptRubi, rejectRubi, null, null, null]);
    }
    //[If Accept]
    private acceptRubi() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You wrap your arms around and pull him into a hug.  You declare that you like him no matter what sort of genitals he happened to be born with.");
    ...
    private rejectRubi() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("With your hands on his shoulders you carefully keep him at arm's length.  Putting it as delicately as you can, you tell him that you're just not interested in him <i>that</i> way, not anymore at least.");
        ...
        flags[FlagEnum.RUBI_DISABLED] = 1;
        return { next: Scenes.camp.returnToCampUseOneHour };
    }
    ...
    internal function rubiIntros(): Function {
        if (flags[FlagEnum.RUBI_SETUP] === 0) {
            initializeRubi();
        }
        if (flags[FlagEnum.RUBI_DISABLED] > 0 || flags[FlagEnum.RUBI_PROSTITUTION] === -2) return null;
        DisplayText("\n");
        //(Haven't met yet)
        if (flags[FlagEnum.RUBI_INTRODUCED] === 0) DisplayText("A cute human girl moves from table to table, bussing the empty ones and serving drinks to the occupied.  She's pretty flat-chested, and wears a pink and white frilly waitress uniform with an apron.  When she turns her back to you, you can tell she's not <i>entirely</i> human, what with the long red spaded tail coming out from under her dress.");
        //(Have met, relationship 0-19)
        else if (rubiAffection() < 30) DisplayText("Rubi is here, moving from table to table efficiently clearing the empty ones and serving new customers.  Her long red tail swishes back and forth, sometimes lifting her dress enough to get a peek at her panties.");
        //(Have met, relationship 40, Bimbo Rubi; cheating scene)
        else if (rubiBimbo() && flags[FlagEnum.RUBI_PROSTITUTION] === 0 && randInt(9) === 0) {
            DisplayText("Strangely, even though you know it's time for Rubi's shift, she's not here.\n");
            //(Rubi button still active, goes to \"<i>Cheating Rubi</i>\" scene)
            //(Have met, relationship 40, Bimbo Rubi)
            return cheatingRubi;
        }
        else if (rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] === 0) DisplayText("Rubi is here, moving from table to table, not quite as efficiently as [rubi ey] used to, but with a lot more exuberance.  [rubi Eir] long red tail swishes around, seemingly drawn towards the crotch of patrons and co-workers alike.");
        else DisplayText("Rubi is here, moving from table to table efficiently clearing the empty ones and serving new customers.  [rubi Eir] long red tail swishes back and forth excitedly.");
        DisplayText("\n");
        return approachRubiScenes;
    }

    //Approach Rubi scenes
    public approachRubiScenes() {
        DisplayImage(images.showImage("rubi-at-cafe"));
        DisplayText().clear();
        rubiSprite();
        //(First time)
        if (flags[FlagEnum.RUBI_INTRODUCED] === 0) {
            ...
            flags[FlagEnum.RUBI_INTRODUCED]++;
        }
        //(Approach repeat, relationship 0-19)
        else if (rubiAffection() < 30) {
            DisplayText("You take a seat at a table, and within seconds Rubi pulls up a chair next to you.  ");
        }
        //(Approach repeat, relationship 20+.  At this stage, randomly select an outfit from what the PC has given him, plus his waitress outfit.  Variants included for Bimbo /Incubus Rubi.)
            ...
            let choices: Array = [1];
            if (flags[FlagEnum.RUBI_SUITCLOTHES] === 1) choices[choices.length] = 2;
            if (flags[FlagEnum.RUBI_FETISH_CLOTHES] === 1) choices[choices.length] = 3;
            ...
            if (flags[FlagEnum.RUBI_GREEN_ADVENTURER] === 1) choices[choices.length] = 4;
            //(Tube Top (Normal))
            if (flags[FlagEnum.RUBI_TUBE_TOP] === 1) choices[choices.length] = 5;
            //(Bodysuit (Normal))
            if (flags[FlagEnum.RUBI_BODYSUIT] === 1) choices[choices.length] = 6;
            //(Long Dress (Normal))
            if (flags[FlagEnum.RUBI_LONGDRESS] === 1) choices[choices.length] = 7;
            //(Tight Leather Pants (Normal))
            if (flags[FlagEnum.RUBI_TIGHT_PANTS] === 1) choices[choices.length] = 8;
            //(Nurse Clothes (Normal))
            if (flags[FlagEnum.RUBI_NURSE_CLOTHES] === 1) choices[choices.length] = 9;
            //(Slutty Swimwear (Normal))
            if (flags[FlagEnum.RUBI_SWIMWEAR] === 1) choices[choices.length] = 10;
            if (flags[FlagEnum.RUBI_BIMBO_MINIDRESS] === 1) choices[choices.length] = 11;
            if (flags[FlagEnum.RUBI_BONDAGE_STRAPS] === 1) choices[choices.length] = 12;
            if (flags[FlagEnum.RUBI_INQUISITORS_CORSET] === 1) choices[choices.length] = 13;
            //RIsque waitress uniform
            if (flags[FlagEnum.RUBI_AFFECTION] >= 100) choices[choices.length] = 14;
            ...
            if (select === 1) {
                ...
                DisplayText(rubiMF("He", "She") + "'s dressed in [rubi eir] usual waitress uniform: a pink blouse and skirt covered by a frilly white apron.");
                if (flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) DisplayText("  Rubi's breasts strain against the tight blouse, threatening to pop a button at any moment.");
                if (flags[FlagEnum.RUBI_COCK_SIZE] >= 9) DisplayText("  You notice Rubi holds [rubi emself] a little awkwardly, and it's only when [rubi ey] sits down that you spy the slight bulge in [rubi eir] skirt.");
            }
            //(Suit Clothes (Normal))
            else if (select === 2) {
                //(Suitclothes)
                DisplayText(rubiMF("He", "She") + "'s dressed differently today, having foregone the normal waitress uniform for a professional black and white suit with a loose tie, though [rubi ey] still wears the waitress headband");
                if (flags[FlagEnum.RUBI_HORNTYPE] > 0) DisplayText("just behind the horns");
                DisplayText(".  In a sexy sort of way, [rubi ey] looks like a girl dressed in her boyfriend's clothes");
                if (flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) DisplayText(", though with [rubi eir] large bosom, [rubi ey] can't quite button up all the way, giving the appearance of a sexy secretary");
                DisplayText(".");
            }
            else if (select === 3) {
                //(Rubber Fetish Outfit (Bimbo))
                //(Rubber Fetish Clothes)
                DisplayText("[rubi Ey]'s dressed differently today, having foregone the normal waitress uniform for a tight, black, rubber shirt, along with a skirt, and stockings.  The holes cut into the skirt, proudly displaying Rubi's ");
                if (flags[FlagEnum.RUBI_NIPPLE_TYPE] > 0) DisplayText("black ");
                DisplayText("nipples, which stand at perky attention in the cool air.  Rubber straps crisscross the outfit in odd places, and the whole ensemble looks like something out of one of Mareth's many fetish dungeons.  The outfit draws attention to [rubi eir]");
                if (flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) DisplayText(" impressive bosom, and sort of makes [rubi em] look like the kind of rubber doll one might buy for those lonely nights.");
                else DisplayText(" relatively flat chest, though with the rest of [rubi eir] figure, it's hard to imagine [rubi em] as anything but a woman.");
            }
            else if (select === 5) {
                //(Tube Top)
                DisplayText("[rubi Ey]'s dressed differently today, having foregone the normal waitress uniform for a neon pink tube top and a pair of tiny denim shorts.");
                //(D breasts or higher)
                if (flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) DisplayText("  [rubi Eir] breasts strain the fabric of the tube top, any wrong move threatening to have them burst forth.");
                //(dick 9" or higher)
                if (flags[FlagEnum.RUBI_COCK_SIZE] >= 4) DisplayText("  [rubi Eir] rather large bulge can't be concealed, however, and you sometimes catch Rubi holding [rubi eir] notepad down across [rubi eir] crotch when [rubi ey] moves from table to table.");
                else DisplayText("[rubi Eir] bulge is well concealed at least.  You're sure it would take a skilled eye, or someone who knew what to look for, to notice Rubi's little package.");
            }
            else if (select === 6) {
                //(Bodysuit)
                DisplayText("[rubi Ey]'s dressed differently today, having foregone the normal waitress uniform for an almost sheer bodysuit covering [rubi em] from the groin up to the neck.  The lacy outfit is covered with flowers and swirling, colorful decorations in strategically placed locations.");
                if (flags[FlagEnum.RUBI_COCK_SIZE] >= 9) DisplayText("  However, with an outfit this tight, it's almost impossible for [rubi em] to hide [rubi eir] impressive package.  Rubi's cock pokes straight upwards, creating a bulge from [rubi eir] crotch up towards [rubi eir] belly button.");
                else if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText("  However, with an outfit this tight you can see a slight outline where [rubi eir] small package is tucked away.  You wonder how many other patrons have noticed the same thing.");
            }
            else if (select === 7) {
                //(Long Dress (Bimbo))
                DisplayText("[rubi Ey]'s dressed differently today, having foregone the normal waitress uniform for an elegant, sequined ballroom gown.");
                if (rubiBimbo()) DisplayText("  [rubi Ey]'s even modified the dress with false gems and a slit up the side to show off [rubi eir] leg.");
                DisplayText("  You are stunned by the breathtaking combination of classy and carnal.  You're sure [rubi ey] would look great at a fancy ball, but here at the bakery [rubi ey] looks out of place, especially as a waitress.");
                //(Breasts flat or A cup)
                if (flags[FlagEnum.RUBI_BREAST_SIZE] <= 1) DisplayText("  The dress must have come with a padded bra, because you're surprised to see Rubi with  breasts.  Though small, they help to accentuate [rubi eir] feminine figure.");
            }
            else if (select === 8) {
                //(Tight Leather Pants (Bimbo))
                DisplayText("[rubi Ey]'s dressed differently today, having foregone the normal waitress uniform for a loose silk white shirt with tight black leather pants.");
                if (flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) DisplayText("  With [rubi eir] large rack, [rubi ey] looks much like a busty piratess.  From the way [rubi eir] breasts strain at the top, you're certain that one wrong breath could send a button flying across the room.");
                else DisplayText("  With [rubi eir] relatively flat chest, however, you wonder if perhaps it makes [rubi em] look a bit too much like a man.  An effeminate man, but a man nonetheless.");
                //Cock variants:
                //(9" or higher)
                if (flags[FlagEnum.RUBI_COCK_SIZE] >= 9) DisplayText("  [rubi Eir] generous cock is on display, much to [rubi eir] chagrin, as a bulge running down the length of one pant leg.");
                else DisplayText("Luckily, given [rubi eir] relatively small package, [rubi eir] bulge is kept nice and secret even with the tightness of the clothing.");
            }
            else if (select === 9) {
                //(Nurse Clothes (Bimbo))
                ...
                //(D breasts or higher)
                if (flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) DisplayText("  Rubi's large breasts strain the fabric, drawing the dress further up [rubi eir] body and showing off more of [rubi eir] legs and ass.");
            }
            //(Slutty Swimwear (Normal))
            else if (select === 10) {
                //(Slutty Swimwear)
                DisplayText("[rubi Ey]'s dressed differently today, having foregone the normal waitress uniform for an impossibly skimpy black bikini.  It's a cute swimsuit, and Rubi looks damn sexy in it, but you can't help but think it's more suited for a day at the beach and not a day working in a bakery.");
                //(D breasts or higher)
                if (flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) DisplayText("  Rubi's tremendous cleavage threatens to spill out of the skimpy bikini top with every movement, and from the red faces around the bakery, you wonder if that's already happened.");
                else if (flags[FlagEnum.RUBI_BREAST_SIZE] <= 1) DisplayText("  The top only emphasizes Rubi's relatively flat chest, though with the rest of [rubi eir] figure, [rubi ey] still looks extremely feminine.");
            }
            //(Minidress Outfit (Bimbo))
                ...
                if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText("  [rubi Eir] skirt rides so high you catch glimpses of [rubi eir] unsurprisingly unpantied crotch, and the enticing package held within.  You wonder how many people have gotten a surprise with their drinks today!");
            }
            //(Bondage Straps)
            else if (select === 12) {
            ...
            //(Risque Waitress Uniform)
                ...
                if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText(rubiCock(33) + ", clad in a cute little frilly pink cocksock");
                if (flags[FlagEnum.RUBI_NO_CUNT] === 0 && flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText(" and ");
                if (flags[FlagEnum.RUBI_NO_CUNT] === 0) {
                    DisplayText("feminine mound trapped beneath a pair of pink panties");
                    //(if Bimbo Brain)
                    if (rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] === 0) DisplayText(" trimmed feminine mons, free and open to the air");
                }
                DisplayText(".");
                //(D breasts or higher)
                if (flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) DisplayText("  Rubi's breasts, large as they are, show off a decent amount of sideboob and threaten to spill out at any moment.  Judging by the flushed faces of some customers, this may have already happened.");
                DisplayText("  This outfit, as risque as it is, is clearly something Rubi never would have worn before [rubi ey] met you.  [rubi Ey] must be really comfortable in [rubi eir] body now!");
            }
            DisplayText("\n\n[rubi Ey] hands you the usual menu and says, \"<i>");
            if (rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] === 0) DisplayText("Like, h");
            else DisplayText("H");
            DisplayText("ere's the menu, babe.  Think about what you want and let me know when you're ready.</i>\"  [rubi Ey] stands and moves to clean off a nearby table, [rubi eir] red spaded tail swishing behind [rubi em] sensually.");
            ...
            if (select === 4) {
                ...
                if (flags[FlagEnum.RUBI_BREAST_SIZE] >= 4) DisplayText("  In an odd sort of way, [rubi ey] looks like an adventurer who met the wrong end of a witch's curse.");
            }
            //(Default Waitress Uniform)
            if (select === 1) {
                DisplayText("\n\nAs [rubi ey] bends over, you get a good look up [rubi eir] skirt, ");
                if (rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] === 0) {
                    DisplayText("surprised to see [rubi ey]'s wearing nothing at all underneath, showing off [rubi eir] asshole");
                    if (flags[FlagEnum.RUBI_NO_CUNT] === 0 && flags[FlagEnum.RUBI_COCK_SIZE] === 0) DisplayText(" and wet cunt");
                    else if (flags[FlagEnum.RUBI_NO_CUNT] > 0 && flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText(" and limp cock");
                    else DisplayText(", wet cunt, and limp cock");
                    DisplayText(" to anyone behind [rubi em]");
                }
                else DisplayText("excited to see Rubi's choice of frilly pink panties clinging to [rubi eir] supple ass");
                DisplayText(".");
            }
            //(Dashing Outfit)
            else if (select === 8) DisplayText("\n\nAs [rubi ey] bends over, you get a good look at [rubi eir] ass, emphasized by the tight leather pants.");
            //(Adventurer's Outfit)
            else if (select === 15) {
                ...
                //If Bimbo Body:
                if (rubiBimbo()) DisplayText("  You even spy a wet patch in the crotch, with rivulets of moisture running down the legs, which [rubi ey] must have trouble hiding... if [rubi ey] even bothers to try.");
            }
            //(Long Dress)
            else if (select === 7) {
                DisplayText("\n\nAs [rubi ey] moves to another table, Rubi sways back and forth, as if dancing with an invisible partner.  ");
                if (rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] === 0) DisplayText("And then [rubi ey] begins to shake [rubi eir] ass, making spanking motions and effectively ruining the air of sophistication.");
            }
            //(Nurse Uniform)
            else if (select === 9) {
                DisplayText("\n\nAs [rubi ey] bends over, you get a good look up [rubi eir] skirt, ");
                ...
                else DisplayText("excited to see Rubi's choice of frilly white panties clinging to [rubi eir] supple ass");
                DisplayText(".");
            }
            //(Rubber Fetish Outfit)
                ...
                else DisplayText("excited to see a pair of translucent rubber panties clinging to [rubi eir] supple ass");
                DisplayText(".");
            }
            //(Suitclothes)
            else if (select === 2) {
            ...
            //(Bimbo Minidress)
                ...
                else DisplayText("excited to see a cute, little thong wedged into [rubi eir] supple ass");
                DisplayText(".");
            }
            //(Bodysuit)
            else if (select === 6) {
                ...
                //If Bimbo Body:
                if (rubiBimbo()) DisplayText("  You even spy a moist patch in the crotch, which must be impossible to hide... if [rubi ey] even bothers to try.");
            }
            //(Bondage Straps)
            else if (select === 12) {
            ...
            //(Risque Waitress Uniform)
                ...
                else DisplayText("excited to see a pair of pink frilly panties clinging to [rubi eir] supple ass");
                DisplayText(".");
            }
        }
        ...
        if (flags[FlagEnum.RUBI_ADMITTED_GENDER] >= 1) place = rubisFuckingHouseYouPervert;
    ...
    public buyRubiMilk() {
        ...
        if (rubiAffection() >= 30 && flags[FlagEnum.RUBI_ADMITTED_GENDER] === 0) return { next: specialRelationship20scene };
        else return { next: Scenes.camp.returnToCampUseOneHour };
    }

    //Choose Tea?
    public getTeaFromRubi() {
        DisplayText().clear();
        rubiSprite();
        //(Reduce arousal and fatigue slightly?  +1 relationship)
        //(Relationship 0-19)
        if (rubiAffection() < 30) {
            DisplayText("You flag Rubi down and tell her you'd like to order a cup of tea.  She nods and soon you've got a piping hot cup of fragrant tea in front of you.  \"<i>It's our relaxing blend, soothes the body and mind,</i>\" she says.  You take a sip and sigh.  It is indeed relaxing.");
        ...
        //(Relationship 20+, normal Rubi)
            ...
            DisplayText("You flag Rubi down and tell [rubi em] you'd like to order a cup of tea.  [rubi Ey] nods and soon you've got a piping hot cup of fragrant tea in front of you.  \"<i>It's our relaxing blend, soothes the body and mind,</i>\" [rubi ey] says.  You take a sip and sigh.  It is indeed relaxing.");
    ...
    public chatWithRubi() {
        ...
        else if (rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] === 0) {
    ...
    //Choose Rubi's Place?
    public rubisFuckingHouseYouPervert() {
        DisplayImage(images.showImage("rubi-at-house"));
        DisplayText().clear();
        rubiSprite();
        //(Not available to those with a centaur, naga or drider body.)
        //(First Time?)
        if (flags[FlagEnum.RUBIS_HOUSE_FIRST_TIME] === 0) {
            DisplayText("Rather than order anything, you stop Rubi as he's going by your table and pull [rubi em] close.  \"<i>How about we get out of here?</i>\" you whisper into [rubi eir] ear.");
            ...
            flags[FlagEnum.RUBIS_HOUSE_FIRST_TIME]++;
        }
        //Bimbo Shopping!
        else if (rubiBimbo() && (player.perks.has(PerkType.BimboBrains) || player.perks.has(PerkType.FutaFaculties)) && randInt(7) === 0) {
            flags[FlagEnum.RUBI_BIMBO_MINIDRESS] = 1;
            DisplayText("Rather than order anything, you stop Rubi as she's going by your table and pull her close.  \"<i>How about we get out of here?</i>\"  you whisper into her ear.");
            ...
            if (flags[FlagEnum.RUBI_GOT_BIMBO_SKIRT] === 0) {
                ...
                inventory.takeItem(armors.BIMBOSK, Scenes.camp.returnToCampUseOneHour);
                return;
            }
        ...
        //(Repeat, Bimbo Rubi)
            ...
            if (flags[FlagEnum.RUBI_PROSTITUTION] === 2 && flags[FlagEnum.RUBI_PROFIT] > 0) {
                ...
                player.inventory.gems += flags[FlagEnum.RUBI_PROFIT];
                flags[FlagEnum.RUBI_PROFIT] = 0;
                statScreenRefresh();
            }
        ...
        //Repeat unirubi variant
            ...
            DisplayText("Rather than order anything, you stop Rubi as [rubi ey]'s going by your table and pull [rubi em] close.  \"<i>How about we get out of here?</i>\" you whisper into [rubi eir] ear.");
        ...
        MainScreen.addButton(0, "Sex", rubiSexMenu);
        MainScreen.addButton(1, "Closet", goInRubisClosetSoThatYouCanComeOutOfTheCloset);
        MainScreen.addButton(2, "Talk", talkToRubiInHouse);
        if (rubiAffection() >= 50 && !rubiBimbo() && flags[FlagEnum.RUBI_COCK_SIZE] > 0 && (!rubiBimbo() || flags[FlagEnum.RUBI_DEBIMBO] > 0)) {
            //First Time:
            if (flags[FlagEnum.TIMES_RUBI_MASSAGED] === 0) DisplayText("\n\nRubi gestures to some bottles and a box from the bakery before mentioning, \"<i>You look awful tense, babe.  I know you've got it hard out there, and well, I-I thought maybe I could really pamper you today, if you'd like.</i>\"");
        ...
        if (player.torso.hips.legs.isNaga() && flags[FlagEnum.RUBI_BIMBO] === 0 && flags[FlagEnum.RUBI_COCK_SIZE] < player.torso.cocks.sort(Cock.LargestCockArea)[0].length && player.torso.cocks.count > 0 && flags[FlagEnum.RUBI_SHE] === 0 && player.stats.cor >= 85 && flags[FlagEnum.RUBI_BREAST_SIZE] <= 2) {
    ...
    private rubiSexMenu() {
        ...
        if (player.stats.lust >= 33) {
            if (player.torso.cocks.count > 0 || (player.torso.vaginas.count > 0 && player.torso.clit.length >= 4)) MainScreen.addButton(0, "Fuck", fuckRubi);
        ...
        if (flags[FlagEnum.RUBI_BLU_BALLS] > 0 && flags[FlagEnum.RUBI_ORGASM_DENIAL] > 0 && !rubiBimbo() && flags[FlagEnum.RUBI_COCK_SIZE] > 0) MainScreen.addButton(3, "Release", releaseRubi);

        if (flags[FlagEnum.RUBI_AFFECTION] >= 75 && rubiCapacity() < player.torso.cocks.sort(Cock.LargestCockArea)[0].area && player.torso.cocks.count > 0)
            MainScreen.addButton(4, "Train", anallyTrainYourBitchBoySlutHowToBeAnalForDCL);
        //Get Fucked
        //PC gets fucked by Rubi
        //Rubi must have at least a 5" cock
        //Requires 75 Affection
        if (flags[FlagEnum.RUBI_AFFECTION] >= 75 && flags[FlagEnum.RUBI_COCK_SIZE] >= 5)
            MainScreen.addButton(5, "Get Fucked", getFuckedByRubi);
        //Hotdogging
        //PC hotdogs Rubi's ass
        //PC must have a penis
        if (player.torso.cocks.count > 0) MainScreen.addButton(6, "Hotdogging", rubiHotdogging);
        MainScreen.addButton(9, "Back", rubisFuckingHouseYouPervert);
    }


    //Talk
    private talkToRubiInHouse() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You just want to talk with Rubi for now.  What do you want to talk about?");
        
        MainScreen.addButton(0, "Identity", rubiIdentity);
    }

    private rubiIdentity() {
        DisplayText().clear();
        rubiSprite();
        //Identity
        if (flags[FlagEnum.TIMES_DISCUSSED_RUBIS_IDENTITY] === 0) {
            DisplayText("You've noticed Rubi always refers to himself as a \"he\", despite ");
            if (flags[FlagEnum.RUBI_NO_CUNT] === 1)
                DisplayText("his attempts to appear otherwise");
            else
                DisplayText("his relatively recent transformation to the contrary");
            DisplayText(".  Questioning him about it, he simply blushes.");
            ...
            if (flags[FlagEnum.RUBI_NO_CUNT] === 0)
                DisplayText("  Though now that I am, I absolutely love it!");
            DisplayText("</i>\" he confides, fidgeting slightly.  \"<i>If you wanted to call me a \"she\", I'd be OK with that, just so you know.</i>\"");
            DisplayText("\n\nWhich type of pronoun would you prefer Rubi to use?");
        }
        //(Repeating?)
        else {
            DisplayText("You talk with Rubi at length about [rubi eir] gender identity again.  All in all, [rubi ey] seems pretty open to being either gender, with no real preference one way or another as long as [rubi ey] gets [rubi eir] pretty dresses.");
            DisplayText("\n\nWhich type of pronoun would you prefer Rubi to use?");
        }
        flags[FlagEnum.TIMES_DISCUSSED_RUBIS_IDENTITY]++;
        //[He] [She]
        
        MainScreen.addButton(0, "He", rubiIsAHe);
        MainScreen.addButton(1, "She", rubiIsAShe);
    }
    ...
    private rubiIsAHe() {
        DisplayText().clear();
        rubiSprite();
        flags[FlagEnum.RUBI_SHE] = 0;
        DisplayText("Rubi nods, and will refer to himself as a \"he\" from now on.");
    ...
    private rubiIsAShe() {
        ...
        rubiSprite();
        flags[FlagEnum.RUBI_SHE] = 1;
        DisplayText("Rubi nods, and will refer to herself as a \"she\" from now on.");
    ...
    public fuckRubi() {
        DisplayImage(images.showImage("rubi-fuck"));
        let x: number;
        ...
        rubiSprite();
        //(-100 Arousal)
        DisplayText("You take Rubi by the hand and head into the bedroom, clutching [rubi em] close.  You sit on the edge of the soft fluffy bed and pull the little demon " + rubiMF("boy", "girl") + " onto your lap.  The two of you kiss, letting your " + tongueDescript() + " explore the depths of your partner's mouth.");
        ...
        if (player.torso.cocks.count > 0) {
            ...
            DisplayText(Desc.Cock.describeCock(player, x));
        }
        else
            DisplayText(Desc.Vagina.describeClit(player));
        DisplayText(".");

        let size: number = 0;
        let cock: boolean = player.torso.cocks.count > 0;
        //(If player cock/clit at or under 4</i>\")
        if (player.torso.cocks.count > 0) {
            if (player.torso.cocks.get(x).length <= 4) size = 0;
            else if (player.cockThatFits(rubiCapacity()) >= 0) size = 1;
            else size = 2;
        }
        else {
            if (player.torso.clit.length <= 4) size = 0;
            else if (player.torso.clit.length <= 12 || flags[FlagEnum.HYPER_HAPPY]) size = 1;
            else size = 2;
        }
        if (size === 0) {
            DisplayText("\n\n\"<i>Oh, not packing much here, hm?</i>\"  Rubi asks, smiling, \"<i>");
            if (flags[FlagEnum.RUBI_COCK_SIZE] <= 5) DisplayText("I guess we're in the same boat.</i>\"");
            else if (rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] === 0) DisplayText("That's too bad, I was soooo looking forward to a real fucking.</i>\"");
            else DisplayText("That's pretty sad, getting shown up by a dainty little thing like me...</i>\"");
        }

        else if (size === 1) DisplayText("\n\n\"<i>Oh wow, you're quite the stud,</i>\" Rubi says, eyes sparkling with anticipation.");
        ...
        DisplayText(", Rubi slips off your lap and kneels on the floor.  [rubi Ey] pauses a moment, taking in your ");
        if (player.torso.cocks.count > 0) DisplayText(Desc.Cock.describeCock(player, x));
        else DisplayText("[clit]");
        DisplayText(" with [rubi eir] eyes before leaning in and giving it a tentative lick, which sends a shiver up your spine.  Seeing that, [rubi ey] gives a little smile and licks again, running [rubi eir] smooth tongue all along the length of your ");
        if (player.torso.cocks.count > 0) DisplayText("swollen member");
        else DisplayText("impressive love button");
        DisplayText(".  Rubi's mouth dances along its length, at times flicking delicately and at others slurping loudly.  [rubi Eir] tongue ripples along the underside of ");
        if (player.torso.cocks.count > 0) DisplayText("your cockhead");
        else DisplayText("the tip of your clit");
        DisplayText(", sending shivers of pleasure up your spine.");
        //(If Cock)
        if (player.torso.cocks.count > 0) DisplayText("  Amazingly, with the expert ministrations of your lover's mouth, you can't hold yourself in much longer, and begin to tense up, but at the last minute you pull yourself away from Rubi's grip and lips, halting yourself before you can cum.");
        ...
        if (size < 2) {
            ...
            DisplayText(" at [rubi eir] tight ");
            if (flags[FlagEnum.RUBI_NO_CUNT] > 0) DisplayText("tailhole");
            else DisplayText("pussy");
            DisplayText(", rubbing the mixture of [rubi eir] saliva and your juices into [rubi eir] cleft to get [rubi em] suitably lubricated before the real fun begins");
            if (rubiBimbo()) DisplayText(", not that [rubi ey] needs the extra lubrication, as [rubi ey]'s practically dripping like a faucet");
            DisplayText(".");
            DisplayText("  You press forward, sinking your entire length into Rubi's depths, which clench and unclench sensually.  Aided by the additional lubrication, you slip easily in and out, pumping slowly at first.");
        }
        //(These two paragraphs for cocks over 24 area)
            ...
            DisplayText(" at the space just below [rubi eir] ");
            if (flags[FlagEnum.RUBI_NO_CUNT] > 0) DisplayText("balls");
            else DisplayText("cunt");
            DisplayText(", rubbing the mixture of [rubi eir] saliva and your juices into [rubi eir] legs to get [rubi em] suitably lubricated before the real fun begins");
            //(if bimbo)
            if (rubiBimbo()) DisplayText(", not that [rubi ey] needs the extra lubrication, as she's practically dripping like a faucet, which dribbles down [rubi eir] legs suitably enough");
            DisplayText(".");
            DisplayText("\n\nYou press forward, your enormous ");
            if (player.torso.cocks.count > 0) DisplayText(Desc.Cock.describeCock(player, x));
            else DisplayText("[clit]");
            DisplayText(" slipping between Rubi's thighs, which grip you tightly.  You feel [rubi eir] ");
            if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText("hard cock");
            else DisplayText("stiff clit");
            DisplayText(" pressing against the top of ");
            if (player.torso.cocks.count > 0) DisplayText("your dick");
            else DisplayText("your clit");
            DisplayText(", and know that with every thrust you give, [rubi ey]'ll get a good thrill too.");
        }
        ...
        if (flags[FlagEnum.RUBI_ORGASM_DENIAL] > 0 && !rubiBimbo()) DisplayText("  You give Rubi's ass a hard smack and remind [rubi em] that no matter what happens, [rubi ey] still can't cum yet.  [rubi Eir] eyes flutter open and [rubi ey] actually whimpers a little, but can't form the words to object.");
        //(If Orgasm Denial is NOT activated; male/herm Rubi)
        else if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText("  Rubi’s body shivers and trembles, and almost collapses.  [rubi Ey] grasps at the blankets with wild abandon as ropes of pearly white cum erupt from [rubi eir] cock, splattering against [rubi eir] chest and the blankets.");
        //(Female Rubi)
        else DisplayText("  Rubi's body shivers and trembles, and [rubi ey] almost collapses.  [rubi Ey] grasps at the blankets with wild abandon and gives and orgasmic scream as [rubi eir] cunt clenches and unclenches fiercely.  A spray of femjuices erupts outwards from [rubi em], coating your crotch and dripping down both yours and [rubi eir] legs.");
        ...
        if (player.torso.cocks.count > 0 && x.area <= rubiCapacity()) {
            DisplayText("Your " + Desc.Cock.describeCock(player, x) + " engorges for a second before erupting deep inside your demonic little fucktoy.  ");
            if (player.cumQ() < 250) DisplayText("Cum dribbles out from [rubi eir] hole as you give a couple extra thrusts.");
            else if (player.cumQ() < 1000) DisplayText("A generous amount of cum trickles from [rubi eir] depths as you give a couple extra thrusts.");
            else if (player.cumQ() < 2000) {
                DisplayText("Cum practically floods from  [rubi eir] abused ");
                if (flags[FlagEnum.RUBI_NO_CUNT] > 0) DisplayText("ass");
                else DisplayText("cunt");
                DisplayText(" as you give a couple extra thrusts.");
            }
        }
        else if (player.torso.cocks.count > 0) {
            DisplayText("Your " + Desc.Cock.describeCock(player, x) + " engorges for a second before erupting beneath Rubi, splattering [rubi em] and the blankets ");
            if (player.cumQ() < 250) DisplayText("with a fair amount of your seed.");
            else if (player.cumQ() < 1000) DisplayText("with a generous amount of your spunk.");
            else DisplayText("with a veritable torrent of potent pearlescent spooge.");
        }
        //(Player has clit?)
        else DisplayText("  Your " + Desc.Vagina.describeClit(player) + " throbs and engorges briefly as your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " reflexively contracts and a warm feeling rocks through your entire body.  Femjuices run down your [legs] and relief overtakes you like a powerful wave upon a beach.");
        ...
        if (rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] === 0) DisplayText("like, totally ");
        else DisplayText("absolutely ");
        DisplayText("fantastic babe,</i>\" [rubi ey] murmurs before drifting off into a sex-filled dreamland.");
        ...
        return { next: Scenes.camp.returnToCampUseOneHour };
        rubiAffection(1);
        if (flags[FlagEnum.RUBI_ORGASM_DENIAL] > 0) flags[FlagEnum.RUBI_BLU_BALLS]++;
    }

    //Dildo Fuck
    //(If the PC has the Deluxe Dildo) (If Orgasm Denial is activated, adds +2 to Blue Balls (Yay arousal drugs!))
    public dildoFuckRubi() {
        DisplayImage(images.showImage("rubi-dildo-fuck"));
        DisplayText().clear();
        rubiSprite();
        //(If PC has no cock)
        if (player.torso.cocks.count <= 0) DisplayText("You don't have the equipment to give Rubi a good fucking, but there is one thing you do have.");
        //(If PC has cock)
        else DisplayText("While you do have the requisite equipment to give Rubi a good fucking, you'd much rather use something else.");
        //(Add)
        DisplayText("  You produce the goblin-made dildo from your pack, and Rubi's eyes immediately go wide.  \"<i>Wow,</i>\" [rubi ey] says, reaching out to touch it.  [rubi Ey] runs [rubi eir] hands along its length, marveling at its color, shape and size.");
        ...
        //(If PC has a cunt)
        if (player.torso.vaginas.count > 0) DisplayText("You press the tip into your cunt quickly, giving it a taste of what is to come.  The brief contact is enough to make you shudder slightly with pleasure.  ");
        //(If PC has fuckable nipples)
        if (player.torso.chest.find(BreastRow.FuckableNipples)) DisplayText("You let the dildo meander up your body until it reaches your breasts, and with a bit of a grin you insert the tip right into one nipple, enjoying the feeling of your breasts swell up with arousal.  You do the same with the other nipple, and take a minute going back and forth.  ");
        DisplayText("Finally you bring the dildo up to your mouth, licking the juices off of it, eagerly downing the goblin sex-drug.");
        ...
        //(Normal/Incubus Rubi)
        if (!rubiBimbo()) DisplayText("They part shakily and [rubi eir] tongue reaches out to taste the dildo.  ");
        //(Bimbo Rubi)
        else DisplayText("They easily part, and [rubi eir] tongue takes a long slurp, just to taste the dildo.  ");

        DisplayText("[rubi Ey] apparently finds it pleasing, because [rubi ey] immediately wraps [rubi eir] lips around the tip, sucking and bobbing on it like it was a real cock.");
        ...
        if (flags[FlagEnum.RUBI_NO_CUNT] === 1) DisplayText("against [rubi eir] tight ass");
        else DisplayText("against [rubi eir] tight pussy");
        DisplayText(", which resists a little at first, but once the drug has a moment to work, it slips right in.  Rubi actually coos a little as it slips inside.");
        ...
        //(If player has pussy)
        if (player.torso.vaginas.count > 0) DisplayText("  Your " + Desc.Vagina.describeClit(player) + " stiffens, aching for attention.  It peeks from your folds like a shy little lewd faerie.");
        //(If player has a cock)
        if (player.torso.cocks.count > 0) DisplayText("  Meanwhile, [eachCock] stiffens, throbbing as the drug overtakes it.");
        if (player.lactationQ() > 0) DisplayText("  As the warmth spreads up your body, not even your breasts are left alone.  Your nipples harden and begin leaking milk, which pours down the sides of your body and is soaked up into the cushions beneath you.");
        ...
        //(Orgasm Denial on?)
        if (flags[FlagEnum.RUBI_ORGASM_DENIAL] > 0) DisplayText("\n\nIn a sudden moment of clarity, you raise your head and look straight at Rubi, shouting that [rubi ey]'d better not cum during all this, or else you'll be quite upset.  [rubi Eir] eyes go wide, \"<i>W—What?  I need to cum, " + Desc.Gender.mf(player, "sir", "miss") + "!  My body, it's so hot!</i>\"  You growl that it's expressly forbidden, and that [rubi ey]'d better learn to control [rubi em]self, or else.");
        //(Normal/Incubus Rubi, Orgasm Denial off)
        else if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) {
            DisplayText("\n\n[rubi Eir] body shudders and convulses, and [rubi eir] ");
            if (!rubiIncubus()) DisplayText("little cock");
            else DisplayText("demonic cock");
            DisplayText(" twitches, launching jets of cum onto [rubi eir] stomach, chest and face.  In [rubi eir] sex-filled stupor, [rubi ey] eagerly swallows down whatever lands in [rubi eir] mouth, and actually uses [rubi eir] fingers to collect the rest , gulping that down as well.");
        }
        //(Bimbo/Female Rubi)
        else {
        ...
        //(PC has cock?)
        if (player.torso.cocks.count > 0) DisplayText("[eachCock], stroking it fervently, in desperate need of release.");
        //(PC doesn't have cock, but has pussy?)
        else if (player.torso.vaginas.count > 0) DisplayText("your " + Desc.Vagina.describeClit(player) + ", rubbing it frantically, in desperate need of release.");
        //(PC doesn't have either, but breasts above A-cup?)
        else DisplayText("your [chest], rubbing your nipples vigorously, in desperate need of release.");
        //(Bonus, fuckable nipples?)
        if (player.torso.chest.find(BreastRow.FuckableNipples)) DisplayText("  Your fingers easily slip inside each nipple, pumping in and out like little cocks.");
        //(No cock, no pussy, no breasts?)
        if (player.gender === Gender.NONE && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating === 0) DisplayText("  Your hands roam all along your body, from your bare crotch to your nipples, even through your hair, all in search of something to give you the release you so desperately crave.");
        ...
        //(PC has cock?)
        if (player.torso.cocks.count > 0) {
            DisplayText("  [EachCock] twitches and shudders, loosing several jets of cum onto your body, ");
            if (player.cumQ() < 250) DisplayText("splattering messily");
            else if (player.cumQ() < 500) DisplayText("coating you lightly");
            else if (player.cumQ() < 1000) DisplayText("covering you in a copious layer of cum");
            else DisplayText("covering you in a thick layer of your potent seed");
            DisplayText(".");
        }
        //(PC has pussy?)
        if (player.torso.vaginas.count > 0) DisplayText("  Your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " clenches down on the enormous invader, which at this point feels like an enormous steel rod lodged firmly inside you, and a spray of fem-juices erupts from your nethers, coating the already slick dildo and Rubi.");
        //(PC lactating?)
        if (player.lactationQ() > 0) DisplayText("  Your breasts shudder and quake as the orgasmic tsunami washes over them, spraying your sweet smelling milk all over yourself, Rubi and the couch.");
        ...
        DisplayText("\n\nRubi rubs [rubi eir] ");
        if (flags[FlagEnum.RUBI_NO_CUNT] === 0) DisplayText("pussy");
        else DisplayText("asshole");
        DisplayText(" with a slight smile on [rubi eir] face, and says, \"<i>Well, that was interesting.</i>\"  You laugh at the understatement of the century, and stand, helping Rubi up off the couch as well.");
        ...
        player.orgasm();
        rubiAffection(1);
        if (flags[FlagEnum.RUBI_ORGASM_DENIAL] > 0) flags[FlagEnum.RUBI_BLU_BALLS] += 2;
        return { next: Scenes.camp.returnToCampUseOneHour };
    }

    //Tease
    public teaseRubi() {
        DisplayImage(images.showImage("rubi-tease"));
        DisplayText().clear();
        rubiSprite();
        //(Normal and Incubus Rubi.  Activates Orgasm Denial and adds 1 to Blue Balls.)
        flags[FlagEnum.RUBI_ORGASM_DENIAL] = 1;
        if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) {
            DisplayText("Having something quite different in mind, you order Rubi to strip down.  With a curious look on [rubi eir] face, [rubi ey] does so, slowly.  Each garment [rubi ey] peels from [rubi eir] soft ");
            if (flags[FlagEnum.RUBI_SKIN] === 0) DisplayText("pale");
            else if (flags[FlagEnum.RUBI_SKIN] === 2) DisplayText("fur-covered");
            else DisplayText("red");
            DisplayText(" skin seems to take an eternity, until finally [rubi ey]'s standing in front of you in naught but [rubi eir] panties.");
            ...
            //(Normal Rubi)
            DisplayText("  [rubi Ey] blushes, looking back at you with a curious, yet lustful expression.  [rubi Ey] gulps and lifts [rubi eir] tail, giving you free access to [rubi eir] pantied rump.");
            ...
            DisplayText("\n\n\"<i>Nnng,</i>\" Rubi grunts, gripping the couch cushions.  You make sure to remind [rubi em] [rubi ey] can't cum under any circumstances.  [rubi Ey] nods, and says, \"<i>Y—Yes " + Desc.Gender.mf(player, "sir", "miss") + ".</i>\"");
            ...
            DisplayText("\n\n\"<i>Aah, " + Desc.Gender.mf(player, "sir", "miss") + ", I can't take much more of this,</i>\" [rubi ey] says, half way between a whisper and a moan.");
        ...
        //Girl Rubi!
            ...
            DisplayText("Having something quite different in mind, you order Rubi to strip down.  With a curious look on [rubi eir] face, [rubi ey] does so, eagerly.  Each garment [rubi ey] peels from [rubi eir] soft ");
            if (flags[FlagEnum.RUBI_SKIN] === 0) DisplayText("pale");
            else if (flags[FlagEnum.RUBI_SKIN] === 1) DisplayText("crimson");
            else if (flags[FlagEnum.RUBI_SKIN] === 2) DisplayText("furred");
            else DisplayText("striped");
            DisplayText(" skin seems to take an eternity.  In fact, [rubi ey] seems to take great delight in stripping, giving little twirls and tossing [rubi eir] clothes towards you, until [rubi ey]'s standing in front of you in naught but [rubi eir] birthday suit.");
            ...
            DisplayText("  \"<i>Ooh, YES, right there,</i>\" Rubi gasps.  Your hand pinches [rubi eir] clit tight, eliciting a yelp and a coo of pleasure at the same time.");
            ...
            if (rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] === 0) DisplayText("like, ");
            DisplayText("cum!</i>\"");
    ...
    //Release
    public releaseRubi() {
        DisplayImage(images.showImage("rubi-orgasm-denial-release"));
        DisplayText().clear();
        rubiSprite();
        //Will deactivate Orgasm Denial.  To start Denying again, the PC will have to Tease.
        flags[FlagEnum.RUBI_ORGASM_DENIAL] = 0;
        DisplayText("You order Rubi to strip down.  Knowing the drill, [rubi ey] does so slowly.  There's no striptease, no show, [rubi ey] just disrobes as quickly as possible until [rubi ey]'s standing in naught but [rubi eir] panties, ");
        if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText("[rubi eir] [rubi cock] straining at the seams, trying to break free.");
        else DisplayText("clinging tight to [rubi eir] flat little mound.");
        ...
        //(If Blueballs 1-3)
        if (flags[FlagEnum.RUBI_BLU_BALLS] <= 3) {
            DisplayText("  Not to mention [rubi eir] tiny balls");
            if (flags[FlagEnum.RUBI_BALLS_TYPE] === 1) DisplayText(", trapped in their little sissy sack");
            DisplayText(", which you think have swelled somewhat, although you haven't been teasing [rubi em] long.");
        }
        //(if Blueballs 4-9)
        else if (flags[FlagEnum.RUBI_BLU_BALLS] <= 9) DisplayText("  Not to mention [rubi eir] sizeable balls, which seem to have swelled up to twice their normal size, thanks to your relentless teasing.  They even have a slight bluish tinge to them, although it's not immediately visible.");
        else DisplayText("  Not to mention [rubi eir] enormous balls, which, thanks to your endless teasing, are seemingly the size of baseballs.  They even have a noticeable blue tinge to them, obviously aching for relief.");
        ...
        if (flags[FlagEnum.RUBI_BLU_BALLS] <= 3) DisplayText("swollen");
        else if (flags[FlagEnum.RUBI_BLU_BALLS] <= 9) DisplayText("slightly blue");
        else DisplayText("incredibly swollen blue");
        DisplayText(" balls.  Once it's properly lubed up, you press two fingers inside.");
        ...
        if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText("P-spot");
        else DisplayText("G-spot");
        //else DisplayText("most sensitive spot");
        DisplayText(".  When Rubi jerks and wiggles involuntarily, you know you've found it.  Pre-cum begins leaking from Rubi's [rubi cock] in copious amounts, running down your hand and arms, and forming a puddle on the coffee table.  Rubi's body trembles and shakes, wriggling on the table like a fish out of water.");

        DisplayText("\n\nFeeling a little more adventurous, you slip a third finger into the little demon " + rubiMF("boy", "girl") + "'s ");
        if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText("rectum");
        else DisplayText("folds");
        DisplayText(".  With all three fingers you press and massage [rubi eir] prostate, increasing the amount of pre-cum flowing from [rubi eir] tip.");
        ...
        //(Blueballs 1-3)
        if (flags[FlagEnum.RUBI_BLU_BALLS] <= 3) DisplayText("  It's quickly followed by another, less forceful jet of pearly white cum, which lands on [rubi eir] face, and several even smaller jets which splatter the area around [rubi em].  [rubi Eir] cock twitches and spasms, then slowly softens.");
        //(Blueballs 4-5)
        else if (flags[FlagEnum.RUBI_BLU_BALLS] <= 5) DisplayText("  It's quickly followed by another, less forceful jet of pearly white cum, then another, and another, each landing closer to [rubi eir] face.  Finally, it gives a few more token jerks, spurting out tiny globules of cum onto the surrounding area.  [rubi Eir] cock twitches and spasms, squirting nothing but air, as if [rubi eir] body is still craving release.");
        //(Blueballs 6-7)
        else if (flags[FlagEnum.RUBI_BLU_BALLS] <= 7) DisplayText("  It's quickly followed by another, more forceful jet of pearly white cum which splatters against the ceiling, dripping down onto the both of you.  Another spurt of cum bursts forth, then another, and another, each one liberally covering Rubi's stomach, chest and face.  [rubi Eir] cock finally gives a few last twitches and spasms, squirting nothing but air, as if [rubi eir] body is still craving release.");
        //(Blueballs 8-9)
        else if (flags[FlagEnum.RUBI_BLU_BALLS] <= 9) DisplayText("  It's quickly followed by another, more forceful jet of pearly white cum, which splatters against the ceiling, dripping down onto the both of you, and then another, and another.  It isn't long before the ceiling has a huge white cum-covered blotch right above you, drizzling down on you like dew dripping from trees.  Rubi's cock doesn't stop there, however, it continues twitching and spurting [rubi eir] seed until [rubi ey] is completely covered in the stuff.  Finally, mercifully, it ends, though [rubi eir] [rubi cock] continues to spasm, squirting nothing but air, as if [rubi eir] body is still craving release.");
        //(Blueballs 10+)
        else DisplayText("  It's quickly followed by another, more powerful jet of pearly white cum, which splatters against the ceiling, dripping down onto the both of you, and then another, and another.  It isn't long before the ceiling has a huge white cum-covered blotch right above you, drizzling down on you like dew dripping from the trees.  Rubi's cock doesn't stop there, however, it continues twitching and spurting [rubi eir] seed, liberally covering [rubi em]self, you and the surrounding area until everything appears to be in a white haze.");
        ...
        //(Blueballs 1-5)
        if (flags[FlagEnum.RUBI_BLU_BALLS] <= 5) DisplayText("You clean yourself up, leave Rubi a little \"<i>See you next time</i>\" note, and head out the door, back to camp.");
        //(Blueballs 6-9)
        else if (flags[FlagEnum.RUBI_BLU_BALLS] <= 9) DisplayText("You head into the bathroom and wash the cum off you, taking a short shower.  Afterwards, you dry up, leave Rubi a little \"<i>See you next time</i>\" note, and head out the door, back to camp.");
        //(Blueballs 10+)
        else DisplayText("You head into the bathroom and wash the cum off you, no easy feat to be sure.  Afterwards, you dry up, leave Rubi a little \"<i>See you next time</i>\" note, and head out the door, back to camp.  You certainly don't want to be there when [rubi ey] wakes up, lest you have to clean the entire house with [rubi em].");
        rubiAffection(flags[FlagEnum.RUBI_BLU_BALLS]);
        flags[FlagEnum.RUBI_BLU_BALLS] = 0;
        flags[FlagEnum.RUBI_ORGASM_DENIAL] = 0;
        return { next: Scenes.camp.returnToCampUseOneHour };
    }
    ...
    public cheatingRubi() {
        ...
        DisplayText("You take a seat at a table, expectantly waiting for Rubi to slide into your lap...  but she never does.  You look around the place for her, but again, there's no sight of her.  You flag down Dia, the caninemorph waitress, who looks haggard, and inquire as to the location of her co-worker.");
    ...
    //(Don't Care)
    private dontCareAboutNoCheatingRubis() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You shrug your shoulders.  It's not really your problem, is it?  You're free to fuck who you choose, and so is she.");
    ...
    //(Find Her)
    private findBimboCheatster() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You let out a disgruntled sigh.  Rubi's seeing someone else?!  Not on your watch!  You leave the bakery and make your way towards her neighborhood.  It doesn't take long, you've gone with her there enough times to memorize the path, even without her leading.");
        ...
        //[Interrupt] [Wait]
        MainScreen.simpleChoices(["Interrupt", "Watch n Wait", "", "", ""], [interruptTheNTRsYouCrazyFool, waitAndGetNTRedLikeTheBoyBitchYouAre, null, null, null]);
    }
    //(Interrupt)
    private interruptTheNTRsYouCrazyFool() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("No one's sticking their dick in your ");
        if (User.settings.silly()) DisplayText("waifu");
        else DisplayText("slut");
        DisplayText(" if you've got a say in it!  You get out of your hiding place, shout a word of warning and burst inside the vibrant pink house.");
    ...
    //(Wait)
    private waitAndGetNTRedLikeTheBoyBitchYouAre() {
        DisplayImage(images.showImage("rubi-get-ntr-ed"));
        DisplayText().clear();
        rubiSprite();
        ...
        if (!player.torso.hips.legs.isTaur()) {
            ...
            else if (player.torso.cocks.count > 0) DisplayText("[eachCock]");
            DisplayText(".  ");
        }
        else DisplayText("\n\n");
        DisplayText("The two go on like this for five minutes, then ten, then fifteen.  Each position, you find, is more intriguing than the last.  ");
        if (User.settings.silly()) DisplayText("They're even doing positions that aren't available in the game!  ");
        DisplayText("Half an hour later, Rubi and the wolfman sit exhausted on the couch, his seed pouring from each of her holes.");
    ...
    //(Resolution)
    private NTRbimboBitchResolution() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You look at Rubi and say that you're definitely going to have to talk about what just happened.");
        ...
        MainScreen.simpleChoices(["No", "Yes", "Break Up", "Pimp", ""], [noBimboNTR, yesBimboNTR, breakUpWithRubi, pimpOutRubi, null]);
    }
    ...
    private noBimboNTR() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("Folding your arms, you shake your head.  You tell her that if this is going to work out, she has to remain faithful to you.  Sex toys and masturbation will have to do to keep her \"<i>urges</i>\" under control.  She looks a little sad, but nods as you speak.");
        ...
        flags[FlagEnum.RUBI_PROSTITUTION] = -1;
        return { next: Scenes.camp.returnToCampUseOneHour };
    }
    //(Yes)
    private yesBimboNTR() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You contemplate it for a moment, and then slowly nod your head.  What's the harm in letting her blow off steam?  You know she'll always come back to you.  You assent to her wishes, to which she squeals and hugs you tightly, tossing the pink cushion aside.");
    ...
    //(Break Up)
    private breakUpWithRubi() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You think about it for a long moment and then sigh sadly.  You look Rubi dead in the eyes and explain you can't be with a woman who wants to cheat on her partner.  Tears form in her eyes as she sniffles, but she nods with you.");
        ...
        flags[FlagEnum.RUBI_PROSTITUTION] = -2;
        flags[FlagEnum.RUBI_DISABLED] = 1;
        return { next: Scenes.camp.returnToCampUseOneHour };
    }
    //(Pimp)
    ...
    //Actual Scene:
    private chocoRubiMassage() {
        DisplayImage(images.showImage("rubi-get-massage"));
        DisplayText().clear();
        rubiSprite();
        ...
        if (player.stats.cor >= 66) DisplayText("start sensually undressing, turning the simple act into a seductive show that causes Rubi's petite boner to jerk unpredictably beneath [rubi eir] clothes.  A small, damp stain appears at the tip by the time you finish your salacious tease.");
        else if (player.stats.cor >= 33) DisplayText("get undressed without much pause or fanfare.  Rubi makes a show of turning [rubi eir] head, but you can see [rubi em] peeping glimpses out of the corners of [rubi eir] eyes, and [rubi eir] boner seems to be bigger and harder than it was moments before.");
        else {
            DisplayText("make [rubi em] turn around while you undress.  You quickly shuck your clothes and ask [rubi em] what to do next, holding a hand across your [chest] and groin");
            if (player.torso.cocks.sort(Cock.LargestCockArea)[0].area >= 40 || player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 8) DisplayText(", not that it does much good");
            DisplayText(".");
        }

        DisplayText("\n\nRubi politely holds a towel between you as you climb into the bed face down, your head coming to rest on a down-filled pillow.  The fluffy fabric barrier is draped across your [butt], though your feminine boyfriend is sure to fold it so that your [legs] are fully exposed.  The box is placed in front of your " + Desc.Face.describeFace(player) + " and opened.  Inside is a candied, fruity treasure - chocolate covered strawberries.  Rubi's slender fingers pluck up one of the delectable treats and press it into your yielding lips, the sweet treat gliding onto your tongue whereupon it goes to work releasing starbursts of flavor.  You hum contently as you devour the confectionary gift, already feeling much more relaxed than you had been.  The rest are within easy reach, and you languidly set to snacking on them.");
        ...
        if (flags[FlagEnum.RUBI_BLU_BALLS] < 3) DisplayText("small");
        else if (flags[FlagEnum.RUBI_BLU_BALLS] < 8) DisplayText("big");
        else DisplayText("huge, stretched");
        ...
        MainScreen.addButton(0, "Release", releaseRubiMassage);
        MainScreen.addButton(1, "Bottle Up", bottleUpRubiMassage);
    }

    //Bottle Up:
    private bottleUpRubiMassage() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You bite the strawberry in half and mouth, \"<i>No, not yet.  Just let it build up.</i>\"  [rubi Ey] whines plaintively, but you cock your head to the side and say, \"<i>Come on, do it for me.  Just, don't let any out....  Here, come off of there and come into bed with me.  We can snuggle and let you relax a bit.</i>\"");
        ...
        flags[FlagEnum.RUBI_ORGASM_DENIAL] = 1;
        flags[FlagEnum.RUBI_BLU_BALLS]++;
        return { next: Scenes.camp.returnToCampUseTwoHours };
    }

    //Release/Not Pent Up
    private releaseRubiMassage() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You figure after all the pampering [rubi ey] did, [rubi ey] deserves a treat.");
    ...
    //Closet
    private goInRubisClosetSoThatYouCanComeOutOfTheCloset() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You ask to see Rubi's wardrobe.  [rubi Eir] eyes light up and [rubi ey] claps [rubi eir] hands together excitedly.  \"<i>");
        if (rubiBimbo() && flags[FlagEnum.RUBI_DEBIMBO] === 0) DisplayText("Oh my god!  ");
        DisplayText("Really?  It's right over here!</i>\"  Rubi clasps your hand and drags you into the bedroom, then through another doorway.");
        ...
        MainScreen.addButton(0, "Next", rubiAppearance);

    }

    //Appearance
    private rubiAppearance() {
        DisplayImage(images.showImage("rubi-inspect-appearance"));
        DisplayText().clear();
        rubiSprite();
        DisplayText("You look over your naked, demon-morph lover.");
        DisplayText("\n\nRubi is about five feet tall, with a very lithe, feminine body.  [rubi Eir] skin is ");

        if (flags[FlagEnum.RUBI_SKIN] === 0) DisplayText("pale, almost like porcelain");
        else if (flags[FlagEnum.RUBI_SKIN] === 1) DisplayText("an exotic cherry red");
        else if (flags[FlagEnum.RUBI_SKIN] === 2) DisplayText("covered with a fine layer of silky-smooth pink fur");
        else DisplayText("an exotic cherry red which contrasts well with a grey belly and stripes that run up and down [rubi eir] legs and arms");

        if (flags[FlagEnum.RUBI_SKIN] != 2) DisplayText(" and completely hairless, except for [rubi eir]");
        else DisplayText(", and [rubi ey] sports");

        if (flags[FlagEnum.RUBI_HAIR.LENGTH] === 0) DisplayText(" short");
        else if (flags[FlagEnum.RUBI_HAIR.LENGTH] === 1) DisplayText(" shoulder-length");
        else DisplayText(" long");
        if (flags[FlagEnum.RUBI_HAIR] === 0) DisplayText(" black");
        else DisplayText(" blonde");
        DisplayText(" hair, which [rubi ey] keeps cut in a cute, feminine style.  Rubi's face is soft and rounded");
        if (flags[FlagEnum.RUBI_WHISKERS] > 0) DisplayText(", with a cute set of whiskers sprouting from [rubi eir] cheeks");
        DisplayText(", [rubi eir] lips painted a ");
        //(normal/furry skin)
        if (flags[FlagEnum.RUBI_SKIN] === 0 || flags[FlagEnum.RUBI_SKIN] === 2) DisplayText("crimson red");
        //(red/siren skin)
        else DisplayText("dusky black");
        DisplayText(", [rubi eir] ");
        //if(flags[FlagEnum.RUBI_EYE_TYPE] === 1) DisplayText("slitted ");
        if (flags[FlagEnum.RUBI_EYE_TYPE] === 0 || rubiBimbo()) DisplayText("green");
        else if (flags[FlagEnum.RUBI_EYE_TYPE] === 1)
            DisplayText("solid black");
        //else DisplayText("black");
        DisplayText(" eyes framed with eyeliner, belying [rubi eir] masculine origins.  ");
        if (flags[FlagEnum.RUBI_HORNTYPE] > 0) DisplayText("Two tiny horns extend upwards from [rubi eir] hairline, solid black in color.  ");
        DisplayText("A pair of ");
        if (flags[FlagEnum.RUBI_EAR_TYPE] === 0) DisplayText("cute, normal ears protrude from the sides of [rubi eir] head, each one pierced with a small dangling earring");
        else if (flags[FlagEnum.RUBI_EAR_TYPE] === 1) DisplayText("cute, cat-like ears pop up from the top of [rubi eir] head, each one seemingly able to move independently");
        else if (flags[FlagEnum.RUBI_EAR_TYPE] === 2) DisplayText("feathered ears protrude from the sides of [rubi eir] head, each one fluttering cutely from time to time, and each pierced with a small dangling earring");
        DisplayText(".");

        DisplayText("\n\n[rubi Ey] has a ");
        if (flags[FlagEnum.RUBI_BREAST_SIZE] < 1) DisplayText("flat-chest ");
        else DisplayText("[rubi breasts]");
        DisplayText(" supporting two perky ");
        if (flags[FlagEnum.RUBI_NIPPLE_TYPE] === 1) DisplayText("black ");
        DisplayText("nipples.");

        DisplayText("\n\nBetween [rubi eir] legs is a ");
        if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) {
            DisplayText(rubiCock() + ", which starts to perk up under your scrutiny. From experience, you know it can reach a impressive " + flags[FlagEnum.RUBI_COCK_SIZE] + " inches when erect.  ");
            DisplayText("Under [rubi eir] cock swings a set of ");
            if (flags[FlagEnum.RUBI_BLU_BALLS] >= 4 && flags[FlagEnum.RUBI_BLU_BALLS] < 9) DisplayText("cum-swollen ");
            else if (flags[FlagEnum.RUBI_BLU_BALLS] >= 9) DisplayText("engorged, blue-tinged ");
            DisplayText("testicles");
            if (flags[FlagEnum.RUBI_BALLS_TYPE] === 1) DisplayText(", which cling tight to [rubi eir] body in a cute spherical pouch");
        }
        if (flags[FlagEnum.RUBI_COCK_SIZE] > 0 && flags[FlagEnum.RUBI_NO_CUNT] === 0) DisplayText(" and a ");
        if (flags[FlagEnum.RUBI_NO_CUNT] === 0) {
            DisplayText("cute ");
            if (flags[FlagEnum.RUBI_CUNTTYPE] > 0) DisplayText("black ");
            DisplayText("pussy topped off by a little love button");
            DisplayText(".  Though you can't see it, you know Rubi's got a perfect little asshole, between [rubi eir] buttcheeks, right where it belongs.");
        }
        else {
            DisplayText(".");
        }
        ...
        if (flags[FlagEnum.RUBI_FEET] === 1) DisplayText("Finally, Rubi stands on a long black spike that makes [rubi em] walk as though [rubi ey] were wearing high heels.");

        //[Give Item] [Get Dressed] [Back]
        
        MainScreen.addButton(0, "Give Item", pickAnItemToFeedRubi);
        MainScreen.addButton(1, "Get Dressed", playDressUp);
        MainScreen.addButton(4, "Back", rubisFuckingHouseYouPervert);
    }
    ...
    private playDressUp() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You look over Rubi's vast collection of clothes.  What outfit would you like Rubi to put on?\n\n");
        
        let closet: Array = [];
        let buttonNames: Array = [];
        if (flags[FlagEnum.RUBI_SUITCLOTHES] === 1) {
        ...
        if (flags[FlagEnum.RUBI_BONDAGE_STRAPS] === 1) {
        ...
        if (closet.length > 0) {
            DisplayText("<b>Rubi's Closet Contains:</b>\n");
            temp = 0;
            while (temp < closet.length) {
                DisplayText(closet[temp] + "\n");
                temp++;
            }
            DisplayText("\n");
        }
        ...
        while (button < 9 && button < buttonNames.length) {
            ...
            if (button < 8 || closet.length < 9) MainScreen.addButton(button, buttonNames[button], dressUpRouter, closet[button]);
            else MainScreen.addButton(8, "More", playDressUp2);
            button++;
        }
        MainScreen.addButton(9, "Back", rubiAppearance);
    }
    private playDressUp2() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You look over Rubi's vast collection of clothes.  What outfit would you like Rubi to put on?\n\n");
        ...
        while (button - 7 < 9 && button < buttonNames.length) {
            if (button === 7) MainScreen.addButton(0, "Previous", playDressUp);
            else if (button <= closet.length) MainScreen.addButton(button - 7, buttonNames[button], dressUpRouter, closet[button]);
            button++;
        }
        MainScreen.addButton(9, "Back", rubiAppearance);
    }

    private dressUpRouter(arg: string) {
        DisplayText().clear();
        rubiSprite();
    ...
    //Waitress Uniform
    private putRubiInAWaitressUniform() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You tell Rubi you'd like to see [rubi em] in the waitress uniform.  \"<i>Hm, well all right, but it's just my work clothes,</i>\" [rubi ey] replies.");
        DisplayText("\n\nRubi starts off with a white bra trimmed with lace");
        if (flags[FlagEnum.RUBI_BREAST_SIZE] < 1) DisplayText(", though it's obvious [rubi ey] doesn't need it with [rubi eir] flat-chest");
        DisplayText(".  [rubi Ey] slips [rubi eir] arms through the holes and expertly latches the back.  Next [rubi ey] takes a black garter belt and slips it up [rubi eir] slender ");
        if (flags[FlagEnum.RUBI_SKIN] != 2) DisplayText("hairless");
        else DisplayText("furred");
        DisplayText(" legs, until it rests snugly around [rubi eir] waist.  A matching pair of sheer stockings quickly follow, held up by the garter.");

        if (!rubiBimbo() || flags[FlagEnum.RUBI_DEBIMBO] > 0) DisplayText("\n\nYour demon-morphed lover next trots over to a chest of drawers and rummages through it until [rubi ey] produces a pair of frilly pink panties, complete with a little bow on the front.  [rubi Ey] steps into the underwear one stocking-clad leg at a time and pulls them up until they fit snugly around [rubi eir] round ass and [rubi cock].");
        ...
        DisplayText("  Rubi approaches a rack of clothes, searching quietly for a moment before picking two hangars off it: a pink blouse and a matching pink skirt.");

        DisplayText("\n\nThe blouse goes on first, followed by the skirt.  Rubi only buttons the blouse about half way up");
        if (flags[FlagEnum.RUBI_BREAST_SIZE] < 1) DisplayText(", showing off some of [rubi eir] flat chest");
        else DisplayText(", showing off a generous portion of cleavage");
        DisplayText(", and tucks the hem of the blouse into the skirt.  Finally, Rubi selects a pair of ");
        if (flags[FlagEnum.RUBI_FEET] === 1) DisplayText("what appears to be flats, with a hole in the heel for [rubi eir] demonic heels");
        else DisplayText("short heels");
        DisplayText(" and slips them on.  [rubi Ey] gives a little twirl in the waitress uniform, and bends over several times to show off [rubi eir] ");
        if (!rubiBimbo() || flags[FlagEnum.RUBI_DEBIMBO] > 0) DisplayText("girly panties");
        else DisplayText("naked nethers");
        DisplayText(".");
        ...
        MainScreen.addButton(0, "Date", dateIntro, iceCreamDate);
        MainScreen.addButton(4, "Back", rubiAppearance);
    }
    //Dashing Outfit
    private putOnADashingOutfitYouWhore() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You tell Rubi you'd like to see [rubi em] in the dashing outfit.  \"<i>Ohh, feeling like plundering a little booty today?</i>\" [rubi ey] asks with a coy smile.");
        ...
        if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText("tucking [rubi eir] cock firmly into the triangular strip of cloth covering [rubi eir] crotch, while ");
        ...
        if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText(", not to mention the sizeable package in [rubi eir] crotch");
        DisplayText(".");
        ...
        if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText("  Combined with the sizeable bulge in the crotch, Rubi looks like quite the dashing, if slutty, piratess.");
    ...
    //Adventurer's Outfit
    private goOnAnAnalAdventureRubiNotReallyJustAnAdventurersOutfit() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You tell Rubi you'd like to see [rubi em] in the adventurer's outfit.  \"<i>Ah yes, am I going to be the Champion now?</i>\" Rubi asks, balling up [rubi eir] dainty hands and putting them on [rubi eir] hips.  \"<i>Rubi, Champion of Mareth!  Slayer of demons and ");
        if (User.settings.silly()) DisplayText("collector of waifus!");
        else DisplayText("layer of ladies");
        DisplayText("!  Has a bit of a ring to it doesn't it?</i>\"  [rubi Ey] giggles for a moment and then gathers up the clothes.");

        DisplayText("\n\n[rubi Ey] starts with a simple, only slightly lacy white bra");
        if (flags[FlagEnum.RUBI_BREAST_SIZE] < 1) DisplayText(", though it's obvious [rubi ey] doesn't need it with [rubi eir] flat-chest");
        DisplayText(".  [rubi Ey] slips [rubi eir] arms through the holes and expertly latches the back.  Following that is a matching pair of panties that [rubi ey] slips up [rubi eir] long, smooth legs.  [rubi Ey] pulls them up so that they hug [rubi eir] round, cute ass tightly.");
        ...
        if (flags[FlagEnum.RUBI_EAR_TYPE] === 1) DisplayText(", covering [rubi eir] feline ears");
        DisplayText(".  The boots that go with the outfit are mid-calf length and made of supple, fine leather");
        if (flags[FlagEnum.RUBI_FEET] === 1) DisplayText(" with a hole cut out in the heel, apparently for Rubi's demonic heels to fit through");
        DisplayText(".  Once those are on, Rubi completes the look by retrieving a wooden sword from somewhere while you were too distracted by [rubi eir] ass.");
    ...
    //Long Dress
    private giveRubiALongDressToHideHerHideousBody() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You tell Rubi you'd like to see [rubi em] in the long dress.  \"<i>Oohh, I just </i>love<i> that one! One second!</i>\" [rubi ey] says as [rubi ey] pulls out the dress and several accessories.");
        DisplayText("\n\nRubi starts with an off-pink strap-less bra");
        if (flags[FlagEnum.RUBI_BREAST_SIZE] < 1) DisplayText(", despite the fact that [rubi ey] clearly doesn't need one");
        DisplayText(", which [rubi ey] expertly fastens behind [rubi em]");
        DisplayText(".  Next, [rubi ey] takes a white garter belt and slips it up [rubi eir] slender ");
        if (flags[FlagEnum.RUBI_SKIN] != 2) DisplayText("hairless");
        else DisplayText("furred");
        DisplayText(" legs, until it rests snugly around [rubi eir] waist.  A matching pair of white stockings quickly follow, held up by the garter.  The next thing is an off-pink pair of panties that [rubi ey] slides up [rubi eir] legs and pulls up so that they cling tightly to [rubi eir] ample rump.");
        ...
        if (flags[FlagEnum.RUBI_SKIN] === 1 || flags[FlagEnum.RUBI_SKIN] === 3) DisplayText(" (or darkens at least)");
        DisplayText(", and [rubi ey] returns to the task at hand.  The dress goes on neatly, and once it's in place you help to pull the strings on the built-in corset, tying them off with a neat little bow in back. To top it all off is a pair of pink pumps");
        if (flags[FlagEnum.RUBI_FEET] === 1) DisplayText(" with the heels cut off and a hole for Rubi's demonic heel to fit through");
        DisplayText(", which [rubi ey] slips on quickly.");
        DisplayText("\n\nAll in all, [rubi ey] looks ready for a night at some kind of fancy ball.");
        DisplayText("\n\nNow that Rubi is dressed up, what do you want to do?");
        //[Date (go to fancy dinner date)] [Back]
        
        MainScreen.addButton(0, "Date", dateIntro, fancyDate);
        MainScreen.addButton(4, "Back", rubiAppearance);
    }

    //Nurse's Clothes
    private rubiPutsOnNursesClothesSoSheCanCheckYourTemperature() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You tell Rubi you'd like to see [rubi em] in the nurse's uniform.  \"<i>Ohh, coming down with a bit of a fever?  Do you need a sponge bath?</i>\" [rubi ey] says coyly, before gathering up the clothing.");
        DisplayText("\n\n[rubi Ey] starts with a simple, only slightly lacy white bra");
        if (flags[FlagEnum.RUBI_BREAST_SIZE] < 1) DisplayText(", though it's obvious [rubi ey] doesn't need it with [rubi eir] flat-chest");
        DisplayText(".  [rubi Ey] slips [rubi eir] arms through the holes and expertly latches the back.  Next [rubi ey] takes a white garter belt and slips it up [rubi eir] slender ");
        if (flags[FlagEnum.RUBI_SKIN] === 2) DisplayText("furred ");
        else DisplayText("hairless ");
        DisplayText("legs, until it rests snugly around [rubi eir] waist.  A matching pair of white fishnet stockings quickly follow, held up by the garter.  The next thing is a pair of white cotton panties that [rubi ey] slides up [rubi eir] legs and pulls up so that they cling tightly to [rubi eir] ample rump.");
        ...
        if (flags[FlagEnum.RUBI_BREAST_SIZE] < 1) DisplayText("heart");
        else DisplayText("left breast");
        DisplayText(".  [rubi Ey] places the matching hat on [rubi eir] head and adjusts it before putting on a pair of white pumps");
        if (flags[FlagEnum.RUBI_FEET] > 0) DisplayText(" with the heels removed and a hole for [rubi eir] demonic heels");
        DisplayText(".");
    ...
    //Rubber Fetish Clothes
    private rubiHasARubberFetish() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You tell Rubi you'd like to see [rubi em] in the rubber fetish outfit.  \"<i>Ohoh,</i>\" [rubi ey] giggles and quirks an eyebrow at you, \"</i>feeling a bit kinky are we?</i>\"");
        ...
        if (flags[FlagEnum.RUBI_SKIN] != 2) DisplayText("skin");
        else DisplayText("furred body");
        DisplayText(", starting with [rubi eir] legs and moving up to the torso and arms.  Once Rubi's ");
        if (flags[FlagEnum.RUBI_SKIN] != 2) DisplayText("skin ");
        else DisplayText("fur ");
        DisplayText("glistens with the oils it's time to put on the actual clothes.  ");
        //(If Not Bimbo)
        if (!rubiBimbo() || flags[FlagEnum.RUBI_DEBIMBO] > 0) DisplayText("[rubi Ey] first takes a pair of transparent rubber panties and slides them up [rubi eir] legs until they lovingly cling around [rubi eir] ass.  ");
        DisplayText("Rubi next slides into a transparent latex shirt, long sleeved, with holes cut out to accommodate [rubi eir] nipples.  It takes some work, and you have to help Rubi to put on the difficult garment, but once it's on, it looks fantastic, clinging to [rubi eir] body like a second skin.");
        ...
        if (flags[FlagEnum.RUBI_SKIN] === 2) DisplayText("furred ");
        DisplayText("legs.  You wonder how [rubi ey] does this when you're not around!  Finally, Rubi slips on a pair of heels");
        if (flags[FlagEnum.RUBI_FEET] === 1) DisplayText(", the actual heel cut off to make room for [rubi eir] demonic heel,");
        DisplayText(" to complete the look.  All in all, [rubi ey] looks ready for a night in the dungeons.");
        DisplayText("\n\nNow that Rubi is dressed up, what do you want to do?");
        //[Date (go to bar date)] [Back]
        
        MainScreen.addButton(0, "Date", dateIntro, barDate);
        MainScreen.addButton(4, "Back", rubiAppearance);
    }

    //Suitclothes
    private putOnASuitYouSlut() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You tell Rubi you'd like to see [rubi em] in the suit.  \"<i>Oh, really?  It looks nice, but you don't think it's too manly?</i>\"");
        DisplayText("\n\nRubi starts with a black, lacy bra");
        if (flags[FlagEnum.RUBI_BREAST_SIZE] < 1) DisplayText(", despite the fact that [rubi ey] clearly doesn't need one");
        DisplayText(", which [rubi ey] expertly fastens behind [rubi em].  Next, [rubi ey] takes a black garter belt and slips it up [rubi eir] slender ");
        if (flags[FlagEnum.RUBI_SKIN] != 2) DisplayText("hairless");
        else DisplayText("furred");
        ...
        if (flags[FlagEnum.RUBI_FEET] === 1) DisplayText(", with the actual heel cut off to make room for [rubi eir] demonic heel,");
        DisplayText(" and quickly combs back [rubi eir] hair.  All in all, [rubi ey] looks like a girl dressed in her boyfriend's clothes... which is to say, quite sexy.");
        DisplayText("\n\nNow that Rubi is dressed up, what do you want to do?");
    ...
    //Tube-Top
    private putOnATubeTopYouWhore() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You tell Rubi you'd like to see [rubi em] in the tube top and shorts.  \"<i>Ooh, I like that one.  It's a little risque, but so cute!</i>\"");
        DisplayText("\n\nRubi starts off with what seems to be the skimpiest g-string you've ever seen.  [rubi Ey] steps into the tiny underwear and pulls them up, ");
        if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText("tucking [rubi eir] cock firmly into the triangular strip of cloth covering [rubi eir] crotch, while ");
        ...
        if (flags[FlagEnum.RUBI_BREAST_SIZE] > 2) DisplayText(", which strain the flimsy piece of material");
        DisplayText(".  Completing the look is a simple pair of sandals");
        if (flags[FlagEnum.RUBI_FEET] === 1) DisplayText(", with a hole cut in the sole to make room for Rubi's demonic heels");
        DisplayText(".");
        DisplayText("\n\nNow that Rubi is dressed up, what do you want to do?");
        //[Date (go to exhibitionist date)] [Back]
        
        MainScreen.addButton(0, "Date", dateIntro, exhibitionistDate);
        MainScreen.addButton(4, "Back", rubiAppearance);
    }

    //Slutty Swimwear
    private putOnSluttySwimwearYouSkank() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You tell Rubi you'd like to see [rubi em] in the swimsuit.  \"<i>Oh, but there's no pool or lake nearby.  Or do you just want to see me in a bikini?</i>\"  Rubi giggles a moment.");
        ...
        if (flags[FlagEnum.RUBI_BREAST_SIZE] < 3) DisplayText(" sit loosely over Rubi's small chest");
        else DisplayText("strain thanks to Rubi's ample busom");
        DisplayText(".");

        DisplayText("\n\nTopping it off, your demonic lover slips on a pair of sandals");
        if (flags[FlagEnum.RUBI_FEET] > 0) DisplayText(", with a hole in the sole to make room for [rubi eir] demonic heels");
        DisplayText(".");
    ...
    //Bimbo Minidress
    private putOnADressYouBimbo() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You tell Rubi you'd like to see [rubi em] in the minidress.  \"<i>Oh that cute little thing we picked up together?  For sure!  It's so cute!</i>\"");
        ...
        if (flags[FlagEnum.RUBI_FEET] > 0) DisplayText(", with the heels removed to make room for [rubi eir] own demonic heels");
        DisplayText(".");
    ...
    //Bodysuit
    private putOnMyBodysuitYouWhore() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You tell Rubi you'd like to see [rubi em] in the bodysuit.  \"<i>Oh, that thing is pretty cute,</i>\" [rubi ey] says, \"<i>Though there's not much to it.  But I suppose that's the point, huh?</i>\"");
        ...
        if (flags[FlagEnum.RUBI_SKIN] != 2) DisplayText("smooth");
        else DisplayText("furred");
        DisplayText(" legs, then up the rest of [rubi eir] body until [rubi ey] slips [rubi eir] arms into the long, sheer sleeves.  After a few adjustments, making sure ");
        if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText("[rubi eir] package is neatly tucked away");
        else DisplayText("a red rose is neatly covering up [rubi eir] pubic mound");
        DisplayText(", and the butt of the suit is clinging perfectly to [rubi eir] own, [rubi ey] poses for you.");
    ...
    //Bondage Straps
    private putOnBondageStrapsYouBondageTrap() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You tell Rubi you'd like to see [rubi em] in the bondage straps.  \"<i>Oh!  That outfit, huh?</i>\"  Rubi blushes brightly");
        if (flags[FlagEnum.RUBI_SKIN] > 0) DisplayText(", or so you imagine");
        DisplayText(".  \"<i>That one's so embarrassing... but for you, I'll wear it.</i>\"");
    ...
    //Inquisitor's Corset
    private whyTheFuckIsRubiWearingAnInquisitorsCorset() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You tell Rubi you'd like to see [rubi em] in the inquisitor's corset.  \"<i>Oh my, I really love that one.  It's so pretty and lacy!</i>\"");
        ...
        if (flags[FlagEnum.RUBI_FEET] === 1) DisplayText(", with the heels removed to make room for [rubi eir] own demonic heels");
        DisplayText(".");
        DisplayText("\n\nNow that Rubi is dressed up, what do you want to do?");
        //[Date (go to bar date)] [Back]
    ...
    //Risque Waitress Uniform
    private putOnAWaitressUniformYouWhore() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You tell Rubi you'd like to see [rubi em] in the risque version of [rubi eir] normal waitress uniform.  \"<i>Oh, you like that outfit, huh?  I thought you might,</i>\" Rubi says with a giggle.");
        DisplayText("\n\nOf course, there's not much to this outfit, which starts off with a skirt so short it could be confused for a belt.  ");
        //(not bimbo)
        if (!rubiBimbo() || flags[FlagEnum.RUBI_DEBIMBO] > 0) {
            DisplayText("Next [rubi ey] slips on a pair of pink frilly panties, sliding them up [rubi eir] slender legs until they come up to cling to [rubi eir] ample ass.  ");
            if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText("Oddly enough, these panties only seem to cover Rubi's balls, leaving [rubi eir] cock right out in the open.  ");
        }
        DisplayText("Taking a simple white apron, Rubi throws that on next, looping the halter-like top over [rubi eir] neck and tying off the straps around [rubi eir] waist.");
        if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText("  To top it all off, Rubi takes a pink lacy cocksock and slips it down [rubi eir] [rubi cock], fastening it into place with a pink ribbon that [rubi ey] ties into a bow around the base of [rubi eir] cock.");

        DisplayText("\n\nAfter all that, [rubi ey] simply puts on a pair of black, open-toed heels");
        if (flags[FlagEnum.RUBI_FEET] === 1) DisplayText(", with the heels removed to make room for [rubi eir] own demonic heels");
        DisplayText(", and poses for you in the skimpy outfit.");
        DisplayText("\n\nNow that Rubi is dressed up, what do you want to do?");
        //[Date (go to cooking date)] [Back]
        
        MainScreen.addButton(0, "Date", dateIntro, cookingDate);
        MainScreen.addButton(4, "Back", rubiAppearance);
    }
    private cookingDate() {
    ...
    //Dates
    private dateIntro(date: Function) {
        DisplayText().clear();
        rubiSprite();
        //(First Time)
        if (flags[FlagEnum.TIMES_RUBI_DATED] === 0) {
            DisplayText("An idea strikes you.  While you have talked with Rubi and gotten to know [rubi em] at work, you haven't really been out with [rubi em] anywhere else.  You wonder, and ask if Rubi would like to go out somewhere, maybe to pick up some food and talk.");
        ...
        //(Repeats)
            ...
            DisplayText("Seeing Rubi all dressed up like this, you figure it might be time to go on another date.  You ask, and Rubi predictably makes a gleeful sound and assents.");
        }
        flags[FlagEnum.TIMES_RUBI_DATED]++;
    ...
    private iceCreamDate() {
        ...
        if (flags[FlagEnum.RUBI_AFFECTION] >= 100 && flags[FlagEnum.RUBI_ICECREAM_CONFESSION] === 0) {
            ...
            flags[FlagEnum.RUBI_ICECREAM_CONFESSION]++;
        }
    ...
    private fancyDate() {
        ...
        if (player.inventory.gems >= 10) {
            ...
            if (flags[FlagEnum.RUBI_AFFECTION] >= 100 && flags[FlagEnum.RUBI_FANCY_CONFESSION] === 0) {
                ...
                flags[FlagEnum.RUBI_FANCY_CONFESSION]++;
            }
            ...
            //(if PC has met Callu)
            if (flags[FlagEnum.MET_OTTERGIRL] === 1) DisplayText(" though not as good as Callu's, the ottergirl fisherman you met at the lake.");
            else DisplayText(", which makes you wonder who Rubi is thinking about.  Perhaps one of [rubi eir] parents?");
    ...
    private exhibitionistDate() {
        ...
        if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText(rubiCock());
        else DisplayText("glistening, wet pussy");
        DisplayText(".");
    ...
    private barDate() {
        ...
        //(If Relationship 100, happens once)
        if (flags[FlagEnum.RUBI_AFFECTION] >= 100 && flags[FlagEnum.RUBI_BAR_CONFESSION] === 0) {
            ...
            flags[FlagEnum.RUBI_BAR_CONFESSION]++;
        }
    ...
    private anallyTrainYourBitchBoySlutHowToBeAnalForDCL() {
        DisplayText().clear();
        rubiSprite();
        flags[FlagEnum.RUBI_TIMES_ANALLY_TRAINED]++;
        //First Time
        if (flags[FlagEnum.RUBI_TIMES_ANALLY_TRAINED] === 1) {
            ...
            if (rubiBimbo() || flags[FlagEnum.RUBI_DEBIMBO] === 0) DisplayText("totally ");
            DisplayText("delicious, but I'm afraid it's too big!  I couldn't possibly fit that entire thing inside me... not comfortably at least.</i>\"");
            ...
            if (flags[FlagEnum.RUBI_SKIN] === 1 || flags[FlagEnum.RUBI_SKIN] === 3) DisplayText(", or redder at least");
            else DisplayText(", what little of it you can see");
            DisplayText(", and [rubi ey] sits knock-kneed behind the chest.");
            ...
            if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText(", [rubi eir] dick wobbling enticingly");
            DisplayText(".");
            ...
            if (flags[FlagEnum.RUBI_NIPPLE_TYPE] === 1) DisplayText("black ");
            DisplayText("nipples, which jut straight up, straining against the sheer fabric of the negligee.  A soft gasp escapes Rubi's soft lips as [rubi ey] pinches one ");
            if (flags[FlagEnum.RUBI_NIPPLE_TYPE] === 1) DisplayText("ebon ");
            DisplayText("nipple, while [rubi eir] other hand flutters down between [rubi eir] thighs to stroke [rubi eir] ");
            if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText(rubiCock());
            else DisplayText("quickly dampening pussy");
            DisplayText(".");
            ...
            DisplayText(" lips, licking the ");
            if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText("pre-cum");
            else DisplayText("juices");
            DisplayText(" off of them with a smile.  [rubi Eir] newly cleaned fingers move to the bottle of lube sitting nearby.  After pumping a handful into [rubi eir] palm, the rich scent of raspberries seems to fill the air, making your nostrils twitch.  Scented lube?");
            ...
            if (flags[FlagEnum.RUBI_COCK_SIZE] === 0) DisplayText("slick vagina");
            else DisplayText("hungry asshole");
            DisplayText(".");
            ...
            if (flags[FlagEnum.RUBI_COCK_SIZE] === 0) DisplayText("cunt");
            else DisplayText("asshole");
            DisplayText(" clinging so tightly to the toy you imagine it must be like a vice.");
            ...
            if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText(rubiCock() + " is standing at full attention, bobbing up and down as Rubi fucks [rubi emself] on the dildo");
            if (flags[FlagEnum.RUBI_COCK_SIZE] > 0 && flags[FlagEnum.RUBI_NO_CUNT] === 0) DisplayText(", and [rubi eir] ");
            if (flags[FlagEnum.RUBI_NO_CUNT] === 0) DisplayText("cunt lips are glistening red with arousal, just begging for attention");
            DisplayText(".  Rubi's cries grow louder, [rubi eir] lips parting and [rubi eir] tongue lolling out as [rubi ey] pants for breath.  You can tell [rubi ey]'s building up to something, something powerful at that.");

            DisplayText("\n\n[rubi Eir] hips spin and oscillate, working vigorously to keep [rubi eir] ");
            if (flags[FlagEnum.RUBI_NO_CUNT] === 1) DisplayText("backside");
            else DisplayText("fuck hole");
            ...
            if (flags[FlagEnum.RUBI_NO_CUNT] === 1) DisplayText("asshole");
            else DisplayText("cunt");
            DisplayText(", seeking freedom from its sweet, fleshy prison.  A long supine sigh escapes your slender lover's mouth.  [rubi Ey]'s clearly exhausted and happy for this rapturous release.");
            ...
            if (flags[FlagEnum.RUBI_NO_CUNT] === 1) DisplayText("ass");
            else DisplayText("pussy");
            DisplayText(".");
            ...
            if (flags[FlagEnum.RUBI_EAR_TYPE] === 0) DisplayText("ear");
            else if (flags[FlagEnum.RUBI_EAR_TYPE] === 1) DisplayText("feline ear");
            else DisplayText("feathered ear");
            DisplayText(" that [rubi ey] should keep that in for a few hours, just to make sure this training takes hold.  Rubi gives you a small smile in response along with a small peck on the cheek before slipping under the sheets for a quick nap.");

            DisplayText("\n\nYou decide this is the best time to head out as well, and make your way out of the apartment and back to camp.");
            dynStats("lus", 20 + player.stats.lib / 5, "resisted", false);
        }
        //Repeatable
        //Must have a cock greater than Rubi's capacity.
        //Rubi's capacity won't go larger than 100.
        else {
            DisplayText("You bring up the subject of training Rubi's ass again.  [rubi Ey] looks up at you with a sheepish smile on [rubi eir] face and confesses, \"<i>I was hoping you'd say that, babe.  I've actually been practicing a little bit on my own.  I... Well, I've been itching to get that bad boy you've got inside me.</i>\"");
            ...
            DisplayText("'s mouth, exploring every inch and entwining with Rubi's own.");
            ...
            if (flags[FlagEnum.RUBI_SKIN] === 0) DisplayText("go bright red");
            else if (flags[FlagEnum.RUBI_SKIN] === 1 || flags[FlagEnum.RUBI_SKIN] === 3) DisplayText("go a vivid purple");
            else DisplayText("would be blushing bright red now, were it not for the fine layer of fur");
            DisplayText(", and [rubi ey] snaps back to the present.  \"<i>Eheh... I guess I deserved that.</i>\"");
            ...
            DisplayText(", making sure to cover every inch.  You're certain you even see a few fingers press inside [rubi eir] hole.");
            ...
            if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText("ass");
            else DisplayText("pussy");
            DisplayText(" once it's been properly trained.");
            ...
            if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText("asshole");
            else DisplayText("cunt");
            DisplayText(" is stretched nearly to its limits.  Finally [rubi ey] lets out a sigh of relief as the knot pops completely into place.  \"<i>Oh... fuck me it's so big,</i>\" [rubi ey] squeals, [rubi eir] body visibly shaking.  Rubi's tongue lolls out, dripping a bit of spittle down [rubi eir] chin.");
            ...
            if (flags[FlagEnum.RUBI_NO_CUNT] === 1) DisplayText("anus");
            else DisplayText("cunt");
            DisplayText(" just widened slightly.  Rubi's eye go wide as [rubi ey] looks up at you with a mouth full of cock, moaning something you can't understand.");
            ...
            if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText("You can feel your lover's cock straining against your leg, aching for some release.  You idly press your foot against it, feeling a slick layer of precum coating it from top to bottom already.  ");
            DisplayText("A pleading look enters Rubi's eyes.  You hold up the pump and ask [rubi em] if [rubi ey] wants you to use it again.  [rubi Ey] gives [rubi eir] head a quick nod of assent, and reaffirms this by taking your [cock biggest] even deeper into [rubi eir] mouth.");

            DisplayText("\n\nYou give it a quick pump, followed by a second, final one.  Rubi's ");

            if (flags[FlagEnum.RUBI_EYE_TYPE] === 0 || rubiBimbo()) DisplayText("green");
            else if (flags[FlagEnum.RUBI_EYE_TYPE] === 1) DisplayText("purple");
            else DisplayText("shiny black");
            DisplayText(" eyes close tight as [rubi ey] succumbs to a brain-melting orgasm.  [rubi Eir] entire body trembles, from [rubi eir] curling toes right up [rubi eir] twitching, painted fingers.");
            if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText("  Even, no, <i>especially</i> [rubi eir] cock twitches and spasms jerking upwards and splattering globules of sticky white goo against the inside of Rubi's babydoll before dripping down onto the floor between [rubi eir] legs, creating a rapidly growing puddle.");
            ...
            DisplayText(", to make [rubi em] squeal from your actions, and not that of a lifeless dildo.  You imagine the " + rubiMF("demon", "demoness") + "'s hot, wet walls clamped around you, milking you for everything you've got.");
    ...
    //Rubi must have at least a 5" cock
    //Requires 75 Affection
    private getFuckedByRubi() {
        DisplayImage(images.showImage("rubi-fucks-you"));
        DisplayText().clear();
        rubiSprite();
        DisplayText("Rather than wait for you to initiate something, Rubi takes you by the hands and leads you into the bedroom.  [rubi Ey] puts one arm on your shoulder and runs [rubi eir] fingers up through the hair on the back of your head before pulling you into a deep kiss.  Rubi's ");
        //(normal/furry skin)
        if (flags[FlagEnum.RUBI_SKIN] === 0 || flags[FlagEnum.RUBI_SKIN] === 2) DisplayText("red");
        else DisplayText("black");
        DisplayText(" lips feel so delicate against yours, but they quickly part as [rubi eir] tongue slithers between them, exploring the depths of your warm, wet mouth.");
        ...
        if (flags[FlagEnum.RUBI_EYE_TYPE] > 0) DisplayText(" slitted");
        DisplayText(" eyes constantly meet yours, looking for approval, before and after every act.  You're certain you're still the one in charge here... but you're simply letting Rubi take the lead.");
        ...
        if (player.torso.vaginas.count <= 0) DisplayText("asshole");
        else DisplayText("cunt");
        DisplayText(" of yours all ready for me.</i>\"  Rubi's head descends between your legs");
        if (player.torso.cocks.count > 0) DisplayText(", ignoring your " + Desc.Cock.describeMultiCockShort(player) + " completely,");
        DisplayText(" and presses [rubi eir] face into your [vagOrAss], inhaling deeply.  [rubi Eir] lips pucker as [rubi ey] plants a kiss directly onto your ");
        if (player.torso.vaginas.count <= 0) DisplayText("ass");
        else DisplayText("moistening pussy");
        DisplayText(".  You let out a small moan and lean backwards, taking your eyes off the ");
        if (flags[FlagEnum.RUBI_HAIR] === 0) DisplayText("black-haired");
        else DisplayText("blonde");
        DisplayText(" head betwixt your thighs and taking in the sensations.");
        ...
        if (this.rubiGetCockType() === CockType.HUMAN) DisplayText("pink, mushroom-like tip");
        else if (this.rubiGetCockType() === CockType.DEMON) DisplayText("purple, nodule-laden tip");
        else if (this.rubiGetCockType() === CockType.HORSE) DisplayText("blunted, musky tip");
        else if (this.rubiGetCockType() === CockType.TENTACLE) DisplayText("tentacle-ringed crown");
        else DisplayText("<b>ERROR: Rubi Cock Type set invalid. Currently: " + this.rubiGetCockType() + "</b>  ")
        DisplayText(" emerges from the foreskin surrounding it as [rubi eir] cock strains, aching for something, anything to fill.");
        ...
        if (player.torso.vaginas.count <= 0) Mod.Butt.displayStretchButt(player, flags[FlagEnum.RUBI_COCK_SIZE] * flags[FlagEnum.RUBI_COCK_SIZE] / 6, true, true, false);
        else Mod.Vagina.displayStretchVagina(player, flags[FlagEnum.RUBI_COCK_SIZE] * flags[FlagEnum.RUBI_COCK_SIZE] / 6, true, true, false);
        ...
        if (player.torso.vaginas.count <= 0) DisplayText("prostate");
        else DisplayText("g-spot");
        DisplayText(" briefly, but enough to send a hot shiver up your spine.  Seeing this reaction, Rubi's smile widens into a grin and begins to thrust in and out at a slow pace.  Every thrust brushes past your sensitive ");
        if (player.torso.vaginas.count <= 0) DisplayText("prostate");
        else DisplayText("g-spot");
        DisplayText(", and one by one your limbs start to tingle.  In and out, [rubi ey] goes, and you can feel something building inside you.");
        ...
        if (flags[FlagEnum.RUBI_EYE_TYPE] > 0) DisplayText(" slitted");
        DisplayText(" eyes, \"<i>Can I come inside you?</i>\"");
        ...
        if (flags[FlagEnum.RUBI_BLU_BALLS] < 4) { }
            ...
            DisplayText("There's so much of it that it oozes out from your abused ");
            if (player.torso.vaginas.count <= 0) DisplayText("ass");
            else DisplayText("cunt");
            DisplayText(", dribbling down your buttcheeks and pooling onto the bed and floor below.  ");
        }
        //(Blueballs 6-9)
        else if (flags[FlagEnum.RUBI_BLU_BALLS] <= 9) {
            DisplayText("Your insides rumble suddenly as Rubi's impressive load begins to fill you up, [rubi eir] seed slipping into every available nook and cranny.  What doesn't fit slips out of your abused ");
            if (player.torso.vaginas.count <= 0) DisplayText("ass");
            else DisplayText("cunt");
            DisplayText(" in a small torrent, pooling onto the bed and floor below.  ");
        }
        //(Blueballs 10)
        else DisplayText("Your insides rumble dangerously and your stomach actually swells up, gaining a noticeable paunch as Rubi's lavish load coats your insides, filling every available nook and cranny and even creating new ones.  What doesn't fit slips out of your abused hole in a flood, coating the bed and floor beneath you in a slick, white glaze.  ");
        DisplayText("Rubi doesn't stop there, however; [rubi ey] continues to assail your [vagOrAss], bringing you to several more miniature, body-shaking orgasms, until [rubi ey] just can't handle any more, and [rubi ey] pulls out, [rubi eir] cock still rather erect and looking cozy in its jacket-like foreskin.");
    ...
    //-100 arousal for PC
    private rubiHotdogging() {
        DisplayImage(images.showImage("rubi-hotdogged"));
        DisplayText().clear();
        rubiSprite();
        ...
        if (flags[FlagEnum.RUBI_SKIN] === 0) DisplayText("pale skinned");
        else if (flags[FlagEnum.RUBI_SKIN] === 1 || flags[FlagEnum.RUBI_SKIN] === 3) DisplayText("red skinned");
        else DisplayText("fur-covered");
        DisplayText(" " + rubiMF("boy", "girl") + " lays naked on your lap.  Rubi giggles and dips a hand between [rubi eir] thighs ");
        if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText("taking hold of [rubi eir] cock");
        else DisplayText("rubbing a finger along [rubi eir] clit");
        DisplayText(" while [rubi ey] says, \"<i>So babe, what do you intend to do?  Are you going to hold me down and ravish me?  What will it be?</i>\"");
        ...
        if (flags[FlagEnum.RUBI_SKIN] != 2) DisplayText("tender skin");
        else DisplayText("silky fur");
        DisplayText(".  Rubi's back arches and a soft gasp escapes [rubi eir] lips.  [rubi Eir] body reflexively jerks towards your hand, hoping for more than a simple teasing touch, but you pull your hand away and give [rubi eir] round ass a playful smack, chastising [rubi em].");
        ...
        if (flags[FlagEnum.RUBI_NO_CUNT] === 0) DisplayText("cunt moistening");
        if (flags[FlagEnum.RUBI_NO_CUNT] === 0 && flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText(" and ");
        if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText("cock hardening");
        DisplayText(".  [rubi Ey] obviously enjoys this light, playful teasing.  You give Rubi's ass another playful smack, watching [rubi eir] ");
        if (flags[FlagEnum.RUBI_SKIN] === 0) DisplayText("pale skin");
        else if (flags[FlagEnum.RUBI_SKIN] === 1 || flags[FlagEnum.RUBI_SKIN] === 3) DisplayText("crimson skin");
        else DisplayText("fur-covered flesh");
        DisplayText(" jiggle with a smile.");
        ...
        if (flags[FlagEnum.RUBI_SKIN] != 2) DisplayText("skin");
        else DisplayText("coat");
        DisplayText(" against your manhood as you slowly begin to pump in and out of Rubi's ass cheeks.");
        ...
        DisplayText(", while your index and middle fingers rub slowly around [rubi eir] ");
        if (flags[FlagEnum.RUBI_NO_CUNT] === 0) DisplayText("clit");
        else DisplayText("perineum");
        DisplayText(".");
        ...
        //[Tease] [Pop]
        MainScreen.addButton(0, "Tease Rubi", teaseButtjobs);
        MainScreen.addButton(1, "Pop", popButtjobs);
    }

    //Tease
    private teaseButtjobs() {
        DisplayImage(images.showImage("rubi-tease-buttjob"));
        DisplayText().clear();
        rubiSprite();
        DisplayText("Grinning to yourself, you decide to tease the little demon " + rubiMF("boy", "girl") + " a little further.  You plunge your thumb further into [rubi eir] ");
        if (flags[FlagEnum.RUBI_NO_CUNT] === 1) DisplayText("ass");
        else DisplayText("pussy");
        DisplayText(", finding [rubi eir] ");
        if (flags[FlagEnum.RUBI_NO_CUNT] === 1) DisplayText("prostate");
        else DisplayText("g-spot");
        DisplayText(" and rubbing circles around it, drawing out a long moan from Rubi's smeared lips.  With every thrust into Rubi's soft ass-pillows you draw another loop around [rubi eir] most sensitive spot with your digit, until you can feel [rubi eir] body start to tense up.");
        ...
        if (player.cumQ() <= 50) DisplayText("in one go, with a small, proud look on [rubi eir] face");
        else if (player.cumQ() <= 150) DisplayText("with one, throat-straining swallow, a small, proud smile taking [rubi eir] face when [rubi ey] stops panting for breath");
        else DisplayText("as it comes, mouthful after mouthful of cum practically injected straight into [rubi eir] stomach. There's so much of it, [rubi ey] can't help but smile mid-way through, proud to take such a huge load");
        if (player.cumQ() >= 1000) DisplayText(".  After guzzling down so much, you're certain Rubi's actually sporting a bit of a belly now, almost as if [rubi ey] were several months pregnant");
        DisplayText(".");
    ...
    //Pop
    private popButtjobs() {
        DisplayImage(images.showImage("rubi-pop-buttjob"));
        DisplayText().clear();
        rubiSprite();
        DisplayText("No, you figure Rubi has been so great lately [rubi ey] definitely deserves to cum.  You plunge your thumb further into [rubi eir] ");
        if (flags[FlagEnum.RUBI_NO_CUNT] === 1) DisplayText("ass");
        else DisplayText("pussy");
        DisplayText(", finding [rubi eir] ");
        if (flags[FlagEnum.RUBI_NO_CUNT] === 1) DisplayText("prostate");
        ...
        DisplayText("\n\nRubi's ");
        if (flags[FlagEnum.RUBI_NO_CUNT] === 0) DisplayText("cunt clamps down on your thumb and gushes, coating your hand in [rubi eir] slick juices");
        else DisplayText("asshole quivers and attempts to suck in more of your thumb before clamping down, leaking slick fluids all over your hand");
        DisplayText(".  [rubi Eir] entire body spasms as the climax wracks it, one sexy leg suddenly jerking back and forth uncontrollably.  Rubi grips at a pillow, biting it and stifling another orgasmic moan.");
        //(If Rubi has Penis)
        if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) DisplayText("  Beneath [rubi em], [rubi eir] cock jerks and twitches, loosing a spray of pearly white cum which splatters against the couch below");
        if (flags[FlagEnum.RUBI_BLU_BALLS] < 4) { }
        //((blueballs 4-5)
        else if (flags[FlagEnum.RUBI_BLU_BALLS] <= 5) DisplayText(" which is quickly followed by another, more powerful stream of cum, which splashes against [rubi eir] stomach, coating [rubi eir] [rubi breasts] in semen");
        else if (flags[FlagEnum.RUBI_BLU_BALLS] <= 7) DisplayText(" which is quickly followed by another, more powerful stream of cum, and a third, and a fourth.  Before you know it, Rubi's entire torso, including [rubi eir] [rubi breasts] are practically glazed with cum");
        else if (flags[FlagEnum.RUBI_BLU_BALLS] <= 9) DisplayText(" which is followed up by several more, each seemingly more poweful than the one previous.  Rubi's body twitches and shudders with every jet, until [rubi eir] entire torso, and the couch, is covered in the musky jizz");
        else DisplayText(" which is followed by a dozen more, each one thicker and more powerful than the last.  Rubi's cock twitches uncontrollably, just like [rubi eir] body, as it coats the entire couch, [rubi eir] entire upper body, and even some of the wall five feet away in the musky stuff");
        DisplayText(".");
        ...
        if (player.torso.balls.quantity > 1) DisplayText("churn");
        else DisplayText("churns");
        DisplayText(" pleasurably, your [cock biggest] stiffens suddenly, and you let loose a long, drawn-out sigh as your dick erupts, jetting cum upwards, which immediately splatters against Rubi's tail and back before dripping down [rubi eir] bountiful asscheeks.");
        ...
        DisplayText("You laugh and give the demon-" + rubiMF("boy", "girl") + " a kiss, remarking how lucky you are to be with [rubi em].");

        DisplayText("\n\n\"<i>No, babe, I'm lucky to have found you,</i>\" [rubi ey] says, snuggling [rubi eir] naked body up against yours as the two of you drift off into a lazy, sex-induced nap.");
        player.orgasm();
        flags[FlagEnum.RUBI_BLU_BALLS] = 0; //Since he just came so he should be cured of blue balls
        return { next: Scenes.camp.returnToCampUseOneHour };
    }
    ...
    private giveRubiATFItem(itype: ItemType) {
        DisplayText().clear();
        rubiSprite();
        //First Time Giving a TF Item
        if (flags[FlagEnum.RUBI_TIMES_GIVEN_AN_ITEM] === 0) {
            DisplayText("Rubi looks at the offered item, curiously.  [rubi Ey] turns it over in [rubi eir] hands, carefully examining the liquid within.  Finally [rubi ey] asks apprehensively, \"<i>Is this going to... change my body?</i>\"");
        ...
        flags[FlagEnum.RUBI_TIMES_GIVEN_AN_ITEM]++;
        //[go to TF scene]
        //Succubus Milk
        if (itype === consumables.SUCMILK || itype === consumables.P_S_MLK) {
            if (itype === consumables.SUCMILK) {
                ...
                DisplayText("You hand over the bottle of milky white fluids to Rubi.  [rubi Ey] takes a moment to read over the pink label before pulling out the cork and downing the whole bottle in one gulp.");
            }
            else {
                ...
                DisplayText("You hand over the bottle of purified milky white fluids to Rubi.  [rubi Ey] takes a moment to read over the pink label before pulling out the cork and downing the whole bottle in one gulp.");
            }
            //(If Bust size increases)
            if (flags[FlagEnum.RUBI_BREAST_SIZE] < 7 || (flags[FlagEnum.RUBI_BREAST_SIZE] < 30 && flags[FlagEnum.HYPER_HAPPY])) {
            ...
            if ((flags[FlagEnum.RUBI_COCK_SIZE] > 4) && (!flags[FlagEnum.HYPER_HAPPY])) {
                ...
                flags[FlagEnum.RUBI_COCK_SIZE]--;
                DisplayText(Math.round(flags[FlagEnum.RUBI_COCK_SIZE] * 10) / 10 + "-inch penis.");
            }
            //(If Penis is removed)
            else if (flags[FlagEnum.RUBI_COCK_SIZE] <= 4 && flags[FlagEnum.RUBI_COCK_SIZE] > 0) {
                ...
                if (flags[FlagEnum.RUBI_NO_CUNT] === 1) {
                    ...
                    flags[FlagEnum.RUBI_NO_CUNT] = 0;
                    flags[FlagEnum.RUBI_SHE] = 1;
                    flags[FlagEnum.RUBI_CUNTTYPE] = 0;
                }
            }
            //(If Rubi randomly gets vagina)
            if (randInt(3) === 0 && flags[FlagEnum.RUBI_NO_CUNT] > 0) {
            ...
            if (flags[FlagEnum.RUBI_FEET] === 0 && randInt(3) === 0 && itype === consumables.SUCMILK) {
                ...
                flags[FlagEnum.RUBI_FEET] = 1;
            }
            //(If Rubi gets horns)
            if (randInt(3) === 0 && flags[FlagEnum.RUBI_HORNTYPE] === 0 && itype === consumables.SUCMILK) {
                ...
                flags[FlagEnum.RUBI_HORNTYPE] = 1;
            }
            //(If Rubi gets red skin)
            if (randInt(3) === 0 && flags[FlagEnum.RUBI_SKIN] != 1 && itype === consumables.SUCMILK) {
                DisplayText("\n\nRubi clutches [rubi eir] stomach suddenly as you notice red spots appearing on [rubi eir] ");
                if (flags[FlagEnum.RUBI_SKIN] != 2) DisplayText("skin");
                else DisplayText("coat of fur");
                DisplayText(".  The blotches soon spread, rapidly overtaking [rubi eir] body.  ");
                if (flags[FlagEnum.RUBI_SKIN] === 2) DisplayText("Meanwhile the silky soft fur that covered [rubi eir] body seems to be falling out as this red menace takes over.  ");
                DisplayText("In no time at all, [rubi eir] smooth, hairless skin now appears to be a deep crimson, matching the color of [rubi eir] spade tail that [rubi ey] received long ago.");
                flags[FlagEnum.RUBI_SKIN] = 1;
            }
        }
            ...
            if (itype === consumables.INCUBID) DisplayText("You hand over the bottle of milky white fluids to Rubi.  [rubi Ey] takes a moment to read over the label before pulling out the cork and downing the whole bottle in one gulp.");
            ...
            else DisplayText("You hand over the bottle of purified milky white fluids to Rubi.  [rubi Ey] takes a moment to read over the pink label before pulling out the cork and downing the whole bottle in one gulp.");

            //(If Bust size decreases)
            if ((flags[FlagEnum.RUBI_BREAST_SIZE] > 0) && (!flags[FlagEnum.HYPER_HAPPY])) {
                ...
                flags[FlagEnum.RUBI_BREAST_SIZE]--;
                if (flags[FlagEnum.RUBI_BREAST_SIZE] < 0) flags[FlagEnum.RUBI_BREAST_SIZE] = 0;
            }
            //(If Penis size increases)
            if (flags[FlagEnum.RUBI_COCK_SIZE] > 0 && ((flags[FlagEnum.RUBI_COCK_SIZE] < 12 || (this.rubiGetCockType() === CockType.HORSE && flags[FlagEnum.RUBI_COCK_SIZE] < 20)) ||
                (flags[FlagEnum.RUBI_COCK_SIZE] < 30 && flags[FlagEnum.HYPER_HAPPY]))) {
            ...
            //(If Rubi has no penis; instead)
                ...
                if (flags[FlagEnum.RUBI_SKIN] === 1 || flags[FlagEnum.RUBI_SKIN] === 3) DisplayText("red");
                else DisplayText("pink");
                DisplayText(" protrusion... a brand new cock!  Rubi shudders, [rubi eir] delicate little fingers wrapped around the new, five-inch cock.");
                flags[FlagEnum.RUBI_COCK_SIZE] = 5;
            }
            //(If Vagina removed)
            if (flags[FlagEnum.RUBI_NO_CUNT] === 0 && randInt(3) === 0) {
            ...
            //(If Rubi gets red skin)
            if (randInt(3) === 0 && flags[FlagEnum.RUBI_SKIN] != 1 && itype === consumables.INCUBID) {
                DisplayText("\n\nRubi clutches [rubi eir] stomach suddenly as you notice red spots appearing on [rubi eir] ");
                if (flags[FlagEnum.RUBI_SKIN] != 2) DisplayText("skin");
                else DisplayText("coat of fur");
                DisplayText(".  The blotches soon spread, rapidly overtaking [rubi eir] body.  ");
            ...
            //(If Rubi gets horns)
            if (randInt(3) === 0 && flags[FlagEnum.RUBI_HORNTYPE] === 0 && itype === consumables.INCUBID) {
                DisplayText("\n\nRubi's hands suddenly dart up, clutching [rubi eir] head as though [rubi ey] has a massive headache.  [rubi Ey] feels around blindly, trying to locate the source of the pain, and then you see it.  Two small bulges appear on [rubi eir] forehead, just at the hairline.  They grow larger and larger until the skin splits cleanly open revealing shiny black bone.  Horns.  They grow up and backwards, forming light ridges.  They don't quite grow completely past [rubi eir] head, however, before the growth stops.  Rubi calms and stands.  [rubi Eir] hands roam over every inch of [rubi eir] new addition as glances in the mirror, marveling at [rubi eir] demonic appearance.");
                flags[FlagEnum.RUBI_HORNTYPE] = 1;
            }
        }
        ...
        //Can choose Boobs or Penis. Grow that body part by 2.
            ...
            MainScreen.addButton(0, "Breast", rubiGrowPlusBreasts);
            MainScreen.addButton(1, "Penis", rubiPenisGroPlus);
            MainScreen.addButton(4, "Back", pickAnItemToFeedRubi);
            return;
        }
            ...
            MainScreen.addButton(0, "Breasts", rubiBoobsReducto);
            MainScreen.addButton(1, "Penis", rubiPenisReducto);
            MainScreen.addButton(4, "Back", pickAnItemToFeedRubi);
            return;
        }
        //Bimbo Liqueur (Modified for variant genders)
        else if (itype === consumables.BIMBOLQ) {
            DisplayText("You hold out the bottle of Bimbo Liqueur.  Rubi takes it, and looks it over tentatively.  \"<i>Bimbo... liqueur?</i>\" [rubi ey] asks, quirking an eyebrow at you.  \"<i>But what does this do?  There's a huge warning label right here.  Effects are permanent, strong...</i>\" [rubi eir] voice drifts off as [rubi ey] reads.");
            ...
            DisplayText(" hair suddenly bursts outwards, long platinum blonde locks flow past [rubi eir] shoulders like a waterfall.  [rubi Eir] already dainty hands grow more feminine, as do [rubi eir] feet.");
            flags[FlagEnum.RUBI_HAIR.LENGTH] = 2;
            ...
            if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) {
                ...
                if (flags[FlagEnum.RUBI_NO_CUNT] === 0) DisplayText("[rubi eir] vagina");
                else DisplayText("a pristine new vagina");
                DisplayText(".  It's pretty clear, this former male is now completely female.  Rubi collapses backwards onto the couch, the transformations seemingly done.");
                flags[FlagEnum.RUBI_COCK_SIZE] = 0;
                flags[FlagEnum.RUBI_NO_CUNT] = 0;
                flags[FlagEnum.RUBI_SHE] = 1;
            }
            ...
            //(if boobs <E)
            if (flags[FlagEnum.RUBI_BREAST_SIZE] < 7) {
                ...
                if (flags[FlagEnum.RUBI_NIPPLE_TYPE] > 0) DisplayText("black ");
                DisplayText("nipples begins to tremble and then expand.  You swear you hear a whooshing sound, like someone is blowing up a balloon.");
                //(if flat-chested)
                if (flags[FlagEnum.RUBI_BREAST_SIZE] === 0) DisplayText("  Within seconds, Rubi has the chest of a pubescent girl, within a minute they're at least a B-cup.");
                DisplayText("  You both sit with morbid curiosity, watching the flesh grow outwards, bit by bit and cup by cup.  After five minutes the growth stops, and Rubi carefully cradles [rubi eir] new enormous E-cup breasts.");
                flags[FlagEnum.RUBI_BREAST_SIZE] = 7;
            }
            ...
            if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating < 1) DisplayText("yours");
            else DisplayText("you");
            DisplayText(".  \"<i>Like, thank you so much baby!  You're the freaking best!  Now... you want to break this body in?</i>\"");
        ...
        //Grows cock. (As Incubus Draft above)
            ...
            if (this.rubiGetCockType() != CockType.HORSE) {
                ...
                flags[FlagEnum.RUBI_COCK_TYPE] = CockType.HORSE;
                ...
                if (flags[FlagEnum.RUBI_BLU_BALLS] >= 4 && flags[FlagEnum.RUBI_BLU_BALLS] < 9) DisplayText("swollen ");
                else if (flags[FlagEnum.RUBI_BLU_BALLS] >= 9) DisplayText("engorged ");
                if (flags[FlagEnum.RUBI_BALLS_TYPE] === 1) DisplayText("tight, trappy ");
                DisplayText("ballsack.  The colors move forward, seemingly stopping at the thick ring that marks the edge of [rubi eir] sheathe... except patches of  dusky gray appear along the next few inches of [rubi eir] length, creating a nice contrast and pattern with the bright pink of [rubi eir] cockflesh.");
                ...
                flags[FlagEnum.RUBI_COCK_SIZE] += 3 + randInt(5);
            }
        }
        //Trap Oil
            ...
            if (flags[FlagEnum.RUBI_COCK_SIZE] > 0) {
                DisplayText("  Rubi's [rubi cock] twitches, already leaking precum from its tip as it attempts to reach the skies above");
            }
            else DisplayText("  Rubi's cunt glistens, soaked through entirely thanks to your ministrations");
            if (flags[FlagEnum.RUBI_BREAST_SIZE] > 0) DisplayText(", while [rubi eir] [rubi breasts] gently rock up and down with the motions of [rubi eir] breathing");
            DisplayText(".");
            ...
            //(If Bust size decreases)
            if ((flags[FlagEnum.RUBI_BREAST_SIZE] > 1) && (!flags[FlagEnum.HYPER_HAPPY])) {
                DisplayText("\n\nRubi gasps a little and [rubi eir] hands immediately dart to [rubi eir] chest.  Before your eyes, Rubi's breasts begin to dwindle, shrinking down an entire cup size.  Rubi cups [rubi eir] breasts experimentally, getting used to their new, reduced, weight.");
                flags[FlagEnum.RUBI_BREAST_SIZE]--;
                if (flags[FlagEnum.RUBI_BREAST_SIZE] < 1) flags[FlagEnum.RUBI_BREAST_SIZE] = 1;
            }
            //(If Bust size increases)
            else if (flags[FlagEnum.RUBI_BREAST_SIZE] < 1) {
                DisplayText("\n\nRubi gasps a little and [rubi eir] hands immediately dart to [rubi eir] chest.  Before your eyes, Rubi's breasts begin to swell, growing an entire cup size.  Rubi cups [rubi eir] breasts experimentally, getting used to their new weight.");
                flags[FlagEnum.RUBI_BREAST_SIZE]++;
                if (flags[FlagEnum.RUBI_BREAST_SIZE] > 1) flags[FlagEnum.RUBI_BREAST_SIZE] = 1;
            }
            //(If Penis size decreases)
            else if ((flags[FlagEnum.RUBI_COCK_SIZE] > 3) && (!flags[FlagEnum.HYPER_HAPPY])) {
                DisplayText("\n\nRubi's [rubi cock] swells up suddenly, growing painfully hard.  [rubi Ey] grasps it and moans suddenly as it begins to dwindle in size, shrinking down by at least an inch, leaving [rubi em] with a ");
                flags[FlagEnum.RUBI_COCK_SIZE]--;
                if (flags[FlagEnum.RUBI_COCK_SIZE] < 3) flags[FlagEnum.RUBI_COCK_SIZE] = 3;
                DisplayText(Math.round(flags[FlagEnum.RUBI_COCK_SIZE] * 10) / 10 + "-inch penis.");
            }
            //(Trappy ballsack)
            else if (flags[FlagEnum.RUBI_BALLS_TYPE] > 0) {
                ...
                flags[FlagEnum.RUBI_BALLS_TYPE] = 1;
            }
            //(Black Nipples)
            else if (flags[FlagEnum.RUBI_NIPPLE_TYPE] != 1) {
                ...
                flags[FlagEnum.RUBI_NIPPLE_TYPE] = 1;
            }
            //(Black Eyes)
            else if (flags[FlagEnum.RUBI_EYE_TYPE] != 1) {
                ...
                flags[FlagEnum.RUBI_EYE_TYPE] = 1;
            }
        ...
        //Progression of furriness: cat ears, then whiskers and paws, then cat penis, then furry body.
            ...
            //(Gaining Cat Ears)
            if (flags[FlagEnum.RUBI_EAR_TYPE] != 1) {
                ...
                flags[FlagEnum.RUBI_EAR_TYPE] = 1;
            }
            //(Gaining Whiskers and Paws)
            else if (flags[FlagEnum.RUBI_WHISKERS] != 1 || flags[FlagEnum.RUBI_HANDS] != 1) {
                ...
                flags[FlagEnum.RUBI_WHISKERS] = 1;
                flags[FlagEnum.RUBI_HANDS] = 1;
            }
            else if (flags[FlagEnum.RUBI_LOWERBODY] != 1) {
                ...
                flags[FlagEnum.RUBI_LOWERBODY] = 1;
            }
            //(Gaining Cat Penis)
            else if (this.rubiGetCockType() != CockType.CAT && flags[FlagEnum.RUBI_COCK_SIZE] > 0) {
                ...
                flags[FlagEnum.RUBI_COCK_TYPE] = CockType.CAT;
            }
            //(Gaining Fur)
            else if (flags[FlagEnum.RUBI_SKIN] != 2) {
        ...
        if (itype === consumables.PURPEAC) {
            ...
            if (flags[FlagEnum.RUBI_SKIN] === 0) DisplayText("skin");
            else if (flags[FlagEnum.RUBI_SKIN] === 1) DisplayText("red skin");
            else if (flags[FlagEnum.RUBI_SKIN] === 2) DisplayText("furred skin");
            else DisplayText("striped skin");
            DisplayText(" underneath.  The demon-marked " + rubiMF("boy", "girl") + " rubs [rubi eir] stomach appreciatively.  \"<i>Those were some great peaches, babe.</i>\"");

            //(Any changes? Yes)
            if (flags[FlagEnum.RUBI_EAR_TYPE] != 2 || flags[FlagEnum.RUBI_SKIN] != 3 || (flags[FlagEnum.RUBI_COCK_SIZE] > 0 && this.rubiGetCockType() != CockType.TENTACLE)) {
                DisplayText("\n\nRubi's eyes suddenly dart open and [rubi ey] clutches [rubi eir] stomach.  \"<i>Oh gods...  My belly feels so warm all of a sudden.</i>\"");
                //(Feathery ears)
                if (flags[FlagEnum.RUBI_EAR_TYPE] != 2) {
                    ...
                    if (flags[FlagEnum.RUBI_EAR_TYPE] === 0) DisplayText("normal");
                    else DisplayText("feline");
                    DisplayText(" ears seem to pull into [rubi eir] skull before disappearing, leaving nothing but skin and hair behind.  You wonder if Rubi's lost [rubi eir] ears permanently, but as the thought finishes crossing your mind you notice two small nubs growing where normal ears would be.");
                    ...
                    flags[FlagEnum.RUBI_EAR_TYPE] = 2;
                }
                //(Shark Skin)
                else if (flags[FlagEnum.RUBI_SKIN] != 3) {
                    ...
                    if (flags[FlagEnum.RUBI_SKIN] === 0) DisplayText("its normal paleness replaced with a vibrant cherry red");
                    else if (flags[FlagEnum.RUBI_SKIN] === 1) DisplayText("which is barely noticeable on [rubi eir] already cherry red flesh");
                    else if (flags[FlagEnum.RUBI_SKIN] === 2) DisplayText("the fur rapidly retreating and replaced with bright red flesh underneath");
                    DisplayText(", but it doesn't seem to be even.  The majority of Rubi's body is cherry red, while [rubi eir] belly actually lightens, becoming a sweet, silvery gray.  A number of similarly silver stripes appear, seemingly at random, up and down Rubi's arms and legs, and even down [rubi eir] demonic tail, amazingly enough.");
                    ...
                    flags[FlagEnum.RUBI_SKIN] = 3;
                }
                //(Anemone Penis, must have a penis)
                else if (flags[FlagEnum.RUBI_COCK_SIZE] > 0 && this.rubiGetCockType() != CockType.ANEMONE) {
                    ...
                    if (this.rubiGetCockType() != CockType.HUMAN) DisplayText(", first shifting towards a more human-like appearance");
                    DisplayText(".  Small nodules appear, around the crown of the penis, and around its base.  Rubi's delicate hands run across two of the nodules, and [rubi eir] cock suddenly stiffens, harder than you've ever seen it!  Whatever those things are, they're quite sensitive.");
                ...
                flags[FlagEnum.RUBI_COCK_TYPE] = CockType.ANEMONE;
            }
            //(No)
            else {
    ...
    //GRO+/REDUCTO
    private rubiGrowPlusBreasts() {
        DisplayImage(images.showImage("rubi-gro-plus-tits"));
        DisplayText().clear();
        rubiSprite();
        DisplayText("You hand over the vial of blueish liquid.  Rubi takes a moment to look it over, before taking a deep breath and injecting half of the vial into one breast and the rest into [rubi eir] other breast.");
        ...
        if (flags[FlagEnum.RUBI_BREAST_SIZE] < 7 || (flags[FlagEnum.RUBI_BREAST_SIZE] < 30 && flags[FlagEnum.HYPER_HAPPY])) {
            ...
            flags[FlagEnum.RUBI_BREAST_SIZE] += 2 + randInt(2);
            if (flags[FlagEnum.HYPER_HAPPY]) {
                // trace("Hyper happy grow");
                if (flags[FlagEnum.RUBI_BREAST_SIZE] > 30) flags[FlagEnum.RUBI_BREAST_SIZE] = 30;
            }
            else {
                // trace("Normal grow");
                if (flags[FlagEnum.RUBI_BREAST_SIZE] > 7) flags[FlagEnum.RUBI_BREAST_SIZE] = 7;
            }
        }
    ...
    //(If Penis)
    private rubiPenisGroPlus() {
        DisplayImage(images.showImage("rubi-gro-plus-penis"));
        DisplayText().clear();
        rubiSprite();
        DisplayText("You hand over the vial of greenish liquid.  Rubi takes a moment to look it over, before taking a deep breath and injecting the entire thing into the base of [rubi eir] [rubi cock].");

        //(If Penis increase)
        if (flags[FlagEnum.RUBI_COCK_SIZE] < 12 || (this.rubiGetCockType() === CockType.HORSE && flags[FlagEnum.RUBI_COCK_SIZE] < 20) || (flags[FlagEnum.RUBI_COCK_SIZE] < 30 && flags[FlagEnum.HYPER_HAPPY])) {
            ...
            flags[FlagEnum.RUBI_COCK_SIZE] += 2 + randInt(2);

            if (flags[FlagEnum.HYPER_HAPPY]) {

                if (flags[FlagEnum.RUBI_COCK_SIZE] > 30) {
                    // trace("Limiting Rubi's Cock-Size");
                    flags[FlagEnum.RUBI_COCK_SIZE] = 30;
                }
            }
            else {
                if (this.rubiGetCockType() != CockType.HORSE) {
                    if (flags[FlagEnum.RUBI_COCK_SIZE] > 12) flags[FlagEnum.RUBI_COCK_SIZE] = 12;
                }
                else {
                    if (flags[FlagEnum.RUBI_COCK_SIZE] > 20) flags[FlagEnum.RUBI_COCK_SIZE] = 20;
                }
            }
        }
        else {
            DisplayText("\n\nSadly, apart from making [rubi eir] cock hard, it doesn't seem to have much of an effect on Rubi's ");
            if (this.rubiGetCockType() === CockType.HORSE) DisplayText("sheath-stuffing horse-dick");
            else DisplayText("giant dick.");
        }
        player.inventory.items.consumeItem(consumables.GROPLUS);
    ...
    //(If Breasts)
    private rubiBoobsReducto() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You hand over the jar of paste.  Rubi looks it over and, after giving it a sniff, uses two fingers to scoop up some of the pale substance, smearing it over [rubi eir] breasts.  Before long, the jar is empty and [rubi eir] [rubi breasts] are covered in the stuff.");
        if (flags[FlagEnum.RUBI_BREAST_SIZE] > 0) {
    ...
    //(If Penis)
    private rubiPenisReducto() {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You hand over the jar of paste.  Rubi looks it over and, after giving it a sniff, uses two fingers to scoop up some of the pale substance, smearing it over [rubi eir] penis.  Before long, the jar is empty and [rubi eir] [rubi cock] is covered in the stuff.");
        if (flags[FlagEnum.RUBI_COCK_SIZE] >= 3) {
    ...
    private pickAnItemToFeedRubi() {
        ...
        if (closet.length > 0) {
            DisplayText("<b>Rubi's Closet:</b>");
            for (let x: number = 0; x < closet.length; x++) DisplayText("\n" + closet[x]);
            DisplayText("\n\n");
        }

        if (gifts.length > 0) {
            DisplayText("After a quick glance around Rubi's things, you bet [rubi ey] could really use... " + gifts[randInt(gifts.length)] + ".\n\n");
        }
        else {
            DisplayText("You've given Rubi all the clothes [rubi ey] would want to make use of.");
            if (User.settings.silly()) DisplayText("  (Achievement Unlocked: Dress-tacular)");
            DisplayText("\n\n");
        }

        if (!rubiBimbo() && !rubiIncubus() && rubiAffection() >= 100) {
            DisplayText("You could give Rubi some incubi draft to make [rubi em] a little more manly but corrupt, or you could give Rubi a Bimbo Draft to turn [rubi em] into a smoking hot, female sex-bomb.\n\n");
        }
        DisplayText("What will you give [rubi em]?");
        ...
        /*	
            ...
            while(counter < events.length-1) {
                if(events[counter] != 0) MainScreen.addButton(counter,eventNames[counter],functions[counter],events[counter]);
                counter++;
            }
            MainScreen.addButton(9, "Back", rubiAppearance);
        */
    ...
    //(Give Clothes)
    public giveRubiClothes(itype: ItemType = null) {
        DisplayText().clear();
        rubiSprite();
        DisplayText("You hand over the spare set of clothes, and Rubi's eyes light up.  \"<i>For me?</i>\"  the little demon practically screams, ecstatic.  \"<i>ThankyouthankyouthankyouTHANKYOU!</i>\"");
        ...
        else if (itype === armors.TUBETOP) flags[FlagEnum.RUBI_TUBE_TOP] = 1;
        else if (itype === armors.T_BSUIT) flags[FlagEnum.RUBI_BODYSUIT] = 1;
        else if (itype === armors.B_DRESS) flags[FlagEnum.RUBI_LONGDRESS] = 1;
        else if (itype === armors.LTHRPNT) flags[FlagEnum.RUBI_TIGHT_PANTS] = 1;
        else if (itype === armors.NURSECL) flags[FlagEnum.RUBI_NURSE_CLOTHES] = 1;
        else if (itype === armors.S_SWMWR) flags[FlagEnum.RUBI_SWIMWEAR] = 1;
        else if (itype === armors.I_CORST) flags[FlagEnum.RUBI_INQUISITORS_CORSET] = 1;
        else if (itype === armors.BONSTRP) flags[FlagEnum.RUBI_BONDAGE_STRAPS] = 1;
        rubiAffection(20);
        return { next: rubisFuckingHouseYouPervert };
    }

    //Rubi's Bimbofication
    public hypnoBimboficationForRubiSloots() {
        DisplayText().clear();
        DisplayText("You slither out of your [armor], flicking your tongue hungrily at the air, tasting the petite boy's desire. He is so attracted to you, and yet, he's trying so hard to be classy about it. It's cute in its own way, but you'd rather see him be more... honest about his drives. You want the cute, ");
        if (flags[FlagEnum.RUBI_BREAST_SIZE] < 1) DisplayText("femmy boy");
        else if (flags[FlagEnum.RUBI_NO_CUNT] > 0) DisplayText("dick-girl");
        else DisplayText("little slut");
        DisplayText(" ready to lust after your cock openly, to cling to you whenever you arrive and grope your bulge in public. He needs to be less worried about propriety and more focused on sexual satiation and appeal... yes.");
        ...
        if (kGAMECLASS.monk >= 5) DisplayText("familiar ");
        else DisplayText("strange ");
        DisplayText("dark power welling up within you.");
        ...
        if (flags[FlagEnum.RUBI_NO_CUNT] > 0) DisplayText("femboy");
        else DisplayText("hermaphrodite");
        DisplayText(". The longer he stares into your power, the slacker his jaw becomes.");

        DisplayText("\n\n\"<i>That's it, Rubi,</i>\" you soothe. \"<i>Look closer. You can trust me.</i>\"");
        DisplayText("\n\nHe mouths, \"<i>...can trust you....</i>\"");
        DisplayText("\n\nYou slide closer and ");
        if (player.tallness >= 72) DisplayText("bend down");
        else DisplayText("lean forward");
        ...
        if (flags[FlagEnum.RUBI_NO_CUNT] > 0) DisplayText("femboy");
        else DisplayText("dickgirl");
        DisplayText(" must have quite the mind on him if it's making him drip THAT fast.");
        ...
        DisplayText("\n\nBeyond words, the ");
        if (flags[FlagEnum.RUBI_NO_CUNT] === 0) DisplayText("dick-");
        DisplayText("girl");
        if (flags[FlagEnum.RUBI_NO_CUNT] > 0) DisplayText("y-boy");
        DisplayText(" simply gives the slightest of nods as a reply.");
        ...
        DisplayText("\n\n<b>Rubi is now a ");
        if (flags[FlagEnum.RUBI_NO_CUNT] > 0) DisplayText("shemale ");
        DisplayText("bimbo! She's going to be a lot more lusty and airheaded, but isn't that what you wanted?</b>");
        flags[FlagEnum.RUBI_BIMBO] = 1;
        flags[FlagEnum.RUBI_SHE] = 1;
        flags[FlagEnum.RUBI_BREAST_SIZE] = 5;
        //Sex menu
