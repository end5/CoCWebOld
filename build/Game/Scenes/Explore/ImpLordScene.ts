...
export function defeatImpLord(character: Character, imp: Character): NextScreenChoices {
    ...
    if (imp.stats.HP < 1) {
        ...
        // Leave // Rape]
        const choices = [];
        if (character.stats.lust >= 33) choices.push(["Sex", sexAnImpLord]);
...
function impLordBumPlug(character: Character, imp: Character): NextScreenChoices {
    DisplayText().clear();
    DisplayImage(images.showImage("implord-win-male-fuck"));
    let cockThatFits: Cock = character.torso.cocks.find(Cock.CockThatFits(imp.analCapacity()));
    if (cockThatFits) cockThatFits = character.torso.cocks.sort(Cock.SmallestCockArea)[0];
    ...
    DisplayText("\n\nYou drape your " + Desc.Cock.describeCock(character, cockThatFits) + " between the imp's butt cheeks.  Teasingly you begin thrusting between the muscular cleft of his ass, much to the creature's dismay.  The greater imp whines submissively, desperate for you to enter him.  His " + Desc.Cock.describeCockShort(imp.torso.cocks.get(0)) + " drools as you tease his ass, creating a small puddle of pre between his hooves.  The poor creature becomes so desperate that he reaches back and spreads his muscular cheeks with his hands.  You catch a small glimpse of his now gaping hole as your cock continues to slide between his cheeks.  You tease him with one last empty thrust, before grabbing his shoulders and forcing your " + Desc.Cock.describeCock(character, cockThatFits) + " deep inside the poor creature.");

    DisplayText("\n\nThe imp loses control immediately and nearly collapses from the massive orgasm.  His large, corruption-bloated balls clench up, and semen floods out of his " + Desc.Cock.describeCockShort(imp.torso.cocks.get(0)) + " like a fountain.  The hot demon seed puddles around his feet, soaking the ground in his thick boy-goo.");

    DisplayText("\n\nHis orgasm also cause his anus to tighten and spasm around your " + Desc.Cock.describeCock(character, cockThatFits) + ", as if he was milking you of your seed.  Unable to resist, you start plunging yourself rapidly in and out of the spasming hole.");
    // (No line break)
    ...
    if (character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 1 && character.lactationQ() >= 200) {
        ...
        if (character.torso.chest.reduce(BreastRow.AverageNipplesPerBreast, 0) > 1) DisplayText("[nipples]");
    ...
    if (character.torso.balls.quantity >= 4) {
        DisplayText("\n\nAs much as you would love to continue the pleasure you can't last any longer.  You howl in intense pleasure and cum.  Your [balls] tighten against your body, and empty their contents. Your " + Desc.Cock.describeCock(character, cockThatFits) + " pulses and spasms, releasing wave and wave of semen into the greater imp's belly.  The pleasure is intense, and almost painful as your cum.");
        // if(character is a herm)
        if (character.gender === Gender.HERM) DisplayText("\n\nAs your " + Desc.Cock.describeCock(character, cockThatFits) + " erupts, your " + Desc.Vagina.describeVagina(character, character.torso.vaginas.get(0)) + " tenses up tightly, spasming, desperate to be filled.  After a moment your girl juice begins to soak your inner thighs.  Your legs begin to tremble from the intensity of it all, and you question if you'll be able to make it back to your camp after this.");

        DisplayText("\n\nThe imp howls as his ass is flooded with cum.  His stomach begins to expand from the sheer amount of fluid you pump into him, and the sensation of being over filled causes the poor creature to cum a second time, spilling more semen into the already massive puddle.  His " + Desc.Cock.describeCockShort(imp.torso.cocks.get(0)) + " pumps wave after wave of corrupt seed onto the ground, soaking his hooves even further.");

        DisplayText("\n\nYou're sure you must have blacked out at some point as you feel the last of your seed force its way out of your " + Desc.Cock.describeCock(character, cockThatFits) + " and into the imp's demon belly.  You wobble slightly and lean down, grabbing the ragged remains of the imp's loincloth.  Weakly you pull out of the demon's hole, and quickly stuff the cloth in its place as a makeshift plug.");
...
function getBlownByAnImpLord(character: Character): NextScreenChoices {
    DisplayText().clear();
    DisplayImage(images.showImage("implord-win-male-bj"));
    ...
    DisplayText("\n\n");
    // if(cock thickness < 7)
    if (character.torso.cocks.sort(Cock.LargestCockArea)[0].thickness < 7) {
...
// FEMALE VAGINAL
function femaleVagRape(character: Character, imp: Character): NextScreenChoices {
    DisplayText().clear();
    DisplayImage(images.showImage("implord-win-female-fuck"));
    DisplayText("With little ceremony you grab the imp's " + Desc.Cock.describeCockShort(imp.torso.cocks.get(0)) + " and begin to jerk your hand up and down roughly.  The little muscular beast begins to whine loudly, in protest to the rough and likely painful mistreatment of his " + Desc.Cock.describeCockShort(imp.torso.cocks.get(0)) + ".  In spite of the protests, the rough treatment goes over well, as the creature begins to leak hot demon pre across your hand, which you smear across the shaft as a natural hot lube.");
...
export function feederBreastfeedRape(character: Character, imp: Character): NextScreenChoices {
    DisplayText().clear();
    DisplayImage(images.showImage("implord-win-female-breastfeed"));
    ...
    if (character.tallness > 48 && character.tallness < 60 && character.torso.hips.legs.type !== LegType.CENTAUR) {
        ...
        if (character.torso.vaginas.count > 0) {
            DisplayText("  The intense stimulation of your [chest] is causing your [vagina] to become wet with girl juice... so much so that your femcum has started to seep down your taint towards your ass.  It's probably a good thing, as it's now become a lubricant for the greater imp's " + Desc.Cock.describeCockShort(imp.torso.cocks.get(0)) + ".");
        }
        ...
        if (character.torso.chest.count === 1) DisplayText("\n\nAs your second breast's milk flow begins to slow, the imp curls up against you contently.  You cradle him for a moment, before laying the creature down, where he burps and falls asleep.  You chuckle at how cute these creatures are when they're passive.");
            ...
            DisplayText("\n\nNodding your acceptance, he continues his work much more carefully.  He's taking his time again instead of just sucking wildly.  You reach down curiously, and tug on the imp's " + Desc.Cock.describeCockShort(imp.torso.cocks.get(0)) + " but find that it's shrinking.  As you hold it, it shrinks more and more. You wonder what will happen to him if he continues to nurse.");
...
function loseToImpLord(character: Character): NextScreenChoices {
    ...
    DisplayText("\n\nAfter longer than you'd have hoped for, the painful stretching sensation begins to disappear; and the pleasurable sensation of the imp's " + Desc.Cock.describeCockShort(imp.torso.cocks.get(0)) + " thrusting in and out of your [asshole] becomes entirely pleasurable.  The way his " + Desc.Cock.describeCockShort(imp.torso.cocks.get(0)) + " fills every inch of your ass, and rubs all your most sensitive spots.  The weird sensation his warm, demonic pre-cum coats your insides.  You find your lust-blinded mind has become lost in the sensations - so lost that you don't even notice the imp increasing his pace.");

    DisplayText("\n\nWithin moments the beast is wildly thrusting in and out of your [asshole].  Pre-cum is pumping out of his " + Desc.Cock.describeCockShort(imp.torso.cocks.get(0)) + " like a faucet. The hot demon pre begins to spill back out of your abused [asshole], coating your [hips], and dripping to the ground beneath.  The imp gives you a few more rough thrusts before cumming hard into your [asshole].  The little demon's " + Desc.Cock.describeCockShort(imp.torso.cocks.get(0)) + " spasms as he continues to roughly thrust and pump you full of his burning hot demon seed.");
...
// FEMALE LOSE
function getRapedAsAGirl(character: Character, imp: Character): NextScreenChoices {
    DisplayText().clear();
    DisplayImage(images.showImage("implord-loss-female"));
    ...
    DisplayText("\n\nYou try to move, but before you can even begin to squirm a hoof presses hard between your " + Desc.Breast.describeBreastRow(character.torso.chest.get(0)) + ".  You gasp as the air is temporarily knocked out of your lungs.  The demon chuckles at your last feeble attempt to free yourself.  He holds his " + Desc.Cock.describeCockShort(imp.torso.cocks.get(0)) + " stroking it lewdly, a cruel smirk stretching across his face.  You watch as several beads of pre begin to drip from his tip onto your stomach.");
