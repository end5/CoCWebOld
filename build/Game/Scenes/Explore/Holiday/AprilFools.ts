...
export class AprilFools {
    // Encounter Chance 1 out of 40 and only if you're a centaur
    public poniesYN(): boolean {
        if (player.hips.legs.type === LegType.CENTAUR && date.date === 1 && date.month === 3 && Flags.list[FlagEnum.UNKNOWN_FLAG_NUMBER_00118] === 0) {
            DisplayText().clear();
            ...
            MainScreen.simpleChoices(["Approach", "", "", "", "Leave"], [approachPonies, null, null, null, leavePonies]);
    ...
    public approachPonies(): void {
        ...
        DisplayText("The purple one turns to you looking apologetic and a bit exasperated, \"<i>Sorry for our friend startling you. She gets excited <b>really</b> easily.</i>\"\n\n");

        DisplayText("You respond that it's no problem, but she should be more careful approaching strangers");
        if (player.weaponName != "fists") DisplayText(", especially armed strangers");
        DisplayText(".\n\n");

        DisplayText("The blue one looks at you in a puzzled manner while hovering above the ground, \"<i>Actually, you are the first person we have seen who isn't a pony.</i>\"\n\n");

        DisplayText("To be fair, you tell them, you haven't seen any one who IS a pony.  The pink one, now slightly calmer, interjects herself into the conversation \"<i>Ponies, not ponies, whatever!  We have a new friend, and I say PARTY!!!</i>\"\n\n");
        ...
        DisplayText("What should you do? You could party with the horses; after all, you haven't had a reason to party since getting here. You can politely decline for now or leave these oddly colored and slightly disturbing creatures for the more familiar sight of demons.\n\n");

        DisplayText("Whichever you choose, something tells you that you won't see these ponies again.");
    ...
    public derpyParty(): void {
        ...
        player.hips.legs.type = LegType.PONY;
