...
export class XmasOther {
    public candyCaneTrapDiscovery() {
        DisplayText().clear();
        DisplayText("Shortly after embarking into the plains, your thoughts drift back to your life and home in your village.  The slight chill in the air today reminds you of a time of cheer celebrated by your former friends and family.  As days grew shorter and colder, people exchanged humble trinkets, and great feasts were prepared.  ");
        if (player.stats.cor < 50) DisplayText("The memory brings a nostalgic smile to your [face].");
        else DisplayText("The memory reminds you of how bland your world truly was in comparison to Mareth.");
    ...
    //LEAVE
    public leaveBeforeMeetingCandyCaneBun() {
        DisplayText().clear();
        DisplayText("You're not willing to run the risk of getting a foreign cock stuffed up your [butt].");
        //(If corruption or libido>50) 
        if (player.stats.cor > 50 || player.stats.lib > 50) DisplayText("  At least not right now.  You turn back, navigating your way back to your camp.");
        return { next: Scenes.camp.returnToCampUseOneHour };
    }

    //INVESTIGATE
    public investigateCandyCaneBun() {
        DisplayText().clear();
        DisplayText("You make the decision to find out just what's going on.  You cautiously follow the small pools of goo, slightly on edge.  You approach a wall of low hanging vine which you quickly pull aside.  Sitting in the largest pudddle of semen you've seen yet, a trembling bunny-like figure kneels on its knees.");
        //(If corruption and libido<50) 
        if (player.stats.cor < 50 && player.stats.lib < 50) DisplayText("  Your eyes are quickly drawn to the rabbit morph's stark nudity.");
        //(If corruption or libido>50) 
        else DisplayText("  Your eyes are quickly drawn to the rabbit morph's finely curved ass.");
        DisplayText("  A pair of heavily hanging balls confirm the figure to be at least partially male.");
        DisplayText("\n\nYou make the move to ");
        //(If corruption and libido<50)
        if (player.stats.cor < 50 && player.stats.lib < 50) DisplayText("leave, ");
        //(If corruption or libido<50)
        else DisplayText("approach, ");
        DisplayText("but one of your [feet] snaps a twig and causes the bunny to turn at the sound. As its large eyes lock with your, you get your first good look at the figure.  It is predominantly human looking, but the floppy ears sprouting from its head and fluff adorning him from the thighs down indicates at least a little mutation.  Two small nipples lie on a flat chest, confirming him to be a male.  His eight inch dick and kiwi-sized balls leave even less to your imagination.  He is lightly tanned, his skin colored a non abrasive brown.  The hair on his body stands in a sharp contrast to his relatively regular skin.  A mop of shoulder length hair on his head is as white as freshly fallen snow, with sprinklings of red stripes highlighting it.  You can't help but notice how nicely it frames his surprisingly soft, feminine face.  The fur covering his legs is smiliary covered.  His hips jut out notably, adding even further to his overall girly appearance.");
        DisplayText("\n\n\"<i>You're not going to rape me?</i>\"  The bunny boy's remarkably high voice breaks your train of thought.  ");
        //(If corruption<50)
        if (player.stats.cor < 50) DisplayText("You calmly inform him that you didn't have the intention of violating him. ");
        //(If corruption>50)
        else DisplayText("Your train of thought is derailed, but you agree to let the bunny believe whatever he wants to.");
        //(If silly mode on) 
        if (User.settings.silly()) DisplayText("  You're willing to say anything to calm him down.  This kid's voice is even more shrill than that asshole from Glee!");
        DisplayText("  A smile lights up his face, his shoulders visibly drop and lose their tension.   He sits like that for but a moment, before he realizes the full scope of the current situation.  His eyes grow wide while he throws down his arms with a rather unmanly yelp, covering his dripping cock.  Despite his embarrassment, it's evident that he's not going to get off unassisted.");
    ...
    public helpWithTheCandyCane() {
        ...
        //(If corruption<50) 
        if (player.stats.cor < 50) DisplayText("  Not wanting to disturb him, you make your way back to camp and attempt to ignore the burning fire in your loins.");
        //(If corruption>50) 
        else DisplayText("  Despite your rising urge to take advantage of the unknowing bunny, you decide to retire and return to camp.");
    ...
    //[Leave]
    public leaveXmasChicken() {
        DisplayText().clear();
        DisplayText("Too exasperated by the plain absurdity of the situation to deal with it, you bury your face in your hands and decide to leave her to it.  Maybe some imps will shut her up for you.  You head back to camp with her piercing cries following you all the way, and prepare for the day ahead.  Her cries fade a while later; the snow thaws and the air warms soon after.");
        return { next: Scenes.camp.returnToCampUseOneHour };
    }

    //[Talk]
    public talkToXmasChicken() {
        DisplayText().clear();
        DisplayText("Figuring you should at least try asking her to pipe down, you continue your approach.  She finally takes notice of your footsteps crunching the snow beside her and goes still and silent for a moment as she looks up at you.  Now that you're closer, you can make out some more of her features against the snowy ground.  She wears a light dress, almost as pale as her skin and the snow around her, thin enough that you can just see her perky nipples through its white fabric.  A shiver of your own makes you wonder why she's wearing something like that on a day like this.  The thick, fluffy-looking feathers of her wings even manage to stand out as somehow even whiter than the frosty backdrop of the snow.  A faint, silvery glow seems to radiate from her, but it's probably just a trick of the light against the snowfall.");
        ...
        //(if male)
        if (player.gender === Gender.MALE) DisplayText("How's a blowjob sound to you?");
        //(if female)
        else if (player.gender === Gender.FEMALE) DisplayText("How about I eat you out?");
        //(if herm)
        else DisplayText("How about I give you a few minutes to do whatever you want with my mouth?");
        DisplayText("</i>\"  You raise an eyebrow, not entirely sure what to make of her suddenly lascivious nature.  It's certainly quite a change.  She giggles.  \"<i>Oh, don't be surprised.  Where do you think we are");
        //(if sillymode)
        if (User.settings.silly()) DisplayText(", scrub");
        DisplayText("?</i>\"");
    ...
    public acceptChristmasBJs() {
        ...
        if (player.gender === Gender.MALE) {
            ...
            DisplayText().clear();
            DisplayText("Would you like her to service your male parts or lady bits?");
    ...
    public acceptChristmasManBJ() {
        DisplayText().clear();
        let x: number = player.cockThatFits(80);
        if (x < 0) x = player.torso.cocks.sort(Cock.LargestCockArea)[0];
        //(if cock >= ridonkulous)
        if (x.area >= 80) {
            DisplayText("You free your [cock " + (x + 1) + "] from your [armor], signaling your acceptance.  Your monster flops free, and the harpy grabs at her throat reflexively when she looks at it.  She glances up to you with wide eyes and back down at it.  \"<i>A-a promise is a promise, but...</i>\"");
        ...
        //(else if cock would please Goldilocks) (area<= 30 or some shit i dunno whatever the standard is)
            ...
            DisplayText("You free your [cock " + (x + 1) + "] from your [armor], signaling your acceptance.  The immediate rush of cold air that greets it makes you hope she won't be wasting any time.  The pale harpy woman squeals at the sight, raising her hands to her chin and practically bouncing in place.  In a flash she's on her knees, stroking and licking at your meat until it's hard in front of her face.  Her hot breath against your shaft has you more than ready for the sanctuary from the biting cold.  She eyes your erect prick");
            if (player.torso.cocks.count > 1) DisplayText("s");
            DisplayText(" almost hungrily, licking her lips before swiftly engulfing almost its entire length in her mouth.  Her expert technique astounds you; you certainly didn't expect her to have any idea what she was doing but she's able to take you nearly to the base of your shaft in one motion.");
            ...
            //(if only one cock)
            if (player.torso.cocks.count === 1) DisplayText("She grabs onto your hips, enabling herself to move all the quicker.");
            //(if two cocks)
            else if (player.torso.cocks.count === 2) DisplayText("With her other hands she starts to work the shaft of the second cock, pumping in time with the motions of her head.");
            //(if 3+ cocks)
            else DisplayText("She grabs onto two of your other erect cocks with her hands and begins pumping away at them in time with the motions of her head.");
            ...
            if (player.cumQ() < 250) DisplayText("  She closes her eyes and looks quite contented while she swallows your spunk.");
            //(else if cum output is high)
            else if (player.cumQ() < 1000) DisplayText("  Semen gushes out of you for what seems like forever, but the harpy doesn't seem to mind.  She closes her eyes and looks quite contented while she swallows your still-torrenting spunk.");
            //(else if cum output is ridonkulous)
            else DisplayText("  Though at first she looks like she'll be able to handle it, the river of semen exploding from the rip of your cock doesn't stop.  Her eyes widen as the almost unending torrent of spunk fills her, rushing down her throat and into her belly.  You can almost see her bloating with the stuff before it starts to spill back out, dripping white from her lips around your shaft.");
            //(if multicock)
            if (player.torso.cocks.count > 1) DisplayText("  Your cum sprays over her, coating her skin and plumage in a white to rival their own.  Strands of your semen dripping and running over her hardly even stand out on her bright body.");
            DisplayText("  When you finish, you let the harpy go.  She looks just as satisfied as you are.  A trail of cum and saliva still connects your [cockHead " + (x + 1) + "] to her; she breaks it with a lick of her lips and gives your softening member a longing glance before standing up, panting from exertion.");
    ...
    public approachKamiTheChristmasRoo() {
        ...
        if (Flags.list[FlagEnum.KAMI_ENCOUNTER] === 1) {
            ...
            DisplayText("\n\nThe note reads: \"<i>\<i>Hey there " + Desc.Gender.mf(player, "stud", "babe") + "\</i>.  My name's Kami.  Wanna come out the back and fuck?  It can be my special present to you.</i>\"  She signed it with a winking smiley face");
            if (User.settings.silly()) DisplayText(" and found space to scrawl a little penis in one corner. Clever");
            DisplayText(".");
    ...
    public skipOutAfterOpeningKamisBox() {
        DisplayText().clear();
        DisplayText("You politely decline Kami's sexual advances, apologizing before making a turn for the door. You begin to wonder why you didn't buy anything at the bakery.");
    ...
    public chooseLetsGoKami() {
        DisplayText().clear();
        DisplayText("You decide to give Kami exactly what she wants - 'tis the season for giving, after all!  You grin approvingly before heading out into the back alley, taking your time to stare at her wobbling ass.");
        ...
        MainScreen.addButton(1, "Receive Anal", takeItRooButtStyle);
        MainScreen.addButton(2, "Give BJ", KamiBlowJob);
        if (player.torso.cocks.count > 1 && player.cockThatFits(70) >= 0 && player.cockThatFits2(70) >= 0) MainScreen.addButton(3, "DoubleFuck", KamiDoubleDickFuck);
    }


    //[Player chooses 'Give Anal']
    public giveKamiTheChristmasRooAnal() {
        DisplayText().clear();
        let x: number = player.cockThatFits(70);
        DisplayText("You head briskly towards the voluptuous waitress, rearing her around by the wrists and pressing her against the wall.  \"<i>H-Hey, wait!</i>\" Kami yelps, but you sense excitement in her delicate plea.");
        ...
        if (player.cumQ() < 1000) DisplayText("  \"<i>Mmm... just like that, babe,</i>\" Kami moans submissively, her asshole clenching one last time before you pull out and climb off your satiated conquest, your " + Desc.Cock.describeCock(player, x) + " still dripping creamy white spunk.");
        //{if cum quantity ≧ heavy}
        else
            DisplayText("  One of Kami's hands flies from her dick to her suddenly swollen belly as your mighty seed gushes forth.  The unexpectedly heavy girl sinks further into the ground, your load swishing back up and loosening her tunnel around you and forcing you to remove your still-spewing " + Desc.Cock.describeCock(player, x) + " before something unfortunate happens.  Semen starts tumbling out as soon as you dismount your kangaroo conquest, but judging by the slow pace, she's going to look heavily pregnant for a while.");
    ...
    public takeItRooButtStyle() {
        ...
        if (player.torso.balls.quantity > 0) DisplayText("your own");
        //[Else if have vagina]
        else if (player.torso.vaginas.count > 0) DisplayText("your pussy");
        //[Else if neither]
        else DisplayText("your taint");
        DisplayText(".  Each time, the waitress fondles your form with her eager hands.  ");

        DisplayText("\n\n");
        //[If player has pussy] 
        if (player.torso.vaginas.count > 0) DisplayText("Your cunt oozes in return, envious of the tenderized hole above it.  ");
        DisplayText("You dedicate a few more minutes to soaking up every moment of her animalistic romping, savoring the feeling of a hung rod in your punished posterior.  Your climax quickly comes to loom over you, however, the potent, ");
        if (player.torso.cocks.count > 0) DisplayText("prostate");
        else DisplayText("ass");
        DisplayText("-numbing sensations at your rear becoming too much to bear.  Slamming back into Kami's pole one last time, you breathe a shuddered sigh of relief as your orgasm takes over, the sensation spreading throughout your entire body like electricity.");
        //[If male]
        if (player.torso.cocks.count > 0) DisplayText("  Your pent-up cock ejaculates there and then, shamelessly gushing sloppy semen against the wall in front of you.");
        //[If female]
        if (player.gender === Gender.HERM) DisplayText("  Meanwhile, y");
        else if (player.torso.vaginas.count > 0) DisplayText("Y");
        if (player.torso.vaginas.count > 0) DisplayText("our eager pussy buckles in arousal, gushing fem-juices on the floor below you.");
        DisplayText("  You're truly spent.");
    ...
    public KamiBlowJob() {
        ...
        if (player.torso.cocks.count > 0) DisplayText("  You think you might still need to find an outlet for that itch of yours, with [eachCock] still erect and needing release.");
    ...
    public KamiDoubleDickFuck() {
        ...
        DisplayText("\n\n\"<i>I could do this all goddamn day!</i>\"  Kami screams, your wicked thrashing leaving her pelvis moist with sheer bliss.  You can already feel your own orgasm approaching, so with due haste you grab Kami's ample bust and pick up your thrusts energetically, ramming your cocks home with as much speed as you can muster.  Your ass-engorged " + Desc.Cock.describeCock(player, x) + " cums first, packing Kami's tight hole with your sticky seed, shortly followed by your other " + Desc.Cock.describeCock(player, y) + " giving her delicate-yet-punished pussy the same treatment.");
    ...
    public getWinterPudding() {
        DisplayText().clear();
        if (player.inventory.gems < 35) {
            DisplayText("Sadly, the pudding isn't free, and it looks like you don't have the 35 gems you'd need to try a bite!");
            return { next: telAdre.bakeryScene.bakeryuuuuuu };
            return;
        }
        player.inventory.gems -= 35;
        statScreenRefresh();
        DisplayText("You opt for a slice of the tasty treat - you've only got until the festivities wade away, so you might as well take one now!");
        ...
        if (player.torso.neck.head.horns === 0 && randInt(2) === 0) {
            ...
            player.torso.neck.head.hornType = HornType.ANTLERS;
            player.torso.neck.head.horns = 4 + randInt(12);
        }
        //[Show this description instead if the player already had horns when the transformation occurred.] 
        else if (player.torso.neck.head.horns > 0 && player.torso.neck.head.hornType != HornType.ANTLERS && randInt(2) === 0) {
    ...
    //Unwrap
    public unwrapPolarPete() {
        DisplayText().clear();
        DisplayText("The temptation too strong, you walk closer to Pete.  Sinking down to your knees before him as he adjusts his hips to your level.  Your hands on his crimson pants, pulling them down with bated breath.  His dark ebony length flops free, long and swollen with heat before your nose. Your hand rubs along it's smooth black surface coaxing a pleased groan from Pete.");
        ...
        if (player.torso.cocks.count > 0) DisplayText("  [EachCock] grows stiff and soaks your [armor] with precum.");
        //(if vagina) 
        if (player.torso.vaginas.count > 0) DisplayText("  Your [vagina] grows moist, dripping eager juices and soaking your [armor].");
        ...
        player.orgasm();
        inventory.takeItem(consumables.PEPPWHT, Scenes.camp.returnToCampUseOneHour);
    }

    //Leave
    public leaveDisFukkinPolarBear() {
        DisplayText().clear();
        DisplayText("You've decided not to trust the stranger, with a nod of your head you walk away from the man.  \"<i>Wait!  Before you go...</i>\" he speaks as he moves his massive sack in front of himself with a huff.  He leans forward, his upper body rummaging through the sack before reappearing.  He hands you a brightly wrapped gift.");
    ...
    public peppermintWhite(player: Player) {
        DisplayText().clear();
    ...
    public nieveBuilding() {
        ...
        //Alters Nieve's eyes in scenes
            ...
            DisplayText("You approach your snow" + nieveMF("man", "woman") + " again, looking it over.");
            ...
            MainScreen.addButton(4, "Back", Scenes.camp.returnToCampUseOneHour);
        }
        //Fourth Step: The Nose
        else if (Flags.list[FlagEnum.NIEVE_STAGE] === 3) {
            DisplayText("You approach your nearly finished snowman. To others, it might be a perfectly acceptable creation, but not to you. It needs a carrot to finish it off.");
            ...
            //(Else) 
                ...
                DisplayText("Unfortunately, you've yet to find one in your adventures.  You suppose you'll have to look more carefully.  Who knows, there might be a farm right under your nose.");
            }
            return { next: Scenes.camp.returnToCampUseOneHour };
        }
        else {
            DisplayText("Your snowman is done!  There's nothing more to add to it.  It looks mighty fine however, and just looking at it brings a nostalgia-fueled smile to your lips.");
            return { next: Scenes.camp.returnToCampUseOneHour };
        }
    }
    //Coal
    //Add to previous text if possible?
    public nieveCoalEyes() {
        DisplayText().clear();
        player.inventory.items.consumeItem(consumables.COAL___);
        DisplayText("Luckily, you happen to have two lumps of coal.");
    ...
    public nieveGemEyes() {
        DisplayText().clear();
        if (player.inventory.gems >= 9) {
            DisplayText("Taking a handful of gems out of your pouch, you spread them evenly around the Snowman's face, giving it a nice, vacant smile.  It still needs a nose, however, and for that, you'll need a carrot.  Perhaps there's a farm nearby?");
            player.inventory.gems -= 9;
            statScreenRefresh();
        ...
        //Too broke to use the gems option? What the fuck is wrong with you?
            ...
            DisplayText("You open up your pouch, and frown.  Unfortunately, you don't have enough gems to create the eyes and mouth. With a sigh you march your broke ass back to camp.");
        }
        return { next: Scenes.camp.returnToCampUseOneHour };
    }

    //Snowwoman
    public nieveSnowWoman() {
        DisplayText().clear();
        //Add to existing text if possible, rather than a new window?
        DisplayText("You grin mischievously to yourself and set about making two more balls of powdery snow.  It takes less time than any of the others, and before you know it you've attached two icy-breasts to the snowman.  They aren't terribly big, any heavier and you're sure they'd fall off, but they get the point across.");
    ...
    public nieveSnowMan() {
        DisplayText().clear();
        DisplayText("You decide to leave it as is. Not everything has to have breasts, of course, even in Mareth.");
    ...
    public findACarrot() {
        DisplayText().clear();
        DisplayText("As you explore the farm, you come across several rows of green plants.  Getting closer, you recognize them... carrots!  You pull one of them from the ground.  It's expectedly dirty, but bright orange, and straight enough to be the perfect nose for your snowman.");
    ...
    public nieveComesToLife() {
        ...
        if (Flags.list[FlagEnum.NIEVE_GENDER] === 1) DisplayText("  It appears to be male, judging by the modestly sized cock sprouting from its loins.");
        else DisplayText("  It appears to be female, judging by the modestly sized tits on its chest, and the lack of a penis, which is quite a feat in Mareth.");
        DisplayText("  Pure white hair spills down from its head, almost blending in with the surrounding snow.");
        ...
        if (Flags.list[FlagEnum.NIEVE_MOUTH] === "gems") DisplayText("glittering purple");
        else DisplayText("coal black");
        DisplayText(" orbs, fall on you, as if registering you for the first time.  Then a hint of recognition hits " + nieveMF("him", "her") + ".  \"<i>Oh!  [Master]!  You're my [master]!</i>\"  In a flash " + nieveMF("he", "she") + "'s standing, looking pleased as punch, with " + nieveMF("his", "her") + " hands clasped excitedly in front of " + nieveMF("himself", "herself") + ".");
    ...
    public yesKeepNieve() {
        DisplayText().clear();
        DisplayText("You actually laugh at the question.  Of course you want " + nieveMF("him", "her") + " to stay!");
        //[Silly mode: 
        if (User.settings.silly()) DisplayText("  A free follower, no worries about buying worthless dye or raising their affection?  Fuck yeah, you'll take " + nieveMF("him", "her") + "!");
        DisplayText("  " + nieveMF("He", "She") + " doesn't seem to be a threat, and indeed seems sincere in the fact that " + nieveMF("he", "she") + " was sent here to be your lover.");
    ...
    public noNoKeepNieve() {
        DisplayText().clear();
        DisplayText("You shake your head.  Of course not!  While " + nieveMF("he", "she") + " certainly seems like a nice person on the surface, you can't help but think that's just the tip of the iceberg.  For all you know, " + nieveMF("he", "she") + "'s a frigid bitch underneath, a trap set by the demons to lure you into a false sense of security.  Clever bastards, you conclude.  They certainly know the best way to serve revenge, but you won't have any of it.");
    ...
    //Followers Menu
    //Camp Description
    public nieveCampDescs() {
        //6:00
        if (model.time.hours === 6) DisplayText("Nieve is sitting cross-legged in the snowdrift, munching on what looks to be icicles.  As you watch, " + nieveMF("he", "she") + " reaches down into the glittering powder surrounding " + nieveMF("him", "her") + " and produces another one.");
        //7:00
        else if (model.time.hours === 7) DisplayText("Nieve is giggling and laughing as a fresh flurry flutters down on " + nieveMF("him", "her") + ".  You wonder idly if " + nieveMF("he", "she") + "'s causing it to happen.");
        //8:00
        else if (model.time.hours === 8) DisplayText("Nieve is sitting in the white wintery wonderland, carefully constructing a snowman.  It strikes you as strange, and almost masturbatory in a way.  " + nieveMF("He", "She") + " spots you and gives you a wave and a smile.  For a moment, you think the snowman has too, but you eventually conclude it's your imagination.");
        //9:00
        else if (model.time.hours === 9) DisplayText("Nieve is sprawled out in the icy field, creating a series of snow angels without a care in the world.  When " + nieveMF("he", "she") + " catches sight of you, she gives a friendly wave, then dives back into the snow.");
        //10:00
        else if (model.time.hours === 10) DisplayText("Nieve, the ice spirit, sits quietly at the edge of the snowdrift, looking out at the landscape beyond.  When " + nieveMF("he", "she") + " sees you looking, " + nieveMF("he", "she") + " gives a somber wave.");
        //11:00
        else if (model.time.hours === 11) DisplayText("Nieve is in your camp, poking around your supplies and water barrels. You notice " + nieveMF("he", "she") + " seems a little uncomfortable to be away from " + nieveMF("his", "her") + " snowdrift.");
        //12:00
        else if (model.time.hours === 12) DisplayText("Nieve is carefully compressing snowballs in " + nieveMF("his", "her") + " little camp.  To your surprise, " + nieveMF("he", "she") + " picks one up and begins eating it like you would an apple.  When " + nieveMF("he", "she") + " catches you looking mid-bite, " + nieveMF("he", "she") + " gives you a grin with her mouth full, " + nieveMF("his", "her") + " cheeks puffed out.");
        //13:00
        else if (model.time.hours === 13) DisplayText("Nieve is sitting cross-legged, in the middle of what appears to be an argument with a snowman " + nieveMF("he", "she") + "'s just made.");
        //14:00
        else if (model.time.hours === 14) DisplayText("Nieve is in " + nieveMF("his", "her") + " usual spot, seemingly deep in concentration.  Around " + nieveMF("him", "her") + " snow begins to fall slowly, then quicker and quicker, whipping " + nieveMF("his", "her") + " hair around wildly.  Although strangely, the air is still where you are.");
        //15:00
        else if (model.time.hours === 15) DisplayText("\n\nNieve the ice spirit is whirling around " + nieveMF("his", "her") + " area, practicing with what appears to be a translucent blue spear.  " + nieveMF("He", "She") + " jabs and thrusts, spins and swipes.  " + nieveMF("He", "She") + " may be new to Mareth, but you've got to hand it to " + nieveMF("him", "her") + ", it looks like " + nieveMF("he", "she") + " could take care of " + nieveMF("him", "her") + "self.");
        //16:00
        else if (model.time.hours === 16) DisplayText("\n\nNieve is at first nowhere to be seen.  Then you see " + nieveMF("his", "her") + " head pop out of a snowdrift.  " + nieveMF("He", "She") + " looks around quickly, then leaps into the air, diving head first into another pile of the powdery stuff.");
        //17:00
        else if (model.time.hours === 17) DisplayText("\n\nNieve appears to have constructed a firepit constructed from translucent blue logs and rocks.  A vivid blue flame roars in the pit, over which Nieve appears to be roasting... marshmallows?  As you watch " + nieveMF("he", "she") + " peels the burnt black skin off a marshmallow, gobbles it down, and begins roasting the rest.");
        //18:00
        else if (model.time.hours === 18) DisplayText("\n\nNieve seems to be working on a spear, sharpening it with a jagged piece of ice.  When " + nieveMF("he", "she") + " catches sight of you " + nieveMF("he", "she") + " waves and gives a friendly smile.");
        //19:00
        else if (model.time.hours === 19) DisplayText("\n\nNieve is sitting at the edge of " + nieveMF("his", "her") + " icy expanse, staring off at the distant mountains.  You wonder if " + nieveMF("he", "she") + "'s looking for home.");
        //20:00
        else if (model.time.hours === 20) DisplayText("\n\nNieve is, surprisingly, outside of " + nieveMF("his", "her") + " cold camp.  " + nieveMF("He", "She") + "'s dancing along the battered, parched ground, calling down snow.  Wherever a flake hits, it's immediately devoured by the thirsty earth.  Is Nieve perhaps trying to... water the ground?");
        //21:00
        else DisplayText("\n\nYou can hear Nieve sleeping soundly from within a small ice-fort.");
    }

    //Appearance Screen
    public approachNieve() {
        DisplayText().clear();
        DisplayText("You wave at Nieve, getting " + nieveMF("his", "her") + " attention and calling for " + nieveMF("him", "her") + ".  The icy spirit-" + nieveMF("man", "woman") + " happily walks over, smiling.  " + nieveMF("He", "She") + " takes " + nieveMF("his", "her") + " time, allowing you to take in " + nieveMF("his", "her") + " naked body.");
        ...
        //[Silly Mode:]
        if (User.settings.silly()) DisplayText("  And you presume a butthole nestled between " + nieveMF("his", "her") + " cheeks, right where it belongs.");
        DisplayText("  " + nieveMF("His", "Her") + " stomach is flat and toned, " + nieveMF("as is his chest", "and she possesses a pair of perky B-cup breasts") + ". " + nieveMF("His", "Her") + " face is the same pale blue as the rest of " + nieveMF("his", "her") + " body, though it is off-set by " + nieveMF("his", "her") + " glittering, ");
        if (Flags.list[FlagEnum.NIEVE_MOUTH] === "coal") DisplayText("dusky black");
        else DisplayText("vibrant purple");
        DisplayText(" eyes and pure white hair, which " + nieveMF("barely goes past his ears", "tumbles down past her shoulders") + ".  Much of " + nieveMF("his", "her") + " body glimmers with a fine layer of powdered snow or ice.");
    ...
    public nieveSexMenu() {
        DisplayText().clear();
        DisplayText("What will you do with your oh-so-cool lover?");
        if (player.stats.lust < 33) DisplayText("  You aren't quite turned on enough for normal fucking.");
        ...
        if ((player.torso.cocks.count > 0 || (player.torso.vaginas.count > 0 && player.torso.clit.length >= 3.5)) && player.stats.lust >= 33) {
    ...
    public lickNieve() {
        DisplayText().clear();
        DisplayText("You explain your intentions and how you'd like to be the one to give her pleasure.  She looks rather surprised at first, but then she smiles.  \"<i>You want to go down... on me?</i>\"  You nod and she giggles, \"<i>That would be wonderful!</i>\"");
        ...
        if (!silly()) {
            ...
            if (player.torso.hips.legs.isTaur()) DisplayText(", or as close to it with your body,");
            DisplayText(" with Nieve straddling your face.  \"<i>Mm, this is much better.  Now you wanted to lick me, [Master]? So... lick,</i>\" she whispers, lowering her chilly cunt down onto your face.");
        ...
        //Silly Mode:
            ...
            //(no Kid A: 
            if (Flags.list[FlagEnum.ANEMONE_KID] != 1) DisplayText(", Nieve takes a ladle full of water and pours it onto her snatch");
            //Kid A: 
            else DisplayText(", your little anemone spawn giggles and laughs at your predicament before finally ladling some water onto your lover's snatch");
            DisplayText(", effectively allowing you to pull your tongue away.");
    ...
    public fuckNieve() {
        ...
        if (x >= 0) {
            if (player.torso.cocks.get(x).length < 6) DisplayText("  \"<i>It's so cute, [Master],</i>\" Nieve says while catching a breath, \"<i>Like a little toy cock.</i>\"");
            //If overly large (>130): 
            else if (player.torso.cocks.get(x).length >= 24) DisplayText("  She has to take a breath almost immediately, saying, \"<i>By the fat man's beard this thing is huge.  You must make the rest of this world jealous.</i>\"");
            else DisplayText("  \"<i>Mmm,</i>\" she smacks her lips and takes a quick breath, \"<i>Such a nice example of a candycane right here.  Let's go in for another taste.</i>\"");
        }
        else {
            if (player.torso.clit.length < 6) DisplayText("\"<i>It's so cute, [Master],</i>\" Nieve says while catching a breath, \"<i>Like a little toy cock.</i>\"");
            else if (player.torso.clit.length >= 24) DisplayText("  She has to take a breath almost immediately, saying, \"<i>By the fat man's beard this thing is huge.  You must make the rest of this world jealous.</i>\"");
            else DisplayText("  \"<i>Mmm,</i>\" she smacks her lips and takes a quick breath, \"<i>Such a nice example of a candycane right here.  Let's go in for another taste.</i>\"");
        }
        DisplayText("  And with that she dives back down onto your " + cockClit(x) + ".  Her head bobs and bows, giving the sensation of an icecube running up and down your ");
        if (x >= 0) DisplayText("dick");
        else DisplayText("clit");
        DisplayText(" at great speed.");

        //(Variable depending on dick-type)
        if (x >= 0) {
            //(Human) 
            if (player.torso.cocks.get(x).type === CockType.HUMAN) DisplayText("  Your normal, unaltered cock seems to take on a slight blue tint, although you can confirm it's not going numb.  Quite the contrary, it seems to be getting more sensitive as Nieve's lips roll over it.");
            //(Equine) 
            else if (player.torso.cocks.get(x).type === CockType.HORSE) DisplayText("  Nieve fawns over your horsecock like a cultist praying to a god.  She sucks and kisses, running the tip of her tongue all along the sides until she reaches the sheathe, and then right back up the other side.");
            //(Canine) 
            else if (player.hasKnot(x)) {
                DisplayText("  Nieve eyes the knot on your cock as though it were a challenge.  She inches down your shaft, getting ever closer.");
                //(Cocksize >40)
                if (x.area >= 40) DisplayText("  Eventually, though, she admits defeat and goes back to working your shaft, though she eyes that knot from time to time with no small amount of anger.");
                //(Cocksize <40) 
                else DisplayText("  It takes her a few good minutes, but eventually she works her way down and takes the entire knot into her mouth.  She raises her hands in triumph and lets out a \"<i>Mmmmrrrrrpphhh!</i>\" before pulling back for a breath, inadvertently exposing your wet hard cock to the cold, wintery air.");
            }
            /*Cock types
            0 - human
            1 - horse
            ...
            */
            //(Demonic)
            else if (player.torso.cocks.get(x).type === CockType.DEMON) DisplayText("  Your purple, demonic shaft pulses and throbs, seemingly immune to the unnatural cold of Nieve's mouth. Seemingly oblivious to its nature, Nieve slavers over each nodule, giving every ridge and bump a kiss and more than just a lick.");
            //(Tentacle) 
            else if (player.torso.cocks.get(x).type === CockType.TENTACLE) DisplayText("  Nieve does not seem put off by your tentacle cock in the least, in fact she confesses, \"<i>It reminds me of a comic I read before coming here.</i>\"  As she fawns over your cock, you're certain you hear whispers of \"<i>Oh, you sexy tentacle beast...</i>\" coming from her mouth when she gets her breath.");
            //(Feline) 
            else if (player.torso.cocks.get(x).type === CockType.CAT) DisplayText("  Though the barbs on your cock are soft and pliable, they reflexively harden up after Nieve's tongue washes over them.  She doesn't pay particular attention to the barbs, but she does seem rather careful not to get one hooked on her lip.");
            //(Lizard)
            else if (player.torso.cocks.get(x).type === CockType.LIZARD) DisplayText("  Nieve fawns over your scaled and ridged reptilian cock, slurping and smooching up and down its length. It seems a bit more resistant to the biting cold of Nieve's mouth, which is lucky when she backs off, exposing your entire wet, frigid length to the wintery air surrounding you.");
            //(Anemone) 
            else if (player.torso.cocks.get(x).type === CockType.ANEMONE) DisplayText("  Nieve giggles whenever one of your wriggling tentacles stings her, seemingly immune to its effects, and kisses it back.  Soon after she devours your whole length, delighted to feel the sting of your cock in the back of her throat.");
            //(Kangaroo) 
            else if (player.torso.cocks.get(x).type === CockType.KANGAROO) DisplayText("  It takes Nieve a little while to catch on that your cock isn't quite human, and by the time she figures it out, she mutters, \"<i>Huh, that's strange... I thought kangaroos had balls above the cock,</i>\" before shrugging and getting back to work.");
            //(If you add dicks later, placeholder text:) 
            else DisplayText("  Nieve works on your mutant cock, spending time working every unique little facet of it.");
        }
        //(Clitoris) 
        else {
            DisplayText("  Nieve nuzzles and lavishes your elongated love button, showering it with attention. \"<i>I didn't know clits could get this big,</i>\" she says with wonder in her voice. \"<i>");
            if (player.torso.clit.length < 5) DisplayText("It's just like a little cock");
            else if (player.torso.clit.length < 9) DisplayText("It's just like a cock");
            else DisplayText("I bet even men get jealous of this bad girl");
            DisplayText(".</i>\"");
        }
        ...
        if (x >= 0) DisplayText("  Your " + cockClit(x) + " quivers and spurts cum directly into Nieve's eager mouth, who gulps it down as if she were drinking direct from the tap.");
        DisplayText("\n\nAs you come down from your little orgasmic high, you glance down at the ice woman, ");
        if (x < 0 || player.cumQ() <= 250) DisplayText("who has a bit of cum dribbling down her chin");
        else if (player.cumQ() <= 400) DisplayText("who has a steady stream of cum pouring from her mouth");
        else if (player.cumQ() < 600) DisplayText("who is patting her stomach as though she'd just had a large meal");
        else if (player.cumQ() < 1000) DisplayText("whose stomach appears slightly distended from the amount of cum you've poured into her");
        else DisplayText("whose stomach is rather large, like a pregnant woman's, from the sheer amount of cum you've pumped into her gullet");
        DisplayText(".  With a smirk, you tell her it's not over yet.  That was just the appetizer.");
        ...
        if (x >= 0) DisplayText("with your " + Desc.Cock.describeCock(player, x) + "");
        else DisplayText("with yours");
        DisplayText(", and just slipping the tip in before pulling out.  You do this until your snowbound lover is biting her lip, squirming with barely contained pleasure.");

        DisplayText("\n\nRight as she begins to open her mouth, to beg you to just stop it and fuck her, you slip your " + cockClit(x) + " in ");
        //(if too big: 
        if (x >= 0) {
            if (x.area >= 130) DisplayText("at least as far as it will go, ");
        }
        DisplayText("with a wet \"<i>schluck.</i>\"  The words coming out of her mouth are lost in a loud \"<i>ooohhhh</i>\" as her eyes roll backwards, closely followed by her head.  Her wet pussy grips you tightly, giving you a strange tingling sensation.  Your hot rod combined with her arctic nethers actually causes some steam to rise up, and you're a little captivated to see more tufts of steam rise with every slow, gentle thrust you make.");
        ...
        if (x >= 0) {
            //((cocksize 3/Clit 3)
            if (x.area <= 7) DisplayText("Your meager penis isn't much, but at this angle you manage to hit all the right places.  Soon Nieve is wriggling around, eyes staring up at you, urging you on faster and faster.");
            //(Cocksize 30/Clit 12) 
            else if (x.area <= 30) DisplayText("At this angle you manage to hit all the right places.  Soon Nieve is wriggling around, eyes tightly shut as she whispers dirty words you can't even make out.");
            //(Cocksize 70/Clit 24)
            else if (x.area <= 70) DisplayText("Nieve's cunt feels like an icy vice on your oversized cock, but even so, you manage to hit all the right places.  Soon she is wriggling around, eyes tightly shut as she pants for breath.");
            //(Cocksize 130+/Clit 48+)
            else DisplayText("Though you can't fit your entire " + cockClit(x) + " inside, you are amazed at how much she takes.  Her pale blue stomach bulges with every thrust.  Soon Nieve is wriggling, her eyes slightly open as she bites her lip in ecstasy.");
        }
        else {
            if (player.torso.clit.length <= 4) DisplayText("Your meager clit isn't much, but at this angle you manage to hit all the right places.  Soon Nieve is wriggling around, eyes staring up at you, urging you on faster and faster.");
            else if (player.torso.clit.length <= 12) DisplayText("At this angle you manage to hit all the right places.  Soon Nieve is wriggling around, eyes tightly shut as she whispers dirty words you can't even make out.");
            else if (x.area <= 24) DisplayText("Nieve's cunt feels like an icy vice on your oversized clitty, but even so, you manage to hit all the right places.  Soon she is wriggling around, eyes tightly shut as she pants for breath.");
            //(Cocksize 130+/Clit 48+)
            else DisplayText("Though you can't fit your entire " + cockClit(x) + " inside, you are amazed at how much she takes.  Her pale blue stomach bulges with every thrust.  Soon Nieve is wriggling, her eyes slightly open as she bites her lip in ecstasy.");
        }
        ...
        //((Clitfucking?)
        if (x < 0) DisplayText("  Your [clit] suddenly feels suddenly warm, like every nerve ending inside were on fire. It aches and pulses with the rest of your body, as though it were a little cock longing to cum.");
        else {
            DisplayText("  Your " + cockClit(x) + " aches and pulses with need before ");
            if (player.cumQ() < 100) DisplayText("spilling its meager offering into the ice spirit's womb");
            else if (player.cumQ() < 250) DisplayText("ejecting streams of white hot jizz into the ice spirit's hungry snatch");
            else if (player.cumQ() < 400) DisplayText("painting the ice spirit's insides with your cream until some begins to pour out");
            else if (player.cumQ() < 700) DisplayText("arcs of pearly white cum shoot forth, filling the ice spirit's womb so much her belly visibly inflates");
            else DisplayText("erupting inside, filling the ice spirit up with so much of your hot cum so much that she looks visibly pregnant");
            DisplayText(".");
        }
    ...
    //Any Gender
    //Nieve's cock is 9x1.5
    public nieveFucksYou() {
        DisplayText().clear();
        DisplayText("Your arrival to Nieve's part of the camp elicits a friendly smile from the ice man.  He notices the longing in your eyes with a grin, and speaks in a gentle, yet low tone, \"<i>[Master], you seem to need help from me...</i>\"");
        DisplayText("\n\nTo which you nod, ");
        if (player.stats.cor < 50) DisplayText("telling him that you find his form... appeasing to say the least, and would like to know how his 'parts' are working properly.");
        else DisplayText("leting your creation know that you are interested in 'how' it can please you, giving him no room to imagine what you're insinuating you then tell him bluntly that you want him to fuck you like the plaything he is.");
        ...
        //{Silly Mode:}
        if (User.settings.silly()) DisplayText("   He did say that his kind were effectively love slaves, and boy does this prove it!");
        //[if (hasVagina = true) 
        if (player.torso.vaginas.count > 0) DisplayText("  Your [vag] is a sodden box so soon after he started, and you start to want more, much more!");
        //[if (hasCock = true) 
        if (player.torso.cocks.count > 0) DisplayText("  Your [cock biggest] is also rock hard in a short while and you quickly desire some more stimulation from your icy love slave.");
        ...
        if (player.torso.vaginas.count > 0) MainScreen.addButton(1, "Vaginal", takeNieveVaginal);
    }
    //Anal
    public takeNieveAnal() {
        DisplayText().clear();
        ...
        Mod.Butt.displayStretchButt(player, 9, true, true, false);
        ...
        //[if (hasCock = true) 
        if (player.torso.cocks.count > 0) DisplayText("  Nieve then bends forward while keeping his pace to give you a reach-around.  Grabbing your own [cock biggest] in one hand, he starts to pump it to the speed of his thrusts, torturing you with the extra-stimulation in such a good way.");
        //[if (hasVagina = true)
        else if (player.torso.vaginas.count > 0) DisplayText("  As Nieve continues to pound away, he places one hand on your sopping wet cunt and begins to finger you in time with his assault, making you lust-drunk with the stimulation as he finger bangs you.");

        DisplayText("\n\nNieve continues to grunt as he prods and plunges into your ");
        //if (silly mode) 
        if (User.settings.silly()) DisplayText("pooper");
        else DisplayText("[asshole]");
        DisplayText(" as you feel an oncoming climax.  Nieve, furiously impaling you with his long and cold cock, comes first, and in one final thrust drives his penis as deeply as possible, flooding your bowels with a crisp current of cum as his body rocks from the orgasm.  You join him in the throes of ecstasy as soon as his jizz hits your inner walls");
        if (player.gender > 0) {

            //[if (hasCock = true) 
            if (player.torso.cocks.count > 0) {
                DisplayText(", ");
                DisplayText("while your body undulates and seizes as cum starts to spew forth from [eachCock]");
                //(normal volume: 
                if (player.cumQ() < 200) DisplayText(", as you cum, you leave a small puddle on the floor below you");
                else if (player.cumQ() < 500) DisplayText(", as you cum you start to dress your arms and hands with your own cream as you shoot your stuff out onto the floor");
            }
            //[if (hasVagina = true)
            if (player.torso.vaginas.count > 0) DisplayText(".  Your soaking vagina makes your body quake and shudder as you orgasm, splashing your femcum all over Nieve and the ground");
        }
        DisplayText(".");
    ...
    //Vaginal
    public takeNieveVaginal() {
        DisplayText().clear();
        DisplayText("You're already wet from the foreplay, and it shows.  Moreso, Nieve notices, with a grin similar to the one he gave you earlier.  The look on his face suggests he knows what you want, he's just merely waiting for his [master] to give the word.");
        ...
        //[if (hasBalls = true) 
        if (player.torso.balls.quantity > 0) DisplayText(", gently moving your [balls] out of the way in the process");
        DisplayText(".  He slams his nine inch blue rod balls deep into your sodden box.  A loud squelch fills the air at the penetration, giving Nieve the go ahead to start in top gear, pistoning in and out at a hurried pace.");
        //[if (hasCock = true)
        if (player.torso.cocks.count > 0) DisplayText("  During the pounding Nieve grabs your dangling [cock biggest] and starts to pump it in time with his own thrusts, quite eager to please his [master].  Every time your feminine half cums, so does your male half, splashing its happy seed into the valley that your combined bodies have made.");
        //[if (isLactating = true) 
        if (player.lactationQ() >= 100) DisplayText("  Your ice man reaches out with a free hand to caress your [chest], twiddling his fingers around your delicate [nipples]. After some rough pulling and flicking, pounding away all the while, he feels milk dribble onto his hand.  He catches a quick glance of the liquid before diving head-first to suckle on your milk faucets, drinking heavily of your cream as he's about to give you his.");
    ...
    public nieveIsOver() {
        ...
        if (Flags.list[FlagEnum.NIEVE_STAGE] < 5) {
            DisplayText("You wake up with a yawn and a stretch. It feels just like any other day, though a bit warmer.  Quite a bit warmer in fact.  It feels like winter's grasp on Mareth has slipped, and it's entering Spring.  Curiously, you glance over towards the snow which had been recently deposited in your camp, and note it's melting away before your eyes.");
            DisplayText("\n\nOh well, not a huge loss, you figure.  It was fun while it lasted.");
            Flags.list[FlagEnum.NIEVE_STAGE] = 0;
        }
        //Nieve Completed
        else {
            DisplayText("You wake up with a yawn and a stretch.  It feels just like any other day, though a bit warmer.  Quite a bit warmer in fact.  It feels like winter's grasp on Mareth has slipped, and it's entering Spring.  You consider the fact absently at first, but then you stop dead in your tracks.  What about Nieve?!");
            ...
            if (player.stats.cor < 50) DisplayText("  You even tear up yourself, sad to see such a good friend leave, and so abruptly.");
            //Corruption >50: 
            else DisplayText("  You even feel a little sad yourself, it's such a pity to see a good, obedient fucktoy leave.");
            DisplayText("  The snowman collapses on itself, melting away before your very eyes.  In a matter of hours, this entire area will be nothing but a dried up, dehydrated desert once more.  But as the jingling fades out into the distance, you vow to hold on to the tear.");
    ...
    private fixNieve() {
        if (Flags.list[FlagEnum.NIEVE_GENDER] === 0) {
            DisplayText().clear();
            DisplayText("(There was an error with stat tracking that cleared Nieve's stats out at the end of last year's event. <b>What gender do you want Nieve to be?</b>)");
            ...
            DisplayText().clear();
            DisplayText("(There was an error with stat tracking that cleared Nieve's stats out at the end of last year's event. <b>What were Nieve's eyes and mouth made out of?</b>)");
    ...
    private fixNieveGender(arg: number = 1) {
        Flags.list[FlagEnum.NIEVE_GENDER] = arg;
        fixNieve();
    }
    private fixNieveMouth(arg: number = 1) {
    ...
    public returnOfNieve() {
        DisplayText().clear();
        DisplayText("As you awake in the morning you find yourself shivering slightly.  A cool breeze sweeps over your camp, while in the distance jingling bells can be heard.  How odd.  You haven't heard bells like that since...");
    ...
    private nieveReturnsPartII() {
        DisplayText().clear();
        DisplayText("You sigh, resigning yourself to your companion's fate.  However, as you rise from the snow to return to camp, you hear a soft, muffled voice.  Perplexed, you crawl forward in the snow, frantically seeking out the source.");
        ...
        if (Flags.list[FlagEnum.NIEVE_MOUTH] === "coal") DisplayText("coal black");
        else DisplayText("glittering purple");
        DisplayText(" eyes shine with crystallizing tears, not entirely unlike the ones " + nieveMF("he", "she") + " left with you on your last meeting.  \"<i>I'm so glad to be back!  I was worried I would be sent somewhere else, but I had so much fun here, and I told the big man that, and then Winter came and I wasn't sure you were still here, and then it turned out I might have gone to someone else, but oh my goodness I'm so happy I came back!</i>\" Nieve belts out without pause for breath before wrapping " + nieveMF("his", "her") + " arms around you once again.");
