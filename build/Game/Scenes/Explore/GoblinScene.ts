...
export class GoblinScene {
	...
	public goblinRapesPlayer(): void {
		...
		if (player.perks.has(PerkType.BimboBrains) || player.perks.has(PerkType.FutaFaculties)) {
			...
			if (player.torso.vaginas.count > 0) {
				DisplayText(images.showImage("goblin-loss-female-bimbodildo"));
				...
				DisplayText("Your body is already hot and your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + "s harden involuntarily in response to the tiny slut's totally understandable display of desire. Your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " leaks enough girl-juice to form a puddle beneath you, doing its best to show your fellow slut how ready you are. You 'subtly' (at least in your little mind) push your chest forward, making your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + "s easy targets for the goblin. She grabs hold of your breasts");
				//--[If nipplecunts]
				if (player.torso.chest.hasFuckableNipples()) DisplayText(", fingering your wet nipplecunts and pinching the now-puffy areolas tightly, sending uncontrollable waves of pleasure from your chest to your groin.");
				//-[If not]
				...
				DisplayText("Your green slut counterpart stands up and steps over your crotch, positioning herself perpendicular to you. The warm wetness of her dripping cunt splashes your thighs when she works the free end of the double-dong into her own slick twat. Your pussy squelches against hers wetly as they meet in the middle, your " + VaginaDescriptor.describeClit(player) + " pressed on by her nether lips. The goblin twists, grinding and scissoring her thighs, the hard bud of her clit rubbing against your " + VaginaDescriptor.describeClit(player) + " again and again.\n\n");
		...
		if (player.torso.cocks.count > 0) {
			DisplayText(images.showImage("goblin-loss-male-raped"));
		...
		if (player.torso.vaginas.count > 0) {
			DisplayText(images.showImage("goblin-loss-female-raped"));
			...
			else DisplayText("It rapidly forms into a puddle as your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " does its best to show just how ready you are.  ");
		}
		DisplayText("Your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + "s poke upwards, giving the goblin easy targets.  She grabs hold of them ");
		if (player.torso.chest.hasFuckableNipples()) DisplayText("slipping her thumbs inside the tender cunts and pinching against them tightly");
		...
		if (player.torso.cocks.count > 0 && (player.torso.vaginas.count <= 0 || randInt(2) === 0)) {
			...
			if (player.torso.cocks.get(0).area > monster.vaginalCapacity()) {
				...
				else DisplayText("the base of your " + CockDescriptor.describeCock(player, player.torso.cocks.get(0)));
				DisplayText(" before she buries it deep into your " + LowerBodyDescriptor.assholeOrPussy(player) + ".  Her thighs, breasts, and body surround you with a shroud of warm green flesh that wriggles and twists around you with feverish lust.  The lips of her still-partially stretched cunt do their best to devour your member's head when the flexible little minx curves her hips back down to grind on it.\n\n");
				...
				if (player.torso.vaginas.count > 0) DisplayText("caressing your " + VaginaDescriptor.describeClit(player));
			...
			//[DICK FITS]
				...
				if (player.torso.balls.quantity > 0) DisplayText("cup your " + BallsDescriptor.describeBalls(true, true, player) + " in one hand, squeezing gently as the drug begins to affect them");
		...
		//[FEMALEZ]
			...
			player.displayStretchVagina(player.vaginalCapacity(), true);
			player.orgasm();
			cleanupAfterCombat();
		}
	}
	...
	public gobboRapeIntro(): void {
		...
		DisplayText().clear();
		//[HP Intro]
		if (monster.stats.HP < 1) DisplayText("The goblin falls down, smashing her tits flat on the ground and crying softly from the pain.  She looks up at you and sniffles.");
		//[Lust Intro]
		else {
		...
		if (player.torso.cocks.count > 0) {
			...
			if (player.cockArea(player.torso.cocks.sort(Cock.LargestCockArea)[0]) > monster.vaginalCapacity() && player.stats.cor > 80 && kGAMECLASS.monk > 2)
				corruptTooBig = rapeAGoblinCorruptTooBig;
			//Regular too big scene
			if (player.cockArea(player.torso.cocks.sort(Cock.LargestCockArea)[0]) > monster.vaginalCapacity())
			...
			//Spidercondom
			if (player.torso.tailType === TailType.SPIDER_ABDOMEN && player.cockThatFits(monster.vaginalCapacity()) >= 0)
		...
		if (player.stats.lust >= 33 && player.gender > 0 && (fitsFuck != null || cuntFuck != null || tooBig != null ||
			...
			if (feeder != null) DisplayText("your nipples ache with the desire to feed her your milk.  Do you feed her milk or leave?</b>");
			else DisplayText("your abdomen aches with the desire to impregnate her full of insect eggs.  Do you?</b>");
			MainScreen.simpleChoices(["Feed", "Lay Eggs", "", "", "Leave"], [feeder, eggs, null, null, cleanupAfterCombat]);
	...
	private giveGoblinAMilkMustache(): void {
		DisplayText().clear();
		DisplayText("You slowly walk up to the downed goblin, gently telling her that everything will be all right now. She looks at you a bit incredulously and spreads her legs, obviously hoping that you will satisfy the urges that she has. You shake your head at her and instead cup your hands under your " +  Desc.Breast.describeBreastSize(character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating)  + " and tell her that it's feeding time. The goblin looks at you annoyed and says, \"<i>I don't want your breasts! I want your naughty bits!</i>\" You laugh at her and grab her arms, pulling them behind her head.\n\n");

		DisplayText("She struggles against your grip, trying to get something, anything inside her needy pussy while yelling \"<i>Come on " +  player.mf("slut", "stud")  + ", you know you want to - mmph!</i>\"  You cut her off by shoving her mouth onto your " +  Desc.Breast.describeNipple(character, character.torso.chest.get(0))  + ". She gasps involuntarily, filling her mouth with your milk. In an instant she freezes, then slowly swallows the milk in her mouth. She relaxes in your arms a moment later, gently suckling at your nipple. Her old lust-filled self is gone, replaced with a pliant girl who now wants nothing but your milk. You slowly lower your hand and start rubbing at her still-slick pussy. In response, she puts her hand on your other " +  Desc.Breast.describeNipple(character, character.torso.chest.get(0))  + ", playing with it and teasing you.\n\n");

		DisplayText("After a while, you feel the goblin fall asleep in your arms. Even then, she still continues suckling gently on your " +  Desc.Breast.describeNipple(character, character.torso.chest.get(0))  + ". You smile, satisfied, and gently lift the goblin off your chest. You pat her shoulder softly, and she stirs awake again. She gives you a bit of a dazed look before you give her a gentle push, and she starts walking away with a vacant, drooling stare.");
	...
	private gobboButtSecks(): void {
		...
		if (x < 0) x = 0;
		DisplayText().clear();
		DisplayText(images.showImage("goblin-win-male-buttsex"));
		...
		DisplayText("With a sneer, you take the crown of your " + CockDescriptor.describeCock(player, x) + " and press it not against her drenched pussy lips, but her asshole!\n\n");
		...
		DisplayText("\"<i>This isn't what I wanted!</i>\" she cries, \"<i>This isn't what I wanted!</i>\" If your " + CockDescriptor.describeCock(player, x) + " expanding her ass wasn't enough, she can feel your ");
		if (player.torso.balls.quantity > 0) DisplayText(BallsDescriptor.describeSack(player));
		else DisplayText(LowerBodyDescriptor.describeHips(player));
	...
	private gobboGetsRapedFem(): void {
		...
		if (player.torso.hips.legs.isTaur()) {
			DisplayText(images.showImage("goblin-win-female-taur-rapedfem"));
		...
		//Goblin victory rape, female naga:
			...
			DisplayText(images.showImage("goblin-win-female-naga-rapedfem"));
			...
			DisplayText(images.showImage("goblin-win-female-rapedfem"));
	...
	private rapeAGoblinCorruptTooBig(): void {
		...
		DisplayText().clear();
		DisplaySprite(24);
		DisplayText(images.showImage("goblin-win-male-corruptedfuck"));
		DisplayText("You begin to remove your " + player.inventory.equipment.armor.displayName + ", looking down on your helpless would-be-attacker and soon-to-be victim while licking your lips hungrily. Your " + CockDescriptor.describeMultiCockShort(player));
		...
		if (player.torso.cocks.count > 1) DisplayText("one of your " + CockDescriptor.describeMultiCockShort(player) + " comes to rest on top of her and the fact that it's ");
		else DisplayText("your " + CockDescriptor.describeCock(player, x) + " comes to rest on top of her and the fact that it's ");
		...
		DisplayText("You have a seat, legs wide, on the ground and hold the little goblin whore with her relatively tiny slit resting at the tip of your " + CockDescriptor.describeCock(player, x) + ". Finally out of her stupor a look of extreme conflict crosses her face as her need for cock and cum goes to war with her survival instincts. On the brink of defeat, those instinct suddenly regroup and beat back her lust once she feels the pain of just the " + player.cockHead(x) + " of your " + CockDescriptor.describeCock(player, x) + " starting to stretch out her " + monster.VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ". She does all she can to resist, but with the way you're holding this is little more than flailing wildly and pushing against your " + CockDescriptor.describeCock(player, x) + " with her feet, practically climbing it like the tree trunk it must look like from her perspective.\n\n");
		DisplayText("Both of you dripping with sweat from your respective exertions, you slowly begin to realize the combination of her furious struggling and the tightness of her " + monster.VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " is going to keep you from the penetration you were so looking forward to. However, as you begin to consider finishing off in her throat, a darkness stirs and another idea crosses your mind.\n\n");
		DisplayText("The goblin relaxes a little when she feels you no longer pressing her down onto your " + CockDescriptor.describeCock(player, x) + ". She absolutely thrills when you bring her tiny pussy to your lips and begin to have at it. Your tongue plays around both on the inside and outside of her " + monster.VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " until you coax her little nub out from hiding. You wrap your lips around it and begin putting your corruption to task. As you drive the goblin slut closer and closer to orgasm, working over her clit with an expertise rarely found outside of the infernal ranks, bolts of corrupt energies travel from your tongue, through her clit, and deep into her core. As she cums, screaming, you pull off of her, admiring the outward signs of your demonic handiwork. What before was an average sized love button has swollen five times its size into a proud, engorged clit... and that's not the only thing you enlarged.\n\n");

		DisplayText("You reposition your fucktoy so that now she's facing away from you and again bring your " + CockDescriptor.describeCock(player, x) + " into contact with her " + monster.VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ". Again she struggles, but after her orgasm she doesn't have the strength to put up a decent fight; all her resistance does is arouse you further, now that she's fighting the inevitable. You begin to press her down onto yourself.\n\n");
		...
		DisplayText("You ram home the rest of your " + CockDescriptor.describeCock(player, x) + " left outside of your newly christened dick jockey. It's enough to orgasm the bitch, and you haven't even gotten started. You ");
		...
		DisplayText(" and it drives your lust even higher. You withdraw more cock from the hole than your cum starved slut has body, causing her eyes to roll into the back of her head as her " + monster.VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " clamps down on you and she cums again. Every time you pull out or hammer home brings her off to the point that by the time you've both gotten down on all fours, rutting like a pair of wild animals, she can only lie there, practically foaming at the wide open mouth");
		if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 1) {
			DisplayText(", her head resting between your " + BreastDescriptor.describeBreastRow(player.torso.chest.get(0)));
			if (player.torso.chest.sort(BreastRow.LactationMultipierLargest)[0].lactationMultiplier >= 1) {
				DisplayText(" as your " + BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) + " ");
		...
		if (player.torso.vaginas.count > 0) {
			DisplayText("while your " + VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " ");
			...
			if (player.torso.cocks.count > 1) DisplayText("and the rest of your " + CockDescriptor.describeMultiCockShort(player) + " drench her, ");
		}
		else if (player.torso.cocks.count > 1) {
			DisplayText("while your other " + CockDescriptor.nounCock(CockType.HUMAN));
		...
		if (player.torso.balls.quantity >= 2) DisplayText("Your " + BallsDescriptor.describeBalls(true, true, player) + " are trying but, h");
		else DisplayText("H");
		DisplayText("er spasming " + monster.VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + " is clamping down so hard on your " + CockDescriptor.describeCock(player, x) + " that it can't release.\n\n");
		...
		DisplayText("Your " + CockDescriptor.describeCock(player, x) + " is pressed so firmly against her skin that you can see the cum working its way up and out of your shaft and bloating your goblin toy with only the first shot. ");
		if (player.torso.cocks.count > 2) DisplayText("Your remaining " + CockDescriptor.describeMultiCockShort(player) + " blast geysers into the air, coating you both in your spunk. ");
		...
		DisplayText("Finally you blow your last wad into this latest piece of ass, shoving her off of your " + CockDescriptor.describeCock(player, x) + " and letting her fall to the ground. The impact sends torrents of cum sluicing out of her.\n\n");
	...
	private manRapesGoblinTooBig(): void {
		DisplayText().clear();
		DisplayText(images.showImage("goblin-win-male-corruptedbj"));
		let x: number = player.torso.cocks.sort(Cock.LargestCockArea)[0];
		DisplaySprite(24);
		DisplayText("You whip out your stiffening maleness, revealing its ");
		if (player.cockArea(x) < 80) DisplayText("nearly ");
		...
		DisplayText(" of your " + CockDescriptor.describeCock(player, x) + " visibly pulses in excitement as you brush it against her already-slick folds.  She squirms under you, clearly enjoying the feeling of you pushing against her opening.  With painful slowness, you begin pushing forward, feeling her body stretch around your ");
		if (player.torso.cocks.get(0).type === CockType.HORSE) DisplayText("flare");
		...
		if (player.torso.cocks.filter(Cock.HasKnot).length > 0) DisplayText(" hooking the heels of her feet just behind your knot");
		DisplayText(".\n\n");
	...
	private gobboGetsRapedMaleFits(): void {
		...
		DisplayText().clear();
		DisplayText(images.showImage("goblin-win-male-getridden"));
		...
		if (player.cockArea(x) > monster.vaginalCapacity() * .8) {
			...
			else if (player.torso.cocks.filter(Cock.HasKnot).length > 0) DisplayText("stretching painfully around your knot");
			else if (player.torso.cocks.get(0).type === CockType.HORSE || player.torso.cocks.get(0).type.Index > 3) DisplayText("feeling painfully tight around you");
			...
			if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 1) DisplayText(player.BreastDescriptor.describeAllBreasts(player));
			...
			DisplayText("She pants happily, her tongue rolling free from her mouth as she comments, \"<i>So full. . .</i>\"  Still wincing from the monster inside her she begins to cheer you on, \"<i>oooh go-ah-faster! I wanna bounce!</i>\"\n\n");
			DisplayText("It's all the encouragement you need, and you break into a run, feeling her lithe form bounce on your " + CockDescriptor.describeCock(player, x) + ", drawing out a cacophony of cries ranging from happy wails and moans to slight yelps of pain. Her tiny fists dig into your ");
			...
			DisplayText("The familiar tightness of a coming orgasm grows in your groin, tightening as you near release. You pick up the pace, full out sprinting, letting the girl bounce and jiggle as she clings to you, supported entirely by your " + CockDescriptor.describeCock(player, x) + ". ");
			if (player.torso.balls.quantity > 0) DisplayText("Your " + BallsDescriptor.describeBalls(true, true, player) + " tighten, releasing the seed of your orgasm.  ");
			...
			if (player.cumQ() >= 100) DisplayText("  Your enhanced body easily stuffs her full of cream, pudging her belly out slightly, your seed staying embedded in her womb with nowhere to escape, her cunt plugged tightly with your " + CockDescriptor.describeCock(player, x) + ".");
			if (player.cumQ() >= 500) DisplayText("  The orgasm is so potent that by the time you wind down, she looks to be sporting a pregnancy the size of a medicine ball.  Your cum is trapped inside her, unable to find any gap between her walls and your " + CockDescriptor.describeCock(player, x) + ".");
			else if (player.cumQ() >= 250) DisplayText("  The orgasm is so potent that by the time you wind down, she looks heavily pregnant.  Your cum is unable to find any gap between her walls and your " + CockDescriptor.describeCock(player, x) + ".");
			DisplayText("\n\n");
		...
		//(FITS – Get ridden)
			...
			else if (player.torso.cocks.filter(Cock.HasKnot).length > 0) DisplayText("stretching tightly around your knot");
			else if (player.torso.cocks.get(0).type === CockType.HORSE || player.torso.cocks.get(0).type.Index > 3) DisplayText("feeling absolutely perfect around you");
			...
			DisplayText("It's all the encouragement you need, and you break into a run, feeling her lithe form bounce on your " + CockDescriptor.describeCock(player, x) + ", drawing out a cacophony of happy wails and moans.  Her tiny fists dig into your ");
			if (player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating >= 1) DisplayText("tits");
			else DisplayText("skin");
			...
			DisplayText("The familiar tightness of a coming orgasm grows in your groin, tightening as you near release.  You pick the pace, full out sprinting, letting the girl bounce and jiggle as she clings to you, supported entirely by your " + CockDescriptor.describeCock(player, x) + ".  ");
			if (player.torso.balls.quantity > 0) DisplayText("Your " + BallsDescriptor.describeBalls(true, true, player) + " tighten, releasing the seed of your orgasm.  ");
			DisplayText("The howl of a powerful orgasm fills your ears as your cumming sets off the little green cock-sleeve.  One of her hands lets go, and starts rubbing her belly while she kisses and licks your belly-button.");
			if (player.cumQ() >= 250) {
			...
			if (player.cumQ() >= 500) DisplayText("  The orgasm is so potent that by the time you wind down, she looks heavily pregnant and your cum squirts out of any gap it can find between her walls and your " + CockDescriptor.describeCock(player, x) + ".");
			DisplayText("\n\n");
			DisplayText("You pant and stop, pulling the stuffed goblin off you and setting her on the ground, smiling in satisfaction as your cum ");
			if (player.cumQ() >= 250) DisplayText("pours out in a river");
			else DisplayText("leaks");
	...
	private goblinCondomed(): void {
		...
		let x: number = player.cockThatFits(monster.vaginalCapacity());
		DisplayText().clear();
		DisplayText(images.showImage("goblin-win-male-goblincondomed"));
		...
		DisplayText("Impatiently, she thrusts her legs out and hooks her toes around your lower body, trying to pull you closer while still keeping her needy hole accessible. Her olive feet clench around your flesh, her soles firm and muscular on your " + player.skinFurScales() + " as she slides up and down the outsides of your " + LowerBodyDescriptor.describeHips(player) + ". Dragging her heels across your thighs, the goblin pushes her feet together on either side of your " + CockDescriptor.describeCock(player, x) + ", the balls of her jade skin pressing against ");
	...
	private gatsGoblinBoners(): void {
		...
		DisplayText(images.showImage("goblin-win-male-goblinboners"));
		...
		DisplayText("Eventually you begin to feel yourself coming to a climax, your movements getting faster and faster as you build up to your release.  The goblin below you has already lost herself to the pleasure of your " + CockDescriptor.describeCock(player, x) + ", her eyes rolled upwards and her tongue drooling out of her mouth while her slutty face rubs against the ground you're currently pounding her on.  With a final thrust, your hips lurch forward as you paint her insides with your thick spunk, relishing in the feeling of your ejaculate filling her up to the brim and plugging her entrance");
		...
		DisplayText("You pick yourself back up, jerking yourself slowly as cum dribbles from your " + CockDescriptor.describeCock(player, x) + " onto the collapsed body of the goblin.  It'll be awhile before she comes back to consciousness, but you're certain she'll have a better appreciation for sex when she does.");
		player.orgasm();
		cleanupAfterCombat();
	}
	...
	private laySomeDriderEggsInGobboTwat(): void {
		DisplayText().clear();
		DisplayText(images.showImage("goblin-win-drider-egged"));
