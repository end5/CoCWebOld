...
export class GoblinAssassinScene {
	...
	public gobboAssassinBeatYaUp(): void {
		...
		//50% odds for herms
		if (player.cockThatFits(monster.vaginalCapacity()) >= 0 && player.torso.cocks.count > 0 && (player.torso.vaginas.count <= 0 || randInt(2) === 0)) {
			...
			DisplayText(player.mf("strapping young man", "sexy breeder") + " will have enough jizz to pay for " + player.mf("his", "her") + " trespass?</i>\"");
			...
			else if (player.torso.vaginas.count > 0) DisplayText(VaginaDescriptor.describeVagina(player, player.torso.vaginas.get(0)) + ", the tips giving your soaked mound only the barest ghost of a touch.");
			...
			if (player.torso.cocks.hasSheath()) DisplayText(" the sheath");
			...
			DisplayText("\n\n\"<i>");
			if (silly()) DisplayText("Stop right there, criminal scum.");
		...
		//ii. Lust Loss - Male - No Fit
		else if (player.torso.cocks.biggestCocks[0].area > monster.vaginalCapacity() && player.torso.cocks.count > 0 && (player.torso.vaginas.count <= 0 || randInt(2) === 0)) {
			...
			if (player.biggestCockLength() > player.tallness / 2) DisplayText("  Your incredibly long phallus blocks your view of the sultry goblin, and you never see the blow coming.");
			...
			if (player.biggestCockLength() < player.tallness / 2) DisplayText("sitting on your face, the " + player.cockHead(x) + " of your maleness jutting against her drooling lips.");
			else DisplayText("sitting on the ground ‘above’ you, your " + player.cockHead(x) + " reaching all the way up to spear her moist box, judging by what you’re feeling anyway.");
			...
			if (player.biggestCockLength() >= player.tallness / 2) DisplayText("pulsating, iron-hard cock");
		...
		//iii. Lust Loss - Female
			...
			if (player.torso.chest.hasFuckableNipples()) DisplayText("piston in and out of your [nipple], masturbating it just as hard as the real thing.");
