...
const gargoyleFlags: GargoyleFlags = {
    name: "",
    confidence: 0,
    ritualIntro: 0,
...
User.flags.set(CharacterType.Gargoyle, gargoyleFlags);
...
function gargoyleConfidence(arg: number = 0): number {
    if (arg !== 0) {
        gargoyleFlags.confidence += arg;
        if (gargoyleFlags.confidence > 100) gargoyleFlags.confidence = 100;
        else if (gargoyleFlags.confidence < 0) gargoyleFlags.confidence = 0;
    }
    return gargoyleFlags.confidence;
}
...
function breakZeChains(character: Character): NextScreenChoices {
    ...
    // [Display a textbox, into which the character can type in a name. This new value will be called " + data.name + ", henceforth. ] (Confidence +10)
...
function nameZeGargoyle(character: Character, textElement: InputTextElement): NextScreenChoices {
    ...
    DisplayText().clear();
    DisplayText("\"<i>" + gargoyleFlags.name + ",</i>\" she purrs, \"<i>" + gargoyleFlags.name + ", " + gargoyleFlags.name + ".  Master has interesting taste.</i>\"");
...
function firstGargoyleTalk(character: Character): NextScreenChoices {
    ...
    DisplayText("\n\nSuddenly, " + gargoyleFlags.name + " breaks the stare and looks away, as if in shame.  \"<i>The demons' doom that even now blights Mareth took its toll here, too.  The people of a town no longer here believed they were safe under Marae's roof.  Protected.  Their faith was. . . Misplaced.</i>\"");
...
function reassureTheGargoyle(character: Character): NextScreenChoices {
    ...
    DisplayText("\n\n" + gargoyleFlags.name + " looks up at you, her ruby-red eyes shining in the dim hit of light in the cathedral.  You cup her cheek and give her a smile.  \"<i>Come on, now,</i>\" you reply, offering her a hand up.  \"<i>Let's get out of here.</i>\"");
...
export function returnToCathedral(character: Character, woken: boolean = false): NextScreenChoices {
    ...
    if (Time.hour <= 9 || Time.hour >= 18 || woken) {
        if (!woken) {
            ...
            DisplayText(" light streaming in from the broken windows, you see " + gargoyleFlags.name + " poking around some of the rubble, slowly but surely making efforts to repair the damage done to the Cathedral.  Seeing you, however, she immediately drops what she was doing and ");
            if (gargoyleConfidence() < 70) DisplayText("kneels before you, head bowed low.");
            else if (gargoyleConfidence() < 99) DisplayText("bows low before you, a small smile on her stone lips.");
            ...
            DisplayText("\n\n\"<i>Master has returned,</i>\" " + gargoyleFlags.name + " says ");
            if (gargoyleConfidence() > 50) DisplayText("happily.");
    ...
    // [b]Cathedral Interior: 10:00 –> 17:00[/b]
        ...
        if (!woken) {
            DisplayText("\n\nWithin the ruins, you can see " + gargoyleFlags.name + " perched atop her pedestal in a low, predatory crouch, much as you found her.  Waving a hand in front of her eyes, you confirm she's in some sort of dormant mode.  A simple touch from her Master could revive her");
...
function gargoyleAnal(character: Character): NextScreenChoices {
    ...
    DisplayText("\n\n" + gargoyleFlags.name + " makes a sharp little gasp as you push your finger into her.  You slide into her with surprising ease, reveling in how <i>cold</i> her interior is.  She's like ice inside!  You start thrusting your finger faster, trying to get her warmed up a little as your prepare her anus.  " + gargoyleFlags.name + " squirms and moans softly as you finger-fuck her bum, slowly changing from moans of discomfort to little gasps of pleasure.");
    ...
    if (character.torso.cocks.count > 0 && character.torso.cocks.sort(Cock.LongestCocks)[0].length >= 50) {
        DisplayText("\n\nKnowing full well the living statue doesn't have much in the way of anatomy, you continue to push more and more of monstrous manhood into " + gargoyleFlags.name + "'s asshole, watching with glee as her stomach begins to distend with the massive size of your prick.  Then her chest bulges, making her gasp in pain and clutch at her breast; that doesn't stop you, though.  With one mighty, massive thrust, you slam yourself up to the hilt in " + gargoyleFlags.name + "'s ass, and watch with glee as the head bursts out of her mouth, squirting pre and drool all over the floor.  The poor gargoyle makes a muffled scream and starts flailing around helplessly, trying to do <i>something</i> about the massive member now protruding from her mouth.");

        DisplayText("\n\nWith gusto, you start hammering away at your living cock-sleeve, using her entire body as one great big ona-hole to take your gargantuan girth.  Resigned to her fate, " + gargoyleFlags.name + " slumps forward and grasps the head of your [cock biggest], giving you a handy through her mouth as her innards milk the rest of your shaft.");
...
function gargoyleCoochiiGetsPlowed(character: Character): NextScreenChoices {
    ...
    DisplayText("You gently push " + gargoyleFlags.name + " onto her back and spread her long, marble legs, giving you a clear view of her tight little pussy.  You brush your hand along her thighs, making her gasp with the sensation as your knuckles run along her smooth, cool flesh.  Your fingers come to her small clit, and you caress her tiny pleasure buzzer, eliciting a tiny moan from the gargoyle.  Grinning, you give it another flick; " + gargoyleFlags.name + " squirms, trying to either enhance or end the sensation, you can't be sure.  You start to finger her clitty harder, creating little circles around it as you slip another finger into her tight passage.");
...
function titFuckTheGargoyle(character: Character): NextScreenChoices {
    ...
    DisplayText("\n\nOnce you're satisfied at your cock's rigid hardness, you tell " + gargoyleFlags.name + " to put those big, soft tits of hers to work.  \"<i>M-Master?</i>\" she gasps, making a cute little “o” with her mouth just an inch from your shaft.  You repeat the order, and giver her a thump on the nose with your cock for emphasis.  Shyly, she cups her D-cups and leans into you, squeezing your [cock biggest] between them.");

    DisplayText("\n\nWith a little encouragement, " + gargoyleFlags.name + " starts to rise and fall on her knees, dragging your shaft through her soft, cool mounds.  With a pleasured sigh, you reach down and stroke her hair, petting the girl as she begins to slowly but steadily titfuck you.  Still mumbling encouragements, you sit back and let the pleasure run over you, reveling in the incredible softness of her sizable chest massaging your cock, making steady progress toward stroking the cum out of you.");

    DisplayText("\n\nSuddenly, you feel an overwhelming wetness on your cock.  You look down, expecting to see pre flowing freely from you, but are instead greeted with the sight of " + gargoyleFlags.name + " sliding the tip of your cock into her mouth, starting to fellate you on top of the tittyfuck.  You give her a few more words of encouragement and press her head down further onto your shaft, forcing her to take all that's left over from her breasts.  Despite her marble nature, her mouth is moist and surprisingly warm, and her long tongue is quick to wrap around your [cock biggest], wringing it in time with her chest-thrusts.");
    ...
    DisplayText("\n\nYou grab " + gargoyleFlags.name + " and push her down hard, slamming your cock down her throat as you cum.  A human would surely have choked with this maneuver – instead, " + gargoyleFlags.name + " squirms and flails, but you can see and feel her swallow your cum as it pours into her mouth, her throat massaging your flaring head, milking it harder than any tit-job ever could have.  Once her face has been ground into your pubes, you buck your hips sharply, thrusting into her throat as the last of your jizz spurts out and down her gizzard.");

    DisplayText("\n\nBy the time you're done, " + gargoyleFlags.name + "'s face is an utter mess, with white lines trickling down her cheeks.  You drag your [cock biggest] out of her with a wet POP, unleashing a dam of jizz that quickly leaks out of her agape mouth to stain her breasts and belly.  You chuckle at the sight");
    if (gargoyleConfidence() >= 70) DisplayText(", and to your surprise, so does " + gargoyleFlags.name + "");
    DisplayText(".");
...
function strapOnGargoyle(character: Character): NextScreenChoices {
    ...
    DisplayText("\n\n\"<i>Closer?</i>\" she asks, confused.  You roll your eyes and tell her to grab the holy rod sitting on the altar and put it through a leather strap.  She hesitates for a moment, but dutifully does as you ask, returning a moment later with the smooth, silver cylinder and a leather thong.  With a bit of work, you've created a simple strap-on out of the holy rod, and instruct " + gargoyleFlags.name + " to put it on.  Now she's getting it!  The gargoyle slips into the leather, giving a cute little shiver as one end of the rod slips inside her.  Before she has a chance to recompose herself, you grab her rod-cock in one hand and give her a gentle push with the other.");

    DisplayText("\n\nShe lays down with you atop her, straddling her waist as you stroke her rod, rewarded with sharp pleasure-gasps from the statue-girl.  You're hardly stimulating her pussy with these actions – she must be able to feel <i>through</i> the rod, as if it were a real cock.  Amused, you start to jerk the rod off, sliding it through your clenched fist with slow, measured strokes.  " + gargoyleFlags.name + " squirms under the pleasurable attention, and is soon groping her big breasts and bucking her hips into your fists, shoving more of her rod-cock into your hand with each of your jerks.");

    DisplayText("\n\n\"<i>Well, someone's enjoying herself,</i>\" you tease, and immediately " + gargoyleFlags.name + " freezes, her mouth agape with embarrassment.  With a little laugh, you give her a pat on the thigh and start to strip out of your [armor].  But you can't just leave " + gargoyleFlags.name + " without SOME stimulation: ");
    // (If Human Legs/Feet [ie, needs toes], as new paragraph):
    if (!character.torso.hips.legs.isTaur() && !character.torso.hips.legs.isGoo() && !character.torso.hips.legs.isDrider() && !character.torso.hips.legs.isNaga()) DisplayText("you reveal your leg first, moving slowly and sensually, giving her a tantalizing look at your nethers before hiding them again.  With a wide grin, you creep your foot up to her thighs and gently run your heel along her smooth, tender flesh.  You come to the base of her rod, standing stark straight in the air like a flag-pole.  As you strip, you grasp her cock between your big and second toe, sliding them up the smooth shaft to the tip.  " + gargoyleFlags.name + " moans, rolling her head back as you start to foot-fuck her cock, slowly putting more and more pressure on the steel member until it's pressed against her belly.  Your heel takes over, running quickly along the underside of her cock as you finally rid yourself of your [armor].");
    // Else, continued from above paragraph:
    else DisplayText("you giver her a good show, putting on a brief striptease for the gargoyle.  You strip your [armor] slowly, sensually, showing only the barest hints of your functions until the last moment, when the last piece of your clothing falls to the floor.");
...
function tailFuckGargoyleScene(character: Character): NextScreenChoices {
    ...
    DisplayText("\n\nBefore you can get used to the anal intruder, your eyes have rolled back in your head with pleasure.  Girlcum spurts from your cunt, splattering " + gargoyleFlags.name + "'s face with your juices as you cum.  Your holes squeeze down on her appendages, trapping them inside you as your ride out the orgasm, bouncing on her rigid, rapidly-moistening tail until your body settles down.");

    DisplayText("\n\nWhen you've finished, you carefully extricate yourself from the gargoyle's tail and, a bit bow-legged, gather up your [armor].  By the time you're dressed, " + gargoyleFlags.name + "'s cleaned herself off and is eagerly awaiting some token of your approval.");

    DisplayText("\n\n\"<i>Was Master satisfied?</i>\" she asks");
    if (gargoyleConfidence() > 70) DisplayText(" with just a hint of a smile");
    DisplayText(".");
...
function useGargoyleMaleHerm(character: Character): NextScreenChoices {
    ...
    DisplayText("\n\nBut you aren't quite satisfied just pounding an insensate hole.  Knowing that you have the power to \"revive\" " + gargoyleFlags.name + " through touch, you decide to put this to rather devious use.  You reach around her body and grab one of her breasts, giving it a few hard squeezes until you hear the faintest gasp from the little gargoyle.  You release it immediately.  Your hand switches down to her taut ass and gives it a loud swat.  You repeat the action twice, and finally feel her cunt contract – just a bit – around your shaft.  You switch your attentions down to her thighs, gently running your hands along both expanses of gray skin beneath your thrusting [cock biggest].  You feel her shudder, ever so slightly, but just as she's on the verge of reviving, you pull back your hands.");

    DisplayText("\n\nYou return to thrusting into her, going fast and furious as your pleasure climbs steadily towards climax.  Only when you're bursting over the threshold of orgasm do you lay your hands on " + gargoyleFlags.name + " again, grabbing her ass and giving one last massive thrust into her, shoving your [cock biggest] in to the hilt.  You cum inside her just as she awakens, moaning lustily into her ear as spunk drizzles out around your member.");

    DisplayText("\n\n\"<i>M-Master?</i>\" " + gargoyleFlags.name + " gasps, giving a full-body shudder as you slowly pull out from inside her.  Your cum drools out of her snatch and pools beneath her.  Playfully, you jam your fingers up between her legs, damming your sperm inside her.  To your glee, this simple act sends " + gargoyleFlags.name + " right over the edge: convulsing, she collapses on the floor with a heavy THUNK!");
...
function ritualGargoyle(character: Character): NextScreenChoices {
    ...
    if (gargoyleFlags.ritualIntro === 0) {
        ...
        DisplayText("You note that the sanctuary's altar has been cleared of rubble, and a number of candles have been lit upon it. Resting atop the flat stone surface is a simple iron bowl filled with what appears to be water. Curious, you ask " + gargoyleFlags.name + " what she's been up to.");
        ...
        if (gargoyleConfidence() < 50) DisplayText(" Forgive my impertinence, but");
    ...
    // [b]Ritual (Any after First)[/b]
        ...
        DisplayText("You walk to the church's alter, shortly followed by " + gargoyleFlags.name + ".  You ask her if she's still able to perform the church's Rituals, to which she replies \"<i>Of course, Master.  Do you have need of one?</i>\"");
...
function mindGargoyleRitual(character: Character): NextScreenChoices {
    ...
    DisplayText("  " + gargoyleFlags.name + " brings the invocation to a quiet close, and rests the bowl back upon the altar. \"<i>Drink of this, Master,</i>\" she says, offering you the bowl, \"<i>and be healed of mind.</i>\"");

    DisplayText("\n\nYou do so, taking a long draught of the holy water.  By the time you've set the bowl down, you feel light-headed, as if a fog has receded from your mind.  You gasp happily, able to think more clearly than you have in days.  You thank " + gargoyleFlags.name + ", give her a pat on the head, and head back to camp with a skip in your step.");
...
function banishmentGargoyleRitual(character: Character): NextScreenChoices {
    ...
    DisplayText("\n\nFrom beneath the altar, " + gargoyleFlags.name + " withdraws what looks like a perfectly cylindrical dildo, covered with dozens of dark, dimly-glowing blue runes.  Slowly, she makes her way around you and kneels behind your [butt].  \"<i>Take a deep breath, Master, and relax yourself.  It will be better this way.</i>\"");
    ...
    DisplayText("\n\n\"<i>Do not worry, Master,</i>\" " + gargoyleFlags.name + " says");
    if (gargoyleConfidence() >= 50) DisplayText(" soothingly, using her free hand to stroke your thighs");
    ...
    DisplayText(" been absorbed and purged by the rod, which " + gargoyleFlags.name + " proceeds to chuck out a window.  Hopefully she has more, in case you ever get ");
    if (character.statusAffects.has(StatusAffectType.Exgartuan)) DisplayText("possessed");
    else DisplayText("infested");
    DisplayText(" again.");

    DisplayText("\n\nStill panting and gasping from your ordeal, you notice " + gargoyleFlags.name + " looming over you, smiling.  You could be a dick for all that pain she caused you, or thank her for helping you.");
...
function gargoyleKinkyRituals(character: Character): NextScreenChoices {
    ...
    // (While Confidence 70+)
        ...
        DisplayText("You walk to the church's alter, shortly followed by " + gargoyleFlags.name + ".  You tell her you need one of her holy rituals, but that you'd prefer to have some fun as well.  Grinning, she says, \"<i>It would be my pleasure, my Master. What shall we do?</i>\"");
    }
    // (Display Options: [Body] [Mind] [Banish (only with Exgartuan or Worms)] and [Leave])
    let banish = false;
    if (character.statusAffects.has(StatusAffectType.Infested) || character.statusAffects.has(StatusAffectType.Exgartuan)) banish = true;
...
function kinkyBodyRitual(character: Character): NextScreenChoices {
    ...
    // (Confidence +2)
    // (PC decreases Libido and Sensitivity. Suffers 20% Max HP damage, to a minimum of 1 pt. remaining)
    DisplayText("You tell " + gargoyleFlags.name + " that you could stand to reverse some of the corruption that has affected your body.  With an eager nod, she strips you down, takes you to a side alcove near the altar and instructs you to spread your arms and legs.  Putting your trust in your little gargoyle, you do so, allowing her to bind you spread-eagle with long leather straps.");
    ...
    DisplayText("\n\nShe guides you over to the altar and helps you lie atop it to rest.  Once you're comfortably situated, " + gargoyleFlags.name + " slips up beside you, resting her cheek ");
    ...
    DisplayText("\n\nYou spend a few minutes cuddling with " + gargoyleFlags.name + ", but sadly, you know you can't leave your camp unattended for too long.  Giving her a pat on the head and a kiss on the brow, you gather up your clothes and head on back to camp.");
...
function banishPervRitual(character: Character): NextScreenChoices {
    ...
    DisplayText("\n\nWith her rod firmly inside you, " + gargoyleFlags.name + " puts her hands on your [hips] for leverage and begins to pull out again, leaving a hot, intense tingling sensation in your rectum.  She thrusts in, this time much more quickly than you were expecting, making you gasp and moan as her hips slam into yours.");
    // If PC has Worms:
    ...
    DisplayText("  " + gargoyleFlags.name + " begins to fuck you in earnest, hammering your [asshole] with the strap-on until you're both panting and gasping, nearly overwhelmed by sensation.");
    // (If PC has Exgartuan:
    ...
    DisplayText("\n\nSuddenly, the holy rod jammed up your [asshole] makes a loud hissing sound, and withdraws. Panting, you collapse to the ground, gripping your gut.  It's over, and though a part of you feels suddenly very empty, it's soon forgotten as " + gargoyleFlags.name + " slips down beside you, having already tossed the holy rod out the window.  The creature");
    ...
    DisplayText("\n\nStill panting and gasping from your ordeal, you slip an arm around " + gargoyleFlags.name + "'s shoulders, bringing her into a tight embrace.  Smiling cutely, she nuzzles her cheek against your chest, and winds her large bat-wings around the two of you, wrapping you together.  You give her a long, tender kiss on the brow, and let her remain like that for a good long while, basking in each others' presence.");

    DisplayText("\n\nEventually, though, you know you need to get back to camp.  You try to tell " + gargoyleFlags.name + ", but to your surprise, she's fast asleep, snoring softly.  With a grin, you untangle yourself from her embrace and find something to cover her with before giving her another kiss and making your way home.");
    // (Return PC to camp, advance time 1 hour)
    if (character.statusAffects.has(StatusAffectType.Exgartuan)) character.statusAffects.remove(StatusAffectType.Exgartuan);
    else character.statusAffects.remove(StatusAffectType.Infested);
    character.stats.HP -= Math.round(character.stats.maxHP() / 3);
...
function talkToGargoyleOutput(): NextScreenChoices {
    ...
    // If Confidence <50:
    if (gargoyleConfidence() < 50) DisplayText("  She meekly approaches her Master and kneels before you, head bowed submissively.");
    // if Confidence = 51-69:
...
function comfortGargoyleDumbness(): NextScreenChoices {
    ...
    // (Confidence +3)
    gargoyleConfidence(10);
    DisplayText("You sigh and reach over to " + gargoyleFlags.name + "; ");
    // if Confidence <50:
...
function historyOfGargoylesB(): NextScreenChoices {
    ...
    // if Confidence >70:
    if (gargoyleConfidence() > 70) DisplayText(", and I despise them with every fiber of my being");
...
function gargoyleHistoryC(character: Character): NextScreenChoices {
    ...
    //     DisplayText("You ask " + data.name + " to tell you a little bit about the world of Mareth.  Giving it a few moments of quiet thought, she answers: \"<i>I do not have any experience outside of the church grounds, Master, but I remember the words of the last Master, and the parishioners before her.  There are... the world has a vast expanse of plains and woodlands, broken up by a mountain range and lake.  Those last two are populated by savage beasts, minotaurs, cow-girls, anemone, and shark-people, both of whom will rend your flesh or rape you half to death.</i>\"");
    //     DisplayText("\n\nWhoa, there. You stop " + data.name + " and explain to her that some of your lovers – mates, even – are just such beasts.  You tell her a little bit about Izma and Isabella, enforcing the fact that they're both not only quite friendly, but sweet as well.  They're just regular people");
    //     if (gargoyleConfidence() > 50) DisplayText(" just like her");
    //     DisplayText(".  If she could, you're sure " + data.name + " would be blushing bright crimson as you educate her on a little inter-special tolerance and understanding.  Chastened, she nods her comprehension when you're finished and, after a long pause, asks, \"<i>I-If Master really has many... girlfriends... does... no, it's silly, of course not...</i>\"");
    ...
    //     MainScreen.getBottomButton(0).modify("Dodge", izmaAndIsabellaDodge);
    //     MainScreen.getBottomButton(1).modify("Slave", gargoylesAreSlavesYo);
    //     MainScreen.getBottomButton(2).modify("Girlfriend", gargoylesAreGirlfriends);
    // }
    // // (If PC has Marble as a follower)
    // else if (character.statusAffects.has(StatusAffectType.CampMarble)) {
    //     DisplayText("You ask " + data.name + " to tell you a little bit about the world of Mareth.  Giving it a few moments of quiet thought, she answers: \"<i>I do not have any experience outside of the church grounds, Master, but I remember the words of the last Master, and the parishioners before her.  I believe there is a lake not far from here.  Beyond that, the world has a vast expanse of plains and woodlands, broken up by a great mountain range – strange monsters live there, but worst of all are savage bull-like beasts called Minotaurs.  And the cow-girls... Lacta-Bovines, I think they're called, who're just as nasty.</i>\"");
    //     DisplayText("\n\nYou cut " + data.name + " off right there, and explain that one of those 'savage beasts' is your lover, Marble, who's really quite sweet.  The gargoyle makes a horrified gasp and covers her mouth.  \"<i>I'm so sorry, Master! I didn't know...  All I'd heard were stories, years ago.  I didn't mean... but...  I-I didn't know Master had a girlfriend,</i>\" she whines pitifully.  Obviously, knowing that she's not your only lady-friend has upset the poor thing.");
    //     DisplayText("\n\nWell.  What do you say to that: that you're not exclusive with Marble, or that " + data.name + " doesn't count, being a statue and all?");
    //     // (Display Options: [Not Exclusive] [Doesn't Count])
    //     MainScreen.hideBottomButtons();
    //     MainScreen.getBottomButton(0).modify("Not Exclusive", marbleAintExclusiveBiatch);
    //     MainScreen.getBottomButton(1).modify("Doesn'tCount", gargoyleDoesntCountAsAWaifu);
    // }
    // // (If PC has Izma as a follower)
    // else if (kGAMECLASS.izmaScene.izmaFollower()) {
    //     DisplayText("You ask " + data.name + " to tell you a little bit about the world of Mareth.  Giving it a few moments of quiet thought, she answers: \"<i>I do not have any experience outside of the church grounds, Master, but I remember the words of the last Master, and the parishioners before her.  There are... the world has a vast expanse of plains and woodlands, broken up by a great mountain range.  There is a lake not far from here, if I recall; a race of very dangerous shark-folk live there.</i>\"  With a chuckle, you tell " + data.name + " that you're more than well aware of that; indeed, you consider one such creature to be your mate, your beta.  She makes a slight gasp, and gushes out an apology.  \"<i>I'm so sorry, Master!  I hadn't realized... I thought...</i>\" she falls silent for a moment, then says shyly, \"<i>I wasn't aware Master had a girlfriend.</i>\"  Obviously, knowing that she's not your only lady-friend has upset the poor thing.");

    //     DisplayText("\n\nWell. What do you say to that: that you're not exclusive with Izma, or that she doesn't count, being a statue and all?");
    //     // (Display Options: [Not Exclusive] [Doesn't Count])
    //     MainScreen.hideBottomButtons();
    //     MainScreen.getBottomButton(0).modify("Not Exclusive", sharkgirlsArentExclusiveBiatch);
    //     MainScreen.getBottomButton(1).modify("Doesn'tCount", gargoylesDontCountAsSharkWaifus);
    // }
    // else
    {
...
function gargoylesAreGirlfriends(character: Character): NextScreenChoices {
    ...
    gargoyleConfidence(15);
    DisplayText("You smile at " + gargoyleFlags.name + " and tussle her hair.  Warmly, you tell her that, if she wants, she can be your girlfriend too.  Polyamory is just your thing, you suppose.  With a happy gasp, the little gargoyle leaps into your lap and wraps her arms around you, hugging you so tight you almost can't breathe.  She gives you a flurry of little kisses all along your neck and cheek before planting a long, deep one on your lips.  You kiss her back, soon laughing amicably as you return her embrace.  Once she's calmed down, " + gargoyleFlags.name + " settles into your lap, resting her cheek against your chest and wrapping her tail and wings lovingly around you.  You give her a little peck on the forehead and nuzzle your chin into her hair, content to stay like this forever.");

    DisplayText("\n\nYet, you're soon surprised when " + gargoyleFlags.name + " withdraws slightly from you, though her arms are still hooked about your neck.  \"<i>Master... if you want me to be your girlfriend, then... we should – if you want, Master, of course – maybe...  Master, will you make love to me?</i>\" she blurts, suddenly taken aback by her own forwardness.");
...
function cathedralC(): NextScreenChoices {
    ...
    if (gargoyleConfidence() > 69) DisplayText(" unless it were to be with you, that is");
...
function comfortGargoyle(): NextScreenChoices {
    ...
    DisplayText("\n\nTo your surprise, " + gargoyleFlags.name + " follows you, running to catch up with you.  She takes your hand in hers and wraps one of her wings around you as you walk, holding you close as you make your way out the doors and onto the grounds.  Soon, though, you see " + gargoyleFlags.name + " slowing and grimacing – approaching the edge of her boundary.  The two of you stop at the wrought-iron fence surrounding the Cathedral, and face each other.");
...
function garNameA(character: Character): NextScreenChoices {
    ...
    if (gargoyleConfidence() > 69) DisplayText("  She continues, \"<i>though even were I not compelled to, I would still serve you,</i>\" she says with a playful wink.");
    ...
    // If Confidence =<69:
    if (gargoyleConfidence() <= 69) DisplayText("\n\nAfter what she did to me, I cannot enjoy sex; just the thought of it makes me shudder.</i>\"");
    else DisplayText("After what she did to me, I never thought I'd enjoy sex again.</i>\"  With a loving smile, " + gargoyleFlags.name + " takes your hand in hers and brushes it against her cheek.  \"<i>But for you, Master, I think I could make an exception.</i>\"");
    ...
    // If Confidence =<69:
    if (gargoyleConfidence() <= 69) DisplayText("  \"<i>I'm sorry, Master, I can't... it hurts too much to think about it.</i>\"");
...
function garNameB(): NextScreenChoices {
    ...
    DisplayText("\n\nYou don't need to prompt " + gargoyleFlags.name + " this time.  \"<i>She left, perhaps ten years ago.  One day I was following orders to Master's pleasure, and the next I was bound to my pedestal, wearing the chains that Master broke to set me free.</i>\"");
...
function comfortGarNameC(character: Character): NextScreenChoices {
    ...
    DisplayText("\n\nShe smiles weakly, and rests her head on your chest.  \"<i>As you command, Master,</i>\" she says, and falls silent.  You sit there for a long while, stroking the gargoyle's soft hair and hoping you have made an impression on her.  But, eventually, you know you have to part.  You give " + gargoyleFlags.name + " a little kiss on the forehead and tell her to do whatever she wants while you're gone.");
