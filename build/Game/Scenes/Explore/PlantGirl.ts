...
interface PlantGirlData {
    timesMet: number;
}

export class PlantGirl {
    private data: PlantGirlData;
    public constructor() {
        this.data = {
            timesMet: 0,
        };
    ...
    public encounterPlantWoman() {
        if (randInt(2) === 0) {
            monster.statusAffects.set(new StatusAffect("happy", 0, 0, 0, 0));
        }
        else if (randInt(2) === 0) {
            monster.statusAffects.set(new StatusAffect("horny", 0, 0, 0, 0));
        }
        else {
            monster.statusAffects.set(new StatusAffect("grouchy", 0, 0, 0, 0));
        }
        DisplayText().clear();
        if (this.data.timesMet === 0) {
            DisplayText("While out exploring the deep woods you come across a thick bush full of vines and thorns that is obstructing your path.  Strange...that wasn't there a few days ago.  You remember the perverted, sexual vegetation, but not this.  You scan the brush for a way to get through to no avail.  As you begin to turn, you hear an eerie, ominous coo which causes you to freeze, listening for further movement or noise.  After a few moments, you sigh and begin to head back the way you came... only to hit another patch of dense foliage, blocking your advance once again.");
    ...
    public plantyLadySeeWhatHappens(character: Character) {
        DisplayText().clear();
        DisplayText("You decide to not offer any resistance as the vines begin to tighten around your body.  When they have you nearly constricted, they stop suddenly.  Looking downward confirms the vines relaxing and slowly uncoiling from around your legs." + (character.stats.cor >= 66 ? "  A sense of disappointment comes to you at the sight of the retreating vegetation, robbing you of potential fun." : "") + "  Standing there in utter confusion, you scratch the back of your head and survey the thick bush that now surrounds you, when a creature appears with a soft rustle to the left!");
        ...
        MainScreen.getBottomButton(0).modify("Let's Fuck!", () => { this.plantLadyLetsFuck(1.1); });
        MainScreen.getBottomButton(1).modify("Decline", this.declinePlantSmex);
        MainScreen.getBottomButton(2).modify("Escape", this.plantWomanStruggle);
    }

    // [Let's fuck!]
    public plantLadyLetsFuck(from: number = 1.1) {
        DisplayText().clear();
        DisplayText("Throwing caution to the wind, you decide to engage in a little 'carnal' pleasure with this creature. How to do you want to sex the plant woman?");
    ...
    public plantWomanStruggle(character: Character) {
        ...
        if (character.stats.str >= 40) {
            ...
            MainScreen.hideBottomButtons();
            MainScreen.getBottomButton(0).modify("Sex", undefined);
            MainScreen.getBottomButton(1).modify("Decline", this.declinePlantSmex);
            MainScreen.getBottomButton(4).modify("Try to Run", undefined);
        }
        // (struggle: failure, or Intro 1.2.3) (< 40 STR)
        else {
            DisplayText("Despite your best efforts, you can't seem to break free from the vines as they tense around your limbs and then your waist.  You bite your lower lip and brace for whatever is coming to \"greet\" you.");
            // (leads to 1.2.3m)
    ...
    public sexFailureIntro() {
        ...
        MainScreen.hideBottomButtons();
        MainScreen.getBottomButton(0).modify("Do It!", this.struggleFailDoItPlantSmex);
        MainScreen.getBottomButton(1).modify("Decline", this.declinePlantSmex);
        MainScreen.getBottomButton(2).modify("Get Lost", this.seriouslyGetLostPlantLady);
    }

    // [Decline, or Decline 1.0]
    public declinePlantSmex() {
        DisplayText().clear();
        DisplayText("As she returns to teasing you again with a pseudo-kiss, you pull back and shake your head.  The rejection apparent in your facial features, the plant woman stops and stares at you with glassy eyes.  You signal (as best as you can while constricted) that you aren't into this and that you need to leave. Her eyes become watery as she turns away, quietly sobbing as her vines loosen up a bit.");
        ...
        if (monster.hasStatusAffect(StatusAffects.happy) >= 0) {
            ...
            monster.statusAffects.remove("happy");
    ...
    public seriouslyGetLostPlantLady(character: Character) {
        ...
        startCombat(monster); // TODO create PlantGirl
        monster.stats.HP -= 20;
        if (character.torso.neck.head.horns.amount > 0 && character.torso.neck.head.horns.type > HornType.NONE) monster.stats.HP -= 15;
        return { next: 1 };
    }

    // [Sex failure: Do it! (Trapped sex Intro (TsI))]
    public struggleFailDoItPlantSmex(character: Character) {
        DisplayText().clear();
        DisplayText("As she goes in for another pseudo kiss you lurch forward and connect with your lips, pressing them hard against her and fiercely returning her intimate gesture. She responds by snaking her tongue in your mouth, slapping hers against yours and twisting them around like a frustrated child with a knotted shoe lace.");
        ...
        const choices = [];
        // IN ZE PC BUTT
        choices.push(this.plantRapingAssTrappedSexIntro);
        // GIVE HER A BJ
        choices.push(this.trappedFlowerSexBlowjob);
        // SHE RIDES UR DICK
        if (character.torso.cocks.count > 0) choices.push(this.trappedPlantSexPussy);
        // SHE BLOWS YA
        if (character.torso.cocks.count > 0) choices.push(this.trappedPlantSexWantsToBlowYa);
        // SHE WANTS UR DICK IN HER ASS
        if (character.torso.cocks.count > 0) choices.push(this.strugglePlantLadyFailAnal);
        // SHE WANTS 69 - ANY GENDER
        if (character.gender > 0) choices.push(this.failStruggle69);
        // SHE FUCKS YER CUNT, YA GIT
        if (character.torso.vaginas.count > 0) choices.push(this.struggleFailPlantFucksPussy);
    ...
    public escapeFromPlantWomanAndDeclineSex(character: Character) {
        DisplayText().clear();
        ...
        DisplayText("You stare at the plant woman and shake your head.  Sorry, you're not interested.  The plant woman continues to stare at you, uncertain of what you are saying.  You gesture to the monster that ");
        if (character.stats.cor < 50) DisplayText("you are taking your leave and don't wish for any trouble.");
        else DisplayText("following or attacking would be a bad idea.");
        // (Connects to failure to persuade)
        // (Success, SD 1.1s)
        if (9999 === 9999) {
            DisplayText("\n\nThe monster, from what you can tell, deliberates your gesture of ");
            if (character.stats.cor < 50) DisplayText("walking away");
            else DisplayText("intimidation");
            DisplayText(" and shrugs.  She promptly flees back into the woods.  You are left wondering just how messed up the local fauna and wildlife in Mareth is before promptly returning to camp.");
            return { next: Scenes.camp.returnToCampUseOneHour };
        }
        // Failure to persuade/intimidate (SD 1.1f)
        else {
            DisplayText("\n\nThe plant monster scowls at you when you attempt to leave.  You try to ");
            if (character.stats.cor < 50) DisplayText("calm her down");
            else DisplayText("remind her that attacking you will earn her a world of pain");
            DisplayText(", but she isn't buying any of it.  You ");
            if (character.stats.cor < 50) DisplayText("shrug and turn you back, beginning the long trek back towards camp");
            else DisplayText("take a step forward and ready your weapon");
            DisplayText("; she responds by attempting to whip you with one of her vine appendages, causing you to ");
            if (!character.torso.hips.legs.isTaur()) DisplayText("leap");
            else DisplayText("gallop");
            DisplayText(" out of the way.  She's misses, but it's obvious she will not take no for an answer. Looks like you have a fight on your hands...");
            // (leads to Fight 1.0]
            startCombat(monster); // TODO create PlantGirl
            return { next: 1 };
        }
    ...
    public fuckMyAssPlantLadyWhileImTrapped(character: Character) {
        ...
        if (character.analCapacity() < 25) DisplayText(", but the bulbous member sliding inside of you hurts nevertheless");
        DisplayText(".");
        ...
        if (character.torso.cocks.count > 0) {
            const shortestCock = character.torso.cocks.sort(Cock.ShortestCocks)[0];
            if (shortestCock.length < 48) {
                DisplayText("\n\nYour ");
                if (character.torso.balls.quantity > 0) DisplayText("[balls] and ");
                DisplayText(CockDescriptor.describeCock(character, shortestCock) + " flap");
                if (character.torso.balls.quantity > 0) DisplayText("s");
                DisplayText(" against your thighs as she fucks you harder and harder as her hand finds its way down to your prick");
                if (character.torso.cocks.count > 1) DisplayText("s");
                DisplayText(".  The creature twists and jerks at the head of your cock and works diligently to stimulate your rock hard " + CockDescriptor.describeMultiCockShort(character) + ", intent on making you moan out for her.");
            }
        }
        // PC has a vagina:
        else if (character.torso.vaginas.count > 0) DisplayText("\n\nYour [vagina] leaks girl cum profusely as your innards are brutally assaulted by the creature, aching for the sensual touch of you or your lover as the plant woman mashes against you.");
        ...
        // (if long tongued:
        if (character.torso.neck.head.face.tongue.type > TongueType.HUMAN) DisplayText("  In a cute but spiteful way, your tongue is pulled forcefully out of your mouth and into her lips as she pulls her head back; the plant woman doesn't get too far from your face before you run out of tongue to give.  This suits her just fine as she takes to sucking on your incredibly long tongue in a manner similar to fellatio.  Pursed on the bottom of yours, she darts her head back and forth along the length of it, edging further into her mouth as she nears closer and closer to your own.  Her dominance without question, she reaches your lips and slips her tongue in, sealing it in with a firm kiss.");
        // Not long tongued:
        else DisplayText("  Your tongue is viciously attacked by the sudden intrusion of her own.  However, you are more than happy to return the favor as you counter attack, swapping each other's saliva in the process.");

        DisplayText("The feminine creature's hand then finds its way ");
        if (character.torso.balls.quantity === 0) DisplayText("up");
        else DisplayText("down");
        DisplayText(" to your ");
        if (character.torso.balls.quantity === 0) DisplayText(BreastDescriptor.describeNipple(character, character.torso.chest.get(0)));
        else DisplayText("[balls]");
        DisplayText(", massaging and caressing them gently as she moves to kissing at your neck.  Her thrusts become shorter in length as her breathing becomes frantic, signaling that her orgasmic peak is near. The plant woman ");
        // PC has hair: (grabs you by the hair)
        if (character.torso.neck.head.hair.length > 0) DisplayText("grabs you by the hair");
        // PC doesn't have hair: embraces you)
        else DisplayText("embraces you");
        DisplayText(" as she cries out in a fierce moan, her cock convulsing as she thrusts wildly against you and planting her hot seed deep into you.  The sensual act finally over, your head droops down as you pant heavily; the plant girl slumps her head down onto your shoulder and uses you to hold herself up.  She's breathing heavily in your ear while regaining some of her energy and composure.");
    ...
    public trappedFlowerSexBlowjobAhoy(character: Character) {
        DisplayText().clear();
        DisplayText("\"<i>MHHMMpffffrmPH!</i>\ you mumble as her cock hits the back of your mouth, the plant creature clearly intent on jumping into the rougher sex sooner rather than later. The plant girl giggles as you struggle to breathe and not vomit from the violent intrusion. Placing both of her hands on the back of your head she begins to pump her hips, gliding her cock along your (tongue descript). You gasp for air as you try to push yourself off of the girl and get some room to breathe, but she has a pretty firm grip. She continues thrusting, but is increasingly annoyed that you aren't putting much effort into it right now.  A pair a vines ");
        // (If PC has balls:
        if (character.torso.balls.quantity > 0) DisplayText("wrap themselves around your ([balls] and begin to squeeze");
        // (If PC has a cock(s):
        else if (character.torso.cocks.count > 0) DisplayText("wrap themselves around your " + CockDescriptor.describeMultiCockShort(character) + " and begin to squeeze");
        // (If PC has a pussy:
        else if (character.torso.vaginas.count > 0) DisplayText("take hold of you VaginaDescriptor.describeClit(character) and tug hard");
        // (If PC is genderless:
        else DisplayText("take hold of your [nipples], fiercely pulling on them to try and convince you to put some work into it");
        DisplayText(".");
    ...
    public trappedPlantSexPussyGOOO(character: Character) {
        DisplayText().clear();
        DisplayText("The vines around you begin to shift, and you fear that she might have misinterpreted your preferences for sex, gritting your teeth as you await the inevitable violation of your body.  The continuing sound of cracking vines and growth intrigues you enough to open your eyes, witnessing both yourself and the plant woman being lifted off of the ground by her cabal of creepers.  Lifting you further and further into the air, the vegetation draw her close to your restrained, naked body.  The floral beauty's petal like vagina is soaked in anticipation and glistening in the warm glow of the sun.");
        let x: number = character.cockThatFits(9999); // Undefined capacity for now.
        if (x < 0) x = character.torso.cocks.sort(Cock.SmallestCockArea)[0];
        // (If  cock(s) are 4 inches or less)
        if (character.torso.cocks.get(x).length < 4) {
            DisplayText("\n\nThe plant woman goes to massage your " + CockDescriptor.describeCock(character, x) + ", but stops when she sees how small you are.  Rather than dumping you on your ass, one of the vines that holds you wraps itself around [eachCock] and then proceeds to pierce the sensitive flesh of your glans with what you assume are barbs.  Unconsciously, you scream out in pain; it isn't the worst agony you have ever felt, but the sharp stinging that floods your sensitive member is enough to be a mood killer. In increasing agony, you gaze down at your " + CockDescriptor.describeMultiCockShort(character) + " to witness your member");
            if (character.torso.cocks.count > 1) DisplayText("s");
            DisplayText(" swelling up. The head");
            if (character.torso.cocks.count > 1) DisplayText("s");
            DisplayText(" of your cock");
            if (character.torso.cocks.count > 1) DisplayText("s");
            DisplayText(" swell");
            if (character.torso.cocks.count === 1) DisplayText("s");
            DisplayText(" to what seems to be double ");
            if (character.torso.cocks.count > 1) DisplayText("their ");
            else DisplayText("its ");
            DisplayText("original size, and your unsubstantial shaft");
            if (character.torso.cocks.count > 1) DisplayText("s gain");
            else DisplayText(" gain");
            DisplayText(" some length in the process.  Is this some kind of allergic reaction?  A chemical?  Some kind of hormone? Regardless of how this is done, your once insignificant penis");
            if (character.torso.cocks.count > 1) DisplayText("es are now ");
            else DisplayText(" is now ");
            DisplayText("of sufficient length to penetrate this creature; though, you suspect the effects are temporary.");
            // (cock size temporarily increased to 7 inches)
            character.torso.cocks.get(x).length += 7;
        }
        // (if > 4 inches)
        else DisplayText("\n\nShe presses her body against yours, causing your rock hard CockDescriptor.describeMultiCockShort(character) to jut upward against her leafy stomach.");
        DisplayText("\n\nRaising herself ever so slightly, the plant woman wraps her legs around and straddles you as she lowers her pussy onto the tip of " + CockDescriptor.describeCock(character, x) + ".  The female perversion of nature slowly takes the entirety of your cock in her, causing your prick to sensually ease through her tight cunt like a mole crawling through the fertile earth.  The sensation is too much, causing you to shudder and twitch uncontrollably.  Your dominant partner rests her ");
// (If height is 36 inches to 72 inches:
else if (character.tallness < 72) DisplayText("raises her face to yours ");
        // If height is greater than 72 inches, but less than 96 inches:
        else if (character.tallness < 84) DisplayText("head against your [chest] ");
        // (If Height is less than 96 inches:
        else if (character.tallness < 96) DisplayText("forehead against your chest, sniffing you intensely as she takes in your musk.  The creature of flora and warped flesh moans and shivers as she acquires your " + character.mf("manly", "feminine") + " scent, and ");
        // (If height is greater than 96 inches:
        else DisplayText("places her head against your stomach, eyes closing as the creature takes in the rising and descent of your stomach ");
        DisplayText(" as she slowly begins to work her hips, grinding the innards of her hot and moist sex against your " + CockDescriptor.describeCock(character, x) + ".");
        // (if one cock:
        if (character.torso.cocks.count === 1) DisplayText("  The plant creature begins to speed up her thrusting against your cock, eager to drive you over the edge with the tightness of her hole.");
        else {
            // (If multi-cocked:
        ...
        // (Multi-cocked:
        if (character.torso.cocks.count > 1) DisplayText("  Buried deep in the makeshift vaginas, [eachCock] thrusts forward and eagerly deposits the nourishment the creature craves.");
        // Vagina:
        if (character.torso.vaginas.count > 0) DisplayText("  Cumming hard, your " + VaginaDescriptor.describeClit(character) + " releases a liberal stream of juices down the vine that fucks you, dissolving into the appendage as you moan in furious release.");
    ...
    public trappedPlantSexWantsToBlowYa() {
        DisplayText().clear();
        DisplayText("The plant woman looks your body up and down, seemingly torn between what tickles her fancy and how she can take your body.  Her almost childlike darting of her eyes stops when she lays her sight on your CockDescriptor.describeMultiCockShort(character), sending an aroused shiver through her body as she appears to finalize her decision.  Like a child walking to the local stream for a swim, she proceeds towards you in an almost skipping like fashion.  Reaching your naked body, she promptly sinks to her knees and takes your " + CockDescriptor.describeMultiCockShort(character) + " into her hand");
        // (if more than 4 cocks or PC cocks are more than 10 inches in diameter combined:
        if (character.torso.cocks.count >= 4 || character.totalCockThickness() >= 10) DisplayText("... or she tries to, as she fumbles with your freakish bundle of dicks.  Unable to fit them all into her hand she shrugs");
        DisplayText(", staring up at you with her leafy hair semi-obstructing her flirtatious and suggestive eyes.");
    ...
    // [Nope, TsI: 1.4r]
    public trappedPlantSexDontBlowMe() {
        DisplayText().clear();
        DisplayText("It may seem she wants to take your [oneCock] and suck on it like a sugar cane, but you aren't keen on her choice of how to pleasure you.  Remaining flaccid, you stare down at the plant woman and tell her you aren't interest in a little oral love for your " + CockDescriptor.describeMultiCockShort(character) + ", aware of the fact that she can't understand you.  Despite your words likely sounding like jibberish, the plant woman understands your intent and motions for you to make up your mind; given the slightly annoyed expression on her face, you had better make it quick.");
        // (taken back to sex menu)
        return { next: 9999 };
    }

    // [Blow me!, TsI: 1.4a]
    public trappedPlantSexBlowMe() {
        DisplayText().clear();
        ...
        if (character.longestCockLength() < 24) {
            ...
            DisplayText(CockDescriptor.describeMultiCockShort(character) + " stare");
            if (character.torso.cocks.count === 1) DisplayText("s");
            DisplayText(" defiantly at the plant woman; erect in a such a fashion that suggests insult at the attractive creature before ");
            if (character.torso.cocks.count === 1) DisplayText("it");
            else DisplayText("them");
            DisplayText(" that will be tending to them today.  Eager to rise to said challenge, the plant woman attempts to take your " + CockDescriptor.describeMultiCockShort(character) + " into her mouth and ");

            // (if cock(s) do not exceed 6 inches in total diameter:)
            if (character.totalCockThickness() < 6) {
                DisplayText(" manages to fit ");
                if (character.torso.cocks.count === 1) DisplayText("it");
                else DisplayText("them");
                DisplayText(" into her mouth with little difficulty");
                if (character.torso.cocks.count > 1) DisplayText(".  What a champ");
            }
            // if total cocks exceed 6 inches total diameter, but up to two cocks fall under 6 inches:
            else if (character.twoDickRadarSpecial(6)) {
                DisplayText(" manages to take two of your pricks into her mouth without dislocating a jaw or showing any obvious of discomfort");
            }
            else DisplayText(" manages to swallow your dick into her mouth");
            DisplayText(".");
            // (If cock count is more than two:
            if (character.torso.cocks.count > 2) {
                DisplayText("  The rest of your left out cocks ache for her touch; as she takes the two cocks she has further into her mouth, she raises her arms and attempts to take hold of your members, stroking them as best as she can as she works you with her mouth");
                // (cock(s) are 3-12 inches:
                if (character.torso.cocks.list[character.thinnestCockIndex()].cockLength < 12) DisplayText(" against her face, while planting her hands against your thighs");
                // Cocks are 13-24 inches:
                else DisplayText(" in-between her shoulder and neck, using them both to form a tight hole for your other cocks to fuck");
                DisplayText(".");
            }
            // 		(jump to Continued... (“normal” dick sized oral) )
            // Continued... (“normal” dick sized sexing)
            DisplayText("With your member");
            if (character.torso.cocks.count > 1) DisplayText("s");
            DisplayText(" inside of her mouth, she begins the long awaited sucking and stimulation; starting out slowly at first, she slides the top of her tongue along the bottom of your prick");
            if (character.torso.cocks.count > 1) DisplayText("s");
            DisplayText(", going so gradual that you believe she is savoring the taste of you in her mouth.  To your surprise, she pulls back and lets your cock");
            if (character.torso.cocks.count > 1) DisplayText("s");
            DisplayText(" pop out of her maw, smacking and licking her lips as she grunts her approval like someone who has just enjoyed a fine meal.  Shaking her now intrusive hair out of the way of her eyes, she darts her head forward and takes your ");
            if (character.torso.cocks.count >= 2) DisplayText("two ");
            DisplayText("saliva covered member");
            if (character.torso.cocks.count > 1) DisplayText("s");
            DisplayText(" into her mouth; with little regard for her gag reflex, she raises her hands on your ");
            if (character.torso.cocks.count >= 4) DisplayText("upper shafts");
            else if (character.torso.cocks.count >= 3) DisplayText("upper shaft");
            else DisplayText("[hips]");
            DisplayText(" as she better positions herself for the entirety of your ");
            DisplayText("cock");
            if (character.torso.cocks.count > 1) DisplayText("s as they make");
            else DisplayText(" as it makes");
            DisplayText(" contact with the back of her throat.  You feel a bit of pre-cum leak out from the tip");
            if (character.torso.cocks.count > 1) DisplayText("s");
            DisplayText(" of your ");
            if (character.torso.cocks.count > 1) DisplayText("two ");
            DisplayText("dick");
            if (character.torso.cocks.count > 1) DisplayText("s");
            DisplayText(" and ooze against the back of her throat, causing her to moan in delight as your sticky fluids coat her throat.");

            // (If Naga:
            if (character.torso.hips.legs.isNaga()) DisplayText("\n\nYour snake tail finds its way to her body and begins to wrap itself around her torso, cusping her breasts tightly as you work your way up her body.  With her firmly in your grip, you edge the tip of your tail against the back of her head and 'encourage' her to mimic your motions, helping her time her movements with your pleasure.");
            // If Goo legs:
                ...
                if (character.torso.cocks.count > 1) DisplayText("s");
                DisplayText(" as you assert a degree of control and influence on your pleasuring.  The sticky mess that is your body solicits a series of lustfully moans from the woman who tends to your loins with her mouth.");
            }
            DisplayText("\n\nBobbing her head back and forth against you, she sucks and roughs up your cock");
            if (character.torso.cocks.count > 1) DisplayText("s");
            DisplayText(" with her energetic efforts");
            if (character.torso.cocks.count === 1) DisplayText(".");
            else {
                DisplayText(", while she cheek fucks the remainder of your " + CockDescriptor.describeMultiCockShort(character) + ".");
            }
            ...
            if (character.torso.balls.quantity > 0) {
                DisplayText(" and the base of your shaft");
                if (character.torso.cocks.count > 1) DisplayText("s");
            }
            DisplayText(", she squeezes with considerable force, almost as if she is attempting to bottle up pressure in your ");
            if (character.torso.balls.quantity > 0) DisplayText("[sack]");
            else DisplayText("loins");
            DisplayText(" so that your creamy contents will explode forth into her craw.  Like a knotted up hose line, your cock");
            if (character.torso.cocks.count > 1) DisplayText("s");
            DisplayText(" begin");
            if (character.torso.cocks.count === 1) DisplayText("s");
            DisplayText(" to ache due to the irritating discomfort in your shaft");
            if (character.torso.cocks.count > 1) DisplayText("s");
            DisplayText("; you're unsure if you will be able to achieve orgasm with such a tense grip on your spear");
            if (character.torso.cocks.count > 1) DisplayText("s");
            DisplayText("!  In a rhythmic fashion, the creature ascends your member");
            if (character.torso.cocks.count > 1) DisplayText("s");
            DisplayText(" and promptly descends back downward, flooding your nerves with pleasure and a growing need to cum as she darts back and forth like a grounded raven chasing after a worm.");
            ...
            DisplayText(" tightens considerably;  with gusto, she starts stroking the base of your shaft like a squeeze tube, eager to aid your sperm in their journey to her stomach as they unknowingly rush to their certain doom.  Bleat red, her face compresses against your loins as she takes load after load into her throat, moaning like a wanton harlot and swallowing every once in a while to ensure that there will be room for more splooge in her mouth.  Just when you think she is going to injure you with her fierce grip, she lets you go; and not a moment too soon it seems, judging from the redness in your ");
            if (character.torso.balls.quantity > 0) DisplayText("scrotum's");
            else {
                if (character.torso.cocks.count === 1) DisplayText("cock's");
                else DisplayText("cocks'");
            }
            DisplayText(" skin.  Content with your 'donation', the plant woman releases your cock");
            if (character.torso.cocks.count > 1) DisplayText("s");
            DisplayText(" from her mouth with a loud pop; leaving trails of drool and semen on her chin as she does so.  The vines holding you retract from around your limbs and allows you to suit up and leave, whilst she cradles her sated stomach and hums to herself.");
            character.orgasm();
        }
        else {
            // (If all cocks exceed 24 inches, and two to three cocks measure 6 inches in diameter or less total:
            DisplayText("  Despite having to position herself a considerable distance away from your body, the plant woman manages to take [oneCock] and fits it into her mouth.  Due to the distance between you two, the plant woman has to ");
            if (character.torso.cocks.count === 1) DisplayText("prop some of your length up on her average body to prevent it from straining towards the ground.");
            else DisplayText("prop your other dicks up with her average body to prevent them from hitting the dirt covered ground.  Obviously she has plans for them, and doesn't want her prize catches getting dirty.");
            // (jump to Continued... (huge dick sized oral)
            // 	Continued... (huge dick sized oral)
            // (All cocks exceed 4  inches in diameter individually:
            DisplayText("\n\nHaving to scoot back a fair bit to enjoy her catch, the plant woman kneels against the dirty ground and attempts to take your " + CockDescriptor.describeCock(character, character.torso.cocks.get(0)) + " into her mouth.  You feel her attempt to fit your rod into her little mouth; teeth accidentally scraping along your head, the buckling of jaws, and soft lips that are rebutted are all the signs you need to understand that your " + CockDescriptor.describeMultiCockShort(character));
            if (character.torso.cocks.count === 1) DisplayText("is");
            else DisplayText("are");
            DisplayText(" much too large to fit inside of her mouth.  Undeterred by your freakish large member");
            if (character.torso.cocks.count > 1) DisplayText("s");
            DisplayText(", the plant woman smothers her face against your " + CockDescriptor.describeCock(character, character.torso.cocks.get(0)) + ", allowing your pre-cum to lather up her face well. Stopping to sheepishly grin up at you, you see that the plant woman has trails of pre-cum connecting between her face and the head of your dick.  Like a kitten with a ball of string, she chases after the strands of pre-cum and attempts to catch them in her mouth.  You grunt and cough at her childish display, to remind her of what she's SUPPOSED to be doing; odd, coming from someone who's trapped in a vine prison.  Embarrassed, the plant woman nods her head apologetically, and takes to your " + CockDescriptor.describeCock(character, character.torso.cocks.get(0)) + " with ferocious effort, smearing her face along the head of your cock while dragging her tongue along it with wanton abandon.  You groan at the sensation, and tense up as your body is fluid with relaxing sexual chemicals, easing you up and getting you ready for the pleasure that follows.");
            ...
            DisplayText("\n\nWithout noticing, your orgasm arrives and launches forth from your " + CockDescriptor.describeMultiCockShort(character) + ", with enough force to startle the plant woman as it slaps her across the face.  A series of passionate moans rise up from under your dicks as the creature is liberally covered in your steamy jism.  Her tongue now free from your urethra, you breathe a sigh of relief as you finally get to relax; the sensation of tickling gone from your body.  Closing your eyes, you can't help but slump to the ground, mentally drained as the plant woman lunges after your torrent of cum like an orphan to food. With the last shot of cum, the plant woman rises to get feet, ");
            if (character.cumQ() < 500) DisplayText("covered");
            else DisplayText("drenched");
            DisplayText(" in your salty seed as she grins like a fool; obviously dumbfounded at her sheer dumb luck to find  a virile specimen such as yourself. The vines holding you retract, allowing your drained body to slowly fall to the ground. Panting heavily, you lie stomach first against the ground and collect your composure as the plant woman walks up to you and strokes your back and hair. She stays at your side until you muster the will and energy to pull yourself to your feet; at which point she helps you to your [armor] and helps you to put it back on. With a smile, she sees you off as you head back to camp.  “Strange that she didn't leave you and let something else have fun with me”, you wonder in your head as you leave the deep woods.");
            character.orgasm();
        }
        return { next: Scenes.camp.returnToCampUseOneHour };
    }
    ...
    public plantAnalGivingTheDick() {
        DisplayText().clear();
        DisplayText("The sight of this nice, ample ass and the thought of fucking a supple  little pucker piques your " + CockDescriptor.describeMultiCockShort(character) + ", causing ");
        if (character.torso.cocks.count === 1) DisplayText("it");
        else DisplayText("them");
        DisplayText(" to become semi-erect.  As you reach her full ass, your prick");
        if (character.torso.cocks.count > 1) DisplayText("s poke");
        else DisplayText(" pokes");
        DisplayText(" at her soft behind, signaling to her that now is the time guide you into her tight pucker.  Slowly, she edges your cock against the tight opening of her asshole.  Realizing that the asshole ");
        if (character.torso.cocks.count === 1) DisplayText("it");
        else DisplayText("they");
        DisplayText(" will hopefully penetrate is a winner in the tightness department, your " + CockDescriptor.describeMultiCockShort(character) + " surge");
        if (character.torso.cocks.count > 1) DisplayText("s");
        DisplayText(" with vigor and anticipation.   The plant woman can barely contain her surprise as your dick");
        if (character.torso.cocks.count > 1) DisplayText("s");
        DisplayText(" become");
        if (character.torso.cocks.count === 1) DisplayText("s");
        DisplayText(" rock hard and push");
        if (character.torso.cocks.count === 1) DisplayText("es");
        DisplayText(" against her butt hole, her hand shooting up her mouth to contain a giggle.  Looking back over her shoulder at you, she smirks as she pushes against your cock head");
        if (character.torso.cocks.count > 1) DisplayText("s");
        DisplayText(", easing ");
        if (character.torso.cocks.count === 1) DisplayText("it");
        else DisplayText("them");
        DisplayText(" into her tight pucker.  With a sudden and rough slide forward your cock");
        if (character.torso.cocks.count > 1) DisplayText("s");
        DisplayText(" satisfyingly penetrate");
        if (character.torso.cocks.count === 1) DisplayText("s");
        DisplayText(" her anus, leading to a slight gasp as the plant woman takes you inside of her.");

        DisplayText("\n\nKeeping her comfort in mind, the plant creature stares at the trunk in front of her and takes her time as she gradually eases more and more of you inside of her hot innards.  You grunt as the walls of her ass painfully yield to your spear(s) of lust;  The rough stretching of her contours along your glans continuing until she loosens up, to where both of you can be comfortable with the act.  Throwing caution to the wind, the plant woman “tests the waters” by bounces her ass off of your thighs, eventually increasing in tempo as she adapts to the " + CockDescriptor.describeCock(character, character.torso.cocks.get(0)) + " inside of her.  The initial throes of anal sex that your captor inflicts on herself begins to overwhelm your cock");
        if (character.torso.cocks.count > 1) DisplayText("s");
        DisplayText(", causing you to shudder in delicious pleasure as you moan loudly.");
        ...
        if (character.torso.cocks.count > 1) DisplayText("s");
        DisplayText(" as she weakly bounces against you, squeezing down on you with just the right amount of force that it drives you past the point of no return, unleashing a ");
        // Cum output low:
        if (character.cumQ() < 20) DisplayText("small amount of semen");
        else if (character.cumQ() < 250) DisplayText("generous amount of cum");
        else DisplayText("flood of sticky spunk");
        DisplayText(" from your cock");
        if (character.torso.cocks.count > 1) DisplayText("s");
        DisplayText(" and into her tight asshole.");

        // (If there are free cocks:
        if (character.torso.cocks.count > 1) DisplayText("\n\nThe rest of your " + CockDescriptor.describeMultiCockShort(character) + " erupt as well, coating her butt and thighs in sticky jizz.  Like a true slut, the plant woman grabs your ejaculating prick and crams it into her fuck-hole, her pussy milking you of all the cum you can muster up.");
        ...
        if (character.torso.cocks.count > 1) DisplayText("s");
        DisplayText(" from her body.  Surprisingly, no cum (yours or otherwise) leaks out of her love button... weird.  With your lust quenched, you decide to head back to camp and get back to... whatever you were doing.  As you put your [armor] back on, the plant woman slips in quietly and kisses you tenderly on the neck.  Smiling, you wave to the delicate plant woman and make for camp.");
        character.orgasm();
        return { next: Scenes.camp.returnToCampUseOneHour };
    }

    // [Failure: 69! (TsI: 1.6)] (cock(s)less than 24 inches in length, or 4 inches width total if PC has tentacle dicks.
    public failStruggle69() {
        DisplayText().clear();
        ...
        else DisplayText(ButtDescriptor.describeButthole(character.torso.butt) + " as she dives nose first into the under crevice of your " + ButtDescriptor.describeButt(character) + ", rubbing the bridge of it between your cheeks as she giggles");
        DisplayText(".");
        ...
        if (character.torso.cocks.count > 0) DisplayText(CockDescriptor.describeMultiCockShort(character));
        else if (character.torso.vaginas.count > 0) DisplayText("[clit]");
        else DisplayText("[asshole]");
        DisplayText(" with her hands as she moves in for the 'kill'.");

        MainScreen.hideBottomButtons();
        // [No way!, leads to TsI: 1.6r]     [69!, eat her pussy, leads to TsI: 1.6a]    [69! give her oral, leads to TsI: 1.6a]
        MainScreen.getBottomButton(0).modify("No Way!", nowayStruggle69Plant);
        MainScreen.getBottomButton(1).modify("69 EatPuss", struggle69Plant, 2);
        MainScreen.getBottomButton(2).modify("69 EatDick", struggle69Plant, 1);
        MainScreen.getBottomButton(3).modify("69 EatAll", struggle69Plant, 3);
    }
    // [No way!, TsI. 1.6r]
    public nowayStruggle69Plant() {
        DisplayText().clear();
        DisplayText("The dizzying sensation of being hung upside down like a hunter's catch in a tree is bad enough; the idea of this thing forcing her fuck hole against your face and licking your genitalia is more than enough for you to try and break free.  Thrashing around in the vine prison like a fly caught in a spider web, you try to show the plant woman that you are not okay with her choice in 'having fun' with you.  Your erratic and frenzied rejection dawns on the plant woman, and she pouts as she dejectedly flips you up right.  Her stare carries with it a plea for you to reconsider, but you ensure that you will not be swayed on this matter.  With a sad expression on her face, she stares down at the ground, lamenting your denial; raising an eyebrow, you call out the plant creature and assure her through your ");
        if (character.torso.cocks.count > 0) DisplayText(multiCockDescriptLight);
        if (character.torso.vaginas.count > 0) DisplayText("[clit]");
        DisplayText(" that you still want to fool around.  Conveying your preference, you gesture to her that she should... ");
        // (leads to trapped scene selection)
        return { next: 9999 };
    }

    // [69! pussy, service both, leads to TsI: 1.6a]
    public struggle69Plant(pt: number = 1) {
        DisplayText().clear();
        ...
        let target: number = 0;
        if (character.gender === Gender.MALE) target = 1;
        if (character.gender === Gender.HERM) target = 1 + randInt(2);
        if (character.gender === Gender.FEMALE) target = 2;

        // [69!, TsI: 1.6a, eat her pussy]
        if (pt === 2) {
            DisplayText("Abandoning yourself to the plant woman's desires, you proceed to stretch your tongue and take her sweet cunt, allowing her soft sex to drag across it.  The soft tongue of your lover against your ");
            if (target === 2) DisplayText("[clit]");
            else DisplayText("[cock]");
            DisplayText(" flushes your body with arousal as she teases you");

            // PC pussy chosen:
            if (target === 2) DisplayText(", allowing her soft lips to surround your " + VaginaDescriptor.describeClit(character) + ", 'trapping' it for her much anticipated assault.");
            // PC cock chosen:
            else DisplayText(".  Wasting no time with 'formalities,' she stuffs your " + CockDescriptor.describeCock(character, character.torso.cocks.get(0)) + " into her mouth, firmly sealing her lips around your shaft and suckling along the sensitive flesh of your glans.");
        }
        // [69!, TsI: 1.6a, give her oral]
            ...
            // PC pussy chosen:
            if (target === 2) DisplayText(" as she allows her soft lips to surround your [clit], 'trapping' it for her much anticipated assault.");
            // PC cock chosen:
            else DisplayText(" as she wastes no time with 'formalities,' stuffing your " + CockDescriptor.describeCock(character, character.torso.cocks.get(0)) + " into her mouth, firmly sealing her lips around your shaft and suckling along the sensitive flesh of your glans.");
        }
        // [69!, TsI: 1.6a, service both]
            ...
            if (target === 2) DisplayText(" as she allows her soft lips to surround your [clit], “trapping” it for her much anticipated assault.");
            // PC cock chosen:
            else DisplayText(" as she wastes no time with 'formalities,' stuffing your [cock] into her mouth, firmly sealing her lips around your shaft and suckling along the sensitive flesh of your glans.");
            DisplayText("  Not to be an inconsiderate 'guest,' you decide to give a little love to her clit as well, soliciting a girlish little squeal of surprise for your lover.  Seems she lucked out this time with such an open-minded partner.");
        }
        // Clit is chosen for plant woman to focus on
        if (target === 2) {
            // PC vagina (mainly, for herms; some recognition will be given to PC cocks):
            DisplayText("\n\nThe soft texture of your labia now along her tongue, the plant woman spreads your clit with her hands and allows herself a greater angle to attack your eager sex.  With a nice view of your [vagina], the flora girl sinks her tongue into your fuck-hole like an arrow into bare flesh.  Rending her way through your love canal as her saliva mixes with your pussy fluids, the warmth of her wetness making itself known after every lick of your sex.  Pressing her thighs against your chest, her wet clit slides against your moaning face with prompting force, conveying her need for you to control yourself, center your focus, and lick that tender slit of love button like a good " + character.mf("boy", "girl") + ".  Mustering your concentration, you press your face hard against her clit and dart your tongue inward, sliding it ever so gently along her tight contours.");
            ...
            // (Herm support:
            if (character.torso.cocks.count > 0) {
                DisplayText("\n\nIf that wasn't bad enough, the unsporting woman also takes to your " + CockDescriptor.describeMultiCockShort(character) + " and strokes ");
                if (character.torso.cocks.count === 1) DisplayText("it");
                else DisplayText("them");
                DisplayText(" teasingly.  The pompous creature gives you a stare down at your form and grins at the “superior” position she thinks she has you in.");
            }
            ...
            if (character.torso.cocks.count > 0) {
                DisplayText("  With your feminine release comes a slightly painful build up of pressure in your loins; then, the hot and heavy eruption of cum in the air as your prick");
                if (character.torso.cocks.count > 1) DisplayText("s explode");
                else DisplayText(" explodes");
                DisplayText(", coating the floral vixen in your seed.");
            }
        ...
        // Cock(s):
            ...
            DisplayText("\n\nPossessed by savage sexual instinct, the feminine flora hybrid takes to sucking your rod so fiercely that she roughs up the sensitive skin on your head with her bumpy tongue.  Each drag of flesh against cock is getting closer to being painful rather than pleasurable.  Thankfully she applies just the right amount of pressure and suction to avoid chaffing you horribly.   Twitching in your vine prison like an insect hung by a single limb, your body juts back and forth as it tries to withdraw your " + CockDescriptor.describeCock(character, character.torso.cocks.get(0)) + " from her mouth.  The pleasure from the vigorous and brutal blowjob is certainly grand, but her harsh servicing of your cock is constantly triggering your flight instinct as she overwhelms your body.  Like a finger trap from back home, the plant creature seems firmly latched onto the head of your cock, sucking up the pre-cum on your shaft like an addict.  Cooing softly, she looks down at you through partially covered eyes, her leafy hair getting in the way of her slutty gaze as she watches you grit your teeth and grimace.");
            ...
            if (pt === 1) DisplayText("cock");
            else if (pt === 2) DisplayText("love button");
            else DisplayText("love button and cock");
            DisplayText(", trying your best to guide your tongue around her ");
            if (pt === 2) DisplayText("clit and vagina");
            else if (pt === 1) DisplayText("glans and shaft");
            else DisplayText("clit, vagina, glans, and shaft");
            DisplayText(" while you hang upside down.  No doubt loving every second of this, the plant woman starts gyrating and grinding her sex against your face, moaning and whimpering between mouthfuls of cock as you ");
            if (pt === 2) DisplayText("probe her depths");
            else DisplayText("do your best to service the stiff prick in your mouth");
            DisplayText(".");

            // (PC is herm:
            if (character.gender === Gender.HERM) DisplayText("  In reward for remembering your “duties,” a set of fingers find their way along your feminine sex and into your honey pot, her teasing of your [vagina] ensuring that no sexes on your body are needlessly left out.");
    ...
    public victoryVsPlant() {
        DisplayText().clear();
        // (Victory, V 1.1L):
        DisplayText("The plant woman sinks to her knees, clutching her ");
        // PC HP victory:
        if (monster.stats.HP < 1) DisplayText("shoulder and sides.");
        else DisplayText("breasts and pussy, twisting at her teats and finger herself like a woman possessed.");
    ...
    public defeatOrDomPlantAndCuntFuck() {
        ...
        if (character.torso.cocks.count > 1) {
            DisplayText("  Your extra endowment");
            if (character.torso.cocks.count === 2) DisplayText(" flaps");
            else DisplayText("s flap");
            DisplayText(" wildly against her thighs and ass, quietly acknowledging the call to arms and prepare to do their part.");
        }
        ...
        DisplayText(".  The sensation is too much, and you cry out as you flood her vagina with your hot semen.  The plant woman senses your orgasmic release and begins to squeeze your sides with her legs, as if pumping you for all of your seed.  Your ");
        if (character.torso.balls.quantity === 0) DisplayText(CockDescriptor.describeMultiCockShort(character));
        else DisplayText(BallsDescriptor.describeBalls(true, true, character));
        DisplayText(" acknowledge");
        if (character.torso.balls.quantity === 0 && character.torso.cocks.count === 1) DisplayText("s");
        DisplayText(" her desire and continue to shoot thick streams of your seed into her");
        if (character.torso.cocks.count > 1) DisplayText(" as well as on her stomach and the ground beneath you");
        if (character.torso.vaginas.count > 0) DisplayText("; your VaginaDescriptor.describeVagina(character, character.torso.vaginas.get(0)) quivers with the need for release, letting loose with its own torrent of lady fluids as you moan in orgasm");
        DisplayText(".  After a few moments, you slump down on the plant woman's chest, completely spent. A few minutes rest does you good, and with time you rise back up and look down at your partner; a dazed, blissful expression has fallen on her features, the creature lost in pure nirvana as she cradles her stomach. Satisfied with how things turned out, you give her one last kiss and struggle to get back up.  Damn, your ");
        if (character.torso.balls.quantity > 0) DisplayText("[balls] really hurt");
        else {
            DisplayText("dick really hurts");
            if (character.torso.cocks.count > 1) DisplayText("s really hurt");
        }
        DisplayText(", but looking back at the heap of a woman behind you, you concur that it was totally worth it.");
        ...
        if (character.stats.HP < hpMax()) {
            DisplayText("Upon returning to camp, you notice that some of the wounds the plant creature kissed have actually healed.  You're not sure how exactly this worked, but at least you don't have to put up with too many cut and bruises now.");
            // (HP restored +50).
            // 9999 add HP
        }
        character.orgasm();
    ...
    public victoryOverPlantGalWithPCCockCorrupted() {
        ...
        if (character.torso.hips.legs.isGoo()) {
            ...
            MainScreen.hideBottomButtons();
            MainScreen.getBottomButton(0).modify("Yes", corruptDomWinFuckGooGal);
            MainScreen.getBottomButton(1).modify("No", victoryOverPlantGalWithPCCockCorrupted, true);
        }
        else victoryOverPlantGalWithPCCockCorrupted(false);
    }
    // [Yes] - goofuck
    public corruptDomWinFuckGooGal() {
        DisplayText().clear();
        DisplayText("Taking the slimy mass that is your lower body, you push against her tight ass and let it ooze into her anus.  The wet and sloppy intrusion at her pucker causes a cry of fulfilled desire to escape from her mouth.  Her tongue - not content with being the odd one out - hangs past her open mouth as she giggles like a whore, the creature overwhelmed and utterly obedient to your desires as you fuck her roughly and cause her to leak her sexual fluids like a boat with a hole in it.");
        victoryOverPlantGalWithPCCockCorrupted(false);
    }

    // End scene for Vc 1.1
    public victoryOverPlantGalWithPCCockCorrupted(newl: boolean = false) {
        if (newl) DisplayText().clear();
        else DisplayText("\n\n");
        DisplayText("After an eternity of ravaging her pussy, you feel your load building up pressure and getting ready to explode.  You tighten your grip on the plant woman's hair and with one last thrust fill her womb with your corrupted semen.  You jerk her hair back, forcing her head back towards your mouth as you begin to neck her.  \"<i>AuuughhhAHHHHHHHHHHHHHHH!</i>\" she screams out as you pull on her hair, with her pussy clamping down on your " + CockDescriptor.describeCock(character, x) + ".  Her fuck hole ever so eager to receive your seed, her body convulses violently as you continue to fire more semen into the floral vixen's cervix.  The excretion is too much, and with a groan, you pass out on top of her.");
    ...
    public victoryDefeatPlantGirlGetBJMainSCREENON() {
        ...
        // PC has balls: (
        if (character.torso.balls.quantity > 0) DisplayText(" cusps your [balls] in her hand, and suck on the sides of your [sack]. She licks upwards from your testicles and caresses them before");
        else DisplayText(" licks up from your gouch before");
        DisplayText(" slowly proceeding towards your glans and swirling her tongue around it like a tootsie roll.  It isn't long before the plant woman suddenly stops and looks up at you with mischievous eyes.  She raises her eyebrows twice, and then ");

        // [If cock(s) are tentacle dicks and greater than 24 inches in length, jump to (V and D 1.2tc)]
        if (character.torso.cocks.filter(Cock.FilterType(CockType.TENTACLE) > 0 && (character.torso.cocks.countType(CockType.TENTACLE) === character.torso.cocks.count || randInt(2) === 0))).length {
            plantGirlPCWinGetTentacleBJ();
            return;
        }
        else {
            // [if cock size is 4-14 inches]
            if (character.torso.cocks.list[character.torso.cocks.sort(Cock.LargestCockArea)[0]].cockLength < 14) DisplayText("takes the entirety of your [cock biggest] in her mouth and down her throat.  You lurch forward as your cock hits the back of her throat, moaning intensely at the plant girl's ability to take your whole penis in her mouth.");

            // [If cock size is greater than 14 inches]
            else DisplayText("attempts to take as much of your member as she possibly can.  Your [cock biggest] slams against the back of her throat, and the plant woman tries to take a little more of you to no avail.  She looks up at you apologetically, but you pet her head and ensure that she knows you aren't upset with her.");

            DisplayText("  She stares up at you with a flirtatious look and pulls your cock out of her throat, seeking to get a little more intimate with the head of your dick.  You grip on her hair tightly with every pleasurable sensation as her head bobs back and forth between your thighs.");
        ...
        // ______________________________________________________________________________
        // (if Non-goo:
        if (!character.torso.hips.legs.isGoo()) DisplayText(".  She pulls your dick out of her mouth and closes her eyes as she gets ready to catch the rest of your load with her mouth.  Thick globs of semen erupt from your CockDescriptor.describeMultiCockShort(character) and strike her in the face.  She herself orgasms as your torrent of semen washes over her head and chest, covering both liberally.");
        // (If goo:
        else DisplayText(", bucking hard against her face as you grunt furiously through gritted teeth and a fierce expression of release.  The plant woman inhales deeply just as you cum into her hot, moist mouth, producing a satisfied series of muffled groans as you deposit your seed.");
    ...
    public plantGirlPCWinGetTentacleBJ() {
        ...
        if (character.torso.cocks.get(x).length < 48) {
            ...
            if (character.torso.balls.quantity > 0) "[sack]"; )
		else DisplayText(BreastDescriptor.describeNipple(character, character.torso.chest.get(0)));
            DisplayText(" and begins to squeeze tighter and tighter.  “Owww! Owww! Okay, Okay! I get it!” you cry out as you once again ram your " + CockDescriptor.describeCock(character, x) + " down her throat.  The plant woman responds by massaging your ");
            if (character.torso.balls.quantity > 0) DisplayText("balls");
            else DisplayText("nipples");
            DisplayText(", showing that this is what she wants.  You laugh out loud at the ridiculousness of the situation: you're using a plant creature's throat as a makeshift cunt, and she doesn't seem to be running out of air anytime soon.");
        ...
        // [If tentacle cock is longer than 48 inches]
            ...
            DisplayText("\n\nYour loins begin to ache for release, and you feel the muscles in your " + CockDescriptor.describeCock(character, x) + " tensing up,  cum bulging up your cock(s) as your load works its way down her throat and into her stomach.  The instant it hits, the plant woman bucks wildly, her sex spraying you in the face with her nectar-y pussy juice.  You bury your face against her vagina and lap up all of the juice you can as she orgasms.  As you both finish cumming, you and her collapse in a cum-stained heap, panting heavily as you both recover from the ordeal.");
        }
        // (V and D 1.2tc) conclusion
        // (V and D 1.2tc) conclusion:
        DisplayText("After a few minutes, you gather enough strength to rise to your feet and pull your flaccid cock from her mouth.  Content, she gives it one last lick before passing out into a deep sleep.  You smile and put your [armor] back on, eager to get back to camp.");
        character.orgasm();
        cleanupAfterCombat();
    }
    ...
    public beatPlantGirlDomLetsFuckAnal() {
        ...
        if (inCombat) {
            ...
            // Testicle check:
            // ([balls]/pelvis)
            if (character.torso.balls.quantity > 0) ouptutText("Your [balls] slap");
            else DisplayText("Your pelvis slaps");
            DisplayText(" against her pussy, causing wet slapping sounds and coating your [sack] liberally with her sexual secretions.  The plant woman moans weakly");
            if (character.torso.cocks.count > 1) DisplayText(" grabs a of your penis and guides it to her vagina.");
            else {
                DisplayText(", while she ");
                // PC has testicles: (
                if (character.torso.balls.quantity > 0) DisplayText("fondles your testicles sensually; surprisingly she doesn't injure you by squeezing too hard despite the rough fucking.");
            }
            // Vagina check:
            if (character.torso.vaginas.count > 0) {
                if (character.torso.cocks.count === 1 && character.torso.balls.quantity > 0) DisplayText("  Not content with just teasing your balls, the");
                else DisplayText("  The");
                DisplayText(" attractive creature darts her hand down to your [vagina], fingering you passionately as she takes the length of your meat into her hungry sex.");
            }
            ...
            DisplayText("\n\nYou grunt and moan as your " + CockDescriptor.describeCock(character, x) + " shoots its anal payload ");
            if (character.torso.cocks.count > 1) DisplayText(" along with its vagina-clad brother");
            DisplayText(".  She slumps against you, lost in pleasure and sensation as you drench her interior with hot, sticky cum.  The plant woman's nectar-flavored milk shoots forth from her breasts and coats the ground in front of the both of you, causing her to shake and moan furiously.");
            if (character.torso.vaginas.count > 0) DisplayText("Your [vagina] obliges, and a few steams of lady spunk flow from your [clit] and intertwine with the fauna creature's fluids on the ground.");
            DisplayText("  As the plant woman comes down from her climax, you lick your hands clean of her nectar and pull out from her.  You shoot her a look of appreciation, and she fires back a look of utter contentment and bliss.  You leave her to recuperate from the act and head back to camp.");
            character.orgasm();
            cleanupAfterCombat();
        }
        // (Dominant Let's Fuck! (giving anal) D 1.3
        else {
            DisplayText("Before the plant gets a chance to take two steps forward,you put your hand out towards her, signaling her to stop. Almost immediately the plant woman pauses her movement towards you, like a herbivore as it listens for the sounds of predators. Knuckles against your chin, you stare longingly at the creature's hips as she stands motionless, wondering why you wanted her to stop. Signaling with your index finger, you instruct the perplexed female to turn around. Reluctantly she spins around and exposes her ample rump to your gaze, causing a violent twitch in your pants as her full bottom comes into view. Satisfied, you proceed towards the plant woman, disrobing as you go.");
            DisplayText("\n\nThe sound of your gear hitting the ground causes the plant woman to look over her unimposing shoulder, and she grins as you embrace her from behind. Taking her left, average breast in your hand, you fondle both her mound and her anther-like nipple as you caress her stomach with your free hand.  The creature shudders from your touch, and you feel a thin stream of her moist pussy juice slowly run down your leg.  The need to fuck her becomes too overwhelming for you, and [eachCock] presses itself against her ample ass, pulsating with desire.  As you lower a hand to guide your CockDescriptor.describeCock(character, suitable cock) into her tight asshole, her hand darts out and grabs your dick instead, beating you to the punch.  Surprised at her speed, you slowly draw your hand back and gasp as the plant woman pushes against your CockDescriptor.describeCock(character, suitable cock)'s head, taking it past her tight pucker and into her anus.");
    ...
    public pcBeatsPlantgirlAndCorruptAssFuck() {
        ...
        // (If Naga:
            ...
            MainScreen.hideBottomButtons();
            MainScreen.getBottomButton(0).modify("Penetrate", nagaPenetrateWithTailDuringPlantGirlMeanAnalPCWin);
            MainScreen.getBottomButton(1).modify("Stick2Anal", pcBeatsPlantgirlAndCorruptAssFuckPt2);
            return;
        }
    ...
    public victorySuckPlantgirlsDick() {
        ...
        if (inCombat) {
            ...
            if (character.torso.cocks.count > 0) DisplayText("[eachCock] is rigidly erect");
            else if (character.torso.vaginas.count > 0) DisplayText("your [vagina] is gazed over with your secretions");
            // Genderless:
            else DisplayText("your body is flushed with sexual need");
        ...
        dynStats("lus", 10 + character.stats.lib / 5);
        if (inCombat)
            cleanupAfterCombat();
        else return { next: Scenes.camp.returnToCampUseOneHour };
    }
    ...
    public beatPlantGirlAndTakePlantCockInButt() {
        ...
        if (character.torso.cocks.count > 0) DisplayText("[eachCock] becomes rock solid");
        if (character.gender === Gender.HERM) DisplayText(" while ");
        if (character.torso.vaginas.count > 0) DisplayText("your pussy becomes incredibly wet");
        if (character.gender === Gender.NONE) DisplayText("nipples become erect");
        DisplayText(" in response to your own arousal and desire.  The tip of her penis presses against your cheeks and pushes its way into you as you allow it past your anus and into the depths of your ass.  The creature opens her mouth in an 'ohh' expression, enjoying the sensation of breaking past your [asshole].  As she bottoms out, ");
        if (character.torso.cocks.count > 0) DisplayText("[eachCock] jerks upward, the last shot of blood filling and hitting them like a thunderous punch");
        if (character.gender === Gender.HERM) DisplayText(", while ");
        if (character.torso.vaginas.count > 0) DisplayText("your pussy leaks a bit of girl cum.  It slowly drools out onto her pelvis");
        if (character.gender === Gender.NONE) DisplayText("your body quivers from the sensation");
        DisplayText(".  ");
        // PC has balls:
        if (character.torso.balls.quantity > 0) DisplayText("Your [balls] laying on top of her pelvis, y");
        else if (character.torso.vaginas.count > 0) DisplayText("Your vaginal muscles clenching up along with your sphincter, y");
        else DisplayText("Y");
        DisplayText("ou ");
        if (character.torso.balls.quantity > 0) DisplayText("begin to massage your [sack] as you raise your hips upward");
        else if (character.torso.vaginas.count > 0) DisplayText("relish in the moment and allow a hand to drop to your sex as you raise your hips, eager to please both ends during this act");
        else DisplayText("grab her nipples and begin to play with them");
        DisplayText(".  Slowly, you lower your hips back down once again onto the plant woman's penile member, causing your partner to moan in lustful appreciation.  A sense of fullness floods you as you allow yourself to take her entire member inside of you.  You ");
        if (!character.torso.vaginas.count > 0) DisplayText("place both hands on her shoulders");
        else DisplayText("continue to finger yourself");
        DisplayText(" as you bob your [asshole] against her lap, moaning loudly as you do.  The plant woman's hands shoot out and ");
        // PC has a pussy (and/or) a dick:
        if (character.gender > 0) {
            DisplayText("grab both your [chest] and ");
            if (character.torso.cocks.count > 0) DisplayText("[oneCock]");
            if (character.torso.vaginas.count > 0) DisplayText("your vulva");
        }
        // PC is genderless:
        else DisplayText("your nipples, playfully tugging on them");
        DisplayText(", intent on magnifying your own pleasure.");

        // Dicked and/or pussy equipped characters only, genderless skips this part
        DisplayText("\n\nThe pace quickens as your cheeks begin to slam against her pelvis, while the creature ");
        if (character.torso.cocks.count > 0) DisplayText("strokes [oneCock] with her hands");
        if (character.gender === Gender.HERM) DisplayText(" as she also ");
        if (character.torso.vaginas.count > 0) DisplayText("massages your labia");
        if (character.gender === Gender.NONE) DisplayText("massages you");
        DisplayText(".");
        DisplayText("  The plant woman thrusts in kind,  mashing her pelvis against your own as you bounce off the creature. You manage to time your descent with her thrusts to intensify both of your pleasures.  She ");
        if (character.torso.cocks.count > 0) DisplayText("quickens her pace of stroking your cock");
        if (character.gender === Gender.HERM) DisplayText(" and ");
        if (character.torso.vaginas.count > 0) DisplayText("fingers the interior of your pussy, causing you to roll your eyes into the back of your head as she sensually teases the inside of your sex");
        if (character.gender === Gender.NONE) DisplayText("raises herself to deliver a passionate kiss");
        DisplayText(".  You feel her thrusts getting shorter now, and you know that her she going to blow her load soon.  With gradually closing eyes and increasingly frequent panting and moaning, she grabs your hips and holds them in place as she thrusts upward one last time.  With a lusty moan, her spunk shoots into your hot innards with significant force, allowing you to feel the impact of every shot of cum in your anus.");
        DisplayText("\n\nYou cry out in fierce orgasm, ");
        if (character.torso.cocks.count > 0) {
            DisplayText("[eachCock] erupting its own payload onto her face and chest");
        }
        if (character.gender === Gender.HERM) DisplayText(", ");
        if (character.torso.vaginas.count > 0) DisplayText("vaginal muscles clamping down and drooling pussy juice all over the plant woman's lap");
        if (character.gender === Gender.NONE) DisplayText("twitching futilely around the plant-girl's swollen seeder");
        DisplayText(".  With a groan, you collapse against the creature's chest, desperately trying to catch your breath from the taxing ordeal.  The plant woman hums approvingly, stroking your hair as your body ascends and descends from her own because of your heavy breathing.  After some time, you are able to muster the strength to pull yourself off of the plant creature, her cock slipping out of your ass and unleashing a flood of sappy jism from your anus.  Content, she passes out into a deep sleep.  You grin to yourself, putting your [armor] back on and heading back to camp.");
        character.orgasm();
        if (inCombat)
            cleanupAfterCombat();
        return { next: Scenes.camp.returnToCampUseOneHour };
    ...
    public defeatPlantgirlBlowAndButtfuckCorrupt() {
        ...
        Flags.list[FlagEnum.TIMES_GOT_CORRUPT_BJ_ANAL_PLANTGIRL_SCENE]++;

    }

    // Victory and Dominant: Fuck her cock w/ your pussy (light side)
    public defeatPlantgirlBlowAndButtfuckPure() {
        // V 1.6 and D 1.6
        DisplayText().clear();
        DisplayText("With your hands on your hips you " + character.mf("walk", "sway"));

        // (Victory, V 1.6:)
        if (inCombat)
            DisplayText(" towards the battered creature as she struggles to get back to her feet.  Her efforts are quite for naught as her arms give out, causing her to hit the ground hard.");
        // (Scene leads to V and D 1.6)
        // (Dominant, D 1.6:
        else DisplayText(" towards the plant woman, causing her to giggle in response as she beckons you over to her.");
        // (Scene leads to V and D 1.6)
    }
    ...
    public defeatPlantgirlBlowAndButtfuckPure() {
        ...
        if (character.torso.hips.legs.isNaga()) DisplayText("now exposed ");
        DisplayText(VaginaDescriptor.describeClit(character) + ".  The plant woman yelps in surprise as the vine makes contact with your labia, causing you to jump a little.  You turn to face the plant woman, who is blushing profusely and resting her hand on her pussy.  Looking at the vegetation, you deduce that she must be able to feel you along her vines and that these creepers may also function as penile appendages.  A grin spreads across your face as the realization hits you, and you bring the vine back to your sex and tease your vulva.  Soft moans escape the plant girl as the head of the perverted vegetation brushes past your clit, and you ");
        // (pure:
        if (character.stats.cor < 50) DisplayText("giggle");
        // Corrupted:
        else DisplayText("smirk");
        DisplayText(" at the woman's predicament.  Deciding to stop teasing the poor girl, you take the head of the creeper and push it into your pussy, edging it upwards at a gradual pace so that your partner can feel the contours of your " + VaginaDescriptor.describeVagina(character, character.torso.vaginas.get(0)) + ".");
    ...
    public vineAndCockWithPlantGirlPCWin(scene: number = 1) {
...
PC has a cock(s)(Overcome by sensation, your CockDescriptor.describeMultiCockShort(character) release their hot semen all over the plant woman, which the feminine creature quickly picks up with her fingers and stuffs into her craw, slurping it up hungrily as she does so.)

Once your contractions pass (and you finish with (Both)  your "deposit(s)"), you pull the vine out of your VaginaDescriptor.describeVagina(character, character.torso.vaginas.get(0)) and rise to your feet. You quickly don your (armor) and set off for camp, winking at the worn out, (masturbating/content) plant creature as you walk away.

[If you chose vines, V and D 1.6mvp]:
Not content with just one vine you reach out to the bush and grab two more creepers, bringing them to your ass and mouth.  The plant woman whimpers desperately as she brushes her penis against your thigh, pleading through her body language to tend to her aching member.  You stare into her eyes and smile as you insert one vine past your VaginaDescriptor.describeVagina(character, character.torso.vaginas.get(0)) and ButtDescriptor.describeButthole(character.torso.butt), while taking the third one in your mouth.  The plant woman lets out a frustrated, furious groan as she raises her clenched fists to her mouth. Your sly smile just adds to her "torment' as you go crazy on yourself with the vines.  Abandoning yourself to pure sexual instinct, you pump and fuck as hard as you can, causing the creature that lies underneath to writhe and grab at your wrists.  As the beauty of fauna braces herself against the torrent of pleasure she's receiving, you work the vine in your mouth as deep as possible and use the back of your throat to massage her penile appendage.  You continue your teasing and fucking of the overwhelmed plant girl until you feel the buildup of release approaching.  With a mighty grunt, you cum all over the vines and coat them in your juices, while your female "friend" cradles you close to her as she herself cums, wrapping around you with her arms and legs. Her penis and pussy both erupt like a volcano; her penis covering your (pelvis and stomach/ gooey body) in her sappy jism, and her pussy soaks the ground underneath the two of you.
...
Both of you moan as your sexes touch one another, sensation running through your vulva like wildfire. Twitching ever so slightly, the plant woman grabs you by your LowerBodyDescriptor.describeHips(character) as she angles her cock to breach the mouth of your VaginaDescriptor.describeVagina(character, character.torso.vaginas.get(0)).  Without wasting any more time you buck your hips downward, driving her member past your hungry lips and through the walls of your sex, flooding your body with endorphins and bringing on a sensation of giddiness.  Your partner squirms under you as her cock bottoms out inside of you, unable to control herself as her member takes the full blunt of your pussy. Grabbing her by the shoulders you begin to fuck the plant woman, bobbing your [butt] up and down against her lap; the melancholy of your intimate act ringing forth with the sounds of sloppy, wet sex. The nerves in your pussy light up as her member brushes back and forth inside of your fuck-hole, making you cry out occasionally as she “abuses” your cunt like it owes you money.  In response, you make the rough ride you're giving her faster and more savage.  Hands darting upward, the plant woman hungrily grabs your BreastDescriptor.describeBreastRow(character.torso.chest.get(0)),

(if breasts: pulling on your BreastDescriptor.describeNipple(character, character.torso.chest.get(0)) as she bucks her head back in utter bliss. You let out a pained cry and twist her nipples in response, soliciting a similar reaction from her. Both of	 you grin manically at each other, and you lower yourself to give her a quick kiss before rising back up into your straddling position, eager to abuse her cock until you get her sweet splooge inside of you)
...
(if herm: slips her hand down to your CockDescriptor.describeMultiCockShort(character), taking (it/one at a time) into her hands, stroking (it/them) furiously as you pound her moist love canal.  You let up on the rough fucking for a brief moment as she tends to your dick(s), taking in the euphoria of her soft, leaf like texture on your naughty bit(s). )
...
Eventually all good things must come to an end;  Your inevitable orgasm approaches, body trembling in anticipation at the thought of covering this creature in cum.  Sexual fluids rush out of you as you concentrate on pushing, forcing them out. Instinct takes hold as you mash your pelvis against the plant woman's lap. (Your CockDescriptor.describeMultiCockShort(character)) explode(s),
...
As you wake from the land of dreams, you feel a hand moving through your HeadDescriptor.describeHair(character).  Eyes fluttering open, you turn and look up; the plant woman returns your sleepy gaze with a caring smile.  You smile back at her and stretch contently.  Grabbing you by the side and helping you to your feet, the floral vixen leads you back to your clothes and helping you to armor back up. She walks you to the path you first encountered her on, and sees you off as you head back to camp.

Victory or Dominant Let's Fuck: Perform cunnilingus on the plant woman

 V 1.7 and D 1.7, respectively]

(Victory, V 1.7: Aroused from the fight and pent up lust, you disrobe in front of the defeated plant woman; she did, after all, want to have sex with you.  And now that you are in the mood, you feel like burying your face in that petal-like vulva between her leafy, well shaped thighs.  Turning to see what fate her victorious opponent has in store for, she shuffles backwards at the unexpected sight of your (CockDescriptor.describeMultiCockShort(character)/VaginaDescriptor.describeVagina(character, character.torso.vaginas.get(0))/multiple sexes/ generless sex). (Scene leads to V and D 1.7)
...
(Victory and Dominant scenes converge here, V and D 1.7): Catching the scent of her lubricating fluids on her delicate clitty, you breathe heavily, shoulders rising and falling in tandem with your respiration. (If dominant: Looking downward, the plant woman sights) Your CockDescriptor.describeMultiCockShort(character) [and]/ (clitdescript()/
If genderless: BreastDescriptor.describeBreastRow()
...
(if vagina and/or cock: (VaginaDescriptor.describeVagina(character, character.torso.vaginas.get(0))/   /and/ CockDescriptor.describeMultiCockShort(character)/
...
(VaginaDescriptor.describeVagina(character, character.torso.vaginas.get(0))/ CockDescriptor.describeMultiCockShort(character)/ assholeDescript(0) ).
...
(CockDescriptor.describeMultiCockShort(character)/ VaginaDescriptor.describeVagina(character, character.torso.vaginas.get(0))/assholeDescript( ) )
against her face, slathering (it/them) down her forehead and past her nose as
PC has cock(s): (gravity takes your CockDescriptor.describeMultiCockShort(character) and prods (it/them) against her lips)
...
(if genderless. Vc 1.7g: prods the crack of your ass as she parts your [butt], and licks your ButtDescriptor.describeButthole(character.torso.butt) with her energetic tongue; oddly enough, it resembles the texture of flower petals, and having that sensation against your hole causes you to quiver.)  Her moist tongue slaps against your hole, leaving it nice and wet as she prods and lathers your hole with her tongue. )
