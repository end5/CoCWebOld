 ...
 */
		...
		if (Flags.list[FlagEnum.URTA_QUEST_STATUS] > 0 && Flags.list[FlagEnum.URTA_QUEST_STATUS] < 1) {
			if (player.short === "Urta") {
				if (player.torso.cocks.count > 0) {
					if (player.torso.cocks.get(0).length === 20 && player.torso.cocks.get(0).thickness === 3) {
						if (player.torso.vaginas.count > 0) {
							if (player.torso.chest.get(0).rating === 7) return true;
						}
					}
				}
			}
	...
	private comfortUrtaAfterFamFamTalk(): void {
		...
		if (urta.urtaDrunk()) DisplayText("boozy ");
		DisplayText("kiss on your lips as she hugs you tight - tight enough that you feel every vein through the concealing silk of her outfit.  \"<i>I'm a lucky girl.</i>\"");
	...
	public infertilityQuestions(): void {
		...
		DisplayText("\n\nWell, ");
		if (player.stats.cor < 33) DisplayText("you have nothing to complain about there.  Your sexual romps with Urta are always a delight.");
		else if (player.stats.cor < 66) DisplayText("she's definitely a handful in the sack, you've got to give her that.");
		else DisplayText("she's a willing fuck, so no complaints there.  Now if she would just get on with it - if you wanted to flirt, you would've instigated it.");
	...
	public startUrtaQuest(): void {
		...
		player.short = "Urta";
		player.tallness = 71;
		player.torso.neck.head.hair.color = "gray";
		...
		player.hips.legs.type = LegType.FOX;
		player.torso.neck.head.earType = EarType.FOX;
		player.torso.neck.head.face.type = FaceType.FOX;
		player.torso.tailType = TailType.FOX;
		player.torso.tailVenom = 1;
		player.torso.cocks.add(new Cock());
		player.torso.cocks.get(0).type = CockType.HORSE;
		player.torso.cocks.get(0).length = 20;
		player.torso.cocks.get(0).thickness = 3;
		player.torso.balls.quantity = 2;
		player.torso.balls.size = 4;
		player.cumMultiplier = 80;
		player.thickness = 30;
		player.tone = 50;
		player.femininity = 90;
		player.torso.hipRating = 12;
		player.torso.butt.rating = 12;
		player.torso.butt.looseness = 2;
		player.statusAffects.add(StatusAffectType.BonusVCapacity, 58, 0, 0, 0);
		player.createVagina();
		player.torso.vaginas.get(0).wetness = VaginaWetness.DROOLING;
		player.torso.vaginas.get(0).looseness = VaginaLooseness.NORMAL;
		...
		DisplayText("<b>You are Urta, a hermaphroditic fox-morph.</b>");
	...
	private getHelBangedAsUrta(): void {
		...
		if (Flags.list[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] > 0) DisplayText(", and even how they worked together to bring down an evil sorceress and save her father. Your heart swells with pride as she recounts " + player2.short + " doing so a noble thing, and for a moment you know you made the right pick in a lover..");
		DisplayText(".  You sit and listen for a long few minutes, sipping at the beer until the liquid courage starts to work its wonders, giving you a little tingle... and helping to open you up, too.  Soon, you're drawn into the conversation, joking and teasing with the other girls as if you'd been friends for ages.");
	...
	private fuckHelAndKitsunesAsUrta(): void {
		...
		//If PC has a cock:
		if (player2.hasCock()) {
			DisplayText("  \"<i>I've got a better one ");
			if (helFollower.followerHel()) DisplayText("at home");
			else DisplayText("at my beck and call");
			DisplayText(", you know...  But we've got that in common, don't we?  Mmm, maybe we can share it some day?  A horny girl like you'd just love that, woudn't you? Getting to worship " + player2.mf("his", "her") + " cock beside me while my tail fills your ass so full you scream....</i>");
		}
	...
	private visitPCPreUrtaQuest(truth: boolean): void {
		...
		if (player.statusAffects.has(StatusAffectType.JojoNightWatch) && player.statusAffects.has(StatusAffectType.PureCampJojo))
			DisplayText("You easily sneak past a mouse monk.  He's looking towards the sky mostly, perhaps watching for imps.  ");
		if (Flags.list[FlagEnum.ANEMONE_WATCH] > 0 && Flags.list[FlagEnum.ANEMONE_KID] > 0)
			DisplayText("A confused-looking anemone with a " + ItemType.lookupItem(Flags.list[FlagEnum.ANEMONE_WEAPON_ID]).longName + " nearly sees you, but you duck around a rock and escape her notice.  ");
		if (Flags.list[FlagEnum.FUCK_FLOWER_LEVEL] >= 4 && Flags.list[FlagEnum.FUCK_FLOWER_KILLED] === 0 && Flags.list[FlagEnum.HOLLI_DEFENSE_ON] > 0)
			DisplayText("A dangling tentacle nearly hits you in the dark.  You roll aside at the last moment, looking up to see dozens of the things idly dangling around.  When did this camp get such a corrupt tree in it?  You suppose it must have its uses in defending against foes from the sky...  ");
		if (camp.companionsCount() > 2) DisplayText("There are a number of people in the camp, but you avoid them as you head towards " + player2.short + "'s bunk.  ");
		DisplayText("The camp looks pretty nice actually.  Living out here must have given " + player2.mf("him", "her") + " plenty of time to improve it.");
		...
		if (player.statusAffects.has(StatusAffectType.DungeonShutDown)) DisplayText(", no matter why " + player2.mf("he", "she") + " was sent here");
		...
		if (truth) {
			...
			if (player2.cor < 33) DisplayText(player2.mf("he", "she") + " argues that " + player2.mf("he", "she") + " should come anyway.  You tell " + player2.mf("him", "her") + " no, firmly, even though it breaks your heart.");
			else if (player2.cor < 66) DisplayText(player2.mf("he", "she") + " nods understandingly, not surprised that you have to handle this quest alone.  The two of you are alike in that way - each with their own burden to bear.");
			else DisplayText(player2.mf("he", "she") + " nods dismissively, trusting you to handle it alone you suppose.  " + player2.short + " always was hard like that, even though " + player2.mf("he", "she") + " clearly likes you.");
			DisplayText("  You hold on to " + player2.mf("him", "her") + ", for comfort, dreading the long day ahead.  " + player2.mf("He", "She") + " pats your back comfortingly as you cry, and when you break from the hug, " + player2.mf("he", "she") + "'s looking at you expectantly.");
			if (Flags.list[FlagEnum.SLEEP_WITH] != "" && Flags.list[FlagEnum.SLEEP_WITH] != 0) DisplayText("  " + player2.short + " suggests the two of you move off, so as not to wake " + Flags.list[FlagEnum.SLEEP_WITH] + " in the night.");
		}
		//{LIE}
		else {
	...
	private urtaPutsHerDickInPCsWithDicks(truth: boolean): void {
		...
		else DisplayText(" and let your tits sway over " + player2.mf("his", "her") + " belly");
		DisplayText(", giving " + player2.mf("him", "her") + " something to enjoy while you begin to shift position, dragging your dick down " + player2.mf("his", "her") + " length");
		if (player2.count > 0) DisplayText(", past " + player2.mf("his", "her") + " pussy,");
		DisplayText(" and towards the prize you've been wanting since you got here - " + player2.mf("his", "her") + " asshole.");
		...
		if (player2.analCapacity() > 70) DisplayText("  Thankfully, " + player2.mf("he", "she") + "'s nice and stretched from all the butt-fucking " + player2.mf("he", "she") + "'s had to take, and you easily pop inside.  It feels so good that your flare grows wider, pulling you deeper into that welcoming, tight anal heat.");
		else if (player2.analCapacity() > 50) DisplayText("  Thankfully, " + player2.mf("he", "she") + "'s got plenty of give in " + player2.mf("his", "her") + " back-door, and you pop inside after applying a little more pressure.  It feels so good that your flare grows wider, slowing your descent into that tight, anal heat.");
		else DisplayText("  Unfortunately, there's not much give in " + player2.mf("his", "her") + " back-door, and you have to sit there smearing pre over it, again and again, each time pushing a little harder.  After what feels like the fiftieth try, you finally pop your fat flare into " + player2.mf("his", "her") + " rectum, the tight anal heat causing you to flare from the force of " + player2.mf("his", "her") + " pressure.  Slowly, you push inch after throbbing inch of your swollen, stallion-like erection into " + player2.mf("his", "her") + " asshole.");
		if (player2.ass.looseness === 0) {
			DisplayText("  <b>You've taken " + player2.mf("his", "her") + " anal virginity!</b>");
			player2.ass.looseness = 1;
		}
		DisplayText("  " + player2.short + " writhes as " + player2.mf("he", "she") + " takes you, " + player2.mf("his", "her") + " hips lifting slightly off the ground to make sure you grind into " + player2.mf("his", "her") + " prostate.  A long strand of clear excitement oozes from " + player2.mf("his", "her") + " tip");
		if (player2.cockTotal() > 1) DisplayText("s");
		DisplayText(", thick, viscous, and clinging to the drying mess you left behind.");
		...
		if (player2.cockTotal() > 1) DisplayText("s");
		DisplayText(" between your bodies, " + player2.mf("his", "her") + " liquid love exploding all over your cock-locked forms.  Each time you cum, a fresh-squeezed spurt of semen splatters into your tits, " + player2.mf("his", "her") + " prostate violently being milked by the heady flows of vixen-spunk you force inside " + player2.mf("his", "her") + " rectum.");
		if (player2.count > 0) DisplayText("  You don't notice the female fluids that drip from her sodden slit - they soak into your big balls, already forgotten.");
		...
		if (player2.cumQ() >= 500) {
			DisplayText(", mixing with " + player2.mf("his", "her") + " own ");
			if (player2.cumQ() < 1000) DisplayText("puddly");
			else if (player2.cumQ() < 2500) DisplayText("small lake-sized");
			else DisplayText("massive, lake-sized");
			DisplayText(" mess - only now do you realize just how much your lover could produce!  The");
		}
		else DisplayText(", but the");
		DisplayText(" semen-inundated fabric quickly begins to dry as the parched, wasteland dirt reclaims the moisture.");
	...
	private putAPCDickInUrtaCoochies(truth: boolean): void {
		...
		if (player2.isTaur() || player2.isDrider()) {
			...
			if (!player2.isDrider()) DisplayText("quadrapedal");
		...
		//Else:
			...
			if (player2.count > 0) DisplayText(", her female jizz spattering unnoticed onto the ground beneath the pair of you");
			DisplayText(".");
			...
			if (player2.cumQ() >= 1000) DisplayText(" not to mention the weight of your overstuffed belly");
			DisplayText(" you don't really feel inclined to move, either.  Instead, you yawn loudly, make yourself comfortable upon your lover's stomach, and allow yourself to drift off to sleep - already, you can hear the quiet breathing of " + player2.short + " as " + player2.mf("he", "she") + " does the same thing.");
		}
		player.orgasm();
		player.stats.sens += -2;
		menu();
	...
	private putUrtasWangInPCsCunt(truth: boolean): void {
		...
		DisplayText("pulsing interior.  The sight is exciting in the extreme, immediately sending your heartbeat through the roof.  Your tremendous horse-cock is immediately stiffened by your ardor, along with your coal-black nipples, and your armor-plated skirt begins to rise, as if by magic.");
		...
		if (player2.pregnancyIncubation > 0 && player.pregnancyIncubation < 150) DisplayText("the curve of her pregnant belly");
		else if (player2.tone >= 65) DisplayText("her toned abdomen");
		else if (player2.tone < 30) DisplayText("her soft, forgiving middle");
		else DisplayText("her belly");
		...
		//{centaur}
		if (player2.isTaur()) {
			DisplayText("and climb off the 'taur-woman.  She smirks up at you, until you bid her to rise, telling her that you plan to mount her like the fertile filly she is. Reluctantly, " + player2.short + " clatters up onto her hooves.");
			if (player2.tailType > TailType.NONE) DisplayText("  You stroke her tail as you shift it aside");
			else DisplayText("  You stroke her swollen sex as you admire it");
			DisplayText(", making sure you've got the perfect view of her lube-slimed horse-cunt.  Unbidden, your cock thickens, disgorging a potent trail of animal-pre down its underside in anticipation.");
			if (player2.wetness() >= 4) DisplayText("  Fuck, she's wetter than Edryn!  Just look at those heavy trails of slick moisture as they run down her rump and hindlegs.");
			else if (player2.wetness() >= 2) DisplayText("  Fuck, she's as wet as Edryn!  Beads of moisture are forming on her clit, ready to drop to the ground.");
			else DisplayText("  Fuck, she's almost as wet as Edryn!  Just getting a sniff of her horny horse-snatch has your head swimming!");
			DisplayText("  You give it a lick and shudder, taking in a few deep lungfuls of her scent.");
			if (player2.tallness >= 84) DisplayText("  Before you know what you're doing, you've pulled her back to a rock and climbed atop it, bringing your cock level with that heavenly hole.");
			else DisplayText("  Before you know what you're doing, you've raised yourself up to your full height, bringing your cock level with that heavenly hole.");
		}
		//{goo}
		else if (player2.isGoo()) {
			DisplayText("and roll the gooey woman so she's face down.  She holds herself up off the dry dirt with her palms, sweet, slimy bottom upraised above her goopy undercarriage.  Amazingly, there's a slight parting in the mass of slime, right between where her legs would be, if she had them.  A narrow, barely visible divide can be spotted inside, capped with the telltale bulge of her ");
			if (player2.clitLength < .5) DisplayText("tiny ");
			else if (player2.clitLength >= 2.5) DisplayText("massive ");
		...
		//{nagas}
			...
			DisplayText("and roll the snake-woman over so she's prostrate on the ground, barely holding her front half up with her arms.  Her loins are flat on the ground, undulating slightly as she grinds her moist clit against it for sensation.  Your lover's anatomy poses a problem for your choice of positions, but then, inspiration strikes.  You come up under the hind part of her snake-tail, and hoisting it up, you support half over your shoulder.  As expected, she has all the flexibility of the serpent she so resembles, and her tail bends down just far enough for her cunt to hang a mere inch above your rigid tool, right where her body changes from snake to person.");
			if (player2.wetness() >= 3) DisplayText("  Her moisture drizzles from that wanton slit, basting you with her fragrant femininity, and you have a hard time not taking her here and now.");
			DisplayText("  The tail you're holding up coils in, sliding around your arm and middle, wrapping you up in snakey snugness.  With impeccable control, she squeezes a little bit, bringing her cunt close enough to breathe foggy moisture on your equine tool.");
		}
		//{biped}
		else {
			DisplayText("and roll her over on her hands and knees, lifting that sweet butt up in the air where you can admire it.  You kneel behind her and, pressing your nose into her fragrant quim, you extend your tongue, letting the smooth oral muscle taste her juices and tease at her engorging clit.  " + player2.short + " moans and presses back into you, crying out like a true bitch in heat, one in need of a thorough rutting.  The thought hits your cock like a lightning bolt, and it fills out to painful proportions, every vein and bulge standing out in stark relief, thumping obscenely to the tune of your insatiable lust.  You moan into her lovely-tasting snatch, gulping down ");
			if (player2.wetness() < 3) DisplayText("every drop of nectar that she gives up");
			else if (player2.wetness() < 4) DisplayText("the copious drops of nectar she gives up");
			else if (player2.wetness() < 5) DisplayText("the drizzling moisture that oozes from her twat");
			else DisplayText("the rivers of girl-lube that run unimpeded from her sopping twat");
			DisplayText(".  You sniff in great lungfuls of her, your head swimming with need, and before you know it, you've risen behind her, your oozing flare just inches from that delectable opening.");
		}
		...
		if (player2.vaginalCapacity() >= 200) DisplayText(", though you know she can handle cocks with Edryn's ease");
		...
		if (player2.pregnancyIncubation > 0 && player2.pregnancyIncubation < 150) DisplayText(" (more than it already is)");
		DisplayText(" from your prodigious deposit.  The illusion of knocking her up takes hold of you, and your orgasm redoubles, fresh waves of horse-cum pouring out of you unceasingly.  " + player2.short + " moans, rubbing her taut");
		if (player2.pregnancyIncubation > 0 && player2.pregnancyIncubation < 150) DisplayText(" pregnant");
		DisplayText(" dome, until at last you finally spend the last of your spunk.");
	...
	private snuggleWithUrta(truth: boolean): void {
		...
		if (player2.isTaur() || player2.isDrider()) {
			DisplayText("  It's rather awkward, thanks to " + player2.mf("his", "her") + " clunky ");
			if (player2.isTaur() DisplayText("centaur");
			else DisplayText("drider");
			DisplayText(" body, but you manage to press up against " + player2.mf("his", "her") + " ");
			if (player2.isTaur() DisplayText("forehoofs");
			else DisplayText("first two pairs of spidery limbs");
			DisplayText(".");
		}
		//[if PC Naga:]
		else if (player2.isNaga()) DisplayText("  " + player2.mf("His", "Her") + " serpentine coils loop snugly around your waist, curling around your legs and holding you gently in place.");
		//[if PC Goo:
		else if (player.torso.hips.legs.isGoo()) DisplayText("  " + player2.mf("His", "Her") + " soft, squishy goo-body eagerly accepts you, the gelatinous mass of " + player2.mf("his", "her") + " lower torso reaching out to softly envelop you in its cool, wet embrace.");
		//[Else:
		else DisplayText("  Knees to knees, it takes you a few moments to figure out how to properly position your legs, but you eventually manage it.");
		...
		if (player2.cor < 33) DisplayText("playful pat");
		else if (player2.cor < 66) DisplayText("gentle stroke");
		else DisplayText("lascivious caress");
		DisplayText(" before pulling you tighter into " + player2.mf("his", "her") + " embrace.  Eager to forget your embarrassment, you focus on holding " + player2.mf("him", "her") + " tight and close.  Your prick slowly slides back into its sheath from the lack of stimulus as you focus on how safe and content you feel to be here, being held like this.");
	...
	private morningAfterCampVisitEmbark(truth: boolean): void {
		...
		model.time.days++;
		statScreenRefresh();
		DisplayText("Sadly, morning comes, and your time with " + player2.short + " comes to an end.  You push " + player2.mf("his", "her") + " shoulder gently to rouse " + player2.mf("him", "her") + ", smiling at " + player2.mf("him", "her") + " when " + player2.mf("he", "she") + " wakes.  You kiss " + player2.mf("him", "her") + " on the lips before rolling out from under the blanket.  The two of you eat a quiet breakfast together, simply enjoying each other's company.  ");

		if (truth) {
			DisplayText("" + player2.mf("He", "She") + " asks again about coming with you, but you tell " + player2.mf("him", "her") + " Gul's instructions were quite specific.  You'll need to tackle this alone.  The rest of breakfast passes in uncomfortable silence, broken when you rise to leave.");
		}

		DisplayText("\n\nYou embrace " + player2.short + ", kissing " + player2.mf("him", "her") + " on the lips");

		if (truth) {
			DisplayText(" before you promise, \"<i> I'll come back to you, stronger and ready for kids, I promise.</i>\"\n\n");
		}
		else {
			DisplayText(".  ");
		}
	...
	public winFuckAGoblinBroodmotherAsUrta(): void {
		...
		else DisplayText("feverishly masturbating");
		DisplayText(" form of your opponent and can't resist licking your black lips in anticipation - any guilt about fucking someone besides " + player2.short + " that might make it through the haze of lust consuming you is quickly extinguished by your conviction that " + player2.mf("he", "she") + " would understand.  Your growing tower of fuck-meat is already lifting the hem of your armored skirt, making it even easier to grab its lower edge and flip it up out of the way as you advance towards your assailant, eager to see what you can do about blowing off some steam...");
	...
	private urtaSecondWind(): void {
		...
		monster.statusAffects.add(StatusAffectType.UrtaSecondWinded, 0, 0, 0, 0);
		HPChange(Math.round(player.maxHP() / 2), false);
		fatigue(-50);
		player.stats.lust += -50;
		DisplayText("Closing your eyes for a moment, you focus all of your willpower on pushing yourself to your absolute limits, forcing your lusts down and drawing on reserves of energy you didn't know you had!\n\n");
		kGAMECLASS.enemyAI();
	}
	...
	private urtaComboAttack(): void {
		...
		if (!player.statusAffects.has(StatusAffectType.Attacks))
			player.statusAffects.add(StatusAffectType.Attacks, 3, 0, 0, 0);
	else {
			player.addStatusValue(StatusAffects.Attacks, 1, -1);
			trace("DECREMENDED ATTACKS");
			if (player.statusAffects.get(StatusAffectType.Attacks).value1 <= 1) {
		...
		let damage: number;
		//Determine if dodged!
		if (monster.findStatusAffect(StatusAffects.Blind) < 0 && (randInt(3) === 0 || (player.statusAffects.has(StatusAffectType.Blind) && randInt(2) === 0) || (monster.spe - player.stats.spe > 0 && int(Math.random() * (((monster.spe - player.stats.spe) / 4) + 80)) > 80))) {
			if (monster.spe - player.stats.spe < 8) DisplayText(monster.capitalA + monster.short + " narrowly avoids your attack!");
			if (monster.spe - player.stats.spe >= 8 && monster.spe - player.stats.spe < 20) DisplayText(monster.capitalA + monster.short + " dodges your attack with superior quickness!");
			if (monster.spe - player.stats.spe >= 20) DisplayText(monster.capitalA + monster.short + " deftly avoids your slow attack.");
			DisplayText("\n");
			if (player.statusAffects.has(StatusAffectType.Attacks)) {
				urtaComboAttack();
				return;
		...
		if (randInt(100) <= 4 || (player.perks.has(PerkType.Tactician) && player.stats.int >= 50 && (player.stats.int - 50) / 5 > randInt(100))) {
			crit = true;
			damage *= 2;
		}
		//Start figuring enemy damage resistance
		let reduction: number = randInt(monster.tou);
		...
		if (damage <= 0) {
			damage = 0;
			DisplayText("Your attacks are deflected or blocked by " + monster.a + monster.short + ".");
		}
		else {
			DisplayText("You hit " + monster.a + monster.short + "! (" + damage + ")");
			if (crit) DisplayText(" <b>*CRIT*</b>");
		}
		if (player.perks.has(PerkType.BrutalBlows) && player.stats.str > 75) {
			if (monster.armorDef > 0) DisplayText("\nYour hits are so brutal that you damage " + monster.a + monster.short + "'s defenses!");
			if (monster.armorDef - 10 > 0) monster.armorDef -= 10;
			else monster.armorDef = 0;
		}
		DisplayText("\n");
		//Kick back to main if no damage occured!
		if (monster.stats.HP >= 1 && monster.lust <= 99) {
			if (player.statusAffects.has(StatusAffectType.Attacks)) {
				trace("MORE ATTACK");
				urtaComboAttack();
				return;
			...
			if (monster.stats.HP <= 0) return { next: kGAMECLASS.endHpVictory };
			else return { next: kGAMECLASS.endLustVictory };
		}
	}
	...
	private urtaDirtKick(): void {
		...
		//Blind
		if (player.statusAffects.has(StatusAffectType.Blind)) {
			DisplayText("You attempt to dirt kick, but as blinded as you are right now, you doubt you'll have much luck!  ");
		}
		else DisplayText("Spinning about, you drag your footpaw through the dirt, kicking a wave of debris towards " + monster.a + monster.short + "!  ");
		//Dodged!
		if (randInt(20) + 1 + monster.spe / 20 > 15 + player.stats.spe / 20) {
			DisplayText(monster.mf("He", "She") + " manages to shield " + monster.mf("his", "her") + " eyes.  Damn!\n\n");
			kGAMECLASS.enemyAI();
			return;
		}
		else if (monster.statusAffects.has(StatusAffectType.Blind)) {
			DisplayText(monster.mf("He", "She") + "'s already blinded.  What a waste.\n\n");
		}
		else {
			DisplayText(monster.mf("He", "She") + "'s blinded!\n\n");
			monster.statusAffects.set(new StatusAffect("Blind", 2 + randInt(3))), 0, 0, 0);
		}
		kGAMECLASS.enemyAI();
	}
	...
	private urtaSidewinder(): void {
		...
		//Blind
		if (player.statusAffects.has(StatusAffectType.Blind)) {
			DisplayText("You attempt to hit with a vicious blow to the side, but as blinded as you are right now, you doubt you'll have much luck!  ");
		}
		else DisplayText("You make a wide swing to the side, hoping to stun your foe!  ");
		let damage: number;
		//Determine if dodged!
		if ((player.statusAffects.has(StatusAffectType.Blind) && randInt(2) === 0) || (monster.spe - player.stats.spe > 0 && int(Math.random() * (((monster.spe - player.stats.spe) / 4) + 80)) > 80)) {
			if (monster.spe - player.stats.spe < 8) DisplayText(monster.capitalA + monster.short + " narrowly avoids your attack!");
			if (monster.spe - player.stats.spe >= 8 && monster.spe - player.stats.spe < 20) DisplayText(monster.capitalA + monster.short + " dodges your attack with superior quickness!");
			if (monster.spe - player.stats.spe >= 20) DisplayText(monster.capitalA + monster.short + " deftly avoids your slow attack.");
			DisplayText("\n\n");
		...
		if (monster.findStatusAffect(StatusAffects.Stunned) < 0 && monster.findPerk(PerkLib.Resolute) < 0 && damage > 0) {
			if (monster.tou / 10 + randInt(20) + 1 < 20) {
	...
	private urtaVaultAttack(): void {
		...
		if (player.statusAffects.has(StatusAffectType.Sealed) && player.statusAffects.get(StatusAffectType.Sealed).value2 === 0) {
			DisplayText("You attempt to attack, but at the last moment your body wrenches away, preventing you from even coming close to landing a blow!  The seals have made normal attack impossible!  Maybe you could try something else?\n\n");
			kGAMECLASS.enemyAI();
			return;
		}
		//Blind
		if (player.statusAffects.has(StatusAffectType.Blind)) {
			DisplayText("You attempt to make a high, vaulting attack, but as blinded as you are right now, you doubt you'll have much luck!  ");
		}
		else DisplayText("You leap into the air, intent on slamming your " + player.weaponName + " into your foe!  ");
		let damage: number;
		//Determine if dodged!
		if ((player.statusAffects.has(StatusAffectType.Blind) && randInt(2) === 0) || (monster.spe - player.stats.spe > 0 && int(Math.random() * (((monster.spe - player.stats.spe) / 4) + 80)) > 80)) {
			if (monster.spe - player.stats.spe < 8) DisplayText(monster.capitalA + monster.short + " narrowly avoids your attack!");
		...
		//Determine if critical hit!
		let crit: boolean = false;
		if (monster.statusAffects.has(StatusAffectType.Stunned) || randInt(100) <= 4 || (player.perks.has(PerkType.Tactician) && player.stats.int >= 50 && (player.stats.int - 50) / 5 > randInt(100))) {
			crit = true;
			damage *= 2;
		}
		//Start figuring enemy damage resistance
	...
	public urtaBeatsUpSiriusRadio(): void {
		...
		//Beaten via HP:
		else DisplayText("Your latest blow lands with a crunch on his jaw and he collapses bodily onto the ground at your feet, clearly out cold.");
	...
	public urtaLosesToSirriusSnakeRadio(): void {
		...
		//Loss by HP:
		if (player.stats.HP < 1) DisplayText("You cry out in pain as the bruises and buffets you've taken overwhelm you.  You struggle to stay upright, but to no avail; your injured body has reached its limits and you collapse, helpless and at the mercy of your assailant.");
		//Loss by Lust:
		else DisplayText("With a hollow moan of anguish, you cast aside your halberd and fall to your knees, tears stinging your eyes as you desperately jerk and maul at your huge horse-prick.  The cursed thing has betrayed you again, and now you're too addled by lust and need to defend yourself from whatever this naga has in store for you.");
	...
	private gnollAlphaBitchIntro(): void {
		...
		monster.stats.HP = monster.eMaxHP(); // TODO extract to AlphaGnoll class
	...
	public winRapeHyenaPrincess(): void {
		...
		if (monster.stats.HP < 1) DisplayText(".  In spite of her wounded state, she seems to be quite excited by the situation");
		DisplayText(".");
	...
	public useSuccubiMilkOnGnollPrincesses(): void {
		...
		if (Flags.list[FlagEnum.URTA_FUCKED_HEL] === 1) DisplayText("that salamander slut's cunt");
		else DisplayText("a mino-cum addicted whore's sodden cunt");
		DisplayText(".  As you start to pick up the pace, bucking your hips into the thick cushion of her rump, the gnoll princess starts mewling pathetically, panting and moaning like a little slut.");
	...
	public loseToGnollPrincessAndGetGangBanged(): void {
		...
		if (player.stats.lust > 99) {
			DisplayText("Sinking to your knees, utterly overwhelmed by lust, you gaze up the clit-cocked amazon, licking your chops. She responds by disdainfully pushing you down with one of her soft, padded paws.  \"<i>Did you really think you could trespass in my mother's territory and go unpunished?</i>\"");
		...
		//Lose Via HP✓
			...
			DisplayText("Flopping down to the ground, you struggle to rise.  You're badly hurt, and your muscles barely move at this point.  With a pain-wracked spasm, you roll onto your back, unable to even rise to your knees.  The gnoll smirks and proclaims, \"<i>You'll probably make a good slave once we've broken you in.</i>\"");
	...
	private loseToGnollAsUrtaEpilogue(): void {
		...
		//Loss Epilogue✓
		//[HP]
		if (player.stats.HP < 1) DisplayText("Ouch... what happened to you?  You blink a few times, ignoring the buzzing throb of pain that rises up like a dormant monster in the back of your head.");
		//[Lust]
		else DisplayText("Oh, gods... what happened to you?  You blink a few times, ignoring the gentle throb coming from your well-used member.");
		DisplayText("  There's a constant, loud thrum of many voices talking all at once that makes it hard to think, and when you finally come to focus in the dim lighting, you can see why.  There are dozens, if not hundreds, of gnolls here, assembled in the faint light of some kind of feast hall, most of them male.");
		...
		if (player.stats.HP < 1) DisplayText("I hope I wasn't too rough on you");
		else DisplayText("I hope you enjoyed the journey here");
		DisplayText(".</i>\"");
		...
		DisplayText("\"<i>Good girl,</i>\" she says encouragingly.  ");
		if (player.stats.HP >= 1) DisplayText("You shudder at the memory of your meeting, leaking a little more lubricant.  ");
	...
	public urtaSubmitsToMinotaurBadEnd(): void {
		...
		DisplayText("  Your once foe removes his loincloth to fully expose the mammoth between his legs, three feet long and nearly twice as girthy as your own.  You're so dazed by his imposing manhood, that you totally miss him flinging his loincloth at you.  It smacks wetly into your face, smothering you in his syrupy spooge.");
	...
	public urtaLosesToMinotaurRoughVersion(): void {
		...
		else DisplayText("weak from lust");
		DisplayText(" to continue fighting.  The hulking bull-man snorts as he approaches you, ");
		if (monster.lust < 70) DisplayText("barely deigning to give your far smaller cock a contemptuous look, ");
	...
	public milkyUrtaTic(): void {
		...
		else DisplayText("Those orbs full of milky goodness are starting to look more attractive by the minute... if you don't finish this quickly you might end up giving up!");
		DisplayText("\n\n");
		player.addStatusValue(StatusAffects.MilkyUrta, 1, -1);
		if (player.statusAffects.get(StatusAffectType.MilkyUrta).value1 <= 0) player.statusAffects.remove("MilkyUrta");
		player.stats.lust += 10;
	}
	...
	public urtaBeatsUpCowcubi(): void {
		...
		if (monster.stats.HP < 1) {
			DisplayText("The succubus trips and falls on her face, dropping her whip at your feet.  You quickly take her whip and use it to bind her hands behind her back.");
		...
		//Lust:
			...
			DisplayText("The succubus drops her whip and begins masturbating furiously, jilling her dripping cunt with abandon.  You quickly run towards her, planting your foot on her back and sending her tumbling on her face, then you use her whip to bind her hands behind her back.");
	...
	public urtaLosesToCowCubi(): void {
		...
		else DisplayText("you start pulling at your [armor], struggling with every strap and piece of restraining, constricting leather.  Your lust-clouded mind can barely focus to work your fumbling fingers against the snaps and straps.  In your haste, with balancing forgotten, you tumble onto your face, smearing your dick exquisitely between your fallen form and a patch of soft moss.  You mewl pitifully in protest.  You need to fuck!");
		...
		if (player.stats.HP < player.maxHP()) DisplayText(" and healing your wounds");
		DisplayText(".  Your own nipples have long since become fully engorged, taut black tit-caps, pebbly and sensitive.  They drag up and down the fat bottoms of the demoness's swollen underbust with every swallow and gasping breath you take.");
	...
	private urtaArrivesAtCampForFukks(): void {
		...
		//if genderless:
		if (player.gender === Gender.NONE) DisplayText("seeing as how you can't make babies");
		else DisplayText("if you'd rather not");
		DisplayText(", I can always go and use Edryn...</i>\"");
		...
		if (player.gender > 0) DisplayText(" fuck the horny fox and help her out of her predicament, or");
		DisplayText(" send her to Tel'Adre to find a suitable partner, like Edryn?");
		/*if(player.torso.cocks.count > 0) {
	...
	private knockUpUrtaWithGodChild(): void {
		...
		if (player.cumQ() > 1000) DisplayText(", belly rounded with wobbly, womb-packing spunk");
		DisplayText(".");
	...
	private postFuckUrtaUp(): void {
		...
		if (player.statusAffects.has(StatusAffectType.DungeonShutDown)) DisplayText(" - besides, given you know that you're nothing but a sacrifice the demons are too lazy to collect, it's not like they'll really send an invasion through");
		DisplayText(".  Urta's joyous expression makes it quite clear that you chose the right choice.  The two of you gather your things, dress Urta in her clothes as best you can (adding a blanket for extra protection and modesty), ");
		if (camp.followersCount() >= 2) DisplayText("leave instructions to your fellow camp-dwellers to look after the place, ");
	...
	private preggedUrtaWithGodChildEpilogue(): void {
		...
		if (player.stats.cor > 66) DisplayText(", what do you care, anyway?");
		else DisplayText(".");
	...
	private getKnockedUpWithGodChild(): void {
		...
		if (player.torso.balls.quantity > 0) DisplayText(" and lifting your balls");
		DisplayText(", exposing your puffy lips, and spreading them wide to expose your ");
		if (player.torso.vaginas.get(0).vaginaType != 5) DisplayText("pink, ");
		else DisplayText("dusky, ");
		...
		if (player.torso.cocks.count > 0) {
			DisplayText(", your own forgotten length");
			if (player.torso.cocks.count > 1) DisplayText("s");
			DisplayText(" laying on your stomach, bouncing fitfully but ultimately ignored.");
		}
		else DisplayText(", the slippery wetness of your [vagina] and the sheer hardness of your [clit] overwhelming.");
		DisplayText("  This feels better than it really ought to, perhaps in part thanks to your partner's copious fluids and supernaturally warm, feverish dick.");
		...
		if (player.torso.balls.quantity > 0) DisplayText("both sets of");
		else DisplayText("her");
		DisplayText(" balls, slathering them with slick spunk.");
		...
		let choice: number;
		if (ballWorship.length > 0) choice = ballWorship[randInt(ballWorship.length)];
		else choice = -1;
		...
		//{If Jojo/Amily pet}
			...
			if (jojoScene.campCorruptJojo()) DisplayText("Jojo");
			else DisplayText("Amily");
		...
		//{Bimbo Sophie ball worship here}
			...
			if (player.torso.balls.quantity > 0) DisplayText("  She does the same to your own [balls], even giving your [asshole] a happy, sloppy kiss before returning to polishing the supple, skin-shrouded spheres just ahead.");
			DisplayText("  Helia seems thrilled to be egging you both on to new heights of pleasure, and when Urta turns back to you with a blissful smile on her face, you realize she's enjoying it too.");
		}
		//{Latexy}
		else if (choice === 4) {
		...
		if (player.torso.tailType > TailType.NONE) DisplayText(", though getting your tail angled right to allow her entrance is a bit of a challenge");
		...
		else if (player.torso.vaginas.get(0).wetness >= 3) DisplayText("Lady-spunk drools out of your spasming twat in thick streams, glazing Urta's bouncy ballsack with your liquid lust.");
		else if (player.torso.vaginas.get(0).wetness >= 1) DisplayText("Lady-spunk drips from your spasming twat in steady trickles, glazing Urta's bouncy ballsack with your liquid lust.");
		DisplayText("  The hungry mouth between her legs wastes no time in cleaning them.");
		DisplayText("  Again and again, muscular contractions wiggle through your canal, wringing Urta's swollen staff of impregnation, milking it for more, even though your womb is STILL slightly distended from her last deposit.");
	...
	private getKnockedUpUrtaEpilogue(): void {
		...
		if (player.pregnancyIncubation > 0 && player.pregnancyIncubation < 150) DisplayText("than before");
		else DisplayText("- very pregnant");
	...
	private getKnockedUpByUrtaEpilogueII(): void {
		...
		if (player.pregnancyIncubation > 0 && player.pregnancyIncubation < 150) DisplayText("  Somehow, you're still just as pregnant with something as you were before.");
		DisplayText("  You're filled with vigor at the touch, the stress of the birth fading immediately.  Even your poor, tortured vagina tingles, the pain fading.  You can feel the pregnant gape receding, as if the birth never happened.");
	...
	private urtaAndEdrynGodChildEpilogueII(): void {
		...
		DisplayText("horse-girl.");
		if (kGAMECLASS.telAdre.edryn.pregnancy.isPregnant) DisplayText("  Just how did she get pregnant again when she was already knocked up?  ...Magic, maybe.");
		DisplayText("  Urta leads you around a bend, squeezing your hand for comfort.  \"<i>Thank you for everything, " + player.short + ", most of all being so understanding.</i>\"  The gray-furred fox pushes past the sixth pair of guards you've seen since entering the tower, just inside to see Edryn splayed on the floor, groaning and heaving, her ");
		if (kGAMECLASS.telAdre.edryn.pregnancy.isPregnant) DisplayText("massively ");
		DisplayText("pregnant flanks rippling with muscular contractions.   Something is bulging against her nether-lips, stretching the massive horse-cunt wider and wider with each passing moment.");
		...
		if (kGAMECLASS.telAdre.edryn.pregnancy.isPregnant) DisplayText("  The centaur guardswoman is still just as pregnant with your offspring as before.  Your child is unharmed by whatever just transpired.");
		DisplayText("  Edryn's eyes immediately close, sinking into a deep, restful sleep.");
