...
export class BrigidScene {
	...
	public pcDefeatsBrigid(): void {
		...
		else DisplayText("too badly beaten");
	...
	public pcDefeatedByBrigid(): void {
		...
		if (Flags.list[FlagEnum.HEL_HARPY_QUEEN_DEFEATED] === 0) {
			...
			DisplayText("\"<i>That's it?!</i>\" Brigid screams, as you collapse in front of her.  \"<i>A weak little piece of trash like you took out our queen?</i>\"");
			...
			return { next: brigitPostQueenDeathBadEndII };
		}
	}
	...
	private brigitPostQueenDeathBadEndII(): void {
		...
		if (player.gender <= 1 || player.pregnancyIncubation > 0) {
			...
			if (player.torso.cocks.count <= 0) {
				...
				if (player.pregnancyIncubation > 0) DisplayText("  Once this disgusting thing you're carrying falls out of you, we'll use you more traditionally, but...</i>\"  She pauses to press uncomfortably on your pregnant belly, then resumes speaking.  \"<i>I have no intention of letting you sit idle for however long that takes.");
				DisplayText("  For now, I've given you the most obvious modification - I can think of something else later.</i>\"");
