...
this.baseStats.int = 100;
	...
	override protected performCombatAction(): void {
		...
		if (statusAffects.has(StatusAffectType.TentagrappleCooldown)) {
			statusAffects.get(StatusAffectType.TentagrappleCooldown).value1 = -1;
			if (statusAffects.get(StatusAffectType.TentagrappleCooldown).value1 <= 0) {
				statusAffects.remove("TentagrappleCooldown");
			}
		}

		if (player.statusAffects.has(StatusAffectType.ShowerDotEffect)) {
			showerDotEffect();
		...
		else if (!player.statusAffects.has(StatusAffectType.GardenerSapSpeed) && this.statusAffects.has(StatusAffectType.VineHealUsed)) {
			sapSpeed();
		}
		else {
	...
	private vineHeal(): void {
		...
		if (findStatusAffect(StatusAffects.Stunned) < 0) {
			...
			if (this.lustVuln <= 0.15) {
				DisplayText(" <b>You aren't sure, but she seems to be leering at you a little more openly.</b>");
			}
			else if (this.lustVuln <= 0.45) {
				DisplayText(" <b>She's definitely leering at you now.</b>");
			}
			else {
				DisplayText(" <b>There's no disguising the lecherous look on her face while she eye-humps you.</b>");
			}

			if (this.lustVuln <= 0.3) DisplayText(" Whatever is in that healing nectar must be weakening her self-control.");
		}
		else {
			...
			if (this.lustVuln <= 0.15) {
				DisplayText(" are slightly glassy.");
			}
			else if (this.lustVuln <= 0.45) {
				DisplayText(" are even glassier.");
			}
			else {
				DisplayText(" are dilated and a little slow.");
			}
	...
	private tentagrapple(): void {
		...
		DisplayText("A web of interwoven vines lashes out from behind the succubus, somehow leaving her untouched by the wave of advancing greenery. They're trying to grab you!");

		if (combatMiss() || combatEvade() || combatFlexibility() || combatMisdirect()) {
			//Dodge
			DisplayText(" You slip aside at the last moment, barely avoiding being wrapped in the squirming mass. It snaps back, perhaps at the limits of its reach, leaving you once more eye to eye with the alluring gardener.");
		}
		else {
			...
			player.statusAffects.add(StatusAffectType.Tentagrappled, 0, 0, 0, 0);
		}
	}
	...
	public grappleStruggle(): void {
		...
		if (randInt(player.stats.str) > this.str / (1 + (numRounds / 2))) {
			DisplayText("You scrabble desperately against the tentacles enveloping your body, pulling against the cast-iron grip around your limbs. You tug against them again and again, and with one final mighty heave, you slip free of their grasp!");
			...
			DisplayText("You scrabble desperately against the tentacles enveloping your body, pulling against the cast-iron grip around your limbs. You tug against them again and again");
			...
			player.statusAffects.get(StatusAffectType.Tentagrappled).value1 = 1;
			player.takeDamage(75 + randInt(15));
			game.dynStats("lus+", 3 + randInt(3));
	...
	private sicem(): void {
		...
		for (let i: number = 0; i < 10; i++) {
			if (!(combatMiss() || combatEvade() || combatFlexibility() || combatMisdirect())) {
				damage += 2 + randInt(1 + player.stats.lib / 20) + randInt(1 + player.stats.sens / 20);
			}
		}
		...
		if (damage >= 0) {
			...
			DisplayText(" The sinuous plant-based tentacles lash at you like a dozen tiny whips! Preparing for stinging pain, you're somewhat taken aback when they pull back at the last moment, sensually caressing your most sensitive places! (" + sL + ")");
		}
		else {
			DisplayText(" The sinuous plant-based tentacles strike at you like a dozen tiny whips, but, somehow, you manage to avoid every single one of their lashes!");
		}
	}
	...
	private corruptiveShower(): void {
		...
		if (combatMiss() || combatEvade() || combatFlexibility() || combatMisdirect()) {
			//Dodge
			DisplayText(" Somehow, you manage to twist out from under the organic raincloud without getting stained by a single drop, though your breath has quickened, and not just from the physical effort.");
		}
		else {
			//Fail
			if (!player.statusAffects.has(StatusAffectType.ShowerDotEffect))
				player.statusAffects.add(StatusAffectType.ShowerDotEffect, 3, 0, 0, 0);
				else
	player.statusAffects.get(StatusAffectType.ShowerDotEffect).value1 = 3;
	...
	{
		game.dynStats("lus+", 2 + randInt(2));
			
		player.statusAffects.get(StatusAffectType.ShowerDotEffect).value1 = -1;
			
		//Dot effect
		if(player.stats.lust < 50) DisplayText("The tentacles' sex-juices are still covering you - still slowly arousing you. You've got a good handle on it for now.\n\n");
		else if (player.stats.lust < 60) 
		{
			DisplayText("You try to wipe some of the fragrant seed from your palm, but all you succeed in doing is smearing it into your [hips].");
			if (player.stats.cor < 50) DisplayText(" You're ashamed to admit");
			else DisplayText(" You're a little irritated to admit");
			DisplayText(" that it's starting to feel really good.\n\n");
		}
			else if (player.stats.lust < 70)
		{
			DisplayText("You groan at the warm slipperiness enveloping your [skinFurScales] as the tainted tentacles' fluids go to work on you. There's nothing you can do but try to endure it. If only it didn't feel so... hot to be drenched in. If you wind up losing, you hope she'll do this again....\n\n");
		}
		...
		{
			DisplayText("Ohhhh, you're close now. You can feel the need hammering inside of you, soaking in through your [skinFurScales] to stoke the fires between your [legs] into a blazing inferno, one you couldn't resist even if you wanted to. Then... then you'll be free to cum. You shake your head. Gotta hold it together");
			if (player.torso.cocks.count > 0) {
				DisplayText(", even while your rigid cock" + ((player.torso.cocks.count > 1) ? "s are" : " is") + " drizzling ropes of pre unimpeded.");
			}
			else if (player.torso.vaginas.count > 0) {
				DisplayText(" soaked twat threatens to " + ((player.torso.vaginas.get(0).wetness === 4) ? "drown" : "further soak") + " your [legs].");
			}
			else DisplayText(".");
		}
			else
			...
			if (player.statusAffects.get(StatusAffectType.ShowerDotEffect).value1 < 0) {
			player.statusAffects.remove("ShowerDotEffect");
			DisplayText("<b>The lust-inducing effects of the plant-spunk feel like they've dissipated...</b>\n\n");
		}
	}
	...
	{
		DisplayText("<i>“Oh fuck it,”</i> the demoness growls, stalking forward. <i>“We both need this, don't we, pet?”</i> She slips inside your guard, pressing her pendulous melons against your face");
			
		if(player.tallness <= this.tallness - 6) DisplayText(", somehow short enough in spite of the height differences.");
		else if (player.tallness >= this.tallness + 6) DisplayText(", somehow tall enough in spite of the height differences.");
			
		DisplayText(" They're so soft and pillowy that you can't help but enjoy the feel of them on your skin, and you take a deep, contented breath before remembering where you are and struggling out of the creamy valley.");
		...
		private sapSpeed():void
	{
		...
		player.statusAffects.set(new StatusAffect("GardenerSapSpeed", speedSapped, 0, 0, 0)));
		...
		private lustAuraCast():void
	{
		...
		if(this.statusAffects.has(StatusAffectType.LustAura)) {
		...
		private tasteTheEcstasy():void
	{
		...
		//Fail strength check!
		if(randInt(player.stats.str - 30) + 30 > this.str) {
