...
export class HermCentaurScenes {
	...
	public beatThePony(hpVictory: boolean) {
		DisplayText().clear();
		DisplayText("With a heavy 'thud', the demonic red-head slumps onto her side in the grass.  Her tail flails fitfully")
		if (hpVictory) DisplayText(" as she tries to rise");
		else DisplayText(" as she tries to stretch for her pulsating, cum-dripping member");
		DisplayText(", but it's a futile gesture.  She doesn't have the power to face you on her own terms anymore, and she watches you with a");
		if (hpVictory) DisplayText(" resigned glare");
		else DisplayText(" lustful glare");
		DisplayText(" once she realizes she's utterly spent.");
		...
		if (player.torso.cocks.count > 0) MainScreen.addButton(0, "Fuck Her", maleFuckHer);
		if (player.torso.vaginas.count > 0) MainScreen.addButton(1, "Get Fucked", femFuckHer);
	...
	private killHer() {
		DisplayText().clear();
		DisplayText("Sighing, you advance on the downed demon, trying to ignore the horrified look in her eyes as you end her.");
	...
	private letHerGo() {
		DisplayText().clear();
		DisplayText("You dust off your [armor] and wave nonchalantly at the equine demoness.  She slowly staggers up, watching you warily.  Dismissively, you tell her to leave before she earns your ire - her queen will be dealt with soon enough.  She clops off in a huff towards the exit.");
	...
	private maleFuckHer() {
		...
		if (y < 0) y = player.torso.cocks.sort(Cock.SmallestCockArea)[0];
		...
		if (player.cumQ() >= 1000) DisplayText(" You quake at the sensation of dumping so much of your creamy batter into her stomach, nearly closing your eyes in bliss.  Luckily, you hold onto your wits.");
		if (player.cumQ() >= 5000) DisplayText(" It doesn't make much difference to the demon beneath you, as her belly slowly rounds from the obscene deposit you're plugging her with.");
		if (player.cumQ() >= 15000) DisplayText(" Bigger and bigger, she bloats until she looks fat or pregnant, jiggling with your thick spooge.");
		...
		if (player.cumQ() >= 15000) DisplayText(", practically dragging her gut on the stones as she runs");
		DisplayText(".  Well, she might have gotten away, but she won't be able to stop you from getting to Lethice.  Indeed, it seems she was running for the exit.");
	...
	private femFuckHer() {
		...
		if (User.settings.silly()) MainScreen.addButton(0, "HORZ GOG", femFuckHerII, HORZGOG);
	...
	private femFuckHerII(cockType: number) {
		DisplayText().clear();
		//[Tentacle]
		if (cockType === TENTACOCK) {
			DisplayText("You give her throbbing maleness an affection pat and proclaim, \"<i>Make it a tentacle, one that will fit me properly.  Then we can fuck.</i>\"");
			...
			if (player.torso.clit.length >= 6) DisplayText(" A coil of cock loops around your gigantic buzzer, effectively frotting it like another cock and chance at protest from your lips.");
			else DisplayText(" A bend in the cock grinds against your buzzer with particular intensity, effectively robbing any chance at protest from your lips.");
			...
			Mod.Butt.displayStretchButt(player, player.analCapacity(), true, true, false);
			...
			if (player.torso.clit.length >= 6) DisplayText(" - you swear it spends as much time caressing your [clit] as squeezing it off");
			DisplayText(".  You buck your hips, dragging the butt-piercing wang along for the ride as you start to fuck in earnest.");
			...
			DisplayText("You give her throbbing maleness an affection pat and proclaim, \"<i>Make it a dog-cock, one that fit to knot properly.  Then we can fuck.</i>\"");
			...
			DisplayText("You give her throbbing maleness an affection pat and proclaim, \"<i>Keep it equine, the flare looks delicious.  Just make sure that it will fit, and then we can fuck.</i>\"");
			...
			if (player.torso.clit.length >= 6) DisplayText(" The hard, feminine pole twitches nicely at your touch, a far more sensitive rod than any cock.");
			DisplayText(" Your fingers sink into your gash with ease and set to work with ease.  Of course, they're soaked in a second.");
			...
			if (player.torso.vaginas.get(0).wetness < 4) DisplayText(" practically");
			...
			DisplayText("\n\nYou nervelessly slide off the pole into the ground, earning a fresh bukkake of creamy cum to plaster your " + Desc.Skin.skinFurScales(player) + ".  While you recover, your demonic lover's bulk shifts away, and you soon here the telltale clopping of her moving away.  It sounds like she's heading towards the south - the exit.  You missed your chance to do anything about that demon...");
		}
	...
	public inSovietCoCPonyRidesYou(hpVictory: boolean, pcCameWorms: boolean) {
		if (player.torso.cocks.count > 0 && player.torso.vaginas.count > 0) {
			if (randInt(2) === 0) maleLoss(hpVictory);
			else femLoss(hpVictory);
		}

		if (player.torso.cocks.count > 0 && player.torso.vaginas.count <= 0) maleLoss(hpVictory);
		else if (player.torso.vaginas.count > 0 && player.torso.cocks.count <= 0) femLoss(hpVictory);
		else if (player.torso.vaginas.count <= 0 && player.torso.cocks.count <= 0) femLoss(hpVictory);
	}

	private femLoss(hpVictory: boolean) {
		DisplayText().clear();
		DisplayText("Dropping down, you");
		if (hpVictory) DisplayText(" try to pull yourself up with on your palms, utterly defeated in spite of the centaur's complete lack of physical attacks.");
		else {
			DisplayText("put your rump up in the air, fully exposing your");
			...
			DisplayText(" to the pervy, sexy equine.");
		}
		DisplayText(" She smirks as she trots up, commenting, \"<i>That's more like it, slut.  I don't know how you got this far, but your quest ends here.</i>\"  She casually reaches down and grabs you by the scruff of the neck, hauling you bodily up until your head nestles between her tits");
		if (player.tallness >= 7 * 12) DisplayText(", even though your [feet] still touch the ground");
		DisplayText(".  Brushing through your hair, a hand gently caresses your back as its owner coos, \"<i>You'll make a fine mare for my stable, but lets get you in the right mindset first...</i>\"");
		...
		DisplayText("Your [vagina] is so puffy and inflamed with passion that it feels ready to burst out of your [armor], even as the drooling fem-cum makes it squish wetly with your every motion.");
		if (player.torso.cocks.count > 0) DisplayText(" [EachCock] fares little better, a turgid mess of pre-cum leaking lust that seems to grow harder and more sensitive by the moment.");
		DisplayText(" That wonderful warmth excites you sexually, a deepening puddle of lust in your loins that makes you so hot and wet that you swear it must be what's dripping out onto the cobble below.");
	...
	private maleLoss(hpVictory: boolean) {
		...
		DisplayText("Dropping down, you");
		if (hpVictory) {
			DisplayText(" try to pull yourself up with on your palms, utterly defeated in spite of the centaur's complete lack of physical attacks.");
		}
		else {
			DisplayText(" try to pull [oneCock] out of your [armor] in an attempt to masturbate.  You're beaten, you just want to fuck or be fucked until you're both covered in a sheen of lady-spunk or your own thick cum.");
		}
		DisplayText(" She smirks as she trots up, commenting, \"<i>That's more like it, slut.  I don't know how you got so far thinking you were some kind of " + Desc.Gender.mf(player, "stud", "queen bitch") + ".</i>\"  She casually reaches down and grabs you by the scruff of the neck, hauling you bodily up until your head nestles between her tits");
		if (player.tallness >= 7 * 12) DisplayText(", even though your [feet] still touch the ground");
		DisplayText(".  Brushing through your hair, a hand gently caresses your back as its owner coos, \"<i>I'm not sure you have what it takes to be one of my stallions...</i>\"");
		...
		if (player.torso.vaginas.count > 0) DisplayText(" Your [vagina] fares no better, puffing up and drooling like its big brother while your [clit] grows taut and sensitive.");
		DisplayText(" That wonderful warmth excites you sexually, moulding your loins into a simmering blaze of desire just looking for a something to cool its ache.");
		...
		if (Flags.list[FlagEnum.D1_OMNIBUS_KILLED] != 1) DisplayText(", and a familiar one at that");
		...
		if (Flags.list[FlagEnum.D1_OMNIBUS_KILLED] != 1) DisplayText(" overseer asks, \"<i>Remember me?  I thought it only proper that I thank you for letting me live.  Now, be a good boy and suck my dick - show all everyone just what a horny, helplessly fuckable toy you are.</i>\"");
		else DisplayText(" asks, \"<i>I hope you don't mind slurping down a little cock, because you're going to anyway.</i>\"");

		DisplayText("\n\nThe second herm grabs you by the");
		if (player.torso.neck.head.hornType != 0) DisplayText(" horns");
		else DisplayText(" head");
		DisplayText(" and sidles forward, ramming her glans into your mouth and down the back of your throat.  You can feel her glans pulsing deep inside you, and as soon as the herm starts moaning, the centaur starts trotting forward again.  Amazingly, having her holding you up makes it easy to maintain your position and keep your dick bouncing in that hot twat. Fuck it! You're too horny to care anymore.");

		DisplayText("\n\nSlurping hard, you lick and suck the dick in your throat, so busy tending to the omnibus that you barely the notice the throbbing ache in your loins.  You cast lusty, inviting eyes around at every feminine form you see during journey to the stables, located in some backwater, seemingly forgotten part of the fortress.  You quiver with need and explode in passion, moaning into the Omnibus's cock as your " + Desc.Cock.describeCock(player, y) + " spends its lust into the oozing horse-pussy, wasting your champion-seed to father nothing more than a batch of shiftless imps.");
