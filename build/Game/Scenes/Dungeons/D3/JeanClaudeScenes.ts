...
export class JeanClaudeScenes {
	...
	public gogoFuckTheseBasilisks() {
		...
		if (player.hasKeyItem("Laybans") >= 0) DisplayText(", put your shades on")
		DisplayText(" and stride out onto the gantry. The plan is simple. You’ll keep your eyes up and march across as if you belong there, as if you are on official business.");
		if (player.demonScore() > 4) DisplayText("  You can’t be mistaken for anything but a demon, after all.");
		DisplayText("  If or when the lizards realize who you are, you’ll make a break for it, and hopefully by then you’ll be in reach of the exit. It’s straight-forward, it’s elegant, it will work, whatever your hammering pulse has to say on the matter.");

		DisplayText("\n\nYou try and not look down as you proceed, back straight and as aloof as you can.");

		if (player.torso.hips.legs.isNaga()) DisplayText("  You’re probably imagining things but the passage of your sinuous form over the wire surface seems absurdly loud, an endless rubbing of sandpaper which seems to fill the hall.");
		else if (player.torso.hips.legs.isGoo()) DisplayText("  Godsdamn this form... you try not to think as you slide across the wire surface, intensely aware of the slimy sound you make as you go, of the steady drip of your fluids through the mesh, trying not to think of it splashing down on curious, blunt heads.");
		else DisplayText("  Clack, clack, clack. You are intensely aware of the sound of your [feet] on the wire surface. You close your eyes and tell yourself you’re only imagining that it’s loud, that it echoes around the hall, that the basilisks would have to be deaf not to hear you.");
		...
		if (player.hasKeyItem("Laybans") < 0) {
			...
			if (player.stats.spe < 90) {
				...
				if (!player.canFly()) DisplayText(" almost");
				else DisplayText(" literally");
	...
	public capturedByBasilisks() {
		...
		if (player.torso.cocks.count > 0) DisplayText(" [eachCock] stiffening");
		if (player.torso.cocks.count > 0 && player.torso.vaginas.count > 0) DisplayText(" and");
		if (player.torso.vaginas.count > 0) DisplayText(" your [vagina] beading with excitement");
		if (player.torso.cocks.count <= 0 && player.torso.vaginas.count <= 0) DisplayText(" your loins aflame with excitement");
	...
	private regularBadEnd() {
		...
		if (player.torso.vaginas.count > 0) DisplayText(" your [vagina] beads with excitement");
		if (player.torso.cocks.count > 0 && player.torso.vaginas.count > 0) DisplayText(" and");
		if (player.torso.cocks.count > 0) DisplayText(" [eachCock] hardens");
		if (player.torso.cocks.count > 0 || player.torso.vaginas.count > 0) DisplayText(" as");
		DisplayText(" your body reacts uncontrollably to the presence of the being whose claws reach deeper and deeper into your mind, touching and triggering your basest desires.");
		if (player.statusAffects.has(StatusAffectType.RemovedArmor)) DisplayText("  You realize vaguely you already have taken your clothes off, undoubtedly your own subconscious preparing you for this moment.");
		else DisplayText("  You really are uncomfortably hot underneath the glare of those eyes. Hardly even aware you’re doing it, you begin to take off your [armor]. Piece after piece falls to the floor until you are naked before him.");
		...
		if (player.skinAdj != "smooth" || player.skinAdj != "latex" || player.skinAdj != "rubber") {
			...
			if (player.skin.type === SkinType.FUR) DisplayText("  Your fur drops away in ragged clumps as you drink.");
		...
		if (player.torso.vaginas.count > 0) DisplayText(" your [vagina] dribbling excitement");
		if (player.torso.vaginas.count > 0 && player.torso.cocks.count > 0) DisplayText(" and");
		if (player.torso.cocks.count > 0) DisplayText(" your [cock] straining helplessly");
		DisplayText(" to this depth of slutty surrender you have plumbed, your neck muscles seizing up and your limbs tightening as the thoughts sink into you.");
		...
		if (player.torso.hips.legs.isBiped()) DisplayText("  The transformation isn’t quite complete; your mouth opens as a will not your own forces your arms and legs to open, leaving you completely exposed. It takes you a few seconds to realize that your mouth opening wasn’t your doing, either. It won’t close.");
		else DisplayText("  The transformation isn’t quite complete; your mouth opens as a will not your own forces your arms to open and your monstrous bottom half to take on a stance as close to complete submission as it will allow, your [hips] flared to the world. It takes you a few seconds to realize that your mouth opening wasn’t your doing, either. It won’t close.");
		...
		DisplayText(" push roughly against your [asshole]");
		if (player.torso.vaginas.count > 0) DisplayText(" and [vagina]");
		DisplayText(", it disappears into the depths forever. Your mind is a blank canvas of pure, submissive pleasure and it, like your limitless future, is golden.");
	...
	private bwombBadEnd() {
		...
		if (player.statusAffects.has(StatusAffectType.RemovedArmor)) {
			DisplayText(" You already have taken your clothes off, undoubtedly your own subconscious preparing you for this moment, and you require no direction from Jean-Claude to sit yourself down and eagerly");
			if (player.torso.hips.legs.isBiped()) DisplayText(" spread your [hips]");
			else DisplayText(" position yourself");
			DisplayText(" for him, your breeder’s body aching to be taken.");
		}
		else {
			DisplayText(" You require no direction from Jean-Claude to take off your [armor] once you are done. Piece after piece falls to the floor until you are naked before him. Anticipation thrumming through you, you sit yourself down and eagerly");
			if (player.torso.hips.legs.isBiped()) DisplayText(" spread your [hips]");
			else DisplayText(" position yourself");
			DisplayText(" for him, your breeder’s body aching to be taken.");
		}
		...
		if (player.torso.cocks.count <= 0) DisplayText("  His other dick bats into your stomach, thrusting upwards towards your [chest] as he speaks, his eyes transfixing you, drawing you further into his savage, potent, golden dreams of lust and conquest as he speaks.");
		else DisplayText("  His other dick thrusts against your own [cock], hardening it with incessant rubbing as he speaks, his eyes transfixing you, drawing you further into his savage, potent, golden dreams of lust and conquest as he speaks.");
