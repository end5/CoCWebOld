...
export class IncubusMechanicScenes {
	...
	private metIncubusMechanicInD1(): boolean {
		if (player.statusAffects.has(StatusAffectType.FactoryIncubusDefeated)) return true;
		if (player.statusAffects.has(StatusAffectType.IncubusBribed)) return true;
		return false;
	}

	private const MECHNIC_NO_SELECTION: number = 0;
	private const MECHANIC_PAID: number = 1;
	private const MECHANIC_SUCKED: number = 2;
	private const MECHANIC_FOUGHT: number = 3;

	private const MECHANIC_KILLED: number = 1;
	private const MECHANIC_RELEASED: number = 2;
	private const MECHANIC_FUCKED: number = 3;
	private const MECHANIC_FUCKED_YOU: number = 4;

	public meetAtElevator() {
		if (Flags.list[FlagEnum.D3_MECHANIC_LAST_GREET] === MECHNIC_NO_SELECTION) {
			DisplayText("\n\nA");
			if (metIncubusMechanicInD1()) DisplayText(" familiar,");
			DisplayText(" demonic mechanic lounges against a gleaming, metallic control panel");
			if (player.statusAffects.has(StatusAffectType.IncubusBribed)) DisplayText(", a rolled-up hentai magazine tucked neatly into the chest-pocket of his overalls");
			...
			else DisplayText(". He sports a pair of stained coveralls, with the crotch torn open, revealing a cock that looks like an odd hybrid of tentacle, a bumpy demon-dong, and a stinger-ringed anemone cock. While you watch, it idly divides into two... four... eight narrow, writhing tendriles before merging back together into a three-inch-thick monster-cock.")
			DisplayText(" It stiffens under your ocular attentions, and a timeless voice with the smoothness of silk, yet an undercurrent of rumbling gravel calls, \"<i>Eyes up here, Champion.</i>\"");

			DisplayText("\n\nScowling a little at being caught so openly studying his erection, you level your gaze at the haughty incubus.");

			DisplayText("\n\n");
			if (metIncubusMechanicInD1()) DisplayText("\"<i>Once more, you bumble");
			else DisplayText("\"<i>You've bumbled");
			...
			if (player.inventory.gems >= 500) {
				MainScreen.addButton(2, "Pay Toll", payDaToll);
			}
			else {
				DisplayText("\n\n<b>You do not have enough gems to pay the required toll!</b>");
			}

			MainScreen.addButton(3, "Suck Dick", suckIncubusDick);
			MainScreen.addButton(4, "Fight", startCombatImmediate, new IncubusMechanic());
		}
		else if (Flags.list[FlagEnum.D3_MECHANIC_LAST_GREET] === MECHANIC_SUCKED) {
		...
		else if (Flags.list[FlagEnum.D3_MECHANIC_LAST_GREET] === MECHANIC_PAID) {
		...
		else if (Flags.list[FlagEnum.D3_MECHANIC_LAST_GREET] === MECHANIC_FOUGHT) {
			MainScreen.addButton(2, "Lift", useLiftPostDefeat);
		}
	}

	private payDaToll() {
		Flags.list[FlagEnum.D3_MECHANIC_LAST_GREET] = MECHANIC_PAID;
		...
		MainScreen.addButton(0, "Next", Game.d3.exitD3);
	}

	private useLiftPostDefeat() {
		DisplayText().clear();
		DisplayText("You spend a minute or two fiddling with the lift controls, trying to understand how, exactly, to control the contraption. When you think you've got a handle on the system, you tug on a lever with authority and hastily step onto the platform.");
	...
	public suckIncubusDick() {
		Flags.list[FlagEnum.D3_MECHANIC_LAST_GREET] = MECHANIC_SUCKED;

		DisplayText().clear();

		if (player.inventory.gems < 500) DisplayText("You spare a glance to your depleted pouches and admit, \"<i>I guess I'll suck your dick then.</i>\"");
		else DisplayText("You shrug and smile. \"<i>I guess now is as good a time as any to suck some dick, huh?</i>\"");
		...
		if (player.torso.vaginas.count > 0) {
			DisplayText(" You find your crotch becoming");
			if (player.torso.vaginas.get(0).wetness <= 2) DisplayText(" even more soaked than normal");
			else if (player.torso.vaginas.get(0).wetness <= 4) DisplayText(" wet with fresh lust");
			else DisplayText(" sticky with lust")
			DisplayText(" at his masculine scent and appearance.")
		}
		else if (player.torso.cocks.count > 0) {
			DisplayText(" You feel a heady, turgid warmth growing in your own crotch, mirroring the ardor displayed by the cock you're about to service.");
		}
		DisplayText(" A patch of neatly trimmed pubic hairs lead up his abdomen to a set well-defined muscles, half-hidden under his loose not-so-coveralls.");
	...
	public beatDaMechanic(hpVictory: boolean) {
		Flags.list[FlagEnum.D3_MECHANIC_LAST_GREET] = MECHANIC_FOUGHT;

		DisplayText().clear();
		DisplayText("The mechanic collapses, his overalls");
		if (hpVictory) DisplayText(" falling off his prone from, shredded.");
		else DisplayText(" hanging open as he reaches down for his exposed, dangling cock to wildly masturbate.");
		DisplayText(" He seems surprisingly human like that, just as mortal as anyone. You could let him go, play with him some, or cleanse the world of another demon.");

		
		MainScreen.addButton(0, "Kill", killMechanic);
		MainScreen.addButton(1, "Let Go", letMechanicGo, hpVictory);
		if (player.torso.cocks.count > 0) MainScreen.addButton(2, "Buttfuck", buttfuckTheMechanic, hpVictory);
		if (player.torso.vaginas.count > 0) MainScreen.addButton(3, "Ride Cock", rideMechanicsCock);
	}

	private killMechanic() {
	...
	private letMechanicGo(hpVictory: boolean) {
		Flags.list[FlagEnum.D3_MECHANIC_FIGHT_RESULT] = MECHANIC_RELEASED;
		DisplayText().clear();
		...
		DisplayText("\n\nThe incubus");
		if (hpVictory) DisplayText(" slowly struggles to his feet");
		else DisplayText(" reluctantly stops masturbating");
		DisplayText(", nodding. His skin cracks as two leathery wings unfurl. <i>\"If that's how it has to be.... For what it's worth, I hope you win. Lethice is a bitch.\"</i> Slowly walking towards the cliff, he beats his wings and lifts off the ground. \"<i>Come visit me sometime. Maybe I can show you a good time as thanks.</i>\"");
	...
	private buttfuckTheMechanic(hpVictory: boolean) {
		Flags.list[FlagEnum.D3_MECHANIC_FIGHT_RESULT] = MECHANIC_FUCKED;
		...
		if (!hpVictory) DisplayText(" throbbing hard");
		else DisplayText(" gently bobbing");
		DisplayText(" dick, but you've got a better idea. You got more than one look at the curvature of his butt while you were fighting, tempting your burgeoning bulge to the point of distraction. You hastily remove your [armor] and smile. [EachCock] springs up, full and hard. You take");
		if (player.torso.cocks.count > 1) DisplayText(" one");
		else DisplayText(" it");
		DisplayText(" in hand, pumping slowly as you eye your tainted prey. He whimpers");
		if (hpVictory) DisplayText(" in need, and");
		else DisplayText(", but");
		DisplayText(" his cock seems to get even bigger and harder. The poor guy doesn't realize what a reaming he's in for.");
		...
		DisplayText("\n\nThe incubus's dick-flopping diminishes as it begins to properly engorge once more, aiming higher with each beat of the demon's heart as he nears his orgasm. You push him down a bit so that your " + Desc.Cock.describeCock(player, x) + " will press more powerfully across his prostate, panting from your own exertion. It's enough. The demon mewls out a long, high-pitched moan and erupts, painting a line of white up his overalls all the way to his chin. The next blobs of cum splatter off his chin. Each new ejaculation seems a little larger than the one before. You're doing your best to milk him from behind, and he's doing his best to whimper like a bitch and paint his face. A smile breaks across your face. It's more than a fair trade.");

		DisplayText("\n\nEventually, the spent demon finishes, dribbling semen so weak that it's practically clear into a salty puddle between his knees. You don't slow or stop. In fact, your " + Desc.Cock.describeCock(player, x) + " feels bigger and harder than it was a few seconds. The velvety folds of the demon's pre-slickened asshole are caressing you wonderfully, slowly egging you towards a climax of your own. You fuck him hard enough to knock him off-balance, dropping his cheek into his own spooge-puddle. Then, you cum, flooding your toy's asshole with its reward for a job well done.");

		////Low cum exit
		if (player.cumQ() <= 250) {
			DisplayText("\n\nYou pull");
			if (player.hasKnot(x)) DisplayText(", popping your knot");
			DisplayText(" out after depositing your meager load into your demonic fuck-hole. One slick strand of creamy spunk is leaking out over the demon's ballsack.");
		}
		//Medium cum exit
		else if (player.cumQ() <= 1000) {
			DisplayText("\n\nYou pull");
			if (player.hasKnot(x)) DisplayText(", popping your knot");
			DisplayText(" out of the demon's creampied fuck-hole with a smile. His stretched-out ass looks virtually glazed with cum, and you can see small rivers of it trickling down over his balls and slowly shrinking cock.");
		}
		//High cum exit
		else if (player.cumQ() <= 2500) {
			DisplayText("\n\nYou pull");
			if (player.hasKnot(x)) DisplayText(", popping your knot");
			DisplayText(" out of the demon's well-fucked asshole with a smile. As soon as the bulk of your dick unplugs him, a river of white spills out, washing over his balls and cock to drip down into his puddle. You smirk at the white-glazed demon. His butt is still full of your spunk.");
		}
		//Huge cum
		else {
			DisplayText("\n\nYou pull");
			if (player.hasKnot(x)) DisplayText(", popping your knot");
			DisplayText(" out after depositing a massive, belly-bulging load directly into the demon's guts. A tidal wave of goo bursts out of his over-pressurized asshole to fall on the ground, and as it dies off, small rivers of ivory spunk escape the lake in his ass to run down his balls and cock. His belly is");
			if (player.cumQ() <= 5000) DisplayText(" pudgy");
			else if (player.cumQ() <= 10000) DisplayText(" swollen");
			else DisplayText(" absolutely gravid");
			DisplayText(" with the distended weight of the cum packing his body. From time to time, fresh streamers of sperm squirt out of his thoroughly cream-pied backdoor.");
		}

		//No new paragraph
		DisplayText("The incubus slumps down into the puddled semen, gasping in exhaustion.");

		DisplayText("\n\nYou wipe your " + Desc.Cock.describeCock(player, x) + " off on his practically glowing bottom before turning to re-dress. When you turn back, he's gone, though a trail of white drops indicates the direction of his flight. It looks like he fled towards the cliff, perhaps growing wings. Oh well, that's one less obstacle between you and Lethice.");
	...
	private rideMechanicsCock() {
		Flags.list[FlagEnum.D3_MECHANIC_FIGHT_RESULT] = MECHANIC_FUCKED_YOU;
		...
		if (player.torso.tailType != 0) DisplayText(", tail draped across his face");
		DisplayText(", and casually begin to undress. Looking over your shoulders, you see hope in his soulless eyes. You tease, \"<i>I wouldn't want to stain my clothes when I finish you off, would I?</i>\" His dick wilts a little at that, and he looks about to faint. Giggling, you toss aside your [armor] and grab hold of his mostly-hard cock, feeling the corrupted, textured nodules that encircle his shaft flaring up at your gentle squeeze. He firms up nicely in your grip, filling back to a pulsating, rigid hardness in seconds.");
		...
		MainScreen.addButton(0, "Demon", rideMechanicsCockII, MECHANIC_DEMON_COCK);
		MainScreen.addButton(1, "Horse", rideMechanicsCockII, MECHANIC_HORZ_GOG);
		MainScreen.addButton(2, "Dog", rideMechanicsCockII, MECHANIC_DOG_COCK);
	}

	private rideMechanicsCockII(cType: number) {
		Flags.list[FlagEnum.D3_MECHANIC_COCK_TYPE_SELECTION] = cType;
		...
		//Demon first:
			...
			DisplayText("Giving the incubus's sweaty tool an affectionate squeeze, you decide, \"<i>Make it a horse-cock. If I'm going to play with you, I want you to have the biggest, strongest, most inhumanly virile cock to put inside me.</i>\" He starts twitching and leaking pre-cum halfway through your explanation.");
			...
			monster.cocks[0].cockType = CockType.HORSE;
		}
		//Dogdick
		else if (cType === MECHANIC_DOG_COCK) {
			DisplayText("Giving the incubus's sweaty tool an affectionate squeeze, you decide, \"<i>Grow a dog-cock. Let's see if I can take the knot, shall we, pet?</i>\"");
			...
			monster.cocks[0].cockType = CockType.DOG;
		}
		...
		if (player.torso.vaginas.get(0).wetness <= 2) DisplayText(", letting your free-flowing pussyjuice wash over it, lubricating it with your glorious girlcum");
		else if (player.torso.vaginas.get(0).wetness <= 3) DisplayText(", letting your oh-so-juicy vagina drip and dribble, soaking it with your copious girlcum");
		else DisplayText(", letting your lubricant drip over it");
		DisplayText(". You stuff two fingers inside and stroke deep before pulling them out and using them to spread you wide open. The " + monster.cockDescriptShort() + " pulsates with every beat of its demonic owner's heart, needing you every bit as much as you now need it.");
		...
		if (tightFit) DisplayText(" well-stretched");
		else DisplayText(" stuffed");
		DisplayText(" twat to silence him.");
		...
		if (player.statusAffects.has(StatusAffectType.IncubusBribed)) DisplayText(" Nothing but hentai mags for you to drain your balls with....");
		DisplayText("</i>\"");
		...
		if (player.torso.clit.length >= 5) DisplayText(" You grab hold of it in your hand and start to stroke it to the tempo your rising and falling body is setting, pumping the oversensitive organ like a dick. Every stroke sends your walls into fits of fluttering ecstasy. It feels so good that you don't think you'll be able to stop until you're cumming and gushing all over your demonic lover.");
		else DisplayText(" You thumb at it, rubbing in slow circles around it. Sometimes you push on its hood. Other times you oh-so-gently squeeze at it. It feels so good that the touches drive you crazy with ecstasy, and your walls flutter excitedly around your demonic lover.");
		DisplayText(" His only reaction is to gasp and groan, balls lurching in his sack.");
		...
		if (cType === MECHANIC_HORZ_GOG) DisplayText(", the head flaring to seal itself against your cervix");
		else if (cType === MECHANIC_DOG_COCK) DisplayText(", the knot popping in just in time to start bloating, locking you in place");
		DisplayText(", and jism begins to shoot out in waves.");
		if (!player.isPregnant()) {
			DisplayText(" Your empty womb drinks up the virile, seed-filled sludge with abandon.");
			if (cType === MECHANIC_HORZ_GOG) DisplayText(" After all, that bestial horse-cock doesn't really leave it anywhere else to go. The demonic animal-cunt just continues spurting straight on in to have its way with your eggs, and you're sure there isn't a contraceptive this side of Mareth that could stop you from having his imps.");
			else if (cType === MECHANIC_DOG_COCK) DisplayText(" After all, that immense knot doesn't really leave it anywhere else to go. Once every fold is bathed in alabaster doggie-cream, the continued ejaculations start pouring in after your eggs. His knot is so big and stretching you so wide; you don't think he would pop out even if he filled your belly to look nine months pregnant. Worse, or perhaps better, you're sure with how thick and virile his seed feels that you'll be getting pregnant with his imps from this.");
			player.knockUp(PregnancyType.IMP, PregnancyType.INCUBATION_IMP);
		}
		else {
			DisplayText(" Your blocked womb deflects the seed, forcing it all to stay in your channel");
			if (cType != MECHANIC_DOG_COCK) DisplayText(" and drip out over his balls, much to his chagrin.");
			else DisplayText(" and stretch you further.");
		}
		...
		if (player.torso.vaginas.get(0).wetness < 4) DisplayText(", even though you aren't normally a squirter");
		DisplayText(". The pleasure is so great that you can feel electric impulses arcing between neurons, short-circuiting your throughs into involuntary ecstasy. Your eyes roll back, and your [hips] twitch and shake, vibrating around the object that brough you to this perfect");
		if (!player.isPregnant()) DisplayText(" insemination");
		else DisplayText(" tryst");
		DisplayText(".");
	...
	private maleLossToMechanic(hpVictory: boolean) {
		DisplayText().clear();
		...
		//Lust loss
		if (!hpVictory) {
			DisplayText("Your [legs] are quivering too wildly to support you anymore, and you collapse, splaying yourself out so that your rigid cock");
			...
			//Medium cocks
				...
				DisplayText("\"<i>You're so... average. I suppose you'll do, if the succubi feel like having a drab, everyday orgasm.</i>\" The incubus shrugs. \"<i>A dick will do...</i>\"");
			}
			//Big cocks
			else if (player.torso.cocks.sort(Cock.LargestCockArea)[0].length < 18) {
			...
			//Huge cocks
				...
				DisplayText("\"<i>Seriously?</i>\" the incubus asks. \"<i>How am I supposed to work with something so gargantuan?</i>\" He disdainfully pulls a vial from his pocket and upends it over your crotch. The effect is immediate. [Eachcock] shrinks from ridiculously large down to horse-sized. <i>\"There, that's better.\"</i>");
				//Shrink down to 18" by 3"
				player.torso.cocks.list[player.torso.cocks.sort(Cock.LargestCockArea)[0]].cockLength = 18;
				player.torso.cocks.list[player.torso.cocks.sort(Cock.LargestCockArea)[0]].cockThickness = 3;
			}
			...
			DisplayText(" length inside any hole and thrust until you're quivering in orgasmic release. That gorgeous demon-cock taunts you, swaying just out of reach.");
		}
		//HP Loss
		else {
			...
			DisplayText("\n\nLeering down at you, the incubus swiftly tears away your [armor], his fingers suddenly razor-sharp claws. He is precise in his attentions, leaving your [skin] pristine and unbroken. You get the impression he doesn't want damaged goods. He regards your " + Desc.Cock.describeMultiCockShort(player) + " with interest, judging them.");
		...
		if (player.torso.cocks.count > 1) {
			...
			if (player.torso.cocks.filter(Cock.FilterType(CockType.DEMON) === player.torso.cocks.count)).length allDemon = true;

			if (!allDemon) DisplayText(" The skin of your transforming length is darkening to a purplish hue, and in the wake of the magical cock-ring, nodules are slowly emerging, growing into an imitation of the cock you're still suckling.");
			DisplayText(" The incubus chuckles, easily pushing the ring the rest of the way down, smiling at the way your demonic tip pulsates and leaks pre-cum. Once it's nestled firmly against your groin, your new master withdraws his hands (and his cock). <b>Your singular, demonic cock is impossibly hard.</b>");
		}
		else {
			//Single
			...
			//Has nondemon dick
			if (player.torso.cocks.filter(Cock.FilterType(CockType.DEMON) === 0)).length {
		...
		// [Next]
		return { next: maleLossToMechanicII };
	}

	private maleLossToMechanicII() {
		DisplayText().clear();
		DisplayText("An agonizingly perfect pussy is suddenly pressing down on your cock, subsuming it in slick, hot depths that cling and squeeze with undulations so skillful you'd be cumming already if it weren't for the magic ring.");
		...
		// [Next]
		return { next: maleLossToMechanicIII };
	}

	private maleLossToMechanicIII() {
		DisplayText().clear();
		DisplayText("The champion finally got to cum once Lethice had her turn, but by that point, pussy was all " + Desc.Gender.mf(player, "he", "she") + " could think about. Release came more frequently with the demon queen's permission. It didn't really matter, though. The champion was addicted to riding on the edge of climax, broken into nothing more than a demonic vibrator.");
		Game.gameOver();
	}


	private errybodyelseLossToMechanic(hpVictory: boolean) {
		DisplayText().clear();

		//Get turned into girl-lube pot.
		//Genderless Lust Loss
		if (player.torso.vaginas.count <= 0 && !hpVictory) {
		...
		//Genderless HP Loss
		else if (player.torso.vaginas.count <= 0 && hpVictory) {
			DisplayText("Collapsing onto the ground, you look up at the demonic aggressor with hate in your eyes, unwilling to admit defeat even when staring full-on into its face. The incubus, for his part, simply smiles down at you as he steps closer, claws clicking noisily on the smooth, tiled pavement.");
			...
			if (player.torso.chest.find(BreastRow.FuckableNipples)) DisplayText(" slipping inside a [nipple] to thrust just like your other hand. You work your upper and lower pussies like a woman possessed.");
			else {
				DisplayText(" pinching and tugging on");
				if (player.torso.chest.countNipples() > character.torso.chest.count * 2) DisplayText(" nipple after nipple, your myriad breastly endowments providing you with oh so many buttons to tend to");
				else DisplayText("your [nipples] one after another");
			}
			DisplayText(" Alas, all good things must come to an end, and your lovely, lovely little lollipop is rudely pulled from your questing lips with a noisy pop.");
		...
		if (player.torso.hips.legs.isNaga()) DisplayText("r coils don't get sore");
		else if (!player.torso.hips.legs.isBiped()) DisplayText(" don't even need to support yourself anymore");
		else DisplayText(" knees don't get sore");
		DisplayText(". How wonderful! If you move just right, you can make the whole contraption sway slightly to get a proper dick-sucking rhythm going. His pearly pre-cream oozes onto your tongue, and you swallow as much of it as you can.");
		...
		if (player.torso.clit.length > 5) DisplayText(", even if that makes it truly monstrous indeed");
