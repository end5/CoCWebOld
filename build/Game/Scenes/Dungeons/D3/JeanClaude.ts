	...
	public handleTease(lustDelta: number, successful: boolean): void {
		if (!player.statusAffects.has(StatusAffectType.RemovedArmor)) {
			...
			if (!successful) {
				...
				player.statusAffects.add(StatusAffectType.RemovedArmor, 0, 0, 0, 0);

				if (this.findStatusAffect(StatusAffects.JCLustLevel) < 0) {
					this.statusAffects.add(StatusAffectType.JCLustLevel, 1, 0, 0, 0);
					lustVuln += 0.1;
				}
				else {
					this.statusAffects.get(StatusAffectType.JCLustLevel).value1 = 1;
				}
