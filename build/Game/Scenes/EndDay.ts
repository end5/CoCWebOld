...
export function goNext(time: number, needNext: boolean): boolean {
    ...
    if (player.inventory.equipment.armor.displayName === "lusty maiden's armor") {
        ...
        // Tits gone or too small
            ...
            DisplayText("\nThe fine chain that makes up your lewd bikini-top is dangling slack against your flattened chest.  Every movement and step sends it jangling noisily, slapping up against your [nipples], uncomfortably cold after being separated from your " + SkinDescriptor.skinFurScales(player) + " for so long.  <b>There's no two ways about it - you'll need to find something else to wear.</b>\n\n");
            player.inventory.items.add(player, player.inventory.equipment.armor.unequip(), Menus.Player);
            return true;
        }
    }
