...
export class BeeButtPreg implements IPregnancyEvent {
    public incubationDisplay(player: Player, incubationTime: number) {
        ...
        if (incubationTime === 20) {
            ...
            if (player.torso.cocks.count === 1) {
                ...
                DisplayText(Desc.Cock.describeCock(player, firstCock));
                if (player.torso.cocks.filter(Cock.FilterType(CockType.HORSE)).length === 1) DisplayText(", each inhalation making it bigger, harder, and firmer.  You suck in huge lungfuls of air, until your " + Desc.Cock.describeCock(player, firstCock) + " is twitching and dripping, the flare swollen and purple.  ");
                if (player.torso.cocks.filter(Cock.FilterType(CockType.DOG)).length === 1) DisplayText(", each inhalation making it thicker, harder, and firmer.  You suck in huge lungfuls of air, desperate for more, until your " + Desc.Cock.describeCock(player, firstCock) + " is twitching and dripping, its knot swollen to the max.  ");
                if (player.torso.cocks.filter(Cock.FilterType(CockType.HUMAN)).length === 1) DisplayText(", each inhalation making it bigger, harder, and firmer.  You suck in huge lungfuls of air, until your " + Desc.Cock.describeCock(player, firstCock) + " is twitching and dripping, the head swollen and purple.  ");
                // FAILSAFE FOR NEW COCKS
                if (player.torso.cocks.filter(Cock.FilterType(CockType.HUMAN)).length === 0 && player.torso.cocks.filter(Cock.FilterType(CockType.DOG)).length === 0 && player.torso.cocks.filter(Cock.FilterType(CockType.HORSE)).length === 0) DisplayText(", each inhalation making it bigger, harder, and firmer.  You suck in huge lungfuls of air until your " + Desc.Cock.describeCock(player, firstCock) + " is twitching and dripping.  ");
            }
            if (player.torso.cocks.count > 1) DisplayText("groin.  Your " + Desc.Cock.describeMultiCockShort(player) + " fill and grow with every lungful of the stuff you breathe in.  You suck in great lungfuls of the tainted air, desperate for more, your cocks twitching and dripping with need.  ");
    ...
    public birthScene(player: Player) {
        ...
        if (player.torso.vaginas.count > 0) DisplayText("Your " + Desc.Vagina.describeVagina(player, player.torso.vaginas.get(0)) + " becomes engorged and sensitive.  ");
        ...
        if (player.torso.chest.reduce(BreastRow.AverageLactation, 0) >= 1 && player.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 2) DisplayText("Seeking even greater sensation, your hands gather the honey and massage it into your " + Desc.Breast.describeBreastRow(player.torso.chest.get(0)) + ", slowly working up to your nipples.  Milk immediately begins pouring out from the attention, flooding your chest with warmth.  ");
        ...
        player.stats.lib += 4;
        player.stats.sens += 3;
        if (Mod.Butt.displayStretchButt(player, 20, true))
            DisplayText().newline();
