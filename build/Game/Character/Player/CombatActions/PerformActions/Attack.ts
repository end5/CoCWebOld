...
export default class Attack implements CombatAction {
    ...
    public use(character: Character, monster: Character): void {
        ...
        const crit = this.crit(character);
        const damage = this.determineDamage(character, monster, crit);
        this.applyDamage(character, monster, damage, crit);
    }
    ...
    private miss(character: Character, monster: Character): void {
        ...
        //     DisplayText("It's all or nothing!  With a bellowing cry you charge down the treacherous slope and smite the sandtrap as hard as you can!  ");
        ...
        //     else {
        //         character.combat.effects.set(CombatEffectType.FirstAttack, CombatEffectFactory.create(CombatEffectType.FirstAttack, 0, 0, 0, 0));
        //         DisplayText("Utilizing your skills as a bareknuckle brawler, you make two attacks!\n");
        //     }
        // }
        // Blind
        ...
        //     if (character.stats.int / 5 + randInt(20) < 25) {
        ...
        //     else {
        //         DisplayText("Holding the basilisk in your peripheral vision, you charge forward to strike it.  Before the moment of impact, the reptile shifts its posture, dodging and flowing backward skillfully with your movements, trying to make eye contact with you. You twist unexpectedly, bringing your " + character.inventory.equipment.weapon.displayname + " up at an oblique angle; the basilisk doesn't anticipate this attack!  ");
        //     }
        // }
        // Worms are special
        if (monster.desc.short === "worms") {
            // 50% chance of hit (int boost)
            if (randInt(100) + character.stats.int / 3 >= 50) {
                let damage = Math.floor(character.stats.str / 5 - randInt(5));
                if (damage === 0) damage = 1;
                DisplayText("You strike at the amalgamation, crushing countless worms into goo, dealing " + damage + " damage.\n\n");
        ...
        // Determine if dodged!
        if ((character.combat.effects.has(CombatEffectType.Blind) && randInt(2) === 0) || (monster.stats.spe - character.stats.spe > 0 && randInt(((monster.stats.spe - character.stats.spe) / 4) + 80) > 80)) {
            // Akbal dodges special education
            if (monster.desc.short === "Akbal")
                DisplayText("Akbal moves like lightning, weaving in and out of your furious strikes with the speed and grace befitting his jaguar body.\n");
            else if (monster.desc.short === "plain girl")
                DisplayText("You wait patiently for your opponent to drop her guard. She ducks in and throws a right cross, which you roll away from before smacking your " + character.inventory.equipment.weapon.displayname + " against her side. Astonishingly, the attack appears to phase right through her, not affecting her in the slightest. You glance down to your " + character.inventory.equipment.weapon.displayname + " as if betrayed.\n");
            else if (monster.desc.short === "kitsune") {
                // Player Miss:
                DisplayText("You swing your [weapon] ferociously, confident that you can strike a crushing blow.  To your surprise, you stumble awkwardly as the attack passes straight through her - a mirage!  You curse as you hear a giggle behind you, turning to face her once again.\n\n");
            }
            else {
        ...
        // BLOCKED ATTACK:
        if (monster.combat.effects.has(CombatEffectType.Earthshield) && randInt(4) === 0) {
    ...
    private crit(character: Character): boolean {
        return randInt(100) <= 4 || (character.perks.has(PerkType.Tactician) && character.stats.int >= 50 && (character.stats.int - 50) / 5 > randInt(100));
    }

    private determineDamage(character: Character, monster: Character, crit: boolean): number {
        let damage: number = 0;
        // Determine damage
        ...
        if (character.perks.has(PerkType.DoubleAttack) && character.stats.spe >= 50 && character.stats.str > 61 && (User.flags.get("Player") as PlayerFlags).DOUBLE_ATTACK_STYLE === 0) {
            damage = 60.5;
        }
        else
        ...
        if (character.inventory.equipment.weapon.displayname !== "jeweled rapier" && character.inventory.equipment.weapon.displayname !== "deadly spear") {
        ...
        if (character.inventory.equipment.weapon.displayname === "katana") {
            // Knock off 5
    ...
    private applyDamage(character: Character, monster: Character, damage: number, crit: boolean): void {
        // ANEMONE SHIT
        if (monster.desc.short === "anemone") {
            // hit successful:
            // special event, block (no more than 10-20% of turns, also fails if PC has >75 corruption):
            if (randInt(10) <= 1) {
                DisplayText("Seeing your " + character.inventory.equipment.weapon.displayname + " raised, the anemone looks down at the water, angles her eyes up at you, and puts out a trembling lip.  ");
                if (character.stats.cor < 75) {
                    DisplayText("You stare into her hangdog expression and lose most of the killing intensity you had summoned up for your attack, stopping a few feet short of hitting her.\n");
                    damage = 0;
                    ...
                    DisplayText("Though you lose a bit of steam to the display, the drive for dominance still motivates you to follow through on your swing.");
            }
        }
        ...
        if (damage <= 0) {
            damage = 0;
            DisplayText("Your attacks are deflected or blocked by " + monster.desc.a + monster.desc.short + ".");
        }
        else {
            DisplayText("You hit " + monster.desc.a + monster.desc.short + "! (" + damage + ")");
            if (crit)
        ...
        if (damage > 0) {
            ...
            if (monster.stats.lustVuln > 0) {
                if (character.inventory.equipment.weapon.perk === "Aphrodisiac Weapon") {
                ...
                if (character.inventory.equipment.weapon.displayname === "coiled whip" && randInt(2) === 0) {
                ...
                if (character.inventory.equipment.weapon.displayname === "succubi whip") {
            ...
            // Weapon Procs!
            if (character.inventory.equipment.weapon.displayname === "huge warhammer" || character.inventory.equipment.weapon.displayname === "spiked gauntlet" || character.inventory.equipment.weapon.displayname === "hooked gauntlets") {
                // 10% chance
                if (randInt(10) === 0 && !monster.perks.has(PerkType.Resolute)) {
                    DisplayText("\n" + monster.desc.capitalA + monster.desc.short + " reels from the brutal blow, stunned.");
                    monster.combat.effects.set(CombatEffectType.Stunned, CombatEffectFactory.create(CombatEffectType.Stunned, 0, 0, 0, 0));
                }
                // 50% Bleed chance
                if (character.inventory.equipment.weapon.displayname === "hooked gauntlets" && randInt(2) === 0 && monster.combat.stats.defense() < 10 && !monster.combat.effects.has(CombatEffectType.IzmaBleed)) {
                    if (monster.charType === CharacterType.LivingStatue) {
                        ...
                        monster.combat.effects.set(CombatEffectType.IzmaBleed, CombatEffectFactory.create(CombatEffectType.IzmaBleed, 3, 0, 0, 0));
                        if (monster.desc.plural)
