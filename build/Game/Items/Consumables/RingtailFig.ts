...
export class RingtailFig extends Consumable {
    ...
    public use(character: Character) {
        ...
        if (character.torso.cocks.count > 0) {
            // gain ball size
            if (character.torso.balls.quantity > 0 && character.torso.balls.size < 15 && randInt(4) === 0 && changes < changeLimit) {
            ...
            if (character.torso.balls.quantity === 0 && character.skin.type === SkinType.FUR && randInt(3) === 0 && changes < changeLimit) {
        ...
        // bodypart changes:
        if (character.torso.tails.reduce(Tail.HasType(TailType.RACCOON), false) && randInt(4) === 0 && changes < changeLimit) {
            // grow da tail
            // from no tail:
        ...
        // gain fur
        if ((character.torso.hips.legs.type === LegType.RACCOON && character.torso.neck.head.ears.type === EarType.RACCOON) && character.skin.type !== SkinType.FUR && changes < changeLimit && randInt(4) === 0) {
        ...
        // gain coon ears
        if (character.torso.tails.reduce(Tail.HasType(TailType.RACCOON), false) && character.torso.neck.head.ears.type !== EarType.RACCOON && randInt(4) === 0 && changes < changeLimit) {
        ...
        // gain feet-coon
        if (character.torso.neck.head.ears.type === EarType.RACCOON && character.torso.hips.legs.type !== LegType.RACCOON && changes < changeLimit && randInt(4) === 0) {
        ...
        if (character.torso.neck.head.face.type !== FaceType.RACCOON_MASK && character.torso.neck.head.face.type !== FaceType.RACCOON && randInt(4) === 0 && changes < changeLimit) {
        ...
        else if (character.torso.neck.head.face.type === FaceType.RACCOON_MASK && character.torso.hips.legs.type === LegType.RACCOON && character.skin.type === SkinType.FUR && randInt(4) === 0 && changes < changeLimit) {
        ...
        else if (randInt(2) === 0 && changes < changeLimit && (character.torso.neck.head.face.type !== FaceType.RACCOON_MASK && character.torso.neck.head.face.type !== FaceType.RACCOON)) {
