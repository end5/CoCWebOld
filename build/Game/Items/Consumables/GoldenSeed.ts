...
export class GoldenSeed extends Consumable {
    ...
    public use(character: Character) {
        ...
        else if (changes < changeLimit && character.torso.chest.count === 1 && randInt(3) === 0 && character.torso.chest.get(0).rating >= 7 && !User.settings.hyperHappy) {
            changes++;
        ...
        if (changes < changeLimit && character.torso.chest.get(0).rating < 2 && randInt(3) === 0) {
            changes++;
        ...
        // -Remove old wings
        if (character.torso.wings.type !== WingType.FEATHERED_LARGE && character.torso.wings.type > WingType.NONE && changes < changeLimit && randInt(4) === 0) {
        ...
        // -Feathery Arms
        if (character.torso.arms.type !== ArmType.HARPY && changes < changeLimit && (this.enhanced || character.torso.neck.head.hair.type === 1) && randInt(4) === 0) {
        ...
        if ((character.torso.neck.head.ears.type !== EarType.HUMAN && character.torso.neck.head.ears.type !== EarType.ELFIN) && changes < changeLimit && randInt(4) === 0) {
        ...
        if (!character.perks.has(PerkType.HarpyWomb) && character.torso.hips.legs.type === LegType.HARPY && character.torso.tails.reduce(Tail.HasType(TailType.HARPY), false) && randInt(4) === 0 && changes < changeLimit) {
