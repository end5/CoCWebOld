...
export class Equinum extends Consumable {
    ...
    public warning(character: Character) {
        if (character.skin.type === SkinType.FUR && character.torso.neck.head.face.type === FaceType.HORSE && character.torso.tails.reduce(Tail.HasType(TailType.HORSE), false) && (character.torso.hips.legs.type !== LegType.HOOFED)) {
            // WARNINGS
            // Repeat warnings
            if (character.statusAffects.has(StatusAffectType.HorseWarning) && randInt(3) === 0) {
            ...
            // Bad End
            if (randInt(4) === 0 && character.statusAffects.has(StatusAffectType.HorseWarning)) {
    ...
    public use(character: Character) {
        ...
        //
        // MALENESS.
        if ((character.gender === 1 || character.gender === 3) && randInt(1.5) === 0 && changes < changeLimit) {
            ...
            if (randInt(2) === 0 && changes < changeLimit && character.torso.balls.size <= 3 && cocks.filter(Cock.FilterType(CockType.HORSE)).length > 0) {
        ...
        // Tail -> Ears -> Fur -> Face
        // Centaur if hooved
        if (changes < changeLimit && randInt(6) === 0 && character.torso.hips.legs.type === LegType.HOOFED) {
            changes++;
        ...
        if (character.torso.neck.head.face.type !== FaceType.HORSE && character.skin.type === SkinType.FUR && changes < changeLimit &&
            randInt(5) === 0 && character.torso.neck.head.ears.type === EarType.HORSE) {
        ...
        if (character.skin.type !== SkinType.FUR && changes < changeLimit &&
            randInt(4) === 0 && character.torso.tails.reduce(Tail.HasType(TailType.HORSE), false)) {
        ...
        // Ears - requires tail
        if (character.torso.neck.head.ears.type !== EarType.HORSE && character.torso.tails.reduce(Tail.HasType(TailType.HORSE), false) && changes < changeLimit &&
            randInt(3) === 0) {
        ...
        // Tail - no-prereq
        if (character.torso.tails.filter(Tail.FilterType(TailType.HORSE)).length < 0 && randInt(2) === 0 && changes < changeLimit) {
            // no tail
