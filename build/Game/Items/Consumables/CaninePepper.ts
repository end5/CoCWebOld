...
export class CaninePepper extends Consumable {
        ...
        // If character doesnt have 2 dogdicks
            ...
            if (character.torso.cocks.filter(Cock.FilterType(CockType.DOG)).length === 0) {
                ...
                if (character.torso.cocks.count === 0) {
                    character.torso.cocks.add(new Cock(7 + randInt(7), 1.5 + randInt(10) / 10));
                    character.torso.cocks.add(new Cock(7 + randInt(7), 1.5 + randInt(10) / 10));
    ...
    public use(character: Character) {
        ...
        if (this.pepperType <= 0 && character.skin.type === SkinType.FUR && character.torso.neck.head.face.type === FaceType.DOG && character.torso.tails.reduce(Tail.HasType(TailType.DOG), false) && character.torso.neck.head.ears.type === EarType.DOG && character.torso.hips.legs.type === LegType.DOG && character.statusAffects.has(StatusAffectType.DogWarning) && randInt(3) === 0) {
        ...
        if (cocks.count > 0) {
            ...
            if (character.cumMultiplier < 2 && randInt(2) === 0 && changes < changeLimit) {
        ...
        if (randInt(4) === 0 && changes < changeLimit &&
            character.torso.hips.legs.type === LegType.DOG && character.torso.tails.reduce(Tail.HasType(TailType.DOG), false) &&
            character.skin.type !== SkinType.FUR) {
        ...
        if (randInt(3) === 0 && character.torso.hips.legs.type !== LegType.DOG && character.torso.tails.reduce(Tail.HasType(TailType.DOG), false) && character.torso.neck.head.ears.type === EarType.DOG && changes < changeLimit) {
            // Feet -> paws
        ...
        if (randInt(2) === 0 && character.torso.neck.head.ears.type !== EarType.DOG && character.torso.tails.reduce(Tail.HasType(TailType.DOG), false) && changes < changeLimit) {
        ...
        if (randInt(3) === 0 && changes < changeLimit && character.torso.tails.get(0).type !== TailType.DOG) {
        ...
        if (character.skin.type === SkinType.FUR && changes < changeLimit && randInt(3) === 0) {
