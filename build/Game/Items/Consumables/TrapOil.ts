...
export default class TrapOil extends Consumable {
    ...
    public use(character: Character) {
        ...
        if (character.stats.sens < 70 && character.torso.cocks.count > 0 && randInt(3) === 0 && changes < changeLimit) {
            DisplayText("\n\nA light breeze brushes over you and your skin tingles.  You have become more sensitive to physical sensation.");
            character.stats.sens += 5;
            changes++;
        }
        // Libido Increase:
        if (character.stats.lib < 70 && character.torso.vaginas.count > 0 && randInt(3) === 0 && changes < changeLimit) {
        ...
        if (character.torso.hips.rating >= 10 && randInt(4) === 0 && changes < changeLimit) {
            ...
            if (character.torso.hips.rating > 15) character.torso.hips.rating -= 2 + randInt(3);
            changes++;
        }
        // Thigh Gain: (towards �girly�)
        if (character.torso.hips.rating < 6 && randInt(4) === 0 && changes < changeLimit) {
        ...
        if (character.torso.chest.count > 0) {
            ...
            if (character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating > 1 && randInt(4) === 0 && changes < changeLimit) {
            ...
            if (character.torso.chest.sort(BreastRow.BreastRatingLargest)[0].rating < 1 || character.torso.chest.get(0).rating < 1 && randInt(4) === 0 && changes < changeLimit) {
        ...
        // Testicle Reduction:
        if (character.torso.balls.quantity > 0 && character.torso.cocks.count > 0 && (character.torso.balls.size > 1 || !character.statusAffects.has(StatusAffectType.Uniball)) && randInt(4) === 0 && changes < changeLimit) {
        ...
        if (character.torso.butt.wetness < 5 && randInt(4) === 0 && changes < changeLimit) {
        ...
        // Fertility Decrease:
        if (character.torso.vaginas.count > 0 && randInt(4) === 0 && changes < changeLimit) {
            ...
            character.fertility -= 1 + randInt(3);
        ...
        if (character.gender === 1) {
            ...
            if (character.tone > 20 && randInt(4) === 0 && changes < changeLimit) {
        ...
        // Female Effects
            ...
            // Muscle tone gain:
            if (character.tone < 80 && randInt(4) === 0 && changes < changeLimit) {
        ...
        // Nipples Turn Black:
        if (!character.statusAffects.has(StatusAffectType.BlackNipples) && randInt(6) === 0 && changes < changeLimit) {
        ...
        // PC Trap Effects
        if (character.torso.neck.head.face.eyes.type !== EyeType.BLACK_EYES_SAND_TRAP && randInt(4) === 0 && changes < changeLimit) {
        ...
        // Vagina Turns Black:
        if (character.torso.vaginas.count > 0 && character.torso.vaginas.get(0).type !== VaginaType.BLACK_SAND_TRAP && randInt(4) === 0 && changes < changeLimit) {
        ...
        // Dragonfly Wings:
        if (character.torso.wings.type !== WingType.GIANT_DRAGONFLY && randInt(4) === 0 && changes < changeLimit) {
