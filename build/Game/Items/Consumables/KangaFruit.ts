...
export class KangaFruit extends Consumable {
    ...
    public use(character: Character) {
        ...
        // ****************
        // -Hip widening funtimes
        if (changes < changeLimit && randInt(4) === 0 && character.torso.hips.rating < 40) {
        ...
        // ****************
        // -Face (Req: Fur + Feet)
        if (character.torso.neck.head.face.type !== FaceType.KANGAROO && ((character.skin.type === SkinType.FUR && character.torso.hips.legs.type === LegType.KANGAROO) || this.enhanced) && changes < changeLimit && randInt(4) === 0) {
        ...
        // -Fur (Req: Footsies)
        if (character.skin.type !== SkinType.FUR && (character.torso.hips.legs.type === LegType.KANGAROO || this.enhanced) && changes < changeLimit && randInt(4) === 0) {
        ...
        if (character.torso.hips.legs.type !== LegType.KANGAROO && (this.enhanced || character.torso.tails.reduce(Tail.HasType(TailType.KANGAROO), false)) && changes < changeLimit && randInt(4) === 0) {
        ...
        // -Roo tail (Req: Ears)
        if (!character.torso.tails.reduce(Tail.HasType(TailType.KANGAROO), false) && changes < changeLimit && randInt(4) === 0 && (!this.enhanced || character.torso.neck.head.ears.type === EarType.KANGAROO)) {
            // gain roo tail:
        ...
        // -Roo ears
        if (character.torso.neck.head.ears.type !== EarType.KANGAROO && changes < changeLimit && randInt(4) === 0) {
        ...
        if (!character.perks.has(PerkType.Diapause) && RaceScore.kangaScore(character) > 4 && randInt(4) === 0 && changes < changeLimit && character.torso.vaginas.count > 0) {
