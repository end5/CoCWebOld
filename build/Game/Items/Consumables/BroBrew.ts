    ...
    public use(character: Character) {
        ...
        if (character.perks.has(PerkType.BimboBody)) {
            ...
            if (character.torso.cocks.count <= 0) {
                ...
                character.torso.cocks.add(new Cock(10, 2));
        ...
        if (character.perks.has(PerkType.BroBody) || character.perks.has(PerkType.FutaForm)) {
            ...
            StatModifier.displayCharacterHPChange(character, 100);
            return;
        }
        ...
        if (character.torso.chest.count > 0) {
            const topBreastRow = character.torso.chest.get(0);
            if (topBreastRow.rating >= 1) {
                DisplayText("A tingle starts in your " + BreastDescriptor.describeNipple(character, topBreastRow) + "s before the tight buds grow warm, hot even.  ");
                ...
                DisplayText("They pinch in towards your core, shrinking along with your flattening " + BreastDescriptor.describeAllBreasts(character) + ".  You shudder and flex in response.  Your chest isn't just shrinking, it's reforming, sculping itself into a massive pair of chiseled pecs.  ");
                if (character.torso.chest.count > 1) {
                    DisplayText("The breasts below vanish entirely.  ");
                    let chestCount: number = character.torso.chest.count;
                    while (chestCount > 1) {
                        character.torso.chest.remove(chestCount - 1);
                        chestCount--;
                    }
                }
        ...
        if (character.tone >= 70) DisplayText("further define themselves");
        else DisplayText("become extraordinarily visible");
        DisplayText(".  The strange, muscle-building changes flow down your " + LegDescriptor.describeLegs(character) + ", making them just as fit and strong as the rest of you.  You curl your arm and kiss your massive, flexing bicep.  You're awesome!\n\n");
        ...
        if (character.torso.cocks.count > 0) {
            const cock = character.torso.cocks.get(0);
            if (cock.length < 10) {
                DisplayText("As if on cue, the familiar tingling gathers in your groin, and you dimly remember you have one muscle left to enlarge.  If only you had the intelligence left to realize that your penis is not a muscle.  In any event, your " + CockDescriptor.describeCock(character, cock) + " swells in size, ");
            ...
            if (character.torso.balls.quantity > 0) {
                DisplayText("Churning audibly, your " + BallsDescriptor.describeSack(character) + " sways, but doesn't show any outward sign of change.  Oh well, it's probably just like, getting more endurance or something.");
            }
            else {
                DisplayText("Two rounded orbs drop down below, filling out a new, fleshy sac above your " + LegDescriptor.describeLegs(character) + ".  Sweet!  You can probably cum buckets with balls like these.");
                character.torso.balls.quantity = 2;
                character.torso.balls.size = 3;
            }
            DisplayText("\n\n");
        ...
        if (character.femininity > 0) {
            ...
            BodyModifier.displayModFem(character, 0, 100);
        }
        ...
        BodyModifier.displayModTone(character, 100, 100);
        BodyModifier.displayModThickness(character, 100, 50);
