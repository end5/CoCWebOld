...
export class GoblinAle extends Consumable {
    ...
    public use(character: Character) {
        ...
        if (changes < changeLimit && randInt(3) === 0 && character.torso.vaginas.count > 0 && character.statusAffects.get(StatusAffectType.BonusVCapacity).value1 < 40) {
        ...
        // Boost fertility
        if (changes < changeLimit && randInt(4) === 0 && character.fertility < 40 && character.torso.vaginas.count > 0) {
        ...
        else if (character.torso.cocks.count === 1 && randInt(2) === 0 && changes < changeLimit && !User.settings.hyperHappy) {
        ...
        if ((character.torso.wings.type === WingType.BEE_LIKE_SMALL || character.torso.wings.type === WingType.BEE_LIKE_LARGE || character.torso.wings.type >= WingType.HARPY) && changes < changeLimit && randInt(4) === 0) {
        ...
        // Skin/fur
        if (character.skin.type !== SkinType.PLAIN && changes < changeLimit && randInt(4) === 0 && character.torso.neck.head.face.type === FaceType.HUMAN) {
        ...
        // skin.tone
        if (character.skin.tone !== "green" && character.skin.tone !== "grayish-blue" && character.skin.tone !== "dark green" && character.skin.tone !== "pale yellow" && changes < changeLimit && randInt(2) === 0) {
            if (randInt(10) !== 0) character.skin.tone = "dark green";
            else {
                if (randInt(2) === 0) character.skin.tone = "pale yellow";
        ...
        // Face!
        if (character.torso.neck.head.face.type !== FaceType.HUMAN && changes < changeLimit && randInt(4) === 0 && character.torso.neck.head.ears.type === EarType.ELFIN) {
            changes++;
