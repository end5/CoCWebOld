...
export class GenericTransforms {
    public static demonChanges(character: Character): void {
        ...
        // grow horns!
        if (character.torso.neck.head.horns.amount === 0 || (randInt(character.torso.neck.head.horns.amount + 3) === 0)) {
        ...
        // remove fur
        if ((character.torso.neck.head.face.type !== FaceType.HUMAN || character.skin.type !== SkinType.PLAIN) && randInt(3) === 0) {
        ...
        // Demon tongue
        if (character.torso.neck.head.face.tongue.type === TongueType.SNAKE && randInt(3) === 0) {
        ...
        if (character.skin.type === SkinType.PLAIN && randInt(4) === 0) {
        ...
        // Grow demon wings
        if (character.torso.wings.type !== WingType.BAT_LIKE_LARGE && randInt(8) === 0 && character.stats.cor >= 50) {
