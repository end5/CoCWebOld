        ...
        if (character.torso.cocks.count <= 0) {
            ...
            if (character.torso.balls.quantity > 0) {
                if (character.torso.balls.size > 5)
                    DisplayText("You do your best to put the thong on, and while the material is very stretchy, it simply can't even begin to cover everything, and your " + BallsDescriptor.describeBalls(true, true, character) + " hang on the sides, exposed.  Maybe if you shrunk your male parts down a little...");
                else
            ...
            if (character.torso.cocks.count === 1) {
                DisplayText("You grunt in discomfort, your " + CockDescriptor.describeCock(character, character.torso.cocks.get(0)) + " flopping free from the thong's confines. The tight material rubbing against your dick does manage to turn you on slightly.  ");
            }
            else {
                DisplayText("You grunt in discomfort, your " + CockDescriptor.describeMultiCockShort(character) + " flopping free from the thong's confines. The tight material rubbing against your dicks does manage to turn you on slightly.  ");
            }
            character.stats.lust += 5;
            if (character.torso.cocks.sort(Cock.LargestCockArea)[0].area >= 20)
                DisplayText("You do your best to put the thong on, and while the material is very stretchy, it simply can't even begin to cover everything, and your " + CockDescriptor.describeCock(character, character.torso.cocks.sort(Cock.LargestCockArea)[0]) + " has popped out of the top, completely exposed.  Maybe if you shrunk your male parts down a little...");
